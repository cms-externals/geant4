...
for each category your Topic Branches touches with a short overall description
of the purpose of the changes made. The information added to the
file *must* be in reverse chronological order (so newest at the top) and be formatted
as follows:

1. A single line consisting of the date in ``DD Month, Year`` format, your name, and
   a "tag" for the changes using the ``category-VX-Y-Z`` format as per the Topic
   Branch name
...
.. code-block:: console
  ...
  6 June, 2019 B. Morgan (foo-V10-05-05)
  - foobar the baz
  - Fixes Bug #42
...
commits or Merge Request Description.
