...
successful testing and validation, via pull requests submitted through our
...
provide a good guide and overall reference to GitLab, in particular use of
`ssh keys for authentication <https://gitlab.cern.ch/help/user/ssh.md>`_.
...
- `Geant4 Coding and C++ Feature Use Guidelines <https://gitlab.cern.ch/geant4/geant4-dev/-/blob/master/CODING_GUIDELINES.rst>`_
...
We strongly suggest that you `set up SSH keys on GitLab <https://gitlab.cern.ch/help/user/ssh.md>`_
...
at least "Participate" so that you will receive emails for all Issues/Merge Requests
you submit or comment on, as well as when others mention you on Issues/Merge Requests.


Add the Geant4 Robot and Collaboration
^^^^^
When you submit your developments to the main repository for integration, they will be checked
and tested by an automated Continuous Integration system, and reviewed by collaboration members. 
To enable this and to keep you informed of progress, the "robot" that oversees the system, and
the collaboration requires access to your private fork. 

To give the robot access, to ``Project information -> Members`` on the menu that appears on the GitLab
...
To give the collaboration members access, the same process is similar, but in ``Project information -> Members``
use the ``Invite Group`` form to add the `geant4 <https://gitlab.cern.ch/groups/geant4>`_ group account 
as a Member with ``Reporter`` access in ``Select a role`` and click ``Invite``. 
...
This will give you a Local clone of your fork under a ``geant4-dev`` subdirectory of the directory where you run this command.
...
``topic-name``. Whilst branches can have any name, for clarity you are recommended to name your 
...
process. Deciding what constitutes a coherent topic requires some judgement and
experience but:

1. You **must not** mix technical changes with style/format (whitespace,
   variable naming/ordering for example) as all changes appear in the later Merge 
   Request and can thus make review more time consuming. A good rule of thumb here
   is if "this work fixes X *and* changes Y" then there are *two* topics involved.
2. You **should not** create multiple branches (and thus Merge Requests) that implement
   the same change across multiple categories/examples as this reduces the
   *coherence* of the change and increases the review/testing burden. The corresponding
   rule of thumb here is if "this work fixes X in examples 1...N", then this is *one*
   topic, not *N*.

Git branches are cheap in CPU/Storage, so use them to organise your
...
code for release, you *must not* make commits directly to them, only ever committing to
Topic Branches. This helps both your work and the upstream integration avoid conflicts and
ease testing.

.. tip::
   To easily create a topic branch with the correct name and a preliminary set of changes,
   run the ``GitUtilities/g4-topic-branch`` script from inside your targeted directory and
   with the "start point" checked out.
...
If you have already pushed your Topic Branch to your fork of Geant4, the fork will need to be
updated.  We'll discuss how to handle this topic :ref:`later in this section<topic_pushed_to_fork>`.
...
Commit messages **must be informative yet concise**. It is strongly suggested that you write
...
of the purpose of the changes made. **The History files must never be used as a substitute for writing good commit messages**.
These files exist only as a high level changelog to assist the Release Manager in preparing final notes for release/patch tags 
when sufficient information has not been supplied in the commits or Merge Request Description. Each entry is written using Markdown `Markdown <https://en.wikipedia.org/wiki/Markdown>`__
...
.. code-block:: markdown
  ...
  - Entry starts with a [Markdown level 2 heading](https://www.markdownguide.org/basic-syntax/#headings) whose
    ...
    - Then the tag name in braces, whose format is `(tagname-VXX-YY-PP)` where
      ...
      - `PP` is the Nth (counted from `00`) Merge Request to this category since Geant4 release `XX-YY`
  ...
  - Older entries follow, i.e. must be in reverse chronological order
    - If two Merge Requests are made to the same category on the same date, just increment the `PP` number
      as appropriate
...
as changing this can result in Git merging tag entries out of order. It is also critical that you follow the above
formatting rules to ensure easy extraction of information, and that GitLab will render
the information correctly**. You should aim to keep the entry concise, leaving details to the commit logs. For example

.. code-block:: markdown

  ## 2021-12-21 Ben Morgan (tagname-V11-00-01)
  - Fixed [Bugzilla 1234](https://bugzilla-geant4.kek.jp/show_bug.cgi?id=1234)

is perfectly sufficient as a high level description.
...
.. _topic_pushed_to_fork:

If you have pushed to your fork and then rebased your Topic Branch, the next
push will need to be forced as rebasing changes the Git history. Git may tell you that
the branches have diverged, and may suggest a pull. Instead,
...
- ``Description`` **must be filled in with an overview of, and motivation for, the proposed changes,
  ...
  - We recommend that you use the *same* information entered into each ``History``
    file touched by the Topic Branch as a guide or basis for this description
...
- You do not need set the ``Assignee`` entry as this will be handled by the Shifter.
- You can optionally set:
  - ``Reviewer`` to add any developers you specifically want to review your MR. 
  - ``Milestone`` to the release your change is targeting.
  - ``Label`` to add those appropriate to your change (e.g. ``bug`` if this change fixes one),
  - If you're unsure what to set for these, simply leave them blank and the Shifter will handle anything
...
- It is **recommended** to tick the ``Remove source branch when Merge Request is accepted`` box as
  this will simplify cleanup for you once the Merge Request is merged.
...
of the Topic Branch through the review and test process. Don't worry if you missed some information
or need to change it. You can edit the Merge Request at any time by clicking on the ``Edit`` button
at the top of the page and to the right of the ``Title``. 
...
1. The Shifter will assign themselves and review the basic information such as ``Title`` and ``Description``,
   and make requests for changes if required. They will also add Working Group Coordinators for
   categories your Topic Branch changes as reviewers.
...
4. Code Review
   ...
   - Shifter adds the relevant Working Group Coordinators, and technical experts if required.
   - You are welcome to ask any other collaborators to review the changes.
   - Review may be light touch for simple technical changes, or may be in depth for larger changes.
     The process is intended to be collaborative and iterative, helping to ensure changes are clear
     and technically sound. This is also why we require Merge Requests to focus on a single, coherent
     topic. 
   - Use the Merge Request comment thread to keep discussion focussed and
...
The ``Draft`` status may also be used to place a Merge Request "on hold" should
testing identify issues that require you to do more investigation. This means you don't
need to resubmit as a different Merge Request, but the ``Draft`` status marks to the
Shifter that they shouldn't stage the Merge Request for Nightlies. As above, make sure
you communicate clearly with the Shifter and Reviewers on the need/intent here.
...
Transfering GitHub Pull Requests to GitLab
-----
Geant4's public releases are published on CERN's GitLab instance at https://gitlab.cern.ch/geant4/geant4
and mirrored to GitHub at https://github.com/geant4/geant4. This is done for visibility given
GitHub's wide use and familiarity and thus to have a Geant4 presence there. We support submission
of patches to Geant4 through GitHub `Pull Requests <https://docs.github.com/en/pull-requests>`_ which
are identical in concept to GitLab Merge Requests. Working Group Coordinators or delegated Working Group members
are responsible for monitoring submitted Pull Requests at https://github.com/Geant4/geant4/pulls, and will
be automatically assigned as reviewers to consider the proposed changes for inclusion in Geant4. 

Because the public release Git repository is completely separate from the development repository discussed above, 
integrating a Pull Request requires its patch file to be downloaded and applied to a Topic Branch on your fork
of the main development repository (``https://gitlab.cern.ch/geant4/geant4-dev``) which can then be submitted
as above. In short, the process is:

.. code-block:: console

  # ... create a Topic Branch as normal
  $ git checkout -b add-github-pr master
  ...
  # ... download the Pull Requests's patch file, where [PRID] is GitHub's numeric ID for it
  # ... listed on the webpage for the Pull Request
  $ curl -OL https://github.com/geant4/geant4/pull/[PRID].patch
  ...
  # ... check if the patch will apply
  $ git apply --check [PRID].patch
  # ... should return without error
  ...
  # ... apply the patch
  $ git am [PRID].patch

Should the ``git apply`` or ``git am`` steps fail, please consult their `Git`_  documentation
for command line options to assist with resolving problems, or contact the Software Management WG
for help in setting the Topic Branch and patch up. We use ``apply`` and ``am``
as the latter automatically creates the commits for you and also ensures the changes are
attributed to the Pull Request submitter.

You can then build/test the changes as normal for a Topic Branch, and review with
the submitter on the GitHub Pull Request thread. Should the changes not be suitable for inclusion,
you should explain the reasons for this on the GitHub Pull Request thread before closing it.
Otherwise, if it is suitable but fixes are required, you can either make these directly, or
request the submitter to do so and then reapply the patch as above. Once you are happy with the
changes, update the ``History`` files for the touched categories as for a standard Topic Branch
and submit it to the development repository as a normal Merge Request for testing and integration.

.. tip::
   When you run the ``GitUtilities/g4-topic-branch`` script, it will update the History file automatically.

Once the changes are merged in the development repository, you should report this on the
GitHub Pull Request thread and *close* it. You *must not* merge the Pull Request on GitHub though
this may seem odd. This is because the GitHub repo is only used for publishing public releases
and cannot diverge from this model of "one commit per public release". In this sense, we use GitHub
as a patch submission system very much in the vein of how Git was used initially for Linux kernel
development.
...
The main local config setting it applies is the use of ``.git-blame-ignore-revs``:

.. conde-block:: console

  $ git config blame.ignoreRevsFile .git-blame-ignore-revs

This helps if you use the ``git blame`` command to check file histories. The
above file contains commits in Geant4 that simply changed things like formatting
(i.e. layout rather than technical). Git will then use that file to show you the
actual last technical change made.
...
recommend or support this functionality for fork synchronization,
...
.. _Geant4 CDash: https://cdash-sft.web.cern.ch/index.php?project=Geant4
