...
set SOURCE=%WORKSPACE%/geant4
set BINARY=%BUILDDIR%/build

rem ---default build option, to be modified for non default buildtypes
set G4_XOPTS=-DGEANT4_USE_OPENGL_WIN32=ON;-DGEANT4_BUILD_MSVC_MP=ON

rem --- Install datasets if not present, will reuse from this location otherwise
set G4_XOPTS=%G4_XOPTS%;-DGEANT4_INSTALL_DATA=ON;-DGEANT4_INSTALL_DATADIR=D:/geant4-data;-DGEANT4_INSTALL_DATASETS_TENDL=ON
...
rem --  attempt tp set G4_XOPTS global, will only keep last expansion done within loop,
...
FOR %%a IN (%BUILDOPTIONS%) DO ( 
	...
	IF        "%%a"=="staticlibs" (
		...
		SETLOCAL ENABLEDELAYEDEXPANSION
	        ...
	        ENDLOCAL
	        set OPT_CXX=-DCMAKE_CXX_STANDARD=17
		SETLOCAL ENABLEDELAYEDEXPANSION
	) ELSE IF "%%a"=="CXX20" (
		ENDLOCAL
		set OPT_CXX=-DCMAKE_CXX_STANDARD=20
		SETLOCAL ENABLEDELAYEDEXPANSION
	) ELSE IF "%%a"=="CXX23" (
		ENDLOCAL
		set OPT_CXX=-DCMAKE_CXX_STANDARD=23
		SETLOCAL ENABLEDELAYEDEXPANSION
		...
		set OPT_TASK=MT
		SETLOCAL ENABLEDELAYEDEXPANSION
	) ELSE IF "%%a"=="RM_Tasking" (
	        ENDLOCAL
		set OPT_TASK=Tasking
		SETLOCAL ENABLEDELAYEDEXPANSION
	) ELSE IF "%%a"=="RM_TBB" (
	        ENDLOCAL
		set OPT_TASK=TBB
		...
		echo Info: Ignore Buildoption none 		
	) ELSE IF 1==1 (
...
IF "%BUILDOPTIONS%"=="none" SET BUILDOPTIONS=
...
IF "%THREAD%" == "MT" (
   ...
   set OPT_MT=-DGEANT4_BUILD_MULTITHREADED=ON
	set G4FORCENUMBEROFTHREADS=max
	 ...
	 set OPT_MT=-DGEANT4_BUILD_MULTITHREADED=OFF
)	

rem -- for MT in static for now....

rem -- IF DEFINED OPT_MT  set G4_XOPTS=%G4_XOPTS%;-DBUILD_STATIC_LIBS=ON;-DBUILD_SHARED_LIBS=OFF
...
IF DEFINED OPT_CXX     set G4_XOPTS=%G4_XOPTS%;%OPT_CXX%
IF DEFINED OPT_TASK    set G4FORCE_RUN_MANAGER_TYPE=%OPT_TASK%
