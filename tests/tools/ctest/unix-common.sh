...
if [ -z "$G4_XOPTS" ]; then
  ...
  case $OSname in
    ...
    el) 
      ...
      if test "${OSvers}" = "9" ; then
        G4_XOPTS="${G4_XOPTS};-DGEANT4_USE_OPENGL_X11=ON;-DGEANT4_USE_XM=ON"
      else
        G4_XOPTS="${G4_XOPTS};-DGEANT4_USE_OPENGL_X11=ON"
      fi   
      ;;
    *)
...
#add optional setting from geant4 setup when not using a view
...
test -n "${Use_QT}"    && G4_XOPTS="${G4_XOPTS};${Use_QT}"
...
for option in ${BUILDOPTIONS//,/ } ; do
  case "${option}" in 
    ...
    UseTBB)
      ...
      ;;            
    UseQt6)
      echo "Using Qt6" # view with Qt6 is set in jk-setup.sh, package in g4-setup.sh
      ;;
    UseQt5)
      echo "Using Qt5" # view with Qt5 is set in jk-setup.sh, package in g4-setup.sh
      G4_XOPTS="${G4_XOPTS};-DGEANT4_USE_QT_QT5=ON"
      ;;
    UseVTK)
    ...
    UseLTO)
      ...
      ;;
    UseXcodeWarnings)
        if echo $OSName | grep -q "mac"; then
	  G4_XOPTS="${G4_XOPTS};-DGEANT4_BUILD_XCODE_WARNINGS=ON"
	  echo "enabling Xcode clang warning switches for $OSname."
	else
	  echo "Ignoring build option UseXcodeWarnings"
	fi
      ;;	    
