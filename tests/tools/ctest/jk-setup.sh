...
#import scripts for common functions and package versions
...
case $OSname in
  ...
  AlmaLinux)
    ...
    echo "running on AlmaLinux arch $ARCH"
    FPE_OPT="FPE"
    ;;
  Ubuntu)
    OSname="ubuntu" 
...
if echo ${BUILDOPTIONS} | grep -q "noView" ; then
   #  with option noView, do not check if views exist and don't add to BUILDOPTIONS 
    true
else
   ...
   if test -e ${dir_view_qt6}  && ! echo "$BUILDOPTIONS" | grep -qe "UseQt5" ; then
...
if ! ( echo $BUILDOPTIONS | grep -qe "noView" ) && test -r $setup   \
  ...
  if ( echo $BUILDOPTIONS | grep -qe "noView" ) ; then
    echo "Option noView given..."
  else 
    test -d "${dir_view}" \
