...
# if qt5 view does not exist, try to use qt6 based view
if test -e ${dir_view_qt5} ; then
   dir_view=${dir_view_qt5}
else   
   if test -e ${dir_view_qt6} ; then 
     echo "$BUILDOPTIONS" | grep -qe "UseQt6" && true ||   BUILDOPTIONS="${BUILDOPTIONS},UseQt6"
   fi 
fi   
echo "$BUILDOPTIONS" | grep -qe "UseQt6" && dir_view=${dir_view_qt6}
...
if test -r $setup  && ! ( echo $BUILDOPTIONS | grep -qe "noView" ) \
  ...
  test -r ${dir_view}/bin/qmake        && Use_QT="ON"
  ...
  test -d ${dir_view} \
    ...
    || echo "No such view found: ${dir_view}"
