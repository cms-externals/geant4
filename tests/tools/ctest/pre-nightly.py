...
Source_repository = os.environ["WORKSPACE"] + "/" + os.environ["Source_dir"]
...
def get_staged_MRs():
    logging.info("Source_repository : " + Source_repository)
    repo = Repo(Source_repository)

    global baseCommit, tipCommit
    baseCommit = str(repo.merge_base("refs/remotes/origin/master", "HEAD")[0])
