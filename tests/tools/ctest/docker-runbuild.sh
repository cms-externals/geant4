...
if test -n "${OS}" ; then
  ...
  case $OS in
    centos8)  
      ...
      ;;
    *-arm64)
      DOCKER_IMAGE=$(echo $DOCKER_IMAGE | sed -e 's/-arm64/:aarch64/')
      ;; 
  esac
fi
...
log_info "${SCRIPT_NAME}: Pulling image ${DOCKER_IMAGE}"
...
log_info "${SCRIPT_NAME}: ${scripts[0]} == ${scripts[1]} == ${scripts[2]} =="
...
log_info "${SCRIPT_NAME}: build scripts=$build_script."
...
log_info "${SCRIPT_NAME}: extra env: $extra_env"
...
log_info "${SCRIPT_NAME}: Checking CVMFS volume mounts >>>"
stat /cvmfs/geant4.cern.ch/geant4
stat /cvmfs/sft.cern.ch/lcg
log_info "${SCRIPT_NAME}: <<< Done checking CVMFS volume mounts"
...
log_info "${SCRIPT_NAME}: Docker return code $?"
