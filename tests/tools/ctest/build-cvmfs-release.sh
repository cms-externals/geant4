...
case ${COMPILER} in 
  gcc*)
    ...
    elif echo "10 11 12 13 14 15" | grep -q -e "$gccver" ; then 
      gccshort=`echo ${gccver} | awk -F. '{ printf"gcc%d",$1 }'`
    else
...
#---Motif, not in alma10,...----------------------------------------------------------------
useXM=" " 
if echo ${platform} | grep -q -el9 ; then
    useXM=" -DGEANT4_USE_XM=ON "    
fi
...
if [ $build_SC == 1 ] ; then
  # ROOT - pre-check, actual call of .sh is done after build of Geant4, just prior to build of grid executables.
  echo "Setting up ROOT for platform: ${lcgplatform}"
  if [ -z "${ROOTSYS}" ] ; then
    errexit "environment variable ROOTSYS not defined or empty" 
  else
    root_env=${ROOTSYS}/ROOT-env.sh
    rootdir=${ROOTSYS}
  fi   

  if [ ! -e ${root_env} ];then
    echo "ERROR: No ROOT setup file ROOT_env.sh found ($rootdir) Cannot continue"
    exit 1
  fi
  
  echo "Using root setup $root_env" 
...
if [ $build_SC == 1 ] ; then
  ...
  echo "softlink for root_env: ln -s ${root_env}  ${g4installDir}/cvmfs/geant4.cern.ch/opt/$Version/${platform}-${gccshort}-${type}/ROOT-env.sh"
   
  test -L ${g4installDir}/cvmfs/geant4.cern.ch/opt/$Version/${platform}-${gccshort}-${type}/ROOT-env.sh \
    && rm ${g4installDir}/cvmfs/geant4.cern.ch/opt/$Version/${platform}-${gccshort}-${type}/ROOT-env.sh
  
  ln -s ${root_env} ${g4installDir}/cvmfs/geant4.cern.ch/opt/$Version/${platform}-${gccshort}-${type}/ROOT-env.sh
  # set up root  - 
  test -z "${rootenv}" && source ${root_env} || ${rootenv}
...
Build_test(){
  ...
  pwd 
  cmake -DCMAKE_BUILD_TYPE=${CMAKE_BUILD_TYPE} -DGeant4_DIR=${buildDir} ${srcPath}/${testName} 
