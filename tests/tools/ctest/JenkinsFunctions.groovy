...
def getLABEL() {
  if (params.OS =~ '^centos' ) LABEL = 'docker-host-noafs'
  if (params.OS =~ '^alma')     LABEL = 'docker-host-noafs'
  if (params.OS =~ '^ubuntu' ) LABEL = 'docker-host-noafs'
...
def GetBuildOptionDir() {
  dir=params.BuildOptions.replaceAll(' ','').replaceAll(',','_')
  if (params.BuildOptions.equals('none')) dir=''
  return dir
}

def GetBuildOptionText() {
  txt='-' + GetBuildOptionDir()
  if (txt.equals('-')) txt=''
  return txt
}

def GetToday() {
  txt="${new Date().format('yyyy-MM-dd')}"
  return txt
}
...
def doBuild() {
  try {
    // need to use Platform here to allow catching... do not assign, this binds the first value encountered
    println "submit job for ${Platform} ${THREAD}"
