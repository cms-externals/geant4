...
:Detector(Det)
{
  ...
  fTCmd = new G4UIcmdWithoutParameter("/testhadr/histo/group2",this);
  fTCmd->SetGuidance("set type2 binning in angle and energy");
  fTCmd->AvailableForStates(G4State_PreInit);
...
void DetectorMessenger::SetNewValue(G4UIcommand* command, G4String newValue)
{
  ...
  else if (command == fTCmd)
    h->DefineTargetGroup2();
