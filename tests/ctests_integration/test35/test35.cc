...
int main(int argc, char** argv)
{
  ...
  for (G4int run = 0; run < 20; ++run)
  {
    ...
    if (!material)
    {
      cout << "Material <" << nameMat << "> is not found out" << endl;
      exit(1);
    }
    ...
    if (!proc)
    {
      if ("" != nameGen)
      {
        cout << "For particle: " << part->GetParticleName() << " generator " << nameGen
             << " is unavailable" << endl;
      }
      continue;
    }
    ...
    if (verbose > 0)
    {
      G4cout << "The material:  " << material->GetName() << "  Z= " << Z << "  A= " << A
             << "\nThe step:      " << theStep / mm << " mm"
             << "\nThe position:  " << aPosition / mm << " mm"
             << "\nThe direction: " << aDirection << "\nThe time:      " << aTime / ns << " ns"
             << G4endl;
    }
    ...
    {
      G4cout << "### Wrong particle type: " << part->GetParticleName() << G4endl;
      exit(1);
    }
    ...
    G4cout << "### factor  = " << factor << "    cross(b)= " << cross_sec / barn << G4endl;

    G4double coeff = cross_sec * GeV * 1000.0 / (barn * (G4double)nevt);
    G4int nmomtet[50][20];
    G4int nmomtetm[50][20];
    G4int nmomtet0[50][20];
    G4int nmomtetp[50][20];
    G4double harpcs[50][20];
    G4double respip[50][20];
    G4double respin[50][20];
    G4double respi0[50][20];
    G4double resp[50][20];
    G4double errpip[50][20];
    G4double errpin[50][20];
    G4double errpi0[50][20];
    G4double errp[50][20];
    ...
    if (verbose > 0)
    {
      G4cout << " nbinTheta= " << nanglpi << " nbinP= " << nmompi << " ang0= " << angpi[0]
             << G4endl;
    }
    ...
    for (G4int iter = 0; iter < nevt; ++iter)
    {
      ...
      labv = G4LorentzVector(0., 0., std::sqrt(e0 * (e0 + 2.0 * mass)), e0 + mass + amass);
      ...
      */
      for (G4int i = 0; i < n; ++i)
      {
        ...
        {
          G4cout << i << "-th secondary: " << pd->GetParticleName() << "  p= " << p
                 << "  theta= " << theta << G4endl;
        }
            ...
            && (pd == pip || pd == pin || pd == pi0 || pd == proton))
        {
          ...
          {
            ++kang;
          } while (theta > angpi[kang]);
          if (verbose >= 2) cout << " kp= " << kp << " kang= " << kang << endl;
    ...
    for (k = 0; k < nmompi; ++k)
    {
      ...
      for (j = 0; j < nanglpi; ++j)
      {
        ...
        crossp = resp[k][j];
        G4cout << "  " << cross;
        (*fout) << mom0 / GeV << " " << mom1 / GeV << " " << ang0 << " " << ang1 << " " << cross
                << " " << errpip[k][j] << std::endl;
        (*fout1) << mom0 / GeV << " " << mom1 / GeV << " " << ang0 << " " << ang1 << " " << crossm
                 << " " << errpin[k][j] << std::endl;
        (*fout2) << mom0 / GeV << " " << mom1 / GeV << " " << ang0 << " " << ang1 << " " << cross0
                 << " " << errpi0[k][j] << std::endl;
        (*fout3) << mom0 / GeV << " " << mom1 / GeV << " " << ang0 << " " << ang1 << " " << crossp
                 << " " << errp[k][j] << std::endl;
    ...
    G4cout << G4endl;
    G4cout << G4endl;
    G4cout << "## ds/do(mb/strad) for pi+ with momentum cut: " << mompi[0] << "  -  "
           << mompi[nmompi - 1] << "  MeV/c" << G4endl;
    for (j = 0; j < nanglpi; ++j)
    {
      G4cout << "  " << dsda[j];
    }
    G4cout << G4endl;
    G4cout << "## ds/dp(mb/GeV) for pi+ with theta cut " << angpi[0] << "  -  "
           << angpi[nanglpi - 1] << " radian" << G4endl;
    cross = 0.0;
    mom0 = 0.0;
    for (k = 0; k < nmompi; ++k)
    {
    ...
    G4cout << "#################################################################" << G4endl;
    G4cout << "## ds/do(mb/strad) for pi0 with momentum cut: " << mompi[0] << "  -  "
           << mompi[nmompi - 1] << "  MeV/c" << G4endl;
    cross = 0.0;
    for (j = 0; j < nanglpi; ++j)
    {
    ...
    G4cout << G4endl;
    G4cout << G4endl;
    G4cout << "## ds/dp(mb/GeV) for pi0 with theta cut " << angpi[0] << "  -  "
           << angpi[nanglpi - 1] << " radian" << G4endl;
    cross = 0.0;
    mom0 = 0.0;
    for (k = 0; k < nmompi; ++k)
    ...
    G4cout << "#################################################################" << G4endl;
    G4cout << "## ds/do(mb/strad) for protons with momentum cut: " << mompi[0] << "  -  "
           << mompi[nmompi - 1] << "  MeV/c" << G4endl;
    cross = 0.0;
    for (j = 0; j < nanglpi; ++j)
    {
