  ...
  for(G4int run=0; run<20; ++run) {
	...
	char* st = std::getenv("statistic");
	  ...
	  if (st0 > 0) {  
        ...
        for(G4int k=0; k<nanglpi; ++k) {(*fin) >> angpi[k];}
      } else if(line == "#nmompi") {
        (*fin) >> nmompi;
      } else if(line == "#mompi(MeV/c)") {
        for(G4int k=0; k<nmompi; ++k) {(*fin) >> mompi[k];}
      } else if(line == "#dangle") {
        (*fin) >> dangl;
      } else if(line == "#angles") {
        for(G4int k=0; k<nangl; ++k) {(*fin) >> ang[k];}
    ...
    for (G4int iter=0; iter<nevt; ++iter) {
      ...
      labv = G4LorentzVector(0., 0., std::sqrt(e0*(e0 + 2.0*mass)), 
			     e0 + mass + amass);
