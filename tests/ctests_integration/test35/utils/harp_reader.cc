...
int main(int argc, char** argv)
{
  ...
  do
  {
    ...
    if (1 < verbose)
    {
      ...
      std::cout << "First symbols= " << line[0] << line[1] << line[2] << std::endl;
    }
    ...
    else if (line[0] == 'A' && line[1] == 'D' && line[2] == 'E')
    {
      ...
      do
      {
        ...
        if (1 < verbose)
        {
          std::cout << "an= " << an / degree << " e1= " << e1 << " e2= " << e2 << " cross= " << x
                    << " +- " << xs << " %" << std::endl;
        }
        ...
        if ((xs == 0.0 && an / degree > 10.) || !enddata)
        {
          ...
          if (0 < verbose)
          {
            std::cout << "Save data vector for  "
                      << " Angle(degree)= " << (*angle)[angle->size() - 1] / degree << G4endl;
            (*fout_c) << "#####..Result.of.parcing..####### "
                      << " Angle(degree)= " << (*angle)[angle->size() - 1] / degree << G4endl;
            for (int ii = 0; ii < nbin; ii++)
            {
              (*fout_c) << "e(MeV)= " << 0.5 * ((*energy)[ii] + (*energy)[ii + 1])
                        << " cross(mb/MeV/sr)= " << (*cross)[ii] << std::endl;
            }
          }
        }
      ...
      if (0 < verbose)
      {
        (*fout_a) << "#####..Result.of.integration..#####.." << G4endl;
        (*fout_b) << "#####..Result.of.integration..#####.. Elim(MeV)= " << elim / MeV << G4endl;
        for (int i = 0; i < nbin; i++)
        {
          x = 0.0;
        ...
        for (int j = 0; j < na - 1; j++)
        {
          ...
          (*fout_b) << "cos(theta)= " << an << " cross(mb/sr)= " << x << std::endl;
        }
      }
    }
  } while (end);
