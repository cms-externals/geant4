...
//
//
//               FTF test: Pi- + C --> Pi+/Pi-/K+/K-/Pro/AntiPro at 158 and 350 GeV/c; NA61/SHINE data
//
// -------------------------------------------------------------------
...
//---------------------------------------------------------------------- pi- C 2 P/Pbar
G4double protP[22] ={1.6, 2.2, 3.1, 3.6, 4.4, 5.5, 7.0, 9.0, 11.0, 14.0, 18.0, 22.0, 28.0, 36.0, 46.0,56.0,66.0,76.0,86.0,96.0,106.0, 116.0};
G4double protDP[22]={0.3, 0.3, 0.3, 0.4, 0.5, 0.8, 1.0, 1.5,  1.5,  2.0,  2.0,  2.0,  4.0,  5.0,  5.0, 5.0, 5.0, 5.0, 5.0, 5.0,  5.0,   5.0};

G4double piP[22] ={0.6, 1.4, 2.2, 2.6, 3.0, 3.6, 4.4, 5.5,  7.0,  9.0, 11.0, 14.0, 18.0, 22.0, 28.0,35.0,45.0,57.0,72.0,92.0,102.0, 122.0};
G4double piDP[22]={0.2, 0.2, 0.2, 0.2, 0.3, 0.3, 0.4, 0.5,  1.0,  1.0,  1.0,  2.0,  2.0,  2.0,  4.0, 5.0, 5.0, 6.0, 6.0,10.0, 10.0,  10.0};

G4double kP[22] ={1.0, 1.2, 1.8, 2.6, 3.2, 3.6, 4.3, 5.5, 7.0, 9.0, 11.0, 15.0, 18.0, 23.0, 28.0, 36.0, 46.0,58.0,72.0,82.0,92.0,102.0};
G4double kDP[22]={0.2, 0.2, 0.4, 0.4, 0.2, 0.4, 0.3, 1.0, 1.0, 1.0,  1.5,  1.5,  1.5,  2.5,  4.0,  4.0,  6.0, 6.0, 7.0, 5.0, 5.0,  5.0};

/*
G4cout<<"Protons"<<G4endl;
for(G4int i=0; i<22; i++) {G4cout<<protP[i]<<" "<<protDP[i]<<" "<<protP[i]-protDP[i]<<" "<<protP[i]+protDP[i]<<G4endl;}

G4cout<<"Pions"<<G4endl;
for(G4int i=0; i<22; i++) {G4cout<<piP[i]<<" "<<piDP[i]<<" "<<piP[i]-piDP[i]<<" "<<piP[i]+piDP[i]<<G4endl;}

G4cout<<"Kaons"<<G4endl;
for(G4int i=0; i<22; i++) {G4cout<<kP[i]<<" "<<kDP[i]<<" "<<kP[i]-kDP[i]<<" "<<kP[i]+kDP[i]<<G4endl;}
*/
//-----------------------------------------------------------------------


G4double XUzhi[22][7];                 // -1 -- +1 x=2 Pz/sqrt(S)
for(G4int ii=0; ii<22; ii++){for(G4int j=0;j<7;j++) XUzhi[ii][j]=0.;}
...
//+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
      ...
      for(G4int ii=0; ii<n; ++ii)              // Loop over produced particles
      {
        ...
        if(verbose>=1) G4cout<<" Part  "<<ii<<" "<<pname
...
//+++++++++++++++++ For each particle in the event ++++++++++++++++++++++
	 ...
	 if  (pname == "proton")
         {
          for(G4int i=0; i<22; i++) {if(std::abs(Pmod-protP[i]) <= protDP[i]) XUzhi[i][0]+=Pmod;}
         };
	 ...
	 if ( pname == "pi+" )
         {
          for(G4int i=0; i<22; i++) {if(std::abs(Pmod-piP[i]) <= piDP[i]) XUzhi[i][2]+=Pmod;}
         };

	 if ( pname == "pi-" )
         {
          for(G4int i=0; i<22; i++) {if(std::abs(Pmod-piP[i]) <= piDP[i]) XUzhi[i][3]+=Pmod;}
         };

	 if ( pname == "kaon+" )
         {
          for(G4int i=0; i<22; i++) {if(std::abs(Pmod-kP[i]) <= kDP[i]) XUzhi[i][4]+=Pmod;}
         };

	 if ( pname == "kaon-" )
         {
          for(G4int i=0; i<22; i++) {if(std::abs(Pmod-kP[i]) <= kDP[i]) XUzhi[i][5]+=Pmod;}
         };

	 if ( pname == "anti_proton" )
         {
          for(G4int i=0; i<22; i++) {if(std::abs(Pmod-protP[i]) <= protDP[i]) XUzhi[i][6]+=Pmod;}
         };
...
// ----------------------------- P distributions----------------------
    ...
    {
     XUzhi[ii][0]*= 1./Ninelast/2./protDP[ii];  XUzhi[ii][6]*= 1./Ninelast/2./protDP[ii];
    }

    for(G4int ii=0; ii <22; ii++)
    {
     XUzhi[ii][2]*= 1./Ninelast/2./piDP[ii];  XUzhi[ii][3]*= 1./Ninelast/2./piDP[ii];
    }

    for(G4int ii=0; ii <22; ii++)
    {
     XUzhi[ii][4]*= 1./Ninelast/2./kDP[ii];  XUzhi[ii][5]*= 1./Ninelast/2./kDP[ii];
    }

    for(G4int ii=0; ii <22; ii++)
    {
     hAp<<protP[ii]<<" "<<XUzhi[ii][0]<<" "<<piP[ii]<<" "<<XUzhi[ii][2]<<" "<<piP[ii]<<" "<<XUzhi[ii][3]<<" "
                                           << kP[ii]<<" "<<XUzhi[ii][4]<<" "<< kP[ii]<<" "<<XUzhi[ii][5]<<" "
           <<protP[ii]<<" "<<XUzhi[ii][6]<<G4endl;
    }
