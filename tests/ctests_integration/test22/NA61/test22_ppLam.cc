...
G4double YUzhi[100];
for(G4int ii=0; ii<100; ii++) {YUzhi[ii]=0.;}


G4double XfUzhi[100];
for(G4int ii=0; ii<100; ii++) {XfUzhi[ii]=0.;}

//------------------------------------------------------------------
G4double PtY[6][20];
for(G4int ii=0; ii < 6; ii++)
{
 for(G4int jj=0; jj < 20; jj++) PtY[ii][jj]=0.;
}
...
{
 for(G4int jj=0; jj < 20; jj++) PtX[ii][jj]=0.;
}
...
// Particles in Lab. sys.
        if(verbose>0) G4cout<<" Part  "<<i<<" "<<pname
...
//Yyyyyyy
        ...
        G4int IinterY=-1;
        for(G4int Iint=0; Iint<6; Iint++){if((LlimitY[Iint] <= rapidity) && (rapidity < UlimitY[Iint])) IinterY=Iint;}

        G4int IinterX=-1;
        for(G4int Iint=0; Iint<8; Iint++){if((LlimitX[Iint] <= feynmanX) && (feynmanX < UlimitX[Iint])) IinterX=Iint;}

        if(verbose>=1) G4cout<<" Part  "<<i<<" "<<pname
                             <<" "<<fm/GeV
                             <<fm.mag()/GeV<<" "<<rapidity<<" "<<NyUzhi<<G4endl;
//
...
//----------------------------- Rapidity distributions------------------// Uzhi ++++
    G4cout<< "******** Rapidity ******* at Plab "<<Plab<<" Xin " << sigIn<<" "<<SqrtS/2000.<< G4endl;
    PP158y<<G4Version<<G4endl;
    PP158y<<"  Y    Lamb"<< G4endl;

    for(G4int ii=0; ii <100; ii++) {YUzhi[ii]*= 10./Ninelast*sigIn;};
...
// ----------------------------- Xf distributions----------------------// Uzhi ++++
    ...
    for(G4int ii=0; ii <100; ii++) {XfUzhi[ii]*= 1./Ninelast*sigIn/0.02;}
...
std::ofstream PP158pty("pp158ptyLam.dat",std::ios::out);
   ...
   PP158pty<<"  Pt Ym1p5  Ym1p0 Ym0p5 Y0 Y0p5 Y0p1"<< G4endl;
    ...
    for(G4int ii=0; ii <6; ii++)
    {for(G4int jj=0;jj < 20; jj++){PtY[ii][jj]*= 1./Ninelast*sigIn/0.1/0.5;};};
    ...
    for(G4int ii=0; ii <20; ii++)
    {
     Pt+=0.1;
     PP158pty<<Pt<<" ";
     for(G4int jj=0;jj < 6; jj++){PP158pty << PtY[jj][ii]<<" ";}
     PP158pty<<G4endl;
    }
...
std::ofstream PP158ptx("pp158ptxLam.dat",std::ios::out);
 ...
 PP158ptx<<"  Pt Xm0p35  Xm0p25 Xm0p15 Xm0p05 X0p05 X0p15 X0p25 X0p35"<< G4endl;
    ...
    for(G4int ii=0; ii <8; ii++)
    {for(G4int jj=0;jj < 20; jj++){PtX[ii][jj]*= 1./Ninelast*sigIn/0.1/0.1;};};
    ...
    for(G4int ii=0; ii <20; ii++)
    {
     Pt+=0.1;
     PP158ptx<<Pt<<" ";
     for(G4int jj=0;jj < 8; jj++){PP158ptx << PtX[jj][ii]<<" ";}
     PP158ptx<<G4endl;
    }
