				      ...
				      G4Material* mat)
{
  ...
  } else if(gen_name == "ftfqgsfragp") {  // FTF with QGS-Fragmentation + Preco
    G4TheoFSGenerator* theModel = new G4TheoFSGenerator;
    G4FTFModel* theStringModel = new G4FTFModel();
    G4GeneratorPrecompoundInterface* theCascade = 
      new G4GeneratorPrecompoundInterface;
  ...
  } else if(gen_name == "ftfqgsfragb") {  // FTF with QGS-Fragmentation + Bic
    G4TheoFSGenerator* theModel = new G4TheoFSGenerator();
    G4FTFModel* theStringModel= new G4FTFModel();
    G4BinaryCascade* theCascade = new G4BinaryCascade();
    theCascade->SetDeExcitation(thePreCompound);
  ...
  } else if(gen_name == "qgslundp") {  // QGS with Lund-string-fragmnetation + Preco
    G4TheoFSGenerator* theModel = new G4TheoFSGenerator();
    G4GeneratorPrecompoundInterface* theCascade = 
      new G4GeneratorPrecompoundInterface();
    theCascade->SetDeExcitation(thePreCompound);  
  ...
  } else if(gen_name == "qgslundb") {  // QGS with Lund-string-fragmnetation + Bic
    G4TheoFSGenerator* theModel = new G4TheoFSGenerator();
    G4BinaryCascade* theCascade = new G4BinaryCascade();
    theCascade->SetDeExcitation(thePreCompound);  
    G4QGSModel< G4QGSParticipants > * theStringModel = 
