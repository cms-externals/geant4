...
#include "FTFtest4.icc"  // -------- Start run processing
      ...
      for (G4int iter = 0; iter < nevt; ++iter)
      {
        ...
        for (G4int i = 0; i < n; ++i)  // Loop over produced particles
        {
          ...
          if (verbose >= 1)
            G4cout << " Part  " << i << " " << pname  // if(verbose>=1)
                   << " " << sec->Get4Momentum() / GeV << sec->Get4Momentum().mag() / GeV << G4endl;
        ...
        */
        ...
        G4int Nother = Nproton + NprotonBar + NneutronBar + Npim + Npi0 + NKm + NKp + NK0s + NK0l
                       + NLambda + NLambdaBar + NSigma0 + NSigma0Bar + NSigmap + NSigmapBar
                       + NSigmam + NSigmamBar + Neta + Neta_prime + Ngamma;
        // G4cout<<"Nch Other Nneutron Npi0 "<<Ncharged<<' '<<Nother<<" "<<Nneutron<<"
        // "<<Npi0<<G4endl;
        ...
        Nother = NprotonBar + Nneutron + NneutronBar + Npim + NKm + NKp + NK0s + NK0l + NLambda
                 + NLambdaBar + NSigma0 + NSigma0Bar + NSigmap + NSigmapBar + NSigmam + NSigmamBar
        ...
        Nother = NprotonBar + Nneutron + NneutronBar + Npi0 + NKm + NKp + NK0s + NK0l + NLambda
                 + NLambdaBar + NSigma0 + NSigma0Bar + NSigmap + NSigmapBar + NSigmam + NSigmamBar
                 + Neta + Neta_prime + Ngamma;
        // G4cout<<"Nch Other Nneutron Npip Npim "<<Ncharged<<' '<<Nother<<" "<<Nneutron<<"
        // "<<Npip<<" "<<Npim<<G4endl;
        ...
        Nother = NprotonBar + Nneutron + NneutronBar +
                 //
                 NKm + NKp + NK0s + NK0l + NLambda + NLambdaBar + NSigma0 + NSigma0Bar + NSigmap
                 + NSigmapBar + NSigmam + NSigmamBar + Neta + Neta_prime + Ngamma;
        // G4cout<<"Nch Other Nproton Npi0 Npim "<<Ncharged<<' '<<Nother<<" "<<Nproton<<" "<<Npi0<<"
        // "<<Npim<<G4endl;
        if ((Ncharged == 4) && (Nother == 0) && (Nproton == 1) && (Npip == 2) && (Npim == 1)
            && (Npi0 == 1))
        {
        ...
        Nother = Nproton + Nneutron + NprotonBar + NneutronBar + Npim + Npip + Npi0 + NKm + NK0s
                 + NK0l + NLambda + NLambdaBar + NSigma0 + NSigma0Bar + NSigmapBar + NSigmam
                 + NSigmamBar + Neta + Neta_prime;

        // G4cout<<"Nch Other NSigmap NKp "<<Ncharged<<' '<<Nother<<" "<<NSigmap<<" "<<NKp<<G4endl;
        if ((Ncharged == 2) && (Nother == 0) && (NSigmap == 1) && (NKp == 1))
        {
          Xs[Uzhi_run][8]++;
        }  // G4cout<<" 8 "<<G4endl;G4int Uzhi; G4cin>>Uzhi;}

        Nother = Nproton + Nneutron + NprotonBar + NneutronBar + Npim + Npi0 + NKm + NK0s + NK0l
                 + NLambdaBar + NSigma0 + NSigma0Bar + NSigmap + NSigmapBar + NSigmam + NSigmamBar
                 + Neta + Neta_prime;
        ...
        Nother = NprotonBar + Nneutron + NneutronBar + Npim + Npi0 + NKm + NKp + NK0s + NK0l
                 + NLambda + NLambdaBar + NSigma0 + NSigma0Bar + NSigmap + NSigmapBar + NSigmam
                 + NSigmamBar + Neta + Neta_prime;

        // G4cout<<"Nch Other Nproton Npim "<<Ncharged<<' '<<Nother<<" "<<Nproton<<"
        // "<<Npim<<G4endl;
        if ((Ncharged == 2) && (Nother == 0) && (Nproton == 1) && (Npip == 1))
        {
          Xs[Uzhi_run][10]++;
          TopoUzhi[Uzhi_run][2]--;
        }  // G4cout<<" 8 "<<G4endl;} Ntotal--;
      ...
      if (nevt - Nfault != 0)
        Xs[Uzhi_run][10] *= sigTot / (nevt - Nfault);  // Elastic cross section
  ...
  PipPchan << " Plab    Xtot    Xel    Xin     N2Pip    PPipPi0    P2pipPim  P2pipPimPi0  SpKp  "
              "LamKpPip  PipP"
           << G4endl;
