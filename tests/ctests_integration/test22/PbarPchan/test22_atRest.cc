...
//-------------------------- Global histograms  -------------------------
  ...
  G4double Xs[50];
  for(G4int ii=0; ii<50; ii++){Xs[ii]=0.;}

  G4double MultDistr[14];
  for(G4int ii=0; ii<14; ii++) {MultDistr[ii]=0.;}

  G4double MomDistr[40];
  for(G4int ii=0; ii<40; ii++){MomDistr[ii]=0.;}
...
//        G4double CosTheta=std::cos(theta);
	...
	if      ( pname == "pi-" )    {Imom=G4int(p/25.);}
	else if ( pname == "pi+" )    {Imom=G4int(p/25.);}
	else                          {}

        if((Imom != 0) && (Imom <40)) {MomDistr[Imom]++;}
...
//+++++++++++++++++ For each particle in the event ++++++++++++++++++++++
	...
	else if ( pname == "sigma+" )     {NSigma_p++;               }
	else if ( pname == "sigma-" )     {NSigma_m++;               }
	else if ( pname == "anti_sigma+" ){NSigma_pBar++;            }
	else if ( pname == "anti_sigma-" ){NSigma_mBar++;            }

	else if ( pname == "xi0" )         NXi0++;
	else if ( pname == "xi-" )        {NXi_m++;                  }
	else if ( pname == "anti_xi0" )    NXi0Bar++;
	else if ( pname == "anti_xi-" )   {NXi_mBar++;               }
...
//+++++++++++++++++ Store after each event ++++++++++++++++++++++++++++++
      if((Npip+Npim+Npi0+NKm+NKp < 14)&&(Nproton+Nneutron+NprotonBar+NneutronBar == 0))
      {
...
{Xs[30]++; Ninelast--;} // G4cout<<" 0 "<<G4endl;}
...
if((Npi0 == 2) && (Neta == 1) && (Nother == 0)) // !!!! *****************
{Xs[14]++;} // G4cout<<" 14 "<<G4endl;} //  G4int Uzhi; G4cin>>Uzhi;
...
{Xs[15]++;} // G4cout<<" 15 "<<G4endl;} //  G4int Uzhi; G4cin>>Uzhi;
...
{
 MultDistr[ii]/=Ninelast; AverMult+=ii*MultDistr[ii];
