...
#include "FTFtest4.icc"  // -------- Start run processing
      ...
      for (G4int iter = 0; iter < nevt; ++iter)
      {
        ...
        for (G4int i = 0; i < n; ++i)  // Loop over produced particles
        {
          ...
          if (pname == "neutron")
          {
            double effect = 0.820652 + 0.00689154 * e - 0.00423934 * std::pow(e, 2.)
                            + 0.000370167 * std::pow(e, 3.) - 0.0000180244 * std::pow(e, 4.)
                            + 0.00000052007 * std::pow(e, 5.) - 8.97261e-09 * std::pow(e, 6.)
                            + 9.06944e-11 * std::pow(e, 7.) - 4.95096e-13 * std::pow(e, 8.)
                            + 1.12679e-15 * std::pow(e, 9.);
      ...
      G4cout << "******** Multiplicity distr ******* at Plab " << Plab << " Xin " << sigIn
             << G4endl;
      ...
      for (G4int ii = 0; ii < 40; ii++)
      {
        ...
        {
          MultUzhi[ii][jj] *= 1. / Ninelast * sigIn;
        }
      }
