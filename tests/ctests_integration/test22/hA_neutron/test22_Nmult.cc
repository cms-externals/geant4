...
G4double MultUzhi[40][5];                 // 0 / 1200 MeV/c momentum
for(G4int ii=0; ii<40; ii++){for(G4int j=0;j<5;j++) MultUzhi[ii][j]=0.;}
...
if(nameGen=="ftfp"){ FileName="Nmultp.dat";}
if(nameGen=="ftfb"){ FileName="Nmultb.dat";}

std::ofstream Mpart(FileName, std::ios::out);
...
double effect=0.820652+0.00689154*e-0.00423934*std::pow(e,2.)+0.000370167*std::pow(e,3.)-
0.0000180244*std::pow(e,4.)+0.00000052007*std::pow(e,5.)-8.97261e-09*std::pow(e,6.)+
9.06944e-11 * std::pow(e,7.)- 4.95096e-13 * std::pow(e,8.) + 1.12679e-15 * std::pow(e,9.);

 if (e > 80.0) effect=0.15;
//effect=1.;
      if (G4UniformRand() <= effect ) Nneut++;
    };
...
//+++++++++++++++++ Store after each event ++++++++++++++++++++++++++++++
      ...
      {
       if((Ncharged>=0)&&(Ncharged < 40)) MultUzhi[Ncharged][0]++;
       if((Npos>=0)    &&(Npos     < 40)) MultUzhi[Npos][1]++;
       if((Nneg>=0)    &&(Nneg     < 40)) MultUzhi[Nneg][2]++;
       if((Nprot>=0)   &&(Nprot    < 40)) MultUzhi[Nprot][3]++;
       if((Nneut>=0)   &&(Nneut    < 40)) MultUzhi[Nneut][4]++;
      }
//+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
...
// ----------------------------- Momentum distributions----------------------
    ...
    Mpart<<G4Version<<G4endl;
    Mpart<<" Mult  Charg     Pos       Neg        Prot       Neut "<< G4endl;
    ...
    {
     Mpart<<ii<<"    ";
     for(G4int jj=0;jj < 5; jj++){Mpart << MultUzhi[ii][jj]<<"    ";}
     Mpart<<G4endl;
    }
