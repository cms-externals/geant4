...
#include "FTFtest4.icc"  // -------- Start run processing
      ...
      if (nameGen == "ftfp")
      {
        FileName = "PbA_1p22Enp.dat";
      }
      if (nameGen == "ftfb")
      {
        FileName = "PbA_1p22Enb.dat";
      }

      std::ofstream PbA_1p22En(FileName, std::ios::out);
      ...
      G4cout << "******** Neutron Energy distr ******* at Plab " << Plab << " Xin " << sigIn
             << G4endl;
      ...
      G4double dOmega50 =
        2. * pi * (std::cos(37.5 * pi / 180.) - std::cos(62.5 * pi / 180.));  //  50 grad
      G4double dOmega145 =
        2. * pi * (std::cos(132.5 * pi / 180.) - std::cos(157.5 * pi / 180.));  // 145 grad

      for (G4int ii = 0; ii < 100; ii++)
      {
        for (G4int jj = 0; jj < 2; jj++)
        {
          EnerUzhi[ii][jj] *= sigIn / Ninelast / 4. / dOmega50;
        }
      }  // sigIn

      for (G4int ii = 0; ii < 100; ii++)
      {
        for (G4int jj = 2; jj < 4; jj++)
        {
          EnerUzhi[ii][jj] *= sigIn / Ninelast / 4. / dOmega145;
        }
      }  // sigIn
