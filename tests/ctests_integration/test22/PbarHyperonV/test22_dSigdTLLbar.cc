...
//-------------------------- Current histograms -------------------------
  ...
  G4double DistrT[2][50]; for(G4int i=0; i<50; i++){DistrT[0][i]=0.; DistrT[1][i]=0.;}

  G4double TminLLb=0.015;  // GeV^2
  G4double TminLS0=0.023;  // GeV^2
//+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++ 
...
//-------------------------------------------------------
   ...
   G4double Tmin;
   G4double MassLam=1116.683;
   Tmin=sqrt(SS/4.-MassLam*MassLam)-sqrt(SS/4.-938.*938.);
   TminLLb=Tmin*Tmin/GeV/GeV;
     ...
     { 
      for(G4int i=0; i<n; ++i)                    // Loop over produced particles
      {
        ...
        if(id==-3122) {           
                       ...
                       G4double T=-(Proj4Mom - fm).mag2()/GeV/GeV - TminLLb;
                       G4int Nbin=G4int(T/0.04); if((Nbin >= 0 ) && (Nbin <= 49)) DistrT[0][Nbin]++;
                      }
      } //     end of the loop on particles
     }  //     end of if((n == 2) && (AntiLam ==1) && (Lam == 1))


     if((n == 2) && (AntiLam ==1) && (Sigma0 == 1)) // AntiLambda Sigma0
     { 
      for(G4int i=0; i<n; ++i)                      // Loop over produced particles
      {
        sec = aChange->GetSecondary(i)->GetDynamicParticle();
        ...
        if(id==-3122) {           
                       fm.boost(-bst);            // Transformation to CM system
                       G4double T=-(Proj4Mom - fm).mag2()/GeV/GeV - TminLS0;
                       G4int Nbin=G4int(T/0.04); if((Nbin >= 0 ) && (Nbin <= 49)) DistrT[1][Nbin]++;
                      }
      } //     end of the loop on particles
     }  //     end of if((n == 2) && (AntiLam ==1) && (Sigma0 == 1))
...
//std::ofstream dSdT("dSdT.dat",std::ios::out);
std::ofstream dSdT(OutFile,std::ios::out);
dSdT<<G4Version<<G4endl;
dSdT<<"Tllbar  LamBar Tlsig Sigma"<<G4endl;

G4double X_b=sigTot*1000.0;                              // MicroBarn
G4double SumLLbar(0.0), SumLbarS(0.0);

G4double Tllbar=-0.02; //TminLLb -0.04;
G4double Tlsig =-0.02; //TminLS0 -0.04;
for(G4int i=0;i < 50; i++)
{
 Tllbar +=0.04;  Tlsig +=0.04;
 SumLLbar += DistrT[0][i]; SumLbarS += DistrT[1][i];
 DistrT[0][i] *=X_b/0.04/Ntotal; DistrT[1][i] *=X_b/0.04/Ntotal;
 dSdT<<Tllbar<<" "<<DistrT[0][i]<<" "<<Tlsig<<" "<<DistrT[1][i]<<G4endl;
}
