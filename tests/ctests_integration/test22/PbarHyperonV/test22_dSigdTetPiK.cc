...
G4double CosDi[2][50]; for(G4int i=0; i<50; i++) {CosDi[0][i]=0.0;  CosDi[1][i]=0.0;}
...
G4int Km =0;
      for(G4int i=0; i<n; ++i)              // Loop over produced particles
      {
        ...
        if(pd == electron) {};   // Vova  to erase warning message
     ...
     { 
      for(G4int i=0; i<n; ++i)                    // Loop over produced particles
      {
        ...
        if(id==-211) {                                                       // Pi-
                        ...
                        G4int Ihist=G4int((CosTheta+1.0)/0.04); if((Ihist >= 0) && (Ihist <= 49)) CosDi[0][Ihist]++;
        }  //  end of if(id==-211)
      }    //  end of the loop on particles
     }     //  end of if((n == 2) && (Pip == 1) && (Pim == 1)) 

     if((n == 2) && (Kp == 1) && (Km == 1)) // K+  K-
     { 
      for(G4int i=0; i<n; ++i)              // Loop over produced particles
      {
        sec = aChange->GetSecondary(i)->GetDynamicParticle();
        ...
        if(id==-321) {                                                       // K-
                       ...
                       theta=fm.theta();        
                       G4double CosTheta=std::cos(theta);
                       G4int Ihist=G4int((CosTheta+1.0)/0.04); if((Ihist >= 0) && (Ihist <= 49)) CosDi[1][Ihist]++;
        } //     end of if(id==-321)
      }   //     end of the loop on particles       
...
CosLb1_476<<"CosThet  Pim Km"<<G4endl;

G4double X_b=sigTot*1000.0;                             //X_b=15141.; // MicroBarn
G4double SumPim(0.0), SumKm(0.0);
...
{
 CosC +=0.04;
 SumPim += CosDi[0][i]; SumKm += CosDi[1][i];
 CosDi[0][i] *=X_b/dOmega/Ntotal; CosDi[1][i] *=X_b/dOmega/Ntotal;
 CosLb1_476<<CosC<<" "<<CosDi[0][i]<<" "<<CosDi[1][i]<<G4endl;
}
G4cout<<"N Pi+ Pi-   K+ K- "<<SumPim<<" "<<SumKm<<G4endl;
