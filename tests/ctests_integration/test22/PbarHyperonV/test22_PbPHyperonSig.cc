...
#include "FTFtest2.icc"  // Initialization
  ...
  //-------------------------- Global histograms  -------------------------
  std::ofstream K0sFile("KsLam_sig.dat", std::ios::out);

  G4int Uzhi_run = 0;
  G4double Pl_proj[100], Sig_Lam[100], Sig_Ks[100];
...
#include "FTFtest4.icc"  // -------- Start run processing
      ...
      for (G4int iter = 0; iter < nevt; ++iter)
      {
        ...
        for (G4int i = 0; i < n; ++i)  // Loop over produced particles
        {
          ...
          // if(track->GetDefinition()->GetPDGEncoding() == 3222) daughters = track->Decay();   //
          // Sigma+
          if (pname == "sigma0")
          {
            NLambda++;
          };  // Sigma0
          // if(pname == "anti_sigma0") {NLambdaBar++;};   // Anti-Sigma0
          // if(track->GetDefinition()->GetPDGEncoding() == 3112) daughters = track->Decay();   //
          // Sigma-
      ...
      // Sig_Ks[Uzhi_run] =sigIn*G4double(Nk0s)   /G4double(nevt);
      // Sig_Lam[Uzhi_run]=sigIn*G4double(NLambda)/G4double(nevt);
      Sig_Ks[Uzhi_run] = sigTot * G4double(Nk0s) / G4double(nevt);
      Sig_Lam[Uzhi_run] = sigTot * G4double(NLambda) / G4double(nevt);
      //-------------------------------------------------------------------
      G4cout << "***********************************************************" << G4endl;
  ...
  K0sFile << G4Version << G4endl;
  K0sFile << " Plab         SigK0s    SigLam" << G4endl;

  for (G4int ii = 0; ii < Uzhi_run; ii++)
  {
    K0sFile << Pl_proj[ii] << " " << Sig_Ks[ii] << " " << Sig_Lam[ii] << G4endl;
  };
...
void G4DecKineticTracks::Decay(G4KineticTrackVector* tracks) const
{
  ...
  for (size_t i = 0; i < tracks->size(); ++i)
  {
    ...
    G4cout << i << " " << track->GetDefinition()->GetParticleName() << " "
           << track->GetDefinition()->IsShortLived() << G4endl;
  }
  //
  ...
  for (size_t i = 0; i < tracks->size(); ++i)
  {
    ...
    G4cout << " ----- " << track->GetDefinition()->GetParticleName() << " "
           << track->GetDefinition()->IsShortLived() << " " << track->GetDefinition()->GetPDGMass()
    ...
    if (track->GetDefinition()->GetPDGEncoding() > 3000)
      G4cout << "Track " << track->GetDefinition()->GetParticleName() << G4endl;
    ...
    if (track->GetDefinition()->GetPDGEncoding() > 3000)
    {
      G4cout << "Daughter " << daughters->size() << G4endl;
      G4int Uzhi;
      G4cin >> Uzhi;
    }
