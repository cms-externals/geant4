...
if(pname == "sigma0") {NLambda++;};      // Sigma0
...
<<track->GetDefinition()->GetPDGCharge()   <<track->GetDefinition()->GetPDGEncoding()<<G4endl;
...
if(track->GetDefinition()->GetPDGEncoding() > 3000) G4cout<<"Track "<<track->GetDefinition()->GetParticleName()<<G4endl;

if(track->GetDefinition()->GetPDGEncoding() == 3222) daughters = track->Decay();   // Sigma+
if(track->GetDefinition()->GetPDGEncoding() == 3212) daughters = track->Decay();   // Sigma0
if(track->GetDefinition()->GetPDGEncoding() == 3112) daughters = track->Decay();   // Sigma-

if(track->GetDefinition()->GetPDGEncoding() > 3000) {G4cout<<"Daughter "<<daughters->size()<<G4endl; G4int Uzhi; G4cin>>Uzhi;}
