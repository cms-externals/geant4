...
//-------------------------- Global histograms  -------------------------
std::ofstream PipP18y("PipP18y.dat",std::ios::out);
std::ofstream PipP18x("PipP18x.dat",std::ios::out);
std::ofstream PipP18pt2("PipP18pt2.dat",std::ios::out);
...
//----------------------------- Rapidity distributions------------------// Uzhi ++++
    G4cout<< "******** Rapidity ******* at Plab "<<Plab<<" Xin " << sigIn<< G4endl;
     ...
     PipP18y<<G4Version<<G4endl;
     PipP18y<<"  Y         P         Neut       Pip        Pim       Kp"
         <<"      Km       Pbar "<< G4endl;

    for(G4int ii=0; ii <100; ii++)
    {for(G4int jj=0;jj < 7; jj++){YUzhi[ii][jj]*= 10./Ninelast*sigIn/pi;};};
    ...
    for(G4int ii=0; ii <100; ii++)
    {
     Yu+=0.1;
     PipP18y<<Yu<<" ";
     for(G4int jj=0;jj < 7; jj++){PipP18y<<YUzhi[ii][jj]<<" ";}
     PipP18y<<G4endl;
    }
//
// ----------------------------- Xf distributions----------------------// Uzhi ++++
    G4cout<< "******** Xf distr ******* at Plab "<<Plab<<" Xin " << sigIn<< G4endl;
    PipP18x<<G4Version<<G4endl;
    PipP18x<<"  Xf       P      Neut       Pip      Pim      Kp"
        <<"      Km       Pbar"<< G4endl;
    ...
    for(G4int ii=0; ii <100; ii++)
    {
     ...
     {
      PipP18x<<Xu<<" ";
      for(G4int jj=0;jj < 7; jj++){PipP18x << XfUzhi[ii][jj]/3.14159<<" ";}
      PipP18x<<G4endl;
     }
    }
...
//----------------------------- Pt2 distributions----------------------// Uzhi ++++
    ...
    PipP18pt2<<G4Version<<G4endl;
    PipP18pt2<<"  Pt2      P      Neut       Pip      Pim      Kp"
           <<"     Km     Pbar"<< G4endl;

    for(G4int ii=0; ii <100; ii++)
    {for(G4int jj=0;jj < 7; jj++){Pt2Uzhi[ii][jj]*= 1./Ninelast*sigIn/0.015/pi;};};
    ...
    for(G4int ii=0; ii <99; ii++)  // 100
    {
     Pt2u+=0.015;
     PipP18pt2<<Pt2u<<" ";
     for(G4int jj=0;jj < 7; jj++){PipP18pt2 << Pt2Uzhi[ii][jj]<<" ";}
     PipP18pt2<<G4endl;
    }
