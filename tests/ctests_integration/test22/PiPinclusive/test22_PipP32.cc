...
//      SqrtS=labv.mag();
      ...
      G4LorentzVector Pprojectile=G4LorentzVector(0.0, 0.0, std::sqrt(e0*(e0 + 2.*mass)),
                                                  e0 + mass); // Projectile 4-momentum in Lab.
      Pprojectile.boost(-bst);          // Projectile 4-momentum in CMS.
      G4double PmaxCMS=Pprojectile.vect().mag(); // Projectile |\vec p| in CMS.
...
//        G4double costcm = std::cos(fm.theta());
        ...
        G4double feynmanX=fm.z()/PmaxCMS;
//      G4double PartWeight=1.;
        G4double PartWeight=fm.t()/PmaxCMS;
...
std::ofstream PipP32x("PipP32x.dat",std::ios::out);
std::ofstream PipP32pt2("PipP32pt2.dat",std::ios::out);

/*
//----------------------------- Rapidity distributions------------------// Uzhi ++++
    G4cout<< "******** Rapidity ******* at Plab "<<Plab<<" Xin " << sigIn<< G4endl;
    PipP32y<<G4Version<<G4endl;
    PipP32y<<"  Y        P       Neut      Pip       Pim      Kp"
         <<"     Km       Pbar "<< G4endl;

    for(G4int ii=0; ii <100; ii++)
  {for(G4int jj=0;jj < 7; jj++){YUzhi[ii][jj]*= 10./Ninelast*sigIn/sigIn/pi;};};
    ...
    for(G4int ii=0; ii <100; ii++)
    {
     Yu+=0.1;
     PipP32y<<Yu<<" ";
     for(G4int jj=0;jj < 7; jj++){PipP32y<<YUzhi[ii][jj]<<" ";}
     PipP32y<<G4endl;
    }
*/
//
// ----------------------------- Xf distributions----------------------// Uzhi ++++
    ...
    PipP32x<<G4Version<<G4endl;
    PipP32x<<"  Xf      P       Neut       Pip          Pim      Kp"
        <<"     Km       Pbar"<< G4endl;

    for(G4int ii=0; ii <100; ii++)
    {for(G4int jj=0;jj < 7; jj++){XfUzhi[ii][jj]*= 1./Ninelast*sigIn/0.02/pi;};};
    ...
    for(G4int ii=0; ii <100; ii++)
    {
     ...
     {
      PipP32x<<Xu<<" ";
      for(G4int jj=0;jj < 7; jj++){PipP32x << XfUzhi[ii][jj]<<" ";}
      PipP32x<<G4endl;
     }
    }
//
...
//----------------------------- Pt2 distributions----------------------// Uzhi ++++
    ...
    PipP32pt2<<G4Version<<G4endl;
    PipP32pt2<<"  Pt2      P      Neut       Pip      Pim      Kp"
           <<"    Km     Pbar"<< G4endl;

    for(G4int ii=0; ii <100; ii++)
    {for(G4int jj=0;jj < 7; jj++){Pt2Uzhi[ii][jj]*= 1./Ninelast*sigIn/0.015/pi;};}; // 2 forward and backward
    ...
    for(G4int ii=0; ii <99; ii++)  // 100
    {
     Pt2u+=0.015;
     PipP32pt2<<Pt2u<<" ";
     for(G4int jj=0;jj < 7; jj++){PipP32pt2 << Pt2Uzhi[ii][jj]<<" ";}
     PipP32pt2<<G4endl;
    }
//
//+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++ Uzhi
