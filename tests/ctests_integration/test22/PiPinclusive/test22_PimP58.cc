...
#include "FTFtest2.icc"  // Initialization
  ...
  //-------------------------- Global histograms  -------------------------
  // std::ofstream PimP58y("PimP58y.dat",std::ios::out);
  std::ofstream PimP58x("PimP58x.dat", std::ios::out);
  // std::ofstream PimP58pt2("PimP58pt2.dat",std::ios::out);
...
#include "FTFtest4.icc"  // -------- Start run processing
      ...
      for (G4int iter = 0; iter < nevt; ++iter)
      {
        ...
        // G4double Plab=std::sqrt(e0*(e0 + 2.*mass));  // It was determined before
        ...
        for (G4int i = 0; i < n; ++i)  // Loop over produced particles
        {
          ...
          G4double feynmanX = fm.z() / Plab;
          G4double PartWeight = e / Plab;
          //        G4double PartWeight=2*fm.t()/SqrtS;

          G4int NyUzhi = int((rapidity + 2.) / 0.1);  // For CMS
      ...
      */
      ...
      PimP58x << G4Version << G4endl;
      PimP58x << "  Xf        P        Neut          Pip          Pim          Kp"
              << "       Km          Pi0" << G4endl;
      ...
      //----------------------------- Pt2 distributions----------------------// Uzhi ++++
          ...
          for(G4int ii=0; ii <100; ii++)
          {for(G4int jj=0;jj < 7; jj++){Pt2Uzhi[ii][jj]*= 2./Ninelast*sigIn/0.015;};}; // 2 forward
      and backward
