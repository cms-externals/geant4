...
RunAction::RunAction(EventAction* eventAction) : G4UserRunAction(), fEventAction(eventAction)
{
  ...
  for (G4int i = 0; i < NofH2; ++i)
  {
    G4String name = "PX-PY-";
    name += std::to_string(i);
    analysisManager->CreateH2(name, "PX PY 0 in absorber layer #2", 50, -10. * MeV, 10 * MeV, 50,
                              -10. * MeV, 10 * MeV);
  }
  ...
  for (G4int i = 0; i < NofH3; ++i)
  {
    ...
    name += std::to_string(i);
    analysisManager->CreateH3(name, "PX PY PZ in absorber layer #2", 50, -10. * MeV, 10 * MeV, 50,
                              -10. * MeV, 10 * MeV, 50, -10. * MeV, 10 * MeV);
  }
  ...
  for (G4int i = 0; i < NofP1; ++i)
  {
    ...
    name += std::to_string(i);
    analysisManager->CreateP1(name, "Longit Edep pseudo-profile", NofZLayers, 0,
                              NofZLayers * LayerZSize, 0., 10. * GeV);
  }
  ...
  for (G4int i = 0; i < NofP2; ++i)
  {
    ...
    name += std::to_string(i);
    analysisManager->CreateP2(name, "Longit Edep pseudo-profile", NofXYLayers,
                              -NofXYLayers * LayerXYSize / 2, NofXYLayers * LayerXYSize / 2,
                              NofXYLayers, -NofXYLayers * LayerXYSize / 2,
                              NofXYLayers * LayerXYSize / 2, 0, 10 * GeV);
  }
...
void RunAction::PrintStatistics()
{
  ...
  if (isMaster)
  {
    ...
    for (G4int i = 0; i < NofH2; ++i)
    {
      ...
      if (h2 != nullptr)
      {
        G4cout << "   H2: mean_x: " << h2->mean_x() << "   mean_y: " << h2->mean_y() << G4endl;
      }
    }
    ...
    for (G4int i = 0; i < NofH3; ++i)
    {
      ...
      if (h3 != nullptr)
      {
        G4cout << "   H3: mean_x: " << h3->mean_x() << "   mean_y: " << h3->mean_y()
    ...
    for (G4int i = 0; i < NofP2; ++i)
    {
      ...
      if (p2 != nullptr)
      {
        G4cout << "   P2: mean_x: " << p2->mean_x() << "   mean_y: " << p2->mean_y() << G4endl;
      }
    }
  }
}
...
void RunAction::DefineCommands()
{
  ...
  fMessenger = new G4GenericMessenger(this, "/runAction/", "Run action commands");

  // printStatistic command
  auto& printStatisticCmd = fMessenger->DeclareMethod(
    "printStatistics", &RunAction::PrintStatistics, "Print statistic at the end of Run.");
