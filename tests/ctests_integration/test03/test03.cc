...
namespace {
  void PrintUsage() {
    ...
    G4cerr << " testAnalysis "
           ...
           << "    [-ascii   on|off]" << G4endl
           << "    [-dir     on|off]" << G4endl
           << "    [-nfiles  value|max]" << G4endl
           << "    [-rowWise on|off]" << G4endl
           << "    [-rowMode on|off]" << G4endl
           << "    [-merge   on|off]" << G4endl
           << "    [-vl      value]" << G4endl
           << "    [-rvl     value]" << G4endl
           << G4endl;
...
int main(int argc,char** argv)
{
  ...
  for ( G4int i=1; i<argc; i=i+2 ) {
    ...
    else if ( G4String(argv[i]) == "-o" )   FileName = argv[i+1];
    ...
    else if ( G4String(argv[i]) == "-plot" )    TestPlot = GetOption(argv[i+1]);
    else if ( G4String(argv[i]) == "-ascii" )   TestAscii = GetOption(argv[i+1]);
    else if ( G4String(argv[i]) == "-dir" )     TestDirectory = GetOption(argv[i+1]);
    else if ( G4String(argv[i]) == "-nfiles" )  sNofNtupleFiles = argv[i+1];
    else if ( G4String(argv[i]) == "-rowWise" ) NtupleRowWise = GetOption(argv[i+1]);
    else if ( G4String(argv[i]) == "-rowMode" ) NtupleRowMode = GetOption(argv[i+1]);
    else if ( G4String(argv[i]) == "-merge" )   MergeNtuple = GetOption(argv[i+1]);
    else if ( G4String(argv[i]) == "-vl" )      VerboseLevel = GetValue(argv[i+1]);
    else if ( G4String(argv[i]) == "-rvl" )     ReadVerboseLevel = GetValue(argv[i+1]);
    else {
      PrintUsage();
      return 1;
    }
