...
CalorimeterSD::CalorimeterSD(const G4String& name)
  : G4VSensitiveDetector(name), fZHitsCollection(0), fXYHitsCollection(0)
{
...
void CalorimeterSD::Initialize(G4HCofThisEvent* hce)
{
  ...
  fZHitsCollection = new CalorHitsCollection(SensitiveDetectorName, collectionName[0]);
  fXYHitsCollection = new CalorHitsCollection(SensitiveDetectorName, collectionName[1]);

  // Add this collection in hce
  G4int zhcID = G4SDManager::GetSDMpointer()->GetCollectionID(collectionName[0]);
  hce->AddHitsCollection(zhcID, fZHitsCollection);

  G4int xyhcID = G4SDManager::GetSDMpointer()->GetCollectionID(collectionName[1]);
...
G4bool CalorimeterSD::ProcessHits(G4Step* step, G4TouchableHistory*)
{
  ...
  G4int xyLayerNumber = xLayerNumber * NofXYLayers + yLayerNumber;
  ...
  CalorHit* zhitTotal = (*fZHitsCollection)[fZHitsCollection->entries() - 1];
  CalorHit* xyhitTotal = (*fXYHitsCollection)[fXYHitsCollection->entries() - 1];
  ...
  if (zLayerNumber == 2)
  {
    ...
    if (TestH1)
    {
      analysisManager->FillH1(10, momentum.x());
      analysisManager->FillH1(11, momentum.x());
      analysisManager->FillH1(12, momentum.x());
      analysisManager->FillH1(13, momentum.x());
    }
    if (TestH2)
    {
      analysisManager->FillH2(0, momentum.x(), momentum.y());
      analysisManager->FillH2(1, momentum.x(), momentum.y());
      analysisManager->FillH2(2, momentum.x(), momentum.y());
    }
    if (TestH3)
    {
      analysisManager->FillH3(0, momentum.x(), momentum.y(), momentum.z());
      analysisManager->FillH3(1, momentum.x(), momentum.y(), momentum.z());
      analysisManager->FillH3(2, momentum.x(), momentum.y(), momentum.z());
    }
  }
...
void CalorimeterSD::EndOfEvent(G4HCofThisEvent*)
{
  ...
  for (G4int i = 0; i < NofZLayers; i++)
  {
    ...
    if (TestP1)
    {
      analysisManager->FillP1(0, dLCumul, sumEdep / dLCumul);
      analysisManager->FillP1(1, dLCumul, sumEdep / dLCumul);
      analysisManager->FillP1(2, dLCumul, sumEdep / dLCumul);
    }
  }
  ...
  for (G4int ix = 0; ix < NofXYLayers; ix++)
  {
    ...
    for (G4int iy = 0; iy < NofXYLayers; iy++)
    {
      ...
      if (TestP2)
      {
        analysisManager->FillP2(0, dXCumul, dYCumul, edep);
        analysisManager->FillP2(1, dXCumul, dYCumul, edep);
        analysisManager->FillP2(1, dXCumul, dYCumul, edep);
      }
    }
  }
  ...
  if (verboseLevel > 1)
  {
    G4cout << "\n-------->Z Hits Collection: in this event they are " << nofZHits
           << " hits: " << G4endl;
    for (G4int i = 0; i < nofZHits; i++)
      (*fZHitsCollection)[i]->Print();
  }
}
