...
void RunAction::TestWriting() const
{
  ...
  if ( MergeNtuple ) {
    G4cout << " mergeNtuple = " << MergeNtuple
           << " nofNtupleFiles = " << NofNtupleFiles << G4endl;
  } else {
    G4cout << G4endl;
  }
  ...
  if ( TestH1 ) {
    ...
    std::vector<G4double> edges2 =
      { 0., 10.*MeV, 30.*MeV, 40.*MeV, 60.*MeV, 70.*MeV, 90.*MeV, 100.*MeV };
  ...
  if ( TestP2 ) {
    // 2D profiles
    analysisManager->CreateP2("Edep_Vs_XY", "Longit Edep XY pseudo-profile",        // ID = 0
                              NofXYLayers, - NofXYLayers*LayerXYSize/2, NofXYLayers*LayerXYSize/2,
                              NofXYLayers, - NofXYLayers*LayerXYSize/2, NofXYLayers*LayerXYSize/2,
                              0, 10*GeV);
    analysisManager->CreateP2("Edep_Vs_XY_set", "Longit Edep XY pseudo-profile #2", // ID = 1
                              NofXYLayers, - NofXYLayers*LayerXYSize/2, NofXYLayers*LayerXYSize/2,
                              NofXYLayers, - NofXYLayers*LayerXYSize/2, NofXYLayers*LayerXYSize/2,
                              0, 10*GeV);
    analysisManager->CreateP2("Edep_Vs_XY_set_dim", "Longit Edep XY pseudo-profile #3", // ID = 2
                              NofXYLayers, - NofXYLayers*LayerXYSize/2, NofXYLayers*LayerXYSize/2,
                              NofXYLayers, - NofXYLayers*LayerXYSize/2, NofXYLayers*LayerXYSize/2,
                              0, 10*GeV);
    // Ascii
  ...
  if ( TestNtuple ) {
    ...
    analysisManager
      ->CreateNtupleSColumn("LabelVector", fEventAction->GetLabelVector());
...
void RunAction::TestReading() const
{
  ...
  if ( TestNtuple ) {
    ...
    ntupleId
      = analysisReader->GetNtuple("TrackL", "testAnalysis.root", ntupleDir);
#endif
#ifdef TEST_ANALYSIS_GENERIC
    // Root output filename depends on NofNtupleFiles,
    // lets always take testAnalysis.root
    ntupleId
      = analysisReader->GetNtuple("TrackL", "testAnalysisG.root", ntupleDir);
#endif
#else
    if ( ! G4Threading::IsMultithreadedApplication() ||
      ...
      ntupleId
        = analysisReader->GetNtuple("TrackL", "testAnalysis_t1.hdf5", ntupleDir);
#endif
#ifdef TEST_ANALYSIS_XML
      ntupleId
        = analysisReader->GetNtuple("TrackL", "testAnalysis_nt_TrackL_t1.xml", ntupleDir);
#endif
#ifdef TEST_ANALYSIS_CSV
      ntupleId
        = analysisReader->GetNtuple("TrackL", "testAnalysis_nt_TrackL_t1.csv", ntupleDir);
#endif
    }
#endif
    if ( ntupleId >= 0 ) {
    ...
    // lets always take testAnalysis.root
    ntupleId
      = analysisReader->GetNtuple("EDep", "testAnalysis.root", ntupleDir);
#endif
#ifdef TEST_ANALYSIS_GENERIC
    // Root output filename depends on NofNtupleFiles,
    // lets always take testAnalysis.root
    ntupleId
      = analysisReader->GetNtuple("EDep", "testAnalysisG.root", ntupleDir);
#endif
#else
    if ( ! G4Threading::IsMultithreadedApplication() ||
         ( G4Threading::IsMultithreadedApplication() && ! isMaster ) ) {
         // In MT application only ntuples are written only on workers
      ntupleId
        = analysisReader->GetNtuple("EDep", "", ntupleDir);
            // file name can be omitted then the file name will be
...
#ifdef TEST_ANALYSIS_HDF5
      ntupleId
        = analysisReader->GetNtuple("EDep", "testAnalysis_t1.hdf5", ntupleDir);
#endif
#ifdef TEST_ANALYSIS_XML
      ntupleId
        = analysisReader->GetNtuple("EDep", "testAnalysis_nt_EDep_t1.xml", ntupleDir);
#endif
#ifdef TEST_ANALYSIS_CSV
      ntupleId
        = analysisReader->GetNtuple("EDep", "testAnalysis_nt_EDep_t1.csv", ntupleDir);
#endif
    }
#endif
