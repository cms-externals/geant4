...
EventAction::EventAction()
  : G4UserEventAction(), fZAbsHCID(-1), fEdepVector(), fTrackLengthVector(), fLabelVector()
{}
...
void EventAction::EndOfEventAction(const G4Event* event)
{
  ...
  G4String label = "event" + std::to_string(eventID);
  // Fill label vector
  fLabelVector.push_back(label);
  ...
  if (TestH1)
  {
    ...
    analysisManager->FillH1(15, absoHit->GetEdep());
  }
  for (G4int i = 1; i < 7; i++)
  {
  ...
  if (TestNtuple)
  {
    ...
    analysisManager->FillNtupleSColumn(0, 2, label);
  ...
  if (TestNtuple && eventID < 10)
  {
    ...
    G4cout << "   LabelVector: ";
    for (G4int i = 0; i < G4int(fLabelVector.size()); ++i)
    {
      G4cout << fLabelVector[i] << "  ";
    }
    G4cout << G4endl;
  }
}
