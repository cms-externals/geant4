  ...
  */
  ...
  for (G4int run = 0; run < 100; run++)
  {
    ...
    /*
        ...
        std::cout << " For material " << material->GetName() << ", number of elements = " << nEl <<
       std::endl;
        ...
        G4cout << " For materials " << material->GetName() << " Number of NISTIsotopes = " << NIso
       << G4endl;
    ...
    */
    ...
    //    GetCaptureCrossSection(&dParticle, Z ); // 2nd arg used to be G4Element*, but now it's
    //    G4int Z
    ...
    */
    ...
    if (verbose >= 2)
    {
      G4cout << "Possible secondary status codes: "
             << " fAlive " << fAlive << " fStopButAlive " << fStopButAlive << " fStopAndKill "
             << fStopAndKill << " fKillTrackAndSecondaries " << fKillTrackAndSecondaries
             << " fSuspend " << fSuspend << " fPostponeToNextEvent " << fPostponeToNextEvent
             << G4endl;
    }
    ...
    for (G4int iter = 0; iter < nevt; ++iter)
    {
      ...
      for (G4int i = 0; i < nsecnd; ++i)
      {
        ...
        if (verbose >= 1)
        {
          G4cout << "Secondary: " << i << " is " << sName << ", " << sdp->GetPDGcode()
                 << ", status: " << sts << " with kinetic energy of "
                 << sdp->GetKineticEnergy() / GeV << " and momentum of "
                 << sdp->GetTotalMomentum() / GeV << " GeV" << G4endl;
        }
        ...
        if (sdp->GetPDGcode() > 2000000000)
        {
          if (namePart != "mu-" || nsecnd != 1)
          {
            ...
            G4cout << "!!!!! " << namePart << " nsecondaries "
                   << "nsecnd " << " expected mu- and 1 secondary" << G4endl;
          }
          ...
          if (verbose >= 0 and iter == 50000 * (iter / 50000))
            G4cout << "##### " << nssecnd << " nsecondaries ##### of " << sName << G4endl;
          else if (verbose >= 1)
            G4cout << "##### " << nssecnd << " nsecondaries ##### of " << sName << G4endl;
          ...
          for (G4int k = 0; k < nssecnd; ++k)
          {
            ...
            if (verbose >= 1)
            {
              G4cout << "Secondary: " << k << " is " << ssName << ", " << ssdp->GetPDGcode()
                     << ", status: " << ssts << " with kinetic energy of "
                     << ssdp->GetKineticEnergy() / GeV << " and momentum of "
                     << ssdp->GetTotalMomentum() / GeV << " GeV" << G4endl;
            }
