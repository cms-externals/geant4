...
//
// standard interface for pi-, K- and Sigma-
//
...
G4VProcess* TestStoppingPhysics::GetProcess(const G4String& gen_name, const G4String& part_name)
{
  G4cout << part_name << gen_name << " initializing 0" << G4endl;
  ...
  else if (part_name == "mu-")
  {
    ...
    while ((*myParticleIterator)())
    {
      ...
      if (verboseLevel > 1)
      {
        G4cout << "TestStoppingPhysics::GetProcess"
               << " : Checking Process Manager for " << particle->GetParticleName() << G4endl;
      }
      ...
      if (pmanager == 0)
      {
        ...
        {
          G4cout << "TestStoppingPhysics::GetProcess"
                 << " : Creating Process Manager for " << particle->GetParticleName() << G4endl;
        }
    ...
    if (verboseLevel > 1)
    {  // use verboseLevel to suppress compilation warning
  ...
  G4cout << part_name << gen_name << " initializing 1" << G4endl;
  ...
  )
  {
    ...
    else if (part_name == "mu-")
    {
      ...
      else if (gen_name == "newmustopping")
      {
        ...
        G4cout << " Model <" << gen_name << "> is initialized" << G4endl;
        return theProcess;
      }
#endif
    }
  ...
  else if (gen_name == "Bertini" || gen_name == "BertiniPreCo")
  {
    if (part_name == "kaon-")
    {
      ...
      */
      ...
      //
      theProcess = new G4HadronicAbsorptionBertini(G4KaonMinus::Definition());
    }
    else if (part_name == "sigma-")
    {
      theProcess = new G4HadronicAbsorptionBertini(G4SigmaMinus::Definition());
    }
    else if (part_name == "pi-")
    {
      ...
      theProcess = new G4HadronicAbsorptionBertini(G4PionMinus::Definition());
      /*
              TestBertiniStopping* proc = new TestBertiniStopping();
              if ( gen_name == "BertiniPreCo" )
              {
  ...
  else if (gen_name == "FTF")
  {
    if (part_name == "anti_proton")
    {
      theProcess = new G4HadronicAbsorptionFritiof(G4AntiProton::Definition());
    }
  }
  else
  {
    G4cout << gen_name << " generator is unkown - no hadron production" << G4endl;
  }
