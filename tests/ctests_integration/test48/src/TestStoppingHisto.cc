...
void TestStoppingHisto::Init()
{
  ...
  G4cout << "Initializing histograms for: " << fHistoTitle << G4endl;
...
void TestStoppingHisto::InitHistoGeneral()
{
  ...
  fHisto.push_back(
    new TH1F("ChargedPionMomentum", fHistoTitle.c_str(), nbins_mom_pions, bins_mom_pions));  // 8
...
int TestStoppingHisto::Topology(G4VParticleChange* aChange)
{
  ...
  if ((NSec - firstHadronPos) < 2)
    return channel;  // it should be at least 2 secondaries lest at the end of the list
...
void TestStoppingHisto::Write(int stat)
{
  ...
  G4cout << "Writing histogram file: " << fname << G4endl;
