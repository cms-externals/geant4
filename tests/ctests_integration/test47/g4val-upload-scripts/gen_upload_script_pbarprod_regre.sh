...
regression=(g4-10.1-p03 g4-10.2-p02)

script=${testname}-pbarprod-upload-regre.xml
if [ -e ${script} ]; then
/bin/rm ${script}
fi
...
printf "</score>\n" >> ${script}
printf "<imageblob>${beam[$i]}-${target[$k]}-${mom}GeV-pbar-${angle[$n]}deg-${model[$m]}-regre.gif</imageblob>\n" >> ${script}

printf "<taglist>\n" >> ${script}
printf "<tag>\n" >> ${script}
printf "<name>Model</name>\n" >> ${script}
printf "<value>${model[$m]}</value>\n" >> ${script}
printf "</tag>\n" >> ${script}
