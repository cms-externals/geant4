                         ...
                         G4ThreeVector& aPosition)
{
  ...
  if (debug)
    G4cout << "HistoEPTest47:: DpByP " << dpbyp << " R:Z " << r << ":" << z << " Angle(X:Y) "
           << angx << ":" << angy << G4endl;
  hiDpByp->Fill(dpbyp);
  hiR->Fill(r);
  hiX->Fill(x);
...
void HistoEPTest47::initialize(std::string namePart, std::string nameMat, G4double momentum,
                               std::string nameGen)
{
  pin = momentum;
  ...
  if (hiAngX) delete hiAngX;
  if (hiAngY) delete hiAngY;
  snprintf(name, sizeof(name), "PxBalance");
  snprintf(title, sizeof(title), "Px Balance in %s + %s at %6.2f GeV/c (%s)", namePart.c_str(),
           nameMat.c_str(), momentum, nameGen.c_str());
  hiPx = new TH1F(name, title, 5000, -5.0, 5.0);
  hiPx->Sumw2();
  snprintf(name, sizeof(name), "PyBalance");
  snprintf(title, sizeof(title), "Py Balance in %s + %s at %6.2f GeV/c (%s)", namePart.c_str(),
           nameMat.c_str(), momentum, nameGen.c_str());
  hiPy = new TH1F(name, title, 5000, -5.0, 5.0);
  hiPy->Sumw2();
  snprintf(name, sizeof(name), "PzBalance");
  snprintf(title, sizeof(title), "Pz Balance in %s + %s at %6.2f GeV/c (%s)", namePart.c_str(),
           nameMat.c_str(), momentum, nameGen.c_str());
  hiPz = new TH1F(name, title, 5000, -5.0, 5.0);
  hiPz->Sumw2();
  snprintf(name, sizeof(name), "EBalance");
  snprintf(title, sizeof(title), "E Balance in %s + %s at %6.2f GeV/c (%s)", namePart.c_str(),
           nameMat.c_str(), momentum, nameGen.c_str());
  hiE = new TH1F(name, title, 5000, -5.0, 195.0);
  hiE->Sumw2();
  snprintf(name, sizeof(name), "KEBalance");
  snprintf(title, sizeof(title), "KE Balance in %s + %s at %6.2f GeV/c (%s)", namePart.c_str(),
           nameMat.c_str(), momentum, nameGen.c_str());
