...
HistoPbarProdTest47::HistoPbarProdTest47(G4String htitle) : TstHistoSet(htitle)
{
  ...
  fMultEvtWithPbar =
    new TH1F("MultEvtWithPbar", "Multiplicity of interactions that have pbar", 50, 0., 50.);
...
void HistoPbarProdTest47::FillEvt(G4VParticleChange* aChange, const G4LorentzVector&,
...
void HistoPbarProdTest47::Write(G4int nevt /* stat */, G4double xsec)
{
  ...
  for (unsigned int i = 0; i < fInvXSecVsMomPbar.size(); ++i)
  {
    ...
    // need to convert to microbarn as the 10GeV data are given in these units
    // also need to consider what to do with !GeV data as they're given in nanobarns
    G4double scale = (xsec * CLHEP::millibarn / CLHEP::microbarn)
                     / ((double)nevt * 2. * CLHEP::pi * fDeltaCosThetaSecOut[i]);
