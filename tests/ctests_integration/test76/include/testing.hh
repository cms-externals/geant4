...
#define EXPECT_NE(lhs, rhs) if(lhs == rhs) { \
    std::stringstream ss; \
    ss << "Expectation failed : " \
       << #lhs << " == " << #rhs << " @ line " \
       << __LINE__ << " of " << __FILE__ << " (" \
...
#define ASSERT_CLOSE(lhs, rhs, epsilon) if(fabs( lhs - rhs ) > epsilon ) { \
    std::stringstream ss; \
    double _diff = fabs( lhs - rhs ); \
    ss << "failed. \n\tExpression: fabs( " << lhs << " - " << rhs << " ) < " \
       << std::scientific << std::setprecision(6) << epsilon \
       << " [ diff = " << _diff << " ] " \
       << "@ line " \
       << __LINE__ << " of " << __FILE__; \
    std::cerr << ss.str() << std::endl; \
...
#define TEST_SUMMARY(argv_0, ntest_counter, nfail_counter) { \
    ...
    if(num_fail > 0) \
        ...
        ss << "Tests passed: " << (ntest_counter - nfail_counter) << "/" << ntest_counter << std::endl; \
    std::cout << ss.str(); \
    }
...
#define RUN_TEST(func, ntest_counter, nfail_counter) { \
    try { \
        std::cout << "Running test " << #func << "... " << std::flush; \
        ntest_counter += 1; \
        func (); \
    } \
    catch(std::exception& e) { \
