...
Tst202DetectorConstruction::Tst202DetectorConstruction() : verbosity(0)
{
  ...
  volumeSelection["Ellipsoid"] = true;
  ...
  volumeSelection["G4Torus"] = true;

  G4cout << "Selectable volume items (with default value):"
  ...
  {
    G4cout << i->first << " (" << std::boolalpha << i->second << std::noboolalpha << ")" << G4endl;
  }
...
G4VPhysicalVolume* Tst202DetectorConstruction::Construct()
{
  ...
  //------------------------------ experimental hall
  G4Box* experimentalHall_box = new G4Box("expHall_b", expHall_x, expHall_y, expHall_z);
  ...
  G4VPhysicalVolume* experimentalHall_phys =
    new G4PVPlacement(0, G4ThreeVector(), "expHall_P", experimentalHall_log, 0, false, 0);
  ...
  {
    G4Box* calorimeter_box = new G4Box("calorimeter_b", calBox_x, calBox_y, calBox_z);
    G4Material* calMat;
    ...
    calorimeter_log = new G4LogicalVolume(calorimeter_box, calMat, "calo_L", 0, 0, 0);
    G4VisAttributes* calorimeterVisAtt = new G4VisAttributes(G4Colour(0., 0., 1., 0.5));
    ...
    for (G4int i = 0; i < 3; i++)
    {
      ...
      calo_phys = new G4PVPlacement(G4Transform3D(rm, G4ThreeVector(0. * cm, i * calPos, 0. * cm)),
                                    "calo_phys", calorimeter_log, experimentalHall_phys, false, i);
      if (verbosity) calo_phys->GetObjectRotationValue().print(G4cout);
    }
  }
  ...
  if (volumeSelection["box_in_box"])
  {
    ...
    new G4PVPlacement(G4Translate3D(0., 600. * cm, 0.), log, name, experimentalHall_log, false, 0);
  }
  ...
  {
    G4Tubs* tracker_tube =
      new G4Tubs("tracker_tube", 0. * cm, trackerRadius, trackerHight, 0. * deg, 360. * deg);
    /*
      G4cout << "Tracker tube polyhedron:\n"
      << (HepPolyhedron)(*(tracker_tube->GetPolyhedron())) << G4endl;
    */
    G4LogicalVolume* tracker_log = new G4LogicalVolume(tracker_tube, Ar, "tracker_L", 0, 0, 0);
    G4VisAttributes* trackerVisAtt = new G4VisAttributes(G4Colour(0., 0., 1.));
    ...
    if (verbosity) tracker_rm->print(G4cout);
    G4VPhysicalVolume* tracker_phys =
      ...
      new G4PVPlacement(G4Transform3D(*tracker_rm, G4ThreeVector(0. * cm, trackerPos, 200. * cm)),
                        ...
                        "tracker_phys", tracker_log, experimentalHall_phys, false, 0);
    if (verbosity) tracker_phys->GetObjectRotationValue().print(G4cout);
  }
  ...
  if (volumeSelection["displaced_box"])
  {
    ...
    G4DisplacedSolid* displaced_box = new G4DisplacedSolid(
      "displaced_box", undisplaced_box, G4Transform3D(rm_db, G4ThreeVector(200. * cm, 0., 0.)));
    if (verbosity) G4cout << "Displaced box extent:\n" << displaced_box->GetExtent() << G4endl;
    G4LogicalVolume* displaced_box_log =
      new G4LogicalVolume(displaced_box, Ar, "displaced_box_L", 0, 0, 0);
    new G4PVPlacement(0, G4ThreeVector(0. * cm, -200. * cm, 0. * cm), "displaced_box_phys",
                      displaced_box_log, experimentalHall_phys, false, 0);
  }
  ...
  if (volumeSelection["Boolean_solids"])
  {
    ...
    G4IntersectionSolid* cyl1Ibox1 =
      new G4IntersectionSolid("cylinder1-intersection-box1", cylinder1, box1, rm1,
                              G4ThreeVector(30.01 * cm, 30.01 * cm, 0.01 * cm));
    if (verbosity)
      G4cout << "cylinder1-intersection-box1 extent:\n" << cyl1Ibox1->GetExtent() << G4endl;
    G4IntersectionSolid* cyl1Ibox1Ibox2 =
      new G4IntersectionSolid("cylinder1-intersection-box1-intersection-box2", cyl1Ibox1, box2, rm2,
                              G4ThreeVector(0.01, 40.01 * cm, 0.01 * cm));
    if (verbosity)
    ...
    const G4VisAttributes* bool_red = new G4VisAttributes(G4Colour(1., 0., 0.));
    intersection_log->SetVisAttributes(bool_red);
    new G4PVPlacement(0, G4ThreeVector(100. * cm, -50 * cm, 0. * cm), "intersection_phys",
                      intersection_log, experimentalHall_phys, true, 0);

    G4SubtractionSolid* cyl1Sbox1 =
      new G4SubtractionSolid("cylinder1-subtraction-box1", cylinder1, box1, rm1,
                             G4ThreeVector(30. * cm, 30. * cm, 1. * cm));
    G4SubtractionSolid* cyl1Sbox1Sbox2 =
      new G4SubtractionSolid("cylinder1-subtraction-box1-subtraction-box2", cyl1Sbox1, box2, rm2,
                             G4ThreeVector(0., 40 * cm, 2. * cm));
    G4LogicalVolume* subtraction_log =
      new G4LogicalVolume(cyl1Sbox1Sbox2, Ar, "subtraction_L", 0, 0, 0);
    const G4VisAttributes* bool_green = new G4VisAttributes(G4Colour(0., 1., 0.));
    subtraction_log->SetVisAttributes(bool_green);
    new G4PVPlacement(0, G4ThreeVector(200. * cm, -50 * cm, 0. * cm), "subtraction_phys",
                      subtraction_log, experimentalHall_phys, true, 0);

    G4UnionSolid* cyl1Ubox1 = new G4UnionSolid("cylinder1-union-box1", cylinder1, box1, rm1,
                                               G4ThreeVector(30. * cm, 30. * cm, 1. * cm));
    G4UnionSolid* cyl1Ubox1Ubox2 = new G4UnionSolid("cylinder1-union-box1-union-box2", cyl1Ubox1,
                                                    box2, rm2, G4ThreeVector(0., 40 * cm, 2. * cm));
    G4LogicalVolume* union_log = new G4LogicalVolume(cyl1Ubox1Ubox2, Ar, "union_L", 0, 0, 0);
    const G4VisAttributes* bool_blue = new G4VisAttributes(G4Colour(0., 0., 1.));
    union_log->SetVisAttributes(bool_blue);
    new G4PVPlacement(0, G4ThreeVector(350. * cm, -50 * cm, 0. * cm), "union_phys", union_log,
                      experimentalHall_phys, true, 0);
  }
  ...
  if (volumeSelection["Tubes_etc"])
  {
    ...
    G4VisAttributes* grey = new G4VisAttributes(G4Colour(0.5, 0.5, 0.5, 0.1));

    G4double tube_dPhi = twopi;
    G4Tubs* tube = new G4Tubs("tube", 20 * cm, 50 * cm, 30 * cm, 0., tube_dPhi);
    G4LogicalVolume* tube_log = new G4LogicalVolume(tube, Ar, "tube_L", 0, 0, 0);
    ...
    new G4PVPlacement(0, G4ThreeVector(-200. * cm, 0., 0. * cm), "tube_phys", tube_log,
                      experimentalHall_phys, false, 0);

    G4double divided_tube_dPhi = tube_dPhi / 6.;
    G4Tubs* divided_tube = new G4Tubs("divided_tube", 20 * cm, 50 * cm, 30 * cm,
    ...
    new G4PVReplica("divided_tube_phys", divided_tube_log, tube_log, kPhi, 6, divided_tube_dPhi);
    /************
  G4int iCopy;
  ...
  G4ThreeVector()),
    ...
    *********/
    ...
    G4Tubs* divided_tube_inset =
      new G4Tubs("divided_tube_inset", 20 * cm + eps, 50 * cm - eps, 30 * cm - eps,
                 -divided_tube_inset_dPhi / 2., divided_tube_inset_dPhi);
    divided_tube_inset_log =
    ...
    new G4PVPlacement(0, G4ThreeVector(), divided_tube_inset_log, "divided_tube_inset_phys",
                      divided_tube_log, false, 0);

    G4double sub_divided_tube_dPhi = divided_tube_inset_dPhi / 4.;
    G4Tubs* sub_divided_tube =
      new G4Tubs("sub_divided_tube", 20 * cm + eps, 50 * cm - eps, 30 * cm - eps,
    ...
    new G4PVReplica("sub_divided_tube_phys", sub_divided_tube_log, divided_tube_inset_log, kPhi, 4,
                    sub_divided_tube_dPhi, -divided_tube_inset_dPhi / 2.);
    /************
  ...
  (-divided_tube_inset_dPhi/2.
    ...
    ************/
    ...
    G4VisAttributes* daughter_box_VisAtt = new G4VisAttributes(G4Colour(0., 0., 1., 0.3));
    ...
    G4VisAttributes* grand_daughter_box_VisAtt = new G4VisAttributes(G4Colour(1., 1., 0.));
    ...
    grand_daughter_box2_log =
      new G4LogicalVolume(grand_daughter_box2, Ar, "grand_daughter_box2_L", 0, 0, 0);
    G4VisAttributes* grand_daughter_box2_VisAtt = new G4VisAttributes(G4Colour(1., 0., 1.));
    grand_daughter_box2_log->SetVisAttributes(grand_daughter_box2_VisAtt);
    new G4PVPlacement(0, G4ThreeVector(-2 * cm, 0., 0.), grand_daughter_box_log,
                      "grand_daughter_box_phys", daughter_box_log, false, 0);
    new G4PVPlacement(0, G4ThreeVector(2 * cm, 0., 0.), grand_daughter_box2_log,
                      "grand_daughter_box2_phys", daughter_box_log, false, 0);
    new G4PVPlacement(0, G4ThreeVector(40 * cm, 0., 0.), daughter_box_log, "daughter_box_phys",
                      sub_divided_tube_log, false, 0);

    //------------------------------------------------ Extra placements
    // For good measure, as a test of drawn volume path, place in one of
    // the earlier volumes...
    new G4PVPlacement(0, G4ThreeVector(), "divided_tube_inset_phys", divided_tube_inset_log,
                      calo_phys, false, 0);  // Place in PV.
    new G4PVPlacement(0, G4ThreeVector(), grand_daughter_box2_log, "grand_daughter_box2_phys",
  ...
  {
    G4Sphere* PD_vol_sphere = new G4Sphere("Test_Sphere",
                                           100. * cm,  // inner radius
                                           200. * cm,  // outer radius
                                           0.,  // start phi
                                           90. * degree,  // delta phi
    ...
    G4LogicalVolume* PD_log_sphere = new G4LogicalVolume(PD_vol_sphere, Ar, "Test_Sphere");

    G4VisAttributes* PD_att_sphere = new G4VisAttributes(G4Colour(1., 0., 1.));
    PD_log_sphere->SetVisAttributes(PD_att_sphere);

    G4RotationMatrix rm;

    new G4PVPlacement(G4Transform3D(rm, G4ThreeVector(200. * cm, 300. * cm, 0)), "PD_physical",
                      PD_log_sphere, experimentalHall_phys, false, 0);
  }
  ...
  if (volumeSelection["Polyhedra"])
  {
    ...
    G4Polyhedra* polyhedra_solid =
      new G4Polyhedra("Polyhedra_Test", 0., 270. * deg, 6, numRZ, polyhedra_r, polyhedra_z);
    // 0.,twopi,6,numRZ,polyhedra_r,polyhedra_z);
    if (verbosity) polyhedra_solid->DumpInfo();

    G4LogicalVolume* polyhedra_log = new G4LogicalVolume(polyhedra_solid, Ar, "Polyhedra_Test");

    G4VisAttributes* polyhedra_atts = new G4VisAttributes(G4Colour(0., 1., 1.));
    polyhedra_log->SetVisAttributes(polyhedra_atts);

    G4RotationMatrix rm;

    new G4PVPlacement(G4Transform3D(rm, G4ThreeVector(200. * cm, 100. * cm, 0)), "Polyhedra_Test",
                      polyhedra_log, experimentalHall_phys, false, 0);
  }
  ...
  if (volumeSelection["Polycone"])
  {
    ...
    G4GenericPolycone* polycone_solid =
      new G4GenericPolycone("Polycone_Test", 0. * deg, 270. * deg, numRZ1, polycone_r, polycone_z);
    // 0.,twopi,numRZ,polycone_r,polycone_z);
    if (verbosity) polycone_solid->DumpInfo();

    G4LogicalVolume* polycone_log = new G4LogicalVolume(polycone_solid, Ar, "Polycone_Test");

    G4VisAttributes* polycone_atts = new G4VisAttributes(G4Colour(1., 0.5, 0.5));
    polycone_log->SetVisAttributes(polycone_atts);

    G4RotationMatrix rm;

    new G4PVPlacement(G4Transform3D(rm, G4ThreeVector(400. * cm, 120. * cm, 0)), "Polycone_Test",
                      polycone_log, experimentalHall_phys, false, 0);
    ...
    for (int j = 0; j < 9; j++)
    {
      ...
      tempInner.push_back((2.5 - std::sqrt(1 - ((1.9 - 0.1 * j) * (1.9 - 0.1 * j)) / 4) * 2.5)
                          * rplanet);
      tempOuter.push_back((2.5 + std::sqrt(1 - ((1.9 - 0.1 * j) * (1.9 - 0.1 * j)) / 4) * 2.5)
                          * rplanet);
    }
    for (int j = 0; j < 23; j++)
    {
      tempZ.push_back((1.1 - 0.1 * j) * rplanet);
      tempInner.push_back((std::sqrt(1 - ((1.1 - 0.1 * j) * (1.1 - 0.1 * j)) / (1.2 * 1.2)) * 1.2)
                          * rplanet);
      tempOuter.push_back((2.5 + std::sqrt(1 - ((1.1 - 0.1 * j) * (1.1 - 0.1 * j)) / 4) * 2.5)
                          * rplanet);
    }
    for (int j = 0; j < 9; j++)
    {
      tempZ.push_back((-1.1 - 0.1 * j) * rplanet);
      tempInner.push_back((2.5 - std::sqrt(1 - ((1.1 + 0.1 * j) * (1.1 + 0.1 * j)) / 4) * 2.5)
                          * rplanet);
      tempOuter.push_back((2.5 + std::sqrt(1 - ((1.1 + 0.1 * j) * (1.1 + 0.1 * j)) / 4) * 2.5)
                          * rplanet);
    }
    ...
    for (int i = 0; i < 41; i++)
    {
      ...
      zPlane[i] = -tempZ[i];
      if (verbosity)
        G4cout << i << ' ' << zPlane[i] << ' ' << rInner[i] << ' ' << rOuter[i] << G4endl;
    }
    G4double R[82], Z[82];
    for (int i = 0; i < 41; i++)
    {
    ...
    G4Polycone* VanAllenBelt =
      new G4Polycone("VanAllenBelt", 0, 2 * pi, 41, zPlane, rInner, rOuter);
    ...
    new G4PVPlacement(G4Transform3D(rm, G4ThreeVector(600. * cm, 400. * cm, 0)), "VanAllenBelt",
                      logicVanAllenBelt, experimentalHall_phys, false, 0);
  }
  ...
  {
    G4Orb* orb = new G4Orb("Test_orb", 100. * cm);

    G4LogicalVolume* orb_log = new G4LogicalVolume(orb, Ar, "Test_orb");

    G4VisAttributes* orb_att = new G4VisAttributes(G4Colour(1., 0., 1.));
    orb_log->SetVisAttributes(orb_att);

    new G4PVPlacement(G4Translate3D(-300. * cm, 200. * cm, 0), "Test_orb", orb_log,
                      experimentalHall_phys, false, 0);
  }
  ...
  if (volumeSelection["Trapezoid"])
  {
    ...
    new G4PVPlacement(G4Transform3D(rm, G4ThreeVector(-200. * cm, 200. * cm, -200. * cm)),
                      "trap1_phys", trap1_log, experimentalHall_phys, false, 0);
    ...
    new G4PVPlacement(G4Transform3D(rm, G4ThreeVector(-200. * cm, 400. * cm, -200. * cm)),
                      "trap2_phys", trap2_log, experimentalHall_phys, false, 0);
  }
  ...
  if (volumeSelection["Elliptical_Tube"])
  {
    ...
    new G4PVPlacement(G4Translate3D(G4ThreeVector(-400. * cm, 0., 0)), "e-tube-phys", eTubeLog,
                      experimentalHall_phys, false, 0);
  }
  ...
  if (volumeSelection["Elliptical_Cone"])
  {
    ...
    new G4PVPlacement(G4Translate3D(G4ThreeVector(-500. * cm, 400. * cm, 0)), "e-cone-phys",
                      eConeLog, experimentalHall_phys, false, 0);
  }

  //-------------------------------------------- Ellipsoid
  if (volumeSelection["Ellipsoid"])
  {
    G4VSolid* eTube = new G4Ellipsoid("ellipdsoid", 30. * cm, 50. * cm, 70. * cm);
    G4LogicalVolume* eTubeLog = new G4LogicalVolume(eTube, Ar, "ellipdsoid-log");
    new G4PVPlacement(G4Translate3D(G4ThreeVector(-500. * cm, 200. * cm, 0)), "ellipdsoid-phys",
                      eTubeLog, experimentalHall_phys, false, 0);
  }
  ...
  {
    G4VSolid* eCons =
      new G4Cons("e-cons", 50. * cm, 70. * cm, 100. * cm, 140. * cm, 200. * cm, 0, twopi);
    G4LogicalVolume* eConsLog = new G4LogicalVolume(eCons, Ar, "e-cons-log");
    new G4PVPlacement(G4Translate3D(G4ThreeVector(-300. * cm, -200. * cm, 60 * cm)), "e-cons-phys",
                      eConsLog, experimentalHall_phys, false, 0);
  }
  ...
  if (volumeSelection["Simple_shared"])
  {
    ...
    new G4PVPlacement(G4Translate3D(G4ThreeVector(600. * cm, 0., 0.)), "B", log2,
                      experimentalHall_phys, false, 0);
    new G4PVPlacement(G4Translate3D(G4ThreeVector(900. * cm, 0., 0.)), "C", log2,
                      experimentalHall_phys, false, 0);
    G4VSolid* boxThree = new G4Box("Box3", 50. * cm, 50. * cm, 50. * cm);
    G4LogicalVolume* log3 = new G4LogicalVolume(boxThree, Ar, "Log3");
    new G4PVPlacement(G4Translate3D(), log3, "D", log2, false, 0);
    G4VSolid* boxFour = new G4Box("Box3", 30. * cm, 30. * cm, 30. * cm);
    G4LogicalVolume* log4 = new G4LogicalVolume(boxFour, Ar, "Log4");
    new G4PVPlacement(G4Translate3D(), log4, "E", log3, false, 0);
  }
  ...
  if (volumeSelection["Boolean_for_logical_volume_test"])
  {
    ...
    G4Box* box1B = new G4Box("alSLayer_box", alSLayer_x / 2., alSLayer_y / 2., alSLayer_z / 2.);

    G4Tubs* Cylinder1 =
      new G4Tubs("Cylinder#1", 0. * mm, alSLayer_x / 2., alSLayer_x / 2., 0., 2. * CLHEP::pi);

    G4UnionSolid* b1UnionC1 = new G4UnionSolid("Box+Cylinder", box1B, Cylinder1);

    G4LogicalVolume* alSpaceCraft_log = new G4LogicalVolume(b1UnionC1, Al, "alLayer_log", 0, 0, 0);

    new G4PVPlacement(0, G4ThreeVector(900. * cm, 200. * cm, 0.), alSpaceCraft_log, "alLayer_phys",
                      experimentalHall_log, false, 0);
    // new G4PVPlacement(G4Translate3D(G4ThreeVector(900.*cm, 200.*cm,
    // 0.)),"alLayer_phys",alSpaceCraft_log,experimentalHall_log,false,0);
  }
  ...
  if (volumeSelection["Radially_replicated_tube_sector"])
  {
    ...
    G4VSolid* rrTubs = new G4Tubs("rrTubs", rMin, rMin + DeltaR, 200 * cm, 180 * deg, 90 * deg);
    G4LogicalVolume* rrTubsLog = new G4LogicalVolume(rrTubs, Ar, "rrTubs-log");
    rrTubsLog->SetVisAttributes(G4VisAttributes::GetInvisible());
    new G4PVPlacement(G4Translate3D(G4ThreeVector(400. * cm, -200. * cm, 0)), "rrTubs-phys",
                      rrTubsLog, experimentalHall_phys, false, 0);
    G4double deltaR = DeltaR / 6.;
    G4Tubs* drrTubs = new G4Tubs("drrTubs", rMin, rMin + deltaR, 200 * cm, 180 * deg, 90 * deg);
    G4LogicalVolume* drrTubsLog = new G4LogicalVolume(drrTubs, Ar, "drrTubs-log");
    drrTubsLog->SetVisAttributes(G4Colour::Red());
    new G4PVReplica("drrTubs-phys", drrTubsLog, rrTubsLog, kRho, 6, deltaR, rMin);
  }
  ...
  {
    G4VSolid* tet = new G4Tet("tet", G4ThreeVector(), G4ThreeVector(0., 100. * cm, 0.),
                              G4ThreeVector(0., 0., 100. * cm), G4ThreeVector(100 * cm, 0., 0.));
    G4LogicalVolume* tet_log = new G4LogicalVolume(tet, Ar, "tet-log");
    tet_log->SetVisAttributes(G4VisAttributes(G4Colour(0., 1., 0.)));
    new G4PVPlacement(G4Translate3D(G4ThreeVector(300. * cm, -400. * cm, 0.)), "tet-phys", tet_log,
                      experimentalHall_phys, false, 0);
  }
  ...
  if (volumeSelection["Twisted_Tubs"])
  {
    ...
    aSolid =
      new G4TwistedTubs("aTwistedTubs", fTwistAngle, fTrackerR1, fTrackerR2, fTrackerpDz, fPhi);
    // G4cout << "aTwistedTubs: volume: "
    //        << aSolid->GetCubicVolume() << G4endl;
    aLog = new G4LogicalVolume(aSolid, Ar, "aTwistedTubs-log");
    aLog->SetVisAttributes(G4VisAttributes(G4Colour(1., 1., 0.)));
    new G4PVPlacement(G4Translate3D(G4ThreeVector(200. * cm, -400. * cm, 0.)), "aTwistedTubs-phys",
                      aLog, experimentalHall_phys, false, 0);
  }
  ...
  if (volumeSelection["Twisted_Box"])
  {
    ...
    aSolid = new G4TwistedBox("aTwistedBox", fTwistAngle, fTrackerpDx1, fTrackerpDy1, fTrackerpDz);
    aLog = new G4LogicalVolume(aSolid, Ar, "aTwistedBox-log");
    aLog->SetVisAttributes(G4VisAttributes(G4Colour(1., 1., 0.)));
    new G4PVPlacement(G4Translate3D(G4ThreeVector(100. * cm, -400. * cm, 0.)), "aTwistedBox-phys",
                      aLog, experimentalHall_phys, false, 0);
  }
  ...
  if (volumeSelection["Twisted_Trd"])
  {
    ...
    aSolid = new G4TwistedTrd("aTwistedTrd", fTrackerpDx1, fTrackerpDx2, fTrackerpDy1, fTrackerpDy2,
                              fTrackerpDz, fTwistAngle);
    aLog = new G4LogicalVolume(aSolid, Ar, "aTwistedTrd-log");
    aLog->SetVisAttributes(G4VisAttributes(G4Colour(1., 1., 0.)));
    new G4PVPlacement(G4Translate3D(G4ThreeVector(000. * cm, -400. * cm, 0.)), "aTwistedTrd-phys",
                      aLog, experimentalHall_phys, false, 0);
  }
  ...
  if (volumeSelection["Another_Twisted_Trd"])
  {
    ...
    aSolid = new G4TwistedTrd("anotherTwistedTrd", fTrackerpDx1, fTrackerpDx2, fTrackerpDy1,
                              fTrackerpDy2, fTrackerpDz, fTwistAngle);
    aLog = new G4LogicalVolume(aSolid, Ar, "anotherTwistedTrd-log");
    aLog->SetVisAttributes(G4VisAttributes(G4Colour(1., 1., 0.)));
    new G4PVPlacement(G4Translate3D(G4ThreeVector(-100. * cm, -400. * cm, 0.)),
  ...
  if (volumeSelection["G4Paraboloid"])
  {
    ...
    new G4PVPlacement(G4Translate3D(G4ThreeVector(-200. * cm, -400. * cm, 0.)), "paraboloid-phys",
                      aLog, experimentalHall_phys, false, 0);
  }
  ...
  if (volumeSelection["G4GenericTrap"])
  {
    ...
    G4LogicalVolume* testTrap_log = new G4LogicalVolume(testTrap, Ar, "TestTrap-log");
    new G4PVPlacement(G4Translate3D(G4ThreeVector(600. * cm, -300. * cm, 0.)), "TestTrapCut-phys",
                      testTrap_log, experimentalHall_phys, false, 0);

    // Boolean subtraction
    G4VSolid* testCut = new G4Box("TestCut", 10. * cm, 10. * cm, 10. * cm);
    G4VSolid* testTrapCut = new G4SubtractionSolid("TestTrapCut", testTrap, testCut,
                                                   G4Translate3D(50. * cm, 50. * cm, 100. * cm));
    G4LogicalVolume* testTrapCut_log = new G4LogicalVolume(testTrapCut, Ar, "TestTrapCut-log");
    new G4PVPlacement(G4Translate3D(G4ThreeVector(800. * cm, -300. * cm, 0.)), "TestTrapCut-phys",
                      testTrapCut_log, experimentalHall_phys, false, 0);
  }
  ...
  if (volumeSelection["Simple_tessellated"])
  {
    ...
    G4TriangularFacet* facet1 = new G4TriangularFacet(
      G4ThreeVector(-targetSize, -targetSize, 0.0), G4ThreeVector(+targetSize, -targetSize, 0.0),
      G4ThreeVector(0.0, 0.0, +targetSize), ABSOLUTE);
    G4TriangularFacet* facet2 = new G4TriangularFacet(
      G4ThreeVector(+targetSize, -targetSize, 0.0), G4ThreeVector(+targetSize, +targetSize, 0.0),
      G4ThreeVector(0.0, 0.0, +targetSize), ABSOLUTE);
    G4TriangularFacet* facet3 = new G4TriangularFacet(
      G4ThreeVector(+targetSize, +targetSize, 0.0), G4ThreeVector(-targetSize, +targetSize, 0.0),
      G4ThreeVector(0.0, 0.0, +targetSize), ABSOLUTE);
    G4TriangularFacet* facet4 = new G4TriangularFacet(
      G4ThreeVector(-targetSize, +targetSize, 0.0), G4ThreeVector(-targetSize, -targetSize, 0.0),
      G4ThreeVector(0.0, 0.0, +targetSize), ABSOLUTE);
    ...
    G4LogicalVolume* testTess_log = new G4LogicalVolume(testTess, Ar, "TestTess-log");
    new G4PVPlacement(G4Translate3D(G4ThreeVector(1000. * cm, -300. * cm, 0.)), "TestTess-phys",
                      testTess_log, experimentalHall_phys, false, 0);

    // Boolean subtraction
    G4VSolid* testCut1 = new G4Box("TestCut", 100. * cm, 100. * cm, 100. * cm);
    G4VSolid* testTessCut =
      new G4SubtractionSolid("TestTessCut", testTess, testCut1, G4Translate3D(0., 0., 150. * cm));
    G4LogicalVolume* testTessCut_log = new G4LogicalVolume(testTessCut, Ar, "TestTessCut-log");
    new G4PVPlacement(G4Translate3D(G4ThreeVector(1300. * cm, -300. * cm, 0.)), "TestTessCut-phys",
                      testTessCut_log, experimentalHall_phys, false, 0);
  }
  ...
  if (volumeSelection["Simple_union"])
  {
    ...
    G4VSolid* cylinder = new G4Tubs("Cylinder",  // Name
                                    0. * cm, 20. * cm, 6.5 * cm, 0. * deg, 360. * deg);
    G4UnionSolid* volume1 = new G4UnionSolid("Target1_solid_and_Cylinder ", target1Box, cylinder);
    G4LogicalVolume* volume1Logical =
      new G4LogicalVolume(volume1, Ar, "Target1_solid_and_Cylinder", 0, 0, 0);
    new G4PVPlacement(0, G4ThreeVector(1100. * cm, 200. * cm, 0. * cm), volume1Logical,
                      "Target1_solid_and_Cylinder", experimentalHall_log, false, 0);
  }

  if (volumeSelection["G4Torus"])
  {
    G4Torus* torus = new G4Torus("Torus", 40. * cm, 60. * cm, 200. * cm, 0., 90. * deg);
    G4LogicalVolume* torusLog = new G4LogicalVolume(torus, Ar, "Torus_log");
    torusLog->SetVisAttributes(G4VisAttributes(G4Colour::Red()));
    new G4PVPlacement(G4Translate3D(G4ThreeVector(1000. * cm, 500. * cm, 0))
                        * G4RotateZ3D(90. * deg),
                      "Torus_phys", torusLog, experimentalHall_phys, false, 0);
  }
