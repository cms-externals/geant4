...
G4VPhysicalVolume* Tst202DetectorConstruction::Construct()
{
  ...
  if (volumeSelection["Sphere"]) {
    ...
    G4LogicalVolume* PD_log_sphere
      = new G4LogicalVolume(PD_vol_sphere,Ar,"Test_Sphere");

    G4VisAttributes * PD_att_sphere
      = new G4VisAttributes(G4Colour(1.,0.,1.));
...
//    G4PVPlacement(0,G4ThreeVector(600.*cm,400.*cm,0),"VanAllenBeltPV",logicVanAllenBelt,experimentalHall_phys,false,0);
    new G4PVPlacement(G4Transform3D(rm,G4ThreeVector(600.*cm,400.*cm,0)),
                      "VanAllenBelt", logicVanAllenBelt,
                      experimentalHall_phys,false,0);
  ...
  if (volumeSelection["Ellipsoid"]) {
    ...
    new G4PVPlacement(G4Translate3D(G4ThreeVector(-500.*cm,200.*cm,0)),
                      "ellipdsoid-phys", eTubeLog,
                      experimentalHall_phys,false,0);
  }
  ...
  if (volumeSelection["Simple_tessellated"]) {
    ...
    (G4Translate3D(G4ThreeVector(1000.*cm,-300.*cm,0.)),
     "TestTess-phys", testTess_log, experimentalHall_phys, false, 0 );
  ...
  if (volumeSelection["G4Torus"]) {
    ...
    new G4PVPlacement(G4Translate3D(G4ThreeVector(1000.*cm,500.*cm,0))*G4RotateZ3D(90.*deg),
                      "Torus_phys", torusLog,
                      experimentalHall_phys,false,0);
  }
