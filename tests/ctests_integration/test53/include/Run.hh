...
class Run : public G4Run
{
  public:
    ...
    void EndOfRun(std::vector<G4double> edepTrue, std::vector<G4double> rmsTrue,
                  std::vector<G4double> limitTrue);

    void fillPerEvent(G4int, G4double, G4double);

    void sumEnergyFlow(G4int plane, G4double Eflow) { fEnergyFlow[plane] += Eflow; };
    void sumLateralEleak(G4int cell, G4double Eflow) { fLateralEleak[cell] += Eflow; };
