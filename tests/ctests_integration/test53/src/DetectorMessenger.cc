...
void DetectorMessenger::SetNewValue(G4UIcommand* command,G4String newValue)
{
  if( command == SizeYZCmd )
   { fDetector->SetCalorSizeYZ(SizeYZCmd->GetNewDoubleValue(newValue));}

  if( command == NbLayersCmd )
   { fDetector->SetNbOfLayers(NbLayersCmd->GetNewIntValue(newValue));}

  if( command == NbAbsorCmd )
   { fDetector->SetNbOfAbsor(NbAbsorCmd->GetNewIntValue(newValue));}
