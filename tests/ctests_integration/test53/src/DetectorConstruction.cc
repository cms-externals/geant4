...
void DetectorConstruction::PrintCalorParameters() const
{
  G4cout << "\n-------------------------------------------------------------"
...
void DetectorConstruction::SetNbOfLayers(G4int ival)
{
  ...
  if (ival < 1)
  {
    G4cout << "\n --->warning from SetNbOfLayers: " << ival
...
void DetectorConstruction::SetNbOfAbsor(G4int ival)
{
  ...
  {
    G4cout << "\n ---> warning from SetNbOfAbsor: " << ival << " must be at least 1 and and most "
           << MaxAbsor - 1 << ". Command refused" << G4endl;
    return;
  }
  fNbOfAbsor = ival;
}
...
void DetectorConstruction::SetCalorSizeYZ(G4double val)
{
  ...
  if (val <= DBL_MIN)
  {
    G4cout << "\n --->warning from SetCalorSizeYZ: thickness " << val
           << " out of range. Command refused" << G4endl;
    return;
  }
  fCalorSizeYZ = val;
