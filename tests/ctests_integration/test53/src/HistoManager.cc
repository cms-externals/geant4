...
HistoManager::HistoManager(G4int nbAbsor, G4int maxAbsor)
  : fNbAbsor(nbAbsor),fMaxAbsor(maxAbsor)
{
...
{ 
  ...
  G4int iCmpt=1;
  for (G4int k=1; k<fMaxAbsor; k++) SetHisto( iCmpt++,  nbins, vmin, vmax, vunit_MeV);

  const G4String vunit_none="none";
  nbins=52;
  vmin=0.; vmax=52.;
  for (G4int k=fMaxAbsor; k<2*fMaxAbsor; k++) SetHisto( iCmpt++,  nbins, vmin, vmax, vunit_none);

  nbins=102;
  vmin=0.; vmax=102.;
  for (G4int k=2*fMaxAbsor; k<=fMaxHisto; k++) SetHisto( iCmpt++,  nbins, vmin, vmax, vunit_none);
...
{   
 ...
 // histo 22 : lateral energy leak (MeV)  
  ...
  if (ih < fMaxAbsor) {			 
    ...
    vmin = valmin/vunit; vmax = valmax/vunit;  
  } 
