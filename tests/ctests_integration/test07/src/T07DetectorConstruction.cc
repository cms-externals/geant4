...
void T07DetectorConstruction::DefineMaterials()
{
  ...
  G4Material* CO2 = new G4Material(name = "CarbonicGas", density, ncomponents = 2, kStateGas,
  ...
  G4Material* steam =
    new G4Material(name = "WaterSteam", density, ncomponents = 1, kStateGas, temperature, pressure);
...
G4VPhysicalVolume* T07DetectorConstruction::ConstructCalorimeter()
{
  ...
  if (GapThickness > 0.)
  {
    solidGap = new G4Box("Gap", GapThickness / 2, CalorSizeYZ / 2, CalorSizeYZ / 2);

    logicGap = new G4LogicalVolume(solidGap, GapMaterial, "Gap");
  ...
  if (logicGap) logicGap->SetSensitiveDetector(calorimeterSD);
