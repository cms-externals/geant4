...
T07CalorimeterSD::T07CalorimeterSD(G4String name, T07DetectorConstruction* det)
...
{
  CalCollection = new T07CalorHitsCollection(SensitiveDetectorName, collectionName[0]);
...
G4bool T07CalorimeterSD::ProcessHits(G4Step* aStep, G4TouchableHistory*)
{
  ...
  if (HitID[LayerNumber] == -1)
  {
    ...
    if (verboseLevel > 0) G4cout << " New Calorimeter Hit on layer: " << LayerNumber << G4endl;
  }
  else
  {
    if (physVol == Detector->GetAbsorber())
      (*CalCollection)[HitID[LayerNumber]]->AddAbs(edep, stepl);
    if (physVol == Detector->GetGap()) (*CalCollection)[HitID[LayerNumber]]->AddGap(edep, stepl);
    if (verboseLevel > 0) G4cout << " Energy added to Layer: " << LayerNumber << G4endl;
  }
