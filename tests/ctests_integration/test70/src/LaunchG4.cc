...
LaunchG4::~LaunchG4()
{
  if (fpVisManager) delete fpVisManager;

  if (fpSession) delete fpSession;

  if (fpRunManager) delete fpRunManager;
}

void LaunchG4::Initialize()
{
   ...
   * not destroyed.
   */
  if (CommandLineParser::GetParser()->GetCommandIfActive("-nochem") == 0)
  {
...
{
  // Define UI fpSession for interactive mode.
  if (fpSession) fpSession->SessionStart();
}
...
void LaunchG4::BuildGUIFrame()
{
  if (fpSession && fpSession->IsGUI())
  {
    ...
    UI->ApplyCommand(
      "/gui/addButton viewer \"Update viewer (interaction or end-of-file)\" "
      "\"/vis/viewer/update\"");
    UI->ApplyCommand(
