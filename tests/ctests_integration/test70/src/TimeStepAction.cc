...
{
  G4ConfigurationIterator speciesIt = G4MoleculeTable::Instance()->GetConfigurationIterator();
  ...
  while (speciesIt())
  {
    ...
    if (!counter->IsReactantIgnored(speciesType->GetDefinition()))
    {
      ...
      if (!MoleculeIndex)
      {
        G4Exception("TimeStepAction::UserPostTimeStepAction", "INVALID_CAST", JustWarning,
                    "Failed to cast index to G4MoleculeCounterIndex.");
        return;
      }

      int number = counter->GetNbMoleculesAtTime(*MoleculeIndex, time);
      ...
      if (trackListSize != number)
      {
        ...
        errMsg << "There seems to be a problem related to G4MoleculeCounter" << G4endl
               << " Species = " << speciesType->GetName() << " n tracks = " << trackListSize
               << " numberFromCounter = " << number << G4endl;
        G4Exception("TimeStepAction::UserPostTimeStepAction", "CHECK_COUNTER_CONSISTENCY",
