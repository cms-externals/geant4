...
void TimeStepAction::UserPostTimeStepAction()
{
  ...
  while(speciesIt())
  {
    G4MolecularConfiguration* speciesType = speciesIt.value();

    if(G4MoleculeCounter::Instance()->IsRegistered(speciesType->GetDefinition())
        == true)
    {
    ...
    int number = G4MoleculeCounter::Instance()->GetNMoleculesAtTime(speciesType,
                                                                    time);
