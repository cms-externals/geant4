...
void SetSeed();
unsigned TokenizeString(const std::string& i_source, const std::string& i_seperators,
                        bool i_discard_empty_tokens, std::vector<std::string>& o_tokens);
...
int main(int argc, char* argv[])
{
  ...
  parser->AddCommand("-nochem", Command::WithoutOption, "Chemistry deactivated");
  ...
  if (g4session)
  {
    ...
    {
      g4->NewSession(argc, argv, commandLine->GetOption());
    }
    else
    {
