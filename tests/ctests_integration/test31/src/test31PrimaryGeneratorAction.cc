...
void test31PrimaryGeneratorAction::GeneratePrimaries(G4Event* anEvent)
{
  ...
  if (m_gauss == "flatE")
    kinEnergy = minE + (maxE - minE) * G4UniformRand();
  ...
  else if (m_gauss == "flatBeta")
  {
    ...
    kinEnergy += G4RandGauss::shoot(0.0, sigmaE);
  ...
  if (verbose > 0)
  {
    G4cout << "Event#  " << counter
           << "  Beam particle is generated by test31PrimaryGeneratorAction " << G4endl;
    G4cout << "ParticleName= " << particle->GetParticleName()
           << "  PDGcode= " << particle->GetPDGEncoding() << std::setprecision(5)
           << "   KinEnergy(GeV)= " << energy / GeV << "   x(mm)= " << x / mm
           << " y(mm)= " << y / mm << " z(mm)= " << z / mm << "   ux= " << ux << " uy= " << uy
           << " uz= " << uz << G4endl;
  }
...
void test31PrimaryGeneratorAction::SetBeamBeta(G4double val)
{
  ...
  G4cout << "test31PrimaryGeneratorAction: KinEnergy(MeV)= " << energy / MeV << G4endl;
  minE = energy;
  maxE = energy;
  minBeta = val;
  maxBeta = val;
