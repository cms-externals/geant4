...
test31DetectorMessenger::test31DetectorMessenger(test31DetectorConstruction* h) : hDet(h)
{
...
void test31DetectorMessenger::SetNewValue(G4UIcommand* command, G4String newValue)
{
  ...
  {
    (test31Histo::GetPointer())->SetMaxEnergy(DeltaECmd->GetNewDoubleValue(newValue));
  }
}
