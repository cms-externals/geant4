...
test31TrackingAction::test31TrackingAction() : theHisto(test31Histo::GetPointer()) {}
    ...
    */
    if (1 < theHisto->GetVerbose())
    {
      G4cout << "test31TrackingAction: Primary kinE(MeV)= " << kinE / MeV
             << "; m(MeV)= " << mass / MeV << "; pos= " << pos << ";  dir= " << dir << G4endl;
    }
...
void test31TrackingAction::PostUserTrackingAction(const G4Track* aTrack)
{
  ...
  {
    const G4VProcess* process = aTrack->GetStep()->GetPostStepPoint()->GetProcessDefinedStep();
