...
void test31SD::Initialize(G4HCofThisEvent*)
{
  ...
  if (0 < theHisto->GetVerbose())
    G4cout << "test31SD: Begin Of Event # " << evno << "  numAbs= " << numAbs << G4endl;

  energy.resize(numAbs);
  for (G4int i = 0; i < numAbs; i++)
  {
    energy[i] = 0.0;
  }
  backEnergy = 0.0;
...
G4bool test31SD::ProcessHits(G4Step* aStep, G4TouchableHistory*)
{
  ...
  if (j < 0 || j >= numAbs)
  {
    G4cout << "Warning!!! test31SD: cannot add " << edep / MeV << " MeV to the slice # " << j
           << G4endl;
  }
  else
  {
  ...
  {
    G4cout << "test31SD: energy = " << edep / MeV << " MeV is deposited at " << j
           << "-th absorber slice "
           << " TrackID= " << trIDnow << G4endl;
  }
  ...
  if (track->GetNextVolume() != track->GetVolume() && (zend <= delta || zend >= zmax))
  {
    outAbs = true;
  }
  ...
  if (outAbs && part_is_out)
  {
    ...
    if (track->GetDefinition() == G4Positron::Positron()) e += 2. * electron_mass_c2;
...
void test31SD::EndOfEvent(G4HCofThisEvent*)
{
  ...
  */
  ...
  if (0 < theHisto->GetVerbose())
  {
    ...
    G4cout << "Edep(MeV)= " << etot / MeV << "; backE(MeV)= " << backEnergy / MeV
           << "; leakE(MeV)= " << leakEnergy / MeV << G4endl;
  }
}
