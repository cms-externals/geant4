...
class G4plfTest {
public:
  G4plfTest(G4String plname, bool isEnv, int verbose,
            int factoryVerbose, int expectation);
  ...
  void UpdateTypeInfo(G4VModularPhysicsList* physics,
                      G4String& mangled, G4String& demangled);
...
G4plfTest::G4plfTest(G4String plname, bool isEnv, int verbose,
                     int factoryVerbose, int expectation)
  : _plname(plname), _isEnv(isEnv), _verbose(verbose),
    _factoryVerbose(factoryVerbose), _expectation(expectation),
    _success(0), _physicsNew(0), _physicsOld(0) { ; }
int G4plfTest::RunTest(bool doOld, bool doNew)
{
  ...
  if ( ! _isEnv ) {
...
#ifndef _WIN32
    const char* prevEnvVal = getenv("PHYSLIST");
    if ( _plname == "" )     unsetenv("PHYSLIST"); // force UNsetting ...
    else                     setenv("PHYSLIST",_plname,1); // force user value
#else
    ...
    const char* prevEnvVal = getenv("PHYSLIST");
    if ( _plname == "" )     _putenv_s("PHYSLIST",""); // force UNsetting ...
    else                     _putenv_s("PHYSLIST",_plname); // force user value
#endif
    if ( doNew ) {
  ...
  if ( _verbose || _failure ) {
    ...
    if ( doNew ) G4cout << "NEW Requested \"" << _plname << "\" got "
                        << _mangledNew << " (" << _demangledNew << ")" << G4endl;
    if ( doOld ) G4cout << "OLD Requested \"" << _plname << "\" got "
                        << _mangledOld << " (" << _demangledOld << ")" << G4endl;
    G4cout << "   _success " << _success << "   _expectation " << _expectation
           << "   _failure " << _failure << G4endl;
    if ( _failure ) {
#ifndef _WIN32
      char esc = '\033'; // 0x1B;
      ...
      G4cout << makeCyan << makeRedBack << G4endl
             << "                                      " << G4endl
             << "   unexpected result:  failure code " << _failure << "  "<< G4endl
             << "                                      " << G4endl
             << makeNormal << G4endl;
    }
    G4cout << "--Results end\n" << G4endl;
...
G4String G4plfTest::StateAsString() const
{
  ...
  if ( _isEnv ) {
    if ( _plname != "" ) req = "$PHYLIST=" + _plname;
    else                 req = "$PHYLIST=<<unset>>";
  }
...
void G4plfTest::UpdateTypeInfo(G4VModularPhysicsList* physics,
                               G4String& mangled, G4String& demangled)
{
  if ( ! physics ) {
    mangled   = _snullptr;
...
int main(int argc,char** argv)
{
  ...
  if ( gPrintFactoryOld > 0 ) {
    ...
    G4cout << sepline << gPrgname << ": state of the _old_ factory:-" << G4endl
           << sepline << G4endl;
  ...
  if ( gPrintCtorList > 1 || gPrintPLRegList > 1 ) {
    ...
    G4cout << sepline << gPrgname
           << ": is pre-linked against g4plft1 library, "
           << "  list what is available before loading g4plft2 library" << G4endl
           << sepline << G4endl;
#endif
    ...
    G4cout << gPrgname
           << ": (before) phys ctors " << npc1 << ", phys lists " << npl1base
...
#else
  G4cout << "skip attempt to load g4plftest38_lib[1|2]"
	 << G4endl;
#endif

  G4cout << gPrgname
         << ": now add extensions mappings ALTDK, NEWPHY and XYZZY "
         << G4endl;
  ...
  G4cout << gPrgname
         << ": (after)  phys ctors " << npc2 << ", phys lists " << npl2base
         << ", ext " << npl2ext << ", em " << npl2em << G4endl;
  G4cout << G4endl;
  ...
  if ( gPrintFactory > 0 ) {
    G4cout << sepline << gPrgname << ": state of the new factory:-" << G4endl
           << sepline << G4endl;
...
void PrintUsage() {
  G4cout << gPrgname
         << ":  G4PLFactoryTest - "
         << "a simplified Geant4 app for testing "
         << G4endl
         << "the G4PhysListFactory"
         << G4endl;
  G4cout << "   " << gPrgname << " [options] [physList1 physList2[=N]]"
            << G4endl;
  ...
  G4cout << "  -e --env=PNAME  PhysicsList to use as env variable ["
            << gPlenv << "]" << G4endl;
  ...
  G4cout
    ...
    << "    new (" << STATUS_NEW << "), old (" << STATUS_OLD << "), both (" << STATUS_BOTH << ") or neither (" << STATUS_NONE << ") factory;\n"
               ...
               std::vector<G4plfTest>& testlist)
{
  ...
  int c;
  while ( true ) {
    c = getopt_long ( argc, argv, "hvoe:DV:fFcrlxE",
    ...
    case 0: {
      ...
      else if ( oname == "env" ) gPlenv = optarg;
      break;
    }
    case 'h':
      PrintUsage();
               ...
               std::vector<G4plfTest>& testlist)
{
  ...
  if ( (cmdLong  = parser->GetCommandIfActive("--verbose")) ||
    ...
    if ( cmdLong ) {
      if ( cmdLong->GetOption() == "" ) gVerbosity++;
      else { gVerbosity += atoi(cmdLong->GetOption().c_str()); }
    }
    if ( cmdShort ) {
      if ( cmdShort->GetOption() == "" ) gVerbosity++;
      else { gVerbosity += atoi(cmdShort->GetOption().c_str()); }
    }
  }
  ...
  if ( (cmdLong  = parser->GetCommandIfActive("--Verbose")) ||
    ...
    gFactoryVerbosity = 0;
    if ( cmdLong ) {
      if ( cmdLong->GetOption() == "" ) gFactoryVerbosity++;
      else { gFactoryVerbosity = atoi(cmdLong->GetOption().c_str()); }
    }
    if ( cmdShort ) {
      if ( cmdShort->GetOption() == "" ) gFactoryVerbosity++;
      else { gFactoryVerbosity = atoi(cmdShort->GetOption().c_str()); }
    }
  ...
  if ( (cmdLong  = parser->GetCommandIfActive("--reglist")) ||
    ...
    if ( cmdLong ) {
      if ( cmdLong->GetOption() == "" ) gPrintPLRegList++;
      else { gPrintPLRegList += atoi(cmdLong->GetOption().c_str()); }
    }
    if ( cmdShort ) {
      if ( cmdShort->GetOption() == "" ) gPrintPLRegList++;
      else { gPrintPLRegList += atoi(cmdShort->GetOption().c_str()); }
    }
  }
...
void AddDefaults(std::vector<G4plfTest>& testlist)
{
  ...
  testlist.push_back(G4plfTest("G4GenericPhysicsList",false,v1,v2,STATUS_NEW )); //! NOTE old doesn't know
...
bool TryManySharedLibraries(const G4String& libbase)
{
  ...
  for (int ipre=0; ipre < npre; ++ipre) {
    for (int isuf=0; isuf < nsuf; ++isuf) {
      ...
      if ( success ) {
        G4cout << gPrgname << ": dynamically loaded '" << libname << "'"
               << G4endl;
        return true;
      } else {
  ...
  G4cout << gPrgname << ": tried to dynamically load '" << libbase << "'"
         << " but found no SUFFIX/PREFIX combination that worked"
         << G4endl << G4endl
         << errorMsgs.str()
         << G4endl;
...
#else
  // Windows ...
  G4cout << gPrgname << ": WIN32 dynamic library loading try '"
         << libname << "'" << G4endl;
                                        ...
                                        bool gVerbose)
{
  ...
  if (gVerbose)
    G4cout << "values " << values
              << " separated by \"" << sepchar << "\"" << G4endl;
  ...
  while ( pos_beg != std::string::npos && pos_beg < str_end ) {
    ...
    if (gVerbose)
      G4cout << " onevalue \"" << onevalue  << "\" in ["
                << pos_beg << "," << pos_end << ")"<< G4endl;
    pos_beg = pos_end+1;
