...
TestNA49Histo::TestNA49Histo(G4String htitle) : TstHistoSet(htitle)
{
  std::string title;
  std::string outcome;
  std::string hname;
  ...
  fHistoSecProton.push_back(
    new TProfile("proton_pT", title.c_str(), fNPBinsXF - 1, fPBinsXF, 0., 10.));
  fHistoSecProton.push_back(
    new TProfile("proton_pT2", title.c_str(), fNPBinsXF - 1, fPBinsXF, 0., 10.));
                                              ...
                                              // while all other are N+1 !!!
  {
    ...
    fHistoPTProton.push_back(
      new TH1D(hname.c_str(), (title + subtitle).c_str(), fNPBinsPT - 1, fPBinsPT));
  }
  ...
  for (int nb = 0; nb < fNPbarBinsXF; ++nb)
  {
    ...
    fHistoPTAntiProton.push_back(
      new TH1D(hname.c_str(), (title + subtitle).c_str(), fNPbarBinsPT[nb], fPbarBinsPT[nb]));
  }
  ...
  */
  ...
  //  fHistoSecPiMinus.push_back( new TProfile( "piminus_pT2",  title.c_str(), npibins, pibins,
  //  0., 10. ) );
  fHistoSecPiMinus.push_back(
    new TProfile("piminus_pT2", title.c_str(), fNPiBinsXF - 1, fPiBinsXF, 0., 10.));
  ...
  for (int nb = 0; nb < fNPiBinsXF - 1; ++nb)
  {
    ...
    //     fHistoPTPiMinus.push_back( new TH1D( hname.c_str(), (title+subtitle).c_str(), npibins_pt,
    //     pibins_pt) );
    fHistoPTPiMinus.push_back(
      new TH1D(hname.c_str(), (title + subtitle).c_str(), fNPiBinsPT - 1, fPiBinsPT));
  }
  ...
  fHistoSecPiPlus.push_back(
    new TProfile("piplus_pT2", title.c_str(), fNPiBinsXF - 1, fPiBinsXF, 0., 10.));
  ...
  for (int nb = 0; nb < fNPiBinsXF - 1; ++nb)
  {
    ...
    //     fHistoPTPiPlus.push_back( new TH1D( hname.c_str(), (title+subtitle).c_str(), npibins_pt,
    //     pibins_pt ) );
...
void TestNA49Histo::FillEvt(G4VParticleChange* aChange, const G4LorentzVector&,
  ...
  for (G4int i = 0; i < NSec; i++)
  {
    ...
    /*
      if ( xF < -1. || xF > 1. )
      {
         ...
         std::cout << " mom: " << mom.x() << " " << mom.y() << " " << mom.z() << " ekin+mass = " <<
      ekin+mass << std::endl;
      }
    */
    double pT = mom.perp();
    ...
    else if (pname == "proton")
    {
      ...
      int nb = -1;
      for (int ib = 0; ib < fNPBinsXF;
           ++ib)  // # number of proton bins are N and the size is N+1, while for pions # is N+1
...
void TestNA49Histo::Write(G4int stat, G4double)
{
  ...
  for (size_t i = 0; i < fHistoPTProton.size(); ++i)
  {
    fHistoPTProton[i]->Scale(
      1. / ((double)stat));  // Note: NO scaling with "width" because it had to be E/dP3
  ...
  for (size_t i = 0; i < fHistoPTPiPlus.size(); ++i)
  {
    fHistoPTPiPlus[i]->Scale(
      1. / ((double)stat));  // NO scaling with "width" - see earlier comment for fHistoPTPiMinus
...
double TestNA49Histo::CalculateBinWeight(const G4LorentzVector& labp, const G4String& pname,
                                         double Ekin, double mass, double pT, int xFbin,
                                         double sqrt_s)
{
  double wei = 1.;
  int pTbin = -1;
  ...
  {
    dpT2 = fPbarBinsPT[xFbin][pTbin + 1] * fPbarBinsPT[xFbin][pTbin + 1]
           - fPbarBinsPT[xFbin][pTbin] * fPbarBinsPT[xFbin][pTbin];
