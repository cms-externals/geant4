...
// Retain this ordering to avoid shadowing warnings between CLHEP/ROOT.
// TODO: Fix!
// clang-format off
using namespace std;
...
int main(int argc, char** argv)
{
  ...
  do
  {
      ...
      */
      ...
      for (G4int i = 0; i < nsec; ++i)
      {
        /*
              ...
              std::cout << " tmpsec type = " << tmpsec->GetDefinition()->GetParticleName() <<
           std::endl; std::cout << " ekin = " << tmpsec->GetKineticEnergy() << std::endl; std::cout
           << " parent = " << tmpsec->GetParentID() << std::endl;
        */
    ...
    G4ParticleDefinition* partDef =
      G4ParticleTable::GetParticleTable()->FindParticle(exec->GetBeam()->GetBeamPartName());
    ...
    const auto time =
      ...
      / 1e6;
    G4cout << G4endl;
    G4cout << "Processed " << NEvts << " events (collisions) in " << std::scientific << time
           << " seconds."
           << " Average: " << std::defaultfloat << (time * 1E3 / NEvts) << " ms / event." << G4endl;
    G4cout << G4endl;
