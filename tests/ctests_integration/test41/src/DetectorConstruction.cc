...
DetectorConstruction::DetectorConstruction(RunAction* r, PrimaryGeneratorAction* p) : run(r), gen(p)
{
  physWorld = 0;
  physAbs = 0;
...
G4VPhysicalVolume* DetectorConstruction::Construct()
{
  ...
  physAbs =
    new G4PVPlacement(0, G4ThreeVector(0., 0., 0.5 * width), logicAbs, "Abs", logicWorld, false, 0);

  G4cout << "### Absorber width(mm)= " << width / mm << " made of " << matAbsorber->GetName()
         << G4endl;
