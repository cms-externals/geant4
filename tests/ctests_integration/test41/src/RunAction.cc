...
void RunAction::EndOfRunAction(const G4Run* aRun)
{
  if (nevt == 0)
  {
    G4cout << "### Run " << aRun->GetRunID() << " ended with zero events" << G4endl;
    return;
  }
  G4double x[11];
  G4double dx[11];
  ...
  if (!fout.is_open())
  {
    G4cout << "### Error in open of file " << fname << G4endl;
    return;
  }
  G4cout << "###  " << nevt << "  events of mu+  with p(MeV/c)= " << ptot / MeV << " scattered off "
         << width / mm << " mm of " << matname << G4endl;
