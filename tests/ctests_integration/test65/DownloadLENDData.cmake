...
# fails for any reason, a FATAL_ERROR is emitted, and is duplicated 
# to stdout to give a clear message.
#
# After successful download, the tarball is unpacked and its root
# directory relocated to <CWD>/Data/g4lend.
#
#
set(LENDDATA_FILENAME "LEND_GNDS2.0_ENDF.BVII.1.tar.gz")
set(LENDDATA_TAR_ROOTDIR "LEND_GNDS2.0_ENDF.BVII.1")
set(LENDDATA_HASH_MD5 "95ac8c938262e8193f3f7a4194269877")
...
set(LLNL_URL "ftp://gdo-nuclear.ucllnl.org/LEND_GNDS2.0/${LENDDATA_FILENAME}")
...
if(LENDDATA_NEEDS_DOWNLOAD)
  set(DownloadReturnStatus 0)
  set(DownloadStringReturnStatus)

  if(ALTERNATE_URL)  
    message(STATUS "test65-data: No existing file, attempting download: ${ALTERNATE_URL}")
    ...
    list(GET DownloadStatus 1 DownloadStringReturnStatus)
  endif()

  if(DownloadReturnStatus)
    if(ALTERNATE_URL)
      message(STATUS "test65-data: Download from ${ALTERNATE_URL} failed (${DownloadStringReturnStatus}), trying ${LLNL_URL}")
    else()
      message(STATUS "test65-data: No existing file, attempting default download from: ${LLNL_URL}")
    endif()
  ...
  list(GET DownloadStatus 1 DownloadStringReturnStatus)
  if(DownloadReturnStatus)
    # Duplicate error message to stdout for clarity
    message(STATUS      "test65-data: download FAILED: ${DownloadReturnStatus}, ${DownloadStringReturnStatus}")
  ...
  if(NOT __lenddata_existing_md5 STREQUAL ${LENDDATA_HASH_MD5})
    message(STATUS      "MD5 hash error: md5sum of download file = ${__lenddata_existing_md5}, expected ${LENDDATA_HASH_MD5}")
...
if(__lenddata_untar_result)
  message(STATUS      "test65-data: failed to untar file : ${LENDDATA_LOCAL_FILENAME}")
...
if(__lenddata_rename_result)
  message(STATUS      "test65-data: failed to rename : ${LENDDATA_WORKING_DIR}/${LENDDATA_TAR_ROOTDIR} -> ${LENDDATA_LOCAL_ROOTDIR}")
