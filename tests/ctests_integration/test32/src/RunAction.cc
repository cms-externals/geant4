...
RunAction::RunAction(EventAction* eventAction) : G4UserRunAction(), fEventAction(eventAction)
{
  ...
  for (G4int i = 0; i < NofH2; ++i)
  {
    G4String name = "PX-PY-";
    name += std::to_string(i);
    analysisManager->CreateH2(name, "PX PY 0 in absorber layer #2", 10, -10. * MeV, 10 * MeV, 10,
                              -10. * MeV, 10 * MeV);
  }
  ...
  for (G4int i = 0; i < NofH3; ++i)
  {
    ...
    name += std::to_string(i);
    analysisManager->CreateH3(name, "PX PY PZ in absorber layer #2", 10, -10. * MeV, 10 * MeV, 10,
                              -10. * MeV, 10 * MeV, 10, -10. * MeV, 10 * MeV);
  }
...
{
  if ((!MultipleWrite) || (MultipleWrite && run->GetRunID() == 0))
  {
...
void RunAction::EndOfRunAction(const G4Run* run)
{
  ...
  if ((!MultipleWrite) || (MultipleWrite && run->GetRunID() == 1))
  {
