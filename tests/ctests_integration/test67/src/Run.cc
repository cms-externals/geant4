...
Run::Run() : counter(0), counterTot(0), fPrimaryEnergy(-1), hcID(-1)
{
  // Retrieve primary energy, if available
  const PrimaryGeneratorAction* primGen = dynamic_cast<const PrimaryGeneratorAction*>(
...
void Run::RecordEvent(const G4Event* evt)
{
  ...
  if (HCE) CHC = (HitsCollection*)(HCE->GetHC(hcID));
