...
//....oooOO0OOooo........oooOO0OOooo........oooOO0OOooo........oooOO0OOooo....
ROOTAnalysis::ROOTAnalysis() : fFile(0), fHistograms(0), fGraph(0), fGraph2(0), fTree(0)
{
  fPhysicsListName = new TObjString("undefined");
  fVersionName = new TObjString(G4Version);
  fPE = 0;
  fIon = 0;
  fComp = 0;
  fEfficiency = 0;
...
ROOTAnalysis::~ROOTAnalysis()
{
  ...
  if (fGraph2) delete fGraph2;
  if (fVersionName) delete fVersionName;
  if (fPhysicsListName) delete fPhysicsListName;
...
void ROOTAnalysis::BookNewHistogram(G4int runID, G4double primaryEnergy)
{
  ...
  if (!fFile)
  {
    ...
    if (filename[0] == 'N')
    {
      ...
      filename = vs.substr(5, vs.size() - 5).c_str();
    }
    filename += "_";
  ...
  if (!fHistograms) fHistograms = new std::map<G4int, TH1D*>;
  if (!fGraph) fGraph = new TGraphErrors();
  if (!fGraph2) fGraph2 = new TGraphErrors();
  if (!fTree)
  {
    ...
    fTree->Branch("NSecondaries", &fSecondaries, "Secondaries/D");
    fTree->Branch("NStepsPerTrack", &fTotalSteps, "StepsPerTrack/D");
  }
  if (!(fHistograms->count(runID)))
  {
    ...
    TH1D* histo = new TH1D(id, title, nbins, 0, (primaryEnergy + 10 * keV) / keV);
...
void ROOTAnalysis::AddEventEnergy(G4int runID, G4double val)
{
  ...
  if (!(fHistograms->count(runID)))
  {
    ...
    G4Exception("ROOTAnalysis::AddEventEnergy", "tst67_01", FatalException, ed);
    return;
  }
...
void ROOTAnalysis::AddGammaSecondary(G4String name)
{
  G4AutoLock l(&dataManipulationMutex);
  if (name == "compt")
    fComp += 1.0;
...
void ROOTAnalysis::OneMoreSecondary()
{
  G4AutoLock l(&dataManipulationMutex);
  fSecondaries += 1.0;
}
...
void ROOTAnalysis::CountSteps(G4int st)
{
  G4AutoLock l(&dataManipulationMutex);
  fTotalSteps += (G4double)st;
  // G4cout << "Total steps: " << fTotalSteps << G4endl;
}
//....oooOO0OOooo........oooOO0OOooo........oooOO0OOooo........oooOO0OOooo....
...
void ROOTAnalysis::CloseFile()
{
  ...
  if (fVersionName) fVersionName->Write("Version");
  if (fPhysicsListName) fPhysicsListName->Write("List");
  if (fGraph) fGraph->Write("PeakEff");
  if (fGraph2) fGraph2->Write("FullEff");
...
void ROOTAnalysis::SetListName(G4String name)
{
  ...
  fPhysicsListName->SetString(name);
  return;
}

//....oooOO0OOooo........oooOO0OOooo........oooOO0OOooo........oooOO0OOooo....

void ROOTAnalysis::EndOfRun(G4int runID, G4double energy, G4int nEventsTotal, G4int nEventsGood,
                            G4int nEventsFull)
{
  G4AutoLock l(&dataManipulationMutex);
  if (!fGraph || !fGraph2 || !nEventsTotal)
  {
    G4Exception("ROOTAnalysis::EndOfRun()", "tst67_04", FatalException, "Unable to create TGraphs");
    return;
  }
  ...
  // Steps per secondary
  fTotalSteps /= fSecondaries;

  fSecondaries /= nEventsTotal;
  ...
  for (G4int i = 0; i < fTree->GetEntries(); i++)
  {
    ...
    if (TMath::Abs(energy - energyRef) < 10 * eV)
    {
      ...
      if (TMath::Abs(effRef - eff) > 6.0 * effErr)
      {
        ...
        G4cerr << "Reference: " << effRef / perCent << " %, Data: (" << eff / perCent << " +/- "
               << effErr / perCent << ")" << G4endl;
        failCode = 1;
      }
      ...
      if (TMath::Abs(effFullRef - effFull) > 6.0 * effFullErr)
      {
        G4cerr << "ROOTAnalysis::CheckPhysicalResults(). Fails for total efficiency at "
               << energy / keV << " keV" << G4endl;
        G4cerr << "Reference: " << effFullRef / perCent << " %, Data: (" << effFull / perCent
               << " +/- " << effFullErr / perCent << ")" << G4endl;
        failCode = 1;
      }
      ...
      if (!CompRef)
      {
        if (Comp < CompRef / factor || Comp > factor * CompRef)
        {
          G4cerr << "ROOTAnalysis::CheckPhysicalResults(). Fails for Compton fluo yield at "
                 << energy / keV << " keV" << G4endl;
          G4cerr << "Reference: " << CompRef << "Data: " << Comp << G4endl;
          failCode = 1;
        }
      }
      ...
      if (!IoniRef)
      {
        if (Ioni < IoniRef / factor || Ioni > factor * IoniRef)
        {
          G4cerr << "ROOTAnalysis::CheckPhysicalResults(). Fails for ionisation fluo yield at "
                 << energy / keV << " keV" << G4endl;
          G4cerr << "Reference: " << IoniRef << "Data: " << Ioni << G4endl;
          failCode = 1;
        }
      }
      ...
      if (!PERef)
      {
        if (PE < PERef / factor || PE > factor * PERef)
        {
          G4cerr << "ROOTAnalysis::CheckPhysicalResults(). Fails for photoelectric fluo yield at "
                 << energy / keV << " keV" << G4endl;
          G4cerr << "Reference: " << PERef << "Data: " << PE << G4endl;
          failCode = 1;
        }
      }
    }
