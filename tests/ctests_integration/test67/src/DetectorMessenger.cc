...
DetectorMessenger::DetectorMessenger(DetectorConstruction* detC) : theDetector(detC)
{
...
{
  if (command == geometryCmd) theDetector->SetGeometryType(geometryCmd->GetNewIntValue(newValue));
}
