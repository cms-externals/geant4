...
void RunAction::BeginOfRunAction(const G4Run* aRun)
{
  ...
  {
    ROOTAnalysis::getInstance()->BookNewHistogram(theRun->GetRunID(), theRun->GetPrimaryEnergy());
  }
#endif
  ...
  if (!outFile)
  {
    ...
    const PhysicsList* physicsList =
      dynamic_cast<const PhysicsList*>(G4RunManager::GetRunManager()->GetUserPhysicsList());
...
void RunAction::EndOfRunAction(const G4Run* aRun)
{
  ...
  if (nEvents)
  {
    ...
    {
      G4Exception("RunAction::EndOfRunAction()", "tst67_05", FatalException,
                  "Unable to open output file");
      return;
    }

#ifdef G4_USE_ROOT
    ROOTAnalysis::getInstance()->EndOfRun(theRun->GetRunID(), theRun->GetPrimaryEnergy(),
                                          (G4int)nEvents, theRun->GetCounter(),
