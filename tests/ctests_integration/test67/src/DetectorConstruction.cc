...
DetectorConstruction::DetectorConstruction() : fGeSensLog(0)
{
  // Initialization
...
G4VPhysicalVolume* DetectorConstruction::Construct()
{
  ...
  G4LogicalVolume* world_log =
    new G4LogicalVolume(world_box, G4Material::GetMaterial("Galactic"), "world_log", 0, 0, 0);
  G4VPhysicalVolume* world_phys =
    new G4PVPlacement(0, G4ThreeVector(), "world", world_log, 0, false, 0);
  ...
  G4Tubs* alCasingTube =
    new G4Tubs("alCasingTube", 0, casingDiameter / 2., casingHeight / 2., 0, twopi);
  G4LogicalVolume* alCasingLog =
    new G4LogicalVolume(alCasingTube, G4Material::GetMaterial("Aluminum"), "alCasingLog", 0, 0, 0);
  alCasingLog->SetVisAttributes(new G4VisAttributes(grey));
  new G4PVPlacement(0, G4ThreeVector(0, 0, 0), alCasingLog, "AlCasing", world_log, false, 0);
  ...
  G4Tubs* vacCasingTube =
    new G4Tubs("vacCasingTube", 0, vacDiameter / 2., vacHeight / 2., 0, twopi);
  G4LogicalVolume* vacCasingLog = new G4LogicalVolume(
    vacCasingTube, G4Material::GetMaterial("Galactic"), "vacCasingLog", 0, 0, 0);
  new G4PVPlacement(0, G4ThreeVector(0, 0, 0), vacCasingLog, "CasingVacuum", alCasingLog, false, 0);
  ...
  G4LogicalVolume* geLog =
    new G4LogicalVolume(geTube, G4Material::GetMaterial("Germanium"), "geLog", 0, 0, 0);
  new G4PVPlacement(0, G4ThreeVector(0, 0, 0), geLog, "DeadLayer", vacCasingLog, false, 0);
  ...
  G4Tubs* geSensTube = new G4Tubs("geSensTube", 0, sensDiameter / 2., sensHeight / 2., 0, twopi);
  fGeSensLog =
    new G4LogicalVolume(geSensTube, G4Material::GetMaterial("Germanium"), "geSensLog", 0, 0, 0);
  G4double tolerance = 0.1 * micrometer;
  G4double zshift = -0.5 * DLThickness + tolerance;
  new G4PVPlacement(0, G4ThreeVector(0, 0, zshift), fGeSensLog, "SensitiveGe", geLog, false, 0);
  ...
  G4Tubs* holeTubs = new G4Tubs("holeTube", 0, holeDiameter / 2., holeHeight / 2., 0, twopi);
  G4LogicalVolume* holeLog =
    new G4LogicalVolume(holeTubs, G4Material::GetMaterial("Galactic"), "holeLog", 0, 0, 0);
  zshift = -sensHeight / 2. + holeHeight / 2. + tolerance;
  new G4PVPlacement(0, G4ThreeVector(0, 0, zshift), holeLog, "InnerBoreHole", fGeSensLog, false, 0);
  ...
  if (geometryType == 3)
  {
    ...
    G4Tubs* waterTube = new G4Tubs("waterTube", 0, waterDiameter / 2., waterHeight / 2., 0, twopi);
    G4LogicalVolume* waterLog =
      new G4LogicalVolume(waterTube, G4Material::GetMaterial("HeavyWater"), "waterLog", 0, 0, 0);
    waterLog->SetVisAttributes(new G4VisAttributes(blue));
    new G4PVPlacement(0, G4ThreeVector(0, 0, waterZPosition), waterLog, "WaterSource", world_log,
                      false, 0);
  }
  else
...
void DetectorConstruction::ConstructSDandField()
{
  ...
  G4SDManager::GetSDMpointer()->AddNewDetector(pointerscint);
...
void DetectorConstruction::SetGeometryType(G4int gt)
{
  if (gt < 2 || gt > 3)
  {
    G4cout << "Valid geometry types are only 2 or 3 (geoetry #2 and geometry#3, respectively)"
           << G4endl;
