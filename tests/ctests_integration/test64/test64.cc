...
int main(int argc, char** argv)
{
  if (argc != 2)
  {
    G4cout << "Error: wrong number of arguments!" << G4endl << "Usage: mymain filename" << G4endl
           << G4endl;
    return 1;
  }
  ...
  while (getline(inputFile, inputLine))
  {
    ...
    G4cout << " ==========  Required RUN  ============" << G4endl << " Model = " << modelName
           << G4endl << " Particle = " << particleName << G4endl << " Target = " << materialName
           << G4endl << " Nevents = " << numberOfEvents << G4endl;
    ...
    // initialisation of process and models
    processNew->PreparePhysicsTable(*part);
    processNew->BuildPhysicsTable(*part);
    ...
    if (part->GetParticleName() == "pi+" || part->GetParticleName() == "pi-"
        || part->GetParticleName() == "pi0" || part->GetParticleName() == "kaon+"
        || part->GetParticleName() == "kaon-" || part->GetParticleName() == "kaon0S"
    ...
    else if (part->GetParticleName() == "sigma-" || part->GetParticleName() == "xi-"
    ...
    for (G4int loop = 0; loop < numberOfEvents; loop++)
    {  // Event loop
      ...
      {  // Produced secondary loop
        ...
        if (secName == "gamma" || secName == "pi+" || secName == "pi-" || secName == "pi0"
        ...
        else if (secName == "neutron" || secName == "proton" || secName == "deuteron"
                 || secName == "triton" || secName == "alpha" || secName == "He3")
        {
        ...
        else if (secName == "anti_proton" || secName == "anti_neutron" || secName == "anti_lambda"
                 || secName == "anti_sigma0" || secName == "anti_sigma+" || secName == "anti_sigma-"
                 || secName == "anti_xi-" || secName == "anti_xi0" || secName == "anti_omega-")
        {
          sumEnergySecondaries += secTE + 0.939;
        }
        ...
        else if (secName == "mu-" || secName == "mu+" || secName == "tau-" || secName == "tau+"
                 || secName == "nu_e" || secName == "anti_nu_e" || secName == "nu_mu"
                 || secName == "anti_nu_mu" || secName == "nu_tau" || secName == "anti_nu_tau")
        {
          sumEnergySecondaries += secTE;
        }
        else
    ...
    G4cout << "\t -------------  RUN SUMMARY  -------------" << G4endl
           ...
           << "\t initialTotalEnergy           = " << initialTotalEnergy << " GeV" << G4endl
           << "\t average sumEnergySecondaries = "
           << cumulatedSumEnergySecondaries / (numberOfEvents * 1.0) << " GeV" << G4endl
           << "\t maxSumEnergySecondaries      = " << maxSumEnergySecondaries << " GeV" << G4endl
           << "\t minSumEnergySecondaries      = " << minSumEnergySecondaries << " GeV" << G4endl
           << "\t -----------------------------------------" << G4endl << G4endl;
