...
Test17EventAction::Test17EventAction(Test17RunAction* Test17RA)
  : runaction(Test17RA), evtNo(-1), good(false)
{
  verbose = 0;
}
...
void Test17EventAction::BeginOfEventAction(const G4Event* evt)
{
  ...
  if (verbose > 0) G4cout << "<<< Event  " << evtNo << " started." << G4endl;
...
void Test17EventAction::EndOfEventAction(const G4Event*)
{
  ...
  if (verbose > 0)
  {
    G4cout << " Ncharged= " << nCharged << "; Nneutral= " << nNeutral
           << "; Length(mm)= " << totLength / mm << "; Edep(MeV)= " << edep / MeV << G4endl;
  }
}
