...
Test17RunAction::Test17RunAction()
  : theProton(G4Proton::Proton()), theElectron(G4Electron::Electron())
{}
...
void Test17RunAction::EndOfRunAction(const G4Run*)
{
  ...
  if (sigl > 0.0)
    sigl = std::sqrt(sigl / nev);
  else
    sigl = 0.0;

  G4double sigx = xend2 - xend * xend;
  if (sigl > 0.0)
    sigx = std::sqrt(sigx / nev);
  else
    sigx = 0.0;
  ...
  G4cout << " end of Run TotNbofEvents = " << nEvents << " for " << name
         << " with Ekin = " << kinEnergy0 / MeV << " MeV" << " p= " << p << " MeV/c" << G4endl;
  G4cout << "    Track Length in absorber = " << length / micrometer << " +- " << sigl / micrometer
         << "  microns " << G4endl;
  G4cout << "    CSDA  Range  in absorber = " << xend / micrometer << " +- " << sigx / micrometer
         << "  microns " << G4endl;
  G4cout << G4endl;
  G4cout << "    Energy deposit in absorber = " << edepTot / MeV << "  MeV " << G4endl;
  G4cout << G4endl;

  if (icru)
  {
    G4cout << "### Comparison with the ICRU49 data: " << G4endl;
    G4cout << "    Track Length (G4 - ICRU49) = " << (length - protL) / micrometer << " +- "
           << sigl / micrometer << " microns " << G4endl;
    G4cout << "    CSDA  Range  (G4 - ICRU49) = " << (xend - protR) / micrometer << " +- "
           << sigx / micrometer << " microns " << G4endl;
    G4cout << G4endl;
  }
