...
Test17SteppingAction::Test17SteppingAction(Test17EventAction* EA, Test17RunAction* RA)
  : eventaction(EA), runaction(RA), IDnow(-2), IDold(-1), prim(false)
{}
...
void Test17SteppingAction::UserSteppingAction(const G4Step* aStep)
{
  ...
  if (IDnow != IDold)
  {
    ...
    if (trno == 1)
    {
      ...
      runaction->FillDef(aStep->GetTrack()->GetDynamicParticle()->GetDefinition());
    }
    ...
    else
    {
      ...
      else if ((aStep->GetTrack()->GetDynamicParticle()->GetDefinition()->GetParticleName())
               == "gamma")
      {
  ...
  if (prim)
  {
    ...
    if (0.0 == Tkin || 0.0 > xend || xend >= 100.0 * mm)
    {
      ...
      {
        G4cout << "EvtNo= " << evno << "  length= " << eventaction->TrackLength()
               << "  xend= " << xend << G4endl;
