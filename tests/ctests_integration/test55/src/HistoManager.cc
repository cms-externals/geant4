...
HistoManager::HistoManager(DetectorConstruction* det) : fDetector(det)
{
  fNbHisto = 0;
  fDefaultAct = true;
  fVerbose = false;
...
void HistoManager::Book()
{
  ...
  if (analysisManager->GetNofH1s() == 0)
  {
    ...
    {
      fHistoId[i] = analysisManager->CreateH1(fIds[i], fTitle[i], fNbins[i], fXmin[i], fXmax[i]);
...
void HistoManager::Add1D(G4int histoId, const G4String& name, G4int nb, G4double x1, G4double x2,
                         G4String u1, G4String u2)
{
  std::stringstream sg;
  sg << histoId;
  ...
  if (fVerbose)
  {
    G4cout << "New histogram will be booked: #" << id << "  <" << name << "  " << nb << "  " << x1
           << "  " << x2 << "  " << u1 << " " << u2 << G4endl;
  }
  fNbHisto++;
...
void HistoManager::SetHisto1D(G4int i, G4int nb, G4double x1, G4double x2, G4double unit)
{
  if (i >= 0 && i < fNbHisto)
  {
    if (fVerbose)
    {
      G4cout << "Update histogram: #" << i << "  " << nb << "  " << x1 << "  " << x2 << "  "
             << G4endl;
    }
    fNbins[i] = nb;
    fUnit1[i] = unit;
...
void HistoManager::FillHisto(G4int ih, G4double x, G4double w)
{
  if (fVerbose)
  {
    G4cout << "fill histogram: #" << ih << " at x= " << x << "  weight= " << w << G4endl;
  }
