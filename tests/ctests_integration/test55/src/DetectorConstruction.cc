...
G4VPhysicalVolume* DetectorConstruction::Construct()
{
  ...
  if (fTallyNumber > 0) {
    for (G4int j=0; j<fTallyNumber; ++j) {
       ...
       sTally[j] = new G4Box("Tally",
                   fTallySize[j].x()/2,fTallySize[j].y()/2,fTallySize[j].z()/2);
                      
       fLTally[j] = new G4LogicalVolume(sTally[j],fAbsorMaterial,"Tally");
...
void DetectorConstruction::SetSizeX(G4double value)
{
  ...
  if(sWorld) { 
    sWorld->SetXHalfLength(fWorldSizeX*0.5);
    sAbsor->SetXHalfLength(fAbsorSizeX*0.5);
  }
}
...
void DetectorConstruction::SetSizeYZ(G4double value)
{
  ...
  if(sWorld) { 
    sWorld->SetYHalfLength(fWorldSizeYZ*0.5);
    sWorld->SetZHalfLength(fWorldSizeYZ*0.5);
    sAbsor->SetYHalfLength(fAbsorSizeYZ*0.5);
    sAbsor->SetZHalfLength(fAbsorSizeYZ*0.5);
  }
}  
...
void DetectorConstruction::SetMaxChargedStep(G4double x) 
{ 
  fMaxStep = x; 
  G4RunManager::GetRunManager()->PhysicsHasBeenModified();
}
