...
void RunAction::TestDNAStopping()
{
  ...
  for (G4int i=0; i<=nd; ++i) {
    ...
    G4cout << std::setw(2) << i << "." << std::setw(5)
           << " E(keV)=" << std::setw(7) << e/CLHEP::keV
	   << " sigP(cm-2)=" << std::setw(7) << y1*f
	   << " sigHe4(cm-2)=" << std::setw(7) << y2*f
	   << " sigC12(cm-2)=" << std::setw(7) << y3*f;
    if(y1 > 0.0 && y1 < DBL_MAX) {
      G4cout << " He4/P=" << std::setw(7) << y21/(y1*4)
	     << " C12/P=" << y31/(y1*36);
    }
    G4cout << G4endl;
    e *= fac;
