...
void SteppingAction::UserSteppingAction(const G4Step* aStep)
{
  ...
  run->FillEdep(edep,aStep->GetNonIonizingEnergyDeposit());
  ...
  if(aStep->GetTrack()->GetTrackID() == 1) {
     ...
     if( preVolume != postVolume ) {
       G4VPhysicalVolume* detVolume = fDetector->GetAbsorPhysVol();
       if( detVolume == postVolume ) {
         run->FillEnIncoming( prePoint -> GetKineticEnergy() );
       } else if( detVolume == preVolume ) {
         run->FillEnOutgoing( postPoint -> GetKineticEnergy() );
       }
     }
  }
  ...
  G4double x = run->GetOffsetX() + x1 + G4UniformRand()*(x2-x1);
  run->FillHisto(1, x , edep);
  run->FillHisto(2, x , edep);

  //fill tallies
  //
  G4int copyNb = prePoint->GetTouchableHandle()->GetCopyNumber();
  if (copyNb > 0 && copyNb <= kMaxTally) { run->FillTallyEdep(copyNb-1, edep); }
}
