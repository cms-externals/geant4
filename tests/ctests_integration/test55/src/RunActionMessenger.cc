...
RunActionMessenger::RunActionMessenger(RunAction* run) : runAction(run)
{
  ...
  rangeTestCmd->AvailableForStates(G4State_PreInit, G4State_Idle);
  ...
  elossTestCmd->AvailableForStates(G4State_PreInit, G4State_Idle);
  ...
  dnaTestCmd = new G4UIcmdWithABool("/testem/run/testDNA", this);
  dnaTestCmd->SetGuidance("Test DNA cross sections:");
  dnaTestCmd->AvailableForStates(G4State_PreInit);
}
...
void RunActionMessenger::SetNewValue(G4UIcommand* command, G4String newValue)
{
  ...
  if (command == dnaTestCmd)
  {
    runAction->SetDNATest(dnaTestCmd->GetNewBoolValue(newValue));
  }
}
