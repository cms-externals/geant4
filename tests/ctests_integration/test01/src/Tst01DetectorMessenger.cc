...
Tst01DetectorMessenger::Tst01DetectorMessenger(Tst01DetectorConstruction* myDC) : myDetector(myDC)
{
  ...
  switchCSGcmd->SetGuidance("In cese detector name is associated to this command,");

  switchCSGcmd->SetGuidance("\"/mydet/SelectCSG\" will be invoked and then switched.");
  ...
  switchBoolCmd->SetGuidance("Assign the selected Boolean geometry to G4RunManager.");

  switchBoolCmd->SetGuidance("In cese detector name is associated to this command,");

  switchBoolCmd->SetGuidance("\"/mydet/SelectBool\" will be invoked and then switched.");
...
Tst01DetectorMessenger::~Tst01DetectorMessenger()
{
  delete switchBoolCmd;
  delete selBoolCmd;
  delete switchCSGcmd;
  delete selCSGcmd;
  delete selMatCmd;
...
void Tst01DetectorMessenger::SetNewValue(G4UIcommand* command, G4String newValues)
{
  ...
  if (command == switchCmd)
  {
    if (newValues == "SimpleBox" || newValues == "Honeycomb" || newValues == "Assembly"
        || newValues == "Assembly2" || newValues == "Assembly3")
    {
      myDetector->SelectDetector(newValues);
    }
  ...
  {
    if (newValues == "Box" || newValues == "Tubs" || newValues == "Cons" || newValues == "Sphere")
    {
      myDetector->SelectCSG(newValues);
    }
  ...
  {
    if (newValues == "Intersection" || newValues == "Union" || newValues == "Subtraction")
    {
      myDetector->SelectBoolean(newValues);
    }
