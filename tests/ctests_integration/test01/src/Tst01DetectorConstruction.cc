...
Tst01DetectorConstruction::Tst01DetectorConstruction()
  : simpleBoxLog(0),
    ...
    fChoiceCSG(0),
    fChoiceBool(0),
    AssemblyDetectorLog(0),
    AssemblyDetector(0),
    AssemblyCellLog(0),
    AssemblyDetector2(0),
    AssemblyDetector3(0),
    simpleBoxVisAtt(0),
    experimentalHallVisAtt(0),
    calorimeterBoxVisAtt(0),
    calorimeterTubeVisAtt(0)
{
  wSize = 2000. * cm;
  SelectDetector("SimpleBox");
  SelectMaterial("Pb");
  SelectCSG("Box");
  SelectBoolean("Intersection");
...
Tst01DetectorConstruction::~Tst01DetectorConstruction()
{
  ...
  delete simpleBoxVisAtt;
  delete experimentalHallVisAtt;
  delete calorimeterBoxVisAtt;
  delete calorimeterTubeVisAtt;
}
...
void Tst01DetectorConstruction::SwitchCSG()
{
  ...
  fTestVol = new G4PVPlacement(0, G4ThreeVector(), "testVol", fTestLog, fWorldPhysVol, false, 0);
...
void Tst01DetectorConstruction::SwitchBoolean()
{
  ...
  if (!fDisPb)
  {
    ...
    fDisPb = new G4DisplacedSolid("disPb", fPb3, &identity, G4ThreeVector(0, 0, 60 * cm));
  ...
  fTestBoolLog = new G4LogicalVolume(fTestBool, selectedMaterial, "testBoolLog", 0, 0, 0);
  ...
  fTestBoolVol =
    new G4PVPlacement(0, G4ThreeVector(), "testBoolVol", fTestBoolLog, fWorldPhysVol, false, 0);
  ...
  fTestD1Log = new G4LogicalVolume(fSphere, selectedMaterial, "testD1Log", 0, 0, 0);

  if (fTestD1Vol)
  {
    delete fTestD1Vol;
  }
  fTestD1Vol = new G4PVPlacement(0, putDaughter, "testD1Vol", fTestD1Log, fTestBoolVol, false, 0);
...
void Tst01DetectorConstruction::ConstructDetectors()
{
  ...
  simpleBoxLog = new G4LogicalVolume(mySimpleBox, Air, "SLog", 0, 0, 0);
  simpleBoxDetector = new G4PVPlacement(0, G4ThreeVector(), "SPhys", simpleBoxLog, 0, false, 0);
  ...
  G4LogicalVolume* myWorldLog = new G4LogicalVolume(myWorldBox, Air, "WLog", 0, 0, 0);
  G4LogicalVolume* myCalLog = new G4LogicalVolume(myCalBox, Al, "CLog", 0, 0, 0);
  G4LogicalVolume* myTargetLog = new G4LogicalVolume(myTargetTube, Pb, "TLog", 0, 0, 0);

  honeycombDetector = new G4PVPlacement(0, G4ThreeVector(), "WPhys", myWorldLog, 0, false, 0);

  G4PVPlacement* myCalPhys =
    new G4PVPlacement(0, G4ThreeVector(), "CalPhys", myCalLog, honeycombDetector, false, 0);
  ...
  for (j = 1; j <= 25; j++)
  {
    ...
    for (i = 1; i <= 50; i++)
    {
      ...
      new G4PVPlacement(0, G4ThreeVector(xTlate, yTlate, 0 * cm), tName1.str().c_str(), myTargetLog,
                        myCalPhys, false, copyNo++);
    }
  }
  for (j = 1; j <= 26; j++)
  {
    ...
    for (i = 1; i <= 50; i++)
    {
      ...
      new G4PVPlacement(0, G4ThreeVector(xTlate, yTlate, 0 * cm), tName2.str().c_str(), myTargetLog,
                        myCalPhys, false, copyNo++);
    }
  }
  ...
  //
  G4Box* AssemblyWorldBox = new G4Box("AssemblyWorldBox", worldX, worldY, worldZ);
  G4LogicalVolume* AssemblyWorldLog =
    new G4LogicalVolume(AssemblyWorldBox, selectedMaterial, "AssemblyWorldLog", 0, 0, 0);
  // G4VPhysicalVolume* AssemblyWorld    =
  new G4PVPlacement(0, G4ThreeVector(), "AssemblyWorld", AssemblyWorldLog, 0, false, 0);

  // The Assembly detector
  //
  G4Box* AssemblyBox = new G4Box("AssemblyBox", worldX / 2, worldY / 2, worldZ / 2);
  AssemblyDetectorLog =
    new G4LogicalVolume(AssemblyBox, selectedMaterial, "AssemblyDetectorLog", 0, 0, 0);
  AssemblyDetector = new G4PVPlacement(0, G4ThreeVector(), AssemblyDetectorLog, "AssemblyDetector",
  ...
  G4AssemblyVolume* AssemblyCalo = new G4AssemblyVolume();
  G4AutoDelete::Register(AssemblyCalo);
  ...
  G4VSolid* worldS = new G4Box("worldS", wSize, wSize, wSize);
  G4LogicalVolume* worldV2 = new G4LogicalVolume(worldS, Air, "worldV2");
  AssemblyDetector2 = new G4PVPlacement(0, G4ThreeVector(), worldV2, "world2", 0, false, 0);
  ...
  G4AutoDelete::Register(tplate);

  // plate volume
  G4Box* plateS = new G4Box("plateS", xplate, yplate, 1. * cm);
  G4LogicalVolume* plateV = new G4LogicalVolume(plateS, Al, "PLATE");

  // tooth volume
  G4Box* toothS = new G4Box("toothS", xtooth, ytooth, 1. * cm);
  G4LogicalVolume* toothV = new G4LogicalVolume(toothS, Al, "TOOTH");
  ...
  G4AutoDelete::Register(cell);

  // Make a hexagone cell out of 6 toothplates. These can zip together
  ...
  // World
  //
  G4LogicalVolume* worldV3 = new G4LogicalVolume(worldS, Air, "worldV3");
  AssemblyDetector3 = new G4PVPlacement(0, G4ThreeVector(), worldV3, "world3", 0, false, 0);

  // Assembly
  //
  G4AssemblyVolume* assembly = new G4AssemblyVolume();
  G4AutoDelete::Register(assembly);

  // Volumes
  //
  G4VSolid* consS =
    new G4Cons("consS", 10. * cm, 40. * cm, 20. * cm, 60. * cm, 50 * cm, 0., 360. * deg);
  G4LogicalVolume* consV = new G4LogicalVolume(consS, Al, "CONS");
  ...
  HepGeom::Transform3D transform2 =
    HepGeom::ReflectX3D() * HepGeom::Translate3D(110. * cm, 0., 0.) * HepGeom::RotateY3D(90. * deg);
  ...
  HepGeom::Transform3D transform4 = HepGeom::ReflectY3D() * HepGeom::Translate3D(0., 110. * cm, 0.)
  ...
  simpleBoxVisAtt = new G4VisAttributes(G4Colour(1.0, 1.0, 1.0));
  simpleBoxVisAtt->SetVisibility(true);
  simpleBoxLog->SetVisAttributes(simpleBoxVisAtt);

  experimentalHallVisAtt = new G4VisAttributes(G4Colour(1.0, 1.0, 1.0));
  experimentalHallVisAtt->SetVisibility(false);
  myWorldLog->SetVisAttributes(experimentalHallVisAtt);

  calorimeterBoxVisAtt = new G4VisAttributes(G4Colour(0.0, 0.0, 1.0));
  ...
  calorimeterTubeVisAtt = new G4VisAttributes(G4Colour(0.0, 0.5, 0.5));
