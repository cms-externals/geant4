...
Tst01PrimaryGeneratorAction::Tst01PrimaryGeneratorAction(Tst01DetectorConstruction* detector)
  : generatorAction(standardGun), particleGun(0), messenger(0), fGunPosition(0.0, 0.0, 0.0)
{
  G4int n_particle = 1;
  ...
  switch (generatorAction)
  {
    ...
    case randomPositionGun:
      ...
      particleGun->SetParticlePosition(
        G4ThreeVector(CLHEP::RandFlat::shoot(-fSize / 2, fSize / 2),
    ...
    case viewerGun:
      ...
      if (fPosition)
      {
        direction = G4ThreeVector(-fGunPosition.x() / fPosition, -fGunPosition.y() / fPosition,
                                  -fGunPosition.z() / fPosition);

        costheta =
          CLHEP::RandFlat::shoot(fPosition / std::sqrt(fPosition * fPosition + fSize * fSize), 1.);
        sintheta = std::sqrt(1. - costheta * costheta);
        phi = CLHEP::RandFlat::shoot(twopi);
        cosphi = std::cos(phi);
        sinphi = std::sin(phi);
        position = G4ThreeVector(sintheta * cosphi, sintheta * sinphi, costheta);
    ...
    case planeGun:
      ...
      {
        G4Exception("Tst01PrimaryGeneratorAction::GeneratePrimaries", "InvalidSetup",
                    FatalException, "Invalid setting for gun");
      }
      particleGun->GeneratePrimaryVertex(anEvent);
      break;
  }
