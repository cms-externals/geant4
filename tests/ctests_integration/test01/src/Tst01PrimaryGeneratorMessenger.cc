  ...
  : fPrimaryGeneratorAction(myPGA)
{
  ...
  mydetDir = new G4UIdirectory("/tsGun/");
  ...
  randomGun->SetCandidates("randomDirection randomPosition randomPositionAndDirection");
...
Tst01PrimaryGeneratorMessenger::~Tst01PrimaryGeneratorMessenger()
{
  delete planeCmd;
  delete viewerCmd;
  delete randomGun;
...
void Tst01PrimaryGeneratorMessenger::SetNewValue(G4UIcommand* command, G4String newValues)
{
  if (command == standardGun)
  {
    fPrimaryGeneratorAction->SelectPrimaryGeneratorAction(Tst01PrimaryGeneratorAction::standardGun);
  }
  ...
  {
    fPrimaryGeneratorAction->SetGunPosition(viewerCmd->GetNew3VectorValue(newValues));
    fPrimaryGeneratorAction->SelectPrimaryGeneratorAction(Tst01PrimaryGeneratorAction::viewerGun);
  }
  if (command == planeCmd)
  {
    fPrimaryGeneratorAction->SetGunPosition(planeCmd->GetNew3VectorValue(newValues));
    fPrimaryGeneratorAction->SelectPrimaryGeneratorAction(Tst01PrimaryGeneratorAction::planeGun);
  }
  return;
}
