...
Tst10DetectorConstruction::Tst10DetectorConstruction()
  : aVolume(0), PhysicalVolume(0), Water(0), Water1(0), aSurface(0), fHallSize(1 * m)
{
...
G4VPhysicalVolume* Tst10DetectorConstruction::SelectDetector(const G4String& val)
{
  //------------------- A Volume ----------------------
  ...
  if (val == "Sphere")
    aVolume =
      new G4Sphere("aSphere", 8.0 * cm, 10.0 * cm, 0.0 * deg, 360.0 * deg, 0.0 * deg, 130.0 * deg);
  else if (val == "Orb")
    aVolume = new G4Orb("aOrb", 10.0 * cm);
  else if (val == "Box")
    aVolume = new G4Box("aBox", 10 * cm, 10 * cm, 10 * cm);
  else if (val == "Cone")
    aVolume = new G4Cons("aCone", 2 * cm, 6 * cm, 8 * cm, 14 * cm, 10 * cm, 10 * deg, 300 * deg);
  else if (val == "Tube")
    ...
    aVolume = new G4Trap("aTrap", 50 * cm, 0, 0, 50 * cm, 50 * cm, 50 * cm, -45 * deg, 50 * cm,
                         50 * cm, 50 * cm, -45 * deg);
  else if (val == "Trd")
  ...
  else if (val == "ExtrudedSolid")
  {
    ...
    zsections.push_back(
      G4ExtrudedSolid::ZSection(-4.0 * cm, G4TwoVector(-2.0 * cm, 1.0 * cm), 1.5));
  ...
  {
    G4Exception("Tst10DetectorConstruction::SelectDetector", "Tst10_01", FatalException,
                "Invalid shape!");
  }

  G4Box* Hall = new G4Box("Hall", fHallSize, fHallSize, fHallSize);
  G4LogicalVolume* Hall_log = new G4LogicalVolume(Hall, Water, "Hall_L", 0, 0, 0);
  PhysicalVolume = new G4PVPlacement(0, G4ThreeVector(), "Hall_P", Hall_log, 0, false, 0);
  ...
  G4VPhysicalVolume* aVolume_phys1 = new G4PVPlacement(0, G4ThreeVector(50 * cm, 0 * cm, 0 * cm),
                                                       val, aVolume_log, PhysicalVolume, false, 0);
  G4VPhysicalVolume* aVolume_phys2 = new G4PVPlacement(mat, G4ThreeVector(-50 * cm, 0 * cm, 0 * cm),
                                                       val, aVolume_log, PhysicalVolume, false, 0);
  G4VPhysicalVolume* aVolume_phys3 = new G4PVPlacement(mat, G4ThreeVector(0 * cm, 50 * cm, 0 * cm),
                                                       val, aVolume_log, PhysicalVolume, false, 0);
  G4VPhysicalVolume* aVolume_phys4 = new G4PVPlacement(mat, G4ThreeVector(0 * cm, -50 * cm, 0 * cm),
                                                       val, aVolume_log, PhysicalVolume, false, 0);
  G4VPhysicalVolume* aVolume_phys5 = new G4PVPlacement(0, G4ThreeVector(0 * cm, 0 * cm, 50 * cm),
                                                       val, aVolume_log, PhysicalVolume, false, 0);
  G4VPhysicalVolume* aVolume_phys6 = new G4PVPlacement(0, G4ThreeVector(0 * cm, 0 * cm, -50 * cm),
                                                       val, aVolume_log, PhysicalVolume, false, 0);
  ...
  // BorderSurfaces[0] =
  new G4LogicalBorderSurface("VolumeSurface", PhysicalVolume, aVolume_phys1, aSurface);
  // BorderSurfaces[1] =
  new G4LogicalBorderSurface("VolumeSurface", PhysicalVolume, aVolume_phys2, aSurface);
  // BorderSurfaces[2] =
  new G4LogicalBorderSurface("VolumeSurface", PhysicalVolume, aVolume_phys3, aSurface);
  // BorderSurfaces[3] =
  new G4LogicalBorderSurface("VolumeSurface", PhysicalVolume, aVolume_phys4, aSurface);
  // BorderSurfaces[4] =
  new G4LogicalBorderSurface("VolumeSurface", PhysicalVolume, aVolume_phys5, aSurface);
  // BorderSurfaces[5] =
  new G4LogicalBorderSurface("VolumeSurface", PhysicalVolume, aVolume_phys6, aSurface);
  // BorderSurfaces[6] =
  new G4LogicalBorderSurface("VolumeSurface", aVolume_phys1, PhysicalVolume, aSurface);
  // BorderSurfaces[7] =
  new G4LogicalBorderSurface("VolumeSurface", aVolume_phys2, PhysicalVolume, aSurface);
  // BorderSurfaces[8] =
  new G4LogicalBorderSurface("VolumeSurface", aVolume_phys3, PhysicalVolume, aSurface);
  // BorderSurfaces[9] =
  new G4LogicalBorderSurface("VolumeSurface", aVolume_phys4, PhysicalVolume, aSurface);
  // BorderSurfaces[10] =
  new G4LogicalBorderSurface("VolumeSurface", aVolume_phys5, PhysicalVolume, aSurface);
  // BorderSurfaces[11] =
  new G4LogicalBorderSurface("VolumeSurface", aVolume_phys6, PhysicalVolume, aSurface);
...
void Tst10DetectorConstruction::SetMaterial()
{
  ...
  G4double PHENERGY[NUMENTRIES] = {0.01, 1.0, 2.0, 3.0, 4.0};
