...
Tst34DetectorConstruction::Tst34DetectorConstruction()
  : m_experimentalHall_log(0), m_calo_log(0), m_experimentalHall_phys(0), m_calo_phys(0)
{
...
G4VPhysicalVolume* Tst34DetectorConstruction::Construct()
{
  ...
  m_experimentalHall_log =
    new G4LogicalVolume(m_experimentalHall_box, matManager->getMaterial("Air"),
  ...
  G4Box* calo_box = new G4Box("CMS calorimeter",  // its name
                              m_calo_xside / 2.,  // size
                              m_calo_yside / 2., m_calo_zside / 2.);
  m_calo_log = new G4LogicalVolume(calo_box, matManager->getMaterial("PbWO4"),
                                   "calo log",  // its name
                                   0,  // opt: fieldManager
                                   0,  // opt: SensitiveDetector
                                   0);  // opt: UserLimit
  ...
  Tst34SensitiveDetector* CaloSD = new Tst34SensitiveDetector("Calorimeter");
  ...
  auto m_theFastShowerModel = new GFlashShowerModel("fastShowerModel", aRegion);
  auto m_theParametrisation = new GFlashHomoShowerParameterisation(matManager->getMaterial(mat));
  m_theFastShowerModel->SetParameterisation(m_theParametrisation);
  // GlashStuff
  auto m_theParticleBounds = new GFlashParticleBounds();  // Energy Cuts to kill particles
  m_theFastShowerModel->SetParticleBounds(m_theParticleBounds);
  auto m_theHMaker = new GFlashHitMaker();  // Makes the EnergieSpots
