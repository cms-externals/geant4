...
class Tst69INCLXXTallyAnalysis : public G4INCLXXVInterfaceTally
{
  public:
    ...
    Tst69INCLXXTallyAnalysis(G4String const& physList, const G4bool histos = true,
                             const G4bool ntuple = false);
    virtual ~Tst69INCLXXTallyAnalysis() = default;
    virtual void Open();
    virtual void Close();
    virtual void Tally(G4HadProjectile const& aTrack, G4Nucleus const& theNucleus,
                       G4HadFinalState const& result);

  protected:

    G4int whichIncomingParticle(const G4int B, const G4int A, const G4int Z);
