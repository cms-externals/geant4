...
const char* Tst69INCLXXTallyAnalysis::incPartDict[nIncPart] = {"N",   "Pi",         "Alpha",
                                                               "C12", "AntiProton", "AntiNeutron"};

const char* Tst69INCLXXTallyAnalysis::outPartDict[nOutPart] = {"n", "p", "Pi", "Alpha", "Li7"};

const char* Tst69INCLXXTallyAnalysis::incPartLongDict[nIncPart] = {
  "nucleon", "pion", "alpha", "C12", "AntiProton", "AntiNeutron"};

const char* Tst69INCLXXTallyAnalysis::outPartLongDict[nOutPart] = {"neutrons", "protons", "pions",
                                                                   "alphas", "Li7"};

const char* Tst69INCLXXTallyAnalysis::eSliceDict[nESlice] = {"LE", "ME", "HE"};

const G4float Tst69INCLXXTallyAnalysis::eSliceLow[nESlice + 1] = {0., 200., 3000., 20000};

const G4int Tst69INCLXXTallyAnalysis::nBins = 60;

const G4int Tst69INCLXXTallyAnalysis::nZBins = 100;

Tst69INCLXXTallyAnalysis::Tst69INCLXXTallyAnalysis(G4String const& physList, const G4bool histos,
                                                   const G4bool ntuple)
  : factoryOn(false), withHistos(histos), withNtuple(ntuple)
{
...
void Tst69INCLXXTallyAnalysis::Open()
{
  ...
  if (withNtuple)
  {
    ...
    colIDBp = analysisManager->CreateNtupleIColumn("Bp");
    ...
    for (int iIncPart = 0; iIncPart < nIncPart; ++iIncPart)
    {
      for (int iESlice = 0; iESlice < nESlice; ++iESlice)
      {
        for (int iOutPart = 0; iOutPart < nOutPart; ++iOutPart)
        {
          ...
          ss.str("");
          ss << "Energy spectrum of " << outPartLongDict[iOutPart] << " from reactions induced by "
             << incPartLongDict[iIncPart] << " between " << eSliceLow[iESlice] << " and "
             << eSliceLow[iESlice + 1] << " AMeV";
          hDescription = ss.str();

          G4cout << "Creating histogram " << hName << " with description: " << hDescription
                 << G4endl;
      ...
      hName += incPartDict[iIncPart];
      ss.str("");
      ss << "Energy spectrum of reactions induced by " << incPartLongDict[iIncPart];
      hDescription = ss.str();
      ...
      for (int iIncPart = 0; iIncPart < nIncPart; ++iIncPart)
      {
        for (int iESlice = 0; iESlice < nESlice; ++iESlice)
        {
          for (int iOutPart = 0; iOutPart < nOutPart; ++iOutPart)
          {
            ...
            G4cout << outPartLongDict[iOutPart] << " from " << incPartLongDict[iIncPart]
                   << "-induced reactions (" << eSliceLow[iESlice] << "-" << eSliceLow[iESlice + 1]
                   << " MeV): " << mean << " +- " << rms << " MeV" << G4endl;
          }
        }
      }
    }
...
void Tst69INCLXXTallyAnalysis::Tally(G4HadProjectile const& aTrack, G4Nucleus const& theNucleus,
                                     G4HadFinalState const& result)
{
  G4ParticleDefinition const* const trackDefinition = aTrack.GetDefinition();
  G4int Ap = trackDefinition->GetAtomicMass();
  const G4int Bp = trackDefinition->GetBaryonNumber();
  const G4int Zp = (G4int)trackDefinition->GetPDGCharge();
  if (trackDefinition->GetPDGEncoding() == -2112)
  {
    Ap = 1;
  }
  ...
  if (withNtuple)
  {
    ...
    analysisManager->FillNtupleIColumn(ntupleID, colIDBp, Bp);
  ...
  if (withHistos)
  {
    ...
    const G4int iIncPart = whichIncomingParticle(Bp, Ap, Zp);
    if (iIncPart < 0) return;
    ...
    for (G4int i = 0; i < nParticles; ++i)
    {
      ...
      if (A > 0) analysisManager->FillH1(hID_Z_ie[iIncPart][iESlice], Z);

      const G4int iOutPart = whichOutgoingParticle(A, Z);
      if (iOutPart < 0) continue;
...
G4int Tst69INCLXXTallyAnalysis::whichIncomingParticle(const G4int B, const G4int A, const G4int Z)
{
  if (A == 1 && B == 1)
    return 0;
  else if (A == 0)
...
G4int Tst69INCLXXTallyAnalysis::whichEnergySlice(const G4double E)
{
  for (int iESlice = 0; iESlice < nESlice; ++iESlice)
  {
    if (E >= eSliceLow[iESlice] && E < eSliceLow[iESlice + 1]) return iESlice;
  }
  return -1;
}
