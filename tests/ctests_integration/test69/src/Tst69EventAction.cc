...
void Tst69EventAction::BeginOfEventAction(const G4Event* anEvent)
{
  ...
  if (secondsSinceLastPrint > nSecondsReport)
  {
    G4cout << "Tst69EventAction: event " << eventID << " starting after more than "
           << nSecondsReport << " s since last printout (" << secondsSinceLastPrint
           << " s), printout forced" << G4endl;
    forcePrint = true;
  }
  if (forcePrint || (eventID - lastEventIDPrint) % eventPrintModulo == 0)
  {
    G4cout << "Tst69EventAction: starting event " << eventID << ", "
           << (currentEventStart - actionStart) / CLOCKS_PER_SEC << " s after action creation, "
           << secondsSinceLastPrint << " s after last printout. " << G4endl;
...
void Tst69EventAction::EndOfEventAction(const G4Event*)
{
  ...
  {
    G4cout << "Tst69EventAction: event " << eventID << " completed after more than "
           << nSecondsPerEventMax << " s (" << secondsSinceEventStart << " s)" << G4endl;
  }
