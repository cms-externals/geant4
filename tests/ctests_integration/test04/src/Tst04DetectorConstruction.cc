...
#include "Tst04DetectorMessenger.hh"
#include "Tst04TargetParameterisation.hh"

Tst04DetectorConstruction::Tst04DetectorConstruction()
  : simpleBoxLog(0),
    selectedMaterial(0),
    Air(0),
    Al(0),
    ...
    isoU(0)
{
  detectorMessenger = new Tst04DetectorMessenger(this);
...
void Tst04DetectorConstruction::SelectMaterial(G4String val)
{
  materialChoice = val;
...
void Tst04DetectorConstruction::SelectMaterialPointer()
{
  //--------- Material definition ---------
  ...
  */
  ...
  if (simpleBoxLog)
  {
    ...
    ((Tst04TargetParameterisation*)pVPVParameterisation)->SetMaterial(selectedMaterial);
  }
}

G4VPhysicalVolume* Tst04DetectorConstruction::Construct()
{
  SelectMaterialPointer();

  G4Box* mySimpleBox = new G4Box("SBox", 5 * cm, 5 * cm, 20 * m);
  simpleBoxLog = new G4LogicalVolume(mySimpleBox, selectedMaterial, "SLog", 0, 0, 0);
  pVPVParameterisation = new Tst04TargetParameterisation(1, selectedMaterial);

  G4LogicalVolume* parameterisedSimpleBoxLV =
    new G4LogicalVolume(mySimpleBox, selectedMaterial, "ParameterisedSimpleBoxLV");

  new G4PVParameterised("ParameterisedSimpleBox", parameterisedSimpleBoxLV, simpleBoxLog,
                        kUndefined, 1, pVPVParameterisation);

  G4VPhysicalVolume* simpleBoxDetector =
    new G4PVPlacement(0, G4ThreeVector(), "SPhys", simpleBoxLog, 0, false, 0);

  return simpleBoxDetector;
}

#include "G4NistManager.hh"
void Tst04DetectorConstruction::defineNISTMaterials()
{
...
void Tst04DetectorConstruction::defineTKMaterials()
{
  /*
     G4NistManager* nistMan = G4NistManager::Instance();
     ...
     G4Material* Hydrogen1 = new G4Material("Hydrogen1" , density= 1.0*g/cm3 , ncomponents = 1 ,
     kStateSolid , 0.*kelvin , 1*atmosphere ); Hydrogen1->AddElement( elH1 , fractionmass = 1 );

     // Create Element for Thermal Scattering

     G4Element* elTSHW= new G4Element( "TS_H_of_Water" , "H_WATER" , 1.0 , 1.0079*g/mole );

     G4Element* elTSH= new G4Element( "TS_H_of_Polyethylene" , "H_POLYETHYLENE" , 1.0
     , 1.0079*g/mole );
     ...
     G4Element* elTSDW =
                new G4Element( "TS_D_of_Heavy_Water" , "D_HEAVY_WATER" , 1.0 , 1875.61/931.494
     *g/mole );
     ...
     //           new G4Element( "TS_D_of_Para_Deuterium", "D_PARA_D2" , 1.0 ,
     1875.61/931.494*g/mole );
     //G4Element* elTSDorthoD2=
     //           new G4Element( "TS_D_of_Ortho_Deuterium", "D_ORTHO_D2" , 1.0 ,
     1875.61/931.494*g/mole );
     ...
     G4Material* matCH2_TS = new G4Material( "Polyethylene_TS" , density = 0.94*g/cm3 , ncomponents=
     2 ); matCH2_TS -> AddElement(elTSH,natoms=2); matCH2_TS ->
     AddElement(nistMan->FindOrBuildElement( "C" ), natoms= 1);

     G4Material* matC_TS = new G4Material( "Graphite_TS" , density = 2.267*g/cm3 , ncomponents= 1 );
     matC_TS -> AddElement(elTSC,natoms=1);


     G4Material* matD2O_TS = new G4Material( "Heavy_Water_TS" , density = 1.107*g/cm3 , ncomponents=
     2 ); matD2O_TS -> AddElement(elTSDW,natoms=2); matD2O_TS ->
     AddElement(nistMan->FindOrBuildElement( "O" ), natoms = 1);
     ...
     G4Material* matBeO_TS = new G4Material( "Beryllium_Oxide_TS" , density = 3.02*g/cm3 ,
     ncomponents= 2 ); matBeO_TS -> AddElement(elTSBeBeO,natoms=1); matBeO_TS ->
     AddElement(elTSOBeO, natoms = 1);

     //G4Material* matUO2_TS = new G4Material( "Uranium_Dioxide_TS" , density = 10.97*g/cm3 ,
     ncomponents= 2 );
...
void Tst04DetectorConstruction::SetMaterial(G4String mat)
{
  // search the material by its name
  selectedMaterial = G4Material::GetMaterial(mat);
}

void Tst04DetectorConstruction::createIsotopeMaterial(G4int iZ, G4int iA, G4String name,
                                                      G4double temp, G4int ism)
{
  G4NistManager* nistMan = G4NistManager::Instance();
  ...
  {
    anIsotope =
      new G4Isotope(name, iZ, iA, nistMan->GetAtomicMass(iZ, iA) / 931.494028 * g / mole, ism);
  ...
  G4Material* theMaterial =
    new G4Material(name, density, 1, kStateSolid, temp * kelvin, 1 * atmosphere);
  theMaterial->AddElement(theElement, 1);
}

void Tst04DetectorConstruction::defineENDFVIIMaterials()
{
  //                      Z    A                       m
...
void Tst04DetectorConstruction::ConstructSDandField() {}
