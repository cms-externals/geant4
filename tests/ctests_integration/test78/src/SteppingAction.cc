...
void SteppingAction::UserSteppingAction(const G4Step* aStep)
{
  ...
  if (aStep->GetTrack()->GetTrackStatus() == fAlive)
  {
    ...
    The azimuth can be selected depending on application where G4JAEAPolarizedElasticScattering is
    used. Here, the plane parallel to polarization of incident photon is selected (phi=0). Most
    accurate case when phi=0 exactly. But a window is selected to see good results at reasonable
    beamOn.
    */
    if (trPhi <= 0.005 || abs(trPhi - CLHEP::pi) <= 0.005 || abs(trPhi - 2 * CLHEP::pi) <= 0.005)
    {
      analysisManager->FillH2(1, acos(direction.z()) * 180. / CLHEP::pi,
                              endPoint->GetPolarization().x());
    }
  }
