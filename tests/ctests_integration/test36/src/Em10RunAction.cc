    ...
    nbinvertexz(0)
{
  ...
  EnergySumAbs = EnergySquareSumAbs = tlSumAbs = tlsquareSumAbs = nStepSumCharged =
    nStepSum2Charged = nStepSumNeutral = nStepSum2Neutral = TotNbofEvents = SumCharged =
      SumNeutral = Sum2Charged = Sum2Neutral = Selectron = Spositron = Transmitted = Reflected =
        dStep = entryStep = underStep = overStep = dEn = entryEn = underEn = overEn = dTt =
          entryTt = underTt = overTt = Ttmean = Tt2mean = dTb = entryTb = underTb = overTb =
            Tbmean = Tb2mean = dTsec = entryTsec = underTsec = overTsec = dTh = entryTh = underTh =
              overTh = dThback = entryThback = underThback = overThback = dR = entryR = underR =
                overR = Rmean = R2mean = dEGamma = entryGamma = underGamma = overGamma = dz =
                  entryvertexz = undervertexz = oververtexz = 0.;
}
...
void Em10RunAction::EndOfRunAction(const G4Run*)
{
  ...
  G4cout << "    mean charged track length   in absorber=" << tlSumAbs / mm << " +- " << sAbs / mm
         << "  mm  " << G4endl;
  G4cout << G4endl;
  G4cout << "            mean energy deposit in absorber=" << EnergySumAbs / MeV << " +- "
         << sigAbs / MeV << "  MeV " << G4endl;
  G4cout << G4endl;
  G4cout << " mean number of steps in absorber (charged) =" << nStepSumCharged << " +- " << sigch
         << "      " << G4endl;
  G4cout << " mean number of steps in absorber (neutral) =" << nStepSumNeutral << " +- " << signe
         << "      " << G4endl;
  G4cout << G4endl;
  G4cout << "   mean number of charged secondaries = " << SumCharged << " +- " << sigcharged
         << G4endl;
  G4cout << G4endl;
  G4cout << "   mean number of neutral secondaries = " << SumNeutral << " +- " << signeutral
         << G4endl;
  G4cout << G4endl;
  ...
  G4cout << "(number) transmission coeff=" << Transmitted << "  reflection coeff=" << Reflected
         << G4endl;
  G4cout << G4endl;
  ...
  if (nbinStep > 0)
  {
    ...
    if (entryStep > 0.)
    {
      ...
      for (G4int iss = 0; iss < nbinStep; iss++)
      {
        ...
        G4cout << std::setw(5) << iss << std::setw(10) << E << std::setw(12) << distStep[iss]
  ...
  if (nbinEn > 0)
  {
    ...
    G4cout << "#entries=" << entryEn << "    #underflows=" << underEn << "    #overflows=" << overEn
           << G4endl;
    if (entryEn > 0.)
    {
      E = Enlow - dEn;
      ...
      for (G4int ien = 0; ien < nbinEn; ien++)
      {
        ...
        G4cout << std::setw(5) << ien << std::setw(10) << E / keV << std::setw(12) << distEn[ien]
               << std::setw(12) << dnorm << G4endl;
      ...
      G4cout << " Emp = " << std::setw(15) << Emp / MeV << "   width=" << std::setw(15)
             << (E2 - E1) / MeV << "  MeV " << G4endl;
      G4cout << G4endl;
    }
  }
  if (nbinTt > 0)
  {
    ...
    G4cout << "#entries=" << entryTt << "    #underflows=" << underTt << "    #overflows=" << overTt
           << G4endl;
    if (entryTt > 0.)
    {
      Ttmean /= entryTt;
      ...
      G4cout << " mean energy of transmitted particles=" << Ttmean / keV << " +- " << sig / keV
             << "  keV." << G4endl;
      E = Ttlow - dTt;
      ...
      for (G4int itt = 0; itt < nbinTt; itt++)
      {
        ...
        G4cout << std::setw(5) << itt << std::setw(10) << E << std::setw(12) << distTt[itt]
               << std::setw(12) << dnorm << G4endl;
      }
      G4cout << G4endl;
    }
  ...
  if (nbinTb > 0)
  {
    ...
    G4cout << "#entries=" << entryTb << "    #underflows=" << underTb << "    #overflows=" << overTb
           << G4endl;
    if (entryTb > 0.)
    {
      Tbmean /= entryTb;
      ...
      G4cout << " mean energy of backscattered particles=" << Tbmean / keV << " +- " << sig / keV
             << "  keV." << G4endl;
      E = Tblow - dTb;
      ...
      for (G4int itt = 0; itt < nbinTb; itt++)
      {
        ...
        G4cout << std::setw(5) << itt << std::setw(10) << E << std::setw(12) << distTb[itt]
               << std::setw(12) << dnorm << G4endl;
      }
      G4cout << G4endl;
    }
  ...
  if (nbinTsec > 0)
  {
    ...
    if (entryTsec > 0.)
    {
      ...
      for (G4int itt = 0; itt < nbinTsec; itt++)
      {
        ...
        G4cout << std::setw(5) << itt << std::setw(10) << E << std::setw(12) << distTsec[itt]
               << std::setw(12) << dnorm << G4endl;
      }
      G4cout << G4endl;
    }
  ...
  if (nbinR > 0)
  {
    ...
    G4cout << "#entries=" << entryR << "    #underflows=" << underR << "    #overflows=" << overR
           << G4endl;
    if (entryR > 0.)
    {
      Rmean /= entryR;
      ...
      if (sig <= 0.)
        ...
        sig = std::sqrt(sig / entryR);
      G4cout << " mean lateral displacement at exit=" << Rmean / mm << " +- " << sig / mm << "  mm."
             << G4endl;
      R = Rlow - dR;
      ...
      for (G4int ier = 0; ier < nbinR; ier++)
      {
        ...
        G4cout << std::setw(5) << ier << std::setw(10) << R << std::setw(12) << distR[ier]
               << std::setw(12) << dnorm << G4endl;
      }
      G4cout << G4endl;
    }
  ...
  if (nbinTh > 0)
  {
    ...
    G4cout << "#entries=" << entryTh << "    #underflows=" << underTh << "    #overflows=" << overTh
           << G4endl;
    if (entryTh > 0.)
    {
      Th = Thlow - dTh;
      ...
      G4cout << " bin nb  Thlowdeg      Thlowrad      " << " entries         normalized " << G4endl;
      Thpere = 0.;
      sum = 0.;
      Thmean = 0.;
      for (G4int ien = 0; ien < nbinTh; ien++)
      {
        ...
        if (fnorm > pere) Thpere = Th;
        G4cout << std::setw(5) << ien << std::setw(10) << Thdeg << "   " << std::setw(10) << Th
               << "  " << std::setw(12) << distTh[ien] << "  " << std::setw(12) << dnorm << "  "
               << std::setw(12) << fnorm << G4endl;
      }
      Thmean /= sum;
      G4cout << G4endl;
      G4cout << " mean = " << Thmean << "  rad  or " << 180. * Thmean / pi << " deg." << G4endl;
      G4cout << " theta(1/e)=" << Thpere << " - " << Thpere + dTh << " rad   "
             << " or " << 180. * Thpere / pi << " - " << 180. * (Thpere + dTh) / pi << " deg."
             << G4endl;
      G4cout << G4endl;
    }
  }
  ...
  if (nbinThback > 0)
  {
    ...
    if (entryThback > 0.)
    {
      ...
      for (G4int ien = 0; ien < nbinThback; ien++)
      {
        ...
        if (fnormb > pereb) Thpereb = Thb;
        G4cout << std::setw(5) << ien << std::setw(10) << Thdegb << "   " << std::setw(10) << Thb
               << "  " << std::setw(12) << distThback[ien] << "  " << std::setw(12) << dnormb
               << "  " << std::setw(12) << fnormb << G4endl;
      }
      Thmeanb /= sumb;
      G4cout << G4endl;
      G4cout << " mean = " << Thmeanb << "  rad  or " << 180. * Thmeanb / pi << " deg." << G4endl;
      G4cout << " theta(1/e)=" << Thpereb << " - " << Thpereb + dThback << " rad   "
             << " or " << 180. * Thpereb / pi << " - " << 180. * (Thpereb + dThback) / pi << " deg."
             << G4endl;
      G4cout << G4endl;
    }
  }
  ...
  if (nbinGamma > 0)
  {
    ...
    if (entryGamma > 0.)
    {
      ...
      for (G4int itt = 0; itt < nbinGamma; itt++)
      {
        ...
        G4cout << std::setw(5) << itt << std::setw(13) << E << std::setw(12) << distGamma[itt]
  ...
  if (nbinvertexz > 0)
  {
    ...
    if (entryvertexz > 0.)
    {
      ...
      for (G4int iez = 0; iez < nbinvertexz; iez++)
      {
        ...
        G4cout << std::setw(5) << iez << std::setw(10) << z << std::setw(12) << distvertexz[iez]
               << std::setw(12) << dnorm << G4endl;
      }
      G4cout << G4endl;
    }
