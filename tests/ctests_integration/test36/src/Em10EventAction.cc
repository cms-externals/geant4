...
void Em10EventAction::BeginOfEventAction(const G4Event* evt)
{
  ...
  if (verboselevel > 1) G4cout << "<<< Event  " << evtNb << " started." << G4endl;
...
void Em10EventAction::EndOfEventAction(const G4Event* evt)
{
  ...
  if (HCE) CHC = (Em10CalorHitsCollection*)(HCE->GetHC(calorimeterCollID));
  ...
  if (CHC)
  {
    ...
    if (verboselevel == 2)
      G4cout << "   Absorber: total energy: " << std::setw(7) << G4BestUnit(totEAbs, "Energy")
    ...
    if (verboselevel == 2) G4cout << " Ncharged=" << Nch << "  ,   Nneutral=" << Nne << G4endl;
  ...
  if (verboselevel > 0) G4cout << "<<< Event  " << evt->GetEventID() << " ended." << G4endl;
