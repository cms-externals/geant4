...
XTRComptonScattering::XTRComptonScattering(const G4String& processName, G4ProcessType type)
...
void XTRComptonScattering::SetPhysicsTableBining(G4double lowE, G4double highE, G4int nBins)
{
  LowestEnergyLimit = lowE;
  HighestEnergyLimit = highE;
  NumbBinTable = nBins;
}
...
G4double XTRComptonScattering::ComputeCrossSectionPerAtom(G4double GammaEnergy, G4double Z)
...

{
  ...
  static const G4double d1 = 2.7965e-1 * barn, d2 = -1.8300e-1 * barn, d3 = 6.7527 * barn,
...
G4double XTRComptonScattering::ComputeMeanFreePath(G4double GammaEnergy, G4Material* aMaterial)
...

{
  ...
  for (size_t elm = 0; elm < aMaterial->GetNumberOfElements(); elm++)
  {
    SIGMA += NbOfAtomsPerVolume[elm]
             * ComputeCrossSectionPerAtom(GammaEnergy, (*theElementVector)[elm]->GetZ());
  }
...
G4double XTRComptonScattering::GetCrossSectionPerAtom(G4DynamicParticle* aDynamicGamma,
                                                      G4Element* anElement)
...

{
  ...
  if (GammaEnergy < LowestEnergyLimit || GammaEnergy > HighestEnergyLimit)
    ...
    crossSection =
      (*theCrossSectionTable)(anElement->GetIndex())->GetValue(GammaEnergy, isOutRange);
...
G4double XTRComptonScattering::GetMeanFreePath(const G4Track& aTrack, G4double, G4ForceCondition*)
...

{
  ...
  if (GammaEnergy > HighestEnergyLimit || GammaEnergy < LowestEnergyLimit)
    ...
    MeanFreePath =
      (*theMeanFreePathTable)(aMaterial->GetIndex())->GetValue(GammaEnergy, isOutRange);
  return MeanFreePath;
}

//....oooOO0OOooo........oooOO0OOooo........oooOO0OOooo........oooOO0OOooo......

G4VParticleChange* XTRComptonScattering::PostStepDoIt(const G4Track& aTrack, const G4Step& aStep)
//
...

{
  ...
  */
  ...
  const vector<G4double>* deltaCutEnergyGamma = theCoupleTable->GetEnergyCutsVector(idxG4GammaCut);
  ...
  if (ElecKineEnergy > fMinElectronEnergy)
  {
    ...
    G4DynamicParticle* aElectron =
      new G4DynamicParticle(G4Electron::Electron(), ElecDirection, ElecKineEnergy);
                                               ...
                                               const G4String& directory, G4bool ascii)
{
  ...
  {
    G4cout << " FAIL theCrossSectionTable->StorePhysicsTable in " << filename << G4endl;
    return false;
  }
  ...
  {
    G4cout << " FAIL theMeanFreePathTable->StorePhysicsTable in " << filename << G4endl;
    return false;
  }

  G4cout << GetProcessName() << " for " << particle->GetParticleName()
         << ": Success to store the PhysicsTables in " << directory << G4endl;
  return true;
}
...
void XTRComptonScattering::PrintInfoDefinition()
{
  ...
  G4cout << G4endl << GetProcessName() << ":  " << comments << "\n        PhysicsTables from "
         << G4BestUnit(LowestEnergyLimit, "Energy") << " to "
         << G4BestUnit(HighestEnergyLimit, "Energy") << " in " << NumbBinTable << " bins. \n";
}
