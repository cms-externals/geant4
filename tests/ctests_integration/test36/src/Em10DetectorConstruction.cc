...
Em10DetectorConstruction::Em10DetectorConstruction()
  :fWorldChanged(false), fAbsorberMaterial(nullptr),  
   fGapMat(nullptr),fSetUp("simpleALICE"),
   fWorldMaterial(nullptr),  fSolidWorld(nullptr),
   fLogicWorld(nullptr), fPhysicsWorld(nullptr),
   fLogicRadSlice(nullptr),
   fSolidRadiator(nullptr), fLogicRadiator(nullptr), fPhysicsRadiator(nullptr),
   fRadiatorMat(nullptr), fPipe(false), fPipeField(false),
   fSolidAbsorber(nullptr), fLogicAbsorber(nullptr), fPhysicsAbsorber(nullptr),
   fMagField(nullptr), fCalorimeterSD(nullptr), fRegGasDet(nullptr), 
   fRadRegion(nullptr), fMat(nullptr)
{
...
G4VPhysicalVolume* Em10DetectorConstruction::SimpleSetUpALICE()
{
  ...
  if( fRadRegion != nullptr ) { delete fRadRegion; } 
