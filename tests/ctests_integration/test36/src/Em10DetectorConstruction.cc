...
Em10DetectorConstruction::Em10DetectorConstruction()
  : fWorldChanged(false),
    ...
    fSolidWorld(nullptr),
    fLogicWorld(nullptr),
    fPhysicsWorld(nullptr),
    fLogicRadSlice(nullptr),
    fSolidRadiator(nullptr),
    fLogicRadiator(nullptr),
    fPhysicsRadiator(nullptr),
    fRadiatorMat(nullptr),
    fPipe(false),
    fPipeField(false),
    fSolidAbsorber(nullptr),
    fLogicAbsorber(nullptr),
    fPhysicsAbsorber(nullptr),
    fMagField(nullptr),
    fCalorimeterSD(nullptr),
    fRegGasDet(nullptr),
    fRadRegion(nullptr),
    fMat(nullptr)
{
...
G4VPhysicalVolume* Em10DetectorConstruction::SimpleSetUpALICE()
{
  ...
  fPhysicsWorld = new G4PVPlacement(0, G4ThreeVector(), "World", fLogicWorld, 0, false, 0);
  ...
  fSolidRadiator =
    new G4Box("Radiator", 1.1 * fAbsorberRadius, 1.1 * fAbsorberRadius, 0.5 * fRadThick);

  fLogicRadiator = new G4LogicalVolume(fSolidRadiator, fRadiatorMat, "Radiator");

  fPhysicsRadiator = new G4PVPlacement(0, G4ThreeVector(0, 0, fRadZ), "Radiator", fLogicRadiator,
                                       fPhysicsWorld, false, 0);

  // create region for window inside windowR for

  if (fRadRegion != nullptr)
  {
    delete fRadRegion;
  }
  ...
  G4Box* solidWindowR = new G4Box("WindowR", fAbsorberRadius + 0.001, fAbsorberRadius + 0.001,
                                  fWindowThick / 2. + 0.001);

  G4LogicalVolume* logicWindowR = new G4LogicalVolume(solidWindowR, fWorldMaterial, "WindowR");

  G4VPhysicalVolume* physiWindowR = new G4PVPlacement(0, G4ThreeVector(0., 0., fWindowZ), "WindowR",
                                                      logicWindowR, fPhysicsWorld, false, 0);
  // window

  G4Box* solidWindow = new G4Box("Window", fAbsorberRadius, fAbsorberRadius, fWindowThick / 2.);

  G4LogicalVolume* logicWindow = new G4LogicalVolume(solidWindow, fWindowMat, "Window");

  // G4VPhysicalVolume*    physiWindow =
  new G4PVPlacement(0, G4ThreeVector(0., 0., 0.), "Window", logicWindow, physiWindowR, false, 0);
  ...
  fSolidAbsorber = new G4Box("Absorber", fAbsorberRadius, fAbsorberRadius, fAbsorberThickness / 2.);

  fLogicAbsorber = new G4LogicalVolume(fSolidAbsorber, fAbsorberMaterial, "Absorber");

  fPhysicsAbsorber = new G4PVPlacement(0, G4ThreeVector(0., 0., fAbsorberZ), "Absorber",
                                       fLogicAbsorber, fPhysicsWorld, false, 0);

  if (fRegGasDet != nullptr)
  {
    delete fRegGasDet;
  }
...
G4VPhysicalVolume* Em10DetectorConstruction::SetUpALICE06()
{
  ...
  */
  ...
  G4double zPipe = zElectrode2 + fElectrodeThick / 2. + pipeDist / 2. + fPipeLength / 2.;

  if (fPipe)
  {
    G4Box* solidPipe =
      new G4Box("Pipe", fAbsorberRadius * 0.5, fAbsorberRadius * 0.5, fPipeLength / 2.);
    ...
    new G4PVPlacement(0, G4ThreeVector(0., 0., zPipe), "Pipe1", logicPipe, fPhysicsWorld, false, 0);
    ...
    if (fPipeField)
    {
      ...
      fMagField = new G4UniformMagField(
        G4ThreeVector(fieldStrength * std::sin(alphaB), 0., fieldStrength * std::cos(alphaB)));
    ...
    G4cout << "No Helium pipe is used" << G4endl << G4endl;
  ...
  G4Box* solidMylar =
    new G4Box("MylarB", fAbsorberRadius * 0.6, fAbsorberRadius * 0.6, fMylarThick / 2.);

  G4LogicalVolume* logicMylar = new G4LogicalVolume(solidMylar, fWindowMat, "MylarL");

  if (fPipe)
  {
    // G4VPhysicalVolume* physiMylar1 =
    new G4PVPlacement(0, G4ThreeVector(0., 0., zMylar1), "Mylar1", logicMylar, fPhysicsWorld, false,
                      0);

    // G4VPhysicalVolume* physiMylar2 =
    new G4PVPlacement(0, G4ThreeVector(0., 0., zMylar2), "Mylar2", logicMylar, fPhysicsWorld, false,
                      0);
  ...
  new G4PVPlacement(0, G4ThreeVector(0., 0., zMylar), "Mylar", logicMylar, fPhysicsWorld, false, 0);
  ...
  if (fRegGasDet != nullptr) delete fRegGasDet;
  fRegGasDet = new G4Region("XTRdEdxDetector");
  fRegGasDet->AddRootLogicalVolume(fLogicAbsorber);
...
G4VPhysicalVolume* Em10DetectorConstruction::SetUpBari05()
{
  ...
  G4Box* solidElectrode =
    new G4Box("Electrode", fAbsorberRadius * 1.1, fAbsorberRadius * 1.1, fElectrodeThick / 2.);

  G4LogicalVolume* logicElectrode = new G4LogicalVolume(solidElectrode, fElectrodeMat, "Electrode");

  G4double zElectrode1 = fRadZ - fRadThick / 2. - fElectrodeThick / 2.;
  G4double zElectrode2 = fRadZ + fRadThick / 2. + fElectrodeThick / 2.;

  // G4VPhysicalVolume*    physiElectrode1 =
  new G4PVPlacement(0, G4ThreeVector(0., 0., zElectrode1), "Electrode1", logicElectrode,
                    fPhysicsWorld, false, 0);

  // G4VPhysicalVolume*    physiElectrode2 =
  new G4PVPlacement(0, G4ThreeVector(0., 0., zElectrode2), "Electrode1", logicElectrode,
                    fPhysicsWorld, false, 0);
  ...
  if (std::fabs(pipe) > 1.e-15)
  {
    ...
    G4LogicalVolume* logicPipe = new G4LogicalVolume(solidPipe, fPipeMat, "Pipe");
  ...
  G4Box* solidMylar =
    new G4Box("Mylar", fAbsorberRadius * 0.6, fAbsorberRadius * 0.6, fMylarThick / 2.);

  G4LogicalVolume* logicMylar = new G4LogicalVolume(solidMylar, fWindowMat, "Mylar");
  ...
  fSolidAbsorber = new G4Box("Absorber",
                             ...
                             10. * mm, 10. * mm, fAbsorberThickness / 2.);
