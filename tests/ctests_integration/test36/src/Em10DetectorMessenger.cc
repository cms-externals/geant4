...
void Em10DetectorMessenger::SetNewValue(G4UIcommand* command, G4String newValue)
{
  ...
  {
    Em10Detector->SetRadiatorThickness(RadiatorThickCmd->GetNewDoubleValue(newValue));
  }

  if (command == GasGapThickCmd)
  {
    Em10Detector->SetGasGapThickness(GasGapThickCmd->GetNewDoubleValue(newValue));
  }
