...
Em10CalorimeterSD::Em10CalorimeterSD(G4String name, Em10DetectorConstruction* det)
...
{
  CalCollection = new Em10CalorHitsCollection(SensitiveDetectorName, collectionName[0]);
  for (G4int j = 0; j < 1; j++)
  {
    HitID[j] = -1;
  };
...
G4bool Em10CalorimeterSD::ProcessHits(G4Step* aStep, G4TouchableHistory*)
{
  ...
  if (HitID[Em10Number] == -1)
  {
    ...
    if (verboseLevel > 0) G4cout << " New Calorimeter Hit on Em10: " << Em10Number << G4endl;
  }
  else
  {
    if (physVol == Detector->GetAbsorber())
      (*CalCollection)[HitID[Em10Number]]->AddAbs(edep, stepl);
    if (verboseLevel > 0) G4cout << " Energy added to Em10: " << Em10Number << G4endl;
  }
