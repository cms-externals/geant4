...
XTRPhotoElectricEffect::XTRPhotoElectricEffect(const G4String& processName, G4ProcessType type)
  : G4VDiscreteProcess(processName, type), fMinElectronEnergy(1 * eV)
...
inline G4bool XTRPhotoElectricEffect::IsApplicable(const G4ParticleDefinition& particle)
{
...
// returns the photoElectric cross Section in GEANT4 internal units
{
  ...
  mat->GetSandiaTable()->GetSandiaCofPerAtom((G4int)AtomicNumber, GammaEnergy, SandiaCof);

  G4double energy2 = GammaEnergy * GammaEnergy, energy3 = GammaEnergy * energy2,
           energy4 = energy2 * energy2;

  return SandiaCof[0] / GammaEnergy + SandiaCof[1] / energy2 + SandiaCof[2] / energy3
         + SandiaCof[3] / energy4;
}

//....oooOO0OOooo........oooOO0OOooo........oooOO0OOooo........oooOO0OOooo......

G4double XTRPhotoElectricEffect::ComputeMeanFreePath(G4double GammaEnergy, G4Material* aMaterial)

// returns the gamma mean free path in GEANT4 internal units
{
  const G4double* SandiaCof = aMaterial->GetSandiaTable()->GetSandiaCofForMaterial(GammaEnergy);

  G4double energy2 = GammaEnergy * GammaEnergy, energy3 = GammaEnergy * energy2,
           energy4 = energy2 * energy2;

  G4double SIGMA = SandiaCof[0] / GammaEnergy + SandiaCof[1] / energy2 + SandiaCof[2] / energy3
                   + SandiaCof[3] / energy4;
...
G4double XTRPhotoElectricEffect::GetMeanFreePath(const G4Track& aTrack, G4double, G4ForceCondition*)

// returns the gamma mean free path in GEANT4 internal units
{
  G4double GammaEnergy = aTrack.GetDynamicParticle()->GetKineticEnergy();
  const G4double* SandiaCof =
    aTrack.GetMaterial()->GetSandiaTable()->GetSandiaCofForMaterial(GammaEnergy);
...
G4VParticleChange* XTRPhotoElectricEffect::PostStepDoIt(const G4Track& aTrack, const G4Step& aStep)
//
...

{
  ...
  const G4Element* anElement = SelectRandomAtom(aDynamicPhoton, aMaterial);
  ...
  G4int i = 0;
  while ((i < NbOfShells) && (PhotonEnergy < anElement->GetAtomicShell(i)))
    ++i;
...
const G4Element* XTRPhotoElectricEffect::SelectRandomAtom(const G4DynamicParticle* aDynamicPhoton,
                                                          const G4Material* aMaterial)
{
  ...
  {
    PartialSumSigma +=
      NbOfAtomsPerVolume[elm]
      * ComputeCrossSectionPerAtom(GammaEnergy, (*theElementVector)[elm]->GetZ(), aMaterial);
