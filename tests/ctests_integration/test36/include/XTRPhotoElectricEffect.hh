...

{
  ...
  public:  // with description
    ...
    G4double ComputeCrossSectionPerAtom(G4double PhotonEnergy, G4double AtomicNumber,
                                        const G4Material*);

    G4double ComputeMeanFreePath(G4double PhotonEnergy, G4Material* aMaterial);

    inline void SetMinElectronEnergy(G4double E) { fMinElectronEnergy = E; };

  protected:

    virtual G4double ElecThetaDistribution(G4double ElecKineEnergy);

  private:

    const G4Element* SelectRandomAtom(const G4DynamicParticle* aDynamicPhoton,
