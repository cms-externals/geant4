...
G4VPhysicalVolume* Tst14DetectorConstruction::Construct()
{
  ...
  // modifided sizes and position
  if (physiWorld)
  {
    solidWorld->SetOuterRadius(WorldSizeR);
    solidWorld->SetZHalfLength(WorldSizeZ / 2.);

    solidAbsorber->SetOuterRadius(AbsorberRadius);
    solidAbsorber->SetZHalfLength(AbsorberThickness / 2.);

    physiAbsorber->SetTranslation(G4ThreeVector(0., 0., zAbsorber));
...
void Tst14DetectorConstruction::SetAbsorberMaterial(G4String materialChoice)
{
  ...
  if (pttoMaterial && AbsorberMaterial != pttoMaterial)
  {
    ...
    if (logicAbsorber)
    {
      logicAbsorber->SetMaterial(AbsorberMaterial);
    }
