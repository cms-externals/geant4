...
void Tst14ProcCallSA::execute(const G4Step* aStep)
{
  ...
  const G4Material* material = postStepPoint->GetMaterial();
  G4String matname = material->GetName();

  G4String index = procname + G4String(" for ") + particleType + G4String(" in ") + matname;
  calls[index]++;
  ...
  if (start[sIndex] == 0)
  {
    G4String name = G4String("All ") + sIndex + G4String(" : logE (MeV)");
