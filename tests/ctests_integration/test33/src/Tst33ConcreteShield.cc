...
Tst33ConcreteShield::Tst33ConcreteShield() : fWorldVolume(0), fConcrete(0), fGalactic(0) {}
...
void Tst33ConcreteShield::Construct()
{
  ...
  if (!fWorldVolume)
  {
    G4Exception("Tst33ConcreteShield::Construct()", "TST33-03", FatalException,
  ...
  G4VPhysicalVolume* pvol = new G4PVPlacement(0, G4ThreeVector(pos_x, pos_y, pos_z), aShield_log,
                                              name, worldCylinder_log, false, 0, true);
...
G4GeometryCell Tst33ConcreteShield::GetGeometryCell(G4int, const G4String&) const
{
