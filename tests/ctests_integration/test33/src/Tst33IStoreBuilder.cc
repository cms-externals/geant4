...
G4IStore* Tst33IStoreBuilder::CreateIStore(Tst33VGeometry* samplegeo, G4bool paraGeom)
{
  ...
  if (!istore)
  {
    G4Exception("Tst33IStoreBuilder::CreateIStore()", "TST33-04", FatalException,
                " new failed to create G4IStore!");
  }
  ...
  for (i = 1; i <= 19; ++i)
  {
    ...
    else
    {
      ...
      istore->AddImportanceGeometryCell(
        imp, gCellMinus.GetPhysicalVolume(),
        i);  // ASO - change overloading of AddImportanceGeometryCell?
