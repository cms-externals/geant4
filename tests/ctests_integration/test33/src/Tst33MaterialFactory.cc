...
void Tst33MaterialFactory::FillElementMap(const G4String& name, const G4String& symbol, G4int Z,
                                          G4double A)
{
  Tst33MapSymbolElement::iterator it = fMapSymbolElement.find(symbol);
  if (it != fMapSymbolElement.end())
  {
    G4cout << "Tst33MaterialFactory::FillElementMap: symbol: " << symbol << ", already defined"
           << G4endl;
  }
  else
  {
    ...
    {
      G4Exception("Tst33MaterialFactory::FillElementMap()", "TST33-05", FatalException,
                  " new failed to create G4Element!");
    }
  }
  return;
}
...
G4Material* Tst33MaterialFactory::CreateConcrete()
{
  ...
  Concrete = new G4Material("Concrete", density, fConcreteFractions.size());
...
G4Material* Tst33MaterialFactory::CreateLightConcrete()
{
  ...
  LightConcrete = new G4Material("LightConcrete", density, fConcreteFractions.size());
  for (Tst33MapElementFraction::iterator it = fConcreteFractions.begin();
       it != fConcreteFractions.end(); ++it)
  {
...
G4Material* Tst33MaterialFactory::CreateGalactic()
{
  ...
  return new G4Material("Galactic", 1., 1.01 * g / mole, density, kStateGas, temperature, pressure);
}
