...
void Tst33TimedEventAction::EndOfEventAction(const G4Event*)
{
  ...
  if (fProcessTime >= fMaxRunTime)
  {
    ...
    G4cout << "Tst33TimedEventAction::EndOfEventAction: aborted after"
           << " running for: " << fProcessTime << " (mu)seconds" << G4endl;
    CalculateFOM();
    fSig.Init();
    fProcessTime = 0.;
    fOld_lwe = 0;
...
void Tst33TimedEventAction::CalculateFOM()
{
  ...
  G4cout << "Tst33TimedEventAction::CalculateFOM(): FOM: " << G4endl;
  G4cout << "    mean= " << mean << ", error= " << R << ", fom*time= " << fom
         << ", time= " << fProcessTime << ", n= " << fSig.GetEntries() << G4endl;
}
