...
Tst33ParallelGeometry::Tst33ParallelGeometry(G4String worldName, G4VPhysicalVolume* ghostworld)
  : G4VUserParallelWorld(worldName), fLogicalVolumeVector(), fGalactic(0)
{
...
void Tst33ParallelGeometry::Construct()
{
  ...
  */
  ...
  G4LogicalVolume* aShield_logI1 = new G4LogicalVolume(aShieldI1, fGalactic, "aShieldI1_log");

  fLogicalVolumeVector.push_back(aShield_logI1);  // ASO

  G4LogicalVolume* aShield_logI2 = new G4LogicalVolume(aShieldI1, fGalactic, "aShieldI2_log");
  ...
  for (i = 1; i <= 18; ++i)
  {
    ...
    G4LogicalVolume* aShield_log = new G4LogicalVolume(aShield, fGalactic, "aShield_log");
  ...
  G4VPhysicalVolume* pvol = new G4PVPlacement(0, G4ThreeVector(pos_x, pos_y, pos_z), aRest_log,
                                              name, worldCylinder_log, false, 19, true);  // ASO???
  //		      0);
  G4GeometryCell cell(*pvol, 0);
  fPVolumeStore.AddPVolume(cell);
...
G4GeometryCell Tst33ParallelGeometry::GetGeometryCell(G4int i, const G4String& nameExt) const
{
