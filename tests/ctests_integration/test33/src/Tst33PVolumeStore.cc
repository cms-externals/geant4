...
{
  Tst33SetGeometryCell::iterator it = fSetGeometryCell.find(cell);
  if (it != fSetGeometryCell.end())
  {
    G4cout << "Tst33PVolumeStore::AddPVolume: cell already stored" << G4endl;
    return;
  }
...
G4GeometryCell Tst33PVolumeStore::GetGeometryCell(G4int i, const G4String& nameExt) const
{
  ...
  {
    G4cout << "Tst33PVolumeStore::GetGeometryCell: no G4GeometryCell named: " << cellName
           << ", found" << G4endl;
    G4Exception("Tst33PVolumeStore::GetGeometryCell()", "TST33-07", FatalException,
                "G4GeometryCell not found");
  }
  return *pCell;
}
