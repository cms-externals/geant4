...
void Tst33AppStarter::CreateMassGeometry()
{
  ...
  else
  {
    ...
    fDetectorConstruction->SetWorldVolume(&fMassGeometry->GetWorldVolumeAddress());
  }
...
void Tst33AppStarter::CreateParallelGeometry()
{
  ...
  else
  {
    ...
    fParallelGeometry =
      new Tst33ParallelGeometry(parallelName, &fMassGeometry->GetWorldVolumeAddress());
    if (!fParallelGeometry)
    {
...
G4bool Tst33AppStarter::CheckCreateIStore()
{
  ...
  // if (fIStore) {
  //   G4cout << "Tst33AppStarter::CheckCreateIStore: an importance store already exists!" <<
  //   G4endl; createistore = false;
  // }
  return createistore;
}
...
G4bool Tst33AppStarter::CheckCreateWeightWindowStore()
{
  ...
  // if (fWWStore) {
  //   G4cout << "Tst33AppStarter::CheckCreateWeightWindowStore: an weight window store already
  //   exists!" << G4endl; createWWstore = false;
  // }
...
void Tst33AppStarter::CreateWeightWindowStore(G4PlaceOfAction poa, G4bool zeroWindow)
{
  ...
  if (CheckCreateWeightWindowStore())
  {
    ...
    fSampler->PrepareWeightWindow(G4WeightWindowStore::GetInstance(), fWWAlg, poa);
    //				  onBoundary);
...
G4bool Tst33AppStarter::CheckCreateWeightRoulette()
{
  ...
  if (fWeightroulette)
  {
    G4cout << "Tst33AppStarter::CheckCreateWeightRoulette: weigyht roulette already exists!"
           << G4endl;
    createWR = false;
  }
  return createWR;
...
void Tst33AppStarter::CreateVisApplication()
{
  if (CheckCreateApp())
  {
    ...
    if (parallel_geometry)
    {
      ...
      //    fSampler = new G4GeometrySampler(&fParallelGeometry->GetWorldVolume(),
      //			       "neutron");
      fSampler = new G4GeometrySampler(&fParallelGeometry->GetWorldVolumeAddress(), "neutron");
...
void Tst33AppStarter::NewFailed(const G4String& function, const G4String& cl)
{
