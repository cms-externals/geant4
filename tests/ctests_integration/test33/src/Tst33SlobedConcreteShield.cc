...
void Tst33SlobedConcreteShield::Construct()
{
  ...
  G4Tubs* worldCylinder = new G4Tubs("worldCylinder", innerRadiusCylinder, outerRadiusCylinder,
                                     halfheightCylinder, startAngleCylinder, spanningAngleCylinder);
  ...
  fWorldVolume =
    new G4PVPlacement(0, G4ThreeVector(0, 0, 0), worldCylinder_log, name, 0, false, 0, true);
  if (!fWorldVolume)
  {
    G4Exception("Tst33SlobedConcreteShield::Construct()", "TST33-09", FatalException,
                " new failed to create G4PVPlacement!");
  }
  ...
  G4Tubs* aShield = new G4Tubs("aShield", innerRadiusShield, outerRadiusShield, halfheightShield,
                               startAngleShield, spanningAngleShield);

  G4Tubs* aShieldI1 = new G4Tubs("aShieldI1", innerRadiusShield, 50 * cm, 1 * cm, startAngleShield,
                                 spanningAngleShield);

  // logical shield

  G4LogicalVolume* aShield_logI1 = new G4LogicalVolume(aShieldI1, fConcrete, "aShieldI1_log");
  fLogicalVolumeVector.push_back(aShield_logI1);  // ASO

  G4LogicalVolume* aShield_logI2 = new G4LogicalVolume(aShieldI1, fConcrete, "aShieldI2_log");
  fLogicalVolumeVector.push_back(aShield_logI2);  // ASO

  G4VisAttributes* pShieldVis = new G4VisAttributes(G4Colour(0.5, 0.5, 0.5));
  ...
  for (i = 1; i <= 18; i++)
  {
    ...
    G4LogicalVolume* aShield_log = new G4LogicalVolume(aShield, fConcrete, "aShield_log");

    fLogicalVolumeVector.push_back(aShield_log);  // ASO

    G4VPhysicalVolume* pvolIMinus =
      new G4PVPlacement(0, G4ThreeVector(0, 0, -0.5 * halfheightShield), aShield_logI1,
                        name + "I1-", aShield_log, false, i, true);  // ASO
    //			0);
    fPhysicalVolumeVector.push_back(pvolIMinus);

    G4VPhysicalVolume* pvolIPlus =
      new G4PVPlacement(0, G4ThreeVector(0, 0, +0.5 * halfheightShield), aShield_logI2,
                        name + "I1+", aShield_log, false, i, true);  // ASO
    //			0);
    ...
    {
      G4cout
        << "Tst33SlobedConcreteShield::Construct: Error: pvolIPlus is not an ancestor of pvolIMinus"
        << G4endl;
    }
    ...
    G4VPhysicalVolume* pvol = new G4PVPlacement(0, G4ThreeVector(pos_x, pos_y, pos_z), aShield_log,
                                                name, worldCylinder_log, false, i, true);  // ASO
    //			0);
  ...
  G4Tubs* aRest = new G4Tubs("Rest", innerRadiusShield, outerRadiusShield, halfheightShield,
                             startAngleShield, spanningAngleShield);

  G4LogicalVolume* aRest_log = new G4LogicalVolume(aRest, fGalactic, "aRest_log");
  ...
  G4VPhysicalVolume* pvol_rest =
    new G4PVPlacement(0, G4ThreeVector(pos_x, pos_y, pos_z), aRest_log, name, worldCylinder_log,
                      false, 19, true);  // ASO???
  //		      0);
...
G4GeometryCell Tst33SlobedConcreteShield::GetGeometryCell(G4int i, const G4String& nameExt) const
{
