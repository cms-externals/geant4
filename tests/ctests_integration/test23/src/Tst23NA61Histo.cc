...
Tst23NA61Histo::Tst23NA61Histo(G4String htitle) : TstHistoSet(htitle)
{
  G4String title;
  G4String outcome;
...
void Tst23NA61Histo::FillEvt(G4VParticleChange* aChange, const G4LorentzVector&,
                             const G4LorentzVector&)
{
  ...
  for (G4int i = 0; i < NSec; i++)
  {
    ...
    if (pname == "proton" || pname == "pi+" || pname == "pi-" || pname == "kaon+"
        || pname == "kaon-")
    {
      if (theta >= 0. && theta < 10.)
      {
...
void Tst23NA61Histo::Write(G4int, G4double xsec)
{
  ...
  // NOTE: bear in mind that fHistoNSec->Integral() is is the number of entries WITHIN histo
  // xmin-xmax
  //       while fHistoNSec->GetEntries() is the total number of entries, incl. under/overflow (i.e.
  //       the actual NEvents) however, AFTER the NORMALIZATION of fHistoSec, GetEntries() and
  //       Integral() give the same result
  //
