...
void Tst23SteppingAction::UserSteppingAction(const G4Step* theStep)
{
  ...
  /*
     // the beam particle can go through without interactions or at least without changing its
     identity;
     ...
     // but even then there isn't much way to say what exactly caused it because there's a "model"
     that
  ...
  */
  ...
  const TstPrimaryGeneratorAction* beam = dynamic_cast<const TstPrimaryGeneratorAction*>(
