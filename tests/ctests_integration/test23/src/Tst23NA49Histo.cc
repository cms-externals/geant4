...
Tst23NA49Histo::Tst23NA49Histo(G4String htitle) : TstHistoSet(htitle), TstHistoSetForNu()
{
  G4String title;
  G4String outcome;
  G4String hname;
  ...
  fHistoSecProtonTot.push_back(
    new TProfile("proton_pT_total", title.c_str(), 100, -1., 1., 0., 10.));
  fHistoSecProtonTot.push_back(
    new TProfile("proton_pT2_total", title.c_str(), 100, -1., 1., 0., 10.));
  ...
  //
  double pbarbins[] = {-0.55,   -0.45,   -0.35,   -0.25,  -0.175, -0.125, -0.0875,
                       -0.0625, -0.0375, -0.0125, 0.0125, 0.0375, 0.075,  0.125,
                       0.175,   0.25,    0.35,    0.45,   0.55};
  ...
  fHistoSecAntiProton.push_back(
    new TProfile("antiproton_pT", title.c_str(), npbarbins, pbarbins, 0., 10.));
  fHistoSecAntiProton.push_back(
    new TProfile("antiproton_pT2", title.c_str(), npbarbins, pbarbins, 0., 10.));

  fHistoPTvsXFAntiProton = 0;

  fHistoSecAntiProtonTot.push_back(
    new TH1D("antiproton_dXSecdxF_total", title.c_str(), 40, -1., 1.));
  fHistoSecAntiProtonTot.push_back(new TH1D("antiproton_dNdxF_total", title.c_str(), 40, -1., 1.));
  fHistoSecAntiProtonTot.push_back(
    new TProfile("antiproton_pT_total", title.c_str(), 100, -1., 1., 0., 10.));
  fHistoSecAntiProtonTot.push_back(
    new TProfile("antiproton_pT2_total", title.c_str(), 100, -1., 1., 0., 10.));
  ...
  double pibins[] = {-0.550,  -0.450,  -0.350, -0.275, -0.225, -0.175, -0.1375, -0.1125,
                     -0.0875, -0.0625, -0.045, -0.035, -0.025, -0.015, -0.005,  0.005,
                     0.015,   0.025,   0.035,  0.045,  0.0625, 0.0875, 0.1125,  0.1375,
                     0.175,   0.225,   0.275,  0.350,  0.450,  0.55};
  int npibins = sizeof(pibins) / sizeof(double) - 1;
  //  double pibins_pt = { 0.05, 0.1, 0.15, 0.2, 0.25, 0.3, 0.4, 0.5, 0.6, 0.7, 0.8,
  //  0.9, 1.0, 1.2, 1.4, 1.6, 1.8 }
  double pibins_pt[] = {0.025, 0.075, 0.125, 0.175, 0.225, 0.275, 0.35, 0.45, 0.55,
                        0.65,  0.75,  0.85,  0.95,  1.1,   1.3,   1.5,  1.7};
  ...
  fHistoSecPiMinus.push_back(
    new TProfile("piminus_pT", title.c_str(), fNPiBinsXF - 1, fPiBinsXF, 0., 10.));
  fHistoSecPiMinus.push_back(new TProfile("piminus_pT2", title.c_str(), npibins, pibins, 0., 10.));

  fHistoPTvsXFPiMinus =
    new TH2D("piminis_pTvsxF", title.c_str(), npibins, pibins, npibins_pt, pibins_pt);
  ...
  for (int nb = 0; nb < fNPiBinsXF; ++nb)
  {
    ...
    fHistoPTPiMinus.push_back(
      new TH1D(hname.c_str(), (title + subtitle).c_str(), npibins_pt, pibins_pt));
  }
  ...
  fHistoSecPiMinusTot.push_back(
    new TProfile("piminus_pT_total", title.c_str(), 100, -1., 1., 0., 10.));
  fHistoSecPiMinusTot.push_back(
    new TProfile("piminus_pT2_total", title.c_str(), 100, -1., 1., 0., 10.));
  ...
  for (NuERange r = kR_0_2; r <= kR_20_50; r = NuERange(r + 1))
  {
    ...
    fHistoPiMinusFrom1stInt[r].push_back(
      new TProfile(hname.c_str(), title.c_str(), npibins, pibins, 0., 10.));

    hname = "sec_proton_energy_to_pim_" + osCount.str();
    fHistoPiMinusFromReint[kFromP][r].push_back(
      new TH1D(hname.c_str(), title.c_str(), 300, 0., 150.));

    hname = "sec_pim_energy_to_pim_" + osCount.str();
    fHistoPiMinusFromReint[kFromPim][r].push_back(
      new TH1D(hname.c_str(), title.c_str(), 300, 0., 150.));

    hname = "sec_pip_energy_to_pim_" + osCount.str();
    fHistoPiMinusFromReint[kFromPip][r].push_back(
      new TH1D(hname.c_str(), title.c_str(), 300, 0., 150.));
  }
  ...
  fHistoSecPiPlus.push_back(
    new TProfile("piplus_pT", title.c_str(), fNPiBinsXF - 1, fPiBinsXF, 0., 10.));
  fHistoSecPiPlus.push_back(new TProfile("piplus_pT2", title.c_str(), npibins, pibins, 0., 10.));

  fHistoPTvsXFPiPlus =
    new TH2D("piplus_pTvsxF", title.c_str(), npibins, pibins, npibins_pt, pibins_pt);
  ...
  fHistoSecPiPlusTot.push_back(
    new TProfile("piplus_pT_total", title.c_str(), 100, -1., 1., 0., 10.));
  fHistoSecPiPlusTot.push_back(
    new TProfile("piplus_pT2_total", title.c_str(), 100, -1., 1., 0., 10.));
  ...
  for (NuERange r = kR_0_2; r <= kR_20_50; r = NuERange(r + 1))
  {
    ...
    fHistoPiPlusFrom1stInt[r].push_back(
      new TProfile(hname.c_str(), title.c_str(), npibins, pibins, 0., 10.));

    hname = "sec_proton_energy_to_pip_" + osCount.str();
    fHistoPiPlusFromReint[kFromP][r].push_back(
      new TH1D(hname.c_str(), title.c_str(), 300, 0., 150.));
    //     hname = "from_p_" + pip_names[0] + osCount.str();
    //     fHistoPiPlusFromReint[kFromP][r].push_back( new TH1D( hname.c_str(), title.c_str(), 40,
    //     -1., 1. ) ); hname = "from_p_" + pip_names[1] + osCount.str();
    //     fHistoPiPlusFromReint[kFromP][r].push_back( new TProfile( hname.c_str(),   title.c_str(),
    //     100, -1., 1., 0., 10. ) );

    hname = "sec_pim_energy_to_pip_" + osCount.str();
    fHistoPiPlusFromReint[kFromPim][r].push_back(
      new TH1D(hname.c_str(), title.c_str(), 300, 0., 150.));
    //     hname = "from_pim_" + pip_names[0] + osCount.str();
    //     fHistoPiPlusFromReint[kFromPip][r].push_back( new TH1D( hname.c_str(), title.c_str(), 40,
    //     -1., 1. ) ); hname = "from_pim_" + pip_names[1] + osCount.str();
    //     fHistoPiPlusFromReint[kFromPip][r].push_back( new TProfile( hname.c_str(), title.c_str(),
    //     100, -1., 1., 0., 10. ) );

    hname = "sec_pip_energy_to_pip_" + osCount.str();
    fHistoPiPlusFromReint[kFromPip][r].push_back(
      new TH1D(hname.c_str(), title.c_str(), 300, 0., 150.));
    //     hname = "from_pip_" + pip_names[0] + osCount.str();
    //     fHistoPiPlusFromReint[kFromPip][r].push_back( new TH1D( hname.c_str(), title.c_str(), 40,
    //     -1., 1. ) ); hname = "from_pip_" + pip_names[1] + osCount.str();
    //     fHistoPiPlusFromReint[kFromPip][r].push_back( new TProfile( hname.c_str(), title.c_str(),
    //     100, -1., 1., 0., 10. ) );
  }
...
void Tst23NA49Histo::FillEvt(G4VParticleChange* aChange, const G4LorentzVector&,
                             const G4LorentzVector& labp)
{
  ...
  /*
     ...
     // well, this way sqrt(s) will actually end up at 17.27 which is a bit more than 17.2 (taken
     from SPS hadrons site)
     ...
     Ecm   =
     sqrt(2.0*pow(G4Proton::Proton()->GetPDGMass()/GeV,2.0)+2.*BeamEnergy*G4Proton::Proton()->GetPDGMass()/GeV);
     beta  =
     sqrt(pow(BeamEnergy,2.0)-pow(G4Proton::Proton()->GetPDGMass()/GeV,2.0))/(BeamEnergy+G4Proton::Proton()->GetPDGMass()/GeV);
  ...
  for (G4int i = 0; i < NSec; i++)
  {
    ...
    else if (pname == "pi-")
    {
      ...
      else  // reinteraction
      {
        ...
        fHistoPiMinusFromReint[IntType][fNubarERange][0]->Fill(incomingtrk->GetVertexKineticEnergy()
                                                               / GeV);
      }
    }
    else if (pname == "pi+")
    {
      ...
      {
        if (IntType == kOther) continue;
        fHistoPiPlusFromReint[IntType][fNuERange][0]->Fill(incomingtrk->GetVertexKineticEnergy()
                                                           / GeV);
      }
    }
...
void Tst23NA49Histo::Write(G4int, G4double)
{
  ...
  for (size_t i = 0; i < fHistoPTPiMinus.size(); ++i)
  {
    fHistoPTPiMinus[i]->Scale(1.
                              / norm);  // Note: NO scaling with "width" because it had to be E/dP3
  ...
  for (size_t i = 0; i < fHistoPTPiPlus.size(); ++i)
  {
    fHistoPTPiPlus[i]->Scale(
      1. / norm);  // NO scaling with "width" - see earlier comment for fHistoPTPiMinus
...
double Tst23NA49Histo::CalculateBinWeight(const G4LorentzVector& labp, double Ekin, double mass,
                                          double pT, int xFbin, double sqrt_s)
{
  double wei = 1.;
  int pTbin = -1;
