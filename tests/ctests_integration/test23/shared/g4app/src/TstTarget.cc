...
G4VPhysicalVolume* TstTarget::Construct()
{
  ...
  fWorld =
    new G4PVPlacement(0, G4ThreeVector(), "world_phys",
                      new G4LogicalVolume(new G4Box("world_solid", 10. * m, 10. * m, 10. * m), mat,
                                          "world_log", 0, 0, 0),  // mat=Vacuum !!!
                      0, false, 0);

  // G4Box* solidSubWorld = new G4Box( "subworld_solid", 9.999*m, 9.999*m, 9.999*m );
  fSubWorld = new G4PVPlacement(
    0, G4ThreeVector(), "subworld_phys",
    new G4LogicalVolume(new G4Box("subworld_solid", 9.999 * m, 9.999 * m, 9.999 * m), mat,
                        "subworld_log", 0, 0, 0),  // mat=Vacuum again
    fWorld, false, 0);
...
G4VPhysicalVolume* TstTarget::ConstructTarget()
{
  ...
  */
  ...
  if (fShape == "G4Box")
  {
    solid = new G4Box("target_solid", 0.5 * fX1, 0.5 * fX2,
                      0.5 * fX3);  // size is already done in the right units (in TstReader)
  }
  ...
  fPhysTarget = new G4PVPlacement(0, G4ThreeVector(), "target_phys", fLogTarget,
...
G4double TstTarget::GetRMax() const
{
  double RMax = 0;

  if (fShape == "G4Box")
  {
    RMax = 0.5 * std::min(fX1, fX2);
  }
  else if (fShape == "G4Tubs")
  {
    RMax = fX2;
