  ...
  public:
    ...
    Beam()
      : fBeamPartName(""),
        fBeamPartMass(0.),
        fBeamEnergy(0.),
        fBeamMomentum(G4ThreeVector()),
        fBeamPosition(G4ThreeVector()),  // new for test47
        fLabV(G4LorentzVector()),
        fLabP(G4LorentzVector()),
        fBeamKineByEvt(false),
        fdpByp(0.)  // new - for test47
    {  // new - for test47
    ...
    void SetBeam(G4String name, G4double mass, G4double energy)
    {
      fBeamPartName = name;
      fBeamPartMass = mass;
      fBeamEnergy = energy;
    ...
    void SetLabV(G4LorentzVector lv)
    {
      fLabV = lv;
      return;
    }
    void SetLabP(G4LorentzVector lp)
    {
      fLabP = lp;
      return;
    }
    ...
    // new - for test47
    //
    void SetAngleRange(G4double x, G4double dx, G4double y, G4double dy)
    {
      fAngleX[0] = x;
      fAngleX[1] = dx;
      fAngleY[0] = y;
    ...
    void SetBeamMomentum(const G4ThreeVector& bmom)
    {
      fBeamMomentum = bmom;
      return;
    }
    void SetdpByp(G4double dp)
    {
      fdpByp = dp;
      return;
    }
    ...
    void SetPosSigma(G4double dx, G4double dy)
    {
      fPosSigma[0] = dx;
      fPosSigma[1] = dy;
      return;
    ...
    void ResetBeamEnergy(G4double e)
    {
      fBeamEnergy = e;
      return;
    }

    void BeamKineByEvtOn()
    {
      fBeamKineByEvt = true;
      return;
    }
