...
void SQLCompose(int tdid, string beam, string target, string energy, string momentum,
                string secondary, string model, string g4version, string histoname,
                string reaction_details, string xlabel, string ylabel,  // x-y axis label(s)
                string experiment, string observable, string status,
                int mask)  // mask=0 -> energy; mask=1 -> momentum
{
  ...
  if (!experiment.empty())
  {
    for (unsigned int i = 0; i < experiment.size(); ++i)
      experiment[i] = tolower(experiment[i]);
  ...
  sqlfile << "beammomentum,beamenergy,secondary,observable,xdes,ydes,nbins,minx,maxx,binvalues,"
             "bincenter,binwidth,errorup,errorlow,mask)";
  sqlfile << " VALUES (";
  sqlfile << tdid << ",";
  sqlfile << "'" << tname << "',";
  sqlfile << "'" << g4version << "',";
...
string ComposeG4Tag(string g4version)
{
  ...
  if (itr3 == string::npos)  // no patch or ref
  {
    ...
    for (size_t i = itr2 + 1; i < g4version.length(); ++i)
      g4tag += g4version[i];
    return g4tag;
  }

  if ((itr3 - itr2) < 3) g4tag += "0";
  for (size_t i = itr2 + 1; i < itr3; ++i)
    g4tag += g4version[i];
  ...
  if (reltype != string::npos)
  {
    ...
    //
    g4tag += "-ref-";
    for (size_t i = reltype + 3; i < g4version.length(); ++i)
      g4tag += g4version[i];
    return g4tag;
  }
  ...
  if (reltype != string::npos)
  {
    ...
    //
    g4tag += "-patch-";
    for (size_t i = reltype + 1; i < g4version.length(); ++i)
      g4tag += g4version[i];
    return g4tag;
  }
...
string ComposeFileNameGeneric(string beam, string target, string momentum, string model)
{
  string fname = "";
...
string ComposeFileNameTest47(string beam, string target, string momentum, string model)
{
  string fname = "";
