...
UrbanMscModel93::UrbanMscModel93(const G4String& nam) : G4VMscModel(nam)
{
  ...
  coeffc1 = 0.;
  coeffc2 = 0.;
  scr1ini = fine_structure_const * fine_structure_const * electron_mass_c2 * electron_mass_c2
            / (0.885 * 0.885 * 4. * pi);
...
void UrbanMscModel93::Initialise(const G4ParticleDefinition* p, const G4DataVector&)
{
  ...
  if (p->GetPDGMass() > MeV)
  {
    G4cout << "### WARNING: UrbanMscModel93 model is used for " << p->GetParticleName() << " !!! "
           << G4endl;
...
G4double UrbanMscModel93::ComputeCrossSectionPerAtom(const G4ParticleDefinition* part,
                                                     ...
                                                     G4double, G4double, G4double)
{
  ...
  static const G4double Tdat[22] = {100 * eV,  200 * eV,  400 * eV,  700 * eV,  1 * keV,  2 * keV,
  ...
  static const G4double bg2lim =
    Tlim * (Tlim + 2. * electron_mass_c2) / (electron_mass_c2 * electron_mass_c2);

  static const G4double sig0[15] = {0.2672 * barn, 0.5922 * barn, 2.653 * barn, 6.235 * barn,
  ...
  G4double beta2 =
    eKineticEnergy * (eTotalEnergy + electron_mass_c2) / (eTotalEnergy * eTotalEnergy);
  G4double bg2 =
    eKineticEnergy * (eTotalEnergy + electron_mass_c2) / (electron_mass_c2 * electron_mass_c2);
                                                     ...
                                                     G4double& currentMinimalStep)
{
  ...
  if (steppingAlgorithm == fUseDistanceToBoundary)
  {
    ...
    else if (geomlimit < geombig)
    {
      ...
      {
        if (tlimit > geomlimit - 0.999 * skindepth) tlimit = geomlimit - 0.999 * skindepth;
      }
      else
      {
        insideskin = true;
  ...
  else if (steppingAlgorithm == fUseSafety)
  {
    ...
    if (firstStep || stepStatus == fGeomBoundary)
    {
      ...
      if (mass < masslimite)
      {
        if (lambda0 > currentRange) rangeinit = lambda0;
        if (lambda0 > lambdalimit) fr *= 0.75 + 0.25 * lambda0 / lambdalimit;
      }
...
G4double UrbanMscModel93::ComputeTrueStepLength(G4double geomStepLength)
{
  ...
  if (geomStepLength == zPathLength && tPathLength <= currentRange) return tPathLength;
...
G4double UrbanMscModel93::ComputeTheta0(G4double trueStepLength, G4double KineticEnergy)
{
  ...
  G4double betacp =
    sqrt(currentKinEnergy * (currentKinEnergy + 2. * mass) * KineticEnergy
         * (KineticEnergy + 2. * mass) / ((currentKinEnergy + mass) * (KineticEnergy + mass)));
...
G4double UrbanMscModel93::SampleCosineTheta(G4double trueStepLength, G4double KineticEnergy)
{
  ...
  else
  {
    ...
    {
      if (par1 * trueStepLength < 1.) tau = -par2 * log(1. - par1 * trueStepLength);
    ...
    else if (tau >= tausmall)
    {
      ...
      if (relloss > rellossmax) return SimpleScattering(xmeanth, x2meanth);
      ...
      if (G4UniformRand() < qprob)
      {
        ...
        if (KineticEnergy > 5 * GeV && cth < 0.9)
        {
          G4cout << "UrbanMscModel93::SampleCosineTheta: E(GeV)= " << KineticEnergy / GeV
                 << " 1-cosT= " << 1 - cth << " length(mm)= " << trueStepLength << " Zeff= " << Zeff
                 << " tau= " << tau << " prob= " << prob << " var= " << var << G4endl;
          G4cout << "  c= " << c << " qprob= " << qprob << " eb1= " << eb1 << " ebx= " << ebx
                 << " c1= " << c1 << " b= " << b << " b1= " << b1 << " bx= " << bx << " d= " << d
      ...
      else
      {
        ...
        {
          G4cout << "UrbanMscModel93::SampleCosineTheta: E(GeV)= " << KineticEnergy / GeV
                 << " length(mm)= " << trueStepLength << " Zeff= " << Zeff << " qprob= " << qprob
                 << G4endl;
        }
      }
    }
...
G4double UrbanMscModel93::SampleDisplacement()
{
  ...
  if ((currentTau >= tausmall) && !insideskin)
  {
    ...
    {
      rmean =
        kappa * currentTau * currentTau * currentTau * (1. - kappapl1 * currentTau * 0.25) / 6.;
    }
    else
    {
      G4double etau = 0.0;
    ...
    if (rmean > 0.)
      rmean = 2. * lambdaeff * sqrt(rmean / 3.0);
    else
      rmean = 0.;
  }
...
G4double UrbanMscModel93::LatCorrelation()
{
  ...
  if ((currentTau >= tausmall) && !insideskin)
  {
    if (currentTau < taulim)
      latcorr =
        lambdaeff * kappa * currentTau * currentTau * (1. - (kappa + 1.) * currentTau / 3.) / 3.;
    else
    {
      G4double etau = 0.;
      if (currentTau < taubig) etau = exp(-currentTau);
