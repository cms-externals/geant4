...
LimiterPhysics::LimiterPhysics(const G4String& name) : G4VPhysicsConstructor(name) {}
...
void LimiterPhysics::SetParameters(const G4String& part, const G4String& reg, G4double lim,
                                   G4double fac, G4double rms)
{
  if (reg != "EcalRegion" && reg != "HcalRegion")
  {
    G4cout << "LimiterPhysics::SetParameters WARNING: wrong region name: " << reg << G4endl;
    return;
  }
  G4int n = fPart.size();
  for (G4int i = 0; i < n; ++i)
  {
    ...
    {
      G4cout << "LimiterPhysics::SetParameters overwrite parameters for " << part << " inside "
             << reg << " : \n"
             << "  Limit(MeV)= " << lim << " Factor= " << fac << " RMS= " << rms << G4endl;
      fLimit[i] = lim;
      fFactor[i] = fac;
      fRMS[i] = rms;
      return;
  ...
  G4cout << "LimiterPhysics::SetParameters for " << part << " inside " << reg << " : \n"
         << "  Limit(MeV)= " << lim << " Factor= " << fac << " RMS= " << rms << G4endl;
