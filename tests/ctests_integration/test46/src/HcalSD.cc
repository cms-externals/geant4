...
HcalSD::HcalSD(const G4String& name) : G4VSensitiveDetector(name)
{
  theHisto = HistoManager::GetPointer();
  emSaturation = G4EmParameters::Instance()->GetEmSaturation();
}
...
{
  G4double edep = emSaturation->VisibleEnergyDepositionAtAStep(aStep);
