...
HistoMessenger::HistoMessenger(Histo* hist)
  :fHisto(hist)
{
  fHistoDir = new G4UIdirectory("/testhadr/histo/", false);
  ...
  fFactoryCmd->AvailableForStates(G4State_PreInit);

  fTupCmd = new G4UIcmdWithAString("/testhadr/histo/ntuple",this);
  fTupCmd->SetGuidance("set name for the ntuple");
  fTupCmd->AvailableForStates(G4State_PreInit);
  ...
  fHistoCmd->SetGuidance("  nbBins; valMin; valMax");
  fHistoCmd->AvailableForStates(G4State_PreInit);
  //
...
void HistoMessenger::SetNewValue(G4UIcommand* command, G4String newValues)
{
  ...
  else if (command == fFileCmd) { fHisto->SetFileType(newValues); }
  else if (command == fTupCmd) { fHisto->AddTuple(newValues); }
  else if (command == fHistoCmd) {
    G4int ih,nbBins; 
    G4double vmin,vmax;
    std::istringstream is(newValues);
    G4String unts;
    is >> ih >> nbBins >> vmin >> vmax;
    fHisto->SetHisto1D(ih,nbBins,vmin,vmax);
  }
}
