...
DetectorConstruction::DetectorConstruction(LimiterPhysics* lim)
  : logicWorld(nullptr),
    physiWorld(nullptr),
    logicPS(nullptr),
    logicECal(nullptr),
    logicCrystal(nullptr)
{
  ...
  detectorMessenger = new DetectorMessenger(this, lim);
  magField = nullptr;
  ...
  const G4double absThick[17] = {12.977, 7.815, 4.932, 4.932, 4.932, 4.932, 4.932, 4.932, 4.932,
                                 4.932,  5.371, 5.516, 5.516, 5.516, 5.516, 5.516, 9.734};
  const G4double scThick[17] = {9,   3.7, 3.7, 3.7, 3.7, 3.7, 3.7, 3.7, 3.7,
                                3.7, 3.7, 3.7, 3.7, 3.7, 3.7, 3.7, 9};
  ...
  regionECAL = nullptr;
  cutsECAL = new G4ProductionCuts();
  cutsECAL->SetProductionCut(1.0 * mm);
...
G4VPhysicalVolume* DetectorConstruction::Construct()
{
  ...
  man->SetHcalStart(posCenterHcalZ - hcalThickness / 2.);
  man->SetHcalCell(hcalWidth / 10.);

  G4cout << "### New geometry is constructed HCAL width(cm)= " << hcalWidth / cm << G4endl;
  if (man->GetVerbose() > 0) G4cout << *(G4Material::GetMaterialTable()) << G4endl;

  G4double w1 = logicECal->GetMass(true, true);
  G4cout << "WEIGHTS: " << w1 << G4endl;
  G4double w2 = logicWorld->GetMass(true, true);
  G4cout << "WEIGHTS: " << w1 << "   " << w2 << G4endl;
...
{
  regionECAL = new G4Region("EcalRegion");
  regionECAL->SetProductionCuts(cutsECAL);
  ...
  G4VPhysicalVolume* physE = new G4PVPlacement(0, G4ThreeVector(0., 0., posCenterEcalZ), "VolECal",
                                               logicECal, physiWorld, false, 0);
  regionECAL->AddRootLogicalVolume(logicECal);
  ...
  G4VPhysicalVolume* physGap =
    new G4PVPlacement(0, G4ThreeVector(0., 0., 0.), "VolECal", logicGap, physE, false, 0);
  ...
  // Crystals
  G4Box* solidCrystal =
    new G4Box("Crystal", crystalWidth * 0.5, crystalWidth * 0.5, crystalLength * 0.5);
  ...
  for (i = 0; i < 5; i++)
  {
    ...
    for (j = 0; j < 5; j++)
    {
      new G4PVPlacement(0, G4ThreeVector(x, y, 0.), "Ecal", logicCrystal, physGap, false, k);
      k++;
      x += crystalWidth + gap;
    }
    y += crystalWidth + gap;
...
void DetectorConstruction::ConstructHCAL()
{
  ...
  G4VPhysicalVolume* physHcal = new G4PVPlacement(0, G4ThreeVector(0., 0., posCenterHcalZ), "HCal",
                                                  logicHcal, physiWorld, false, 0);
  regionHCAL->AddRootLogicalVolume(logicHcal);
...
void DetectorConstruction::ConstructAllMatTest()
{
  ...
  G4int nmat = mat.size();
  G4cout << "### DetectorConstruction::ConstructAllMatTest() added " << nmat << " materials "
         << G4endl;
...
void DetectorConstruction::SetPSMaterial(const G4String& mat)
{
  ...
  if (pttoMaterial && pttoMaterial != psMaterial)
  {
    ...
    if (logicPS)
    {
      logicPS->SetMaterial(psMaterial);
    }
...
void DetectorConstruction::SetEcalMaterial(const G4String& mat)
{
  ...
  if (pttoMaterial && pttoMaterial != ecalMaterial)
  {
    ...
    if (logicCrystal)
    {
      logicCrystal->SetMaterial(ecalMaterial);
    }
    if (logicECal)
    {
      logicECal->SetMaterial(ecalMaterial);
    }
    G4RunManager::GetRunManager()->PhysicsHasBeenModified();
  }
}
