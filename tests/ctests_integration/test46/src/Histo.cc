...
void Histo::Book()
{
  ...
  fManager->SetHistoDirectoryName("histo");
  fManager->SetNtupleDirectoryName("ntuple");
  fManager->SetFirstHistoId(0);
  ...
  if (fNtupleActive)
  {
    ...
    G4cout << "##### defined n-tuples " << G4endl;
...
void Histo::SetHisto1D(G4int i, G4int nb, G4double x1, G4double x2)
{
  if (i >= 0 && i < fNHisto)
  {
    if (fVerbose > 0)
    {
      G4cout << "Histo::SetHisto1D: #" << i << "  " << nb << "  " << x1 << "  " << x2 << G4endl;
    }
    fBins[i] = nb;
    fXmin[i] = x1;
    fXmax[i] = x2;
