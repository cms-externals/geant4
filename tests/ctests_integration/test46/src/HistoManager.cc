...
void HistoManager::EndOfRun()
{
  ...
  G4cout << std::setprecision(4) << "Visible HCAL Edep(GeV)     =   " 
	 << x*hcal/CLHEP::GeV << G4endl;
  G4cout << std::setprecision(4) << "Visible HCAL e-Edep(GeV)   =   " 
	 << x*ehcal/CLHEP::GeV << G4endl;
  G4cout << std::setprecision(4) << "Total HCAL Edep(GeV)       =   " 
	 << x*(ehcal + abshcal)/CLHEP::GeV 
	 << " +- " << r1/CLHEP::GeV
	 << G4endl;
  G4cout<<"=========================================================="<<G4endl;
  G4cout << nam[6] << " = " << std::setw(10) << sum/CLHEP::GeV
	 << " +- " << std::setw(10) << r/CLHEP::GeV 
	 << "    RMS= " << y/CLHEP::GeV << G4endl;
  G4cout << nam[7] << " = " << std::setw(10) << sum1/CLHEP::GeV
	 << " +- " << std::setw(10) << r1/CLHEP::GeV 
	 << "    RMS= " << y1/CLHEP::GeV << G4endl;
  G4cout<<"=========================================================="<<G4endl;
			      ...
			      const G4ThreeVector& pos)
{
  ...
  G4int i = 2;
  if(x <= xyHcalSell && y <= xyHcalSell) { i = 0; }
  else if(x <= 3*xyHcalSell && y <= 3*xyHcalSell) { i = 1; }

  G4int j = 0;
  if(copyNo >= 10) { j = 3; } 
  else if(copyNo >= 5) { j = 2; }
  else if(copyNo >= 2) { j = 1; }
  HCAL[j][i] += de;
}
