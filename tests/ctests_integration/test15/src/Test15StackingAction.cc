...
Test15StackingAction::Test15StackingAction(Test15EventAction* eventAction) : evtAction(eventAction)
{
  fNumber_newtracks = 0;
  fNeutron = 0;
  fProton = 0;
...
void Test15StackingAction::printCrossSection(G4ParticleDefinition*, const G4Element*)
{
  /*
     ...
     for ( G4int aProc=0; aProc < processes->size(); aProc++ )
     {
          ...
          )
       {
          ...
          for ( G4double mom=1.*MeV, energy=std::sqrt(sqr(mass)+sqr(mom)); energy < 1.*TeV; mom
*=1.01, energy=std::sqrt(sqr(mass)+sqr(mom)))
          {
            const G4DynamicParticle * dynParticle=new G4DynamicParticle(particleType,
G4ThreeVector(1,0,0), energy-mass); G4double cross_sec =
hproc->GetMicroscopicCrossSection(dynParticle, element, 293.);
...
G4ClassificationOfNewTrack Test15StackingAction::ClassifyNewTrack(const G4Track* aTrack)
{
  // G4ClassificationOfNewTrack classification_w = fWaiting;
  // return classification_w;
  fNumber_newtracks++;
  if (fNumber_newtracks % 20000 == 0)
    G4cout << " current number of tracks: " << fNumber_newtracks << G4endl
           << " Neutron: " << fNeutron << " Proton: " << fProton << " Deuteron: " << fDeuteron
           << " Other: " << fOther << G4endl;
  ...
  else if (particleType->GetAtomicMass() != 208 && particleType->GetAtomicNumber() == 82
           && aTrack->GetTrackID() != 1)
  {
    leadName = "lead";
    particle_type = 8;
    ...
    //     G4cout << G4endl;
    if (particleType->GetAtomicMass() == 204 && particleType->GetAtomicNumber() == 82
        && aTrack->GetTrackID() != 1)
    {
      particleName = "pb204";
    }
    else if (particleType->GetAtomicMass() == 206 && particleType->GetAtomicNumber() == 82
             && aTrack->GetTrackID() != 1)
    {
      particleName = "pb206";
    }
    else if (particleType->GetAtomicMass() == 207 && particleType->GetAtomicNumber() == 82
             && aTrack->GetTrackID() != 1)
    {
      particleName = "pb207";
    }
  ...
  else if (particleType->GetAtomicMass() == 2 && particleType->GetAtomicNumber() == 1
           && aTrack->GetTrackID() != 1)
  {
    particle_type = 9;
    fDeuteron++;
  ...
  else if (particleType->GetAtomicMass() == 3 && particleType->GetAtomicNumber() == 1
           && aTrack->GetTrackID() != 1)
  {
    particle_type = 10;
    particleName = "triton";
  ...
  {
    if (particleType->GetAtomicMass() > 41)
      particle_type = 11;
    else
      particle_type = 12;
    fOther++;
  ...
  //   analyse->analyseStack(KineticEnergy, particleName, Momentum, leadName, atomicNumber,
  //   atomicMass, mass);
...
*/
  ...
  // to go in stacking action to see which process created which process created which particle -
  // eg. 600 keV protons in lead from 2.5GeV protons...
  //
  ...
  " "
      ...
      G4cout << ProcName << " 3 parent ID: " << aTrack->GetParentID() << " track id: " <<
  aTrack->GetTrackID() << " sum energy: " << energy << G4endl; G4cout << ProcName << " 4 neutron
  position(m): " << aTrack->GetPosition()/m
       << G4endl;
