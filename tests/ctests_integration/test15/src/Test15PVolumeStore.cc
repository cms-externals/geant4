...
{
  Test15SetGeometryCell::iterator it = fSetGeometryCell.find(cell);
  G4cout << " adding to pVolumeStore " << G4endl;
  if (it != fSetGeometryCell.end())
  {
    G4cout << "Test15PVolumeStore::AddPVolume: cell already stored" << G4endl;
    return;
  }
...
const G4VPhysicalVolume* Test15PVolumeStore::GetPVolume(const G4String& name) const
{
  ...
  if (!pvol)
  {
    G4cout << "Test15PVolumeStore::GetPVolume: no physical volume named: " << name << ", found"
           << G4endl;
  }
  return pvol;
}
