  ...
  :G4VUserParallelWorld(worldName),fLogicalVolumeVector(),olap_test(true)
{
  ...
  G4double radii_start[] = {200.0*cm,190.0*cm,185.0*cm,175.0*cm,165.0*cm,150.0*cm,140.0*cm,130.0*cm,120.0*cm,110.0*cm,100.0*cm,90.0*cm,80.0*cm,70.0*cm,60.0*cm,50.0*cm,45.7*cm,40.0*cm,30.0*cm,25.0*cm,20.0*cm,15.0*cm,10.0*cm,8.0*cm,5.0*cm,3.0*cm}; // care not to double count 45.7cm shell
...
{  
  ...
  ghostWorld = GetWorld();
  G4cout << " Test15ShellDetectorConstruction:: ghostWorldName = " 
  ...
  /*    
  G4Sphere* shellSphere = new G4Sphere("shellSphere",shell_inner_radius,shell_outer_radius,0.*deg, 360.*deg, 0.*deg, 180.*deg);
  // G4Box* shellSphere = new G4Box("shellSphere",0.5*shell_inner_radius,0.5*shell_outer_radius,0.5*shell_outer_radius);
    
  G4cout << " OUTER -shell is: " << shell_inner_radius << " outer: " << shell_outer_radius << G4endl;
  ...
  shellPhysical = new G4PVPlacement(0, G4ThreeVector(-0., -0., -0.), name, shellLogical, ghostWorld, false,0,olap_test);
...
*/
  ...
  // still needed? 27/09/15:
    ...
    G4cout << " i-shell is: " << i << " inner_radius[i]: " << inner_radius[i] << " outer: " << outer_radius[i] << G4endl;
    G4Sphere* radialSphere = new G4Sphere("shellSphere",inner_radius[i],outer_radius[i],0.*deg, 360.*deg, 0.*deg, 180.*deg);
    // G4Box* radialSphere = new G4Box("shellSphere",0.5*inner_radius[i],0.5*outer_radius[i],0.5*outer_radius);
    ...
    radialLogical[i] = 
      new G4LogicalVolume(radialSphere, dummyMat, logical_name);
    ...
    radialPhysical[i] = new G4PVPlacement(0, G4ThreeVector(0.0, 0.0, 0.0), name, radialLogical[i], ghostWorld, false,i,olap_test);
...
const G4VPhysicalVolume &Test15ShellDetectorConstruction::
...
G4GeometryCell Test15ShellDetectorConstruction::GetGeometryCell(G4int i){
  ...
  if (p) {
    ...
    G4cout << "Test15ShellDetectorConstruction::GetGeometryCell: couldn't get G4GeometryCell" << G4endl;
...
G4VIStore* Test15ShellDetectorConstruction::CreateImportanceStore()
{
  ...
  G4cout << " Test15ShellDetectorConstruction:: Creating Importance Store " 
         << G4endl;
  if (!fPVolumeStore.Size())
  {
    G4Exception("Test15ShellDetectorConstruction::CreateImportanceStore"
               ,"exampleTest15_0001",RunMustBeAborted
