  ...
  : G4UserSteppingAction(), evtAction(eventAction)
{
  ...
  // G4double radii_start[] =
  // {200.0*cm,190.0*cm,185.0*cm,175.0*cm,165.0*cm,150.0*cm,140.0*cm,130.0*cm,120.0*cm,110.0*cm,100.0*cm,90.0*cm,80.0*cm,70.0*cm,60.0*cm,50.0*cm,45.7*cm,40.0*cm,30.0*cm,25.0*cm,20.0*cm,15.0*cm,10.0*cm,8.0*cm,5.0*cm,3.0*cm};
  G4double radii_start[] = {200.0 * cm, 190.0 * cm, 185.0 * cm, 175.0 * cm, 165.0 * cm,
                            150.0 * cm, 140.0 * cm, 130.0 * cm, 120.0 * cm, 110.0 * cm,
                            100.0 * cm, 90.0 * cm,  80.0 * cm,  70.0 * cm,  60.0 * cm,
                            50.0 * cm,  40.0 * cm,  30.0 * cm,  25.0 * cm,  20.0 * cm,
                            15.0 * cm,  10.0 * cm,  8.0 * cm,   5.0 * cm,   3.0 * cm};
...
void Test15SteppingAction::UserSteppingAction(const G4Step* fStep)
{
  ...
  */
  ...
  // xfull  if(analysis->GetNtupleFull()) G4cout << " ntuple full is changed in stepping action " <<
  // G4endl;
  ...
  G4double angle =
    fStep->GetPreStepPoint()->GetMomentum().angle(fStep->GetPreStepPoint()->GetPosition());
  ...
  if (StepNo == 1 && particleName == "neutron")
  {
    ...
    // x2018 evtAction->analyseParticleSource(partEnergy, particleName, partTime, partMomentum,
    // zMomentum);
  ...
  */
  ...
  // xfull  if(TrackID == 5243 && StepNo > 1990) G4cout << " step: " << StepNo << " ntuple_full: "
  // << analysis->GetNtupleFull() << G4endl;
  ...
  if (StepNo == 1 && TrackID != 1)
  {
    ...
    {
      //	G4cout << " TrackID: " << TrackID << " parentID: " << parent_particleID[temp_id] << "
      //TempID: " << temp_id << " parent:" << ParentTrackID << " number of generations: " <<
      //number_generations << G4endl;
    ...
    // xfull      if(analysis->GetNtupleFull()) G4cout << " BEFORE SECONDARIES ntuple full is
    // changed in stepping action " << G4endl;

    //       G4cout << " calling analyse secondaries with ntuple full: " <<
    //       analysis->GetNtupleFull() << G4endl;

    evtAction->analyseSecondaries(partEnergy, particleName, partTime, partMomentum, ParentTrackID,
                                  primaryEnergy, parent_energy[ParentTrackID],
                                  parent_particle[ParentTrackID], reduced_tally,
                                  number_generations);
  ...
  else
  {
    ...
    if (particleName == "Pb207" || particleName == "Pb208")
      evtAction->OtherEnergyTime(partEnergy, partTime, startEnergy);
  }
  ...
  if (particleName == "neutron")
  {
    ...
    if (fStep->GetTrack()->GetNextVolume())
    {
      ...
      // xtemp      if(PostVol == "sample_phys2" && PostReplica == 0 && PostParentReplica == 51)
      // entering_sph_front = true; xtemp      if(PreVol == "sample_phys2" && PreReplica == 0 &&
      // PreParentReplica == 51) exiting_sph_front = true;
      ...
      {
        //	G4cout << " exiting stepping action, prevol: " << PreVol << " postvol: " << PostVol <<
        //G4endl;
        exiting = true;
    ...
    //    if(((radiusPre && radiusPost) > 44.6*cm) && ((radiusPre && radiusPost) < 46.6*cm) &&
    ...
    //    G4cout << " outer radius: " << G4BestUnit(outer_radius,"Length") << " inner radius: " <<
    //    G4BestUnit(inner_radius,"Length") << G4endl;

    // debug    G4cout << " outer_radius: " << outer_radius << " inner_radius: " << inner_radius <<
    // G4endl;
    //     if(((radiusPre > inner_radius ) && (radiusPost > inner_radius)) && ((radiusPre <
    //     outer_radius) && (radiusPost < outer_radius)) && particleName == "neutron")
    G4double my_tolerance = 1e-9 * mm;
    G4bool pre_inside = false;
    G4bool post_inside = false;

    if ((radiusPre <= (shell_outer_radius + my_tolerance))
        && (radiusPre >= (shell_inner_radius - my_tolerance)) && particleName == "neutron")
    {
      //      G4cout << " pre-step is inside shell " << radiusPre << G4endl;
      pre_inside = true;
    }
    if ((radiusPost <= (shell_outer_radius + my_tolerance))
        && (radiusPost >= (shell_inner_radius - my_tolerance)) && particleName == "neutron")
    {
    ...
    //     if(pre_inside && post_inside != true) G4cout << " PROBLEM pre? radiusPost= " <<
    //     radiusPost << " radiusPre= " << radiusPre << G4endl; if(post_inside && pre_inside !=
    //     true) G4cout << " PROBLEM post? radiusPost= " << radiusPost << " radiusPre= " <<
    //     radiusPre << G4endl;

    //     if((((radiusPre < (outer_radius-my_tolerance) ) && (radiusPre >
    //     (inner_radius+my_tolerance))) || ((radiusPost < (outer_radius-my_tolerance)) &&
    //     (radiusPost > (inner_radius-my_tolerance)))) && particleName == "neutron") {

    // 27/09/15 still needed...
    //  G4cout << " Test15SteppingAction:: got here 1 " << " post_inside: " << post_inside << "
    //  pre_inside: " << pre_inside << G4endl;
    if (post_inside && pre_inside)
    {
      // G4cout << " Test15SteppingAction:: got here 2 " << G4endl;
      // getchar();
      //       if(radiusPre < inner_radius) G4cout << " pre inner failed " << radiusPre << "
      //       difference: " << radiusPre-inner_radius << " post_inside is: " << post_inside << "
      //       pre_inside is: " << pre_inside << G4endl; if(radiusPost < inner_radius) G4cout << "
      //       post inner failed " << radiusPost << " difference: " << radiusPost-inner_radius << "
      //       post_inside is: " << post_inside << " pre_inside is: " << pre_inside << G4endl;
      //       if(radiusPre > outer_radius) G4cout << " pre outer failed " << radiusPre << "
      //       difference: " << radiusPre-outer_radius << " post_inside is: " << post_inside << "
      //       pre_inside is: " << pre_inside << G4endl; if(radiusPost > outer_radius) G4cout << "
      //       post outer failed " << radiusPost << " difference: " << radiusPost-outer_radius << "
      //       post_inside is: " << post_inside << " pre_inside is: " << pre_inside << G4endl;
      //       if((radiusPre < inner_radius) || (radiusPost < inner_radius) || (radiusPre >
      //       outer_radius) || (radiusPost > outer_radius)) G4cout << " STEP IS STRADDLING " << "
      //       outer: " << outer_radius << " and post-step: " << radiusPost << " inner: " <<
      //       inner_radius << " and pre-step: " << radiusPre << G4endl;
      //      G4cout << " calling neutron shell " << G4endl;
      // xbugcheck      G4cout << " radiusPre: " << G4BestUnit(radiusPre,"Length") << " radiusPost:
      // " << G4BestUnit(radiusPost,"Length") << G4endl;
      evtAction->analyseNeutronShellFluence(
        partEnergy, partTime, startEnergy, TrackID, ParentTrackID, zMomentum, startTime, radius,
        zPos, parent_energy[ParentTrackID], parent_particle[ParentTrackID], StepLength,
        entering_sph, entering_cyl, exiting_sph, exiting_cyl, Volume, entering_sph_front,
        exiting_sph_front, 0, 0, PreReplica, PostReplica);
    ...
    for (G4int i = 0; i < 25; ++i)
    {
      ...
      if ((radiusPre <= (radial_outer_radius + radial_tolerance))
          && (radiusPre >= (radial_inner_radius - radial_tolerance)) && particleName == "neutron")
      {
        //      if((radiusPre <= radial_outer_radius) && (radiusPre >= radial_inner_radius) &&
        //      particleName == "neutron") { G4cout << " pre-step is inside shell " << radiusPre <<
        //      G4endl;
        pre_inside_radial = true;
      }
      if ((radiusPost <= (radial_outer_radius + radial_tolerance))
          && (radiusPost >= (radial_inner_radius - radial_tolerance)) && particleName == "neutron")
      {
        //      if((radiusPost <= radial_outer_radius) && (radiusPost >= radial_inner_radius) &&
        //      particleName == "neutron") { G4cout << " post-step is inside shell " << radiusPost
        //      << G4endl;
        post_inside_radial = true;
      }

      // xbugcheck       if(pre_inside_radial && radiusPost <= radial_outer_radius)
      // post_inside_radial = true; xbugcheck       if(pre_inside_radial && radiusPost >=
      // radial_inner_radius) post_inside_radial = true; xbugcheck       if(post_inside_radial &&
      // radiusPre <= radial_outer_radius) pre_inside_radial = true; xbugcheck if(post_inside_radial
      // && radiusPre >= radial_inner_radius) pre_inside_radial = true;

      //       if(pre_inside_radial && post_inside_radial != true) G4cout << " PROBLEM pre?
      //       radiusPost= " << radiusPost << " radiusPre= " << radiusPre << " whilst
      //       radial_outer_radius = " << radial_outer_radius << " and inner = " <<
      //       radial_inner_radius << G4endl; if(post_inside_radial && pre_inside_radial != true)
      //       G4cout << " PROBLEM post? radiusPost= " << radiusPost << " radiusPre= " << radiusPre
      //       << G4endl;

      // xbugcheck      if(post_inside_radial || pre_inside_radial)
      if (post_inside_radial && pre_inside_radial)
      {
        //	  G4cout << " SHELLS: radiusPre: " << G4BestUnit(radiusPre,"Length") << " radiusPost: "
        //<< G4BestUnit(radiusPost,"Length") << G4endl;
        ...
        // xRadial 	  if(i==17) G4cout << " called neutron radial fluence with step: " << StepLength
        // << G4endl;
      }
    }
    // problem? 100907
    ...
    if (Volume == "sample_phys" || Volume == "sampleTube_phys" || Volume == "sample_phys2")
    {
      ...
      //      G4cout << " parentReplica: " << PreParentReplica << " and: " << PostParentReplica << "
      //      and volume: " << Volume << G4endl; G4cout << " GOT HERE 3" << G4endl;
    ...
    //    static const
    G4double radii[] = {16.8 * cm, 40.4 * cm,  45.6 * cm,  69.1 * cm,  81.1 * cm,
                        98.6 * cm, 105.3 * cm, 113.5 * cm, 124.8 * cm, 153.9 * cm};

    for (G4int i = 0; i < 10; ++i)
    {
      if ((radiusPre < radii[i] && radiusPost > radii[i])
          || (radiusPre > radii[i] && radiusPost < radii[i]))
      {
        radius = radii[i];
        //      G4cout << " calling analyse neutron flux, step: " <<  StepNo << " trackID: " <<
        //      TrackID << " parent: " << ParentTrackID << G4endl;
        //    radius = 45.6*cm;
        //	if((radiusPre<radius && radiusPost>radius) || (radiusPre>radius && radiusPost<radius)) {

        // xfull	if(TrackID == 5243 && StepNo > 1990) G4cout << " step: " << StepNo << "
        // ntuple_full: " << analysis->GetNtupleFull() << " and calling analyseNeutronFlux " <<
        // G4endl;
        //  still needed? 27/09/15
        //  metal band at Hallenstadion 26/09/15
        //  G4cout << " Test15SteppingAction:: GOT HERE 1!!!! " << " for i: " << i << G4endl
        //  	     << " RadiusPre: " << radiusPre << " radiusPost: " << radiusPost << " vs. " <<
        //  radii[i] << G4endl;
        evtAction->analyseNeutronFlux(partEnergy, partTime, startEnergy, TrackID, ParentTrackID,
                                      zMomentum, startTime, radius, zPos,
                                      parent_energy[ParentTrackID], parent_particle[ParentTrackID],
                                      cos_angle, number_generations, particleName, reduced_tally);
