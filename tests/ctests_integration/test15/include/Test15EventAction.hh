...
class Test15EventAction : public G4UserEventAction {
  ...
  void analyseNeutronFlux(G4double, G4double, G4double, G4int, G4int, G4double, G4double, G4double, G4double, G4double, G4String, G4double, G4int, G4String, G4bool);

  //fill histograms with data from Test15NeutronFlux / secondary history
  //x2018 void analyseNeutronFluence(G4double, G4double, G4double, G4int, G4int, G4double, G4double, G4double, G4double, G4double, G4String, G4double, G4bool, G4bool, G4bool, G4bool, G4String,G4bool,G4bool,G4int,G4int,G4int,G4int);

  void analyseNeutronShellFluence(G4double, G4double, G4double, G4int, G4int, G4double, G4double, G4double, G4double, G4double, G4String, G4double, G4bool, G4bool, G4bool, G4bool, G4String,G4bool,G4bool,G4int,G4int,G4int,G4int);
...
//x2018 inline void Test15EventAction::analyseParticleSource(G4double source_energy, G4String source_name, G4double source_time, G4double source_momentum, G4double source_zMomentum)
// {
  /*
  ...
  if(name == "neutron") {
    // G4cout << " filling neutron lethargy with: energy = " << energy/eV << " and momentum = " << 1/momentum << G4endl;
    analysisManager->FillH1(2, energy/eV,1/momentum);
...
inline void Test15EventAction::analyseSecondaries(G4double energy, G4String name, G4double time, G4double momentum, G4int ParentID, G4double primaryEnergy, G4double parentEnergy, G4String parentParticle, G4bool reduced_flux, G4int number_generations)
{
  // get analysis manager
  G4AnalysisManager* analysisManager = G4AnalysisManager::Instance();
  ...
  if(name == "gamma") {
    ...
    if(energy > 0.1*eV && energy < 10.0*keV) run->AddFlux("neutron_fluence");// neutron_fluence++;
  }    
  else if(name == "e-") {
    run->AddFlux(name);
    // electron_flux++;
  ...
  else if (parentParticle == "neutron") iParent = 2;
  else if(reduced_flux) iParent = -2;
  else if (parentParticle == "e-")      iParent = 3;
  else if (parentParticle == "pi-")     iParent = 4;
  else if (parentParticle == "pi+")     iParent = 5;
  else if (parentParticle == "pi0")     iParent = 6;
  else if (parentParticle == "e+")      iParent = 7;
  else if (parentParticle == "proton")  iParent = 8;
  else if (parentParticle == "proton")  iParent = 8;
  else if (parentParticle == "mu-")  iParent = 9;
  else if (parentParticle == "mu+")  iParent = 10;
  ...
  if(ntuple_full) {
    ...
    // ntupleSecondary->fill( ntupleSecondary->findColumn( "numgen" ), (G4float) number_generations);
    // ntupleSecondary->fill( ntupleSecondary->findColumn( "event" ), (G4float) event_id);
    // ntupleSecondary->addRow();
  }
...
//   if(exit_cyl) G4cout << " EXITING CYLINDER " << G4endl;
  ...
  if(temp_energy<lithium_energy[100]) {
	...
	if(Volume == "sample_phys" && (preReplica == 0 || postReplica == 0)) lithium_fluence_step[i] += steplength/mm;
	if(Volume == "sample_phys2" && (preReplica == 0 || postReplica == 0) && (preParentReplica == 51 || postParentReplica == 51)) lithium_fluence_front_step[i] += steplength/mm;
  ...
  if(temp_energy<low_energy[100]) {
	...
	if(Volume == "sample_phys" && (preReplica == 0 || postReplica == 0)) low_fluence_step[i] += steplength/mm;
	if(Volume == "sample_phys2" && (preReplica == 0 || postReplica == 0) && (preParentReplica == 51 || postParentReplica == 51)) low_fluence_front_step[i] += steplength/mm;
	...
	if(Volume == "sample_phys" && (preReplica == 0 || postReplica == 0)) fluence_step[i] += steplength/mm;
	if(Volume == "sample_phys2" && (preReplica == 0 || postReplica == 0) && (preParentReplica == 51 || postParentReplica == 51)) fluence_front_step[i] += steplength/mm;
	  //xx	G4cout << " i: " << i << " temp_energy: " << temp_energy << " flux_energy[i]:" << flux_energy[i] << " flux_energy[i+1]: " << flux_energy[i+1] << " flux: " << flux[i] << G4endl;
	flux_idx = i;
  ...
  //     G4cout << " energy: " << energy/eV << " flux energy0: " << flux_energy[i] << " flux energy1: " << flux_energy[i+1] << G4endl;
...
//     if(temp_energy<lithium_energy[100]) {
//       for(G4int i = 0; i<100; ++i) {
// 	if(temp_energy > lithium_energy[i] && temp_energy < lithium_energy[i+1]) lithium_fluence_step[i] += steplength/mm;
//       }
      
//       if(temp_energy<low_energy[100]) {
// 	for(G4int i = 0; i<100; ++i) {
// 	  if(temp_energy > low_energy[i] && temp_energy < low_energy[i+1]) low_fluence_step[i] += steplength/mm;
// 	}
//       }
//       else if(temp_energy>flux_energy[0]) {
// 	for(G4int i = 0; i<21; ++i) {
// 	  if(temp_energy > flux_energy[i] && temp_energy < flux_energy[i+1]) fluence_step[i] += steplength/mm;
// 	}
//       } 
...
inline void Test15EventAction::analyseNeutronRadialFluence(G4double energy, G4double time, G4double steplength, G4int radius_index)
{
  // get analysis manager
  //  G4AnalysisManager* analysisManager = G4AnalysisManager::Instance();
...
//xRadial  if(radius_index == 17) G4cout << " temp_energy: " << temp_energy << " lithium_radial_energy_upper[10]: " << lithium_radial_energy_upper[9] << G4endl;
...
inline void Test15EventAction::NeutronEnergyTime(G4double energy, G4double time, G4double startEnergy)
{

  // get analysis manager
  G4AnalysisManager* analysisManager = G4AnalysisManager::Instance();
  if(time > 0. && energy > 0.) analysisManager->FillH2(1,log10(time/microsecond),log10(energy/eV),1.0);
...
inline void Test15EventAction::analyseNeutronShellFluence(G4double energy, G4double time, G4double startEnergy, G4int TrackID, G4int ParentID, G4double zMomentum, G4double startTime, G4double radius, G4double zPos, G4double parentEnergy, G4String parentParticle, G4double steplength, G4bool enter_sph, G4bool enter_cyl, G4bool exit_sph, G4bool exit_cyl, G4String Volume, G4bool enter_sph_front, G4bool exit_sph_front, G4int preParentReplica, G4int postParentReplica, G4int preReplica, G4int postReplica)
{
  ...
  run->analyseNeutronShellFluence(energy,time,startEnergy,TrackID,ParentID,zMomentum,startTime,radius,zPos,parentEnergy,parentParticle,steplength,enter_sph,enter_cyl,exit_sph,exit_cyl,Volume,enter_sph_front,exit_sph_front,preParentReplica,postParentReplica,preReplica,postReplica);
...
//   if(exit_cyl) G4cout << " EXITING CYLINDER " << G4endl;
  ...
  if(ntuple_full) {
    ...
    ntupleShellFluence->fill( ntupleShellFluence->findColumn( "starte" ), (G4float) temp_startenergy);
    ntupleShellFluence->fill( ntupleShellFluence->findColumn( "trackid" ), (G4float) TrackID);
    ntupleShellFluence->fill( ntupleShellFluence->findColumn( "parentid" ), (G4float) ParentID);
    ntupleShellFluence->fill( ntupleShellFluence->findColumn( "zmom" ), (G4float) zMomentum);
    ntupleShellFluence->fill( ntupleShellFluence->findColumn( "startt" ), (G4float) startTime);
    ntupleShellFluence->fill( ntupleShellFluence->findColumn( "radius" ), (G4float) radius);
    ntupleShellFluence->fill( ntupleShellFluence->findColumn( "e_parent" ), (G4float) parentEnergy);
    ntupleShellFluence->fill( ntupleShellFluence->findColumn( "parent" ), (G4float) iParent);
    ntupleShellFluence->fill( ntupleShellFluence->findColumn( "step" ), (G4float) steplength);
    ntupleShellFluence->fill( ntupleShellFluence->findColumn( "dupli" ), (G4float) duplicate_neutron2);
...
inline void Test15EventAction::analyseNeutronFlux(G4double energy, G4double time, G4double startEnergy, G4int TrackID, G4int ParentID, G4double zMomentum, G4double startTime, G4double radius, G4double zPos, G4double parentEnergy, G4String parentParticle, G4double cos_angle, G4int number_generations, G4String Particle, G4bool reduced_tally)
{
  ...
  Test15Run* run 
    = static_cast<Test15Run*>(G4RunManager::GetRunManager()->GetNonConstCurrentRun());
  run->analyseNeutronFlux(energy,time,startEnergy,TrackID,ParentID,zMomentum,startTime,radius,zPos,parentEnergy,parentParticle,cos_angle,number_generations,Particle,reduced_tally);
  ...
  if(Particle == "neutron") {
    if(TrackID == oldTrackID && std::abs(radius - 456.0*mm)<0.1) {
      duplicate_neutron++;
      //x    G4cout << " duplicate_neutron " << duplicate_neutron << " trackID: " << TrackID << G4endl;
    }  else {
      duplicate_neutron = 0;
    }
    oldTrackID = TrackID;
  ...
  if(Particle == "neutron") {
    ...
    for (G4int i=0; i<10; ++i) {
      //xfull      G4cout << " inside analyseNeutronFlux 1a, index: " << i << " ntuple_full: " << ntuple_full << G4endl;
      if( std::abs(radius-radii[i])<0.1) {
	for (G4int j=0; j<10; ++j) {
	  if ( std::abs(energy-radii_energies[j])<fractional_bin_width*radii_energies[j]) flux_radius[i][j] += 1.0/std::abs(cos_angle);
	}
      }
    }
	...
	integral_scintillation_E += energy/eV*1.0/std::abs(cos_angle);
      ...
      if(n < 100) {
	//xfull	G4cout << " inside analyseNeutronFlux 1b4 " << ntuple_full << " angle: " << cos_angle << " n: " << n << " fluence: " << fluence_spectrum[n] << " ntuple_full again: " << ntuple_full << G4endl;
  ...
  if(Particle == "neutron") {
	...
	if(energy > fine_energy[j]/1000000.0 && energy < fine_energy[j+1]/1000000.0) fine_eflux[j] += energy/eV;
