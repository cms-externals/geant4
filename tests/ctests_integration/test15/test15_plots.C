...
// bert100_3.5.root
  ...
  // for(int j=0; j<2; ++j) {
    ...
    radial_h1 = new TH2F("radial_h1_"+j,"Radial in Test15",1000,-200.,200.,1000,1.,25000000);
    ...
    Float_t bin_width = (std::log(10.0**5.0)-std::log(0.01))/100.0;
    // bin_width = 0.1611809
    ...
    // tarc_data_fluence[j] = new TH1F("fluence_data_"+j,"Fluence Data in Test15",123,xbins);
    tarc_data_fluence_high = new TH1F("fluence_data_"+j,"Fluence Data in Test15 High",21,xbins_high);
    tarc_data_fluence_he3 = new TH1F("fluence_data_"+j,"Fluence Data in Test15 He3",101,xbins_low);
    tarc_data_fluence_li = new TH1F("fluence_data_li_"+j,"Fluence Data in Test15 Li",101,xbins_low);
    tarc_data_fluence_high_error = new TH1F("fluence_data_error_"+j,"Fluence Data in Test15 High Combined Error",21,xbins_high);
    tarc_data_fluence_he3_error = new TH1F("fluence_data_error_"+j,"Fluence Data in Test15 He3 Combined Error",101,xbins_low);
    tarc_data_fluence_li_error = new TH1F("fluence_data_li_error_"+j,"Fluence Data in Test15 Li Combined Error",101,xbins_low);

    tarc_g4_fluence_high = new TH1F("fluence_g4_high_"+j,"Fluence G4 in Test15",21,xbins_high);
    tarc_g4_fluence_he3 = new TH1F("fluence_g4_he3_"+j,"Fluence G4 in Test15",101,xbins_low);
    tarc_g4_fluence_li = new TH1F("fluence_g4_li_"+j,"Fluence G4 in Test15",101,xbins_low);

    tarc_g4_ratio_high = new TH1F("ratio_g4_high_"+j,"Fluence Ratio G4 in Test15",21,xbins_high);
    tarc_g4_ratio_he3 = new TH1F("ratio_g4_he3_"+j,"Fluence Ratio G4 in Test15",101,xbins_low);
    tarc_g4_ratio_li = new TH1F("ratio_g4_li_"+j,"Fluence Ratio G4 in Test15",101,xbins_low);
  ...
  // analysisManager->CreateNtupleDColumn("g4_shell");
    ...
    exiting_spectrum = new TH1F("exiting_spectrum_"+j,"Exiting Neutron Spectrum Test15",100,0.01,2.);
	...
	// // radial_h1[j]->Fill(radius/10,fluence/energy,1.);
      ...
      TAxis *xaxis = tarc_g4_fluence_high->GetXaxis();
      Int_t binx = xaxis->FindBin(energy);
    ...
    for (int irow=0;irow<my_tupleD2->GetEntries();++irow){
      ...
      TAxis *xaxis = tarc_g4_fluence_he3->GetXaxis();
      Int_t binx = xaxis->FindBin(energy);
    ...
    for (int irow=0;irow<my_tupleD3->GetEntries();++irow){
      ...
      TAxis *xaxis = tarc_g4_fluence_li->GetXaxis();
      Int_t binx = xaxis->FindBin(energy);
    ...
    c1 = new TCanvas("c1","Test15 summary",700,500);
    c1->Divide(2,2); // divide c1 in 2x2 subpads    
    gStyle->SetTitleX(0.3);

    c1->cd(1);       // to change current pad to top left pad
    gStyle->SetTitleX(0.3);
    gPad->SetLogx();
    gPad->SetLogy();
    radial_h1->SetLineColor(kBlue);
    radial_h1->SetLineWidth(2);
    radial_h1->GetXaxis()->SetTitle("Energy Deposit /keV");
    radial_h1->GetYaxis()->SetTitle("dN/dE / source gamma");
    radial_h1->GetYaxis()->SetTitleOffset(1.4);
    // if(j==0) radial_h1->Draw("HIST");
    // else radial_h1->Draw("HIST SAME");
    // if(j==0) h1->Draw();
    tarc_g4_fluence_high->Draw();
    // else h1->Draw("SAME");
    c1->cd(2);
    gPad->SetLogx();
    gPad->SetLogy();
    // if(j==1) h1->Draw();
    tarc_g4_fluence_li->Draw();
    c1->cd(3);
    ...
    gPad->SetLogx();
    gPad->SetLogy();
// null 0.001 10000000 1000 25000000
// atit 'Energy/eV' 'EdF/dE n/cm^2!/10^9!p' 
    // tarc_data_fluence->GetXaxis()->SetRangeUser(0.001,10000000);
    // tarc_data_fluence->GetYaxis()->SetRangeUser(1000,25000000);
    ...
    TString my_title1 = names[j] + " Gev/c protons 100 events ";
    // TLatex* tlx=new TLatex(0.23, 0.93, "100 events binary 2.5 GeV/c proton");
    TLatex* tlx=new TLatex(0.23, 0.93, my_title1);
    ...
    tarc_g4_fluence_high->Draw("SAME E");
    tarc_g4_fluence_he3->SetLineColor(kBlack);
    tarc_g4_fluence_he3->SetLineWidth(2);
    tarc_g4_fluence_he3->Draw("SAME E");
    tarc_g4_fluence_li->SetLineColor(kBlack);
    tarc_g4_fluence_li->SetLineWidth(2);
    tarc_g4_fluence_li->Draw("SAME E");
    
    // if(j==0) tarc_g4_fluence->Draw();
    gStyle->SetTitleX(0.3);
    c1->cd(4);
    h1->Draw("COLZ");
    ...
    h2->Draw("SAME COLZ");
    h1->Draw("SAME COLZ");
    // if(j==1) tarc_data_fluence->Draw();
    
    TString file1 = names[j] + "plot1.pdf";
    c1->Print(file1);
    TString file1g = names[j] + "plot1.png";
    c1->Print(file1g);
    // getchar();
    c1->Close();
    
    c2 = new TCanvas("c2","Test15 Fluence summary",700,500);
    gStyle->SetHistLineWidth(3);  // Default:1(GetHistLineWidth) --- Line width of histogram
    gStyle->SetLineWidth(0.3);        // Default:1 --- Line width of axis.
    gStyle->SetTitleX(0.2);
    gPad->SetLogx();
    ...
    c3 = new TCanvas("c3","Test15 Fluence Ratio G4/Data summary",700,500);
    gStyle->SetHistLineWidth(3);  // Default:1(GetHistLineWidth) --- Line width of histogram
    gStyle->SetLineWidth(0.3);        // Default:1 --- Line width of axis.
    gStyle->SetTitleX(0.2);
    gPad->SetLogx();
    ...
    // TLatex* tlx=new TLatex(0.23, 0.93, "100 events binary 2.5 GeV/c proton");
    TLatex* tlx=new TLatex(0.23, 0.93, my_title3);
    ...
    c4 = new TCanvas("c4","Test15 Time vs. Energy",700,500);
    c4->Divide(2,2);
    c4->cd(1);
    gStyle->SetTitleX(0.3);
    h1->SetStats(kFALSE);
    h2->SetStats(kFALSE);
    h1->GetXaxis()->SetTitle("log10(time) / microsecond");
    h1->GetYaxis()->SetTitle("log10(energy) / eV");
    h2->GetXaxis()->SetTitle("log10(time) / microsecond");
    h2->GetYaxis()->SetTitle("log10(energy) / eV");
    h1->Draw("COLZ");
    c4->cd(2);
    h1->Draw();
    c4->cd(3);
    ...
    TString file4 = names[j] + "_energy.pdf";
    c4->Print(file4);
    TString file4g = names[j] + "_energy.png";
    c4->Print(file4g);
    c4->Close();
    
    // radial:
    c5 = new TCanvas("c2","Test15 Radial Fluence",700,500);
    gStyle->SetHistLineWidth(3);  // Default:1(GetHistLineWidth) --- Line width of histogram
    gStyle->SetLineWidth(0.3);        // Default:1 --- Line width of axis.
    gStyle->SetTitleX(0.2);
    gPad->SetLogy();
    ...
    // TLatex* tlx=new TLatex(0.23, 0.93, "100 events binary 2.5 GeV/c proton");
    TLatex* tlx=new TLatex(0.23, 0.93, my_title4);
    // TLatex* tlx=new TLatex(0.23, 0.93, "10000 events binary 2.5 GeV/c proton");
...
mytext 0. 1400000 ' 0.1 eV ' 0.3 ! c
    ...
    c6 = new TCanvas("c6","Test15 Exiting Flux",700,500);
    c6->Divide(1,2);
    c6->cd(1);
    gStyle->SetHistLineWidth(3);  // Default:1(GetHistLineWidth) --- Line width of histogram
    gStyle->SetLineWidth(0.3);        // Default:1 --- Line width of axis.
    ...
    exiting_spectrum->SetMaximum(50);
    exiting_spectrum->SetLineColor(kRed);
    exiting_spectrum->SetLineWidth(0.85);
    exiting_spectrum->Draw();
    ...
    TString file6 = names[j] + "exit.pdf";
    c6->Print(file6);
    TString file6g = names[j] + "exit.png";
    c6->Print(file6g);
    c6->Close();
