...
void BeamTestDetectorMessenger::SetNewValue(G4UIcommand* command, G4String newValue)
{
  ...
  if (command == theNumberOfChambersCmd)
  {
    ...
    static_cast<BeamTestEventAction*>(theEventAction)
      ->SetNumberOfChambers(theNumberOfChambersCmd->GetNewIntValue(newValue));
  }
  if (command == theUpdateCommand) theDetector->UpdateGeometry();
}
