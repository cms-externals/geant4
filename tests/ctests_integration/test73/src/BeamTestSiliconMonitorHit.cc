...
void BeamTestSiliconMonitorHit::Print()
{
  ...
  // std::cout << "Exiting X position in silicon monitor " << std::setprecision(5) <<
  // fEPosition.x()/mm << " in mm" << std::endl;
  ...
  // out << fEPosition.x()/mm << ", " << fEPosition.y()/mm << ", " << fEPosition.z()/mm <<
  // std::endl; std::ofstream outfile ("test.cvs"); outfile << std::setprecision(10) <<
  // fEPosition.x()/mm; outfile.close();
}
