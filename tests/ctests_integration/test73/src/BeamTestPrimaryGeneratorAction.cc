...
G4cout << G4endl;
G4cout << "Angle " << xzAngle(A) << " " << A.getP() << " " << A.getPT() << " " << "New Pz " << pZ(A)
<< G4endl; G4cout << G4endl;
...
//particleGun->SetNumberOfParticles(100);
  ...
  {  // First event of the run, setup stuff...
    ...
    G4UserEventAction* theEventAction =
    ...
    G4cout << "Primaries " << particleGun->GetParticleDefinition()->GetParticleName()
           << " will be generated with Momentum: " << G4BestUnit(mymom, "Energy")
           << "/c , at Postion: " << G4BestUnit(Position, "Length") << G4endl;
  }
