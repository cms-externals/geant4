...
void BeamTestEventAction::EndOfEventAction(const G4Event* event)
{
  ...
  G4ThreeVector M;
  G4ThreeVector P;
  // Too many variables here but...ipT is the main one used for the inverseof the transverse
  // momentum
    ...
    for (G4int idx = 0; idx < numberOfHits; ++idx)
    {
      ...
      if (aHit->GetTrackId() == 1)
      {
            ...
            && aHit->GetStepStatus() == fGeomBoundary)
        {
          ...
          // check as this could be I-X in this backward case but want to got from IX^-> vector
          // hence should be ok
          b = X - I;
          // G4cout << "AND FINALLY B: " << b << G4endl;

          // G4cout << "Information of " << i+1 << " th Silicon Monitor Hit of this event." <<
          // G4endl;

          // G4cout << "Exiting Particle Name and Kinetic Energy " <<
          // aHit->GetExitDefinition()->GetParticleName() 	<< " " << E << " MeV" << G4endl; G4cout
          // << "Exiting position in silicon monitor "<< P << " in mm" << G4endl; G4cout << "Exiting
          // Momentum  " << M << G4endl;
