...
void Tst24DetectorConstruction::SelectMaterialPointer()
{
  ...
  {
    selectedMaterial = G4NistManager::Instance()->FindOrBuildMaterial(materialChoice);
    if (selectedMaterial)
      G4cout << " Using geant4 material " << selectedMaterial->GetName() << G4endl;
    else
      G4cout << " material " << materialChoice << " not found, using default material " << G4endl;
    ;
  }
