...
void merge(const char* type, const _Container& run_local)
{
  ...
  for (auto local_itr = run_local.begin(); local_itr != run_local.end(); ++local_itr)
  {
    ...
    if (local_obj || total_obj)
    {
      ...
      std::cout << "[" << type << "] Testing merge " << std::setw(3) << local_idx << " ... "
                << std::flush;
      if (!local_obj)
      {
...
void verify(const char* type, const _Container& run_container)
{
  for (auto itr = run_container.begin(); itr != run_container.end(); ++itr)
  {
    ...
    if (obj)
    {
      std::cout << "[" << type << "] Testing index " << std::setw(3) << idx << " ... "
                << std::flush;
...
void check_map_dbl_ptr()
{
  verify(__FUNCTION__, dbl_ptr_run_map);
}
void check_mmap_dbl_ptr()
{
  verify(__FUNCTION__, dbl_ptr_run_mmap);
}
void check_uomap_dbl_ptr()
{
  verify(__FUNCTION__, dbl_ptr_run_uomap);
}
void check_uommap_dbl_ptr()
{
  verify(__FUNCTION__, dbl_ptr_run_uommap);
}
void check_deque_dbl_ptr()
{
  verify(__FUNCTION__, dbl_ptr_run_deque);
}
void check_vector_dbl_ptr()
{
  verify(__FUNCTION__, dbl_ptr_run_vector);
}
void check_deque_dbl_pod()
{
  verify(__FUNCTION__, dbl_pod_run_deque);
}
void check_vector_dbl_pod()
{
  verify(__FUNCTION__, dbl_ptr_run_vector);
}

void check_map_stat_ptr()
{
  verify(__FUNCTION__, stat_ptr_run_map);
}
void check_mmap_stat_ptr()
{
  verify(__FUNCTION__, stat_ptr_run_mmap);
}
void check_uomap_stat_ptr()
{
  verify(__FUNCTION__, stat_ptr_run_uomap);
}
void check_uommap_stat_ptr()
{
  verify(__FUNCTION__, stat_ptr_run_uommap);
}
void check_deque_stat_ptr()
{
  verify(__FUNCTION__, stat_ptr_run_deque);
}
void check_vector_stat_ptr()
{
  verify(__FUNCTION__, stat_ptr_run_vector);
}
void check_deque_stat_pod()
{
  verify(__FUNCTION__, stat_pod_run_deque);
}
void check_vector_stat_pod()
{
  verify(__FUNCTION__, stat_ptr_run_vector);
}
...
void merge_map_dbl_ptr()
{
  merge(__FUNCTION__, dbl_ptr_run_map);
}
void merge_mmap_dbl_ptr()
{
  merge(__FUNCTION__, dbl_ptr_run_mmap);
}
void merge_uomap_dbl_ptr()
{
  merge(__FUNCTION__, dbl_ptr_run_uomap);
}
void merge_uommap_dbl_ptr()
{
  merge(__FUNCTION__, dbl_ptr_run_uommap);
}
void merge_deque_dbl_ptr()
{
  merge(__FUNCTION__, dbl_ptr_run_deque);
}
void merge_vector_dbl_ptr()
{
  merge(__FUNCTION__, dbl_ptr_run_vector);
}
void merge_deque_dbl_pod()
{
  merge(__FUNCTION__, dbl_pod_run_deque);
}
void merge_vector_dbl_pod()
{
  merge(__FUNCTION__, dbl_ptr_run_vector);
}

void merge_map_stat_ptr()
{
  merge(__FUNCTION__, stat_ptr_run_map);
}
void merge_mmap_stat_ptr()
{
  merge(__FUNCTION__, stat_ptr_run_mmap);
}
void merge_uomap_stat_ptr()
{
  merge(__FUNCTION__, stat_ptr_run_uomap);
}
void merge_uommap_stat_ptr()
{
  merge(__FUNCTION__, stat_ptr_run_uommap);
}
void merge_deque_stat_ptr()
{
  merge(__FUNCTION__, stat_ptr_run_deque);
}
void merge_vector_stat_ptr()
{
  merge(__FUNCTION__, stat_ptr_run_vector);
}
void merge_deque_stat_pod()
{
  merge(__FUNCTION__, stat_pod_run_deque);
}
void merge_vector_stat_pod()
{
  merge(__FUNCTION__, stat_ptr_run_vector);
}
...
int main(int argc, char** argv)
{
  ...
  if (argc > 1) nevent = atol(argv[1]);
  if (argc > 2) nvoxel = atol(argv[2]);

  std::cout << "\nExecuting " << argv[0] << ". # events = " << nevent << ", # voxels = " << nvoxel
            << "..." << std::endl;
...

  {
    ...
    for (auto itr = exclude_voxels.begin(); itr != exclude_voxels.end(); ++itr)
    {
      ...
      if (static_cast<size_t>(_dist + 1) != exclude_voxels.size()) ss << ", ";
    }
    std::cout << "\n" << ss.str() << "\n" << std::endl;
  }
