...
RunAction::RunAction(DetectorConstruction* det, PhysicsList* phy, PrimaryGeneratorAction* kin)
...
void RunAction::EndOfRunAction(const G4Run* aRun)
{
  ...
  // nuclear energy loss
  NonIonEnergyDeposit /= TotNbofEvents;
  NonIonEnergyDeposit2 /= TotNbofEvents;
  G4double rmsEnondep = NonIonEnergyDeposit2 - NonIonEnergyDeposit * NonIonEnergyDeposit;
  if (rmsEnondep > 0.)
    rmsEnondep = std::sqrt(rmsEnondep / TotNbofEvents);
  else
    rmsEnondep = 0;

  // mean sum of T( kinetic energy of secondary)x L(T) partition energy
  sum_TL /= TotNbofEvents;
  sum_TL2 /= TotNbofEvents;
  G4double rmssum_TL = sum_TL2 - sum_TL * sum_TL;
  if (rmssum_TL > 0.)
    rmssum_TL = std::sqrt(rmssum_TL / TotNbofEvents);
  else
    rmssum_TL = 0;
  ...
  Nsteps /= TotNbofEvents;
  Nsteps2 /= TotNbofEvents;
  G4double rmsSteps = Nsteps2 - Nsteps * Nsteps;
  if (rmsSteps > 0)
    rmsSteps = std::sqrt(rmsSteps / TotNbofEvents);
  else
    rmsSteps = 0;

  // scattering angle
  theta /= TotNbofEvents;
  theta2 /= TotNbofEvents;
  G4double rmsTheta = theta2 - theta * theta;
  if (rmsTheta > 0.)
    rmsTheta = std::sqrt(rmsTheta / TotNbofEvents);
  else
    rmsTheta = 0;

  // track length
  TrakLenPrim /= TotNbofEvents;
  TrakLenPrim2 /= TotNbofEvents;
  G4double rmsTLPrim = TrakLenPrim2 - TrakLenPrim * TrakLenPrim;
  if (rmsTLPrim > 0.)
    rmsTLPrim = std::sqrt(rmsTLPrim / TotNbofEvents);
  else
    rmsTLPrim = 0.;
  ...
  G4cout << "\n The run was " << TotNbofEvents << " " << partName << " of "
         << G4BestUnit(energy, "Energy") << ", through " << G4BestUnit(thickness, "Length")
  ...
  G4cout << "\n Nuclear energy deposit in absorber:\n " << G4BestUnit(NonIonEnergyDeposit, "Energy")
         << " +/- " << G4BestUnit(rmsEnondep, "Energy") << G4endl;
  G4cout << "-----> dE/dx  nucl = " << meandEdx_nucl / (MeV / cm) << " MeV/cm"
         << "\t(" << stopPower_nucl / (MeV * cm2 / g) << " MeV*cm2/g)" << G4endl;

  G4cout << "\n NIEL in absorber (Th>" << Th / eV << " eV):\n " << G4BestUnit(sum_TL, "Energy")
         << " +/- " << G4BestUnit(rmssum_TL, "Energy") << G4endl;
  G4cout << "-----> NIEL = " << meandEdx_sumTL / (MeV / cm) << " MeV/cm"
         << "\t(" << stopPower_sumTL / (MeV * cm2 / g) << " MeV*cm2/g)" << G4endl;
