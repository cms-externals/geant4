...
void EventAction::EndOfEventAction(const G4Event* event)
{
  ...
  G4int nmbHitsCollections = G4SDManager::GetSDMpointer()->GetCollectionCapacity();

  for (G4int i = 0; i < nmbHitsCollections; i++)
  {
    TargetLayerHitsCollection* hitsCollection = (TargetLayerHitsCollection*)hitsCollEvent->GetHC(i);
    ...
    if (hitsCollection)
    {
      ...
      {
        Secondaries secParticles = ((*hitsCollection)[j])->GetSecParticles();
        ...
        {
          G4cout << iter->first << " " << iter->second << std::endl;
        }
      }
    }
  }
