...
DetectorConstruction::DetectorConstruction() : worldVolPhys(0)
{
...
{
  if (worldVolPhys) return worldVolPhys;

  G4Material* worldVolMat = Materials::Instance()->GetMaterial("Galactic");

  G4Box* worldVolSolid = new G4Box("World", 50 * cm, 50 * cm, 50 * cm);

  G4LogicalVolume* worldVolLogic = new G4LogicalVolume(worldVolSolid, worldVolMat, "World");
  worldVolLogic->SetVisAttributes(G4VisAttributes::GetInvisible());

  worldVolPhys = new G4PVPlacement(0, G4ThreeVector(), "World", worldVolLogic, 0, false, 0);
  ...
  SemiInfiniteTarget* infiniteSlab =
    new SemiInfiniteTarget("infiniteTargetLayer", updateManager, worldVolPhys);
...
void DetectorConstruction::CreateFrontLayer(const G4String& layerName)
{
  TargetComponent* currentFrontLayer = updateManager->GetFrontLayer();

  AddOnTargetLayer* frontLayer = new AddOnTargetLayer(currentFrontLayer, updateManager, layerName);

  G4Region* targetRegion = G4RegionStore::GetInstance()->GetRegion(targetRegionName);
  targetRegion->AddRootLogicalVolume(frontLayer->GetVolLog());
}

void DetectorConstruction::SetLayerRadius(G4double radi)
{
  updateManager->SetRadius(radi);
}
...
void DetectorConstruction::SetLayerMaterial(const G4String& mat)
{
...
{
  AnalysisManager::Instance()->CreateCalorimeter(zPosition, calThickness,
                                                 updateManager->GetRadius());
}
