...
DataManager::DataManager(G4double zLow, G4double zUp)
  : zAxisLowerBound(zLow), zAxisUpperBound(zUp), eps(0.0000000001)
{}
...
void DataManager::ScoreEnergyDeposit(G4double en, G4double x, G4double y, G4double z,
                                     const G4String& type)
{
  ...
  collector::iterator iter =
    std::upper_bound(dataCollectors.begin(), dataCollectors.end(), z, zCompare());

  if (iter != dataCollectors.begin()) (iter - 1)->second->ScoreEnergyDeposit(en, x, y, z, type);
}

void DataManager::ScoreParticleEnergy(G4double en, G4double x, G4double y, G4double z,
                                      const G4String& type)
{
  if (z < zAxisLowerBound || z >= zAxisUpperBound) return;

  collector::iterator iter =
    std::lower_bound(dataCollectors.begin(), dataCollectors.end(), z, zCompare());
...
void DataManager::AddDataCollector(DataManager* comp)
{
  ...
  {
    G4cout << "Error. Slab with lower bound z=" << zLow << " exceeds boundary of mother volume."
           << G4endl;
  ...
  if (!OverlapsWithOtherChild(zLow, zUp))
  {
    ...
    G4cout << "INFORMATION. Slab with center at z=" << zPos << " and thickness " << thickn
           << " added." << G4endl;
  }
  else
  {
    G4cout << "Error. Slab with lower bound z=" << zLow << " overlaps with other slab." << G4endl;
    garbage.push_back(std::make_pair(zLow, comp));
    return;
  }
...
{
  if (std::binary_search(dataCollectors.begin(), dataCollectors.end(), z, zCompare()))
  {
    return true;
  }
...
bool DataManager::OverlapsWithOtherChild(G4double zLow, G4double zUp)
{
  ...
  for (; iter != iter_end; iter++)
  {
    if (zLow < iter->second->GetLowerBound() && zUp > (iter->second->GetLowerBound() + eps))
      return true;

    if (zLow < (iter->second->GetUpperBound() - eps) && zUp > iter->second->GetUpperBound())
      return true;
