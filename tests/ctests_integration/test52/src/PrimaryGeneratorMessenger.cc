  ...
  : primaryGenerator(primGen)
{
  ...
  primEnergyCmd = new G4UIcmdWithADoubleAndUnit("/source/primEnergy", this);
  ...
  sigmaEnergyCmd = new G4UIcmdWithADoubleAndUnit("/source/sigmaEnergy", this);
  ...
  sigmaSpatialCmd = new G4UIcmdWithADoubleAndUnit("/source/sigmaSpatial", this);
  ...
  incidAngleCmd = new G4UIcmdWithADoubleAndUnit("/source/incidentAngle", this);
...
void PrimaryGeneratorMessenger::SetNewValue(G4UIcommand* cmd, G4String val)
{
  if (cmd == primEnergyCmd)
    primaryGenerator->SetPrimaryKineticEnergy(primEnergyCmd->GetNewDoubleValue(val));
  if (cmd == sigmaEnergyCmd)
    primaryGenerator->SetSigmaKineticEnergy(sigmaEnergyCmd->GetNewDoubleValue(val));
  if (cmd == sigmaSpatialCmd)
    primaryGenerator->SetSigmaSpatialPlacement(sigmaSpatialCmd->GetNewDoubleValue(val));
  if (cmd == incidAngleCmd)
    primaryGenerator->SetIncidentAngle(incidAngleCmd->GetNewDoubleValue(val));
}
