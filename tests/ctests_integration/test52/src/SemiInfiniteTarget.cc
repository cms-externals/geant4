...
SemiInfiniteTarget::SemiInfiniteTarget(G4String layerName, TargetGeometryManager* geomManager,
                                       G4VPhysicalVolume* world, G4String mat, G4double thickn,
                                       G4double radi, G4double maxStep)
  : TargetComponent(geomManager, layerName, thickn, mat),
    worldVolPhys(world),
    radius(radi),
    maxStepSize(maxStep),
    ...
    semiInfTargetPhys(0)
{
  ...
  semiInfTargetSolid = new G4Tubs(Name(), 0.0, radius, Thickness() * 0.5, 0.0, 360.0 * deg);

  semiInfTargetLogic = new G4LogicalVolume(semiInfTargetSolid, Material(), Name());

  semiInfTargetPhys = new G4PVPlacement(0, G4ThreeVector(0.0, 0.0, Thickness() * 0.5), Name(),
                                        semiInfTargetLogic, worldVolPhys, false, 0);
  ...
  G4VisAttributes* semiInfTargetVisAtt = new G4VisAttributes(G4Colour::Yellow());
...
void SemiInfiniteTarget::GeometryUpdate(TargetGeometryManager* geomManager)
{
  if (geomManager == updateManager)
  {
    ...
    semiInfTargetPhys->SetTranslation(G4ThreeVector(0.0, 0.0, position));
  }
}
