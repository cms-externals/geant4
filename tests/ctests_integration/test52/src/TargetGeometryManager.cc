...
void TargetGeometryManager::SetRadius(G4double radi)
{
  layerRadius = radi;
  Notify();
}
...
void TargetGeometryManager::SetMaterial(const G4String& mat)
{
  frontLayerMaterial = mat;
  Notify();
}

void TargetGeometryManager::SetMaxStepSize(G4double maxs)
{
  maxStepSize = maxs;
  Notify();
}
