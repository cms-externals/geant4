...
void AnalysisManager::CreateCalorimeter(G4double pos, 
                                        G4double thickn, 
                                        G4double rad) {
...
void AnalysisManager::ScoreParticleEnergy(G4double en, 
                                          G4double x, 
                                          G4double y, 
                                          G4double z,
...
void AnalysisManager::ScoreEnergyDeposit(G4double en, 
                                         G4double x, 
                                         G4double y, 
	                                 G4double z,
                                         const G4String& ptype) {
...
void AnalysisManager::ScoreEnteringParticles(G4double en, 
                                             G4double, 
                                             G4double, 
                                             G4double,
                                             const G4String& ptype) {
...
void AnalysisManager::ScoreExitingParticles(G4double en, 
                                            G4double, 
                                            G4double, 
	                                    G4double,
                                            const G4String& ptype,
...
void AnalysisManager::PrintResults() {
  ...
  if(nmbElecExitTarget > 0) {
    elecEnergyExitTargetErrSqu =
	   ...
	   elecEnergyExitTarget) - 1.0 / G4double(nmbElecExitTarget);
  }
  ...
  if(nmbPhotExitTarget > 0) {
    photEnergyExitTargetErrSqu =
	   ...
	   photEnergyExitTarget) - 1.0 / G4double(nmbPhotExitTarget);
  }
  ...
  G4cout << "  Fraction of incid. electr. energy escaping as photons: "
       ...
       << G4double(nmbElecExitTarget)/G4double(nmbElecEnterTarget)
       << G4endl     
       << "  Fraction of electrons backscattered, only primaries " 
       << "(direct calcul.): "
       << G4double(nmbPrimElecExitTarget)/G4double(nmbElecEnterTarget)
       << G4endl;      
