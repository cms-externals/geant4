  ...
  : detectorConstruction(detConstr)
{
  ...
  frontLayerCmd = new G4UIcmdWithAString("/target/frontLayer", this);
  frontLayerCmd->SetGuidance("Specification of additional front layer");
  frontLayerCmd->SetParameterName("frontLayer", false);
  frontLayerCmd->AvailableForStates(G4State_PreInit);

  layerRadiusCmd = new G4UIcmdWithADoubleAndUnit("/target/radius", this);
  ...
  layerRadiusCmd->AvailableForStates(G4State_PreInit);

  layerThicknessCmd = new G4UIcmdWithADoubleAndUnit("/target/layerThickness", this);
  ...
  layerThicknessCmd->AvailableForStates(G4State_PreInit);

  layerMaterialCmd = new G4UIcmdWithAString("/target/layerMaterial", this);
  layerMaterialCmd->SetGuidance("Specification of layer material");
  layerMaterialCmd->SetParameterName("layerMaterial", false);
  layerMaterialCmd->AvailableForStates(G4State_PreInit);

  layerMaxStepSizeCmd = new G4UIcmdWithADoubleAndUnit("/target/maxStepSize", this);
  ...
  layerMaxStepSizeCmd->AvailableForStates(G4State_PreInit);

  calPositionCmd = new G4UIcmdWithADoubleAndUnit("/target/calPosition", this);
  ...
  calThicknessCmd = new G4UIcmdWithADoubleAndUnit("/target/calThickness", this);
  ...
  calThicknessCmd->AvailableForStates(G4State_PreInit);
}
...
void DetectorMessenger::SetNewValue(G4UIcommand* cmd, G4String val)
{
  ...
  {
    detectorConstruction->SetLayerRadius(layerRadiusCmd->GetNewDoubleValue(val));
  }
  if (cmd == layerThicknessCmd)
  {
    detectorConstruction->SetLayerThickness(layerThicknessCmd->GetNewDoubleValue(val));
  }
  ...
  {
    detectorConstruction->SetLayerMaxStepSize(layerMaxStepSizeCmd->GetNewDoubleValue(val));
  }
  if (cmd == calPositionCmd)
  {
    detectorConstruction->CreateCalorimeter(calPositionCmd->GetNewDoubleValue(val));
  }
  if (cmd == calThicknessCmd)
  {
    detectorConstruction->SetCalorimeterThickness(calThicknessCmd->GetNewDoubleValue(val));
  }
}
