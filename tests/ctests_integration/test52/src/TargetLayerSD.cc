...
TargetLayerSD::TargetLayerSD(G4String SDuniqueName)
  : G4VSensitiveDetector(SDuniqueName), hitsCollectionID(-1)
{
...
{
  hitsCollection = new TargetLayerHitsCollection(SensitiveDetectorName, collectionName[0]);
...
G4bool TargetLayerSD::ProcessHits(G4Step* step, G4TouchableHistory*)
{
  ...
  hit->StoreParticleType(step->GetTrack()->GetDefinition()->GetParticleName());
...
void TargetLayerSD::EndOfEvent(G4HCofThisEvent*)
{
  ...
  for (G4int i = 0; i < nmbHits; i++)
  {
    ...
    analysisManager->ScoreEnergyDeposit((*hitsCollection)[i]->GetEnergyDeposit(), x, y, z,
