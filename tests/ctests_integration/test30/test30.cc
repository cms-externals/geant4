...
int main(int argc, char** argv)
{
  ...
  float bestZ[250] = {0.0,  1.0,  1.0,  2.0,  2.0,  0.0,  0.0,  4.0,  0.0,  0.0,  // 0
  ...
  for (G4int run = 0; run < 100; run++)
  {
    ...
    do
    {
      ...
      else if (line == "#generator")
      {
        nameGen = "";
        (*fin) >> nameGen;
        if (nameGen == "" || nameGen == "chips" || nameGen == "lepar" || nameGen == "rpg")
        {
        ...
        if (nullptr == cc)
        {
          G4cout << "Generator <" << nameGen << "> is not included in the "
                 << " list SetModels.csh, so is ignored!" << G4endl;
      ...
      else if (line == "#random")
      {
        ...
        if (verbose > 0) G4cout << "Random Engine restored from file <" << sss << ">" << G4endl;
      ...
      else if (line == "#MFenergy(MeV)")
      {
        G4double tmin;
        (*fin) >> tmin;
        G4cout << " Min energy for multi-fragmentation is set to " << tmin << " MeV" << G4endl;
    ...
    if (!ionParticle)
    {
      part = partTable->FindParticle(namePart);
    }
    else
    {
      part = partTable->GetIonTable()->GetIon(ionZ, ionA, 0.0);
    }
    if (nullptr == part)
    {
    ...
    if (verbose > 0)
    {
      G4cout << "The material:  " << material->GetName() << " Z= " << Z << " A= " << A
             << "  Amax= " << maxn << G4endl;
    ...
    // G4double recmass = G4NucleiProperties::GetNuclearMass(A, Z);
    ...
    G4cout << "### Log10 scale from " << logmin << " to " << logmax << " in " << nbinlog << " bins"
           << G4endl;
    ...
    if (usehis && !isInitH)
    {
      ...
      for (i = nanglpr; i < 11; ++i)
      {
        histo.Activate(3 + i, false);
      }
      ...
      for (i = nangl; i < 6; ++i)
      {
        histo.Activate(51 + i, false);
      }

      // elastic
      histo.Add1D("58", "Ekin(MeV) for primary particle", 80, (energy - recmax), energy);
      ...
      histo.Add1D("63", "Ekin (MeV) for recoil particle", 100, 0., recmax);
    ...
    if (usehis)
    {
      ...
      histo.Book();
      G4cout << "Histograms are booked output file <" << hFile << ".root> " << G4endl;
    }
    ...
    if (extraproc && namegen1 == "Elas")
    {
      ...
      else if (part == proton)
      {
        if (xschips)
        {
          cs = new G4ChipsProtonElasticXS();
        }
        else
        {
          cs = new G4BGGNucleonElasticXS(part);
        }
      }
      else if (part == neutron)
      {
        if (xschips)
        {
          cs = new G4ChipsNeutronElasticXS();
        }
        else
        {
      ...
      else if (part == pip)
      {
        if (xschips)
        {
          cs = new G4ChipsPionPlusElasticXS();
        }
        else
        {
          cs = new G4BGGPionElasticXS(part);
        }
      }
      else if (part == pin)
      {
        if (xschips)
        {
          cs = new G4ChipsPionMinusElasticXS();
        }
        else
        {
          cs = new G4BGGPionElasticXS(part);
      ...
      if (nameGen == "Elastic")
      {
        els = new G4HadronElastic();
      }
      else if (nameGen == "ElasticLHEP")
      {
        els = new G4HadronElastic();
      }
      else if (nameGen == "ElasticHE")
      {
        els = new G4ElasticHadrNucleusHE();
      }
      else if (nameGen == "ElasticDIFF")
      {
        els = new G4DiffuseElastic();
      }
      else if (nameGen == "ElasticCHIPS")
      {
        els = new G4ChipsElasticModel();
      }
      else if (nameGen == "ElasticHP")
      {
        els = new G4ParticleHPElastic();
      }
      else if (nameGen == "ElasticLE")
      {
        els = new G4LowEHadronElastic();
      }
    ...
    G4cout << "### factor  = " << factor << "### factora = " << factora
           << "### factorb = " << factorb << "    cross(mb)= " << cross_sec * 1000. / barn
           << G4endl;
    ...
    if (nangl > 0)
    {
      for (G4int k = 0; k < nangl; k++)
      {
        ...
        cng[k] = cross_sec * MeV * 1000.0 * (G4double)nbinsd
                 / (twopi * (std::cos(degree * bng1[k]) - std::cos(degree * bng2[k])) * barn * emax
                    * (G4double)nevt);
      }
    }
    ...
    if (nanglpr > 0)
    {
      for (G4int k = 0; k < nanglpr; k++)
      {
        ...
        cngpr[k] = cross_sec * MeV * 1000.0 * (G4double)nbinsd
                   / (twopi * (std::cos(degree * bng1pr[k]) - std::cos(degree * bng2pr[k])) * barn
                      * emax * (G4double)nevt);
      }
    }
    ...
    if (nanglpi > 0)
    {
      for (G4int k = 0; k < nanglpi; k++)
      {
        ...
        cngpi[k] = cross_sec * MeV * 1000.0 * (G4double)nbinspi
                   / (twopi * (std::cos(degree * bngpi1[k]) - std::cos(degree * bngpi2[k])) * barn
                      * emaxpi * (G4double)nevt);
      }
    }
    ...
    for (G4int iter = 0; iter < nevt; ++iter)
    {
      ...
      if (saverand)
      {
        CLHEP::HepRandom::saveEngineStatus("random.txt");
      }
      ...
      G4cout << "Mnuc= " << amass/GeV << " GeV"
       ...
       << G4endl;
      */
      labv = G4LorentzVector(0.0, 0.0, std::sqrt(e0 * (e0 + 2. * mass)), e0 + mass + amass);
      ...
      for (G4int i = 0; i < n; ++i)
      {
        ...
        {
          warn++;
          G4cout << "Warning! evt# " << iter << "  " << i << "-th sec  " << pd->GetParticleName()
                 << "   Ekin(MeV)= " << e / MeV << " Pt(MeV/c)= " << pt / MeV
                 << " Ebeam(MeV)= " << e0 / MeV << G4endl;
        }
        de += e;
        if (verbose > 1)
        {
          G4cout /*<< "Warning! evt# " << iter*/
            << "  " << i << "-th sec  " << pd->GetParticleName() << "  Ekin(MeV)= " << e / MeV
            << "  p(MeV)= " << mom / MeV << "  m(MeV)= " << mass1 / MeV
            << "  Etot(MeV)= " << (e + mass1) / MeV << "  pt(MeV)= " << pt / MeV
            << "  std::sin(tet)= " << pt / p << "  phi(deg)= " << mom.phi() / degree << G4endl;
        }
        ...
        if (usehis)
        {
          if (pd)
          {
            ...
            if (std::abs(Z0 - Z1) < 0.1 || Z0 == 0.0) histo.Fill(26, N, factorb);
          }
      ...
      if (aChange->GetTrackStatus() == fAlive && extraproc)
      {
        ...
        {
          G4cout /*<< "Warning! evt# " << iter*/
            << "primary  " << namePart << "  Ekin(MeV)= " << ekin / MeV
            << "  p(MeV)= " << mom1 / MeV << "  m(MeV)= " << mass / MeV
            << "  Etot(MeV)= " << (ekin + mass) / MeV << "  std::sin(tet)= " << std::sin(theta)
            << "  phi(deg)= " << dir.phi() / degree << G4endl;
        }
      }
      ...
      pt = std::sqrt(px * px + py * py);
      if (verbose > 1 || (verbose > 0 && (std::abs(labv.e()) > 0.1 * MeV || p > 100 * MeV)))
      {
        G4cout << iter << "-event Energy/Momentum balance= " << labv << " p=" << p << " pt= " << pt
               << G4endl;
      }
