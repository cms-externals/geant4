...
public:
  ...
  explicit Test30Physics();

  ~Test30Physics();

  G4VProcess* GetProcess(const G4String&, const G4ParticleDefinition*, G4Material*);

  inline G4double GetNucleusMass() {return theProcess->GetMass();};

  inline G4PreCompoundModel* GetPreCompound() {return thePreCompound;};

  inline void SetA(G4int A) {if(theProcess) theProcess->SetA(A);};
