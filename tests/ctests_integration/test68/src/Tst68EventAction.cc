...
Tst68EventAction::~Tst68EventAction()
{
  ...
  if (numberOfEvents > 0)
  {
    G4cout << G4endl << " ----------------------------------- " << G4endl
           ...
           << sumTotalDepositedEnergy / static_cast<G4double>(numberOfEvents) << "  MeV " << G4endl
           << " ----------------------------------- " << G4endl << G4endl;
  }
}
...
void Tst68EventAction::EndOfEventAction(const G4Event* evt)
{
  ...
  if (evt->GetEventID() == 0)
  {
    G4cout << "\t ---------------------------------------" << G4endl
           ...
           << "\t Beam Particle Kinetic Energy = " << incidentParticleEnergy / GeV << " GeV"
           << G4endl << "\t ---------------------------------------" << G4endl;
  }
  ...
  G4cout << " ---  EndOfEventAction  ---  event= " << evt->GetEventID() << "   t=";
  eventTimer->Stop();
...
{
  G4UserSteppingAction* theUserAction =
    const_cast<G4UserSteppingAction*>(G4RunManager::GetRunManager()->GetUserSteppingAction());
  if (theUserAction == 0)
  {
