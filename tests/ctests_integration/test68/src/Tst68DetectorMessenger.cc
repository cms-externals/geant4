...
Tst68DetectorMessenger::Tst68DetectorMessenger(Tst68DetectorConstruction* myDet)
  : theDetector(myDet)
{
...
void Tst68DetectorMessenger::SetNewValue(G4UIcommand* command, G4String newValue)
{
  ...
  {
    theDetector->SetAbsorberTotalLength(theAbsorberTotalLength->GetNewDoubleValue(newValue));
  }

  if (command == theCalorimeterRadius)
  {
    theDetector->SetCalorimeterRadius(theCalorimeterRadius->GetNewDoubleValue(newValue));
  }
  ...
  {
    theDetector->SetActiveLayerSize(theActiveLayerSize->GetNewDoubleValue(newValue));
  }
  ...
  {
    theDetector->SetIsRadiusUnitInLambda(theIsRadiusUnitInLambda->GetNewBoolValue(newValue));
  }
