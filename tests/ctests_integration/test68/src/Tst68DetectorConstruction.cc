...
void Tst68DetectorConstruction::DefineMaterials()
{
  ...
  Vacuum = new G4Material(name = "Vacuum", density, nel = 1, kStateGas, temperature, pressure);
...
G4VPhysicalVolume* Tst68DetectorConstruction::ConstructCalorimeter()
{
  ...
  if (!areParametersOK())
  {
    G4cout << " Tst68DetectorConstruction::ConstructCalorimeter() : ***ERROR*** " << G4endl
  ...
  if (theIsUnitInLambda)
  {
    ...
    {
      lambda =
        15.056 * cm;  // Lack of PDG data: I am assuming the same as Copper.  // X0     = 1.4353*cm;
    }
  ...
  /*
    G4double radiusBinSize = theRadiusBinSize;
...
G4bool Tst68DetectorConstruction::areParametersOK()
{
  ...
  {
    isOk = false;
    G4cout << " Tst68DetectorConstruction::areParametersOK() : UNDEFINED active material" << G4endl;
  }
  ...
  if (theReadoutLayerNumber <= 0 || theActiveLayerNumber % theReadoutLayerNumber != 0)
  {
    isOk = false;
...
{
  G4cout << G4endl << G4endl << " ------  Tst68DetectorConstruction::PrintParameters() ------ "
         << G4endl << " Absorber Material = ";
  if (theAbsorberMaterial)
  {
