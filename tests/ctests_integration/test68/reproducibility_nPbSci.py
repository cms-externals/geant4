...
#  keep copmpatible with python 2 
...
#BfieldValue = "OFF"  # Use "OFF" for no magnetic field
BfieldValue = "4 tesla"
#
...
if ( NumEvents <= 0 ) :
    print(' Warning: NumEvents = ', NumEvents, '  <= 0 : set it to 1 !')
    NumEvents = 1
if ( NumSingleEventChecks <= 0 ) :
    print(' Warning: NumSingleEventChecks = ', NumSingleEventChecks, '  <= 0 : set it to 1 !')
    NumSingleEventChecks = 1
...
for iCase in listCases :
    ...
    if ( NumEvents > 0  and  NumSingleEventChecks > 0 ) :
        for i in range( NumSingleEventChecks+1 ) :
            ...
            if ( i == 0 ) :
                g4file.write( "/run/beamOn " + str( NumEvents ) + " \n" )
    ...
    if ( NumSingleEventChecks > 0 ) :
        ...
        for i in range( NumSingleEventChecks+1 ) :
            ...
            if ( not vecRandom ) : 
                print(' ERROR : not found  "random="  in ', nameOut, file=sys.stderr)
                vecRandom.append( "MISSING" )
    ...
    if ( NumSingleEventChecks > 0 ) :
        ...
        for i in range( NumSingleEventChecks+1 ) :
            if i == 0 :
                continue
            longRunValue = "MISSING"
            shortRunValue = "MISSING"
            response = ""
            if ( vecRuns[ 0 ] == [ "MISSING" ] ) :
                response = "SKIPPED"
            else : 
                longRunValue = vecRuns[ 0 ][ i-1 ]
            if ( vecRuns[ i ] == [ "MISSING" ] ) :
                response = "SKIPPED"
            else :
                shortRunValue = vecRuns[ i ][ 0 ]
            if ( not response ) :
                if ( longRunValue == shortRunValue ) :
                    response = "OK"
                else :
                    response = "NO"
                    isOK = 0
            print('  test', str( i ), response, longRunValue, shortRunValue)
        print('  ****************************** ' )
        if ( isOK ) :
            print('  *** REPRODUCIBILITY ?  YES *** ')
        else :
            print('NO REPRODUCIBILITY', file=sys.stderr)
