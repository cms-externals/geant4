...
public:
  ...
  { fTrackLenCharged += length; fTrackLenCharged2 += length*length; };
  
  void AddTrakLenNeutr (G4double length)
  { fTrackLenNeutral += length; fTrackLenNeutral2 += length*length; };
  ...
  G4double ComputeMscHighland() const;
  
  void AddEnergyLeak (G4double eleak, G4int index)
  { fEnergyLeak[index] += eleak; fEnergyLeak2[index] += eleak*eleak; };

  void FillHisto(G4int histoId, G4double v);
  void FillHisto(G4int histoId, G4double v1, G4double v2);
  G4double GetHistoBinWidth(G4int histoId) const;
  G4double GetHistoUnit(G4int histoId) const;
...
private:
  ...
  G4double fTrackLenCharged, fTrackLenCharged2;
  G4double fTrackLenNeutral, fTrackLenNeutral2;
