...
void TrackingAction::PreUserTrackingAction(const G4Track* aTrack)
{
  ...
  {
    xstartAbs = fDetector->GetxstartAbs();
    xendAbs = fDetector->GetxendAbs();
...
void TrackingAction::PostUserTrackingAction(const G4Track* aTrack)
{
  ...
  if (id > 0) run->FillHisto(id, aTrack->GetKineticEnergy());
  ...
  if (id > 0)
  {
    theta = std::acos(direction.x());
    if (theta > 0.0)
    {
      G4double dteta = run->GetHistoBinWidth(id);
      G4double unit = run->GetHistoUnit(id);
      G4double weight = (unit * unit) / (twopi * std::sin(theta) * dteta);
      run->FillHisto(id, theta, weight);
    }
  }
  ...
  if ((aTrack->GetParentID() == 1) && charged)
  {
    ...
    run->FillHisto(4, xVertex);
    if (notabsor) run->FillHisto(5, xVertex);
  }
}
