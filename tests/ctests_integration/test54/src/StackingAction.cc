...
G4ClassificationOfNewTrack StackingAction::ClassifyNewTrack(const G4Track* aTrack)
{
  ...
  if (charge != 0.) run->FillHisto(2, energy);

  if (aTrack->GetDefinition() == G4Gamma::Gamma()) run->FillHisto(3, std::log10(energy / MeV));

  // stack or delete secondaries
  G4ClassificationOfNewTrack status = fUrgent;
  if (fKillSecondary)
  {
    if (fKillSecondary == 1) fEventAction->AddEnergy(energy);
    status = fKill;
  }
