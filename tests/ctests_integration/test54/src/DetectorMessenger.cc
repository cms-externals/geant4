...
void DetectorMessenger::SetNewValue(G4UIcommand* command, G4String newValue)
{
  ...
  {
    fDetector->SetAbsorberThickness(AbsThickCmd->GetNewDoubleValue(newValue));
  }

  if (command == AbsSizYZCmd)
  {
    fDetector->SetAbsorberSizeYZ(AbsSizYZCmd->GetNewDoubleValue(newValue));
  }

  if (command == AbsXposCmd)
  {
    fDetector->SetAbsorberXpos(AbsXposCmd->GetNewDoubleValue(newValue));
  }

  if (command == WorldXCmd)
  {
    fDetector->SetWorldSizeX(WorldXCmd->GetNewDoubleValue(newValue));
  }

  if (command == WorldYZCmd)
  {
    fDetector->SetWorldSizeYZ(WorldYZCmd->GetNewDoubleValue(newValue));
  }
