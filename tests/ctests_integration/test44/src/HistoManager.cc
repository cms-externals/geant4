...
{
  G4cout << "HistoManager::BeginOfRun()" << G4endl;
  absZ0 = -0.5 * length;
  n_evt = 0;
  n_step = 0;
  ...
  analysisManager->SetFileName("test44");
  analysisManager->SetVerboseLevel(1);
  analysisManager->SetFirstHistoId(1);  // start histogram numbering from 1
  analysisManager->SetActivation(true);  // enable inactivation of histograms

  G4cout << "HistoManager::BeginOfRun() - create H1" << G4endl;
  analysisManager->CreateH1("h1", "Energy deposition (MeV/mm/event) in the target", nSlices, 0.0,
                            length / mm);
  G4cout << "HistoManager::BeginOfRun() - open file" << G4endl;
...
void HistoManager::AddTargetStep(const G4Step* step)
{
  ...
  if (edep >= DBL_MIN)
  {
    ...
    edepEvt += edep;
    G4AnalysisManager* analysisManager = G4AnalysisManager::Instance();
    analysisManager->FillH1(1, z, edep);

    if (1 < verbose)
      G4cout << "HistoManager::AddEnergy: e(keV)= " << edep / keV << "; z(mm)= " << z / mm
             << "; step(mm)= " << step->GetStepLength() / mm << " by "
             << currentDef->GetParticleName() << " E(MeV)= " << currentKinEnergy / MeV << G4endl;
  }
}
