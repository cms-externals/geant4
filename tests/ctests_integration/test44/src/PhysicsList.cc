...
void PhysicsList::SetBuilderList2()
{
  ...
  hadronPhys.push_back(new G4StoppingPhysics(verboseLevel));
  hadronPhys.push_back(new G4IonPhysicsXS(verboseLevel));
  hadronPhys.push_back(new G4IonElasticPhysics(verboseLevel));
  if (!G4HadronicParameters::Instance()->EnableNeutronGeneralProcess())
  {
    hadronPhys.push_back(new G4NeutronTrackingCut(verboseLevel));
  }
}
...
{
  G4cout << "### PhysicsLists available: FTFP_BERT FTFP_BERT_EMV FTFP_BERT_EMX FTF_BIC" << G4endl;
  G4cout << "                            LHEP LHEP_EMV QBBC QGS_BIC QGSP" << G4endl;
  G4cout << "                            QGSC_BERT QGSP_BERT QGSP_BERT_EMV QGSP_BIC_EMY" << G4endl;
  G4cout << "                            QGSP_BERT_EMX QGSP_BERT_HP QGSP_BIC QGSP_BIC_HP" << G4endl;
}
