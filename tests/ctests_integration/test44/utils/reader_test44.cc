...
int main(int argc, char** argv)
{
  ...
  finName[1] = fname + "_opt2.root";
  finName[2] = fname + "_opt3.root";

  TString legend[3] = {"QBBC opt0", "QBBC opt2", "QBBC opt3"};
  ...
  break;
    ...
    */
    ...
    if (j == 0)
    {
      ...
      hh[j] = (TH1F*)f0.Get("h1");
      cout << "Histo is accessed " << hh[j] << endl;
      hh[j]->Print();
    }
    else if (j == 1)
    {
    ...
    // double ymax = 1;
    double ymax = hh[j]->GetMaximumStored();
    cout << "Ymax= " << ymax << endl;
    hh[j]->Scale(1. / ymax);
    cout << "Draw" << endl;
    hh[j]->SetLineColor(j + 2);
  ...
  TString fout = "A_" + fnm[idx] + "_water.png";
  c1->Print(fout);
  return 0;
}
