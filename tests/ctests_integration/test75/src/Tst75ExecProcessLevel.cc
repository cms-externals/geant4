...
Tst75ExecProcessLevel::Tst75ExecProcessLevel(const TstReader* pset) : ExecProcessLevel(pset)
{
  InitProcess(pset);
  InitSetup(pset);
  InitBeam(pset);
  ...
  fXSecOnTarget = fProcWrapper->GetElementCrossSection(
    &dParticle, GetTarget()->GetCurrentMaterial()->GetElement(0),
...
void Tst75ExecProcessLevel::InitProcess(const TstReader* pset)
{
  ...
  if (!fProcWrapper)
  {
    G4cout << " generator " << NameModel << " is unavailable" << G4endl;
    exit(1);
  }
