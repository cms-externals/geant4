...
void Tst50RunAction::EndOfRunAction(const G4Run* aRun)
{
  ...
  primary = (Tst50PrimaryGeneratorAction*)(runManager->GetUserPrimaryGeneratorAction());
  G4double primaryParticleEnergy = primary->GetInitialEnergy();
  G4String primaryParticleName = primary->GetParticle();
  detector = (Tst50DetectorConstruction*)(runManager->GetUserDetectorConstruction());
  ...
  if (primaryParticleName == "gamma")
  {
    ...
    G4double gammaTransmittedFractionError =
      1 / (gammaTransmitted * (targetThickness * absorberMaterialDensity));
    G4double gammaAttenuationCoefficient =
      -(std::log(gammaTransmittedFraction)) / (targetThickness * absorberMaterialDensity);
    Tst50AnalysisManager* analysis = Tst50AnalysisManager::getInstance();
    analysis->AttenuationGammaCoeffiecient(runID, primaryParticleEnergy / MeV,
                                           gammaAttenuationCoefficient / (cm2 / g),
                                           gammaTransmittedFractionError / (cm2 / g));
  }
  ...
  if (flag)
  {
    if (primaryParticleName == "e-" || primaryParticleName == "e+")
    {
      ...
      G4double particleBackscatteredFractionError =
        (std::sqrt(particleBackscattered)) / numberEvents;
      Tst50AnalysisManager* analysis = Tst50AnalysisManager::getInstance();
      analysis->ParticleTransmission(runID, primaryParticleEnergy / MeV,
                                     particleTransmittedFraction, particleBackscatteredFraction,
                                     particleTransmittedFractionError,
                                     particleBackscatteredFractionError);
