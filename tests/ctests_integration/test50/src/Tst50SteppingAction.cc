...
#ifdef G4ANALYSIS_USE
Tst50SteppingAction::Tst50SteppingAction(Tst50PrimaryGeneratorAction* primary, Tst50RunAction* run,
                                         Tst50DetectorConstruction* det)
  : primaryAction(primary), runAction(run), detector(det)
#else
Tst50SteppingAction::Tst50SteppingAction(Tst50PrimaryGeneratorAction* primary, Tst50RunAction* run,
                                         Tst50DetectorConstruction*)
  : primaryAction(primary), runAction(run)
#endif
{}
...
void Tst50SteppingAction::UserSteppingAction(const G4Step* aStep)
{
  ...
  G4double particleZMomentumDirection = aStep->GetTrack()->GetMomentumDirection().z();

  // ---------- primary transmitted particles test -----------//
  if (aStep->GetTrack()->GetParentID() == 0)
  {
    G4double charge = aStep->GetTrack()->GetDynamicParticle()->GetDefinition()->GetPDGCharge();
    ...
    if (aStep->GetTrack()->GetParentID() == 0)
    {
      ...
      if (charge != 0)
      {
        ...
        // Stopping Power test ...
        {
          ...
          if (0 == initialStepXPosition && 0 == initialStepYPosition && 0 == initialStepZPosition)
          {
            ...
            if (stepLength != 0)
            {
              ...
              analysis->StoppingPower(runID, primaryParticleEnergy / MeV,
        ...
        if (0.0 == ParticleKineticEnergy)
        {
          ...
          analysis->CSDARange(runID, primaryParticleEnergy / MeV, nistRange / (g / cm2));
