...
void RunAction::EndOfRunAction(const G4Run* aRun)
{
  ...
  for (i = 0; i < nLbin; i++)
  {
    ...
    rmsELongit[i] =
      norme * std::sqrt(std::abs(NbOfEvents * sumE2Longit[i] - sumELongit[i] * sumELongit[i]));

    MeanELongitCumul[i] = norme * sumELongitCumul[i];
    rmsELongitCumul[i] = norme
                         * std::sqrt(std::abs(NbOfEvents * sumE2LongitCumul[i]
  ...
  for (i = 0; i < nRbin; i++)
  {
    ...
    rmsERadial[i] =
      norme * std::sqrt(std::abs(NbOfEvents * sumE2Radial[i] - sumERadial[i] * sumERadial[i]));

    MeanERadialCumul[i] = norme * sumERadialCumul[i];
    rmsERadialCumul[i] = norme
                         * std::sqrt(std::abs(NbOfEvents * sumE2RadialCumul[i]
  ...
  G4double rmsChargTrLength =
    norme
    * std::sqrt(std::abs(NbOfEvents * sum2ChargTrLength - sumChargTrLength * sumChargTrLength));

  G4double MeanNeutrTrLength = norme * sumNeutrTrLength;
  G4double rmsNeutrTrLength =
    norme
    * std::sqrt(std::abs(NbOfEvents * sum2NeutrTrLength - sumNeutrTrLength * sumNeutrTrLength));
  ...
  for (i = 0; i < nLbin; i++)
  {
    ...
    G4cout << std::setw(8) << inf << "->" << std::setw(5) << sup << " radl: " << std::setw(7)
           ...
           << "      0->" << std::setw(5) << sup << " radl: " << std::setw(7) << MeanELongitCumul[i]
           << "%  " << std::setw(7) << rmsELongitCumul[i] << "% " << G4endl;
  }
  ...
  for (i = 0; i < nRbin; i++)
  {
    ...
    G4cout << std::setw(8) << inf << "->" << std::setw(5) << sup << " radl: " << std::setw(7)
           ...
           << "      0->" << std::setw(5) << sup << " radl: " << std::setw(7) << MeanERadialCumul[i]
           << "%  " << std::setw(7) << rmsERadialCumul[i] << "% " << G4endl;
  }
  G4cout << G4endl;
  G4cout << std::setw(37) << "SUMMARY" << G4endl;
  G4cout << std::setw(42) << "energy deposit : " << std::setw(7) << MeanELongitCumul[nLbin - 1]
         << " % E0 +- " << std::setw(7) << rmsELongitCumul[nLbin - 1] << " % E0" << G4endl;
  G4cout << std::setw(42) << "charged traklen: " << std::setw(7) << MeanChargTrLength << " radl +- "
         << std::setw(7) << rmsChargTrLength << " radl" << G4endl;
  G4cout << std::setw(42) << "neutral traklen: " << std::setw(7) << MeanNeutrTrLength << " radl +- "
