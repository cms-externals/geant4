...
void DetectorConstruction::DefineMaterials()
{
  ...
  G4Material* PbWO = new G4Material("PbWO4", density = 8.28 * g / cm3, ncomponents = 3);
  PbWO->AddElement(O, natoms = 4);
  PbWO->AddElement(Pb, natoms = 1);
  PbWO->AddElement(W, natoms = 1);

  G4Material* w = new G4Material("Tungsten", density = 19.30 * g / cm3, ncomponents = 1);
  ...
  G4Material* pb = new G4Material("Lead", density = 11.35 * g / cm3, ncomponents = 1);
...
G4VPhysicalVolume* DetectorConstruction::ConstructVolumes()
{
  ...
  G4double deltaL = dLradl * Radl, dR = dRradl * Radl;
  EcalLength = nLtot * deltaL;
  EcalRadius = nRtot * dR;
  ...
  G4LogicalVolume* logicW = new G4LogicalVolume(solidW, worldMaterial, "World");
  G4VPhysicalVolume* world = new G4PVPlacement(0, G4ThreeVector(), "World", logicW, 0, false, 0);
  ...
  G4VPhysicalVolume* physVV = new G4PVPlacement(0, G4ThreeVector(0., 0., -EcalLength * 0.6), "VolV",
                                                logicVV, world, false, 0);
  ...
  for (G4int i = 0; i < nRtot; i++)
  {
    ...
    physiRing = new G4PVPlacement(0, G4ThreeVector(), "Ring", logicRing, physiEcal, false, i);

    // Slice
    solidSlice = new G4Tubs("Slice", i * dR, (i + 1) * dR, 0.5 * deltaL, 0., 360 * deg);
    logicSlice = new G4LogicalVolume(solidSlice, myMaterial, "Slice", 0, 0, 0);
    if (nLtot > 1)
      physiSlice = new G4PVReplica("Slice", logicSlice, physiRing, kZAxis, nLtot, deltaL);
    else
      physiSlice = new G4PVPlacement(0, G4ThreeVector(), "Slice", logicSlice, physiRing, false, 0);
  }

  G4cout << "Absorber is " << G4BestUnit(EcalLength, "Length") << " of " << myMaterial->GetName()
         << G4endl;
  ...
  new G4PVPlacement(0, G4ThreeVector(0., 0., EcalLength * 0.8), "VolM", logicM, world, false, 0);
...
void DetectorConstruction::SetMaterial(const G4String& materialChoice)
{
  ...
  if (pttoMaterial && myMaterial != pttoMaterial)
  {
    ...
    if (logicRing)
    {
      logicRing->SetMaterial(myMaterial);
    }
    if (logicSlice)
    {
      logicSlice->SetMaterial(myMaterial);
    }
