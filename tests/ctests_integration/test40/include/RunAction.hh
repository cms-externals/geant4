...
inline void RunAction::fillPerTrack(G4double charge, G4double trkLength)
{
  if (charge != 0.)
  {
    ChargTrLength += trkLength;
  }
  else
  {
    NeutrTrLength += trkLength;
  }
}

//....oooOO0OOooo........oooOO0OOooo........oooOO0OOooo........oooOO0OOooo......

inline void RunAction::fillPerStep(G4double dEstep, G4int Lbin, G4int Rbin)
{
  if (Lbin < nLbin && Lbin >= 0)
  {
    dEdL[Lbin] += dEstep;
  }
  if (Rbin < nRbin && Rbin >= 0)
  {
    dEdR[Rbin] += dEstep;
  }
}
...
inline void RunAction::particleFlux(G4ParticleDefinition* particle, G4int Lplan)
{
  if (Lplan < nLbin && Lplan >= 0)
  {
    if (particle == G4Gamma::Gamma())
      ...
      electronFlux[Lplan] += 1.0;
    else if (particle == G4Positron::Positron())
      positronFlux[Lplan] += 1.0;
  }
}
