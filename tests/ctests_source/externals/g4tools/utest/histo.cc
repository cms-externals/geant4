...
#include "detail/htoc.hh"
#include "detail/randd.hh"
...
static bool test_p1d_rand(std::ostream& a_out, bool a_verbose)
{
  ...
  {
    for (unsigned int count = 0; count < entries; count++)
      h.fill(rgx.shoot() - 1, rgy.shoot(), 1);
    if (!tools::equal<double>(a_out, __FILE__, __LINE__, h.mean(), 0, 1e-3, tools::dfabs))
      return false;
    if (!tools::equal<double>(a_out, __FILE__, __LINE__, h.rms(), 0.5, 1e-3, tools::dfabs))
      return false;
...
bool test_h2d_rand(std::ostream& a_out, bool a_verbose)
{
  ...
  for (unsigned int count = 0; count < entries; count++)
    h.fill(rgx.shoot(), rgy.shoot(), 0.8);
...
static bool test_h3d_rand(std::ostream& a_out, bool a_verbose)
{
  ...
  for (unsigned int count = 0; count < entries; count++)
    h.fill(rgx.shoot(), rgy.shoot(), rgz.shoot());
  if (!tools::equal<double>(a_out, __FILE__, __LINE__, h.mean_x(), 1, 1e-3, tools::dfabs))
    return false;
  if (!tools::equal<double>(a_out, __FILE__, __LINE__, h.rms_x(), 0.5, 1e-3, tools::dfabs))
    return false;
...
static bool test_htoc(std::ostream& a_out, bool a_verbose)
{
...

  {
    ...
    if (!ch) {
      a_out << "chisto_duiuid_alloc() failed." << std::endl;
      return false;
    }
    ...
    if (!tools::equal<double>(a_out, __FILE__, __LINE__, h.mean(), hc.mean(), tolerance,
                              tools::dfabs))
      return false;
    if (!tools::equal<double>(a_out, __FILE__, __LINE__, h.rms(), hc.rms(), tolerance,
                              tools::dfabs))
      return false;
  }

  {
    ...
    hc.copy_from_data(hdata);
    if (!tools::equal<double>(a_out, __FILE__, __LINE__, h.mean_x(), hc.mean_x(), tolerance,
                              tools::dfabs))
      return false;
    if (!tools::equal<double>(a_out, __FILE__, __LINE__, h.rms_x(), hc.rms_x(), tolerance,
                              tools::dfabs))
      return false;
    if (!tools::equal<double>(a_out, __FILE__, __LINE__, h.mean_y(), hc.mean_y(), tolerance,
                              tools::dfabs))
      return false;
    if (!tools::equal<double>(a_out, __FILE__, __LINE__, h.rms_y(), hc.rms_y(), tolerance,
                              tools::dfabs))
      return false;
  }

  {
    ...
    {
      for (unsigned int count = 0; count < 20000; count++)
        h.fill(rgx.shoot(), rgy.shoot(), 1);
    }

    cprofile_duiuidd* cp = tools::histo::cprofile_duiuidd_alloc(h.get_histo_data());
    if (!cp) {
      a_out << "cprofile_duiuidd_alloc() failed." << std::endl;
      return false;
    }
    ...
    if (!tools::equal<double>(a_out, __FILE__, __LINE__, h.mean(), pc.mean(), tolerance,
                              tools::dfabs))
      return false;
    if (!tools::equal<double>(a_out, __FILE__, __LINE__, h.rms(), pc.rms(), tolerance,
                              tools::dfabs))
      return false;
    if (!tools::equal<double>(a_out, __FILE__, __LINE__, h.get_Svw(), pc.get_Svw(), tolerance,
                              tools::dfabs))
      return false;
    if (!tools::equal<double>(a_out, __FILE__, __LINE__, h.get_Sv2w(), pc.get_Sv2w(), tolerance,
                              tools::dfabs))
      return false;
  }
