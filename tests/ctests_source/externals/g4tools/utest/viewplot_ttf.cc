...
void plot_file(tools::viewplot& a_viewer, const std::string& a_filename, unsigned int a_columns,
               unsigned int a_rows, float a_plotter_scale, const std::string& a_style, int a_n,
               tools::histo::h1d& a_h1, bool a_verbose = false)
{
  if (a_verbose)
    a_viewer.out() << "plot file : file name " << a_filename << ", " << a_n << " plots in "
                   << a_columns << "x" << a_rows << ", layout " << a_style << " ..." << std::endl;

  a_viewer.open_file(a_filename, true);

  a_viewer.plots()
    .init_sg();  // it will recreate the sg::plotters and then reset the styles on new ones.
...
inline bool test_write_file(std::ostream& a_out, bool a_verbose, const std::string& a_file,
                            unsigned int a_cols, unsigned int a_rows, float a_plotter_scale,
                            const std::string& a_style)
{
  ...
  plot_file(viewer, a_file, a_cols, a_rows, a_plotter_scale, a_style, a_cols * a_rows, h1,
            a_verbose);
...
bool test_viewplot_ttf(std::ostream& a_out, bool a_verbose)
{
  ...
  if (!tools::test_with_file(a_out, a_verbose, "out_viewplot_1x2_r_", ".ps", _remove,
                             utest_viewplot_out_1x2_r_ps, test_write_file, 1, 2, 0.9f,
                             "ROOT_default")
      && stop_if_failed)
    return false;
  if (!tools::test_with_file(a_out, a_verbose, "out_viewplot_1x2_h_", ".ps", _remove,
                             utest_viewplot_out_1x2_h_ps, test_write_file, 1, 2, 1, "hippodraw")
      && stop_if_failed)
    return false;
  if (!tools::test_with_file(a_out, a_verbose, "out_viewplot_1x2_d_", ".ps", _remove,
                             utest_viewplot_out_1x2_d_ps, test_write_file, 1, 2, 1, "inlib_default")
      && stop_if_failed)
    return false;
