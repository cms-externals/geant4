...
inline void chisto_duiuid_assign(chisto_duiuid& a_c,
                                 const histo_data<double, unsigned int, unsigned int, double>& a_hd)
{
  ...
  // etc :
  a_c.m_in_range_plane_Sxyw =
    cmem_alloc_copy<TC>(vec_data(a_hd.m_in_range_plane_Sxyw), dim_planes(a_hd.m_dimension));
  {
    ...
    {
      a_c.m_annotations[index] = str_dup((*it).first.c_str());
      index++;
      a_c.m_annotations[index] = str_dup((*it).second.c_str());
      index++;
    }
  }
  // fast getters :
...
inline chisto_duiuid*
chisto_duiuid_alloc(const histo_data<double, unsigned int, unsigned int, double>& a_hd)
{
...
inline void chisto_duiuid_clear(chisto_duiuid& a_c)
{
...

  {
    for (unsigned int ibin = 0; ibin < a_c.m_bin_number; ibin++)
      cmem_free(a_c.m_bin_Sxw[ibin]);
    cmem_free(a_c.m_bin_Sxw);
  }
  {
    for (unsigned int ibin = 0; ibin < a_c.m_bin_number; ibin++)
      cmem_free(a_c.m_bin_Sx2w[ibin]);
...

  {
    for (unsigned int iaxis = 0; iaxis < a_c.m_dimension; iaxis++)
      caxis_dui_free(a_c.m_axes[iaxis]);
    cmem_free(a_c.m_axes);
  }

  {
    for (unsigned int i = 0; i < (2 * a_c.m_number_of_annotations); i++)
      str_del(a_c.m_annotations[i]);
...
inline void histo_data_duiuid_assign(histo_data<double, unsigned int, unsigned int, double>& a_hd,
...
inline cprofile_duiuidd*
cprofile_duiuidd_alloc(const profile_data<double, unsigned int, unsigned int, double, double>& a_pd)
{
...
inline void
profile_data_duiuidd_assign(profile_data<double, unsigned int, unsigned int, double, double>& a_pd,
