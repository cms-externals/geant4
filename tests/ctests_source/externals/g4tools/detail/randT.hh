...
class rgauss
{
  ...
  public:
    rgauss(const rgauss& a_from)
      : m_flat(a_from.m_flat), m_mean(a_from.m_mean), m_std_dev(a_from.m_std_dev)
    {}
...
class rpoiss
{
  ...
  public:
    UINT shoot(const REAL& a_two_pi, REAL (*a_sqrt)(REAL), REAL (*a_log)(REAL), REAL (*a_exp)(REAL),
               REAL (*a_cos)(REAL)) const
    {
      UINT number = 0;
      if (m_mean <= 16) {
