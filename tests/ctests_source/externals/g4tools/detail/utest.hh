...
class test
{
  ...
  public:
    ...
    test(bool a_do_it, func a_func)
      : m_do_it(a_do_it), m_func(a_func), m_func2(0), m_func3(0), m_cfunc(0)
    {}
    test(bool a_do_it, func2 a_func)
      : m_do_it(a_do_it), m_func(0), m_func2(a_func), m_func3(0), m_cfunc(0)
    {}
    test(bool a_do_it, func3 a_func)
      : m_do_it(a_do_it), m_func(0), m_func2(0), m_func3(a_func), m_cfunc(0)
    {}
    test(bool a_do_it, cfunc a_func)
      : m_do_it(a_do_it), m_func(0), m_func2(0), m_func3(0), m_cfunc(a_func)
    {}
    virtual ~test() {}
...

      {
        tools_vforcit(args::arg, a_args.get_args(), it)
        {
          ...
          tools_vforit(named_test, *this, itn)
          {
            ...
            if (key == std::string("-no_" + name)) {
              found = true;
              break;
            }  // valid arg, but not a test.
          }
          if (!found) {
