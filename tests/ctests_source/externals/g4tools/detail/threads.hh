...
class threads
{
  ...
  public:
    threads(std::ostream& a_out, bool a_verbose = false)
      : m_out(a_out), m_mutex(), m_threads(), m_verbose(a_verbose)
    {
      m_mutex.initialize();
    }
    virtual ~threads() { stop(); }

  protected:
    threads(const threads& a_from)
      : m_out(a_from.m_out), m_mutex(), m_threads(), m_verbose(a_from.m_verbose)
    {
      m_mutex.initialize();
    }
  ...
  public:
    ...
    void stop()
    {
      ...
      tools_typename_vforit(idargs, m_threads, it)
      {
        ...
        while (!exit_set(*_args)) {
          sleep_milli_secs(100);
        }
...
class thread_streambuf : public std::streambuf
{
  public:
    thread_streambuf(mutex& a_mutex, const std::string& a_prefix)
      : m_mutex(a_mutex), m_prefix(a_prefix), m_buffer(0), m_size(4095), m_count(0)
    {
      m_buffer = new char[m_size + 1];
    }

    virtual ~thread_streambuf() { delete[] m_buffer; }

  private:
    thread_streambuf(const thread_streambuf& a_from)
      : std::basic_streambuf<char>(), m_mutex(a_from.m_mutex)
    {}
