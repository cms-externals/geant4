...
class args
{
  ...
  public:
    ...
    args(const std::vector<std::string>& a_args, bool a_strip = false) { add(a_args, a_strip); }
  ...
  public:
    ...
    bool find(const std::string& a_key, std::string& a_value,
              const std::string& a_def = std::string()) const
    {
      tools_vforcit(arg, m_args, it)
      {
        if ((*it).first == a_key)
    ...
    bool add(const std::string& a_key, const std::string& a_value = std::string(),
    ...
    bool insert_begin(const std::string& a_key, const std::string& a_value = std::string(),
                      bool a_override = true)
    {
      if (a_override)
      {
    ...
    void add_keyvals(const std::vector<std::string>& a_args, bool a_strip = false)
    {
      ...
      for (std::vector<std::string>::const_iterator it = a_args.begin(); it != a_args.end();
           it += 2)
      {
        if (a_strip)
        {
    ...
    void add(const args& a_from) { tools_vforcit(arg, a_from.m_args, it) m_args.push_back(*it); }
    ...
    void remove_first()
    {
      if (m_args.size()) m_args.erase(m_args.begin());
    }
    void remove_last()
    {
      if (m_args.size()) m_args.erase(m_args.end() - 1);
    }
    ...
    bool file(std::string& a_file) const
    {
      ...
      if ((m_args.size() > 1)  // first arg is the program name !
          && last(slast, _s) && (slast.find('-') != 0) && (_s.empty()))
      {
    ...
    {  // true and not false as in the upper.
    ...
    void dump(std::ostream& a_out, const std::string& a_comment = std::string(),
              const std::string& a_prefix = std::string()) const
    {
      if (a_comment.size()) a_out << a_comment << std::endl;
      tools_vforcit(arg, m_args, it)
      {
        a_out << a_prefix << "key = " << sout((*it).first) << ", value = " << sout((*it).second)
              << std::endl;
      }
    }
