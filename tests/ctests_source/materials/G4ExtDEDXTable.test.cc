...
bool Test(const G4String& fileName)
{
  // ########################################################################
  ...
  G4double E[31] = {.025, .03, .04, .05, .06, .07, .08, .09, .1, .15, .2, .25, .3, .4, .5, .6,
                    .7,   .8,  .9,  1,   1.5, 2,   2.5, 3,   4,  5,   6,  7,   8,  9,  10};

  // Lithium ions in hydrogen
  G4double H_3[31] = {8.31,  8.919, 9.838, 10.46, 10.87, 11.14, 11.3,  11.4,   11.43, 11.21, 10.73,
                      10.21, 9.696, 8.773, 7.989, 7.326, 6.761, 6.275, 5.853,  5.485, 4.175, 3.377,
                      2.843, 2.46,  1.948, 1.62,  1.393, 1.225, 1.095, 0.9921, 0.9082};

  G4PhysicsFreeVector* physVecZ3MatH = new G4PhysicsFreeVector(31, E[0] * MeV, E[30] * MeV, true);

  for (size_t i = 0; i < 31; i++) {
    physVecZ3MatH->PutValues(i, E[i] * MeV, H_3[i] * factor);
  }
  physVecZ3MatH->FillSecondDerivatives();

  // Lithium ions in tissue
  G4double A_3[31] = {2.748,  2.992,  3.386,  3.674,  3.877,  4.016,  4.108, 4.166,
                      4.2,    4.186,  4.059,  3.906,  3.751,  3.458,  3.2,   2.974,
                      2.777,  2.603,  2.449,  2.313,  1.807,  1.485,  1.263, 1.1,
                      0.8801, 0.7372, 0.6368, 0.5623, 0.5046, 0.4586, 0.4209};

  G4PhysicsFreeVector* physVecZ3MatTissue =
    new G4PhysicsFreeVector(31, E[0] * MeV, E[30] * MeV, true);

  for (size_t i = 0; i < 31; i++) {
    physVecZ3MatTissue->PutValues(i, E[i] * MeV, A_3[i] * factor);
  }
  physVecZ3MatTissue->FillSecondDerivatives();
  ...
  G4cout << G4endl << "##### Testing AddPhysicsVector() #####" << G4endl;
  ...
  G4cout << G4endl << "##### Testing GetPhysicsVector() #####" << G4endl;
  ...
  G4cout << G4endl << "##### Testing StorePhysicsTable() #####" << G4endl;
  ...
  G4cout << G4endl << "##### Testing RemovePhysicsVector() #####" << G4endl;
  ...
  G4cout << G4endl << "##### Testing RetrievePhysicsTable() #####" << G4endl;
...
int main()
{
  return Test("usertable.dat") ? EXIT_SUCCESS : EXIT_FAILURE;
}
