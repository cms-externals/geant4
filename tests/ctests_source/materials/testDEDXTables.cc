...
G4bool Compare(G4PhysicsVector* physicsVector, G4double refDEDXValue, G4double kinEnergyPerNucleon,
               G4int atomicNumberIon, const G4String& matIdentifier)
{
  ...
  if (physicsVector != 0)
  {
    ...
    if (kinEnergyPerNucleon <= upperEnergyEdge && kinEnergyPerNucleon >= lowerEnergyEdge)
    {
      G4bool b;
  ...
  G4cout << " E/A1(MeV) = " << kinEnergyPerNucleon / MeV << ", Z(Ion) = " << atomicNumberIon
         << ", Mat = " << matIdentifier
         << " -- dE/dx(MeV*cm2/mg) = " << tableDEDXValue / (MeV * cm2 / (0.001 * g))
         << ", Ref(MeV*cm2/mg) = " << refDEDXValue / (MeV * cm2 / (0.001 * g)) << ":";

  G4double eps = 0.0001 * (MeV * cm2 / (0.001 * g));

  if (refDEDXValue < (tableDEDXValue - eps) || refDEDXValue > (tableDEDXValue + eps))
  {
...
G4bool CompareDEDXValues(G4VIonDEDXTable* table, G4double refDEDXValue,
                         G4double kinEnergyPerNucleon, G4int atomicNumberIon, G4int atomicNumberMat)
{
  G4PhysicsVector* physicsVector = table->GetPhysicsVector(atomicNumberIon, atomicNumberMat);

  std::stringstream ss;
  ss << atomicNumberMat;

  return Compare(physicsVector, refDEDXValue, kinEnergyPerNucleon, atomicNumberIon, ss.str());
}

G4bool CompareDEDXValues(G4VIonDEDXTable* table, G4double refDEDXValue,
                         G4double kinEnergyPerNucleon, G4int atomicNumberIon,
                         const G4String& matName)
{
  G4PhysicsVector* physicsVector = table->GetPhysicsVector(atomicNumberIon, matName);
  return Compare(physicsVector, refDEDXValue, kinEnergyPerNucleon, atomicNumberIon, matName);
}

G4bool TestApplicability(G4VIonDEDXTable* table, G4bool shouldBeApplicable, G4int atomicNumberIon,
                         const G4String& matName)
{
  G4bool isApplicable = table->IsApplicable(atomicNumberIon, matName);

  G4cout << " Z(Ion) = " << atomicNumberIon << ", Mat = " << matName << " -- ";
...
int main()
{
  ...
  G4SimpleMaterialStoppingICRU73* icru73elem = new G4SimpleMaterialStoppingICRU73();
