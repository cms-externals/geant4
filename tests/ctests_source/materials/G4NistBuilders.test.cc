...
int main()
{
  ...
  // define Materials
  //
  G4NistMaterialBuilder nistMat(&nistElm, verbose = 0);

  // nistMat.ListNistSimpleMaterials();

  G4Material* Al = nistMat.FindOrBuildMaterial("G4_Al");
  nistMat.FindOrBuildMaterial("G4_lAr", buildIsotopes = false);
  nistMat.FindOrBuildMaterial("G4_Cu", buildIsotopes = false);
  G4Material* Pb = nistMat.FindOrBuildMaterial("G4_Pb");

  // nistMat.ListNistCompoundMaterials();

  G4Material* H2O = nistMat.FindOrBuildMaterial("G4_WATER");
  ...
  for (G4int j = 0; j < 3; j++) {
    elm.push_back(gelSiElm[j]);
    atom.push_back(gelSiAtom[j]);
    w.push_back(gelSiW[j]);
  }

  // by atom count
  G4Material* Aerog = nistMat.ConstructNewMaterial("gel_silicate1", elm, atom, 0.2);
