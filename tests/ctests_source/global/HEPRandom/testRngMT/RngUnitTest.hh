...
void RngTest(G4SharedFuture<void> fut, uintmax_t n, atomic_size_t* gfail)
{
  ...
  // inform
  l.lock();
  std::cout << "Thread #" << _tid << " is initialized...\n";
  l.unlock();

  // barrier until signaled
  fut.wait();
  l.lock();
  std::cout << "Thread #" << _tid << " starting...\n";
  l.unlock();
  ...
  for (uintmax_t i = 0; i < n; ++i) {
    try {
      ...
      if (!std::isfinite(rand) || rand < 0.0 || !(rand < 1.0)) ++lfail;
    }
    catch (...) {
      ++lfail;
    }
  ...
  l.lock();
  std::cout << "--> Total invalid random numbers: " << lfail << " out of " << n << std::endl;
...
template<typename _RNG_t>
uintmax_t RngRun(uintmax_t nthread = G4Threading::G4GetNumberOfCores(), uintmax_t nitr = 50000)
{
  ...
  const uint_least8_t max_ret = std::numeric_limits<uint_least8_t>::max();
