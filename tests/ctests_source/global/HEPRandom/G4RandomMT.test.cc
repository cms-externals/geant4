...
// ********************************************************************
//
// Test for RNG algorithms for Geant4 sequential & multi-threaded
//
// Author: A. Dotti
//
//
  ...
  if (!pdefaultEngine) {
    switch (engineSwitch) {
      ...
      case 5:
        ...
        break;
      case 6:
        pdefaultEngine = new CLHEP::RanluxppEngine();
        break;
      default:
        std::cerr << "Non valid selection for RandomEngine, valid values from 0 (default) to 6"
                  << std::endl;
        abort();
        break;
    }
...
G4ThreadFunReturnType mythreadfunc2(G4ThreadFunArgType arg)
{
  ...
  G4Random::setTheSeed(seed);
  char buf[200];
  snprintf(buf, sizeof buf, "%s.%d", STATFILE,
           int(std::hash<std::thread::id>{}(G4Threading::G4GetPidId())));
...
G4ThreadFunReturnType mythreadfunc3(G4ThreadFunArgType arg)
{
  ...
  for (int i = (conf->halfway) + 1; i < conf->histories; ++i) {
    ...
    if (val != run1[i]) {
      ...
      std::cerr << "Error for run1[" << i << "]=" << run1[i] << " , value=" << val
                << " (ThreadID:" << G4GetPidId() << ")" << std::endl;
    }
  }
  return (G4ThreadFunReturnType)NULL;
}
...
G4ThreadFunReturnType mythreadfunc4(G4ThreadFunArgType arg)
{
  ...
  G4Random::setTheSeed(std::hash<std::thread::id>{}(G4Threading::G4GetPidId()));
...
Ret_t sequence(rng_t rng)
{
  ...
  // G4Random::setTheSeed( seed ); //This is not enough for MTwistEngine, need full status
  for (long n = 0; n < max; ++n)
    value2 = rng();
  if (value1 == value2)
    return SUCCESS;
...
Ret_t threads3(rng_t rng)
{
  ...
  for (int i = 0; i < NTHREADS; ++i) {
    ...
    G4Thread* tr = &threads[i];
    G4THREADCREATE(tr, mythreadfunc3, &myarg);
  }
  for (int i = 0; i < NTHREADS; ++i) {
    G4THREADJOIN(threads[i]);
  }
...
Ret_t threads4(rng_t rng)
{
  ...
  for (int i = 0; i < NTHREADS; ++i) {
    arg_t myarg = {G4RandFlat::shootInt(100000), TRIALS, TRIALS / 2, rng};
    G4Thread* tr = &threads[i];
    G4THREADCREATE(tr, mythreadfunc4, &myarg);
  }
  for (int i = 0; i < NTHREADS; ++i) {
    G4THREADJOIN(threads[i]);
  }
...
Ret_t threads5(rng_t rng)
{
  ...
  for (int i = 0; i < NTHREADS; ++i) {
    arg_t myarg = {G4RandFlat::shootInt(100000), TRIALS, TRIALS / 2, rng};
    G4Thread* tr = &threads[i];
    G4THREADCREATE(tr, mythreadfunc5, &myarg);
  }
  for (int i = 0; i < NTHREADS; ++i) {
    G4THREADJOIN(threads[i]);
  }
...
tt tests[] = {
  ...
  {&sequence, "WeakSequential",
   "Weak Reproducibility: run twice from same initial seed. Check final values are the same."},
  {&repro, "StrongSequential",
   "Strong Reproducibility: run twice with reseed half way. Check final values are the same."},
#ifdef G4MULTITHREADED
  {&threads1, "MTSimple",
   "MT simple: start threads with same random seed, check final values are the same."},
  {&threads2, "MTWeak", "MT Weak Reproducibility: Same as in Weak reproducibility for sequential."},
  {&threads3, "MTStrong",
   "MT Strong Reproducibility: same as Strong reproducibility for sequential."},
  {&threads4, "MTNoMem1",
   "MT no-memory: threads are reseeded with common seed halfway, check final values area the "
   "same."},
  {&threads5, "MTNoMem2",
   "MT no-memory 2: threads start with same seed and are reseeded thalfway with different seed, "
   "check final values are different."},
#endif
  {NULL, NULL, NULL}};
...
int main(int, char**)
{
...
#endif
  std::cout << "Number histories: " << TRIALS << std::endl;
  ...
  // 0-6
  for (int t = 0; t < 7; ++t)  // Loop on engine types
  {
    if (pdefaultEngine) {
