...
HepJamesRandom theHepJamesEngine;
...
void even_test(int seq)
{
  ...
  for (int k = 0; k < seq; ++k)
  {
    ...
    if (k == (seq % 100))
    {
      G4cout << "\tDistribution of a single run (rand_valuex100) ..." << G4endl << G4endl;
  ...
  G4cout << "\tDistribution of Chi-squared for " << seq << " sequences ..." << G4endl << G4endl;
  G4cout << "\t\t\t       %" << "\tChi-sq " << "\tExpected" << G4endl;
  for (j = 0; j < 8; ++j)
    G4cout << "\t\t\tX <= " << perc[j] << ":\t" << chi[j] / fac << "\t" << prob[j] << G4endl;
  G4cout << G4endl;
  G4cout << G4endl;
...
void corr_test(int seq)
{
  ...
  G4cout << "Mean value predicted : " << mv << "\t"
         << "Standard deviation predicted: " << std << G4endl << "Mean value effective : " << mve
         << "\t"
...
static unsigned long twotoj[16] = {0x1L,    0x2L,    0x4L,    0x8L,   0x10L,  0x20L,
                                   0x40L,   0x80L,   0x100L,  0x200L, 0x400L, 0x800L,
...
void spat_test()
{
  ...
  for (j = 1; j <= 15; j++)
  {
    ...
    if ((jpower <= 32768) && (jpower != 0))
      G4cout << "\t\t\t" << jpower << "\t" << yprob[1] << "\t" << yprob[2] << "\t" << yprob[3]
             << G4endl;
  }
  G4cout << G4endl;
  G4cout << "\t\t\tactual" << "\t" << PI << "\t" << 4. * PI / 3. << "\t" << .5 * PI * PI << G4endl
         << G4endl;
...
void init()
{
  ...
  G4cout << "-------------------------- Random distribution test ---------------------------"
         << G4endl;
  G4cout << "                           ------------------------                            "
         << G4endl;
...
void start_test()
{
  ...
  G4cout << G4endl << G4endl;
  G4cout << "-------------------------  Test on MixMaxEngine  ----------------------------"
         << G4endl;
  G4cout << G4endl;
  layout(seq);
  G4cout << G4endl << G4endl;
  more = 0;
  G4cout << "---------------------------  Test on HepJamesRandom  ------------------------------"
         << G4endl;
  G4cout << G4endl;
  HepRandom::setTheEngine(&theHepJamesEngine);
  layout(seq);
  G4cout << G4endl << G4endl;
  G4cout << "---------------------  Test on RanluxEngine (luxury 4) ------------------------"
         << G4endl;
  G4cout << G4endl;
  HepRandom::setTheEngine(&theRanluxEngine);
  layout(seq);
  G4cout << G4endl << G4endl;
  G4cout << "-------------------------  Test on RanluxEngine++ ----------------------------"
         << G4endl;
  G4cout << G4endl;
