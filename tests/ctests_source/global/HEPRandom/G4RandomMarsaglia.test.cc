...
// per-thread engine
static G4ThreadLocal CLHEP::HepRandomEngine* pdefaultEngine = nullptr;
...
G4ThreadFunReturnType genRandomNumbers(G4ThreadFunArgType arg)
{
  ...
  auto conf = (arg_t*)arg;
  long seed = conf->seed;
  auto values = new std::list<uint32_t>;
  ...
  return (G4ThreadFunReturnType) nullptr;
}
...
void generate()
{
  ...
  for (int i = 0; i < NTHREADS; ++i)
  {
    ...
    G4THREADCREATE(tr, genRandomNumbers, &myarg);
  }
  for (int i = 0; i < NTHREADS; ++i)
  {
...
int main(int argc, char** argv)
{
  ...
  std::cout << "Running with " << NTHREADS
            << " threads, total number histories: " << TRIALS * NTHREADS << std::endl;
  if (argc > 1)
    InitMe(atoi(argv[1]));
  else
    InitMe();
  ...
  while (!outputLists.empty())
  {
    ...
    for (auto it = nums->begin(); it != nums->end(); ++it)
    {
      ...
      {
        if (i % 1000000 == 0)
          std::cout << std::dec << i << ": 0x" << std::setfill('0') << std::setw(8) << std::hex
                    << anum << " (" << std::dec << anum << ")" << std::endl;
        myFile << std::endl;
      }
    }
    delete nums;
