...
int main()
{
  ...
  timer.Start();
  for (G4int i = 0; i < iMax; ++i)
    isoVector = GMIsotropicSphereRand();
  timer.Stop();
  ...
  for (G4int j = 0; j < jMax; ++j)
  {
    cosThetaTmp = -1. + 2. * j / jMax;
    phiTmp = twopi * j / jMax;
    G4cout << cosThetaTmp << "\t\t" << cosThetaDistr[j] << "\t\t\t" << phiTmp / degree << "\t\t"
           << phi[j] << G4endl;
  }
  ...
  relDelta = rms / mean;
  G4cout << G4endl;
  G4cout << "meanCosThetaDistr = " << mean << " +- " << rms << " with relative error = " << relDelta
         << G4endl;
  ...
  relDelta = rms / mean;
  G4cout << G4endl;
  G4cout << "meanPhiDistr = " << mean << " +- " << rms << " with relative error = " << relDelta
         << "\n"
         << G4endl;
