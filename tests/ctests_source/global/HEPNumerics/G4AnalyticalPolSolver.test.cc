...
G4bool ApproxEqual(const G4double check, const G4double target)
{
  ...
  if (delta < kApproxEqualTolerance)
    result = true;
  else
    result = false;
  return result;
...
int main()
{
  ...
  for (k = 1; k <= 4; k++) {
    ...
    if (ApproxEqual(tmp, a) || ApproxEqual(tmp, b) || ApproxEqual(tmp, c) || ApproxEqual(tmp, d))
      continue;
    else {
      G4cout << "k    = " << k << G4endl;
      G4cout << "a    = " << a << G4endl;
    ...
    case k4:
      ...
      for (i = 0; i < iMax; i++) {
        ...
        for (k = 1; k <= 4; k++) {
          ...
          if (ApproxEqual(tmp, a) || ApproxEqual(tmp, b) || ApproxEqual(tmp, c)
              || ApproxEqual(tmp, d))
            continue;
          else {
