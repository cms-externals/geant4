...
- Readability
  ...
  - English and self-explaining names for constants, variables and functions must be used.
...
- Consistency
  ...
  - The non-inline implementations of each class's code must go into a single source file which must use a filename of the form ``G4<name>.cc``
  - Template and `inline` functions must only be declared in the class body, with the implementation either:

    - Following the class body if the total number of lines occupied by inline function definitions **does
      not exceed the length of the class body**, e.g.
      ...
      .. code-block:: cpp
        ...
        inline void G4Name::DoSomething()
        {
          // Do something...
        }
    ...
    - In a separate header file otherwise. This file must have the same name as the main header but with the suffix
      ``.icc``, e.g. ``G4<name>.hh`` and ``G4<name>.icc`` . This implementation file must be included at the end of the main header file, e.g.
      ...
      with the implementation file ``G4Name.icc`` containing the implementations of the inline functions:
      ...
      .. code-block:: cpp
        ...
        // ... implementation of other inline methods
