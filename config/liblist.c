...
int main (int argc, char** argv) {
  ...
  if(optl) /* This option is used for compiling the file libname.map
	...
	snprintf(libname, sizeof libname, "%s/lib%s.a", libpath, libmapPtr->lib);
        if (access(libname,R_OK))
	{
	  snprintf(libname, sizeof libname, "%s/lib%s.so", libpath, libmapPtr->lib);
          if (!access(libname,R_OK))
	  {
          ...
          {
	    snprintf(libname, sizeof libname, "%s/lib%s.dylib", libpath, libmapPtr->lib);
            if (!access(libname,R_OK))
	    {
              printf("-l%s ",libmapPtr->lib);
	    }
