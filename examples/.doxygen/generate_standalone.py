...
def generate(example_category, example_dir):
    ...
    original_doxyfile_backed_up = False
    ...
    try:
        ...
        # Handle existing Doxyfile by backing it up ---
        if os.path.exists("Doxyfile"):
            backup_path = "Doxyfile.bck"
            if os.path.exists(backup_path):
                os.remove(backup_path) # Remove previous backup if it exists
            os.rename("Doxyfile", backup_path)
            original_doxyfile_backed_up = True
            print(f"Backed up existing Doxyfile to Doxyfile.bck in {example_dir}")
        ...
        if original_doxyfile_backed_up:
            backup_path = "Doxyfile.bck"
            if os.path.exists(backup_path):
                os.rename(backup_path, "Doxyfile")
                print(f"Restored original Doxyfile from Doxyfile.bck in {example_dir}")
...
extended_examples = [
    ...
    "medical/dna/microtrack", "medical/dna/microyz", "medical/dna/molcounters/basic",
    "medical/dna/molcounters/sdcounters", "medical/dna/neuron",
    "medical/dna/phasespace", "medical/dna/pdb4dna", "medical/dna/radial",
