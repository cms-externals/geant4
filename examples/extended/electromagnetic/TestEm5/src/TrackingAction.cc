...
void TrackingAction::PostUserTrackingAction(const G4Track* aTrack)
{
  ...
  G4bool transmit = ((fDirX >= 0. && position.x() >= fXendAbs) && energy > 0.);
  G4bool reflect = ((fDirX <= 0. && position.x() <= fXstartAbs) && energy > 0.);
  G4bool notabsor = (transmit || reflect);

  if (transmit)
    fEventAction->AddEnergyTransmitted(energy);
  if (reflect)
    fEventAction->AddEnergyReflected(energy);
  ...
  if (id > 0) {
    if (direction.x() != 0.0) {
      G4double tet = std::atan(direction.y() / std::abs(direction.x()));
      analysisManager->FillH1(id, tet);
      if (transmit && (flag == 2)) run->AddMscProjTheta(tet);

      tet = std::atan(direction.z() / std::abs(direction.x()));
      analysisManager->FillH1(id, tet);
      if (transmit && (flag == 2)) run->AddMscProjTheta(tet);
    }
  }
