...
void Run::Merge(const G4Run* run)
{
  ...
  fEnergyTransmitted += localRun->fEnergyTransmitted;
  fEnergyTransmitted2 += localRun->fEnergyTransmitted2;
  fEnergyReflected += localRun->fEnergyReflected;
  fEnergyReflected2 += localRun->fEnergyReflected2;
...
void Run::EndOfRun()
{
  ...
  fEnergyTransmitted /= TotNbofEvents;
  fEnergyTransmitted2 /= TotNbofEvents;
  G4double rmsEtrans = fEnergyTransmitted2 - fEnergyTransmitted * fEnergyTransmitted;
  if (rmsEtrans > 0.)
    rmsEtrans = std::sqrt(rmsEtrans / TotNbofEvents);
  else
    rmsEtrans = 0.;

  fEnergyReflected /= TotNbofEvents;
  fEnergyReflected2 /= TotNbofEvents;
  G4double rmsErefl = fEnergyReflected2 - fEnergyReflected * fEnergyReflected;  
  if (rmsErefl > 0.)
    rmsErefl = std::sqrt(rmsErefl / TotNbofEvents);
  else
    rmsErefl = 0.;

  G4double etot = fEnergyDeposit + fEnergyTransmitted + fEnergyReflected;
  ...
  G4cout << "\n Total energy transmitted per event = "
         << G4BestUnit(fEnergyTransmitted, "Energy")
         << " +- " << G4BestUnit(rmsEtrans, "Energy")
         << G4endl;

  G4cout << "\n Total energy reflected per event = "
         << G4BestUnit(fEnergyReflected , "Energy")
         << " +- " << G4BestUnit(rmsErefl, "Energy")
         << G4endl;
  // ******************************************************************
  G4cout << "\n Energy deposit + energy transmitted + energy reflected = " << 
            G4BestUnit(etot, "Energy") << G4endl;     

  G4cout << "\n Energy transmitted in % of the primary energy = "
         << 100.*fEnergyTransmitted/etot << " +- " << 100.*rmsEtrans/etot << G4endl;
  G4cout << "\n Energy reflected   in % of the primary energy = "
         << 100.*fEnergyReflected  /etot << " +- " << 100.*rmsErefl /etot << G4endl;
