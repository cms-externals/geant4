...
MuCrossSections::MuCrossSections()
{
  ...
  muonMass = G4MuonMinus::MuonMinus()->GetPDGMass();
				    ...
				    G4double pairEnergy)
{
  ...
  */
  ...
  static const G4double factorForCross = 2./(3*CLHEP::pi)*pow(CLHEP::fine_structure_const*CLHEP::classic_electr_radius/mueRatio, 2);
  
  if (pairEnergy <= 2. * muonMass)
    return 0.0;

  G4double totalEnergy = tkin + muonMass;
  ...
  G4double Y = 10 * sqrt(muonMass/totalEnergy);
  G4double U[8];
                                 ...
                                 G4double pairEnergy, const G4double B)
{
  ...
  static const G4double sqe = 2*std::sqrt(G4Exp(1.0))*muonMass*muonMass;
