...
void Run::EndOfRun()
{
  ...
  if (analysisMan->GetH1Activation(histo_id_trans)) {
    if (fPolarized) {
      G4double rindex1 = det->GetTankMaterial()->GetMaterialPropertiesTable()
                            ->GetProperty(kRINDEX)->Value(fEkin);
      G4double rindex2 = det->GetWorldMaterial()->GetMaterialPropertiesTable()
                            ->GetProperty(kRINDEX)->Value(fEkin);
      ...
      for (size_t i=0; i<histo_trans->axis().bins()-1; ++i) {
        if (histo_trans->bin_height(i)>0. && histo_trans->bin_height(i+1)==0.){
