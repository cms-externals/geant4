...
void SteppingAction::UserSteppingAction(const G4Step* step)
{
  ...
  if ( preVolume  == fpDetectorConstruction->GetLogicalWorld() &&
       postVolume == fpDetectorConstruction->GetLogicalScorer() )
  {    
    // G4cout << "---> One particle has crossed the boundary..." << G4endl;

    G4double PDGCode = step->GetTrack()->GetParticleDefinition()->GetPDGEncoding();
  
    G4double x = postStep->GetPosition().x() / micrometer;
    G4double y = postStep->GetPosition().y() / micrometer;
    G4double z = postStep->GetPosition().z() / micrometer;

    G4ThreeVector direction = postStep->GetMomentumDirection();
    G4double xMom = direction.x();
    G4double yMom = direction.y();
    G4double zMom = direction.z();
  
    G4double energy = postStep->GetKineticEnergy() / keV;
