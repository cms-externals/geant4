...
void PulseAction::Initialize()
{
  ...
  while (!input.eof()) {
    ...
    G4double pTDummy;
    input >> aTDummy;
    aTDummy = aTDummy * CLHEP::us;  // better to assign unit right after reading
    if (aTDummy != fPulseVector.back()) {
      fPulseVector.push_back(aTDummy);
    }
    input >> pTDummy;
    fPulseData[aTDummy] = pTDummy;
    if (aTDummy > fPulseLarger) fPulseLarger = aTDummy;
  }
  input.close();  // Le Tuan Anh: close after reading
...
double PulseAction::PulseSpectrum(G4double time)
{
  ...
  auto t2 = std::upper_bound(fPulseVector.begin(),
                             fPulseVector.end(), time);
  auto t1 = t2 - 1;
  valueT1 = *t1;
  valueT2 = *t2;
