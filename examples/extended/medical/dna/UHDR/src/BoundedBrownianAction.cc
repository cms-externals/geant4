...
void BoundedBrownianAction::BouncingAction(G4ThreeVector& nextposition) const
{
  ...
  G4double Ly = RyM - Rym;
  G4double Lz = RzM - Rzm;
  G4double trunx = trunc(abs(nextposition.getX() - Rxm) / Lx);
  G4double truny = trunc(abs(nextposition.getY() - Rym) / Ly);
  G4double trunz = trunc(abs(nextposition.getZ() - Rzm) / Lz);
  G4double hx = std::fmod(trunx, 2);
  G4double hy = std::fmod(truny, 2);
  G4double hz = std::fmod(trunz, 2);
  G4double x = Rxm + hx * Lx + (1 - 2 * hx) * abs(std::fmod(nextposition.getX() - Rxm, Lx));
  G4double y = Rym + hy * Ly + (1 - 2 * hy) * abs(std::fmod(nextposition.getY() - Rym, Ly));
  G4double z = Rzm + hz * Lz + (1 - 2 * hz) * abs(std::fmod(nextposition.getZ() - Rzm, Lz));
