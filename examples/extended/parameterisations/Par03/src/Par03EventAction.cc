...
void Par03EventAction::EndOfEventAction(const G4Event* aEvent)
{
  ...
  G4double cosDirection = cos(primaryDirection.theta());
  G4double sinDirection = sin(primaryDirection.theta());
  ...
  for(size_t iHit = 0; iHit < hitsCollection->entries(); iHit++)
  {
    ...
    if(hitEn > 0)
    {
      ...
      tSecondMoment += hitEn * pow(tDistance - tFirstMoment, 2);
      rSecondMoment += hitEn * pow(rDistance - rFirstMoment, 2);
    }
  }
