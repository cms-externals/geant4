...
void XrayTESdetPrimaryGeneratorAction::GeneratePrimaries(G4Event* anEvent)
{
  ...
  while (true)
  {
    ...
    T = ene*0.001;
    flux = 1.9*pow(((T + phi)*(T + phi + 2*tr)), -1.39)*T*(T + 2*tr)/((1 + 0.4866*pow(((T + phi)*(T + phi + 2*tr)),-1.255))*(T + phi)*(T + phi + 2*tr));
    if (u1 <= flux)
    {
      energy = ene;
      break;
  ...
  theta0 = acos(1. - 2.*G4UniformRand());
  theta1 = acos(1. - 2.*G4UniformRand());
  phi0 = twopi*G4UniformRand();
  phi1 = twopi*G4UniformRand();
  ax = r*sin(theta0)*cos(phi0);
  ay = r*sin(theta0)*sin(phi0);
  az = r*cos(theta0) + 80*mm;

  bx = r*sin(theta1)*cos(phi1);
  by = r*sin(theta1)*sin(phi1);
  bz = r*cos(theta1) + 80*mm;
  vx = bx - ax;
  vy = by - ay;
  vz = bz - az;
