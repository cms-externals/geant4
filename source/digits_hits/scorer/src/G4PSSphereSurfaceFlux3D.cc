...
G4PSSphereSurfaceFlux3D::G4PSSphereSurfaceFlux3D(const G4String& name, G4int direction, G4int ni,
                                                 G4int nj, G4int nk, G4int depi, G4int depj,
                                                 G4int depk)
  : G4PSSphereSurfaceFlux(name, direction), fDepthi(depi), fDepthj(depj), fDepthk(depk)
{
  SetNijk(ni, nj, nk);
}
