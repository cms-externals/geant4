...
G4PSCellFlux3D::G4PSCellFlux3D(const G4String& name, G4int ni, G4int nj, G4int nk, G4int depi,
                               G4int depj, G4int depk)
  : G4PSCellFlux(name), fDepthi(depi), fDepthj(depj), fDepthk(depk)
{
  SetNijk(ni, nj, nk);
}

G4PSCellFlux3D::G4PSCellFlux3D(const G4String& name, const G4String& unit, G4int ni, G4int nj,
