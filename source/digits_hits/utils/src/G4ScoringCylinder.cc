...
G4ScoringCylinder::G4ScoringCylinder(const G4String& wName) : G4VScoringMesh(wName)
{
...
void G4ScoringCylinder::SetupGeometry(G4VPhysicalVolume* fWorldPhys)
{
  ...
  if (verboseLevel > 9)
  {
    G4cout << "R min, R max., Dz =: " << fSize[0] << ", " << fSize[1] << ", " << fSize[2] << G4endl;
  }
  ...
  new G4PVPlacement(fRotationMatrix, fCenterPosition, tubsLogical, tubsName + "0", worldLogical,
                    false, 0);

  if (verboseLevel > 9)
    G4cout << " # of segments : r, phi, z =: " << fNSegment[IR] << ", " << fNSegment[IPHI] << ", "
           << fNSegment[IZ] << G4endl;
  ...
  {
    if (verboseLevel > 9)
      G4cout << "G4ScoringCylinder::Construct() : Replicate along z direction" << G4endl;
    if (G4ScoringManager::GetReplicaLevel() > 0)
    {
      if (verboseLevel > 9) G4cout << "G4ScoringCylinder::Construct() : Replica" << G4endl;
      new G4PVReplica(layerName[0], layerLogical[0], tubsLogical, kZAxis, fNSegment[IZ],
                      2. * fSize[2] / fNSegment[IZ]);
    }
    else
    {
  ...
  else if (fNSegment[IZ] == 1)
  {
    ...
    new G4PVPlacement(nullptr, G4ThreeVector(0., 0., 0.), layerLogical[0], layerName[0],
                      tubsLogical, false, 0);
  }
  else
  {
    G4cerr << "G4ScoringCylinder::SetupGeometry() : invalid parameter (" << fNSegment[IZ] << ") "
  ...
  layerSolid[1] = new G4Tubs(layerName[1], fSize[0], fSize[1], fSize[2] / fNSegment[IZ], fAngle[0],
                             fAngle[1] / fNSegment[IPHI]);
  layerLogical[1] = new G4LogicalVolume(layerSolid[1], nullptr, layerName[1]);
  if (fNSegment[IPHI] > 1)
  {
    if (verboseLevel > 9)
      G4cout << "G4ScoringCylinder::Construct() : Replicate along phi direction" << G4endl;
    if (G4ScoringManager::GetReplicaLevel() > 1)
    {
      if (verboseLevel > 9) G4cout << "G4ScoringCylinder::Construct() : Replica" << G4endl;
      new G4PVReplica(layerName[1], layerLogical[1], layerLogical[0], kPhi, fNSegment[IPHI],
                      fAngle[1] / fNSegment[IPHI], fAngle[0]);
    }
    else
    {
      if (verboseLevel > 9) G4cout << "G4ScoringCylinder::Construct() : Division" << G4endl;
  ...
  {
    if (verboseLevel > 9) G4cout << "G4ScoringCylinder::Construct() : Placement" << G4endl;
    new G4PVPlacement(nullptr, G4ThreeVector(0., 0., 0.), layerLogical[1], layerName[1],
                      layerLogical[0], false, 0);
  }
  else
    G4cerr << "ERROR : G4ScoringCylinder::SetupGeometry() : invalid parameter (" << fNSegment[IPHI]
           << ") "
  ...
  G4VSolid* elementSolid =
    new G4Tubs(elementName, fSize[0], (fSize[1] - fSize[0]) / fNSegment[IR] + fSize[0],
  ...
  {
    if (verboseLevel > 9)
      G4cout << "G4ScoringCylinder::Construct() : Replicate along r direction" << G4endl;

    if (G4ScoringManager::GetReplicaLevel() > 2)
    {
      if (verboseLevel > 9) G4cout << "G4ScoringCylinder::Construct() : Replica" << G4endl;
      new G4PVReplica(elementName, fMeshElementLogical, layerLogical[1], kRho, fNSegment[IR],
                      (fSize[1] - fSize[0]) / fNSegment[IR], fSize[0]);
    }
    else
    {
      if (verboseLevel > 9) G4cout << "G4ScoringCylinder::Construct() : Division" << G4endl;
...
void G4ScoringCylinder::List() const
{
  ...
  G4cout << " Size (Rmin, Rmax, Dz): (" << fSize[0] / cm << ", " << fSize[1] / cm << ", "
         << fSize[2] / cm << ") [cm]" << G4endl;
  G4cout << " Angle (start, span): (" << fAngle[0] / deg << ", " << fAngle[1] / deg << ") [deg]"
         << G4endl;

  G4VScoringMesh::List();
}

void G4ScoringCylinder::Draw(RunScore* map, G4VScoreColorMap* colorMap, G4int axflg)
{
    ...
    if (axflg / 10 == 1)
    {
      ...
      for (G4int phi = 0; phi < fNSegment[IPHI]; phi++)
      {
        for (G4int z = 0; z < fNSegment[IZ]; z++)
        {
          ...
          //-
          G4ThreeVector zpos(0., 0., -fSize[2] + fSize[2] / fNSegment[IZ] * (1 + 2. * z));
          G4Transform3D trans;
...
void G4ScoringCylinder::DrawColumn(RunScore* map, G4VScoreColorMap* colorMap, G4int idxProj,
                                   G4int idxColumn)
{
  G4int projAxis = 0;
  switch (idxProj)
  ...
  {
    G4cerr << "Warning : Column number " << idxColumn << " is out of scoring mesh [0,"
           << fNSegment[projAxis] - 1 << "]. Method ignored." << G4endl;
    return;
  }
  G4VVisManager* pVisManager = G4VVisManager::GetConcreteInstance();
  if (pVisManager != nullptr)
  {
    ...
    if (projAxis == IR)
    {
      ...
      G4double zhalf = fSize[2] / fNSegment[IZ];
      G4double rsize[2] = {fSize[0] + (fSize[1] - fSize[0]) / fNSegment[IR] * idxColumn,
      ...
      for (int phi = 0; phi < fNSegment[IPHI]; phi++)
      {
        for (int z = 0; z < fNSegment[IZ]; z++)
        {
          G4double angle = fAngle[0] + fAngle[1] / fNSegment[IPHI] * phi;
          G4double dphi = fAngle[1] / fNSegment[IPHI];
          G4Tubs cylinder("z-phi", rsize[0], rsize[1], zhalf, angle, dphi * 0.99999);
    ...
    else if (projAxis == IZ)
    {
      ...
      for (G4int phi = 0; phi < fNSegment[IPHI]; phi++)
      {
        for (G4int r = 0; r < fNSegment[IR]; r++)
        {
          ...
          G4ThreeVector zpos(0., 0., -fSize[2] + fSize[2] / fNSegment[IZ] * (idxColumn * 2 + 1));
          G4Transform3D trans;
          if (fRotationMatrix != nullptr)
          {
            trans = G4Rotate3D(*fRotationMatrix).inverse() * G4Translate3D(zpos);
    ...
    else if (projAxis == IPHI)
    {
      ...
      for (G4int z = 0; z < fNSegment[IZ]; z++)
      {
        for (G4int r = 0; r < fNSegment[IR]; r++)
        {
          ...
          G4ThreeVector zpos(0., 0., -fSize[2] + fSize[2] / fNSegment[IZ] * (2. * z + 1));
          G4Transform3D trans;
          if (fRotationMatrix != nullptr)
          {
            trans = G4Rotate3D(*fRotationMatrix).inverse() * G4Translate3D(zpos);
    ...
    switch (lvl)
    {
      ...
      case 1:
        G4cout << (*itr)->GetName()
               ...
               << "\t surface = " << G4BestUnit((*itr)->GetSurfaceArea(), "Surface") << G4endl;
        break;
      default:
        (*itr)->DumpInfo();
        break;
    ...
    if ((*itr)->GetNoDaughters() > 0)
    {
      switch ((*itr)->CharacteriseDaughters())
      {
        ...
        case kReplica:
          ...
          break;
        case kParameterised:
          G4cout << " (parameterized : " << (*itr)->GetDaughter(0)->GetMultiplicity() << ")";
          break;
        default:;
      }
    }
