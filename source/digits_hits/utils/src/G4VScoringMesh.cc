...
G4VScoringMesh::G4VScoringMesh(const G4String& wName)
  : fWorldName(wName), fMFD(new G4MultiFunctionalDetector(wName))
{
...
void G4VScoringMesh::SetSize(G4double size[3])
{
  ...
  else
  {
    ...
    G4Exception("G4VScoringMesh::SetSize()", "DigiHitsUtilsScoreVScoringMesh000", JustWarning,
                message);
  }
}
...
{
  if (sizeIsSet) return G4ThreeVector(fSize[0], fSize[1], fSize[2]);
...
void G4VScoringMesh::SetNumberOfSegments(G4int nSegment[3])
{
  ...
  else
  {
    ...
    message += "  This method is ignored.";
    G4Exception("G4VScoringMesh::SetNumberOfSegments()", "DigiHitsUtilsScoreVScoringMesh000",
                JustWarning, message);
  }
}
...
{
  if (fRotationMatrix == nullptr) fRotationMatrix = new G4RotationMatrix();
...
void G4VScoringMesh::SetPrimitiveScorer(G4VPrimitiveScorer* prs)
{
  if (!ReadyForQuantity())
  {
    G4cerr << "ERROR : G4VScoringMesh::SetPrimitiveScorer() : " << prs->GetName()
           << " does not yet have mesh size or number of bins. Set them first." << G4endl
           << "This Method is ignored." << G4endl;
    return;
  }
  if (verboseLevel > 0)
    G4cout << "G4VScoringMesh::SetPrimitiveScorer() : " << prs->GetName() << " is registered."
           << " 3D size: (" << fNSegment[0] << ", " << fNSegment[1] << ", " << fNSegment[2] << ")"
           << G4endl;

  prs->SetNijk(fNSegment[0], fNSegment[1], fNSegment[2]);
  fCurrentPS = prs;
  fMFD->RegisterPrimitive(prs);
  auto map = new G4THitsMap<G4StatDouble>(fWorldName, prs->GetName());
...
void G4VScoringMesh::SetFilter(G4VSDFilter* filter)
{
  ...
  if (verboseLevel > 0)
    G4cout << "G4VScoringMesh::SetFilter() : " << filter->GetName() << " is set to "
...
{
  if (fMFD == nullptr) return nullptr;
...
{
  G4cout << " # of segments: (" << fNSegment[0] << ", " << fNSegment[1] << ", " << fNSegment[2]
         << ")" << G4endl;
  G4cout << " displacement: (" << fCenterPosition.x() / cm << ", " << fCenterPosition.y() / cm
         << ", " << fCenterPosition.z() / cm << ") [cm]" << G4endl;
  if (fRotationMatrix != nullptr)
  {
    G4cout << " rotation matrix: " << fRotationMatrix->xx() << "  " << fRotationMatrix->xy() << "  "
           << fRotationMatrix->xz() << G4endl << "                  " << fRotationMatrix->yx()
           << "  " << fRotationMatrix->yy() << "  " << fRotationMatrix->yz() << G4endl
           << "                  " << fRotationMatrix->zx() << "  " << fRotationMatrix->zy() << "  "
  ...
  for (G4int i = 0; i < nps; ++i)
  {
    ...
    if (prs->GetFilter() != nullptr) G4cout << "     with  " << prs->GetFilter()->GetName();
    G4cout << G4endl;
  }
}
...
void G4VScoringMesh::DrawMesh(const G4String& psName, G4int idxPlane, G4int iColumn,
...
void G4VScoringMesh::Construct(G4VPhysicalVolume* fWorldPhys)
{
  if (fConstructed)
  {
    ...
    if (verboseLevel > 0) G4cout << fWorldName << " --- All quantities are reset." << G4endl;
    ResetScore();
  }
  else
  {
