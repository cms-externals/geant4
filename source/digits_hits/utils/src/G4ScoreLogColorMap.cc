...
void G4ScoreLogColorMap::GetMapColor(G4double val, G4double color[4])
{
  if(fMinVal < 0. || fMaxVal < 0.)
  {
    ...
    ed << "The minimum value (fMinVal=" << fMinVal 
       << ") or the maximum value (fMaxVal=" << fMaxVal
       << ") is negative for this log-scale color map.";
    G4Exception("G4ScoreLogColorMap::GetMapColor()",
                "DigiHitsUtilsScoreLogColorMap000", JustWarning, ed);
    color[0] = 0.;
    color[1] = 0.;
    color[2] = 0.;
    color[3] = 0.;
  ...
  if(val < 0.)
  {
    ...
    G4Exception("G4ScoreLogColorMap::GetMapColor()",
                "DigiHitsUtilsScoreLogColorMap001", JustWarning, ed);
    color[0] = 0.;
    color[1] = 0.;
    color[2] = 0.;
    color[3] = -1.;
