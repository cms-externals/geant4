...
G4ScoringBox::G4ScoringBox(const G4String& wName) : G4VScoringMesh(wName), fSegmentDirection(-1)
{
...
void G4ScoringBox::SetupGeometry(G4VPhysicalVolume* fWorldPhys)
{
  ...
  auto boxLogical = new G4LogicalVolume(boxSolid, nullptr, boxName + "_0");
  new G4PVPlacement(fRotationMatrix, fCenterPosition, boxLogical, boxName + "0", worldLogical,
                    false, 0);
  ...
  layerSolid[0] = new G4Box(layerName[0], fSize[0] / fNSegment[0], fSize[1], fSize[2]);
  ...
  if (fNSegment[0] > 1)
  {
    ...
    {
      new G4PVReplica(layerName[0], layerLogical[0], boxLogical, kXAxis, fNSegment[0],
                      fSize[0] / fNSegment[0] * 2.);
    }
    else
    {
      new G4PVDivision(layerName[0], layerLogical[0], boxLogical, kXAxis, fNSegment[0], 0.);
    }
  }
  else if (fNSegment[0] == 1)
  {
    if (verboseLevel > 9) G4cout << "G4ScoringBox::Construct() : Placement" << G4endl;
    new G4PVPlacement(nullptr, G4ThreeVector(0., 0., 0.), layerLogical[0], layerName[0], boxLogical,
                      false, 0);
  }
  else
    G4cerr << "ERROR : G4ScoringBox::SetupGeometry() : invalid parameter (" << fNSegment[0] << ") "
           << "in placement of the first nested layer." << G4endl;

  if (verboseLevel > 9)
  {
    G4cout << fSize[0] / fNSegment[0] << ", " << fSize[1] << ", " << fSize[2] << G4endl;
    G4cout << layerName[0] << ": kXAxis, " << fNSegment[0] << ", " << 2. * fSize[0] / fNSegment[0]
           << G4endl;
  }
  ...
  layerSolid[1] =
    new G4Box(layerName[1], fSize[0] / fNSegment[0], fSize[1] / fNSegment[1], fSize[2]);
  ...
  if (fNSegment[1] > 1)
  {
    ...
    {
      new G4PVReplica(layerName[1], layerLogical[1], layerLogical[0], kYAxis, fNSegment[1],
                      fSize[1] / fNSegment[1] * 2.);
    }
    else
    {
      new G4PVDivision(layerName[1], layerLogical[1], layerLogical[0], kYAxis, fNSegment[1], 0.);
    }
  }
  else if (fNSegment[1] == 1)
  {
    ...
    G4cerr << "ERROR : G4ScoringBox::SetupGeometry() : invalid parameter (" << fNSegment[1] << ") "
           << "in placement of the second nested layer." << G4endl;

  if (verboseLevel > 9)
  {
    G4cout << fSize[0] / fNSegment[0] << ", " << fSize[1] / fNSegment[1] << ", " << fSize[2]
           << G4endl;
    G4cout << layerName[1] << ": kYAxis, " << fNSegment[1] << ", " << 2. * fSize[1] / fNSegment[1]
           << G4endl;
  }
  ...
  G4VSolid* elementSolid = new G4Box(elementName, fSize[0] / fNSegment[0], fSize[1] / fNSegment[1],
                                     fSize[2] / fNSegment[2]);
  ...
  if (fNSegment[2] > 1)
  {
    ...
    {
      new G4PVReplica(elementName, fMeshElementLogical, layerLogical[1], kZAxis, fNSegment[2],
                      2. * fSize[2] / fNSegment[2]);
    }
    else
    {
      new G4PVDivision(elementName, fMeshElementLogical, layerLogical[1], kZAxis, fNSegment[2], 0.);
    }
  }
  else if (fNSegment[2] == 1)
  {
    if (verboseLevel > 9) G4cout << "G4ScoringBox::Construct() : Placement" << G4endl;
    new G4PVPlacement(nullptr, G4ThreeVector(0., 0., 0.), fMeshElementLogical, elementName,
                      layerLogical[1], false, 0);
  }
  else
  ...
  if (verboseLevel > 9)
  {
    ...
    G4cout << elementName << ": kZAxis, " << fNSegment[2] << ", " << 2. * fSize[2] / fNSegment[2]
           << G4endl;
  }
...
void G4ScoringBox::List() const
{
  ...
  G4cout << " Size (x, y, z): (" << fSize[0] / cm << ", " << fSize[1] / cm << ", " << fSize[2] / cm
         << ") [cm]" << G4endl;
    ...
    if (axflg / 100 == 1)
    {
      ...
      for (G4int x = 0; x < fNSegment[0]; x++)
      {
        for (G4int y = 0; y < fNSegment[1]; y++)
        {
          ...
          G4Box xyplate("xy", fSize[0] / fNSegment[0], fSize[1] / fNSegment[1], thick);
...
G4ThreeVector G4ScoringBox::GetReplicaPosition(G4int x, G4int y, G4int z)
{
  ...
  G4ThreeVector pos(-fSize[0] + 2 * (x + 0.5) * width.x(), -fSize[1] + 2 * (y + 0.5) * width.y(),
...
void G4ScoringBox::DrawColumn(RunScore* map, G4VScoreColorMap* colorMap, G4int idxProj,
                              G4int idxColumn)
{
  G4int iColumn[3] = {2, 0, 1};
  if (idxColumn < 0 || idxColumn >= fNSegment[iColumn[idxProj]])
  {
    G4cerr << "ERROR : Column number " << idxColumn << " is out of scoring mesh [0,"
           << fNSegment[iColumn[idxProj]] - 1 << "]. Method ignored." << G4endl;
    return;
  }
  G4VVisManager* pVisManager = G4VVisManager::GetConcreteInstance();
  if (pVisManager != nullptr)
