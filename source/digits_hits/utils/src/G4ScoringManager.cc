...
G4ScoringManager::G4ScoringManager() : verboseLevel(0), fCurrentMesh(nullptr)
{
...
void G4ScoringManager::Accumulate(G4VHitsCollection* map)
{
  ...
  if (verboseLevel > 9)
  {
    G4cout << "G4ScoringManager::Accumulate() for " << map->GetSDname() << " / " << map->GetName()
           << G4endl;
...
void G4ScoringManager::DrawMesh(const G4String& meshName, const G4String& psName,
                                const G4String& colorMapName, G4int axflg, G4bool refreshDraw,
                                G4int drawColorChart)
{
  ...
  if (mesh != nullptr)
  {
    ...
    mesh->RefreshBeforeDraw(refreshDraw);
    mesh->DrawColorChart(drawColorChart);
...
void G4ScoringManager::DrawMesh(const G4String& meshName, const G4String& psName, G4int idxPlane,
...
void G4ScoringManager::DumpQuantityToFile(const G4String& meshName, const G4String& psName,
...
void G4ScoringManager::DumpAllQuantitiesToFile(const G4String& meshName, const G4String& fileName,
                                               const G4String& option)
{
  ...
  {
    G4cerr << "ERROR : G4ScoringManager::DrawAllQuantitiesToFile() --- <" << meshName
           << "> is not found. Nothing is done." << G4endl;
  }
}
...
void G4ScoringManager::RegisterScoreColorMap(G4VScoreColorMap* colorMap)
{
  ...
  {
    G4cerr << "ERROR : G4ScoringManager::RegisterScoreColorMap -- " << colorMap->GetName()
