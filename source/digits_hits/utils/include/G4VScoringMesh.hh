...
// Author: Makoto Asai
// --------------------------------------------------------------------
#ifndef G4VSCORINGMESH_HH
#define G4VSCORINGMESH_HH
...
class G4VScoringMesh
{
  ...
  public:
    ...
    // draw a column of a quantity on a current viewer
    virtual void DrawColumn(RunScore* map, G4VScoreColorMap* colorMap, G4int idxProj,
                            G4int idxColumn) = 0;
    ...
    {
      if (fRotationMatrix != nullptr) return *fRotationMatrix;
    ...
    inline G4bool IsCurrentPrimitiveScorerNull() { return fCurrentPS == nullptr; }
    ...
    inline void SetMeshElementLogical(G4LogicalVolume* val) { fMeshElementLogical = val; }
    inline G4LogicalVolume* GetMeshElementLogical() const { return fMeshElementLogical; }

    inline void SetParallelWorldProcess(G4ParallelWorldProcess* proc)
    {
      fParallelWorldProcess = proc;
    }
    inline G4ParallelWorldProcess* GetParallelWorldProcess() const { return fParallelWorldProcess; }
    ...
    // Following two methods affect only the next drawing and then reset
    // Refreshing previous drawing
    inline void RefreshBeforeDraw(G4bool val) { refreshDraw = val; }
    // Drawing the color chart
    inline void DrawColorChart(G4int val) { drawColorChart = val; }
  ...
  protected:
    ...
    G4String fWorldName;
    G4VPrimitiveScorer* fCurrentPS = nullptr;
    G4bool fConstructed = false;
    G4bool fActive = true;
    MeshShape fShape = MeshShape::undefined;
    ...
    G4bool sizeIsSet = false;
    G4bool nMeshIsSet = false;

    G4String fDrawUnit;
    G4double fDrawUnitValue = 1.0;
    G4String fDrawPSName;

    G4String fDivisionAxisNames[3];

    G4LogicalVolume* fMeshElementLogical = nullptr;

    G4ParallelWorldProcess* fParallelWorldProcess = nullptr;
    G4bool fGeometryHasBeenDestroyed = false;

    G4int copyNumberLevel = 0;
    ...
    G4bool layeredMassFlg = false;

    G4bool refreshDraw = true;
    G4int drawColorChart = 0;
};
