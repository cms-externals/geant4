...
class HasName
{
  public:
    ...
    bool operator()(const T& value) const { return value.GetName() == fName; }
...
G4bool ExtractAttDef(const T& object, const G4String& name, G4AttDef& def)
{
  ...
  // NOLINTNEXTLINE(modernize-use-auto): Explicitly want a const_iterator despite attDefs being
  // const
...
G4bool ExtractAttValue(const T& object, const G4String& name, G4AttValue& attVal)
{
  ...
  auto iter = std::find_if(attValues->cbegin(), attValues->cend(), HasName<G4AttValue>(name));
