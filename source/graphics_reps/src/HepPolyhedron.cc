...
std::ostream& operator<<(std::ostream& ostr, const HepPolyhedron& ph)
{
  ...
  {
    ostr << "xyz(" << i << ")=" << ph.pV[i].x() << ' ' << ph.pV[i].y() << ' ' << ph.pV[i].z()
         << std::endl;
  }
  ...
  : nvert(0), nface(0), pV(nullptr), pF(nullptr)
{
  ...
  for (G4int i = 1; i <= nvert; i++)
    pV[i] = from.pV[i];
  for (G4int k = 1; k <= nface; k++)
    pF[k] = from.pF[k];
}
 ...
 ***********************************************************************/
{
  ...
  if (i == 4)
  {
    std::cerr << "HepPolyhedron::FindNeighbour: face " << iFace << " has no node " << iNode
              << std::endl;
    return 0;
  }
  if (iOrder < 0)
 ...
 ***********************************************************************/
{
  ...
  {
    std::cerr << "HepPolyhedron::SetVertex: vertex index = " << index << " is out of range\n"
              << "   N. of vertices = " << nvert << "\n"
...
void HepPolyhedron::SetFacet(G4int index, G4int iv1, G4int iv2, G4int iv3, G4int iv4)
/***********************************************************************
 *                                                                     *
 ...
 ***********************************************************************/
{
  ...
  {
    std::cerr << "HepPolyhedron::SetFacet: facet index = " << index << " is out of range\n"
              << "   N. of vertices = " << nvert << "\n"
              << "   N. of facets = " << nface << std::endl;
    return;
  }
  if (iv1 < 1 || iv1 > nvert || iv2 < 1 || iv2 > nvert || iv3 < 1 || iv3 > nvert || iv4 < 0
      || iv4 > nvert)
  {
    std::cerr << "HepPolyhedron::SetFacet: incorrectly specified facet"
              << " (" << iv1 << ", " << iv2 << ", " << iv3 << ", " << iv4 << ")\n"
              << "   N. of vertices = " << nvert << "\n"
              << "   N. of facets = " << nface << std::endl;
    return;
 ...
 ***********************************************************************/
{
  ...
  if (n < nMin)
  {
    std::cerr << "HepPolyhedron::SetNumberOfRotationSteps: attempt to set the\n"
              << "number of steps per circle < " << nMin << "; forced to " << nMin << std::endl;
...
void HepPolyhedron::RotateEdge(G4int k1, G4int k2, G4double r1, G4double r2, G4int v1, G4int v2,
                               G4int vEdge, G4bool ifWholeCircle, G4int nds, G4int& kface)
/***********************************************************************
 *                                                                     *
...
void HepPolyhedron::SetSideFacets(G4int ii[4], G4int vv[4], G4int* kk, G4double* r, G4double dphi,
                                  G4int nds, G4int& kface)
/***********************************************************************
 *                                                                     *
...
void HepPolyhedron::RotateAroundZ(G4int nstep, G4double phi, G4double dphi, G4int np1, G4int np2,
                                  const G4double* z, G4double* r, G4int nodeVis, G4int edgeVis)
/***********************************************************************
 *                                                                     *
 ...
 ***********************************************************************/
{
  ...
  for (i = i1beg; i < i1end; i++)
  {
    ...
    RotateEdge(kk[i], kk[i + 1], r[i], r[i + 1], v1, v2, edgeVis, ifWholeCircle, nSphi, k);
  }
  if (ifClosed)
  {
    RotateEdge(kk[i1end], kk[i1beg], r[i1end], r[i1beg], nodeVis, nodeVis, edgeVis, ifWholeCircle,
               nSphi, k);
  }
  ...
  if (absNp2 > 1)
  {
    ...
    for (i = i2beg; i < i2end; i++)
    {
      ...
      RotateEdge(kk[i + 1], kk[i], r[i + 1], r[i], v2, v1, edgeVis, ifWholeCircle, nSphi, k);
    }
    if (ifClosed)
    {
      RotateEdge(kk[i2beg], kk[i2end], r[i2beg], r[i2end], nodeVis, nodeVis, edgeVis, ifWholeCircle,
                 nSphi, k);
    }
  }
  ...
  if (!ifClosed)
  {
    if (ifSide1)
    {
      RotateEdge(kk[i2beg], kk[i1beg], r[i2beg], r[i1beg], 1, 1, -1, ifWholeCircle, nSphi, k);
    }
    if (ifSide2)
    {
      RotateEdge(kk[i1end], kk[i2end], r[i1end], r[i2end], 1, 1, -1, ifWholeCircle, nSphi, k);
    }
  }
  ...
  if (k - 1 != nface)
  {
    std::cerr << "HepPolyhedron::RotateAroundZ: number of generated faces (" << k - 1
              << ") is not equal to the number of allocated faces (" << nface << ")" << std::endl;
  }
}

void HepPolyhedron::RotateContourAroundZ(G4int nstep, G4double phi, G4double dphi,
                                         const std::vector<G4TwoVector>& rz, G4int nodeVis,
                                         G4int edgeVis)
/***********************************************************************
 *                                                                     *
 ...
 ***********************************************************************/
{
  ...
  for (G4int i = 0; i < Nrz; ++i)
  {
    ...
    RotateEdge(kk[i1], kk[i2], r[i1], r[i2], nodeVis, nodeVis, edgeVis, ifWholeCircle, nSphi,
               kfree);
  }
  ...
  if (kfree - 1 != nface)
  {
    std::cerr << "HepPolyhedron::RotateContourAroundZ: number of generated faces (" << kfree - 1
              << ") is not equal to the number of allocated faces (" << nface << ")" << std::endl;
  }
}
...
G4bool HepPolyhedron::CheckSnip(const std::vector<G4TwoVector>& contour, G4int a, G4int b, G4int c,
 ...
 ***********************************************************************/
{
  ...
  for (iface = 1; iface <= nface; iface++)
  {
    ...
    for (iedge = 0; iedge < nedge; iedge++)
    {
      ...
      if (cur == nullptr)
      {
        ...
        if (freeList == nullptr)
        {
          std::cerr << "Polyhedron::SetReferences: bad link " << std::endl;
          break;
        }
      ...
      if (cur->v2 == k2)
      {
        ...
        if (i1 != i2)
        {
          std::cerr << "Polyhedron::SetReferences: different edge visibility " << iface << "/"
                    << iedge << "/" << pF[iface].edge[iedge].v << " and " << cur->iface << "/"
                    << cur->iedge << "/" << pF[cur->iface].edge[cur->iedge].v << std::endl;
        }
        continue;
      }
  ...
  for (i = 0; i < nvert; i++)
  {
    ...
    {
      std::cerr << "Polyhedron::SetReferences: List " << i << " is not empty" << std::endl;
    }
  }
 ...
 ***********************************************************************/
{
  ...
  for (G4int icur = 1; icur <= nface; ++icur)
  {
    ...
    if (pF[icur].edge[0].f < icur && pF[icur].edge[1].f < icur && pF[icur].edge[2].f < icur)
      continue;
    ...
    for (G4int k = 0; k < 3; ++k)
    {
      ...
      if (pF[itmp].edge[0].v == 0 || pF[itmp].edge[3].v != 0) continue;
 ...
 ***********************************************************************/
{
  ...
  {
    std::cerr << "HepPolyhedron::GetVertex: irrelevant index " << index << std::endl;
    return G4Point3D();
  }
  return pV[index];
}
...
G4bool HepPolyhedron::GetNextEdgeIndices(G4int& i1, G4int& i2, G4int& edgeFlag, G4int& iface1,
                                         G4int& iface2) const
/***********************************************************************
 *                                                                     *
...
G4bool HepPolyhedron::GetNextEdge(G4Point3D& p1, G4Point3D& p2, G4int& edgeFlag, G4int& iface1,
                                  G4int& iface2) const
/***********************************************************************
 *                                                                     *
...
void HepPolyhedron::GetFacet(G4int iFace, G4int& n, G4int* iNodes, G4int* edgeFlags,
                             G4int* iFaces) const
/***********************************************************************
 *                                                                     *
 ...
 ***********************************************************************/
{
  ...
  {
    std::cerr << "HepPolyhedron::GetFacet: irrelevant index " << iFace << std::endl;
    n = 0;
  }
  else
  {
...
void HepPolyhedron::GetFacet(G4int index, G4int& n, G4Point3D* nodes, G4int* edgeFlags,
                             G4Normal3D* normals) const
/***********************************************************************
 *                                                                     *
...
G4bool HepPolyhedron::GetNextFacet(G4int& n, G4Point3D* nodes, G4int* edgeFlags,
                                   G4Normal3D* normals) const
/***********************************************************************
 *                                                                     *
 ...
 ***********************************************************************/
{
  if (iFace < 1 || iFace > nface)
  {
    std::cerr << "HepPolyhedron::GetNormal: irrelevant index " << iFace << std::endl;
    return G4Normal3D();
  }
 ...
 ***********************************************************************/
{
  if (iFace < 1 || iFace > nface)
  {
    std::cerr << "HepPolyhedron::GetUnitNormal: irrelevant index " << iFace << std::endl;
    return G4Normal3D();
  }
...
G4int HepPolyhedron::createTwistedTrap(G4double Dz, const G4double xy1[][2],
...
G4int HepPolyhedron::createPolyhedron(G4int Nnodes, G4int Nfaces, const G4double xyz[][3],
                                      const G4int faces[][4])
/***********************************************************************
 *                                                                     *
...
HepPolyhedronTrd2::HepPolyhedronTrd2(G4double Dx1, G4double Dx2, G4double Dy1, G4double Dy2,
                                     G4double Dz)
/***********************************************************************
 *                                                                     *
...
HepPolyhedronTrd1::HepPolyhedronTrd1(G4double Dx1, G4double Dx2, G4double Dy, G4double Dz)
...
HepPolyhedronTrap::HepPolyhedronTrap(G4double Dz, G4double Theta, G4double Phi, G4double Dy1,
                                     G4double Dx1, G4double Dx2, G4double Alp1, G4double Dy2,
...
HepPolyhedronPara::HepPolyhedronPara(G4double Dx, G4double Dy, G4double Dz, G4double Alpha,
...
HepPolyhedronParaboloid::HepPolyhedronParaboloid(G4double r1, G4double r2, G4double dz,
 ...
 ***********************************************************************/
{
  ...
  if (k != 0)
  {
    ...
    std::cerr << " r1=" << r1;
    std::cerr << " r2=" << r2;
    std::cerr << " dz=" << dz << " sPhi=" << sPhi << " dPhi=" << dPhi << std::endl;
    return;
  }
...
HepPolyhedronHype::HepPolyhedronHype(G4double r1, G4double r2, G4double sqrtan1, G4double sqrtan2,
                                     G4double halfZ)
/***********************************************************************
 *                                                                     *
...
HepPolyhedronCons::HepPolyhedronCons(G4double Rmn1, G4double Rmx1, G4double Rmn2, G4double Rmx2,
 ...
 ***********************************************************************/
{
  ...
  if (k != 0)
  {
    ...
    std::cerr << " Dz=" << Dz << " Phi1=" << Phi1 << " Dphi=" << Dphi << std::endl;
    return;
  }
...
HepPolyhedronCone::HepPolyhedronCone(G4double Rmn1, G4double Rmx1, G4double Rmn2, G4double Rmx2,
                                     G4double Dz)
...
HepPolyhedronTubs::HepPolyhedronTubs(G4double Rmin, G4double Rmax, G4double Dz, G4double Phi1,
                                     G4double Dphi)
...
HepPolyhedronTube::HepPolyhedronTube(G4double Rmin, G4double Rmax, G4double Dz)
  : HepPolyhedronCons(Rmin, Rmax, Rmin, Rmax, Dz, 0 * deg, 360 * deg)
{}

HepPolyhedronTube::~HepPolyhedronTube() = default;

HepPolyhedronPgon::HepPolyhedronPgon(G4double phi, G4double dphi, G4int npdv, G4int nz,
                                     const G4double* z, const G4double* rmin, const G4double* rmax)
/***********************************************************************
 *                                                                     *
 ...
 ***********************************************************************/
{
  ...
  {
    std::cerr << "HepPolyhedronPgon/Pcon: wrong delta phi = " << dphi << std::endl;
    return;
  }

  if (nz < 2)
  {
    std::cerr << "HepPolyhedronPgon/Pcon: number of z-planes less than two = " << nz << std::endl;
    return;
  }

  if (npdv < 0)
  {
    std::cerr << "HepPolyhedronPgon/Pcon: error in number of phi-steps =" << npdv << std::endl;
    return;
  }
  ...
  for (i = 0; i < nz; i++)
  {
    ...
    {
      std::cerr << "HepPolyhedronPgon: error in radiuses rmin[" << i << "]=" << rmin[i] << " rmax["
                << i << "]=" << rmax[i] << std::endl;
      return;
    }
  }
...
HepPolyhedronPgon::HepPolyhedronPgon(G4double phi, G4double dphi, G4int npdv,
 ...
 ***********************************************************************/
{
  ...
  if (nrz < 3)
  {
    std::cerr << "HepPolyhedronPgon/Pcon: invalid number of nodes in rz-contour = " << nrz
              << std::endl;
    return;
  }
...
HepPolyhedronPcon::HepPolyhedronPcon(G4double phi, G4double dphi, G4int nz, const G4double* z,
...
HepPolyhedronSphere::HepPolyhedronSphere(G4double rmin, G4double rmax, G4double phi, G4double dphi,
 ...
 ***********************************************************************/
{
  ...
  if (dphi <= 0. || dphi > twopi)
  {
    std::cerr << "HepPolyhedronSphere: wrong delta phi = " << dphi << std::endl;
    return;
  }

  if (the < 0. || the > pi)
  {
    std::cerr << "HepPolyhedronSphere: wrong theta = " << the << std::endl;
    return;
  }

  if (dthe <= 0. || dthe > pi)
  {
    std::cerr << "HepPolyhedronSphere: wrong delta theta = " << dthe << std::endl;
    return;
  }

  if (the + dthe > pi)
  {
    std::cerr << "HepPolyhedronSphere: wrong theta + delta theta = " << the << " " << dthe
              << std::endl;
    return;
  }
  ...
  if (rmin < 0. || rmin >= rmax)
  {
    std::cerr << "HepPolyhedronSphere: error in radiuses"
...
HepPolyhedronTorus::HepPolyhedronTorus(G4double rmin, G4double rmax, G4double rtor, G4double phi,
                                       G4double dphi)
/***********************************************************************
 *                                                                     *
 ...
 ***********************************************************************/
{
  ...
  if (dphi <= 0. || dphi > twopi)
  {
    std::cerr << "HepPolyhedronTorus: wrong delta phi = " << dphi << std::endl;
    return;
  }

  if (rmin < 0. || rmin >= rmax || rmax >= rtor)
  {
    std::cerr << "HepPolyhedronTorus: error in radiuses"
              << " rmin=" << rmin << " rmax=" << rmax << " rtorus=" << rtor << std::endl;
    return;
  }
...
HepPolyhedronTet::HepPolyhedronTet(const G4double p0[3], const G4double p1[3], const G4double p2[3],
                                   const G4double p3[3])
/***********************************************************************
 *                                                                     *
...
HepPolyhedronEllipsoid::HepPolyhedronEllipsoid(G4double ax, G4double by, G4double cz,
 ...
 ***********************************************************************/
{
  ...
  {
    std::cerr << "HepPolyhedronEllipsoid: wrong zCut1 = " << zCut1 << " zCut2 = " << zCut2
              << " for given cz = " << cz << std::endl;
    return;
  }
  if (cz <= 0.0)
  {
    std::cerr << "HepPolyhedronEllipsoid: bad z semi-axis: cz = " << cz << std::endl;
    return;
  }
  ...
  {
    G4Exception("HepPolyhedronEllipsoid::HepPolyhedronEllipsoid", "greps1002", FatalException,
                "Out of memory");
  }
...
HepPolyhedronEllipticalCone::HepPolyhedronEllipticalCone(G4double ax, G4double ay, G4double h,
                                                         G4double zTopCut)
/***********************************************************************
 *                                                                     *
...
HepPolyhedronHyperbolicMirror::HepPolyhedronHyperbolicMirror(G4double a, G4double h, G4double r)
/***********************************************************************
 *                                                                     *
 ...
 ***********************************************************************/
{
  ...
  if (nnodes == 0)
  {
    std::cerr << "HepPolyhedronTetMesh: Empty tetrahedron mesh" << std::endl;
    return;
  }
  G4int ntet = nnodes / 4;
  if (nnodes != ntet * 4)
  {
    std::cerr << "HepPolyhedronTetMesh: Number of nodes = " << nnodes
              << " in tetrahedron mesh is NOT multiple of 4" << std::endl;
    return;
  }
...
HepPolyhedronBoxMesh::HepPolyhedronBoxMesh(G4double sizeX, G4double sizeY, G4double sizeZ,
