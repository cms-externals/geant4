...
G4AttCheck::G4AttCheck(const std::vector<G4AttValue>* values,
  ...
  : fpValues(values), fpDefinitions(definitions)
{
  Init();

  if (fFirst)
  {  // Initialise static containers.
    fFirst = false;
    ...
    {
      if (fUnitCategories->find(units[i]->GetName()) != fUnitCategories->end())
      {
...
G4bool G4AttCheck::Check(const G4String& leader) const
{
  ...
  for (iValue = fpValues->begin(); iValue != fpValues->end(); ++iValue)
  {
    ...
    map<G4String, G4AttDef>::const_iterator iDef = fpDefinitions->find(valueName);
    if (iDef == fpDefinitions->end())
    {
      ...
      if (print)
      {
        ...
        G4cerr << "\nG4AttCheck: ERROR " << iError << ": No G4AttDef for G4AttValue \"" << valueName
               << "\": " << value << "\n*******************************************************"
               << G4endl;
      }
    }
    else
    {
      ...
      if (fCategories->find(category) == fCategories->end())
      {
        ...
        if (print)
        {
          ...
          G4cerr << "\nG4AttCheck: ERROR " << iError << ": Illegal Category Field \"" << category
      ...
      if (category == "Physics" && fUnits->find(extra) == fUnits->end())
      {
        ...
        if (print)
        {
          ...
          G4cerr << "\nG4AttCheck: ERROR " << iError << ": Illegal Extra field \"" << extra
                 << "\" for G4AttValue \"" << valueName << "\": " << value
      ...
      if (fValueTypes->find(valueType) == fValueTypes->end())
      {
        ...
        if (print)
        {
          ...
          G4cerr << "\nG4AttCheck: ERROR " << iError << ": Illegal Value Type field \"" << valueType
                 << "\" for G4AttValue \"" << valueName << "\": " << value
...
std::ostream& operator<<(std::ostream& os, const G4AttCheck& ac)
{
  ...
  for (iValue = ac.fpValues->begin(); iValue != ac.fpValues->end(); ++iValue)
  {
    ...
    const G4String& value = iValue->GetValue();
    // NOLINTNEXTLINE(modernize-use-auto): Explicitly want a const_iterator
    map<G4String, G4AttDef>::const_iterator iDef = ac.fpDefinitions->find(valueName);
    G4bool error = false;
    if (iDef == ac.fpDefinitions->end())
    {
      error = true;
      os << "G4AttCheck: ERROR: No G4AttDef for G4AttValue \"" << valueName << "\": " << value
         << endl;
    }
    else
    {
      ...
      {
        error = true;
        os << "G4AttCheck: ERROR: Illegal Category Field \"" << category << "\" for G4AttValue \""
           << valueName << "\": " << value << "\n  Possible Categories:";
        set<G4String>::iterator i;
      ...
      {
        error = true;
        os << "G4AttCheck: ERROR: Illegal Extra field \"" << extra << "\" for G4AttValue \""
           << valueName << "\": " << value
           << "\n  Possible Extra fields if Category==\"Physics\":\n    ";
        set<G4String>::iterator i;
      ...
      {
        error = true;
        os << "G4AttCheck: ERROR: Illegal Value Type field \"" << valueType
           << "\" for G4AttValue \"" << valueName << "\": " << value << "\n  Possible Value Types:";
        set<G4String>::iterator i;
    ...
    if (!error)
    {
      os << iDef->second.GetDesc() << " (" << valueName << "): " << value;
      if (iDef->second.GetCategory() == "Physics" && !iDef->second.GetExtra().empty())
      {
...
void G4AttCheck::AddValuesAndDefs(std::vector<G4AttValue>* standardValues,
                                  std::map<G4String, G4AttDef>* standardDefinitions,
                                  const G4String& oldName, const G4String& name,
                                  const G4String& value, const G4String& extra,
    ...
    else
    {
      ...
      else
      {
        ...
        {  // Simply copy...
          ...
          (*standardDefinitions)[valueName] = fpDefinitions->find(valueName)->second;
        }
        else
        {  // "Physics"...
          if (extra.empty())
          {  // Dimensionless...
            ...
            {  // Split vector into 3...
              G4ThreeVector internalValue = G4UIcommand::ConvertTo3Vector(value);
              AddValuesAndDefs(standardValues, standardDefinitions, valueName, valueName + "-X",
                               G4UIcommand::ConvertToString(internalValue.x()), "",
                               fpDefinitions->find(valueName)->second.GetDesc() + "-X");
              AddValuesAndDefs(standardValues, standardDefinitions, valueName, valueName + "-Y",
                               G4UIcommand::ConvertToString(internalValue.y()), "",
                               fpDefinitions->find(valueName)->second.GetDesc() + "-Y");
              AddValuesAndDefs(standardValues, standardDefinitions, valueName, valueName + "-Z",
          ...
          {  // Dimensioned...
            ...
            if (fUnitCategories->find(unitCategory) != fUnitCategories->end())
            {
              ...
              G4double valueOfStandardUnit = G4UnitDefinition::GetValueOf(standardUnit);
              ...
              {  // Split vector into 3...
                ...
                AddValuesAndDefs(
                  standardValues, standardDefinitions, valueName, valueName + "-X",
                  G4UIcommand::ConvertToString(internalValue.x() / valueOfStandardUnit),
                  standardUnit, fpDefinitions->find(valueName)->second.GetDesc() + "-X");
                AddValuesAndDefs(
                  standardValues, standardDefinitions, valueName, valueName + "-Y",
                  G4UIcommand::ConvertToString(internalValue.y() / valueOfStandardUnit),
                  standardUnit, fpDefinitions->find(valueName)->second.GetDesc() + "-Y");
                AddValuesAndDefs(
                  standardValues, standardDefinitions, valueName, valueName + "-Z",
                  G4UIcommand::ConvertToString(internalValue.z() / valueOfStandardUnit),
                  standardUnit, fpDefinitions->find(valueName)->second.GetDesc() + "-Z");
              }
              else
              {
                G4double internalValue = G4UIcommand::ConvertToDimensionedDouble(valueAndUnit);
                AddValuesAndDefs(standardValues, standardDefinitions, valueName, valueName,
                                 G4UIcommand::ConvertToString(internalValue / valueOfStandardUnit),
                                 standardUnit);
              }
            }
          }
