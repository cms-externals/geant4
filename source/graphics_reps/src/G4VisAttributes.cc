...
G4VisAttributes::G4VisAttributes(G4bool visibility, const G4Colour& colour)
  : fVisible(visibility),
    fDaughtersInvisible(false),
    fColour(colour),
    fLineStyle(unbroken),
...
void G4VisAttributes::SetForceNumberOfCloudPoints(G4int nPoints)
{
  ...
  if (nPoints <= 0)
  {
    G4cout << "G4VisAttributes::SetForceNumberOfCloudPoints: number of cloud points"
              " set to "
...
void G4VisAttributes::SetForceLineSegmentsPerCircle(G4int nSegments)
{
  ...
  if (nSegments < nSegmentsMin)
  {
    ...
    G4cout << "G4VisAttributes::SetForcedLineSegmentsPerCircle: attempt to set the"
              "\nnumber of line segments per circle < "
           << nSegmentsMin << "; forced to " << nSegments << G4endl;
  }
