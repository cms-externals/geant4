  ...
  : fManager(manager)
{
  ...
  fListCmd = std::make_unique<G4UIcmdWithABool>("/analysis/list",this);
  G4String guidance = "List all/activate analysis objects.";
  fListCmd->SetGuidance(guidance);
  fListCmd->SetParameterName("OnlyIfActive",true);
...
void G4AnalysisMessenger::SetNewValue(G4UIcommand* command, G4String newValues)
{
  ...
  else if ( command == fSetActivationCmd.get() ) {
