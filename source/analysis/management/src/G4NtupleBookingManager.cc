  ...
  G4int ntupleId, const G4String& fileName)
{
  ...
  if ( ntupleBooking->fFileName == fileName ) {
    return;
  }
  ...
  if ( ! extension.size() ) {
    ...
    description
      << "      "
      << "No file extension is defined. Setting will be ignored";
  ...
  if ( ! fFileType ) {
    fFileType = extension;
    ntupleFileName = fileName;
  }
  else if ( fFileType != extension ) {
    // multiple file types are not suported
    ntupleFileName = baseFileName + "." + fFileType;
    G4ExceptionDescription description;
    description
      << "      "
