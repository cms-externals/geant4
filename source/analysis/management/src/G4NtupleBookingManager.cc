                                                                    ...
                                                                    G4bool warn) const
{
  ...
  if (index < 0 || index >= G4int(fNtupleBookingVector.size()))
  {
    if (warn)
    {
      Warn("Ntuple booking " + to_string(id) + " does not exist.", fkClass, functionName);
    }
    return nullptr;
  }
...
//_____________________________________________________________________________
G4bool G4NtupleBookingManager::CheckName(const G4String& name, const G4String& objectType) const
{
  if (name.size() == 0u)
  {
    Warn("Empty " + objectType + " name is not allowed.\n" + objectType + " was not created.",
         fkClass, "CheckName");
    return false;
  }
  return true;
...
//_____________________________________________________________________________
G4int G4NtupleBookingManager::CreateNtuple(const G4String& name, const G4String& title)
{
...
//_____________________________________________________________________________
G4int G4NtupleBookingManager::CreateNtupleIColumn(const G4String& name, std::vector<int>* vector)
{
  return CreateNtupleIColumn(GetCurrentNtupleId(), name, vector);
}

//_____________________________________________________________________________
G4int G4NtupleBookingManager::CreateNtupleFColumn(const G4String& name, std::vector<float>* vector)
{
  return CreateNtupleFColumn(GetCurrentNtupleId(), name, vector);
}

//_____________________________________________________________________________
G4int G4NtupleBookingManager::CreateNtupleDColumn(const G4String& name, std::vector<double>* vector)
{
...
//_____________________________________________________________________________
G4int G4NtupleBookingManager::CreateNtupleIColumn(G4int ntupleId, const G4String& name,
...
//_____________________________________________________________________________
G4int G4NtupleBookingManager::CreateNtupleFColumn(G4int ntupleId, const G4String& name,
...
//_____________________________________________________________________________
G4int G4NtupleBookingManager::CreateNtupleDColumn(G4int ntupleId, const G4String& name,
...
//_____________________________________________________________________________
G4int G4NtupleBookingManager::CreateNtupleSColumn(G4int ntupleId, const G4String& name,
                                                  std::vector<std::string>* vector)
{
...
G4bool G4NtupleBookingManager::SetFirstNtupleColumnId(G4int firstId)
{
  ...
  {
    Warn("Cannot set FirstNtupleColumnId as its value was already used.", fkClass,
...
//_____________________________________________________________________________
void G4NtupleBookingManager::SetActivation(G4int ntupleId, G4bool activation)
{
  auto ntupleBooking = GetNtupleBookingInFunction(ntupleId, "SetActivation");
...
{
  auto ntupleBooking = GetNtupleBookingInFunction(ntupleId, "GetActivation");
...
//_____________________________________________________________________________
void G4NtupleBookingManager::SetFileName(G4int ntupleId, const G4String& fileName)
{
  auto ntupleBooking = GetNtupleBookingInFunction(ntupleId, "SetFileName");
  if (ntupleBooking == nullptr) return;
...
//_____________________________________________________________________________
G4String G4NtupleBookingManager::GetFileName(G4int ntupleId) const
{
  auto ntupleBooking = GetNtupleBookingInFunction(ntupleId, "GetFileName");
  if (ntupleBooking == nullptr) return "";
...
G4bool G4NtupleBookingManager::List(std::ostream& output, G4bool onlyIfActive)
{
  ...
  for (auto g4NtupleBooking : fNtupleBookingVector)
  {
    ...
    // print selected info
    output << "   id: " << std::setw((G4int)maxIdWidth) << GetFirstId() + counter++ << " name: \""
           << std::setw((G4int)maxNameLength) << std::left << ntupleBooking.name() + "\""
           << " title: \"" << std::setw((G4int)maxTitleLength) << std::left
           << ntupleBooking.title() + "\"";
...
void G4NtupleBookingManager::SetFileType(const G4String& fileType)
{
  ...
  for (auto ntupleBooking : fNtupleBookingVector)
  {
    ...
    if (extension.size() != 0u)
    {
      Warn("Writing ntuples in files of different output types " + fFileType + ", " + extension
             + " is not supported.",
           fkClass, "SetFileType");
    }
...
//_____________________________________________________________________________
tools::ntuple_booking* G4NtupleBookingManager::GetNtuple(G4bool warn, G4bool onlyIfActive) const
{
  return GetNtuple(fFirstId, warn, onlyIfActive);
}

//_____________________________________________________________________________
tools::ntuple_booking* G4NtupleBookingManager::GetNtuple(G4int ntupleId, G4bool warn,
                                                         G4bool onlyIfActive) const
{
  ...
  if ((g4Booking->GetDeleted()) || (onlyIfActive && (!g4Booking->fActivation))) return nullptr;
