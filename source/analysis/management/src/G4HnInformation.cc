...
{
  G4cout << "NBins: " << fNBins << " minValue: " << fMinValue << " maxValue: " << fMaxValue << ";"
         << " edges: ";
...
{
  G4cout << "Unit name: " << fUnitName << " Fcn Name: " << fFcnName
         << " BinSchemeName: " << fBinSchemeName << " Unit: " << fUnit
         << " BinScheme: " << static_cast<int>(fBinScheme) << G4endl;
}
...
void Update(G4double& value, const G4HnDimensionInformation& hnInfo)
{
  ...
  {
    // Should never happen
    Warn("Illegal unit value (0), 1. will be used instead", kNamespaceName, "UpdateBins");
    unit = 1.;
  }
...
void Update(G4HnDimension& bins, const G4HnDimensionInformation& hnInfo)
{
  ...
  if (binScheme == G4BinScheme::kLinear)
  {
    ...
    G4Analysis::ComputeEdges(bins.fNBins, bins.fMinValue, bins.fMaxValue, unit, fcn, binScheme,
                             bins.fEdges);
...
{
  if (hnInfo.fFcnName != "none")
  {
    title += " ";
    title += hnInfo.fFcnName;
    title += "(";
  }
  if (hnInfo.fUnitName != "none")
  {
    title += " [";
    title += hnInfo.fUnitName;
    title += "]";
  }
  if (hnInfo.fFcnName != "none")
  {
...
//_____________________________________________________________________________
G4bool CheckDimension(unsigned int idim, const G4HnDimension& dimension,
                      const G4HnDimensionInformation& info)
{
  ...
  {
    Warn("Illegal value of number of " + xyz.substr(idim, 1) + " bins: nbins <= 0.", kNamespaceName,
         "CheckDimension");
    result = false;
  }

  // Check min/max
  if ((dimension.fMaxValue <= dimension.fMinValue) && (info.fBinScheme != G4BinScheme::kUser))
  {
    Warn("Illegal value of " + xyz.substr(idim, 1) + " (min >= max)", kNamespaceName,
         "CheckDimension");
    result = false;
  }
  ...
  if (info.fBinScheme == G4BinScheme::kUser)
  {
    ...
    {
      Warn(xyz.substr(idim, 1) + " edges vector is empty.", kNamespaceName, "CheckDimension");
      result = false;
    }
    ...
    for (size_t i = 1; i < dimension.fEdges.size(); ++i)
    {
      ...
      {
        Warn(xyz.substr(idim, 1) + " edges vector values must be defined in increasing order.",
  ...
  if ((info.fBinScheme == G4BinScheme::kLog || info.fFcnName == "log" || info.fFcnName == "log10")
      && (dimension.fMinValue == 0))
  {
    Warn("Illegal value of " + xyz.substr(idim, 1)
