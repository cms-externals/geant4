...
{
  std::string first = hnType.substr(0,1);
  if (first == "h") return "Histogram";
  if (first == "p") return "Profile";

  // other possibilities not handled
  return "";
}
...
//_____________________________________________________________________________
G4AnalysisMessengerHelper::G4AnalysisMessengerHelper(G4String hnType)
  : fHnType(std::move(hnType))
{}
...
//_____________________________________________________________________________
std::unique_ptr<G4UIcommand>
G4AnalysisMessengerHelper::CreateListCommand(G4UImessenger* messenger) const
{
  auto parOnlyIfActive = new G4UIparameter("onlyIfActive", 'b', true);
  parOnlyIfActive->SetGuidance(Update("Option whether to list only active OBJECT"));
  parOnlyIfActive->SetDefaultValue("true");

  std::unique_ptr<G4UIcommand> command(
    new G4UIcommand(Update("/analysis/HNTYPE_/list"), messenger));
  command->SetGuidance(Update("List all/activate NDIM_D LOBJECT"));
  command->SetParameter(parOnlyIfActive);
  command->AvailableForStates(G4State_Idle, G4State_GeomClosed, G4State_EventProc);
