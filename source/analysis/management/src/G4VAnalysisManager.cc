...
G4bool G4VAnalysisManager::SetH1(G4int id,
                                ...
                                const G4String& binSchemeName)
{
  if (! CheckNbins(nbins)) return false;
  if (! CheckMinMax(xmin, xmax, fcnName, binSchemeName)) return false;
...
G4bool G4VAnalysisManager::SetH1(G4int id,
                                ...
                                const G4String& unitName, const G4String& fcnName)
{
  if (! CheckEdges(edges)) return false;
...
G4bool G4VAnalysisManager::SetH2(G4int id,
                                ...
                                const G4String& ybinSchemeName)
{
  if (! CheckNbins(nxbins)) return false;
  if (! CheckMinMax(xmin, xmax, xfcnName, xbinSchemeName)) return false;

  if (! CheckNbins(nybins)) return false;
  if (! CheckMinMax(ymin, ymax, yfcnName, ybinSchemeName)) return false;
...
G4bool G4VAnalysisManager::SetH2(G4int id,
                                ...
                                const G4String& xfcnName, const G4String& yfcnName)
{
  if (! CheckEdges(xedges)) return false;
  if (! CheckEdges(yedges)) return false;
                                ...
                                const G4String& zbinSchemeName)
{
  ...
  if (! CheckNbins(nzbins)) return false;
  if (! CheckMinMax(zmin, zmax, zfcnName, zbinSchemeName)) return false;
                                ...
                                const G4String& zfcnName)
{
  if (! CheckEdges(xedges)) return false;
  if (! CheckEdges(yedges)) return false;
  if (! CheckEdges(zedges)) return false;
                                ...
                                const G4String& xbinSchemeName)
{
  ...
  if ( ymin != 0. || ymax != 0. ) {
    // Do not check  default values
    if (! CheckMinMax(ymin, ymax)) return false;
  }
                              ...
                              const G4String& ybinSchemeName)
{
  ...
  if ( zmin != 0. || zmax != 0. ) {
    // Do not check  default values
    if (! CheckMinMax(zmin, zmax)) return false;
  }
