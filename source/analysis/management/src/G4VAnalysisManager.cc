...
G4bool G4VAnalysisManager::Write()
{
  if ( ! IsOpenFile() ) {
    G4ExceptionDescription description;
    description
      << "Cannot write file. File is not open.";
    G4Exception("G4VFileManager::WriteFile()",
...
G4bool G4VAnalysisManager::CloseFile(G4bool reset)
{
  if ( ! IsOpenFile() ) {
    ...
    G4Exception("G4VFileManager::CloseFile()",
                "Analysis_W021", JustWarning, description);
    return false;
  }
...
{
  return fNtupleBookingManager->SetFirstId(firstId);
}  

//_____________________________________________________________________________
G4bool G4VAnalysisManager::SetFirstNtupleColumnId(G4int firstId) 
{
  return fNtupleBookingManager->SetFirstNtupleColumnId(firstId);
}
