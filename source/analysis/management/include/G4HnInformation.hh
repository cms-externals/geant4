...
#ifndef G4HNINFORMATION_HH
#define G4HNINFORMATION_HH
...
struct G4HnDimension
{
    G4HnDimension(G4int nbins, G4double minValue, G4double maxValue)
      : fNBins(nbins), fMinValue(minValue), fMaxValue(maxValue)
    {
      fEdges.clear();
    }

    G4HnDimension(const std::vector<G4double>& edges) : fEdges(edges) {}
...
struct G4HnDimensionInformation
{
    ...
    G4HnDimensionInformation() : G4HnDimensionInformation("none", "none", "linear") {}
...
class G4HnInformation
{
  public:
    ...
    G4HnInformation(G4String name, G4int nofDimensions) : fName(std::move(name))
    {
...
G4bool CheckDimensions(const std::array<G4HnDimension, DIM>& bins,
...
inline void G4HnInformation::AddDimension(const G4HnDimensionInformation& hnDimensionInformation)
{
...
inline const G4HnDimensionInformation&
G4HnInformation::GetHnDimensionInformation(G4int dimension) const
{
  return fHnDimensionInformations[dimension];
}
...
template<unsigned int DIM>
inline G4bool G4Analysis::CheckDimensions(const std::array<G4HnDimension, DIM>& bins,
                                          const std::array<G4HnDimensionInformation, DIM>& hnInfo,
                                          G4bool isProfile)
{
  G4bool result = true;
