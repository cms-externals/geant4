...
G4GenericFileManager::GetFileManager(const G4String& fileName)
{
  ...
  if ( ! extension.size() ) {
    ...
    description 
      << "      " 
      << "Cannot create file. No output type is defined.";
...
{
  // Check consistency if the default file manager already exists
  if ( fDefaultFileManager ) {
    G4ExceptionDescription description;
    description
      << "Cannot open file " << fileName << G4endl
      << "Default file manager is NOT set.";
    G4Exception("G4GenericFileManager::OpenFile",
                "Analysis_W001", JustWarning, description);
    return false;
  } else {
    // Construct the default file manager according to the extension
    fDefaultFileManager = GetFileManager(fileName);
...
//_____________________________________________________________________________
G4bool G4GenericFileManager:: WriteFile()
{
  if ( ! fDefaultFileManager ) {
    G4ExceptionDescription description;
    description 
      << "Cannot write. File is not open.";
    G4Exception("G4GenericFileManager::WriteFile",
                "Analysis_W021", JustWarning, description);
    return false;
  }

  return fDefaultFileManager->WriteFile();
}

//_____________________________________________________________________________
G4bool G4GenericFileManager::CloseFile()
{
  if ( ! fDefaultFileManager ) {
    G4ExceptionDescription description;
    description 
      << "Cannot close file. File is not open.";
    G4Exception("G4GenericFileManager::CloseFile",
                "Analysis_W021", JustWarning, description);
    // Add warning
    return false;
  }

  return fDefaultFileManager->CloseFile();
} 
