...
void NotExistWarning(const G4String& what, G4int id, std::string_view className,
                     std::string_view functionName)
{
  Warn(what + " id= " + to_string(id) + " does not exist.", className, functionName);
}
...
G4RootPNtupleManager::GetNtupleDescriptionInFunction(G4int id, std::string_view functionName,
                                                     G4bool warn) const
{
                                                ...
                                                tools::wroot::ntuple* mainNtuple)
{
  ...
  if (!file)
  {
    Warn("Cannot create pntuple. Main ntuple file does not exist.", fkClass,
  ...
  if (fRowWise)
  {
    ...
    auto mtNtuple = new tools::wroot::mt_ntuple_row_wise(
      G4cout, rfile->byte_swap(), rfile->compression(), mainNtuple->dir().seek_directory(),
      *mainBranch, mainBranch->basket_size(),
      ntupleDescription->GetDescription().GetNtupleBooking(), verbose);

    ntupleDescription->SetNtuple(static_cast<tools::wroot::imt_ntuple*>(mtNtuple));
    ntupleDescription->SetBasePNtuple(static_cast<tools::wroot::base_pntuple*>(mtNtuple));
  }
  else
  {
    ...
    auto mtNtuple = new tools::wroot::mt_ntuple_column_wise(
      G4cout, rfile->byte_swap(), rfile->compression(), mainNtuple->dir().seek_directory(),
      ntupleDescription->GetMainBranches(), basketSizes,
      ntupleDescription->GetDescription().GetNtupleBooking(), fRowMode, basketEntries, verbose);
...
//_____________________________________________________________________________
G4bool G4RootPNtupleManager::FillNtupleIColumn(G4int ntupleId, G4int columnId, G4int value)
{
  return FillNtupleTColumn<int>(ntupleId, columnId, value);
}

//_____________________________________________________________________________
G4bool G4RootPNtupleManager::FillNtupleFColumn(G4int ntupleId, G4int columnId, G4float value)
{
  return FillNtupleTColumn<float>(ntupleId, columnId, value);
}

//_____________________________________________________________________________
G4bool G4RootPNtupleManager::FillNtupleDColumn(G4int ntupleId, G4int columnId, G4double value)
{
  return FillNtupleTColumn<double>(ntupleId, columnId, value);
}

//_____________________________________________________________________________
G4bool G4RootPNtupleManager::FillNtupleSColumn(G4int ntupleId, G4int columnId,
...
G4bool G4RootPNtupleManager::AddNtupleRow(G4int ntupleId)
{
  ...
  auto result = ntupleDescription->GetNtuple()->add_row(toolsLock, *rfile);

  if (!result)
  {
    Warn("NtupleId " + to_string(ntupleId) + "adding row failed.", fkClass, "AddNtupleRow");
  }
...
G4bool G4RootPNtupleManager::Merge()
{
  for (auto ntupleDescription : fNtupleDescriptionVector)
  {
    ...
    lock.unlock();
    mutex toolsLock(lock);
    auto result = ntupleDescription->GetNtuple()->end_fill(toolsLock, *rfile);
...
void G4RootPNtupleManager::SetActivation(G4int ntupleId, G4bool activation)
{
