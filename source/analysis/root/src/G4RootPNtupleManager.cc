                             ...
                             tools::wroot::ntuple* mainNtuple)
{
  ...
  auto rfile = std::get<0>(*fMainNtupleManager->GetNtupleFile());
  // auto directory = std::get<2>(*fMainNtupleManager->GetNtupleFile());
...
// This function is called from G4AnalysisManager::OpenFile.
  ...
  for ( auto g4NtupleBooking : g4NtupleBookings ) {
    ...
    ntupleDescription->fNtupleBooking = g4NtupleBooking->fNtupleBooking;
    ntupleDescription->fActivation = g4NtupleBooking->fActivation;
    fNtupleDescriptionVector.push_back(ntupleDescription);  
  }
  
  if ( fCreateMode == G4PNtupleCreateMode::kUndefined ) {
    if ( g4NtupleBookings.size() ) {
...
{
// Create pntuple from tools ntuple_booking

#ifdef G4VERBOSE
  if ( fState.GetVerboseL4() ) 
    fState.GetVerboseL4()->Message("create", "pntuple", booking->fNtupleBooking.name());
#endif
  ...
  // Create pntuple description
  auto index = fNtupleDescriptionVector.size();
  auto ntupleDescription = new G4RootPNtupleDescription();
  fNtupleDescriptionVector.push_back(ntupleDescription);  

  // Save g4booking, activation in pntuple booking
  ntupleDescription->fNtupleBooking = booking->fNtupleBooking;
  ntupleDescription->fActivation = booking->fActivation;

  // Ntuple Id
  auto ntupleId = index + fFirstId;

  // Create pntuple
  // if ( fMainNtupleManager->GetNtupleFile() ) {
  if ( fCreateMode == G4PNtupleCreateMode::kSlaveAfterOpen ) {
    auto mainNtuple = GetMainNtupleInFunction(ntupleId, "CreateNtuple");
    if ( ! mainNtuple ) {
       G4ExceptionDescription description;
       description 
         << "Main ntuple " << ntupleId << " not found.";
       G4Exception("G4RootPNtupleManager::CreateNtuple",
  ...
  if ( fState.GetVerboseL2() ) {
    G4ExceptionDescription description;
    description << booking->fNtupleBooking.name() << " ntupleId " << ntupleId;
    fState.GetVerboseL2()->Message("create", "pntuple", description);
  } 
#endif
