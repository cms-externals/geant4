...
#ifndef G4HDF5NTUPLEMANAGER_HH
#define G4HDF5NTUPLEMANAGER_HH
...
template<>
G4bool
G4TNtupleManager<toolx::hdf5::ntuple, G4Hdf5File>::FillNtupleTColumn(G4int ntupleId, G4int columnId,
                                                                     const std::string& value);

class G4Hdf5NtupleManager : public G4TNtupleManager<toolx::hdf5::ntuple, G4Hdf5File>
{
...
inline void G4Hdf5NtupleManager::SetFileManager(std::shared_ptr<G4Hdf5FileManager> fileManager)
{
                                                                     ...
                                                                     const std::string& value)
{
  ...
  {
    G4Analysis::Warn("ntupleId " + to_string(ntupleId) + " columnId " + to_string(columnId)
                       + " does not exist.",
  ...
  if (column == nullptr)
  {
    G4Analysis::Warn("Column type does not match: ntupleId " + to_string(ntupleId) + " columnId "
                       + to_string(columnId) + " value " + value,
                     fkClass, "FillNtupleTColumn");
    return false;
  }
