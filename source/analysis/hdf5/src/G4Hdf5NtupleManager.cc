...
//
//_____________________________________________________________________________
void G4Hdf5NtupleManager::CreateTNtuple(Hdf5NtupleDescription* ntupleDescription, G4bool warn)
{
  ...
  if (!ntupleFile)
  {
    if (warn)
    {
      Warn("Cannot create ntuple. Ntuple file does not exist.", fkClass, "CreateTNtuple");
    }
    return;
  }
  ...
  ntupleDescription->SetNtuple(
    new toolx::hdf5::ntuple(G4cout, directory, ntupleBooking, compressionLevel, basketSize));
}
