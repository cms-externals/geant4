...
{
  // Do nothing if there is no file
  if ( GetFileHid() < 0 ) return true;

#ifdef G4VERBOSE
  if ( fState.GetVerboseL4() ) {
    fState.GetVerboseL4()->Message("close", "file", GetFullFileName(fFileName));
  }
#endif

  ::H5Gclose(GetHistoDirectory());
  ::H5Gclose(GetNtupleDirectory());
  ::H5Fclose(GetFileHid());
  ...
  if ( fState.GetVerboseL1() ) {
    fState.GetVerboseL1()->Message("close", "file", GetFullFileName(fFileName), true);
  }
#endif
...
//_____________________________________________________________________________
hid_t G4Hdf5FileManager::GetFileHid() const
{
  if ( ! fFile ) return kInvalidId;

  return std::get<0>(*fFile);
}
