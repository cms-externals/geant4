  ...
  if ( xbinScheme != G4BinScheme::kLog ) {
    ...
    if ( ymin == 0. && ymax == 0.) {
      return new tools::histo::p1d(
        title, nbins, xfcn(xmin/xunit), xfcn(xmax/xunit));
    }
    return new tools::histo::p1d(
      title, nbins, xfcn(xmin / xunit), xfcn(xmax / xunit), yfcn(ymin / yunit), yfcn(ymax / yunit));
  }
  else {
    // Compute edges
...
//_____________________________________________________________________________
G4bool G4P1ToolsManager::List(std::ostream& output, G4bool onlyIfAcive)
{
  return G4THnManager<tools::histo::p1d>::List(output, onlyIfAcive);
}
