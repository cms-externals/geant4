                         ...
                         std::string_view className)
{
  ...
  if ( xbinScheme != G4BinScheme::kLog && ybinScheme !=  G4BinScheme::kLog) {
    ...
    return new tools::histo::p2d(title, xedges, yedges, zfcn(zmin / zunit), zfcn(zmax / zunit));
  }
}
                         ...
                         const G4String& zfcnName)
{
  ...
  return new tools::histo::p2d(title, xnewEdges, ynewEdges, zfcn(zmin / zunit), zfcn(zmax / zunit));
             // p2 objects are deleted in destructor and reset when
             // closing a file.
}
...
//_____________________________________________________________________________
G4bool G4P2ToolsManager::List(std::ostream& output, G4bool onlyIfAcive)
{
  return G4THnManager<tools::histo::p2d>::List(output, onlyIfAcive);
}
