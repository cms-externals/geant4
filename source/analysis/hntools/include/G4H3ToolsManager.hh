                         ...
                         public G4THnManager<tools::histo::h3d>
{
  public:
    ...
    ~G4H3ToolsManager() override = default;
  ...
  protected:
    ...
    G4bool ScaleH3(G4int id, G4double factor) final;

    // Method to fill histograms
    //
    G4bool FillH3(
      G4int id, G4double xvalue, G4double yvalue, G4double zvalue, G4double weight = 1.0) final;

    // Methods to manipulate histograms
    //

    // Access methods
    G4int GetH3Id(const G4String& name, G4bool warn = true) const final;

    // Access to H3 parameters
    G4int GetH3Nxbins(G4int id) const final;
    G4double GetH3Xmin(G4int id) const final;
    G4double GetH3Xmax(G4int id) const final;
    G4double GetH3XWidth(G4int id) const final;
    G4int GetH3Nybins(G4int id) const final;
    G4double GetH3Ymin(G4int id) const final;
    G4double GetH3Ymax(G4int id) const final;
    G4double GetH3YWidth(G4int id) const final;
    G4int GetH3Nzbins(G4int id) const final;
    G4double GetH3Zmin(G4int id) const final;
    G4double GetH3Zmax(G4int id) const final;
    G4double GetH3ZWidth(G4int id) const final;

    // Setters for attributes for plotting
    G4bool SetH3Title(G4int id, const G4String& title) final;
    G4bool SetH3XAxisTitle(G4int id, const G4String& title) final;
    G4bool SetH3YAxisTitle(G4int id, const G4String& title) final;
    G4bool SetH3ZAxisTitle(G4int id, const G4String& title) final;

    // Access attributes for plotting
    G4String GetH3Title(G4int id) const final;
    G4String GetH3XAxisTitle(G4int id) const final;
    G4String GetH3YAxisTitle(G4int id) const final;
    G4String GetH3ZAxisTitle(G4int id) const final;

    // Methods to list/print histograms
    G4bool WriteOnAscii(std::ofstream& output) final;
    G4bool List(std::ostream& output, G4bool onlyIfActive = true) final;

    // Access to Hn manager
    std::shared_ptr<G4HnManager> GetHnManager() final;
