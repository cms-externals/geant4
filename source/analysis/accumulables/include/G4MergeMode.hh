...
#ifndef G4MERGEMODE_HH
#define G4MERGEMODE_HH
...
G4MergeFunction<T> GetMergeFunction(G4MergeMode mergeMode)
{
  ...
  G4Exception("G4Accumulables::GetMergeFunction<T>", "Analysis_F001", FatalException,
              "Undefined merge mode");
  return [](const T&, const T&) {
    return T();
  };
}
...
G4MergeFunction<G4bool> GetMergeFunction(G4MergeMode mergeMode)
{
  switch (mergeMode)
  {
    ...
    case G4MergeMode::kMaximum:
      ...
      return [](const G4bool& x, const G4bool& y) {
        return x || y;
      };
