...
G4PhysListRegistry::G4PhysListRegistry() : verbose(1), unknownFatal(0), systemDefault("FTFP_BERT")
{
...
G4VModularPhysicsList* G4PhysListRegistry::GetModularPhysicsList(const G4String& name)
{
  ...
  G4bool allKnown = DeconstructPhysListName(name, plBase, physExt, physReplace, verbose);

  std::size_t npc = physExt.size();
  if (verbose > 0)
  {
    G4cout << "G4PhysListRegistry::GetModularPhysicsList <" << name << ">"
  ...
  if (!allKnown)
  {
    ...
    G4cout << "### G4PhysListRegistry WARNING: " << name << " is not known" << G4endl << G4endl;
    ...
    G4Exception("G4PhysListRegistry::GetModularPhysicsList", "PhysicsList002", FatalException, ED);
    return nullptr;
  }
  ...
  for (std::size_t ipc = 0; ipc < npc; ++ipc)
  {
    ...
    G4String pcname = ((physReplace[ipc] & isCtorName)) ? extName : physicsExtensions[extName];
    ...
    if (verbose > 0)
      G4cout << "<<< " << reporreg << " with " << pcname << " \"" << extName << "\"" << G4endl;
  }
...
G4VModularPhysicsList* G4PhysListRegistry::GetModularPhysicsListFromEnv()
{
  ...
  else
  {
    ...
    G4cout << "### G4PhysListRegistry WARNING: "
           ...
           << "    Default Physics Lists " << name << " is instantiated" << G4endl;
  }
...
G4bool G4PhysListRegistry::DeconstructPhysListName(const G4String& name, G4String& plBase,
                                                   std::vector<G4String>& physExt,
                                                   std::vector<G4int>& replace, G4int verb) const
{
  ...
  const std::vector<G4String>& availPhysCtors = physConstRegistry->AvailablePhysicsConstructors();
  ...
  while (!workingName.empty())
  {
    ...
    if (extraKnown)
    {
      ...
      std::map<G4String, G4String>::const_iterator itr = physicsExtensions.find(extraName);
      G4String pcname = "";
      ...
      if (verb > 2)
      {
        G4cout << "  extraName \"" << extraName << "\" maps to physics ctor \"" << pcname
               << "\" which is itself realknown " << realknown << G4endl;
      }
#endif
    }
    else
    {
      ...
      extraKnown = FindLongestMatch(workingName, "physCtors", availPhysCtors, extraName);
    ...
    if (verb > 2)
    {
      G4cout << "  physextra " << name << " [" << workingName << "]"
             << ", extra known " << extraKnown << " chosen extra \"" << extraName << "\""
    ...
    else
    {
      ...
      if (verb > 2)
      {
        G4cout << "  workingName \"" << workingName << "\""
               << " couldn't be found in the extensions list" << G4endl;
      }
#endif
      allKnown = false;
...
G4bool G4PhysListRegistry::FindLongestMatch(const G4String& workingName, const G4String& searchName,
                                            const std::vector<G4String>& validNames,
                                            G4String& bestMatch, G4int verb) const
{
  bestMatch = "";
  G4bool found = false;
    ...
    if (ipos == 0)
    {
      ...
      if (searchName == "base")
      {
        ...
        if (workNameSize > testNameSize)
        {
          ...
          if (verb > 3)
          {
            G4cout << "  " << searchName << " longer " << testName << " nextChar " << nextChar
                   << G4endl;
          }
        }
      }
      if (verb > 3)
      {
        G4cout << "  " << searchName << " match " << (match ? "yes" : "no") << " " << testName
               << G4endl;
      }
      if (match)
      {
        if (testName.size() > bestMatch.size())
        {
          ...
          if (verb > 3)
          {
            G4cout << "  " << searchName << " current best guess: " << testName << G4endl;
          }
        }
        else
        {
          if (verb > 3)
          {
            G4cout << "  " << searchName << " match but shorter than previous: " << testName
                   << G4endl;
          }
        }
      }
      else
      {
        if (verb > 3)
        {
          G4cout << "  " << searchName << " ipos=0 but no match " << testName << G4endl;
        }
      }
    }
    else
...
void G4PhysListRegistry::PrintAvailablePhysLists() const
{
  ...
  G4cout << "Base G4VModularPhysicsLists in G4PhysListRegistry are:" << G4endl;
  if (avail.empty())
    G4cout << "... no registered lists" << G4endl;
  else
  {
  ...
  std::map<G4String, G4String>::const_iterator itr;
  G4cout << "Replacement mappings in G4PhysListRegistry are:" << G4endl;
  for (itr = physicsExtensions.begin(); itr != physicsExtensions.end(); ++itr)
  {
    G4bool known = physConstRegistry->IsKnownPhysicsConstructor(itr->second);

    G4cout << "    " << std::setw(10) << itr->first << " => " << std::setw(30) << itr->second << " "
           << ((known) ? "" : "[unregistered physics]") << G4endl;
  }
  G4cout << "Use these mapping to extend physics list; append with _EXT or +EXT" << G4endl
         << "   to use ReplacePhysics() (\"_\") or RegisterPhysics() (\"+\")." << G4endl;
}
