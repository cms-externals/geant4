...
Shielding::Shielding(G4int verbose, const G4String& n_model, const G4String& HadrPhysVariant,
                     G4bool useLightIonQMD)
{
  ...
  if (LEN_model == "LEND") emExtraPhysics->LENDGammaNuclear(true);
  ...
  if (LEN_model == "LEND") ((G4HadronPhysicsShielding*)hpc)->UseLEND();

  RegisterPhysics(hpc);

  // Activate production of fission fragments in NeutronHP
  if (LEN_model == "HP") G4ParticleHPManager::GetInstance()->SetProduceFissionFragments(true);

  // Add LEND processes (neutron and photon induced)
  if (LEN_model == "LEND") RegisterPhysics(new G4HadronPhysicsLEND(verbose, evaluation));
