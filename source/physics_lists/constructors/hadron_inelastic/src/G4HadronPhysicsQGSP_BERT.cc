  ...
  : G4VPhysicsConstructor(name)
{
  ...
  minQGSP_proton = minQGSP_neutron = minQGSP_pik = param->GetMinEnergyTransitionQGS_FTF();
  maxFTFP_proton = maxFTFP_neutron = maxFTFP_pik = param->GetMaxEnergyTransitionQGS_FTF();
  minFTFP_proton = minFTFP_neutron = minFTFP_pik = param->GetMinEnergyTransitionFTF_Cascade();
  maxBERT_proton = maxBERT_neutron = maxBERT_pik = param->GetMaxEnergyTransitionFTF_Cascade();
...
void G4HadronPhysicsQGSP_BERT::ConstructProcess()
{
  ...
  if (G4Threading::IsMasterThread() && param->GetVerboseLevel() > 0) DumpBanner();
...
void G4HadronPhysicsQGSP_BERT::DumpBanner()
{
  ...
  if (maxBIC_proton > 0.0 || maxBIC_neutron > 0.0)
  {
    G4cout << "    0) between BIC and BERT for p, n over the interval "
           << minBERT_proton / CLHEP::GeV << " to " << maxBIC_proton / CLHEP::GeV << " GeV. "
           << G4endl;
  }
  G4cout << "    1) between BERT  and FTF/P over the interval " << minFTFP_proton / CLHEP::GeV
         << " to " << maxBERT_proton / CLHEP::GeV << " GeV. " << G4endl;
  G4cout << "    2) between FTF/P and QGS/P over the interval " << minQGSP_proton / CLHEP::GeV
         << " to " << maxFTFP_proton / CLHEP::GeV << " GeV. " << G4endl;
