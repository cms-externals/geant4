...
void G4HadronPhysicsQGS_BIC::PiK()
{
  G4HadronicParameters* param = G4HadronicParameters::Instance();
  G4bool useFactorXS = param->ApplyFactorXS();
  ...
  G4QGSBinaryPionBuilder qgs( QuasiElasticQGS );
  qgs.SetMinEnergy( minQGSP_pik );
  pib.RegisterMe( &qgs );

  G4FTFBinaryPionBuilder ftf( QuasiElasticFTF );
  ftf.SetMaxEnergy( maxFTFP_pik );
  ftf.SetMinEnergy( minFTFP_pik );
  pib.RegisterMe( &ftf );

  G4BertiniPionBuilder bert;
  bert.SetMinEnergy( minBERT_proton );
  bert.SetMaxEnergy( maxBERT_pik );
  pib.RegisterMe( &bert );

  if ( maxBIC_proton > 0.0 ) {
    G4BinaryPionBuilder bic;
    bic.SetMaxEnergy( maxBIC_proton );
    pib.RegisterMe( &bic );
  }

  // build all models for pions
  pib.Build();

  G4KaonBuilder k;

  G4QGSBinaryKaonBuilder qgsk( QuasiElasticQGS );
  qgsk.SetMinEnergy( minQGSP_pik );
  k.RegisterMe( &qgsk );
  
  G4FTFBinaryKaonBuilder ftfk( QuasiElasticFTF );
  ftfk.SetMaxEnergy( maxFTFP_pik );
  k.RegisterMe( &ftfk );
  G4BertiniKaonBuilder bertk;
  bertk.SetMaxEnergy( maxBERT_pik );
  k.RegisterMe( &bertk );

  // build all models for kaons
  k.Build();
