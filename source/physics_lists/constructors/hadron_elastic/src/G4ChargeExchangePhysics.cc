...
G4ChargeExchangePhysics::G4ChargeExchangePhysics(G4int ver)
  : G4VPhysicsConstructor("chargeExchange"), fLowEnergyLimit(12 * CLHEP::GeV)
{
  ...
  if (ver > 1)
  {
    G4cout << "### ChargeExchangePhysics above " << fLowEnergyLimit / CLHEP::GeV << " GeV."
           << G4endl;
  }
}
...
void G4ChargeExchangePhysics::ConstructProcess()
{
  ...
  if (G4HadronicParameters::Instance()->GetVerboseLevel() > 1)
  {
    G4cout << "### ChargeExchangePhysics Construct Processes with the model <"
           << model->GetModelName() << "> and x-section <" << xs->GetName()
           << ">  XSFactorPi=" << fXSFactorPi << ", XSFactorK=" << fXSFactorK << G4endl;
  }

  // pi-
  G4ParticleDefinition* part = G4PionMinus::PionMinus();
  auto proc = new G4HadronInelasticProcess(part->GetParticleName() + "ChargeEx", part);
  proc->AddDataSet(xs);
