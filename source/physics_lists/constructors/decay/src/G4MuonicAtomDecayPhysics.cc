  ...
  : G4VPhysicsConstructor("G4MuonicAtomDecay")
{
  ...
  {
    G4cout << "G4MuonicAtomDecayPhysics() with verboseLevel " << verboseLevel << G4endl;
  }
#endif
}

G4MuonicAtomDecayPhysics::G4MuonicAtomDecayPhysics(const G4String&) : G4MuonicAtomDecayPhysics(1) {}
...
void G4MuonicAtomDecayPhysics::ConstructProcess()
{
  ...
  if (GetVerboseLevel() > 0)
  {
    G4cout << "G4MuonicAtomDecayPhysics::ConstructProcess() verboseLevel " << GetVerboseLevel()
           << " to be set to " << plhvl << G4endl;
  }
#endif
  G4bool rc1 = plh->RegisterProcess(
    new G4MuonicAtomDecay(nullptr),  // default G4HadronicInteraction* will be set
    G4GenericMuonicAtom::GenericMuonicAtom());
  plh->SetVerboseLevel(plhvl);
  if (!(rc1))
  {
    G4cout << " G4MuonicAtomDecayPhysics::ConstructProcess() : "
           << " RegisterProcess failed for G4GenericMuonicAtom " << G4endl;
  }
}
