...
G4EmDNAChemistry_option2::G4EmDNAChemistry_option2() : G4VUserChemistryList(true)
{
...
void G4EmDNAChemistry_option2::ConstructMolecule()
{
  ...
  G4MoleculeTable::Instance()->CreateConfiguration("Deoxyribose", G4Deoxyribose::Definition());
  G4MoleculeTable::Instance()->CreateConfiguration("Phosphate", G4Phosphate::Definition());
  G4MoleculeTable::Instance()->CreateConfiguration("Adenine", G4Adenine::Definition());
  G4MoleculeTable::Instance()->CreateConfiguration("Thymine", G4Thymine::Definition());
  G4MoleculeTable::Instance()->CreateConfiguration("Guanine", G4Guanine::Definition());
  G4MoleculeTable::Instance()->CreateConfiguration("Cytosine", G4Cytosine::Definition());
  G4MoleculeTable::Instance()->CreateConfiguration("Histone", G4Histone::Definition());

  // damaged DNAElement Configuration

  G4MoleculeTable::Instance()->CreateConfiguration("Damaged_Deoxyribose",
                                                   G4DamagedDeoxyribose::Definition());
  G4MoleculeTable::Instance()->CreateConfiguration("Damaged_Adenine",
                                                   G4DamagedAdenine::Definition());
  G4MoleculeTable::Instance()->CreateConfiguration("Damaged_Thymine",
                                                   G4DamagedThymine::Definition());
  G4MoleculeTable::Instance()->CreateConfiguration("Damaged_Guanine",
                                                   G4DamagedGuanine::Definition());
  G4MoleculeTable::Instance()->CreateConfiguration("Damaged_Cytosine",
                                                   G4DamagedCytosine::Definition());
}
...
void G4EmDNAChemistry_option2::ConstructReactionTable(G4DNAMolecularReactionTable* theReactionTable)
{
  //-----------------------------------
  ...
  G4MolecularConfiguration* adenine = G4MoleculeTable::Instance()->GetConfiguration("Adenine");
  G4MolecularConfiguration* guanine = G4MoleculeTable::Instance()->GetConfiguration("Guanine");
  G4MolecularConfiguration* thymine = G4MoleculeTable::Instance()->GetConfiguration("Thymine");
  G4MolecularConfiguration* cytosine = G4MoleculeTable::Instance()->GetConfiguration("Cytosine");
  G4MolecularConfiguration* histone = G4MoleculeTable::Instance()->GetConfiguration("Histone");
  ...
  reactionData = new G4DNAMolecularReactionData(1.80e9 * (1e-3 * m3 / (mole * s)), deoxyribose, OH);
  reactionData->AddProduct(damage_deoxyribose);
  theReactionTable->SetReaction(reactionData);

  // adenine + OH -> ...
  reactionData = new G4DNAMolecularReactionData(6.10e9 * (1e-3 * m3 / (mole * s)), adenine, OH);
  reactionData->AddProduct(damage_adenine);
  theReactionTable->SetReaction(reactionData);

  // guanine + OH -> ...
  reactionData = new G4DNAMolecularReactionData(9.20e9 * (1e-3 * m3 / (mole * s)), guanine, OH);
  reactionData->AddProduct(damage_guanine);
  theReactionTable->SetReaction(reactionData);

  // thymine + OH -> ...
  reactionData = new G4DNAMolecularReactionData(6.40e9 * (1e-3 * m3 / (mole * s)), thymine, OH);
  reactionData->AddProduct(damage_thymine);
  theReactionTable->SetReaction(reactionData);

  // cytosine + OH -> ...
  reactionData = new G4DNAMolecularReactionData(6.10e9 * (1e-3 * m3 / (mole * s)), cytosine, OH);
  ...
  // adenine + Hydrated e- -> ...
  reactionData = new G4DNAMolecularReactionData(9e9 * (1e-3 * m3 / (mole * s)), adenine, e_aq);
  reactionData->AddProduct(damage_adenine);
  theReactionTable->SetReaction(reactionData);

  // guanine + Hydrated e- -> ...
  reactionData = new G4DNAMolecularReactionData(14e9 * (1e-3 * m3 / (mole * s)), guanine, e_aq);
  reactionData->AddProduct(damage_guanine);
  theReactionTable->SetReaction(reactionData);

  // thymine + Hydrated e- -> ...
  reactionData = new G4DNAMolecularReactionData(18e9 * (1e-3 * m3 / (mole * s)), thymine, e_aq);
  reactionData->AddProduct(damage_thymine);
  theReactionTable->SetReaction(reactionData);

  // cytosine + Hydrated e- -> ...
  reactionData = new G4DNAMolecularReactionData(13e9 * (1e-3 * m3 / (mole * s)), cytosine, e_aq);
  reactionData->AddProduct(damage_cytosine);
  theReactionTable->SetReaction(reactionData);
  ...
  reactionData = new G4DNAMolecularReactionData(0.029e9 * (1e-3 * m3 / (mole * s)), deoxyribose, H);
  reactionData->AddProduct(damage_deoxyribose);
  ...
  // adenine + Radical H -> ...
  reactionData = new G4DNAMolecularReactionData(0.10e9 * (1e-3 * m3 / (mole * s)), adenine, H);
  reactionData->AddProduct(damage_adenine);
  theReactionTable->SetReaction(reactionData);

  // thymine + Radical H -> ...
  reactionData = new G4DNAMolecularReactionData(0.57e9 * (1e-3 * m3 / (mole * s)), thymine, H);
  reactionData->AddProduct(damage_thymine);
  theReactionTable->SetReaction(reactionData);

  // cytosine + Radical H -> ...
  reactionData = new G4DNAMolecularReactionData(0.092e9 * (1e-3 * m3 / (mole * s)), cytosine, H);
  reactionData->AddProduct(damage_cytosine);
  theReactionTable->SetReaction(reactionData);

  // histone + all molecules -> modification(or "damage")

  reactionData = new G4DNAMolecularReactionData(0.0 * (1e-3 * m3 / (mole * s)), histone, OH);
  reactionData->AddProduct(histone);
  reactionData->SetEffectiveReactionRadius(2.4 * nm + G4OH::Definition()->GetVanDerVaalsRadius());
  theReactionTable->SetReaction(reactionData);

  reactionData = new G4DNAMolecularReactionData(0.0 * (1e-3 * m3 / (mole * s)), histone, OHm);
  reactionData->AddProduct(histone);
  reactionData->SetEffectiveReactionRadius(2.4 * nm + G4OH::Definition()->GetVanDerVaalsRadius());
  theReactionTable->SetReaction(reactionData);

  reactionData = new G4DNAMolecularReactionData(0.0 * (1e-3 * m3 / (mole * s)), histone, e_aq);
  reactionData->AddProduct(histone);
  reactionData->SetEffectiveReactionRadius(2.4 * nm
                                           + G4Electron_aq::Definition()->GetVanDerVaalsRadius());
  theReactionTable->SetReaction(reactionData);

  reactionData = new G4DNAMolecularReactionData(0.0 * (1e-3 * m3 / (mole * s)), histone, H2);
  reactionData->AddProduct(histone);
  reactionData->SetEffectiveReactionRadius(2.4 * nm + G4H2::Definition()->GetVanDerVaalsRadius());
  theReactionTable->SetReaction(reactionData);

  reactionData = new G4DNAMolecularReactionData(0.0 * (1e-3 * m3 / (mole * s)), histone, H3Op);
  reactionData->AddProduct(histone);
  reactionData->SetEffectiveReactionRadius(2.4 * nm + G4H3O::Definition()->GetVanDerVaalsRadius());
  theReactionTable->SetReaction(reactionData);

  reactionData = new G4DNAMolecularReactionData(0.0 * (1e-3 * m3 / (mole * s)), histone, H);
  reactionData->AddProduct(histone);
  reactionData->SetEffectiveReactionRadius(2.4 * nm
                                           + G4Hydrogen::Definition()->GetVanDerVaalsRadius());
  theReactionTable->SetReaction(reactionData);

  reactionData = new G4DNAMolecularReactionData(0.0 * (1e-3 * m3 / (mole * s)), histone, H2O2);
  reactionData->AddProduct(histone);
  reactionData->SetEffectiveReactionRadius(2.4 * nm + G4H2O2::Definition()->GetVanDerVaalsRadius());
  theReactionTable->SetReaction(reactionData);
}
...
void G4EmDNAChemistry_option2::ConstructTimeStepModel(G4DNAMolecularReactionTable* reactionTable)
{
