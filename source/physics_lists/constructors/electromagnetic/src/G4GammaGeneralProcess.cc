...
void G4GammaGeneralProcess::ProcessDescription(std::ostream& out) const
{
  ...
  if (nullptr != theRayleigh) { theRayleigh->ProcessDescription(out); }
  if (nullptr != theGammaNuclear) { theGammaNuclear->ProcessDescription(out); }
  if (nullptr != theConversionMM) { theConversionMM->ProcessDescription(out); }
}
