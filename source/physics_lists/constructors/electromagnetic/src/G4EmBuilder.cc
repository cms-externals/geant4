                                          ...
                                          const std::vector<G4int>& partList)
{
  ...
  for (auto& pdg : partList)
  {
    ...
    if (part == nullptr || part->GetPDGCharge() == 0.0)
    {
      continue;
    }
...
void G4EmBuilder::ConstructLightHadrons(G4ParticleDefinition* part1, G4ParticleDefinition* part2,
                                        G4bool isHEP, G4bool isProton, G4bool isWVI)
{
  ...
  if (isWVI)
  {
    ph->RegisterProcess(ss, part2);
  }
}

void G4EmBuilder::ConstructLightHadronsSS(G4ParticleDefinition* part1, G4ParticleDefinition* part2,
                                          G4bool isHEP)
{
  G4PhysicsListHelper* ph = G4PhysicsListHelper::GetPhysicsListHelper();
...
void G4EmBuilder::ConstructCharged(G4hMultipleScattering* hmsc, G4NuclearStopping* nucStopping,
                                   G4bool isWVI)
{
  G4PhysicsListHelper* ph = G4PhysicsListHelper::GetPhysicsListHelper();
