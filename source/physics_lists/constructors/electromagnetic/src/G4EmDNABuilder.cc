...
namespace {
  ...
  constexpr G4double highEnergyEmfietzoglou = 10*CLHEP::keV;
};
                                           ...
                                           const G4bool)
{
  ...
  if(emin_proton < emax) {
    ...
    StandardHadronPhysics(part, highEnergyElastic, emin_proton, emax,
                          mscType, false);
  }
  ...
  if(emin_ion < emax) {
    ...
    StandardHadronPhysics(ion, 0.0, emin_ion, emax,
                          dnaUrban, true);
    // deuteron
    ion = G4Deuteron::Deuteron();
    StandardHadronPhysics(ion, highEnergyElastic, emin_ion, emax,
                          dnaUrban, false);
    // alpha
    G4ParticleDefinition* part = G4Alpha::Alpha();
    StandardHadronPhysics(part, highEnergyElastic, emin_ion, emax,
                          dnaUrban, true);
                                            ...
                                            const G4Region* reg)
{
  ...
  if (emaxE > 0.0) {
    ...
    modE->SetHighEnergyLimit((opt == 4) ? highEnergyOpt4 : emaxE);
    //
...
void
G4EmDNABuilder::ConstructDNAProtonPhysics(const G4double emaxProtonDNA,
                                          const G4int opt,
                                          const G4bool fast,
                                          const G4bool stationary,
                                          const G4Region* reg)
