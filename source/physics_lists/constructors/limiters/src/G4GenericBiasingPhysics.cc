...
G4GenericBiasingPhysics::G4GenericBiasingPhysics(const G4String& name)
  : G4VPhysicsConstructor(name),
    fPhysBiasAllCharged(false),
    fNonPhysBiasAllCharged(false),
    fPhysBiasAllChargedISL(false),
    fNonPhysBiasAllChargedISL(false),
    fPhysBiasAllNeutral(false),
    fNonPhysBiasAllNeutral(false),
    fPhysBiasAllNeutralISL(false),
    fNonPhysBiasAllNeutralISL(false),
    fVerbose(false)
{
  ;
}
...
//....oooOO0OOooo........oooOO0OOooo........oooOO0OOooo........oooOO0OOooo......
void G4GenericBiasingPhysics::PhysicsBiasAddPDGRange(G4int PDGlow, G4int PDGhigh,
                                                     G4bool includeAntiParticle)
{
  if (PDGlow > PDGhigh)
    G4cout
...
//....oooOO0OOooo........oooOO0OOooo........oooOO0OOooo........oooOO0OOooo......
void G4GenericBiasingPhysics::NonPhysicsBiasAddPDGRange(G4int PDGlow, G4int PDGhigh,
                                                        G4bool includeAntiParticle)
{
  if (PDGlow > PDGhigh)
  {
    G4cout << " G4GenericBiasingPhysics::NonPhysicsBiasAddPDGRange(...) :  PDGlow > PDGhigh, call "
              "ignored."
           << G4endl;
    return;
  }
...
//....oooOO0OOooo........oooOO0OOooo........oooOO0OOooo........oooOO0OOooo......
void G4GenericBiasingPhysics::BiasAddPDGRange(G4int PDGlow, G4int PDGhigh,
                                              G4bool includeAntiParticle)
{
  if (PDGlow > PDGhigh)
  {
    G4cout << " G4GenericBiasingPhysics::BiasAddPDGRange(...) :  PDGlow > PDGhigh, call ignored."
           << G4endl;
    return;
  }
...
void G4GenericBiasingPhysics::AddParallelGeometry(
  const G4String& particleName, const std::vector<G4String>& parallelGeometryNames)
{
  for (G4String geometry : parallelGeometryNames)
    AddParallelGeometry(particleName, geometry);
}

void G4GenericBiasingPhysics::AddParallelGeometry(G4int PDGlow, G4int PDGhigh,
                                                  const G4String& parallelGeometryName,
                                                  G4bool includeAntiParticle)
{
  if (PDGlow > PDGhigh)
  {
    G4cout << "G4GenericBiasingPhysics::AddParallelGeometry( G4int PDGlow, G4int PDGhigh, const "
              "G4String& parallelGeometryName , G4bool includeAntiParticle = true ), PDGlow > "
              "PDGhigh : call ignored"
           << G4endl;
    return;
  }
...
void G4GenericBiasingPhysics::AddParallelGeometry(
  G4int PDGlow, G4int PDGhigh, const std::vector<G4String>& parallelGeometryNames,
  G4bool includeAntiParticle)
{
  if (PDGlow > PDGhigh)
  {
    G4cout << "G4GenericBiasingPhysics::AddParallelGeometry( G4int PDGlow, G4int PDGhigh, const "
              "std::vector< G4String >& parallelGeometryNames, G4bool includeAntiParticle = true "
              "), PDGlow > PDGhigh : call ignored"
           << G4endl;
    return;
  }

  for (G4String geometry : parallelGeometryNames)
    AddParallelGeometry(PDGlow, PDGhigh, geometry, includeAntiParticle);
}

void G4GenericBiasingPhysics::AddParallelGeometryAllCharged(const G4String& parallelGeometryName,
                                                            G4bool includeShortLived)
{
  G4bool isKnown = false;
...
void G4GenericBiasingPhysics::AddParallelGeometryAllCharged(
  const std::vector<G4String>& parallelGeometryNames, G4bool includeShortLived)
{
  for (G4String geometry : parallelGeometryNames)
    AddParallelGeometryAllCharged(geometry, includeShortLived);
}
...
void G4GenericBiasingPhysics::AddParallelGeometryAllNeutral(
  const std::vector<G4String>& parallelGeometryNames, G4bool includeShortLived)
{
  for (G4String geometry : parallelGeometryNames)
    AddParallelGeometryAllNeutral(geometry, includeShortLived);
}
...
void G4GenericBiasingPhysics::ConstructProcess()
{
  ...
  while ((*particleIterator)())
  {
    ...
    if (std::find(fNonPhysBiasedParticles.begin(), fNonPhysBiasedParticles.end(), particleName)
    ...
    std::vector<G4String>::const_iterator particleIt =
      std::find(fBiasedParticles.begin(), fBiasedParticles.end(), particleName);
    if (particleIt == fBiasedParticles.end()) continue;

    std::vector<G4String>& biasedProcesses =
      fBiasedProcesses[particleIt - fBiasedParticles.begin()];
  ...
  while ((*particleIterator)())
  {
    ...
    if (std::find(fBiasedParticles.begin(), fBiasedParticles.end(), particleName)
    ...
    if ((physBias == false) && (nonPhysBias == false))
    {
      ...
      {
        if (fPhysBiasAllCharged)
          if (fPhysBiasAllChargedISL || !particle->IsShortLived()) physBias = true;
        if (fNonPhysBiasAllCharged)
          if (fNonPhysBiasAllChargedISL || !particle->IsShortLived()) nonPhysBias = true;
      }
      else
      {
        if (fPhysBiasAllNeutral)
          if (fPhysBiasAllNeutralISL || !particle->IsShortLived()) physBias = true;
        if (fNonPhysBiasAllNeutral)
          if (fNonPhysBiasAllNeutralISL || !particle->IsShortLived()) nonPhysBias = true;
      }
    }
...
void G4GenericBiasingPhysics::AssociateParallelGeometries()
{
  ...
  while ((*particleIterator)())
  {
    ...
    if (std::find(fParticlesWithParallelGeometries.begin(), fParticlesWithParallelGeometries.end(),
                  particleName)
        != fParticlesWithParallelGeometries.end())
    {
      ...
      for (G4String& world : parallelWorlds)
        limiter->AddParallelWorld(world);
    }
  }
    ...
    while ((*particleIterator)())
    {
      ...
      if ((particlePDG >= PDGlow) && (particlePDG <= PDGhigh))
      {
        ...
        for (auto& geometry : geometries)
          limiter->AddParallelWorld(geometry);
      }
    }
