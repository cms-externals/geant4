...
G4EmMessenger::G4EmMessenger(G4EmExtraPhysics* ab)
{
  ...
  theNuOscDistanceBF = new G4UIcmdWithADouble("/physics_lists/em/NuOscDistanceBias",this);
  ...
  theNuODN = new G4UIcmdWithAString("/physics_lists/em/NuOscDistanceName",this);  
...
void G4EmMessenger::SetNewValue(G4UIcommand* aComm, G4String aS)
{
  ...
  if(aComm==theNuOscDistanceBF)  theB->SetNuOscDistanceBias(theNuOscDistanceBF->GetNewDoubleValue(aS));
  ...
  if(aComm==theNuODN)     theB->SetNuOscDistanceName(aS);
}
