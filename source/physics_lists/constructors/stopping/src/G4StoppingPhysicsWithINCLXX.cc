...
void G4StoppingPhysicsWithINCLXX::ConstructProcess()
{
  ...
  while ((*myParticleIterator)())
  {
        ...
        && !particle->IsShortLived())
    {
      ...
      if (particle == G4AntiNeutron::Definition() || particle == G4AntiLambda::Definition()
          || particle == G4AntiSigmaZero::Definition() || particle == G4AntiSigmaPlus::Definition()
      ...
      else if (particle == G4AntiProton::Definition())
      {
        if (hINCLXXProcess->IsApplicable(*particle))
        {
          ...
          if (verbose > 1)
          {
            G4cout << "### G4HadronicAbsorptionINCLXX added for " << particle->GetParticleName()
                   << G4endl;
          }
        }
