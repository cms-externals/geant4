  ...
  if ( ! pdefaultEngine )
  {
      switch (engineSwitch) {
          ...
          case 6:
              ...
              break;
          default:
              std::cerr<<"Non valid selection for RandomEngine, valid values from 0 (default) to 6"<<std::endl;
              abort();
              break;
      }
  } 
...
G4ThreadFunReturnType mythreadfunc2(G4ThreadFunArgType arg)
{
  ...
  G4Random::setTheSeed( seed );
  char buf[200];
  snprintf(buf,sizeof buf,"%s.%d",STATFILE,int(std::hash<std::thread::id>{}(G4Threading::G4GetPidId())));
