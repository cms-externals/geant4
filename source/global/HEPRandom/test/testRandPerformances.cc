...
// ********************************************************************
//
// Test for RNG algorithms for Geant4
//
// Author: A. Dotti
//
//
...
// 2 min for 3M TRIALS, set a reasonable number.
// Scales linearly - default: 100 millions trials
// 
...
void InitMe( int engineSwitch = 0 )
{
  if ( ! pdefaultEngine )
  {
      switch (engineSwitch) {
          case 0:
              pdefaultEngine = G4Random::getTheEngine();  // MixMax default
              break;
          case 1:
	      ...
	      pdefaultEngine = new CLHEP::RanluxppEngine();
 	      break;
          default:
              std::cerr << "Non valid selection for RandomEngine." << std::endl
                        << "Valid values from 0 (default) to 7" << std::endl;
              abort();
              break;
      }
  } 
  ...
  if ( result == FAIL ) 
    {
      ...
      std::cerr << "Sub-Test: " << t.name << ":" << t.mess << " FAILED"
                << std::endl;
    }
  else 
    {
...
int main(int,char**) 
{
    ...
    int i = 0;
    while (true) {
        if ( tests[i].aTest == nullptr ) break;
    ...
    for ( int t = 0 ; t < max ; ++t ) //Loop on engine types
    {
        ...
        std::cout << "\t================================================="
                  << std::endl;
        std::cout << "\tTesting Random Engine: " << pdefaultEngine->name()
                  << std::endl;
        std::cout << "\t================================================="
                  << std::endl;
        G4Timer someT;
        ...
        someT.Stop();
        std::cout << "\tDone, user elapsed time: " << someT.GetUserElapsed()
                  << " s." << std::endl;
        if (t>0) delete pdefaultEngine;
        pdefaultEngine = nullptr;
    }
