...
#if defined(G4UNIX) && (defined(__GNUC__) || defined(__clang__) || defined(_INTEL_COMPILER))
...
inline G4String G4Demangle(const char* _str)
{
  ...
  if ((_ret != nullptr) && _status == 0) return G4String(const_cast<const char*>(_ret));
  return _str;
#else
  return _str;
#endif
...
class G4Backtrace
{
  ...
  public:
    ...
    struct actions
    {
        ...
        const id_list_t identifiers = {
          ...
          id_entry_t("SIGTTIN", SIGTTIN, "background read attempted from control terminal"),
          id_entry_t("SIGTTOU", SIGTTOU, "background write attempted to control terminal"),
  ...
  public:
    ...
    static std::array<G4ResultOf_t<FuncT, const char*>, Depth>
    GetMangled(FuncT&& func = FrameFunctor());
    ...
    template<std::size_t Depth, std::size_t Offset = 0, typename FuncT = frame_func_t>
    static std::array<G4ResultOf_t<FuncT, const char*>, Depth>
    GetDemangled(FuncT&& func = FrameFunctor());
...
{
  static frame_func_t _instance = [](const char* inp) {
    return G4String(inp);
  };
  return _instance;
}
...
{
  static signal_set_t _instance = {SIGQUIT, SIGILL, SIGABRT, SIGKILL, SIGBUS, SIGSEGV};
  return _instance;
}
...
inline std::array<G4ResultOf_t<FuncT, const char*>, Depth> G4Backtrace::GetMangled(FuncT&& func)
{
...
template<std::size_t Depth, std::size_t Offset, typename FuncT>
inline std::array<G4ResultOf_t<FuncT, const char*>, Depth> G4Backtrace::GetDemangled(FuncT&& func)
{
...
inline void G4Backtrace::Message(G4int sig, siginfo_t* sinfo, std::ostream& os)
{
  ...
  os << "\n### CAUGHT SIGNAL: " << sig << " ### ";
  if (sinfo != nullptr) os << "address: " << sinfo->si_addr << ", ";
  os << Description(sig) << ". ";
  ...
  else if (sig == SIGFPE)
  {
    ...
    else
    {
      ...
      if (sinfo != nullptr) os << ": " << sinfo->si_code;
      os << ". ";
    }
  }
  ...
  char prefix[64];
  snprintf(prefix, 64, "[PID=%i, TID=%i]", (G4int)getpid(), (G4int)G4Threading::G4GetThreadId());
  std::size_t sz = 0;
  for (auto& itr : bt)
  {
  ...
  {
    os << prefix << "[" << std::setw(_w) << std::right << i << '/' << std::setw(_w) << std::right
       << sz << "]> " << std::left << bt.at(i) << '\n';
  }
  os << std::flush;
...
inline G4int G4Backtrace::Enable(const signal_set_t& _signals)
{
  ...
  {
    if (itr < 0) continue;
    if (GetData().is_active[itr]) continue;
    ++cnt;
...
inline G4int G4Backtrace::GetSignal(const std::string& sid)
{
  ...
  {
    if (std::get<0>(itr) == sid) return std::get<1>(itr);
  }
  return -1;
}
...
inline std::string G4Backtrace::Description(G4int sig)
{
  for (auto&& itr : GetData().identifiers)
  {
    ...
    {
      std::stringstream ss;
      ss << " signal = " << std::setw(8) << std::get<0>(itr) << ", value = " << std::setw(4)
         << std::get<1>(itr) << ", description = " << std::get<2>(itr);
      return ss.str();
    }
  }
  std::stringstream ss;
...
class G4Backtrace
{
  ...
  public:
    ...
    static frame_func_t& FrameFunctor()
    {
      static frame_func_t _instance = [](const char* _s) {
        return G4String(_s);
      };
      return _instance;
    }
