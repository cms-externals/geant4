...
G4ThreadFunReturnType myfunc( G4ThreadFunArgType val)
{
  ...
  asc[1]=std::hash<std::thread::id>{}(G4Threading::G4GetPidId())%100000;
  asc[2]=2.2;
  asc[30]=3.3;
  CHECKVAL( asc[1]==std::hash<std::thread::id>{}(G4Threading::G4GetPidId())%100000,"Wrong first element");
...
int main(int,char**)
{
  ...
  for ( G4MapCache<int,double>::const_iterator it = aSharedCache.Begin(); it!=aSharedCache.End(); ++it )
  {
  ...
  cout<<"Final content of cache:"<<endl;
  cout<<vc.Get()<<endl;
