...
void G4PhysicsModelCatalog::Initialize()
{
  ...
  if (theVectorOfModelIDs == nullptr && theVectorOfModelNames == nullptr)
  {
    ...
    // 11.0 to 11.4.
    // Starting with version 11.5, the method "RegisterCustomModel" has been
...
G4bool G4PhysicsModelCatalog::SanityCheck()
{
  ...
  if (theVectorOfModelIDs->size() != theVectorOfModelNames->size())
  {
    ...
    G4Exception("G4PhysicsModelCatalog::SanityCheck()", "PhysModelCatalog001", FatalException, ed,
                "should be the same!");
  }
  else
  {
    ...
    if (isModelIDOutsideRange || isModelIDRepeated || isModelNameRepeated)
    {
      ...
      {
        ed << "theVectorOfModelIDs has NOT all entries between " << GetMinAllowedModelIDValue()
      ...
      G4Exception("G4PhysicsModelCatalog::SanityCheck()", "PhysModelCatalog002", FatalException, ed,
                  "cannot continue!");
    }
...
const G4String G4PhysicsModelCatalog::GetModelNameFromIndex(const G4int modelIndex)
{
  ...
  return (modelIndex >= 0 && modelIndex < Entries()) ? (*theVectorOfModelNames)[modelIndex]
                                                     : G4String("Undefined");
}
...
void G4PhysicsModelCatalog::PrintAllInformation()
{
  ...
  for (G4int idx = 0; idx < Entries(); ++idx)
  {
    G4int modelID = (*theVectorOfModelIDs)[idx];
    G4String modelName = (*theVectorOfModelNames)[idx];
    G4cout << "\t index=" << idx << "\t modelName=" << modelName << "\t modelID=" << modelID
           << G4endl;
  }
  G4cout << G4endl << " ****************************************** " << G4endl
