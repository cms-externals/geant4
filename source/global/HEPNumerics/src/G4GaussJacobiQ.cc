...
G4GaussJacobiQ::G4GaussJacobiQ(function pFunction, G4double alpha, G4double beta, G4int nJacobi)
...

{
  ...
  G4int i = 1, k = 1;
  G4double root = 0.;
  G4double alphaBeta = 0.0, alphaReduced = 0.0, betaReduced = 0.0, root1 = 0.0, root2 = 0.0,
           root3 = 0.0;
  G4double a = 0.0, b = 0.0, c = 0.0, newton1 = 0.0, newton2 = 0.0, newton3 = 0.0, newton0 = 0.0,
  ...
  for (i = 1; i <= nJacobi; ++i)
  {
    if (i == 1)
    {
      ...
      root1 =
        (1.0 + alpha) * (2.78002 / (4.0 + nJacobi * nJacobi) + 0.767999 * alphaReduced / nJacobi);
    ...
    if (k > maxNumber)
    {
      G4Exception("G4GaussJacobiQ::G4GaussJacobiQ()", "OutOfRange", FatalException,
                  "Too many iterations in constructor.");
    }
    fAbscissa[i - 1] = root;
