...
G4VSIntegration::ComputeIntegral(const G4double emin, const G4double  emax)
{
  ...
  for(G4int i=0; i<=nbin; ++i) {
    ...
    res += del;
    // end of the loop 
    if ( (fP2 > 0.0 && del < fAcc*res) || endpoint) { break; }
    problast = y;
...
G4double G4VSIntegration::SampleValue()
{
  ...
  // 2d and 3d areas are considered
  if (fP2 > 0.0 && fE2 < fEmax) {
    Q3 = 2*ProbabilityDensityFunction(fEmax);
    // exclude 3d area from sampling
    if (Q3 > fP2*fFactor1 || Q3 <= 0.0) {
      fE2 = fEmax;
    } else {
      b = G4Log(fP2/Q3)/(fEmax - fE2);
      p3 = (fP2 - Q3)/b;
    }
  }
  ...
  do {
    ...
    if ((gg > gmax || n > nmax) && fVerbose > 1) {
      G4cout << "### G4VSIntegration::SampleValue() for E= " << e 
             << " n= " << n << " gg/gmax=" << gg/gmax << " prob="
	     << gg << " gmax=" << gmax << " pmax=" << fPmax 
             << " Emin=" << fEmin << " Emax=" << fEmax << G4endl;
    }
  } while(gmax*rndm->flat() > gg && n < nmax);
#ifdef G4VERBOSE
  if (fVerbose > 1) {
    G4cout << "### G4VSIntegration::SampleValue: "
           << " E=" << e << " Ntry=" << n
           << " Emin=" << fEmin << " Emax=" << fEmax << G4endl;
  }
#endif
