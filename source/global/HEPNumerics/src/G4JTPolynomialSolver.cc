...
G4int G4JTPolynomialSolver::FindRoots(G4double* op, G4int degr, G4double* zeror, G4double* zeroi)
{
  ...
  do
  {
    ...
    {
      Quadratic(p[0], p[1], p[2], &zeror[degr - 2], &zeroi[degr - 2], &zeror[degr - 1],
                &zeroi[degr - 1]);
      n -= 2;
      return degr - n;
    }
    ...
    for (jj = 0; jj < 5; ++jj)
    {
      ...
      if (zerok == 0)  // Use a scaled form of recurrence if k at 0 is nonzero.
      {
        ...
        k[0] = p[0];
        zerok = static_cast<G4int>(std::fabs(k[n - 1]) <= std::fabs(bb) * eta * 10.0);
      }
...
void G4JTPolynomialSolver::ComputeFixedShiftPolynomial(G4int l2, G4int* nz)
{
  ...
  G4double betas = 0.25, betav = 0.25, oss = sr, ovv = v, ss = 0.0, vv = 0.0, ts = 1.0, tv = 1.0;
...
void G4JTPolynomialSolver::QuadraticPolynomialIteration(G4double* uu, G4double* vv, G4int* nz)
{
...
void G4JTPolynomialSolver::RealPolynomialIteration(G4double* sss, G4int* nz, G4int* iflag)
{
...
void G4JTPolynomialSolver::ComputeNewEstimate(G4int type, G4double* uu, G4double* vv)
{
  // Compute new estimates of the quadratic coefficients
  // using the scalars computed in calcsc.

  G4double a4 = 0.0, a5 = 0.0, b1 = 0.0, b2 = 0.0, c1 = 0.0, c2 = 0.0, c3 = 0.0, c4 = 0.0,
           temp = 0.0;
...
void G4JTPolynomialSolver::QuadraticSyntheticDivision(G4int nn, G4double* uu, G4double* vv,
                                                      std::vector<G4double>& pp,
                                                      std::vector<G4double>& qq, G4double* aa,
                                                      G4double* bb)
{
...
void G4JTPolynomialSolver::Quadratic(G4double aa, G4double b1, G4double cc, G4double* ssr,
                                     G4double* ssi, G4double* lr, G4double* li)
{
