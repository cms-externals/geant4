...
int main()
{
   ...
   G4double test, tolerance, truth =  ExpFunction(x) - 1.0 ;
   for(i=5;i<=n;i++)
   {
      G4ChebyshevApproximation myChebyshev(ExpFunction,a,b,i) ;  // integral
      test = myChebyshev.ChebyshevEvaluation(x) ;
      tolerance = 2*(truth-test)/(truth+test) ;
      G4cout<<"n = "<<i<<"\t"<<"true = "<<truth
   ...
   G4double test, tolerance, truth =  SinFunction(x) ;
   for(i=1+m;i<=n;i++)
   {
   ...
   G4double test, tolerance, truth =  TestFunction(x) ;
   for(i=1;i<n;i++)
   {
