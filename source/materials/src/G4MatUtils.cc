...
G4ExtendedPhysicsVector* G4MatUtils::BuildExtendedVector(const G4String& dirpath,
                                                         ...
                                                         const G4int nxsections, const G4int length,
                                                         const G4double unitE, const G4double unitS)
{
  ...
  if (nxsections < 1 || length <= 0 || nxsections > nmax)
  {
    ...
    ed << " Wrong data size: nsections=" << nxsections << "  length=" << length
       << "  nmax=" << nmax;
    G4Exception("G4MatUtils::BuildExtendedVector(..)", "mat004", FatalException, ed,
                "Check input values");
    return nullptr;
  }
