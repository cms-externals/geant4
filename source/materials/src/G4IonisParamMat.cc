...
void G4IonisParamMat::ComputeFluctModel()
{
  ...
  fF2fluct = (Zeff > 2.001) ? 2.0 / Zeff : 0.0;
  fF1fluct = 1. - fF2fluct;
  fEnergy2fluct = 10. * Zeff * Zeff * CLHEP::eV;

  // the value of fEnergy2fluct = 1.e-5 for Zeff = 1
  // for extreme case of Zeff=0 an arbitrary number DBL_EPSILON is used
  fEnergy2fluct = std::max(fEnergy2fluct, DBL_EPSILON);
