...
G4double G4DensityEffectCalculator::FermiDeltaCalculation(G4double x)
{
  ...
  if (sternrho <= 0. || sternrho > 100.)
  {
    if (fVerbose > 0)
    {
      ...
      {
        G4ExceptionDescription ed;
        ed << "Sternheimer computation failed for " << fMaterial->GetName() << ", x = " << x
           << ":\n"
           ...
           << "Number of levels: " << nlev << " Mean ionization energy(eV): " << meanexcite
           << " Plasma energy(eV): " << plasmaE << "\n";
        for (G4int i = 0; i < nlev; ++i)
        {
          ed << "Level " << i << ": strength " << sternf[i] << ": energy(eV)= " << levE[i] << "\n";
        }
        G4Exception("G4DensityEffectCalculator::SetupFermiDeltaCalc", "mat008", JustWarning, ed);
      }
    }
    return -1.;
  }
...
G4double G4DensityEffectCalculator::Newton(G4double start, G4bool first)
{
  ...
  if (fVerbose > 2)
  {
    G4cout << "G4DensityEffectCalculator::Newton: strat= " << start << " type: " << first << G4endl;
  }
  while (true)
  {
    if (first)
    ...
    if (nbad > maxIter || std::isnan(value) || std::isinf(value))
    {
      break;
    }
  }
  if (fVerbose > 2)
  {
    G4cout << "  Failed to converge last value= " << value << " dvalue= " << dvalue
  ...
  for (G4int i = 0; i < nlev; ++i)
  {
    if (sternf[i] > 0.)
    {
      ans += sternf[i] * gpow->powN(levE[i], 2) * rho
             / (gpow->powN(levE[i] * rho, 2) + 2. / 3. * sternf[i] * gpow->powN(plasmaE, 2));
    }
  }
  return ans;
}
...
G4double G4DensityEffectCalculator::FRho(G4double rho)
{
  ...
  for (G4int i = 0; i < nlev; ++i)
  {
    ...
    {
      ans += sternf[i]
             * G4Log(gpow->powN(levE[i] * rho, 2) + 2. / 3. * sternf[i] * gpow->powN(plasmaE, 2));
    }
  }
...
G4double G4DensityEffectCalculator::DeltaOnceSolved(G4double sternL)
{
  ...
  for (G4int i = 0; i < nlev; ++i)
  {
    ...
    {
      ans += sternf[i]
             * G4Log((gpow->powN(sternl[i], 2) + gpow->powN(sternL, 2)) / gpow->powN(sternl[i], 2));
    }
  }
  ...
  if (fConductivity > 0)
  {
    ans += fConductivity * G4Log((fConductivity + gpow->powN(sternL, 2)) / fConductivity);
  }
