...
G4UIQt::G4UIQt(G4int argc, char** argv)
  : fMainWindow(nullptr),
    ...
    fSceneTreeItemTreeWidget(nullptr),
...
void G4UIQt::SetDefaultIconsToolbar()
{
  if (fDefaultIcons) {
    ...
    QAction* actionViewerProperties = fToolbarApp->addAction(
      QIcon(*fParamIcon), "Viewer properties", signalMapperViewerProperties, SLOT(map()));
    connect(signalMapperViewerProperties, SIGNAL(mapped(G4int)), this,
      SLOT(ViewerPropertiesIconCallback(int)));
    G4int intVP = 0;
 ...
 */
void G4UIQt::CreateSceneTreeComponent()
{
  auto vLayout = fNewSceneTreeWidget->layout();
  // reduce margins
  vLayout->setContentsMargins(0,0,0,0);

  fSceneTreeItemTreeWidget = new QTreeWidget();
  fSceneTreeItemTreeWidget->setSelectionMode(QAbstractItemView::SingleSelection);
  vLayout->addWidget(fSceneTreeItemTreeWidget);

  connect(fSceneTreeItemTreeWidget, &QTreeWidget::itemClicked,
...
void G4UIQt::SceneTreeItemClicked(QTreeWidgetItem* item)
{
  ...
  switch (sceneTreeItem->GetType()) {
    ...
    case G4SceneTreeItem::pvmodel:
      ...
      if (item->checkState(0) == Qt::Checked) argument = "true";
      uiMan->ApplyCommand("/vis/scene/activateModel " + itemText + ' ' + argument);
      break;
    case G4SceneTreeItem::ghost:
      [[fallthrough]];
    case G4SceneTreeItem::touchable:
      // Construct and apply touchable commands
      G4String setTouchable = "/vis/set/touchable " + sceneTreeItem->GetPVPath();
      if (item->checkState(0) == Qt::Checked) argument = "true";
      G4String touchableSet = "/vis/touchable/set/visibility " + argument;
      uiMan->ApplyCommand(setTouchable);
      uiMan->ApplyCommand(touchableSet);
      break;
  }
}
...
void G4UIQt::UpdateSceneTree(const G4SceneTreeItem& root)
{
  ...
  fSceneTreeItemTreeWidget->clear();
  // (I think this deletes everything - the top level items and their children.)

  // Build a new GUI-side tree
  fSceneTreeItemTreeWidget->setHeaderLabel (root.GetDescription().c_str());
  for (const auto& model : root.GetChildren()) {
    QStringList qStringList;
    qStringList.append(model.GetModelType().c_str());
    auto item = new QTreeWidgetItem(fSceneTreeItemTreeWidget,qStringList);
    Qt::CheckState checkState = Qt::Unchecked;
    if (model.GetVisAttributes().IsVisible()) checkState = Qt::Checked;
    ...
    fSceneTreeItemTreeWidget->addTopLevelItem(item);
...
void G4UIQt::BuildPVQTree(const G4SceneTreeItem& g4stItem, QTreeWidgetItem* qtwItem)
{
  ...
  for (const auto& g4stChild: g4stChildren) {
    ...
    Qt::CheckState checkState = Qt::Unchecked;
    if (g4stChild.GetVisAttributes().IsVisible()) checkState = Qt::Checked;
    newQTWItem->setCheckState(0, checkState);
...
#    if QT_VERSION >= 0x050000
  QString message =
    QString("This Qt version [") + qVersion() + "] has some issues with the OpenGL viewer.\n" +
    "To prevent problems, you are not allowed to open a Stored nor Immediate viewer.\n" + "\n" +
    "Please upgrade to Qt version >= 5.1\n";

  QMessageBox::warning(
    fMainWindow, tr("Warning"), tr(message.toStdString().c_str()), QMessageBox::Ok);
  return false;
#    endif
#  endif
#endif
  ...
  const char* aLabel, const char* aIconFile, const char* aCommand, const char* aFileName)
{
...
#if QT_VERSION < 0x050600
    connect(
      signalMapper, SIGNAL(mapped(const QString&)), this, SLOT(OpenIconCallback(const QString&)));
...
#if QT_VERSION < 0x050600
    connect(
      signalMapper, SIGNAL(mapped(const QString&)), this, SLOT(SaveIconCallback(const QString&)));
    signalMapper->setMapping(action, QString(txt));
#else
    currentToolbar->addAction(
...
#if QT_VERSION < 0x050600
    connect(
      signalMapper, SIGNAL(mapped(const QString&)), this, SLOT(ChangeCursorAction(const QString&)));
           ...
           (std::string(aIconFile) == "solid") || (std::string(aIconFile) == "wireframe"))
  {
#if QT_VERSION < 0x050600
    connect(
      signalMapper, SIGNAL(mapped(const QString&)), this, SLOT(ChangeSurfaceStyle(const QString&)));
    signalMapper->setMapping(action, QString(aIconFile));
#else
    QString txt = QString(aIconFile);
    QAction* action = currentToolbar->addAction(
    ...
    connect(signalMapper, SIGNAL(mapped(const QString&)), this,
      SLOT(ChangePerspectiveOrtho(const QString&)));
    signalMapper->setMapping(action, QString(aIconFile));
#else
    QString txt = QString(aIconFile);
    QAction* action = currentToolbar->addAction(
...
#if QT_VERSION < 0x050600
    connect(
      signalMapper, SIGNAL(mapped(const QString&)), this, SLOT(ButtonCallback(const QString&)));
    signalMapper->setMapping(action, QString(aCommand));
#else
    ...
    for (G4int i_thParameter = 0; i_thParameter < n_parameterEntry; i_thParameter++) {
      ...
      if (isStillColorParameter && (nbColorParameter != 0)) {
        if ((label->text() == "red") || (label->text() == "red_or_string")) {
          ...
          connect(
            signalMapper, SIGNAL(mapped(QWidget*)), this, SLOT(ChangeColorCallback(QWidget*)));
#else
