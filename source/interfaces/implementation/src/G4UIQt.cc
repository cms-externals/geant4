  ...
  switch (sceneTreeItem->GetType()) {
    ...
    case G4SceneTreeItem::pvmodel:
      ...
      if (newCheckState != oldCheckState) {
        ...
        // activateModel searches for a match - it's best to ignore spaces, if any
        const G4String& md = sceneTreeItem->GetModelDescription();
        G4String matchString = md.substr(0, md.find_first_of(' '));
        uiMan->ApplyCommand("/vis/scene/activateModel " + matchString + ' ' + argument);
      }
      break;
          ...
          for (const auto& action : alphabetical) {
            ...
            if (action.second == withoutParameter) {
              ...
              connect(qAction, &QAction::triggered, this,
                      [=]{ActWithoutParameter(af, sceneTreeItem);});
              ...
              connect(qActionTrue, &QAction::triggered, this,
                      [=]{ActWithABool(af, sceneTreeItem, true);});
              ...
              connect(qActionFalse, &QAction::triggered, this,
                      [=]{ActWithABool(af, sceneTreeItem, false);});
              ...
              topMenu.addAction(qAction);
              connect(qAction, &QAction::triggered, this,
                      [=]{ActWithAnInteger(af, sceneTreeItem);});
              ...
              topMenu.addAction(qAction);
              connect(qAction, &QAction::triggered, this,
                      [=]{ActWithADouble(af, sceneTreeItem);});
              ...
              topMenu.addAction(qAction);
              connect(qAction, &QAction::triggered, this,
                      [=]{ActWithAString(af, sceneTreeItem);});
            }
          }

#if (QT_VERSION < QT_VERSION_CHECK(6, 0, 0))
          topMenu.exec(ev->globalPos());
#else
          topMenu.exec(ev->globalPosition().toPoint());
#endif
