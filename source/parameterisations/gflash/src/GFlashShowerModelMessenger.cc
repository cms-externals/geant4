...
G4String GFlashShowerModelMessenger::GetCurrentValue(G4UIcommand* command)
{
  ...
  if (command == FlagCmd) {
    ...
    os << "/GFlash/Ekill " << myModel->GetPBound()->GetEneToKill(*G4Electron::ElectronDefinition()) / GeV
       << " GeV" << '\0';
    returnValue = G4String(os.str());
  }
    ...
    os << "/GFlash/Emin "
       << myModel->GetPBound()->GetMinEneToParametrise(*G4Electron::ElectronDefinition()) / GeV << " GeV"
       << '\0';
    returnValue = G4String(os.str());
  }
    ...
    os << "/GFlash/Emax "
       << myModel->GetPBound()->GetMaxEneToParametrise(*G4Electron::ElectronDefinition()) / GeV << " GeV"
       << '\0';
    returnValue = G4String(os.str());
  }
