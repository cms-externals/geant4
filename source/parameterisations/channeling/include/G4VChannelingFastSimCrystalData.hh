...
#ifndef G4VCHANNELINGFASTSIMCRYSTALDATA_HH
#define G4VCHANNELINGFASTSIMCRYSTALDATA_HH
...
class G4VChannelingFastSimCrystalData
{
  public:
    ...
    G4double ElectronDensity(G4double x, G4double y)
    {
      ...
      {
        nel0 = 0.;
      }  // exception, errors of interpolation functions
      return nel0;
    }
    ...
    ///(to do only once)
    virtual void SetMaterialProperties(const G4Material* crystal, const G4String& lattice,
    ...
    void SetCrystallineUndulatorParameters(G4double amplitude, G4double period, G4double phase,
    ...
    void SetParticleProperties(G4double etotal, G4double mp, G4double charge,
    ...
    G4ThreeVector CoulombElectronScattering(G4double eMinIonization, G4double electronDensity,
                                            G4double step);
    /// ionization losses
  ...
  private:
    ...
    std::unordered_map<G4int, G4ThreeVector>
      fMapCUAmplitudePeriodPhase;  // the map of
