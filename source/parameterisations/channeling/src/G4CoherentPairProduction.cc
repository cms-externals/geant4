...
G4CoherentPairProduction::G4CoherentPairProduction(const G4String& aName, G4ProcessType)
...
G4double G4CoherentPairProduction::GetMeanFreePath(const G4Track& aTrack, G4double,
  ...
  if (eGamma > ModelMinPrimaryEnergy())
  {
    ...
    if (crystallogic->GetRegion()->GetName() == fG4RegionName)
    {
      ...
      momentumDirectionGamma =
        (aTrack.GetTouchableHandle()->GetHistory()->GetTopTransform().NetRotation().inverse())
        * aTrack.GetMomentumDirection();

      // the coordinates of the photon in the local reference system of the volume
      xyzGamma0 = aTrack.GetTouchableHandle()->GetHistory()->GetTopTransform().TransformPoint(
        aTrack.GetPosition());
      ...
      modelTrigger = (momentumDirectionGamma.z() > 0. && std::abs(angle) < GetHighAngleLimit());
    }
  }
  ...
  if (modelTrigger)
  {
    ...
    for (G4int i = 0; i < fNMCPairs; i++)
    {
      ...
      for (G4int j = 0; j < 2; j++)
      {
        ...
        fCrystalData->SetParticleProperties(etotal, fMass, charge[j], particleName[j]);
        ...
        G4double momentumDirectionZGamma =
          1. / std::sqrt(1. + std::pow(std::tan(tx), 2) + std::pow(std::tan(ty), 2));
        ...
        if (axangle > fCrystalData->GetLindhardAngle() + DBL_EPSILON)
        {
          paramParticleAngle +=
            ...
            - std::sqrt(axangle * axangle
                        - fCrystalData->GetLindhardAngle() * fCrystalData->GetLindhardAngle());
        }
        else
        {
        ...
        if (paramParticleAngle > CLHEP::halfpi - DBL_EPSILON)
        {
          paramParticleAngle = CLHEP::halfpi;
        }
        ...
        G4double rhocut = CLHEP::halfpi / paramParticleAngle;  // radial angular cut of
                                                               // the distribution
        G4double norm =
          std::atan(rhocut * rhocut) * CLHEP::pi * paramParticleAngle * paramParticleAngle;
        ...
        for (G4int k = 0; k < fNTrajectorySteps; k++)
        {
          ...
          tx3 =
            tx
            + (kvx1 - kvx2 + kvx3 - fCrystalData->GetCurv(z) * fCrystalData->GetCorrectionZ()) * dz;
          if (fCrystalData->GetModel() == 2)
          {
          ...
          if (fIncoherentScattering)
          {
            ...
            scatteringAnglesAndEnergyLoss += fCrystalData->CoulombElectronScattering(
              fCrystalData->MinIonizationEnergy(x, y), fCrystalData->ElectronDensity(x, y),
              momentumDirectionStep);
              ...
              || std::abs(ty) > CLHEP::halfpi - DBL_EPSILON)
          {
            G4cout << "Warning: particle angle is beyond +-pi/2 range => "
                      "skipping the calculation of its probability"
                   << G4endl;
            fbreak = true;
            break;
          }
          ...
          skIx = vxin / faseAfter - vxno / faseBefore
                 + dzmod * (axt / faseBefore - vxno * fa2dfaseBefore2);
          skIy = vyin / faseAfter - vyno / faseBefore
                 + dzmod * (ayt / faseBefore - vyno * fa2dfaseBefore2);
                                                          ...
                                                          const G4Step& aStep)
{
  ...
  const G4ParticleDefinition* chargedParticleDefinition[2] = {G4Electron::Electron(),
                                                              G4Positron::Positron()};

  // the coordinates of the photon in the local reference system of the volume
  G4ThreeVector xyzGamma0 =
    aTrack.GetTouchableHandle()->GetHistory()->GetTopTransform().TransformPoint(
      aTrack.GetPosition());
  ...
  for (G4int j = 0; j < 2; j++)
  {
    ...
    newParticleCoordinateXYZ =
      aTrack.GetTouchableHandle()->GetHistory()->GetTopTransform().Inverse().TransformPoint(
        newParticleCoordinateXYZ);
    ...
    // in the local reference system of the volume
    G4ThreeVector momentumDirectionParticle = G4ThreeVector(
      momentumDirectionZ * std::tan(tx0), momentumDirectionZ * std::tan(ty0), momentumDirectionZ);
    // the same in the global reference system
    momentumDirectionParticle =
      (aTrack.GetTouchableHandle()->GetHistory()->GetTopTransform().NetRotation())
      * momentumDirectionParticle;

    G4DynamicParticle* chargedParticle = new G4DynamicParticle(
      chargedParticleDefinition[j], momentumDirectionParticle, fullVectorEtotal[ipair][j] - fMass);
