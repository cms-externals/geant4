...
void G4VChannelingFastSimCrystalData::SetGeometryParameters(const G4LogicalVolume* crystallogic)
{
  ...
  (fMapMiscutAngle.count(crystalID) > 0) ? SetMiscutAngle(fMapMiscutAngle[crystalID], crystallogic)
...
void G4VChannelingFastSimCrystalData::SetCrystallineUndulatorParameters(
  G4double amplitude, G4double period, G4double phase, const G4LogicalVolume* crystallogic)
{
  if (amplitude < DBL_EPSILON || period < DBL_EPSILON)
  {
    ...
    G4cout << "Warning: The crystalline undulator parameters are out of range "
              "=> the crystalline undulator mode switched off"
           << G4endl;
  }
...
void G4VChannelingFastSimCrystalData::SetCrystallineUndulatorParameters(
  const G4LogicalVolume* crystallogic, const G4String& filename)
{
  G4int crystalID = crystallogic->GetInstanceID();
  fMapImportCrystalGeometry[crystalID] = true;

  G4PhysicsLinearVector vec(
    true);  // CU geometry data x=f(z) - transverse coordinate vs longitudinal
  ...
  if (!vfilein)
  {
    G4String message = "Input file " + filename + " is not found!";
  ...
  G4cout << "Importing the Crystal geometry from the file: " << filename << G4endl;
  ...
  {
    if (std::abs((vec.Energy(i + 1) - vec.Energy(i)) - (vec.Energy(i) - vec.Energy(i - 1)))
        > 100000000. * DBL_EPSILON)  // single precision
    {
                                                      ...
                                                      const G4LogicalVolume* crystallogic)
{
  ...
  fImportCrystalGeometry =
    (fMapImportCrystalGeometry.count(crystalID) > 0) ? fMapImportCrystalGeometry[crystalID] : false;
  ...
  if (fCUAmplitude > DBL_EPSILON && period > DBL_EPSILON)
  {
    ...
    if (fBendingAngle > DBL_EPSILON)
    {
      ...
      G4cout << "Warning: crystalline undulator is not compatible with "
                "a bent crystal mode => setting bending angle to 0."
             << G4endl;
    }
...
void G4VChannelingFastSimCrystalData::SetParticleProperties(G4double etotal, G4double mass,
                                                            G4double charge,
                                                            const G4String& particleName)
{
  ...
  fTmax =
    fMe2Gamma * fGamma * fV2
    / (CLHEP::electron_mass_c2 / mass * CLHEP::electron_mass_c2 / mass + 1. + fMe2Gamma / mass);
                                                           ...
                                                           G4double charge)
{
  ...
  return std::sqrt(2 * std::abs(charge) * fVmax
                   / pv0);  // Calculate the value of the Lindhard angle
...
G4double G4VChannelingFastSimCrystalData::GetMaxSimulationStep(G4double etotal, G4double mass,
                                                               G4double charge)
{
                                                                       ...
                                                                       G4int ielement)
{
  ...
  else
  {
    ...
    G4double tetamsi = e1
                       * (G4Log(aa1) + (1. - std::exp(-aa * fBB[ielement])) / aa1
                          + fBBDEXP[ielement] * (expint(fBB[ielement] * aa1) - fE1XBbb[ielement]));
                                                                         ...
                                                                         G4double step)
{
  ...
  if (eMinIonization < 0.5 * CLHEP::eV)
  {
    eMinIonization = 0.5 * CLHEP::eV;
  }
...
G4double G4VChannelingFastSimCrystalData::IonizationLosses(G4double dz, G4int ielement)
{
  ...
  if (fParticleName == "e-")
  {
    loge += (-G4Log(2.) + 1. - (2. * fGamma - 1.) / fGamma / fGamma * G4Log(2.)
             + 1. / 8. * ((fGamma - 1.) / fGamma) * ((fGamma - 1.) / fGamma));
  }
  else if (fParticleName == "e+")
  {
    loge += (-fV2 / 12.
             * (11. + 14. / (fGamma + 1.) + 10. / (fGamma + 1.) / (fGamma + 1.)
...
G4double G4VChannelingFastSimCrystalData::expint(G4double X)
{
  ...
  else if (X <= 1.)
  {
    ...
    for (int K = 1; K <= 25; K++)
    {
      ...
      E1 = E1 + R;
      if (std::abs(R) <= std::abs(E1) * 1.0e-15)
      {
        break;
      }
    }
