...
G4bool G4ChannelingFastSimModel::ModelTrigger(const G4FastTrack& fastTrack)
{
  ...
  if (ekinetic > GetLowKineticEnergyLimit(particleDefinitionID))
  {
    ...
    angle = fCrystalData->AngleXFromBoxToLattice(
      angle,
      (fCrystalData->CoordinatesFromBoxToLattice(fastTrack.GetPrimaryTrackLocalPosition())).z());
    if (fCrystalData->GetModel() == 2)
    {
      angle = std::sqrt(angle * angle
                        + std::pow(std::atan(momentumDirection.y() / momentumDirection.z()), 2));
    }
    ...
    modelTrigger =
      (crystallogic->GetSolid()->Inside(xyz0) == kInside) && momentumDirection.z() > 0.
      && std::abs(angle) < std::max(GetLindhardAngleNumberHighLimit(particleDefinitionID)
                                      * fCrystalData->GetLindhardAngle(etotal, mass, charge),
...
void G4ChannelingFastSimModel::DoIt(const G4FastTrack& fastTrack, G4FastStep& fastStep)
{
  ...
  lindhardAngleNumberHighLimit0 = GetLindhardAngleNumberHighLimit(
    fastTrack.GetPrimaryTrack()->GetParticleDefinition()->GetParticleDefinitionID());
  highAngleLimit0 = GetHighAngleLimit(
    fastTrack.GetPrimaryTrack()->GetParticleDefinition()->GetParticleDefinitionID());
  ...
  do
  {
    ...
    x3 = x + (tx - tx1 + tx2) * dz;
    tx3 =
      tx + (kvx1 - kvx2 + kvx3 - fCrystalData->GetCurv(z) * fCrystalData->GetCorrectionZ()) * dz;
    if (fCrystalData->GetModel() == 2)
    {
    ...
    for (G4int i = 0; i < fCrystalData->GetNelements(); i++)
    {
      ...
      scatteringAnglesAndEnergyLoss +=
        fCrystalData->CoulombAtomicScattering(effectiveStep, momentumDirectionStep, i);
    ...
    {
      // if low energy
      if (etotal - mass <= GetLowKineticEnergyLimit(
            fastTrack.GetPrimaryTrack()->GetParticleDefinition()->GetParticleDefinitionID()))
      {
        inCrystal = false;
      }  // escape the cycle
      ...
      else if (fCrystalData->GetModel() == 1)  // 1D model, field of planes
      {
        ...
        if (std::abs(tx) >= std::max(
              lindhardAngleNumberHighLimit0 * fCrystalData->GetLindhardAngle(), highAngleLimit0))
        {
          inCrystal = false;
        }  // escape the cycle
      }
      else if (fCrystalData->GetModel() == 2)  // 2D model, field of axes
      {
        ...
        if (std::sqrt(tx * tx + ty * ty) >= std::max(
              lindhardAngleNumberHighLimit0 * fCrystalData->GetLindhardAngle(), highAngleLimit0))
        {
          inCrystal = false;
        }  // escape the cycle
      ...
      if (fRad)
      {
        ...
        if (fBaierKatkov->DoRadiation(
              etotal, mass, tx0, ty0, scatteringAnglesAndEnergyLoss.x(),
              scatteringAnglesAndEnergyLoss.y(), momentumDirectionStep, tGlobal, xyz0,
              crystallogic->GetSolid()->Inside(xyz0) != kInside && inCrystal))
      ...
      else
      {
        ...
        {
          G4cout << "Warning in G4ChannelingFastSimModel: " << ekinetic << "<" << 1 * CLHEP::keV
                 << " !" << G4endl;
          eDeposited -= (1 * CLHEP::keV - ekinetic);
          ekinetic = 1 * CLHEP::keV;
          G4cout << "Setting deposited energy=" << eDeposited << " & ekinetic=" << ekinetic
                 << G4endl;
      ...
      if (crystallogic->GetSolid()->Inside(xyz0) != kInside)
      {
    ...
    else
    {
      ...
      else if (crystallogic->GetSolid()->Inside(xyz0) == kOutside)
      {
        inCrystal = false;
      }  // escape the cycle
    }
  ...
  if (ekinetic < 1 * CLHEP::keV)
  {
    ...
    eDeposited -= (1 * CLHEP::keV - ekinetic);
    ekinetic = 1 * CLHEP::keV;
    G4cout << "Setting deposited energy=" << eDeposited << " & ekinetic=" << ekinetic << G4endl;
  }
  ...
  fastStep.ProposePrimaryTrackFinalMomentumDirection(G4ThreeVector(
