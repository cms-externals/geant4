...
G4ChannelingFastSimInterpolation::G4ChannelingFastSimInterpolation(G4double dx0, G4double dy0,
                                                                   G4int nPointsx0, G4int nPointsy0,
                                                                   G4int iModel0)
{
  fDx = dx0;
  fDy = dy0;
...
void G4ChannelingFastSimInterpolation::SetCoefficients1D(G4double AI0, G4double BI0, G4double CI0,
                                                         G4double DI0, G4int i)
{
  fAI[i] = AI0;
  fBI[i] = BI0 / CLHEP::cm;
  fCI[i] = CI0 / CLHEP::cm2;
...
void G4ChannelingFastSimInterpolation::SetCoefficients2D(G4double AI3D0, G4double BI3D0,
                                                         G4double CI3D0, G4int i, G4int j, G4int k)
{
  if (k == 0)
  {
...
G4double G4ChannelingFastSimInterpolation::Spline2D(G4double xx,
                                                    G4double yy)  // cubic
                                                                  // spline of 2-variable function
{
  ...
  G4double spl3d1 = fCI3D[mmx][mmy] * tt2y3 + fCI3D[mmx][mmy + 1] * tt1y3 + fAI3D[mmx][mmy] * tt2y
                    + fBI3D[mmx][mmy] * tt1y;
  G4double spl3d2 = fCI3D[mmx + 1][mmy] * tt2y3 + fCI3D[mmx + 1][mmy + 1] * tt1y3
                    + fAI3D[mmx + 1][mmy] * tt2y + fBI3D[mmx + 1][mmy] * tt1y;

  G4double spl3d = spl3dxx1 * tt23 + spl3dxx2 * tt13 + (spl3d1 * 6. - spl3dxx1 * fStepi2) * tt2
                   + (spl3d2 * 6. - spl3dxx2 * fStepi2) * tt1;
