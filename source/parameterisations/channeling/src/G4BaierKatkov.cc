...
void G4BaierKatkov::SetSpectrumEnergyRange(G4double emin, G4double emax, G4int numberOfBins)
{
  ...
  for (G4int j = 0; j < fNBinsSpectrum; j++)
  {
    ...
    fPhotonEnergyInSpectrum.push_back(fMinPhotonEnergy
                                      * (std::exp(fLogEmaxdEmin * j / fNBinsSpectrum)
                                         + std::exp(fLogEmaxdEmin * (j + 1) / fNBinsSpectrum))
                                      / 2.);
  }
...
void G4BaierKatkov::AddStatisticsInPhotonEnergyRegion(G4double emin, G4double emax,
                                                      G4int timesPhotonStatistics)
{
  if (timesPhotonStatistics <= 1)
  {
    G4cout << "G4BaierKatkov model, "
           ...
           << emin / CLHEP::MeV << " MeV, " << emax / CLHEP::MeV << " MeV, "
  ...
  else
  {
    ...
    if (setrange)
    {
      ...
      G4cout << "G4BaierKatkov model: increasing the statistics of photon sampling "
             ...
             << timesPhotonStatistics << G4endl;
      G4cout << "in the energy spectrum range: (" << emin / CLHEP::MeV << " MeV, "
...
void G4BaierKatkov::SetPhotonSamplingParameters(G4double ekin, G4double minPhotonAngleX,
                                                G4double maxPhotonAngleX, G4double minPhotonAngleY,
...
void G4BaierKatkov::GeneratePhotonSampling()
{
  ...
  for (G4int j = 0; j < fNMCPhotons; j++)
  {
    ...
    fIBinsSpectrum.push_back((G4int)std::trunc(ksi * fNBinsSpectrum / fLogEmaxdEmin));
    ...
    {
      fNPhotonsPerBin[fIBinsSpectrum[j]] += 1;
    }
    ...
    {
      if (ksi > fLogAddRangeEmindEmin[j2] && ksi < fLogAddRangeEmaxdEmin[j2])
      {
  ...
  for (G4int j2 = 0; j2 < nAddRange; j2++)
  {
    ...
    for (G4int j = moreStatistics[j2]; j < totalAddRangeStatistics; j++)
    {
      ksi = fLogAddRangeEmindEmin[j2]
            + G4UniformRand()
                * (std::min(fLogAddRangeEmaxdEmin[j2], fLogEdEmin) - fLogAddRangeEmindEmin[j2]);
      fIBinsSpectrum.push_back((G4int)std::trunc(ksi * fNBinsSpectrum / fLogEmaxdEmin));
      /*   //we consider also the energy outside the spectrum  output range
         //(E>Emax => fLogEdEmin>fLogEmaxdEmin)
         //in this case we don't count the photon in the spectrum output
  ...
  G4double norm = std::atan(rhocut * rhocut) * CLHEP::pi * fParamPhotonAngleX * fParamPhotonAngleY;
  ...
  for (G4int j = 0; j < nmctotal; j++)
  {
    ...
    fPhotonAngleInIntegralX.push_back(fMeanPhotonAngleX
                                      + fParamPhotonAngleX * rho * std::cos(CLHEP::twopi * ksi));
    fPhotonAngleInIntegralY.push_back(fMeanPhotonAngleY
                                      + fParamPhotonAngleY * rho * std::sin(CLHEP::twopi * ksi));
    ...
    if (fVirtualCollimatorTypeID == 1)  // round or ellipse collimator
    {
      fInsideVirtualCollimator.push_back(
        1. > std::sqrt((fPhotonAngleInIntegralX[j] - fVirtualCollimatorAngularCenterX)
    ...
    {
      fInsideVirtualCollimator.push_back(true);
    }  // default - infinite collimator
  }
...
G4double G4BaierKatkov::RadIntegral(G4double etotal, G4double mass,
                                    ...
                                    std::vector<G4double>& vectorSteps, G4int imin)
{
  ...
  for (std::size_t k = kmin; k < nparts; k++)
  {
    ...
    // accelerations
    axt[k] = (vectorParticleAnglesX[k] - vectorScatteringAnglesX[k] - vectorParticleAnglesX[k - 1])
             / dz[k];
    ayt[k] = (vectorParticleAnglesY[k] - vectorScatteringAnglesY[k] - vectorParticleAnglesY[k - 1])
             / dz[k];
  }
  ...
  // the final number of photons may be different from fNMCPhotons
  {
    ...
    for (std::size_t k = kmin; k < nparts; k++)
    {
      ...
      skIx = vxin / faseAfter - vxno / faseBefore
             + dzmod * (axt[k] / faseBefore - vxno * fa2dfaseBefore2);
      skIy = vyin / faseAfter - vyno / faseBefore
             + dzmod * (ayt[k] / faseBefore - vyno * fa2dfaseBefore2);
    ...
    {
      fSpectrum[fIBinsSpectrum[j]] += totalRadiationProbabilityPhj / (om * coefNormLogdNMC);
    }
...
G4bool G4BaierKatkov::SetPhotonProductionParameters(G4double etotal, G4double mass)
{
  ...
  if (ksi < fTotalRadiationProbabilityAlongTrajectory.back())  // photon produced
  {
    ...
    G4double momentumDirectionZ =
      1.
      / std::sqrt(1. + std::pow(std::tan(photonAngleX), 2) + std::pow(std::tan(photonAngleY), 2));

    // momentum direction vector of the photon produced
    PhMomentumDirection.set(momentumDirectionZ * std::tan(photonAngleX),
                            momentumDirectionZ * std::tan(photonAngleY), momentumDirectionZ);
    ...
    auto it = std::find_if(fTotalRadiationProbabilityAlongTrajectory.begin(),
                           fTotalRadiationProbabilityAlongTrajectory.end(), [&](G4double value) {
...
G4bool G4BaierKatkov::DoRadiation(G4double etotal, G4double mass, G4double angleX, G4double angleY,
                                  G4double angleScatteringX, G4double angleScatteringY,
                                  G4double step, G4double globalTime, G4ThreeVector coordinateXYZ,
  ...
  */
  ...
  if ((imax == fImin0 + fNSmallTrajectorySteps) || flagEndTrajectory)
  {
    ...
    if (fImin0 == 0)
    {
      ...
      SetPhotonSamplingParameters(
        etotal - mass,
        *std::min_element(fParticleAnglesX.begin(), fParticleAnglesX.end()) - radiationAngleLimit,
        *std::max_element(fParticleAnglesX.begin(), fParticleAnglesX.end()) + radiationAngleLimit,
        *std::min_element(fParticleAnglesY.begin(), fParticleAnglesY.end()) - radiationAngleLimit,
        *std::max_element(fParticleAnglesY.begin(), fParticleAnglesY.end()) + radiationAngleLimit);
    ...
    fTotalRadiationProbability =
      RadIntegral(etotal, mass, fParticleAnglesX, fParticleAnglesY, fScatteringAnglesX,
                  fScatteringAnglesY, fSteps, fImin0);
    ...
    if (fTotalRadiationProbability > fSinglePhotonRadiationProbabilityLimit || flagEndTrajectory)
    {
...
void G4BaierKatkov::GeneratePhoton(G4FastStep& fastStep)
{
  const G4DynamicParticle theGamma =
    G4DynamicParticle(G4Gamma::Gamma(), PhMomentumDirection, fEph0);
