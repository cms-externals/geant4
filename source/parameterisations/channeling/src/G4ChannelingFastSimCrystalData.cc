                                                           ...
                                                           const G4String& filePath)
{
  ...
  {
    G4cout << "=======================================================================" << G4endl;
    G4cout << "======                 Crystal lattice data                    ========" << G4endl;
    G4cout << "=======================================================================" << G4endl;
  ...
  if (lattice.compare(0, 1, "(") == 0)
  {
    ...
    if (fVerbosity) G4cout << "Crystal planes: " << lattice << G4endl;
  }
  else if (lattice.compare(0, 1, "<") == 0)
  {
    iModel = 2;  // axes
    filename = filename + "_axes_";  // temporary name
    if (fVerbosity) G4cout << "Crystal axes: " << lattice << G4endl;
  }
  ...
  if (!vfilein.is_open())
  {
    G4String outputMessage = "Input file " + filename + " is not found!";
    G4Exception("SetMaterialProperties", "001", FatalException, outputMessage);
  }
  ...
  fElectricFieldX = new G4ChannelingFastSimInterpolation(fDx, fDy, fNpointsx, fNpointsy, iModel);
  if (iModel == 2)
  {
    fElectricFieldY = new G4ChannelingFastSimInterpolation(fDx, fDy, fNpointsx, fNpointsy, iModel);
  }
  fElectronDensity = new G4ChannelingFastSimInterpolation(fDx, fDy, fNpointsx, fNpointsy, iModel);
  ...
  if (iModel == 1)
  {
    ...
    for (G4int i = 0; i < fNpointsx; i++)
    {
      ...
      fElectricFieldX->SetCoefficients1D(ai * unitIF, bi * unitIF, ci * unitIF, di * unitIF, i);

      // reading the coefficients of cubic spline
      vfilein >> ai >> bi >> ci >> di;
      // setting spline coefficients for nuclear density (first element)
      unitIF = 1.;
      fNucleiDensity[0]->SetCoefficients1D(ai * unitIF, bi * unitIF, ci * unitIF, di * unitIF, i);

      // reading the coefficients of cubic spline
      vfilein >> ai >> bi >> ci >> di;
      // setting spline coefficients for electron density
      unitIF = 1. / CLHEP::cm3;
      fElectronDensity->SetCoefficients1D(ai * unitIF, bi * unitIF, ci * unitIF, di * unitIF, i);

      // reading the coefficients of cubic spline
      vfilein >> ai >> bi >> ci >> di;
      // setting spline coefficients for minimal ionization energy
      unitIF = CLHEP::eV;
      fMinIonizationEnergy->SetCoefficients1D(ai * unitIF, bi * unitIF, ci * unitIF, di * unitIF,
                                              i);
      ...
      for (G4int ii = 1; ii < fNelements; ii++)
      {
        ...
        unitIF = 1.;
        fNucleiDensity[ii]->SetCoefficients1D(ai * unitIF, bi * unitIF, ci * unitIF, di * unitIF,
                                              i);
      }
    }
  }
    ...
    for (G4int j = 0; j < fNpointsy; j++)
    {
      for (G4int i = 0; i < fNpointsx + 1; i++)
      {
        for (G4int k = 0; k < 2; k++)
        {
          ...
          unitIF = CLHEP::eV;
          // setting spline coefficients for minimal ionization energy
          fMinIonizationEnergy->SetCoefficients2D(ai3D * unitIF, bi3D * unitIF, ci3D * unitIF, i, j,
                                                  k);
          ...
          unitIF = CLHEP::eV / CLHEP::cm;
          fElectricFieldX->SetCoefficients2D(ai3D * unitIF, bi3D * unitIF, ci3D * unitIF, i, j, k);

          // reading the coefficients of cubic spline
          vfilein >> ai3D >> bi3D >> ci3D;
          // setting spline coefficients for vertical electric field
          unitIF = CLHEP::eV / CLHEP::cm;
          fElectricFieldY->SetCoefficients2D(ai3D * unitIF, bi3D * unitIF, ci3D * unitIF, i, j, k);

          // reading the coefficients of cubic spline
          vfilein >> ai3D >> bi3D >> ci3D;
          // setting spline coefficients for nuclear density (first element)
          unitIF = 1.;
          fNucleiDensity[0]->SetCoefficients2D(ai3D * unitIF, bi3D * unitIF, ci3D * unitIF, i, j,
                                               k);
          ...
          // setting spline coefficients for electron density
          unitIF = 1. / CLHEP::cm3;
          fElectronDensity->SetCoefficients2D(ai3D * unitIF, bi3D * unitIF, ci3D * unitIF, i, j, k);
          ...
          for (G4int ii = 1; ii < fNelements; ii++)
          {
            ...
            // setting spline coefficients for nuclear density (other elements if any)
            unitIF = 1.;
            fNucleiDensity[ii]->SetCoefficients2D(ai3D * unitIF, bi3D * unitIF, ci3D * unitIF, i, j,
                                                  k);
          }
        }
      }
  ...
  for (G4int i = 0; i < fNelements; i++)
  {
    fRF.push_back((std::pow(9 * CLHEP::pi * CLHEP::pi / 128 / fZ1[i], 1 / 3.)) * 0.5291772109217
                  * CLHEP::angstrom);  // Thomas-Fermi screening radius
...
G4ThreeVector G4ChannelingFastSimCrystalData::CoordinatesFromBoxToLattice(const G4ThreeVector& pos0)
{
  ...
  if (fBent)
  {
    // for bent crystal
    G4double rsqrt =
      std::sqrt(fBendingRsquare - fBending2R * (x0 * fCosMiscutAngle - z0 * fSinMiscutAngle)
                + x0 * x0 + z0 * z0);
  ...
  if (fBent)
  {
    fCorrectionZ = fBendingR / (fBendingR - fNChannelx * fDx);
  }
