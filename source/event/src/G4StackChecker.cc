...
G4ClassificationOfNewTrack G4StackChecker::ClassifyNewTrack(const G4Track* track)
{
  ...
  if ((!(e < 0.0) && !(e > 0.0) && !(e == 0.0)) || track->GetMomentumDirection() == nullDirection)
  {
    result = fKill;
    ...
    G4cout << "### " << nam << " have been produced by the process "
           << track->GetCreatorProcess()->GetProcessName() << " trackID= " << track->GetTrackID()
           << " parentID= " << track->GetParentID() << G4endl;
    G4cout << "### E= " << track->GetKineticEnergy() << " position= " << track->GetPosition()
           << " direction= " << track->GetMomentumDirection() << " time= " << track->GetGlobalTime()
           << G4endl;
  }
  return result;
}
