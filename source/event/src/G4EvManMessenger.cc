...
G4EvManMessenger::G4EvManMessenger(G4EventManager* fEvMan) : fEvManager(fEvMan)
{
  ...
  storeEvtCmd->SetGuidance(
    "Given the potential large memory size of G4Event and its data-member objects stored in "
    "G4Event,");
  storeEvtCmd->SetGuidance(
