...
{
  G4AutoLock l(&mutex);
  G4double ehi = input.x(), val = input.y();
  if (verbosityLevel > 1)
  {
...
void G4SPSEneDistribution::ArbEnergyHistoFile(const G4String& filename)
{
  ...
  if (!infile)
  {
    G4Exception("G4SPSEneDistribution::ArbEnergyHistoFile", "Event0301", FatalException,
...
void G4SPSEneDistribution::LinearInterpolation()  // MT: Lock in caller
{
  ...
  if (!EnergySpec)
  {
    ...
    if (pdef == nullptr)
    {
      G4Exception("G4SPSEneDistribution::LinearInterpolation", "Event0302", FatalException,
  ...
  while (i < maxi)
  {
    ...
    Area_seg[i] = ((Arb_grad[i] / 2) * (Arb_x[i] * Arb_x[i] - Arb_x[i - 1] * Arb_x[i - 1])
...
void G4SPSEneDistribution::LogInterpolation()  // MT: Lock in caller
{
  ...
  if (!EnergySpec)
  {
    ...
    if (pdef == nullptr)
    {
      G4Exception("G4SPSEneDistribution::LogInterpolation", "Event0302", FatalException,
                  "Error: particle not defined");
    }
    else
    {
  ...
  if (Arb_ezero != nullptr)
  {
    delete[] Arb_ezero;
    Arb_ezero = nullptr;
  }
  if (Arb_Const != nullptr)
  {
    delete[] Arb_Const;
    Arb_Const = nullptr;
  }
...
void G4SPSEneDistribution::ExpInterpolation()  // MT: Lock in caller
{
  ...
  if (!EnergySpec)
  {
    ...
    if (pdef == nullptr)
    {
      G4Exception("G4SPSEneDistribution::ExpInterpolation", "Event0302", FatalException,
                  "Error: particle not defined");
    }
    else
    {
  ...
  while (i < maxi)
  {
    ...
    if (test > 0. || test < 0.)
    {
      ...
      Area_seg[i] = -(Arb_Const[i] * Arb_ezero[i])
                    * (std::exp(-Arb_x[i] / Arb_ezero[i]) - std::exp(-Arb_x[i - 1] / Arb_ezero[i]));
    }
    else
    {
      G4Exception("G4SPSEneDistribution::ExpInterpolation", "Event0302", JustWarning,
...
void G4SPSEneDistribution::SplineInterpolation()  // MT: Lock in caller
{
  ...
  if (!EnergySpec)
  {
    ...
    if (pdef == nullptr)
    {
      G4Exception("G4SPSEneDistribution::SplineInterpolation", "Event0302", FatalException,
                  "Error: particle not defined");
    }
    else
    {
  ...
  while (i < maxi)
  {
    ...
    for (count = 0; count < 101; ++count)
    {
      ...
      if (prob[count] < 0.)
      {
        ...
        ED << "Warning: G4DataInterpolation returns value < 0  " << prob[count] << " " << ei[count]
           << G4endl;
        G4Exception("G4SPSEneDistribution::SplineInterpolation", "Event0303", FatalException, ED);
      }
      area += prob[count] * de;
    }
...
void G4SPSEneDistribution::GenerateLinearEnergies(G4bool bArb = false)
{
  ...
  G4double bracket =
    ((params.grad / 2.) * (emaxsq - eminsq) + params.cept * (params.Emax - params.Emin));
...
void G4SPSEneDistribution::GenerateBiasPowEnergies()
{
  ...
  params.weight =
    GetProbability(params.particle_energy) / (std::pow(params.particle_energy, biasalpha) / normal);
...
void G4SPSEneDistribution::GenerateExpEnergies(G4bool bArb = false)
{
  ...
  params.particle_energy =
    -params.Ezero
    * (std::log(rndm
                  * (std::exp(-params.Emax / params.Ezero) - std::exp(-params.Emin / params.Ezero))
...
void G4SPSEneDistribution::GenerateBremEnergies()
{
  ...
  if (expmax == 0.)
  {
    G4Exception("G4SPSEneDistribution::GenerateBremEnergies", "Event0302", FatalException,
  ...
  G4double tempvar = rndm
                     * ((-k) * Temp * (params.Emax * expmax - params.Emin * expmin)
  ...
  for (i = 1; i < 1000; ++i)
  {
    ...
    diff =
      etest * (std::exp(-etest / (k * Temp))) + k * Temp * (std::exp(-etest / (k * Temp))) - bigc;
...
void G4SPSEneDistribution::GenerateCdgEnergies()
{
  ...
  //
  G4double ene =
    (std::pow(ene_line[i - 1], omalpha[i - 1])
     + (std::pow(ene_line[i], omalpha[i - 1]) - std::pow(ene_line[i - 1], omalpha[i - 1])) * rndm2);
...
void G4SPSEneDistribution::GenUserHistEnergies()
{
  ...
  if (!IPDFEnergyExist)
  {
    ...
    if ((!EnergySpec) && (threadLocalData.Get().particle_definition == nullptr))
    {
      G4Exception("G4SPSEneDistribution::GenUserHistEnergies", "Event0302", FatalException,
                  "Error: particle definition is NULL");
    }

    if (maxbin > 1024)
    {
      G4Exception("G4SPSEneDistribution::GenUserHistEnergies", "Event0302", JustWarning,
...
void G4SPSEneDistribution::GenArbPointEnergies()
{
  ...
  {
    G4Exception("G4SPSEneDistribution::GenArbPointEnergies", "Event0302", FatalException,
                "Error: IntType unknown type");
  }
}
...
void G4SPSEneDistribution::ConvertEPNToEnergy()  // MT: lock in caller
{
  ...
  else
  {
    ...
    if (maxcount < 1)
    {
      G4Exception("G4SPSEneDistribution::ConvertEPNToEnergy()", "gps001", FatalException,
...
G4double G4SPSEneDistribution::GenerateOne(G4ParticleDefinition* a)
{
  ...
  while ((EnergyDisType == "Arb")
           ? (params.particle_energy < ArbEmin || params.particle_energy > ArbEmax)
           : (params.particle_energy < params.Emin || params.particle_energy > params.Emax))
  {
    if (Biased)
    {
...
G4double G4SPSEneDistribution::GetProbability(G4double ene)
{
  ...
  if (EnergyDisType == "Lin")
  {
    if (prob_norm == 1.)
    {
      prob_norm = 0.5 * params.grad * params.Emax * params.Emax + params.cept * params.Emax
                  - 0.5 * params.grad * params.Emin * params.Emin - params.cept * params.Emin;
    }
  ...
  else if (EnergyDisType == "Arb")
  {
    ...
    if (prob <= 0.)
    {
      G4cout << " Warning:G4SPSEneDistribution::GetProbability: prob<= 0. " << prob << " " << ene
             << G4endl;
      prob = 1e-30;
    }
  }
