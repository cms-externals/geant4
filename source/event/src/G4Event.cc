...
G4Event::G4Event(G4int evID) : eventID(evID) {}
  ...
  if (!(fSubEvtVector.empty()))
  {
    for (auto& se : fSubEvtVector)
    {
      G4cout << "SubEvent " << se << " belongs to " << se->GetEvent()
             << " (eventID=" << se->GetEvent()->GetEventID() << ") that has " << se->GetNTrack()
             << " stacked tracks" << G4endl;
    }
    G4ExceptionDescription ed;
    ed << "Deleting G4Event (id:" << eventID << ") that has " << fSubEvtVector.size()
...
G4int G4Event::SpawnSubEvent(G4SubEvent* se)
{
  ...
  {
    G4ExceptionDescription ed;
    ed << "Sub-event " << se << " of type " << se->GetSubEventType() << " with " << se->GetNTrack()
       << " tracks has already spawned.";
    G4Exception("G4Event::SpawnSubEvent", "SubEvent9001", FatalException, ed);
  }
...
G4int G4Event::TerminateSubEvent(G4SubEvent* se)
{
  ...
  if (ss == fSubEvtVector.end())
  {
    G4ExceptionDescription ed;
    ed << "Sub-event " << se << " of type " << se->GetSubEventType() << " with " << se->GetNTrack()
       << " tracks of event " << se->GetEvent()->GetEventID() << " in event " << se->GetEvent()
       << " has never been spawned.";
    G4Exception("G4Event::TerminateSubEvent", "SubEvent9002", FatalException, ed);
  }
  ...
  if (ss != fSubEvtVector.end())
  {
    ...
    G4Exception("G4Event::TerminateSubEvent", "SubEvent9003", FatalException, ed);
  }
