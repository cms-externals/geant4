...
{
  G4AutoLock l(&mutex);
  if (atype != "iso" && atype != "cos" && atype != "user" && atype != "planar" && atype != "beam1d"
      && atype != "beam2d" && atype != "focused")
  {
...
void G4SPSAngDistribution::DefineAngRefAxes(const G4String& refname, const G4ThreeVector& ref)
{
  G4AutoLock l(&mutex);
  if (refname == "angref1")
    AngRef1 = ref.unit();  // x'
  ...
  if (verbosityLevel == 2)
  {
    G4cout << "Angular distribution rotation axes " << AngRef1 << " " << AngRef2 << " " << AngRef3
           << G4endl;
  }
}
...
void G4SPSAngDistribution::GenerateIsotropicFlux(G4ParticleMomentum& mom)
{
  ...
  {  // for plane and surface source
    ...
    {
      finx = (px * posDist->GetSideRefVec1().x()) + (py * posDist->GetSideRefVec2().x())
             + (pz * posDist->GetSideRefVec3().x());
      finy = (px * posDist->GetSideRefVec1().y()) + (py * posDist->GetSideRefVec2().y())
             + (pz * posDist->GetSideRefVec3().y());
      finz = (px * posDist->GetSideRefVec1().z()) + (py * posDist->GetSideRefVec2().z())
...
void G4SPSAngDistribution::GenerateCosineLawFlux(G4ParticleMomentum& mom)
{
  ...
  rndm = angRndm->GenRandTheta();
  sintheta = std::sqrt(
    rndm * (std::sin(MaxTheta) * std::sin(MaxTheta) - std::sin(MinTheta) * std::sin(MinTheta))
...
void G4SPSAngDistribution::GenerateUserDefFlux(G4ParticleMomentum& mom)
{
  ...
  else  // UserWRTSurface = true
  {
    ...
    if (verbosityLevel > 1)
    {
      G4cout << "SideRefVecs " << posDist->GetSideRefVec1() << posDist->GetSideRefVec2()
             << posDist->GetSideRefVec3() << G4endl;
