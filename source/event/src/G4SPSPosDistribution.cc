...
void G4SPSPosDistribution::GeneratePointsInPlane(G4ThreeVector& pos)
{
  ...
  //
  if ((CentreCoords.x() > 0. && Rotz.x() < 0.) || (CentreCoords.x() < 0. && Rotz.x() > 0.)
      || (CentreCoords.y() > 0. && Rotz.y() < 0.) || (CentreCoords.y() < 0. && Rotz.y() > 0.)
      || (CentreCoords.z() > 0. && Rotz.z() < 0.) || (CentreCoords.z() < 0. && Rotz.z() > 0.))
  {
    // Invert y and z
    //
  ...
  if (verbosityLevel == 2)
  {
    G4cout << "Reference vectors for cosine-law " << td.CSideRefVec1 << " " << td.CSideRefVec2
...
void G4SPSPosDistribution::GeneratePointsOnSurface(G4ThreeVector& pos)
{
  ...
  else if (Shape == "Ellipsoid")
  {
    ...
    G4double tempzvar;
    tempzvar = (numXinZ * numXinZ) / (halfx * halfx) + (numYinZ * numYinZ) / (halfy * halfy)
               + 1. / (halfz * halfz);
    tempzvar = 1. / tempzvar;
    G4double coordz = std::sqrt(tempzvar);

    lhs = std::sqrt((coordx * coordx) / (halfx * halfx) + (coordy * coordy) / (halfy * halfy)
  ...
  else if (Shape == "Para")
  {
    ...
    if (testrand < Probs[0])
    {
      ...
      G4ThreeVector xdash(halfz * std::tan(ParTheta) * std::cos(ParPhi),
                          halfz * std::tan(ParTheta) * std::sin(ParPhi), halfz / std::cos(ParPhi));
...
void G4SPSPosDistribution::GeneratePointsInVolume(G4ThreeVector& pos)
{
  ...
  else if (Shape == "Ellipsoid")
  {
    ...
    while (temp > 1.)
    {
      ...
      temp =
        ((x * x) / (halfx * halfx)) + ((y * y) / (halfy * halfy)) + ((z * z) / (halfz * halfz));
    }
  }
  else if (Shape == "Cylinder")
  {
