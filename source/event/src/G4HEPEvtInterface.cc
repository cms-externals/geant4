...
G4HEPEvtInterface::G4HEPEvtInterface(const char* evfile, G4int vl) : vLevel(vl)
{
...
void G4HEPEvtInterface::GeneratePrimaryVertex(G4Event* evt)
{
  ...
  if (inputFile.eof())
  {
    G4Exception("G4HEPEvtInterface::GeneratePrimaryVertex", "Event0202", RunMustBeAborted,
  ...
  if (vLevel > 0)
  {
    G4cout << "G4HEPEvtInterface - reading " << NHEP << " HEPEvt particles from " << fileName << "."
           << G4endl;
  }
  for (G4int IHEP = 0; IHEP < NHEP; ++IHEP)
  {
    ...
    if (inputFile.eof())
    {
      G4Exception("G4HEPEvtInterface::GeneratePrimaryVertex", "Event0203", FatalException,
                  "Unexpected End-Of-File in the middle of an event");
    }
    if (vLevel > 1)
    {
      G4cout << " " << ISTHEP << " " << IDHEP << " " << JDAHEP1 << " " << JDAHEP2 << " " << PHEP1
             << " " << PHEP2 << " " << PHEP3 << " " << PHEP5 << G4endl;
    }
    ...
    //
    auto* hepParticle = new G4HEPEvtParticle(particle, ISTHEP, JDAHEP1, JDAHEP2);
