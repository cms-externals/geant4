...
G4Allocator<G4Trajectory>*& masterTrajectoryAllocator()
{
  static G4Allocator<G4Trajectory>* master_instance = nullptr;
...
G4Trajectory* G4Trajectory::CloneForMaster() const
{
  if(masterTrajectoryAllocator()==nullptr) G4Exception("G4Trajectory::CloneForMaster()","XXXXXXX",FatalException,"HERE IT IS!");
  G4Trajectory* cloned = masterTrajectoryAllocator()->MallocSingle();
  cloned->fTrackID = fTrackID;
  cloned->fParentID = fParentID;
  cloned->PDGEncoding = PDGEncoding;
  cloned->PDGCharge = PDGCharge;
  //cloned->ParticleName = std::move(ParticleName);
  cloned->initialKineticEnergy = initialKineticEnergy;
  cloned->initialMomentum = initialMomentum;
  cloned->positionRecord = new G4TrajectoryPointContainer();

  for (auto& i : *positionRecord) {
    auto point = (G4TrajectoryPoint*)i;
    cloned->positionRecord->push_back(point->CloneForMaster());
  }
  return cloned;
}
