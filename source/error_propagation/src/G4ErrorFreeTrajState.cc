...
//------------------------------------------------------------------------
G4ErrorFreeTrajState::G4ErrorFreeTrajState(const G4String& partName, const G4Point3D& pos,
                                           const G4Vector3D& mom, const G4ErrorTrajErr& errmat)
...
G4ErrorFreeTrajState::G4ErrorFreeTrajState(const G4ErrorSurfaceTrajState& tpSD)
  : G4ErrorTrajState(tpSD.GetParticleType(), tpSD.GetPosition(), tpSD.GetMomentum())
{
  ...
  G4double TVW1 = std::sqrt(
    mom2 / (mom2 + tpSDparam.GetPV() * tpSDparam.GetPV() + tpSDparam.GetPW() * tpSDparam.GetPW()));
  ...
  G4Vector3D vTN =
    vTVW.x() * vectorU + vTVW.y() * tpSDparam.GetVectorV() + vTVW.z() * tpSDparam.GetVectorW();
  ...
  if (iverbose >= 5)
  {
    ...
    G4cout << " CHECK: pc2 " << pc2 << " = " << GetParameters().GetLambda() << " diff "
           << pc2 - GetParameters().GetLambda() << G4endl;
    G4cout << " CHECK: pc3 " << pc3 << " = " << GetParameters().GetPhi() << " diff "
           << pc3 - GetParameters().GetPhi() << G4endl;
  }
#endif
  ...
  if (iverbose >= 5)
  {
    G4cout << " CHECK: vUN " << vUN << " = " << vUperp << " diff " << (vUN - vUperp).mag()
           << G4endl;
    G4cout << " CHECK: vVN " << vVN << " = " << vVperp << " diff " << (vVN - vVperp).mag()
           << G4endl;
  }
#endif
...
G4int G4ErrorFreeTrajState::PropagateError(const G4Track* aTrack)
{
  ...
  if (G4ErrorPropagatorData::GetErrorPropagatorData()->GetStage() == G4ErrorStage_Deflation)
    stepLengthCm *= -1.;
  ...
  if (iverbose >= 2)
  {
    G4cout << "G4EP: vposPre " << vposPre << G4endl << "G4EP: vposPost " << vposPost << G4endl;
    G4cout << "G4EP: vpPre " << vpPre << G4endl << "G4EP: vpPost " << vpPost << G4endl;
  ...
  if (iverbose >= 2)
    G4cout << "G4EP: vpPreNorm " << vpPreNorm << " vpPostNorm " << vpPostNorm << G4endl;
  ...
  if (charge != 0. && field)
  {
    ...
    G4ThreeVector HPre = G4ThreeVector(h1[0], h1[1], h1[2]) / tesla
    ...
    if (iverbose >= 2)
    {
      G4cout << "G4EP: h1 = " << h1[0] << ", " << h1[1] << ", " << h1[2] << G4endl;
      G4cout << "G4EP: pos1/mm = " << pos1[0] << ", " << pos1[1] << ", " << pos1[2] << G4endl;
      G4cout << "G4EP: pos2/mm = " << pos2[0] << ", " << pos2[1] << ", " << pos2[2] << G4endl;
      G4cout << "G4EP: B-filed in KGauss HPre " << HPre << G4endl << "G4EP: in KGauss HPost "
             << HPost << G4endl;
    }
#endif
    ...
    if (magHPre + magHPost != 0.)
    {
      ...
      if (iverbose >= 2)
      {
        G4cout << " G4EP: gam " << gam << " alphaSqr " << alphaSqr << " diffHSqr " << diffHSqr
               << G4endl;
...
#ifdef G4EVERBOSE
      if (iverbose >= 2)
        G4cout << " G4EP: HaverNorm " << vHAverNorm << " magHAver " << HAver << " charge " << charge
               << G4endl;
#endif
...
#ifdef G4EVERBOSE
      if (iverbose >= 2)
        G4cout << " G4EP: AN2 " << AN2 << "  gamma:" << gamma << "  theta=" << thetaAver << G4endl;
#endif
      ...
      G4ThreeVector vVPost(-vpPostNorm.z() * vUPost.y(), vpPostNorm.z() * vUPost.x(),
                           vpPostNorm.x() * vUPost.y() - vpPostNorm.y() * vUPost.x());
#ifdef G4EVERBOSE
      G4cout << " vpPostNorm " << vpPostNorm << G4endl;
      if (iverbose >= 2)
        G4cout << " G4EP: AU " << AU << " vUPre " << vUPre << " vVPre " << vVPre << " vUPost "
               << vUPost << " vVPost " << vVPost << G4endl;
#endif
...
#ifdef G4EVERBOSE
      if (iverbose >= 2)
        G4cout << " G4EP: QP " << QP << " QAver " << QAver << " pAver " << pAver << G4endl;
#endif
...
#ifdef G4EVERBOSE
      if (iverbose >= 2)
        G4cout << "G4EP: OMcosThetaAver " << OMcosThetaAver << " cosThetaAver " << cosThetaAver
               << " thetaAver " << thetaAver << " QAver " << QAver << " stepLengthCm "
               << stepLengthCm << G4endl;
#endif
      ...
      G4ThreeVector vHUPre(-vHAverNorm.z() * vUPre.y(), vHAverNorm.z() * vUPre.x(),
      ...
      transf[0][3] =
        -deltaPInv / stepLengthCm * (vUPre.x() * vpPostNorm.x() + vUPre.y() * vpPostNorm.y());

      transf[0][4] =
        -deltaPInv / stepLengthCm
        * (vVPre.x() * vpPostNorm.x() + vVPre.y() * vpPostNorm.y() + vVPre.z() * vpPostNorm.z());
...
#ifdef G4EVERBOSE
      if (iverbose >= 3)
        G4cout << "ctransf10= " << transf[1][0] << " " << -QP << " " << ANV << " " << vpPostNorm.x()
               << " " << deltaPos.x() << " " << vpPostNorm.y() << " " << deltaPos.y() << " "
               << vpPostNorm.z() << " " << deltaPos.z() << " " << deltaPInv << " " << pAver
               << G4endl;
#endif

      transf[1][1] =
        cosThetaAver * (vVPre.x() * vVPost.x() + vVPre.y() * vVPost.y() + vVPre.z() * vVPost.z())
        + sinThetaAver
            * (vHVPre.x() * vVPost.x() + vHVPre.y() * vVPost.y() + vHVPre.z() * vVPost.z())
        + OMcosThetaAver
            * (vHAverNorm.x() * vVPre.x() + vHAverNorm.y() * vVPre.y() + vHAverNorm.z() * vVPre.z())
      ...
      transf[1][2] =
        ...
        + sinThetaAver
            * (vHUPre.x() * vVPost.x() + vHUPre.y() * vVPost.y() + vHUPre.z() * vVPost.z())
...
#ifdef G4EVERBOSE
      if (iverbose >= 3)
        G4cout << "ctransf20= " << transf[2][0] << " " << -QP << " " << ANU << " " << vpPostNorm.x()
               << " " << deltaPos.x() << " " << vpPostNorm.y() << " " << deltaPos.y() << " "
               << vpPostNorm.z() << " " << deltaPos.z() << " " << sinpPostInv << " " << deltaPInv
               << " " << pAver << G4endl;
#endif
      transf[2][1] =
      ...
      transf[2][3] =
        -QAver * ANU * (vUPre.x() * vpPostNorm.x() + vUPre.y() * vpPostNorm.y()) * sinpPostInv;
#ifdef G4EVERBOSE
      if (iverbose >= 3)
        G4cout << "ctransf23= " << transf[2][3] << " " << -QAver << " " << ANU << " " << vUPre.x()
               << " " << vpPostNorm.x() << " " << vUPre.y() << " " << vpPostNorm.y() << " "
               << sinpPostInv << G4endl;
#endif
      ...
      transf[3][0] =
        pAver * (vUPost.x() * deltaPos.x() + vUPost.y() * deltaPos.y()) * (1. + deltaPInv * pAver);
#ifdef G4EVERBOSE
      if (iverbose >= 3)
        G4cout << "ctransf30= " << transf[3][0] << " " << pAver << " " << vUPost.x() << " "
               << deltaPos.x() << " " << vUPost.y() << " " << deltaPos.y() << " " << deltaPInv
               << " " << pAver << G4endl;
#endif
...
#ifdef G4EVERBOSE
      if (iverbose >= 3)
        G4cout << "ctransf32= " << transf[3][2] << " " << sinThetaAver << " " << vUPre.x() << " "
               << vUPost.x() << " " << vUPre.y() << " " << vUPost.y() << " " << OMcosThetaAver
               << " " << vHUPre.x() << " " << vUPost.x() << " " << vHUPre.y() << " " << vUPost.y()
               << " " << TMSINT << " " << vHAverNorm.x() << " " << vUPost.x() << " "
               << vHAverNorm.y() << " " << vUPost.y() << " " << vHAverNorm.x() << " " << vUPre.x()
               << " " << vHAverNorm.y() << " " << vUPre.y() << " " << sinpPre << " " << QAver
               << G4endl;
#endif
      ...
      // ***   Zt
      transf[4][0] =
        pAver * (vVPost.x() * deltaPos.x() + vVPost.y() * deltaPos.y() + vVPost.z() * deltaPos.z())
        * (1. + deltaPInv * pAver);

      transf[4][1] =
        (sinThetaAver * (vVPre.x() * vVPost.x() + vVPre.y() * vVPost.y() + vVPre.z() * vVPost.z())
         + OMcosThetaAver
             * (vHVPre.x() * vVPost.x() + vHVPre.y() * vVPost.y() + vHVPre.z() * vVPost.z())
         + TMSINT
             * (vHAverNorm.x() * vVPost.x() + vHAverNorm.y() * vVPost.y()
...
#ifdef G4EVERBOSE
      if (iverbose >= 3)
        G4cout << "ctransf41= " << transf[4][1] << " " << sinThetaAver << " " << OMcosThetaAver
               << " " << TMSINT << " " << vVPre << " " << vVPost << " " << vHVPre << " "
               << vHAverNorm << " " << QAver << G4endl;
#endif
  ...
  */
  ...
  if (iverbose >= 2)
    G4cout << " tf * err " << theTransfMat * fError << G4endl << " transf matrix "
           << theTransfMat.T() << G4endl;
#endif
...
#ifdef G4EVERBOSE
  if (iverbose >= 4)
    G4cout << std::setprecision(6) << std::setw(6) << "G4EP:MSC: RI=X/X0 " << RI << " stepLengthCm "
           << stepLengthCm << " radlen/cm " << (mate->GetRadlen() / cm)
...
//------------------------------------------------------------------------
void G4ErrorFreeTrajState::CalculateEffectiveZandA(const G4Material* mate, G4double& effZ,
                                                   G4double& effA)
{
  effZ = 0.;
  effA = 0.;
...
G4int G4ErrorFreeTrajState::PropagateErrorIoni(const G4Track* aTrack)
{
  ...
  // *     Calculate xi factor (keV).
  G4double XI =
    153.5 * effZ * stepLengthCm * (mate->GetDensity() / mg * mole) / (effA * beta * beta);

#ifdef G4EVERBOSE
  if (iverbose >= 2)
  {
    G4cout << "G4EP:IONI: XI/keV " << XI << " beta " << beta << " gamma " << gamma << G4endl;
    G4cout << " density " << (mate->GetDensity() / mg * mole) << " effA " << effA << " step "
           << stepLengthCm << G4endl;
  }
#endif
  ...
  */
  ...
  if (XI / Emax < 0.01)
    dedxSq *= XI / Emax * 100;  // Quench for low Elos, see above: newVar=odVar *k/0.01
...
#ifdef G4EVERBOSE
  if (iverbose >= 2)
    G4cout << "G4:IONI Etot/GeV: " << Etot << " err_dedx^2/GeV^2: " << dedxSq << " p^6: " << pPre6
           << G4endl;
  if (iverbose >= 2)
