...
//-----------------------------------------------------------------------
G4int G4ErrorPropagator::Propagate(G4ErrorTrajState* currentTS, const G4ErrorTarget* target,
                                   G4ErrorMode mode)
{
  ...
  if (currentTS->GetMomentum().mag() < 1.E-9 * MeV)
  {
    ...
    G4Exception("G4ErrorPropagator::Propagate()", "GEANT4e-Notification", JustWarning, message);
    return -3;
  }
  ...
  if (verbose >= 1)
  {
    G4cout << " =====> starting GEANT4E tracking for " << currentTS->GetParticleType()
...
G4int G4ErrorPropagator::PropagateOneStep(G4ErrorTrajState* currentTS)
{
  ...
  if ((g4edata->GetState() == G4ErrorState_PreInit)
      || (G4StateManager::GetStateManager()->GetCurrentState() != G4State_GeomClosed))
  {
    std::ostringstream message;
    message << "Called before initialization is done for this track!";
    G4Exception("G4ErrorPropagator::PropagateOneStep()", "InvalidCall", FatalException, message,
  ...
  if (currentTS->GetMomentum().mag() < 1.E-9 * MeV)
  {
    ...
    message << "Energy too low to be propagated: "
            << G4BestUnit(currentTS->GetMomentum().mag(), "Energy");
    G4Exception("G4ErrorPropagator::PropagateOneStep()", "GEANT4e-Notification", JustWarning,
                message);
    return -3;
  }
...
G4Track* G4ErrorPropagator::InitG4Track(G4ErrorTrajState& initialTS)
{
  ...
  if (particle == 0)
  {
    ...
    G4Exception("G4ErrorPropagator::InitG4Track()", "InvalidSetup", FatalException, message);
  }

  G4DynamicParticle* DP = new G4DynamicParticle(particle, initialTS.GetMomentum());
  ...
  if (verbose >= 3)
  {
    G4cout << " G4ErrorPropagator new track of energy: " << theG4Track->GetKineticEnergy()
           << G4endl;
  }
#endif
  ...
  {
    G4Exception("G4ErrorPropagator::InitG4Track()", "InvalidSetup", FatalException,
                "G4SteppingManager not initialized yet!");
  }
  else
  {
...
G4int G4ErrorPropagator::MakeOneStep(G4ErrorFreeTrajState* currentTS_FREE)
{
  ...
  if (theG4Track->GetStep()->GetPostStepPoint()->GetProcessDefinedStep()->GetProcessName()
      == "Transportation")
  {
  ...
  else if (theG4Track->GetStep()->GetPostStepPoint()->GetProcessDefinedStep()->GetProcessName()
  ...
  if (ierr != 0)
  {
    ...
    G4Exception("G4ErrorPropagator::MakeOneStep()", "GEANT4e-Notification", JustWarning, message,
...
G4ErrorFreeTrajState* G4ErrorPropagator::InitFreeTrajState(G4ErrorTrajState* currentTS)
{
  ...
  else
  {
    ...
    G4Exception("G4ErrorPropagator::InitFreeTrajState()", "InvalidState", FatalException, message);
  }
                                          ...
                                          const G4ErrorTarget* target)
{
  ...
  if (verbose >= 1)
  {
    G4cout << " G4ErrorPropagator::Propagate: final state " << G4int(g4edata->GetState())
           << " TSType " << currentTS->GetTSType() << G4endl;
  }
#endif
  ...
  else if (currentTS->GetTSType() == G4eTS_OS)
  {
    ...
    {
      G4Exception("G4ErrorPropagator:GetFinalTrajState()", "InvalidSetup", FatalException,
    ...
    *currentTS = G4ErrorSurfaceTrajState(*(static_cast<G4ErrorFreeTrajState*>(currentTS_FREE)),
...
G4bool G4ErrorPropagator::CheckIfLastStep(G4Track* aTrack)
{
  ...
  if (verbose >= 4)
  {
    G4cout << " G4ErrorPropagator::CheckIfLastStep G4ErrorState= " << G4int(g4edata->GetState())
           << G4endl;
  }
#endif
  ...
  else if (aTrack->GetNextVolume() == 0)
  {
    ...
    if (verbose >= 1)
    {
      ...
      G4Exception("G4ErrorPropagator::CheckIfLastStep()", "GEANT4e-Notification", JustWarning,
                  message);
    }
