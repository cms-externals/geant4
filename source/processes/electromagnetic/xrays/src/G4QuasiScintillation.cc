...
G4QuasiScintillation::G4QuasiScintillation(const G4String& processName, G4ProcessType type)
...
//....oooOO0OOooo........oooOO0OOooo........oooOO0OOooo........oooOO0OOooo......
void G4QuasiScintillation::BuildInverseCdfTable(const G4MaterialPropertyVector* MPV,
...
// representing the inverse C.D.F. as a function of photon energy.
{
  if (MPV && (*MPV)[0] >= 0.0)
  {
    ...
    {
      cdf[ii] =
        cdf[ii - 1] + 0.5 * (MPV->Energy(ii) - MPV->Energy(ii - 1)) * ((*MPV)[ii] + (*MPV)[ii - 1]);
    }
...
//....oooOO0OOooo........oooOO0OOooo........oooOO0OOooo........oooOO0OOooo......
G4VParticleChange* G4QuasiScintillation::AtRestDoIt(const G4Track& aTrack, const G4Step& aStep)
...
//....oooOO0OOooo........oooOO0OOooo........oooOO0OOooo........oooOO0OOooo......
G4VParticleChange* G4QuasiScintillation::PostStepDoIt(const G4Track& aTrack, const G4Step& aStep)
...
// evenly along the track segment and uniformly into 4pi.
{
  ...
  if (fScintillationByParticleType)
  {
    MeanNumberOfPhotons = GetScintillationYieldByParticleType(
      aTrack, aStep, yield1, yield2, yield3, timeconstant1, timeconstant2, timeconstant3);
  }
  else
  {
  ...
  else
  {
    ...
    ed << "G4MaterialPropertiesTable for " << aMaterial->GetName() << " is not found!" << G4endl;
    G4Exception("G4QuasiScintillation::PostStepDoIt", "Scint04", FatalException, ed);
  }
  ...
  G4double deltaVelocity = pPostStepPoint->GetVelocity() - pPreStepPoint->GetVelocity();
  ...
  for (G4int scnt = 0; scnt < N_timeconstants; ++scnt)
  {
    ...
    if (fOffloadingFlag)
    {
      ...
      auto quasiPhoton = new G4DynamicParticle(G4QuasiOpticalPhoton::QuasiOpticalPhotonDefinition(),
      ...
      G4QuasiOpticalData quasiTrackData{aMaterial->GetIndex(),
                                        numPhot,
                                        aParticle->GetDefinition()->GetPDGCharge(),
                                        aStep.GetStepLength(),
                                        pPreStepPoint->GetVelocity(),
                                        deltaVelocity,
                                        aStep.GetDeltaPosition()};
      aSecondaryTrack->SetAuxiliaryTrackInformation(
        secID, new G4ScintillationQuasiTrackInfo(quasiTrackData, scintTime, riseTime));
      aParticleChange.AddSecondary(aSecondaryTrack);

      // Exit early when offloading is enabled
      continue;
    }
    ...
    for (G4int i = 0; i < numPhot; ++i)
    {
      ...
      G4double deltaTime = delta / (pPreStepPoint->GetVelocity() + 0.5 * rand * deltaVelocity);
      if (riseTime == 0.0)
      {
  ...
  if (verboseLevel > 1)
  {
    G4cout << "\n Exiting from G4QuasiScintillation::DoIt -- "
           << " NumberOfSecondaries = " << aParticleChange.GetNumberOfSecondaries() << G4endl;
  }
...
//....oooOO0OOooo........oooOO0OOooo........oooOO0OOooo........oooOO0OOooo......
G4double G4QuasiScintillation::GetMeanLifeTime(const G4Track&, G4ForceCondition* condition)
{
  *condition = Forced;
  return DBL_MAX;
}
...
G4double G4QuasiScintillation::GetScintillationYieldByParticleType(
  const G4Track& aTrack, const G4Step& aStep, G4double& yield1, G4double& yield2, G4double& yield3,
  G4double& timeconstant1, G4double& timeconstant2, G4double& timeconstant3)
{
  ...
  if (yieldVector == nullptr)
  {
    ...
    ed << "\nG4QuasiScintillation::PostStepDoIt(): "
       ...
       << "needed (hint: PARTICLE might not be the primary particle." << G4endl;
    G4String comments =
      "Missing MaterialPropertiesTable entry - No correct "
      "entry in MaterialPropertiesTable";
    G4Exception("G4QuasiScintillation::PostStepDoIt", "Scint01", FatalException, ed, comments);
  ...
  else
  {
    ...
    if (verboseLevel > 0 && fNumEnergyWarnings <= 10)
    {
      ...
      G4Exception("G4QuasiScintillation::GetScintillationYieldByParticleType()", "Scint03",
                  JustWarning, ed, cmt);
    }
  ...
  if ((aTrack.GetTrackStatus() == fStopButAlive)
      or (aStep.GetPostStepPoint()->GetStepStatus() == fGeomBoundary))
  {
...
void G4QuasiScintillation::SetScintillationByParticleType(const G4bool scintType)
{
  ...
  {
    G4Exception("G4QuasiScintillation::SetScintillationByParticleType", "Scint02", JustWarning,
