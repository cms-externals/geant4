...
G4TransitionRadiation::G4TransitionRadiation(const G4String& processName, G4ProcessType type)
...
G4double G4TransitionRadiation::IntegralOverEnergy(G4double energy1, G4double energy2,
                                                   G4double varAngle) const
{
  ...
  return h
         * (SpectralAngleTRdensity(energy1, varAngle) + SpectralAngleTRdensity(energy2, varAngle)
...
G4double G4TransitionRadiation::IntegralOverAngle(G4double energy, G4double varAngle1,
                                                  G4double varAngle2) const
{
  ...
  return h
         * (SpectralAngleTRdensity(energy, varAngle1) + SpectralAngleTRdensity(energy, varAngle2)
            + 4.0 * sumOdd + 2.0 * sumEven)
         / 3.0;
}
                                                          ...
                                                          G4double varAngle2) const
{
  ...
  for (i = 1; i < fSympsonNumber; ++i)
  {
    sumEven += IntegralOverEnergy(fMinEnergy, fMinEnergy + 0.3 * (fMaxEnergy - fMinEnergy),
                                  varAngle1 + 2 * i * h)
               + IntegralOverEnergy(fMinEnergy + 0.3 * (fMaxEnergy - fMinEnergy), fMaxEnergy,
                                    varAngle1 + 2 * i * h);
    sumOdd += IntegralOverEnergy(fMinEnergy, fMinEnergy + 0.3 * (fMaxEnergy - fMinEnergy),
                                 varAngle1 + (2 * i - 1) * h)
              + IntegralOverEnergy(fMinEnergy + 0.3 * (fMaxEnergy - fMinEnergy), fMaxEnergy,
                                   varAngle1 + (2 * i - 1) * h);
  }
  ...
  return h
         * (IntegralOverEnergy(fMinEnergy, fMinEnergy + 0.3 * (fMaxEnergy - fMinEnergy), varAngle1)
            + IntegralOverEnergy(fMinEnergy + 0.3 * (fMaxEnergy - fMinEnergy), fMaxEnergy,
                                 varAngle1)
...
G4double G4TransitionRadiation::EnergyIntegralDistribution(G4double energy1, G4double energy2) const
{
  ...
  sumOdd +=
    IntegralOverAngle(energy1 + (2 * fSympsonNumber - 1) * h, 0.0, 0.01 * fMaxTheta)
    + IntegralOverAngle(energy1 + (2 * fSympsonNumber - 1) * h, 0.01 * fMaxTheta, fMaxTheta);
  ...
  return h
         * (IntegralOverAngle(energy1, 0.0, 0.01 * fMaxTheta)
            ...
            + IntegralOverAngle(energy2, 0.01 * fMaxTheta, fMaxTheta) + 4.0 * sumOdd
            + 2.0 * sumEven)
         / 3.0;
}
