...
G4StandardCerenkovModel::G4StandardCerenkovModel() : G4VXRayModel("theCerenkov")
{
  if (nullptr == fBetaLim)
  {
    isInitializer = true;
  ...
  if (isInitializer && !isInitialized)
  {
    ...
    for (std::size_t i = 0; i < nVolumes; ++i)
    {
      ...
      for (G4int k = 0; k < nvec; ++k)
      {
        ...
        if (deltae > 0.0)
        {
          (*(*fMeanNumberOfPhotons)[i])[k] = sum / deltae;
        }
        if (sum > 0.0)
        {
          sum = 1.0 / sum;
...
//....oooOO0OOooo........oooOO0OOooo........oooOO0OOooo........oooOO0OOooo......
void G4StandardCerenkovModel::SampleXRays(std::vector<G4Track*>& out, const G4Step& step)
{
