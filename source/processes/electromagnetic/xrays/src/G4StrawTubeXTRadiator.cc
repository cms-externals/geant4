...
G4StrawTubeXTRadiator::G4StrawTubeXTRadiator(G4LogicalVolume* anEnvelope, G4Material* foilMat,
                                             G4Material* gasMat, G4double a, G4double b,
                                             G4Material* mediumMat, G4bool unishut,
                                             const G4String& processName)
  : G4VXTRenergyLoss(anEnvelope, foilMat, gasMat, a, b, 1, processName)
{
  if (verboseLevel > 0) G4cout << "Straw tube X-ray TR  radiator EM process is called" << G4endl;
  ...
  if (unishut)
  {
    ...
    if (verboseLevel > 0)
      G4cout << "straw uniform shooting: "
             << "fAlphaPlate = " << fAlphaPlate << " ; fAlphaGas = " << fAlphaGas << G4endl;
  }
  else
  {
    fAlphaPlate = 0.5;
  ...
  if (verboseLevel > 0) G4cout << "medium material = " << mediumMat->GetName() << G4endl;
...
G4double G4StrawTubeXTRadiator::GetStackFactor(G4double energy, G4double gamma, G4double varAngle)
{
  ...
  G4complex R = (Z1 - Z2) * (Z1 - Z2) * (1. - H2 * H) + (Z2 - Z3) * (Z2 - Z3) * (1. - H3)
...
G4double G4StrawTubeXTRadiator::GetMediumFormationZone(G4double omega, G4double gamma,
                                                       G4double varAngle)
{
  G4double cof, lambda;
...
G4complex G4StrawTubeXTRadiator::GetMediumComplexFZ(G4double omega, G4double gamma,
                                                    G4double varAngle)
{
