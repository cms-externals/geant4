...
G4ForwardXrayTR::G4ForwardXrayTR(const G4String& matName1, const G4String& matName2,
  ...
  : G4TransitionRadiation(processName)
{
  ...
  {
    G4Exception("G4ForwardXrayTR::G4ForwardXrayTR", "ForwardXrayTR01", JustWarning,
  ...
  if (build)
  {
    ...
    if (iMat == numOfCouples)
    {
      G4Exception("G4ForwardXrayTR::G4ForwardXrayTR", "ForwardXrayTR02", JustWarning,
...
G4ForwardXrayTR::G4ForwardXrayTR(const G4String& processName) : G4TransitionRadiation(processName)
{
  fPtrGamma = nullptr;
  fGammaCutInKineticEnergy = nullptr;
  fGammaTkinCut = fMinEnergyTR = fMaxEnergyTR = fMaxThetaTR = 0.0;
    ...
    for (jMat = 0; jMat < numOfCouples; ++jMat)  // transition iMat -> jMat !!!
    {
      ...
      else
      {
        ...
        for (iTkin = 0; iTkin < fTotBin; ++iTkin)  // Lorentz factor loop
        {
          ...
          auto angleVector = new G4PhysicsLinearVector(0.0, fMaxThetaTR, fBinTR);
          ...
          {
            energySum += fCofTR
                         * EnergySum(energyVector->GetLowEdgeEnergy(iTR),
                                     energyVector->GetLowEdgeEnergy(iTR + 1));

            angleSum += fCofTR
                        * AngleSum(angleVector->GetLowEdgeEnergy(iTR),
...
G4double G4ForwardXrayTR::SpectralAngleTRdensity(G4double energy, G4double varAngle) const
{
  ...
  formationLength1 = 1.0 / (1.0 / (fGamma * fGamma) + fSigma1 / (energy * energy) + varAngle);
...
G4double G4ForwardXrayTR::AngleSum(G4double varAngle1, G4double varAngle2) const
{
  ...
  for (i = 1; i < fSympsonNumber; ++i)
  {
    ...
    sumOdd += EnergyInterval(fMinEnergyTR, fMaxEnergyTR, varAngle1 + (2 * i - 1) * h);
  }
  sumOdd += EnergyInterval(fMinEnergyTR, fMaxEnergyTR, varAngle1 + (2 * fSympsonNumber - 1) * h);

  return h
         * (EnergyInterval(fMinEnergyTR, fMaxEnergyTR, varAngle1)
            + EnergyInterval(fMinEnergyTR, fMaxEnergyTR, varAngle2) + 4.0 * sumOdd + 2.0 * sumEven)
         / 3.0;
}
...
G4double G4ForwardXrayTR::SpectralDensity(G4double energy, G4double x) const
{
  ...
  return ((a + b) * std::log((x + b) / (x + a)) / (a - b) + a / (x + a) + b / (x + b)) / energy;
}
...
{
  return SpectralDensity(energy, varAngle2) - SpectralDensity(energy, varAngle1);
}
...
G4double G4ForwardXrayTR::EnergySum(G4double energy1, G4double energy2) const
{
  ...
  sumOdd += AngleInterval(energy1 + (2 * fSympsonNumber - 1) * h, 0.0, fMaxThetaTR);

  return h
         * (AngleInterval(energy1, 0.0, fMaxThetaTR) + AngleInterval(energy2, 0.0, fMaxThetaTR)
...
G4VParticleChange* G4ForwardXrayTR::PostStepDoIt(const G4Track& aTrack, const G4Step& aStep)
{
  ...
  const G4MaterialCutsCouple* iCouple =
    pPreStepPoint->GetPhysicalVolume()->GetLogicalVolume()->GetMaterialCutsCouple();
  const G4MaterialCutsCouple* jCouple =
    pPostStepPoint->GetPhysicalVolume()->GetLogicalVolume()->GetMaterialCutsCouple();
  ...
  if (iMat == jMat
      || ((fMatIndex1 >= 0 && fMatIndex2 >= 0) && (iMat != fMatIndex1 && iMat != fMatIndex2)
  ...
  if (iTkin == fTotBin)  // TR plato, try from left
  {
    numOfTR = (G4int)G4Poisson(
      ((*(*fEnergyDistrTable)(iPlace))(0) + (*(*fAngleDistrTable)(iPlace))(0)) * chargeSq * 0.5);
    if (numOfTR == 0)
    {
      return G4VDiscreteProcess::PostStepDoIt(aTrack, aStep);
    }
  ...
  else
  {
    ...
    else  // general case: Tkin between two vectors of the material
    {
      ...
      numOfTR = (G4int)G4Poisson(
        (((*(*fEnergyDistrTable)(iPlace))(0) + (*(*fAngleDistrTable)(iPlace))(0)) * W1
         + ((*(*fEnergyDistrTable)(iPlace + 1))(0) + (*(*fAngleDistrTable)(iPlace + 1))(0)) * W2)
        * chargeSq * 0.5);
      if (numOfTR == 0)
      {
        return G4VDiscreteProcess::PostStepDoIt(aTrack, aStep);
      ...
      else
      {
        ...
        for (iTR = 0; iTR < numOfTR; ++iTR)
        {
          ...
          anglePos =
            ((*(*fAngleDistrTable)(iPlace))(0) * W1 + (*(*fAngleDistrTable)(iPlace + 1))(0) * W2)
            * G4UniformRand();
          for (iTransfer = 0; iTransfer < fBinTR - 1; ++iTransfer)
          {
          ...
          theta =
            std::sqrt(((*fAngleDistrTable)(iPlace)->GetLowEdgeEnergy(iTransfer - 1)) * W1
                      + ((*fAngleDistrTable)(iPlace + 1)->GetLowEdgeEnergy(iTransfer - 1)) * W2);
          ...
          G4Track* aSecondaryTrack =
            new G4Track(aPhotonTR, aTrack.GetGlobalTime(), aTrack.GetPosition());
          aSecondaryTrack->SetTouchableHandle(aStep.GetPostStepPoint()->GetTouchableHandle());
          aSecondaryTrack->SetParentID(aTrack.GetTrackID());
          aSecondaryTrack->SetCreatorModelID(secID);
          aParticleChange.AddSecondary(aSecondaryTrack);
  ...
  else
  {
    ...
    {  // use trivial mean half/half
      ...
      else
      {
        ...
        for (iTR = 0; iTR < numOfTR; ++iTR)
        {
          energyPos = ((*energyVector1)(0) * W1 + (*energyVector2)(0) * W2) * G4UniformRand();
          for (iTransfer = 0; iTransfer < fBinTR - 1; ++iTransfer)
          {
            if (energyPos >= ((*energyVector1)(iTransfer)*W1 + (*energyVector2)(iTransfer)*W2))
              break;
          }
...
G4double G4ForwardXrayTR::GetThetaTR(G4int, G4int, G4int) const
{
  return 0.0;
}
