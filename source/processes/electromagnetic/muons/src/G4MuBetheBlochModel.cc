...
G4double G4MuBetheBlochModel::xgi[] = {0.0199, 0.1017, 0.2372, 0.4083,
                                       0.5917, 0.7628, 0.8983, 0.9801};

G4double G4MuBetheBlochModel::wgi[] = {0.0506, 0.1112, 0.1569, 0.1813,
                                       0.1813, 0.1569, 0.1112, 0.0506};

//....oooOO0OOooo........oooOO0OOooo........oooOO0OOooo........oooOO0OOooo......

G4MuBetheBlochModel::G4MuBetheBlochModel(const G4ParticleDefinition* p, const G4String& nam)
  : G4VEmModel(nam),
...
void G4MuBetheBlochModel::SampleSecondaries(std::vector<G4DynamicParticle*>* vdp,
                                            const G4MaterialCutsCouple* couple,
                                            const G4DynamicParticle* dp, G4double minKinEnergy,
                                            G4double maxEnergy)
{
  ...
  do
  {
    ...
    if (f > grej)
    {
      G4cout << "G4MuBetheBlochModel::SampleSecondary Warning! "
             << "Majorant " << grej << " < " << f << " for edelta= " << tkin
             << " tmin= " << minKinEnergy << " max= " << maxKinEnergy << G4endl;
    }
  ...
  if (UseAngularGeneratorFlag())
  {
    ...
    deltaDirection =
      GetAngularDistribution()->SampleDirection(dp, tkin, SelectRandomAtomNumber(mat), mat);
  }
  else
  {
    G4double deltaMom = std::sqrt(tkin * (tkin + 2.0 * CLHEP::electron_mass_c2));
    G4double totalMom = totEnergy * std::sqrt(beta2);
    G4double cost = tkin * (totEnergy + CLHEP::electron_mass_c2) / (deltaMom * totalMom);
