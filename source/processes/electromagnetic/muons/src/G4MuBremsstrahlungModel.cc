...
const G4double G4MuBremsstrahlungModel::xgi[] = {0.03377, 0.16940, 0.38069,
                                                 0.61931, 0.83060, 0.96623};
const G4double G4MuBremsstrahlungModel::wgi[] = {0.08566, 0.18038, 0.23396,
                                                 0.23396, 0.18038, 0.08566};
G4double G4MuBremsstrahlungModel::fDN[] = {0.0};

G4MuBremsstrahlungModel::G4MuBremsstrahlungModel(const G4ParticleDefinition* p, const G4String& nam)
  : G4VEmModel(nam),
...
G4double G4MuBremsstrahlungModel::MinPrimaryEnergy(const G4Material*, const G4ParticleDefinition*,
                                                   G4double cut)
{
...
void G4MuBremsstrahlungModel::Initialise(const G4ParticleDefinition* p, const G4DataVector& cuts)
{
  SetParticle(p);
                                                       ...
                                                       G4double kineticEnergy, G4double cutEnergy)
{
  ...
  {
    G4double loss = ComputMuBremLoss((*theElementVector)[i]->GetZ(), kineticEnergy, cut);
...
G4double G4MuBremsstrahlungModel::ComputMuBremLoss(G4double Z, G4double tkin, G4double cut)
{
...
G4double G4MuBremsstrahlungModel::ComputeMicroscopicCrossSection(G4double tkin, G4double Z,
                                                                 G4double cut)
{
...
G4double G4MuBremsstrahlungModel::ComputeDMicroscopicCrossSection(G4double tkin, G4double Z,
...
//  differential cross section
{
  ...
  if (gammaEnergy < epmax1)
  {
    ...
    fe = G4Log(rab2 * mass
               / ((1. + delta * rmass / (CLHEP::electron_mass_c2 * sqrte))
                                                ...
                                                G4double maxEnergy)
{
  ...
  G4ThreeVector gamDir =
    GetAngularDistribution()->SampleDirection(dp, gEnergy, Z, couple->GetMaterial());
  ...
  const G4double totMomentum = std::sqrt(kineticEnergy * (kineticEnergy + 2.0 * mass));
  G4ThreeVector dir = (totMomentum * dp->GetMomentumDirection() - gEnergy * gamDir).unit();
