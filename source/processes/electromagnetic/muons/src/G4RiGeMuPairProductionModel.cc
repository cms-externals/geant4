...
G4RiGeMuPairProductionModel::G4RiGeMuPairProductionModel(const G4ParticleDefinition* p)
  : G4VEmModel("muPairProdRiGe"),
    factorForCross(CLHEP::fine_structure_const * CLHEP::fine_structure_const
                   * CLHEP::classic_electr_radius * CLHEP::classic_electr_radius * 4.
                   / (3. * CLHEP::pi)),
...
G4double G4RiGeMuPairProductionModel::MinPrimaryEnergy(const G4Material*,
                                                       const G4ParticleDefinition*, G4double cut)
{
                                             ...
                                             const G4DataVector& cuts)
{
  ...
  if (lowestKinEnergy >= HighEnergyLimit())
  {
    return;
  }
...
G4double G4RiGeMuPairProductionModel::ComputeMicroscopicCrossSection(G4double tkin, G4double Z,
                                                                     G4double cutEnergy)
{
  G4double cross = 0.;
...
G4double G4RiGeMuPairProductionModel::ComputeDMicroscopicCrossSection(G4double tkin, G4double Z,
                                                                      G4double pairEnergy)
...
// Code modified by R.P. Kokoulin, V.N. Ivanchenko (27/01/04)
{
  ...
  if (pairEnergy <= minPairEnergy) return 0.0;

  G4double totalEnergy = tkin + particleMass;
  G4double residEnergy = totalEnergy - pairEnergy;

  if (residEnergy <= 0.75 * sqrte * z13 * particleMass) return 0.0;
  ...
  for (G4int i = 0; i < NINTPAIR; ++i)
  {
    if (xi[i] <= 1000.0)
    {
      be[i] = ((2.0 + rho2[i]) * (1.0 + beta) + xi[i] * (3.0 + rho2[i])) * G4Log(1.0 + xii[i])
...
{
  G4double cross = 0.0;
  if (kineticEnergy <= lowestKinEnergy)
  {
    return cross;
  }
                                                    ...
                                                    G4double tmin, G4double tmax)
{
  ...
  G4double particleFinalMomentum =
    std::sqrt(particleFinalEnergy * particleFinalEnergy - particleMass * particleMass);
  ...
  G4double maxPairEnergy = MaxSecondaryEnergyForElement(kinEnergy, anElement->GetZ());
  ...
  if (randNumbs[7] < W[0])
  {
    ...
    G4double t1 =
      (-(A + C) + 1. / (1. / (A + C + absB * mint3) - absB * t1interval * randNumbs[0])) / absB;
  ...
  else if (randNumbs[7] >= W[0] && randNumbs[7] < W[1])
  {
    ...
    G4double C =
      -2. * particleMomentum * gMomentum * tQMG - 2. * particleMomentum * particleFinalMomentum;

    G4double absB = std::abs(B);
    G4double t3interval = (1. / (A + C + absB * mint3) - 1. / (A + C + absB * maxt3)) / absB;
    G4double t3 =
      (-(A + C) + 1. / (1. / (A + C + absB * mint3) - absB * t3interval * randNumbs[0])) / absB;
  ...
  else if (randNumbs[7] >= W[1] && randNumbs[7] < W[2])
  {
    ...
    G4double A5 = auxVec1.mag2() - 2. * eEnergy * (kinEnergy - particleFinalEnergy)
                  + 2. * particleMomentumVector[2] * eMomentum
                  - 2. * particleFinalMomentum * eMomentum * cost3;
    G4double B5 =
      -2. * particleFinalMomentum * eMomentum * (sint3 * cosp3 * cosp5 + sint3 * sinp3 * sinp5);
    ...
    G4double t6 =
      (-absA6C6 + 1. / (1. / (absA6C6 + absB6 * mint6) - absB6 * t6interval * randNumbs[8]))
      / absB6;
  ...
  else
  {
    ...
    muFinalMomentumVector.set(particleFinalMomentum * sint3 * cosp3,
                              particleFinalMomentum * sint3 * sinp3, particleFinalMomentum * cost3);
    ...
    G4LorentzVector auxVec1 = muFourMomentum - muFinalFourMomentum;
    G4double A6 = auxVec1.mag2() - 2. * pEnergy * (kinEnergy - particleFinalEnergy)
                  + 2. * particleMomentumVector[2] * pMomentum
                  - 2. * particleFinalMomentum * pMomentum * cost3;
    G4double B6 =
      -2. * particleFinalMomentum * pMomentum * (sint3 * cosp3 * cosp6 + sint3 * sinp3 * sinp6);
    ...
    G4double t5 =
      (-absA5C5 + 1. / (1. / (absA5C5 + absB5 * mint5) - absB5 * t5interval * randNumbs[8]))
      / absB5;
    G4double sint5 = std::sin(t5);
    G4double cost5 = std::cos(t5);
...
G4double G4RiGeMuPairProductionModel::FindScaledEnergy(G4int iz, G4double rand, G4double logTkin,
                                                       G4double yymin, G4double yymax)
{
  ...
  if (nullptr != pv)
  {
    ...
    {
      res = pv->FindLinearX((pmin + rand * (pmax - pmin)) / p0, logTkin);
    }
  }
  else
  {
...
void G4RiGeMuPairProductionModel::DataCorrupted(G4int Z, G4double logTkin) const
{
  ...
  ed << "G4ElementData is not properly initialized Z= " << Z << " Ekin(MeV)= " << G4Exp(logTkin)
     << " IsMasterThread= " << IsMaster() << " Model " << GetName();
...
G4bool G4RiGeMuPairProductionModel::RetrieveTables()
{
  for (G4int iz = 0; iz < NZDATPAIR; ++iz)
  {
    ...
    G4Physics2DVector* pv = new G4Physics2DVector(nbiny + 1, nbine + 1);
    std::ostringstream ss;
    ss << G4EmParameters::Instance()->GetDirLEDATA() << "/mupair/" << particle->GetParticleName()
       << Z << ".dat";
