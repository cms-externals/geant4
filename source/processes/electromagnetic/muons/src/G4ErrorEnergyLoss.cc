...
//-------------------------------------------------------------------
G4ErrorEnergyLoss::G4ErrorEnergyLoss(const G4String& processName, G4ProcessType type)
...
G4VParticleChange* G4ErrorEnergyLoss::AlongStepDoIt(const G4Track& aTrack, const G4Step& aStep)
{
  ...
  {
    kinEnergyEnd = theELossForExtrapolator->EnergyBeforeStep(kinEnergyStart, step_length, aMaterial,
                                                             aParticleDef);
    G4double kinEnergyHalfStep = (kinEnergyStart + kinEnergyEnd) * 0.5;

#ifdef G4VERBOSE
    if (G4ErrorPropagatorData::verbose() >= 3)
      G4cout << " G4ErrorEnergyLoss FWD  end " << kinEnergyEnd << " halfstep " << kinEnergyHalfStep
             << G4endl;
#endif

    //--- rescale to energy lost at 1/2 step
    kinEnergyEnd = theELossForExtrapolator->EnergyBeforeStep(kinEnergyHalfStep, step_length,
  ...
  {
    kinEnergyEnd = theELossForExtrapolator->EnergyAfterStep(kinEnergyStart, step_length, aMaterial,
                                                            aParticleDef);
    G4double kinEnergyHalfStep = (kinEnergyStart + kinEnergyEnd) * 0.5;
#ifdef G4VERBOSE
    if (G4ErrorPropagatorData::verbose() >= 3)
      G4cout << " G4ErrorEnergyLoss BCKD  end " << kinEnergyEnd << " halfstep " << kinEnergyHalfStep
             << G4endl;
#endif

    //--- rescale to energy lost at 1/2 step
    kinEnergyEnd = theELossForExtrapolator->EnergyAfterStep(kinEnergyHalfStep, step_length,
                                                            aMaterial, aParticleDef);
    kinEnergyEnd = kinEnergyStart - (kinEnergyHalfStep - kinEnergyEnd);
  }
  ...
  if (G4ErrorPropagatorData::verbose() >= 2)
    G4cout << "AlongStepDoIt Estart= " << kinEnergyStart << " Eend " << kinEnergyEnd << " Ediff "
           << kinEnergyStart - kinEnergyEnd << " step= " << step_length
           << " mate= " << aMaterial->GetName() << " particle= " << aParticleDef->GetParticleName()
           << G4endl;
#endif
...
//-------------------------------------------------------------------
G4double G4ErrorEnergyLoss::GetContinuousStepLimit(const G4Track& aTrack, G4double, G4double,
                                                   G4double&)
{
  ...
  if (G4ErrorPropagatorData::verbose() >= 2)
  {
    G4cout << " G4ErrorEnergyLoss: limiting Step " << delta << " energy(GeV) " << ekin / CLHEP::GeV
