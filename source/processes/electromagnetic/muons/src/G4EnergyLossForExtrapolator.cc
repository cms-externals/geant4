...
std::size_t G4EnergyLossForExtrapolator::FindIndex(const G4Material* mat)
{
  ...
  for (std::size_t i = 0; i < nCouples; ++i) {
    if (mat == theCoupleTable->GetMaterialCutsCouple((G4int)i)->GetMaterial()) { return i; }
  }
  ...
  G4Exception("G4EnergyLossForExtrapolator::FindIndex","em004",
              FatalException, ed, "");
  return 0;
}
                                          ...
                                          const G4Material* mat)
{
  ...
  if(part == electron) { 
    ...
    G4double q = part->GetPDGCharge()/CLHEP::eplus;
    x = ComputeValue(e, GetPhysicsTable(fRangeProton), idx)
      /(q*q*massratio);
  }
  return x;
}
...
void G4EnergyLossForExtrapolator::Initialisation()
{
  ...
  if (verbose>0) {
    G4cout << "### G4EnergyLossForExtrapolator::Initialisation tables= " 
           << tables << "  initializer:" << isInitializer
