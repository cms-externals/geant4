...
G4MuonToMuonPairProductionModel::G4MuonToMuonPairProductionModel(const G4ParticleDefinition* p,
                                                                 const G4String& nam)
...
G4double G4MuonToMuonPairProductionModel::ComputeDMicroscopicCrossSection(G4double tkin, G4double Z,
                                                                          G4double pairEnergy)
...
G4double G4MuonToMuonPairProductionModel::U_func(G4double ZZ, G4double rho2, G4double xi,
                                                 G4double Y, G4double pairEnergy, const G4double B)
{
  G4int Z = G4lrint(ZZ);
  ...
  G4double res = (0.65 * B / (A27 * Z13) * mueRatio)
                 / (1
                    + (2 * sqe * muonMass * muonMass * (B / Z13) * (1 + xi) * (1 + Y))
                                                        ...
                                                        G4double tmin, G4double tmax)
{
  ...
  //
  GetAngularDistribution()->SamplePairDirections(aDynamicParticle, muMinusEnergy, muPlusEnergy,
  ...
  G4DynamicParticle* aParticle2 = new G4DynamicParticle(theMuonPlus, muPlusDirection, muPlusEnergy);
  // Fill output vector
  ...
  if (pairEnergy > SecondaryThreshold())
  {
    ...
    G4DynamicParticle* newdp = new G4DynamicParticle(particle, partDirection, kinEnergy);
    vdp->push_back(newdp);
  }
  else
