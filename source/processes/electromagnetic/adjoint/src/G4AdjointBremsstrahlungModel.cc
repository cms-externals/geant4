...
G4AdjointBremsstrahlungModel::~G4AdjointBremsstrahlungModel()
{
  if (fEmModelManagerForFwdModels) delete fEmModelManagerForFwdModels;
}

////////////////////////////////////////////////////////////////////////////////
void G4AdjointBremsstrahlungModel::SampleSecondaries(const G4Track& aTrack, G4bool isScatProjToProj,
                                                     G4ParticleChange* fParticleChange)
{
  ...
  G4ThreeVector projectileMomentum =
    G4ThreeVector(std::cos(phi) * sint, std::sin(phi) * sint, cost) * projectileP;  // gamma frame
  if (isScatProjToProj)
  {  // the adjoint primary is the scattered e-
    G4ThreeVector gammaMomentum =
      (projectileTotalEnergy - adjointPrimTotalEnergy) * G4ThreeVector(0., 0., 1.);
                                                          ...
                                                          G4ParticleChange* fParticleChange)
{
  ...
  else
  {
    ...
    diffCSUsed = fLastCZ * adjointPrimKinEnergy / projectileKinEnergy / gammaEnergy;
  }
  ...
  // (example Penelope).
  G4double diffCS =
    DiffCrossSectionPerVolumePrimToSecond(fCurrentMaterial, projectileKinEnergy, gammaEnergy);
  ...
  const G4Element* elm =
    fDirectModel->SelectRandomAtom(fCurrentCouple, fElectron, projectileKinEnergy, fTcutSecond);
  G4int Z = elm->GetZasInt();
  G4double energy = aDynPart->GetTotalEnergy() - gammaEnergy;
  G4ThreeVector projectileMomentum =
    fDirectModel->GetAngularDistribution()->SampleDirection(aDynPart, energy, Z, fCurrentMaterial)
    * projectileP;
...
)
{
  ...
  return G4VEmAdjointModel::DiffCrossSectionPerVolumePrimToSecond(aMaterial, kinEnergyProj,
                                                                  kinEnergyProd);
}
                                                           ...
                                                           G4bool isScatProjToProj)
{
  ...
  fLastCZ = fDirectModel->CrossSectionPerVolume(aCouple->GetMaterial(), fDirectPrimaryPart,
                                                100. * MeV, maxEnergy);
  ...
  else
  {
    ...
    if (Emax_proj > Emin_proj)
      Cross =
        fLastCZ
        * std::log((Emax_proj - primEnergy) * Emin_proj / Emax_proj / (Emin_proj - primEnergy));
  }
  return Cross;
}
