...
////////////////////////////////////////////////////////////////////////////////
G4AdjointComptonModel::G4AdjointComptonModel() : G4VEmAdjointModel("AdjointCompton")
{
  ...
  fDirectModel = new G4KleinNishinaCompton(G4Gamma::Gamma(), "ComptonDirectModel");
}
...
////////////////////////////////////////////////////////////////////////////////
void G4AdjointComptonModel::SampleSecondaries(const G4Track& aTrack, G4bool isScatProjToProj,
                                              G4ParticleChange* fParticleChange)
{
  if (!fUseMatrix) return RapidSampleSecondaries(aTrack, isScatProjToProj, fParticleChange);
  ...
  G4double gammaE1;
  gammaE1 = SampleAdjSecEnergyFromCSMatrix(adjointPrimKinEnergy, isScatProjToProj);

  // gammaE2
  G4double gammaE2 = adjointPrimKinEnergy;
  if (!isScatProjToProj) gammaE2 = gammaE1 - adjointPrimKinEnergy;

  // Cos th
  G4double cos_th = 1. + electron_mass_c2 * (1. / gammaE1 - 1. / gammaE2);
  if (!isScatProjToProj)
  {
    cos_th = (gammaE1 - gammaE2 * cos_th) / theAdjointPrimary->GetTotalMomentum();
  }
  G4double sin_th = 0.;
  if (std::abs(cos_th) > 1.)
  {
  ...
  G4ThreeVector gammaMomentum1 =
    gammaE1 * G4ThreeVector(std::cos(phi) * sin_th, std::sin(phi) * sin_th, cos_th);
  ...
  {  // kill the primary and add a secondary
    ...
    fParticleChange->AddSecondary(new G4DynamicParticle(fAdjEquivDirectPrimPart, gammaMomentum1));
  }
  else
  {
...
////////////////////////////////////////////////////////////////////////////////
void G4AdjointComptonModel::RapidSampleSecondaries(const G4Track& aTrack, G4bool isScatProjToProj,
                                                   G4ParticleChange* fParticleChange)
{
  const G4DynamicParticle* theAdjointPrimary = aTrack.GetDynamicParticle();
  ...
  G4double diffCSUsed = 0.1 * fCurrentMaterial->GetElectronDensity() * twopi_mc2_rcl2;
  G4double gammaE1 = 0.;
  G4double gammaE2 = 0.;
  if (!isScatProjToProj)
  {
    ...
    diffCSUsed = diffCSUsed * (1. + 2. * std::log(1. + electron_mass_c2 / adjointPrimKinEnergy))
                 * adjointPrimKinEnergy / gammaE1 / gammaE2;
  }
  else
  {
    G4double Emax = GetSecondAdjEnergyMaxForScatProjToProj(adjointPrimKinEnergy);
    G4double Emin = GetSecondAdjEnergyMinForScatProjToProj(adjointPrimKinEnergy, fTcutSecond);
    if (Emin >= Emax) return;
  ...
  if (diffCS > 0.) diffCS /= fDirectCS;  // here we have the normalised diffCS
...
G4double G4AdjointComptonModel::DiffCrossSectionPerAtomPrimToSecond(G4double gamEnergy0,
                                                                    ...
                                                                    G4double Z, G4double A)
{
  ...
  if (gamEnergy1 > 0.)
    dSigmadEprod = DiffCrossSectionPerAtomPrimToScatPrim(gamEnergy0, gamEnergy1, Z, A);
  return dSigmadEprod;
}

////////////////////////////////////////////////////////////////////////////////
G4double G4AdjointComptonModel::DiffCrossSectionPerAtomPrimToScatPrim(G4double gamEnergy0,
                                                                      G4double gamEnergy1,
                                                                      G4double Z, G4double)
{
  ...
  G4double CS = std::log(one_plus_two_epsi) * (1. - 2. * (1. + epsilon) / (epsilon * epsilon));
  CS += 4. / epsilon + 0.5 * (1. - 1. / (one_plus_two_epsi * one_plus_two_epsi));
  CS /= epsilon;
  ...
  fDirectCS = fDirectModel->ComputeCrossSectionPerAtom(G4Gamma::Gamma(), gamEnergy0, Z, 0., 0., 0.);
...
G4double G4AdjointComptonModel::GetSecondAdjEnergyMaxForScatProjToProj(G4double primAdjEnergy)
{
  ...
  if (inv_e_max > 0.) e_max = std::min(1. / inv_e_max, e_max);
  return e_max;
}
...
{
  if (fUseMatrix)
    return G4VEmAdjointModel::AdjointCrossSection(aCouple, primEnergy, isScatProjToProj);
  ...
  if (!isScatProjToProj)
  {
    ...
    {
      Cross = 0.1
              * std::log((Emax_proj - G4float(primEnergy)) * Emin_proj / Emax_proj
                         / (Emin_proj - primEnergy))
