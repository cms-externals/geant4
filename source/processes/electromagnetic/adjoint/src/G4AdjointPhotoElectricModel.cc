...
////////////////////////////////////////////////////////////////////////////////
G4AdjointPhotoElectricModel::G4AdjointPhotoElectricModel() : G4VEmAdjointModel("AdjointPEEffect")
...
////////////////////////////////////////////////////////////////////////////////
void G4AdjointPhotoElectricModel::SampleSecondaries(const G4Track& aTrack, G4bool isScatProjToProj,
                                                    G4ParticleChange* fParticleChange)
{
  if (isScatProjToProj) return;
  ...
  fPreStepAdjointCS = fTotAdjointCS;  // The last computed CS was at pre step point
  AdjointCrossSection(aTrack.GetMaterialCutsCouple(), electronEnergy, isScatProjToProj);
  fPostStepAdjointCS = fTotAdjointCS;

  // Sample element
  const G4ElementVector* theElementVector = fCurrentMaterial->GetElementVector();
  ...
  {
    if (rand_CS < fXsec[fIndexElement]) break;
  }
  ...
  {
    if (rand_CS < fShellProb[fIndexElement][i]) break;
  }
  G4double gammaEnergy = electronEnergy + (*theElementVector)[fIndexElement]->GetAtomicShell(i);
  ...
  G4DynamicParticle* anAdjointGamma =
    new G4DynamicParticle(G4AdjointGamma::AdjointGamma(), adjoint_gammaDirection, gammaEnergy);
                                                        ...
                                                        G4double projectileKinEnergy, G4bool)
{
  ...
  G4double w_corr =
    G4AdjointCSManager::GetAdjointCSManager()->GetPostStepWeightCorrection() / fFactorCSBiasing;
                                                          ...
                                                          G4bool isScatProjToProj)
{
  ...
  if (aCouple != fCurrentCouple || fCurrenteEnergy != electronEnergy)
  {
    ...
    const G4ElementVector* theElementVector = fCurrentMaterial->GetElementVector();
    const G4double* theAtomNumDensityVector = fCurrentMaterial->GetVecNbOfAtomsPerVolume();
    size_t nelm = fCurrentMaterial->GetNumberOfElements();
    for (fIndexElement = 0; fIndexElement < nelm; ++fIndexElement)
    {
      fTotAdjointCS +=
        AdjointCrossSectionPerAtom((*theElementVector)[fIndexElement], electronEnergy)
                                                                 ...
                                                                 G4double electronEnergy)
{
  ...
  G4double CS =
    fDirectModel->ComputeCrossSectionPerAtom(G4Gamma::Gamma(), gammaEnergy, Z, 0., 0., 0.);
  ...
  for (G4int i = 1; i < nShells; ++i)
  {
    ...
    if (electronEnergy < Bi1 - Bi)
    {
      ...
      CS = fDirectModel->ComputeCrossSectionPerAtom(G4Gamma::Gamma(), gammaEnergy, Z, 0., 0., 0.);
      if (CS > 0.) adjointCS += CS / gammaEnergy;
    }
