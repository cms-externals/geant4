                                           ...
                                           G4ParticleDefinition* aFwdPartDef)
{
  ...
  if (anAdjPartDef && aProcess)
  {
    ...
    {
      if (anAdjPartDef->GetParticleName() == fAdjointParticlesInAction[i]->GetParticleName())
...
void G4AdjointCSManager::BuildCrossSectionMatrices()
{
  ...
  for (const auto& aModel : fAdjointModels)
  {
    ...
    {
      G4cout << "The model " << aModel->GetName() << " does not use cross section matrices"
             << G4endl;
...
void G4AdjointCSManager::BuildTotalSigmaTables()
{
  ...
  for (std::size_t i = 0; i < fAdjointParticlesInAction.size(); ++i)
  {
    ...
    for (std::size_t j = 0; j < theCoupleTable->GetTableSize(); ++j)
    {
      const G4MaterialCutsCouple* couple = theCoupleTable->GetMaterialCutsCouple((G4int)j);
        ...
        for (std::size_t k = 0; k < fForwardLossProcesses[i]->size(); ++k)
        {
          ...
          {  // e is considered already as the scaled energy
            ...
            G4VEmModel* currentModel =
              (*fForwardLossProcesses[i])[k]->SelectModelForMaterial(e, mat_index);
            G4double chargeSqRatio =
              currentModel->GetChargeSquareRatio(fFwdIon, couple->GetMaterial(), e / fMassRatio);
            (*fForwardLossProcesses[i])[k]->SetDynamicMassCharge(fMassRatio, chargeSqRatio);
          }
      ...
      if (!Emin_found) fEminForFwdSigmaTables[i].push_back(fTmax);
      ...
      for (std::size_t eindex = 0; eindex < fNbins; ++eindex)
      {
        ...
        G4double totCS =
          ComputeTotalAdjointCS(couple, thePartDef,
      ...
      if (!Emin_found) fEminForAdjSigmaTables[i].push_back(fTmax);
                                               ...
                                               const G4MaterialCutsCouple* aCouple)
{
  ...
  return (
    ((*fTotalAdjSigmaTable[fCurrentParticleIndex])[fCurrentMatIndex])->Value(Ekin * fMassRatio));
}
                                               ...
                                               const G4MaterialCutsCouple* aCouple)
{
  ...
  DefineCurrentParticle(aPartDef);
  return (
    ((*fTotalFwdSigmaTable[fCurrentParticleIndex])[fCurrentMatIndex])->Value(Ekin * fMassRatio));
}

///////////////////////////////////////////////////////
G4double G4AdjointCSManager::GetAdjointSigma(G4double Ekin_nuc, std::size_t index_model,
                                             G4bool is_scat_proj_to_proj,
                                             const G4MaterialCutsCouple* aCouple)
{
  DefineCurrentMaterial(aCouple);
  if (is_scat_proj_to_proj)
    return (((*fSigmaTableForAdjointModelScatProjToProj[index_model])[fCurrentMatIndex])
              ->Value(Ekin_nuc));
  else
    return (
      ((*fSigmaTableForAdjointModelProdToProj[index_model])[fCurrentMatIndex])->Value(Ekin_nuc));
}

///////////////////////////////////////////////////////
void G4AdjointCSManager::GetEminForTotalCS(G4ParticleDefinition* aPartDef,
                                           const G4MaterialCutsCouple* aCouple, G4double& emin_adj,
                                           G4double& emin_fwd)
{
  DefineCurrentMaterial(aCouple);
  DefineCurrentParticle(aPartDef);
  emin_adj = fEminForAdjSigmaTables[fCurrentParticleIndex][fCurrentMatIndex] / fMassRatio;
  emin_fwd = fEminForFwdSigmaTables[fCurrentParticleIndex][fCurrentMatIndex] / fMassRatio;
}
                                          ...
                                          G4double& e_sigma_max, G4double& sigma_max)
{
  ...
  sigma_max = ((*fTotalFwdSigmaTable[fCurrentParticleIndex])[fCurrentMatIndex])->Value(e_sigma_max);
                                          ...
                                          G4double& e_sigma_max, G4double& sigma_max)
{
  ...
  sigma_max = ((*fTotalAdjSigmaTable[fCurrentParticleIndex])[fCurrentMatIndex])->Value(e_sigma_max);
  e_sigma_max /= fMassRatio;
}
                                                       ...
                                                       G4bool& fwd_is_used)
{
  ...
  {
    if (lastEkin != PreStepEkin || aPartDef != lastPartDef || aCouple != fCurrentCouple)
    {
      DefineCurrentMaterial(aCouple);
...
///////////////////////////////////////////////////////
G4double G4AdjointCSManager::ComputeAdjointCS(G4Material* aMaterial, G4VEmAdjointModel* aModel,
                                              ...
                                              std::vector<G4double>& CS_Vs_Element)
{
  ...
  if (!need_to_compute)
  {
    ...
    for (std::size_t i = 0; i < fIndexOfAdjointEMModelInAction.size(); ++i)
    {
      ...
      if (aModel == fAdjointModels[ind1] && isScatProjToProj == fIsScatProjToProj[i])
      {
  ...
  if (need_to_compute)
  {
    ...
    if (!aModel->GetUseMatrix())
    {
      CS_Vs_Element.push_back(
        aModel->AdjointCrossSection(fCurrentCouple, PrimEnergy, isScatProjToProj));
    }
    else if (aModel->GetUseMatrixPerElement())
    {
      ...
      if (aModel->GetUseOnlyOneMatrixForAllElements())
      {
        ...
        G4double CS = 0.;
        if (PrimEnergy > Tlow) CS = ComputeAdjointCS(PrimEnergy, theCSMatrix, Tlow);
        G4double factor = 0.;
      ...
      else
      {
        for (G4int i = 0; i < (G4int)n_el; ++i)
        {
          ...
          if (isScatProjToProj)
          {
            theCSMatrix = fAdjointCSMatricesForScatProjToProj[ind_model][ind_el];
          }
          else
                                                           ...
                                                           G4bool isScatProjToProj)
{
  std::vector<G4double> CS_Vs_Element;
  G4double CS =
    ComputeAdjointCS(aMaterial, aModel, PrimEnergy, Tcut, isScatProjToProj, CS_Vs_Element);
  G4double SumCS = 0.;
  std::size_t ind = 0;
    ...
    else
    {
      ...
      if (idx < 56)
      {
        const std::vector<G4double>* aVec =
          G4ProductionCutsTable::GetProductionCutsTable()->GetEnergyCutsVector(idx);
        Tlow = (*aVec)[aCouple->GetIndex()];
      }
    }
    if (Ekin <= adjModel->GetHighEnergyLimit() && Ekin >= adjModel->GetLowEnergyLimit())
    {
      if (aPartDef == adjModel->GetAdjointEquivalentOfDirectPrimaryParticleDefinition())
      {
        CS = ComputeAdjointCS(fCurrentMaterial, adjModel, Ekin, Tlow, true, CS_Vs_Element);
        TotalCS += CS;
        (*fSigmaTableForAdjointModelScatProjToProj[i])[fCurrentMatIndex]->PutValue(fNbins, CS);
      }
      if (aPartDef == adjModel->GetAdjointEquivalentOfDirectSecondaryParticleDefinition())
      {
        CS = ComputeAdjointCS(fCurrentMaterial, adjModel, Ekin, Tlow, false, CS_Vs_Element);
        TotalCS += CS;
        (*fSigmaTableForAdjointModelProdToProj[i])[fCurrentMatIndex]->PutValue(fNbins, CS);
      }
    }
    else
    {
      (*fSigmaTableForAdjointModelScatProjToProj[i])[fCurrentMatIndex]->PutValue(fNbins, 0.);
      (*fSigmaTableForAdjointModelProdToProj[i])[fCurrentMatIndex]->PutValue(fNbins, 0.);
    }
  }
  return TotalCS;
}
...
G4AdjointCSManager::BuildCrossSectionsModelAndElement(G4VEmAdjointModel* aModel, G4int Z, G4int A,
                                                      G4int nbin_pro_decade)
{
  ...
  G4double E2 =
    std::pow(10., double(int(std::log10(EkinMin) * nbin_pro_decade) + 1) / nbin_pro_decade) / dE;
  G4double E1 = EkinMin;
  ...
  while (E1 < EkinMaxForProd)
  {
    ...
    std::vector<std::vector<double>*> aMat =
      aModel->ComputeAdjointCrossSectionVectorPerAtomForSecond(E1, Z, A, nbin_pro_decade);
    if (aMat.size() >= 2)
    {
      ...
      for (std::size_t j = 0; j < log_CSVec->size(); ++j)
      {
        if (j == 0)
          ...
          (*log_CSVec)[j] = std::log(1. - std::exp((*log_CSVec)[j] - log_adjointCS) + 1e-50);
      }
      (*log_CSVec)[log_CSVec->size() - 1] = (*log_CSVec)[log_CSVec->size() - 2] - std::log(1000.);
      theCSMatForProdToProjBackwardScattering->AddData(std::log(E1), log_adjointCS, log_ESecVec,
                                                       log_CSVec, 0);
    }
    E1 = E2;
    E2 *= dE;
  ...
  E2 =
    std::pow(10., G4double(int(std::log10(EkinMin) * nbin_pro_decade) + 1) / nbin_pro_decade) / dE;
  E1 = EkinMin;
  // Loop checking, 07-Aug-2015, Vladimir Ivanchenko
  while (E1 < EkinMaxForScat)
  {
    ...
    std::vector<std::vector<G4double>*> aMat =
      aModel->ComputeAdjointCrossSectionVectorPerAtomForScatProj(E1, Z, A, nbin_pro_decade);
    if (aMat.size() >= 2)
    {
      ...
      (*log_CSVec)[log_CSVec->size() - 1] = (*log_CSVec)[log_CSVec->size() - 2] - std::log(1000.);
      theCSMatForScatProjToProjBackwardScattering->AddData(std::log(E1), log_adjointCS, log_ESecVec,
                                                           log_CSVec, 0);
    }
    E1 = E2;
    E2 *= dE;
...
///////////////////////////////////////////////////////
std::vector<G4AdjointCSMatrix*>
G4AdjointCSManager::BuildCrossSectionsModelAndMaterial(G4VEmAdjointModel* aModel,
                                                       G4Material* aMaterial, G4int nbin_pro_decade)
{
  G4AdjointCSMatrix* theCSMatForProdToProjBackwardScattering = new G4AdjointCSMatrix(false);
  G4AdjointCSMatrix* theCSMatForScatProjToProjBackwardScattering = new G4AdjointCSMatrix(true);
  ...
  G4double EkinMaxForScat = aModel->GetHighEnergyLimit() * 0.999;
  G4double EkinMaxForProd = aModel->GetHighEnergyLimit() * 0.999;
  if (aModel->GetSecondPartOfSameType()) EkinMaxForProd /= 2.;
  ...
  while (E1 < EkinMaxForProd)
  {
    ...
    E1 = std::min(EkinMaxForProd, E1);
    std::vector<std::vector<G4double>*> aMat =
      aModel->ComputeAdjointCrossSectionVectorPerVolumeForSecond(aMaterial, E1, nbin_pro_decade);
    if (aMat.size() >= 2)
    {
      std::vector<G4double>* log_ESecVec = aMat[0];
      std::vector<G4double>* log_CSVec = aMat[1];
      ...
      for (std::size_t j = 0; j < log_CSVec->size(); ++j)
      {
        if (j == 0)
          ...
          (*log_CSVec)[j] = std::log(1. - std::exp((*log_CSVec)[j] - log_adjointCS));
      }
      (*log_CSVec)[log_CSVec->size() - 1] = (*log_CSVec)[log_CSVec->size() - 2] - std::log(1000.);
      theCSMatForProdToProjBackwardScattering->AddData(std::log(E1), log_adjointCS, log_ESecVec,
                                                       log_CSVec, 0);
  ...
  // Scattered projectile to projectile backward scattering
  E2 = std::pow(10., G4double(G4int(std::log10(EkinMin) * nbin_pro_decade) + 1) / nbin_pro_decade)
       / dE;
  E1 = EkinMin;
  while (E1 < EkinMaxForScat)
  {
    ...
    E1 = std::min(EkinMaxForScat, E1);
    std::vector<std::vector<G4double>*> aMat =
      aModel->ComputeAdjointCrossSectionVectorPerVolumeForScatProj(aMaterial, E1, nbin_pro_decade);
    if (aMat.size() >= 2)
    {
      std::vector<G4double>* log_ESecVec = aMat[0];
      std::vector<G4double>* log_CSVec = aMat[1];
...
G4AdjointCSManager::GetAdjointParticleEquivalent(G4ParticleDefinition* theFwdPartDef)
{
...
///////////////////////////////////////////////////////
G4ParticleDefinition*
G4AdjointCSManager::GetForwardParticleEquivalent(G4ParticleDefinition* theAdjPartDef)
{
...
void G4AdjointCSManager::DefineCurrentParticle(const G4ParticleDefinition* aPartDef)
{
  if (aPartDef != fCurrentParticleDef)
  {
    ...
    fMassRatio = 1.;
    if (aPartDef == fAdjIon) fMassRatio = proton_mass_c2 / aPartDef->GetPDGMass();
    fCurrentParticleIndex = 1000000;
    for (std::size_t i = 0; i < fAdjointParticlesInAction.size(); ++i)
    {
      if (aPartDef == fAdjointParticlesInAction[i]) fCurrentParticleIndex = i;
    }
  }
}
...
G4double G4AdjointCSManager::ComputeAdjointCS(G4double aPrimEnergy,
                                              G4AdjointCSMatrix* anAdjointCSMatrix, G4double Tcut)
{
  ...
  std::size_t ind = theInterpolator->FindPositionForLogVector(log_E, *theLogPrimEnergyVector);
  ...
  anAdjointCSMatrix->GetData((G4int)ind, aLogPrimEnergy1, aLogCS1, log01, aLogSecondEnergyVector1,
                             aLogProbVector1, aLogProbVectorIndex1);
  anAdjointCSMatrix->GetData(G4int(ind + 1), aLogPrimEnergy2, aLogCS2, log02,
                             aLogSecondEnergyVector2, aLogProbVector2, aLogProbVectorIndex2);
  if (!(aLogProbVector1 && aLogProbVector2 && aLogSecondEnergyVector1 && aLogSecondEnergyVector2))
  {
    return 0.;
  }
  ...
  {  // case where the Tcut plays a role
    ...
    log_minimum_prob1 = theInterpolator->InterpolateForLogVector(log_Tcut, *aLogSecondEnergyVector1,
                                                                 *aLogProbVector1);
    log_minimum_prob2 = theInterpolator->InterpolateForLogVector(log_Tcut, *aLogSecondEnergyVector2,
                                                                 *aLogProbVector2);
    aLogCS1 += log_minimum_prob1;
    aLogCS2 += log_minimum_prob2;
  }

  G4double log_adjointCS =
    theInterpolator->LinearInterpolation(log_E, aLogPrimEnergy1, aLogPrimEnergy2, aLogCS1, aLogCS2);
