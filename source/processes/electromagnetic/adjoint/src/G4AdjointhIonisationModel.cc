...
////////////////////////////////////////////////////////////////////////////////
void G4AdjointhIonisationModel::SampleSecondaries(const G4Track& aTrack, G4bool isScatProjToProj,
                                                       ...
                                                       G4ParticleChange* fParticleChange)
{
  ...
  G4double eEnergy = 0.;
  G4double newCS = fCurrentMaterial->GetElectronDensity() * twopi_mc2_rcl2 * fMass;
  if (!isScatProjToProj)
  {  // 1/E^2 distribution
  ...
  G4double diffCS_perAtom_Used = twopi_mc2_rcl2 * fMass * adjointPrimKinEnergy / projectileKinEnergy
                                 / projectileKinEnergy / eEnergy / eEnergy;
...
{  // Probably here the Bragg Model should be also used for
  ...
  if (kinEnergyProj > Emin_proj && kinEnergyProj <= Emax_proj)
  {
    ...
    {
      sigma1 = fDirectModel->ComputeCrossSectionPerAtom(fDirectPrimaryPart, kinEnergyProj, Z, A, E1,
                                                        1.e20);
      sigma2 = fDirectModel->ComputeCrossSectionPerAtom(fDirectPrimaryPart, kinEnergyProj, Z, A, E2,
                                                        1.e20);
    }
    else
    {
      sigma1 = fBraggDirectEMModel->ComputeCrossSectionPerAtom(fDirectPrimaryPart, kinEnergyProj, Z,
                                                               A, E1, 1.e20);
      sigma2 = fBraggDirectEMModel->ComputeCrossSectionPerAtom(fDirectPrimaryPart, kinEnergyProj, Z,
                                                               A, E2, 1.e20);
    }
    ...
    if (x > 1.e-6)
    {
      ...
      if (gg > 1.0)
      {
        G4cout << "### G4BetheBlochModel in Adjoint Sim WARNING: g= " << g << G4endl;
        gg = 1.;
      }
      dSigmadEprod *= gg;
    }
                                                        ...
                                                        G4bool isScatProjToProj)
{
  ...
  G4double Cross = fCurrentMaterial->GetElectronDensity() * twopi_mc2_rcl2 * fMass;
  ...
  else
  {
    ...
    G4double Emin_proj = GetSecondAdjEnergyMinForScatProjToProj(primEnergy, fTcutSecond);
...
{
  G4double Tmax =
    primAdjEnergy * fOnePlusRatio2 / (fOneMinusRatio2 - 2. * fMassRatio * primAdjEnergy / fMass);
  return Tmax;
}
...
{
  G4double Tmin = (2. * primAdjEnergy - 4. * fMass
