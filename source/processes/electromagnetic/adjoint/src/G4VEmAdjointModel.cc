...
////////////////////////////////////////////////////////////////////////////////
G4VEmAdjointModel::G4VEmAdjointModel(const G4String& nam) : fName(nam)
{
...
G4double G4VEmAdjointModel::AdjointCrossSection(const G4MaterialCutsCouple* aCouple,
                                                G4double primEnergy, G4bool isScatProjToProj)
{
  DefineCurrentMaterial(aCouple);
  fPreStepEnergy = primEnergy;

  std::vector<G4double>* CS_Vs_Element = &fElementCSProdToProj;
  if (isScatProjToProj) CS_Vs_Element = &fElementCSScatProjToProj;
  fLastCS = fCSManager->ComputeAdjointCS(fCurrentMaterial, this, primEnergy, fTcutSecond,
...
G4double G4VEmAdjointModel::DiffCrossSectionPerAtomPrimToSecond(G4double kinEnergyProj,
                                                                G4double kinEnergyProd, G4double Z,
                                                                G4double A)
{
                                                                  ...
                                                                  G4double Z, G4double A)
{
  ...
  if (kinEnergyProd <= 0.)
    ...
    dSigmadEprod = DiffCrossSectionPerAtomPrimToSecond(kinEnergyProj, kinEnergyProd, Z, A);
  return dSigmadEprod;
}
                                                                  ...
                                                                  G4double kinEnergyProd)
{
  ...
  if (kinEnergyProj > Emin_proj && kinEnergyProj <= Emax_proj)
  {
    ...
    G4double sigma1 =
      fDirectModel->CrossSectionPerVolume(aMaterial, fDirectPrimaryPart, kinEnergyProj, E1, 1.e20);
    G4double sigma2 =
      fDirectModel->CrossSectionPerVolume(aMaterial, fDirectPrimaryPart, kinEnergyProj, E2, 1.e20);
    dSigmadEprod = (sigma1 - sigma2) / (E2 - E1);
  }
  return dSigmadEprod;
}
                                                                    ...
                                                                    G4double kinEnergyScatProj)
{
  ...
  if (kinEnergyProd <= 0.)
    ...
    dSigmadEprod = DiffCrossSectionPerVolumePrimToSecond(aMaterial, kinEnergyProj, kinEnergyProd);
  return dSigmadEprod;
}
...
{
  G4double bias_factor = fCsBiasingFactor * fKinEnergyProdForIntegration / kinEnergyProj;

  if (fUseMatrixPerElement)
  {
    return DiffCrossSectionPerAtomPrimToSecond(kinEnergyProj, fKinEnergyProdForIntegration,
                                               fZSelectedNucleus, fASelectedNucleus)
           * bias_factor;
  }
  else
  {
    return DiffCrossSectionPerVolumePrimToSecond(fSelectedMaterial, kinEnergyProj,
...
{
  G4double bias_factor = fCsBiasingFactor * fKinEnergyScatProjForIntegration / kinEnergyProj;
  if (fUseMatrixPerElement)
  {
    return DiffCrossSectionPerAtomPrimToScatPrim(kinEnergyProj, fKinEnergyScatProjForIntegration,
                                                 fZSelectedNucleus, fASelectedNucleus)
           * bias_factor;
  }
  else
  {
    return DiffCrossSectionPerVolumePrimToScatPrim(fSelectedMaterial, kinEnergyProj,
  ...
  G4int nbin_pro_decade)  // nb bins per order of magnitude of energy
{
  ...
  G4double E2 =
    std::pow(10., G4double(G4int(std::log10(minEProj) * nbin_pro_decade) + 1) / nbin_pro_decade);
  ...
  {
    int_cross_section += integral.Simpson(this, &G4VEmAdjointModel::DiffCrossSectionFunction1, E1,
  ...
  G4int nbin_pro_decade)  // nb bins pro order of magnitude of energy
{
  ...
  {
    dE2 = dE1 * fE;
    int_cross_section += integral.Simpson(this, &G4VEmAdjointModel::DiffCrossSectionFunction2,
...
//////////////////////////////////////////////////////////////////////////////
G4double G4VEmAdjointModel::SampleAdjSecEnergyFromCSMatrix(std::size_t MatrixIndex,
                                                           G4double aPrimEnergy,
                                                           G4bool isScatProjToProj)
{
  G4AdjointCSMatrix* theMatrix = (*fCSMatrixProdToProjBackScat)[MatrixIndex];
  if (isScatProjToProj) theMatrix = (*fCSMatrixProjToProjBackScat)[MatrixIndex];
  ...
  G4int ind =
    (G4int)theInterpolator->FindPositionForLogVector(aLogPrimEnergy, *theLogPrimEnergyVector);
  ...
  theMatrix->GetData(ind, aLogPrimEnergy1, aLogCS1, log01, aLogSecondEnergyVector1, aLogProbVector1,
                     aLogProbVectorIndex1);
  theMatrix->GetData(ind + 1, aLogPrimEnergy2, aLogCS2, log02, aLogSecondEnergyVector2,
                     aLogProbVector2, aLogProbVectorIndex2);
  ...
  {  // case where Tcut plays a role
    ...
    if (Emin < Emax)
    {
      if (fApplyCutInRange)
      {
        if (fSecondPartSameType && fTcutSecond > aPrimEnergy) return aPrimEnergy;
      ...
      dE = std::exp(theInterpolator->LinearInterpolation(aLogPrimEnergy, aLogPrimEnergy1,
                                                         aLogPrimEnergy2, log_dE1, log_dE2));
    }
  ...
  {  // Tcut condition is already full-filled
    ...
    log_E1 =
      theInterpolator->Interpolate(log_rand_var, *aLogProbVector1, *aLogSecondEnergyVector1, "Lin");
    log_E2 =
      theInterpolator->Interpolate(log_rand_var, *aLogProbVector2, *aLogSecondEnergyVector2, "Lin");

    Esec = std::exp(theInterpolator->LinearInterpolation(aLogPrimEnergy, aLogPrimEnergy1,
                                                         aLogPrimEnergy2, log_E1, log_E2));
...
//////////////////////////////////////////////////////////////////////////////
G4double G4VEmAdjointModel::SampleAdjSecEnergyFromCSMatrix(G4double aPrimEnergy,
                                                           G4bool isScatProjToProj)
{
  SelectCSMatrix(isScatProjToProj);
  return SampleAdjSecEnergyFromCSMatrix(fCSMatrixUsed, aPrimEnergy, isScatProjToProj);
}
...
void G4VEmAdjointModel::CorrectPostStepWeight(G4ParticleChange* fParticleChange,
                                              ...
                                              G4double projectileKinEnergy, G4bool isScatProjToProj)
{
  G4double new_weight = old_weight;
  G4double w_corr = fCSManager->GetPostStepWeightCorrection() / fCsBiasingFactor;

  fLastCS = fLastAdjointCSForScatProjToProj;
  if (!isScatProjToProj) fLastCS = fLastAdjointCSForProdToProj;
  if ((adjointPrimKinEnergy - fPreStepEnergy) / fPreStepEnergy > 0.001)
  {
    G4double post_stepCS =
      AdjointCrossSection(fCurrentCouple, adjointPrimKinEnergy, isScatProjToProj);
    if (post_stepCS > 0. && fLastCS > 0.) w_corr *= post_stepCS / fLastCS;
  }
...
//////////////////////////////////////////////////////////////////////////////
G4double G4VEmAdjointModel::GetSecondAdjEnergyMaxForScatProjToProj(G4double kinEnergyScatProj)
{
  G4double maxEProj = GetHighEnergyLimit();
  if (fSecondPartSameType) maxEProj = std::min(kinEnergyScatProj * 2., maxEProj);
  return maxEProj;
}
...
G4double G4VEmAdjointModel::GetSecondAdjEnergyMinForProdToProj(G4double primAdjEnergy)
{
  ...
  if (fSecondPartSameType) minEProj = primAdjEnergy * 2.;
  return minEProj;
}
...
void G4VEmAdjointModel::SetHighEnergyLimit(G4double aVal)
{
  ...
  if (fDirectModel) fDirectModel->SetHighEnergyLimit(aVal);
}
...
void G4VEmAdjointModel::SetLowEnergyLimit(G4double aVal)
{
  ...
  if (fDirectModel) fDirectModel->SetLowEnergyLimit(aVal);
}
