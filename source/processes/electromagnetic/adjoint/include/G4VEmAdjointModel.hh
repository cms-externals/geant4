...
#ifndef G4VEMADJOINTMODEL_HH
#define G4VEMADJOINTMODEL_HH
...
class G4VEmAdjointModel
{
  public:
    ...
    virtual G4double AdjointCrossSection(const G4MaterialCutsCouple* aCouple, G4double primEnergy,
    ...
    virtual G4double GetSecondAdjEnergyMinForScatProjToProj(G4double primAdjEnergy,
                                                            G4double tcut = 0.);
    ...
    ComputeAdjointCrossSectionVectorPerAtomForSecond(G4double kinEnergyProd, G4double Z,
                                                     G4double A = 0., G4int nbin_pro_decade = 10);

    std::vector<std::vector<double>*>
    ComputeAdjointCrossSectionVectorPerAtomForScatProj(G4double kinEnergyProd, G4double Z,
                                                       G4double A = 0., G4int nbin_pro_decade = 10);
    ...
    inline G4ParticleDefinition* GetAdjointEquivalentOfDirectPrimaryParticleDefinition() const
    {
      return fAdjEquivDirectPrimPart;
    }

    inline G4ParticleDefinition* GetAdjointEquivalentOfDirectSecondaryParticleDefinition() const
    {
    ...
    inline void SetSecondPartOfSameType(G4bool aBool) { fSecondPartSameType = aBool; }
    ...
    inline void SetUseMatrixPerElement(G4bool aBool) { fUseMatrixPerElement = aBool; }
    ...
    inline G4bool GetUseOnlyOneMatrixForAllElements() const { return fOneMatrixForAllElements; }

    inline G4bool GetApplyCutInRange() const { return fApplyCutInRange; }

    inline const G4String& GetName() const { return fName; }

    inline virtual void SetCSBiasingFactor(G4double aVal) { fCsBiasingFactor = aVal; }

    inline void SetCorrectWeightForPostStepInModel(G4bool aBool) { fInModelWeightCorr = aBool; }

    inline void SetAdditionalWeightCorrectionFactorForPostStepOutsideModel(G4double factor)
    {
  ...
  protected:
    ...
    G4double SampleAdjSecEnergyFromCSMatrix(std::size_t MatrixIndex, G4double prim_energy,
                                            G4bool isScatProjToProj);
    ...
    virtual G4double SampleAdjSecEnergyFromDiffCrossSectionPerAtom(G4double prim_energy,
                                                                   G4bool isScatProjToProj);

    // Post  Step weight correction
    virtual void CorrectPostStepWeight(G4ParticleChange* fParticleChange, G4double old_weight,
    ...
    G4bool fInModelWeightCorr = false;  // correct_weight_for_post_step_in_model, see [1]
