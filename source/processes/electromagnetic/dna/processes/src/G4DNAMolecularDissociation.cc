  ...
  : G4VITRestDiscreteProcess(processName, type)
{
  ...
  if (verboseLevel > 1)
  {
    G4cout << "G4MolecularDissociationProcess constructor " << "  Name:" << processName << G4endl;
  }
#endif
  ...
  if (pMotherMoleculeDefinition->GetDecayTable() != nullptr)
  {
    ...
    if (fVerbose > 1)
    {
      G4cout << "Decay Process : " << pMotherMolecule->GetName()
             << " (trackID :" << track.GetTrackID() << ") " << pDecayChannel->GetName()
             << " Probability : " << pDecayChannel->GetProbability() << G4endl;
    }
#endif
    ...
    if (nbProducts != 0)
    {
      ...
      if (it != fDisplacementMap.end())
      {
        ...
        motherMoleculeDisplacement = pDisplacer->GetMotherMoleculeDisplacement(pDecayChannel);
      }
      else
      {
      ...
      auto pNavigator =
        G4ITTransportationManager::GetTransportationManager()->GetNavigatorForTracking();
      ...
      for (G4int j = 0; j < nbProducts; j++)
      {
        ...
        // double step =
        pNavigator->CheckNextStep(track.GetPosition(), displacement_direction, mag_displacement,
                                  prNewSafety);

        // if(prNewSafety < mag_displacement || step < mag_displacement)
        mag_displacement = std::min(prNewSafety * 0.8, mag_displacement);

        G4ThreeVector product_pos = track.GetPosition() + displacement_direction * mag_displacement;
        ...
        if (track.GetTouchable()->GetSolid()->Inside(localPoint) != EInside::kInside)
        {
          ...
          auto Motherlogic = track.GetTouchable()->GetVolume()->GetMotherLogical();
          if (Motherlogic != nullptr && Motherlogic->GetMaterial() != WaterMaterial)
          {
            G4ExceptionDescription ED;
            ED << "The decayed product is outside of the volume : "
               << track.GetTouchable()->GetVolume()->GetName()
               << " with material : " << Motherlogic->GetMaterial()->GetName() << G4endl;
            G4Exception("G4DNAMolecularDissociation::DecayIt()", "OUTSIDE_OF_MOTHER_VOLUME",
                        JustWarning, ED);
          }
        }
...
G4double G4DNAMolecularDissociation::PostStepGetPhysicalInteractionLength(const G4Track&, G4double,
                                                                          G4ForceCondition*)
{
