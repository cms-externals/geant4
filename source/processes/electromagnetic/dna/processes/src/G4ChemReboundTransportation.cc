...
G4ChemReboundTransportation::G4ChemReboundTransportation(const G4String& aName,
                                                     const G4DNABoundingBox* pB, G4int verbosity)
                                                            ...
                                                            G4double low)
{
  ...
  G4double truncVal = trunc(abs(nextPos - low) / length);
  G4double h = std::fmod(truncVal, 2);
  return low + h * length + (1 - 2 * h) * abs(std::fmod(nextPos - low, length));
}
//....oooOO0OOooo........oooOO0OOooo........oooOO0OOooo........oooOO0OOooo......
