...
#endif
G4DNAScavengerProcess::G4DNAScavengerProcess(const G4String& aName, const G4DNABoundingBox& box,
                                             G4ProcessType type)
  : G4VITProcess(aName, type), fpBoundingBox(&box)
{
...
void G4DNAScavengerProcess::SetReaction(MolType molConf, Data* pData)
{
  ...
  auto materialConf =
    pData->GetReactant1() == molConf ? pData->GetReactant2() : pData->GetReactant1();
  if (verboseLevel > 0)
  {
                                                                     ...
                                                                     G4ForceCondition* pForceCond)
{
  ...
  std::map<G4double /*Propensity*/, std::pair<MolType, G4double>> ReactionDataMap;
  G4double alpha0 = 0;

  for (const auto& mat_it : MaterialMap)
  {
    auto matConf = mat_it.first;
    G4double numMol = fpScavengerMaterial->GetNumberMoleculePerVolumeUnitForMaterialConf(matConf);
    ...
    G4double propensity = numMol * reactionRate / (fpBoundingBox->Volume() * Avogadro);
    ...
    if (verboseLevel > 1)
    {
      G4cout << " Material of " << matConf->GetName() << " : " << propensity << G4endl;
    }
  ...
  *pForceCond = NotForced;
  ...
  if (verboseLevel > 2)
  {
    G4cout << "Material : " << fpMaterialConf->GetName() << " ID: " << track.GetTrackID()
           << " Track Time : " << track.GetGlobalTime() << " name : " << molConf->GetName()
...
G4VParticleChange* G4DNAScavengerProcess::PostStepDoIt(const G4Track& track, const G4Step& /*step*/)
{
  G4Molecule* molecule = GetMolecule(track);
  auto molConf = molecule->GetMolecularConfiguration();
  ...
  if (verboseLevel > 1)
  {
    G4cout << ">>> Beginning of G4DNAScavengerProcess verbose>>> Returned value : "
           << G4BestUnit(fReturnedValue, "Time") << "molecule: " << molConf->GetName() << G4endl;
  ...
  {
    G4ExceptionDescription exceptionDescription;
    exceptionDescription << "No reaction data for scavenger reaction between : "
                         << fpMaterialConf->GetName() << " + " << molConf->GetName() << G4endl;
    G4Exception("G4DNAScavengerProcess::PostStepDoIt", "G4DNAScavengerProcess0001111",
                FatalErrorInArgument, exceptionDescription);
  }
  ...
  for (G4int j = 0; j < nbSecondaries; ++j)
  {
    ...
    if (isScavenger)
    {
      fpScavengerMaterial->AddNumberMoleculePerVolumeUnitForMaterialConf(product,
                                                                         track.GetGlobalTime());
      continue;
    }
    auto pProduct = new G4Molecule(data->GetProduct(j));
    auto pProductTrack = pProduct->BuildTrack(reactionTime, track.GetPosition());
        ...
        else
        {
          ...
          if (trackSize > 0)
          {
            for (G4int it = 0; it < (G4int)trackSize; ++it)
            {
              ...
              G4cout << GetIT(products.at(it))->GetName() << " (" << products.at(it)->GetTrackID()
                     << ")";
            }
          }
        }
  ...
  fpScavengerMaterial->ReduceNumberMoleculePerVolumeUnitForMaterialConf(fpMaterialConf,
                                                                        reactionTime);
  State(fPreviousTimeAtPreStepPoint) = -1;

  if (fpMaterialConf == fH3Op || fpMaterialConf == fH2O || fpMaterialConf == fHOm)
  {  // these scavengers are not changed
