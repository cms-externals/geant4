...
G4DNABornIonisationModel::G4DNABornIonisationModel(const G4ParticleDefinition*, const G4String& nam)
  : G4VEmModel(nam)
{
...
void G4DNABornIonisationModel::Initialise(const G4ParticleDefinition* p, const G4DataVector&)
{
  ...
  if (p == G4Electron::Electron())
  {
    ...
    fHighEnergy = param->MaxDNAElectronEnergy();
    feLimitEnergy = 19 * CLHEP::eV;
    fAbsorptionEnergy = param->MinDNAElectronEnergy();
  ...
  else if (p == G4Proton::Proton())
  {
    ...
    fHighEnergy = param->MaxDNAProtonEnergy();
  ...
  else
  {
    ...
    G4Exception("G4DNABornIonisationModel::Initialise", "em0003", FatalException, ed,
                " it is not available.");
  }
  ...
  if (verbose > 1)
  {
    G4cout << "Born ionisation model is initialized for " << fParticle->GetParticleName() << G4endl;
  }
}
                                                         ...
                                                         G4double, G4double)
{
  ...
  G4double sigma =
    (e > fLowEnergy) ? xsdata->FindValue(e) : xsdata->FindValue(fLowEnergy) * e / fLowEnergy;
  ...
  if (verbose > 1)
  {
    G4cout << "G4DNABornIonisationModel for " << fParticle->GetParticleName()
           << " Ekin(keV)=" << ekin / CLHEP::keV << " sigma(cm^2)=" << sigma / CLHEP::cm2 << G4endl;
  }
  return sigma;
}
...
void G4DNABornIonisationModel::SampleSecondaries(std::vector<G4DynamicParticle*>* fvect,
                                                 ...
                                                 const G4DynamicParticle* dynParticle, G4double,
                                                 G4double)
{
  ...
  if (fPrimaryEnergy < bindingEnergy)
  {
    return;
  }
  ...
  G4ThreeVector deltaDir = GetAngularDistribution()->SampleDirectionForShell(
    dynParticle, esec, Z, fSelectedShell, couple->GetMaterial());
  ...
  if (scatteredEnergy < -tolerance || exc < -tolerance)
  {
    G4cout << "G4DNABornIonisationModel::SampleSecondaries: "
           ...
           << " Ein(keV)=" << fPrimaryEnergy / CLHEP::keV << "  " << fParticle->GetParticleName()
...
G4int G4DNABornIonisationModel::SelectShell()
{
  ...
  for (G4int i = 0; i < 5; ++i)
  {
    ...
    xs = (e > fLowEnergy) ? ptr->FindValue(e) : ptr->FindValue(fLowEnergy) * e / fLowEnergy;
    sum += xs;
    fTemp[i] = sum;
  }
  sum *= G4UniformRand();
...
G4double G4DNABornIonisationModel::SampleCumulative()
{
  ...
  if (verbose > 1)
  {
    G4cout << "G4DNABornIonisationModel::SampleCumulative: " << fParticle->GetParticleName()
           << " Ekin(keV)=" << fPrimaryEnergy / CLHEP::keV << " Ee(keV)=" << e / CLHEP::keV
           << G4endl;
  }
  return e;
}
...
G4double G4DNABornIonisationModel::SampleDifferential()
{
  ...
  if (verbose > 1)
  {
    G4cout << "G4DNABornIonisationModel::SampleDifferential: " << fParticle->GetParticleName()
           << " Ekin(keV)=" << fPrimaryEnergy / CLHEP::keV << " Ee(keV)=" << e / CLHEP::keV
           << G4endl;
  }
  return e;
