                                                    ...
                                                    const G4bool __alongStepReaction)
{
  ...
  for (; k < 3; ++k)
  {
    postStepSeparation += std::pow(__trackA.GetPosition()[k] - __trackB.GetPosition()[k], 2);
  ...
  if (__alongStepReaction)
  {
    ...
    G4double D = pMoleculeA->GetDiffusionCoefficient() + pMoleculeB->GetDiffusionCoefficient();
    ...
    //===================================
    // Brownian bridge
    G4double __probabiltyOfEncounter =
      G4Exp(-(preStepSeparation - __reactionRadius) * (postStepSeparation - __reactionRadius)
