...
G4DNAELSEPAElasticModel::G4DNAELSEPAElasticModel(const G4ParticleDefinition*, const G4String& nam)
  : G4VEmModel(nam)
{
  verboseLevel = 0;
  ...
  for (G4int i = 0; i < numOfCouples; ++i)
  {
    ...
    {  // Protection: H2O only is available
    ...
    if (verboseLevel > 0)
    {
      G4cout << "ELSEPA Elastic model is constructed for " << material->GetName() << G4endl
             << "Energy range: " << flowEnergyLimit / eV << " eV - " << fhighEnergyLimit / MeV
             << " MeV" << G4endl;
    }
  }
...
{
  if (verboseLevel > 3) G4cout << "Calling G4DNAELSEPAElasticModel::Initialise()" << G4endl;
  ...
  {
    G4Exception("G4DNAELSEPAElasticModel::Initialise", "em0001", FatalException,
  ...
  for (G4int i = 0; i < numOfCouples; ++i)
  {
    ...
    {  // Protection: only for single element
      ...
      if (Z > 0)
      {
        ...
        fpData_Au = new G4DNACrossSectionDataSet(new G4LogLogInterpolation, eV, scaleFactor);
        ...
        if (path == nullptr)
        {
          G4Exception("G4DNAELSEPAElasticModel::Initialise", "em0002", FatalException,
        ...
        eFullFileNameZ << path << "/dna/sigmadiff_cumulated_elastic_e_elsepa_Z" << Z
                       << "_muffintin.dat";
        ...
        {
          G4Exception("G4DNAELSEPAElasticModel::Initialise", "em0003", FatalException,
      ...
      if (material == fpBaseWater && !fpData_H2O)
      {
        if (LowEnergyLimit() < 10 * eV)
        {
          G4cout << "G4DNAELSEPAElasticModel: low energy limit increased from "
                 << LowEnergyLimit() / eV << " eV to " << 10 << " eV" << G4endl;
        ...
        if (HighEnergyLimit() > 10. * MeV)
        {
          G4cout << "G4DNAELSEPAElasticModel: high energy limit decreased from "
                 << HighEnergyLimit() / MeV << " MeV to " << 10. << " MeV" << G4endl;
          SetHighEnergyLimit(10. * MeV);
        }

        G4String fileZElectron("dna/sigma_elastic_e_elsepa_muffin");

        fpData_H2O = new G4DNACrossSectionDataSet(new G4LogLogInterpolation, eV, scaleFactor);
        ...
        if (path == nullptr)
        {
          G4Exception("G4DNAELSEPAElasticModel::Initialise", "em0004", FatalException,
                      "G4LEDATA environment variable not set.");
          return;
        }
        ...
        eFullFileNameZ << path << "/dna/sigmadiff_cumulated_elastic_e_elsepa_muffin.dat";

        std::ifstream eDiffCrossSectionZ(eFullFileNameZ.str().c_str());

        if (!eDiffCrossSectionZ)
          G4Exception("G4DNAELSEPAElasticModel::Initialise", "em0005", FatalException,
                      "Missing data file for cumulated DCS");
    ...
    if (verboseLevel > 2)
      G4cout << "Loaded cross section files of ELSEPA Elastic model for" << material->GetName()
             << G4endl;
  ...
  fpMolDensity = G4DNAMolecularMaterial::Instance()->GetNumMolPerVolTableFor(
                                                        ...
                                                        G4double ekin, G4double, G4double)
{
  if (verboseLevel > 3)
  {
    G4cout << "Calling CrossSectionPerVolume() of G4DNAELSEPAElasticModel" << G4endl;
  }
  ...
  if (nelm == 1)  // Protection: only for single element
  {
    ...
    if (atomicNDensity != 0.0)
    {
      if (ekin < fhighEnergyLimit)
      {
        ...
        if (ekin < 10 * eV)
          sigma = fpData_Au->FindValue(10 * eV);
        else
          sigma = fpData_Au->FindValue(ekin);
      }
    }
    if (verboseLevel > 2)
    {
...
G4double G4DNAELSEPAElasticModel::Theta(G4int Z, G4ParticleDefinition* particleDefinition,
                                        G4double k, G4double integrDiff)
{
  ...
  if (particleDefinition == G4Electron::ElectronDefinition())
  {
    ...
    if (Z == 0)
    {
      e2 = std::upper_bound(eEdummyVec_H2O.begin(), eEdummyVec_H2O.end(), k);
    }
    else if (Z == 79)
    {
      e2 = std::upper_bound(eEdummyVec_Au.begin(), eEdummyVec_Au.end(), k);
    }
    ...
    if (Z == 0)
    {
      cum12 = std::upper_bound(eCum_H2O[(*e1)].begin(), eCum_H2O[(*e1)].end(), integrDiff);
    }
    else if (Z == 79)
    {
      cum12 = std::upper_bound(eCum_Au[(*e1)].begin(), eCum_Au[(*e1)].end(), integrDiff);
    }
    ...
    if (Z == 0)
    {
      cum22 = std::upper_bound(eCum_H2O[(*e2)].begin(), eCum_H2O[(*e2)].end(), integrDiff);
    }
    else if (Z == 79)
    {
      cum22 = std::upper_bound(eCum_Au[(*e2)].begin(), eCum_Au[(*e2)].end(), integrDiff);
    }
  ...
  theta = QuadInterpolator(valuecum11, valuecum12, valuecum21, valuecum22, a11, a12, a21, a22,
                           valueE1, valueE2, k, integrDiff);
  return theta;
}

//....oooOO0OOooo........oooOO0OOooo........oooOO0OOooo........oooOO0OOooo....
//
G4double G4DNAELSEPAElasticModel::LogLinInterpolate(G4double e1, G4double e2, G4double e,
...
G4double G4DNAELSEPAElasticModel::LinLogInterpolate(G4double e1, G4double e2, G4double e,
                                                    G4double xs1, G4double xs2)
{
...
G4double G4DNAELSEPAElasticModel::LinLinInterpolate(G4double e1, G4double e2, G4double e,
                                                    G4double xs1, G4double xs2)
{
  G4double d1 = xs1;
  G4double d2 = xs2;
...
G4double G4DNAELSEPAElasticModel::LogLogInterpolate(G4double e1, G4double e2, G4double e,
                                                    G4double xs1, G4double xs2)
{
...
G4double G4DNAELSEPAElasticModel::QuadInterpolator(G4double cum11, G4double cum12, G4double cum21,
                                                   G4double cum22, G4double a11, G4double a12,
                                                   G4double a21, G4double a22, G4double e1,
                                                   G4double e2, G4double t, G4double cum)
{
  G4double value = 0;
