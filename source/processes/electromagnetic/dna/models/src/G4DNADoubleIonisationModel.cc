...
//------------------------------------------------------------------------------
void setup_rejection_function(G4ParticleDefinition* pdef, const G4double ekin, const G4int shell,
                              FuncParams& par)
{
  // Following values provided by M. Dingfelder (priv. comm)
  const G4double Bj[5] = {12.60 * eV, 14.70 * eV, 18.40 * eV, 32.20 * eV, 540.0 * eV};
  ...
  else
  {
    ...
    max_energy =
      2.0 * electron_mass_c2 * (gamma * gamma - 1.0)
      / (1.0 + 2.0 * gamma * (electron_mass_c2 / mass) + std::pow(electron_mass_c2 / mass, 2.0));
  }

  const auto wmax = max_energy / Bj_energy;
  auto c = wmax * (F2 * wmax + F1 * (2.0 + wmax)) / (2.0 * (1.0 + wmax) * (1.0 + wmax));
...
//------------------------------------------------------------------------------
G4double rejection_function(G4ParticleDefinition* pdef, const G4int shell, const FuncParams& par,
                            G4double proposed_ws)
{
  ...
  auto rejection_term = 1.0 + G4Exp(par.alpha_const * (proposed_ws - par.wc) / par.velocity);
...
G4double proposed_sampled_energy(const FuncParams& par)
{
  ...
  auto proposed_ws = par.c * (par.F1 * par.F1 * par.c + 2.0 * rval * (par.F2 - par.F1));
...
// constructor
G4DNADoubleIonisationModel::G4DNADoubleIonisationModel(const G4ParticleDefinition*,
                                                       const G4String& model_name)
  : G4VEmModel(model_name), is_initialized_(false)
{
                                                           ...
                                                           G4double ekin, G4double, G4double)
{
  if (verbose_level_ > 3)
  {
    G4cout << "Calling G4DNADoubleIonisationModel::CrossSectionPerVolume()" << G4endl;
  }
  ...
  if (ekin <= upp_energy_lim)
  {
    ...
    if (pos == xs_tab_.end())
    {
      G4Exception("G4DNADoubleIonisationModel::CrossSectionPerVolume", "em0002", FatalException,
...
G4double G4DNADoubleIonisationModel::GenerateSecondaries(std::vector<G4DynamicParticle*>* vsec,
                                                         ...
                                                         G4int ioni_shell, G4double& theta,
                                                         G4double& phi, G4double& shell_energy)
{
  ...
  auto sample_electron_direction = [this](const G4DynamicParticle* dp, G4double _ekin2, G4int _Z,
                                          G4int _ioni_shell, const G4MaterialCutsCouple* mcc,
                                          G4double& _theta, G4double& _phi) {
    ...
    {
      locdir = GetAngularDistribution()->SampleDirectionForShell(dp, _ekin2, _Z, _ioni_shell,
                                                                 mcc->GetMaterial());
      _theta = locdir.theta();
      _phi = locdir.phi();
    }
                                                   ...
                                                   G4double)
{
  if (verbose_level_ > 3)
  {
    G4cout << "Calling SampleSecondaries() of G4DNADoubleIonisationModel" << G4endl;
  }
  ...
  {
    G4Exception("G4DNADoubleIonisatioModel::SampleSecondaries()", "em2050", FatalException,
  ...
  mioni_manager_->CreateMultipleIonisedWaterMolecule(eDoubleIonisedMolecule, ioni_shell, the_track);
  // ***************************************************************************
}

//------------------------------------------------------------------------------
G4double G4DNADoubleIonisationModel::RandomizeEjectedElectronEnergy(G4ParticleDefinition* pdef,
                                                                    G4double ekin, G4int shell)
{
  //
...
//------------------------------------------------------------------------------
G4int G4DNADoubleIonisationModel::RandomSelect(G4double ekin, G4double scale_param,
                                               const G4String& pname)
{
  ...
  if (pos == xs_tab_.end())
  {
    G4Exception("G4DNADoubleIonisationModel::RandomSelect", "em0002", FatalException,
                "Model not applicable to particle type.");
  }
  ...
  if (table != nullptr)
  {
    ...
    {
      shell--;
      valuesBuffer[shell] = table->GetComponent((G4int)shell)->FindValue(ekin) * scale_param;
