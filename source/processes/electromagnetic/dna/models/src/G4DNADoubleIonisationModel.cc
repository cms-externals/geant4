                              ...
                              const G4int shell, FuncParams& par)
{
  ...
  if (tau < xxx) {
    ...
    v2 = (0.5 * electron_mass_c2 / Bj_energy)
            * (1.0 - (1.0 / pow((1.0 + (tau / electron_mass_c2)), 2.0)));
    beta2 = 1.0 - 1.0 / pow((1.0 + (tau / electron_mass_c2 / A_ion)), 2.0);
  }

  const auto v = sqrt(v2);
  const auto wc = 4.0 * v2 - 2.0 * v - (Ry / (4.0 * Bj_energy));

  const auto L1 = (C1 * pow(v, D1)) / (1.0 + E1 * pow(v, (D1 + 4.0)));
  const auto L2 = C2 * pow(v, D2);
                            ...
                            const FuncParams& par, G4double proposed_ws)
{
  ...
  if (pdef == G4Proton::ProtonDefinition()) {
    ...
    auto x = 100.0 * sqrt(par.beta_squared) / pow(Z, 0.6666667);
...
G4double proposed_sampled_energy(const FuncParams& par)
{
  ...
  proposed_ws = -par.F1 * par.c + 2.0 * rval + sqrt(proposed_ws);
  ...
  G4double& theta, G4double& phi, G4double& shell_energy)
{
  ...
  auto sample_electron_direction = [this](
    ...
    if (_theta > 0.0) {
      ...
      auto costh = cos(_theta);
      auto sinth = sqrt((1.0 - costh) * (1.0 + costh));
      locdir.set(sinth * cos(_phi), sinth * sin(_phi), costh);
