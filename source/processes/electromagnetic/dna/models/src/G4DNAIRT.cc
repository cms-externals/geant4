    ...
    fReactionSet(nullptr)
{
  ...
  fRCutOff = 1.45 * nm + 2 * std::sqrt(8 * 9.46e9 * nm * nm / s * timeMax);  // 95% confidence level

  erfc = new G4ErrorFunction();
}

G4DNAIRT::G4DNAIRT(G4VDNAReactionModel* pReactionModel) : G4DNAIRT()
{
        ...
        for (int n = 0; n < (int)spaceBinned[ii][jj][kk].size(); n++)
        {
          ...
          if (dt > 0)
          {
            ...
            G4double diffusionCoefficient =
              G4Molecule::GetMolecule(spaceBin[n])->GetDiffusionCoefficient();
          ...
          G4double irt = GetIndependentReactionTime(molConfA, molConfB, r0);
          if (irt >= 0 && irt < timeMax - globalTime)
          {
            irt += globalTime;
            if (irt < minTime) minTime = irt;
#ifdef DEBUG
            G4cout << irt << '\t' << molConfA->GetName() << " " << track->GetTrackID() << '\t'
                   << molConfB->GetName() << " " << spaceBin[n]->GetTrackID() << '\n';
#endif
                                              ...
                                              G4double distance)
{
  ...
  if (reactionType == 1)
  {
    ...
    else
    {
      ...
      b = rc / 4
          * (cosh(rc / (2 * r0)) / sinh(rc / (2 * r0))
             - cosh(rc / (2 * sigma)) / sinh(rc / (2 * sigma)));
...
G4int G4DNAIRT::FindBin(G4int n, G4double xmin, G4double xmax, G4double value)
{
  ...
  if (value <= xmin)
    ...
    bin = G4int(n * (value - xmin)
                / (xmax - xmin));  // bin = 1 + G4int( n * ( value - xmin )/( xmax - xmin ) );
...
G4double G4DNAIRT::SamplePDC(G4double a, G4double b)
{
  ...
  while (true)
  {
    ...
    lambdax =
      std::exp(-b * b / X)
      * (1.0 - a * std::sqrt(CLHEP::pi * X) * erfc->erfcx(b / std::sqrt(X) + a * std::sqrt(X)));

    if ((X <= 2.0 * b / a && U <= lambdax) || (X >= 2.0 * b / a && U * M / X <= lambdax)) break;
                                                           ...
                                                           const G4Track& trackB)
{
  ...
  if (dt != 0 && (D1 + D2) != 0 && r0 != 0)
  {
    ...
    else
    {
      ...
      G4ThreeVector S2 = (r1 + (s12 / s22) * r2)
                         + G4ThreeVector(G4RandGauss::shoot(0, s12 + s22 * s22 / s12),
      ...
      S1.setTheta(
        rad
        * std::acos(1.0
                    + 1. / alpha * std::log(1.0 - G4UniformRand() * (1 - std::exp(-2.0 * alpha)))));
  ...
  if (nbProducts != 0)
  {
    ...
    const G4ThreeVector reactionSite =
      sqrD2 * inv_numerator * trackA.GetPosition() + sqrD1 * inv_numerator * trackB.GetPosition();
    ...
    for (G4int u = 0; u < nbProducts; u++)
    {
      ...
      auto productTrack = product->BuildTrack(globalTime, position[u]);
...
G4DNAIRT::FindReaction(G4ITReactionSet* pReactionSet, const G4double /*currentStepTime*/,
                       const G4double fGlobalTime, const G4bool /*reachedUserStepTimeLimit*/)
{
...
G4bool G4DNAIRT::TestReactibility(const G4Track& /*trackA*/, const G4Track& /*trackB*/,
