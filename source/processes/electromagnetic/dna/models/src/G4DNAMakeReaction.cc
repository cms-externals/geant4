...
G4DNAMakeReaction::G4DNAMakeReaction(G4VDNAReactionModel* pReactionModel) : G4DNAMakeReaction()
{
...
G4bool G4DNAMakeReaction::TestReactibility(const G4Track& /*trackA*/, const G4Track& /*trackB*/,
                                                                    ...
                                                                    const G4Track& trackB)
{
  ...
  {
    fpScavengerMaterial->SetEquilibrium(pReactionData, trackA.GetGlobalTime());
  }
  ...
  if (nbProducts != 0)
  {
    ...
    const G4ThreeVector reactionSite =
      sqrD2 * inv_numerator * tA.GetPosition() + sqrD1 * inv_numerator * tB.GetPosition();
...
void G4DNAMakeReaction::UpdatePositionForReaction(G4Track& trackA, G4Track& trackB)
{
  const auto pMoleculeA = GetMolecule(trackA)->GetMolecularConfiguration();
  const auto pMoleculeB = GetMolecule(trackB)->GetMolecularConfiguration();
  ...
  if (distance == 0)
  {
    ...
    exceptionDescription << "Two particles are overlap: " << GetMolecule(trackA)->GetName()
                         << " and " << GetMolecule(trackB)->GetName() << " at "
                         << trackA.GetPosition();
    G4Exception("G4DNAMakeReaction::PrepareForReaction()", "G4DNAMakeReaction003",
  ...
  if (dt > 0)  // irt > 0
  {
    ...
    G4ThreeVector S2 =
      (p1 + (s12 / s22) * p2)
      + G4ThreeVector(G4RandGauss::shoot(0.0, sigma), G4RandGauss::shoot(0.0, sigma),
    ...
    S1.setTheta(
      rad
      * std::acos(
        1.0 + (1. / alpha) * std::log(1.0 - G4UniformRand() * (1. - std::exp(-2.0 * alpha)))));
...
G4DNAMakeReaction::FindReaction(G4ITReactionSet* pReactionSet, const G4double currentStepTime,
