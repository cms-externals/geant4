...
G4DNADiracRMatrixExcitationModel::G4DNADiracRMatrixExcitationModel(const G4ParticleDefinition*,
                                                                   const G4String& nam)
  : G4VEmModel(nam)
{
                                                  ...
                                                  const G4DataVector& /*cuts*/)
{
  if (verboseLevel > 3)
  {
    G4cout << "Calling G4DNADiracRMatrixExcitationModel::Initialise()" << G4endl;
  }
  ...
  {
    G4Exception("G4DNADiracRMatrixExcitationModel::Initialise", "em0001", FatalException,
  ...
  G4double, G4double)
{
  if (verboseLevel > 3)
  {
    G4cout << "Calling CrossSectionPerVolume() of G4DNADiracRMatrixExcitationModel" << G4endl;
  }
...
void G4DNADiracRMatrixExcitationModel::SampleSecondaries(std::vector<G4DynamicParticle*>* /*fvect*/,
                                                         ...
                                                         G4double, G4double)
{
  if (verboseLevel > 3)
  {
    G4cout << "Calling SampleSecondaries() of G4DNADiracRMatrixExcitationModel" << G4endl;
  }

  G4ParticleDefinition* particle = aDynamicParticle->GetDefinition();
  G4double k = aDynamicParticle->GetKineticEnergy();

  G4int level = RandomSelect(couple->GetMaterial(), particle, k);
  ...
  const G4Material* material, const G4ParticleDefinition* particle, G4double kineticEnergy)
{
  ...
  {
    value = value + GetExtendedPartialCrossSection(material, i, particle, kineticEnergy);
  }
...
G4double G4DNADiracRMatrixExcitationModel::GetExtendedPartialCrossSection(
  const G4Material*, G4int level, const G4ParticleDefinition* particle, G4double kineticEnergy)
{
  G4double value = 0;
  ...
  if (particle->GetParticleName() == "e-")
  {
    ...
    {
      // y = [0]+[1]/pow(x-2,2)
      value = paramFuncTCS_5dto6s1[0]
              + paramFuncTCS_5dto6s1[1] / std::pow(kineticEnergy / eV - paramFuncTCS_5dto6s1[2], 2);
    }
    else if (level == 1)
    {
      // y = [0]+[1]/pow(x-2,2)
      value = paramFuncTCS_5dto6s2[0]
              + paramFuncTCS_5dto6s2[1] / std::pow(kineticEnergy / eV - paramFuncTCS_5dto6s2[2], 2);
    }
    else if (level == 2)
    {
      // y = [0]+[1]*log(x-2)/(x-[2])
      value = paramFuncTCS_6sto6p1[0]
              + paramFuncTCS_6sto6p1[1] * G4Log(kineticEnergy / eV - paramFuncTCS_6sto6p1[2])
                  / (kineticEnergy / eV - paramFuncTCS_6sto6p1[2]);
    }
    else if (level == 3)
    {
      // y = [0]+[1]*log(x-2)/(x-[2])
      value = paramFuncTCS_6sto6p2[0]
              + paramFuncTCS_6sto6p2[1] * G4Log(kineticEnergy / eV - paramFuncTCS_6sto6p2[2])
...
G4int G4DNADiracRMatrixExcitationModel::RandomSelect(const G4Material* material,
                                                     ...
                                                     G4double kineticEnergy)
{
  ...
  {
    --i;
    if ((fLowEnergyLimit <= kineticEnergy) && (kineticEnergy < fExperimentalEnergyLimit))
    {
      valuesBuffer[i] = fTableData->GetComponent(i)->FindValue(kineticEnergy);
    }
    else if ((fExperimentalEnergyLimit <= kineticEnergy) && (kineticEnergy < fHighEnergyLimit))
    {
      valuesBuffer[i] = GetExtendedPartialCrossSection(material, i, particle, kineticEnergy);
    }
