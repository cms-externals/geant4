...
namespace
{
G4Mutex ionRuddDynMutex = G4MUTEX_INITIALIZER;
...
void G4DNARuddIonisationDynamicModel::LoadData()
{
  ...
  xsdata_p = G4MatUtils::BuildExtendedVector(dirpath, filename, 5, 645, CLHEP::eV, scaleFactor);

  filename = "dna/sigma_ionisation_alphaplusplus_rudd.dat";
  xsdata_alpha = G4MatUtils::BuildExtendedVector(dirpath, filename, 5, 540, CLHEP::eV, scaleFactor);
...
void G4DNARuddIonisationDynamicModel::Initialise(const G4ParticleDefinition* p, const G4DataVector&)
{
  ...
  if (!isInitialised)
  {
    ...
    fAbsorptionEnergy = param->MinDNAProtonEnergy();
                                                                ...
                                                                G4double kinE, G4double, G4double)
{
  ...
  G4double sigma = (e > fLowestEnergy)
                     ? xsdata->LogLogValue(e, idx)
                                                        ...
                                                        G4double)
{
  ...
  fKinEnergy = dpart->GetKineticEnergy();
  // ion shoud be stopped - check on kinetic energy and not scaled energy
  if (fKinEnergy <= fAbsorptionEnergy)
  {
    fParticleChangeForGamma->SetProposedKineticEnergy(0.);
    fParticleChangeForGamma->ProposeTrackStatus(fStopButAlive);
    fParticleChangeForGamma->ProposeLocalEnergyDeposit(fKinEnergy);
    return;
  }

  fScaledEnergy = fKinEnergy * fMassRate;
  fSelectedShell = SelectShell();
  G4double bindingEnergy =
    (useDNAWaterStructure) ? waterStructure.IonisationEnergy(fSelectedShell) : Bj[fSelectedShell];

  // Si: additional protection if tcs interpolation method is modified
  if (fKinEnergy < bindingEnergy)
  {
    return;
  }
  ...
  G4int Z = 8;
  G4ThreeVector deltaDir = GetAngularDistribution()->SampleDirectionForShell(
    dpart, esec, Z, fSelectedShell, couple->GetMaterial());
  ...
  G4double scatteredEnergy = fKinEnergy - bindingEnergy - esec;
  if (scatteredEnergy < -tolerance || exc < -tolerance)
  {
    G4cout << "G4DNARuddIonisationDynamicModel::SampleSecondaries: "
           ...
           << " Ein(keV)=" << fKinEnergy / CLHEP::keV << "  " << pd->GetParticleName()
  ...
  {
    fParticleChangeForGamma->SetProposedKineticEnergy(fKinEnergy);
    fParticleChangeForGamma->ProposeLocalEnergyDeposit(fKinEnergy - scatteredEnergy);
  }
...
G4double G4DNARuddIonisationDynamicModel::MaxEnergy()
{
  // kinematic limit
  G4double tau = fKinEnergy / fMass;
...
G4double G4DNARuddIonisationDynamicModel::SampleElectronEnergy()
{
  ...
  if (verbose > 1)
  {
    G4cout << "G4DNARuddIonisationDynamicModel::SampleElectronEnergy: "
           << fParticle->GetParticleName() << " E(keV)=" << fKinEnergy / CLHEP::keV
           << " Escaled(keV)=" << fScaledEnergy / CLHEP::keV << " Ee(keV)=" << e / CLHEP::keV
           << " Emax(keV)=" << emax / CLHEP::keV << " shell=" << fSelectedShell << G4endl;
  }
  return e;
}
...
G4double G4DNARuddIonisationDynamicModel::ProbabilityDensityFunction(G4double e)
{
  ...
  if (isHelium)
  {
    ...
    G4double Zeff =
      2.0
      - (sCoefficient[0] * S_1s(fKinEnergy, energyTransfer, slaterEffectiveCharge[0], 1.)
         + sCoefficient[1] * S_2s(fKinEnergy, energyTransfer, slaterEffectiveCharge[1], 2.)
         + sCoefficient[2] * S_2p(fKinEnergy, energyTransfer, slaterEffectiveCharge[2], 2.));
...
G4double G4DNARuddIonisationDynamicModel::S_1s(G4double kine, G4double energyTransfer,
...
G4double G4DNARuddIonisationDynamicModel::S_2s(G4double kine, G4double energyTransfer,
                                               G4double slaterEffCharge, G4double shellNumber)
{
...
G4double G4DNARuddIonisationDynamicModel::S_2p(G4double kine, G4double energyTransfer,
                                               G4double slaterEffCharge, G4double shellNumber)
{
...
G4double G4DNARuddIonisationDynamicModel::Rh(G4double ekin, G4double etrans, G4double q,
                                             G4double shell)
{
  // The following values are provided by M. Dingfelder (priv. comm)
...
{
  // Correction factor only for hydrogen
  const G4double ln10 = fGpow->logZ(10);
  G4double x = 2.0 * ((G4Log(fKinEnergy / CLHEP::eV) / ln10) - 4.2);
  // The following values are provided by M. Dingfelder (priv. comm)
