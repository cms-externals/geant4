...
G4LEPTSElasticModel::G4LEPTSElasticModel(const G4String& modelName) : G4VLEPTSModel(modelName)
{
  theXSType = XSElastic;
...
void G4LEPTSElasticModel::Initialise(const G4ParticleDefinition* aParticle, const G4DataVector&)
{
  ...
  for (matite = materialTable->begin(); matite != materialTable->end(); matite++)
  {
    ...
    theMassTarget[aMaterial] = theMolecularMass[aMaterial] / (6.02214179e+23 / CLHEP::mole)
                               * CLHEP::c_light * CLHEP::c_light;
    theMassProjectile[aMaterial] = CLHEP::electron_mass_c2;

    if (verboseLevel >= 1)
      G4cout << "Material: " << aMaterial->GetName()
             << " MolecularMass: " << theMolecularMass[aMaterial] / (CLHEP::g / CLHEP::mole)
             << " g/mole "
                                            ...
                                            G4double)
{
...
#ifdef DEBUG_LEPTS
  if (verboseLevel >= 2)
    G4cout << " G4LEPTSElasticModel::SampleSecondaries( P1Dir " << P1Dir << " P0Dir " << P0Dir
           << " ang " << ang << G4endl;
#endif
  ...
  G4double Energylost =
    EnergyTransfer(P0KinEn, ang, theMassTarget[aMaterial], theMassProjectile[aMaterial]);
  if (verboseLevel >= 3)
    G4cout << " ELASTIC Energylost " << Energylost << " = " << P0KinEn << " " << ang << " "
           << theMassTarget[aMaterial] << "  " << theMassProjectile[aMaterial] << G4endl;

  G4double P1KinEn = P0KinEn - Energylost;
  if (verboseLevel >= 3)
    G4cout << " ELASTIC " << P1KinEn << " = " << P0KinEn << " - " << Energylost << G4endl;
#ifdef DEBUG_LEPTS
  if (verboseLevel >= 2)
    G4cout << " G4LEPTSElasticModel::SampleSecondaries( SetProposedKineticEnergy " << P1KinEn << " "
...
#ifdef DEBUG_LEPTS
  if (verboseLevel >= 2)
    G4cout << " G4LEPTSElasticModel::SampleSecondaries( ProposeMomentumDirection "
...
G4double G4LEPTSElasticModel::EnergyTransfer(G4double E, G4double ang, G4double MT, G4double MP)
{
  ...
  G4double W = ((E + MP) * si * si + MT - co * std::sqrt(MT * MT - MP * MP * si * si)) * E
               * (E + 2 * MP) / (std::pow((E + MP + MT), 2) - E * co * co * (E + 2 * MP));
