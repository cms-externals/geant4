...
G4bool G4DNAMultipleIonisationManager::CheckShellEnergy(
    const MultipleIonisedModification mod, const G4double* shell_energy)
{
  G4int num_shells{0};
  switch (mod) {
    case eDoubleIonisedMolecule:
  ...
  const G4String& filepath, G4double Z, G4double A)
{
  ...
  if (!fin.is_open()) {
    G4Exception(
            "G4DNAMultipleIonisationManager::LoadAlphaParam",
            "em0007", FatalException,
...
G4double G4DNAMultipleIonisationManager::GetAlphaParam(const G4double energy) const {
  auto find_lower_bound = [this](G4double e) {
    if(Eion_.empty()){
      G4Exception("G4DNAMultipleIonisationManager::GetAlphaParam","em0007",
                  FatalException,"Eion_.empty()");
    }
    auto low = 0;
