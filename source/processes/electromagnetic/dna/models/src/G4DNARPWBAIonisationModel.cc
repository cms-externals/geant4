...
G4DNARPWBAIonisationModel::G4DNARPWBAIonisationModel(const G4ParticleDefinition*,
                                                     const G4String& nam)
  : G4VEmModel(nam)
{
  // Verbosity scale:
...
G4bool G4DNARPWBAIonisationModel::InEnergyLimit(const G4double& k)
{
  ...
  {
    G4Exception("G4DNARPWBAIonisationModel::InEnergyLimit", "em0102", FatalException,
...
//....oooOO0OOooo........oooOO0OOooo........oooOO0OOooo........oooOO0OOooo....
void G4DNARPWBAIonisationModel::InitialiseForProton(const G4ParticleDefinition* part)
{
  if (part != fProtonDef)
  {
    G4Exception("G4DNARPWBAIonisationModel::InitialiseForProton", "em0002", FatalException,
  ...
  if (LowEnergyLimit() < lowEnergyLimit || HighEnergyLimit() > highEnergyLimit)
  {
    ...
    G4Exception("G4DNARPWBAIonisationModel::InitialiseForProton", "em0004", FatalException, ed);
  }

  fpTotalCrossSection =
    make_unique<G4DNACrossSectionDataSet>(new G4LogLogInterpolation, eV, scaleFactor);
  fpTotalCrossSection->LoadData(fileProton);

  // Final state

  std::ostringstream pFullFileName;
  fasterCode ? pFullFileName << path << "/dna/sigmadiff_cumulated_ionisation_p_RPWBA.dat"
  ...
  if (!pDiffCrossSection)
  {
    ...
    G4Exception("G4DNARPWBAIonisationModel::InitialiseForProton", "em0003", FatalException,
                                           ...
                                           const G4DataVector& /*cuts*/)
{
  ...
  if (verboseLevel > 3)
  {
    G4cout << "Calling G4DNARPWBAIonisationModel::Initialise()" << particle->GetParticleName()
           << G4endl;
  }
  ...
  if (verboseLevel > 0)
  {
    G4cout << "RPWBA ionisation model is initialized " << G4endl
           << "Energy range: " << LowEnergyLimit() / MeV << " MeV - " << HighEnergyLimit() / MeV
...
G4double
G4DNARPWBAIonisationModel::CrossSectionPerVolume(const G4Material* material,
                                                 const G4ParticleDefinition* particleDefinition,
                                                 G4double ekin, G4double, G4double)
{
  if (particleDefinition != fProtonDef)
  {
    G4Exception("G4DNARPWBAIonisationModel::CrossSectionPerVolume", "em0402", FatalException,
                "Model not applicable to particle type.");
  }
  if (verboseLevel > 3)
  {
    G4cout << "Calling CrossSectionPerVolume() of G4DNARPWBAIonisationModel" << G4endl;
  }
  G4double sigma;
  ...
  if (verboseLevel > 2)
  {
    ...
    G4cout << "Kinetic energy(eV)=" << ekin / eV << " particle : " << fProtonDef->GetParticleName()
           << G4endl;
                                                  ...
                                                  G4double)
{
  if (verboseLevel > 3)
  {
    G4cout << "Calling SampleSecondaries() of G4DNARPWBAIonisationModel" << G4endl;
  }
  ...
  if (InEnergyLimit(k))
  {
    ...
    {
      secondaryKinetic = RandomizeEjectedElectronEnergyFromCumulatedDcs(k, ionizationShell);
    }
    ...
    size_t secNumberInit;  // need to know at a certain point the energy of
                           // secondaries
    size_t secNumberFinal;  // So I'll make the diference and then sum the energies
    ...
    {
      G4Exception("G4DNARPWBAIonisatioModel::SampleSecondaries()", "em2050", FatalException,
...
G4double G4DNARPWBAIonisationModel::RandomizeEjectedElectronEnergy(const G4double& k,
                                                                   const G4int& shell)
{
  ...
  for (G4double value = IonisationEnergyInShell; value <= 4. * IonisationEnergyInShell;
       value += 0.1 * eV)
  {
    G4double differentialCrossSection = DifferentialCrossSection(kIneV, value / eV, shell);
  ...
  do
  {
    secondaryElectronKineticEnergy = G4UniformRand() * maximumKineticEnergyTransfer;
             ...
             kIneV, (secondaryElectronKineticEnergy + IonisationEnergyInShell) / eV, shell));

  return secondaryElectronKineticEnergy;
}

//....oooOO0OOooo........oooOO0OOooo........oooOO0OOooo........oooOO0OOooo......
G4double G4DNARPWBAIonisationModel::DifferentialCrossSection(const G4double& kine,
...
G4double G4DNARPWBAIonisationModel::Interpolate(const G4double& e1, const G4double& e2,
                                                const G4double& e, const G4double& xs1,
                                                const G4double& xs2)
{
  G4double value = 0.;
...
G4double G4DNARPWBAIonisationModel::QuadInterpolator(const G4double& e11, const G4double& e12,
                                                     const G4double& e21, const G4double& e22,
                                                     const G4double& xs11, const G4double& xs12,
                                                     const G4double& xs21, const G4double& xs22,
                                                     const G4double& t1, const G4double& t2,
                                                     const G4double& t, const G4double& e)
{
...
G4double G4DNARPWBAIonisationModel::GetPartialCrossSection(const G4Material* /*material*/,
                                                           G4int level,
                                                           ...
                                                           G4double kineticEnergy)
{
  ...
  {
    G4Exception("G4DNARPWBAIonisationModel::GetPartialCrossSection", "em0010", FatalException,
                "Model not applicable to particle type.");
  }
...
G4int G4DNARPWBAIonisationModel::RandomSelect(G4double k)
{
  ...
  {
    G4Exception("G4DNARPWBAIonisationModel::RandomSelect", "em0010", FatalException,
                "Model not applicable to particle type.");
  }
  else
  {
...
G4double
G4DNARPWBAIonisationModel::RandomizeEjectedElectronEnergyFromCumulatedDcs(const G4double& k,
                                                                          const G4int& shell)
{
  G4double random = G4UniformRand();
  G4double secondaryKineticEnergy =
    TransferedEnergy(k / eV, shell, random) * eV - waterStructure.IonisationEnergy(shell);
...
G4double G4DNARPWBAIonisationModel::TransferedEnergy(G4double k, G4int ionizationLevelIndex,
