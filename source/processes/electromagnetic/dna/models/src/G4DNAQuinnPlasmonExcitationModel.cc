...
G4DNAQuinnPlasmonExcitationModel::G4DNAQuinnPlasmonExcitationModel(const G4ParticleDefinition*,
                                                                   const G4String& nam)
  : G4VEmModel(nam)
{
...
G4DNAQuinnPlasmonExcitationModel::~G4DNAQuinnPlasmonExcitationModel() = default;
                                                  ...
                                                  const G4DataVector& /*cuts*/)
{
  ...
  if (verboseLevel > 3)
  {
    G4cout << "Calling G4DNAQuinnPlasmonExcitationModel::Initialise()" << G4endl;
  }
  ...
  {
    G4Exception("G4DNAQuinnPlasmonExcitationModel::Initialise", "em0001", FatalException,
  ...
  for (G4int i = 0; i < numOfCouples; i++)
  {
    ...
    if (nelm == 1)  // Protection: only for single element
    {
      ...
      {
        G4Exception("G4DNAQuinnPlasmonExcitationModel::Initialise", "em0002", FatalException,
                    "The model is not applied for z>100");
      }
    }
  ...
  G4double, G4double)
{
  if (verboseLevel > 3)
  {
    G4cout << "Calling CrossSectionPerVolume() of G4DNAQuinnPlasmonExcitationModel" << G4endl;
  }
...
void G4DNAQuinnPlasmonExcitationModel::SampleSecondaries(std::vector<G4DynamicParticle*>* /*fvect*/,
                                                         ...
                                                         G4double, G4double)
{
  if (verboseLevel > 3)
  {
    G4cout << "Calling SampleSecondaries() of G4DNAQuinnPlasmonExcitationModel" << G4endl;
  }
  ...
  if (particle == G4Electron::ElectronDefinition())
  {
    ...
    G4double omega_p = std::sqrt(
      veDens * std::pow(e, 2)
      / (CLHEP::epsilon0 / (1. / cm) * CLHEP::electron_mass_c2 / (CLHEP::c_squared / cm / cm)));
                                                           ...
                                                           G4double kineticEnergy)
{
  ...
  if (particle == G4Electron::ElectronDefinition())
  {
    ...
    G4double p0 = sqrt(2 * CLHEP::electron_mass_c2 / (CLHEP::c_squared / cm / cm) * fEnergy);

    G4double p = sqrt(2 * CLHEP::electron_mass_c2 / (CLHEP::c_squared / cm / cm) * kineticEnergy);

    G4double mfp =
      2 * CLHEP::Bohr_radius / cm * kineticEnergy / (CLHEP::hbar_Planck * omega_p)
      / (G4Log((std::pow(std::pow(p0, 2)
                           + 2 * CLHEP::electron_mass_c2 / (CLHEP::c_squared / cm / cm) * omega_p
                               * CLHEP::hbar_Planck,
                         1. / 2.)
                - p0)
               / (p
                  - std::pow(std::pow(p, 2)
                               - 2 * CLHEP::electron_mass_c2 / (CLHEP::c_squared / cm / cm)
                                   * omega_p * CLHEP::hbar_Planck,
                             1. / 2.))));
...
G4int G4DNAQuinnPlasmonExcitationModel::GetNValenceElectron(G4int z)
{
  ...
  if (verboseLevel > 3)
  {
    G4cout << "Calling GetNValenceElectron() of G4DNAQuinnPlasmonExcitationModel" << G4endl;
  }
  ...
  if (datadir == nullptr)
  {
    ...
    if (datadir == nullptr)
    {
      G4Exception("G4DNAQuinnPlasmonExcitationModel::GetNValenceElectron()", "em0002",
                  FatalException, "Enviroment variable G4LEDATA not defined");
      return 0;
    }
  }
  ...
  if (!fin)
  {
    ...
    G4Exception("G4DNAQuinnPlasmonExcitationModel::GetNValenceElectron()", "em0003", FatalException,
