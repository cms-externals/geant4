                                                           ...
                                                           const G4DataVector& /*cuts*/)
{
  ...
  if (verboseLevel > 1)
  {
    G4cout << "G4DNAUeharaScreenedRutherfordElasticModel::Initialise()" << G4endl;
    G4cout << "Energy range: " << LowEnergyLimit() / eV << " eV - " << HighEnergyLimit() / MeV
           << " MeV" << G4endl;
  }
...
G4double G4DNAUeharaScreenedRutherfordElasticModel::CrossSectionPerVolume(
  ...
  G4double, G4double)
{
  ...
  if (verboseLevel > 3)
  {
    G4cout << "Calling CrossSectionPerVolume() of G4DNAUeharaScreenedRutherfordElasticModel"
           << G4endl;
  }
#endif
  ...
  G4double sigma = 0.;
  if (ekin < iLowEnergyLimit || ekin > iHighEnergyLimit)
  {
    return sigma;
  }
...
G4double G4DNAUeharaScreenedRutherfordElasticModel::RutherfordCrossSection(G4double k, G4double z)
{
  //
  //                               e^4         /      K + m_e c^2      \^2
  // sigma_Ruth(K) = Z (Z+1) -------------------- | --------------------- |
...
G4double G4DNAUeharaScreenedRutherfordElasticModel::ScreeningFactor(G4double k, G4double z)
{
  ...
  G4double value = 0.;
  if (denominator > 0.) value = numerator / denominator;  // form 3
...
void G4DNAUeharaScreenedRutherfordElasticModel::SampleSecondaries(
  ...
  const G4DynamicParticle* aDynamicElectron, G4double, G4double)
{
  ...
  if (verboseLevel > 3)
  {
    G4cout << "Calling SampleSecondaries() of G4DNAUeharaScreenedRutherfordElasticModel" << G4endl;
  }
#endif

  G4double electronEnergy0 = aDynamicElectron->GetKineticEnergy();
  if (electronEnergy0 < iLowEnergyLimit || electronEnergy0 > iHighEnergyLimit) return;
...
G4double G4DNAUeharaScreenedRutherfordElasticModel::BrennerZaiderRandomizeCosTheta(G4double k)
{
  ...
  value = value + (1./(leftDenominator*leftDenominator) + beta/(rightDenominator*rightDenominator))
  / cumul; if (random < value) break;
  }
