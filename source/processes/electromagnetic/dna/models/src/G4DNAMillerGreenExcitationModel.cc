...
G4DNAMillerGreenExcitationModel::~G4DNAMillerGreenExcitationModel() = default;
...
{
  if (verboseLevel > 3) G4cout << "Calling G4DNAMillerGreenExcitationModel::Initialise()" << G4endl;
...
G4double G4DNAMillerGreenExcitationModel::CrossSectionPerVolume(
  ...
  G4double)
{
  ...
  if (verboseLevel > 2)
  {
    ...
    G4cout << "Cross section per water molecule (cm^-1)=" << crossSection * waterDensity / (1. / cm)
           << G4endl;
                                                        ...
                                                        G4double, G4double)
{
  ...
  if (!statCode)
    newEnergy = particleEnergy0 - excitationEnergy;

  else
    newEnergy = particleEnergy0;
...
G4double G4DNAMillerGreenExcitationModel::GetPartialCrossSection(
  const G4Material*, G4int level, const G4ParticleDefinition* particleDefinition,
...
G4double
G4DNAMillerGreenExcitationModel::PartialCrossSection(G4double k, G4int excitationLevel,
                                                     const G4ParticleDefinition* particleDefinition)
{
  ...
  zEff -= (sCoefficient[0][particleTypeIndex]
             * S_1s(k, Eliq[excitationLevel], slaterEffectiveCharge[0][particleTypeIndex], 1.)
           + sCoefficient[1][particleTypeIndex]
               * S_2s(k, Eliq[excitationLevel], slaterEffectiveCharge[1][particleTypeIndex], 2.)
           + sCoefficient[2][particleTypeIndex]
               * S_2p(k, Eliq[excitationLevel], slaterEffectiveCharge[2][particleTypeIndex], 2.));
...
G4int G4DNAMillerGreenExcitationModel::RandomSelect(G4double k,
                                                    const G4ParticleDefinition* particle)
{
  ...
  if (particle == alphaPlusPlusDef || particle == protonDef || particle == hydrogenDef
      || particle == alphaPlusDef || particle == heliumDef)
  {
    while (i > 0)
    {
      i--;
     ...
     )
  {
    while (i>0)
    {
      ...
      if (k*0.511/3728 > 8.23*eV && k*0.511/3728 < 10*MeV ) sigmaExcitation =
  excitationXS->PartialCrossSection(k*0.511/3728,i);
...
G4double G4DNAMillerGreenExcitationModel::S_1s(G4double t, G4double energyTransferred,
...
G4double G4DNAMillerGreenExcitationModel::S_2s(G4double t, G4double energyTransferred,
                                               G4double _slaterEffectiveCharge,
                                               G4double shellNumber)
{
...
G4double G4DNAMillerGreenExcitationModel::S_2p(G4double t, G4double energyTransferred,
                                               G4double _slaterEffectiveCharge,
                                               G4double shellNumber)
{
...
G4double G4DNAMillerGreenExcitationModel::R(G4double t, G4double energyTransferred,
                                            G4double _slaterEffectiveCharge, G4double shellNumber)
{
  ...
  G4double value = std::sqrt(2. * tElectron / H) / (energyTransferred / H)
                   * (_slaterEffectiveCharge / shellNumber);
