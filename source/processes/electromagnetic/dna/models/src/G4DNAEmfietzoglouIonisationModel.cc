...
{
  // Protection to force fasterCode
  fasterCode = true;
  ...
  G4String fileElectron("dna/sigma_ionisation_e_emfietzoglou-x");
  ...
  if (fasterCode)
    eFullFileName << path << "/dna/sigmadiff_cumulated_ionisation_e_emfietzoglou-x.dat";
  if (!fasterCode) eFullFileName << path << "/dna/sigmadiff_ionisation_e_emfietzoglou-x.dat";
  ...
  {
    if (fasterCode)
      G4Exception("G4DNAEmfietzoglouIonisationModel::Initialise", "em0003", FatalException,
                  "Missing data file:/dna/sigmadiff_cumulated_ionisation_e_emfietzoglou-x.dat");

    if (!fasterCode)
      G4Exception("G4DNAEmfietzoglouIonisationModel::Initialise", "em0003", FatalException,
                  "Missing data file:/dna/sigmadiff_ionisation_e_emfietzoglou-x.dat");
  }
...
G4double G4DNAEmfietzoglouIonisationModel::CrossSectionPerVolume(
  const G4Material* material, const G4ParticleDefinition* particleDefinition, G4double ekin,
  G4double, G4double)
{
  if (verboseLevel > 3)
  {
    G4cout << "Calling CrossSectionPerVolume() of G4DNAEmfietzoglouIonisationModel" << G4endl;
  }
                                                         ...
                                                         G4double, G4double)
{
  if (verboseLevel > 3)
  {
    G4cout << "Calling SampleSecondaries() of G4DNAEmfietzoglouIonisationModel" << G4endl;
  }
...
G4double G4DNAEmfietzoglouIonisationModel::RandomizeEjectedElectronEnergy(
...
/*
 void G4DNAEmfietzoglouIonisationModel::RandomizeEjectedElectronDirection(G4ParticleDefinition*
...
//....oooOO0OOooo........oooOO0OOooo........oooOO0OOooo........oooOO0OOooo......
G4double
G4DNAEmfietzoglouIonisationModel::DifferentialCrossSection(G4ParticleDefinition* particleDefinition,
...
G4double G4DNAEmfietzoglouIonisationModel::Interpolate(G4double e1, G4double e2, G4double e,
...
G4double G4DNAEmfietzoglouIonisationModel::QuadInterpolator(
...
G4int G4DNAEmfietzoglouIonisationModel::RandomSelect(G4double k, const G4String& particle)
{
  G4int level = 0;
  ...
  {
    G4Exception("G4DNAEmfietzoglouIonisationModel::RandomSelect", "em0002", FatalException,
...
G4double G4DNAEmfietzoglouIonisationModel::RandomizeEjectedElectronEnergyFromCumulatedDcs(
  G4ParticleDefinition* particleDefinition, G4double k, G4int shell)
{
  ...
  secondaryElectronKineticEnergy = RandomTransferedEnergy(particleDefinition, k / eV, shell) * eV
...
G4double
G4DNAEmfietzoglouIonisationModel::RandomTransferedEnergy(G4ParticleDefinition* particleDefinition,
