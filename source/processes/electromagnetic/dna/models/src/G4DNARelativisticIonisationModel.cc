...
G4DNARelativisticIonisationModel::G4DNARelativisticIonisationModel(const G4ParticleDefinition*,
                                                                   const G4String& nam)
...
G4DNARelativisticIonisationModel::~G4DNARelativisticIonisationModel() = default;
                                                  ...
                                                  const G4DataVector& /*cuts*/)
{
  if (verboseLevel > 3)
  {
    G4cout << "Calling G4DNARelativisticIonisationModel::Initialise()" << G4endl;
  }

  if (fParticleDefinition != nullptr && fParticleDefinition != particle)
  {
    G4Exception("G4DNARelativisticIonisationModel::Initialise", "em0001", FatalException,
  ...
  if (particle == electronDef)
  {
    ...
    if (path == nullptr)
    {
      G4Exception("G4DNARelativisticIonisationModel::Initialise", "em0006", FatalException,
                  "G4LEDATA environment variable not set.");
      return;
    }

    G4ProductionCutsTable* coupletable = G4ProductionCutsTable::GetProductionCutsTable();
    ...
    for (G4int i = 0; i < Ncouple; ++i)
    {
      ...
      const G4Material* material = couple->GetMaterial();
      {
        ...
        eFullFileNameZ << path << "/dna/sigmadiff_cumulated_ionisation_e_RBEBV_Z" << Z << ".dat";
        std::ifstream eDiffCrossSectionZ(eFullFileNameZ.str().c_str());
        if (!eDiffCrossSectionZ)
          G4Exception("G4DNARelativisticIonisationModel::Initialise", "em0003", FatalException,
        ...
        while (!eDiffCrossSectionZ.eof())
        {
          ...
          {
            eDiffCrossSectionZ >> eDiffCrossSectionDataZ[Z][istate][tDummy][eDummy];
            eEjectedEnergyDataZ[Z][istate][tDummy]
                               [eDiffCrossSectionDataZ[Z][istate][tDummy][eDummy]] = eDummy;
  ...
  {
    G4cout << "Error : No particle Definition is found in G4DNARelativisticIonisationModel"
           << G4endl;
    return;
  }
  ...
  fMaterialDensity =
    G4DNAMolecularMaterial::Instance()->GetNumMolPerVolTableFor(G4Material::GetMaterial("G4_Au"));
...
G4double G4DNARelativisticIonisationModel::CrossSectionPerVolume(
  const G4Material* material, const G4ParticleDefinition* particleDefinition, G4double ekin,
  G4double, G4double)
{
  if (verboseLevel > 3)
  {
    G4cout << "Calling CrossSectionPerVolume() of G4DNARelativisticIonisationModel" << G4endl;
  }
  ...
  if (atomicNDensity != 0.0)
  {
    ...
    if (verboseLevel > 2)
    {
      ...
      G4cout << "=== Cross section per atom for Z=" << z << " is (cm^2)" << sigma / cm / cm
             << G4endl;
      G4cout << "=== Cross section per atom for Z=" << z
             << " is (cm^-1)=" << sigma * atomicNDensity / (1. / cm) << G4endl;
                                                         ...
                                                         G4double, G4double)
{
  if (verboseLevel > 3)
  {
    G4cout << "Calling SampleSecondaries() of G4DNARelativisticIonisationModel" << G4endl;
  }
  ...
  if (fLowEnergyLimit <= k && k < fHighEnergyLimit)
  {
    ...
    ejectedE = GetEjectedElectronEnergy(material, particleDef, k, level);
    G4ThreeVector ejectedDir = GetEjectedElectronDirection(particleDef, k, ejectedE);
    ...
    for (G4int iparticle = NumSecParticlesInit; iparticle < NumSecParticlesFinal; iparticle++)
    {
    ...
    {
      G4Exception("G4DNARelativisticIonisationModel::SampleSecondaries()", "em0008", FatalException,
    ...
    if (ejectedE > 0)
    {
      auto ejectedelectron = new G4DynamicParticle(G4Electron::Electron(), ejectedDir, ejectedE);
...
void G4DNARelativisticIonisationModel::LoadAtomicStates(G4int z, const char* path)
{
  if (verboseLevel > 3)
  {
    G4cout << "Calling LoadAtomicStates() of G4DNARelativisticIonisationModel" << G4endl;
  }
  ...
  if (datadir == nullptr)
  {
    ...
    if (datadir == nullptr)
    {
      G4Exception("G4DNARelativisticIonisationModel::LoadAtomicStates()", "em0002", FatalException,
  ...
  while (true)
  {
    ...
    if (!fin.eof())
    {
      ...
      {  // if there is no kinetic energy in the file, kinetic energy
        ...
        G4double radius = std::pow(iShell[z].at(iline), 2) * std::pow(CLHEP::hbar_Planck, 2)
                          * (4 * CLHEP::pi * CLHEP::epsilon0) / CLHEP::electron_mass_c2;
...
G4double G4DNARelativisticIonisationModel::GetTotalCrossSection(
  const G4Material* material, const G4ParticleDefinition* particle, G4double kineticEnergy)
{
  G4double value = 0;
  G4int z = material->GetZ();
  if (z != 79)
...
G4double
G4DNARelativisticIonisationModel::GetPartialCrossSection(const G4Material* material, G4int level,
                                                         ...
                                                         G4double kineticEnergy)
{
  ...
  if (particle == electronDef)
  {
    ...
    G4double phi =
      std::cos(std::sqrt(std::pow(alpha, 2) / (beta_t2 + beta_b2)) * G4Log(beta_t2 / beta_b2));
    G4double constS = 4 * CLHEP::pi * std::pow(CLHEP::Bohr_radius, 2) * Nelectrons[z].at(level)
                      * std::pow(alpha, 4);
    ...
    if (Ebinding[z].at(level) <= kineticEnergy)
    {
      value = constS / ((beta_t2 + (beta_u2 + beta_b2) / iShell[z].at(level)) * 2. * bdash)
              * (1. / 2. * (G4Log(beta_t2 / (1. - beta_t2)) - beta_t2 - G4Log(2. * bdash))
                   * (1. - 1. / std::pow(t, 2.))
                 + 1. - 1. / t
                 - G4Log(t) / (t + 1.) * (1. + 2. * tdash) / (std::pow(1. + tdash / 2., 2.)) * phi
                 + std::pow(bdash, 2) / (std::pow(1 + tdash / 2., 2)) * (t - 1) / 2.);
    }
  }
  return value;
}
...
G4double G4DNARelativisticIonisationModel::GetDifferentialCrossSection(
  const G4Material* material, const G4ParticleDefinition* particle, G4double kineticEnergy,
  G4double secondaryEnergy, G4int level)
{
  G4double value = 0.;
  G4double constRy = 13.6057E-6;  // MeV
...
G4double G4DNARelativisticIonisationModel::GetEjectedElectronEnergy(
  const G4Material* material, const G4ParticleDefinition* particle, G4double energy, G4int ishell)
{
  ...
  {  // for 2D rejection method
    if (particle == electronDef)
    {
      ...
      maximumCrossSection = GetDifferentialCrossSection(material, particle, energy, 0., ishell);
      do
      {
        secondaryEnergy = G4UniformRand() * maximumsecondaryEnergy;
      } while (G4UniformRand() * maximumCrossSection
               > GetDifferentialCrossSection(material, particle, energy, secondaryEnergy, ishell));
    }
  }
  else
  {  // for cumulative method using cumulated DCS file
    ...
    if (particle == G4Electron::ElectronDefinition())
    {
      ...
      {
        auto k2 = std::upper_bound(eVecEZ[z].begin(), eVecEZ[z].end(), energy);
        auto k1 = k2 - 1;

        if (random < eProbaShellMapZ[z][ishell][(*k1)].back()
            && random < eProbaShellMapZ[z][ishell][(*k2)].back())
        {
          auto xs12 = std::upper_bound(eProbaShellMapZ[z][ishell][(*k1)].begin(),
                                       eProbaShellMapZ[z][ishell][(*k1)].end(), random);
          auto xs11 = xs12 - 1;

          auto xs22 = std::upper_bound(eProbaShellMapZ[z][ishell][(*k2)].begin(),
          ...
          secondaryEnergy =
            QuadInterpolator(valueXS11, valueXS12, valueXS21, valueXS22, ejeE11, ejeE12, ejeE21,
                             ejeE22, valueE1, valueE2, energy, random);
        }
      }
    }
  }
...
G4ThreeVector G4DNARelativisticIonisationModel::GetEjectedElectronDirection(
  const G4ParticleDefinition*, G4double energy, G4double secondaryenergy)
{
...
G4double G4DNARelativisticIonisationModel::Interpolate(G4double e1, G4double e2, G4double e,
...
G4double G4DNARelativisticIonisationModel::QuadInterpolator(
  G4double e11, G4double e12, G4double e21, G4double e22, G4double xs11, G4double xs12,
  G4double xs21, G4double xs22, G4double t1, G4double t2, G4double t, G4double e)
{
