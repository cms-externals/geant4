...
G4bool G4DNAMolecularReaction::TestReactibility(const G4Track& trackA, const G4Track& trackB,
                                                ...
                                                bool userStepTimeLimit) /*const*/
{
  ...
  G4bool output = fpReactionModel->FindReaction(trackA, trackB, reactionRadius, separationDistance,
                                                userStepTimeLimit);
  return output;
}
...
G4DNAMolecularReaction::FindReaction(G4ITReactionSet* pReactionSet, const double currentStepTime,
                                     ...
                                     const bool reachedUserStepTimeLimit)
{
  ...
  for (auto tracks_i = reactionPerTrackMap.cbegin(); tracks_i != reactionPerTrackMap.cend();
       tracks_i = reactionPerTrackMap.cbegin())
  {
    ...
    for (auto it = reactionList.cbegin(); it != reactionList.cend(); it = reactionList.cbegin())
    {
      ...
      if (pTrackB == pTrackA)
      {
        ...
        G4Exception("G4ITModelProcessor::FindReaction", "ITModelProcessor005", FatalErrorInArgument,
                    exceptionDescription);
      }
