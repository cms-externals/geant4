...
void G4LEPTSDiffXS::readDXS()
{
  ...
  if (strcmp(DXSTypeName, "KTC") == 0)
    DXSType = 2;  // read DXS & calculate KT
  else if (strcmp(DXSTypeName, "KT") == 0)
    DXSType = 1;  // read DXS & KT
  else
    DXSType = 0;

  //  if( verboseLevel >= 1 ) G4cout << "Read DXS   (" << fileName  <<  ")\t NEg " << NumEn << "
  //  NAng " << NumAng
  ...
  else
  {
    ...
    for (G4int aBin = 0; aBin < NumAng; aBin++)
    {
      for (G4int eBin = 1; eBin <= NumEn; eBin++)
      {
        ...
        KT[eBin][aBin] =
          p * std::sqrt(2. - 2. * std::cos(A * CLHEP::twopi / 360.));  // Mom. Transfer
  ...
  for (G4int aBin = 0; aBin < NumAng - 1; aBin++)
  {
    ...
    for (G4double x = x1; x < (x2 - dx / 10); x += dx)
    {
      for (G4int eBin = 0; eBin <= NumEn; eBin++)
      {
        ...
        {  // log-log ordenada
          ICDXS[eBin][ia] =
            G4Exp((std::log(y1) * std::log(x2 / x) + std::log(y2) * std::log(x / x1))
                  / std::log(x2 / x1));
        }
...
G4double G4LEPTSDiffXS::SampleAngle(G4double Energy)
{
  ...
  if (Energy > Eb[NumEn])
    Ebin = NumEn;
  else if (Energy > (Eb[Ebin] + Eb[Ebin + 1]) * 0.5)
    Ebin++;
