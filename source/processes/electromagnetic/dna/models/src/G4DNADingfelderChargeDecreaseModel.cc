...
G4double G4DNADingfelderChargeDecreaseModel::CrossSectionPerVolume(
  ...
  G4double)
{
  ...
  if (particleDefinition != protonDef && particleDefinition != alphaPlusPlusDef
...
void G4DNADingfelderChargeDecreaseModel::SampleSecondaries(
  ...
  const G4DynamicParticle* aDynamicParticle, G4double, G4double)
{
  ...
  G4double outgoingParticleBindingEnergy =
    OutgoingParticleBindingEnergyConstant(definition, finalStateIndex);
  ...
  {
    if (!statCode)
      outK = inK - n * (inK * electron_mass_c2 / proton_mass_c2) - waterBindingEnergy
             + outgoingParticleBindingEnergy;
    else
      outK = inK;
  }
  ...
  {
    if (!statCode)
      outK = inK - n * (inK * electron_mass_c2 / particleMass) - waterBindingEnergy
             + outgoingParticleBindingEnergy;
    else
      outK = inK;
  }
  ...
  if (outK < 0)
  {
    G4Exception("G4DNADingfelderChargeDecreaseModel::SampleSecondaries", "em0004", FatalException,
...

  {
    if (definition == G4Proton::Proton())
      fParticleChangeForGamma->ProposeLocalEnergyDeposit(
        n * (inK * electron_mass_c2 / proton_mass_c2) + waterBindingEnergy
        - outgoingParticleBindingEnergy);
    else
  ...
  G4DNAChemistryManager::Instance()->CreateWaterMolecule(eIonizedMolecule, 1, theIncomingTrack);
}

//....oooOO0OOooo........oooOO0OOooo........oooOO0OOooo........oooOO0OOooo......

G4int G4DNADingfelderChargeDecreaseModel::NumberOfFinalStates(
...

{
  ...
  if (particleDefinition == alphaPlusDef) return 1;
...
G4ParticleDefinition* G4DNADingfelderChargeDecreaseModel::OutgoingParticleDefinition(
  G4ParticleDefinition* particleDefinition, G4int finalStateIndex)
{
  if (particleDefinition == G4Proton::Proton()) return hydrogenDef;
  ...
  if (particleDefinition == alphaPlusDef) return heliumDef;
...
G4double G4DNADingfelderChargeDecreaseModel::WaterBindingEnergyConstant(
  G4ParticleDefinition* particleDefinition, G4int finalStateIndex)
{
...
G4double G4DNADingfelderChargeDecreaseModel::OutgoingParticleBindingEnergyConstant(
  G4ParticleDefinition* particleDefinition, G4int finalStateIndex)
{
...
G4double G4DNADingfelderChargeDecreaseModel::PartialCrossSection(
  G4double k, G4int index, const G4ParticleDefinition* particleDefinition)
{
  ...
  //
  // f0 has been added to the code in order to manage partial (shell-dependent) cross sections (if
  // no shell dependence is present. f0=1. Sum of f0 over the considered shells should give 1)
  //
  ...
  {
    //
    // if x1 < x0 means that x1 and b1 will be calculated with the following formula (this piece of
    // code is run on all alphas and not on protons)
    //
...
G4int G4DNADingfelderChargeDecreaseModel::RandomSelect(
