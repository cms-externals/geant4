...
G4double G4DNAGillespieDirectMethod::VolumeOfNode(const Voxel& voxel)
{
  ...
  if (V <= 0)
  {
    ...
    G4Exception("G4DNAGillespieDirectMethod::VolumeOfNode", "G4DNAGillespieDirectMethod03",
                FatalErrorInArgument, exceptionDescription);
  }
  return V;
}
G4double G4DNAGillespieDirectMethod::PropensityFunction(const Voxel& voxel, MolType moleType)
{
  ...
  if (it != node.end())
  {
    ...
    G4cout << it->first->GetName() << " " << it->second
           << " D : " << it->first->GetDiffusionCoefficient() << " LengthY : " << LengthY
           << " PropensityFunction : " << alpha << G4endl;
#endif
  }
  return alpha;
}

G4double G4DNAGillespieDirectMethod::PropensityFunction(const Voxel& voxel, ReactionData* data)
{
  G4double value;
  ...
  auto k = data->GetObservedReactionRateConstant() / (Avogadro * VolumeOfNode(voxel));
  if (ConfA == ConfB)
  {
  ...
  {
    G4ExceptionDescription exceptionDescription;
    exceptionDescription << "G4DNAGillespieDirectMethod::PropensityFunction for : "
                         ...
                         << G4BestUnit(data->GetEffectiveReactionRadius(), "Length") << " k : " << k
                         << " volume : " << VolumeOfNode(voxel) << G4endl;
    G4Exception("G4DNAGillespieDirectMethod::PropensityFunction", "G4DNAGillespieDirectMethod013",
                FatalErrorInArgument, exceptionDescription);
  }

#ifdef DEBUG
  if (value > 0)
    G4cout << "G4DNAGillespieDirectMethod::PropensityFunction for : " << ConfA->GetName() << "("
           << typeANumber << ") + " << ConfB->GetName() << "(" << typeBNumber
           << ") : propensity : " << value
           << "  Time to Reaction : " << G4BestUnit(timeToReaction, "Time") << " k : " << k
           << " Index : " << index << G4endl;
#endif
...
void G4DNAGillespieDirectMethod::Initialize()
{
  ...
  fEquilibriumProcesses.emplace(std::make_pair(
    6, std::make_unique<G4ChemEquilibrium>(6, 10 * CLHEP::us)));  // reactionType6 and 10 * us
  fEquilibriumProcesses.emplace(std::make_pair(
    7, std::make_unique<G4ChemEquilibrium>(7, 10 * CLHEP::us)));  // reactionType6 and 10 * us
  fEquilibriumProcesses.emplace(std::make_pair(
    8, std::make_unique<G4ChemEquilibrium>(8, 10 * CLHEP::us)));  // reactionType6 and 10 * us
...
void G4DNAGillespieDirectMethod::CreateEvent(const Index& index)
{
  ...
  {
    G4ExceptionDescription exceptionDescription;
    exceptionDescription << "This voxel : " << index << " is not ready to make event" << G4endl;
    G4Exception("G4DNAGillespieDirectMethod::CreateEvent", "G4DNAGillespieDirectMethod05",
                FatalErrorInArgument, exceptionDescription);
  }
...
#ifdef DEBUG
  G4cout << "r2 : " << r2 << " rAlpha0 : " << rAlpha0 << " dAlpha0 : " << dAlpha0
         << "    rAlpha0 / (dAlpha0 + rAlpha0) : " << rAlpha0 / (dAlpha0 + rAlpha0) << G4endl;
#endif
  if (r2 < rAlpha0 / alphaTotal)
  {
    if (fVerbose > 1)
    {
      G4cout << "=>>>>reaction at : " << timeStep
             << " timeStep : " << G4BestUnit(((1.0 / alphaTotal) * std::log(1.0 / r1)), "Time")
             << G4endl;
    }
  ...
  else if (dAlpha0 > 0)
  {
    if (fVerbose > 1)
    {
      G4cout << "=>>>>jumping at : " << timeStep
             << " timeStep : " << G4BestUnit(((1.0 / alphaTotal) * std::log(1.0 / r1)), "Time")
             << G4endl;
    }

    auto dSelectedIter = fJumpingDataMap.upper_bound(r2 * alphaTotal - rAlpha0);
    auto pDSelected = std::make_unique<std::pair<MolType, Index>>(dSelectedIter->second);
...
G4double G4DNAGillespieDirectMethod::Reaction(const Voxel& voxel)
{
  ...
  G4double alpha0 = 0;
  const auto& dataList = fMolecularReactions->GetVectorOfReactionData();  // shoud make a member
  if (dataList.empty())
  {
    G4ExceptionDescription exceptionDescription;
    exceptionDescription << "MolecularReactionTable empty" << G4endl;
    G4Exception("G4DNAGillespieDirectMethod::Reaction", "G4DNAGillespieDirectMethod01",
                FatalErrorInArgument, exceptionDescription);
  }
...
#ifdef DEBUG
      G4cout << "mole : " << conf->GetName() << " number : " << ComputeNumberInNode(index, conf)
             << " propensity : " << propensity << "  alpha0 : " << alpha0 << G4endl;
#endif
    }
  }
#ifdef DEBUG
...
G4double G4DNAGillespieDirectMethod::ComputeNumberInNode(const Voxel& voxel,
                                                         MolType type)  // depend node ?
{
...
G4bool G4DNAGillespieDirectMethod::FindScavenging(const Voxel& voxel, MolType moletype,
                                                  G4double& numberOfScavenger)
{
  ...
  G4double totalNumber =
    fpScavengerMaterial->GetNumberMoleculePerVolumeUnitForMaterialConf(moletype);
  if (totalNumber == 0)
  {
    return false;
  }
