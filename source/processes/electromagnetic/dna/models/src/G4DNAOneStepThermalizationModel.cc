...
const double Meesungnoen2002::gCoeff[13] = {
  -4.06217193e-08, 3.06848412e-06,  -9.93217814e-05, 1.80172797e-03,  -2.01135480e-02,
  1.42939448e-01,  -6.48348714e-01, 1.85227848e+00,  -3.36450378e+00, 4.37785068e+00,
  -4.20557339e+00, 3.81679083e+00,  -2.34069784e-01};
// fit from Meesungnoen, 2002

const double Meesungnoen2002_amorphous::gCoeff[7] = {
  7.3144e-05, -2.2474e-03, 3.4555e-02, -4.3574e-01, 2.8954e+00, -1.0381e+00, 1.4300e+00};
// fit from Meesungnoen, 2002

const double Terrisol1990::gEnergies_T1990[11] = {0.2, 0.5, 1, 2, 3, 4, 5, 6, 7,
                                                  // The two last are not in the dataset
                                                  8, 9};  // eV

const double Terrisol1990::gStdDev_T1990[11] = {
  17.68 * CLHEP::angstrom, 22.3 * CLHEP::angstrom, 28.49 * CLHEP::angstrom, 45.35 * CLHEP::angstrom,
  70.03 * CLHEP::angstrom, 98.05 * CLHEP::angstrom, 120.56 * CLHEP::angstrom,
  132.73 * CLHEP::angstrom, 142.60 * CLHEP::angstrom,
  ...
  137.9 * CLHEP::angstrom, 120.7 * CLHEP::angstrom};  // angstrom
...
void GetGaussianPenetrationFromRmean3D(G4double r_mean, G4ThreeVector& displacement)
{
  ...
  displacement = G4ThreeVector(G4RandGauss::shoot(0, sigma1D), G4RandGauss::shoot(0, sigma1D),
                               G4RandGauss::shoot(0, sigma1D));
}

void Meesungnoen2002::GetPenetration(G4double k, G4ThreeVector& displacement)
{
  GetGaussianPenetrationFromRmean3D(GetRmean(k), displacement);
}

void Meesungnoen2002_amorphous::GetPenetration(G4double k, G4ThreeVector& displacement)
{
  GetGaussianPenetrationFromRmean3D(GetRmean(k), displacement);
}

void Kreipl2009::GetPenetration(G4double k, G4ThreeVector& displacement)
{
...
void Ritchie1994::GetPenetration(G4double k, G4ThreeVector& displacement)
{
...
double Terrisol1990::Get3DStdDeviation(double energy)
{
  ...
  if (k_eV > 9.)
  {
    ...
    G4Exception("Terrisol1990::Get3DStdDeviation", "INVALID_ARGUMENT", FatalErrorInArgument,
                description);
  }
  ...
  {
    auto it = std::lower_bound(&gEnergies_T1990[0], &gEnergies_T1990[2], k_eV);
...
void Terrisol1990::GetPenetration(G4double energy, G4ThreeVector& displacement)
{
  double sigma3D = Get3DStdDeviation(energy);
...
G4VEmModel* G4DNASolvationModelFactory::Create(const G4String& penetrationModel)
{
  ...
  {
    return new G4TDNAOneStepThermalizationModel<DNA::Penetration::Terrisol1990>(
      G4Electron::Definition(), modelNamePrefix + penetrationModel);
  }
  if (penetrationModel == "Meesungnoen2002")
  {
    return new G4TDNAOneStepThermalizationModel<DNA::Penetration::Meesungnoen2002>(
      G4Electron::Definition(), modelNamePrefix + penetrationModel);
  }
  if (penetrationModel == "Meesungnoen2002_amorphous")
  {
    return new G4TDNAOneStepThermalizationModel<DNA::Penetration::Meesungnoen2002_amorphous>(
      G4Electron::Definition(), modelNamePrefix + penetrationModel);
  }
  if (penetrationModel == "Kreipl2009")
  {
    return new G4TDNAOneStepThermalizationModel<DNA::Penetration::Kreipl2009>(
      G4Electron::Definition(), modelNamePrefix + penetrationModel);
  }
  if (penetrationModel == "Ritchie1994")
  {
    return new G4TDNAOneStepThermalizationModel<DNA::Penetration::Ritchie1994>(
      G4Electron::Definition(), modelNamePrefix + penetrationModel);
  }

  G4ExceptionDescription description;
  description << penetrationModel + " is not a valid model name.";
  G4Exception("G4DNASolvationModelFactory::Create", "INVALID_ARGUMENT", FatalErrorInArgument,
              description, "Options are: Terrisol1990, Meesungnoen2002, Ritchie1994.");
  return nullptr;
}
...
G4VEmModel* G4DNASolvationModelFactory::GetMacroDefinedModel()
{
  ...
  switch (dnaSubType)
  {
    ...
    default:
      G4Exception(
        "G4DNASolvationModelFactory::GetMacroDefinedModel", "DnaSubType", FatalErrorInArgument,
        "The solvation parameter stored in G4EmParameters is unknown. Supported types are: "
        "fRitchie1994eSolvation, fTerrisol1990eSolvation, fMeesungnoen2002eSolvation.");
  }
