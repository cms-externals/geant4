...
//....oooOO0OOooo........oooOO0OOooo........oooOO0OOooo........oooOO0OOooo......
G4double G4VLEPTSModel::GetMeanFreePath(const G4Material* aMaterial, const G4ParticleDefinition*,
                                        G4double kineticEnergy)
{
  G4double MeanFreePath;

  if (verboseLevel >= 3)
    G4cout << aMaterial->GetIndex() << " G4VLEPTSModel::GetMeanFreePath " << kineticEnergy << " > "
           << theHighestEnergyLimit << " < " << theLowestEnergyLimit << G4endl;
...
void G4VLEPTSModel::BuildPhysicsTable(const G4ParticleDefinition& aParticleType)
{
  ...
  if (path == nullptr)
  {
    G4Exception("G4VLEPTSModel", "", FatalException, "variable G4LEDATA not defined");
  }
  ...
  for (matite = materialTable->cbegin(); matite != materialTable->cend(); ++matite)
  {
    ...
    if (verboseLevel >= 2)
      G4cout << GetName() << " : " << theXSType << " " << mateName << " INTEGRALXS "
             << integralXS.size() << G4endl;
    ...
    else
    {
      if (verboseLevel >= 2)
      {
        ...
        {
          G4cout << GetName() << " : " << (*itei).first << " INTEGRALXS NDATA "
                 << (*itei).second.size() << G4endl;
        }
      }
                                           ...
                                           std::map<G4int, std::vector<G4double>>& integralXS)
{
  ...
  std::size_t matIdx = aMaterial->GetIndex();
  auto ptrVector =
    new G4PhysicsLogVector(theLowestEnergyLimit, theHighestEnergyLimit, theNumbBinTable);

  for (G4int ii = 0; ii < theNumbBinTable; ++ii)
  {
    LowEdgeEnergy = ptrVector->Energy(ii);
    if (verboseLevel >= 2)
      G4cout << GetName() << " " << ii << " Energy " << LowEdgeEnergy << " > "
             << theLowestEnergyLimit << " < " << theHighestEnergyLimit << G4endl;
    //-      fValue = ComputeMFP(LowEdgeEnergy, material, aParticleName);
    fValue = 0.;
    if (LowEdgeEnergy >= theLowestEnergyLimit && LowEdgeEnergy <= theHighestEnergyLimit)
    {
      G4double NbOfMoleculesPerVolume =
        aMaterial->GetDensity() / theMolecularMass[aMaterial] * CLHEP::Avogadro;
      ...
      //-	   if( verboseLevel >= 2 )  G4cout << " eVEnergy " << eVEnergy << " LowEdgeE " <<
      // LowEdgeEnergy << " " << integralXS[theXSType][1] << G4endl;
      ...
      else
      {
        ...
        G4double aa, bb;
        for (G4int jj = 1; jj < theNXSdat[aMaterial]; jj++)
        {  // Extrapolate for E > Emax !!!
          if (verboseLevel >= 3)
        ...
        crossSection = (integralXS[theXSType][Bin]
                        + (integralXS[theXSType][Bin + 1] - integralXS[theXSType][Bin]) / (bb - aa)
                            * (eVEnergy - aa))
                       * 1.e-16 * CLHEP::cm2;

        if (verboseLevel >= 3)
          G4cout << " crossSection " << crossSection << " " << integralXS[theXSType][Bin] << " + "
                 << (integralXS[theXSType][Bin + 1] - integralXS[theXSType][Bin]) << " / "
                 << (bb - aa) << " *" << (eVEnergy - aa) << " * " << 1.e-16 * CLHEP::cm2 << G4endl;
        ;
        ...
        if (verboseLevel >= 2)
          G4cout << GetName() << " ADDING SIGMA " << SIGMA << " NAtoms " << NbOfMoleculesPerVolume
                 ...
                 << integralXS[theXSType][Bin] << " " << integralXS[theXSType][Bin + 1] << G4endl;
      }
    ...
    if (verboseLevel >= 2)
      G4cout << GetName() << " BUILDXS " << ii << " : " << LowEdgeEnergy << " = " << fValue
             << G4endl;
  }
...
//....oooOO0OOooo........oooOO0OOooo........oooOO0OOooo........oooOO0OOooo......
G4ThreeVector G4VLEPTSModel::SampleNewDirection(const G4Material* aMaterial, G4ThreeVector P0Dir,
                                                G4double e, G4double el)
{
...
#ifdef DEBUG_LEPTS
  if (verboseLevel >= 2)
    G4cout << " G4VLEPTSModel::SampleNewDirection rotated " << P1Dir << " " << P0Dir << G4endl;
#endif
...
#ifdef DEBUG_LEPTS
  if (verboseLevel >= 2)
    G4cout << aMaterial->GetName() << "SampleEnergyLoss/eV " << el / CLHEP::eV << " eMax/eV "
           << eMax / CLHEP::eV << " "
           << " " << GetName() << G4endl;
#endif
  return el;
...
G4bool G4VLEPTSModel::ReadParam(const G4String& fnParam, const G4Material* aMaterial)
{
  ...
  if (!fin.is_open())
  {
    G4Exception("G4VLEPTSModel::ReadParam", "", JustWarning,
  ...
  if (verboseLevel >= 1)
    G4cout << "Read param   (" << fnParam << ")\t IonisPot: " << IonisPot
  ...
  for (G4int ii = 0; ii < nelem; ++ii)
  {
    ...
    //    G4cout << " MMASS1 " << mmass/CLHEP::g << " " << aMaterial->GetElement(ii)->GetName() << "
    //    " << aMaterial->GetElement(ii)->GetA()/CLHEP::g << G4endl;
  }
  //  G4cout << " MMASS " << MolecularMass << " " << MolecularMass*CLHEP::g <<  " ME " << mmass << "
  //  " << mmass/CLHEP::g << G4endl;
  theMolecularMass[aMaterial] = MolecularMass * CLHEP::g / CLHEP::mole;
  //  theMolecularMass[aMaterial] = aMaterial->GetMassOfMolecule()*CLHEP::Avogadro;  // Material
  //  mixtures do not calculate molecular mass

  if (verboseLevel >= 1)
    G4cout << " IonisPot: " << IonisPot / CLHEP::eV << " eV "
...
//....oooOO0OOooo........oooOO0OOooo........oooOO0OOooo........oooOO0OOooo......
std::map<G4int, std::vector<G4double>> G4VLEPTSModel::ReadIXS(const G4String& fnIXS,
                                                              const G4Material* aMaterial)
{
  ...
  fin >> nXSdat >> nXSsub;
  if (verboseLevel >= 1)
    G4cout << "Read IXS   (" << fnIXS << ")\t nXSdat: " << nXSdat << " nXSsub: " << nXSsub
           << G4endl;
  ...
  for (G4int ie = 1; ie <= nXSdat; ie++)
  {
    for (G4int ip = 0; ip <= nXSsub; ip++)
    {
      ...
      if (verboseLevel >= 3)
        G4cout << GetName() << " FILL IXS " << ip << " " << ie << " = " << integralXS[ip][ie] << " "
               << xsdat << G4endl;
      // xsdat 1e-16*cm2
    }
  }
