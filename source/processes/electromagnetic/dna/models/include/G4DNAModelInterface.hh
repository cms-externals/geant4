...
class G4DNAModelInterface : public G4VEmModel
{
  ...
  public:
    ...
    G4DNAModelInterface&
    operator=(const G4DNAModelInterface& right) = delete;  // prevent assignement
    ...
    void SampleSecondaries(std::vector<G4DynamicParticle*>* fVect,
                           ...
                           const G4DynamicParticle* aDynamicElectron, G4double tmin,
     ...
     */
    inline std::size_t GetSelectedMaterial() { return fSampledMat; }
  ...
  private:
     ...
     * This method will check the energy range of the models to find to good one for the current
     * ekin.
     */
    G4VEmModel* SelectModel(const std::size_t& material, const G4ParticleDefinition* particle,
                            const G4double& ekin);

    G4double GetNumMoleculePerVolumeUnitForMaterial(const G4Material* mat);
    G4double GetNumMolPerVolUnitForComponentInComposite(const G4Material* component,
    ...
    G4double fCSsumTot = 0;  ///< value which contains the sum of all the component cross sections
                             ///< in case of a composite material
