...
class G4VLEPTSModel : public G4VEmModel
{
  public:
    ...
    void
    BuildPhysicsTable(const G4ParticleDefinition& aParticleType);  // G4ParticleWithCuts::SetCut()
    G4double GetMeanFreePath(const G4Material* mate, const G4ParticleDefinition* aParticle,
                             G4double kineticEnergy);

    G4ThreeVector SampleNewDirection(const G4Material* aMaterial, G4ThreeVector Dir, G4double e,
                                     G4double el);
  ...
  protected:
    ...
    virtual std::map<G4int, std::vector<G4double>> ReadIXS(const G4String& fileName,
                                                           const G4Material* aMaterial);
    G4double SampleEnergyLoss(const G4Material* aMaterial, G4double eMin, G4double eMax);
    void BuildMeanFreePathTable(const G4Material* aMaterial,
