...
class G4VDNAModel : public G4VEmModel
{
  public:
    ...
    void SampleSecondaries(std::vector<G4DynamicParticle*>*, const G4MaterialCutsCouple*,
                           const G4DynamicParticle*, G4double tmin = 0,
                           G4double tmax = DBL_MAX) override = 0;
     ...
     */
    G4bool IsParticleExistingInModelForMaterial(const G4ParticleDefinition* particleName,
  ...
  protected:
     ...
     */
    MaterialParticleMapData* GetData() { return &fData; }
     ...
     * Build the material name vector which is used to know the materials the user want to include
     * in the model. \param materials \return a vector with all the material names
     */
     ...
     */
    G4int RandomSelectShell(const G4double& k, const G4ParticleDefinition* particle,
     ...
     * Virtual method that need to be implemented if one wish to use the differential cross
     * sections. The read method for that kind of information is not standardized yet.
     * \param materialName
     * \param particleName
     * \param path
     * \param scaleFactor
     */
    virtual void ReadDiffCSFile(const size_t& materialName,
                                const G4ParticleDefinition* particleName, const G4String& path,
                                const G4double& scaleFactor);
     ...
     * \param materialName
     * \param particleName
     * Meant to fill fTableData with 0 for the specified material and particle, therefore allowing
     * the ModelInterface class to proceed with the material and particle even if no data are
     * registered here. The data should obviously be registered somewhere in the child class. This
     * method is here to allow an easy use of the no-ModelInterface dna models within the
  ...
  private:
     ...
     * implemented in the model. If the user does then only the specified materials contained in
     * this string variable will be activated. The string is like: mat1/mat2/mat3/mat4
     */
