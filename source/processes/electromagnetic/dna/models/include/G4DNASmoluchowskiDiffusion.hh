...
#ifndef G4DNASMOLUCHOWSKIDIFFUSION_HH
#define G4DNASMOLUCHOWSKIDIFFUSION_HH
...
class G4DNASmoluchowskiDiffusion
{
  public:
    ...
    static double GetDifferential(double sTransform)
    {
      ...
      return sTransform * sTransform * G4Exp(-sTransform * sTransform)
             * constant;  // -4*sTransform*sTransform*exp(-sTransform*sTransform)/sqrt(3.141592653589793)
    }
    ...
    struct BoundingBox
    {
        ...
        BoundingBox(double xmin, double xmax, double toleranceY)
          : fXmax(xmax), fXmin(xmin), fToleranceY(toleranceY)
        {
          if (fXmax < fXmin)
          {
            double tmp = fXmin;
          ...
          fIncreasingCumulativeFunction =
            (GetCumulativeProbability(fXmax) - GetCumulativeProbability(fXmin)) / (fXmax - fXmin);
        }

        void Print() { G4cout << "fXmin: " << fXmin << " | fXmax: " << fXmax << G4endl; }

        bool Propose(double proposedXValue, double proposedProba, double nextProba,
                     double& returnedValue)
        {
          ...
          //          << "| nextProba: " << nextProba
          //          << " | fXmin: " << fXmin << " (" <<
          //          G4DNASmoluchowskiDiffusion::GetCumulativeProbability(fXmin) <<")"
          //          << " | fXmax: " << fXmax << " (" <<
          //          G4DNASmoluchowskiDiffusion::GetCumulativeProbability(fXmax) <<")"
          //          << G4endl;
    ...
    void PrepareReverseTable(double xmin, double xmax)
    {
      ...
      // Tips to improve the exactness: get an better value of pi, get better approximation of erf
      // and exp, use long double instead of double
      //    boundingBox.Print();
    }
    ...
    double PlotInverse(double* x, double*) { return GetInverseProbability(x[0]); }

    double Plot(double* x, double*) { return GetDifferential(x[0]); }
