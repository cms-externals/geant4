...
#ifndef G4DNARPWBAIONISATIONMODEL_HH
#define G4DNARPWBAIONISATIONMODEL_HH
...
class G4DNARPWBAIonisationModel : public G4VEmModel
{
  ...
  private:
    ...
    using MapData =
      std::map<G4String, std::unique_ptr<G4DNACrossSectionDataSet>, std::less<G4String>>;
    ...
    G4double RandomizeEjectedElectronEnergyFromCumulatedDcs(const G4double& incomingParticleEnergy,
                                                            const G4int& shell);
    G4double Interpolate(const G4double& e1, const G4double& e2, const G4double& e,
                         const G4double& xs1, const G4double& xs2);
...
inline void G4DNARPWBAIonisationModel::SelectFasterComputation(const G4bool& input)
{
  fasterCode = input;
}
