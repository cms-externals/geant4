...
double
FindClosest(size_t index, vector<G4ThreeVector>& _in)
{
//  size_t _out_i = 0;
  double dis = 3e8;

  G4ThreeVector& pos = _in[index];

  for(size_t i = 0 ; i != _in.size() ; i++)
  {
    if(i == index) continue;
      double tmp= (_in[i]-pos).mag();
      if(tmp < dis)
      {
//        _out_i = i;
        dis = tmp;
...
int main()
{
  ...
  for (int i = 0; i < 10; i++)
  {
    ...
    PushTrack(track);
    // created at random positions : r < 100
  }

  double time;
  G4ITTrackHolder::Instance()->MergeNextTimeToMainList(time);
  // push the new tracks to the current main list
  ...
  assert(fabs(kdtree_dis - FindClosest(0, OH_position)) < 1e-6);
  ...
  std::map<G4ITTrackHolder::Key, PriorityList*>& priorityLists = G4ITTrackHolder::Instance()
      ->GetLists();
  ...
  {
    std::map<G4ITTrackHolder::Key, PriorityList*>::iterator it = priorityLists.begin();
    std::map<G4ITTrackHolder::Key, PriorityList*>::iterator end = priorityLists.end();
    for (; it != end; it++)
    {
  ...
  PriorityList* H2O2_list = priorityLists[G4MoleculeTable::Instance()
      ->GetConfiguration("H2O2")->GetMoleculeID()];
  ...
  PriorityList* H_list = priorityLists[G4MoleculeTable::Instance()
      ->GetConfiguration("H")->GetMoleculeID()];
