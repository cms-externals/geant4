...
std::shared_ptr<CounterType> ConvertBaseToCounterPointer(std::weak_ptr<G4VMoleculeCounter> pIndex)
{
  static_assert(std::is_base_of<G4VMoleculeCounter, CounterType>::value,
                "CounterType must be derived from G4VMoleculeCounter!"
                "No forward declaration is allowed.");

  if (pIndex.expired()) {
    G4ExceptionDescription errMsg;
    errMsg << "Could not cast the pointer to type " << typeid(CounterType).name() << ",\n"
           << "because the weak_ptr has expired since it has been passed, indicating something "
              "very wrong!"
           << G4endl;
    G4Exception(G4String("G4VUserMoleculeCounter::ConvertBaseToCounterPointer<"
                         + G4String(typeid(CounterType).name()) + ">"),
                "BAD_REFERENCE", FatalException, errMsg);
  }

  std::shared_ptr<CounterType> mapIndex = std::dynamic_pointer_cast<CounterType>(pIndex.lock());
  if (mapIndex == nullptr) {
    G4ExceptionDescription errMsg;
    errMsg << "Could not cast the pointer to type " << typeid(CounterType).name() << "!" << G4endl;
    G4Exception(G4String("G4VMoleculeCounter::ConvertBaseToCounterPointer<"
                         + G4String(typeid(CounterType).name()) + ">"),
                "BAD_REFERENCE", FatalException, errMsg);
  }
  return mapIndex;
