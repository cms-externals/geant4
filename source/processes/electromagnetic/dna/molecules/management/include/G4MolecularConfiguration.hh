...
#ifndef G4MOLECULARCONFIGURATION_HH
#define G4MOLECULARCONFIGURATION_HH
...
class G4MolecularConfiguration
{
  public:
    ...
    using G4DiffCoeffParam =
      std::function<double(const G4Material*, double, const G4MolecularConfiguration*)>;
    ...
    GetOrCreateMolecularConfiguration(const G4MoleculeDefinition*, const G4ElectronOccupancy& eOcc);
    ...
    static G4MolecularConfiguration* CreateMolecularConfiguration(const G4String& userIdentifier,
    ...
    static G4MolecularConfiguration* CreateMolecularConfiguration(const G4String& userIdentifier,
                                                                  const G4MoleculeDefinition*,
                                                                  int charge, const G4String& label,
                                                                  bool& wasAlreadyCreated);
    ...
    inline G4double GetDiffusionCoefficient(const G4Material*, double temperature) const;
  ...
  public:
    ...
    class G4MolecularConfigurationManager
    {
      public:
        ...
        G4MolecularConfigurationManager() { fLastMoleculeID = -1; }
        ~G4MolecularConfigurationManager();

        int GetNumberOfCreatedSpecies() { return fLastMoleculeID + 1; }

        //------------------------------------------------------------------------
        // CALLED FROM CONSTRUCTORS
        G4int Insert(const G4MoleculeDefinition* molDef, const G4ElectronOccupancy& eOcc,
                     G4MolecularConfiguration* molConf);

        G4int Insert(const G4MoleculeDefinition* molDef, int charge,
                     G4MolecularConfiguration* molConf);

        G4int Insert(const G4MoleculeDefinition* molDef, const G4String& label,
                     G4MolecularConfiguration* molConf);
        ...
        G4MolecularConfiguration*
        GetOrCreateMolecularConfiguration(const G4MoleculeDefinition* molDef, int charge);

        static G4Mutex fManagerCreationMutex;

        void RemoveMolecularConfigurationFromTable(G4MolecularConfiguration*);

        const std::vector<G4MolecularConfiguration*>& GetAllSpecies() { return fMolConfPerID; }

        std::map<G4String, G4MolecularConfiguration*>& GetUserIDTable() { return fUserIDTable; }

      private:

        //__________________________________________________________________________
        using ElectronOccupancyTable =
        ...
        using MolLabelConfTable = std::map<const G4MoleculeDefinition*,
                                           std::map<const G4String, G4MolecularConfiguration*>>;
  ...
  protected:
    ...
    static double ReturnDefaultDiffCoeff(const G4Material*, double,
...
{
  if (fLabel == nullptr) fLabel = new G4String();
...
inline void G4MolecularConfiguration::AddDiffCoeffParameterization(const G4DiffCoeffParam& para)
{
  fDiffParam = para;
}

inline G4double G4MolecularConfiguration::GetDiffusionCoefficient(const G4Material* material,
