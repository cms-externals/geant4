...
void G4MoleculeCounter::InitializeUser()
{
  ...
  //  while ((mol_iterator)()) {
  //    if (IsIgnored(mol_iterator.value()->GetDefinition()) ||
  //		IsIgnored(mol_iterator.value())) {
  //      continue;
  //    }
  //    std::shared_ptr<G4MoleculeCounterIndex> mapIndex =
...
{
  return std::make_shared<G4MoleculeCounterIndex>(GetMolecule(aTrack)->GetMolecularConfiguration());
}
...
{
  return std::make_shared<G4MoleculeCounterIndex>(configuration);
}
