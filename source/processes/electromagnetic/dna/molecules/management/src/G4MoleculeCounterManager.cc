...
std::shared_ptr<G4MoleculeCounterManager> G4MoleculeCounterManager::fpMasterInstance = nullptr;
std::vector<std::weak_ptr<G4MoleculeCounterManager>> G4MoleculeCounterManager::fWorkerInstances =
  std::vector<std::weak_ptr<G4MoleculeCounterManager>>();
G4ThreadLocal std::shared_ptr<G4MoleculeCounterManager> G4MoleculeCounterManager::fpInstance =
  nullptr;
...
void G4MoleculeCounterManager::RegisterInstance()
{
  if (fInstancesRegistered) {
    ...
    if (G4Threading::IsMasterThread()) {
      G4AutoLock lock(&masterInstanceMutex);
      if (fpMasterInstance.use_count() > 0) {
        G4Exception("G4MoleculeCounterManager::RegisterInstance", "MOLMAN000", FatalException,
...
std::shared_ptr<G4MoleculeCounterManager> G4MoleculeCounterManager::Instance()
{
  if (fpInstance == nullptr) {
    fpInstance = std::make_shared<G4MoleculeCounterManager>(G4MoleculeCounterManager::Private());
                                                      ...
                                                      G4bool belowTimeInclusive)
{
  ...
  auto counter = GetMoleculeCounter(id).lock();
                                                              ...
                                                              G4bool belowTimeInclusive)
{
  ...
  auto counter = GetMoleculeReactionCounter(id).lock();
  counter->SetActiveLowerBound(aboveTime, aboveTimeInclusive);
  counter->SetActiveUpperBound(belowTime, belowTimeInclusive);
}
...
std::weak_ptr<G4VMoleculeCounter> G4MoleculeCounterManager::GetMoleculeCounter(G4int id)
{
...
std::vector<std::weak_ptr<G4VMoleculeCounter>> G4MoleculeCounterManager::GetMoleculeCounters()
{
  std::vector<std::weak_ptr<G4VMoleculeCounter>> output;
...
std::vector<std::weak_ptr<G4VMoleculeCounter>>
G4MoleculeCounterManager::GetMoleculeCounters(G4String name)
{
  std::vector<std::weak_ptr<G4VMoleculeCounter>> output;
...
std::weak_ptr<G4VMoleculeReactionCounter>
G4MoleculeCounterManager::GetMoleculeReactionCounter(G4int id)
{
...
std::vector<std::weak_ptr<G4VMoleculeReactionCounter>>
G4MoleculeCounterManager::GetMoleculeReactionCounters()
{
  std::vector<std::weak_ptr<G4VMoleculeReactionCounter>> output;
...
std::vector<std::weak_ptr<G4VMoleculeReactionCounter>>
G4MoleculeCounterManager::GetMoleculeReactionCounters(G4String name)
{
  std::vector<std::weak_ptr<G4VMoleculeReactionCounter>> output;
  ...
  const G4MoleculeCounterManager* selectedWorker)
{
  ...
  auto throwExceptionOnExpiredWeakPtr = []() {
    G4ExceptionDescription description;
    description << "One of the received weak pointers already expired!";
    G4Exception("G4MoleculeCounterManager::AbsorbWorkerManagerCounters", "MOLMAN999",
                FatalException, description);
  };
  ...
  for (auto const& weak_worker : fWorkerInstances) {
    if (weak_worker.expired()) throwExceptionOnExpiredWeakPtr();

    auto worker = weak_worker.lock();

    if (selectedWorker != nullptr
        && const_cast<const G4MoleculeCounterManager*>(worker.get()) != selectedWorker)
    {
      continue;
    }
    ...
    for (auto& [id, masterCounter] : fpMasterInstance->fCounters) {
      ...
      auto weak_workerCounter = worker->GetMoleculeCounter(id);
      if (weak_workerCounter.expired()) throwExceptionOnExpiredWeakPtr();
      auto workerCounter = weak_workerCounter.lock();
      masterCounter->AbsorbCounter(workerCounter);
    }

    for (auto& [id, masterCounter] : fpMasterInstance->fReactionCounters) {
      // acquire worker counter
      auto weak_workerCounter = worker->GetMoleculeReactionCounter(id);
      if (weak_workerCounter.expired()) throwExceptionOnExpiredWeakPtr();
      auto workerCounter = weak_workerCounter.lock();
      masterCounter->AbsorbCounter(workerCounter);
    }
...
void G4MoleculeCounterManager::DumpMasterCounters() const
{
  ...
  for (auto const& ctr : fpMasterInstance->GetMoleculeCounters()) {
    auto pCounter = ctr.lock();
    G4cout << "===========================================================================  \n"
  ...
  for (auto const& ctr : fpMasterInstance->GetMoleculeReactionCounters()) {
    auto pCounter = ctr.lock();
    G4cout << "===========================================================================  \n"
...
void G4MoleculeCounterManager::DumpWorkerCounters() const
{
  ...
  for (auto const& weak_worker : G4MoleculeCounterManager::fWorkerInstances) {
    auto worker = weak_worker.lock();
    for (auto const& ctr : worker->GetMoleculeCounters()) {
      auto pCounter = ctr.lock();
      G4cout << "===========================================================================  \n"
             << " >> [WORKER<" << worker.get() << ">] Dumping Molecule Counter `"
             << pCounter->GetName() << "`\n"
             << G4endl;
      pCounter->Dump();
      G4cout << "\n===========================================================================  "
    ...
    for (auto const& ctr : worker->GetMoleculeReactionCounters()) {
      auto pCounter = ctr.lock();
      G4cout << "===========================================================================  \n"
             << " >> [WORKER<" << worker.get() << ">] Dumping Molecule Reaction Counter `"
             << pCounter->GetName() << "`\n"
             << G4endl;
      pCounter->Dump();
      G4cout << "\n===========================================================================  "
