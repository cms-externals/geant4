...
{
  fpManagerDir = std::make_shared<G4UIdirectory>("/chem/moleculecounters/", false);
...
{
  fpActiveCmd = std::make_shared<G4UIcmdWithABool>("/chem/moleculecounters/active", this);
  fpActiveCmd->AvailableForStates(G4State_PreInit);

  fpResetBeforeEventCmd =
    std::make_shared<G4UIcmdWithABool>("/chem/moleculecounters/resetBeforeEvent", this);
  fpResetBeforeEventCmd->AvailableForStates(G4State_PreInit);

  fpResetBeforeRunCmd =
    std::make_shared<G4UIcmdWithABool>("/chem/moleculecounters/resetBeforeRun", this);
  fpResetBeforeRunCmd->AvailableForStates(G4State_PreInit);

  fpAccumulateIntoMasterCmd =
    std::make_shared<G4UIcmdWithABool>("/chem/moleculecounters/accumulateIntoMaster", this);
  fpAccumulateIntoMasterCmd->AvailableForStates(G4State_PreInit);

  fpVerboseCmd = std::make_shared<G4UIcmdWithAnInteger>("/chem/moleculecounters/verbose", this);
