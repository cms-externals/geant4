...
G4MoleculeDefinition::G4MoleculeDefinition(const G4String& name, G4double mass, G4double diffCoeff,
                                           G4int charge, G4int electronicLevels, G4double radius,
                                           G4int atomsNumber, G4double lifetime,
                                           const G4String& aType, G4FakeParticleID ID)
  : G4ParticleDefinition(name, mass, 0., charge, 0, 0, 0, 0, 0, 0, "Molecule", 0, 0, ID, false,
                         lifetime, nullptr, false, aType, 0, 0.0),
...
G4MoleculeDefinition* G4MoleculeDefinition::Load(std::istream& in)
{
  ...
  return new G4MoleculeDefinition(name, mass, diffCoeff, charge, electronicLevels, radius,
                                  atomsNumber, lifetime, aType);
}
...
G4MolecularConfiguration* G4MoleculeDefinition::NewConfiguration(const G4String& molConfLabel)
{
  bool alreadyExist(false);
  return G4MolecularConfiguration::CreateMolecularConfiguration(GetName() + "_" + molConfLabel,
                                                                this, molConfLabel, alreadyExist);
}
...
{
  return G4MolecularConfiguration::GetMolecularConfiguration(this, molecularConfLabel);
}

//______________________________________________________________________________

G4MolecularConfiguration* G4MoleculeDefinition::NewConfigurationWithElectronOccupancy(
  const G4String& exStId, const G4ElectronOccupancy& elecConf, G4double decayTime)
{
  G4bool alreadyExist(false);
  G4MolecularConfiguration* conf = G4MolecularConfiguration::CreateMolecularConfiguration(
    GetName() + "_" + exStId, this, exStId, elecConf, alreadyExist);
                                           ...
                                           const G4MolecularDissociationChannel* channel)
{
  ...
  fDecayTable->AddChannel(
    G4MolecularConfiguration::GetMolecularConfiguration(this, molecularConfLabel), channel);
}
...
G4MoleculeDefinition::GetDecayChannels(const G4String& ExState) const
{
  ...
  errMsg << ": no Excited States and Decays for" << GetName() << " are defined.";
...
const vector<const G4MolecularDissociationChannel*>*
G4MoleculeDefinition::GetDecayChannels(const G4MolecularConfiguration* conf) const
{
  if (fDecayTable != nullptr)
  {
    const vector<const G4MolecularDissociationChannel*>* output =
