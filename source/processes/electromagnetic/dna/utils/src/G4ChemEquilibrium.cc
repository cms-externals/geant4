...
{
  Reset();
  MolType H2O = G4MoleculeTable::Instance()->GetConfiguration("H2O");
  MolType H3OpB = G4MoleculeTable::Instance()->GetConfiguration("H3Op(B)");
  MolType OHmB = G4MoleculeTable::Instance()->GetConfiguration("OHm(B)");
  const auto& reactionList = G4DNAMolecularReactionTable::Instance()->GetVectorOfReactionData();
  for (const auto& it : reactionList)
  {
    if (it->GetReactionType() == fRectionType)
    {
      if (it->GetReactant1() != H2O && it->GetReactant1() != H3OpB && it->GetReactant1() != OHmB)
      {
...
{
  G4cout << "Equilibrium reactions : " << fReactant1->GetName() << " + " << fReactantB1->GetName()
         << " <=> " << fReactant2->GetName() << " + " << fReactantB2->GetName()
         << "  Status : " << fAddEquilibrium << " from " << G4BestUnit(fEquilibriumTime, "Time")
         << " to " << G4BestUnit(fEquilibriumTime + fEquilibriumDuration, "Time") << G4endl;
}
    ...
    for (const auto& it : molVector)
    {
      if (it == fReactant1 || it == fReactant2)
      {
        ...
        if (fVerbose > 1)
        {
          ...
          G4cout << "SetEquilibrium : on " << fRectionType
                 << "  fEquilibriumTime : " << G4BestUnit(fEquilibriumTime, "Time") << G4endl;
        }
        break;
      }
    }
