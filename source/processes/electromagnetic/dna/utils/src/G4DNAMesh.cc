...
G4DNAMesh::G4DNAMesh(const G4DNABoundingBox& boundingBox, G4int pixel)
  : fpBoundingMesh(&boundingBox), fResolution((2 * boundingBox.halfSideLengthInY() / pixel))
{}
...
void G4DNAMesh::PrintMesh()
{
  ...
  for (const auto& iter : fVoxelVector)
  {
    ...
    G4cout << "Index : " << std::get<0>(iter) << " number of type : " << std::get<2>(iter).size()
           << G4endl;
    for (const auto& it : data)
    {
      G4cout << "_____________" << it.first->GetName() << " : " << it.second << G4endl;
    }
    G4cout << G4endl;
  }
  G4cout << G4endl;
...
{
  G4cout << "*********PrintVoxel::";
  G4cout << " index : " << index << " number of type : " << this->GetVoxelMapList(index).size()
         << G4endl;
...
G4DNAMesh::FindNeighboringVoxels(const Index& index) const
{
  ...
  auto zMax =
    (G4int)(std::floor((fpBoundingMesh->Getzhi() - fpBoundingMesh->Getzlo()) / fResolution));
...
G4DNAMesh::Index G4DNAMesh::GetIndex(const G4ThreeVector& position) const
{
  if (!fpBoundingMesh->contains(position))
  {
    ...
    G4Exception("G4DNAMesh::GetKey", "G4DNAMesh010", FatalErrorInArgument, exceptionDescription);
  }

  G4int dx = std::floor((position.x() - fpBoundingMesh->Getxlo()) / fResolution);
  G4int dy = std::floor((position.y() - fpBoundingMesh->Getylo()) / fResolution);
  G4int dz = std::floor((position.z() - fpBoundingMesh->Getzlo()) / fResolution);
  if (dx < 0 || dy < 0 || dz < 0)
  {
    G4ExceptionDescription exceptionDescription;
...
G4VDNAMesh::Index G4DNAMesh::ConvertIndex(const Index& index, const G4int& pixels) const
{
  G4int xmax = std::floor((fpBoundingMesh->Getxhi() - fpBoundingMesh->Getxlo()) / fResolution);
  G4int ymax = std::floor((fpBoundingMesh->Getyhi() - fpBoundingMesh->Getylo()) / fResolution);
  G4int zmax = std::floor((fpBoundingMesh->Getzhi() - fpBoundingMesh->Getzlo()) / fResolution);
  ...
  {
    G4ExceptionDescription exceptionDescription;
    exceptionDescription << "the old index: " << index << "  to new index : " << Index(dx, dx, dx);
