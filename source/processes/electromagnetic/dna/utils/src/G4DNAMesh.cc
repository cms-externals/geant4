...
{
  auto xmax = (unsigned int) (floor(
    (fpBoundingMesh->Getxhi() - fpBoundingMesh->Getxlo()) / fResolution));
  auto ymax = (unsigned int) (floor(
...
{
  int dx = floor((position.x() - fpBoundingMesh->Getxlo()) / fResolution);
  int dy = floor((position.y() - fpBoundingMesh->Getylo()) / fResolution);
  int dz = floor((position.z() - fpBoundingMesh->Getzlo()) / fResolution);
...
{
  int xmax =
    floor((fpBoundingMesh->Getxhi() - fpBoundingMesh->Getxlo()) / fResolution);
  int ymax =
    floor((fpBoundingMesh->Getyhi() - fpBoundingMesh->Getylo()) / fResolution);
  int zmax =
    floor((fpBoundingMesh->Getzhi() - fpBoundingMesh->Getzlo()) / fResolution);
...
G4DNAMesh::Key G4DNAMesh::GetKey(const G4ThreeVector& position) const
{
  ...
  auto dx = floor((position.x() - fpBoundingMesh->Getxlo()) / fResolution);
  auto dy = floor((position.y() - fpBoundingMesh->Getylo()) / fResolution);
  auto dz = floor((position.z() - fpBoundingMesh->Getzlo()) / fResolution);
  auto xmax =
    floor((fpBoundingMesh->Getxhi() - fpBoundingMesh->Getxlo()) / fResolution);
  auto ymax =
    floor((fpBoundingMesh->Getyhi() - fpBoundingMesh->Getylo()) / fResolution);
