...
G4DNACPA100LogLogInterpolation::G4DNACPA100LogLogInterpolation() = default;
                                                   ...
                                                   const G4DataVector& data) const
{
  ...
  else if (bin < nBins)
  {
    ...
    // interpolated value below -- S.I., Jun. 2008
    if ((d1 > 0.) && (d2 > 0.) && (e1 > 0.) && (e2 > 0.))
    {
      // Streamline the Log-Log Interpolation formula in order to reduce the required number of
      // log10() function calls Variable oldresult contains the result of old implementation of
      // Log-Log interpolation -- M.G.P. Jun. 2001
      //       oldresult = (std::log10(d1)*std::log10(e2/x) + std::log10(d2)*std::log10(x/e1)) /
      //       std::log10(e2/e1); oldresult = std::pow(10.,oldresult);
      // Variable value contains the result of new implementation, after streamlining the math
      // operation -- N.A.K. Oct. 2008
      value = std::log10(d1) + (std::log10(d2 / d1) / std::log10(e2 / e1) * std::log10(x / e1));
      value = std::pow(10., value);
      // Test of the new implementation result (value variable) against the old one (oldresult) --
      // N.A.K. Dec. 2008
      ...
      //            {
      //              G4cout << "G4DNACPA100LogLogInterpolation> Old Interpolated Value is:" <<
      //              oldresult << G4endl; G4cout << "G4DNACPA100LogLogInterpolation> New
      //              Interpolated Value is:" << value << G4endl << G4endl; G4cerr <<
      //              "G4DNACPA100LogLogInterpolation> Error in Interpolation:" << G4endl; G4cerr <<
      //              "The difference between new and old interpolated value is:" << diffResult <<
      //              G4endl << G4endl;
      //            }
      //        }
    }
                                                   ...
                                                   const G4DataVector& log_data) const
{
  ...
  else if (bin < nBins)
  {
    ...
    // G4cout << "interpol " << log_d1 + (log_d2 - log_d1)*(log_x - log_e1)/(log_e2 - log_e1) << " "
    // << G4endl;
