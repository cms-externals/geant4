...
G4DNAMolecularReactionData::G4DNAMolecularReactionData(G4double reactionRate, Reactant* pReactant1,
                                                       Reactant* pReactant2)
...
void G4DNAMolecularReactionData::ComputeEffectiveRadius()
{
  ...
  if (fpReactant1 == fpReactant2)
  {
    ...
    fEffectiveReactionRadius =
      fObservedReactionRate / (4. * CLHEP::pi * sumDiffCoeff * CLHEP::Avogadro);
  }
  else
  {
    sumDiffCoeff = fpReactant1->GetDiffusionCoefficient() + fpReactant2->GetDiffusionCoefficient();
    fEffectiveReactionRadius =
      fObservedReactionRate / (4. * CLHEP::pi * sumDiffCoeff * CLHEP::Avogadro);
  }
  ...
  fOnsagerRadius = (fpReactant1->GetCharge() * fpReactant2->GetCharge())
                   / (4 * pi * epsilon0 * k_Boltzmann) / (293.15 * 80.1);
  fProbability = 1;
}
...
void G4DNAMolecularReactionData::SetReactants(Reactant* pReactant1, Reactant* pReactant2)
{
  fpReactant1 = pReactant1;
  fpReactant2 = pReactant2;
}
...
void G4DNAMolecularReactionData::SetReactants(const G4String& reactant1, const G4String& reactant2)
{
...
void G4DNAMolecularReactionData::SetReactionType(G4int type)
{
  ...
  if (type == 1)
  {
    ...
    fReactionRadius = fpReactant1->GetVanDerVaalsRadius() + fpReactant2->GetVanDerVaalsRadius();
    ...
    if (fOnsagerRadius == 0)  // Type II
    {
      ...
      fActivationRate =
        fDiffusionRate * fObservedReactionRate / (fDiffusionRate - fObservedReactionRate);
...
double G4DNAMolecularReactionData::PolynomialParam(double temp_K, std::vector<double> P)
{
  ...
  return pow(10, P[0] + P[1] * inv_temp + P[2] * pow(inv_temp, 2) + P[3] * pow(inv_temp, 3)
                   + P[4] * pow(inv_temp, 4))
...
{
  return P[0] * G4Exp(P[1] / temp_K) * (1e-3 * CLHEP::m3 / (CLHEP::mole * CLHEP::s));
}

double G4DNAMolecularReactionData::ScaledParameterization(double temp_K, double temp_init,
...
void G4DNAMolecularReactionTable::SetReaction(G4double reactionRate, Reactant* pReactant1,
                                              Reactant* pReactant2)
{
...
void G4DNAMolecularReactionTable::PrintTable(G4VDNAReactionModel* pReactionModel)
{
  ...
  G4cout << "Number of chemical species involved in reactions = " << fReactantsMV.size() << G4endl;
  ...
  for (itReactives = fReactantsMV.begin(); itReactives != fReactantsMV.end(); ++itReactives)
  {
    ...
    for (G4int iReact = 0; iReact < nbReactants; iReact++)
    {
      ...
      if (!alreadyPrint[moleculeA][moleculeB])
      {
        ...
        if (pReactionModel != nullptr) interactionRange = pReactionModel->GetReactionRadius(iReact);
  ...
  if (pReactionModel != nullptr) title[2] = "Interaction Range for chosen reaction model [nm]";
  ...
  G4cout.fill('-');
  if (pReactionModel != nullptr)
    G4cout.width(maxlengthOutputReaction + 2 + maxlengthOutputReactionRate + 2
...
G4DNAMolecularReactionTable::GetReactionData(Reactant* pReactant1, Reactant* pReactant2) const
{
  ...
  if (it1 == fReactionData.end())
  {
    if (fVerbose)
    {
      ...
      errMsg << "No reaction table was implemented for this molecule Definition : "
                  + pReactant1->GetName();
      G4Exception(G4String("G4MolecularInteractionTable::GetReactionData"), "", JustWarning,
                  errMsg);
    }
    return nullptr;
  }
...
const G4DNAMolecularReactionTable::ReactionDataMap&
...
G4DNAMolecularReactionTable::CanReactWith(Reactant* pMolecule) const
{
  ...
  if (itReactivesMap == fReactantsMV.end())
  {
    ...
    {
      G4String errMsg =
        "No reaction table was implemented for this molecule : " + pMolecule->GetName();
      //        	G4Exception("G4MolecularInteractionTable::CanReactWith","",FatalErrorInArgument,
      //        errMsg);
...
G4DNAMolecularReactionTable::Data*
G4DNAMolecularReactionTable::GetReactionData(const G4String& mol1, const G4String& mol2) const
{
...
void G4DNAMolecularReactionData::SetArrehniusParameterization(double A0, double E_R)
{
...
void G4DNAMolecularReactionData::SetScaledParameterization(double temperature_K, double rateCste)
{
  fRateParam = std::bind(ScaledParameterization, std::placeholders::_1, temperature_K, rateCste);
}
