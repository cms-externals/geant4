...
#ifndef G4DNAREVERTPROBABILITY_HH
#define G4DNAREVERTPROBABILITY_HH
...
class G4DNARevertProbability
{
  public:
    ...
    G4DNARevertProbability(double toleranceY) : fToleranceProba(toleranceY)
    {
      fXmax = 0.;
      fXmin = 0.;
      fXminDef = 0.;
    ...
    void SetTolerance(double toleranceY) { fToleranceProba = toleranceY; }
    ...
    double Revert(double probaForSearchedTime, std::function<double(double)>& funct)
    {
      ...
      fIncreasingCumulativeFunction = (funct(fXmax) - funct(fXmin)) / (fXmax - fXmin);
