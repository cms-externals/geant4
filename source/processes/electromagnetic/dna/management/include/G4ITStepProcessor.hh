...
#ifndef G4ITSTEPPROCESSOR_HH
#define G4ITSTEPPROCESSOR_HH
...
class G4ITStepProcessor
{
  ...
  public:
    ...
    inline G4TrackVector* GetSecondaries() const { return fpSecondary; }
    ...
    G4double GetILTimeStep() { return fILTimeStep; }
    ...
    std::size_t GetAtRestDoItProcTriggered() const { return fAtRestDoItProcTriggered; }

    G4GPILSelection GetGPILSelection() const { return fGPILSelection; }

    G4int GetN2ndariesAlongStepDoIt() const { return fN2ndariesAlongStepDoIt; }

    G4int GetN2ndariesAtRestDoIt() const { return fN2ndariesAtRestDoIt; }

    G4int GetN2ndariesPostStepDoIt() const { return fN2ndariesPostStepDoIt; }

    const G4VITProcess* GetCurrentProcess() const { return fpCurrentProcess; }

    G4double GetPhysIntLength() const { return fPhysIntLength; }
    ...
    std::size_t GetPostStepDoItProcTriggered() const { return fPostStepDoItProcTriggered; }

    const ProcessGeneralInfo* GetCurrentProcessInfo() const { return fpProcessInfo; }

    const G4ITStepProcessorState* GetProcessorState() const { return fpState; }

    const G4VParticleChange* GetParticleChange() const { return fpParticleChange; }

    const G4VPhysicalVolume* GetCurrentVolume() const { return fpCurrentVolume; }

    G4ForceCondition GetCondition() const { return fCondition; }
...
{
  return std::max(fpState->fEndpointSafety
                    - (fpState->fEndpointSafOrigin - fpPostStepPoint->GetPosition()).mag(),
                  kCarTolerance);
}
