...
const G4DNABoundingBox initial =
  G4DNABoundingBox{-std::numeric_limits<G4double>::max(), std::numeric_limits<G4double>::max(),
                   -std::numeric_limits<G4double>::max(), std::numeric_limits<G4double>::max(),
                   -std::numeric_limits<G4double>::max(), std::numeric_limits<G4double>::max()};
const G4DNABoundingBox invalid = G4DNABoundingBox{
  std::numeric_limits<G4double>::quiet_NaN(), std::numeric_limits<G4double>::quiet_NaN(),
  std::numeric_limits<G4double>::quiet_NaN(), std::numeric_limits<G4double>::quiet_NaN(),
  std::numeric_limits<G4double>::quiet_NaN(), std::numeric_limits<G4double>::quiet_NaN()};
...
template<typename Iterator>
G4DNABoundingBox::G4DNABoundingBox(Iterator begin, Iterator end) : G4DNABoundingBox(initial)
{
  for (; begin != end; ++begin)
  {
