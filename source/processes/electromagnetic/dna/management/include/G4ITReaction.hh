...
#ifndef G4ITREACTIONINFO_HH
#define G4ITREACTIONINFO_HH
...
using G4ITReactionPerTrackMap = std::map<G4Track*, G4ITReactionPerTrackPtr, compTrackPerID>;
using G4ReactionPerTrackIt =
  std::list<std::pair<G4ITReactionPerTrackPtr, G4ITReactionList::iterator>>;
...
class G4ITReactionPerTrack : public G4enable_shared_from_this<G4ITReactionPerTrack>
{
  ...
  public:
    ...
    virtual ~G4ITReactionPerTrack() { fReactions.clear(); }

    void AddReaction(G4ITReactionPtr reaction)
    {
      auto it = fReactions.insert(fReactions.end(), reaction);
      reaction->AddIterator(this->shared_from_this(), it);
    }

    void AddIterator(G4ITReactionPerTrackMap::iterator it) { fReactionSetIt.push_back(it); }
    ...
    void RemoveMe()
    {
      ...
      for (auto it = fReactions.begin(); it != fReactions.end(); it = next)
      {
        next = it;
        ++next;
        (*it)->RemoveMe();
    ...
    G4ITReactionList& GetReactionList() { return fReactions; }

    std::list<G4ITReactionPerTrackMap::iterator>& GetListOfIterators() { return fReactionSetIt; }
...
class G4ITReactionSet
{
  ...
  public:
    ...
    G4ITReactionPerTrackMap& GetReactionMap() { return fReactionPerTrack; }
    ...
    {
      for (auto it = fReactionPerTrack.begin(); it != fReactionPerTrack.end();
    ...
    G4bool Empty() { return fReactionPerTrack.empty(); }

    G4ITReactionPerTime& GetReactionsPerTime() { return fReactionPerTime; }

    void SortByTime() { fSortByTime = true; }
