...
// ----------------------------------------------------------------------------
//
void G4ITPathFinder::EnableParallelNavigation(G4bool enableChoice)
{
  /*
...
// ----------------------------------------------------------------------------
//
G4double G4ITPathFinder::ComputeStep(const G4FieldTrack& InitialFieldTrack,
                                     ...
                                     G4VPhysicalVolume* /*currentVolume*/)
{
  ...
  if (navigatorNo >= fNoActiveNavigators)
  {
    ...
    G4Exception("G4ITPathFinder::ComputeStep()", "GeomNav0002", FatalException, message);
  }
#endif
  ...
  else
  {
    if (proposedStepLength < fTrueMinStep)  // For 2nd+ geometry
    {
      ...
      message << "Problem in step size request." << G4endl
              ...
              << "        This can happen due to an error in process ordering." << G4endl
              << "        Check that all physics processes are registered" << G4endl
              << "        before all processes with a navigator/geometry." << G4endl
              << "        If using pre-packaged physics list and/or" << G4endl
              ...
              << "        Difference min-req = " << fTrueMinStep - proposedStepLength << G4endl
              << "     -- Step info> stepNo= " << stepNo << " last= " << fLastStepNo
              << " newTr= " << fNewTrack << G4endl;
      G4Exception("G4ITPathFinder::ComputeStep()", "GeomNav0003", FatalException, message);
    }
    else
    {
  ...
  if (fVerboseLevel > 0)
  {
    G4cout << " G4ITPathFinder::ComputeStep returns " << fCurrentStepSize[navigatorNo]
...
void G4ITPathFinder::PrepareNewTrack(const G4ThreeVector& position, const G4ThreeVector& direction,
                                     G4VPhysicalVolume* massStartVol)
{
  ...
  if (fNoActiveNavigators > G4ITNavigator::fMaxNav)
  {
    ...
    message << "Too many active Navigators / worlds." << G4endl
            ...
            << G4endl << "        This is more than the number allowed = " << G4ITNavigator::fMaxNav
            << " !";
    G4Exception("G4ITPathFinder::PrepareNewTrack()", "GeomNav0002", FatalException, message);
  }
...
void G4ITPathFinder::ReportMove(const G4ThreeVector& OldVector, const G4ThreeVector& NewVector,
                                const G4String& Quantity) const
{
  ...
  message << "Endpoint moved between value returned by ComputeStep()"
          ...
          << (1.0 / mm) * moveVec << " mm " << G4endl << "          Endpoint of ComputeStep() was "
          << OldVector << G4endl << "          and current position to locate is " << NewVector;
  G4Exception("G4ITPathFinder::ReportMove()", "GeomNav1002", JustWarning, message);
  G4cerr.precision(prc);
}

void G4ITPathFinder::Locate(const G4ThreeVector& position, const G4ThreeVector& direction,
                            G4bool relative)
{
  ...
  if ((!fNewTrack) && (!fRelocatedPoint) && (moveLenSq > 10 * kCarTolerance * kCarTolerance))
  {
...
void G4ITPathFinder::ReLocate(const G4ThreeVector& position)
{
  ...
  if ((!fNewTrack) && (longMoveEnd && longMoveSaf))
  {
    ...
    if (longMoveEnd && longMoveSaf && longMoveRevisedEnd && (moveMinusSafety > 0.0))
    {
      ...
      G4Exception("G4ITPathFinder::ReLocate", "GeomNav0003", FatalException, message);
...
G4double G4ITPathFinder::ComputeSafety(const G4ThreeVector& position)
{
  ...
  if (fVerboseLevel > 1)
  {
    G4cout << " G4ITPathFinder::ComputeSafety - returns " << minSafety << " at location "
           << position << G4endl;
  }
#endif
  return minSafety;
...
G4TouchableHandle G4ITPathFinder::CreateTouchableHandle(G4int navId) const
{
  ...
  if (fVerboseLevel > 2)
  {
    G4cout << "G4ITPathFinder::CreateTouchableHandle : navId = " << navId << " -- "
...
G4double G4ITPathFinder::DoNextLinearStep(const G4FieldTrack& initialState,
...
#  ifdef G4DEBUG_PATHFINDER
        G4cout.precision(8);
        G4cout << "G4ITNavigator::ComputeStep> small proposed step = " << proposedStepLength
...
#  endif
...
#endif  // End of G4PATHFINDER_OPTIMISATION 2
      {
        ...
        {
          G4cout.precision(8);
          G4cout << "G4ITNavigator::ComputeStep> long  proposed step = " << proposedStepLength
      ...
      if (fVerboseLevel > 2)
      {
        G4cout << "G4ITPathFinder::DoNextLinearStep : Navigator [" << num << "] -- step size "
               << step << G4endl;
      }
#endif
    }
  ...
  if (fVerboseLevel > 2)
  {
    G4cout << " G4ITPathFinder::DoNextLinearStep : exits returning " << minStep << G4endl;
...
G4double G4ITPathFinder::DoNextCurvedStep(const G4FieldTrack& initialState,
                                          G4double proposedStepLength,
...
*/
  ...
  if (fVerboseLevel > 2)
  {
    G4cout << "G4ITPathFinder::DoNextCurvedStep : " << G4endl << " initialState = " << initialState
    ...
    for (numNav = 0; numNav < fNoActiveNavigators; ++numNav)
    {
      ...
      if (StepError || (fVerboseLevel > 2))
      {
        ...
        if (StepError)
        {
          ...
          G4Exception("G4ITPathFinder::DoNextCurvedStep", "GeomNav0003", FatalException, message);
        }
      }
#endif
    }  // for num Navigators
