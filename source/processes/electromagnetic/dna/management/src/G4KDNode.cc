...
//______________________________________________________________________
G4KDNode_Base::G4KDNode_Base(G4KDTree* tree, G4KDNode_Base* parent) : fTree(tree), fParent(parent)
{
  fSide = 0;
...
G4KDNode_Base::~G4KDNode_Base() = default;
...
void G4KDNode_Base::RetrieveNodeList(std::list<G4KDNode_Base*>& output)
{
  ...
  if (fLeft != nullptr) fLeft->RetrieveNodeList(output);

  if (fRight != nullptr) fRight->RetrieveNodeList(output);
}
