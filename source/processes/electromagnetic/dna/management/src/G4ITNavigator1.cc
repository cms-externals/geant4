...
// ********************************************************************
//
G4VPhysicalVolume* G4ITNavigator1::ResetHierarchyAndLocate(const G4ThreeVector& p,
...
// ********************************************************************
//
G4VPhysicalVolume* G4ITNavigator1::LocateGlobalPointAndSetup(const G4ThreeVector& globalPoint,
      ...
      else if (fEntering)
      {
        switch (VolumeType(fBlockedPhysicalVolume))
        {
          ...
          case kParameterised:
            ...
            break;
          case kExternal:
            G4Exception("G4ITNavigator1::LocateGlobalPointAndSetup()", "GeomNav0001",
  ...
  do
  {
    ...
    switch (CharacteriseDaughters(targetLogical))
    {
      ...
      case kParameterised:
        ...
        break;
      case kExternal:
        G4Exception("G4ITNavigator1::LocateGlobalPointAndSetup()", "GeomNav0001", FatalException,
...
// ********************************************************************
//
void G4ITNavigator1::LocateGlobalPointWithinVolume(const G4ThreeVector& pGlobalpoint)
{
  ...
  if (fHistory.GetTopVolumeType() != kReplica)
  {
    switch (CharacteriseDaughters(motherLogical))
    {
      ...
      case kParameterised:
        ...
        break;
      case kReplica:
        G4Exception("G4ITNavigator1::LocateGlobalPointWithinVolume()", "GeomNav0001",
...
#  endif
  ...
  if (fHistory.GetTopVolumeType() != kReplica)
  {
    switch (CharacteriseDaughters(motherLogical))
    {
      case kNormal:
        ...
        else
        {
          ...
          else  // Regular (non-voxelised) structure
          {
            ...
            {
              G4Exception("G4ITNavigator1::ComputeStep()", "GeomNav1001", JustWarning,
      ...
      case kParameterised:
        ...
        break;
      case kReplica:
        G4Exception("G4ITNavigator1::ComputeStep()", "GeomNav0001", FatalException,
  ...
  if (fLastStepWasZero)
  {
    ...
    {
      G4cout << "G4ITNavigator1::ComputeStep(): another zero step, # " << fNumberZeroSteps << " at "
             << pGlobalpoint << " in volume " << motherPhysical->GetName()
             << " nav-comp-step calls # " << sNavCScalls << G4endl;
    }
#endif
    if (fNumberZeroSteps > fActionThreshold_NoZeroSteps - 1)
    {
      ...
      if ((!fPushed) && (fWarnPush))
      {
        ...
        G4Exception("G4ITNavigator1::ComputeStep()", "GeomNav1002", JustWarning, message,
    ...
    if (fNumberZeroSteps > fAbandonThreshold_NoZeroSteps - 1)
    {
      ...
      G4Exception("G4ITNavigator1::ComputeStep()", "GeomNav0003", EventMustBeAborted, message);
    }
  }
  else
  {
  ...
  if (fExiting)
  {
    ...
    else
    {
      ...
      else
      {
        ...
        G4Exception("G4ITNavigator1::ComputeStep()", "GeomNav0003", JustWarning, desc,
  ...
  for (i = 1; i <= cdepth; i++)
  {
    ...
    switch (fHistory.GetVolumeType(i))
    {
      ...
      case kParameterised: {
        ...
        pLogical->SetSolid(pSolid);
        pLogical->UpdateMaterial(pParam->ComputeMaterial(replicaNo, current, &touchable));
        break;
      }
      case kExternal:
        G4Exception("G4ITNavigator1::SetupHierarchy()", "GeomNav0001", FatalException,
                    "Not applicable for external volumes.");
        break;
    }
  }
...
G4ThreeVector G4ITNavigator1::GetLocalExitNormal(G4bool* valid)
{
  ...
  if (fLastTriedStepComputation)
  {
        ...
        // The following code should also work in case of Replica
        {
          ...
          else
          {
            ...
            if ((fVerbose == 1) && (fCheck))
            {
              ...
              G4Exception("G4ITNavigator1::GetLocalExitNormal()", "GeomNav1001", JustWarning,
                          message);
            }
#endif
          }
    ...
    {
      *valid = false;
      G4Exception("G4ITNavigator1::GetLocalExitNormal()", "GeomNav0003", JustWarning,
  ...
  else  //  ( ! fLastTriedStepComputation ) ie. last call was to Locate
  {
    if (EnteredDaughterVolume())
    {
      ...
      if (std::fabs(ExitNormal.mag2() - 1.0) > CLHEP::perMillion)
      {
        ...
        G4Exception("G4ITNavigator1::GetLocalExitNormal()", "GeomNav0003", FatalException, desc,
    ...
    else
    {
      ...
      else  // We are not at a boundary. ExitNormal remains (0,0,0)
      {
        *valid = false;
        ...
        G4Exception("G4ITNavigator1::GetLocalExitNormal()", "GeomNav0003", JustWarning, message);
      }
    }
  }
  return ExitNormal;
                                             ...
                                             G4int enteringReplicaNo, EVolume enteringVolumeType)
{
  switch (enteringVolumeType)
  {
    ...
    case kReplica:  // Sets the transform in the Replica - tbc
      G4Exception("G4ITNavigator1::GetMotherToDaughterTransform()", "GeomNav0001", FatalException,
...
// ********************************************************************
//
G4ThreeVector G4ITNavigator1::GetLocalExitNormalAndCheck(
#ifdef G4DEBUG_NAVIGATION
...
// ********************************************************************
//
G4ThreeVector G4ITNavigator1::GetGlobalExitNormal(const G4ThreeVector& IntersectPointGlobal,
  ...
  else
  {
    ...
    *pNormalCalculated = fCalculatedExitNormal;
    ...
    if ((!validNormal) && !fCalculatedExitNormal)
    {
      ...
      G4Exception("G4ITNavigator1::GetGlobalExitNormal()", "GeomNav0003", JustWarning, edN,
    ...
    if (validNormal && (std::fabs(localMag2 - 1.0)) > CLHEP::perMillion)
    {
      ...
      edN << "G4ITNavigator1::GetGlobalExitNormal: "
          ...
          << "  Local  Exit Normal = " << localNormal << " || = " << std::sqrt(localMag2) << G4endl
  ...
  if (fLastTriedStepComputation && fExiting)
  {
    ...
    *pNormalCalculated = fCalculatedExitNormal;
    ...
    if (diffNorm.mag2() > perMillion * CLHEP::perMillion)
    {
      ...
      G4Exception("G4ITNavigator1::GetGlobalExitNormal()", "GeomNav0003", JustWarning, edDfn);
    }
  }
#endif
...
// ********************************************************************
//
G4double G4ITNavigator1::ComputeSafety(const G4ThreeVector& pGlobalpoint, const G4double pMaxLength,
                                       const G4bool keepState)
{
  ...
  if (!(endpointOnSurface && stayedOnEndpoint))
  {
    ...
    if (fVerbose >= 2)
    {
      G4cout << "  G4ITNavigator1::ComputeSafety() relocates-in-volume to point: " << pGlobalpoint
             << G4endl;
    }
#endif
    G4VPhysicalVolume* motherPhysical = fHistory.GetTopVolume();
    ...
    if (fHistory.GetTopVolumeType() != kReplica)
    {
      switch (CharacteriseDaughters(motherLogical))
      {
        ...
        case kParameterised:
          ...
          break;
        case kReplica:
          G4Exception("G4ITNavigator1::ComputeSafety()", "GeomNav0001", FatalException,
                      "Not applicable for replicated volumes.");
          break;
        case kExternal:
          G4Exception("G4ITNavigator1::ComputeSafety()", "GeomNav0001", FatalException,
  ...
  else  // if( endpointOnSurface && stayedOnEndpoint )
  {
    ...
    if (fVerbose >= 2)
    {
      G4cout << "    G4ITNavigator1::ComputeSafety() finds that point - " << pGlobalpoint
...
void G4ITNavigator1::PrintState() const
{
  ...
  if (fVerbose == 4)
  {
    ...
    G4cout << "  ValidExitNormal= " << fValidExitNormal << G4endl
           << "  ExitNormal     = " << fExitNormal << G4endl << "  Exiting        = " << fExiting
           << G4endl << "  Entering       = " << fEntering << G4endl << "  BlockedPhysicalVolume= ";
    ...
    G4cout << G4endl << "  BlockedReplicaNo     = " << fBlockedReplicaNo << G4endl
           << "  LastStepWasZero      = " << fLastStepWasZero << G4endl << G4endl;
  }
