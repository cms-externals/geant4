...
void G4ITStepProcessor::Initialize()
{
  ...
  SetNavigator(G4ITTransportationManager::GetTransportationManager()->GetNavigatorForTracking());
  ...
  while ((*theParticleIterator)())
  {
    ...
    if (pm == nullptr)
    {
      G4cerr << "ERROR - G4ITStepProcessor::GetProcessNumber()" << G4endl
             << "        ProcessManager is NULL for particle = " << particle->GetParticleName()
             << ", PDG_code = " << particle->GetPDGEncoding() << G4endl;
      G4Exception("G4ITStepProcessor::GetProcessNumber()", "ITStepProcessor0001", FatalException,
                                                ...
                                                G4ProcessManager* pm)
{
  ...
  if (pm == nullptr)
  {
    G4cerr << "ERROR - G4SteppingManager::GetProcessNumber()" << G4endl
           << "        ProcessManager is NULL for particle = " << particle->GetParticleName()
           << ", PDG_code = " << particle->GetPDGEncoding() << G4endl;
    G4Exception("G4SteppingManager::GetProcessNumber()", "ITStepProcessor0002", FatalException,
                "Process Manager is not found.");
    return;
  }

  auto it = fProcessGeneralInfoMap.find(particle);
...
#ifdef debug
  G4cout << "G4ITStepProcessor::GetProcessNumber: #ofAtRest=" << fpProcessInfo->MAXofAtRestLoops
         << G4endl;
#endif
...
#ifdef debug
  G4cout << "G4ITStepProcessor::GetProcessNumber:#ofAlongStp=" << fpProcessInfo->MAXofAlongStepLoops
         << G4endl;
#endif
...
#ifdef debug
  G4cout << "G4ITStepProcessor::GetProcessNumber: #ofPostStep=" << fpProcessInfo->MAXofPostStepLoops
         << G4endl;
#endif
      ...
      || SizeOfSelectedDoItVector < fpProcessInfo->MAXofPostStepLoops)
  {
    G4cerr << "ERROR - G4ITStepProcessor::GetProcessNumber()" << G4endl
           ...
           << " ; is smaller then one of MAXofAtRestLoops= " << fpProcessInfo->MAXofAtRestLoops
           << G4endl << "        or MAXofAlongStepLoops= " << fpProcessInfo->MAXofAlongStepLoops
           << " or MAXofPostStepLoops= " << fpProcessInfo->MAXofPostStepLoops << G4endl;
    G4Exception("G4ITStepProcessor::GetProcessNumber()", "ITStepProcessor0004", FatalException,
      ...
      && (fpProcessInfo->fpPostStepDoItVector == nullptr))
  {
    ...
    G4Exception("G4ITStepProcessor::DoStepping", "ITStepProcessor0005", FatalErrorInArgument,
      ...
      && fpProcessInfo->MAXofAlongStepLoops > 0)
  {
    fpProcessInfo->fpTransportation = dynamic_cast<G4ITTransportation*>(
      (*fpProcessInfo->fpAlongStepGetPhysIntVector)[G4int(fpProcessInfo->MAXofAlongStepLoops - 1)]);
...
void G4ITStepProcessor::SetTrack(G4Track* track)
{
  ...
  if (fpTrack != nullptr)
  {
    ...
    else
    {
      ...
      G4Exception("G4ITStepProcessor::SetTrack", "ITStepProcessor0007", FatalErrorInArgument,
                  errMsg);
    }
  }
  else
...
#ifdef DEBUG
    G4cout << "*CIL* " << GetIT(track)->GetName() << " ID: " << track->GetTrackID()
           << " at time : " << track->GetGlobalTime() << G4endl;
#endif
...
void G4ITStepProcessor::SetInitialStep()
{
  ...
  if (!fpTrack->GetTouchableHandle())
  {
    ...
    fpNavigator->LocateGlobalPointAndSetup(fpTrack->GetPosition(), &direction, false,
  ...
  else
  {
    ...
    {
      fpNavigator->NewNavigatorState(*((G4TouchableHistory*)fpState->fTouchableHandle()));
      fpITrack->GetTrackingInfo()->SetNavigatorState(fpNavigator->GetNavigatorState());
    }
    ...
    G4VPhysicalVolume* newTopVolume =
      fpNavigator->ResetHierarchyAndLocate(fpTrack->GetPosition(), fpTrack->GetMomentumDirection(),
  ...
  if (fpCurrentVolume == nullptr)
  {
    ...
    if (fpTrack->GetParentID() == 0)
    {
      G4cerr << "ERROR - G4ITStepProcessor::SetInitialStep()" << G4endl
             << "        Primary particle starting at - " << fpTrack->GetPosition()
             << " - is outside of the world volume." << G4endl;
      G4Exception("G4ITStepProcessor::SetInitialStep()", "ITStepProcessor0011", FatalException,
    ...
    G4cout << "WARNING - G4ITStepProcessor::SetInitialStep()" << G4endl
           << "          Initial track position is outside world! - " << fpTrack->GetPosition()
           << G4endl;
  }
  else
  {
...
void G4ITStepProcessor::DoDefinePhysicalStepLength()
{
  ...
  {
    fpCurrentProcess =
      dynamic_cast<G4VITProcess*>((*fpProcessInfo->fpPostStepGetPhysIntVector)[(G4int)np]);
    if (fpCurrentProcess == nullptr)
    {
    ...
    //        << G4endl;
    fPhysIntLength =
      fpCurrentProcess->PostStepGPIL(*fpTrack, fpState->fPreviousStepSize, &fCondition);
    ...
    {
      for (std::size_t nrest = np + 1; nrest < fpProcessInfo->MAXofPostStepLoops; ++nrest)
      {
  ...
  {
    fpCurrentProcess =
      dynamic_cast<G4VITProcess*>((*fpProcessInfo->fpAlongStepGetPhysIntVector)[(G4int)kp]);
    ...
    fpCurrentProcess->SetProcessState(
      fpTrackingInfo->GetProcessState(fpCurrentProcess->GetProcessID()));
    fPhysIntLength =
      fpCurrentProcess->AlongStepGPIL(*fpTrack, fpState->fPreviousStepSize, fpState->fPhysicalStep,
    ...
    if (fPhysIntLength < fpState->fPhysicalStep)
    {
      ...
      if (kp == fpProcessInfo->MAXofAlongStepLoops - 1)
      {
        ...
        if (fpTrack->GetNextVolume() != nullptr)
    ...
    else
    {
      if (fPostStepDoItProcTriggered < fpProcessInfo->MAXofPostStepLoops)
      {
        ...
        {
          (fpState->fSelectedPostStepDoItVector)[fPostStepDoItProcTriggered] = NotForced;
        }
      }
    }
