...
void G4mplIonisationModel::SetParticle(const G4ParticleDefinition* p)
{
  ...
  G4double emax = std::max(HighEnergyLimit(), 10. * mass * (1. / std::sqrt(1. - beta2lim) - 1.));
...
void G4mplIonisationModel::Initialise(const G4ParticleDefinition* p, const G4DataVector&)
{
  if (nullptr == monopole)
  {
    SetParticle(p);
...
G4double G4mplIonisationModel::Dispersion(const G4Material* material, const G4DynamicParticle* dp,
