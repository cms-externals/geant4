  ...
  *condition = NotForced;
  ...
  if (verboseLevel > 2)
  {
    ...
    G4cout << "  Process: " << GetProcessName()
           << " for unknown particle Mass(GeV)=" << fMass / CLHEP::GeV << " charge=" << fCharge
           << "  Material " << fMaterial->GetName() << "  Ekin(MeV)=" << fEkinPreStep / CLHEP::MeV
                                                              ...
                                                              const G4Step& step)
{
  ...
  {
    eloss =
      fUrban->SampleFluctuations(fCouple, track.GetDynamicParticle(), fCut, fTmax, length, eloss);
  }
...
G4double G4DynamicParticleIonisation::GetContinuousStepLimit(const G4Track&, G4double, G4double,
                                                             G4double&)
{
  return DBL_MAX;
}
