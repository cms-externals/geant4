...
G4eeToPGammaModel::G4eeToPGammaModel(G4eeCrossSections* cr, const G4String& npart,
                                     G4double maxkinEnergy, G4double binWidth)
  : G4Vee2hadrons(cr, npart == "pi0" ? 782.62 * MeV : 1019.46 * MeV, maxkinEnergy, binWidth)
{
  G4cout << "####G4eeToPGammaModel & particle:" << npart << "####" << G4endl;
...
G4double G4eeToPGammaModel::ComputeCrossSection(G4double e) const
{
  G4double xs;
  if (particle == pi0)
    xs = cross->CrossSectionPi0G(e);
  else
    xs = cross->CrossSectionEtaG(e);
  return xs;
}

//....oooOO0OOooo........oooOO0OOooo........oooOO0OOooo........oooOO0OOooo....

void G4eeToPGammaModel::SampleSecondaries(std::vector<G4DynamicParticle*>* newp, G4double e,
                                          const G4ThreeVector& direction)
{
  ...
  G4DynamicParticle* p1 = new G4DynamicParticle(particle, dir, tkin);
  G4DynamicParticle* p2 = new G4DynamicParticle(G4Gamma::Gamma(), -dir, egam);
  newp->push_back(p1);
  newp->push_back(p2);
}
