...
void G4eeToHadronsMultiModel::Initialise(const G4ParticleDefinition*, const G4DataVector& cuts)
{
  if (!isInitialised)
  {
    ...
    G4eeToTwoPiModel* m2pi = new G4eeToTwoPiModel(cross, maxKineticEnergy, delta);
    AddEEModel(m2pi, cuts);

    G4eeTo3PiModel* m3pi = new G4eeTo3PiModel(cross, maxKineticEnergy, delta);
    AddEEModel(m3pi, cuts);

    G4ee2KChargedModel* m2kc = new G4ee2KChargedModel(cross, maxKineticEnergy, delta);
    AddEEModel(m2kc, cuts);

    G4ee2KNeutralModel* m2kn = new G4ee2KNeutralModel(cross, maxKineticEnergy, delta);
    AddEEModel(m2kn, cuts);

    G4eeToPGammaModel* mpg1 = new G4eeToPGammaModel(cross, "pi0", maxKineticEnergy, delta);
    AddEEModel(mpg1, cuts);

    G4eeToPGammaModel* mpg2 = new G4eeToPGammaModel(cross, "eta", maxKineticEnergy, delta);
...
void G4eeToHadronsMultiModel::AddEEModel(G4Vee2hadrons* mod, const G4DataVector& cuts)
{
  G4eeToHadronsModel* model = new G4eeToHadronsModel(mod, verbose);
  models.push_back(model);
  G4double elow = mod->LowEnergy();
  ekinMin.push_back(elow);
  if (thKineticEnergy > elow)
  {
    thKineticEnergy = elow;
  }
...
{
  return mat->GetElectronDensity() * ComputeCrossSectionPerElectron(p, kineticEnergy);
}
...
void G4eeToHadronsMultiModel::ModelDescription(std::ostream& outFile) const
{
  ...
  {
    G4double e1 =
      0.5 * thKineticEnergy * thKineticEnergy / electron_mass_c2 - 2.0 * electron_mass_c2;
    G4double e2 =
      0.5 * maxKineticEnergy * maxKineticEnergy / electron_mass_c2 - 2.0 * electron_mass_c2;
    outFile << "      e+ annihilation into hadrons active from " << e1 / GeV << " GeV to "
            << e2 / GeV << " GeV" << G4endl;
  }
}
  ...
  if (fac > 1.0)
  {
    ...
    if (verbose > 0)
    {
      G4cout << "### G4eeToHadronsMultiModel: The cross section for "
             << "G4eeToHadronsMultiModel is increased by " << csFactor << " times" << G4endl;
    }
  }
}
