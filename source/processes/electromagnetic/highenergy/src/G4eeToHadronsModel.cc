...
G4eeToHadronsModel::G4eeToHadronsModel(G4Vee2hadrons* mod, G4int ver, const G4String& nam)
  : G4VEmModel(nam), model(mod), verbose(ver)
{
...
void G4eeToHadronsModel::Initialise(const G4ParticleDefinition*, const G4DataVector&)
{
  ...
  if (verbose > 0)
  {
    ...
    G4cout << "CM System: emin(MeV)= " << emin / MeV << " epeak(MeV)= " << epeak / MeV
           << " emax(MeV)= " << emax / MeV << G4endl;
  }
  ...
  if (verbose > 1)
  {
    G4cout << "G4eeToHadronsModel: Cross sections per electron"
           << " nbins= " << nbins << " emin(MeV)= " << emin / MeV << " emax(MeV)= " << emax / MeV
           << G4endl;
    for (G4int i = 0; i < nbins; ++i)
    {
      G4double e = crossPerElectron->Energy(i);
      ...
      G4cout << "E(MeV)= " << e / MeV << "  cross(nb)= " << s1 / nanobarn
             << "  crossBorn(nb)= " << s2 / nanobarn << G4endl;
    }
  }
}
...
void G4eeToHadronsModel::SampleSecondaries(std::vector<G4DynamicParticle*>* newp,
                                           ...
                                           const G4DynamicParticle* dParticle, G4double, G4double)
{
  G4double t = dParticle->GetKineticEnergy() + 2 * electron_mass_c2;
  G4LorentzVector inlv =
    dParticle->Get4Momentum() + G4LorentzVector(0.0, 0.0, 0.0, electron_mass_c2);
  G4double e = inlv.m();
  ...
  if (e > emin)
  {
    ...
    {
      G4cout << "G4eeToHadronsModel::SampleSecondaries: Ebalance(MeV)= " << t / MeV
             << " primary 4-momentum: " << inlv << G4endl;
    }
  }
}
    ...
    */
    ...
    do
    {
      ...
      */
      ...
      if (f > grej)
      {
        G4cout << "G4DynamicParticle* G4eeToHadronsModel:WARNING " << f << " > " << grej
               << " majorant is`small!" << G4endl;
      }
      if (++ii >= iimax)
      {
        break;
