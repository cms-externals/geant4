...
void G4eeTo3PiModel::SampleSecondaries(std::vector<G4DynamicParticle*>* newp, G4double e,
                                       const G4ThreeVector& direction)
{
  ...
  do
  {
    ...
    gg = (px * px + py * py + pz * pz)
         * norm(1.0 / cross->DpRho(m01) + 1.0 / cross->DpRho(m02) + 1.0 / cross->DpRho(m12));

    if (gg > gmax)
    {
      G4cout << "G4eeTo3PiModel::SampleSecondaries WARNING matrix element g= " << gg << " > "
             << gmax << " (majoranta)" << G4endl;
      gmax = gg;
    }
  ...
  G4DynamicParticle* dp0 = new G4DynamicParticle(G4PionZero::PionZero(), w0);
  G4DynamicParticle* dp1 = new G4DynamicParticle(G4PionPlus::PionPlus(), w1);
  G4DynamicParticle* dp2 = new G4DynamicParticle(G4PionMinus::PionMinus(), w2);
  newp->push_back(dp0);
  newp->push_back(dp1);
  newp->push_back(dp2);
}
