...
#ifndef G4GOUDSMITSAUNDERSONTABLE_HH
#define G4GOUDSMITSAUNDERSONTABLE_HH
...
class G4GoudsmitSaundersonTable
{
  public:
    ...
    // structure to store one GS transformed angular distribution (for a given
    // s/lambda_el,s/lambda_elG1)
    ...
    G4bool Sampling(G4double lambdaval, G4double qval, G4double scra, G4double& cost,
                    G4double& sint, G4double lekin, G4double beta2, G4int matindx,
                    GSMSCAngularDtr** gsDtr, G4int& mcekini, G4int& mcdelti, G4double& transfPar,
                    G4bool isfirst);

    G4double SampleCosTheta(G4double lambdaval, G4double qval, G4double scra, G4double lekin,
                            G4double beta2, G4int matindx, GSMSCAngularDtr** gsDtr, G4int& mcekini,
                            G4int& mcdelti, G4double& transfPar, G4bool isfirst);

    G4double SampleGSSRCosTheta(const GSMSCAngularDtr* gsDrt, G4double transfpar);

    G4double SingleScattering(G4double lambdaval, G4double scra, G4double lekin, G4double beta2,
                              G4int matindx);

    GSMSCAngularDtr* GetGSAngularDtr(G4double scra, G4double& lambdaval, G4double& qval,
                                     G4double& transfpar);
    ...
    // this method returns with the scattering power correction (to avoid double counting of
    // sub-threshold deflections) interpolated from tables prepared at initialisation
  ...
  private:
    ...
    // vector to store all GS transformed angular distributions (cumputed based on the
    // Screened-Rutherford DCS)
