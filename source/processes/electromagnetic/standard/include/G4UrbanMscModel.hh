...
#ifndef G4URBANMSCMODEL_HH
#define G4URBANMSCMODEL_HH
...
inline void G4UrbanMscModel::SetParticle(const G4ParticleDefinition* p)
{
  if (p != particle)
  {
    particle = p;
...
inline G4double G4UrbanMscModel::SimpleScattering()
{
  ...
  return (rndmarray[1] < prob) ? -1. + 2. * G4Exp(G4Log(rndmarray[0]) / (a + 1.))
                               : -1. + 2. * rndmarray[0];
}
...
{
  G4double x =
    (particle == positron) ? 0.7 * msc[idx]->sqrtZ * stepmin : 0.87 * msc[idx]->Z23 * stepmin;
  if (currentKinEnergy < tlow)
  {
    x *= 0.5 * (1. + currentKinEnergy / tlow);
  }
