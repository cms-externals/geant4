...
#ifndef G4ASTARSTOPPING_HH
#define G4ASTARSTOPPING_HH
...
inline G4double G4ASTARStopping::GetElectronicDEDX(G4int idx, G4double energy) const
{
  G4double res = 0.0;
  ...
  if (energy < emin)
  {
    res = (*(sdata[idx]))[0] * std::sqrt(energy / emin);
  }
  else
  {
    res = sdata[idx]->Value(energy);
  }
  return res;
}
...
inline G4double G4ASTARStopping::GetElectronicDEDX(const G4Material* mat, G4double energy) const
{
