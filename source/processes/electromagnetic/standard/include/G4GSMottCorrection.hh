...
#ifndef G4GSMOTTCORRECTION_HH
#define G4GSMOTTCORRECTION_HH
...
class G4GSMottCorrection
{
  ...
  private:
    ...
    struct DataPerDelta
    {
        ...
        G4double fSC;
        G4double fSD;
        G4double* fRejFuntion;  // rejection func. for a given E_{kin}, \delta, e^-/e^+ over the
                                // \sin(0.5\theta) grid
    };
    ...
    {
        DataPerEkin**
          fDataPerEkin;  // per kinetic energy data structure for each kinetic energy value
    };
    //
    ...
    //      2. \beta^2 grid from E_{kin} = 100[keV](~0.300546) - \beta^2=0.9999(~50.5889MeV]) with
    //      linear spacing 16 points:
    //                 # linear interpolation on \beta^2 will be used
    //      3. the overall kinetic energy grid is from E_{kin}=1[keV] -
    //      E_{kin}<=\beta^2=0.9999(~50.5889MeV]) with 31 points
    //  II. Delta value grid [rejection functions at a given kinetic energy(also depends on
    //  \theta;Z,e-/e+)]:
    //      1. \delta=2 Q_{1SR} (\eta_{MCcor})/ [1-2 Q_{1SR} (\eta_{MCcor})] where Q_{1SR} is the
    //      first moment i.e.
    //         Q_{1SR}(\eta_{MCcor}) =s/\lambda_{el}G_{1SR}(\eta_{MCcor}) where s/\lambda_{el} is
    //         the mean number of elastic scattering along the path s and G_{1SR}(\eta_{MCcor}) is
    //         the first, Screened-Rutherford transport coefficient but computed by using the
    //         Mott-corrected Moliere screening parameter
    //      2. the delta value grid is from [0(1e-3) - 0.9] with linear spacing of 28 points:
    //                 # linear interpolation will be used on \delta
    // III. \sin(0.5\theta) grid[rejection function at a given kinetic energy - delta value pair
    // (also depends on Z,e-/e+)]:
    //      1. 32 \sin(0.5\theta) pints between [0,1] with linear spacing: # linear interpolation on
    //      \sin(0.5\theta) will
    //         be used exept the last bin where spline is used (the corresponding 4 spline
  ...
  private:
    ...
    static constexpr G4int gNumEkin =
      31;  // number of kinetic energy grid points for Mott correction
    ...
    static constexpr G4int gMaxZet =
      98;  // max. Z for which Mott-correction data were computed (98)
    static constexpr G4double gMinEkin = 1. * CLHEP::keV;  // minimum kinetic energy value
    static constexpr G4double gMidEkin =
      100. * CLHEP::keV;  // kinetic energy at the border of the E_{kin}-\beta^2 grids
    ...
    std::vector<DataPerMaterial*>
      fMCDataPerElement;  // size will be gMaxZet+1; won't be null only at used Z indices
    std::vector<DataPerMaterial*>
      fMCDataPerMaterial;  // size will #materials; won't be null only at used mat. indices
};
