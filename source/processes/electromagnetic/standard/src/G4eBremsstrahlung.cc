...
G4eBremsstrahlung::G4eBremsstrahlung(const G4String& name) : G4VEnergyLossProcess(name)
{
                                                    ...
                                                    const G4ParticleDefinition*)
{
  if (!isInitialised)
  {
    ...
    {
      SetEmModel(new G4SeltzerBergerModel());
    }
...
void G4eBremsstrahlung::StreamProcessInfo(std::ostream& out) const
{
  if (nullptr != EmModel(0))
  {
    ...
    out << "      LPM flag: " << param->LPM() << " for E > " << EmModel(0)->HighEnergyLimit() / GeV
        << " GeV";
    if (eth < DBL_MAX)
    {
