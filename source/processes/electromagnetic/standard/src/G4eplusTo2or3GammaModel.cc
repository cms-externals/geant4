...
G4eplusTo2or3GammaModel::G4eplusTo2or3GammaModel()
  : G4VEmModel("eplusTo2or3gamma"), fDeltaMin(0.001), fDelta(fDeltaMin), fGammaTh(CLHEP::MeV)
{
...
void G4eplusTo2or3GammaModel::Initialise(const G4ParticleDefinition* p, const G4DataVector& cuts)
{
                                                ...
                                                const G4DynamicParticle* dp, G4double, G4double)
{
  ...
  if (std::abs(cost) > 1.0)
  {
    G4cout << "### G4eeToTwoGammaModel WARNING cost= " << cost
           << " positron Ekin(MeV)= " << posiKinEnergy << " gamma epsil= " << epsil << G4endl;
    if (cost > 1.0)
      cost = 1.0;
    else
      cost = -1.0;
