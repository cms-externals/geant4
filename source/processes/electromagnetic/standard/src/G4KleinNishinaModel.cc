...
G4KleinNishinaModel::G4KleinNishinaModel(const G4String& nam)
  : G4VEmModel(nam), lv1(0., 0., 0., 0.), lv2(0., 0., 0., 0.), bst(0., 0., 0.)
{
                                                         ...
                                                         G4double, G4double)
{
  ...
  static const G4double d1 = 2.7965e-1 * CLHEP::barn, d2 = -1.8300e-1 * CLHEP::barn,
                                            ...
                                            G4double)
{
  ...
  if (nShells > (G4int)fProbabilities.size())
  {
    fProbabilities.resize(nShells);
  }
  ...
  if (std::abs(energy - gamEnergy1 - eKinEnergy - esec - edep) > eV)
  {
    G4cout << "### G4KleinNishinaModel dE(eV)= "
           << (energy - gamEnergy1 - eKinEnergy - esec - edep) / eV << " shell= " << i
           << "  E(keV)= " << energy / keV << "  Ebind(keV)= " << bindingEnergy / keV
           << "  Eg(keV)= " << gamEnergy1 / keV << "  Ee(keV)= " << eKinEnergy / keV
