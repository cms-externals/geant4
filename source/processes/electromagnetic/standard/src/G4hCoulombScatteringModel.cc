...
G4hCoulombScatteringModel::G4hCoulombScatteringModel(G4bool combined)
  : G4VEmModel("hCoulombScattering"), cosThetaMin(1.0), cosThetaMax(-1.0), isCombined(combined)
{
...
G4double G4hCoulombScatteringModel::MinPrimaryEnergy(const G4Material* material,
                                                     const G4ParticleDefinition* part, G4double)
{
  SetupParticle(part);

  // define cut using cuts for proton
  G4double cut = std::max(recoilThreshold, (*pCuts)[CurrentCouple()->GetIndex()]);
                                                               ...
                                                               G4double)
{
  ...
  G4int iz = G4lrint(Z);
  G4double tmass = (1 == iz) ? proton_mass_c2 : fNistManager->GetAtomicMassAmu(iz) * amu_c2;
  ...
  if (cosThetaMax < costmin)
  {
    ...
    G4double costmax = (1 == iz && particle == theProton && cosThetaMax < 0.0) ? 0.0 : cosThetaMax;
    if (costmin > costmax)
    {
                                                  ...
                                                  G4double)
{
  ...
  const G4Element* elm = SelectRandomAtom(couple, particle, kinEnergy, cut, kinEnergy);
  ...
  G4ThreeVector newDirection = wokvi->SampleSingleScattering(costmin, costmax, ratio);
