...
double G4SBBremTable::SampleEnergyTransfer(const G4double eekin, const G4double leekin,
                                           ...
                                           const G4bool isElectron)
{
  ...
  if (gamCutIndx >= stZ->fNumGammaCuts || stZ->fGammaECuts[gamCutIndx] != gcut)
  {
    ...
    G4Exception("G4SBBremTable::SampleEnergyTransfer()", "em0X", FatalException, msg.c_str());
  }
  ...
  if (eekin < fElEnergyVect[elEnergyIndx])
  {
    ...
    {  // take the table at the lower  e- energy bin
  ...
  do
  {
    ...
    if (!isElectron)
    {
      ...
      const G4double iBeta1 =
        (e1 + CLHEP::electron_mass_c2) / std::sqrt(e1 * (e1 + 2. * CLHEP::electron_mass_c2));
      const G4double e2 = eekin - eGamma;
      const G4double iBeta2 =
        (e2 + CLHEP::electron_mass_c2) / std::sqrt(e2 * (e2 + 2. * CLHEP::electron_mass_c2));
...
void G4SBBremTable::BuildSamplingTables()
{
  ...
  const G4ProductionCutsTable* thePCTable = G4ProductionCutsTable::GetProductionCutsTable();
...
{
  const std::size_t numMatCuts = G4ProductionCutsTable::GetProductionCutsTable()->GetTableSize();
  for (G4int iz = 1; iz < fMaxZet + 1; ++iz)
  {
    ...
    {
      if (!stZ->fTablesPerEnergy[iee]) continue;
      ...
      for (std::size_t ic = 0; ic < stZ->fNumGammaCuts; ++ic)
      {
        ...
        if (elEnergy > gamCut)
        {
          ...
          const std::size_t iKLow =
            (cutKappa > 1.e-12) ? std::lower_bound(fKappaVect.cbegin(), fKappaVect.cend(), cutKappa)
                                    - fKappaVect.cbegin() - 1
                                : 0;
          ...
          const G4double alph =
            G4Log(cutKappa / fKappaVect[iKLow]) / G4Log(fKappaVect[iKLow + 1] / fKappaVect[iKLow]);
...
void G4SBBremTable::LoadSTGrid()
{
  ...
  if (!infile.is_open())
  {
    ...
    G4Exception("G4SBBremTable::LoadSTGrid()", "em0006", FatalException, msgc.c_str());
    return;
  }
...
void G4SBBremTable::LoadSamplingTables(G4int iz)
{
  ...
  const G4String fname =
    G4EmParameters::Instance()->GetDirLEDATA() + "/brem_SB/SBTables/sTableSB_" + std::to_string(iz);
  ...
  const G4double minGammaCut =
    zTable->fGammaECuts[std::min_element(std::cbegin(zTable->fGammaECuts),
                                         std::cend(zTable->fGammaECuts))
  ...
  {
    zTable->fMinElEnergyIndx =
      G4int(std::lower_bound(fElEnergyVect.cbegin(), fElEnergyVect.cend(), elEmin)
  ...
  {
    // lower + 1
    zTable->fMaxElEnergyIndx =
      G4int(std::lower_bound(fElEnergyVect.cbegin(), fElEnergyVect.cend(), elEmax)
...
G4int G4SBBremTable::LinSearch(const std::vector<STPoint>& vect, const G4int size,
                               const G4double val)
{
  G4int i = 0;
  while (i + 3 < size)
...
void G4SBBremTable::ReadCompressedFile(const G4String& fname, std::istringstream& iss)
{
  ...
  else
  {
    ...
    G4Exception("G4SBBremTable::ReadCompressedFile", "em0006", FatalException, msg.c_str());
    return;
  }
