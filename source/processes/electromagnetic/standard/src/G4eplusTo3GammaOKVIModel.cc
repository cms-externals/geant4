...
G4eplusTo3GammaOKVIModel::G4eplusTo3GammaOKVIModel(const G4ParticleDefinition*, const G4String& nam)
...
void G4eplusTo3GammaOKVIModel::Initialise(const G4ParticleDefinition*, const G4DataVector&) {}

//....oooOO0OOooo........oooOO0OOooo........oooOO0OOooo........oooOO0OOooo......

// (A.A.) F_{ijk} calculation method
G4double G4eplusTo3GammaOKVIModel::ComputeF(G4double fr1, G4double fr2, G4double fr3,
                                            G4double kinEnergy)
{
  ...
  G4double fres = -rho * (1. / fr1s + 1. / fr2s)
                  ...
                  + (ab / (2. * fr1 * fr2 * (1 - fr3))) * G4Log(2. * gam * (1. - fr3) / (fr1 * fr2))
                  - add;
...
// (A.A.) F_{ijk} calculation method
G4double G4eplusTo3GammaOKVIModel::ComputeF0(G4double fr1, G4double fr2, G4double fr3)
{
  G4double tau = 0.0;
  G4double gam = tau + 1.0;
  G4double gamma2 = gam * gam;
...
G4double G4eplusTo3GammaOKVIModel::ComputeFS(G4double fr1, G4double fr2, G4double fr3,
                                             G4double kinEnergy)
{
  G4double ekin = std::max(eV, kinEnergy);
  G4double tau = ekin / electron_mass_c2;
  ...
  G4double fsum = fr1 * fr1
                  * (ComputeF(fr1, fr2, fr3, ekin) + ComputeF(fr3, fr1, fr2, ekin)
...
G4double G4eplusTo3GammaOKVIModel::ComputeCrossSectionPerElectron(G4double kinEnergy)
{
  ...
  G4double rho =
    (gamma2 + 4 * gam + 1.) * G4Log(gam + bg) / (gamma2 - 1.) - (gam + 3.) / (sqrt(gam * gam - 1.));
...
void G4eplusTo3GammaOKVIModel::SampleSecondaries(vector<G4DynamicParticle*>* vdp,
                                                 ...
                                                 const G4DynamicParticle* dp, G4double, G4double)
{
  ...
  G4LorentzVector lv(dp->GetMomentum(), posiKinEnergy + 2 * CLHEP::electron_mass_c2);
  ...
  G4double border = 1.0 - std::min(std::max(CLHEP::electron_mass_c2 / eGammaCMS, fDelta), 0.1);
