...
G4ICRU73QOModel::G4ICRU73QOModel(const G4ParticleDefinition* p, const G4String& nam)
  : G4VEmModel(nam), particle(nullptr), isInitialised(false)
{
...
G4double G4ICRU73QOModel::DEDX(const G4Material* material, G4double kineticEnergy)
{
  ...
  for (std::size_t i = 0; i < numberOfElements; ++i)
  {
    ...
    eloss += DEDXPerElement(element->GetZasInt(), kineticEnergy) * theAtomicNumDensityVector[i]
             * element->GetZ();
  }
  return eloss;
}
...
G4double G4ICRU73QOModel::DEDXPerElement(G4int AtomicNumber, G4double kineticEnergy)
{
  ...
  {
    G4double NormalizedEnergy = (2.0 * CLHEP::electron_mass_c2 * beta2) / GetShellEnergy(Z, nos);
...
G4double G4ICRU73QOModel::GetOscillatorEnergy(G4int Z, G4int nbOfTheShell) const
{
  ...
  if (idx == -1)
  {
    idx = denEffData->GetElementIndex(Z - 1, kStateUndefined);
  }
  ...
  G4double plasmonTerm =
    0.66667 * G4AtomicShells::GetNumberOfElectrons(Z, nbOfTheShell) * PlasmaEnergy2 / (Z * Z);

  static const G4double exphalf = G4Exp(0.5);
  G4double ionTerm = exphalf * (G4AtomicShells::GetBindingEnergy(Z, nbOfTheShell));
...
const G4int G4ICRU73QOModel::ZElementAvailable[NQOELEM] = {1,   2,  4,  6,  7,  8,  10, 13, 14,
                                                           -18, 22, 26, 28, 29, 32, 36, 42, 47,
                                                           50,  54, 73, 74, 78, 79, 82, 92};

const G4int G4ICRU73QOModel::nbofShellsForElement[NQOELEM] = {
  1, 1, 2, 3, 3, 3, 3, 4, 5, 4, 5, 5, 5, 5, 6, 4, 6, 6, 7, 6, 6, 8, 7, 7, 9, 9};

const G4int G4ICRU73QOModel::startElemIndex[NQOELEM] = {0,  1,  2,  4,  7,  10,  13,  16, 20,
                                                        25, 29, 34, 39, 44, 49,  55,  59, 65,
...
const G4double G4ICRU73QOModel::factorBethe[99] = {
  1.0,    0.9637, 0.9872, 0.9469, 0.9875, 0.91,   0.989,  0.9507, 0.9773, 0.8621, 0.979,  // 1 - 10
