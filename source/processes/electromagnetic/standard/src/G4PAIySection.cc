...
void G4PAIySection::Initialize(const G4Material* material, G4double maxEnergyTransfer,
                               G4double betaGammaSq, G4SandiaTable* sandia)
{
  ...
  if (fVerbose > 0)
  {
    G4cout << "fDensity = " << fDensity << "\t" << fElectronDensity
           << "\t fIntervalNumber = " << fIntervalNumber << " (beta*gamma)^2= " << betaGammaSq
           << G4endl;
  }
...
G4double G4PAIySection::RutherfordIntegral(G4int k, G4double x1, G4double x2)
{
  G4double c1, c2, c3;
...
G4double G4PAIySection::DifPAIySection(G4int i, G4double betaGammaSq)
{
...
void G4PAIySection::IntegralPAIySection()
{
  ...
  for (G4int i = fSplineNumber - 1; i >= 1; i--)
  {
    ...
    {
      fIntegralPAIySection[i] =
        fIntegralPAIySection[i + 1] + SumOverBorder(i + 1, fEnergyInterval[k]);
...
G4double G4PAIySection::SumOverBorder(G4int i, G4double en0)
{
...
G4double G4PAIySection::SumOverBorderdEdx(G4int i, G4double en0)
{
...
G4double G4PAIySection::SumOverBordCerenkov(G4int i, G4double en0)
{
...
G4double G4PAIySection::SumOverBordPlasmon(G4int i, G4double en0)
{
  G4double x0, x1, y0, yy1, a, c, d, e0, result;
