...
G4PAIPhotData::G4PAIPhotData(G4double tmin, G4double tmax, G4int ver)
{
  ...
  fTotBin = (G4int)(nPerDecade * std::log10(fHighestKineticEnergy / fLowestKineticEnergy));

  fParticleEnergyVector =
    new G4PhysicsLogVector(fLowestKineticEnergy, fHighestKineticEnergy, fTotBin);
  if (0 < ver)
  {
    G4cout << "### G4PAIPhotData: Nbins= " << fTotBin
           << " Tmin(MeV)= " << fLowestKineticEnergy / MeV
           << " Tmax(GeV)= " << fHighestKineticEnergy / GeV << "  tmin(keV)= " << tmin / keV
           << G4endl;
  }
}
...
void G4PAIPhotData::Initialise(const G4MaterialCutsCouple* couple, G4double cut,
                               G4PAIPhotModel* model)
{
  ...
  const vector<G4double>* deltaCutInKineticEnergy =
    theCoupleTable->GetEnergyCutsVector(idxG4ElectronCut);
  const vector<G4double>* photonCutInKineticEnergy =
    theCoupleTable->GetEnergyCutsVector(idxG4GammaCut);
  ...
  */
  ...
  auto dEdxCutVector = new G4PhysicsLogVector(fLowestKineticEnergy, fHighestKineticEnergy, fTotBin);

  auto dNdxCutVector = new G4PhysicsLogVector(fLowestKineticEnergy, fHighestKineticEnergy, fTotBin);
  ...
  for (G4int i = 0; i <= fTotBin; ++i)
  {
    ...
    for (G4int k = 0; k < n; k++)
    {
      ...
      transferVector->PutValue(k, t, t * fPAIxSection.GetIntegralPAIxSection(k + 1));
      photonVector->PutValue(k, t, t * fPAIxSection.GetIntegralCerenkov(k + 1));
      plasmonVector->PutValue(k, t, t * fPAIxSection.GetIntegralPlasmon(k + 1));
    ...
    G4double dNdxCut =
      transferVector->Value(deltaCutInKineticEnergyNow) / deltaCutInKineticEnergyNow;
    G4double dNdxCutPhoton =
      photonVector->Value(photonCutInKineticEnergyNow) / photonCutInKineticEnergyNow;
    G4double dNdxCutPlasmon =
      plasmonVector->Value(deltaCutInKineticEnergyNow) / deltaCutInKineticEnergyNow;
    ...
    if (dNdxCutPhoton < 0.0)
    {
      dNdxCutPhoton = 0.0;
    }
    if (dNdxCutPlasmon < 0.0)
    {
      dNdxCutPlasmon = 0.0;
    }
...
G4double G4PAIPhotData::DEDXPerVolume(G4int coupleIndex, G4double scaledTkin, G4double cut) const
{
  ...
  G4bool one = true;
  if (scaledTkin >= fParticleEnergyVector->Energy(nPlace))
  {
    iPlace = nPlace;
  }
...
G4double G4PAIPhotData::CrossSectionPerVolume(G4int coupleIndex, G4double scaledTkin, G4double tcut,
                                              G4double tmax) const
{
...
G4double G4PAIPhotData::GetPlasmonRatio(G4int coupleIndex, G4double scaledTkin) const
{
...
G4double G4PAIPhotData::SampleAlongStepTransfer(G4int coupleIndex, G4double kinEnergy,
...
G4double G4PAIPhotData::SampleAlongStepPhotonTransfer(G4int coupleIndex, G4double kinEnergy,
                                                      G4double scaledTkin,
...
G4double G4PAIPhotData::SampleAlongStepPlasmonTransfer(G4int coupleIndex, G4double kinEnergy,
                                                       G4double scaledTkin,
                                                       G4double stepFactor) const
{
  G4double loss = 0.0;
...
G4double G4PAIPhotData::SamplePostStepTransfer(G4int coupleIndex, G4double scaledTkin) const
{
...
G4double G4PAIPhotData::SamplePostStepPhotonTransfer(G4int coupleIndex, G4double scaledTkin) const
{
  // G4cout<<"G4PAIPhotData::GetPostStepTransfer"<<G4endl;
  G4double transfer = 0.0;
  G4double rand = G4UniformRand();
...
G4double G4PAIPhotData::SamplePostStepPlasmonTransfer(G4int coupleIndex, G4double scaledTkin) const
{
  // G4cout<<"G4PAIPhotData::GetPostStepTransfer"<<G4endl;
  G4double transfer = 0.0;
  G4double rand = G4UniformRand();
...
G4double G4PAIPhotData::GetEnergyTransfer(G4int coupleIndex, std::size_t iPlace,
                                          G4double position) const
{
  G4PhysicsVector* v = (*(fPAIxscBank[coupleIndex]))(iPlace);
  if (position * v->Energy(0) >= (*v)[0])
  {
    return v->Energy(0);
  }
...
G4double G4PAIPhotData::GetEnergyPhotonTransfer(G4int coupleIndex, std::size_t iPlace,
                                                G4double position) const
{
...
G4double G4PAIPhotData::GetEnergyPlasmonTransfer(G4int coupleIndex, std::size_t iPlace,
                                                 G4double position) const
{
