...
//
std::vector<G4GoudsmitSaundersonTable::GSMSCAngularDtr*>
  G4GoudsmitSaundersonTable::gGSMSCAngularDistributions1;
std::vector<G4GoudsmitSaundersonTable::GSMSCAngularDtr*>
  G4GoudsmitSaundersonTable::gGSMSCAngularDistributions2;
//
...
//  - Mott-correction will be included if it was requested by the user (i.e. if
//  fIsMottCorrection=true)
...
G4bool G4GoudsmitSaundersonTable::Sampling(G4double lambdaval, G4double qval, G4double scra,
                                           ...
                                           G4bool isfirst)
{
  ...
  //   - Mott-correction will be included if it was requested by the user (i.e. if
  //   fIsMottCorrection=true)
  cost = SampleCosTheta(lambdaval, qval, scra, lekin, beta2, matindx, gsDtr, mcekini, mcdelti,
                        transfPar, isfirst);
  // add protections
  if (cost < -1.0) cost = -1.0;
  if (cost > 1.0) cost = 1.0;
...
G4double G4GoudsmitSaundersonTable::SampleCosTheta(G4double lambdaval, G4double qval, G4double scra,
                                                   ...
                                                   GSMSCAngularDtr** gsDtr, G4int& mcekini,
                                                   G4int& mcdelti, G4double& transfPar,
                                                   G4bool isfirst)
{
  G4double cost = 1.;
  // determine the base GS angular distribution if it is the first call (when sub-step sampling is
  // used)
  if (isfirst)
  {
  ...
  if (fIsMottCorrection && *gsDtr)
  {  // no Mott-correction in case of izotropic theta
    ...
    G4double val =
      fMottCorrection->GetMottRejectionValue(lekin, beta2, qval, cost, matindx, mcekini, mcdelti);
...
// returns with cost sampled from the GS angular distribution computed based on Screened-Rutherford
// DCS
G4double G4GoudsmitSaundersonTable::SampleGSSRCosTheta(const GSMSCAngularDtr* gsDtr,
                                                       G4double transfpar)
{
  ...
  G4double dum2 =
    delta * delta + gsDtr->fParamA[indxl] * dum0 + gsDtr->fParamB[indxl] * aval * aval;
  G4double sample =
    gsDtr->fUValues[indxl] + dum1 / dum2 * (gsDtr->fUValues[indxl + 1] - gsDtr->fUValues[indxl]);
...
G4GoudsmitSaundersonTable::GetGSAngularDtr(G4double scra, G4double& lambdaval, G4double& qval,
                                           G4double& transfpar)
{
  ...
  if (qval < gQMAX2)
  {
    ...
    // - if the selected lamIndx, qIndx correspond to L(=s/lambda_el) and Q(=s/lambda_el G1) such
    // that G1(=Q/L) > 1
    //   G1 should always be < 1 and if G1 is ~1 -> the dtr is isotropic (this can only happen in
    //   case of the 2. grid)
    //
    ...
    {
      transfpar =
        0.5 * (-2.77164 + lLambda * (2.94874 - lLambda * (0.1535754 - lLambda * 0.00552888)));
    }
    else
    {
      transfpar =
        0.5
        * (1.347
           + lLambda * (0.209364 - lLambda * (0.45525 - lLambda * (0.50142 - lLambda * 0.081234))));
    }
    transfpar *= (lambdaval + 4.0) * scra;
  }
  // return with the selected GS angular distribution that we need to sample cost from (if nullptr
  // => isotropic cost)
  return dtr;
}
...
void G4GoudsmitSaundersonTable::LoadMSCData()
{
  ...
  for (G4int il = 0; il < gLAMBNUM; ++il)
  {
    ...
    if (!infile.is_open())
    {
      ...
      G4Exception("G4GoudsmitSaundersonTable::LoadMSCData()", "em0006", FatalException,
                  msgc.c_str());
      return;
    }
                                                     ...
                                                     G4double lekin, G4double beta2, G4int matindx)
{
  ...
  if (fIsMottCorrection)
  {
    ...
    G4double val =
      fMottCorrection->GetMottRejectionValue(lekin, beta2, q1, cost, matindx, ekindx, deltindx);
    while (G4UniformRand() > val && ++nloop < nlooplim)
    {
                                                         ...
                                                         G4double& mcToQ1, G4double& mcToG2PerG1)
{
  if (fIsMottCorrection)
  {
    fMottCorrection->GetMottCorrectionFactors(logekin, beta2, matindx, mcToScr, mcToQ1,
                                              mcToG2PerG1);
  }
}
...
void G4GoudsmitSaundersonTable::InitMoliereMSCParams()
{
  ...
  for (std::size_t imat = 0; imat < numMaterials; ++imat)
  {
    ...
    gMoliereBc[theMaterial->GetIndex()] =
      const1 * density * zs / sa * G4Exp(ze / zs) / G4Exp(zx / zs);  //[1/cm]
...
// this method is temporary, will be removed/replaced with a more effictien solution
// after 10.3.ref09
G4double
G4GoudsmitSaundersonTable::ComputeScatteringPowerCorrection(const G4MaterialCutsCouple* matcut,
                                                            G4double ekin)
{
