...
G4double G4BraggIonModel::GetChargeSquareRatio(const G4ParticleDefinition* p, const G4Material* mat,
                                               G4double kinEnergy)
{
...
{
  G4double sigma = Z * ComputeCrossSectionPerElectron(p, kinEnergy, cutEnergy, maxEnergy);
  if (isAlpha)
  {
                                                ...
                                                G4double cutEnergy, G4double maxEnergy)
{
  ...
  {
    const G4double zeff =
      material->GetTotNbOfElectPerVolume() / material->GetTotNbOfAtomsPerVolume();
                                               ...
                                               G4double kineticEnergy, G4double cut)
{
  ...
  else
  {
    ...
    if (tmin < tmax)
    {
      ...
      G4double del = (G4Log(x) * (tau + 1.) * (tau + 1.) / (tau * (tau + 2.0)) + 1.0 - x)
...
void G4BraggIonModel::CorrectionsAlongStep(const G4Material* mat, const G4ParticleDefinition* p,
                                           const G4double preKinEnergy, const G4double,
                                           const G4double&, G4double& eloss)
{
...
G4int G4BraggIonModel::HasMaterialForHe(const G4Material* mat) const
{
  ...
  static const G4String molName[numberOfMolecula] = {"CaF_2",
                                                     "Cellulose_Nitrate",
                                                     "LiF",
                                                     "Policarbonate",
...
G4double G4BraggIonModel::HeStoppingPower(const G4double kineticEnergy) const
{
  ...
  if (iMolecula >= 0)
  {
    ...
    static const G4float a[11][5] = {{9.43672f, 0.54398f, 84.341f, 1.3705f, 57.422f},
    ...
    static const G4double atomicWeight[11] = {101.96128f, 44.0098f,  16.0426f, 28.0536f,
...
G4double G4BraggIonModel::HeDEDX(const G4Material* material, const G4double aEnergy)
{
  ...
  if (material != currentMaterial)
  {
    ...
    if (iICRU90 < 0)
    {
      if (isAlpha)
      {
        ...
        if (iASTAR < 0)
        {
          iMolecula = HasMaterialForHe(baseMaterial);
        }
      }
      else
      {
  ...
  if (iICRU90 >= 0)
  {
    eloss = (isAlpha) ? fICRU90->GetElectronicDEDXforAlpha(iICRU90, aEnergy)
                      : fICRU90->GetElectronicDEDXforProton(iICRU90, aEnergy);
    if (eloss > 0.0)
    {
      return eloss * material->GetDensity();
    }
  }
  // PSTAR parameterisation
  if (iPSTAR >= 0)
  {
    return fPSTAR->GetElectronicDEDX(iPSTAR, aEnergy) * material->GetDensity();
  }
  // ASTAR
  if (iASTAR >= 0)
  {
  ...
  else
  {
    ...
    for (std::size_t i = 0; i < numberOfElements; ++i)
    {
      ...
      eloss +=
        HeElectronicStoppingPower(element->GetZasInt(), aEnergy) * theAtomicNumDensityVector[i];
    }
  }
...
G4double G4BraggIonModel::HeEffChargeSquare(const G4double z, const G4double kinEnergyHeInMeV) const
{
