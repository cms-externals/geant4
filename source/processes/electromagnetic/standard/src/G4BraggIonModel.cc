...
G4PSTARStopping* G4BraggIonModel::fPSTAR = nullptr;
                                 ...
                                 const G4DataVector&)
{
  ...
  // initialise static for each run
  if(IsMaster()) {
    if(isAlpha) {
      if(nullptr == fASTAR) {
                                               ...
                                               G4double cut)
{
  ...
  if(kineticEnergy < tlim) {
    dedx = DEDX(material, tlim)*std::sqrt(kineticEnergy/tlim);
  } else {
    dedx = DEDX(material, kineticEnergy);
...
G4double G4BraggIonModel::StoppingPower(const G4double kineticEnergy) const
{
...
G4double G4BraggIonModel::ElectronicStoppingPower(const G4int z,
                               ...
                               const G4double aEnergy)
{
  ...
  if(iMolecula >= 0) {
    ...
    eloss = StoppingPower(aEnergy)*heChargeSquare*material->GetDensity()/amu;

    // pure material
  } else if(1 == numberOfElements) {

    const G4Element* element = (*theElmVector)[0];
    eloss = ElectronicStoppingPower(element->GetZasInt(), aEnergy)
      * (material->GetTotNbOfAtomsPerVolume());
