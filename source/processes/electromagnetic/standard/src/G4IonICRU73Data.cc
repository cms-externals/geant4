...
const G4int NZ = 27;
const G4int zdat[NZ] = {5,  6,  7,  8,  13, 14, 15, 16, 22, 26, 28, 29, 30, 31,
                        32, 33, 34, 40, 47, 48, 49, 51, 52, 72, 73, 74, 79};
}  // namespace
...
G4double G4IonICRU73Data::GetDEDX(const G4Material* mat, const G4int Z, const G4double e,
                                  const G4double loge) const
{
  ...
  G4double res = (e > fEmin) ? v->LogVectorValue(e, loge) : (*v)[0] * std::sqrt(e / fEmin);
  return res;
}
...
void G4IonICRU73Data::Initialise()
{
  ...
  if (1 < fVerbose)
  {
    G4cout << "### G4IonICRU73Data::Initialise() for " << nmat << " materials" << G4endl;
  }
  ...
  for (G4int i = 0; i < (G4int)nmat; ++i)
  {
    ...
    if (1 < fVerbose)
    {
      G4cout << i << ".  material: " << matname << "  idx=" << idx << "  matIdx=" << fMatIndex[idx]
             << G4endl;
    }
    if (fMatIndex[idx] == -1)
    {
...
void G4IonICRU73Data::ReadMaterialData(const G4Material* mat, const G4double coeff,
                                       const G4bool useICRU90)
{
  ...
  for (G4int Z = 3; Z <= ZPROJMAX; ++Z)
  {
    ...
    if (nullptr == (*(fMatData[Z1]))[idx])
    {
      ...
      if (nullptr != v)
      {
        const G4double fact = coeff * mat->GetDensity() * CLHEP::MeV * 1000 * CLHEP::cm2 / CLHEP::g;
        v->ScaleVector(CLHEP::MeV, fact);
        if (2 < fVerbose)
        {
          G4cout << "### Data for " << name << " and projectile Z=" << Z1 << G4endl;
          G4cout << *v << G4endl;
        }
        (*(fMatData[Z1]))[idx] = v;
      }
...
void G4IonICRU73Data::ReadElementData(const G4Material* mat, G4bool useICRU90)
{
  ...
  for (G4int Z = 3; Z <= ZPROJMAX; ++Z)
  {
    if (1 < fVerbose)
    {
      G4cout << "ReadElementData for " << mat->GetName() << " Z=" << Z << " Nelm=" << nelm
             << G4endl;
    }
    G4PhysicsLogVector* v = nullptr;
    if (1 == nelm)
    ...
    else
    {
      ...
      for (G4int i = 0; i <= fNbins; ++i)
      {
        ...
        {
          G4PhysicsLogVector* v1 = FindOrBuildElementData(Z, (*elmv)[j]->GetZasInt(), useICRU90);
          dedx += (*v1)[i] * dens[j];
        }
        v->PutValue(i, dedx);
      }
    ...
    if (nullptr != v)
    {
      const G4double fact = mat->GetDensity() * CLHEP::MeV * 1000 * CLHEP::cm2 / CLHEP::g;
      v->ScaleVector(CLHEP::MeV, fact);
      if (2 < fVerbose)
      {
        G4cout << "### Data for " << mat->GetName() << " for projectile Z=" << Z << G4endl;
        G4cout << *v << G4endl;
      }
    }
  }
...
G4PhysicsLogVector* G4IonICRU73Data::FindOrBuildElementData(const G4int Z, const G4int Z1,
                                                            G4bool useICRU90)
{
  G4PhysicsLogVector* v = nullptr;
  if (Z <= ZPROJMAX && Z1 <= ZTARGMAX)
  {
    ...
    {
      G4int Z2 = Z1;
      G4bool isICRU90 = (useICRU90 && Z <= 18 && (Z1 == 1 || Z1 == 6 || Z1 == 7 || Z1 == 8));
...
G4PhysicsLogVector* G4IonICRU73Data::RetrieveVector(std::ostringstream& ost, G4bool warn)
{
  G4PhysicsLogVector* v = nullptr;
  ...
  if (!filein.is_open())
  {
    if (warn)
    {
      ...
      G4Exception("G4IonICRU73Data::RetrieveVector(..)", "em013", FatalException, ed,
                  "Check G4LEDATA");
    }
  }
  else
  {
    if (fVerbose > 0)
    {
      G4cout << "File " << ost.str() << " is opened by G4IonICRU73Data" << G4endl;
    }
    ...
    if (!fVector->Retrieve(filein, true))
    {
      ...
      G4Exception("G4IonICRU73Data::RetrieveVector(..)", "had015", FatalException, ed,
                  "Check G4LEDATA");
    }
    else
    {
