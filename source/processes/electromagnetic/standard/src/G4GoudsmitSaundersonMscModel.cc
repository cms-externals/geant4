...
G4GoudsmitSaundersonMscModel::G4GoudsmitSaundersonMscModel(const G4String& nam) : G4VMscModel(nam)
{
...
void G4GoudsmitSaundersonMscModel::Initialise(const G4ParticleDefinition* p, const G4DataVector&)
{
  ...
  if (IsMaster())
  {
    ...
    // Mott-correction includes other way of PWA x-section corrections so deactivate it even if it
    // was true when Mott-correction is activated by the user
    ...
    // - Screened-Rutherford DCS based GS angular distributions will be loaded only if they are not
    // there yet
    ...
    // create PWA corrections table if it was requested (and not deactivated because active
    // Mott-correction)
...
void G4GoudsmitSaundersonMscModel::InitialiseLocal(const G4ParticleDefinition*,
                                                   G4VEmModel* masterModel)
{
  fGSTable = static_cast<G4GoudsmitSaundersonMscModel*>(masterModel)->GetGSTable();
  fIsUseMottCorrection =
    static_cast<G4GoudsmitSaundersonMscModel*>(masterModel)->GetOptionMottCorrection();
  fIsUsePWACorrection =
    static_cast<G4GoudsmitSaundersonMscModel*>(masterModel)->GetOptionPWACorrection();
                                                             ...
                                                             G4double)
{
  ...
  if (fIsUseMottCorrection)
  {
    fGSTable->GetMottCorrectionFactors(G4Log(kineticEnergy), beta2, matindx, fMCtoScrA, fMCtoQ1,
                                       fMCtoG2PerG1);
    // ! no scattering power correction since the current couple is not set before this interface
    // method is called scpCor = fGSTable->ComputeScatteringPowerCorrection(currentCouple,
    // kineticEnergy);
  }
  else if (fIsUsePWACorrection)
  {
    fPWACorrection->GetPWACorrectionFactors(G4Log(kineticEnergy), beta2, matindx, fMCtoScrA,
                                            fMCtoQ1, fMCtoG2PerG1);
  ...
  // elastic mean free path in Geant4 internal lenght units: the neglected (1+screening parameter)
  // term is corrected (if Mott-corretion: the corrected screening parameter is used for this (1+A)
  // correction + Moliere b_c is also corrected with the screening parameter correction)
  fLambda0 = beta2 * (1. + fScrA) * fMCtoScrA / (bc * scpCor);
  // first transport coefficient (if Mott-corretion: the corrected screening parameter is used (it
  // will be fully consistent with the one used during the pre-computation of the Mott-correted GS
                                                           ...
                                                           G4double kineticEnergy)
{
  ...
  if (fIsUseMottCorrection)
  {
    fGSTable->GetMottCorrectionFactors(G4Log(kineticEnergy), beta2, matindx, mctoScrA, mctoQ1,
                                       mctoG2PerG1);
    scpCor = fGSTable->ComputeScatteringPowerCorrection(currentCouple, kineticEnergy);
  }
  else if (fIsUsePWACorrection)
  {
    fPWACorrection->GetPWACorrectionFactors(G4Log(kineticEnergy), beta2, matindx, mctoScrA, mctoQ1,
                                            mctoG2PerG1);
    // scpCor = fGSTable->ComputeScatteringPowerCorrection(currentCouple, kineticEnergy);
  }
                                                                  ...
                                                                  G4double& currentMinimalStep)
{
  ...
  currentRange = GetRange(particle, currentKinEnergy, currentCouple, dp->GetLogKineticEnergy());
...
void G4GoudsmitSaundersonMscModel::SampleMSC()
{
  ...
  const G4double eloss =
    currentKinEnergy - GetEnergy(particle, currentRange - fTheTrueStepLenght, currentCouple);
  const G4double midEkin =
    currentKinEnergy - 0.5 * eloss;  // mid-step energy `\tilde{E} := E_0 - 0.5*\Delta E`
  const G4double tau = midEkin / electron_mass_c2;  // `\tau := \tilde{E}/(mc^2)`
  const G4double tau2 = tau * tau;
  const G4double eps0 =
    eloss
    / currentKinEnergy;  // energy loss fraction to pre-step energy: `\epsilon := \Delta E/E_0`
  const G4double epsm =
    eloss
    / midEkin;  // energy loss fraction to the mid-step energy: `\epsilon := \Delta E/\tilde{E}`
  const G4double epsm2 = epsm * epsm;
  const G4double effEner =
    midEkin * (1.0 - epsm2 * (6.0 + 10.0 * tau + 5.0 * tau2) / (24.0 * tau2 + 72.0 * tau + 48.0));
  const G4double dum = 1.0 / ((tau + 1.0) * (tau + 2.0));
  const G4double effStep =
    fTheTrueStepLenght
    * (1.0 - 0.166666 * epsm2 * dum * dum * (4.0 + tau * (6.0 + tau * (7.0 + tau * (4.0 + tau)))));
  ...
  if (0.5 * Qn1 < 7.0)
  {
    ...
    G4bool isMsc =
      fGSTable->Sampling(0.5 * lambdan, 0.5 * Qn1, fScrA, cosTheta1, sinTheta1, lekin, beta2,
                         currentMaterialIndex, &gsDtr, mcEkinIdx, mcDeltIdx, transfPar, true);
    ...
    if (cosTheta1 + cosTheta2 == 2.0)
    {  // no scattering happened
      fTheZPathLenght = fTheTrueStepLenght;
      fIsNoScatteringInMSC = true;
      return;
    }
  ...
  // note: quantities are based on the SR DCS, gamma and delta with (Mott, screening, DPWA)
  // corrections
  ...
  const G4double transportDistance =
    std::min(std::sqrt(rx * rx + ry * ry + rz * rz), fTheTrueStepLenght);
