...
void G4WentzelVIModel::Initialise(const G4ParticleDefinition* p, const G4DataVector& cuts)
{
  ...
  */
  ...
  {
    G4int nelm =
      (G4int)theCoupleTable->GetMaterialCutsCouple(i)->GetMaterial()->GetNumberOfElements();
  ...
  if (useSecondMoment && IsMaster() && nullptr != table)
  {
    ...
    {
      std::size_t n =
        G4EmParameters::Instance()->NumberOfBinsPerDecade() * G4lrint(std::log10(emax / emin));
      if (n < 3)
      {
        n = 3;
      }
...
{
  fSecondMoments = static_cast<G4WentzelVIModel*>(masterModel)->GetSecondMomentTable();
}
                                                      ...
                                                      G4double cutEnergy, G4double)
{
  ...
  {
    G4Exception("G4WentzelVIModel::ComputeCrossSectionPerAtom", "em0011", FatalException,
                                                      ...
                                                      G4double& currentMinimalStep)
{
  ...
  */
  ...
  G4double rlimit =
    std::max(facrange * currentRange, (1.0 - cosTetMaxNuc) * lambdaeff * invssFactor);
  ...
  if (rcut > rlimit)
  {
    rlimit = std::min(rlimit, rcut * std::sqrt(rlimit / rcut));
  }
...
G4double G4WentzelVIModel::ComputeSecondMoment(const G4ParticleDefinition* p, G4double kinEnergy)
{
  G4double xs = 0.0;
