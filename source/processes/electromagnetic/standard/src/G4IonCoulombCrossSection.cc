...
G4IonCoulombCrossSection::G4IonCoulombCrossSection()
  : cosThetaMin(1.0), cosThetaMax(-1.0), alpha2(fine_structure_const * fine_structure_const)
{
...
void G4IonCoulombCrossSection::Initialise(const G4ParticleDefinition* p, G4double CosThetaLim)
{
  SetupParticle(p);
...
void G4IonCoulombCrossSection::SetupTarget(G4double Z, G4double e, G4int)
{
  if (Z != targetZ || e != etag)
  {
    etag = e;
...
G4double G4IonCoulombCrossSection::NuclearCrossSection()
{
  ...
  {
    nucXSection = fac * (cosTetMinNuc - cosTetMaxNuc) / (x1 * (1.0 - cosTetMaxNuc + screenZ));
  }
