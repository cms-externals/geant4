                                                    ...
                                                    const G4double averageLoss)
{
  ...
  //
  if (particleMass > CLHEP::electron_mass_c2 && meanLoss >= minNumberInteractionsBohr * tcut
      && tmax <= 2. * tcut)
  {
    siga = std::sqrt((tmax / beta2 - 0.5 * tcut) * CLHEP::twopi_mc2_rcl2 * length * chargeSquare
  ...
  w2 = (tcut > ipotFluct) ? G4Log(2. * CLHEP::electron_mass_c2 * beta2 * gam2) - beta2 : 0.0;
  return SampleGlandz(rndmEngineF, material, tcut) * scaling;
}

//....oooOO0OOooo........oooOO0OOooo........oooOO0OOooo........oooOO0OOooo......

G4double G4UniversalFluctuation::SampleGlandz(CLHEP::HepRandomEngine* rndmEngineF,
...
G4double G4UniversalFluctuation::Dispersion(const G4Material* material, const G4DynamicParticle* dp,
                                            const G4double tcut, const G4double tmax,
                                            const G4double length)
{
  if (dp->GetDefinition() != particle)
