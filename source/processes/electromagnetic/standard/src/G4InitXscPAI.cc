...
void G4InitXscPAI::KillCloseIntervals()
{
  ...
  for (i = 0; i < fIntervalNumber - 1; i++)
  {
    ...
    if (energy2 - energy1 > 1.5 * fDelta * (energy1 + energy2))
      continue;
    else
    {
...
G4double G4InitXscPAI::RutherfordIntegral(G4int k, G4double x1, G4double x2)
{
...
G4double G4InitXscPAI::ImPartDielectricConst(G4int k, G4double energy1)
{
...
G4double G4InitXscPAI::ModuleSqDielectricConst(G4int k, G4double omega)
{
...
{
  G4int i;
  G4double x0, x02, x03, x04, x05, x1, x2, a1, a2, a3, a4, xx1, xx2, xx12, c1, c2, c3, cof1, cof2,
    xln1, xln2, xln3, result;
...
G4double G4InitXscPAI::DifPAIxSection(G4double omega)
{
  ...
  {
    x2 = -log((1 / betaGammaSq - epsilonRe) * (1 / betaGammaSq - epsilonRe) + epsilonIm * epsilonIm)
         / 2;
  }
  ...
  else
  {
    ...
    x5 = -1 - epsilonRe + be2 * ((1 + epsilonRe) * (1 + epsilonRe) + epsilonIm * epsilonIm);
  ...
  //   if( x4 < 0.0 ) x4 = 0.0 ;
  x8 = (1 + epsilonRe) * (1 + epsilonRe) + epsilonIm * epsilonIm;
...
G4double G4InitXscPAI::PAIdNdxCherenkov(G4double omega)
{
  ...
  {
    logarithm =
      -log((1 / betaGammaSq - epsilonRe) * (1 / betaGammaSq - epsilonRe) + epsilonIm * epsilonIm)
      * 0.5;
  ...
  else
  {
    ...
    x5 = -1.0 - epsilonRe + be2 * ((1.0 + epsilonRe) * (1.0 + epsilonRe) + epsilonIm * epsilonIm);
    if (x3 == 0.0)
      argument = 0.5 * pi;
    else
      argument = atan2(epsilonIm, x3);
    argument *= x5;
  }
  ...
  if (fDensity >= fSolidDensity)
  {
    modul2 = (1.0 + epsilonRe) * (1.0 + epsilonRe) + epsilonIm * epsilonIm;
    dNdxC /= modul2;
  }
  return dNdxC;
...
G4double G4InitXscPAI::PAIdNdxPlasmon(G4double omega)
{
  ...
  if (fDensity >= fSolidDensity)
  {
    modul2 = (1 + epsilonRe) * (1 + epsilonRe) + epsilonIm * epsilonIm;
    dNdxP /= modul2;
  }
  return dNdxP;
...
void G4InitXscPAI::IntegralPAIxSection(G4double bg2, G4double Tmax)
{
  ...
  for (k = fPAIbin - 2; k >= 0; k--)
  {
    ...
    {
      fCurrentInterval = i1;
      result += integral.Legendre10(this, &G4InitXscPAI::DifPAIxSection, energy1, energy2);
    ...
    else
    {
      for (i = i2; i >= i1; i--)
      {
        ...
        if (i == i2)
          ...
          result += integral.Legendre10(this, &G4InitXscPAI::DifPAIxSection, energy1,
                                        (*(*fMatSandiaMatrix)[i + 1])[0]);

        else
          result +=
            integral.Legendre10(this, &G4InitXscPAI::DifPAIxSection, (*(*fMatSandiaMatrix)[i])[0],
                                (*(*fMatSandiaMatrix)[i + 1])[0]);
      }
      fPAIxscVector->PutValue(k, result);
    }
...
void G4InitXscPAI::IntegralPAIdEdx(G4double bg2, G4double Tmax)
{
  ...
  for (k = fPAIbin - 2; k >= 0; k--)
  {
    ...
    {
      fCurrentInterval = i1;
      result += integral.Legendre10(this, &G4InitXscPAI::DifPAIdEdx, energy1, energy2);
    ...
    else
    {
      for (i = i2; i >= i1; i--)
      {
        ...
        if (i == i2)
          ...
          result += integral.Legendre10(this, &G4InitXscPAI::DifPAIdEdx, energy1,
                                        (*(*fMatSandiaMatrix)[i + 1])[0]);

        else
          result +=
            integral.Legendre10(this, &G4InitXscPAI::DifPAIdEdx, (*(*fMatSandiaMatrix)[i])[0],
                                (*(*fMatSandiaMatrix)[i + 1])[0]);
      }
      fPAIdEdxVector->PutValue(k, result);
    }
...
void G4InitXscPAI::IntegralCherenkov(G4double bg2, G4double Tmax)
{
  ...
  for (k = fPAIbin - 2; k >= 0; k--)
  {
    ...
    {
      fCurrentInterval = i1;
      result += integral.Legendre10(this, &G4InitXscPAI::PAIdNdxCherenkov, energy1, energy2);
    ...
    else
    {
      for (i = i2; i >= i1; i--)
      {
        ...
        if (i == i2)
          ...
          result += integral.Legendre10(this, &G4InitXscPAI::PAIdNdxCherenkov, energy1,
                                        (*(*fMatSandiaMatrix)[i + 1])[0]);

        else
          result +=
            integral.Legendre10(this, &G4InitXscPAI::PAIdNdxCherenkov, (*(*fMatSandiaMatrix)[i])[0],
                                (*(*fMatSandiaMatrix)[i + 1])[0]);
      }
      fPAIphotonVector->PutValue(k, result);
    }
...
void G4InitXscPAI::IntegralPlasmon(G4double bg2, G4double Tmax)
{
  ...
  for (k = fPAIbin - 2; k >= 0; k--)
  {
    ...
    {
      fCurrentInterval = i1;
      result += integral.Legendre10(this, &G4InitXscPAI::PAIdNdxPlasmon, energy1, energy2);
    ...
    else
    {
      for (i = i2; i >= i1; i--)
      {
        ...
        if (i == i2)
          ...
          result += integral.Legendre10(this, &G4InitXscPAI::PAIdNdxPlasmon, energy1,
                                        (*(*fMatSandiaMatrix)[i + 1])[0]);

        else
          result +=
            integral.Legendre10(this, &G4InitXscPAI::PAIdNdxPlasmon, (*(*fMatSandiaMatrix)[i])[0],
                                (*(*fMatSandiaMatrix)[i + 1])[0]);
      }
      fPAIelectronVector->PutValue(k, result);
    }
