...
G4DeltaAngle::G4DeltaAngle(const G4String&) : G4VEmAngularDistribution("deltaVI")
{
...
G4ThreeVector& G4DeltaAngle::SampleDirection(const G4DynamicParticle* dp, G4double kinEnergyFinal,
                                             G4int Z, const G4Material*)
{
  ...
  if (idx < 0 || idx >= nShells)
  {
    ...
    {
      sum +=
        G4AtomicShells::GetNumberOfElectrons(Z, idx) / G4AtomicShells::GetBindingEnergy(Z, idx);
      prob[idx] = sum;
    }
    sum *= G4UniformRand();
    for (idx = 0; idx < nShells; ++idx)
  ...
  */
  ...
  do
  {
    ...
    if (dp->GetParticleDefinition() == fElectron)
    {
      ...
      totMomentum = sqrt((totEnergy + electron_mass_c2) * (totEnergy - electron_mass_c2));
    }
