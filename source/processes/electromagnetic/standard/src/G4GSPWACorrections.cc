...
void G4GSPWACorrections::GetPWACorrectionFactors(G4double logekin, G4double beta2, G4int matindx,
                                                 G4double& corToScr, G4double& corToQ1,
                                                 G4double& corToG2PerG1)
{
  ...
  {
    corToScr +=
      remRfaction * (data->fCorScreening[ekinIndxLow + 1] - data->fCorScreening[ekinIndxLow]);
    corToQ1 +=
      remRfaction * (data->fCorFirstMoment[ekinIndxLow + 1] - data->fCorFirstMoment[ekinIndxLow]);
    corToG2PerG1 +=
      remRfaction * (data->fCorSecondMoment[ekinIndxLow + 1] - data->fCorSecondMoment[ekinIndxLow]);
  }
}

void G4GSPWACorrections::Initialise()
{
  // load PWA correction data for each elements that belongs to materials that are used in the
  // detector
  InitDataPerElement();
...
void G4GSPWACorrections::InitDataMaterial(const G4Material* mat)
{
  ...
  for (G4int iek = 0; iek < gNumEkin; ++iek)
  {
    ...
    for (G4int ielem = 0; ielem < numElems; ++ielem)
    {
      ...
      G4double mcScrCF =
        perElem
          ->fCorScreening[iek];  // \kappa_i[1.13+3.76(\alpha Z_i)^2] with \kappa_i=scr_mc/scr_sr
      ...
      perMat->fCorFirstMoment[iek] +=
        nZZPlus1 * (G4Log(1. + 1. / mcScrCF) - 1. / (1. + mcScrCF)) * perElem->fCorFirstMoment[iek];
      ...
      if (ielem == numElems - 1)
      {
        ...
        perMat->fCorFirstMoment[iek] =
          perMat->fCorFirstMoment[iek] / (zs * (dum0 - 1. / (1. + scrCorTed)));
        //
