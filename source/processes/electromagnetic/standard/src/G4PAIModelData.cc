...
G4PAIModelData::G4PAIModelData(G4double tmin, G4double tmax, G4int ver)
{
  ...
  if (0 < ver)
  {
    G4cout << "### G4PAIModelData: Nbins= " << fTotBin << " Tlowest(keV)= " << lowestTkin / keV
...
void G4PAIModelData::Initialise(const G4MaterialCutsCouple* couple, G4PAIModel* model)
{
...
G4double G4PAIModelData::DEDXPerVolume(G4int coupleIndex, G4double scaledTkin, G4double cut) const
{
...
G4double G4PAIModelData::CrossSectionPerVolume(G4int coupleIndex, G4double scaledTkin,
                                               G4double tcut, G4double tmax) const
{
  ...
  if (!one)
  {
    cross2 = (*table)(iPlace + 1)->Value(tcut) / tcut - (*table)(iPlace + 1)->Value(tmax) / tmax;
...
G4double G4PAIModelData::SampleAlongStepTransfer(G4int coupleIndex, G4double kinEnergy,
                                                 G4double scaledTkin, G4double tmax,
...
G4double G4PAIModelData::SamplePostStepTransfer(G4int coupleIndex, G4double scaledTkin,
...
G4double G4PAIModelData::GetEnergyTransfer(G4int coupleIndex, std::size_t iPlace,
                                           G4double position) const
{
  ...
  for (iTransfer = 1; iTransfer <= iTransferMax; ++iTransfer)
  {
    ...
    if (iTransfer == iTransferMax)
    {
      return v->GetMaxEnergy();
    }
  }
