...
const G4double G4eBremsstrahlungRelModel::gBremFactor = 16. * CLHEP::fine_structure_const
...
const G4double G4eBremsstrahlungRelModel::gMigdalConstant =
  4. * CLHEP::pi * CLHEP::classic_electr_radius * CLHEP::electron_Compton_length
...
const G4double G4eBremsstrahlungRelModel::gXGL[] = {1.98550718e-02, 1.01666761e-01, 2.37233795e-01,
                                                    4.08282679e-01, 5.91717321e-01, 7.62766205e-01,
                                                    8.98333239e-01, 9.80144928e-01};
const G4double G4eBremsstrahlungRelModel::gWGL[] = {5.06142681e-02, 1.11190517e-01, 1.56853323e-01,
...
const G4double G4eBremsstrahlungRelModel::gFelLowZet[] = {0.0,    5.3104, 4.7935, 4.7402,
                                                          4.7112, 4.6694, 4.6134, 4.5520};
const G4double G4eBremsstrahlungRelModel::gFinelLowZet[] = {0.0,    5.9173, 5.6125, 5.5377,
...
G4eBremsstrahlungRelModel::~G4eBremsstrahlungRelModel()
{
  if (fIsInitializer)
  {
    ...
    for (auto const& ptr : gElementData)
    {
      delete ptr;
    }
    gElementData.clear();
...
void G4eBremsstrahlungRelModel::SetupForMaterial(const G4ParticleDefinition*, const G4Material* mat,
...
G4double G4eBremsstrahlungRelModel::MinPrimaryEnergy(const G4Material*, const G4ParticleDefinition*,
                                                     G4double cut)
{
...
G4double G4eBremsstrahlungRelModel::ComputeBremLoss(G4double tmax)
{
  ...
  for (G4int l = 0; l < nSub; ++l)
  {
    for (G4int igl = 0; igl < 8; ++igl)
    {
      ...
      const G4double dcs = fIsLPMActive ? ComputeRelDXSectionPerAtom(k)  // DCS WITHOUT LPM
...
void G4eBremsstrahlungRelModel::ComputeScreeningFunctions(G4double& phi1, G4double& phi1m2,
                                                          G4double& psi1, G4double& psi1m2,
                                                          const G4double gam, const G4double eps)
{
  const G4double gam2 = gam * gam;
  phi1 =
    16.863 - 2.0 * G4Log(1.0 + 0.311877 * gam2) + 2.4 * G4Exp(-0.9 * gam) + 1.6 * G4Exp(-1.5 * gam);
                                                  ...
                                                  G4double maxEnergy)
{
  ...
  // min max of the transformed variable: x(k) = ln(k^2+k_p^2) that is in [ln(k_c^2+k_p^2),
  // ln(E_k^2+k_p^2)]
  ...
  do
  {
    ...
    funcVal =
      fIsLPMActive ? ComputeRelDXSectionPerAtom(gammaEnergy) : ComputeDXSectionPerAtom(gammaEnergy);
  ...
  const G4double totMomentum =
    std::sqrt(kineticEnergy * (fPrimaryTotalEnergy + CLHEP::electron_mass_c2));
...
void G4eBremsstrahlungRelModel::ComputeLPMfunctions(G4double& funcXiS, G4double& funcGS,
                                                    G4double& funcPhiS, const G4double egamma)
{
  static const G4double sqrt2 = std::sqrt(2.);
  const G4double redegamma = egamma / fPrimaryTotalEnergy;
  const G4double varSprime =
    std::sqrt(0.125 * redegamma * fLPMEnergy / ((1.0 - redegamma) * fPrimaryTotalEnergy));
  const ElementData* elDat = gElementData[fCurrentIZ];
  ...
  else if (varSprime > condition)
  {
    ...
    funcXiSprime = 1.0 + funcHSprime
                   - 0.08 * (1.0 - funcHSprime) * funcHSprime * (2.0 - funcHSprime) * ilVarS1Cond;
  }
