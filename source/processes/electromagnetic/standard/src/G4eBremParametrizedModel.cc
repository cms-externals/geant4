...
G4eBremParametrizedModel::G4eBremParametrizedModel(const G4ParticleDefinition* p,
  ...
  : G4VEmModel(nam),
    particle(nullptr),
    fMigdalConstant(classic_electr_radius * electron_Compton_length * electron_Compton_length * 4.0
                    * pi),
...
void G4eBremParametrizedModel::SetupForMaterial(const G4ParticleDefinition*, const G4Material* mat,
                                                                       ...
                                                                       G4double Z)
{
  ...
  if (kineticEnergy > tlow)
  {
    ...
    G4double screenfac = 136. * electron_mass_c2 / (Z3 * totalEnergy);
                                                 ...
                                                 G4double maxEnergy)
{
  ...
  const G4Element* elm =
    SelectTargetAtom(couple, particle, kineticEnergy, dp->GetLogKineticEnergy(), cut, emax);
  ...
  do
  {
    ...
    if (f > fMax)
    {
      G4cout << "### G4eBremParametrizedModel Warning: Majoranta exceeded! " << f << " > " << fMax
             << " Egamma(MeV)= " << gammaEnergy << " E(mEV)= " << kineticEnergy << G4endl;
    }
  ...
  G4ThreeVector gammaDirection = GetAngularDistribution()->SampleDirection(
    dp, totalEnergy - gammaEnergy, G4lrint(currentZ), couple->GetMaterial());
  ...
  if (gammaEnergy > SecondaryThreshold())
  {
    ...
    auto el = new G4DynamicParticle(const_cast<G4ParticleDefinition*>(particle), direction, finalE);
    vdp->push_back(el);
