...
G4PAIxSection::G4PAIxSection(G4int materialIndex, G4double maxEnergyTransfer)
{
  fSandia = nullptr;
  fMatSandiaMatrix = nullptr;
  fVerbose = 0;
  ...
  fIntervalNumber = (*theMaterialTable)[materialIndex]->GetSandiaTable()->GetMatNbOfIntervals();
  fIntervalNumber--;
  ...
  {
    if (((*theMaterialTable)[materialIndex]->GetSandiaTable()->GetSandiaCofForMaterial(i - 1, 0)
         >= maxEnergyTransfer)
        || i > fIntervalNumber)
    {
    ...
    fEnergyInterval[i] =
      (*theMaterialTable)[materialIndex]->GetSandiaTable()->GetSandiaCofForMaterial(i - 1, 0);
    fA1[i] =
      (*theMaterialTable)[materialIndex]->GetSandiaTable()->GetSandiaCofForMaterial(i - 1, 1);
    fA2[i] =
      (*theMaterialTable)[materialIndex]->GetSandiaTable()->GetSandiaCofForMaterial(i - 1, 2);
    fA3[i] =
      (*theMaterialTable)[materialIndex]->GetSandiaTable()->GetSandiaCofForMaterial(i - 1, 3);
    fA4[i] =
      (*theMaterialTable)[materialIndex]->GetSandiaTable()->GetSandiaCofForMaterial(i - 1, 4);
...
G4PAIxSection::G4PAIxSection(G4int materialIndex, G4double maxEnergyTransfer, G4double betaGammaSq,
                             G4double** photoAbsCof, G4int intNumber)
{
  ...
  */
  for (i = 1; i <= fIntervalNumber; i++)
  {
    if ((photoAbsCof[i - 1][0] >= maxEnergyTransfer) || i > fIntervalNumber)
    {
      fEnergyInterval[i] = maxEnergyTransfer;
      fIntervalNumber = i;
      break;
  ...
  ComputeLowEnergyCof();
  G4double betaGammaSqRef = fLorentzFactor[fRefGammaNumber] * fLorentzFactor[fRefGammaNumber] - 1;
...
G4PAIxSection::G4PAIxSection(G4int materialIndex, G4double maxEnergyTransfer, G4double betaGammaSq)
{
  ...
  {
    thisMaterialZ[i] = (G4int)(*theMaterialTable)[materialIndex]->GetElement(i)->GetZ();
  }
  ...
  fIntervalNumber = thisMaterialSandiaTable.SandiaIntervals(thisMaterialZ, numberOfElements);
  fIntervalNumber = thisMaterialSandiaTable.SandiaMixing(
    thisMaterialZ, (*theMaterialTable)[materialIndex]->GetFractionVector(), numberOfElements,
    fIntervalNumber);
...
void G4PAIxSection::Initialize(const G4Material* material, G4double maxEnergyTransfer,
                               G4double betaGammaSq, G4SandiaTable* sandia)
{
  if (fVerbose > 0)
  {
    G4cout << G4endl;
  ...
  if (fVerbose > 0)
  {
    G4cout << "fDensity = " << fDensity << "\t" << fElectronDensity
           << "\t fIntervalNumber = " << fIntervalNumber << G4endl;
  }
  fEnergyInterval = G4DataVector(fIntervalNumber + 2, 0.0);
  fA1 = G4DataVector(fIntervalNumber + 2, 0.0);
  fA2 = G4DataVector(fIntervalNumber + 2, 0.0);
  ...
  if (fVerbose > 0)
    G4cout << "last i = " << i << "; " << "fIntervalNumber = " << fIntervalNumber << G4endl;
  ...
  for (i = 1; i < fIntervalNumber; i++)
  {
    ...
    {
      if (fVerbose > 0)
        G4cout << i << "\t" << fEnergyInterval[i] / keV << "\t" << fEnergyInterval[i + 1] / keV;
  ...
  {
    if (fVerbose > 0)
      G4cout << i << "; w = " << fSplineEnergy[i] / keV
             << " keV; dN/dx_>w = " << fIntegralPAIxSection[i] << " 1/mm" << G4endl;
  }
}
...
{
  G4int i;
  G4double betaGammaSq = fLorentzFactor[fRefGammaNumber] * fLorentzFactor[fRefGammaNumber] - 1;
...
void G4PAIxSection::NormShift(G4double betaGammaSq)
{
  ...
  for (i = 1; i <= fIntervalNumber - 1; i++)
  {
    for (j = 1; j <= 2; j++)
    {
      ...
      if (fVerbose > 0)
        G4cout << "cn = " << fSplineNumber << "; " << "w = " << fSplineEnergy[fSplineNumber] / keV
               << " keV" << G4endl;
    }
  }
  ...
  for (i = 2; i <= fSplineNumber; i++)
  {
    ...
    {
      fIntegralTerm[i] =
        fIntegralTerm[i - 1] + RutherfordIntegral(j, fSplineEnergy[i - 1], fSplineEnergy[i]);
    }
    else
    {
      G4double x = RutherfordIntegral(j, fSplineEnergy[i - 1], fEnergyInterval[j + 1]);
      j++;
      fIntegralTerm[i] =
        fIntegralTerm[i - 1] + x + RutherfordIntegral(j, fEnergyInterval[j], fSplineEnergy[i]);
    }
    if (fVerbose > 0)
      G4cout << i << "  Shift: w = " << fSplineEnergy[i] / keV << " keV \t" << fIntegralTerm[i]
             << "\n"
             << G4endl;
  }
  ...
  for (G4int k = 1; k <= fIntervalNumber - 1; k++)
  {
    ...
    {
      i = (k - 1) * 2 + j;
      fImPartDielectricConst[i] = fNormalizationCof * ImPartDielectricConst(k, fSplineEnergy[i]);
      fRePartDielectricConst[i] = fNormalizationCof * RePartDielectricConst(fSplineEnergy[i]);
      ...
      fdNdxResonance[i] = PAIdNdxResonance(i, betaGammaSq);
      if (fVerbose > 0)
        G4cout << i << "  Shift: w = " << fSplineEnergy[i] / keV
...
void G4PAIxSection::SplainPAI(G4double betaGammaSq)
{
  ...
  {
    // if( std::abs(fSplineEnergy[i+1]-fEnergyInterval[k+1]) >
    // (fSplineEnergy[i+1]+fEnergyInterval[k+1])*5.e-7 )
    if (fSplineEnergy[i + 1] > fEnergyInterval[k + 1])
    {
      k++;  // Here next energy point is in next energy interval
      i++;
      if (fVerbose > 0)
        G4cout << "                     in if: i = " << i << "; k = " << k << G4endl;
      continue;
    }
    ...
    if (fVerbose > 0)
      G4cout << "Spline: x1 = " << x1 << "; x2 = " << x2 << ", yy1 = " << yy1 << "; y2 = " << y2
             << G4endl;
    ...
    fImPartDielectricConst[i + 1] =
      fNormalizationCof * ImPartDielectricConst(k, fSplineEnergy[i + 1]);
    fRePartDielectricConst[i + 1] = fNormalizationCof * RePartDielectricConst(fSplineEnergy[i + 1]);
    fIntegralTerm[i + 1] =
      fIntegralTerm[i]
      + fNormalizationCof * RutherfordIntegral(k, fSplineEnergy[i], fSplineEnergy[i + 1]);
    ...
    if (fVerbose > 0)
      G4cout << "Spline, a = " << a << "; b = " << b << "; new xsc = " << y
             << "; compxsc = " << fDifPAIxSection[i + 1] << G4endl;
    ...
    G4double delta =
      2. * (fSplineEnergy[i + 1] - fSplineEnergy[i]) / (fSplineEnergy[i + 1] + fSplineEnergy[i]);
...
G4double G4PAIxSection::RutherfordIntegral(G4int k, G4double x1, G4double x2)
{
  G4double c1, c2, c3;
...
G4double G4PAIxSection::ImPartDielectricConst(G4int k, G4double energy1)
{
...
{
  G4double x0, x02, x03, x04, x05, x1, x2, xx1, xx2, xx12, c1, c2, c3, cof1, cof2, xln1, xln2, xln3,
    result;
...
G4double G4PAIxSection::DifPAIxSection(G4int i, G4double betaGammaSq)
{
  ...
  else
  {
    ...
    x5 = -1 - fRePartDielectricConst[i]
         + be2
             * ((1 + fRePartDielectricConst[i]) * (1 + fRePartDielectricConst[i])
                + fImPartDielectricConst[i] * fImPartDielectricConst[i]);
...
G4double G4PAIxSection::PAIdNdxCerenkov(G4int i, G4double betaGammaSq)
{
  ...
  if (betaGammaSq < 0.01)
    logarithm = std::log(1.0 + betaGammaSq);  // 0.0;
  else
  {
  ...
  else
  {
    ...
    x5 = -1.0 - fRePartDielectricConst[i]
         + be2
             * ((1.0 + fRePartDielectricConst[i]) * (1.0 + fRePartDielectricConst[i])
                + fImPartDielectricConst[i] * fImPartDielectricConst[i]);
    if (x3 == 0.0)
      argument = 0.5 * pi;
    else
      argument = std::atan2(fImPartDielectricConst[i], x3);
    argument *= x5;
  }
...
G4double G4PAIxSection::PAIdNdxMM(G4int i, G4double betaGammaSq)
{
  ...
  if (betaGammaSq < 0.01)
    logarithm = log(1.0 + betaGammaSq);  // 0.0;
  else
  {
    logarithm = -log((1 / betaGammaSq - fRePartDielectricConst[i])
                       * (1 / betaGammaSq - fRePartDielectricConst[i])
  ...
  else
  {
    ...
    if (x3 == 0.0)
      ...
      argument = atan2(fImPartDielectricConst[i], x3);
    argument *= x5;
  }
...
G4double G4PAIxSection::PAIdNdxPlasmon(G4int i, G4double betaGammaSq)
{
...
G4double G4PAIxSection::PAIdNdxResonance(G4int i, G4double betaGammaSq)
{
...
void G4PAIxSection::IntegralPAIxSection()
{
  ...
  for (i = fSplineNumber - 1; i >= 1; i--)
  {
    ...
    {
      fIntegralPAIxSection[i] =
        fIntegralPAIxSection[i + 1] + SumOverBorder(i + 1, fEnergyInterval[k]);
      fIntegralPAIdEdx[i] = fIntegralPAIdEdx[i + 1] + SumOverBorderdEdx(i + 1, fEnergyInterval[k]);
      k--;
    }
    if (fVerbose > 0)
      G4cout << "i = " << i << "; k = " << k << "; intPAIxsc[i] = " << fIntegralPAIxSection[i]
             << G4endl;
  }
...
void G4PAIxSection::IntegralCerenkov()
{
  ...
  for (i = fSplineNumber - 1; i >= 1; i--)
  {
    ...
    {
      fIntegralCerenkov[i] =
        fIntegralCerenkov[i + 1] + SumOverBordCerenkov(i + 1, fEnergyInterval[k]);
      k--;
...
void G4PAIxSection::IntegralMM()
{
  ...
  for (i = fSplineNumber - 1; i >= 1; i--)
  {
    ...
    {
      fIntegralMM[i] = fIntegralMM[i + 1] + SumOverBordMM(i + 1, fEnergyInterval[k]);
      k--;
...
void G4PAIxSection::IntegralPlasmon()
{
  ...
  for (G4int i = fSplineNumber - 1; i >= 1; i--)
  {
    ...
    {
      fIntegralPlasmon[i] = fIntegralPlasmon[i + 1] + SumOverBordPlasmon(i + 1, fEnergyInterval[k]);
      k--;
    }
  }
...
void G4PAIxSection::IntegralResonance()
{
  ...
  for (G4int i = fSplineNumber - 1; i >= 1; i--)
  {
    ...
    {
      fIntegralResonance[i] =
        fIntegralResonance[i + 1] + SumOverBordResonance(i + 1, fEnergyInterval[k]);
      k--;
    }
  }
...
G4double G4PAIxSection::SumOverInterval(G4int i)
{
  ...
  if (fVerbose > 0)
    G4cout << "SumOverInterval i= " << i << " x0 = " << x0 << "; x1 = " << x1 << G4endl;
  ...
  if (fVerbose > 0)
    G4cout << "x0 = " << x0 << "; x1 = " << x1 << ", y0 = " << y0 << "; yy1 = " << yy1 << G4endl;
...
G4double G4PAIxSection::SumOverInterCerenkov(G4int i)
{
  ...
  if (a == 0)
    fIntegralCerenkov[0] += b * log(x1 / x0);
  else
    fIntegralCerenkov[0] += y0 * (x1 * x1 * pow(c, a - 2) - x0 * x0) / a;
...
G4double G4PAIxSection::SumOverInterMM(G4int i)
{
  ...
  if (a == 0)
    fIntegralMM[0] += b * log(c);
  else
    fIntegralMM[0] += y0 * (x1 * x1 * pow(c, a - 2) - x0 * x0) / a;
  // G4cout<<"a = "<<a<<"; b = "<<b<<"; result = "<<result<<G4endl;
  return result;
...
G4double G4PAIxSection::SumOverInterPlasmon(G4int i)
{
  ...
  if (a == 0)
    fIntegralPlasmon[0] += b * log(x1 / x0);
  else
    fIntegralPlasmon[0] += y0 * (x1 * x1 * pow(c, a - 2) - x0 * x0) / a;
...
G4double G4PAIxSection::SumOverInterResonance(G4int i)
{
  ...
  if (a == 0)
    fIntegralResonance[0] += b * log(x1 / x0);
  else
    fIntegralResonance[0] += y0 * (x1 * x1 * pow(c, a - 2) - x0 * x0) / a;
...
G4double G4PAIxSection::SumOverBorder(G4int i, G4double en0)
{
...
G4double G4PAIxSection::SumOverBordCerenkov(G4int i, G4double en0)
{
  ...
  if (a == 0)
    fIntegralCerenkov[0] += b * log(x0 / e0);
  else
    fIntegralCerenkov[0] += y0 * (x0 * x0 - e0 * e0 * pow(d, a - 2)) / a;
  ...
  if (a == 0)
    fIntegralCerenkov[0] += b * log(e0 / x0);
  else
    fIntegralCerenkov[0] += y0 * (e0 * e0 * pow(d, a - 2) - x0 * x0) / a;
...
G4double G4PAIxSection::SumOverBordMM(G4int i, G4double en0)
{
  ...
  if (a == 0)
    fIntegralMM[0] += b * log(x0 / e0);
  else
    fIntegralMM[0] += y0 * (x0 * x0 - e0 * e0 * pow(d, a - 2)) / a;
  ...
  if (a == 0)
    fIntegralMM[0] += b * log(e0 / x0);
  else
    fIntegralMM[0] += y0 * (e0 * e0 * pow(d, a - 2) - x0 * x0) / a;
...
G4double G4PAIxSection::SumOverBordPlasmon(G4int i, G4double en0)
{
  G4double x0, x1, y0, yy1, a, b, c, d, e0, result;
  ...
  if (a == 0)
    fIntegralPlasmon[0] += b * log(x0 / e0);
  else
    fIntegralPlasmon[0] += y0 * (x0 * x0 - e0 * e0 * pow(d, a - 2)) / a;
  ...
  if (a == 0)
    fIntegralPlasmon[0] += b * log(e0 / x0);
  else
    fIntegralPlasmon[0] += y0 * (e0 * e0 * pow(d, a - 2) - x0 * x0) / a;
...
G4double G4PAIxSection::SumOverBordResonance(G4int i, G4double en0)
{
  G4double x0, x1, y0, yy1, a, b, c, d, e0, result;
  ...
  if (a == 0)
    fIntegralResonance[0] += b * log(x0 / e0);
  else
    fIntegralResonance[0] += y0 * (x0 * x0 - e0 * e0 * pow(d, a - 2)) / a;
  ...
  if (a == 0)
    fIntegralResonance[0] += b * log(e0 / x0);
  else
    fIntegralResonance[0] += y0 * (e0 * e0 * pow(d, a - 2) - x0 * x0) / a;
...
const G4double G4PAIxSection::fLorentzFactor[112] =  // fNumberOfGammas+1
  {0.0,          1.094989e+00, 1.107813e+00, 1.122369e+00, 1.138890e+00, 1.157642e+00,
   1.178925e+00, 1.203082e+00, 1.230500e+00, 1.261620e+00, 1.296942e+00,  // 10
   1.337032e+00, 1.382535e+00, 1.434181e+00, 1.492800e+00, 1.559334e+00, 1.634850e+00,
   1.720562e+00, 1.817845e+00, 1.928263e+00, 2.053589e+00,  // 20
   2.195835e+00, 2.357285e+00, 2.540533e+00, 2.748522e+00, 2.984591e+00, 3.252533e+00,
   3.556649e+00, 3.901824e+00, 4.293602e+00, 4.738274e+00,  // 30
   5.242981e+00, 5.815829e+00, 6.466019e+00, 7.203990e+00, 8.041596e+00, 8.992288e+00,
   1.007133e+01, 1.129606e+01, 1.268614e+01, 1.426390e+01,  // 40
   1.605467e+01, 1.808721e+01, 2.039417e+01, 2.301259e+01, 2.598453e+01, 2.935771e+01,
   3.318630e+01, 3.753180e+01, 4.246399e+01, 4.806208e+01,  // 50
   5.441597e+01, 6.162770e+01, 6.981310e+01, 7.910361e+01, 8.964844e+01, 1.016169e+02,
   1.152013e+02, 1.306197e+02, 1.481198e+02, 1.679826e+02,  // 60
   1.905270e+02, 2.161152e+02, 2.451581e+02, 2.781221e+02, 3.155365e+02, 3.580024e+02,
   4.062016e+02, 4.609081e+02, 5.230007e+02, 5.934765e+02,  // 70
   6.734672e+02, 7.642575e+02, 8.673056e+02, 9.842662e+02, 1.117018e+03, 1.267692e+03,
   1.438709e+03, 1.632816e+03, 1.853128e+03, 2.103186e+03,  // 80
   2.387004e+03, 2.709140e+03, 3.074768e+03, 3.489760e+03, 3.960780e+03, 4.495394e+03,
   5.102185e+03, 5.790900e+03, 6.572600e+03, 7.459837e+03,  // 90
   8.466860e+03, 9.609843e+03, 1.090714e+04, 1.237959e+04, 1.405083e+04, 1.594771e+04,
   1.810069e+04, 2.054434e+04, 2.331792e+04, 2.646595e+04,  // 100
   3.003901e+04, 3.409446e+04, 3.869745e+04, 4.392189e+04, 4.985168e+04, 5.658206e+04,
   6.422112e+04, 7.289153e+04, 8.273254e+04, 9.390219e+04,  // 110
   1.065799e+05};
