...
G4eSingleCoulombScatteringModel::G4eSingleCoulombScatteringModel(const G4String& nam)
  : G4VEmModel(nam), cosThetaMin(1.0)
{
...
void G4eSingleCoulombScatteringModel::SetXSectionModel(const G4String& model)
{
  ...
  else if (model == "Precise" || model == "precise")
  {
    XSectionModel = 0;
  }
  else
  {
    G4cout << "G4eSingleCoulombScatteringModel WARNING: " << model << " is not a valid model name"
           << G4endl;
  }
}
                                                                     ...
                                                                     G4double, G4double, G4double)
{
  ...
  // cout<< "Compute Cross Section....cross "<<G4BestUnit(cross,"Surface") << " cm2 "<< cross/cm2
  // <<" Z: "<<Z<<" kinEnergy: "<<kinEnergy<<endl;
                                                        ...
                                                        G4double cutEnergy, G4double)
{
  ...
  currentElement =
    SelectTargetAtom(couple, particle, kinEnergy, dp->GetLogKineticEnergy(), cutEnergy, kinEnergy);
  ...
  // cout<< "Energy: "<<kinEnergy/MeV<<" Z: "<<Z<<"....cross "<<G4BestUnit(cross,"Surface") << " cm2
  // "<< cross/cm2 <<endl;
