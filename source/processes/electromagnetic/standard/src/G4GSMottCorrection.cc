...
void G4GSMottCorrection::GetMottCorrectionFactors(G4double logekin, G4double beta2, G4int matindx,
                                                  G4double& mcToScr, G4double& mcToQ1,
                                                  G4double& mcToG2PerG1)
{
  ...
  {  // linear interpolation on \beta^2
  ...
  else if (logekin >= fLogMinEkin)
  {
    ...
    ekinIndxLow = (G4int)remRfaction;
    remRfaction -= ekinIndxLow;
  }  // the defaults otherwise i.e. use the lowest energy values when ekin is smaller than the minum
     // ekin
  //
...
double G4GSMottCorrection::GetMottRejectionValue(G4double logekin, G4double beta2, G4double q1,
                                                 G4double cost, G4int matindx, G4int& ekindx,
                                                 G4int& deltindx)
{
  G4double val = 1.0;
  ...
  if (ekindx < 0)
  {
    ...
    {  // linear interpolation on \ln(E_{kin})
      probIndxHigh = (logekin - fLogMinEkin) * fInvLogDelEkin;
      ekinIndxLow = (G4int)probIndxHigh;
      probIndxHigh -= ekinIndxLow;
    }  // the defaults otherwise i.e. use the lowest energy values when ekin is smaller than the
       // minum ekin
    //
  ...
  // check if delta value index needs to be determined (note: in case of single scattering deltindx
  // will be set to 0 by by the caller but the ekindx will be -1: kinetic energy index is not known
  // but the delta index is known)
  if (deltindx < 0)
  {
    // note: delta is for sure < gMaxDelta at this point ( and minimum delta value is 0)
    G4double probIndxHigh =
      delta * fInvDelDelta;  // will be the prob. of taking the deltIndxLow+1 bin
  ...
  DataPerDelta* perDelta =
    fMCDataPerMaterial[matindx]->fDataPerEkin[ekindx]->fDataPerDelta[deltindx];
  //
...
{
  // load Mott-correction data for each elements that belongs to materials that are used in the
  // detector
...
void G4GSMottCorrection::InitMCDataPerElement()
{
  ...
  // loop over all materials, for those that are used check the list of elements and load data from
  // file if the corresponding data has not been loaded yet
...
void G4GSMottCorrection::InitMCDataMaterial(const G4Material* mat)
{
  ...
  for (G4int iek = 0; iek < gNumEkin; ++iek)
  {
    ...
    for (G4int ielem = 0; ielem < numElems; ++ielem)
    {
      ...
      G4double mcScrCF =
        perElemPerEkin
          ->fMCScreening;  // \kappa_i[1.13+3.76(\alpha Z_i)^2] with \kappa_i=scr_mc/scr_sr
      // compute the screening parameter correction factor (Z_i contribution to the material)
      // src_{mc} = C \exp\left[ \frac{ \sum_i n_i
      // Z_i(Z_i+1)\ln[Z_{i}^{2/3}\kappa_i(1.13+3.76(\alpha Z_i)^2)] } {\sum_i n_i Z_i(Z_i+1)} with
      // C = \frac{(mc^2)^\alpha^2} {4(pc)^2 C_{TF}^2} = constFactor/(4*(pc)^2) here we compute the
      // \sum_i n_i Z_i(Z_i+1)\ln[Z_{i}^{2/3}\kappa_i(1.13+3.76(\alpha Z_i)^2)] part
      ...
      // src(Z_i)_{mc} = \frac{(mc^2)^\alpha^2 Z_i^{2/3}} {4(pc)^2 C_{TF}^2}
      // \kappa_i[1.13+3.76(\alpha Z_i)^2]
      ...
      // where:
      // A_i(src(Z_i)_{mc}) = [\ln(1+1/src(Z_i)_{mc}) - 1/(1+src(Z_i)_{mc})]; where
      // \sigma(Z_i)_{tr1}^(sr) = A_i(src(Z_i)_{mc}) [2\pi r_0 Z_i mc^2/(pc)\beta]^2 B_i = \beta_i
      // \gamma_i with beta_i(Z_i) = \sigma(Z_i)_{tr1}^(PWA)/\sigma(Z_i,src(Z_i)_{mc})_{tr1}^(sr)
      // and \gamma_i = \sigma(Z_i)_{el}^(MC-DCS)/\sigma(Z_i,src(Z_i)_{mc})_{el}^(sr)
      // C(src_{mc}) = [\ln(1+1/src_{mc}) - 1/(1+src_{mc})]; where \sigma_{tr1}^(sr) = C(src_{mc})
      // [2\pi r_0 Z_i mc^2/(pc)\beta]^2 A_i x B_i is stored in file per e-/e+, E_{kin} and Z_i here
      // we compute the \sum_i n_i Z_i(Z_i+1) A_i  B_i part
      perMatPerEkin->fMCFirstMoment += nZZPlus1 * (G4Log(1. + 1. / mcScrCF) - 1. / (1. + mcScrCF))
                                       * perElemPerEkin->fMCFirstMoment;
      // compute the second moment correction factor
      // [G2/G1]_{mc} = \frac{ \sum_i n_i Z_i(Z_i+1) A_i } {\sum_i n_i Z_i(Z_i+1)} \frac{1}{C}
      // with A_i(Z_i) = G2(Z_i)^{PWA}/G1(Z_i)^{PWA} and
      // C=G2(Z_i,scr_{mc})^{sr}/G1(Z_i,scr_{mc})^{sr}} here we compute the \sum_i n_i Z_i(Z_i+1)
      // A_i part
      ...
      {
        //
        // 1. the remaining part of the sreening correction and divide the corrected screening par.
        // with Moliere's one:
        ...
        //
        // 2. the remaining part of the first moment correction and divide by the one computed by
        // using the corrected
        ...
        perMatPerEkin->fMCFirstMoment =
          perMatPerEkin->fMCFirstMoment / (zs * (dum0 - 1. / (1. + scrCorTed)));
        //
        // 3. the remaining part of the second moment correction and divide by the one computed by
        // using the corrected
        //    screening parameter
        G4double G2PerG1 = 3. * (1. + scrCorTed) * ((1. + 2. * scrCorTed) * dum0 - 2.)
                           / ((1. + scrCorTed) * dum0 - 1.);
        perMatPerEkin->fMCSecondMoment = perMatPerEkin->fMCSecondMoment / (zs * G2PerG1);
        //
        // 4. scale the maximum of the rejection function to unity and correct the last bin spline
        // parameters as well I. loop over delta values
        for (G4int idel = 0; idel < gNumDelta; ++idel)
        {
          DataPerDelta* perMatPerDelta = perMatPerEkin->fDataPerDelta[idel];
          G4double maxVal = -1.;
