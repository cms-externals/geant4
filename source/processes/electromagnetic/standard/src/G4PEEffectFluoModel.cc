...
G4PEEffectFluoModel::G4PEEffectFluoModel(const G4String& nam) : G4VEmModel(nam)
{
...
void G4PEEffectFluoModel::Initialise(const G4ParticleDefinition*, const G4DataVector&)
{
  ...
  {
    fMatEnergyTh[i] =
      (*(G4Material::GetMaterialTable()))[i]->GetSandiaTable()->GetSandiaCofForMaterial(0, 0);
                                                         ...
                                                         G4double, G4double)
{
  ...
  CurrentCouple()->GetMaterial()->GetSandiaTable()->GetSandiaCofPerAtom((G4int)Z, energy,
                                                                        fSandiaCof);

  G4double x1 = 1 / energy;

  return x1 * (fSandiaCof[0] + x1 * (fSandiaCof[1] + x1 * (fSandiaCof[2] + x1 * fSandiaCof[3])));
}

//....oooOO0OOooo........oooOO0OOooo........oooOO0OOooo........oooOO0OOooo....

G4double G4PEEffectFluoModel::CrossSectionPerVolume(const G4Material* material,
                                                    const G4ParticleDefinition*, G4double energy,
                                                    G4double, G4double)
{
  // This method may be used only if G4MaterialCutsCouple pointer
  //   has been set properly
  energy = std::max(energy, fMatEnergyTh[material->GetIndex()]);
  const G4double* SandiaCof = material->GetSandiaTable()->GetSandiaCofForMaterial(energy);

  G4double x1 = 1 / energy;

  return x1 * (SandiaCof[0] + x1 * (SandiaCof[1] + x1 * (SandiaCof[2] + x1 * SandiaCof[3])));
}
...
void G4PEEffectFluoModel::SampleSecondaries(std::vector<G4DynamicParticle*>* fvect,
                                            ...
                                            const G4DynamicParticle* aDynamicPhoton, G4double,
                                            G4double)
{
  ...
  if ((fPEBelowKShell || 0 == i) && i < nShells)
  {
    ...
    {
      auto aParticle =
        new G4DynamicParticle(theElectron,
                              GetAngularDistribution()->SampleDirection(
                                aDynamicPhoton, elecKineEnergy, i, couple->GetMaterial()),
                              elecKineEnergy);
    ...
    {
      G4cout << "### G4PEffectFluoModel dE(eV)= " << (energy - elecKineEnergy - esec - edep) / eV
             << " shell= " << i << "  E(keV)= " << energy / keV
             << "  Ebind(keV)= " << bindingEnergy / keV << "  Ee(keV)= " << elecKineEnergy / keV
             << "  Esec(keV)= " << esec / keV << "  Edep(keV)= " << edep / keV << G4endl;
    }
  }
