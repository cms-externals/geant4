...
G4MollerBhabhaModel::G4MollerBhabhaModel(const G4ParticleDefinition* p, const G4String& nam)
  : G4VEmModel(nam),
    particle(nullptr),
    isElectron(true),
                                                             ...
                                                             G4double cutEnergy, G4double maxEnergy)
{
  ...
  if (cutEnergy < tmax)
  {
    ...
    if (isElectron)
    {
      ...
      cross =
        ((xmax - xmin) * (1.0 - gg + 1.0 / (xmin * xmax) + 1.0 / ((1.0 - xmin) * (1.0 - xmax)))
    ...
    else
    {
      ...
      cross = (xmax - xmin)
                * (1.0 / (beta2 * xmin * xmax) + b2 - 0.5 * b3 * (xmin + xmax)
                                                   ...
                                                   G4double kineticEnergy, G4double cut)
{
  ...
  if (isElectron)
  {
    dedx = G4Log(2.0 * (tau + 2.0) / eexc2) - 1.0 - beta2 + G4Log((tau - d) * d) + tau / (tau - d)
  ...
  else
  {
    ...
    G4double y = 1.0 / (1.0 + gam);
    dedx =
      G4Log(2.0 * (tau + 2.0) / eexc2) + G4Log(tau * d)
