...
G4UrbanMscModel::G4UrbanMscModel(const G4String& nam) : G4VMscModel(nam)
{
  ...
  currentKinEnergy = currentRadLength = lambda0 = lambdaeff = tPathLength = zPathLength = par1 =
    par2 = par3 = rndmarray[0] = rndmarray[1] = 0;
...
G4double G4UrbanMscModel::ComputeCrossSectionPerAtom(const G4ParticleDefinition* part,
                                                     G4double kinEnergy, G4double atomicNumber,
                                                     G4double, G4double, G4double)
{
  ...
  static const G4double hecorr[15] = {120.70, 117.50, 105.00, 92.92, 79.23,  74.510, 68.29, 57.39,
  ...
  G4double beta2 =
    eKineticEnergy * (eTotalEnergy + CLHEP::electron_mass_c2) / (eTotalEnergy * eTotalEnergy);
  G4double bg2 = eKineticEnergy * (eTotalEnergy + CLHEP::electron_mass_c2)
                 / (CLHEP::electron_mass_c2 * CLHEP::electron_mass_c2);

  static const G4double epsfactor = 2. * CLHEP::electron_mass_c2 * CLHEP::electron_mass_c2
                                    * CLHEP::Bohr_radius * CLHEP::Bohr_radius
  ...
  static const G4double Tdat[22] = {
    100 * CLHEP::eV,  200 * CLHEP::eV, 400 * CLHEP::eV,  700 * CLHEP::eV,  1 * CLHEP::keV,
    2 * CLHEP::keV,   4 * CLHEP::keV,  7 * CLHEP::keV,   10 * CLHEP::keV,  20 * CLHEP::keV,
    40 * CLHEP::keV,  70 * CLHEP::keV, 100 * CLHEP::keV, 200 * CLHEP::keV, 400 * CLHEP::keV,
    700 * CLHEP::keV, 1 * CLHEP::MeV,  2 * CLHEP::MeV,   4 * CLHEP::MeV,   7 * CLHEP::MeV,
                                                     ...
                                                     G4double& currentMinimalStep)
{
  ...
  lambda0 = GetTransportMeanFreePath(particle, currentKinEnergy, currentLogKinEnergy);
  ...
  */
  ...
  presafety =
    (stepStatus == fGeomBoundary) ? sp->GetSafety() : ComputeSafety(sp->GetPosition(), tPathLength);
  ...
  G4double distance = (mass < masslimite) ? currentRange * msc[idx]->doverra
                                          // for muons, hadrons
  ...
  */
  ...
  if (steppingAlgorithm == fUseDistanceToBoundary)
  {
    ...
    */
    ...
    if (geomlimit < geombig && geomlimit > 0.0)
    {
      ...
      {
        G4double tm = currentKinEnergy / mass;
        G4double beta2 = tm * (tm + 2.) / ((tm + 1.) * (tm + 1.));
        G4double gr = currentRange * lambda0 / (currentRange + lambda0);
        geomlimit = beta2 * geomlimit + (1. - beta2) * gr;
      }
      tgeom = (stepStatus == fGeomBoundary) ? std::min(geomlimit / facgeom, stepmin)
                                            : facrange * rangeinit;
    }
    ...
    else if (geomlimit > geombig)
    {
      ...
      tgeom = (stepStatus == fGeomBoundary) ? stepmin : currentRange;
    }
    ...
    */
    ...
    if (smallstep <= skin)
    {
      if (stepStatus != fGeomBoundary) tlimit = stepmin;
      insideskin = true;
    }
    else if (geomlimit < geombig)
    {
    ...
    tPathLength = (tlimit < tPathLength && smallstep > skin && !insideskin && tlimit > geommin)
  ...
  else if (steppingAlgorithm == fUseSafety)
  {
    ...
    // step limit
    tlimit =
      (currentRange > presafety) ? std::max(fr * rangeinit, facsafety * presafety) : currentRange;
    ...
    {
      tPathLength = std::min(tPathLength, Randomizetlimit());
    }
  }
  // ----------------------------------------------------------------
  // for simulation with or without magnetic field
  ...
  else if (steppingAlgorithm == fUseSafetyPlus)
  {
    ...
    {
      G4double tmax = drr * currentRange + finalr * (1. - drr) * (2. - finalr / currentRange);
...
G4double G4UrbanMscModel::ComputeGeomPathLength(G4double)
{
  ...
  */
  ...
  {
    zPathLength = (tau < taulim) ? tPathLength * (1. - 0.5 * tau) : lambda0 * (1. - G4Exp(-tau));
  }
  else if (currentKinEnergy < mass || tPathLength == currentRange)
  {
    ...
    {
      zPathLength = (1. - G4Exp(par3 * G4Log(1. - tPathLength / currentRange))) / (par1 * par3);
    }
    else
    {
                                                 ...
                                                 G4double /*safety*/)
{
  ...
  {
    kinEnergy -= tPathLength * GetDEDX(particle, currentKinEnergy, couple, currentLogKinEnergy);
  }
...
G4double G4UrbanMscModel::SampleCosineTheta(G4double trueStepLength, G4double kinEnergy)
{
  G4double cth = 1.0;
  ...
  else if (tau >= tausmall)
  {
    ...
    G4double u =
      !extremesmallstep ? G4Exp(ltau * onesixth) : G4Exp(G4Log(tsmall / lambda0) * onesixth);

    G4double xx = G4Log(lambdaeff / currentRadLength);
    G4double xsi =
      msc[idx]->coeffc1 + u * (msc[idx]->coeffc2 + msc[idx]->coeffc3 * u) + msc[idx]->coeffc4 * xx;
...
G4double G4UrbanMscModel::ComputeTheta0(G4double trueStepLength, G4double kinEnergy)
{
...
void G4UrbanMscModel::SampleDisplacementNew(G4double, G4double phi)
{
  ...
  const G4double rmax = std::sqrt((tPathLength - zPathLength) * (tPathLength + zPathLength));
...
void G4UrbanMscModel::InitialiseModelCache()
{
  ...
  if (numOfCouples != msc.size())
  {
    msc.resize(numOfCouples, nullptr);
  }
