...
G4NuclearStopping::G4NuclearStopping(const G4String& processName) : G4VEmProcess(processName)
{
  isInitialized = false;
...
G4double G4NuclearStopping::AlongStepGetPhysicalInteractionLength(const G4Track&, G4double,
                                                                  G4double, G4double&,
...
G4VParticleChange* G4NuclearStopping::AlongStepDoIt(const G4Track& track, const G4Step& step)
{
  ...
  if (T2 > 0.0 && T2 * massR < Z * Z * MeV)
  {
    ...
    if (length > 0.0)
    {
      ...
      {
        G4double nloss = length * mod->ComputeDEDXPerVolume(couple->GetMaterial(), part, T);
