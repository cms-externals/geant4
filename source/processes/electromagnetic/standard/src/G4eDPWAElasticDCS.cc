...
const G4double G4eDPWAElasticDCS::gXGL[] = {1.98550718E-02, 1.01666761E-01, 2.37233795E-01,
                                            4.08282679E-01, 5.91717321E-01, 7.62766205E-01,
                                            8.98333239E-01, 9.80144928E-01};
const G4double G4eDPWAElasticDCS::gWGL[] = {5.06142681E-02, 1.11190517E-01, 1.56853323E-01,
...
void G4eDPWAElasticDCS::LoadGrid()
{
  ...
  if (!infile.is_open())
  {
    G4String msg = "    Problem while trying to read " + fname + " file.\n"
                   + "    G4LEDATA version should be G4EMLOW7.12 or later.\n";
    G4Exception("G4eDPWAElasticDCS::ReadCompressedFile", "em0006", FatalException, msg.c_str());
    return;
  }
  // read size
  infile >> gNumEnergies;
...
void G4eDPWAElasticDCS::ComputeCSPerAtom(G4int iz, G4double ekin, G4double& elcs, G4double& tr1cs,
                                         G4double& tr2cs, G4double mumin, G4double mumax)
{
  ...
  const G4double lekin =
    std::max(gTheEnergies[0], std::min(gTheEnergies[gNumEnergies - 1], G4Log(ekin)));
  ...
  const std::size_t iMuStart =
    (mumin == 0.0) ? 0
                   : std::distance(theMuVector.begin(),
                                   std::upper_bound(theMuVector.begin(), theMuVector.end(), mumin))
                       - 1;
  // 0.0 < mumax <= 1.0 for sure here
  const std::size_t iMuEnd =
    (mumax == 1.0) ? theMuVector.size() - 2
                   : std::distance(theMuVector.begin(),
                                   std::upper_bound(theMuVector.begin(), theMuVector.end(), mumax))
                       - 1;
...
G4double G4eDPWAElasticDCS::SampleCosineTheta(std::size_t iz, G4double lekin, G4double r1,
...
G4double G4eDPWAElasticDCS::SampleCosineThetaRestricted(std::size_t iz, G4double lekin, G4double r1,
                                                        G4double r2, G4double costMax,
                                                        G4double costMin)
{
...
G4double G4eDPWAElasticDCS::SampleMu(std::size_t izet, std::size_t ie, G4double r1, G4double r2)
{
...
G4double G4eDPWAElasticDCS::FindCumValue(G4double u, const OneSamplingTable& stable,
                                         const std::vector<G4double>& uvect)
{
  const std::size_t iLow =
    std::distance(uvect.begin(), std::upper_bound(uvect.begin(), uvect.end(), u)) - 1;
  const G4double tau =
    (u - uvect[iLow])
    / (uvect[iLow + 1] - uvect[iLow]);  // Note: I could store 1/(fX[iLow+1]-fX[iLow])
  ...
  return std::min(
    stable.fCum[iLow + 1],
    std::max(stable.fCum[iLow],
             stable.fCum[iLow] + dum0 * dum2 * (stable.fCum[iLow + 1] - stable.fCum[iLow]) / dum1));
}

// muMin and muMax : no checks on these
G4double G4eDPWAElasticDCS::SampleMu(std::size_t izet, std::size_t ie, G4double r1, G4double muMin,
                                     G4double muMax)
{
  ...
  //
  const std::vector<G4double>& theUVect = (fIsElectron && ie < gIndxEnergyLim) ? gTheU1 : gTheU2;
  const G4double xiMin =
    (muMin > 0.0) ? FindCumValue((theA + 1.0) * muMin / (theA + muMin), rtn, theUVect) : 0.0;
  const G4double xiMax =
    (muMax < 1.0) ? FindCumValue((theA + 1.0) * muMax / (theA + muMax), rtn, theUVect) : 1.0;
  //
  const G4double xi = xiMin + r1 * (xiMax - xiMin);  // a smaple within the range
  const std::size_t iLow =
    std::distance(rtn.fCum.begin(), std::upper_bound(rtn.fCum.begin(), rtn.fCum.end(), xi)) - 1;
...
void G4eDPWAElasticDCS::ReadCompressedFile(G4String fname, std::istringstream& iss)
{
  ...
  {
    G4String msg = "    Problem while trying to read " + fname + " data file.\n"
                   + "    G4LEDATA version should be G4EMLOW7.12 or later.\n";
    G4Exception("G4eDPWAElasticDCS::ReadCompressedFile", "em0006", FatalException, msg.c_str());
    return;
  }
                                                             ...
                                                             G4double ekin)
{
  ...
  {
    corFactor =
      fSCPCPerMatCuts[imc]->fVSCPC[lindx]
      + remaining * (fSCPCPerMatCuts[imc]->fVSCPC[lindx + 1] - fSCPCPerMatCuts[imc]->fVSCPC[lindx]);
  }
  return corFactor;
}

void G4eDPWAElasticDCS::InitSCPCorrection(G4double lowEnergyLimit, G4double highEnergyLimit)
{
  ...
  for (G4int imc = 0; imc < (G4int)numMatCuts; ++imc)
  {
    ...
    const G4double ecut =
      (*(thePCTable->GetEnergyCutsVector(idxG4ElectronCut)))[matCut->GetIndex()];
...
void G4eDPWAElasticDCS::ComputeMParams(const G4Material* mat, G4double& theBc, G4double& theXc2)
{
