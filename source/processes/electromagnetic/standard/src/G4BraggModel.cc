...
G4BraggModel::G4BraggModel(const G4ParticleDefinition* p, const G4String& nam) : G4VEmModel(nam)
{
...
void G4BraggModel::Initialise(const G4ParticleDefinition* p, const G4DataVector&)
{
  ...
  if (nullptr == fParticleChange)
  {
    ...
    isIon = G4EmTableUtil::IsIon(particle);
...
G4double G4BraggModel::GetChargeSquareRatio(const G4ParticleDefinition* p, const G4Material* mat,
                                            G4double kinEnergy)
{
...
G4double G4BraggModel::GetParticleCharge(const G4ParticleDefinition* p, const G4Material* mat,
                                                      ...
                                                      G4double maxKinEnergy)
{
  ...
  if (cutEnergy < maxEnergy)
  {
    ...
    if (0.0 < spin)
    {
      cross += 0.5 * (maxEnergy - cutEnergy) / energy2;
    }
...
G4double G4BraggModel::CrossSectionPerVolume(const G4Material* material,
                                             ...
                                             G4double cutEnergy, G4double maxEnergy)
{
  GetChargeSquareRatio(p, material, kineticEnergy);
...
G4double G4BraggModel::ComputeDEDXPerVolume(const G4Material* material,
                                            const G4ParticleDefinition* p, G4double kinEnergy,
                                            G4double cut)
{
  const G4double tmax = MaxSecondaryEnergy(p, kinEnergy);
  const G4double tkin = kinEnergy / massRate;
  const G4double cutEnergy = std::max(cut, lowestKinEnergy * massRate);
  GetChargeSquareRatio(p, material, kinEnergy);
  G4double dedx = 0.0;
                                     ...
                                     G4double maxEnergy)
{
  ...
  do
  {
    ...
    if (f > grej)
    {
      G4cout << "G4BraggModel::SampleSecondary Warning! "
             << "Majorant " << grej << " < " << f << " for e= " << deltaKinEnergy << G4endl;
    }
  ...
  if (UseAngularGeneratorFlag())
  {
    ...
    deltaDirection = GetAngularDistribution()->SampleDirection(dp, deltaKinEnergy, Z, mat);
  }
  else
  {
    G4double deltaMomentum = std::sqrt(deltaKinEnergy * (deltaKinEnergy + 2.0 * electron_mass_c2));
    G4double cost =
      deltaKinEnergy * (energy + electron_mass_c2) / (deltaMomentum * dp->GetTotalMomentum());
    if (cost > 1.0)
    {
      cost = 1.0;
    }
...
void G4BraggModel::HasMaterial(const G4Material* mat)
{
  ...
  static const G4String molName[numberOfMolecula] = {"Al_2O_3",
                                                     "CO_2",
                                                     "CH_4",
...
G4double G4BraggModel::StoppingPower(const G4Material* material, G4double kineticEnergy)
{
  ...
  if (iMolecula >= 0)
  {
    ...
    static const G4float a[11][5] = {{1.187E+1f, 1.343E+1f, 1.069E+4f, 7.723E+2f, 2.153E-2f},
    ...
    static const G4float atomicWeight[11] = {101.96128f, 44.0098f,  16.0426f, 28.0536f,
                                             42.0804f,   104.1512f, 44.665f,  60.0843f,
                                             18.0152f,   18.0152f,  12.0f};
...
G4double G4BraggModel::DEDX(const G4Material* material, G4double kineticEnergy)
{
  ...
  if (material != currentMaterial)
  {
    ...
    baseMaterial = material->GetBaseMaterial() ? material->GetBaseMaterial() : material;
    iPSTAR = -1;
    iMolecula = -1;
  ...
  if (iICRU90 >= 0)
  {
    return fICRU90->GetElectronicDEDXforProton(iICRU90, kineticEnergy) * material->GetDensity();
  }
  // PSTAR parameterisation
  if (iPSTAR >= 0)
  {
    return fPSTAR->GetElectronicDEDX(iPSTAR, kineticEnergy) * material->GetDensity();
  }
  ...
  if (iMolecula >= 0)
  {
    eloss = StoppingPower(baseMaterial, kineticEnergy) * material->GetDensity() / amu;
  ...
  {
    G4double z = material->GetZ();
    eloss = ElectronicStoppingPower(z, kineticEnergy) * (material->GetTotNbOfAtomsPerVolume());
  ...
  else if (MolecIsInZiegler1988(material))
  {
    ...
    for (std::size_t i = 0; i < numberOfElements; ++i)
    {
      ...
      eloss125 += ElectronicStoppingPower(z, 125.0 * keV) * theAtomicNumDensityVector[i];
    }
  ...
  else
  {
    ...
    for (std::size_t i = 0; i < numberOfElements; ++i)
    {
      const G4Element* element = (*theElementVector)[i];
      eloss +=
        ElectronicStoppingPower(element->GetZ(), kineticEnergy) * theAtomicNumDensityVector[i];
    }
  }
...
G4bool G4BraggModel::MolecIsInZiegler1988(const G4Material* material)
{
  ...
  static const G4String nameOfMol[numberOfMolecula] = {"H_2O",
                                                       "C_2H_4O",
                                                       "C_3H_6O",
                                                       "C_2H_2",
                                                       "CH_3OH",
  ...
  static const G4float expStopping[numberOfMolecula] = {
    66.1f,  190.4f, 258.7f, 42.2f,  141.5f, 210.9f, 279.6f, 198.8f, 31.0f,  267.5f, 122.8f,
    311.4f, 260.3f, 328.9f, 391.3f, 206.6f, 374.0f, 422.0f, 432.0f, 398.0f, 554.0f, 353.0f,
    326.0f, 74.6f,  220.5f, 197.4f, 362.0f, 170.0f, 330.5f, 211.3f, 262.3f, 349.6f, 51.3f,
    187.0f, 236.9f, 121.9f, 35.8f,  247.0f, 292.6f, 268.0f, 262.3f, 49.0f,  398.9f, 444.0f,
    22.91f, 68.0f,  155.0f, 84.0f,  74.2f,  254.7f, 306.8f, 324.4f, 420.0f};

  static const G4float expCharge[numberOfMolecula] = {
    HeEff, HeEff, HeEff, 1.0f,  HeEff, HeEff, HeEff, HeEff, 1.0f,  1.0f,  1.0f,
    HeEff, HeEff, HeEff, HeEff, HeEff, HeEff, HeEff, HeEff, HeEff, HeEff, HeEff,
    HeEff, 1.0f,  HeEff, HeEff, HeEff, HeEff, HeEff, HeEff, HeEff, HeEff, 1.0f,
    HeEff, HeEff, HeEff, 1.0f,  HeEff, HeEff, HeEff, HeEff, 1.0f,  HeEff, HeEff,
    1.0f,  1.0f,  1.0f,  1.0f,  1.0f,  HeEff, HeEff, HeEff, HeEff};

  static const G4int numberOfAtomsPerMolecula[numberOfMolecula] = {
    3,  7,  10, 4,  6,  9, 12, 7,  4,  24, 12, 14, 10, 13, 5,  5, 14, 18,
    17, 17, 24, 15, 13, 9, 8,  6,  14, 8,  8,  9,  10, 15, 6,  7, 7,  3,
    5,  5,  5,  5,  9,  3, 16, 14, 3,  9,  16, 11, 9,  10, 10, 9, 15};
  ...
  for (std::size_t i = 0; i < numberOfMolecula; ++i)
  {
    ...
    {
      expStopPower125 = ((G4double)expStopping[i]) * (material->GetTotNbOfAtomsPerVolume())
...
G4double G4BraggModel::ChemicalFactor(G4double kineticEnergy, G4double eloss125) const
{
  ...
  G4double factor =
    1.0 + (expStopPower125 / eloss125 - 1.0) * f12525 / (1.0 + G4Exp(1.48 * (beta / beta25 - 7.0)));
