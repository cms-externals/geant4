                              ...
                              const G4DataVector&)
{
  ...
  if(IsMaster()) {
    ...
    if(G4EmParameters::Instance()->UseICRU90Data() && nullptr == fICRU90) { 
...
{
  const G4double tmax = MaxSecondaryEnergy(p, kineticEnergy);
  const G4double tmin = std::max(std::min(cut, tmax), lowestKinEnergy);
  G4double dedx  = 0.0;

  if(kineticEnergy < lowestKinEnergy) {
    dedx = DEDX(material, lowestKinEnergy)*std::sqrt(kineticEnergy/lowestKinEnergy);
  } else {
