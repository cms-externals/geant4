...
const G4double G4PairProductionRelModel::gXGL[] = {1.98550718e-02, 1.01666761e-01, 2.37233795e-01,
                                                   4.08282679e-01, 5.91717321e-01, 7.62766205e-01,
                                                   8.98333239e-01, 9.80144928e-01};
const G4double G4PairProductionRelModel::gWGL[] = {5.06142681e-02, 1.11190517e-01, 1.56853323e-01,
...
const G4double G4PairProductionRelModel::gFelLowZet[] = {0.0,    5.3104, 4.7935, 4.7402,
                                                         4.7112, 4.6694, 4.6134, 4.5520};
const G4double G4PairProductionRelModel::gFinelLowZet[] = {0.0,    5.9173, 5.6125, 5.5377,
...
// CTR
G4PairProductionRelModel::G4PairProductionRelModel(const G4ParticleDefinition*, const G4String& nam)
  : G4VEmModel(nam),
    fIsUseLPMCorrection(true),
    fIsUseCompleteScreening(false),
    fLPMEnergy(0.),
    fG4Calc(G4Pow::GetInstance()),
    fTheGamma(G4Gamma::Gamma()),
    fTheElectron(G4Electron::Electron()),
    fThePositron(G4Positron::Positron()),
...
G4double G4PairProductionRelModel::ComputeDXSectionPerAtom(G4double pEnergy, G4double gammaEnergy,
                                                           G4double Z)
{
  G4double xSection = 0.;
  const G4int iz = std::min(gMaxZet, G4lrint(Z));
...
void G4PairProductionRelModel::SetupForMaterial(const G4ParticleDefinition*, const G4Material* mat,
                                                G4double)
{
...
//          However triplet prodution is not generated.
{
  ...
  else
  {
    ...
    if (gammaEnergy > fCoulombCorrectionThreshold)
    {  // Eg > 50 MeV ?
    ...
    do
    {
      ...
      else
      {
        ...
        if (isLPM)
        {
          ...
          greject =
            lpmXiS
            * ((lpmPhiS + 0.5 * lpmGS) * phi1 + 0.5 * lpmGS * phi2 - 0.5 * (lpmGS + lpmPhiS) * FZ)
            / F20;
        }
        else
        {
  ...
  //
  GetAngularDistribution()->SamplePairDirections(aDynamicGamma, eKinEnergy, pKinEnergy, eDirection,
                                                 pDirection);
...
void G4PairProductionRelModel::InitialiseElementData()
{
  ...
  for (auto const& elem : *elemTable)
  {
    ...
    {  // create it if doesn't exist yet
      ...
      G4double Finel;
      if (iz < 5)
      {  // use data from Dirac-Fock atomic model
        Fel = gFelLowZet[iz];
        Finel = gFinelLowZet[iz];
      }
      else
      {  // use the results of the Thomas-Fermi-Moliere model
...
void G4PairProductionRelModel::ComputeLPMfunctions(G4double& funcXiS, G4double& funcGS,
                                                   G4double& funcPhiS, const G4double eps,
                                                   const G4double egamma, const G4int izet)
{
  ...
  else if (varSprime > condition)
  {
    ...
    funcXiS =
      1.0 + funcHSprime - 0.08 * (1.0 - funcHSprime) * funcHSprime * (2.0 - funcHSprime) * dum;
  }
...
G4double G4PairProductionRelModel::ComputeParametrizedXSectionPerAtom(G4double gammaE, G4double Z)
{
  G4double xSection = 0.0;
  // short versions
  static const G4double kMC2 = CLHEP::electron_mass_c2;
  // zero cross section below the kinematical limit: Eg<2mc^2
  if (Z < 0.9 || gammaE <= 2.0 * kMC2)
  {
    return xSection;
  }
  //
  ...
  if (gammaE < gammaEnergyLimit)
  {
    gammaE = gammaEnergyLimit;
  }
