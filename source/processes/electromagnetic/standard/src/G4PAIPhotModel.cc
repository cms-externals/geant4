    ...
    for (size_t iReg = 0; iReg < numRegions; ++iReg)
    {
      ...
      for (size_t jMat = 0; jMat < numOfMat; ++jMat)
      {
        ...
        if (nullptr != cutCouple)
        {
          if (fVerbose > 1)
          {
            G4cout << "Reg <" << curReg->GetName() << ">  mat <" << mat->GetName()
                   << ">  fCouple= " << cutCouple << ", idx= " << cutCouple->GetIndex() << "  "
                   << p->GetParticleName() << ", cuts.size() = " << cuts.size() << G4endl;
          }
...
G4double G4PAIPhotModel::MinEnergyCut(const G4ParticleDefinition*, const G4MaterialCutsCouple*)
{
  return fLowestTcut;
}
...
G4double G4PAIPhotModel::ComputeDEDXPerVolume(const G4Material*, const G4ParticleDefinition* p,
...
G4double G4PAIPhotModel::CrossSectionPerVolume(const G4Material*, const G4ParticleDefinition* p,
                                               G4double kineticEnergy, G4double cutEnergy,
                                               G4double maxEnergy)
{
  G4int coupleIndex = FindCoupleIndex(CurrentCouple());
...
void G4PAIPhotModel::SampleSecondaries(std::vector<G4DynamicParticle*>* vdp,
                                       ...
                                       const G4DynamicParticle* dp, G4double tmin,
                                       G4double maxEnergy)
{
  ...
  if (G4UniformRand() <= plRatio)
  {
    ...
    // G4cout<<"G4PAIPhotModel::SampleSecondaries; dp
...
G4double G4PAIPhotModel::SampleFluctuations(const G4MaterialCutsCouple* matCC,
                                            const G4DynamicParticle* aParticle, const G4double,
                                            const G4double, const G4double step,
                                            const G4double eloss)
{
  ...
  G4double loss =
    fModelData->SampleAlongStepPhotonTransfer(coupleIndex, Tkin, scaledTkin, step * fChargeSquare);
  loss +=
    fModelData->SampleAlongStepPlasmonTransfer(coupleIndex, Tkin, scaledTkin, step * fChargeSquare);
...
G4double G4PAIPhotModel::Dispersion(const G4Material* material, const G4DynamicParticle* aParticle,
