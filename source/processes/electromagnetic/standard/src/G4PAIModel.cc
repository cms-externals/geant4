...
void G4PAIModel::Initialise(const G4ParticleDefinition* p, const G4DataVector& cuts)
{
  ...
  if (IsMaster())
  {
    ...
    if (fVerbose > 1)
    {
      G4cout << "G4PAIModel instantiates data for  " << p->GetParticleName() << G4endl;
    }
    ...
    for (std::size_t iReg = 0; iReg < numRegions; ++iReg)
    {
      ...
      for (std::size_t jMat = 0; jMat < numOfMat; ++jMat)
      {
        ...
        if (nullptr != cutCouple)
        {
          if (fVerbose > 1)
          {
            G4cout << "Region <" << curReg->GetName() << ">  mat <" << mat->GetName()
                   << ">  CoupleIndex= " << cutCouple->GetIndex() << "  " << p->GetParticleName()
...
void G4PAIModel::InitialiseLocal(const G4ParticleDefinition*, G4VEmModel* masterModel)
{
  ...
  fMaterialCutsCoupleVector = static_cast<G4PAIModel*>(masterModel)->GetVectorOfCouples();
  SetElementSelectors(masterModel->GetElementSelectors());
}

//////////////////////////////////////////////////////////////////////////////

G4double G4PAIModel::MinEnergyCut(const G4ParticleDefinition*, const G4MaterialCutsCouple*)
{
  return fLowestTcut;
}
...
G4double G4PAIModel::ComputeDEDXPerVolume(const G4Material*, const G4ParticleDefinition* p,
...
G4double G4PAIModel::CrossSectionPerVolume(const G4Material*, const G4ParticleDefinition* p,
                                           G4double kineticEnergy, G4double cutEnergy,
                                           G4double maxEnergy)
{
  G4int coupleIndex = FindCoupleIndex(CurrentCouple());
  ...
  G4double scaledTkin = kineticEnergy * fRatio;
  G4double xs =
    fChargeSquare * fModelData->CrossSectionPerVolume(coupleIndex, scaledTkin, cutEnergy, tmax);
  return xs;
}
...
void G4PAIModel::SampleSecondaries(std::vector<G4DynamicParticle*>* vdp,
                                   const G4MaterialCutsCouple* matCC, const G4DynamicParticle* dp,
                                   G4double tmin, G4double maxEnergy)
{
  ...
  G4double deltaTkin = fModelData->SamplePostStepTransfer(coupleIndex, scaledTkin, tmin, tmax);
  ...
  {
    G4cout << "G4PAIModel::SampleSecondaries; deltaKIn = " << deltaTkin / keV << " keV "
           << " Escaled(MeV)= " << scaledTkin << G4endl;
    return;
  }
  if (deltaTkin <= 0.)
  {
  ...
  const G4Element* anElement =
    SelectTargetAtom(matCC, fParticle, kineticEnergy, dp->GetLogKineticEnergy());

  G4int Z = anElement->GetZasInt();

  auto deltaRay = new G4DynamicParticle(
    fElectron, GetAngularDistribution()->SampleDirection(dp, deltaTkin, Z, matCC->GetMaterial()),
    deltaTkin);
...
G4double G4PAIModel::SampleFluctuations(const G4MaterialCutsCouple* matCC,
                                        const G4DynamicParticle* aParticle, const G4double tcut,
                                        const G4double, const G4double step, const G4double eloss)
{
  G4int coupleIndex = FindCoupleIndex(matCC);
  if (0 > coupleIndex)
  {
  ...
  */
  ...
  G4double loss =
    fModelData->SampleAlongStepTransfer(coupleIndex, Tkin, scaledTkin, tcut, step * fChargeSquare);
...
G4double G4PAIModel::Dispersion(const G4Material* material, const G4DynamicParticle* aParticle,
                                const G4double tcut, const G4double tmax, const G4double step)
{
  ...
  G4double siga = (tmax / beta2 - 0.5 * tcut) * twopi_mc2_rcl2 * step * electronDensity * q * q;
...
G4double G4PAIModel::MaxSecondaryEnergy(const G4ParticleDefinition* p, G4double kinEnergy)
{
  ...
  else if (p != fPositron)
  {
    ...
    tmax =
      2.0 * electron_mass_c2 * (gamma * gamma - 1.) / (1. + 2.0 * gamma * ratio + ratio * ratio);
  }
  return tmax;
}
