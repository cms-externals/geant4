...
G4PolarizedAnnihilation::G4PolarizedAnnihilation(const G4String& name)
  : G4eplusAnnihilation(name), fAsymmetryTable(nullptr), fTransverseAsymmetryTable(nullptr)
{
...
//....oooOO0OOooo........oooOO0OOooo........oooOO0OOooo........oooOO0OOooo....
G4double G4PolarizedAnnihilation::GetMeanFreePath(const G4Track& track, G4double previousStepSize,
                                                  G4ForceCondition* condition)
{
  ...
  if (verboseLevel >= 2)
  {
    G4cout << "G4PolarizedAnnihilation::MeanFreePath:  " << mfp / mm << " mm " << G4endl;
  }
  return mfp;
}
                                                                       ...
                                                                       G4ForceCondition* condition)
{
  ...
  G4double x =
    G4VEmProcess::PostStepGetPhysicalInteractionLength(track, previousStepSize, condition);
  G4double x0 = x;
  ...
  if (nullptr != fAsymmetryTable && nullptr != fTransverseAsymmetryTable && x < DBL_MAX)
  {
    ...
    if (nLength > 0.0)
    {
      theNumberOfInteractionLengthLeft = std::max(nLength - previousStepSize / prvLength, 0.0);
    }
    x = theNumberOfInteractionLengthLeft * curLength;
  }
  if (verboseLevel >= 2)
  {
    G4cout << "G4PolarizedAnnihilation::PostStepGPIL: " << std::setprecision(8) << x / mm << " mm;"
           << G4endl << "                         unpolarized value: " << std::setprecision(8)
           << x0 / mm << " mm." << G4endl;
  }
  return x;
}
...
G4double G4PolarizedAnnihilation::ComputeSaturationFactor(const G4Track& track)
{
  ...
  G4StokesVector electronPolarization = polarizationManager->GetVolumePolarization(aLVolume);
  ...
  if (volumeIsPolarized)
  {
    ...
    const G4StokesVector positronPolarization = G4StokesVector(track.GetPolarization());
    ...
    if (aVector && bVector)
    {
      ...
      G4double polZZ = positronPolarization.z() * (electronPolarization * positronDirection0);
      G4double polXX =
    ...
    else
    {
      ...
      G4Exception("G4PolarizedAnnihilation::ComputeSaturationFactor", "em0048", JustWarning, ed,
                  "");
    }
  }
  return factor;
...
void G4PolarizedAnnihilation::BuildAsymmetryTables(const G4ParticleDefinition& part)
{
  ...
  fTransverseAsymmetryTable = G4PhysicsTableHelper::PreparePhysicsTable(fTransverseAsymmetryTable);
  ...
  for (G4int i = 0; i < numOfCouples; ++i)
  {
    if (fAsymmetryTable->GetFlag(i))
    {
      ...
      G4PhysicsTableHelper::SetPhysicsVector(fTransverseAsymmetryTable, i, tVector);
    }
  }
}
...
G4double G4PolarizedAnnihilation::ComputeAsymmetry(G4double energy,
                                                   ...
                                                   G4double cut, G4double& tAsymmetry)
{
  ...
  G4double sigma2 = fEmModel->CrossSection(couple, &aParticle, energy, cut, energy);

  // calculate transversely polarized cross section
  targetPolarization = G4ThreeVector(1., 0., 0.);
  fEmModel->SetTargetPolarization(targetPolarization);
  fEmModel->SetBeamPolarization(targetPolarization);
  G4double sigma3 = fEmModel->CrossSection(couple, &aParticle, energy, cut, energy);

  // calculate unpolarized cross section
  targetPolarization = G4ThreeVector();
  fEmModel->SetTargetPolarization(targetPolarization);
  fEmModel->SetBeamPolarization(targetPolarization);
  G4double sigma0 = fEmModel->CrossSection(couple, &aParticle, energy, cut, energy);
