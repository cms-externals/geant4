...
G4PolarizedComptonModel::G4PolarizedComptonModel(const G4ParticleDefinition*, const G4String& nam)
  : G4KleinNishinaCompton(nullptr, nam), fVerboseLevel(0)
{
...
//....oooOO0OOooo........oooOO0OOooo........oooOO0OOooo........oooOO0OOooo....
G4double G4PolarizedComptonModel::ComputeAsymmetryPerAtom(G4double gammaEnergy, G4double /*Z*/)
{
  ...
  asymmetry /=
    ((k0 - 2.) * k0 - 2.) * sqr(k1) * G4Log(k1) + 2. * k0 * (k0 * (k0 + 1.) * (k0 + 8.) + 2.);
  ...
  if (asymmetry > 1.)
  {
    ...
    ed << "ERROR in G4PolarizedComptonModel::ComputeAsymmetryPerAtom.\n"
       << " asymmetry = " << asymmetry << "\n";
    G4Exception("G4PolarizedComptonModel::ComputeAsymmetryPerAtom", "pol035", JustWarning, ed);
  }
...
G4double G4PolarizedComptonModel::ComputeCrossSectionPerAtom(const G4ParticleDefinition* pd,
                                                             ...
                                                             G4double emax)
{
  G4double xs = G4KleinNishinaCompton::ComputeCrossSectionPerAtom(pd, kinEnergy, Z, A, cut, emax);
                                                ...
                                                G4double)
{
  ...
  do
  {
    do
    {
      ...
      if (nloop > fLoopLim)
      {
        PrintWarning(aDynamicGamma, nloop, greject, onecost, Phi, "too many iterations");
        return;
      }
      ...
      G4double gdist =
        1. / epsilon + epsilon - sint2 - polarization * (1. / epsilon - epsilon) * (1. - onecost);

      greject = gdist / gdiced;

      if (greject > 1.0)
      {
        PrintWarning(aDynamicGamma, nloop, greject, onecost, Phi, "theta majoranta wrong");
      }
    ...
    do
    {
      ...
      G4double gdiced =
        ...
        + std::abs(fBeamPolarization.p3())
            * (std::abs((1. / epsilon - epsilon) * cosTeta * fTargetPolarization.p3())
               + (1. - epsilon) * sinTeta
                   * (std::sqrt(sqr(fTargetPolarization.p1()) + sqr(fTargetPolarization.p2()))))
        + sint2 * (std::sqrt(sqr(fBeamPolarization.p1()) + sqr(fBeamPolarization.p2())));
      ...
      G4double gdist = 1. / epsilon + epsilon - sint2
                       ...
                       - sint2
                           * (std::cos(2. * Phi) * fBeamPolarization.p1()
                              + std::sin(2. * Phi) * fBeamPolarization.p2());
      greject = gdist / gdiced;

      if (greject > 1.0)
      {
        PrintWarning(aDynamicGamma, nloop, greject, onecost, Phi, "phi majoranta wrong");
      }

      if (greject < 1.e-3)
      {
        PrintWarning(aDynamicGamma, nloop, greject, onecost, Phi, "phi loop ineffective");
        // restart theta loop
        end = false;
        break;
      }
  ...
  G4ThreeVector nInteractionFrame = G4PolarizationHelper::GetFrame(gamDirection1, gamDirection0);
  ...
  fCrossSectionCalculator->Initialize(epsilon, E0_m, 0., fBeamPolarization, fTargetPolarization, 2);
  ...
  if (gamEnergy1 > lowestSecondaryEnergy)
  {
    ...
    if (fFinalGammaPolarization.mag() > 1. + 1.e-8)
    {
      ...
      ed << fFinalGammaPolarization << " mag = " << fFinalGammaPolarization.mag() << "\n";
      G4Exception("G4PolarizedComptonModel::SampleSecondaries", "pol033", FatalException, ed);
    }
  ...
  if (eKinEnergy > lowestSecondaryEnergy)
  {
    ...
    if (fVerboseLevel >= 1)
    {
      G4cout << " electronPolarization1 = " << finalElectronPolarization << G4endl;
    }
    ...
    {
      G4cout << " electronPolarization1 = " << finalElectronPolarization << G4endl
             << " ElecDirection = " << eDirection << G4endl;
    }
    ...
    if (finalElectronPolarization.mag() > 1. + 1.e-8)
    {
      ...
      ed << finalElectronPolarization << " mag = " << finalElectronPolarization.mag() << G4endl;
      G4Exception("G4PolarizedComptonModel::SampleSecondaries", "pol034", FatalException, ed);
    }
    aElectron->SetPolarization(finalElectronPolarization.p1(), finalElectronPolarization.p2(),
...
//....oooOO0OOooo........oooOO0OOooo........oooOO0OOooo........oooOO0OOooo......
void G4PolarizedComptonModel::PrintWarning(const G4DynamicParticle* dp, G4int nloop, G4double grej,
                                           G4double onecos, G4double phi, const G4String sss) const
{
  G4ExceptionDescription ed;
  ed << "Problem of scattering sampling: " << sss << "\n"
     << "Niter= " << nloop << " grej= " << grej << " cos(theta)= " << 1.0 - onecos
     << " phi= " << phi << "\n"
     << "Gamma E(MeV)= " << dp->GetKineticEnergy() / MeV << " dir= " << dp->GetMomentumDirection()
     << " pol= " << dp->GetPolarization();
  G4Exception("G4PolarizedComptonModel::SampleSecondaries", "em0044", JustWarning, ed, "");
}
