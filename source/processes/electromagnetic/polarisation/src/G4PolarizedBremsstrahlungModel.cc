...
G4PolarizedBremsstrahlungModel::G4PolarizedBremsstrahlungModel(const G4ParticleDefinition* p,
                                                               const G4String& nam)
  : G4SeltzerBergerModel(p, nam), fCrossSectionCalculator(nullptr)
{}

//....oooOO0OOooo........oooOO0OOooo........oooOO0OOooo........oooOO0OOooo....
G4PolarizedBremsstrahlungModel::~G4PolarizedBremsstrahlungModel()
{
  if (fCrossSectionCalculator) delete fCrossSectionCalculator;
}
                                                ...
                                                const G4DataVector& d)
{
  ...
  if (!fCrossSectionCalculator) fCrossSectionCalculator = new G4PolarizedBremsstrahlungXS();
}
                                                       ...
                                                       G4double maxEnergy)
{
  ...
  if (num > 0)
  {
    ...
    G4ThreeVector nInteractionFrame = G4PolarizationHelper::GetFrame(
      dp->GetMomentumDirection(), fParticleChange->GetProposedMomentumDirection());
    ...
    fCrossSectionCalculator->SetMaterial(GetCurrentElement()->GetN(),  // number of nucleons
                                         GetCurrentElement()->GetZ(),
                                         GetCurrentElement()->GetfCoulomb());
    fCrossSectionCalculator->Initialize(lepEnergy0, gamEnergy1, sintheta, beamPol,
                                        G4StokesVector::ZERO);

    // determine final state polarization
    G4StokesVector newBeamPol = fCrossSectionCalculator->GetPol2();
    newBeamPol.RotateAz(nInteractionFrame, fParticleChange->GetProposedMomentumDirection());
    ...
    if (num != 1)
    {
      ...
      G4Exception("G4PolarizedBremsstrahlungModel::SampleSecondaries", "pol001", JustWarning, ed);
    }
    for (std::size_t i = 0; i < num; ++i)
    {
      ...
      (*vdp)[i]->SetPolarization(photonPol.p1(), photonPol.p2(), photonPol.p3());
    }
  }
  return;
}
