...
//....oooOO0OOooo........oooOO0OOooo........oooOO0OOooo........oooOO0OOooo....
G4double G4PolarizedIonisation::MinPrimaryEnergy(const G4ParticleDefinition*, const G4Material*,
                                                 G4double cut)
{
  G4double x = cut;
  if (fIsElectron)
...
//....oooOO0OOooo........oooOO0OOooo........oooOO0OOooo........oooOO0OOooo....
G4double G4PolarizedIonisation::GetMeanFreePath(const G4Track& track, G4double step,
                                                G4ForceCondition* cond)
{
  ...
  if (verboseLevel >= 2)
  {
    G4cout << "G4PolarizedIonisation::MeanFreePath:  " << mfp / mm << " mm " << G4endl;
  }
  return mfp;
}
                                                                     ...
                                                                     G4ForceCondition* cond)
{
  ...
  if (fAsymmetryTable && fTransverseAsymmetryTable && x < DBL_MAX)
  {
    ...
    if (nLength > 0.0)
    {
      theNumberOfInteractionLengthLeft = std::max(nLength - step / prvLength, 0.0);
    }
    x = theNumberOfInteractionLengthLeft * curLength;
  }
  if (verboseLevel >= 2)
  {
    G4cout << "G4PolarizedIonisation::PostStepGPIL: " << std::setprecision(8) << x / mm << " mm;"
           << G4endl << "                   unpolarized value: " << std::setprecision(8) << x0 / mm
           << " mm." << G4endl;
  }
  return x;
}
...
G4double G4PolarizedIonisation::ComputeSaturationFactor(const G4Track& track)
{
  ...
  G4StokesVector volPolarization = polarizationManager->GetVolumePolarization(aLVolume);
  ...
  if (volumeIsPolarized && !volPolarization.IsZero())
  {
    ...
    if (aVector && bVector)
    {
      ...
      G4double polXX =
        polarization.x() * (volPolarization * G4PolarizationHelper::GetParticleFrameX(direction0));
      G4double polYY =
        polarization.y() * (volPolarization * G4PolarizationHelper::GetParticleFrameY(direction0));
    ...
    else
    {
      ...
      G4Exception("G4PolarizedIonisation::ComputeSaturationFactor", "em0048", JustWarning, ed, "");
    }
  }
  return factor;
}
                                                 ...
                                                 G4double cut, G4double& tAsymmetry)
{
  ...
  if (std::fabs(lAsymmetry) > 1.)
  {
    ...
    G4Exception("G4PolarizedIonisation::ComputeAsymmetry", "pol002", JustWarning, ed);
  }
  if (std::fabs(tAsymmetry) > 1.)
  {
    ...
    G4Exception("G4PolarizedIonisation::ComputeAsymmetry", "pol003", JustWarning, ed);
  }
  return lAsymmetry;
}
