...
//....oooOO0OOooo........oooOO0OOooo........oooOO0OOooo........oooOO0OOooo....
void G4PolarizedAnnihilationModel::ComputeAsymmetriesPerElectron(G4double ene, G4double& valueX,
                                                                 G4double& valueA, G4double& valueT)
{
  // *** calculate asymmetries
  G4double gam = 1. + ene / electron_mass_c2;
  G4double xs0 =
    fCrossSectionCalculator->TotalXSection(0., 1., gam, G4StokesVector::ZERO, G4StokesVector::ZERO);
  G4double xsA =
    fCrossSectionCalculator->TotalXSection(0., 1., gam, G4StokesVector::P3, G4StokesVector::P3);
  G4double xsT1 =
    fCrossSectionCalculator->TotalXSection(0., 1., gam, G4StokesVector::P1, G4StokesVector::P1);
  G4double xsT2 =
    fCrossSectionCalculator->TotalXSection(0., 1., gam, G4StokesVector::P2, G4StokesVector::P2);
  ...
  if ((valueA < -1) || (1 < valueA))
  {
    ...
    ed << " LONG: " << valueX << "\t" << valueA << "\t" << valueT << "   energy = " << gam
       << G4endl;
    G4Exception("G4PolarizedAnnihilationModel::ComputeAsymmetriesPerElectron", "pol004",
                JustWarning, ed);
  }
  if ((valueT < -1) || (1 < valueT))
  {
    ...
    ed << " TRAN: " << valueX << "\t" << valueA << "\t" << valueT << "   energy = " << gam
       << G4endl;
    G4Exception("G4PolarizedAnnihilationModel::ComputeAsymmetriesPerElectron", "pol005",
                JustWarning, ed);
  }
}
                                                     ...
                                                     G4double)
{
  ...
  if (fVerboseLevel >= 1)
  {
    G4cout << "G4PolarizedComptonModel::SampleSecondaries in " << aLVolume->GetName() << G4endl;
  }

  // transfer target electron polarization in frame of positron
  if (targetIsPolarized) fTargetPolarization.rotateUz(dp->GetMomentumDirection());
  ...
  fCrossSectionCalculator->Initialize(epsilmin, gama, 0., fBeamPolarization, fTargetPolarization);
  if (fCrossSectionCalculator->DiceEpsilon() < 0.)
  {
    G4ExceptionDescription ed;
    ed << "ERROR in PolarizedAnnihilationPS::PostStepDoIt\n"
       << "epsilmin DiceRoutine not appropriate ! " << fCrossSectionCalculator->DiceEpsilon()
       << G4endl;
    G4Exception("G4PolarizedAnnihilationModel::SampleSecondaries", "pol006", JustWarning, ed);
  }

  fCrossSectionCalculator->Initialize(epsilmax, gama, 0., fBeamPolarization, fTargetPolarization);
  if (fCrossSectionCalculator->DiceEpsilon() < 0)
  {
    G4ExceptionDescription ed;
    ed << "ERROR in PolarizedAnnihilationPS::PostStepDoIt\n"
       << "epsilmax DiceRoutine not appropriate ! " << fCrossSectionCalculator->DiceEpsilon()
       << G4endl;
    G4Exception("G4PolarizedAnnihilationModel::SampleSecondaries", "pol007", JustWarning, ed);
  }
  ...
  do
  {
    ...
    fCrossSectionCalculator->Initialize(epsil, gama, 0., fBeamPolarization, fTargetPolarization, 1);
    ...
    if (treject > gmax || treject < 0.)
    {
      ...
      G4Exception("G4PolarizedAnnihilationModel::SampleSecondaries", "pol008", JustWarning, ed);
    }
    ++ncount;
    ...
    if (ncount > 1000)
    {
      ...
      ed << "WARNING  in PolarizedAnnihilationPS::PostStepDoIt\n"
         << "eps dicing very inefficient =" << trejectmax / gmax << ", " << treject / gmax
         << ".  For secondary energy = " << epsil << "   " << ncount << G4endl;
      G4Exception("G4PolarizedAnnihilationModel::SampleSecondaries", "pol009", JustWarning, ed);
      break;
    }
  ...
  G4double phi = 0.;
  G4double beamTrans = std::sqrt(sqr(fBeamPolarization.p1()) + sqr(fBeamPolarization.p2()));
  G4double targetTrans = std::sqrt(sqr(fTargetPolarization.p1()) + sqr(fTargetPolarization.p2()));

  do
  {
    phi = twopi * G4UniformRand();
    fCrossSectionCalculator->Initialize(epsil, gama, 0., fBeamPolarization, fTargetPolarization, 2);

    G4double gdiced = fCrossSectionCalculator->getVar(0);
    gdiced +=
      fCrossSectionCalculator->getVar(3) * fBeamPolarization.p3() * fTargetPolarization.p3();
    gdiced += 1.
    ...
    gdist += fCrossSectionCalculator->getVar(3) * fBeamPolarization.p3() * fTargetPolarization.p3();
    gdist +=
      fCrossSectionCalculator->getVar(1)
      * (std::cos(phi) * fBeamPolarization.p1() + std::sin(phi) * fBeamPolarization.p2())
      * (std::cos(phi) * fTargetPolarization.p1() + std::sin(phi) * fTargetPolarization.p2());
    gdist +=
      fCrossSectionCalculator->getVar(2)
      * (std::cos(phi) * fBeamPolarization.p2() - std::sin(phi) * fBeamPolarization.p1())
      * (std::cos(phi) * fTargetPolarization.p2() - std::sin(phi) * fTargetPolarization.p1());
    gdist += fCrossSectionCalculator->getVar(4)
    ...
    if (treject < 1.e-3)
    {
      ...
      G4cout << " epsil = " << epsil << G4endl;
      G4Exception("G4PolarizedAnnihilationModel::SampleSecondaries", "pol010", JustWarning, ed);
    }
  ...
  G4ThreeVector nInteractionFrame = G4PolarizationHelper::GetFrame(PositDirection, Phot1Direction);
  ...
  fCrossSectionCalculator->Initialize(epsil, gama, phi, fBeamPolarization, fTargetPolarization, 2);
  ...
  {
    G4ExceptionDescription ed;
    ed << "ERROR: PolarizedAnnihilation Polarization Vector at epsil = " << epsil
       << " is too large!!! \n"
       << "annihi pol1= " << fFinalGamma1Polarization << ", (" << n1 << ")\n";
    fFinalGamma1Polarization *= 1. / std::sqrt(n1);
    G4Exception("G4PolarizedAnnihilationModel::SampleSecondaries", "pol011", JustWarning, ed);
  }
  ...
  aParticle1->SetPolarization(fFinalGamma1Polarization.p1(), fFinalGamma1Polarization.p2(),
  ...
  if (n2 > 1.)
  {
    ...
    G4Exception("G4PolarizedAnnihilationModel::SampleSecondaries", "pol012", JustWarning, ed);
  ...
  aParticle2->SetPolarization(fFinalGamma2Polarization.p1(), fFinalGamma2Polarization.p2(),
