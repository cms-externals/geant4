...
G4PolarizedIonisationMollerXS::G4PolarizedIonisationMollerXS() : fPhi0(0.)
{
  SetXmax(.5);
...
void G4PolarizedIonisationMollerXS::Initialize(G4double e, G4double gamma, G4double /*phi*/,
                                               ...
                                               const G4StokesVector& pol1, G4int flag)
{
  ...
  if (polarized)
  {
    ...
    {
      G4double xy = 0.;
      G4double xz = -((-1. + 2. * e) * gmo) / (2. * sqrttwo * gamma2 * std::sqrt(-((f * e) / gpo)));
      G4double yx = 0.;
      G4double yz = 0.;
      G4double zx = -((-1. + 2. * e) * gmo) / (2. * sqrttwo * gamma2 * std::sqrt(-((f * e) / gpo)));
      G4double zy = 0.;
  ...
  if (flag >= 1)
  {
    ...
    {
      G4double xxP1K1 =
        (std::sqrt(gpo / (1. + e2 * gmo + gamma - 2. * e * gamma)) * (gamma - e * gpo))
        / (4. * e2 * gamma);
      G4double xyP1K1 = 0.;
      G4double xzP1K1 =
        (-1. + 2. * e * gamma)
        / (2. * sqrttwo * f * gamma * std::sqrt(e * e2 * (1. + e + gamma - e * gamma)));
      G4double yxP1K1 = 0.;
      G4double yyP1K1 = (-gamma2 + e * (-1. + gamma * (2. + gamma))) / (4. * f * e2 * gamma2);
      G4double yzP1K1 = 0.;
      G4double zxP1K1 =
        (1. + 2. * e2 * gmo - 2. * e * gamma)
        / (2. * sqrttwo * f * e * gamma * std::sqrt(e * (1. + e + gamma - e * gamma)));
      G4double zyP1K1 = 0.;
      G4double zzP1K1 = (-gamma + e * (1. - 2. * e * gmo + gamma))
    ...
    {
      G4double xxP1K2 =
        ((1. + e * (-3. + gamma)) * std::sqrt(gpo / (1. + e2 * gmo + gamma - 2. * e * gamma)))
        / (4. * f * e * gamma);
      G4double xyP1K2 = 0.;
      G4double xzP1K2 = (-2. + 2. * e + gamma)
                        / (2. * sqrttwo * f2 * gamma * std::sqrt(e * (1. + e + gamma - e * gamma)));
      G4double yxP1K2 = 0.;
      G4double yyP1K2 =
        (1. - 2. * gamma + e * (-1. + gamma * (2. + gamma))) / (4. * f2 * e * gamma2);
      G4double yzP1K2 = 0.;
    ...
    {
      G4double xxP2K1 =
        (-1. + e + e * gamma) / (4. * f2 * gamma * std::sqrt((e * (2. + e * gmo)) / gpo));
      G4double xyP2K1 = 0.;
      ...
      G4double yyP2K1 =
        (1. - 2. * gamma + e * (-1. + gamma * (2. + gamma))) / (4. * f2 * e * gamma2);
      G4double yzP2K1 = 0.;
      G4double zxP2K1 = (1. + 2. * e * (-2. + e + gamma - e * gamma))
    ...
    if (!pol1.IsZero())
    {
      ...
      G4double zyP2K2 = 0.;
      G4double zzP2K2 =
        (std::sqrt((e * gpo) / (2. + e * gmo)) * (-2. + e * (3. + 2. * e * gmo - gamma) + gamma))
        / (4. * f * e2 * gamma);
...
G4double G4PolarizedIonisationMollerXS::TotalXSection(G4double xmin, G4double xmax, G4double gamma,
                                                      const G4StokesVector& pol0,
                                                      const G4StokesVector& pol1)
{
  ...
  if (xmax != 0.5)
  {
    ...
    G4Exception("G4PolarizedIonisationMollerXS::TotalXSection", "pol020", JustWarning, ed);
  }
