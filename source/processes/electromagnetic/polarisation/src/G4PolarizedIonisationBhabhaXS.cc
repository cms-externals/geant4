...
G4PolarizedIonisationBhabhaXS::G4PolarizedIonisationBhabhaXS() : fPhi0(1.)
{
  fPhi2 = G4ThreeVector();
  fPhi3 = G4ThreeVector();
}
...
void G4PolarizedIonisationBhabhaXS::Initialize(G4double e, G4double gamma, G4double /*phi*/,
                                               ...
                                               const G4StokesVector& pol1, G4int flag)
{
  ...
  {
    G4double xx =
      -((e * gmo - gamma) * (-1. - gamma + e * (e * gmo - gamma) * (3. + gamma))) / (4. * e * gpo3);
    G4double yy =
      (e3 * gmo3 - 2. * e2 * gmo2 * gamma - gpo * (1. + 2. * gamma) + e * (-2. + gamma2 + gamma3))
      / (4. * e * gpo3);
    G4double zz = ((e * gmo - gamma)
                   * (e2 * gmo * (3. + gamma) - e * gamma * (3. + gamma) + gpo * (1. + 2. * gamma)))
                  / (4. * e * gpo3);

    fPhi0 += xx * pol0.x() * pol1.x() + yy * pol0.y() * pol1.y() + zz * pol0.z() * pol1.z();
  ...
  if (flag >= 1)
  {
    ...
    {
      G4double xxPplKpl =
        -((-1. + e) * (e * gmo - gamma) * (-(gamma * gpo) + e * (-2. + gamma + gamma2)))
        / (4. * e2 * gpo
           * std::sqrt(gmo * gpo * (-1. + e + gamma - e * gamma) * (1. + e + gamma - e * gamma)));
      G4double xyPplKpl = 0.;
      G4double xzPplKpl =
      ...
      G4double yxPplKpl = 0.;
      G4double yyPplKpl =
        (gamma2 * gpo + e2 * gmo2 * (3. + gamma) - e * gmo * (1. + 2. * gamma * (2. + gamma)))
        / (4. * e2 * gmo * gpo2);
      G4double yzPplKpl = 0.;
      G4double zxPplKpl =
        ((e * gmo - gamma) * (1. + e * (-1. + 2. * e * gmo - 2. * gamma) * gmo + gamma))
      ...
      fPhi2[0] += xxPplKpl * pol0.x() + xyPplKpl * pol0.y() + xzPplKpl * pol0.z();
      fPhi2[1] += yxPplKpl * pol0.x() + yyPplKpl * pol0.y() + yzPplKpl * pol0.z();
      fPhi2[2] += zxPplKpl * pol0.x() + zyPplKpl * pol0.y() + zzPplKpl * pol0.z();
    }
    // initial electron Kmn
    if (!pol1.IsZero())
    {
      G4double xxPplKmn = ((-1. + e) * (e * (-2. + gamma) * gmo + gamma))
                          / (4. * e * gpo32 * std::sqrt(1. + e2 * gmo + gamma - 2. * e * gamma));
      G4double xyPplKmn = 0.;
      G4double xzPplKmn = (-1. + e * gmo + gmo * gamma)
                          / (2. * sqrttwo * gpo2 * std::sqrt(e * (1. + e + gamma - e * gamma)));
      G4double yxPplKmn = 0.;
      G4double yyPplKmn = (-1. - 2. * gamma + e * gmo * (3. + gamma)) / (4. * e * gpo2);
      G4double yzPplKmn = 0.;
      G4double zxPplKmn =
        (1. + 2. * e2 * gmo2 + gamma + gamma2 + e * (1. + (3. - 4. * gamma) * gamma))
        / (2. * sqrttwo * gpo2 * std::sqrt(e * (1. + e + gamma - e * gamma)));
      G4double zyPplKmn = 0.;
      G4double zzPplKmn =
        -(std::sqrt((1. - e) / (e - e * gamma2 + gpo2))
          * (2. * e2 * gmo2 + gamma + 2. * gamma2 + e * (2. + gamma - 3. * gamma2)))
        / (4. * e * gpo);

      fPhi2[0] += xxPplKmn * pol1.x() + xyPplKmn * pol1.y() + xzPplKmn * pol1.z();
      fPhi2[1] += yxPplKmn * pol1.x() + yyPplKmn * pol1.y() + yzPplKmn * pol1.z();
      fPhi2[2] += zxPplKmn * pol1.x() + zyPplKmn * pol1.y() + zzPplKmn * pol1.z();
    }
    // Final Electron Pmn
    // initial positron Kpl
    if (!pol0.IsZero())
    {
      G4double xxPmnKpl =
        ((-1. + e * gmo) * (2. + gamma)) / (4. * gpo * std::sqrt(e * (2. + e * gmo) * gpo));
      G4double xyPmnKpl = 0.;
      ...
      G4double zyPmnKpl = 0;
      G4double zzPmnKpl =
        (-2. + 2. * e2 * gmo2 + gamma * (-1. + 2. * gamma) + e * (-2. + (5. - 3. * gamma) * gamma))
        / (4. * std::sqrt(e * (2. + e * gmo)) * gpo32);

      fPhi3[0] += xxPmnKpl * pol0.x() + xyPmnKpl * pol0.y() + xzPmnKpl * pol0.z();
      fPhi3[1] += yxPmnKpl * pol0.x() + yyPmnKpl * pol0.y() + yzPmnKpl * pol0.z();
      fPhi3[2] += zxPmnKpl * pol0.x() + zyPmnKpl * pol0.y() + zzPmnKpl * pol0.z();
    }
    // initial electron Kmn
    if (!pol1.IsZero())
    {
      G4double xxPmnKmn =
        -((2. + e * gmo) * (-1. + e * gmo - gamma) * (e * gmo - gamma) * (-2. + gamma))
        / (4. * gmo * egmp32);
      G4double xyPmnKmn = 0.;
      G4double xzPmnKmn =
        ((e * gmo - gamma) * std::sqrt((-1. + e + gamma - e * gamma) / (2. + e * gmo))
      ...
      G4double yzPmnKmn = 0.;
      G4double zxPmnKmn =
        -((-1. + e) * (e * gmo - gamma) * (e * gmo + 2. * e2 * gmo2 - gamma * gpo))
      ...
      fPhi3[0] += xxPmnKmn * pol1.x() + xyPmnKmn * pol1.y() + xzPmnKmn * pol1.z();
      fPhi3[1] += yxPmnKmn * pol1.x() + yyPmnKmn * pol1.y() + yzPmnKmn * pol1.z();
      fPhi3[2] += zxPmnKmn * pol1.x() + zyPmnKmn * pol1.y() + zzPmnKmn * pol1.z();
    }
  }
  fPhi0 *= pref;
  fPhi2 *= pref;
...
G4double G4PolarizedIonisationBhabhaXS::TotalXSection(G4double xmin, G4double /*xmax*/,
                                                      G4double gamma, const G4StokesVector& pol0,
