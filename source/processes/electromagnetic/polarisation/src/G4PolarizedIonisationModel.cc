...
G4PolarizedIonisationModel::G4PolarizedIonisationModel(const G4ParticleDefinition* p,
                                                       const G4String& nam)
  : G4MollerBhabhaModel(p, nam), fCrossSectionCalculator(nullptr)
{
...
{
  G4double xs = G4MollerBhabhaModel::ComputeCrossSectionPerElectron(pd, kinEnergy, cut, emax);
  G4double factor = 1.;
  if (xs != 0.)
  {
    ...
    if (cut < tmax)
    {
      ...
      G4double crossPol = fCrossSectionCalculator->TotalXSection(xmin, xmax, gam, fBeamPolarization,
                                                                 fTargetPolarization);
                                                   ...
                                                   G4double maxEnergy)
{
  ...
  if (isElectron)
  {
    ...
    do
    {
      ...
      if (fCrossSectionCalculator)
      {
        fCrossSectionCalculator->Initialize(x, gam, phi, fBeamPolarization, fTargetPolarization, 1);
        xs = fCrossSectionCalculator->XSection(G4StokesVector::ZERO, G4StokesVector::ZERO);
        z = xs * sqr(x) * 4.;
        if (grej < z)
        {
          ...
          ed << "WARNING : error in Moller rejection routine! \n"
             << " z = " << z << " grej=" << grej << "\n";
          G4Exception("G4PolarizedIonisationModel::SampleSecondaries", "pol019", JustWarning, ed);
        }
      }
      else
      {
  ...
  else
  {
    ...
    grej += y * y * gmo * (3. * gamma2 + 6. * gam + 4.)
            * (1.
               + (polL * (3. * gam + 1.) * (gamma2 + gam + 1.) + polT * ((gam + 2.) * gamma2 + 1.))
                   / (gmo * (3. * gam * (gam + 2.) + 4.)));
    grej /= gpo3;
    grej += -xmin * (2. * gamma2 + 4. * gam + 1.)
            * (1. - gam * (polL * (2. * gam + 1.) + polT) / (2. * gam * (gam + 2.) + 1.)) / gpo2;
    ...
    do
    {
      ...
      if (z > grej)
      {
        ...
        G4Exception("G4PolarizedIonisationModel::SampleSecondaries", "pol020", JustWarning, ed);
      }
      // Loop checking, 03-Aug-2015, Vladimir Ivanchenko
    } while (grej * G4UniformRand() > z);
  }
      ...
      if (xs > grej)
      {
        if (isElectron)
        {
          ...
          ed << "Majorant " << grej << " < " << xs << " for phi= " << phi << "\n"
             ...
             << "PHI DICING\n";
          G4Exception("G4PolarizedIonisationModel::SampleSecondaries", "pol021", JustWarning, ed);
        }
        else
        {
          G4ExceptionDescription ed;
          ed << "Majorant " << grej << " < " << xs << " for phi= " << phi << "\n"
             << " e+e- (Bhabha) scattering\n"
             << "PHI DICING\n";
          G4Exception("G4PolarizedIonisationModel::SampleSecondaries", "pol022", JustWarning, ed);
        }
      }
      // Loop checking, 03-Aug-2015, Vladimir Ivanchenko
  ...
  G4ThreeVector nInteractionFrame = G4PolarizationHelper::GetFrame(direction, deltaDirection);
  ...
  if (fCrossSectionCalculator)
  {
    ...
    fCrossSectionCalculator->Initialize(x, gam, phi, fBeamPolarization, fTargetPolarization, 2);
