                                      ...
                                      G4int flag)
{
  ...
  fPolXS = -sinT2 * pol0.x() + (1. - eps) * sinT * polzx + ((eps2 - 1.) / eps) * cosT * polzz;
  fPolXS *= 0.5;
  ...
  if (flag == 2)
  {
    ...
    G4double phi23 =
      ((eps2 + 1.) / eps) * cosT * pol0.z() - ((1. - eps) / eps) * (eps * cosT2 + 1.) * pol1.z();
    ...
    G4double phi32 =
      -sinT2 * polxy + ((1. - eps) / eps) * sinT * polyz + 0.5 * (cos2T + 3.) * pol1.y();
    phi32 *= 0.5;
    ...
    else
    {
      ...
      phi31add =
        -(-X * X * pol1.z() - 2. * X * (2. * pol0.z() - pol1.z()) - (4. * pol0.x() + 5.) * pol1.z())
        * (1. - eps) / (4. * X);
...
//....oooOO0OOooo........oooOO0OOooo........oooOO0OOooo........oooOO0OOooo......
G4double G4PolarizedComptonXS::TotalXSection(G4double /*xmin*/, G4double /*xmax*/, G4double k0,
                                             const G4StokesVector& pol0, const G4StokesVector& pol1)
{
  G4double k1 = 1. + 2. * k0;

  G4double unit = fZ * CLHEP::pi * CLHEP::classic_electr_radius * CLHEP::classic_electr_radius;

  G4double pre = unit / (sqr(k0) * sqr(1. + 2. * k0));

  G4double xs_0 =
    ((k0 - 2.) * k0 - 2.) * sqr(k1) * std::log(k1) + 2. * k0 * (k0 * (k0 + 1.) * (k0 + 8.) + 2.);
