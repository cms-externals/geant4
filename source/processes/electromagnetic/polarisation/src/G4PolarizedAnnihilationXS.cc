...
//....oooOO0OOooo........oooOO0OOooo........oooOO0OOooo........oooOO0OOooo......
void G4PolarizedAnnihilationXS::Initialize(G4double eps, G4double gam,
                                           G4double,  // phi
                                           ...
                                           G4int flag)
{
  ...
  G4double symmXS =
    0.125
    * ((-1. / sqr(gam + 1.)) / sqr(eps) + ((sqr(gam) + 4. * gam - 1.) / sqr(gam + 1.)) / eps - 1.);
  ...
  helpVar1 = 1. / (gam - 1.);
  helpVar2 = 1. / sqr(gam + 1.);
  calcVector = G4ThreeVector(-(gam * gam + 1.) * helpVar2,
  ...
  if (flag == 2)
  {
    ...
    helpVar1 =
      8. * sqr(1. - eps) * sqr(eps) * (gam - 1.) * sqr(gam + 1.) / std::sqrt(gam * gam - 1.);
    ...
    fPhi2.setZ(circ2 * pol1.z() + circ1 * pol0.z() + circ3 * (pol1.x() + pol0.x()));
    fPhi3.setZ(-circ1 * pol1.z() - circ2 * pol0.z() - circ3 * (pol1.x() + pol0.x()));
...
//....oooOO0OOooo........oooOO0OOooo........oooOO0OOooo........oooOO0OOooo......
G4double G4PolarizedAnnihilationXS::TotalXSection(G4double, G4double, G4double gam,
...
{
  if (choice == -1) return fPolXS / fUnpXS;
  if (choice == 0) return fUnpXS;
