...
G4PolarizedPhotoElectricModel::G4PolarizedPhotoElectricModel(const G4ParticleDefinition*,
                                                             const G4String& nam)
  : G4PEEffectFluoModel(nam), fCrossSectionCalculator(nullptr), fVerboseLevel(0)
{}
                                               ...
                                               const G4DataVector& dv)
{
  ...
  if (!fCrossSectionCalculator) fCrossSectionCalculator = new G4PolarizedPhotoElectricXS();
}
                                                      ...
                                                      G4double maxEnergy)
{
  ...
  if (vdp && !vdp->empty())
  {
    ...
    G4ThreeVector nInteractionFrame =
      G4PolarizationHelper::GetFrame(dp->GetMomentumDirection(), (*vdp)[0]->GetMomentumDirection());
    ...
    fCrossSectionCalculator->Initialize(gamEnergy0, lepEnergy1, sintheta, beamPol,
                                        G4StokesVector::ZERO);
    ...
    if (vdp->size() != 1)
    {
      ...
      G4Exception("G4PolarizedPhotoElectricModel::SampleSecondaries", "pol024", JustWarning, ed);
    }
  }
}
