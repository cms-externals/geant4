...
G4EmUtility::FindRegion(const G4String& regionName, const G4int verbose)
{
  ...
  if(nullptr == reg && verbose > 0) {
    G4cout << "### G4EmUtility::FindRegion WARNING: fails to find a region <"
           << r << G4endl;
  } else if(verbose > 1) {
                                 ...
                                 const G4ParticleDefinition* part)
{
  ...
  for (G4int i=0; i<nn; ++i) {
    ...
    for (G4int j=0; j<=nbin; ++j) {
      G4double sig =
        p->GetCrossSection(e, theCoupleTable->GetMaterialCutsCouple(i));
      if (sig >= sm) {
        em = e;
        sm = sig;
        e = (j+1 < nbin) ? e*x : tmax;
...
G4EmUtility::FillPeaksStructure(G4PhysicsTable* p)
{
  ...
  if(!registry->GetBaseMaterialFlag()) { return ptr; }
