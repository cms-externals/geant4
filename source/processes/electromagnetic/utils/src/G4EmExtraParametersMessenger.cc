  ...
  : theParameters(ptr)
{
  ...
  mtype->SetParameterCandidates(
    "G4EmStandard G4EmStandard_opt1 G4EmStandard_opt2 G4EmStandard_opt3 G4EmStandard_opt4 "
    "G4EmStandardGS G4EmStandardSS G4EmLivermore G4EmPenelope G4RadioactiveDecay");
  ...
  dirSplitTargetCmd =
    new G4UIcmdWith3VectorAndUnit("/process/em/setDirectionalSplittingTarget", this);
  ...
  dirSplitRadiusCmd =
    new G4UIcmdWithADoubleAndUnit("/process/em/setDirectionalSplittingRadius", this);
...
void G4EmExtraParametersMessenger::SetNewValue(G4UIcommand* command, G4String newValue)
{
  ...
  else if (command == StepFuncCmd || command == StepFuncCmd1 || command == StepFuncCmd2
           || command == StepFuncCmd3)
  {
    G4double v1, v2;
    G4String unt;
    std::istringstream is(newValue);
  ...
  {
    theParameters->SetDirectionalSplitting(dirSplitCmd->GetNewBoolValue(newValue));
    physicsModified = true;
  }
