  ...
  : theParameters(ptr)
{
  ...
  dnamioniCmd = new G4UIcmdWithABool("/process/dna/UseDNAMultipleIonisation", this);
  dnamioniCmd->SetGuidance("Enable usage of multiple ionisation for DNA");
  dnamioniCmd->SetParameterName("dnamioni", true);
  dnamioniCmd->SetDefaultValue(false);
  dnamioniCmd->AvailableForStates(G4State_PreInit);
  dnamioniCmd->SetToBeBroadcasted(false);

  minEnECmd = new G4UIcmdWithADoubleAndUnit("/process/dna/electronMinEnergy", this);
  minEnECmd->SetGuidance("Set min electron energy limit for DNA");
  minEnECmd->SetParameterName("dnaMinE", true);
  minEnECmd->SetUnitCategory("Energy");
  minEnECmd->AvailableForStates(G4State_PreInit);
  minEnECmd->SetToBeBroadcasted(false);

  maxEnECmd = new G4UIcmdWithADoubleAndUnit("/process/dna/electronMaxEnergy", this);
  maxEnECmd->SetGuidance("Set max electron energy for DNA");
  maxEnECmd->SetParameterName("dnaMaxE", true);
  maxEnECmd->SetUnitCategory("Energy");
  maxEnECmd->AvailableForStates(G4State_PreInit);
  maxEnECmd->SetToBeBroadcasted(false);

  minEnPCmd = new G4UIcmdWithADoubleAndUnit("/process/dna/protonMinEnergy", this);
  minEnPCmd->SetGuidance("Set min proton energy limit for DNA");
  minEnPCmd->SetParameterName("dnaMinP", true);
  minEnPCmd->SetUnitCategory("Energy");
  minEnPCmd->AvailableForStates(G4State_PreInit);
  minEnPCmd->SetToBeBroadcasted(false);

  maxEnPCmd = new G4UIcmdWithADoubleAndUnit("/process/dna/protonMaxEnergy", this);
  maxEnPCmd->SetGuidance("Set max proton energy for DNA");
  maxEnPCmd->SetParameterName("dnaMaxP", true);
  maxEnPCmd->SetUnitCategory("Energy");
  maxEnPCmd->AvailableForStates(G4State_PreInit);
  maxEnPCmd->SetToBeBroadcasted(false);

  minEnICmd = new G4UIcmdWithADoubleAndUnit("/process/dna/ionMinEnergy", this);
  minEnICmd->SetGuidance("Set min ion energy limit for DNA");
  minEnICmd->SetParameterName("dnaMinI", true);
  minEnICmd->SetUnitCategory("Energy");
  minEnICmd->AvailableForStates(G4State_PreInit);
  minEnICmd->SetToBeBroadcasted(false);

  maxEnICmd = new G4UIcmdWithADoubleAndUnit("/process/dna/ionMaxEnergyPerNucleon", this);
  maxEnICmd->SetGuidance("Set max ion energy per nucleon for DNA");
  maxEnICmd->SetParameterName("dnaMaxI", true);
  maxEnICmd->SetUnitCategory("Energy");
  maxEnICmd->AvailableForStates(G4State_PreInit);
  maxEnICmd->SetToBeBroadcasted(false);
  ...
  type->SetParameterCandidates(
    "DNA_Opt0 DNA_Opt2 DNA_Opt4 DNA_Opt4a DNA_Opt6 DNA_Opt6a DNA_Opt7 DNA_Opt8");
...
void G4EmLowEParametersMessenger::SetNewValue(G4UIcommand* command, G4String newValue)
{
  ...
  else if (command == dnamioniCmd)
  {
    theParameters->SetDNAMultipleIonisation(dnamioniCmd->GetNewBoolValue(newValue));
  }
  else if (command == minEnECmd)
  {
    theParameters->SetMinDNAElectronEnergy(minEnECmd->GetNewDoubleValue(newValue));
  }
  else if (command == maxEnECmd)
  {
    theParameters->SetMaxDNAElectronEnergy(maxEnECmd->GetNewDoubleValue(newValue));
  }
  else if (command == minEnPCmd)
  {
    theParameters->SetMinDNAProtonEnergy(minEnPCmd->GetNewDoubleValue(newValue));
  }
  else if (command == maxEnPCmd)
  {
    theParameters->SetMaxDNAProtonEnergy(maxEnPCmd->GetNewDoubleValue(newValue));
  }
  else if (command == minEnICmd)
  {
    theParameters->SetMinDNAIonEnergy(minEnICmd->GetNewDoubleValue(newValue));
  }
  else if (command == maxEnICmd)
  {
    theParameters->SetMaxDNAIonEnergyPerNucleon(maxEnICmd->GetNewDoubleValue(newValue));
  }
  ...
  else if (command == direFluoCmd)
  {
    ...
    else if (newValue == "ANSTO")
    {
      ttt = fluoANSTO;
    }
    else if (newValue == "XDB_EADL")
    {
      ttt = fluoXDB_EADL;
    }
