      ...
      if (nullptr != baseParticle) {
        ...
        if (verbose > 1) {
          G4cout << "  #  " << p->GetParticleName()
                 ...
                 << " Escaled(MeV)= " << escaled 
                 << " q2=" << chargeSquare
                 << " dEdx(MeV/mm)=" << res << " << cut=" << cut
                 << " isIon:" << isIon << "  " << currentModel->GetName()
                 << G4endl;
        } 
      } else {
        ...
        if (verbose > 1) {
          G4cout << "  *  " << p->GetParticleName() << " E(MeV)=" << kinEnergy
                 << " dE/dx(MeV/mm)=" << res << " q2=" << chargeSquare
                 << " << cut=" << cut << " isIon:" << isIon << G4endl;
        }
      }
      // emulate smoothing procedure
      if (applySmoothing && !isIon &&
  ...
  if (UpdateParticle(part, kinEnergy)) {
    ...
    for (auto const & ptr : vel) {
      ...
      if (ActiveForParticle(part, p)) {
        ...
        dedx += ComputeDEDX(kinEnergy, part, ptr->GetProcessName(),
                            mat, cutenergy[idx]);
      }
    }
  }
                                      ...
                                      G4double kinEnergy)
{
  ...
  if (isIon && nullptr != currentProcess) {
    ...
    if (verbose > 1) {
      G4cout << G4endl;
      G4cout << "    UpdateParticle: " << p->GetParticleName()
             << "  massR= "<< massRatio << "   q2= " 
             << chargeSquare << "  " << currentProcess->GetProcessName() << G4endl;
    }
  }
  return true;
}
...
const G4ParticleDefinition* G4EmCalculator::FindParticle(const G4String& name)
{
  ...
  if(name != currentParticleName) {
    ...
    if(nullptr == p) {
      G4cout << "### G4EmCalculator::FindParticle WARNING: fails to find " 
             << name << G4endl;
    }
  } else {
    p = currentParticle;
...
const G4Material* G4EmCalculator::FindMaterial(const G4String& name)
{
  if(name != currentMaterialName) {
    ...
    if(nullptr == currentMaterial) {
      G4cout << "### G4EmCalculator::FindMaterial WARNING: fails to find " 
             << name << G4endl;
    }
  }
                                            ...
                                            G4double kinEnergy)
{
  ...
  if (verbose > 1) {
    G4cout << "### G4EmCalculator::FindEmModel for " << partname
           ...
           << ") and " << processName
           << " at E(MeV)= " << scaledEnergy << " isIon:" << isIon 
           << G4endl;
    if (isIon) { G4cout << "  GenericIon is the base particle" << G4endl; }
  }
