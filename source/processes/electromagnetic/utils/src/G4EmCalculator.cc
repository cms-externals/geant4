                                 ...
                                 const G4Material* mat, const G4Region* region)
{
  ...
  if (nullptr != couple && UpdateParticle(p, kinEnergy))
  {
    if (FindEmModel(p, currentProcessName, kinEnergy))
    {
      if (isIon)
      {
        ...
        res = manager->GetDEDX(theGenericIon, kinEnergy * massRatio, couple) * q2;
        length = 0.1 * kinEnergy / res;
        G4double eloss = res * length;
        auto const v = G4ProductionCutsTable::GetProductionCutsTable()->GetEnergyCutsVector(1);
  ...
  if (verbose > 1)
  {
    G4cout << "G4EmCalculator::GetDEDX: E(MeV)= " << kinEnergy / MeV
           ...
           << p->GetParticleName() << " in " << mat->GetName() << " isIon= " << isIon << G4endl;
  }
  return res;
}
                                                   ...
                                                   const G4Material* mat, const G4Region* region)
{
  ...
  if (nullptr != couple && UpdateParticle(p, kinEnergy))
  {
    ...
    if (verbose > 1)
    {
      G4cout << " G4EmCalculator::GetRangeFromRestrictedDEDX: E(MeV)= " << kinEnergy / MeV
             << " range(mm)= " << res / mm << "  " << p->GetParticleName() << " in "
             << mat->GetName() << G4endl;
    }
  }
  return res;
...
G4double G4EmCalculator::GetCSDARange(G4double kinEnergy, const G4ParticleDefinition* p,
                                      const G4Material* mat, const G4Region* region)
{
  G4double res = 0.0;
  if (!theParameters->BuildCSDARange())
  {
    ...
    G4Exception("G4EmCalculator::GetCSDARange", "em0077", JustWarning, ed);
    return res;
  }
  ...
  if (nullptr != couple && UpdateParticle(p, 1.0 * GeV))
  {
    ...
    if (verbose > 0)
    {
      G4cout << "G4EmCalculator::GetKinEnergy: Range(mm)= " << range / mm
             << " KinE(MeV)= " << res / MeV << "  " << p->GetParticleName() << " in "
             << mat->GetName() << G4endl;
    }
  }
  return res;
...
G4double G4EmCalculator::GetCrossSectionPerVolume(G4double kinEnergy, const G4ParticleDefinition* p,
    ...
    if (FindEmModel(p, processName, kinEnergy))
    {
      ...
      if (verbose > 0)
      {
        G4cout << "G4EmCalculator::GetXSPerVolume: E(MeV)= " << kinEnergy / MeV
               << " cross(cm-1)= " << res * cm << "  " << p->GetParticleName() << " in "
               << mat->GetName();
        if (verbose > 1)
          G4cout << "  idx= " << idx << "  Escaled((MeV)= " << kinEnergy * massRatio
                 << "  q2= " << chargeSquare;
        G4cout << G4endl;
      }
    }
...
G4double G4EmCalculator::GetShellIonisationCrossSectionPerAtom(const G4String& particle, G4int Z,
...
G4double G4EmCalculator::GetMeanFreePath(G4double kinEnergy, const G4ParticleDefinition* p,
                                         const G4String& processName, const G4Material* mat,
                                         const G4Region* region)
{
  ...
  if (verbose > 1)
  {
    G4cout << "G4EmCalculator::GetMeanFreePath: E(MeV)= " << kinEnergy / MeV
           << " MFP(mm)= " << res / mm << "  " << p->GetParticleName() << " in " << mat->GetName()
           << G4endl;
  }
  return res;
}
...
G4double G4EmCalculator::ComputeDEDX(G4double kinEnergy, const G4ParticleDefinition* p,
                                     const G4String& processName, const G4Material* mat,
                                     G4double cut)
{
  SetupMaterial(mat);
  ...
  if (verbose > 1)
  {
    G4cout << "### G4EmCalculator::ComputeDEDX: " << p->GetParticleName() << " in "
           << currentMaterialName << " e(MeV)= " << kinEnergy / MeV << "  cut(MeV)= " << cut / MeV
           << G4endl;
  }
  if (UpdateParticle(p, kinEnergy))
  {
    if (FindEmModel(p, processName, kinEnergy))
    {
      ...
      {
        res = currentModel->ComputeDEDXPerVolume(mat, baseParticle, escaled, cut) * chargeSquare;
        if (verbose > 1)
        {
          G4cout << "  #  " << p->GetParticleName() << " E(MeV)=" << kinEnergy
                 << " Base particle: " << baseParticle->GetParticleName()
                 << " Escaled(MeV)= " << escaled << " q2=" << chargeSquare
                 << " dEdx(MeV/mm)=" << res << " << cut=" << cut << " isIon:" << isIon << "  "
                 << currentModel->GetName() << G4endl;
        }
      }
      else
      {
        ...
        {
          G4cout << "  *  " << p->GetParticleName() << " E(MeV)=" << kinEnergy
                 << " dE/dx(MeV/mm)=" << res << " q2=" << chargeSquare << " << cut=" << cut
                 << " isIon:" << isIon << G4endl;
        }
      }
      // emulate smoothing procedure
      if (applySmoothing && !isIon && nullptr != loweModel && loweModel != currentModel)
      {
        ...
        if (nullptr != baseParticle)
        {
          res1 = chargeSquare * currentModel->ComputeDEDXPerVolume(mat, baseParticle, eth, cut);
          res0 = chargeSquare * loweModel->ComputeDEDXPerVolume(mat, baseParticle, eth, cut);
        }
        else
        {
      ...
      if (verbose > 0)
      {
        G4cout << "### E(MeV)= " << kinEnergy / MeV << " DEDX(MeV/mm)= " << res * mm / MeV
               << " DEDX(MeV*cm^2/g)= " << res * gram / (MeV * cm2 * mat->GetDensity())
               << " cut(MeV)= " << cut / MeV << "  " << p->GetParticleName() << " in "
               << currentMaterialName << " q^2= " << chargeSquare << " isIon=" << isIon << G4endl;
      }
    }
  }
  return res;
...
G4double G4EmCalculator::ComputeElectronicDEDX(G4double kinEnergy, const G4ParticleDefinition* part,
                                               const G4Material* mat, G4double cut)
{
  ...
  for (auto const& ptr : vel)
  {
    if (nullptr == ptr)
    {
      continue;
    }
    auto p = static_cast<G4VProcess*>(ptr);
    if (ActiveForParticle(part, p))
    {
      dedx += ComputeDEDX(kinEnergy, part, p->GetProcessName(), mat, cut);
    }
  }
  return dedx;
}
...
G4double G4EmCalculator::ComputeDEDXForCutInRange(G4double kinEnergy,
                                                  ...
                                                  const G4Material* mat, G4double rangecut)
{
  SetupMaterial(mat);
  G4double dedx = 0.0;
  if (kinEnergy <= 0.0)
  ...
  if (UpdateParticle(part, kinEnergy))
  {
    ...
    for (auto const& ptr : vel)
    {
      ...
      if (ActiveForParticle(part, p))
      {
        const G4ParticleDefinition* sec = ptr->SecondaryParticle();
        std::size_t idx = 0;
        if (sec == G4Electron::Electron())
        {
          idx = 1;
        }
        else if (sec == G4Positron::Positron())
        {
          idx = 2;
        }

        dedx += ComputeDEDX(kinEnergy, part, ptr->GetProcessName(), mat, cutenergy[idx]);
      }
    }
  }
  return dedx;
                                          ...
                                          const G4Material* mat, G4double cut)
{
  ...
  if (mass > 700. * MeV)
  {
    dedx += ComputeNuclearDEDX(kinEnergy, part, mat);
  }
  return dedx;
}
...
G4double G4EmCalculator::ComputeNuclearDEDX(G4double kinEnergy, const G4ParticleDefinition* p,
  ...
  if (UpdateParticle(p, kinEnergy))
  {
    if (FindEmModel(p, processName, kinEnergy))
    {
      ...
      if (baseParticle && !isIon)
      {
        ...
        res = currentModel->CrossSectionPerVolume(mat, baseParticle, e, aCut, e) * chargeSquare;
      }
      else
      {
      ...
      if (verbose > 0)
      {
        G4cout << "G4EmCalculator::ComputeXSPerVolume: E(MeV)= " << kinEnergy / MeV
               << " cross(cm-1)= " << res * cm << " cut(keV)= " << aCut / keV << "  "
               << p->GetParticleName() << " in " << mat->GetName() << G4endl;
      }
    }
  }
  return res;
...
G4double G4EmCalculator::ComputeCrossSectionPerAtom(G4double kinEnergy,
                                                    const G4ParticleDefinition* p,
                                                    const G4String& processName, G4double Z,
  ...
  if (UpdateParticle(p, kinEnergy))
  {
    ...
    if (FindEmModel(p, processName, kinEnergy))
    {
      ...
      if (baseParticle)
      {
        ...
        res = currentModel->ComputeCrossSectionPerAtom(baseParticle, e, Z, A, aCut) * chargeSquare;
      }
      else
      {
      ...
      if (verbose > 0)
      {
        G4cout << "E(MeV)= " << kinEnergy / MeV << " cross(barn)= " << res / barn << "  "
               << p->GetParticleName() << " Z= " << Z << " A= " << A / (g / mole) << " g/mole"
               << " cut(keV)= " << aCut / keV << G4endl;
      }
    }
  }
  return res;
...
G4double G4EmCalculator::ComputeCrossSectionPerShell(G4double kinEnergy,
                                                     const G4ParticleDefinition* p,
                                                     const G4String& processName, G4int Z,
  ...
  if (UpdateParticle(p, kinEnergy))
  {
    ...
    if (FindEmModel(p, processName, kinEnergy))
    {
      ...
      if (nullptr != baseParticle)
      {
        ...
        res = currentModel->ComputeCrossSectionPerShell(baseParticle, Z, shellIdx, e, aCut)
              * chargeSquare;
      }
      else
      {
      ...
      {
        G4cout << "E(MeV)= " << kinEnergy / MeV << " cross(barn)= " << res / barn << "  "
               << p->GetParticleName() << " Z= " << Z << " shellIdx= " << shellIdx
               << " cut(keV)= " << aCut / keV << G4endl;
      }
    }
  }
...
G4double G4EmCalculator::ComputeGammaAttenuationLength(G4double kinEnergy, const G4Material* mat)
{
  G4double res = 0.0;
...
G4double G4EmCalculator::ComputeShellIonisationCrossSectionPerAtom(const G4String& particle,
                                                                   G4int Z,
                                                                   G4AtomicShellEnumerator shell,
                                                                   G4double kinEnergy,
                                                                   const G4Material* mat)
{
  ...
  if (p && ad)
  {
    res = ad->ComputeShellIonisationCrossSectionPerAtom(p, Z, shell, kinEnergy, mat);
  }
  return res;
}
...
G4double G4EmCalculator::ComputeMeanFreePath(G4double kinEnergy, const G4ParticleDefinition* p,
                                             const G4String& processName, const G4Material* mat,
                                             G4double cut)
{
  ...
  if (verbose > 1)
  {
    G4cout << "E(MeV)= " << kinEnergy / MeV << " MFP(mm)= " << mfp / mm << "  "
           << p->GetParticleName() << " in " << mat->GetName() << G4endl;
  }
  return mfp;
}
...
G4double G4EmCalculator::ComputeEnergyCutFromRangeCut(G4double range,
                                                      ...
                                                      const G4Material* mat)
{
  return G4ProductionCutsTable::GetProductionCutsTable()->ConvertRangeToEnergy(part, mat, range);
}
...
G4bool G4EmCalculator::UpdateParticle(const G4ParticleDefinition* p, G4double kinEnergy)
{
  if (p != currentParticle)
  {
    ...
    if (nullptr != currentProcess)
    {
      ...
      if (nullptr != baseParticle)
      {
        ...
        G4double qb = baseParticle->GetPDGCharge() / CLHEP::eplus;
        if (qb != 0.0)
        {
          chargeSquare /= (qb * qb);
        }
  ...
  if (isIon && nullptr != currentProcess)
  {
    ...
    {
      G4cout << G4endl;
      G4cout << "    UpdateParticle: " << p->GetParticleName() << "  massR= " << massRatio
             << "   q2= " << chargeSquare << "  " << currentProcess->GetProcessName() << G4endl;
    }
  }
  return true;
}
...
const G4ParticleDefinition* G4EmCalculator::FindParticle(const G4String& name)
{
  ...
  if (name != currentParticleName)
  {
    ...
    if (nullptr == p)
    {
      G4cout << "### G4EmCalculator::FindParticle WARNING: fails to find " << name << G4endl;
    }
  }
  else
  {
...
const G4Material* G4EmCalculator::FindMaterial(const G4String& name)
{
  if (name != currentMaterialName)
  {
    ...
    {
      G4cout << "### G4EmCalculator::FindMaterial WARNING: fails to find " << name << G4endl;
    }
  }
                                                       ...
                                                       const G4Region* region)
{
  ...
  if (nullptr != currentMaterial)
  {
    ...
    if (nullptr != r)
    {
      couple = theCoupleTable->GetMaterialCutsCouple(material, r->GetProductionCuts());
    }
    else
    {
      ...
      if (0 < nr)
      {
        ...
        {
          couple =
            theCoupleTable->GetMaterialCutsCouple(material, ((*store)[i])->GetProductionCuts());
          if (nullptr != couple)
          {
            break;
          }
  ...
  {
    G4ExceptionDescription ed;
    ed << "G4EmCalculator::FindCouple: fail for material <" << currentMaterialName << ">";
    if (region)
    {
      ed << " and region " << region->GetName();
    }
    G4Exception("G4EmCalculator::FindCouple", "em0078", FatalException, ed);
  }
  return couple;
}
...
G4bool G4EmCalculator::UpdateCouple(const G4Material* material, G4double cut)
{
  ...
  for (G4int i = 0; i < nLocalMaterials; ++i)
  {
    G4cout << material->GetName() << " " << cut << " " << localCuts[i] << G4endl;
...
void G4EmCalculator::FindLambdaTable(const G4ParticleDefinition* p, const G4String& processName,
  ...
  if (!currentLambda || p != lambdaParticle || processName != lambdaName)
  {
    ...
    if (nullptr != elproc)
    {
      ...
      if (nullptr != currentLambda)
      {
        ...
        if (verbose > 1)
        {
          G4cout << "G4VEnergyLossProcess is found out: " << currentName << G4endl;
        }
      }
      curProcess = elproc;
      return;
    ...
    if (nullptr != msc)
    {
      ...
      if (nullptr != currentModel)
      {
        ...
        if (nullptr != currentLambda)
        {
          ...
          if (verbose > 1)
          {
            G4cout << "G4VMultipleScattering is found out: " << currentName << G4endl;
          }
        }
      }
      curProcess = msc;
...
G4bool G4EmCalculator::FindEmModel(const G4ParticleDefinition* p, const G4String& processName,
                                   G4double kinEnergy)
{
  isApplicable = false;
  ...
  if (verbose > 1)
  {
    G4cout << "### G4EmCalculator::FindEmModel for " << partname
           << " (type= " << p->GetParticleType() << ") and " << processName
           << " at E(MeV)= " << scaledEnergy << " isIon:" << isIon << G4endl;
    if (isIon)
    {
      G4cout << "  GenericIon is the base particle" << G4endl;
    }
  }
  ...
  if (nullptr != currentModel)
  {
    ...
    if (verbose > 1)
    {
      G4cout << "    Model <" << currentModel->GetName()
             << "> Emin(MeV)= " << currentModel->LowEnergyLimit() / MeV << " for "
             << part->GetParticleName();
      if (nullptr != elproc)
      {
        G4cout << " and " << elproc->GetProcessName() << "  " << elproc << G4endl;
      }
                                                        ...
                                                        const G4String& processName)
{
  ...
  const std::vector<G4VEnergyLossProcess*> v = manager->GetEnergyLossProcessVector();
  std::size_t n = v.size();
                                                      ...
                                                      const G4String& processName)
{
  ...
  const std::vector<G4VMultipleScattering*> v = manager->GetMultipleScatteringVector();
  std::size_t n = v.size();
  for (std::size_t i = 0; i < n; ++i)
  {
    if ((v[i])->GetProcessName() == processName)
...
G4bool G4EmCalculator::ActiveForParticle(const G4ParticleDefinition* part, G4VProcess* proc)
{
