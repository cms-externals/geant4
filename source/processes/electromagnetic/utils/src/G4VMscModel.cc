...
G4ParticleChangeForMSC* G4VMscModel::GetParticleChangeForMSC(const G4ParticleDefinition*)
{
...
void G4VMscModel::DumpParameters(std::ostream& out) const
{
  ...
  if (steppingAlgorithm == fUseDistanceToBoundary)
    alg = "DistanceToBoundary";
  else if (steppingAlgorithm == fMinimal)
    alg = "Minimal";
  else if (steppingAlgorithm == fUseSafetyPlus)
    alg = "SafetyPlus";

  out << std::setw(18) << "StepLim=" << alg << " Rfact=" << facrange << " Gfact=" << facgeom
      << " Sfact=" << facsafety << " DispFlag:" << latDisplasment << " Skin=" << skin
      << " Llim=" << lambdalimit / CLHEP::mm << " mm" << G4endl;
}

//....oooOO0OOooo........oooOO0OOooo........oooOO0OOooo........oooOO0OOooo......

void G4VMscModel::SampleSecondaries(std::vector<G4DynamicParticle*>*, const G4MaterialCutsCouple*,
...
G4double G4VMscModel::GetEnergy(const G4ParticleDefinition* part, G4double range,
                                const G4MaterialCutsCouple* couple)
{
  G4double e;
  if (nullptr != ionisation)
  {
    e = ionisation->GetKineticEnergy(range, couple);
  }
  else
  {
    e = localtkin;
