...
{
  if (nullptr == ptr || fNumberOfManagers >= nManagerMax) { return; }
...
{
  if (nullptr == table || isInitialized) { return; }
...
G4bool G4EmDataRegistry::TablesAreBuilt(const G4ParticleDefinition* part,
                                        const G4int id)
{
  if (!parallelIni || id >= nManagerMax) { return true; }
...
G4EmDataRegistry::ListForParallelBuild() const
{
  return fParticle;
}
