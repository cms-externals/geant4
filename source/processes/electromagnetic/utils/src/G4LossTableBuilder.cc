...
void G4LossTableBuilder::BuildDEDXTable(
...
// Build range table from the energy loss table
{
  ...
  G4int verb = theParameters->Verbose(); 
  if (1 < verb) {
    G4cout << "### G4LossTableBuilder::BuildRangeTable: nCouples="
           << nCouples << "  ThreadID=" << G4Threading::G4GetThreadId()
	   << G4endl; 
  }
  if (0 >= nCouples) { return; }
  ...
  for (std::size_t i=0; i<nCouples; ++i) {
    ...
    if (nullptr == pv || (useBaseMaterial && !(*theFlag)[i])) { continue; } 
    ...
    delete (*rangeTable)[i];
    G4PhysicsLogVector* v = (0 == bin0) ? new G4PhysicsLogVector(*pv)
    ...
    if (2 < verb) {
      G4cout << "New Range vector Npoints=" << v->GetVectorLength()
             ...
             << " Elow=" << v->GetMinEnergy() <<" Ehigh=" << v->GetMaxEnergy()
...
void G4LossTableBuilder::BuildInverseRangeTable(
...
void
G4LossTableBuilder::InitialiseBaseMaterials(const G4PhysicsTable* table) const
{
                                       ...
                                       G4bool spline) const
{
  ...
  for (G4int i=0; i<(G4int)numOfCouples; ++i) {
    if (table->GetFlag(i)) {
      ...
      G4double tmin =
        std::max(std::max(emin, model->MinPrimaryEnergy(mat, part)), CLHEP::eV);
      G4int n = nbins;
