...
void G4EmExtraParameters::SetStepFunction(G4double v1, G4double v2)
{
  ...
  else
  {
    ...
    ed << "Values of step function are out of range: " << v1 << ", " << v2 / CLHEP::mm
       << " mm - are ignored";
    PrintWarning(ed);
  }
}
...
void G4EmExtraParameters::AddPAIModel(const G4String& particle, const G4String& region,
                                      const G4String& type)
{
...
void G4EmExtraParameters::AddPhysics(const G4String& region, const G4String& type)
{
  G4String r = CheckRegion(region);
  std::size_t nreg = m_regnamesPhys.size();
  for (std::size_t i = 0; i < nreg; ++i)
  {
    if (r == m_regnamesPhys[i])
    {
      return;
    }
  }
...
void G4EmExtraParameters::SetProcessBiasingFactor(const G4String& procname, G4double val,
                                                  G4bool wflag)
{
  if (val > 0.0)
  {
  ...
  {
    G4ExceptionDescription ed;
    ed << "Process: " << procname << " XS biasing factor " << val << " is negative - ignored";
    PrintWarning(ed);
  }
}

void G4EmExtraParameters::ActivateForcedInteraction(const G4String& procname,
                                                    const G4String& region, G4double length,
                                                    G4bool wflag)
{
  const G4String& r = CheckRegion(region);
  if (length >= 0.0)
  ...
  else
  {
    ...
    ed << "Process: " << procname << " in region " << r
       << " : forced interacttion length= " << length << " is negative - ignored";
    PrintWarning(ed);
  }
}

void G4EmExtraParameters::ActivateSecondaryBiasing(const G4String& procname, const G4String& region,
                                                   G4double factor, G4double energyLim)
{
  ...
  {
    G4ExceptionDescription ed;
    ed << "Process: " << procname << " in region " << r << " : secondary bised factor= " << factor
       << ", Elim= " << energyLim << " - ignored";
    PrintWarning(ed);
  }
}
...
void G4EmExtraParameters::DefineRegParamForLoss(G4VEnergyLossProcess* ptr) const
{
  ...
  for (std::size_t i = 0; i < n; ++i)
  {
    ...
    {
      ptr->SetCrossSectionBiasingFactor(m_factBiasedXS[i], m_weightBiasedXS[i]);
      break;
    }
  }
  n = m_procForced.size();
  for (std::size_t i = 0; i < n; ++i)
  {
    ...
    {
      ptr->ActivateForcedInteraction(m_lengthForced[i], m_regnamesForced[i], m_weightForced[i]);
      break;
    }
  }
  ...
  for (std::size_t i = 0; i < n; ++i)
  {
    ...
    {
      ptr->ActivateSecondaryBiasing(m_regnamesBiasedSec[i], m_factBiasedSec[i], m_elimBiasedSec[i]);
      break;
    }
  }
}
