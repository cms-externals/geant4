...
G4EnergyLossTablesHelper::G4EnergyLossTablesHelper(
  const G4PhysicsTable* aDEDXTable, const G4PhysicsTable* aRangeTable,
  const G4PhysicsTable* anInverseRangeTable, const G4PhysicsTable* aLabTimeTable,
  const G4PhysicsTable* aProperTimeTable, G4double aLowestKineticEnergy,
  G4double aHighestKineticEnergy, G4double aMassRatio, G4int aNumberOfBins)
  : theDEDXTable(aDEDXTable),
    theRangeTable(aRangeTable),
...
G4EnergyLossTablesHelper::G4EnergyLossTablesHelper()
{
  ...
  theMassRatio = 0.0;
  theNumberOfBins = 0;
  theDEDXTable = theRangeTable = theInverseRangeTable = theLabTimeTable = theProperTimeTable =
    nullptr;
}

//....oooOO0OOooo........oooOO0OOooo........oooOO0OOooo........oooOO0OOooo......

void G4EnergyLossTables::Register(const G4ParticleDefinition* p, const G4PhysicsTable* tDEDX,
                                  const G4PhysicsTable* tRange, const G4PhysicsTable* tInverseRange,
                                  const G4PhysicsTable* tLabTime, const G4PhysicsTable* tProperTime,
                                  G4double lowestKineticEnergy, G4double highestKineticEnergy,
                                  G4double massRatio, G4int NumberOfBins)
{
  ...
  (*dict)[p] =
    G4EnergyLossTablesHelper(tDEDX, tRange, tInverseRange, tLabTime, tProperTime,
                             lowestKineticEnergy, highestKineticEnergy, massRatio, NumberOfBins);

  *t = GetTables(p);  // important for cache !!!!!
  lastParticle = (G4ParticleDefinition*)p;
  Chargesquare = (p->GetPDGCharge()) * (p->GetPDGCharge()) / QQPositron;
  if (first_loss)
  {
    *null_loss =
      G4EnergyLossTablesHelper(nullptr, nullptr, nullptr, nullptr, nullptr, 0.0, 0.0, 0.0, 0);
    first_loss = false;
  }
}
...
G4double G4EnergyLossTables::GetDEDX(const G4ParticleDefinition* aParticle, G4double KineticEnergy,
                                     const G4Material* aMaterial)
{
  ...
  if (aParticle != (const G4ParticleDefinition*)lastParticle)
  {
    *t = GetTables(aParticle);
    ...
    Chargesquare = (aParticle->GetPDGCharge()) * (aParticle->GetPDGCharge()) / QQPositron;
    oldIndex = -1;
  }
  ...
  {
    dEdx = (*dEdxTable)(materialIndex)->GetValue(t->theLowestKineticEnergy, isOut)
           * std::sqrt(scaledKineticEnergy / t->theLowestKineticEnergy);
  }
  else if (scaledKineticEnergy > t->theHighestKineticEnergy)
  {
    dEdx = (*dEdxTable)(materialIndex)->GetValue(t->theHighestKineticEnergy, isOut);
  }
  else
  {
    dEdx = (*dEdxTable)(materialIndex)->GetValue(scaledKineticEnergy, isOut);
  }
...
G4double G4EnergyLossTables::GetLabTime(const G4ParticleDefinition* aParticle,
                                        G4double KineticEnergy, const G4Material* aMaterial)
{
  if (!t) t = new G4EnergyLossTablesHelper;
  ...
  if (scaledKineticEnergy < t->theLowestKineticEnergy)
  {
    time = std::exp(ppar * std::log(scaledKineticEnergy / t->theLowestKineticEnergy))
           * (*labtimeTable)(materialIndex)->GetValue(t->theLowestKineticEnergy, isOut);
  }
  else if (scaledKineticEnergy > t->theHighestKineticEnergy)
  {
    time = (*labtimeTable)(materialIndex)->GetValue(t->theHighestKineticEnergy, isOut);
  }
  else
  {
    time = (*labtimeTable)(materialIndex)->GetValue(scaledKineticEnergy, isOut);
  }
...
G4double G4EnergyLossTables::GetDeltaLabTime(const G4ParticleDefinition* aParticle,
                                             G4double KineticEnergyStart, G4double KineticEnergyEnd,
                                             const G4Material* aMaterial)
{
  if (!t) t = new G4EnergyLossTablesHelper;
  ...
  {
    timestart = (*labtimeTable)(materialIndex)->GetValue(t->theHighestKineticEnergy, isOut);
  }
  else
  {
    timestart = (*labtimeTable)(materialIndex)->GetValue(scaledKineticEnergy, isOut);
  }
  ...
  {
    timeend = (*labtimeTable)(materialIndex)->GetValue(t->theHighestKineticEnergy, isOut);
  }
  else
  {
    timeend = (*labtimeTable)(materialIndex)->GetValue(scaledKineticEnergy, isOut);
  }

  deltatime = timestart - timeend;

  if (dTT < dToverT) deltatime *= dTT / dToverT;
                                           ...
                                           G4double KineticEnergy, const G4Material* aMaterial)
{
  ...
  {
    time = std::exp(ppar * std::log(scaledKineticEnergy / t->theLowestKineticEnergy))
           * (*propertimeTable)(materialIndex)->GetValue(t->theLowestKineticEnergy, isOut);
  }
  else if (scaledKineticEnergy > t->theHighestKineticEnergy)
  {
    time = (*propertimeTable)(materialIndex)->GetValue(t->theHighestKineticEnergy, isOut);
  }
  else
  {
    time = (*propertimeTable)(materialIndex)->GetValue(scaledKineticEnergy, isOut);
  }
                                                ...
                                                const G4Material* aMaterial)
{
  ...
  {
    timestart = (*propertimeTable)(materialIndex)->GetValue(t->theHighestKineticEnergy, isOut);
  }
  else
  {
    timestart = (*propertimeTable)(materialIndex)->GetValue(scaledKineticEnergy, isOut);
  }
  ...
  {
    timeend = (*propertimeTable)(materialIndex)->GetValue(t->theHighestKineticEnergy, isOut);
  }
  else
  {
    timeend = (*propertimeTable)(materialIndex)->GetValue(scaledKineticEnergy, isOut);
  }
...
G4double G4EnergyLossTables::GetRange(const G4ParticleDefinition* aParticle, G4double KineticEnergy,
                                      const G4Material* aMaterial)
{
  if (!t) t = new G4EnergyLossTablesHelper;
  ...
  if (scaledKineticEnergy < t->theLowestKineticEnergy)
  {
    Range = std::sqrt(scaledKineticEnergy / t->theLowestKineticEnergy)
            * (*rangeTable)(materialIndex)->GetValue(t->theLowestKineticEnergy, isOut);
  }
  else if (scaledKineticEnergy > t->theHighestKineticEnergy)
  {
    Range = (*rangeTable)(materialIndex)->GetValue(t->theHighestKineticEnergy, isOut)
            + (scaledKineticEnergy - t->theHighestKineticEnergy)
                / (*dEdxTable)(materialIndex)->GetValue(t->theHighestKineticEnergy, isOut);
  }
  else
  {
    Range = (*rangeTable)(materialIndex)->GetValue(scaledKineticEnergy, isOut);
  }
...
G4double G4EnergyLossTables::GetPreciseEnergyFromRange(const G4ParticleDefinition* aParticle,
                                                       G4double range, const G4Material* aMaterial)
// it returns the value of the kinetic energy for a given range
{
  ...
  if (materialIndex != oldIndex)
  {
    ...
    rmin = (*inverseRangeTable)(materialIndex)->GetLowEdgeEnergy(0);
    rmax = (*inverseRangeTable)(materialIndex)->GetLowEdgeEnergy(t->theNumberOfBins - 2);
    Thigh = (*inverseRangeTable)(materialIndex)->GetValue(rmax, isOut);
  }
  ...
  if (scaledrange < rmin)
  {
    scaledKineticEnergy = t->theLowestKineticEnergy * scaledrange * scaledrange / (rmin * rmin);
  }
  else
  {
    if (scaledrange < rmax)
    {
      scaledKineticEnergy = (*inverseRangeTable)(materialIndex)->GetValue(scaledrange, isOut);
    }
    else
    {
      scaledKineticEnergy =
        Thigh + (scaledrange - rmax) * (*dEdxTable)(materialIndex)->GetValue(Thigh, isOut);
    }
  }
                                            ...
                                            G4double KineticEnergy, const G4Material* aMaterial)
{
  ...
  if (scaledKineticEnergy < t->theLowestKineticEnergy)
  {
    dEdx = std::sqrt(scaledKineticEnergy / t->theLowestKineticEnergy)
           * (*dEdxTable)(materialIndex)->GetValue(t->theLowestKineticEnergy, isOut);
  }
  else if (scaledKineticEnergy > t->theHighestKineticEnergy)
  {
    dEdx = (*dEdxTable)(materialIndex)->GetValue(t->theHighestKineticEnergy, isOut);
                                                       ...
                                                       const G4Material* aMaterial)
{
  ...
  G4double Thighr = t->theHighestKineticEnergy * t->theLowestKineticEnergy
                    / (*rangeTable)(materialIndex)->GetLowEdgeEnergy(1);
  ...
  {
    Range = (*rangeTable)(materialIndex)->GetValue(Thighr, isOut)
            + (scaledKineticEnergy - Thighr) / (*dEdxTable)(materialIndex)->GetValue(Thighr, isOut);
  }
  else
  {
    Range = (*rangeTable)(materialIndex)->GetValue(scaledKineticEnergy, isOut);
...
G4double G4EnergyLossTables::GetDEDX(const G4ParticleDefinition* aParticle, G4double KineticEnergy,
                                     const G4MaterialCutsCouple* couple, G4bool check)
{
  if (!t) t = new G4EnergyLossTablesHelper;
                                            ...
                                            const G4MaterialCutsCouple* couple)
{
  ...
  const G4PhysicsTable* dEdxTable = t->theDEDXTable;
  if (!dEdxTable) return G4LossTableManager::Instance()->GetDEDX(aParticle, KineticEnergy, couple);
...
void G4EnergyLossTables::ParticleHaveNoLoss(const G4ParticleDefinition*, const G4String& /*q*/) {}
