...
G4VAtomDeexcitation::G4VAtomDeexcitation(const G4String& modname) : name(modname)
{
  vdyn.reserve(5);
...
void G4VAtomDeexcitation::InitialiseAtomicDeexcitation()
{
  ...
  {
    G4cout << G4endl;
    G4cout << "### ===  Deexcitation model " << name << " is activated for " << nRegions;
    if (1 == nRegions)
    {
  ...
  for (std::size_t j = 0; j < nRegions; ++j)
  {
    ...
    if (nullptr != reg && 0 < nCouples)
    {
      ...
      if (0 < verbose)
      {
        G4cout << "          " << activeRegions[j] << "  " << deRegions[j] << "  "
               << AugerRegions[j] << "  " << PIXERegions[j] << G4endl;
      }
  ...
  {
    G4cout << "### ===  Auger flag: " << flagAuger << G4endl;
  }
  if (0 < verbose)
  {
    G4cout << "### ===  Ignore cuts flag:   " << ignoreCuts << G4endl;
  }
  if (0 < verbose && flagPIXE)
  {
    G4cout << "### ===  PIXE model for hadrons: " << theParameters->PIXECrossSectionModel()
           << G4endl;
    G4cout << "### ===  PIXE model for e+-:     " << theParameters->PIXEElectronCrossSectionModel()
           << G4endl;
  }
}
...
void G4VAtomDeexcitation::SetDeexcitationActiveRegion(const G4String& rname, G4bool valDeexcitation,
                                                      G4bool valAuger, G4bool valPIXE)
{
  // no PIXE in parallel world
  if (rname == "DefaultRegionForParallelWorld")
  {
    return;
  }
  ...
  */
  ...
  if (ss == "DefaultRegionForTheWorld")
  {
    ...
    {
      if (ss == (*regions)[i]->GetName())
      {
        continue;
      }
      SetDeexcitationActiveRegion((*regions)[i]->GetName(), valDeexcitation, valAuger, valPIXE);
    }
  }
}

void G4VAtomDeexcitation::GenerateParticles(std::vector<G4DynamicParticle*>* v,
                                            const G4AtomicShell* as, G4int Z, G4int idx)
{
...
void G4VAtomDeexcitation::AlongStepDeexcitation(std::vector<G4Track*>& tracks, const G4Step& step,
                                                G4double& eLossMax, G4int coupleIndex)
{
  if (!activePIXEMedia[coupleIndex] || eLossMax <= 0.0)
  {
    return;
  }
  ...
  for (std::size_t i = 0; i < nelm; ++i)
  {
    ...
    if (activeZ[Z] && Z < 93)
    {
      ...
      for (G4int ii = 0; ii < nshells; ++ii)
      {
        ...
        {
          G4double sig = rho * GetShellIonisationCrossSectionPerAtom(part, Z, as, ekin, material);
          ...
          if (sig > 0.0)
          {
            ...
            do
            {
              ...
              if (stot > 1.0 || eLossMax < bindingEnergy)
              {
                break;
              }
              // sample deexcitation
