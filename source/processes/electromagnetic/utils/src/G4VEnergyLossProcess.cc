...
namespace
{
G4String tnames[7] = {"DEDX",   "Ionisation", "DEDXnr",      "CSDARange",
                      "Lambda", "Range",      "InverseRange"};
}
  ...
  : G4VContinuousDiscreteProcess(name, type)
{
  ...
  theDensityFactor = reg->GetDensityFactors();
  theDensityIdx = reg->GetCoupleIndexes();
  theFluctuationFlags = reg->GetFluctuationFlags();
...
G4double G4VEnergyLossProcess::MinPrimaryEnergy(const G4ParticleDefinition*, const G4Material*,
                                                G4double cut)
{
  return cut;
}
...
void G4VEnergyLossProcess::AddEmModel(G4int order, G4VEmModel* ptr, G4VEmFluctuationModel* fluc,
...
void G4VEnergyLossProcess::PreparePhysicsTable(const G4ParticleDefinition& part)
{
  if (1 < verboseLevel)
  {
    G4cout << "### G4VEnergyLossProcess::PreparePhysicsTable() for " << part.GetParticleName()
           << G4endl;
  }
  if (nullptr == particle)
  {
  ...
  isIon = G4EmTableUtil::IsIon(&part);

  // for ions tables are not created but G4GenericIon data are used
  if (isIon && part.GetParticleName() != "GenericIon")
  {
  ...
  if (particle != &part)
  {
    ...
    if (1 < verboseLevel)
    {
      G4cout << "### G4VEnergyLossProcess::PreparePhysicsTable()"
             << " interrupted for " << GetProcessName() << " and " << part.GetParticleName()
             << " isIon=" << isIon << " spline=" << spline << G4endl;
    }
    return;
  }

  tablesAreBuilt = false;
  if (GetProcessSubType() == fIonisation)
  {
    SetIonisation(true);
  }
  ...
  if (!actMinKinEnergy)
  {
    minKinEnergy = theParameters->MinKinEnergy();
  }
  if (!actMaxKinEnergy)
  {
    maxKinEnergy = theParameters->MaxKinEnergy();
  }
  if (!actBinning)
  {
    nBins = theParameters->NumberOfBins();
  }
  maxKinEnergyCSDA = theParameters->MaxEnergyForCSDARange();
  nBinsCSDA =
    theParameters->NumberOfBinsPerDecade() * G4lrint(std::log10(maxKinEnergyCSDA / minKinEnergy));
  if (!actLinLossLimit)
  {
    linLossLimit = theParameters->LinearLossLimit();
  }
  ...
  if (!theParameters->Integral())
  {
    fXSType = fEmNoIntegral;
  }
  ...
  if (nullptr != baseParticle)
  {
    ...
    chargeSqRatio = q * q;
    if (chargeSqRatio > 0.0)
    {
      reduceFactor = 1.0 / (chargeSqRatio * massRatio);
    }
  }
  lowestKinEnergy = (initialMass < CLHEP::MeV) ? theParameters->LowestElectronEnergy()
  ...
  if (isMaster && nullptr == baseParticle)
  {
    if (nullptr == theData)
    {
      theData = new G4EmDataHandler(7, particle->GetParticleName());
    }
    ...
    G4EmDataRegistry::Instance()->InitialiseBaseMaterials(theDEDXTable);
    ...
    if (theParameters->BuildCSDARange())
    {
      ...
      if (isIonisation)
      {
        theCSDARangeTable = theData->MakeTable(theCSDARangeTable, 3);
      }
    }
  ...
  baseMat = G4EmDataRegistry::Instance()->GetBaseMaterialFlag();
  numberOfModels = modelManager->NumberOfModels();
  currentModel = modelManager->GetModel(0);
  G4EmTableUtil::UpdateModels(this, modelManager, maxKinEnergy, numberOfModels, secID, biasID,
                              mainSecondaries, baseMat, isMaster,
                              theParameters->UseAngularGeneratorForIonisation());
  theCuts = modelManager->Initialise(particle, secondaryParticle, verboseLevel);
  // subcut processor
  if (isIonisation)
  {
  ...
  if (1 < verboseLevel)
  {
    ...
    G4cout << G4endl;
    G4cout << " chargeSqRatio= " << chargeSqRatio << " massRatio= " << massRatio
...
void G4VEnergyLossProcess::BuildPhysicsTable(const G4ParticleDefinition& part)
{
  if (1 < verboseLevel)
  {
    G4cout << "### G4VEnergyLossProcess::BuildPhysicsTable() for " << GetProcessName()
           << " and particle " << part.GetParticleName() << "; the first particle "
    ...
    G4cout << "    TablesAreBuilt= " << tablesAreBuilt << " isIon= " << isIon
           << " spline=" << spline << " ptr: " << this << " isMaster=" << isMaster << G4endl;
  }
  ...
  if (&part == particle)
  {
    ...
    {
      const auto masterProcess = static_cast<const G4VEnergyLossProcess*>(GetMasterProcess());

      baseMat = masterProcess->UseBaseMaterial();
      numberOfModels = modelManager->NumberOfModels();
      G4EmTableUtil::BuildLocalElossProcess(this, masterProcess, particle, numberOfModels);
      tablesAreBuilt = true;
  ...
  if (isIonisation)
  {
    ...
    {
      if (atomDeexcitation->IsPIXEActive())
      {
  ...
  if (1 < verboseLevel)
  {
    G4cout << "### G4VEnergyLossProcess::BuildPhysicsTable() done for " << GetProcessName()
           << " and particle " << part.GetParticleName();
    if (isIonisation)
    {
      G4cout << "  isIonisation flag=1";
    }
    G4cout << " baseMat=" << baseMat << G4endl;
  }

  // protection against double printout
  if (theParameters->IsPrintLocked())
  {
    return;
  }
  ...
  if (1 < verboseLevel
      || (0 < verboseLevel
          && (num == "e-" || num == "e+" || num == "mu+" || num == "mu-" || num == "proton"
              || num == "pi+" || num == "pi-" || num == "kaon+" || num == "kaon-" || num == "alpha"
              || num == "anti_proton" || num == "GenericIon" || num == "alpha+")))
  {
...
G4PhysicsTable* G4VEnergyLossProcess::BuildDEDXTable(G4EmTableType tType)
{
  ...
  {
    G4cout << "G4VEnergyLossProcess::BuildDEDXTable WARNING: wrong type " << tType << G4endl;
  }
  if (1 < verboseLevel)
  {
    G4cout << "G4VEnergyLossProcess::BuildDEDXTable() of type " << tType << " for "
           << GetProcessName() << " and " << particle->GetParticleName() << " spline=" << spline
           << G4endl;
  }
  if (nullptr == table)
  {
  ...
  G4EmTableUtil::BuildDEDXTable(this, particle, modelManager, table, minKinEnergy, emax, bin,
...
G4PhysicsTable* G4VEnergyLossProcess::BuildLambdaTable(G4EmTableType)
{
  ...
  G4EmTableUtil::BuildLambdaTable(this, particle, modelManager, theLambdaTable, theCuts,
                                  minKinEnergy, maxKinEnergy, scale, verboseLevel, spline);
  return theLambdaTable;
}

//....oooOO0OOooo........oooOO0OOooo........oooOO0OOooo........oooOO0OOooo....

void G4VEnergyLossProcess::StreamInfo(std::ostream& out, const G4ParticleDefinition& part,
                                      G4bool rst) const
{
  G4String indent = (rst ? "  " : "");
  ...
  out << "  XStype:" << fXSType << "  SubType=" << GetProcessSubType() << G4endl
      << "      dE/dx and range tables from " << G4BestUnit(minKinEnergy, "Energy") << " to "
      << G4BestUnit(maxKinEnergy, "Energy") << " in " << nBins << " bins" << G4endl
      << "      Lambda tables from threshold to " << G4BestUnit(maxKinEnergy, "Energy") << ", "
      << theParameters->NumberOfBinsPerDecade() << " bins/decade, spline: " << spline << G4endl;
  if (nullptr != theRangeTableForLoss && isIonisation)
  {
    out << "      StepFunction=(" << dRoverRange << ", " << finalRange / mm << " mm)"
        << ", integ: " << fXSType << ", fluct: " << lossFluctuationFlag
        << ", linLossLim= " << linLossLimit << G4endl;
  }
  ...
  if (nullptr != theCSDARangeTable && isIonisation)
  {
    out << "      CSDA range table up"
        << " to " << G4BestUnit(maxKinEnergyCSDA, "Energy") << " in " << nBinsCSDA << " bins"
        << G4endl;
  }
  if (nSCoffRegions > 0 && isIonisation)
  {
    out << "      Subcutoff sampling in " << nSCoffRegions << " regions" << G4endl;
  }
  if (2 < verboseLevel)
  {
...
void G4VEnergyLossProcess::StartTracking(G4Track* track)
{
  ...
  if (isIon)
  {
    massRatio = CLHEP::proton_mass_c2 / track->GetParticleDefinition()->GetPDGMass();
    logMassRatio = G4Log(massRatio);
  }
...
G4double G4VEnergyLossProcess::AlongStepGetPhysicalInteractionLength(const G4Track& track, G4double,
                                                                     G4double, G4double&,
                                                                     G4GPILSelection* selection)
{
  ...
  *selection = aGPILSelection;
  if (isIonisation && currentModel->IsActive(preStepScaledEnergy))
  {
    ...
    /*
    G4double e1 = preStepScaledEnergy;
  if(currentCoupleIndex != coupleIdxRange || fRangeEnergy != e1) {
    coupleIdxRange = currentCoupleIndex;
    fRangeEnergy = e1;
    G4cout << "e1=" << e1 << G4endl;
    G4double e2 = LogScaledEkin(track);
    G4cout << "e2=" << e2 << " baseCoupleIndex=" << basedCoupleIndex
     << "  " << theRangeTableForLoss << " Rfac=" << reduceFactor << G4endl;
    G4cout << *theRangeTableForLoss << G4endl;
     G4cout << "=== 00" << G4endl;
    fRange = reduceFactor*
      ((*theRangeTableForLoss)[basedCoupleIndex])->LogVectorValue(e1, e2);
    G4cout << "R=" << fRange << G4endl;
    fRange = std::max(fRange, 0.0);
    if (e1 < minKinEnergy) { fRange *= std::sqrt(e1/minKinEnergy); }
  }
    */

    x = (useCutAsFinalRange)
          ? std::min(finalRange, currentCouple->GetProductionCuts()->GetProductionCut(1))
          : finalRange;
    x = (fRange > x) ? fRange * dRoverRange + x * (1.0 - dRoverRange) * (2.0 - x / fRange) : fRange;
                                                                    ...
                                                                    G4ForceCondition* condition)
{
  ...
  *condition = NotForced;
  ...
  if (verboseLevel > 2)
  {
    ...
    G4cout << " for " << track.GetParticleDefinition()->GetParticleName() << " in Material  "
           << currentMaterial->GetName() << " Ekin(MeV)= " << preStepKinEnergy / MeV
           << " track material: " << track.GetMaterial()->GetName() << G4endl;
...
void G4VEnergyLossProcess::ComputeLambdaForScaledEnergy(const G4double e, const G4Track& track)
{
...
G4VParticleChange* G4VEnergyLossProcess::AlongStepDoIt(const G4Track& track, const G4Step& step)
{
  ...
  */
  ...
  if (length >= fRange || preStepKinEnergy <= lowestKinEnergy)
  {
    ...
    if (useDeexcitation)
    {
      atomDeexcitation->AlongStepDeexcitation(scTracks, step, eloss, (G4int)currentCoupleIndex);
      if (!scTracks.empty())
      {
        FillSecondariesAlongStep(weight);
      }
  ...
  // Short step
  eloss = length * GetDEDXForScaledEnergy(preStepScaledEnergy, LogScaledEkin(track));
  /*
  ...
  */
  ...
  if (isIon)
  {
    ...
    currentModel->CorrectionsAlongStep(currentMaterial, track.GetParticleDefinition(),
                                       preStepKinEnergy, cut, length, eloss);
    eloss = std::max(eloss, 0.0);
  }
  ...
  else if ((*theFluctuationFlags)[currentCoupleIndex])
  {
    ...
    eloss = fluc->SampleFluctuations(currentCouple, dynParticle, tcut, tmax, length, eloss);
    /*
    if(-1 < verboseLevel)
  ...
  if (useDeexcitation)
  {
    ...
    atomDeexcitation->AlongStepDeexcitation(scTracks, step, de, (G4int)currentCoupleIndex);
  ...
  else if (isIon)
  {
    fParticleChange.SetProposedCharge(
      currentModel->GetParticleCharge(track.GetParticleDefinition(), currentMaterial, finalT));
  }
  eloss = std::max(eloss, 0.0);
...
void G4VEnergyLossProcess::FillSecondariesAlongStep(G4double wt)
{
  ...
  if (biasManager)
  {
    ...
    {
      weight *= biasManager->ApplySecondaryBiasing(scTracks, (G4int)currentCoupleIndex);
    }
  }
...
G4VParticleChange* G4VEnergyLossProcess::PostStepDoIt(const G4Track& track, const G4Step& step)
{
  ...
  */
  ...
  if (fXSType != fEmNoIntegral)
  {
    ...
    G4double lx = GetLambdaForScaledEnergy(postStepScaledEnergy, logFinalT + logMassRatio);
    lx = std::max(lx, 0.0);
  ...
  if (biasManager)
  {
    if (biasManager->SecondaryBiasingRegion((G4int)currentCoupleIndex))
    {
      ...
      weight *= biasManager->ApplySecondaryBiasing(
        secParticles, track, currentModel, &fParticleChange, eloss, (G4int)currentCoupleIndex, tcut,
  ...
  for (std::size_t i = 0; i < 7; ++i)
  {
    ...
    if (1 < verboseLevel)
    {
      G4cout << "G4VEnergyLossProcess::StorePhysicsTable i=" << i << "  "
             << particle->GetParticleName() << "  " << GetProcessName() << "  " << tnames[i] << "  "
             << theData->Table(i) << G4endl;
    }
    if (!G4EmTableUtil::StoreTable(this, part, theData->Table(i), dir, tnames[i], verboseLevel,
                                   ascii))
    {
      return false;
    }
                                                  ...
                                                  const G4String& dir, G4bool ascii)
{
  ...
  for (std::size_t i = 0; i < 7; ++i)
  {
    ...
    if (!G4EmTableUtil::RetrieveTable(this, part, theData->Table(i), dir, tnames[i], verboseLevel,
                                      ascii, spline))
    {
      return false;
    }
...
G4double G4VEnergyLossProcess::GetDEDXDispersion(const G4MaterialCutsCouple* couple,
                                                 const G4DynamicParticle* dp, const G4double length)
{
  ...
  if (nullptr != fm)
  {
    d = fm->Dispersion(currentMaterial, dp, tcut, tmax, length);
  }
  return d;
}
                                                     ...
                                                     G4double logKineticEnergy)
{
  ...
  if (nullptr != theLambdaTable)
  {
    cross = GetLambdaForScaledEnergy(kineticEnergy * massRatio, logKineticEnergy + logMassRatio);
  }
  else
  {
...
G4double G4VEnergyLossProcess::MeanFreePath(const G4Track& track)
{
  ...
  const G4double cs = GetLambdaForScaledEnergy(kinEnergy * massRatio, logKinEnergy + logMassRatio);
  return (0.0 < cs) ? 1.0 / cs : DBL_MAX;
}

//....oooOO0OOooo........oooOO0OOooo........oooOO0OOooo........oooOO0OOooo....

G4double G4VEnergyLossProcess::ContinuousStepLimit(const G4Track& track, G4double x, G4double y,
                                                   G4double& z)
{
...
G4double G4VEnergyLossProcess::GetMeanFreePath(const G4Track& track, G4double,
                                               G4ForceCondition* condition)
...
G4double G4VEnergyLossProcess::GetContinuousStepLimit(const G4Track&, G4double, G4double, G4double&)
{
  return DBL_MAX;
}
...
void G4VEnergyLossProcess::SetDEDXTable(G4PhysicsTable* p, G4EmTableType tType)
{
  if (1 < verboseLevel)
  {
    G4cout << "### Set DEDX table " << p << "  " << theDEDXTable << "  " << theDEDXunRestrictedTable
           << "  " << theIonisationTable << " for " << particle->GetParticleName()
           << " and process " << GetProcessName() << " type=" << tType
           << " isIonisation:" << isIonisation << G4endl;
  }
  if (fTotal == tType)
  {
...
void G4VEnergyLossProcess::SetLambdaTable(G4PhysicsTable* p)
{
  if (1 < verboseLevel)
  {
    G4cout << "### Set Lambda table " << p << " " << theLambdaTable << " for "
           << particle->GetParticleName() << " and process " << GetProcessName() << G4endl;
  }
  theLambdaTable = p;
  tablesAreBuilt = true;
  ...
  if (isMaster && nullptr != p)
  {
    ...
    if (fEmTwoPeaks == fXSType)
    {
      if (nullptr != fXSpeaks)
      {
        for (auto& ptr : *fXSpeaks)
        {
          delete ptr;
        }
        delete fXSpeaks;
      }
      fXSpeaks = G4EmUtility::FillPeaksStructure(p);
      if (nullptr == fXSpeaks)
      {
        fXSType = fEmOnePeak;
      }
...
{
  return (nullptr != currentModel) ? currentModel->GetCurrentElement(currentMaterial) : nullptr;
}

//....oooOO0OOooo........oooOO0OOooo........oooOO0OOooo........oooOO0OOooo....

void G4VEnergyLossProcess::SetCrossSectionBiasingFactor(G4double f, G4bool flag)
{
  if (f > 0.0)
  {
    biasFactor = f;
    ...
    if (1 < verboseLevel)
    {
      G4cout << "### SetCrossSectionBiasingFactor: for "
             << " process " << GetProcessName() << " biasFactor= " << f << " weightFlag= " << flag
             << G4endl;
    }
  }
}
...
void G4VEnergyLossProcess::ActivateForcedInteraction(G4double length, const G4String& region,
                                                     G4bool flag)
{
  if (nullptr == biasManager)
  {
    biasManager = new G4EmBiasingManager();
  }
  if (1 < verboseLevel)
  {
    G4cout << "### ActivateForcedInteraction: for "
           << " process " << GetProcessName() << " length(mm)= " << length / mm << " in G4Region <"
           << region << "> weightFlag= " << flag << G4endl;
  }
  weightFlag = flag;
...
void G4VEnergyLossProcess::ActivateSecondaryBiasing(const G4String& region, G4double factor,
  ...
  if (0.0 <= factor)
  {
    ...
    if (1 < verboseLevel)
    {
      G4cout << "### ActivateSecondaryBiasing: for "
             << " process " << GetProcessName() << " factor= " << factor << " in G4Region <"
             << region << "> energyLimit(MeV)= " << energyLimit / MeV << G4endl;
    }
  }
}
...
void G4VEnergyLossProcess::SetLinearLossLimit(G4double val)
{
  ...
  {
    PrintWarning("SetLinearLossLimit", val);
  }
}
...
void G4VEnergyLossProcess::SetLowestEnergyLimit(G4double val)
{
  ...
  {
    PrintWarning("SetLowestEnergyLimit", val);
  }
}
...
void G4VEnergyLossProcess::SetMinKinEnergy(G4double e)
{
  ...
  {
    PrintWarning("SetMinKinEnergy", e);
  }
}
...
void G4VEnergyLossProcess::SetMaxKinEnergy(G4double e)
{
  ...
  {
    PrintWarning("SetMaxKinEnergy", e);
  }
}
...
void G4VEnergyLossProcess::PrintWarning(const G4String& tit, G4double val) const
{
  ...
  ed << "Parameter is out of range: " << val << " it will have no effect!\n"
     << "  Process " << GetProcessName() << "  nbins= " << nBins
     << " Emin(keV)= " << minKinEnergy / keV << " Emax(GeV)= " << maxKinEnergy / GeV;
