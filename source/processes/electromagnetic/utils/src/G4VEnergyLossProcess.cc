...
void G4VEnergyLossProcess::BuildPhysicsTable(const G4ParticleDefinition& part)
{
  ...
  if(&part == particle) {
    ...
    if(isMaster) {
      ...
      if(fXSType == fEmOnePeak || fXSType == fEmTwoPeaks) {
	auto twopeaks = masterProcess->TwoPeaksXS();
	SetTwoPeaksXS(twopeaks);
	if(nullptr == twopeaks) {
	  SetEnergyOfCrossSectionMax(masterProcess->EnergyOfCrossSectionMax());
	}
      }
...
void G4VEnergyLossProcess::SetLambdaTable(G4PhysicsTable* p)
{
  ...
  if(isMaster && nullptr != p && 
     (fEmOnePeak == fXSType || fEmTwoPeaks == fXSType)) {
    delete theEnergyOfCrossSectionMax;
    theEnergyOfCrossSectionMax = nullptr;
    if(fEmTwoPeaks == fXSType) {
      delete fXSpeaks;
      fXSpeaks = nullptr;
      auto ptr = G4EmUtility::FillPeaksStructure(p, bld);
      SetTwoPeaksXS(ptr);
      if(nullptr == ptr) {
	SetEnergyOfCrossSectionMax(G4EmUtility::FindCrossSectionMax(p));
      }
    } else if(fXSType == fEmOnePeak) { 
      SetEnergyOfCrossSectionMax(G4EmUtility::FindCrossSectionMax(p));
    }
...
{
  fXSType = (nullptr == p) ? fEmIncreasing : fEmOnePeak;
...
const G4Element* G4VEnergyLossProcess::GetCurrentElement() const
{
  return (nullptr != currentModel) 
    ? currentModel->GetCurrentElement() : nullptr;
}
