  ...
  G4VContinuousDiscreteProcess(name, type)
{
  ...
  theFluctuationFlags = theRegistry->GetFluctuationFlags();
...
G4VEnergyLossProcess::PreparePhysicsTable(const G4ParticleDefinition& part)
{
  ...
  if (1 < verboseLevel) {
    G4cout << "G4VEnergyLossProcess::PrepearPhysicsTable() is done "
           << " for " << GetProcessName() << " and "
           << particle->GetParticleName() << " isIon= " << isIon
           << " spline=" << spline << " id=" << lManager->ThreadID();
    if(baseParticle) { 
...
void G4VEnergyLossProcess::BuildPhysicsTable(const G4ParticleDefinition& part)
{
  if (1 < verboseLevel) {  
    G4cout << "### G4VEnergyLossProcess::BuildPhysicsTable() for "
	   ...
	   << " threadID=" << lManager->ThreadID();
    if (baseParticle) {
      G4cout << "; base: " << baseParticle->GetParticleName(); 
    }
    G4cout << G4endl;
    G4cout << "    TablesAreBuilt= " << tablesAreBuilt << " isIon= " << isIon
           << " spline=" << spline << " isMaster=" << isMaster << G4endl;
  }
  ...
  if(&part == particle) {
    if(isMaster) {
      ...
      numberOfModels = modelManager->NumberOfModels();
      G4bool ok = theRegistry->TablesAreBuilt(particle, lManager->ThreadID());
      if (ok) {
        G4EmTableUtil::WorkerElossProcess(this, masterProcess,
                                          particle, numberOfModels);
	tablesAreBuilt = true;
	lManager->WorkerPhysicsTables(particle, this);
      } else {
	lManager->BuildTables(particle, true);
	auto pr = const_cast<G4VEnergyLossProcess*>(masterProcess);
        G4EmTableUtil::WorkerElossProcess(pr, this,
                                          particle, numberOfModels);
	theRegistry->ResetFlag(particle);
      }
    }
...
G4PhysicsTable* G4VEnergyLossProcess::BuildDEDXTable(G4EmTableType tType)
{
  ...
  G4EmTableUtil::BuildDEDXTable(this, particle, modelManager,
...
void G4VEnergyLossProcess::StartTracking(G4Track* track)
{
  ...
  if(isIon) {
    massRatio =
      CLHEP::proton_mass_c2/track->GetParticleDefinition()->GetPDGMass();
    logMassRatio = G4Log(massRatio);
  }  
                      ...
                      G4GPILSelection* selection)
{
  ...
  *selection = aGPILSelection;
  if(isIonisation && currentModel->IsActive(preStepScaledEnergy)) {
    ...
    x = (fRange > x)
      ? fRange*dRoverRange + x*(1.0 - dRoverRange)*(2.0 - x/fRange)
      : fRange;

#ifdef G4VERBOSE
    if (verboseLevel > 2) {     
      G4cout<<"AlongStepGPIL: " << GetProcessName()<<": e="<<preStepKinEnergy
            << " fRange=" << fRange << " finalRange=" << finalRange
            << " stepLimit=" << x << G4endl;
    }
#endif
  }
  ...
  *condition = NotForced;
  ...
  if(biasManager) {
    if(0 == track.GetParentID() && biasFlag && 
      ...
      return biasManager->GetStepLimit((G4int)currentCoupleIndex,
                                       previousStepSize);
    }
  }
...
void G4VEnergyLossProcess::ComputeLambdaForScaledEnergy(const G4double e,
                                                        const G4Track& track)
{
                                            ...
                                            G4double logKineticEnergy)
{
  ...
  if (nullptr != theLambdaTable) {
    ...
    cross = (!baseMat) ? biasFactor
      : biasFactor*(*theDensityFactor)[currentCoupleIndex];
    cross *= (currentModel->CrossSectionPerVolume(currentMaterial, particle,
                                                  kineticEnergy,
...
void G4VEnergyLossProcess::SetLambdaTable(G4PhysicsTable* p, G4bool ok)
{
  if(1 < verboseLevel) {
