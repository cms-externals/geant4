...
G4RegionModels::G4RegionModels(G4int nMod, std::vector<G4int>& indx, G4DataVector& lowE,
                               const G4Region* reg)
{
...
void G4EmModelManager::AddEmModel(G4int num, G4VEmModel* p, G4VEmFluctuationModel* fm,
                                  const G4Region* r)
{
  if (nullptr == p)
  {
...
G4VEmModel* G4EmModelManager::GetModel(G4int idx, G4bool ver) const
{
  ...
  else if (verboseLevel > 0 && ver)
  {
    G4cout << "G4EmModelManager::GetModel WARNING: "
           << "index " << idx << " is wrong Nmodels= " << nEmModels;
    if (nullptr != particle)
    {
                                                 ...
                                                 G4int verb)
{
  ...
  if (1 < verboseLevel)
  {
    G4cout << "G4EmModelManager::Initialise() for " << p->GetParticleName()
  ...
  if (nEmModels < 1)
  {
    ...
    G4Exception("G4EmModelManager::Initialise", "em0002", FatalException, ed);
  }
  ...
  if (1 < verboseLevel)
  {
    G4cout << "    Nregions= " << nRegions << "  Nmodels= " << nEmModels << G4endl;
  }
  ...
  for (G4int reg = 0; reg < nRegions; ++reg)
  {
    ...
    for (G4int ii = 0; ii < nEmModels; ++ii)
    {
      ...
      if (region == regions[ii])
      {
        ...
        if (1 < verboseLevel)
        {
          ...
          G4cout << ">  "
                 << " tmin(MeV)= " << tmin / MeV << "; tmax(MeV)= " << tmax / MeV
                 << "; order= " << ord << "; tminAct= " << model->LowEnergyActivationLimit() / MeV
                 << "; tmaxAct= " << model->HighEnergyActivationLimit() / MeV << G4endl;
        }
        ...
        if (n > 0)
        {
          ...
          else if (tmin < eHigh[n - 1])
          {
            ...
            for (G4int k = 0; k < n; ++k)
            {
              ...
              if (ord >= modelOrd[k])
              {
                if (tmin < eHigh[k] && tmin >= eLow[k])
                {
                  tmin = eHigh[k];
                }
                if (tmax <= eHigh[k] && tmax > eLow[k])
                {
                  tmax = eLow[k];
                }
                if (tmax > eHigh[k] && tmin < eLow[k])
                {
                  if (tmax - eHigh[k] > eLow[k] - tmin)
                  {
                    tmin = eHigh[k];
                  }
                  else
    ...
    if (1 < verboseLevel)
    {
      ...
      if (region)
      {
        G4cout << region->GetName();
      }
      G4cout << ">  Elow(MeV)= ";
      for (G4int iii = 0; iii <= n; ++iii)
      {
        G4cout << eLow[iii] / MeV << " ";
      }
      G4cout << G4endl;
  ...
  {
    if (secondaryParticle == G4Gamma::Gamma())
    {
      idx = 0;
    }
    else if (secondaryParticle == G4Electron::Electron())
    {
      idx = 1;
    }
    else if (secondaryParticle == G4Positron::Positron())
    {
      idx = 2;
    }
    else
  ...
  for (std::size_t i = 0; i < numOfCouples; ++i)
  {
    ...
    if (1 < verboseLevel)
    {
      G4cout << "G4EmModelManager::Initialise() for " << material->GetName()
             << " indexOfCouple= " << i << " indexOfRegion= " << reg << G4endl;
    }
    ...
    if (nullptr != secondaryParticle)
    {
      ...
      for (G4int jj = 0; jj < nnm; ++jj)
      {
        ...
        if (cutlim > cut)
        {
          if (nullptr == theCutsNew)
          {
            theCutsNew = new G4DataVector(*theCuts);
          }
          (*theCutsNew)[i] = cutlim;
          /*
  ...
  for (G4int jj = 0; jj < nEmModels; ++jj)
  {
    if (1 == isUsed[jj])
    {
      ...
      if (nullptr != flucModels[jj])
      {
        flucModels[jj]->InitialiseMe(particle);
      }
    }
  }
  if (1 == nn)
  ...
  if (1 < verboseLevel)
  {
    G4cout << "G4EmModelManager for " << particle->GetParticleName()
           << " is initialised; nRegions=  " << nRegions << " severalModels: " << severalModels
           << G4endl;
  }
  return theCuts;
}
...
void G4EmModelManager::FillDEDXVector(G4PhysicsVector* aVector, const G4MaterialCutsCouple* couple,
                                      G4EmTableType tType)
{
  ...
  if (1 < verboseLevel)
  {
    G4cout << "G4EmModelManager::FillDEDXVector() for " << couple->GetMaterial()->GetName()
           << "  cut(MeV)= " << cut << "  Type " << tType << "  for " << particle->GetParticleName()
           << G4endl;
  }
  ...
  for (std::size_t j = 0; j < totBinsLoss; ++j)
  {
    ...
    if (nmod > 1)
    {
      ...
      if (k > 0 && k != k0)
      {
        ...
        G4double dedx2 = models[regModels->ModelIndex(k)]->ComputeDEDX(couple, particle, elow, cut);
    ...
    G4double dedx =
      (1.0 + del / e) * models[regModels->ModelIndex(k)]->ComputeDEDX(couple, particle, e, cut);

    if (2 < verboseLevel)
    {
      G4cout << "Material= " << couple->GetMaterial()->GetName() << "   E(MeV)= " << e / MeV
             << "  dEdx(MeV/mm)= " << dedx * mm / MeV << "  del= " << del * mm / MeV << " k= " << k
             << " modelIdx= " << regModels->ModelIndex(k) << G4endl;
    }
...
void G4EmModelManager::FillLambdaVector(G4PhysicsVector* aVector,
                                        const G4MaterialCutsCouple* couple, G4bool startFromNull,
                                        G4EmTableType tType)
{
  std::size_t i = couple->GetIndex();
  G4double cut = (*theCuts)[i];
  ...
  if (1 < verboseLevel)
  {
    G4cout << "G4EmModelManager::FillLambdaVector() for " << particle->GetParticleName() << " in "
           << couple->GetMaterial()->GetName() << " Emin(MeV)= " << aVector->Energy(0)
           << " Emax(MeV)= " << aVector->GetMaxEnergy() << " cut= " << cut << " Type " << tType
           << " nmod= " << nmod << G4endl;
  }
  ...
  for (std::size_t j = 0; j < totBinsLambda; ++j)
  {
    ...
    if (fIsCrossSectionPrim == tType)
    {
      cross *= e;
    }

    if (j == 0 && startFromNull)
    {
      cross = 0.0;
    }
    ...
    if (2 < verboseLevel)
    {
      G4cout << "FillLambdaVector: " << j << ".   e(MeV)= " << e / MeV
             << "  cross(1/mm)= " << cross * mm << " del= " << del * mm << " k= " << k
             << " modelIdx= " << regModels->ModelIndex(k) << G4endl;
    }
...
void G4EmModelManager::DumpModelList(std::ostream& out, G4int verb)
{
  ...
  for (G4int i = 0; i < nRegions; ++i)
  {
    ...
    if (n > 0)
    {
      out << "      ===== EM models for the G4Region  " << reg->GetName() << " ======" << G4endl;
      for (G4int j = 0; j < n; ++j)
      {
        G4VEmModel* model = models[r->ModelIndex(j)];
        G4double emin = std::max(r->LowEdgeEnergy(j), model->LowEnergyActivationLimit());
        G4double emax = std::min(r->LowEdgeEnergy(j + 1), model->HighEnergyActivationLimit());
        if (emax > emin)
        {
          out << std::setw(20);
          out << model->GetName() << " : Emin=" << std::setw(5) << G4BestUnit(emin, "Energy")
              << " Emax=" << std::setw(5) << G4BestUnit(emax, "Energy");
          ...
          if (nullptr != table)
          {
            ...
            for (std::size_t k = 0; k < kk; ++k)
            {
              ...
              if (v)
              {
                ...
                out << " Nbins=" << nn << " " << std::setw(3) << G4BestUnit(v->Energy(0), "Energy")
