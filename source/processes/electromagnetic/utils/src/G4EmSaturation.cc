                                                 ...
                                                 G4double niel) const
{
  ...
  if (bfactor > 0.0)
  {
    ...
    if (22 == p->GetPDGEncoding())
    {
      ...
      evis /=
        (1.0 + bfactor * edep / G4LossTableManager::Instance()->GetRange(electron, edep, couple));
    ...
    else
    {
      ...
      if (nloss > 0.0)
      {
        ...
        */
        G4double range =
          G4LossTableManager::Instance()->GetRange(proton, escaled, couple) / effCharges[idx];
...
{
  if (nMaterials == G4Material::GetNumberOfMaterials())
  {
    return;
  }
...
G4double G4EmSaturation::FindG4BirksCoefficient(const G4Material* mat)
{
  ...
  for (G4int j = 0; j < nG4Birks; ++j)
  {
    ...
    {
      if (verbose > 0)
        G4cout << "### G4EmSaturation::FindG4BirksCoefficient for " << name << " is "
...
void G4EmSaturation::InitialiseBirksCoefficient(const G4Material* mat)
{
  ...
  if (nullptr == electron)
  {
    ...
    if (nullptr == electron)
    {
      G4Exception("G4EmSaturation::InitialiseBirksCoefficient", "em0001", FatalException,
...
void G4EmSaturation::DumpBirksCoefficients()
{
  ...
  for (std::size_t i = 0; i < nMaterials; ++i)
  {
    ...
    if (br > 0.0)
    {
      G4cout << "   " << mat->GetName() << "     " << br * MeV / mm << " mm/MeV" << "     "
             << br * mat->GetDensity() * MeV * cm2 / g
             << " g/cm^2/MeV  massFactor=  " << massFactors[i] << " effCharge= " << effCharges[i]
             << G4endl;
    }
  }
}
