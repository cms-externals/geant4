...
void G4EmConfigurator::SetExtraEmModel(const G4String& particleName, const G4String& processName,
                                       G4VEmModel* mod, const G4String& regionName, G4double emin,
                                       G4double emax, G4VEmFluctuationModel* fm)
{
  if (nullptr == mod)
  {
    return;
  ...
  if (1 < verbose)
  {
    G4cout << " G4EmConfigurator::SetExtraEmModel " << mod->GetName() << " for " << particleName
           << " and " << processName << " in the region <" << regionName
           << "> Emin(MeV)= " << emin / MeV << " Emax(MeV)= " << emax / MeV << G4endl;
  }
  ...
  if (n > 0)
  {
    for (size_t i = 0; i < n; ++i)
    {
      if (nullptr != models[i])
      {
        ...
        {
          --index;
          SetModelForRegion(models[i], flucModels[i], reg, particles[i], processes[i], lowEnergy[i],
                            highEnergy[i]);
        }
      }
    }
...
void G4EmConfigurator::SetModelForRegion(G4VEmModel* mod, G4VEmFluctuationModel* fm,
                                         const G4Region* reg, const G4String& particleName,
                                         const G4String& processName, G4double emin, G4double emax)
{
  if (nullptr == mod)
  {
    return;
  ...
  if (1 < verbose)
  {
    G4cout << " G4EmConfigurator::SetModelForRegion: " << mod->GetName() << G4endl;
    G4cout << " For " << particleName << " and " << processName << " in the region <"
           << reg->GetName() << " Emin(MeV)= " << emin / MeV << " Emax(MeV)= " << emax / MeV;
    if (nullptr != fm)
    {
      G4cout << " FLmodel " << fm->GetName();
    }
    G4cout << G4endl;
  }
  ...
  while ((*myParticleIterator)())
  {
    ...
    if ((part->GetParticleName() == particleName) || (particleName == "all")
        || (particleName == "charged" && part->GetPDGCharge() != 0.0))
    {
      ...
      if (!UpdateModelEnergyRange(mod, emin, emax))
      {
        return;
      }
      // classify process
...
void G4EmConfigurator::PrepareModels(const G4ParticleDefinition* aParticle, G4VEnergyLossProcess* p)
{
  size_t n = particles.size();
  if (1 < verbose)
  {
    G4cout << " G4EmConfigurator::PrepareModels for EnergyLoss n= " << n << G4endl;
  }
  if (n > 0)
  {
...
void G4EmConfigurator::PrepareModels(const G4ParticleDefinition* aParticle, G4VEmProcess* p)
{
  size_t n = particles.size();
  if (1 < verbose)
  {
    G4cout << " G4EmConfigurator::PrepareModels for EM process n= " << n << G4endl;
  }
  if (n > 0)
  {
    G4String particleName = aParticle->GetParticleName();
          ...
          if (nullptr != reg)
          {
            ...
            if (nullptr != mod)
            {
              if (UpdateModelEnergyRange(mod, lowEnergy[i], highEnergy[i]))
              {
                ...
                if (1 < verbose)
                {
                  G4cout << "### Added em model order= " << index << " for " << particleName
                         << " and " << processName << G4endl;
                }
              }
            }
          }
...
void G4EmConfigurator::PrepareModels(const G4ParticleDefinition* aParticle,
                                     G4VMultipleScattering* p, G4TransportationWithMsc* trans)
{
  size_t n = particles.size();
  if (1 < verbose)
  {
    G4cout << " G4EmConfigurator::PrepareModels for MSC process n= " << n << G4endl;
  }
...
G4bool G4EmConfigurator::UpdateModelEnergyRange(G4VEmModel* mod, G4double emin, G4double emax)
{
  // energy limits
  ...
  if (e2 <= e1)
  {
    G4cout << "### G4EmConfigurator WARNING: empty energy interval"
           << " for <" << mod->GetName() << ">  Emin(MeV)= " << e1 / CLHEP::MeV
           << ">  Emax(MeV)= " << e2 / CLHEP::MeV << G4endl;
    return false;
  }
  ...
  if (verbose > 1)
  {
    G4cout << "### G4EmConfigurator for " << mod->GetName() << " Emin(MeV)= " << e1 / MeV
           << " Emax(MeV)= " << e2 / MeV << G4endl;
  }
  return true;
}
