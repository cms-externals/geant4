  ...
  : G4VDiscreteProcess(name, type)
{
  ...
  auto theRegistry = G4EmDataRegistry::Instance();
  theDensityFactor = theRegistry->GetDensityFactors();
  theDensityIdx = theRegistry->GetCoupleIndexes();
}
...
void G4VEmProcess::AddEmModel(G4int order, G4VEmModel* ptr, const G4Region* region)
{
  if (nullptr == ptr)
  {
    return;
...
void G4VEmProcess::PreparePhysicsTable(const G4ParticleDefinition& part)
{
  ...
  if (isTheMaster)
  {
    if (buildLambdaTable)
    {
      if (nullptr == theData)
      {
        theData = new G4EmDataHandler(2, particle->GetParticleName());
      }
      theLambdaTable = theData->MakeTable(theLambdaTable, 0);
      theRegistry->InitialiseBaseMaterials(theLambdaTable);
    }
    // high energy table
    if (minKinEnergyPrim < maxKinEnergy)
    {
      ...
      theRegistry->InitialiseBaseMaterials(theLambdaTablePrim);
    }
  }
  // models
  baseMat = theRegistry->GetBaseMaterialFlag();
  ...
  theCuts =
    G4EmTableUtil::PrepareEmProcess(this, particle, secondaryParticle, modelManager, maxKinEnergy,
                                    secID, tripletID, mainSecondaries, verboseLevel, isTheMaster);
}
...
void G4VEmProcess::BuildPhysicsTable(const G4ParticleDefinition& part)
{
  ...
  G4EmTableUtil::BuildEmProcess(this, masterProc, particle, &part, nModels, verboseLevel,
                                isTheMaster, isLocked, toBuild, baseMat);
}
...
void G4VEmProcess::BuildLambdaTable()
{
  ...
  if (actBinning)
  {
    nbin = std::max(nbin, nLambdaBins);
  }
  scale = nbin / G4Log(scale);

  G4EmTableUtil::BuildLambdaTable(this, particle, modelManager, theLambdaTable, theLambdaTablePrim,
                                  minKinEnergy, minKinEnergyPrim, maxKinEnergy, scale, verboseLevel,
                                  startFromNull, splineFlag);
}

//....oooOO0OOooo........oooOO0OOooo........oooOO0OOooo........oooOO0OOooo....

void G4VEmProcess::StreamInfo(std::ostream& out, const G4ParticleDefinition& part, G4bool rst) const
{
  G4String indent = (rst ? "  " : "");
  ...
  if (buildLambdaTable)
  {
    if (particle == &part)
    {
      for (auto& v : *theLambdaTable)
      {
        if (nullptr != v)
        {
          ...
          {
            out << G4BestUnit(emin, "Energy");
          }
          out << " to " << G4BestUnit(emax, "Energy") << ", "
              << G4lrint(nbin / std::log10(emax / emin)) << " bins/decade, spline: " << splineFlag
              << G4endl;
          break;
        }
      }
    ...
    {
      out << "      Used Lambda table of " << particle->GetParticleName() << G4endl;
    }
  }
  if (minKinEnergyPrim < maxKinEnergy)
  {
    if (particle == &part)
    {
      for (auto& v : *theLambdaTablePrim)
      {
        if (nullptr != v)
        {
          out << "      LambdaPrime table from " << G4BestUnit(v->Energy(0), "Energy") << " to "
              << G4BestUnit(v->GetMaxEnergy(), "Energy") << " in " << v->GetVectorLength() - 1
              << " bins " << G4endl;
          break;
        }
      }
    ...
    {
      out << "      Used LambdaPrime table of " << particle->GetParticleName() << G4endl;
    }
  }
...
void G4VEmProcess::StartTracking(G4Track* track)
{
  ...
  if (isIon)
  {
    massRatio = proton_mass_c2 / currentParticle->GetPDGMass();
  }
                                                            ...
                                                            G4ForceCondition* condition)
{
  *condition = NotForced;
  ...
  if (isIon)
  {
    currentModel->ChargeSquareRatio(track);
  }
  /*
  ...
  */
  ...
  if (biasManager)
  {
    if (0 == track.GetParentID())
    {
      if (biasFlag && biasManager->ForcedInteractionRegion((G4int)currentCoupleIndex))
      {
...
G4VParticleChange* G4VEmProcess::PostStepDoIt(const G4Track& track, const G4Step& step)
{
  ...
  if (fXSType != fEmNoIntegral)
  {
    ...
    {
      G4cout << "WARNING: for " << currentParticle->GetParticleName() << " and " << GetProcessName()
             << " E(MeV)= " << finalT / MeV << " preLambda= " << preStepLambda << " < " << lx
             << " (postLambda) " << G4endl;
    }
#endif
  ...
  if (1 < verboseLevel)
  {
    G4cout << "G4VEmProcess::PostStepDoIt: Sample secondary; E= " << finalT / MeV
           << " MeV; model= (" << currentModel->LowEnergyLimit() << ", "
           << currentModel->HighEnergyLimit() << ")" << G4endl;
  }
#endif

  // sample secondaries
  secParticles.clear();
  currentModel->SampleSecondaries(&secParticles, currentCouple, track.GetDynamicParticle(),
  ...
  if (biasManager)
  {
    if (biasManager->SecondaryBiasingRegion((G4int)currentCoupleIndex))
    {
      ...
      weight *= biasManager->ApplySecondaryBiasing(
        secParticles, track, currentModel, &fParticleChange, eloss, (G4int)currentCoupleIndex,
        (*theCuts)[currentCoupleIndex], step.GetPostStepPoint()->GetSafety());
      if (eloss > 0.0)
      {
    ...
    for (G4int i = 0; i < num; ++i)
    {
      ...
      if (nullptr != dp)
      {
        ...
        if (applyCuts)
        {
          ...
          {
            if (e < (*theCutsElectron)[currentCoupleIndex])
            {
              good = false;
            }
          }
...
G4bool G4VEmProcess::StorePhysicsTable(const G4ParticleDefinition* part, const G4String& directory,
                                       G4bool ascii)
{
  if (!isTheMaster || part != particle)
  {
    return true;
  }
  if (G4EmTableUtil::StoreTable(this, part, theLambdaTable, directory, "Lambda", verboseLevel,
                                ascii)
      && G4EmTableUtil::StoreTable(this, part, theLambdaTablePrim, directory, "LambdaPrim",
                                   verboseLevel, ascii))
  {
    return true;
  }
...
G4bool G4VEmProcess::RetrievePhysicsTable(const G4ParticleDefinition* part, const G4String& dir,
                                          G4bool ascii)
{
  if (!isTheMaster || part != particle)
  {
  ...
  if (buildLambdaTable)
  {
    yes = G4EmTableUtil::RetrieveTable(this, part, theLambdaTable, dir, "Lambda", verboseLevel,
                                       ascii, splineFlag);
  }
  if (yes && minKinEnergyPrim < maxKinEnergy)
  {
    yes = G4EmTableUtil::RetrieveTable(this, part, theLambdaTablePrim, dir, "LambdaPrim",
                                       verboseLevel, ascii, splineFlag);
  }
  return yes;
}
...
G4double G4VEmProcess::GetCrossSection(G4double kinEnergy, const G4MaterialCutsCouple* couple)
{
...
G4double G4VEmProcess::GetMeanFreePath(const G4Track& track, G4double, G4ForceCondition* condition)
{
  *condition = NotForced;
...
G4double G4VEmProcess::ComputeCrossSectionPerAtom(G4double kinEnergy, G4double Z, G4double A,
                                                  G4double cut)
{
  SelectModel(kinEnergy, currentCoupleIndex);
  return (currentModel)
           ? currentModel->ComputeCrossSectionPerAtom(currentParticle, kinEnergy, Z, A, cut)
           : 0.0;
}
...
G4PhysicsVector* G4VEmProcess::LambdaPhysicsVector(const G4MaterialCutsCouple* couple)
{
  ...
  G4PhysicsVector* newv =
    new G4PhysicsLogVector(minKinEnergy, maxKinEnergy, nLambdaBins, splineFlag);
  return newv;
}
...
{
  return (nullptr != currentModel) ? currentModel->GetCurrentIsotope(GetCurrentElement()) : nullptr;
}
...
void G4VEmProcess::SetCrossSectionBiasingFactor(G4double f, G4bool flag)
{
  if (f > 0.0)
  {
    ...
    if (1 < verboseLevel)
    {
      G4cout << "### SetCrossSectionBiasingFactor: for " << particle->GetParticleName()
             << " and process " << GetProcessName() << " biasFactor= " << f
             << " weightFlag= " << flag << G4endl;
    }
  }
}
...
void G4VEmProcess::ActivateForcedInteraction(G4double length, const G4String& r, G4bool flag)
{
  if (nullptr == biasManager)
  {
  ...
  if (1 < verboseLevel)
  {
    G4cout << "### ActivateForcedInteraction: for " << particle->GetParticleName()
           << " and process " << GetProcessName() << " length(mm)= " << length / mm
           << " in G4Region <" << r << "> weightFlag= " << flag << G4endl;
  }
  weightFlag = flag;
...
void G4VEmProcess::ActivateSecondaryBiasing(const G4String& region, G4double factor,
...
void G4VEmProcess::SetMinKinEnergy(G4double e)
{
  ...
  {
    nLambdaBins =
      G4lrint(nLambdaBins * G4Log(maxKinEnergy / e) / G4Log(maxKinEnergy / minKinEnergy));
    minKinEnergy = e;
...
void G4VEmProcess::SetMaxKinEnergy(G4double e)
{
  ...
  {
    nLambdaBins =
      G4lrint(nLambdaBins * G4Log(e / minKinEnergy) / G4Log(maxKinEnergy / minKinEnergy));
    maxKinEnergy = e;
...
{
  if (theParameters->MinKinEnergy() <= e && e <= theParameters->MaxKinEnergy())
  {
    minKinEnergyPrim = e;
  }
  else
  {
    PrintWarning("SetMinKinEnergyPrim", e);
  }
}
...
void G4VEmProcess::PrintWarning(G4String tit, G4double val)
{
  ...
  ed << "Parameter is out of range: " << val << " it will have no effect!\n"
     << "  Process " << GetProcessName() << "  nbins= " << theParameters->NumberOfBins()
