...
G4double G4ElectronIonPair::MeanNumberOfIonsAlongStep(const G4ParticleDefinition* part,
                                                      const G4Material* material, G4double edep,
                                                      G4double niel)
{
  G4double nion = 0.0;
  ...
  if (edep > niel)
  {
    ...
    if (part->GetPDGCharge() != 0.0)
    {
      ...
      if (curMeanEnergy > 0.0)
      {
        nion = (edep - niel) / curMeanEnergy;
      }
    }
  }
  return nion;
...
std::vector<G4ThreeVector>* G4ElectronIonPair::SampleIonsAlongStep(const G4Step* step)
{
  ...
  if (nion > 0)
  {
    ...
    if (verbose > 1)
    {
      G4cout << "### G4ElectronIonPair::SampleIonisationPoints: " << v->size()
             << "  ion pairs are added" << G4endl;
    }
  }
  return v;
}
...
G4int G4ElectronIonPair::ResidualeChargePostStep(const G4ParticleDefinition*, const G4TrackVector*,
                                                 G4int subType) const
{
  G4int nholes = 0;

  if (2 == subType || 12 == subType || 13 == subType)
  {
    nholes = 1;
  }
  return nholes;
...
G4double G4ElectronIonPair::FindG4MeanEnergyPerIonPair(const G4Material* mat) const
{
  ...
  for (G4int j = 0; j < nMaterials; ++j)
  {
    if (name == g4MatNames[j])
    {
      ...
      if (verbose > 0)
      {
        G4cout << "### G4ElectronIonPair::FindG4MeanEnergyPerIonPair for " << name
               << " Epair= " << res / eV << " eV is set" << G4endl;
      }
      break;
    }
  }
...
void G4ElectronIonPair::DumpMeanEnergyPerIonPair() const
{
  ...
  if (nmat > 0)
  {
    G4cout << "### G4ElectronIonPair: mean energy per ion pair available:" << G4endl;
    for (std::size_t i = 0; i < nmat; ++i)
    {
      ...
      if (x > 0.0)
      {
        G4cout << "   " << mat->GetName() << "   Epair=  " << x / eV << " eV" << G4endl;
      }
    }
  }
}
...
void G4ElectronIonPair::DumpG4MeanEnergyPerIonPair() const
{
  if (nMaterials > 0)
  {
    ...
    {
      G4cout << "   " << g4MatNames[i] << "    Epair= " << g4MatData[i] / eV << " eV" << G4endl;
    }
  }
}
