...
G4EmMultiModel::G4EmMultiModel(const G4String& nam) : G4VEmModel(nam)
{
  model.clear();
  cross_section.clear();
}
...
void G4EmMultiModel::Initialise(const G4ParticleDefinition* p, const G4DataVector& cuts)
{
  G4EmParameters* param = G4EmParameters::Instance();
  G4int verb = IsMaster() ? param->Verbose() : param->WorkerVerbose();
  if (verb > 0)
  {
    G4cout << "### Initialisation of EM MultiModel " << GetName() << " including following list of "
           << nModels << " models:" << G4endl;
  }
...
G4double G4EmMultiModel::ComputeDEDXPerVolume(const G4Material* mat, const G4ParticleDefinition* p,
...
G4double G4EmMultiModel::ComputeCrossSectionPerAtom(const G4ParticleDefinition* p,
                                                    G4double kinEnergy, G4double Z, G4double A,
                                                    G4double cutEnergy, G4double maxEnergy)
{
  ...
  for (G4int i = 0; i < nModels; ++i)
  {
    ...
    cross += (model[i])->ComputeCrossSectionPerAtom(p, kinEnergy, Z, A, cutEnergy, maxEnergy);
  }
  return cross;
}
                                       ...
                                       G4double maxEnergy)
{
  ...
  if (nModels > 0)
  {
    ...
    for (i = 0; i < nModels; ++i)
    {
      cross += (model[i])->CrossSection(couple, dp->GetParticleDefinition(), dp->GetKineticEnergy(),
                                        minEnergy, maxEnergy);
