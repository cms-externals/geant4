...
G4EmElementSelector::G4EmElementSelector(G4VEmModel* mod, const G4Material* mat, G4int bins,
                                         G4double emin, G4double emax, G4bool)
  : model(mod), material(mat), nbins(bins), cutEnergy(-1.0), lowEnergy(emin), highEnergy(emax)
{
...
void G4EmElementSelector::Initialise(const G4ParticleDefinition* part, G4double cut)
{
  // G4cout << "G4EmElementSelector initialise for " << material->GetName()
  //   << G4endl;
  if (0 == nElmMinusOne || cut == cutEnergy)
  {
    return;
  }
  ...
  for (G4int j = 0; j <= nbins; ++j)
  {
    ...
    for (G4int i = 0; i <= nElmMinusOne; ++i)
    {
      cross += theAtomNumDensityVector[i]
               * model->ComputeCrossSectionPerAtom(part, (*theElementVector)[i], e, cutEnergy, e);
...
const G4Element* G4EmElementSelector::SelectRandomAtom(const G4double e, const G4double loge) const
{
...
void G4EmElementSelector::Dump(const G4ParticleDefinition* part)
{
  ...
  G4cout << "Last Element in element vector " << (*theElementVector)[nElmMinusOne]->GetName()
         << G4endl;
  G4cout << G4endl;
}
