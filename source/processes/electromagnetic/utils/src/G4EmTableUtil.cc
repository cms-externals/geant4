...
const G4DataVector*
G4EmTableUtil::PrepareEmProcess(G4VEmProcess* proc, const G4ParticleDefinition* part,
                                const G4ParticleDefinition* secPart, G4EmModelManager* modelManager,
                                const G4double& maxKinEnergy, G4int& secID, G4int& tripletID,
                                G4int& mainSec, const G4int& verb, const G4bool& master)
{
  ...
  if (1 < verb)
  {
    G4cout << "### G4EmTableUtil::PreparePhysicsTable() done for " << proc->GetProcessName()
...
void G4EmTableUtil::BuildEmProcess(G4VEmProcess* proc, const G4VEmProcess* masterProc,
                                   const G4ParticleDefinition* firstPart,
                                   const G4ParticleDefinition* part, const G4int nModels,
                                   const G4int verb, const G4bool master, const G4bool isLocked,
                                   const G4bool toBuild, G4bool& baseMat)
{
  G4String num = part->GetParticleName();
  if (1 < verb)
  {
    G4cout << "### G4EmTableUtil::BuildPhysicsTable() for " << proc->GetProcessName()
           << " and particle " << num << " buildLambdaTable=" << toBuild << " master= " << master
           << G4endl;
  }
  ...
  if (firstPart == part)
  {
    ...
    else
    {
      ...
      if (fXSType == fEmOnePeak)
      {
        ...
        if (nullptr == v)
        {
          proc->SetCrossSectionType(fEmIncreasing);
        }
      }
  ...
  if (1 < verb
      || (0 < verb
          && (num == "gamma" || num == "e-" || num == "e+" || num == "mu+" || num == "mu-"
              || num == "proton" || num == "pi+" || num == "pi-" || num == "kaon+" || num == "kaon-"
              || num == "alpha" || num == "anti_proton" || num == "GenericIon" || num == "alpha+"
              || num == "helium" || num == "hydrogen")))
  {
  ...
  if (1 < verb)
  {
    G4cout << "### G4EmTableUtil::BuildPhysicsTable() done for " << proc->GetProcessName()
           << " and particle " << num << " baseMat=" << baseMat << G4endl;
  }
}

//....oooOO0OOooo........oooOO0OOooo........oooOO0OOooo........oooOO0OOooo.....

void G4EmTableUtil::BuildLambdaTable(G4VEmProcess* proc, const G4ParticleDefinition* part,
                                     G4EmModelManager* modelManager, G4PhysicsTable* theLambdaTable,
                                     G4PhysicsTable* theLambdaTablePrim,
                                     const G4double minKinEnergy, const G4double minKinEnergyPrim,
                                     const G4double maxKinEnergy, const G4double scale,
                                     const G4int verboseLevel, const G4bool startFromNull,
                                     const G4bool splineFlag)
{
  if (1 < verboseLevel)
  {
    G4cout << "G4EmTableUtil::BuildLambdaTable() for process " << proc->GetProcessName()
           << " and particle " << part->GetParticleName() << G4endl;
  }
  ...
  G4EmDataRegistry* theRegistry = G4EmDataRegistry::Instance();
    ...
    if (theRegistry->GetFlag(i))
    {
      ...
      if (nullptr != theLambdaTablePrim && minKinEnergyPrim < maxKinEnergy)
      {
        ...
        if (nullptr == bVectorPrim)
        {
          ...
          bin = std::max(bin, 5);
          aVectorPrim = new G4PhysicsLogVector(minKinEnergyPrim, maxKinEnergy, bin, true);
        ...
        modelManager->FillLambdaVector(aVectorPrim, couple, false, fIsCrossSectionPrim);
...
void G4EmTableUtil::BuildLambdaTable(G4VEnergyLossProcess* proc, const G4ParticleDefinition* part,
                                     G4EmModelManager* modelManager, G4PhysicsTable* theLambdaTable,
                                     const G4DataVector* theCuts, const G4double minKinEnergy,
                                     const G4double maxKinEnergy, const G4double scale,
                                     const G4int verboseLevel, const G4bool splineFlag)
{
  if (1 < verboseLevel)
  {
    G4cout << "G4EmTableUtil::BuildLambdaTable() for process " << proc->GetProcessName()
  ...
  for (std::size_t i = 0; i < numOfCouples; ++i)
  {
    if (theRegistry->GetFlag(i))
    {
      ...
      G4double emin = proc->MinPrimaryEnergy(part, couple->GetMaterial(), (*theCuts)[i]);
...
G4bool G4EmTableUtil::IsIon(const G4ParticleDefinition* part)
{
  G4bool isIon = false;
  if (part->GetBaryonNumber() < 1 || 0.0 >= part->GetPDGCharge())
  {
    return isIon;
  }
  else if (part->GetParticleType() == "nucleus")
  {
    isIon = true;
    G4int pdg = part->GetPDGEncoding();
    if (pdg == 1000010020 || pdg == 1000010030 || pdg == 1000020030 || pdg == 1000020040)
    {
      isIon = false;
    }
  }
...
void G4EmTableUtil::UpdateModels(G4VEnergyLossProcess* proc, G4EmModelManager* modelManager,
                                 const G4double maxKinEnergy, const G4int nModels, G4int& secID,
                                 G4int& biasID, G4int& mainSec, const G4bool baseMat, const G4bool,
...
void G4EmTableUtil::BuildDEDXTable(G4VEnergyLossProcess* proc, const G4ParticleDefinition* part,
                                   G4EmModelManager* modelManager, G4PhysicsTable* table,
                                   const G4double emin, const G4double emax, const G4int nbins,
                                   const G4int verbose, const G4EmTableType tType,
                                   const G4bool spline)
{
  // Access to materials
  const G4ProductionCutsTable* theCoupleTable = G4ProductionCutsTable::GetProductionCutsTable();
  ...
  if (1 < verbose)
  {
    G4cout << "G4EmTableUtil::BuildDEDXTable " << numOfCouples << " couples"
           << " minKinEnergy(MeV)= " << emin << " maxKinEnergy(MeV)= " << emax
           << " nbins= " << nbins << G4endl;
  }
  G4PhysicsLogVector* aVector = nullptr;
  ...
  for (std::size_t i = 0; i < numOfCouples; ++i)
  {
    if (1 < verbose)
    {
      G4cout << "  " << i << ". flagTable=" << table->GetFlag(i)
             << " flagBuilder=" << theRegistry->GetFlag(i) << G4endl;
    }
    if (theRegistry->GetFlag(i))
    {
      // create physics vector and fill it
      auto const couple = theCoupleTable->GetMaterialCutsCouple((G4int)i);
      delete (*table)[i];
      if (nullptr != bVector)
      {
  ...
  if (1 < verbose)
  {
    G4cout << "G4EmTableUtil::BuildDEDXTable(): table is built for " << part->GetParticleName()
           << " and process " << proc->GetProcessName() << G4endl;
...
void G4EmTableUtil::PrepareMscProcess(G4VMultipleScattering* proc, const G4ParticleDefinition* part,
                                      G4EmModelManager* modelManager, G4MscStepLimitType& stepLimit,
                                      G4double& facrange, G4bool& latDisplacement, G4bool& isIon,
                                      const G4int verb)
{
  auto param = G4EmParameters::Instance();
  G4bool baseMat = G4EmDataRegistry::Instance()->GetBaseMaterialFlag();
  proc->SetVerboseLevel(verb);
  isIon = IsIon(part);

  if (1 < verb)
  {
    G4cout << "### G4EmTableUtil::PrepearPhysicsTable() for " << proc->GetProcessName()
           << " and particle " << part->GetParticleName() << " isIon: " << isIon << G4endl;
  }
  ...
  // initialisation of models
  G4int numberOfModels = (G4int)modelManager->NumberOfModels();
...
void G4EmTableUtil::BuildMscProcess(G4VMultipleScattering* proc,
                                    ...
                                    const G4ParticleDefinition* firstPart, const G4int nModels,
                                    const G4int verb, const G4bool master)
{
  auto param = G4EmParameters::Instance();
  ...
  {
    G4EmDataRegistry* theRegistry = G4EmDataRegistry::Instance();
    auto bld = theRegistry->GetLossTableBuilder();
    G4bool baseMat = theRegistry->GetBaseMaterialFlag();
    if (master)
    {
      for (G4int i = 0; i < nModels; ++i)
      {
        ...
        {
          G4double emin = std::max(msc->LowEnergyLimit(), msc->LowEnergyActivationLimit());
          G4double emax = std::min(msc->HighEnergyLimit(), msc->HighEnergyActivationLimit());
          emin = std::max(emin, param->MinKinEnergy());
          emax = std::min(emax, param->MaxKinEnergy());
          if (emin < emax)
          {
            auto table =
              bld->BuildTableForModel(msc->GetCrossSectionTable(), msc, &part, emin, emax, true);
    ...
    if (1 < verb
        || (0 < verb
            && (num == "e-" || num == "e+" || num == "mu+" || num == "mu-" || num == "proton"
                || num == "pi+" || num == "pi-" || num == "kaon+" || num == "kaon-"
                || num == "alpha" || num == "anti_proton" || num == "GenericIon" || num == "alpha+"
                || num == "alpha")))
    {
      proc->StreamInfo(G4cout, part);
...
G4bool G4EmTableUtil::StoreMscTable(G4VMultipleScattering* proc, const G4ParticleDefinition* part,
                                    const G4String& dir, const G4int nModels, const G4int verb,
                                    const G4bool ascii)
{
  G4bool ok = true;
  for (G4int i = 0; i < nModels; ++i)
  {
    ...
    if (nullptr != table)
    {
      ...
      G4String name = proc->GetPhysicsTableFileName(part, dir, "LambdaMod" + ss, ascii);
      ...
      if (yes)
      {
        if (verb > 0)
        {
          G4cout << "Physics table are stored for " << part->GetParticleName() << " and process "
                 << proc->GetProcessName() << " with a name <" << name << "> " << G4endl;
        }
      }
      else
      {
        G4cout << "Fail to store Physics Table for " << part->GetParticleName() << " and process "
               << proc->GetProcessName() << " in the directory <" << dir << "> " << G4endl;
        ok = false;
      }
    }
  }
...
G4bool G4EmTableUtil::StoreTable(G4VProcess* ptr, const G4ParticleDefinition* part,
                                 G4PhysicsTable* aTable, const G4String& dir, const G4String& tname,
                                 const G4int verb, const G4bool ascii)
{
  G4bool res = true;
  if (nullptr != aTable)
  {
    const G4String& name = ptr->GetPhysicsTableFileName(part, dir, tname, ascii);
...
G4bool G4EmTableUtil::RetrieveTable(G4VProcess* ptr, const G4ParticleDefinition* part,
                                    G4PhysicsTable* aTable, const G4String& dir,
                                    const G4String& tname, const G4int verb, const G4bool ascii,
                                    const G4bool spline)
{
  G4bool res = true;
  if (nullptr == aTable)
  ...
  if (1 < verb)
  {
    G4cout << tname << " table for " << part->GetParticleName() << " will be retrieved " << G4endl;
  }
  const G4String& name = ptr->GetPhysicsTableFileName(part, dir, tname, ascii);
  if (G4PhysicsTableHelper::RetrievePhysicsTable(aTable, name, ascii, spline))
  {
    ...
    if (0 < verb)
    {
      G4cout << tname << " table for " << part->GetParticleName() << " is retrieved from <" << name
             << ">" << G4endl;
    }
  }
  else
  {
    res = false;
    G4cout << "G4EmTableUtil::RetrieveTable fail to retrieve: " << tname << " from " << name
