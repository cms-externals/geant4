...
G4bool G4EmTableUtil::IsIon(const G4ParticleDefinition* part)
{
  ...
  if (part->GetBaryonNumber() < 1 || 0.0 >= part->GetPDGCharge()) {
    ...
    if (pdg == 1000010020 || pdg == 1000010030 ||
        pdg == 1000020030 || pdg == 1000020040) { isIon = false; }
  }
  return isIon;
}
...
void G4EmTableUtil::WorkerElossProcess(G4VEnergyLossProcess* proc,
                                       ...
                                       const G4int nModels)
{
  ...
  if (proc->IsIonisationProcess()) {
    proc->SetDEDXTable(masterProc->DEDXTable(),fRestricted, false);
    proc->SetDEDXTable(masterProc->DEDXunRestrictedTable(),fTotal, false);
  ...
  proc->SetDEDXTable(masterProc->IonisationTable(),fIsIonisation, false);
  proc->SetLambdaTable(masterProc->LambdaTable(), false);
  proc->SetCrossSectionType(masterProc->CrossSectionType());
  proc->SetEnergyOfCrossSectionMax(masterProc->EnergyOfCrossSectionMax());
                                   ...
                                   const G4bool spline)
{
  ...
  if (1 < verbose) {
    G4cout << "G4EmTableUtil::BuildDEDXTable "
           << numOfCouples << " couples" << " minKinEnergy(MeV)= " << emin
