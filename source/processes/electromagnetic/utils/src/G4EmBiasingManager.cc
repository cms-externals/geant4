...
G4EmBiasingManager::G4EmBiasingManager() : fDirectionalSplittingTarget(0.0, 0.0, 0.0)
{
  fSafetyMin = 1.e-6 * mm;
...
void G4EmBiasingManager::Initialise(const G4ParticleDefinition& part, const G4String& procName,
                                    G4int verbose)
{
  ...
  if (0 < nForcedRegions)
  {
    idxForcedCouple.resize(numOfCouples, -1);
  }
  if (0 < nSecBiasedRegions)
  {
    idxSecBiasedCouple.resize(numOfCouples, -1);
  }
  ...
  {
    G4cout << " Forced Interaction is activated for " << part.GetParticleName() << " and "
           << procName << " inside G4Regions: " << G4endl;
    for (G4int i = 0; i < nForcedRegions; ++i)
    {
  ...
  {
    G4cout << " Secondary biasing is activated for " << part.GetParticleName() << " and "
           << procName << " inside G4Regions: " << G4endl;
    for (G4int i = 0; i < nSecBiasedRegions; ++i)
    {
      ...
      if (r)
      {
        G4cout << "           " << r->GetName() << "  BiasingWeight= " << secBiasedWeight[i]
               << G4endl;
      }
    }
    if (fDirectionalSplitting)
    {
      G4cout << "     Directional splitting activated, with target position: "
             ...
             << " cm; radius: " << fDirectionalSplittingRadius / cm << "cm." << G4endl;
    }
  }
}
...
void G4EmBiasingManager::ActivateForcedInteraction(G4double val, const G4String& rname)
{
  ...
  if (!reg)
  {
    G4cout << "### G4EmBiasingManager::ForcedInteraction WARNING: "
           << " G4Region <" << rname << "> is unknown" << G4endl;
    return;
  }
  ...
  if (val < 0.0)
  {
    G4cout << "### G4EmBiasingManager::ForcedInteraction WARNING: " << val
           << " < 0.0, so no activation for the G4Region <" << rname << ">" << G4endl;
    return;
  }
...
void G4EmBiasingManager::ActivateSecondaryBiasing(const G4String& rname, G4double factor,
                                                  G4double energyLimit)
{
  ...
  if (!reg)
  {
    G4cout << "### G4EmBiasingManager::ActivateBremsstrahlungSplitting "
           << "WARNING: G4Region <" << rname << "> is unknown" << G4endl;
    return;
  }
...
G4double G4EmBiasingManager::GetStepLimit(G4int coupleIdx, G4double previousStep)
{
  if (startTracking)
  {
    startTracking = false;
    ...
    else
    {
      ...
      if (currentStepLimit > 0.0)
      {
        currentStepLimit *= G4UniformRand();
      }
    }
  }
  else
...
G4double G4EmBiasingManager::ApplySecondaryBiasing(std::vector<G4DynamicParticle*>& vd,
                                                   const G4Track& track, G4VEmModel* currentModel,
                                                   G4ParticleChangeForLoss* pPartChange,
                                                   G4double& eloss, G4int coupleIdx, G4double tcut,
                                                   G4double safety)
{
  ...
  if (0 <= index)
  {
    ...
    if ((0 < n && vd[0]->GetKineticEnergy() < secBiasedEnegryLimit[index]) || fDirectionalSplitting)
    {
  ...
  if (0 <= index)
  {
    ...
    if ((0 < n && vd[0]->GetKineticEnergy() < secBiasedEnegryLimit[index]) || fDirectionalSplitting)
    {
      ...
      else
      {
        if (fDirectionalSplitting)
        {
          weight = ApplyDirectionalSplitting(vd, track, currentModel, index, tcut, pPartChange);
        }
        else
        {
          G4double tmpEnergy = pPartChange->GetProposedKineticEnergy();
...
G4double G4EmBiasingManager::ApplySecondaryBiasing(std::vector<G4Track*>& track, G4int coupleIdx)
{
  G4int index = idxSecBiasedCouple[coupleIdx];
  G4double weight = 1.;
  if (0 <= index)
...
void G4EmBiasingManager::ApplyRangeCut(std::vector<G4DynamicParticle*>& vd, const G4Track& track,
...
G4bool G4EmBiasingManager::CheckDirection(G4ThreeVector pos, G4ThreeVector momdir) const
{
...
G4double G4EmBiasingManager::ApplyDirectionalSplitting(std::vector<G4DynamicParticle*>& vd,
                                                       const G4Track& track,
                                                       G4VEmModel* currentModel, G4int index,
                                                       G4double tcut,
                                                       G4ParticleChangeForGamma* partChange)
{
  ...
  if (1 < nsplit && trackWeight > w)
  {
    ...
    {
      if (k > 0)
      {  // for k==0, SampleSecondaries has already been called
        tmpSecondaries.clear();
        // SampleSecondaries modifies primary info stored in partChange
        currentModel->SampleSecondaries(&tmpSecondaries, track.GetMaterialCutsCouple(),
      ...
      for (std::size_t kk = 0; kk < tmpSecondaries.size(); ++kk)
      {
        ...
        {  // keep charged 2ry from first splitting
          vd.push_back(tmpSecondaries[kk]);
          fDirectionalSplittingWeights.push_back(1. / weight);
        }
        else
      ...
      {  // don't add if kinetic energy = 0
        ...
        if (CheckDirection(pos, momdir))
        {
          ...
          {
            auto dp = new G4DynamicParticle(theGamma, partChange->GetProposedMomentumDirection(),
                                                       ...
                                                       G4double tcut)
{
  ...
  {  // no splitting was done; still need weights
    for (std::size_t i = 0; i < vd.size(); ++i)
    {
