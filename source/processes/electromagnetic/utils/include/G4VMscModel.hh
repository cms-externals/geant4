...
// -------------------------------------------------------------------
//
#ifndef G4VMSCMODEL_HH
#define G4VMSCMODEL_HH
...
class G4VMscModel : public G4VEmModel
{
  public:
    ...
    inline void SetIonisation(G4VEnergyLossProcess*, const G4ParticleDefinition* part);
  ...
  public:
    ...
    inline G4double ComputeGeomLimit(const G4Track&, G4double& presafety, G4double limit);

    G4double GetDEDX(const G4ParticleDefinition* part, G4double kineticEnergy,
                     const G4MaterialCutsCouple* couple);

    G4double GetDEDX(const G4ParticleDefinition* part, G4double kineticEnergy,
                     const G4MaterialCutsCouple* couple, G4double logKineticEnergy);

    G4double GetRange(const G4ParticleDefinition* part, G4double kineticEnergy,
                      const G4MaterialCutsCouple* couple);

    G4double GetRange(const G4ParticleDefinition* part, G4double kineticEnergy,
                      const G4MaterialCutsCouple* couple, G4double logKineticEnergy);

    G4double GetEnergy(const G4ParticleDefinition* part, G4double range,
                       const G4MaterialCutsCouple* couple);

    // G4MaterialCutsCouple should be defined before call to this method
    inline G4double GetTransportMeanFreePath(const G4ParticleDefinition* part,
                                             const G4double kinEnergy);

    inline G4double GetTransportMeanFreePath(const G4ParticleDefinition* part,
                                             const G4double kinEnergy, const G4double logKinEnergy);

    //  hide assignment operator
    G4VMscModel& operator=(const G4VMscModel&) = delete;
...
inline G4double G4VMscModel::ComputeSafety(const G4ThreeVector& position, G4double limit)
{
...
inline G4double G4VMscModel::ConvertTrueToGeom(G4double& tlength, G4double& glength)
{
...
inline G4double G4VMscModel::ComputeGeomLimit(const G4Track& track, G4double& presafety,
                                              G4double limit)
{
  return safetyHelper->CheckNextStep(track.GetStep()->GetPreStepPoint()->GetPosition(),
...
inline void G4VMscModel::SetIonisation(G4VEnergyLossProcess* p, const G4ParticleDefinition* part)
{
  ionisation = p;
  currentPart = part;
}
...
inline G4double G4VMscModel::GetTransportMeanFreePath(const G4ParticleDefinition* part,
                                                      const G4double ekin)
{
  G4double x;
...
inline G4double G4VMscModel::GetTransportMeanFreePath(const G4ParticleDefinition* part,
                                                      const G4double ek, const G4double logek)
{
  G4double x;
  if (nullptr != xSectionTable)
  {
    x = pFactor * (*xSectionTable)[basedCoupleIndex]->LogVectorValue(ek, logek) / (ek * ek);
  }
  else
  {
    x = pFactor * CrossSectionPerVolume(pBaseMaterial, part, ek, 0.0, DBL_MAX);
  }
  return (x > 0.0) ? 1.0 / x : DBL_MAX;
}
