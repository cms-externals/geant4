...
public:
  ...
  void WorkerPhysicsTables(const G4ParticleDefinition* aParticle, 
                           G4VEnergyLossProcess* p);

  void BuildTables(const G4ParticleDefinition* part, G4bool);
  ...
  inline G4int ThreadID() const;

  G4VEnergyLossProcess* GetEnergyLossProcess(const G4ParticleDefinition*);
 
  const std::vector<G4VEnergyLossProcess*>&
  GetEnergyLossProcessVector() const;

  const std::vector<G4VEmProcess*>& GetEmProcessVector() const;

  const std::vector<G4VMultipleScattering*>&
  GetMultipleScatteringVector() const;

  const std::vector<const G4ParticleDefinition*>& GetParticleVector() const;
...
private:
  ...
  static std::vector<PD> part_vector;
  static std::vector<PD> base_part_vector;
...
inline G4int G4LossTableManager::ThreadID() const
{
  return threadID;
}
