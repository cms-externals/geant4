...
// ********************************************************************
//
#ifndef G4ELECTRONIONPAIR_HH
#define G4ELECTRONIONPAIR_HH
...
class G4ElectronIonPair
{
  public:
    ...
    G4double MeanNumberOfIonsAlongStep(const G4ParticleDefinition*, const G4Material*,
                                       G4double edepTotal, G4double edepNIEL = 0.0);
...
inline G4double G4ElectronIonPair::MeanNumberOfIonsAlongStep(const G4Step* step)
{
  return MeanNumberOfIonsAlongStep(
    step->GetTrack()->GetParticleDefinition(), step->GetPreStepPoint()->GetMaterial(),
...
inline G4int G4ElectronIonPair::ResidualeChargePostStep(const G4Step* step) const
{
  G4int subtype = -1;
  ...
  return ResidualeChargePostStep(step->GetTrack()->GetParticleDefinition(), step->GetSecondary(),
                                 subtype);
}
