...
#ifndef G4VATOMDEEXCITATION_HH
#define G4VATOMDEEXCITATION_HH
...
class G4VAtomDeexcitation
{
  public:
    ...
    void SetDeexcitationActiveRegion(const G4String& rname, G4bool valDeexcitation, G4bool valAuger,
                                     G4bool valPIXE);
    ...
    void GenerateParticles(std::vector<G4DynamicParticle*>* secVect, const G4AtomicShell*, G4int Z,
                           G4int coupleIndex);

    // generation of deexcitation for given atom and shell vacancy
    virtual void GenerateParticles(std::vector<G4DynamicParticle*>* secVect, const G4AtomicShell*,
                                   G4int Z, G4double gammaCut, G4double eCut) = 0;

    // access or compute PIXE cross section
    virtual G4double GetShellIonisationCrossSectionPerAtom(const G4ParticleDefinition*, G4int Z,
                                                           G4AtomicShellEnumerator shell,
                                                           G4double kinE,
                                                           const G4Material* mat = nullptr) = 0;

    // access or compute PIXE cross section
    virtual G4double ComputeShellIonisationCrossSectionPerAtom(const G4ParticleDefinition*, G4int Z,
                                                               G4AtomicShellEnumerator shell,
                                                               G4double kinE,
                                                               const G4Material* mat = nullptr) = 0;

    // Sampling of PIXE for ionisation processes
    void AlongStepDeexcitation(std::vector<G4Track*>& tracks, const G4Step& step, G4double& eLoss,
                               G4int coupleIndex);
...
inline const std::vector<G4bool>& G4VAtomDeexcitation::GetListOfActiveAtoms() const
{
  return activeZ;
}
...
inline G4bool G4VAtomDeexcitation::CheckDeexcitationActiveRegion(G4int idx)
{
  return (idx < nCouples) ? activeDeexcitationMedia[idx] : false;
}

inline G4bool G4VAtomDeexcitation::CheckAugerActiveRegion(G4int idx)
{
