...
G4Step * GenerateStep(const struct Options & options)
{
 ...
 G4DynamicParticle * dynamicElectron(new G4DynamicParticle(GetSelectedParticle(options), momentumDirection, G4Exp(lnEnergyMin+(lnEnergyMax-lnEnergyMin)*G4UniformRand())));
 ...
 while (step>0)
 {
  ...
  while (iteration>0)
  {
   ...
   while (n>0)
   {
    ...
    iTuple->fill(iTuple->findColumn(strBuffer), G4Log(aSecParticle->GetKineticEnergy()/eV)/g4pow->logZ(10));
