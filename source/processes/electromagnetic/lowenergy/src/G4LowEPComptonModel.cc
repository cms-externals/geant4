...
namespace
{
G4Mutex LowEPComptonModelMutex = G4MUTEX_INITIALIZER;
}
...
G4LowEPComptonModel::G4LowEPComptonModel(const G4ParticleDefinition*, const G4String& nam)
...
void G4LowEPComptonModel::InitialiseLocal(const G4ParticleDefinition*, G4VEmModel* masterModel)
{
...
void G4LowEPComptonModel::ReadData(std::size_t Z, const char* path)
{
  if (verboseLevel > 1)
  {
    G4cout << "G4LowEPComptonModel::ReadData()" << G4endl;
  }
  if (data[Z])
  {
    return;
  ...
  if (!datadir)
  {
    ...
    if (!datadir)
    {
      G4Exception("G4LowEPComptonModel::ReadData()", "em0006", FatalException,
  ...
  if (!fin.is_open())
  {
    ...
    ed << "G4LowEPComptonModel data file <" << ost.str().c_str() << "> is not opened!" << G4endl;
    G4Exception("G4LowEPComptonModel::ReadData()", "em0003", FatalException, ed,
  ...
  else
  {
    if (verboseLevel > 3)
    {
      G4cout << "File " << ost.str() << " is opened by G4LowEPComptonModel" << G4endl;
    }
                                                         ...
                                                         G4double, G4double)
{
  if (verboseLevel > 3)
  {
    G4cout << "G4LowEPComptonModel::ComputeCrossSectionPerAtom()" << G4endl;
  }
  G4double cs = 0.0;
                                            ...
                                            G4double)
{
  ...
  if (verboseLevel > 3)
  {
    G4cout << "G4LowEPComptonModel::SampleSecondaries() E(MeV)= " << photonEnergy0 / MeV << " in "
  ...
  if (bindingE < 0.0)
    G4Exception("G4LowEPComptonModel::SampleSecondaries()", "em2051", FatalException,
...
G4double G4LowEPComptonModel::ComputeScatteringFunction(G4double x, G4int Z)
{
  G4double value = Z;
...
void G4LowEPComptonModel::InitialiseForElement(const G4ParticleDefinition*, G4int Z)
{
