...
namespace
{
G4Mutex LivermorePolarizedComptonModelMutex = G4MUTEX_INITIALIZER;
}
  ...
  : G4VEmModel(nam), isInitialised(false)
{
  ...
  if (verboseLevel > 1) G4cout << "Livermore Polarized Compton is constructed " << G4endl;
...
void G4LivermorePolarizedComptonModel::ReadData(std::size_t Z, const char* path)
{
  if (verboseLevel > 1)
  {
    G4cout << "G4LivermorePolarizedComptonModel::ReadData()" << G4endl;
  }
  if (data[Z])
  {
    return;
  ...
  if (!datadir)
  {
    ...
    if (!datadir)
    {
      G4Exception("G4LivermorePolarizedComptonModel::ReadData()", "em0006", FatalException,
  ...
  if (!fin.is_open())
  {
    ...
    ed << "G4LivermorePolarizedComptonModel data file <" << ost.str().c_str() << "> is not opened!"
       << G4endl;
  ...
  else
  {
    if (verboseLevel > 3)
    {
      G4cout << "File " << ost.str() << " is opened by G4LivermorePolarizedComptonModel" << G4endl;
    }
                                                         ...
                                                         G4double, G4double)
{
  ...
  if (gammaEnergy0 < LowEnergyLimit()) return;
  ...
  do
  {
    ...
    if (sinThetaSqr > 1.)
    {
      G4cout << " -- Warning -- G4LivermorePolarizedComptonModel::SampleSecondaries "
             << "sin(theta)**2 = " << sinThetaSqr << "; set to 1" << G4endl;
      sinThetaSqr = 1.;
    }
    if (sinThetaSqr < 0.)
    {
      G4cout << " -- Warning -- G4LivermorePolarizedComptonModel::SampleSecondaries "
             << "sin(theta)**2 = " << sinThetaSqr << "; set to 0" << G4endl;
      sinThetaSqr = 0.;
    }
    // End protection
  ...
  {
    G4cout << " -- Warning -- G4LivermorePolarizedComptonModel::SampleSecondaries "
           << "cosTheta = " << cosTheta << "; set to 1" << G4endl;
    cosTheta = 1.;
  }
  if (cosTheta < -1.)
  {
  ...
  {
    G4cout << " -- Warning -- G4LivermorePolarizedComptonModel::SampleSecondaries "
           << "sinTheta = " << sinTheta << "; set to 1" << G4endl;
    sinTheta = 1.;
  }
  if (sinTheta < -1.)
  {
      ...
      if (clipBoard)
      {
        ...
        if (clipBoard->IsTrack1Measurement())
        {
          ...
          if (clipBoard->GetTrackB() == fParticleChange->GetCurrentTrack())
          {
            ...
            //            << ", Name: " <<
            //            clipBoard->GetParentParticleDefinition()->GetParticleName()
            //            << G4endl;
            //            // End debug
  ...
  //
  // New polarization
  G4ThreeVector gammaPolarization1 = SetNewPolarization(epsilon, sinThetaSqr, phi, cosTheta);
  ...
  G4ThreeVector ElecDirection((gammaEnergy0 * gammaDirection0 - gammaEnergy1 * gammaDirection1)
                              * (1. / ElecMomentum));
  ...
  if (bindingE < 0.0)
    G4Exception("G4LivermoreComptonModel::SampleSecondaries()", "em2050", FatalException,
...
G4double G4LivermorePolarizedComptonModel::SetPhi(G4double energyRate, G4double sinSqrTh)
{
  G4double rand1;
  G4double rand2;
...
G4ThreeVector G4LivermorePolarizedComptonModel::SetNewPolarization(G4double epsilon,
                                                                   G4double sinSqrTh, G4double phi,
                                                                   G4double costheta)
{
  G4double rand1;
  G4double rand2;
  ...
  if (rand1 < (epsilon + 1.0 / epsilon - 2)
                / (2.0 * (epsilon + 1.0 / epsilon) - 4.0 * sinSqrTh * cosSqrPhi))
  {
    if (rand2 < 0.5)
      theta = pi / 2.0;
    else
