                                            ...
                                            const G4DataVector& cuts)
{
  ...
  {
    G4Exception("G4LivermoreIonisationModel::Initialise", "em0002", FatalException,
  ...
  if (verboseLevel > 3) G4cout << "G4VEnergySpectrum is initialized" << G4endl;
  ...
  crossSectionHandler =
    new G4eIonisationCrossSectionHandler(energySpectrum, interpolation, emin, emax, nbins * ndec);
  crossSectionHandler->Clear();
  crossSectionHandler->LoadShellData("ioni/ion-ss-cs-");
  // This is used to retrieve cross section values later on
  G4VEMDataSet* emdata = crossSectionHandler->BuildMeanFreePathForMaterials(&cuts);
                                                                ...
                                                                G4double)
{
  ...
  if (!crossSectionHandler)
  {
    G4Exception("G4LivermoreIonisationModel::ComputeCrossSectionPerAtom", "em1007", FatalException,
  ...
  if (verboseLevel > 1)
  {
    ...
    G4cout << "Cross section for delta emission > " << cutEnergy / keV << " keV at " << energy / keV
           << " keV and Z = " << iZ << " --> " << cs / barn << " barn" << G4endl;
  }
  return cs;
}
                                                          ...
                                                          G4double cutEnergy)
{
  ...
  for (size_t iel = 0; iel < NumberOfElements; iel++)
  {
    ...
    {
      G4double e = energySpectrum->AverageEnergy(iZ, 0.0, cutEnergy, kineticEnergy, n);
  ...
  if (verboseLevel > 2)
  {
    ...
    G4cout << "Stopping power < " << cutEnergy / keV << " keV at " << kineticEnergy / keV
           << " keV = " << sPower / (keV / mm) << " keV/mm" << G4endl;
  }
                                                   ...
                                                   G4double cutE, G4double maxE)
{
  ...
  G4double energyMax = std::min(maxE, energySpectrum->MaxEnergyOfSecondaries(kineticEnergy));
  G4double energyDelta =
    energySpectrum->SampleEnergy(Z, cutE, energyMax, kineticEnergy, shellIndex);
  ...
  G4DynamicParticle* delta =
    new G4DynamicParticle(electron,
                          GetAngularDistribution()->SampleDirectionForShell(
                            aDynamicParticle, energyDelta, Z, shellIndex, couple->GetMaterial()),
                          energyDelta);
  ...
  {
    G4cout << "G4LivermoreIonisationModel: Negative energy deposit: " << theEnergyDeposit / eV
           << " eV" << G4endl;
  ...
  if (verboseLevel > 1)
  {
    ...
    G4cout << "Total final state: " << (finalKinEnergy + energyDelta + bindingEnergy) << " keV"
           << G4endl;
    G4cout << "-----------------------------------------------------------" << G4endl;
  }
  return;
