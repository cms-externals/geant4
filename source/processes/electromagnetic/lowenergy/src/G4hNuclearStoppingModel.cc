...
void G4hNuclearStoppingModel::InitializeMe()
{
  ...
  {
    G4cout << "G4hLowEnergyIonisation warning: There is no table with the modelName <" << modelName
           << ">"
           << " for nuclear stopping, <ICRU_R49> is applied " << G4endl;
    nStopingPowerTable = new G4hICRU49Nuclear();
  }
                                           ...
                                           const G4Material* material, G4double kineticEnergy)
{
  ...
  G4double nloss = StoppingPower(material, kineticEnergy, z1, m1) * theZieglerFactor;
...
G4double G4hNuclearStoppingModel::StoppingPower(const G4Material* material, G4double kineticEnergy,
                                                G4double z1, G4double m1) const
{
  ...
  for (std::size_t iel = 0; iel < NumberOfElements; ++iel)
  {
    ...
    nloss += (nStopingPowerTable->NuclearStoppingPower(kineticEnergy, z1, z2, m1, m2Local))
