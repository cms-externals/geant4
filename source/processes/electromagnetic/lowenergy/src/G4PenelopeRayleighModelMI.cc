...
{
  if (fVerboseLevel > 3) G4cout << "Calling G4PenelopeRayleighModelMI::Initialise()" << G4endl;
  ...
  if (IsMaster() && part == fParticle)
  {
    ...
    if (fIsMIActive)
    {
      if (!fKnownMaterials) fKnownMaterials = new std::map<G4String, G4String>;
      if (!fKnownMaterials->size()) LoadKnownMIFFMaterials();
      if (!fAngularFunction)
      {
    ...
    if (!fPMaxTable) fPMaxTable = new std::map<const G4Material*, G4PhysicsFreeVector*>;
    if (!fSamplingTable) fSamplingTable = new std::map<const G4Material*, G4PenelopeSamplingData*>;
    ...
    for (G4int i = 0; i < (G4int)theCoupleTable->GetTableSize(); ++i)
    {
      ...
      for (std::size_t j = 0; j < material->GetNumberOfElements(); ++j)
      {
        ...
        if (!fLogAtomicCrossSection[iZ]) ReadDataFile(iZ);
      }
      ...
      if (!fLogFormFactorTable->count(material)) BuildFormFactorTable(material);

      // 3) retrieve or build the sampling table
      if (!(fSamplingTable->count(material))) InitializeSamplingAlgorithm(material);

      // 4) retrieve or build the pMax data
      if (!fPMaxTable->count(material)) GetPMaxTable(material);
    }

    if (fVerboseLevel > 1)
    {
      G4cout << G4endl << "Penelope Rayleigh model v2008 is initialized" << G4endl
             << "Energy range: " << LowEnergyLimit() / keV << " keV - " << HighEnergyLimit() / GeV
             << " GeV" << G4endl;
    }
  }
                                                               ...
                                                               G4double, G4double, G4double)
{
  ...
  if (!fLogAtomicCrossSection[iZ])
  {
    ...
    if (fVerboseLevel > 0)
    {
      ...
      G4Exception("G4PenelopeRayleighModelMI::ComputeCrossSectionPerAtom()", "em2040", JustWarning,
                  ed);
    }
  ...
  if (!atom)
  {
    ...
    G4Exception("G4PenelopeRayleighModelMI::ComputeCrossSectionPerAtom()", "em2041", FatalException,
                ed);
    return 0;
  }
...
void G4PenelopeRayleighModelMI::CalculateThetaAndAngFun()
{
  ...
  for (G4int k = 0; k < fNtheta; k++)
  {
    ...
    if (fVerboseLevel > 3)
      G4cout << "theta[" << k << "]: " << fAngularFunction->Energy(k) << ", angFun[" << k
             << "]: " << (*fAngularFunction)[k] << G4endl;
  }
}
                                                          ...
                                                          G4double energy, G4double, G4double)
{
  ...
  if (G4ProductionCutsTable::GetProductionCutsTable()->GetTableSize() == 0 && !amInAUnitTest)
  {
    ...
    G4Exception("G4PenelopeRayleighModelMI::CrossSectionPerVolume()", "em2019", JustWarning, ed);
  }
  ...
  if (amInAUnitTest)
  {
    ...
    if (fIsMIActive) ReadMolInterferenceData(matname);
    if (!fLogFormFactorTable->count(material)) BuildFormFactorTable(material);
    if (!(fSamplingTable->count(material))) InitializeSamplingAlgorithm(material);
    if (!fPMaxTable->count(material)) GetPMaxTable(material);
  }
  G4bool useMIFF =
    fIsMIActive
    && (fMolInterferenceData->count(matname) || matname.find("MedMat") != std::string::npos);
  if (!useMIFF)
  {
    if (fVerboseLevel > 2)
  ...
  if (fVerboseLevel > 2)
    G4cout << "Rayleigh CS of: " << matname << " calculated through integration of the DCS"
           << G4endl;
  ...
  if (material->IsExtended() && material->GetName() != "CustomMat")
  {
    ...
    G4CrystalExtension* crystalExtension =
      (G4CrystalExtension*)extendedmaterial->RetrieveExtension("crystal");
    if (crystalExtension != 0)
    {
      G4cout << "The material has a crystalline structure, a dedicated diffraction model is used!"
             << G4endl;
      return 0;
    }
  }
  ...
  for (std::size_t i = 0; i < nElements; ++i)
  {
    if (MaxStoichiometricFactor > 0.) (*StoichiometricFactors)[i] /= MaxStoichiometricFactor;
  }
  ...
  for (G4int k = 0; k < fNtheta; k++)
  {
    G4double theta = fAngularFunction->Energy(k);  // the x-value is called "Energy", but is an
                                                   // angle
  ...
  // print CS and mfp
  if (fVerboseLevel > 2)
    G4cout << "Rayleigh CS of " << matname << " at " << energy / keV << " keV: " << cs / barn
           << " barn"
...
void G4PenelopeRayleighModelMI::BuildFormFactorTable(const G4Material* material)
{
  ...
  if (MaxStoichiometricFactor < 1e-16)
  {
    ...
    G4Exception("G4PenelopeRayleighModelMI::BuildFormFactorTable()", "em2042", FatalException, ed);
  }
  for (std::size_t i = 0; i < nElements; ++i)
  ...
  if (dataMI) aFileNameFF = dataMI->GetFilenameFF();
  ...
  else
  {
    ...
    {
      if (fVerboseLevel) G4cout << "Build MIFF from components for " << matname << G4endl;
      ...
      for (std::size_t j = 0; j < 4; ++j)
      {
        ...
        if (fVerboseLevel > 2) G4cout << " -- MedMat comp[" << j + 1 << "]: " << comp[j] << G4endl;
      }
      ...
      if (!path)
      {
        ...
        G4Exception("G4PenelopeRayleighModelMI::BuildFormFactorTable()", "em0006", FatalException,
                    excep);
      }

      if (!fMolInterferenceData->count("Fat_MI")) ReadMolInterferenceData("Fat_MI");
      G4PhysicsFreeVector* fatFF = fMolInterferenceData->find("Fat_MI")->second;

      if (!fMolInterferenceData->count("Water_MI")) ReadMolInterferenceData("Water_MI");
      G4PhysicsFreeVector* waterFF = fMolInterferenceData->find("Water_MI")->second;

      if (!fMolInterferenceData->count("BoneMatrix_MI")) ReadMolInterferenceData("BoneMatrix_MI");
      G4PhysicsFreeVector* bonematrixFF = fMolInterferenceData->find("BoneMatrix_MI")->second;

      if (!fMolInterferenceData->count("Mineral_MI")) ReadMolInterferenceData("Mineral_MI");
    ...
    {
      if (fVerboseLevel) G4cout << "Read MIFF from database " << matname << G4endl;
  ...
  if (fVerboseLevel > 3) DumpFormFactorTable(material);
  delete StoichiometricFactors;
                                                  ...
                                                  G4double)
{
  ...
  if (!fSamplingTable->count(theMat))
  {
    ...
    if (fVerboseLevel > 0)
    {
      // Issue a G4Exception (warning) only in verbose mode
      G4ExceptionDescription ed;
      ed << "Unable to find the fSamplingTable data for " << theMat->GetName() << G4endl;
      ed << "This can happen only in Unit Tests" << G4endl;
      G4Exception("G4PenelopeRayleighModelMI::SampleSecondaries()", "em2019", JustWarning, ed);
    }
    ...
    if (!fLogFormFactorTable->count(theMat)) BuildFormFactorTable(theMat);

    // 2) retrieve or build the sampling table
    if (!(fSamplingTable->count(theMat))) InitializeSamplingAlgorithm(theMat);

    // 3) retrieve or build the pMax data
    if (!fPMaxTable->count(theMat)) GetPMaxTable(theMat);
  ...
  {  // very low momentum transfer
    ...
    do
    {
      ...
      if (G4UniformRand() > G) loopAgain = true;
    } while (loopAgain);
  }
  else
...
void G4PenelopeRayleighModelMI::ReadDataFile(const G4int Z)
{
  ...
  {
    G4String excep = "G4LEDATA environment variable not set!";
    G4Exception("G4PenelopeRayleighModelMI::ReadDataFile()", "em0006", FatalException, excep);
    return;
  }
  ...
  if (!file.is_open())
  {
    ...
    G4Exception("G4PenelopeRayleighModelMI::ReadDataFile()", "em0003", FatalException, excep);
  }
  ...
  if (readZ != Z || nPoints <= 0 || nPoints >= 5000)
  {
    ...
    G4Exception("G4PenelopeRayleighModelMI::ReadDataFile()", "em0005", FatalException, ed);
    return;
  }
  ...
  G4double qext = 0;
  if (fillQGrid)
  {  // fLogQSquareGrid filled only one time
    for (std::size_t i = 0; i < nPoints; ++i)
    {
      file >> qext;
                                                        ...
                                                        const G4String& FFfilename)
{
  if (fVerboseLevel > 2)
  {
    G4cout << "G4PenelopeRayleighModelMI::ReadMolInterferenceData() for material " << matname
           << G4endl;
  }
  ...
  {
    G4String excep = "G4LEDATA environment variable not set!";
    G4Exception("G4PenelopeRayleighModelMI::ReadMolInterferenceData()", "em0006", FatalException,
                excep);
    return;
  }
  ...
  {
    if (fVerboseLevel > 2)
      G4cout << "ReadMolInterferenceData(). Read material: " << matname
             << ", filename: " << aFileName << " " << (isLocalFile ? "(local)" : "(database)")
             << G4endl;
    std::ifstream file;
    ...
    if (!file.is_open())
    {
      ...
      G4Exception("G4PenelopeRayleighModelMI::ReadMolInterferenceData()", "em1031", FatalException,
                  excep);
      return;
    }
    ...
    file.close();
    nPoints--;
    if (fVerboseLevel > 3) G4cout << "Number of nPoints: " << nPoints << G4endl;
    ...
    for (std::size_t i = 0; i < nPoints; ++i)
    {
      ...
      if (file.eof() && i != (nPoints - 1))
      {
        ...
        ed << "Found less than " << nPoints << " entries" << G4endl;
        G4Exception("G4PenelopeRayleighModelMI::ReadMolInterferenceData()", "em1005",
                    FatalException, ed);
      }
    }
    if (!fMolInterferenceData)
    {
      G4Exception("G4PenelopeRayleighModelMI::ReadMolInterferenceData()", "em2145", FatalException,
...
G4double G4PenelopeRayleighModelMI::GetFSquared(const G4Material* mat, const G4double QSquared)
{
  ...
  if (!theVec)
  {
    ...
    G4Exception("G4PenelopeRayleighModelMI::GetFSquared()", "em2046", FatalException, ed);
    return 0;
  }
...
void G4PenelopeRayleighModelMI::InitializeSamplingAlgorithm(const G4Material* mat)
{
  ...
  if (np < 16)
  {
    G4Exception("G4PenelopeRayleighModelMI::InitializeSamplingAlgorithm()", "em2047",
                FatalException, "Too few points to initialize the sampling algorithm");
  }
  if (q2min > (q2max - 1e-10))
  {
    G4cout << "q2min= " << q2min << " q2max= " << q2max << G4endl;
    G4Exception("G4PenelopeRayleighModelMI::InitializeSamplingAlgorithm()", "em2048",
                FatalException, "Too narrow grid to initialize the sampling algorithm");
  }
  ...
  ********************************************************************************/
  ...
  for (std::size_t i = 0; i < NUNIF - 1; ++i)  // build all points but the last
  {
    ...
    if ((*pdfi)[0] < 1e-35) (*pdfi)[0] = 1e-5 * pdfmax;
    ...
    // OK, now get ERR(I), the integral of the absolute difference between the rational
    // interpolation and the true pdf, extended over the interval (X(I),X(I+1))
    ...
    do
    {
      ...
      {
        G4double rr = (*sumi)[k];
        G4double pap = (*area)[i] * (1.0 + ((*a)[i] + (*b)[i] * rr) * rr)
                       * (1.0 + ((*a)[i] + (*b)[i] * rr) * rr)
  ...
  if (x->size() != NUNIF || a->size() != NUNIF || err->size() != NUNIF || area->size() != NUNIF)
  {
    G4ExceptionDescription ed;
    ed << "Problem in building the Table for Sampling: array dimensions do not match" << G4endl;
    G4Exception("G4PenelopeRayleighModelMI::InitializeSamplingAlgorithm()", "em2049",
                FatalException, ed);
  }
  ...
  ********************************************************************************/
  ...
  if (x->size() != np || a->size() != np || err->size() != np || area->size() != np)
  {
    G4Exception(
      "G4PenelopeRayleighModelMI::InitializeSamplingAlgorithm()", "em2050", FatalException,
  ...
  ********************************************************************************/
  ...
  {
    G4Exception("G4PenelopeRayleighModelMI::InitializeSamplingAlgorithm()", "em2051",
                FatalException,
...
void G4PenelopeRayleighModelMI::GetPMaxTable(const G4Material* mat)
{
  ...
  if (fPMaxTable->count(mat)) return;

  // otherwise build it
  if (!fSamplingTable)
  {
    G4Exception("G4PenelopeRayleighModelMI::GetPMaxTable()", "em2052", FatalException,
  ...
  if (!fSamplingTable->count(mat))
  {
    ...
    G4Exception("G4PenelopeRayleighModelMI::GetPMaxTable()", "em2052", FatalException, ed);
    return;
  }
  ...
  for (std::size_t ie = 0; ie < fLogEnergyGridPMax.size(); ++ie)
  {
    ...
    if (Qm2 > firstQ2)
    {
      if (Qm2 < lastQ2)
      {
        ...
        {
          G4double qi = Q1 + k * DQ;
          G4double tau = (qi - Q1) / (theTable->GetX(upperBound + 1) - Q1);
          ...
          if (std::fabs(con1) > 1.0e-16 * std::fabs(con2))
            ...
            etap = tau / con2;
          G4double theFun =
            (theTable->GetPAC(upperBound + 1) - thePAC) * (1.0 + (theA + theB * etap) * etap)
            * (1.0 + (theA + theB * etap) * etap)
        ...
        sum->push_back(0.);
        G4double secondPoint = (*sum)[0] + (5.0 * (*fun)[0] + 8.0 * (*fun)[1] - (*fun)[2]) * CONS;
        ...
        for (std::size_t hh = 2; hh < nip - 1; ++hh)
        {
          ...
          G4double next =
            previous
            + (13.0 * ((*fun)[hh - 1] + (*fun)[hh]) - (*fun)[hh + 1] - (*fun)[hh - 2]) * HCONS;
          sum->push_back(next);
        }
        G4double last = (*sum)[nip - 2]
                        + (5.0 * (*fun)[nip - 1] + 8.0 * (*fun)[nip - 2] - (*fun)[nip - 3]) * CONS;
        sum->push_back(last);
...
void G4PenelopeRayleighModelMI::DumpFormFactorTable(const G4Material* mat)
{
  ...
  G4cout << "*****************************************************************" << G4endl;
  if (!fLogFormFactorTable->count(mat)) BuildFormFactorTable(mat);
...
void G4PenelopeRayleighModelMI::LoadKnownMIFFMaterials()
{
  ...
  fKnownMaterials->insert(
    std::pair<G4String, G4String>("BoneMatrix_MI", "FF_bonematrix_Tartari2002.dat"));
  fKnownMaterials->insert(
    std::pair<G4String, G4String>("Mineral_MI", "FF_mineral_Tartari2002.dat"));
  fKnownMaterials->insert(
    std::pair<G4String, G4String>("adipose_MI", "FF_adipose_Poletti2002.dat"));
  fKnownMaterials->insert(
    std::pair<G4String, G4String>("glandular_MI", "FF_glandular_Poletti2002.dat"));
  fKnownMaterials->insert(
    std::pair<G4String, G4String>("breast5050_MI", "FF_human_breast_Peplow1998.dat"));
  fKnownMaterials->insert(
    std::pair<G4String, G4String>("carcinoma_MI", "FF_carcinoma_Kidane1999.dat"));
  fKnownMaterials->insert(
    std::pair<G4String, G4String>("muscle_MI", "FF_pork_muscle_Peplow1998.dat"));
  fKnownMaterials->insert(
    std::pair<G4String, G4String>("kidney_MI", "FF_pork_kidney_Peplow1998.dat"));
  fKnownMaterials->insert(
    std::pair<G4String, G4String>("liver_MI", "FF_pork_liver_Peplow1998.dat"));
  fKnownMaterials->insert(
    std::pair<G4String, G4String>("heart_MI", "FF_pork_heart_Peplow1998.dat"));
  fKnownMaterials->insert(
    std::pair<G4String, G4String>("blood_MI", "FF_beef_blood_Peplow1998.dat"));
  fKnownMaterials->insert(
    std::pair<G4String, G4String>("grayMatter_MI", "FF_gbrain_DeFelici2008.dat"));
  fKnownMaterials->insert(
    std::pair<G4String, G4String>("whiteMatter_MI", "FF_wbrain_DeFelici2008.dat"));
  fKnownMaterials->insert(std::pair<G4String, G4String>("bone_MI", "FF_bone_King2011.dat"));
  fKnownMaterials->insert(
    std::pair<G4String, G4String>("FatLowX_MI", "FF_fat_Tartari2002_joint_lowXdata_ESRF2003.dat"));
  fKnownMaterials->insert(std::pair<G4String, G4String>(
    "BoneMatrixLowX_MI", "FF_bonematrix_Tartari2002_joint_lowXdata.dat"));
  fKnownMaterials->insert(std::pair<G4String, G4String>(
    "PMMALowX_MI", "FF_PMMA_Tartari2002_joint_lowXdata_ESRF2003.dat"));
  fKnownMaterials->insert(std::pair<G4String, G4String>(
    "dryBoneLowX_MI", "FF_drybone_Tartari2002_joint_lowXdata_ESRF2003.dat"));
  fKnownMaterials->insert(
    std::pair<G4String, G4String>("CIRS30-70_MI", "FF_CIRS30-70_Poletti2002.dat"));
  fKnownMaterials->insert(
    std::pair<G4String, G4String>("CIRS50-50_MI", "FF_CIRS50-50_Poletti2002.dat"));
  fKnownMaterials->insert(
    std::pair<G4String, G4String>("CIRS70-30_MI", "FF_CIRS70-30_Poletti2002.dat"));
  ...
  fKnownMaterials->insert(
    std::pair<G4String, G4String>("Polyethylene_MI", "FF_polyethylene_Kosanetzky1987.dat"));
  fKnownMaterials->insert(
    std::pair<G4String, G4String>("Polystyrene_MI", "FF_polystyrene_Kosanetzky1987.dat"));
  fKnownMaterials->insert(
    std::pair<G4String, G4String>("Formaline_MI", "FF_formaline_Peplow1998.dat"));
  fKnownMaterials->insert(
    std::pair<G4String, G4String>("Acetone_MI", "FF_acetone_Cozzini2010.dat"));
  fKnownMaterials->insert(
    std::pair<G4String, G4String>("Hperoxide_MI", "FF_Hperoxide_Cozzini2010.dat"));
}
