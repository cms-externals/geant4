...
G4QAOLowEnergyLoss::G4QAOLowEnergyLoss(const G4String& name) : G4VLowEnergyModel(name)
{
  numberOfMaterials = 6;
  sizeL0 = 67;
  sizeL1 = 22;
...
G4double G4QAOLowEnergyLoss::HighEnergyLimit(const G4ParticleDefinition*, const G4Material*) const
{
  return 2.0 * MeV;
}
...
G4double G4QAOLowEnergyLoss::LowEnergyLimit(const G4ParticleDefinition*, const G4Material*) const
{
  return 5.0 * keV;
}
                                      ...
                                      const G4Material* material) const
{
  ...
  if ((particle->GetDefinition()) == (G4AntiProton::AntiProtonDefinition()) && hasMaterial)
    isInCharge = true;
                                      ...
                                      const G4Material* material) const
{
  ...
  if (aParticle == (G4AntiProton::AntiProtonDefinition()) && hasMaterial) isInCharge = true;
  return isInCharge;
}
...
G4double G4QAOLowEnergyLoss::TheValue(const G4ParticleDefinition* aParticle,
                                      const G4Material* material, G4double kineticEnergy)
{
...
G4double G4QAOLowEnergyLoss::EnergyLoss(const G4Material* material, G4double kineticEnergy,
                                        G4double zParticle) const
{
  ...
  G4double coeff =
    twopi * proton_mass_c2 * (material->GetTotNbOfElectPerVolume()) / electron_mass_c2;
  ...
  for (G4int nos = 0; nos < nbOfShell; nos++)
  {
    ...
    G4double NormalizedEnergy =
      (2.0 * electron_mass_c2 * v * v) / (c_squared * GetShellEnergy(material, nos));
  ...
  dedx = coeff * zParticle * zParticle
         * (l0Term + zParticle * fBetheVelocity * l1Term
...
G4double G4QAOLowEnergyLoss::GetShellEnergy(const G4Material* material, G4int nbOfTheShell) const
{
  ...
  if (material->GetZ() == 13)
    shellEnergy = alShellEnergy[nbOfTheShell];
  else if (material->GetZ() == 14)
    shellEnergy = siShellEnergy[nbOfTheShell];
  else if (material->GetZ() == 29)
    shellEnergy = cuShellEnergy[nbOfTheShell];
  else if (material->GetZ() == 73)
    shellEnergy = taShellEnergy[nbOfTheShell];
  else if (material->GetZ() == 79)
    shellEnergy = auShellEnergy[nbOfTheShell];
  else if (material->GetZ() == 78)
    shellEnergy = ptShellEnergy[nbOfTheShell];
  else if (material->GetNumberOfElements() == 1)
    shellEnergy = GetOscillatorEnergy(material, nbOfTheShell);
  else
  {
    G4ExceptionDescription ed;
    ed << "The model is not available for " << material->GetName() << G4endl;
    G4Exception("G4QAOLowEnergyLoss::GetShellEnergy()", "em2638", JustWarning, ed);
  }
                                                 ...
                                                 G4int nbOfTheShell) const
{
  ...
  G4double squaredPlasmonEnergy =
    28.816 * 28.816 * 1e-6 * material->GetDensity() / g / cm3 * (Z / element->GetN());

  G4double plasmonTerm =
    0.66667 * GetOccupationNumber(Z, nbOfTheShell) * squaredPlasmonEnergy / (Z * Z);
...
G4double G4QAOLowEnergyLoss::GetShellStrength(const G4Material* material, G4int nbOfTheShell) const
{
  ...
  if (material->GetZ() == 13)
    shellStrength = alShellStrength[nbOfTheShell];
  else if (material->GetZ() == 14)
    shellStrength = siShellStrength[nbOfTheShell];
  else if (material->GetZ() == 29)
    shellStrength = cuShellStrength[nbOfTheShell];
  else if (material->GetZ() == 73)
    shellStrength = taShellStrength[nbOfTheShell];
  else if (material->GetZ() == 79)
    shellStrength = auShellStrength[nbOfTheShell];
  else if (material->GetZ() == 78)
    shellStrength = ptShellStrength[nbOfTheShell];
  else if (material->GetNumberOfElements() == 1)
  {
    G4int Z = (G4int)(material->GetZ());
  ...
  else
  {
    G4ExceptionDescription ed;
    ed << "The model is not available for " << material->GetName() << G4endl;
    G4Exception("G4QAOLowEnergyLoss::GetShellStrength()", "em2639", JustWarning, ed);
  }
...
G4double G4QAOLowEnergyLoss::GetL0(G4double normEnergy) const
{
  ...
  G4double bethe = l0p + (l0 - l0p) * (normEnergy - L0[n - 1][0]) / (L0[n][0] - L0[n - 1][0]);
  return bethe;
}
...
G4double G4QAOLowEnergyLoss::GetL1(G4double normEnergy) const
{
  ...
  G4double barkas = l1p + (l1 - l1p) * (normEnergy - L1[n - 1][0]) / (L1[n][0] - L1[n - 1][0]);
  return barkas;
}
...
G4double G4QAOLowEnergyLoss::GetL2(G4double normEnergy) const
{
  ...
  G4double bloch = l2p + (l2 - l2p) * (normEnergy - L2[n - 1][0]) / (L2[n][0] - L2[n - 1][0]);
  return bloch;
}
...
const G4double G4QAOLowEnergyLoss::taShellEnergy[6] = {88926e-6, 18012e-6, 3210e-6,
                                                       575e-6,   108.7e-6, 30.8e-6};
const G4double G4QAOLowEnergyLoss::taShellStrength[6] = {0.0126, 0.0896, 0.2599,
                                                         0.3413, 0.2057, 0.0908};
const G4double G4QAOLowEnergyLoss::auShellEnergy[6] = {96235e-6, 25918e-6, 4116e-6,
                                                       599e-6,   87.3e-6,  36.9e-6};
const G4double G4QAOLowEnergyLoss::auShellStrength[6] = {0.0139, 0.0803, 0.2473,
                                                         0.423,  0.1124, 0.1231};
const G4double G4QAOLowEnergyLoss::ptShellEnergy[6] = {95017e-6, 25590e-6, 4063e-6,
                                                       576e-6,   81.9e-6,  31.4e-6};
const G4double G4QAOLowEnergyLoss::ptShellStrength[6] = {0.0129, 0.0745, 0.2295,
                                                         0.4627, 0.1324, 0.0879};

const G4double G4QAOLowEnergyLoss::L0[67][2] = {
  {0.00, 0.000001},  {0.10, 0.000001},  {0.12, 0.00001},   {0.14, 0.00005},   {0.16, 0.00014},
  {0.18, 0.00030},   {0.20, 0.00057},   {0.25, 0.00189},   {0.30, 0.00429},   {0.35, 0.00784},
  {0.40, 0.01248},   {0.45, 0.01811},   {0.50, 0.02462},   {0.60, 0.03980},   {0.70, 0.05731},
  {0.80, 0.07662},   {0.90, 0.09733},   {1.00, 0.11916},   {1.20, 0.16532},   {1.40, 0.21376},
  {1.60, 0.26362},   {1.80, 0.31428},   {2.00, 0.36532},   {2.50, 0.49272},   {3.00, 0.61765},
  {3.50, 0.73863},   {4.00, 0.85496},   {4.50, 0.96634},   {5.00, 1.07272},   {6.00, 1.27086},
  {7.00, 1.45075},   {8.00, 1.61412},   {9.00, 1.76277},   {10.00, 1.89836},  {12.00, 2.13625},
  {14.00, 2.33787},  {16.00, 2.51093},  {18.00, 2.66134},  {20.00, 2.79358},  {25.00, 3.06539},
  {30.00, 3.27902},  {35.00, 3.45430},  {40.00, 3.60281},  {45.00, 3.73167},  {50.00, 3.84555},
  {60.00, 4.04011},  {70.00, 4.20264},  {80.00, 4.34229},  {90.00, 4.46474},  {100.00, 4.57378},
  {120.00, 4.76155}, {140.00, 4.91953}, {160.00, 5.05590}, {180.00, 5.17588}, {200.00, 5.28299},
  {250.00, 5.50925}, {300.00, 5.69364}, {350.00, 5.84926}, {400.00, 5.98388}, {450.00, 6.10252},
  {500.00, 6.20856}, {600.00, 6.39189}, {700.00, 6.54677}, {800.00, 6.68084}, {900.00, 6.79905},
  {1000.00, 6.90474}};

const G4double G4QAOLowEnergyLoss::L1[22][2] = {
  {0.00, -0.000001}, {0.10, -0.00001}, {0.20, -0.00049}, {0.30, -0.00084}, {0.40, 0.00085},
  {0.50, 0.00519},   {0.60, 0.01198},  {0.70, 0.02074},  {0.80, 0.03133},  {0.90, 0.04369},
  {1.00, 0.06035},   {2.00, 0.24023},  {3.00, 0.44284},  {4.00, 0.62012},  {5.00, 0.77031},
  {6.00, 0.90390},   {7.00, 1.02705},  {8.00, 1.10867},  {9.00, 1.17546},  {10.00, 1.21599},
  {15.00, 1.24349},  {20.00, 1.16752}};

const G4double G4QAOLowEnergyLoss::L2[14][2] = {
  {0.00, 0.000001}, {0.10, 0.00001},  {0.20, 0.00000},   {0.40, -0.00120}, {0.60, -0.00036},
  {0.80, 0.00372},  {1.00, 0.01298},  {2.00, 0.08296},   {4.00, 0.21953},  {6.00, 0.23903},
  {8.00, 0.20893},  {10.00, 0.10879}, {20.00, -0.88409}, {40.00, -1.13902}};
