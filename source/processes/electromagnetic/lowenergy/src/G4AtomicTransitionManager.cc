...
namespace
{
G4Mutex AtomicTransitionManagerMutex = G4MUTEX_INITIALIZER;
}
...
G4AtomicTransitionManager::G4AtomicTransitionManager()
  : augerData(nullptr), verboseLevel(0), isInitialized(false)
{}
...
//....oooOO0OOooo........oooOO0OOooo........oooOO0OOooo........oooOO0OOooo....
G4AtomicShell* G4AtomicTransitionManager::Shell(G4int Z, std::size_t shellIndex) const
{
  ...
  if (pos != shellTable.end())
  {
    ...
    else
    {
      ...
      ed << "No de-excitation for Z= " << Z << "  shellIndex= " << shellIndex
         << ">=  numberOfShells= " << lastShell;
      if (verboseLevel > 0)
        G4Exception("G4AtomicTransitionManager::Shell()", "de0001", JustWarning, ed,
                    " AtomicShell not found");
      if (lastShell > 0)
      {
  ...
  else
  {
    ...
    G4Exception("G4AtomicTransitionManager::Shell()", "de0001", FatalException, ed, "");
  }
  return nullptr;
}
                                                                  ...
                                                                  std::size_t shellIndex) const
{
  ...
  if (pos != transitionTable.end())
  {
    ...
    {
      G4ExceptionDescription ed;
      ed << "No fluo transition for Z= " << Z << "  shellIndex= " << shellIndex;
      G4Exception("G4AtomicTransitionManager::ReachebleShell()", "de0002", FatalException, ed, "");
    }
  }
  else
  {
    G4ExceptionDescription ed;
    ed << "No transition table for Z= " << Z << "  shellIndex= " << shellIndex;
    G4Exception("G4AtomicTransitionManager::ReachableShell()", "de0001", FatalException, ed, "");
  }
  return 0;
}
...
G4AtomicTransitionManager::ReachableAugerShell(G4int Z, G4int vacancyShellIndex) const
{
...
G4int G4AtomicTransitionManager::NumberOfShells(G4int Z) const
{
  ...
  else
  {
    ...
    G4Exception("G4AtomicTransitionManager::NumberOfShells()", "de0001", FatalException, ed, "");
  }
  return (G4int)res;
}
...
G4int G4AtomicTransitionManager::NumberOfReachableShells(G4int Z) const
{
  ...
  {
    G4ExceptionDescription ed;
    ed << "No deexcitation for Z= " << Z << ", so energy deposited locally";
    G4Exception("G4AtomicTransitionManager::NumberOfReachebleShells()", "de0001", FatalException,
                ed, "");
  }
  return (G4int)res;
}
...
//....oooOO0OOooo........oooOO0OOooo........oooOO0OOooo........oooOO0OOooo....
G4double
G4AtomicTransitionManager::TotalRadiativeTransitionProbability(G4int Z,
                                                               std::size_t shellIndex) const
{
  auto pos = transitionTable.find(Z);
  ...
  if (pos != transitionTable.end())
  {
    ...
    {
      G4ExceptionDescription ed;
      ed << "Zero transition probability for Z=" << Z << "  shellIndex= " << shellIndex;
      G4Exception("G4AtomicTransitionManager::TotalRadiativeTransitionProbability()", "de0002",
                  FatalException, "Incorrect de-excitation");
    }
  }
  else
  {
    G4ExceptionDescription ed;
    ed << "No deexcitation for Z=" << Z << "  shellIndex= " << shellIndex;
    G4Exception("G4AtomicTransitionManager::TotalRadiativeTransitionProbability()", "de0001",
                FatalException, ed, "Cannot compute transition probability");
  }
...
//....oooOO0OOooo........oooOO0OOooo........oooOO0OOooo........oooOO0OOooo....
G4double
G4AtomicTransitionManager::TotalNonRadiativeTransitionProbability(G4int Z,
                                                                  std::size_t shellIndex) const
{
  ...
  {
    G4ExceptionDescription ed;
    ed << "Total probability mismatch Z= " << Z << "  shellIndex= " << shellIndex
       << "  prob= " << prob;
    G4Exception("G4AtomicTransitionManager::TotalNonRadiativeTransitionProbability()", "de0003",
                FatalException, ed, "Cannot compute non-radiative probability");
    return 0.0;
  }
  return prob;
}
...
void G4AtomicTransitionManager::Initialise()
{
  ...
  for (G4int Znum = infTableLimit; Znum <= supTableLimit; ++Znum)
  {
    ...
    for (G4int vacancyIndex = 0; vacancyIndex < numberOfVacancies; ++vacancyIndex)
    {
      ...
      G4int numberOfTransitions = (G4int)fluoManager->NumberOfTransitions(vacancyIndex);
      for (G4int origShellIndex = 0; origShellIndex < numberOfTransitions; ++origShellIndex)
      {
      ...
      G4FluoTransition* transition =
        new G4FluoTransition(finalShell, vectorOfIds, vectorOfEnergies, vectorOfProbabilities);
