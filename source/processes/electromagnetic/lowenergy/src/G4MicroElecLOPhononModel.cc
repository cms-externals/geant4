...
G4MicroElecLOPhononModel::G4MicroElecLOPhononModel(const G4ParticleDefinition*, const G4String& nam)
  : G4VEmModel(nam), isInitialised(false)
{
  G4cout << "Phonon model is constructed " << G4endl << "Phonon Energy = " << phononEnergy / eV
         << " eV  " << G4endl;
}

//....oooOO0OOooo........oooOO0OOooo........oooOO0OOooo........oooOO0OOooo......

void G4MicroElecLOPhononModel::Initialise(const G4ParticleDefinition*, const G4DataVector& /*cuts*/)
{
                                                         ...
                                                         G4double ekin, G4double, G4double)
{
  ...
  if (material->GetName() != "G4_SILICON_DIOXIDE" && material->GetName() != "G4_ALUMINUM_OXIDE"
  ...
  G4double P = (std::pow(e, 2) / (4 * pi * eps0 * h * h)) * (n + 0.5 + signe * 0.5)
               * ((1 / einf) - (1 / eps)) * std::sqrt(m0 / (2 * E)) * hw
               * std::log((1 + racine) / (signe * 1 + ((-signe) * racine)));
                                                 ...
                                                 G4double, G4double)
{
  ...
  G4double cosTheta =
    ((E + Eprim) / (2 * std::sqrt(E * Eprim))) * (1 - std::pow(B, rand)) + std::pow(B, rand);
  if (Interband)
  {
