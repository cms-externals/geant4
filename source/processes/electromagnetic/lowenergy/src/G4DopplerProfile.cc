...
G4DopplerProfile::G4DopplerProfile(G4int minZ, G4int maxZ) : zMin(minZ), zMax(maxZ)
{
...
{
  if (Z < zMin || Z > zMax)
    G4Exception("G4DopplerProfile::Profiles", "em1005", FatalException, "Z outside boundaries");
...
void G4DopplerProfile::LoadBiggsP(const G4String& fileName)
{
  ...
  if (!path)
  {
    G4Exception("G4DopplerProfile::LoadBiggsP", "em0006", FatalException,
  ...
  if (!(lsdp->is_open()))
  {
    ...
    G4Exception("G4DopplerProfile::LoadBiggsP", "em0003", FatalException, excep);
  }
  ...
  if (biggsP.size() != nBiggs)
    G4Exception("G4DopplerProfile::LoadBiggsP", "em1006", FatalException,
                "Number of momenta read in is not 31");
}
...
void G4DopplerProfile::LoadProfile(const G4String& fileName, G4int Z)
{
  ...
  if (!path)
  {
    ...
    G4Exception("G4DopplerProfile::LoadProfile", "em0006", FatalException, excep);
    return;
  }
  ...
  if (!(lsdp->is_open()))
  {
    ...
    G4String s2(" not found");
    G4String excep = s1 + dirFile + s2;
    G4Exception("G4DopplerProfile::LoadProfile", "em0003", FatalException, excep);
  }
