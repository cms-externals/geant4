...
G4PenelopeCrossSection::G4PenelopeCrossSection(size_t nPointsE, size_t nShells)
  : fSoftCrossSections(nullptr),
    fHardCrossSections(nullptr),
    fShellCrossSections(nullptr),
    fShellNormalizedCrossSections(nullptr),
    fNumberOfEnergyPoints(nPointsE),
    fNumberOfShells(nShells)
{
  ...
  if (!fNumberOfEnergyPoints)
  {
    ...
    G4Exception("G4PenelopeCrossSection::G4PenelopeCrossSection()", "em2017", FatalException, ed);
  }
...
//....oooOO0OOooo........oooOO0OOooo........oooOO0OOooo........oooOO0OOooo...
void G4PenelopeCrossSection::AddCrossSectionPoint(size_t binNumber, G4double energy, G4double XH0,
                                                  G4double XH1, G4double XH2, G4double XS0,
                                                  G4double XS1, G4double XS2)
{
  if (!fSoftCrossSections || !fHardCrossSections)
  {
    G4cout << "Something wrong in G4PenelopeCrossSection::AddCrossSectionPoint" << G4endl;
...
void G4PenelopeCrossSection::AddShellCrossSectionPoint(size_t binNumber, size_t shellID,
                                                       G4double energy, G4double xs)
{
  if (!fShellCrossSections)
  {
    G4cout << "Something wrong in G4PenelopeCrossSection::AddShellCrossSectionPoint" << G4endl;
  ...
  {
    G4cout << "Something wrong in G4PenelopeCrossSection::AddShellCrossSectionPoint" << G4endl;
    G4cout << "Trying to fill shell #" << shellID << " while the maximum is " << fNumberOfShells - 1
           << G4endl;
    return;
  }
...
G4double G4PenelopeCrossSection::GetTotalCrossSection(G4double energy) const
{
  ...
  if (!fSoftCrossSections || !fHardCrossSections)
  {
    G4cout << "Something wrong in G4PenelopeCrossSection::GetTotalCrossSection" << G4endl;
...
G4double G4PenelopeCrossSection::GetHardCrossSection(G4double energy) const
{
  ...
  if (!fHardCrossSections)
  {
    G4cout << "Something wrong in G4PenelopeCrossSection::GetHardCrossSection" << G4endl;
    G4cout << "Trying to retrieve from un-initialized tables" << G4endl;
    return result;
  }
...
G4double G4PenelopeCrossSection::GetSoftStoppingPower(G4double energy) const
{
  ...
  if (!fSoftCrossSections)
  {
    G4cout << "Something wrong in G4PenelopeCrossSection::GetSoftStoppingPower" << G4endl;
    G4cout << "Trying to retrieve from un-initialized tables" << G4endl;
    return result;
  }
...
void G4PenelopeCrossSection::NormalizeShellCrossSections()
{
  ...
  for (size_t i = 0; i < fNumberOfEnergyPoints; i++)  // loop on energy
  {
    ...
    {
      G4PhysicsFreeVector* theVec = (G4PhysicsFreeVector*)(*fShellCrossSections)[shellID];
    ...
    for (size_t shellID = 0; shellID < fNumberOfShells; shellID++)
    {
      G4PhysicsFreeVector* theVec = (G4PhysicsFreeVector*)(*fShellNormalizedCrossSections)[shellID];
      G4PhysicsFreeVector* theFullVec = (G4PhysicsFreeVector*)(*fShellCrossSections)[shellID];
