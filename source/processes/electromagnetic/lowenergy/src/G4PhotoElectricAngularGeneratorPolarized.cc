...
//
// Class Description:
// PhotoElectric Electron Angular Generator based on the general Gavrila photoelectron angular
// distribution. Includes polarization effects for K and L1 atomic shells, according to Gavrila
// (1959, 1961). For higher shells the L1 cross-section is used.
//
// The Gavrila photoelectron angular distribution is a complex function which can not be sampled
// using the inverse-transform method (James 1980). Instead a more general approach based on the one
// already used to sample bremsstrahlung 2BN cross section (G4Generator2BN, Peralta, 2005) was used.
//
...
// L. Peralta et al., "A new low-energy bremsstrahlung generator for GEANT4", Radiat. Prot.
// Dosimetry. 116, 59-64 (2005)
//
//
// -------------------------------------------------------------------
//
...
G4ThreeVector& G4PhotoElectricAngularGeneratorPolarized::SampleDirection(
  const G4DynamicParticle* dp, G4double eKinEnergy, G4int shellId, const G4Material*)
{
  ...
  const G4RotationMatrix rotation = PhotoElectronRotationMatrix(direction, polarization);
...
void G4PhotoElectricAngularGeneratorPolarized::PhotoElectronGeneratePhiAndTheta(
  G4int shellLevel, G4double beta, G4double aBeta, G4double cBeta, G4double* pphi,
  G4double* ptheta) const
{
  ...
  do
  {
    ...
    if (crossSectionValue < 0.0)
    {
      crossSectionValue = maxBeta;
    }
...
G4double
G4PhotoElectricAngularGeneratorPolarized::CrossSectionMajorantFunction(G4double theta,
                                                                       G4double cBeta) const
{
                                                                           ...
                                                                           G4double phi) const
{
  ...
  firstTerm =
    sinTheta2 * cosPhi2 / std::pow(oneBetaCosTheta, 4)
    - (1 - sqrtOneBeta2) / (2 * oneBeta2) * (sinTheta2 * cosPhi2) / std::pow(oneBetaCosTheta, 3)
    + (1 - sqrtOneBeta2) * (1 - sqrtOneBeta2) / (4 * oneBeta2_to_3_2) * sinTheta2
        / std::pow(oneBetaCosTheta, 3);

  secondTerm =
    std::sqrt(1 - sqrtOneBeta2) / (std::pow(2., 3.5) * beta2 * std::pow(oneBetaCosTheta, 2.5))
      * (4 * beta2 / sqrtOneBeta2 * sinTheta2 * cosPhi2 / oneBetaCosTheta
         + 4 * beta / oneBeta2 * cosTheta * cosPhi2
         - 4 * (1 - sqrtOneBeta2) / oneBeta2 * (1 + cosPhi2)
         - beta2 * (1 - sqrtOneBeta2) / oneBeta2 * sinTheta2 / oneBetaCosTheta
         + 4 * beta2 * (1 - sqrtOneBeta2) / oneBeta2_to_3_2
         - 4 * beta * (1 - sqrtOneBeta2) * (1 - sqrtOneBeta2) / oneBeta2_to_3_2 * cosTheta)
    + (1 - sqrtOneBeta2) / (4 * beta2 * oneBetaCosTheta * oneBetaCosTheta)
        * (beta / oneBeta2 - 2 / oneBeta2 * cosTheta * cosPhi2
           + (1 - sqrtOneBeta2) / oneBeta2_to_3_2 * cosTheta
           - beta * (1 - sqrtOneBeta2) / oneBeta2_to_3_2);

  dsigma =
    (firstTerm * (1 - pi * fine_structure_const / beta) + secondTerm * (pi * fine_structure_const))
    * std::sin(theta);
...
G4RotationMatrix G4PhotoElectricAngularGeneratorPolarized::PhotoElectronRotationMatrix(
  const G4ThreeVector& direction, const G4ThreeVector& polarization)
{
  ...
  else
  {
    ...
    {
      polarization2 =
        polarization - polarization.dot(direction) / direction.dot(direction) * direction;
    }
  }
...
void G4PhotoElectricAngularGeneratorPolarized::PhotoElectronGetMajorantSurfaceAandCParameters(
  G4int shellId, G4double beta, G4double* majorantSurfaceParameterA,
  G4double* majorantSurfaceParameterC) const
{
  ...
  if (k < 0) k = 0;
  if (k > indexMax) k = indexMax;

  if (k == 0)
    aBeta = std::max(aMajorantSurfaceParameterTable[k][level],
                     aMajorantSurfaceParameterTable[k + 1][level]);
  else if (k == indexMax)
    aBeta = std::max(aMajorantSurfaceParameterTable[k - 1][level],
                     aMajorantSurfaceParameterTable[k][level]);
  else
  {
    aBeta = std::max(aMajorantSurfaceParameterTable[k - 1][level],
                     aMajorantSurfaceParameterTable[k][level]);
  ...
  if (k == 0)
    cBeta = std::max(cMajorantSurfaceParameterTable[k][level],
                     cMajorantSurfaceParameterTable[k + 1][level]);
  else if (k == indexMax)
    cBeta = std::max(cMajorantSurfaceParameterTable[k - 1][level],
                     cMajorantSurfaceParameterTable[k][level]);
  else
  {
    cBeta = std::max(cMajorantSurfaceParameterTable[k - 1][level],
                     cMajorantSurfaceParameterTable[k][level]);
...
G4ThreeVector G4PhotoElectricAngularGeneratorPolarized::PhotoElectronComputeFinalDirection(
  const G4RotationMatrix& rotation, G4double theta, G4double phi) const
{
...
void G4PhotoElectricAngularGeneratorPolarized::PrintGeneratorInformation() const
{
  ...
  G4cout << "PhotoElectric Electron Angular Generator based on the general Gavrila photoelectron "
            "angular distribution"
         << G4endl;
  G4cout << "Includes polarization effects for K and L1 atomic shells, according to Gavrilla "
            "(1959, 1961)."
         << G4endl;
...
G4ThreeVector
G4PhotoElectricAngularGeneratorPolarized::PerpendicularVector(const G4ThreeVector& a) const
{
  G4double dx = a.x();
  G4double dy = a.y();
  G4double dz = a.z();
