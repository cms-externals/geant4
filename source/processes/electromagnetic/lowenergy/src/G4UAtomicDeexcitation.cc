...
void G4UAtomicDeexcitation::GenerateParticles(std::vector<G4DynamicParticle*>* vectorOfParticles,
                                              const G4AtomicShell* atomicShell, G4int Z,
                                              G4double gammaCut, G4double eCut)
{
...
G4double G4UAtomicDeexcitation::GetShellIonisationCrossSectionPerAtom(
  const G4ParticleDefinition* pdef, G4int Z, G4AtomicShellEnumerator shellEnum,
  G4double kineticEnergy, const G4Material* mat)
{
  ...
  {
    return xsec;
  }  // corrected by alf - Z<6 missing
  G4int idx = G4int(shellEnum);
  if (idx >= G4AtomicShells::GetNumberOfShells(Z))
  {
    return xsec;
  }
...
G4double G4UAtomicDeexcitation::ComputeShellIonisationCrossSectionPerAtom(
  const G4ParticleDefinition* p, G4int Z, G4AtomicShellEnumerator shell, G4double kinE,
...
G4DynamicParticle* G4UAtomicDeexcitation::GenerateFluorescence(G4int Z, G4int shellId,
                                                               G4int provShellId)
{
  if (shellId <= 0)
  {
  ...
  G4int transitionSize =
    (G4int)transitionManager->ReachableShell(Z, shellNum)->OriginatingShellIds().size();
  ...
  G4double transitionEnergy =
    transitionManager->ReachableShell(Z, shellNum)->TransitionEnergy(index);
  ...
  G4DynamicParticle* newPart =
    new G4DynamicParticle(G4Gamma::Gamma(), newGammaDirection, transitionEnergy);
...
G4DynamicParticle* G4UAtomicDeexcitation::GenerateAuger(G4int Z, G4int shellId)
{
  ...
  // being the Id of the shell in which there is a vacancy
  {
    ...
    G4int transitionSize = (G4int)(anAugerTransition->TransitionOriginatingShellIds())->size();
    while (transitionLoopShellIndex < transitionSize)
    {
      ...
      G4int numberOfPossibleAuger =
        (G4int)(anAugerTransition->AugerTransitionProbabilities(transitionLoopShellId))->size();
      G4int augerIndex = 0;
      ...
      if (augerIndex < numberOfPossibleAuger)
      {
        ...
        {
          G4double thisProb =
            anAugerTransition->AugerTransitionProbability(augerIndex, transitionLoopShellId);
          partSum += thisProb;
          augerIndex++;
    ...
    while (transitionRandomShellIndex < transitionSize)
    {
      ...
      augerIndex = 0;
      numberOfPossibleAuger =
        (G4int)(anAugerTransition->AugerTransitionProbabilities(transitionRandomShellId))->size();

      while (augerIndex < numberOfPossibleAuger)
      {
        G4double thisProb =
          anAugerTransition->AugerTransitionProbability(augerIndex, transitionRandomShellId);
    ...
    if (IsAugerCascadeActive())
    {
      ...
      vacancyArray.push_back(
        anAugerTransition->AugerOriginatingShellId(augerIndex, transitionRandomShellId));
    }

    return new G4DynamicParticle(G4Electron::Electron(), newElectronDirection, transitionEnergy);
  }
  else
  {
    return nullptr;
