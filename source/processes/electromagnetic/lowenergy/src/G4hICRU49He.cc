...
G4hICRU49He::G4hICRU49He()
  : G4VhElectronicStoppingPower(), rateMass(4.0026 / 1.007276), iMolecula(0)
{
  ;
}
...
G4bool G4hICRU49He::HasMaterial(const G4Material* material)
{
  ...
  static const G4String name[numberOfMolecula] = {"H_2",
                                                  "Be-Solid",
                                                  "C-Solid",
                                                  "Graphite",
                                                  "N_2",
...
G4double G4hICRU49He::StoppingPower(const G4Material* material, G4double kineticEnergy)
{
  ...
  else if (iMolecula < 30)
  {
    ...
    static const G4double c[30][7] = {{8.0080, 3.6287, 23.0700, 14.9900, 0.8507, 0.60, 2.0},
    ...
    // He effective charge
    G4double z = (material->GetTotNbOfElectPerVolume()) / (material->GetTotNbOfAtomsPerVolume());
...
G4double G4hICRU49He::ElectronicStoppingPower(G4double z, G4double kineticEnergy) const
{
  ...
  static const G4double a[92][5] = {{0.35485, 0.6456, 6.01525, 20.8933, 4.3515},
  ...
  {
    G4double slow = a[i][0];
    G4double shigh = std::log(1.0 + a[i][3] * 1000.0 + a[i][4] * 0.001) * a[i][2] * 1000.0;
