...
namespace
{
G4Mutex G4JAEAElasticScatteringModelMutex = G4MUTEX_INITIALIZER;
}
...
void G4JAEAElasticScatteringModel::ReadData(std::size_t Z, const char* path)
{
  if (verboseLevel > 1)
  {
    G4cout << "Calling ReadData() of G4JAEAElasticScatteringModel" << G4endl;
  }
  ...
  if (!datadir)
  {
    ...
    if (!datadir)
    {
      G4Exception("G4JAEAElasticScatteringModel::ReadData()", "em0006", FatalException,
  ...
  */
  ...
  if (!ES_Data_Buffer.is_open())
  {
    ...
    ed << "G4JAEAElasticScattertingModel data file <" << ostCS.str().c_str() << "> is not opened!"
       << G4endl;
    G4Exception(
      "G4JAEAElasticScatteringModel::ReadData()", "em0003", FatalException, ed,
  ...
  else
  {
    if (verboseLevel > 3)
    {
      G4cout << "File " << ostCS.str() << " is opened by G4JAEAElasticScatteringModel" << G4endl;
    }
  }
  if (!ES_Data[Z]) ES_Data[Z] = new G4DataVector();
                                                                  ...
                                                                  G4double, G4double, G4double)
{
  if (verboseLevel > 2)
  {
    G4cout << "G4JAEAElasticScatteringModel::ComputeCrossSectionPerAtom()" << G4endl;
  }
                                                     ...
                                                     G4double, G4double)
{
  if (verboseLevel > 2)
  {
    G4cout << "Calling SampleSecondaries() of G4JAEAElasticScatteringModel." << G4endl;
  }
