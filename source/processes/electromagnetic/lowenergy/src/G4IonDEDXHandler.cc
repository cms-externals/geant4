...
G4IonDEDXHandler::G4IonDEDXHandler(G4VIonDEDXTable* ionTable,
                                   G4VIonDEDXScalingAlgorithm* ionAlgorithm, const G4String& name,
                                   G4int maxCacheSize, G4bool splines)
  : table(ionTable),
    ...
    maxCacheEntries(maxCacheSize)
{
  if (table == nullptr)
  {
    G4cerr << "G4IonDEDXHandler::G4IonDEDXHandler() "
           << " Pointer to G4VIonDEDXTable object is null-pointer." << G4endl;
  }

  if (algorithm == nullptr)
  {
    G4cerr << "G4IonDEDXHandler::G4IonDEDXHandler() "
           << " Pointer to G4VIonDEDXScalingAlgorithm object is null-pointer." << G4endl;
  }

  if (maxCacheEntries <= 0)
  {
    G4cerr << "G4IonDEDXHandler::G4IonDEDXHandler() "
           << " Cache size <=0. Resetting to 5." << G4endl;
    maxCacheEntries = 5;
  }
}
...
G4bool G4IonDEDXHandler::IsApplicable(const G4ParticleDefinition* particle,  // Projectile (ion)
...
{  // Target material
  ...
  else
  {
    ...
    G4int atomicNumberBase = algorithm->AtomicNumberBaseIon(atomicNumberIon, material);
...
G4double G4IonDEDXHandler::GetDEDX(const G4ParticleDefinition* particle,  // Projectile (ion)
...
{  // Kinetic energy of projectile
  ...
  else if (value.dedxVector != 0)
  {
    ...
    factor *= algorithm->ScalingFactorDEDX(particle, material, kineticEnergy);
...
#ifdef PRINT_DEBUG
  G4cout << "G4IonDEDXHandler::GetDEDX() E = " << kineticEnergy / MeV << " MeV * "
         << value.energyScaling << " = " << kineticEnergy * value.energyScaling / MeV
...
G4bool G4IonDEDXHandler::BuildDEDXTable(const G4ParticleDefinition* particle,  // Projectile (ion)
                                        const G4Material* material)
{  // Target material
...
G4bool G4IonDEDXHandler::BuildDEDXTable(G4int atomicNumberIon,  // Projectile (ion)
                                        const G4Material* material)
{  // Target material
  ...
  if (isApplicable)
  {
    stoppingPowerTable[key] = table->GetPhysicsVector(atomicNumberBase, chemFormula);
    return isApplicable;
  }
  ...
  if (isApplicable)
  {
    stoppingPowerTable[key] = table->GetPhysicsVector(atomicNumberBase, materialName);
    return isApplicable;
  }
  ...
  for (size_t i = 0; i < nmbElements; i++)
  {
    ...
    if (isApplicable)
    {
      G4PhysicsVector* dEdx = table->GetPhysicsVector(atomicNumberBase, atomicNumberMat);
  ...
  if (isApplicable)
  {
    if (dEdxTable.size() > 0)
    {
      ...
      G4PhysicsFreeVector* dEdxBragg =
        new G4PhysicsFreeVector(nmbdEdxBins, lowerEdge, upperEdge, useSplines);
      ...
      for (size_t j = 0; j < nmbdEdxBins; j++)
      {
        ...
        for (size_t i = 0; i < nmbElements; i++)
        {
          value += (dEdxTable[i]->GetValue(edge, b)) * massFractionVector[i];
        }

        dEdxBragg->PutValues(j, edge, value);
      }
      if (useSplines) dEdxBragg->FillSecondDerivatives();

#ifdef PRINT_DEBUG
      G4cout << "G4IonDEDXHandler::BuildPhysicsVector() for ion with Z=" << atomicNumberBase
...
G4CacheValue
G4IonDEDXHandler::UpdateCacheValue(const G4ParticleDefinition* particle,  // Projectile (ion)
                                   const G4Material* material)
{  // Target material
  ...
  if (iter != stoppingPowerTable.end())
  {
    ...
    value.energyScaling = algorithm->ScalingFactorEnergy(particle, material) / nmbNucleons;

    size_t nmbdEdxBins = value.dedxVector->GetVectorLength();
    value.lowerEnergyEdge = value.dedxVector->GetLowEdgeEnergy(0);

    value.upperEnergyEdge = value.dedxVector->GetLowEdgeEnergy(nmbdEdxBins - 1);
...
#ifdef PRINT_DEBUG
  G4cout << "G4IonDEDXHandler::UpdateCacheValue() for " << particle->GetParticleName() << " in "
...
G4CacheValue
G4IonDEDXHandler::GetCacheValue(const G4ParticleDefinition* particle,  // Projectile (ion)
                                const G4Material* material)
{  // Target material
  ...
  G4CacheEntry entry;
  CacheEntryList::iterator* pointerIter = (CacheEntryList::iterator*)cacheKeyPointers[key];
  ...
  if (!pointerIter)
  {
    ...
    CacheEntryList::iterator* pointerIter1 = new CacheEntryList::iterator();
    *pointerIter1 = cacheEntries.begin();
    ...
    if (G4int(cacheEntries.size()) > maxCacheEntries)
    {
      ...
      CacheEntryList::iterator* listPointerIter = (CacheEntryList::iterator*)pointerIter2;
...
void G4IonDEDXHandler::ClearCache()
{
  ...
  for (; iter != iter_end; iter++)
  {
    ...
    CacheEntryList::iterator* listPointerIter = (CacheEntryList::iterator*)pointerIter;
...
void G4IonDEDXHandler::PrintDEDXTable(const G4ParticleDefinition* particle,  // Projectile (ion)
                                      const G4Material* material,  // Target material
...
{  // Logarithmic scaling of energy
  ...
  G4cout << "# dE/dx table for " << particle->GetParticleName() << " in material "
         ...
         << GetUpperEnergyEdge(particle, material) / atomicMassNumber / MeV << " MeV" << G4endl
         << "# ------------------------------------------------------" << G4endl;
  ...
  for (int i = 0; i < nmbBins + 1; i++)
  {
    ...
    G4cout << std::setw(14) << std::right << energy / MeV << std::setw(14)
           << energy / atomicMassNumber / MeV << std::setw(14) << loss / MeV * cm << std::setw(14)
           << loss / materialDensity / (MeV * cm2 / (0.001 * g)) << G4endl;
  }
}
...
G4double
G4IonDEDXHandler::GetLowerEnergyEdge(const G4ParticleDefinition* particle,  // Projectile (ion)
                                     const G4Material* material)
{  // Target material
  ...
  if (value.energyScaling > 0) edge = value.lowerEnergyEdge / value.energyScaling;
...
G4double
G4IonDEDXHandler::GetUpperEnergyEdge(const G4ParticleDefinition* particle,  // Projectile (ion)
                                     const G4Material* material)
{  // Target material
  ...
  if (value.energyScaling > 0) edge = value.upperEnergyEdge / value.energyScaling;
