...
namespace
{
G4Mutex LivermorePolarizedRayleighModelMutex = G4MUTEX_INITIALIZER;
}
  ...
  : G4VEmModel(nam), fParticleChange(nullptr), isInitialised(false)
{
  ...
  if (verboseLevel > 0)
  {
    G4cout << "Livermore Polarized Rayleigh is constructed " << G4endl
           ...
           << HighEnergyLimit() / CLHEP::GeV << " GeV" << G4endl;
  }
}
                                                   ...
                                                   const G4DataVector& cuts)
{
  ...
  //                                        W. Heitler,       Oxford at the Clarendon Press, Oxford
  //                                        (1954)
  // Scattering function:                   A simple model of photon transport
  //                                        D.E. Cullen,      Nucl. Instr. Meth. in Phys. Res. B 101
  //                                        (1995) 499-510
  // Polarization of the outcoming photon:  Beam test of a prototype detector array for the PoGO
  // astronomical hard
  //                                        X-ray/soft gamma-ray polarimeter
  //                                        T. Mizuno et al., Nucl. Instr. Meth. in Phys. Res. A 540
  //                                        (2005) 158-168
  if (verboseLevel > 3)
...
void G4LivermorePolarizedRayleighModel::ReadData(std::size_t Z, const char* path)
{
  if (verboseLevel > 1)
  {
    G4cout << "Calling ReadData() of G4LivermoreRayleighModel" << G4endl;
  }
  ...
  if (nullptr == datadir)
  {
    ...
    if (nullptr == datadir)
    {
      G4Exception("G4LivermoreRayleighModelModel::ReadData()", "em0006", FatalException,
  ...
  if (!finCS.is_open())
  {
    ...
    G4Exception("G4LivermorePolarizedRayleighModel::ReadData()", "em0003", FatalException, ed,
  ...
  else
  {
    if (verboseLevel > 3)
    {
      G4cout << "File " << ostFF.str() << " is opened by G4LivermoreRayleighModel" << G4endl;
    }
...
G4double G4LivermorePolarizedRayleighModel::GenerateCosTheta(G4double incomingPhotonEnergy,
                                                             G4int Z) const
{
  //  d sigma                                                                    k0
  // --------- =  r0^2 * pi * F^2(x, Z) * ( 2 - sin^2 theta) * std::sin (theta), x = ----
  // std::sin(theta/2)
  //  d theta                                                                    hc

  //  d sigma                                             k0          1 - y
  // --------- = r0^2 * pi * F^2(x, Z) * ( 1 + y^2), x = ---- std::sqrt ( ------- ), y =
  // std::cos(theta)
  //    d y                                               hc            2
...
G4ThreeVector
G4LivermorePolarizedRayleighModel::GetPhotonPolarization(const G4DynamicParticle& photon)
{
  ...
  if ((!photonPolarization.isOrthogonal(photonMomentumDirection, 1e-6))
      || photonPolarization.mag() == 0.)
  {
    // if |photonPolarization|==0. or |photonPolarization * photonDirection0| > 1e-6 *
    // |photonPolarization ^ photonDirection0| then polarization is choosen randomly.
