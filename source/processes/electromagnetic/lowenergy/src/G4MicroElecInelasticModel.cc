...
{
  if (verboseLevel > 3) G4cout << "Calling G4MicroElecInelasticModel::Initialise()" << G4endl;
  ...
  // Cross section
  G4MicroElecCrossSectionDataSet* tableE =
    new G4MicroElecCrossSectionDataSet(new G4LogLogInterpolation, eV, scaleFactor);
  ...
  if (fasterCode)
    eFullFileName << path << "/microelec/sigmadiff_cumulated_inelastic_e_Si.dat";
  else
  ...
  {
    if (fasterCode)
      G4Exception("G4MicroElecInelasticModel::Initialise", "em0003", FatalException,
  ...
  // Cross section
  G4MicroElecCrossSectionDataSet* tableP =
    new G4MicroElecCrossSectionDataSet(new G4LogLogInterpolation, eV, scaleFactor);
  ...
  if (fasterCode)
    pFullFileName << path << "/microelec/sigmadiff_cumulated_inelastic_p_Si.dat";
  else
...
G4double
G4MicroElecInelasticModel::CrossSectionPerVolume(const G4Material* material,
                                                 ...
                                                 G4double ekin, G4double, G4double)
{
  ...
  if (Mion_c2 > proton_mass_c2)
  {
    ...
    if (verboseLevel > 3)
      G4cout << "Before scaling : " << G4endl << "Particle : " << nameLocal
             << ", mass : " << Mion_c2 / proton_mass_c2 << "*mp, charge " << Zeff
    ...
    if (verboseLevel > 3)
      G4cout << "After scaling : " << G4endl << "Particle : " << nameLocal
             << ", Ekin (eV) = " << ekin / eV << G4endl;
  }
  ...
  if (material == nistSi || material->GetBaseMaterial() == nistSi)
  {
    ...
    if (ekin >= lowLim && ekin < highLim)
    {
      ...
      {
        G4Exception("G4MicroElecInelasticModel::CrossSectionPerVolume", "em0002", FatalException,
    ...
    if (verboseLevel > 3)
    {
      ...
      G4cout << " - Cross section per Si atom (cm^-1)=" << sigma * density * Zeff2 / (1. / cm)
             << G4endl;
    }
                                                  ...
                                                  G4double)
{
  ...
  if (k >= lowLim && k < highLim)
  {
    ...
    //  sigmadiff_ionisation_e_born.dat has non zero partial xs at 18 eV for shell 3
    //  (ionizationShell ==2) sigmadiff_cumulated_ionisation_e_born.dat has zero cumulated partial
    //  xs at 18 eV for shell 3 (ionizationShell ==2) this is due to the fact that the max allowed
    //  transfered energy is (18+10.79)/2=17.025 eV and only transfered energies strictly above this
    //  value have non zero partial xs in sigmadiff_ionisation_e_born.dat (starting at trans = 17.12
    //  eV)
    ...
    if (secondaryKinetic > 0)
    {
      G4DynamicParticle* dp =
        new G4DynamicParticle(G4Electron::Electron(), deltaDirection, secondaryKinetic);
      fvect->push_back(dp);
    }
  }
}
...
G4double
G4MicroElecInelasticModel::RandomizeEjectedElectronEnergy(G4ParticleDefinition* particleDefinition,
                                                          G4double k, G4int shell)
{
  if (particleDefinition == G4Electron::ElectronDefinition())
  {
    G4double maximumEnergyTransfer = 0.;
    if ((k + SiStructure.Energy(shell)) / 2. > k)
      maximumEnergyTransfer = k;
    else
      maximumEnergyTransfer = (k + SiStructure.Energy(shell)) / 2.;
    ...
    while (step > 0)
    {
      ...
      G4double differentialCrossSection =
        DifferentialCrossSection(particleDefinition, k / eV, value / eV, shell);
    ...
    do
    {
      secondaryElectronKineticEnergy =
        G4UniformRand() * (maximumEnergyTransfer - SiStructure.Energy(shell));
    } while (G4UniformRand() * crossSectionMaximum > DifferentialCrossSection(
               particleDefinition, k / eV,
               (secondaryElectronKineticEnergy + SiStructure.Energy(shell)) / eV, shell));
...
/*void G4MicroElecInelasticModel::RandomizeEjectedElectronDirection(G4ParticleDefinition*
 particleDefinition, G4double k, G4double secKinetic, G4double & cosTheta, G4double & phi )
 {
 if (particleDefinition == G4Electron::ElectronDefinition())
 {
...
G4double G4MicroElecInelasticModel::DifferentialCrossSection(
  G4ParticleDefinition* particleDefinition, G4double k, G4double energyTransfer, G4int LevelIndex)
{
  G4double sigma = 0.;
...
G4double G4MicroElecInelasticModel::Interpolate(G4double e1, G4double e2, G4double e, G4double xs1,
                                                G4double xs2)
{
  G4double value = 0.;
...
G4double G4MicroElecInelasticModel::QuadInterpolator(G4double e11, G4double e12, G4double e21,
...
G4int G4MicroElecInelasticModel::RandomSelect(G4double k, const G4String& particle)
{
  ...
  {
    G4Exception("G4MicroElecInelasticModel::RandomSelect", "em0002", FatalException,
                "Model not applicable to particle type.");
  }
...
G4double G4MicroElecInelasticModel::RandomizeEjectedElectronEnergyFromCumulatedDcs(
  G4ParticleDefinition* particleDefinition, G4double k, G4int shell)
{
  G4double secondaryElectronKineticEnergy = 0.;
  G4double random = G4UniformRand();
  secondaryElectronKineticEnergy =
    TransferedEnergy(particleDefinition, k / eV, shell, random) * eV - SiStructure.Energy(shell);
                                                     ...
                                                     G4double random)
{
  ...
  else if (particleDefinition == G4Proton::ProtonDefinition())
  {
    ...
    {
      auto prob12 = std::upper_bound(pProbaShellMap[ionizationLevelIndex][(*k1)].begin(),
                                     pProbaShellMap[ionizationLevelIndex][(*k1)].end(), random);

      auto prob11 = prob12 - 1;

      auto prob22 = std::upper_bound(pProbaShellMap[ionizationLevelIndex][(*k2)].begin(),
