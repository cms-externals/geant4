...
//                             structures. The EM data sets, initialized this way, contain both
//                             non-log and log values. These initialized data sets can enhance the
//                             computing performance of data interpolation operations
//
//                           - BuildMeanFreePathForMaterials method was also revised in order to
//                           calculate the
//                             logarithmic values of the loaded data.
//                             It generates the data values and, then, calculates the respective log
//                             values which later load to seperate data structures. The EM data
//                             sets, initialized this way, contain both non-log and log values.
//                             These initialized data sets can enhance the computing performance of
//                             data interpolation operations
//
//                           - LoadShellData method was revised in order to eliminate the presence
//                           of a potential
//                             memory leak originally identified by Riccardo Capra.
//                             Riccardo Capra Original Comment
//                             Riccardo Capra <capra@ge.infn.it>: PLEASE CHECK THE FOLLOWING PIECE
//                             OF CODE "energies" AND "data" G4DataVector ARE ALLOCATED, FILLED IN
//                             AND NEVER USED OR DELETED. WHATSMORE LOADING FILE OPERATIONS WERE
//                             DONE BY G4ShellEMDataSet EVEN BEFORE THE CHANGES I DID ON THIS FILE.
//                             SO THE FOLLOWING CODE IN MY OPINION SHOULD BE USELESS AND SHOULD
...
G4VCrossSectionHandler::G4VCrossSectionHandler(G4VDataSetAlgorithm* algorithm, G4double minE,
                                               G4double maxE, G4int bins, G4double unitE,
                                               G4double unitData, G4int minZ, G4int maxZ)
...
void G4VCrossSectionHandler::Initialise(G4VDataSetAlgorithm* algorithm, G4double minE,
                                        G4double maxE, G4int numberOfBins, G4double unitE,
                                        G4double unitData, G4int minZ, G4int maxZ)
{
...
void G4VCrossSectionHandler::PrintData() const
{
  for (auto& pos : dataMap)
  {
    G4int z = pos.first;
    G4VEMDataSet* dataSet = pos.second;
    G4cout << "---- Data set for Z = " << z << G4endl;
    dataSet->PrintData();
    G4cout << "--------------------------------------------------" << G4endl;
  }
}
...
void G4VCrossSectionHandler::LoadData(const G4String& fileName)
{
  ...
  for (std::size_t i = 0; i < nZ; ++i)
  {
    ...
    if (!path)
    {
      G4Exception("G4VCrossSectionHandler::LoadData", "em0006", FatalException,
    ...
    if (!(lsdp->is_open()))
    {
      ...
      G4Exception("G4VCrossSectionHandler::LoadData", "em0003", FatalException, excep);
    }
    G4double a = 0;
    G4int k = 0;
    G4int nColumns = 2;
    ...
    G4VEMDataSet* dataSet =
      new G4EMDataSet(Z, orig_reg_energies, orig_reg_data, log_reg_energies, log_reg_data, algo);
    dataMap[Z] = dataSet;
  }
}
...
void G4VCrossSectionHandler::LoadNonLogData(const G4String& fileName)
{
  ...
  for (std::size_t i = 0; i < nZ; ++i)
  {
    ...
    if (!path)
    {
      G4Exception("G4VCrossSectionHandler::LoadNonLogData", "em0006", FatalException,
                  "G4LEDATA environment variable not set");
      return;
    }
    ...
    {
      G4String excep = "data file: " + ost.str() + " not found";
      G4Exception("G4VCrossSectionHandler::LoadNonLogData", "em0003", FatalException, excep);
    }
    G4double a = 0;
    G4int k = 0;
    G4int nColumns = 2;
...
G4double G4VCrossSectionHandler::FindValue(G4int Z, G4double energy) const
{
  ...
  {
    G4cout << "WARNING: G4VCrossSectionHandler::FindValue did not find Z = " << Z << G4endl;
  }
  return value;
}
...
G4double G4VCrossSectionHandler::FindValue(G4int Z, G4double energy, G4int shellIndex) const
{
  G4double value = 0.;
  auto pos = dataMap.find(Z);
  if (pos != dataMap.cend())
    ...
    if (shellIndex >= 0)
    {
      ...
      else
      {
        G4cout << "WARNING: G4VCrossSectionHandler::FindValue did not find"
               << " shellIndex= " << shellIndex << " for  Z= " << Z << G4endl;
      }
    }
    else
    {
...
G4double G4VCrossSectionHandler::ValueForMaterial(const G4Material* material, G4double energy) const
{
  G4double value = 0.;
...
G4VEMDataSet* G4VCrossSectionHandler::BuildMeanFreePathForMaterials(const G4DataVector* energyCuts)
{
  ...
  if (crossSections == nullptr)
  {
    G4Exception("G4VCrossSectionHandler::BuildMeanFreePathForMaterials", "em1010", FatalException,
                "crossSections = 0");
    return 0;
  }
  ...
  for (G4int mLocal = 0; mLocal < numOfCouples; ++mLocal)
  {
    ...
    G4VEMDataSet* dataSet =
      new G4EMDataSet(mLocal, energies, data, log_energies, log_data, algoLocal, 1., 1.);
...
G4int G4VCrossSectionHandler::SelectRandomAtom(const G4MaterialCutsCouple* couple, G4double e) const
{
...
G4int G4VCrossSectionHandler::SelectRandomShell(G4int Z, G4double e) const
{
  ...
  {
    G4Exception("G4VCrossSectionHandler::SelectRandomShell", "em1011", FatalException,
...
void G4VCrossSectionHandler::ActiveElements()
{
  ...
  if (materialTable == nullptr)
    G4Exception("G4VCrossSectionHandler::ActiveElements", "em1001", FatalException,
