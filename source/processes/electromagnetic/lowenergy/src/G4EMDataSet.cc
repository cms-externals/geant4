...
G4EMDataSet::G4EMDataSet(G4int Z, G4VDataSetAlgorithm* algo, G4double xUnit, G4double yUnit,
                         G4bool random)
  : energies(nullptr),
    data(nullptr),
    ...
    randomSet(random)
{
  ...
  {
    G4Exception("G4EMDataSet::G4EMDataSet", "em1012", FatalException, "interpolation == 0");
  }
  else if (randomSet)
  {
    BuildPdf();
...
G4EMDataSet::G4EMDataSet(G4int argZ, G4DataVector* dataX, G4DataVector* dataY,
                         G4VDataSetAlgorithm* algo, G4double xUnit, G4double yUnit, G4bool random)
  : energies(dataX),
    data(dataY),
    log_energies(nullptr),
    log_data(nullptr),
    ...
    randomSet(random)
{
  ...
  else
  {
    ...
    {
      G4Exception("G4EMDataSet::G4EMDataSet", "em1012", FatalException,
...
G4EMDataSet::G4EMDataSet(G4int argZ, G4DataVector* dataX, G4DataVector* dataY,
                         G4DataVector* dataLogX, G4DataVector* dataLogY, G4VDataSetAlgorithm* algo,
                         G4double xUnit, G4double yUnit, G4bool random)
  : energies(dataX),
    data(dataY),
    ...
    randomSet(random)
{
  ...
  {
    if ((energies->size() != data->size()) || (energies->size() != log_energies->size())
...
void G4EMDataSet::SetEnergiesData(G4DataVector* dataX, G4DataVector* dataY, G4int /* componentId */)
{
  if (!dataX || !dataY)
  {
    G4Exception("G4EMDataSet::SetEnergiesData", "em1012", FatalException, "new interpolation == 0");
  }
  else
  {
    if (dataX->size() != dataY->size())
    {
      G4Exception("G4EMDataSet::SetEnergiesData", "em1012", FatalException,
                  "different size for energies and data");
    }
    else
    {
...
void G4EMDataSet::SetLogEnergiesData(G4DataVector* dataX, G4DataVector* dataY,
                                     G4DataVector* data_logX, G4DataVector* data_logY,
                                     G4int /* componentId */)
{
  ...
  {
    if (dataX->size() != dataY->size() || dataX->size() != data_logX->size()
...
G4bool G4EMDataSet::LoadData(const G4String& fileName)
{
  ...
  if (!in.is_open())
  {
    ...
    message += "\" not found";
    G4Exception("G4EMDataSet::LoadData", "em1012", FatalException, message);
    return false;
  }
...
G4bool G4EMDataSet::LoadNonLogData(const G4String& fileName)
{
  ...
  if (!in.is_open())
  {
    ...
    message += fullFileName;
    message += "\" not found";
    G4Exception("G4EMDataSet::LoadNonLogData", "em1012", FatalException, message);
  }
...
G4bool G4EMDataSet::SaveData(const G4String& name) const
{
  ...
  if (!out.is_open())
  {
    ...
    message += fullFileName;
    message += "\"";
    G4Exception("G4EMDataSet::SaveData", "em1012", FatalException, message);
  }
...
G4String G4EMDataSet::FullFileName(const G4String& name) const
{
  ...
  if (!path)
  {
    G4Exception("G4EMDataSet::FullFileName", "em0006", FatalException,
...
G4double G4EMDataSet::RandomSelect(G4int /* componentId */) const
{
  ...
  if (!pdf)
  {
    G4Exception("G4EMDataSet::RandomSelect", "em1012", FatalException,
  ...
  if (bin == 0)
    value = linearAlgo.Calculate(x, bin, *pdf, *energies);
  else
    value = algorithm->Calculate(x, bin, *pdf, *energies);
...
G4double G4EMDataSet::IntegrationFunction(G4double x)
{
  ...
  if (bin == 0)
    y = linearAlgo.Calculate(x, bin, *energies, *data);
  else
    y = algorithm->Calculate(x, bin, *energies, *data);
