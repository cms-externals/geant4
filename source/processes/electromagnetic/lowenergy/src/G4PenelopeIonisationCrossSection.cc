...
G4PenelopeIonisationCrossSection::G4PenelopeIonisationCrossSection()
  : G4VhShellCrossSection("Penelope"), fShellIDTable(nullptr), fCrossSectionHandler(nullptr)
{
...
G4double G4PenelopeIonisationCrossSection::CrossSection(G4int Z, G4AtomicShellEnumerator shell,
                                                        G4double incidentEnergy, G4double,
                                                        const G4Material* material)
{
  ...
  if (!material)
  {
    ...
    G4Exception("G4PenelopeIonisationCrossSection::CrossSection()", "em2042", FatalException, ed);
    return cross;
  }

  if (!fCrossSectionHandler) fCrossSectionHandler = new G4PenelopeIonisationXSHandler();
  ...
  if (G4int(shell) < nmax && incidentEnergy >= fLowEnergyLimit
      && incidentEnergy <= fHighEnergyLimit)
  {
    ...
    const G4PenelopeCrossSection* theXS =
      fCrossSectionHandler->GetCrossSectionTableForCouple(G4Electron::Electron(), material, 0.);
    ...
    if (theOsc->GetParentZ() != Z || theOsc->GetShellFlag() - 1 != G4int(shell))
    {
      ...
      ed << "Retrieved: " << theOsc->GetShellFlag() - 1 << " and Z = " << theOsc->GetParentZ()
         << G4endl;
      G4Exception("G4PenelopeIonisationCrossSection::CrossSection()", "em2043", JustWarning, ed);
      return cross;
    }
    ...
    if (atomsPerMolec) cross = crossPerMolecule / atomsPerMolec;

    if (fVerboseLevel > 0)
    {
      G4cout << "Cross section of shell " << G4int(shell) << " and Z= " << Z;
      G4cout << " of material: " << material->GetName() << " and energy = " << incidentEnergy / keV
             << " keV" << G4endl;
...
//....oooOO0OOooo........oooOO0OOooo........oooOO0OOooo........oooOO0OOooo....
std::vector<G4double> G4PenelopeIonisationCrossSection::GetCrossSection(G4int Z, G4double kinEnergy,
                                                                        G4double, G4double,
...
std::vector<G4double> G4PenelopeIonisationCrossSection::Probabilities(G4int Z, G4double kinEnergy,
                                                                      G4double, G4double,
                                                                      const G4Material* mat)
{
...
G4int G4PenelopeIonisationCrossSection::FindShellIDIndex(const G4Material* mat, G4int Z,
                                                         G4AtomicShellEnumerator shell)
{
  ...
  if (fShellIDTable->count(theKey))  // table already built, and containing the element
  {
    ...
    {
      G4ExceptionDescription ed;
      ed << "Shell ID: " << ishell << " not available for material " << mat->GetName()
         << " and Z = " << Z << G4endl;
  ...
  else
  {
    ...
    for (size_t iosc = 0; iosc < numberOfOscillators; iosc++)
    {
      ...
      if (theOsc->GetParentZ() == Z)
      {
        ...
        if (shFlag < 30) (*dat)[shFlag - 1] = (G4double)iosc;  // index of the given shell
