...
namespace
{
G4Mutex PenelopeBremsstrahlungModelMutex = G4MUTEX_INITIALIZER;
}
...
G4PenelopeBremsstrahlungModel::G4PenelopeBremsstrahlungModel(const G4ParticleDefinition* part,
  ...
  : G4VEmModel(nam),
    ...
    fParticle(nullptr),
    fPenelopeFSHelper(nullptr),
    fPenelopeAngular(nullptr),
    fEnergyGrid(nullptr),
...
G4PenelopeBremsstrahlungModel::~G4PenelopeBremsstrahlungModel()
{
  ...
  {
    ClearTables();
    if (fPenelopeFSHelper) delete fPenelopeFSHelper;
  }
  // This is thread-local at the moment
  if (fPenelopeAngular) delete fPenelopeAngular;
}
...
{
  if (fVerboseLevel > 3) G4cout << "Calling G4PenelopeBremsstrahlungModel::Initialise()" << G4endl;

  SetParticle(part);

  if (IsMaster() && part == fParticle)
  {
    if (!fPenelopeFSHelper) fPenelopeFSHelper = new G4PenelopeBremsstrahlungFS(fVerboseLevel);
    if (!fPenelopeAngular) fPenelopeAngular = new G4PenelopeBremsstrahlungAngular();
    // Clear and re-build the tables
    ClearTables();

    // forces the cleaning of tables, in this specific case
    if (fPenelopeAngular) fPenelopeAngular->Initialize();
    ...
    fEnergyGrid = new G4PhysicsLogVector(LowEnergyLimit(), HighEnergyLimit(),
                                         nBins - 1);  // one hidden bin is added

    fXSTableElectron =
      new std::map<std::pair<const G4Material*, G4double>, G4PenelopeCrossSection*>;
    fXSTablePositron =
      new std::map<std::pair<const G4Material*, G4double>, G4PenelopeCrossSection*>;
...
G4double
G4PenelopeBremsstrahlungModel::CrossSectionPerVolume(const G4Material* material,
                                                     const G4ParticleDefinition* theParticle,
                                                     G4double energy, G4double cutEnergy, G4double)
{
  //
  if (fVerboseLevel > 3)
  ...
  G4PenelopeCrossSection* theXS = GetCrossSectionTableForCouple(theParticle, material, cutEnergy);
                                                                   ...
                                                                   G4double, G4double)
{
  ...
  G4cout << "Penelope Bremsstrahlung model v2008 does not calculate cross section _per atom_ "
         << G4endl;
...
G4double
G4PenelopeBremsstrahlungModel::ComputeDEDXPerVolume(const G4Material* material,
                                                    const G4ParticleDefinition* theParticle,
                                                      ...
                                                      G4double cutG, G4double)
{
  ...
  G4double gammaEnergy = fPenelopeFSHelper->SampleGammaEnergy(kineticEnergy, material, cutG);
  ...
  G4DynamicParticle* theGamma =
    new G4DynamicParticle(G4Gamma::Gamma(), gammaDirection1, gammaEnergy);
  ...
  if (fVerboseLevel > 1)
  {
    ...
    G4cout << "Total final state: " << (residualPrimaryEnergy + gammaEnergy) / keV << " keV"
           << G4endl;
    G4cout << "-----------------------------------------------------------" << G4endl;
  }
  ...
  if (fVerboseLevel > 0)
  {
    ...
    if (energyDiff > 0.05 * keV)
      G4cout << "Warning from G4PenelopeBremsstrahlung: problem with energy conservation: "
             << (residualPrimaryEnergy + gammaEnergy) / keV << " keV (final) vs. "
...
void G4PenelopeBremsstrahlungModel::ClearTables()
{
  if (!IsMaster() && !fLocalTable)
    // Should not be here!
    G4Exception("G4PenelopeBremsstrahlungModel::ClearTables()", "em0100", FatalException,
  ...
  if (fEnergyGrid)
    delete fEnergyGrid;
  */
  if (fPenelopeFSHelper) fPenelopeFSHelper->ClearTables(IsMaster());

  if (fVerboseLevel > 2) G4cout << "G4PenelopeBremsstrahlungModel: cleared tables" << G4endl;
...
void G4PenelopeBremsstrahlungModel::BuildXSTable(const G4Material* mat, G4double cut)
{
  if (!IsMaster() && !fLocalTable)
    // Should not be here!
    G4Exception("G4PenelopeBremsstrahlungModel::BuildXSTable()", "em0100", FatalException,
                "Worker thread in this method");
  ...
  if (fXSTableElectron->count(theKey) && fXSTablePositron->count(theKey)) return;
  ...
  if (fVerboseLevel > 2)
  {
    ...
    G4cout << "for e+/e- in " << mat->GetName() << " for Ecut(gamma)= " << cut / keV << " keV "
           << G4endl;
  }
  ...
  if (fEnergyGrid->GetVectorLength() != nBins)
  {
    ...
    G4Exception("G4PenelopeBremsstrahlungModel::BuildXSTable()", "em2016", FatalException, ed);
  }
  ...
  for (std::size_t bin = 0; bin < nBins; ++bin)
  {
    ...
    G4double XS1A = fPenelopeFSHelper->GetMomentumIntegral(tempData, restrictedCut, 0);
    G4double XS2A = fPenelopeFSHelper->GetMomentumIntegral(tempData, restrictedCut, 1);
    G4double XH1A = 0, XH2A = 0;
    if (restrictedCut <= 1)
    {
      XH1A = fPenelopeFSHelper->GetMomentumIntegral(tempData, 1.0, 0) - XS1A;
      XH2A = fPenelopeFSHelper->GetMomentumIntegral(tempData, 1.0, 1) - XS2A;
    }
    delete[] tempData;
    ...
    XSEntry2->AddCrossSectionPoint(bin, energy, XH0 * posCorrection, XH1 * posCorrection,
                                   XH2 * posCorrection, XS0, XS1 * posCorrection,
                                   XS2 * posCorrection);
  }
                                                             ...
                                                             const G4Material* mat, G4double cut)
{
  if (part != G4Electron::Electron() && part != G4Positron::Positron())
  {
    ...
    G4Exception("G4PenelopeBremsstrahlungModel::GetCrossSectionTableForCouple()", "em0001",
                FatalException, ed);
    return nullptr;
  }
  ...
  if (part == G4Electron::Electron())
  {
    ...
    if (!fXSTableElectron)
    {
      ...
      G4Exception("G4PenelopeBremsstrahlungModel::GetCrossSectionTableForCouple()", "em2013",
                  JustWarning, excep);
      fLocalTable = true;
      fXSTableElectron =
        new std::map<std::pair<const G4Material*, G4double>, G4PenelopeCrossSection*>;
    ...
    else
    {
      ...
      if (fVerboseLevel > 0)
      {
        ...
        G4ExceptionDescription ed;
        ed << "Unable to find e- table for " << mat->GetName() << " at Ecut(gamma)= " << cut / keV
           << " keV " << G4endl;
        ed << "This can happen only in Unit Tests or via G4EmCalculator" << G4endl;
        G4Exception("G4PenelopeBremsstrahlungModel::GetCrossSectionTableForCouple()", "em2009",
                    JustWarning, ed);
      }
                                                                ...
                                                                G4double energy)
{
  ...
  // accuracy
  G4double t =
    G4Log(1.0 + 1e6 * energy / (electron_mass_c2 * fPenelopeFSHelper->GetEffectiveZSquared(mat)));
  G4double corr =
    1.0
    - G4Exp(
      -t
      * (1.2359e-1
         - t
             * (6.1274e-2
                - t
                    * (3.1516e-2
                       - t * (7.7446e-3 - t * (1.0595e-3 - t * (7.0568e-5 - t * 1.8080e-6)))))));
  return corr;
}
