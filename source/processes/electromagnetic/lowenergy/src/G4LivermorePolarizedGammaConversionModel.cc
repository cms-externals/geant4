...
namespace
{
G4Mutex LivermorePolarizedGammaConversionModelMutex = G4MUTEX_INITIALIZER;
}
                                                          ...
                                                          const G4DataVector& cuts)
{
  if (verboseLevel > 1)
  {
    G4cout << "Calling1 G4LivermorePolarizedGammaConversionModel::Initialise()" << G4endl
...
void G4LivermorePolarizedGammaConversionModel::ReadData(std::size_t Z, const char* path)
{
  if (verboseLevel > 1)
  {
    G4cout << "Calling ReadData() of G4LivermorePolarizedGammaConversionModel" << G4endl;
  }
  ...
  if (!datadir)
  {
    ...
    if (!datadir)
    {
      G4Exception("G4LivermorePolarizedGammaConversionModel::ReadData()", "em0006", FatalException,
  ...
  if (!fin.is_open())
  {
    ...
    G4Exception("G4LivermorePolarizedGammaConversionModel::ReadData()", "em0003", FatalException,
  ...
  else
  {
    if (verboseLevel > 3)
    {
      G4cout << "File " << ost.str() << " is opened by G4LivermorePolarizedGammaConversionModel"
             << G4endl;
    }
...
G4double G4LivermorePolarizedGammaConversionModel::ComputeCrossSectionPerAtom(
  const G4ParticleDefinition*, G4double GammaEnergy, G4double Z, G4double, G4double, G4double)
{
  if (verboseLevel > 1)
  {
    G4cout << "G4LivermorePolarizedGammaConversionModel::ComputeCrossSectionPerAtom()" << G4endl;
  }
...
void G4LivermorePolarizedGammaConversionModel::SampleSecondaries(
  ...
  const G4DynamicParticle* aDynamicGamma, G4double, G4double)
{
  ...
  else
  {
    ...
    if (element == nullptr)
    {
      G4cout << "G4LivermorePolarizedGammaConversionModel::SampleSecondaries - element = 0"
             << G4endl;
      return;
    }
    ...
    {
      G4cout << "G4LivermorePolarizedGammaConversionModel::SampleSecondaries - ionisation = 0"
             << G4endl;
      return;
    }
  ...
  SystemOfRefChange(gammaDirection0, electronDirection, gammaPolarization0);

  G4DynamicParticle* particle1 =
  ...
  SystemOfRefChange(gammaDirection0, positronDirection, gammaPolarization0);

  // Create G4DynamicParticle object for the particle2
  G4DynamicParticle* particle2 =
...
void G4LivermorePolarizedGammaConversionModel::SetTheta(G4double* p_cosTheta, G4double* p_sinTheta,
                                                        G4double Energy)
{
...
G4double G4LivermorePolarizedGammaConversionModel::Poli(G4double a, G4double b, G4double c,
                                                        G4double x)
{
  G4double value = 0.;
  if (x > 0.)
...
G4double G4LivermorePolarizedGammaConversionModel::Encu(G4double* p_p1, G4double* p_p2, G4double x0)
{
  ...
  for (i = 0; i < 100; ++i)
  {
    fx = (Flor(p_p1, x) * Glor(p_p1, x) - Ftan(p_p2, x)) / (Fdlor(p_p1, x) - Fdtan(p_p2, x));
    x -= fx;
    if (x > xmax)
    {
      return xmax;
    }
    if (std::fabs(fx) <= x * 1.0e-6)
    {
      break;
    }
  }
...
G4double G4LivermorePolarizedGammaConversionModel::Finvtan(G4double* p_p1, G4double cnor,
                                                           G4double r)
{
  G4double value = 0.;
  G4double a = p_p1[0];
...
G4ThreeVector
G4LivermorePolarizedGammaConversionModel::GetRandomPolarization(G4ThreeVector& direction0)
{
  ...
  const G4ThreeVector& gammaDirection, const G4ThreeVector& gammaPolarization) const
{
  ...
  return gammaPolarization
         - gammaPolarization.dot(gammaDirection) / gammaDirection.dot(gammaDirection)
             * gammaDirection;
}
