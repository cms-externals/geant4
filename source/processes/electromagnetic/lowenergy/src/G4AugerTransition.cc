...
// the final shell in wich the electron goes is needed, to know the data for the auger electron
// emitted (i.e. originating shell id, electron energy and transition probability)
G4AugerTransition::G4AugerTransition(
  G4int finalShell, std::vector<G4int> transIds,
...
const std::vector<G4int>* G4AugerTransition::AugerOriginatingShellIds(G4int startShellId) const
{
  ...
  {
    G4Exception("G4AugerTransition::AugerOriginatingShellIds()", "em2199", JustWarning,
                "Error: no Auger ID found");
    return nullptr;
  }
  const std::vector<G4int>* dataSet = &(*shellId).second;
  if (dataSet->empty())
    G4Exception("G4AugerTransition::AugerOriginatingShellIds()", "em2198", JustWarning,
                "Error: no Auger ID found");
  return dataSet;
}
...
const G4DataVector* G4AugerTransition::AugerTransitionProbabilities(G4int startShellId) const
{
  ...
  {
    G4Exception("G4AugerTransition::AugerTransitionProbabilities()", "de0002", JustWarning,
                "corresponding map element not found, energy deposited locally");
    return 0;
  }
