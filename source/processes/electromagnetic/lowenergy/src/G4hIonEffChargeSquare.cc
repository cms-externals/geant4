...
G4hIonEffChargeSquare::G4hIonEffChargeSquare(const G4String& name)
  : G4VLowEnergyModel(name), theHeMassAMU(4.0026)
{
  ;
}
...
G4double G4hIonEffChargeSquare::HighEnergyLimit(const G4ParticleDefinition*,
...
G4double G4hIonEffChargeSquare::LowEnergyLimit(const G4ParticleDefinition*, const G4Material*) const
{
  return 0.0;
}
...
G4double G4hIonEffChargeSquare::HighEnergyLimit(const G4ParticleDefinition*) const
{
  return 1.0 * TeV;
}
...
G4double G4hIonEffChargeSquare::LowEnergyLimit(const G4ParticleDefinition*) const
{
  return 0.0;
}
...
G4bool G4hIonEffChargeSquare::IsInCharge(const G4DynamicParticle*, const G4Material*) const
{
  return true;
}
...
G4bool G4hIonEffChargeSquare::IsInCharge(const G4ParticleDefinition*, const G4Material*) const
{
  return true;
}
...
G4double G4hIonEffChargeSquare::IonEffChargeSquare(const G4Material* material,
                                                   G4double kineticEnergy, G4double particleMass,
                                                   G4double ionCharge) const
{
  ...
  if ((reducedEnergy > ionCharge * 10.0 * MeV) || (ionCharge < 1.5)) return ionCharge * ionCharge;
  ...
  else
  {
    ...
    G4double q = 1.0 - G4Exp(0.803 * y3 - 1.3167 * y3 * y3 - 0.38157 * y - 0.008983 * y * y);
    if (q < 0.0) q = 0.0;
    ...
    G4double qeff =
      ionCharge * sLocal * (q + 0.5 * (1.0 - q) * std::log(1.0 + lambda * lambda) * v2);
