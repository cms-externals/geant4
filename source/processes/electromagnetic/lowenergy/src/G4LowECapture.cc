...
void G4LowECapture::SetKinEnergyLimit(G4double val)
{
  ...
  if (verboseLevel > 0)
  {
    G4cout << "### G4LowECapture: Tracking cut E(MeV) = " << kinEnergyThreshold / MeV << G4endl;
  }
}
...
void G4LowECapture::BuildPhysicsTable(const G4ParticleDefinition& part)
{
  ...
  for (G4int i = 0; i < nRegions; ++i)
  {
    ...
    if (r && verboseLevel > 0)
    {
      G4cout << "### G4LowECapture: new G4Region <" << regionName[i] << ">  with tracking cut "
  ...
  if (part.GetParticleType() == "nucleus")
  {
    ...
    if (pname != "deuteron" && pname != "triton" && pname != "alpha" && pname != "He3"
        && pname != "alpha+" && pname != "helium" && pname != "hydrogen")
    {
      isIon = true;
    }
  }
...
G4double G4LowECapture::PostStepGetPhysicalInteractionLength(const G4Track& aTrack, G4double,
...
G4VParticleChange* G4LowECapture::PostStepDoIt(const G4Track& aTrack, const G4Step&)
{
...
G4double G4LowECapture::GetMeanFreePath(const G4Track&, G4double, G4ForceCondition*)
{
  return DBL_MAX;
}
