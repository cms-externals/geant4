...
void G4LowEPPolarizedComptonModel::ReadData(std::size_t Z, const char* path)
{
  if (verboseLevel > 1)
  {
    G4cout << "G4LowEPPolarizedComptonModel::ReadData()" << G4endl;
  }
  if (data[Z])
  {
    return;
  ...
  if (!datadir)
  {
    ...
    if (!datadir)
    {
      G4Exception("G4LowEPPolarizedComptonModel::ReadData()", "em0006", FatalException,
  ...
  if (!fin.is_open())
  {
    ...
    ed << "G4LowEPPolarizedComptonModel data file <" << ost.str().c_str() << "> is not opened!"
       << G4endl;
    G4Exception("G4LowEPPolarizedComptonModel::ReadData()", "em0003", FatalException, ed,
  ...
  else
  {
    if (verboseLevel > 3)
    {
      G4cout << "File " << ost.str() << " is opened by G4LowEPPolarizedComptonModel" << G4endl;
    }
                                                                  ...
                                                                  G4double, G4double, G4double)
{
  if (verboseLevel > 3)
  {
    G4cout << "G4LowEPPolarizedComptonModel::ComputeCrossSectionPerAtom()" << G4endl;
  }
  G4double cs = 0.0;
                                                     ...
                                                     G4double, G4double)
{
  ...
  if (verboseLevel > 3)
  {
    G4cout << "G4LowEPPolarizedComptonModel::SampleSecondaries() E(MeV)= " << photonEnergy0 / MeV
  ...
  if (!(photonPolarization0.isOrthogonal(photonDirection0, 1e-6))
      || (photonPolarization0.mag() == 0))
  {
    photonPolarization0 = GetRandomPolarization(photonDirection0);
  }
  else
  {
    if ((photonPolarization0.howOrthogonal(photonDirection0) != 0)
        && (photonPolarization0.howOrthogonal(photonDirection0) > g4d_limit))
    {
  ...
  G4ThreeVector photonPolarization1 = SetNewPolarization(LowEPPCepsilon, sinT2, phi, cosTheta);
  ...
  do
  {
    ...
    do
    {
      ...
      u_temp = sqrt(((ePSI * ePSI) * (vel_c * vel_c))
                    / ((e_mass_kg * e_mass_kg) * (vel_c * vel_c) + (ePSI * ePSI)))
               / vel_c;

      // Sample incident electron direction, amorphous material, to scattering photon scattering
      // plane
      ...
      G4double denominator =
        (1.0 - cosTheta)
        + (gamma_temp * electron_mass_c2 * (1 - subdenom1 - subdenom2) / pEIncident);
    ...
    G4double var_C = (pERecoil - pEIncident)
                     - ((pERecoil * pEIncident) / (gamma_p * electron_mass_c2)) * (1 - cosTheta);

    G4double var_D1 = gamma * electron_mass_c2 * pERecoil;
    G4double var_D2 =
      (1 - (u_temp * cosTheta * cosAlpha) - (u_temp * sinTheta * cosBeta * sinAlpha));
    G4double var_D3 = ((electron_mass_c2 * electron_mass_c2) * (gamma * gamma_p - 1))
                      - (gamma_p * electron_mass_c2 * pERecoil);
    G4double var_D = var_D1 * var_D2 + var_D3;

    G4double var_E1 =
      ((gamma * gamma_p) * (electron_mass_c2 * electron_mass_c2) * (u_temp * u_p_temp) * cosAlpha);
    G4double var_E2 = gamma_p * electron_mass_c2 * pERecoil * u_p_temp * cosTheta;
    G4double var_E = var_E1 - var_E2;

    G4double var_F1 = ((gamma * gamma_p) * (electron_mass_c2 * electron_mass_c2)
                       * (u_temp * u_p_temp) * cosBeta * sinAlpha);
    G4double var_F2 = (gamma_p * electron_mass_c2 * pERecoil * u_p_temp * sinTheta);
    G4double var_F = var_F1 - var_F2;

    G4double var_G = (gamma * gamma_p) * (electron_mass_c2 * electron_mass_c2) * (u_temp * u_p_temp)
                     * sinBeta * sinAlpha;
    ...
    G4double var_Y = 2.0
                     * (((var_A * var_D - var_F * var_C) * (var_F * var_B - var_E * var_A))
                        - ((var_G * var_G) * var_B * var_C));
    ...
    // Confirm that diff less than zero is due FPE, i.e if abs of diff / diff1 and diff/ diff2 is
    // less than 10^(-g4d_order), then set diff to zero
    ...
    // Randomly sample one of the two possible solutions and determin theta angle of ejected Compton
    // electron
    ...
    G4double iSinThetaE =
      std::sqrt(1 + std::tan((pi / 2.0) - ThetaE) * std::tan((pi / 2.0) - ThetaE));
  ...
  SystemOfRefChange(photonDirection0, photonDirection1, photonPolarization0, photonPolarization1);
  ...
  if (pERecoil > 0.)
  {
    ...
    SystemOfRefChangeElect(photonDirection0, eDirection, photonPolarization0);

    G4DynamicParticle* dp =
      new G4DynamicParticle(G4Electron::Electron(), eDirection, eKineticEnergy);
    fvect->push_back(dp);
  }
  else
  {
  ...
  if (bindingE < 0.0)
    G4Exception("G4LowEPPolarizedComptonModel::SampleSecondaries()", "em2051", FatalException,
  ...
  const G4ThreeVector& photonDirection, const G4ThreeVector& photonPolarization) const
{
  ...
  return photonPolarization
         - photonPolarization.dot(photonDirection) / photonDirection.dot(photonDirection)
             * photonDirection;
}
                                                               ...
                                                               G4double costheta)
{
  ...
  if (rand1 < (LowEPPCepsilon + 1.0 / LowEPPCepsilon - 2)
                / (2.0 * (LowEPPCepsilon + 1.0 / LowEPPCepsilon) - 4.0 * sinT2 * cosP2))
  {
    if (rand2 < 0.5)
      theta = pi / 2.0;
    else
                                                     ...
                                                     G4ThreeVector& polarization1)
{
  ...
  polarization1 =
    (polarization_x * Axis_X0 + polarization_y * Axis_Y0 + polarization_z * Axis_Z0).unit();
}
