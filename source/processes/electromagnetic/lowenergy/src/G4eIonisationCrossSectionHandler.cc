...
//
//                           - BuildCrossSectionForMaterials method was revised in order to
//                           calculate the
...
G4eIonisationCrossSectionHandler::G4eIonisationCrossSectionHandler(const G4VEnergySpectrum* spec,
                                                                   ...
                                                                   G4int nbin)
  : G4VCrossSectionHandler(), theParam(spec), verbose(0)
{
...
G4eIonisationCrossSectionHandler::BuildCrossSectionsForMaterials(const G4DataVector& energyVector,
                                                                 const G4DataVector* energyCuts)
{
  ...
  for (G4int mLocal = 0; mLocal < numOfCouples; ++mLocal)
  {
    ...
    if (verbose > 0)
    {
      G4cout << "eIonisation CS for " << mLocal << "th material " << material->GetName()
    ...
    for (G4int i = 0; i < nElements; ++i)
    {
      ...
      for (std::size_t bin = 0; bin < nOfBins; ++bin)
      {
        ...
        if (e > tcut)
        {
          for (G4int n = 0; n < nShells; n++)
          {
            ...
            {
              G4cout << "G4eIonCrossSH: e(MeV)= " << e / MeV << " n= " << n << " cross= " << cross
                     << " p= " << p << " value= " << value << " tcut(MeV)= " << tcut / MeV
                     << " rho= " << density << " Z= " << Z << G4endl;
            }
          }
      ...
      G4VEMDataSet* elSet =
        new G4EMDataSet(i, energies, cs, log_energies, log_cs, algoLocal, 1., 1.);
...
G4double G4eIonisationCrossSectionHandler::GetCrossSectionAboveThresholdForElement(
  G4double energy, G4double cutEnergy, G4int Z)
{
  G4int nShells = NumberOfComponents(Z);
  G4double value = 0.;
  if (energy > cutEnergy)
