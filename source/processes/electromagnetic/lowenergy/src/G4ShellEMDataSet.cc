...
G4ShellEMDataSet::G4ShellEMDataSet(G4int zeta, G4VDataSetAlgorithm* algo, G4double eUnit,
                                   G4double dataUnit)
  : algorithm(algo), unitEnergies(eUnit), unitData(dataUnit), z(zeta)
{
  if (algorithm == nullptr)
    G4Exception("G4ShellEMDataSet::G4ShellEMDataSet()", "em0007", FatalErrorInArgument,
                "Interpolation == 0");
}
...
void G4ShellEMDataSet::SetEnergiesData(G4DataVector* energies, G4DataVector* data,
                                       G4int componentId)
{
...
void G4ShellEMDataSet::SetLogEnergiesData(G4DataVector* energies, G4DataVector* data,
                                          G4DataVector* log_energies, G4DataVector* log_data,
                                          G4int componentId)
{
  G4VEMDataSet* component = components[componentId];
  if (component)
...
G4bool G4ShellEMDataSet::LoadData(const G4String& file)
{
  ...
  do
  {
    ...
    if (a == -1)
    {
      ...
      {
        AddComponent(new G4EMDataSet(shellIndex, orig_shell_energies, orig_shell_data,
                                     log_shell_energies, log_shell_data, algorithm->Clone(),
...
G4bool G4ShellEMDataSet::LoadNonLogData(const G4String& file)
{
  ...
  do
  {
    ...
    if (a == -1)
    {
      ...
      {
        AddComponent(new G4EMDataSet(shellIndex, orig_shell_energies, orig_shell_data,
                                     algorithm->Clone(), unitEnergies, unitData));
        orig_shell_energies = 0;
        orig_shell_data = 0;
      }
    }
