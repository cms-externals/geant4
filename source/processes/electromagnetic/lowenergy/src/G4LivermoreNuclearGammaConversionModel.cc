...
namespace
{
G4Mutex LivermoreNuclearGammaConversionModelMutex = G4MUTEX_INITIALIZER;
}
                                                        ...
                                                        const G4DataVector& cuts)
{
  if (verboseLevel > 1)
  {
    G4cout << "Calling Initialise() of G4LivermoreNuclearGammaConversionModel." << G4endl
...
void G4LivermoreNuclearGammaConversionModel::ReadData(size_t Z, const char* path)
{
  if (verboseLevel > 1)
  {
    G4cout << "Calling ReadData() of G4LivermoreNuclearGammaConversionModel" << G4endl;
  }
  ...
  if (!datadir)
  {
    ...
    if (!datadir)
    {
      G4Exception("G4LivermoreNuclearGammaConversionModel::ReadData()", "em0006", FatalException,
  ...
  if (!fin.is_open())
  {
    ...
    G4Exception("G4LivermoreNuclearGammaConversionModel::ReadData()", "em0003", FatalException, ed,
  ...
  else
  {
    if (verboseLevel > 3)
    {
      G4cout << "File " << ost.str() << " is opened by G4LivermoreNuclearGammaConversionModel"
             << G4endl;
    }
...
G4double G4LivermoreNuclearGammaConversionModel::ComputeCrossSectionPerAtom(
  ...
  const G4DynamicParticle* aDynamicGamma, G4double, G4double)
{
  ...
  else
  {
    ...
    if (element == nullptr)
    {
      G4cout << "G4LivermoreNuclearGammaConversionModel::SampleSecondaries - element = 0" << G4endl;
      return;
    }
  ...
  G4double dxEle = std::sin(thetaEle) * std::cos(phi), dyEle = std::sin(thetaEle) * std::sin(phi),
           dzEle = std::cos(thetaEle);
  G4double dxPos = -std::sin(thetaPos) * std::cos(phi), dyPos = -std::sin(thetaPos) * std::sin(phi),
           dzPos = std::cos(thetaPos);
