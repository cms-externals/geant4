...
namespace
{
G4Mutex LivermoreBremsstrahlungModelMutex = G4MUTEX_INITIALIZER;
}
using namespace std;
...
G4LivermoreBremsstrahlungModel::G4LivermoreBremsstrahlungModel(const G4ParticleDefinition* p,
                                                               const G4String& nam)
...
void G4LivermoreBremsstrahlungModel::ReadData(G4int Z, const char* path)
{
  ...
  if (nullptr == datadir)
  {
    ...
    if (!datadir)
    {
      G4Exception("G4LivermoreBremsstrahlungModel::ReadData()", "em0006", FatalException,
  ...
  if (!fin.is_open())
  {
    ...
    G4Exception("G4LivermoreBremsstrahlungModel::ReadData()", "em0003", FatalException, ed,
  ...
  if (v->Retrieve(fin))
  {
    ...
    {
      v->SetBicubicInterpolation(true);
    }
    dataSB[Z] = v;
  ...
  else
  {
    ...
    G4Exception("G4LivermoreBremsstrahlungModel::ReadData()", "em0005", FatalException, ed,
                "G4LEDATA version should be G4EMLOW8.0 or later.");
    delete v;
  }
}
...
G4double G4LivermoreBremsstrahlungModel::ComputeDXSectionPerAtom(G4double gammaEnergy)
{
  ...
  G4double invb2 = fPrimaryTotalEnergy * fPrimaryTotalEnergy
                   / (fPrimaryKinEnergy * (fPrimaryKinEnergy + 2. * fPrimaryParticleMass));
  ...
  if (!fIsElectron)
  {
    ...
    if (e2 > 0.0)
    {
      G4double invbeta2 = (e2 + fPrimaryParticleMass) / sqrt(e2 * (e2 + 2. * fPrimaryParticleMass));
                                                       ...
                                                       G4double cutEnergy, G4double maxEnergy)
{
  ...
  const G4Element* elm = SelectRandomAtom(couple, fPrimaryParticle, kineticEnergy, cut, emax);
  ...
  */
  ...
  if (fIsElectron && x0 < 0.97 && ((kineticEnergy > epeaklimit) || (kineticEnergy < elowlimit)))
  {
  ...
  do
  {
    ...
    if (!fIsElectron)
    {
      ...
      G4double invbeta1 = (e1 + fPrimaryParticleMass) / sqrt(e1 * (e1 + 2 * fPrimaryParticleMass));
    ...
    if (v > 1.05 * vmax && nwarn < 5)
    {
      ++nwarn;
      G4ExceptionDescription ed;
      ed << "### G4LivermoreBremsstrahlungModel Warning: Majoranta exceeded! " << v << " > " << vmax
         << " by " << v / vmax << " Egamma(MeV)= " << gammaEnergy << " Ee(MeV)= " << kineticEnergy
      ...
      G4Exception("G4LivermoreBremsstrahlungModel::SampleScattering", "em0044", JustWarning, ed,
                  "");
    }
  ...
  G4ThreeVector gammaDirection = GetAngularDistribution()->SampleDirection(
    dp, fPrimaryTotalEnergy - gammaEnergy, Z, couple->GetMaterial());

  // create G4DynamicParticle object for the Gamma
  G4DynamicParticle* gamma = new G4DynamicParticle(fGammaParticle, gammaDirection, gammaEnergy);
  vdp->push_back(gamma);

  G4ThreeVector direction =
    (totMomentum * dp->GetMomentumDirection() - gammaEnergy * gammaDirection).unit();
  ...
  */
  ...
  if (gammaEnergy > SecondaryThreshold())
  {
    ...
    G4DynamicParticle* el =
      new G4DynamicParticle(const_cast<G4ParticleDefinition*>(fPrimaryParticle), direction, finalE);
    vdp->push_back(el);
