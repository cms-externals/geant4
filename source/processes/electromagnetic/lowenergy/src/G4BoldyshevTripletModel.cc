...
void G4BoldyshevTripletModel::Initialise(const G4ParticleDefinition*, const G4DataVector&)
{
  if (verboseLevel > 1)
  {
    G4cout << "Calling Initialise() of G4BoldyshevTripletModel." << G4endl
           << "Energy range: " << LowEnergyLimit() / MeV << " MeV - " << HighEnergyLimit() / GeV
           << " GeV isMaster: " << IsMaster() << G4endl;
  }
  // compute values only once
  energyThreshold = 1.1 * electron_mass_c2;
  momentumThreshold_c =
    std::sqrt(energyThreshold * energyThreshold - electron_mass_c2 * electron_mass_c2);
  G4double momentumThreshold_N = momentumThreshold_c / electron_mass_c2;
  G4double t =
    0.5 * G4Log(momentumThreshold_N + std::sqrt(momentumThreshold_N * momentumThreshold_N + 1.0));
...
G4double G4BoldyshevTripletModel::MinPrimaryEnergy(const G4Material*, const G4ParticleDefinition*,
                                                   G4double)
{
...
void G4BoldyshevTripletModel::ReadData(size_t Z, const char* path)
{
  if (verboseLevel > 1)
  {
    G4cout << "Calling ReadData() of G4BoldyshevTripletModel" << G4endl;
  }
  ...
  if (!datadir)
  {
    ...
    if (!datadir)
    {
      G4Exception("G4BoldyshevTripletModel::ReadData()", "em0006", FatalException,
  ...
  if (!fin.is_open())
  {
    ...
    ed << "G4BoldyshevTripletModel data file <" << ost.str().c_str() << "> is not opened!"
       << G4endl;
    G4Exception("G4BoldyshevTripletModel::ReadData()", "em0003", FatalException, ed,
  ...
  else
  {
    if (verboseLevel > 3)
    {
      G4cout << "File " << ost.str() << " is opened by G4BoldyshevTripletModel" << G4endl;
    }
...
G4double G4BoldyshevTripletModel::ComputeCrossSectionPerAtom(const G4ParticleDefinition* part,
                                                             G4double GammaEnergy, G4double Z,
                                                             G4double, G4double, G4double)
{
  if (verboseLevel > 1)
  {
    G4cout << "Calling ComputeCrossSectionPerAtom() of G4BoldyshevTripletModel" << G4endl;
  }
  ...
  if (verboseLevel > 1)
  {
    G4cout << "*** Triplet conversion xs for Z=" << Z << " at energy E(MeV)=" << GammaEnergy / MeV
                                                ...
                                                G4double)
{
  ...
  if (ener_re >= energyThreshold)
  {
    ...
    particle3 =
      new G4DynamicParticle(G4Electron::Electron(), electronRDirection, electronRKineEnergy);
  }
  else
  {
  ...
  G4double a =
    std::sqrt(16. / xb - 3. - 36. * re * xn + 36. * re * re * xn * xn + 6. * xb * re * xn);
...
namespace
{
G4Mutex BoldyshevTripletModelMutex = G4MUTEX_INITIALIZER;
}
