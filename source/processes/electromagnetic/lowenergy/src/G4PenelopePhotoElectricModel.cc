...
{
  if (fVerboseLevel > 3) G4cout << "Calling  G4PenelopePhotoElectricModel::Initialise()" << G4endl;
  ...
  if (IsMaster() && particle == fParticle)
  {
    ...
    for (G4int i = 0; i < (G4int)theCoupleTable->GetTableSize(); ++i)
    {
      ...
      for (std::size_t j = 0; j < material->GetNumberOfElements(); ++j)
      {
        ...
        if (!fLogAtomicShellXS[iZ]) ReadDataFile(iZ);
      }
    }
...
namespace
{
G4Mutex PenelopePhotoElectricModelMutex = G4MUTEX_INITIALIZER;
}
                                                                  ...
                                                                  G4double, G4double, G4double)
{
  ...
  if (!fLogAtomicShellXS[iZ])
  {
    ...
    if (fVerboseLevel > 0)
    {
      ...
      G4Exception("G4PenelopePhotoElectricModel::ComputeCrossSectionPerAtom()", "em2038",
                  JustWarning, ed);
    }
  ...
  if (!totalXSLog)
  {
    G4Exception("G4PenelopePhotoElectricModel::ComputeCrossSectionPerAtom()", "em2039",
                FatalException, "Unable to retrieve the total cross section table");
    return 0;
  }
                                                     ...
                                                     G4double, G4double)
{
  ...
  const G4Element* anElement = SelectRandomAtom(couple, G4Gamma::GammaDefinition(), photonEnergy);
  G4int Z = anElement->GetZasInt();
  if (fVerboseLevel > 2) G4cout << "Selected " << anElement->GetName() << G4endl;
  ...
  if (shellIndex >= numberOfShells) shellIndex = numberOfShells - 1;
  ...
  if (shellIndex == 9) bindingEnergy = 0. * eV;
  ...
  if (eKineticEnergy > 0.)
  {
    ...
    G4DynamicParticle* electron =
      new G4DynamicParticle(G4Electron::Electron(), electronDirection, eKineticEnergy);
  ...
  {
    G4Exception("G4PenelopePhotoElectricModel::SampleSecondaries()", "em2099", JustWarning,
  ...
  if (fVerboseLevel > 1)
  {
    ...
    G4cout << "-----------------------------------------------------------" << G4endl;
    if (eKineticEnergy) G4cout << "Outgoing electron " << eKineticEnergy / keV << " keV" << G4endl;
  ...
  if (fVerboseLevel > 0)
  {
    G4double energyDiff = std::fabs(eKineticEnergy + energyInFluorescence + localEnergyDeposit
                                    + energyInAuger - photonEnergy);
...
void G4PenelopePhotoElectricModel::ReadDataFile(G4int Z)
{
  if (!IsMaster())
    // Should not be here!
    G4Exception("G4PenelopePhotoElectricModel::ReadDataFile()", "em0100", FatalException,
  ...
  if (!path)
  {
    ...
    G4Exception("G4PenelopePhotoElectricModel::ReadDataFile()", "em0006", FatalException, excep);
    return;
  }
  ...
  */
  ...
  {
    G4String excep =
      "G4PenelopePhotoElectricModel - data file " + G4String(ost.str()) + " not found!";
    G4Exception("G4PenelopePhotoElectricModel::ReadDataFile()", "em0003", FatalException, excep);
  }
  ...
  if (fVerboseLevel > 3) G4cout << "Element Z=" << Z << " , nShells = " << nShells << G4endl;
  ...
  if (readZ != Z || nShells <= 0 || nShells > 50)  // protect nShell against large values
  {
    ...
    G4Exception("G4PenelopePhotoElectricModel::ReadDataFile()", "em0005", FatalException, ed);
    return;
  }
  ...
  if (fVerboseLevel > 2)
  {
    G4cout << "G4PenelopePhotoElectricModel: read " << k << " points for element Z = " << Z
           << G4endl;
  }
...
std::size_t G4PenelopePhotoElectricModel::GetNumberOfShellXS(G4int Z)
{
  if (!IsMaster())
    // Should not be here!
    G4Exception("G4PenelopePhotoElectricModel::GetNumberOfShellXS()", "em0100", FatalException,
                "Worker thread in this method");

  // read data files
  if (!fLogAtomicShellXS[Z]) ReadDataFile(Z);
  // now it should be ok
  if (!fLogAtomicShellXS[Z])
  {
    G4ExceptionDescription ed;
    ed << "Cannot find shell cross section data for Z=" << Z << G4endl;
    G4Exception("G4PenelopePhotoElectricModel::GetNumberOfShellXS()", "em2038", FatalException, ed);
  }
...
G4double G4PenelopePhotoElectricModel::GetShellCrossSection(G4int Z, std::size_t shellID,
                                                            G4double energy)
{
  ...
  if (!totalXSLog)
  {
    G4Exception("G4PenelopePhotoElectricModel::GetShellCrossSection()", "em2039", FatalException,
...
std::size_t G4PenelopePhotoElectricModel::SelectRandomShell(G4int Z, G4double energy)
{
  ...
  if (!fLogAtomicShellXS[Z])
  {
    G4ExceptionDescription ed;
    ed << "Cannot find shell cross section data for Z=" << Z << G4endl;
    G4Exception("G4PenelopePhotoElectricModel::SelectRandomShell()", "em2038", FatalException, ed);
  }
