...
G4PenelopeBremsstrahlungAngular::G4PenelopeBremsstrahlungAngular()
  : G4VEmAngularDistribution("Penelope"),
...
void G4PenelopeBremsstrahlungAngular::ReadDataFile()
{
  ...
  if (!path)
  {
    ...
    G4Exception("G4PenelopeBremsstrahlungAngular::ReadDataFile()", "em0006", FatalException, excep);
    return;
  }
  ...
  if (!file.is_open())
  {
    ...
    G4Exception("G4PenelopeBremsstrahlungAngular::ReadDataFile()", "em0003", FatalException, excep);
    return;
  }
    ...
    for (i = 0; i < fNumberofZPoints; i++)
      for (j = 0; j < fNumberofEPoints; j++)
      {
        ...
        else
        {
          ...
          G4Exception("G4PenelopeBremsstrahlungAngular::ReadDataFile()", "em0005", FatalException,
                      ed);
        }
      }
  file.close();
...
void G4PenelopeBremsstrahlungAngular::PrepareTables(const G4Material* material, G4bool /*isMaster*/)
{
  ...
  if (!fDataRead)
  {
    ReadDataFile();
    if (!fDataRead)
      G4Exception("G4PenelopeBremsstrahlungAngular::PrepareInterpolationTables()", "em2001",
                  FatalException, "Unable to build interpolation table");
  }

  if (!fLorentzTables1) fLorentzTables1 = new std::map<G4double, G4PhysicsTable*>;
  if (!fLorentzTables2) fLorentzTables2 = new std::map<G4double, G4PhysicsTable*>;
  ...
  for (i = 0; i < fNumberofEPoints; i++)
  {
    ...
    {
      G4PhysicsFreeVector* QQ1vector = new G4PhysicsFreeVector(fNumberofZPoints, /*spline =*/true);
      G4PhysicsFreeVector* QQ2vector = new G4PhysicsFreeVector(fNumberofZPoints, /*spline =*/true);
  ...
  G4double pE[fNumberofEPoints] = {1.0e-03 * MeV, 5.0e-03 * MeV, 1.0e-02 * MeV,
                                   5.0e-02 * MeV, 1.0e-01 * MeV, 5.0e-01 * MeV};
  ...
  else
  {
    ...
    delete theTable1;
    delete theTable2;
    G4Exception("G4PenelopeBremsstrahlungAngular::PrepareInterpolationTables()", "em2005",
                FatalException, ed);
  }
...
G4ThreeVector& G4PenelopeBremsstrahlungAngular::SampleDirection(const G4DynamicParticle* dp,
                                                                G4double eGamma, G4int,
                                                                const G4Material* material)
{
  if (!material)
  {
    G4Exception("G4PenelopeBremsstrahlungAngular::SampleDirection()", "em2040", FatalException,
                "The pointer to G4Material* is nullptr");
  ...
  if (!fEffectiveZSq)
  {
    G4Exception("G4PenelopeBremsstrahlungAngular::SampleDirection()", "em2040", JustWarning,
                "EffectiveZSq table does not exist: create it");
  ...
  {
    G4cout << "Effective <Z> for material : " << material->GetName() << " = " << Zmat << G4endl;
  }

  G4double ePrimary = dp->GetKineticEnergy();

  G4double beta =
    std::sqrt(ePrimary * (ePrimary + 2 * electron_mass_c2)) / (ePrimary + electron_mass_c2);
  G4double cdt = 0;
  G4double sinTheta = 0;
  G4double phi = 0;
  ...
  if (ePrimary > 500 * keV)
  {
    ...
    fLocalDirection.set(sinTheta * std::cos(phi), sinTheta * std::sin(phi), cdt);
    // rotate
  ...
  if (!(fLorentzTables1->count(Zmat)) || !(fLorentzTables2->count(Zmat)))
  {
    ...
    G4Exception("G4PenelopeBremsstrahlungAngular::SampleDirection()", "em2006", FatalException, ed);
  }
...
G4double G4PenelopeBremsstrahlungAngular::CalculateEffectiveZ(const G4Material* material)
{
  ...
  if (fEffectiveZSq->count(material)) return fEffectiveZSq->find(material)->second;
