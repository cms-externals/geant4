...
{
  if (fVerboseLevel > 3) G4cout << "Calling G4PenelopeAnnihilationModel::Initialise()" << G4endl;
  SetParticle(part);
...
{
  if (fVerboseLevel > 3)
    G4cout << "Calling ComputeCrossSectionPerAtom() of G4PenelopeAnnihilationModel" << G4endl;
                                                    ...
                                                    G4double, G4double)
{
  ...
  if (kineticEnergy == 0.0)
  {
    ...
    G4DynamicParticle* firstGamma =
      new G4DynamicParticle(G4Gamma::Gamma(), direction, electron_mass_c2);
    G4DynamicParticle* secondGamma =
      new G4DynamicParticle(G4Gamma::Gamma(), -direction, electron_mass_c2);
  ...
  G4DynamicParticle* aParticle1 =
    new G4DynamicParticle(G4Gamma::Gamma(), photon1Direction, photon1Energy);
  ...
  G4DynamicParticle* aParticle2 =
    new G4DynamicParticle(G4Gamma::Gamma(), photon2Direction, photon2Energy);
  ...
  if (fVerboseLevel > 1)
  {
    ...
    G4cout << "Total final state: " << (photon1Energy + photon2Energy) / keV << " keV" << G4endl;
    G4cout << "-----------------------------------------------------------" << G4endl;
  }
  if (fVerboseLevel > 0)
  {
    ...
    if (energyDiff > 0.05 * keV)
      G4cout << "Warning from G4PenelopeAnnihilation: problem with energy conservation: "
             << (photon1Energy + photon2Energy) / keV << " keV (final) vs. "
...
G4double G4PenelopeAnnihilationModel::ComputeCrossSectionPerElectron(G4double energy)
{
  ...
  G4double crossSection =
    fPielr2 * ((gamma2 + 4.0 * gamma + 1.0) * G4Log(gamma + f1) / f2 - (gamma + 3.0) / f1)
    / (gamma + 1.0);
  return crossSection;
}
