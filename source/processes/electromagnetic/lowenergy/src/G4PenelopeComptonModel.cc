...
{
  if (fVerboseLevel > 3) G4cout << "Calling G4PenelopeComptonModel::Initialise()" << G4endl;
  ...
  if (IsMaster() && part == fParticle)
  {
    ...
    if (LowEnergyLimit() < fIntrinsicLowEnergyLimit)
    {
      ...
      ed << "Using the Penelope Compton model outside its intrinsic validity range. " << G4endl;
      ...
      G4Exception("G4PenelopeComptonModel::Initialise()", "em2100", JustWarning, ed);
    }
  }
...
{
  if (fVerboseLevel > 3) G4cout << "Calling  G4PenelopeComptonModel::InitialiseLocal()" << G4endl;
  //
  ...
  if (part == fParticle)
  {
    ...
    const G4PenelopeComptonModel* theModel = static_cast<G4PenelopeComptonModel*>(masterModel);
                                                       ...
                                                       G4double energy, G4double, G4double)
{
  ...
  // which is integrated numerically in cos(theta),
  // G4PenelopeComptonModel::DifferentialCrossSection(). The parametrization includes the J(p)
  ...
  if (fVerboseLevel > 2)
    G4cout << "Compton mean free path at " << energy / keV << " keV for material "
           << material->GetName() << " = " << (csvolume ? (1. / csvolume) / mm : DBL_MAX) << " mm"
           << G4endl;
  return csvolume;
}
...
// G4EmCalculator.
G4double G4PenelopeComptonModel::ComputeCrossSectionPerAtom(const G4ParticleDefinition*, G4double,
                                               ...
                                               G4double)
{
  ...
  else  // photonEnergy0 < 5 MeV
  {
    ...
    do
    {
      do
      {
        ...
        if (A < 0.5)
          pzomc =
            (std::sqrt(0.5) - std::sqrt(0.5 - G4Log(2.0 * A))) / (std::sqrt(2.0) * hartreeFunc);
        else
  ...
  G4double Q2 = photonEnergy0 * photonEnergy0
                + photonEnergy1 * (photonEnergy1 - 2.0 * photonEnergy0 * cosTheta);
  ...
  electron = new G4DynamicParticle(G4Electron::Electron(), eDirection, eKineticEnergy);
  fvect->push_back(electron);

  if (localEnergyDeposit < 0)  // Should not be: issue a G4Exception (warning)
  {
    G4Exception("G4PenelopeComptonModel::SampleSecondaries()", "em2099", JustWarning,
  ...
  if (electron) electronEnergy = eKineticEnergy;
  if (fVerboseLevel > 1)
  {
    G4cout << "-----------------------------------------------------------" << G4endl;
    ...
    G4cout << "Total final state: "
           << (photonEnergy1 + electronEnergy + energyInFluorescence + localEnergyDeposit
               + energyInAuger)
                / keV
           << " keV" << G4endl;
    G4cout << "-----------------------------------------------------------" << G4endl;
  ...
  if (fVerboseLevel > 0)
  {
    G4double energyDiff = std::fabs(photonEnergy1 + electronEnergy + energyInFluorescence
    ...
    if (energyDiff > 0.05 * keV)
      G4cout << "Warning from G4PenelopeCompton: problem with energy conservation: "
             << (photonEnergy1 + electronEnergy + energyInFluorescence + energyInAuger
                 + localEnergyDeposit)
                  / keV
             << " keV (final) vs. " << photonEnergy0 / keV << " keV (initial)" << G4endl;
  }
}
                                                          ...
                                                          G4PenelopeOscillator* osc)
{
  ...
  G4double Pzimax = (aux - electron_mass_c2 * ionEnergy)
                    / (electron_mass_c2 * std::sqrt(2 * aux + ionEnergy * ionEnergy));
  G4double sia = 0.0;
  ...
  if (std::fabs(Pzimax) < pf)
  {
    ...
    G4double dspz = std::sqrt(QCOE2) * (1.0 + ECOE * (ECOE - cosTheta) / QCOE2) * harFunc * 0.25
                    * (2 * p2 - (p2 * p2) / (pf * pf) - (pf * pf));
                                                             ...
                                                             G4PenelopeOscillator* osc)
{
  ...
  const G4int nst = 256;
  static const G4double Abscissas[10] = {
    7.652651133497334e-02,  2.2778585114164508e-01, 3.7370608871541956e-01, 5.1086700195082710e-01,
    6.3605368072651503e-01, 7.4633190646015079e-01, 8.3911697182221882e-01, 9.1223442825132591e-01,
    9.6397192727791379e-01, 9.9312859918509492e-01};
  static const G4double Weights[10] = {
    1.5275338713072585e-01, 1.4917298647260375e-01, 1.4209610931838205e-01, 1.3168863844917663e-01,
    1.1819453196151842e-01, 1.0193011981724044e-01, 8.3276741576704749e-02, 6.2672048334109064e-02,
  ...
  for (G4int i = 2; i <= npoints; i++)
  {
    c = a * Abscissas[i - 1];
    d += Weights[i - 1]
         * (DifferentialCrossSection(b + c, energy, osc)
            + DifferentialCrossSection(b - c, energy, osc));
  }
  ...
  do
  {
    ...
    if (Err < Ctol || LHN == 0) loopAgain = false;  // end of cycle
    LH = LHN;
