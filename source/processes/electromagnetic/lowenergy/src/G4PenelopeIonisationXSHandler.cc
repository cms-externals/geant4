...
G4PenelopeIonisationXSHandler::G4PenelopeIonisationXSHandler(size_t nb)
  : fXSTableElectron(nullptr), fXSTablePositron(nullptr), fDeltaTable(nullptr), fEnergyGrid(nullptr)
{
  fNBins = nb;
  ...
  fXSTableElectron = new std::map<std::pair<const G4Material*, G4double>, G4PenelopeCrossSection*>;
  fXSTablePositron = new std::map<std::pair<const G4Material*, G4double>, G4PenelopeCrossSection*>;
...
G4PenelopeIonisationXSHandler::~G4PenelopeIonisationXSHandler()
{
  ...
  if (fEnergyGrid) delete fEnergyGrid;

  if (fVerboseLevel > 2)
    G4cout << "G4PenelopeIonisationXSHandler. Tables have been cleared" << G4endl;
}

//....oooOO0OOooo........oooOO0OOooo........oooOO0OOooo........oooOO0OOooo....

const G4PenelopeCrossSection* G4PenelopeIonisationXSHandler::GetCrossSectionTableForCouple(
  const G4ParticleDefinition* part, const G4Material* mat, const G4double cut) const
{
  if (part != G4Electron::Electron() && part != G4Positron::Positron())
  {
    G4ExceptionDescription ed;
    ed << "Invalid particle: " << part->GetParticleName() << G4endl;
    G4Exception("G4PenelopeIonisationXSHandler::GetCrossSectionTableForCouple()", "em0001",
                FatalException, ed);
    return nullptr;
  }
  ...
  if (part == G4Electron::Electron())
  {
    if (!fXSTableElectron)
    {
      G4Exception("G4PenelopeIonisationXSHandler::GetCrossSectionTableForCouple()", "em0028",
                  FatalException, "The Cross Section Table for e- was not initialized correctly!");
      return nullptr;
    }
...
void G4PenelopeIonisationXSHandler::BuildXSTable(const G4Material* mat, G4double cut,
                                                 const G4ParticleDefinition* part, G4bool isMaster)
{
  // Just to check
  if (!isMaster)
    G4Exception("G4PenelopeIonisationXSHandler::BuildXSTable()", "em0100", FatalException,
  ...
  if (!(fDeltaTable->count(mat))) BuildDeltaTable(mat);
  ...
  if (fEnergyGrid->GetVectorLength() != fNBins)
  {
    ...
    G4Exception("G4PenelopeIonisationXSHandler::BuildXSTable()", "em2030", FatalException, ed);
  }
  ...
  for (size_t bin = 0; bin < fNBins; bin++)
  {
    ...
    for (size_t iosc = 0; iosc < numberOfOscillators; iosc++)
    {
      ...
      {
        G4ExceptionDescription ed;
        ed << "Problem in calculating the shell XS for shell # " << iosc << G4endl;
        G4Exception("G4PenelopeIonisationXSHandler::BuildXSTable()", "em2031", FatalException, ed);
        delete XSEntry;
        return;
      }
                                                             ...
                                                             const G4double energy) const
{
  ...
  if (!fDeltaTable)
  {
    G4Exception("G4PenelopeIonisationXSHandler::GetDensityCorrection()", "em2032", FatalException,
  ...
  else
  {
    ...
    G4Exception("G4PenelopeIonisationXSHandler::GetDensityCorrection()", "em2033", FatalException,
                ed);
  }
...
void G4PenelopeIonisationXSHandler::BuildDeltaTable(const G4Material* mat)
{
  ...
  if (fEnergyGrid->GetVectorLength() != fNBins)
  {
    ...
    ed << fNBins << " " << fEnergyGrid->GetVectorLength() << G4endl;
    G4Exception("G4PenelopeIonisationXSHandler::BuildDeltaTable()", "em2030", FatalException, ed);
  }
    ...
    if (fdel >= TST)  // if fdel < TST, delta = 0
    {
      ...
      do
      {
        ...
        if (fdel > TST)
          ...
          wl2u = wl2;
        if ((wl2u - wl2l) > 1e-12 * wl2) loopAgain = true;
      } while (loopAgain);
      ...
      for (size_t i = 0; i < numberOfOscillators; i++)
      {
        ...
        delta += theOscLocal3->GetOscillatorStrength() * G4Log(1.0 + (wl2 / (wri * wri)));
      }
...
//....oooOO0OOooo........oooOO0OOooo........oooOO0OOooo........oooOO0OOooo....
G4DataVector* G4PenelopeIonisationXSHandler::ComputeShellCrossSectionsElectron(
  G4PenelopeOscillator* theOsc, G4double energy, G4double cut, G4double delta)
{
  //
  ...
  if (energy < ionEnergy) return result;
  ...
  if (energy > resEne)
  {
    ...
    if (resEne > 1e-6 * energy)
      QM =
        std::sqrt((cp - cp1) * (cp - cp1) + electron_mass_c2 * electron_mass_c2) - electron_mass_c2;
    else
    {
    ...
    G4double SDL1 = 0;
    if (QM < cutoffEne)
      SDL1 = G4Log(cutoffEne * (QM + 2.0 * electron_mass_c2)
  ...
  if (wl < wu - (1e-5 * eV))
  {
    ...
    H2 += (2.0 - amol) * (wu - wl) + (wu * (2.0 * ee - wu) / (ee - wu))
          - (wl * (2.0 * ee - wl) / (ee - wl)) + (3.0 - amol) * ee * G4Log((ee - wu) / (ee - wl))
...
//....oooOO0OOooo........oooOO0OOooo........oooOO0OOooo........oooOO0OOooo....
G4DataVector* G4PenelopeIonisationXSHandler::ComputeShellCrossSectionsPositron(
  G4PenelopeOscillator* theOsc, G4double energy, G4double cut, G4double delta)
{
  //
  // This method calculates the hard and soft cross sections (H0-H1-H2-S0-S1-S2) for
  ...
  if (wl < wu - (1e-5 * eV))
  {
    G4double wlSq = wl * wl;
    G4double wuSq = wu * wu;
    H0 += (1.0 / wl) - (1.0 / wu) - bha1 * G4Log(wu / wl) / energy + bha2 * (wu - wl) / energySq
          - bha3 * (wuSq - wlSq) / (2.0 * energySq * energy)
          + bha4 * (wuSq * wu - wlSq * wl) / (3.0 * energySq * energySq);
    H1 += G4Log(wu / wl) - bha1 * (wu - wl) / energy + bha2 * (wuSq - wlSq) / (2.0 * energySq)
  ...
  S0 += (1.0 / wl) - (1.0 / wu) - bha1 * G4Log(wu / wl) / energy + bha2 * (wu - wl) / energySq
        - bha3 * (wuSq - wlSq) / (2.0 * energySq * energy)
        + bha4 * (wuSq * wu - wlSq * wl) / (3.0 * energySq * energySq);

  S1 += G4Log(wu / wl) - bha1 * (wu - wl) / energy + bha2 * (wuSq - wlSq) / (2.0 * energySq)
        - bha3 * (wuSq * wu - wlSq * wl) / (3.0 * energySq * energy)
        + bha4 * (wuSq * wuSq - wlSq * wlSq) / (4.0 * energySq * energySq);
