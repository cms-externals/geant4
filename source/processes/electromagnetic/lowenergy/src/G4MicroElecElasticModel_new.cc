...
G4MicroElecElasticModel_new::~G4MicroElecElasticModel_new()
{
  ...
  for (iterator_angle = thetaDataStorage.begin(); iterator_angle != thetaDataStorage.end();
       ++iterator_angle)
  {
  ...
  for (iterator_energy = eIncidentEnergyStorage.begin();
       iterator_energy != eIncidentEnergyStorage.end(); ++iterator_energy)
  {
  ...
  for (iterator_proba = eProbaStorage.begin(); iterator_proba != eProbaStorage.end();
       ++iterator_proba)
  {
...
{
  if (verboseLevel > -1) G4cout << "Calling G4MicroElecElasticModel_new::Initialise()" << G4endl;
  // Energy limits
  ...
  for (G4int i = 0; i < numOfCouples; ++i)
  {
    ...
    G4cout << "MicroElasticModel, Material " << i + 1 << " / " << numOfCouples << " : "
    ...
    G4MicroElecCrossSectionDataSet_new* tableE =
      new G4MicroElecCrossSectionDataSet_new(new G4LogLogInterpolation, eV, scaleFactor);
    ...
    eFullFileName << path
                  << "/microelec/Elastic/elsepa_elastic_cumulated_diffcross_e_" + matName + ".dat";
    G4cout << "Elastic Cumulated Diff Cross : " << eFullFileName.str().c_str() << G4endl;
    std::ifstream eDiffCrossSection(eFullFileName.str().c_str());

    if (!eDiffCrossSection)
      G4Exception("G4MicroElecElasticModel_new::Initialise", "em0003", FatalException,
  ...
  if (verboseLevel > 0)
  {
    G4cout << "MicroElec Elastic model is initialized " << G4endl
           ...
           << " MeV" << G4endl;  // system("pause"); linux doesn't like
  }
                                                            ...
                                                            G4double ekin, G4double, G4double)
{
  ...
  if (lowEPos == lowEnergyLimitTable.end() || highEPos == highEnergyLimitTable.end()
      || killEPos == workFunctionTable.end())
  {
  ...
  else
  {
    ...
    if (tablepos != tableTCS.end())
    {
      ...
      if (ekin >= lowEnergyLimit && ekin < highEnergyLimit)
      {
        ...
        {
          G4Exception("G4MicroElecElasticModel_new::ComputeCrossSectionPerVolume", "em0002",
    ...
    // Hsing-YinChangaAndrewAlvaradoaTreyWeberaJaimeMarianab Monte Carlo modeling of low-energy
    // electron-induced secondary electron emission yields in micro-architected boron nitride
    // surfaces - ScienceDirect, (n.d.).
    // https://www.sciencedirect.com/science/article/pii/S0168583X19304069 (accessed April 1, 2022).
...
G4double G4MicroElecElasticModel_new::AcousticCrossSectionPerVolume(G4double ekin, G4double kbz,
                                                                    G4double rho, G4double cs,
                                                                    G4double Aac, G4double Eac,
                                                                    G4double prefactor)
{
  G4double e = 1.6e-19, m0 = 9.10938356e-31, h = 1.0546e-34, kb = 1.38e-23;

  G4double E = (ekin / eV) * e;
  G4double D = (2 / (std::sqrt(2) * std::pow(pi, 2) * std::pow(h, 3))) * (1 + 2 * E)
               * std::pow(m0, 1.5) * std::sqrt(E);

  // Parametres SiO2
  G4double T = 300, Ebz = (std::pow(h, 2) * std::pow(kbz, 2)) / (2 * m0), hwbz = cs * kbz * h,
           nbz = 1.0 / (exp(hwbz / (kb * T)) - 1), Pac;
  ...
  {
    Pac = ((2 * pi * m0 * (2 * nbz + 1)) / (h * rho * hwbz)) * std::pow(Eac, 2) * D * E * 2
          * std::pow((Aac / E), 2) * (((-E / Aac) / (1 + (E / Aac))) + log(1 + (E / Aac)));
  }
  else  // Linear interpolation
  {
    G4double fEbz = ((2 * pi * m0 * (2 * nbz + 1)) / (h * rho * hwbz)) * std::pow(Eac, 2) * D * Ebz
                    * 2 * std::pow((Aac / Ebz), 2)
                    * (((-Ebz / Aac) / (1 + (Ebz / Aac))) + log(1 + (Ebz / Aac)));
    G4double fEbz4 = ((pi * kb * T) / (h * std::pow(cs, 2) * rho)) * (std::pow(Eac, 2) * D)
                     / (1 + ((Ebz / 4) / Aac));
...
G4double G4MicroElecElasticModel_new::DamageEnergy(G4double T, G4double A, G4double Z)
{
  ...
  g_epsilon_d =
    epsilon_d + 0.40244 * std::pow(epsilon_d, (3. / 4.)) + 3.4008 * std::pow(epsilon_d, (1. / 6.));
...
G4double G4MicroElecElasticModel_new::Theta(G4ParticleDefinition* particleDefinition, G4double k,
                                            G4double integrDiff)
{
  G4double theta = 0.;
  G4double valueT1 = 0;
  ...
  if (particleDefinition == G4Electron::ElectronDefinition())
  {
    ...
    if (iterator_angle != thetaDataStorage.end() && iterator_energy != eIncidentEnergyStorage.end()
        && iterator_proba != eProbaStorage.end())
    {
...
G4double G4MicroElecElasticModel_new::LinLogInterpolate(G4double e1, G4double e2, G4double e,
...
G4double G4MicroElecElasticModel_new::LinLinInterpolate(G4double e1, G4double e2, G4double e,
                                                        G4double xs1, G4double xs2)
{
  G4double d1 = xs1;
  G4double d2 = xs2;
...
G4double G4MicroElecElasticModel_new::LogLogInterpolate(G4double e1, G4double e2, G4double e,
                                                        G4double xs1, G4double xs2)
{
...
G4double G4MicroElecElasticModel_new::QuadInterpolator(G4double e11, G4double e12, G4double e21,
...
void G4MicroElecElasticModel_new::SetKillBelowThreshold(G4double threshold)
{
  ...
  {
    G4Exception("*** WARNING : the G4MicroElecElasticModel class is not validated below 5 eV !", "",
                JustWarning, "");
