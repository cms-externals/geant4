...
namespace
{
G4Mutex G4JAEAPolarizedElasticScatteringModelMutex = G4MUTEX_INITIALIZER;
}
using namespace std;
...
void G4JAEAPolarizedElasticScatteringModel::ReadData(std::size_t Z, const char* path)
{
  if (verboseLevel > 1)
  {
    G4cout << "Calling ReadData() of G4JAEAPolarizedElasticScatteringModel" << G4endl;
  }
  ...
  if (!datadir)
  {
    ...
    if (!datadir)
    {
      G4Exception("G4JAEAPolarizedElasticScatteringModel::ReadData()", "em0006", FatalException,
  ...
  if (!ES_Data_Buffer.is_open())
  {
    ...
    G4Exception("G4JAEAPolarizedElasticScatteringModel::ReadData()", "em0003", FatalException, ed,
                "G4LEDATA version should be G4EMLOW7.11 or later. Polarized Elastic Scattering "
                "Data are not loaded");
    return;
  }
  else
  {
    if (verboseLevel > 3)
    {
      G4cout << "File " << ostCS.str() << " is opened by G4JAEAPolarizedElasticScatteringModel"
             << G4endl;
    }
  }

  if (!Polarized_ES_Data[Z]) Polarized_ES_Data[Z] = new G4DataVector();
  ...
  const G4ParticleDefinition*, G4double GammaEnergy, G4double Z, G4double, G4double, G4double)
{
  ...
  if (verboseLevel > 1)
  {
    G4cout << "G4JAEAPolarizedElasticScatteringModel::ComputeCrossSectionPerAtom()" << G4endl;
  }
...
void G4JAEAPolarizedElasticScatteringModel::SampleSecondaries(
  std::vector<G4DynamicParticle*>*, const G4MaterialCutsCouple* couple,
  const G4DynamicParticle* aDynamicGamma, G4double, G4double)
{
  if (verboseLevel > 1)
  {
    G4cout << "Calling SampleSecondaries() of G4JAEAPolarizedElasticScatteringModel." << G4endl;
  }
  ...
  if ((gammaPolarization0.mag()) > 1.05 || (Xi1 * Xi1 > 1.05) || (Xi2 * Xi2 > 1.05)
      || (Xi3 * Xi3 > 1.05))
  {
    G4Exception("G4JAEAPolarizedElasticScatteringModel::SampleSecondaries()", "em1006", JustWarning,
                "WARNING: G4JAEAPolarizedElasticScatteringModel is only compatible with a unit "
                "polarization vector.");
    return;
  }
  ...
  Phi_Linear1 = GeneratePolarizedPhi(aparaSquare + aperpSquare + Xi1 * (aparaSquare - aperpSquare),
                                     aparaSquare + aperpSquare - Xi1 * (aparaSquare - aperpSquare),
                                     InitialAzimuth);
  ...
  Xi1_Prime =
    ...
    / ((aparaSquare + aperpSquare) + Xi1 * (aparaSquare - aperpSquare) * cos(2 * Phi_Linear1));
  Xi2_Prime =
    (-Xi1 * apara_aper_Asterisk * sin(2 * Phi_Linear1))
    / ((aparaSquare + aperpSquare) + Xi1 * (aparaSquare - aperpSquare) * cos(2 * Phi_Linear1));
  Xi3_Prime =
    (-Xi1 * img_apara_aper_Asterisk * sin(2 * Phi_Linear1))
    / ((aparaSquare + aperpSquare) + Xi1 * (aparaSquare - aperpSquare) * cos(2 * Phi_Linear1));
  ...
  if (abs(Xi1) > 0.0)
    dsigmaL1 = 0.25
               * ((aparaSquare + aperpSquare) * (1 + Xi1 * Xi1_Prime * cos(2 * Phi_Linear1))
  ...
  Xi1_Prime =
    ...
    / ((aparaSquare + aperpSquare) + Xi2 * (aparaSquare - aperpSquare) * sin(2 * Phi_Linear2));
  Xi2_Prime =
    (Xi2 * apara_aper_Asterisk * cos(2 * Phi_Linear2))
    / ((aparaSquare + aperpSquare) + Xi2 * (aparaSquare - aperpSquare) * sin(2 * Phi_Linear2));
  Xi3_Prime =
    (Xi2 * img_apara_aper_Asterisk * cos(2 * Phi_Linear2))
    / ((aparaSquare + aperpSquare) + Xi2 * (aparaSquare - aperpSquare) * sin(2 * Phi_Linear2));
  ...
  if (abs(Xi2) > 0.0)
    dsigmaL2 = 0.25
               * ((aparaSquare + aperpSquare) * (1 + Xi2 * Xi1_Prime * sin(2 * Phi_Linear2))
  ...
  if (abs(Xi3) > 0.0)
    dsigmaC =
      0.25
      * (aparaSquare + aperpSquare + Xi1_Prime * (aparaSquare - aperpSquare)
         - Xi3 * Xi2_Prime * img_apara_aper_Asterisk + Xi3 * Xi3_Prime * apara_aper_Asterisk);
  ...
  {
    G4Exception("G4JAEAPolarizedElasticScatteringModel::SampleSecondaries()", "em1007", JustWarning,
