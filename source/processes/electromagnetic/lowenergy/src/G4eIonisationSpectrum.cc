...
G4eIonisationSpectrum::G4eIonisationSpectrum()
  : G4VEnergySpectrum(), lowestE(0.1 * eV), factor(1.3), iMax(24), verbose(0)
{
...
G4double G4eIonisationSpectrum::Probability(G4int Z, G4double tMin, G4double tMax, G4double e,
                                            G4int shell, const G4ParticleDefinition*) const
{
  ...
  G4double bindingEnergy =
    (G4AtomicTransitionManager::Instance())->Shell(Z, shell)->BindingEnergy();
  ...
  if (verbose > 1)
  {
    G4cout << "G4eIonisationSpectrum::Probability: Z= " << Z << "; shell= " << shell
           << "; E(keV)= " << e / keV << "; Eb(keV)= " << bindingEnergy / keV << "; x1= " << x1
           << "; x2= " << x2 << G4endl;
  }
  ...
  else
  {
    G4cout << "WARNING: G4eIonisationSpectrum::Probability "
           << "parameter p[3] <= 0. G4LEDATA dabatase might be corrupted for Z = " << Z
           << ". Please check and/or update it " << G4endl;
  }
  ...
  if (verbose > 1)
  {
    G4cout << "tcut= " << tMin << "; tMax= " << tMax << "; x0= " << x0 << "; x1= " << x1
           << "; x2= " << x2 << "; val= " << val << "; nor= " << nor << "; sum= " << p[0]
           << "; a= " << p[1] << "; b= " << p[2] << "; c= " << p[3] << G4endl;
...
G4double G4eIonisationSpectrum::AverageEnergy(G4int Z, G4double tMin, G4double tMax, G4double e,
                                              G4int shell, const G4ParticleDefinition*) const
{
  ...
  if (verbose > 1)
  {
    G4cout << "G4eIonisationSpectrum::AverageEnergy: Z= " << Z << "; shell= " << shell
           << "; E(keV)= " << e / keV << "; bindingE(keV)= " << bindingEnergy / keV
           << "; x1= " << x1 << "; x2= " << x2 << G4endl;
  }
  ...
  if (verbose > 1)
  {
    G4cout << "tcut(MeV)= " << tMin / MeV << "; tMax(MeV)= " << tMax / MeV << "; x0= " << x0
           << "; x1= " << x1 << "; x2= " << x2 << "; val= " << val << "; nor= " << nor
           << "; sum= " << p[0] << "; a= " << p[1] << "; b= " << p[2] << "; c= " << p[3] << G4endl;
  }
...
G4double G4eIonisationSpectrum::SampleEnergy(G4int Z, G4double tMin, G4double tMax, G4double e,
                                             G4int shell, const G4ParticleDefinition*) const
{
  ...
  if (verbose > 1)
  {
    G4cout << "G4eIonisationSpectrum::SampleEnergy: Z= " << Z << "; shell= " << shell
           << "; E(keV)= " << e / keV << G4endl;
  }
    ...
    do
    {
      ...
      if (fun > amaj)
      {
        G4cout << "WARNING in G4eIonisationSpectrum::SampleEnergy:"
               << " Majoranta " << amaj << " < " << fun << " in the first aria at x= " << x
               << G4endl;
      }
  ...
  else
  {
    ...
    do
    {
      ...
      {
        G4cout << "WARNING in G4eIonisationSpectrum::SampleEnergy:"
               << " Majoranta " << amaj << " < " << fun << " in the second aria at x= " << x
               << G4endl;
      }
  ...
  if (verbose > 1)
  {
    G4cout << "tcut(MeV)= " << tMin / MeV << "; tMax(MeV)= " << tMax / MeV << "; x1= " << x1
           << "; x2= " << x2 << "; a1= " << a1 << "; a2= " << a2 << "; x= " << x
           << "; be= " << bindingEnergy << "; e= " << e << "; tDelta= " << tDelta << G4endl;
  }

  return tDelta;
}

G4double G4eIonisationSpectrum::IntSpectrum(G4double xMin, G4double xMax,
                                            const G4DataVector& p) const
{
  ...
  xs1 = 1. / x1;
  xs2 = 1. / x2;
  q = (xs1 - xs2) * (1.0 - p[0]) - p[iMax] * std::log(x2 / x1) + (1. - p[iMax]) * (x2 - x1)
      + 1. / (1. - x2) - 1. / (1. - x1) + p[iMax] * std::log((1. - x2) / (1. - x1))
...
G4double G4eIonisationSpectrum::AverageValue(G4double xMin, G4double xMax,
                                             const G4DataVector& p) const
{
  G4double sum = 0.0;
  if (xMin >= xMax) return sum;
    ...
    for (size_t i = 0; i < 19; i++)
    {
      ...
      else if (xMin < x2)
      {
        ...
        if (x2 > x1)
        {
          ...
          if (xs2 > xs1)
          {
            sum += std::log(xs2 / xs1) * (ys1 * xs2 - ys2 * xs1) / (xs2 - xs1) + ys2 - ys1;
          }
        }
      }
      x1 = x2;
  ...
  sum += std::log(x2 / x1) * (1.0 - p[0]) + 0.5 * (1. - p[iMax]) * (x2 * x2 - x1 * x1)
         + 1. / (1. - x2) - 1. / (1. - x1) + (1. + p[iMax]) * std::log((1. - x2) / (1. - x1))
         + 0.5 * p[0] * (xs1 - xs2);
