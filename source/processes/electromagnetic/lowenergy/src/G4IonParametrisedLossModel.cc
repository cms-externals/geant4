...
G4IonParametrisedLossModel::G4IonParametrisedLossModel(const G4ParticleDefinition*,
                                                       const G4String& nam)
  : G4VEmModel(nam),
    braggIonModel(0),
    betheBlochModel(0),
                                                        ...
                                                        G4double kineticEnergy)
{
  ...
  G4double tmax =
    2.0 * electron_mass_c2 * tau * (tau + 2.)
    / (1. + 2.0 * (tau + 1.) * cacheElecMassRatio + cacheElecMassRatio * cacheElecMassRatio);
...
{  // Kinetic energy
  ...
  chargeSquareRatio = corrections->EffectiveChargeSquareRatio(particle, material, kinEnergy);
                                            ...
                                            const G4DataVector& cuts)
{
  ...
  if (!isInitialised)
  {
    ...
    isInitialised = true;
    AddDEDXTable("ICRU73", new G4IonStoppingData("ion_stopping_data/icru", icru90),
      ...
      for (; iter != iter_end; ++iter)
      {
        ...
        if ((*iter)->BuildDEDXTable(atomicNumberIon, material))
        {
          ...
          G4cout << "  Atomic Number Ion = " << atomicNumberIon
                 << ", Material = " << material->GetName() << ", Table = " << (*iter)->GetName()
                 << G4endl;
#endif
          break;
        }
...
G4double G4IonParametrisedLossModel::ComputeCrossSectionPerAtom(
  const G4ParticleDefinition* particle, G4double kineticEnergy, G4double atomicNumber, G4double,
  G4double cutEnergy, G4double maxKinEnergy)
{
  ...
  if (cutEnergy < tmax)
  {
    ...
    G4double betaSquared = kineticEnergy * (energy + cacheMass) / (energy * energy);

    crosssection =
      1.0 / cutEnergy - 1.0 / maxEnergy - betaSquared * std::log(maxEnergy / cutEnergy) / tmax;
  ...
  G4cout << "########################################################" << G4endl
         << "# G4IonParametrisedLossModel::ComputeCrossSectionPerAtom" << G4endl
         << "# particle =" << particle->GetParticleName() << G4endl
         << "# cut(MeV) = " << cutEnergy / MeV << G4endl;

  G4cout << "#" << std::setw(13) << std::right << "E(MeV)" << std::setw(14) << "CS(um)"
         << std::setw(14) << "E_max_sec(MeV)" << G4endl
         << "# ------------------------------------------------------" << G4endl;

  G4cout << std::setw(14) << std::right << kineticEnergy / MeV << std::setw(14)
         << crosssection / (um * um) << std::setw(14) << tmax / MeV << G4endl;
#endif
                                                           ...
                                                           G4double cutEnergy, G4double maxEnergy)
{
  ...
  G4double cross =
    ComputeCrossSectionPerAtom(particle, kineticEnergy, nbElecPerVolume, 0, cutEnergy, maxEnergy);
                                                          ...
                                                          G4double cutEnergy)
{
  ...
  if (iter != lossTableList.end())
  {
    ...
    if (transitionEnergy > kineticEnergy)
    {
      ...
      G4double dEdxDeltaRays =
        DeltaRayMeanEnergyTransferRate(material, particle, kineticEnergy, cutEnergy);
      dEdx -= dEdxDeltaRays;
    }
    else
    {
      ...
      {
        dEdx = betheBlochModel->ComputeDEDXPerVolume(material, genericIon, scaledKineticEnergy,
                                                     cutEnergy);

        dEdx *= chargeSquare;

        dEdx += corrections->ComputeIonCorrections(particle, material, kineticEnergy);

        G4double factor = 1.0 + dedxCacheTransitionFactor / kineticEnergy;
  ...
  else
  {
    ...
    {
      dEdx =
        braggIonModel->ComputeDEDXPerVolume(material, genericIon, scaledKineticEnergy, cutEnergy);

      dEdx *= chargeSquare;
    }
    else
    {
      G4double dEdxLimitParam =
        braggIonModel->ComputeDEDXPerVolume(material, genericIon, lowEnergyLimit, cutEnergy);

      G4double dEdxLimitBetheBloch =
        betheBlochModel->ComputeDEDXPerVolume(material, genericIon, lowEnergyLimit, cutEnergy);

      if (particle != genericIon)
      {
        G4double chargeSquareLowEnergyLimit =
          GetChargeSquareRatio(particle, material, lowEnergyLimit / massRatio);

        dEdxLimitParam *= chargeSquareLowEnergyLimit;
        dEdxLimitBetheBloch *= chargeSquareLowEnergyLimit;

        dEdxLimitBetheBloch +=
          corrections->ComputeIonCorrections(particle, material, lowEnergyLimit / massRatio);
      }
      ...
      dEdx =
        betheBlochModel->ComputeDEDXPerVolume(material, genericIon, scaledKineticEnergy, cutEnergy);
...
{  // Logarithmic scaling of energy
  ...
  G4cout << "# dE/dx table for " << particle->GetParticleName() << " in material "
         << material->GetName() << " of density " << materialDensity / g * cm3 << " g/cm3" << G4endl
         << "# Projectile mass number A1 = " << atomicMassNumber << G4endl
         << "# ------------------------------------------------------" << G4endl;
  G4cout << "#" << std::setw(13) << std::right << "E" << std::setw(14) << "E/A1" << std::setw(14)
         << "dE/dx" << std::setw(14) << "1/rho*dE/dx" << G4endl;
  G4cout << "#" << std::setw(13) << std::right << "(MeV)" << std::setw(14) << "(MeV)"
         << std::setw(14) << "(MeV/cm)" << std::setw(14) << "(MeV*cm2/mg)" << G4endl
         << "# ------------------------------------------------------" << G4endl;
  ...
  G4double deltaEnergy = (energyUpperBoundary - energyLowerBoundary) / G4double(nmbBins);
  ...
  for (int i = 0; i < numBins + 1; i++)
  {
    ...
    G4cout.precision(6);
    G4cout << std::setw(14) << std::right << energy / MeV << std::setw(14)
           << energy / atomicMassNumber / MeV << std::setw(14) << dedx / MeV * cm << std::setw(14)
           << dedx / materialDensity / (MeV * cm2 / (0.001 * g)) << G4endl;
  }
}
...
{  // Logarithmic scaling of energy
  ...
  for (; iter != iter_end; iter++)
  {
    ...
    if (isApplicable)
    {
      (*iter)->PrintDEDXTable(particle, material, lowerBoundary, upperBoundary, numBins,
                              logScaleEnergy);
      break;
    }
  }
                                                   ...
                                                   G4double userMaxKinEnergySec)
{
  ...
  G4double maxKinEnergySec = std::min(rossiMaxKinEnergySec, userMaxKinEnergySec);
  ...
  do
  {
    ...
    kinEnergySec =
      cutKinEnergySec * maxKinEnergySec / (maxKinEnergySec * (1.0 - xi) + cutKinEnergySec * xi);
    ...
    if (grej > 1.0)
    {
      G4cout << "G4IonParametrisedLossModel::SampleSecondary Warning: "
             << "Majorant 1.0 < " << grej << " for e= " << kinEnergySec << G4endl;
    }
  ...
  G4DynamicParticle* delta = new G4DynamicParticle(
    electron, GetAngularDistribution()->SampleDirection(particle, kinEnergySec, Z, mat),
    kinEnergySec);
                                                  ...
                                                  const G4MaterialCutsCouple* matCutsCouple)
{
  ...
  if (particle == rangeCacheParticle && matCutsCouple == rangeCacheMatCutsCouple)
  {}
  else
  {
...
void G4IonParametrisedLossModel::UpdateDEDXCache(const G4ParticleDefinition* particle,
                                                 const G4Material* material, G4double cutEnergy)
{
  // ############## Caching ##################################################
  ...
  if (particle == dedxCacheParticle && material == dedxCacheMaterial
  ...
  else
  {
    ...
    if (iter != lossTableList.end())
    {
      ...
      G4double transitionEnergy = (*iter)->GetUpperEnergyEdge(particle, material);
      dedxCacheTransitionEnergy = transitionEnergy;

      // Computing dE/dx from low-energy parameterisation at
      // transition energy
      G4double dEdxParam = (*iter)->GetDEDX(particle, material, transitionEnergy);

      G4double dEdxDeltaRays =
        DeltaRayMeanEnergyTransferRate(material, particle, transitionEnergy, cutEnergy);
      ...
      G4double dEdxBetheBloch = betheBlochModel->ComputeDEDXPerVolume(
        material, genericIon, scaledTransitionEnergy, cutEnergy);
      dEdxBetheBloch *= transitionChargeSquare;

      // Additionally, high order corrections are added
      dEdxBetheBloch += corrections->ComputeIonCorrections(particle, material, transitionEnergy);

      // Computing transition factor from both dE/dx values
      dedxCacheTransitionFactor = (dEdxParam - dEdxBetheBloch) / dEdxBetheBloch * transitionEnergy;
    }
    else
    {
      dedxCacheParticle = particle;
      ...
      dedxCacheGenIonMassRatio = genericIonPDGMass / particle->GetPDGMass();
                                                      ...
                                                      const G4double& length, G4double& eloss)
{
  ...
  if (iter != lossTableList.end())
  {
    ...
    G4cout << "########################################################" << G4endl
           ...
           << "# cut(MeV) = " << cutEnergy / MeV << G4endl;
    G4cout << "#" << std::setw(13) << std::right << "E(MeV)" << std::setw(14) << "l(um)"
           << std::setw(14) << "l*dE/dx(MeV)" << std::setw(14) << "(l*dE/dx)/E" << G4endl
           << "# ------------------------------------------------------" << G4endl;
    G4cout << std::setw(14) << std::right << kineticEnergy / MeV << std::setw(14) << length / um
           << std::setw(14) << eloss / MeV << std::setw(14) << eloss / kineticEnergy * 100.0
           << G4endl;
#endif
    ...
    {
      eloss = ComputeLossForStep(CurrentCouple(), particle, kineticEnergy, length);
  ...
  G4double q2 = corrections->EffectiveChargeSquareRatio(particle, material, energy);
                                                  ...
                                                  const G4MaterialCutsCouple* matCutsCouple)
{
  ...
  G4double logDeltaEnergy = (logUpperEnergyEdge - logLowerEnergyEdge) / G4double(nmbBins);

  G4double logDeltaIntegr = logDeltaEnergy / G4double(nmbSubBins);

  G4PhysicsFreeVector* energyRangeVector =
    new G4PhysicsFreeVector(nmbBins + 1, lowerEnergy, upperEnergy,
                            /*spline=*/true);

  G4double dedxLow = ComputeDEDXPerVolume(material, particle, lowerEnergy, cutEnergy);
  ...
  for (std::size_t i = 1; i < nmbBins + 1; ++i)
  {
    ...
    for (std::size_t j = 0; j < nmbSubBins; ++j)
    {
      ...
      G4double dedxValue = ComputeDEDXPerVolume(material, particle, energyIntegr, cutEnergy);

      if (dedxValue > 0.0) range += deltaIntegr / dedxValue;

#ifdef PRINT_DEBUG_DETAILS
      G4cout << "   E = " << energyIntegr / MeV << " MeV -> dE = " << deltaIntegr / MeV
             << " MeV -> dE/dx = " << dedxValue / MeV * mm
             << " MeV/mm -> dE/(dE/dx) = " << deltaIntegr / dedxValue / mm
             << " mm -> range = " << range / mm << " mm " << G4endl;
#endif
    }
...
#ifdef PRINT_DEBUG_DETAILS
    G4cout << "G4IonParametrisedLossModel::BuildRangeVector() bin = " << i
           << ", E = " << energy / MeV << " MeV, R = " << range / mm << " mm" << G4endl;
#endif
  }
  // vector is filled: activate spline
  energyRangeVector->FillSecondDerivatives();

  G4double lowerRangeEdge = energyRangeVector->Value(lowerEnergy);
  G4double upperRangeEdge = energyRangeVector->Value(upperEnergy);

  G4PhysicsFreeVector* rangeEnergyVector =
    new G4PhysicsFreeVector(nmbBins + 1, lowerRangeEdge, upperRangeEdge,
                            /*spline=*/true);

  for (std::size_t i = 0; i < nmbBins + 1; ++i)
  {
    G4double energy = energyRangeVector->Energy(i);
    rangeEnergyVector->PutValues(i, energyRangeVector->Value(energy), energy);
  }

  rangeEnergyVector->FillSecondDerivatives();

#ifdef PRINT_DEBUG_TABLES
  G4cout << *energyLossVector << *energyRangeVector << *rangeEnergyVector << G4endl;
#endif
...
#ifdef PRINT_DEBUG
    G4cout << "G4IonParametrisedLossModel::ComputeLossForStep() range = " << range / mm
           << " mm, step = " << stepLength / mm << " mm" << G4endl;
#endif
...
G4bool G4IonParametrisedLossModel::AddDEDXTable(const G4String& nam, G4VIonDEDXTable* table,
                                                G4VIonDEDXScalingAlgorithm* algorithm)
{
  if (table == 0)
  {
    G4cout << "G4IonParametrisedLossModel::AddDEDXTable() Cannot "
           << " add table: Invalid pointer." << G4endl;
  ...
  for (; iter != iter_end; ++iter)
  {
    ...
    {
      G4cout << "G4IonParametrisedLossModel::AddDEDXTable() Cannot "
             << " add table: Name already exists." << G4endl;
  ...
  if (scalingAlgorithm == 0) scalingAlgorithm = new G4VIonDEDXScalingAlgorithm;
...
G4bool G4IonParametrisedLossModel::RemoveDEDXTable(const G4String& nam)
{
  LossTableList::iterator iter = lossTableList.begin();
  LossTableList::iterator iter_end = lossTableList.end();
