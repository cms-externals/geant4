...
G4double G4PenelopeGammaConversionModel::fAtomicScreeningRadius[] = {
  0.,  // pad a zero, so to use fAtomicScreeningRadius[Z]
  1.2281e+02, 7.3167e+01, 6.9228e+01, 6.7301e+01, 6.4696e+01, 6.1228e+01, 5.7524e+01, 5.4033e+01,
  5.0787e+01, 4.7851e+01, 4.6373e+01, 4.5401e+01, 4.4503e+01, 4.3815e+01, 4.3074e+01, 4.2321e+01,
  4.1586e+01, 4.0953e+01, 4.0524e+01, 4.0256e+01, 3.9756e+01, 3.9144e+01, 3.8462e+01, 3.7778e+01,
  3.7174e+01, 3.6663e+01, 3.5986e+01, 3.5317e+01, 3.4688e+01, 3.4197e+01, 3.3786e+01, 3.3422e+01,
  3.3068e+01, 3.2740e+01, 3.2438e+01, 3.2143e+01, 3.1884e+01, 3.1622e+01, 3.1438e+01, 3.1142e+01,
  3.0950e+01, 3.0758e+01, 3.0561e+01, 3.0285e+01, 3.0097e+01, 2.9832e+01, 2.9581e+01, 2.9411e+01,
  2.9247e+01, 2.9085e+01, 2.8930e+01, 2.8721e+01, 2.8580e+01, 2.8442e+01, 2.8312e+01, 2.8139e+01,
  2.7973e+01, 2.7819e+01, 2.7675e+01, 2.7496e+01, 2.7285e+01, 2.7093e+01, 2.6911e+01, 2.6705e+01,
  2.6516e+01, 2.6304e+01, 2.6108e+01, 2.5929e+01, 2.5730e+01, 2.5577e+01, 2.5403e+01, 2.5245e+01,
  2.5100e+01, 2.4941e+01, 2.4790e+01, 2.4655e+01, 2.4506e+01, 2.4391e+01, 2.4262e+01, 2.4145e+01,
  2.4039e+01, 2.3922e+01, 2.3813e+01, 2.3712e+01, 2.3621e+01, 2.3523e+01, 2.3430e+01, 2.3331e+01,
  2.3238e+01, 2.3139e+01, 2.3048e+01, 2.2967e+01, 2.2833e+01, 2.2694e+01, 2.2624e+01, 2.2545e+01,
...
G4PenelopeGammaConversionModel::~G4PenelopeGammaConversionModel()
{
  ...
  if (IsMaster() || fLocalTable)
  {
    ...
    if (fEffectiveCharge) delete fEffectiveCharge;
    if (fMaterialInvScreeningRadius) delete fMaterialInvScreeningRadius;
    if (fScreeningFunction) delete fScreeningFunction;
  }
}
  ...
  if (IsMaster() && part == fParticle)
  {
    ...
    for (G4int i = 0; i < (G4int)theCoupleTable->GetTableSize(); ++i)
    {
      ...
      for (std::size_t j = 0; j < material->GetNumberOfElements(); ++j)
      {
        ...
        if (iZ <= fMaxZ && !fLogAtomicCrossSection[iZ]) ReadDataFile(iZ);
      }

      // check if material data are available
      if (!fEffectiveCharge->count(material)) InitializeScreeningFunctions(material);
    }
    if (fVerboseLevel > 0)
    {
...
namespace
{
G4Mutex PenelopeGammaConversionModelMutex = G4MUTEX_INITIALIZER;
}
                                                                    ...
                                                                    G4double, G4double, G4double)
{
  ...
  if (energy < fIntrinsicLowEnergyLimit) return 0;
  ...
  if (!fLogAtomicCrossSection[iZ])
  {
    ...
    if (fVerboseLevel > 0)
    {
      ...
      G4Exception("G4PenelopeGammaConversionModel::ComputeCrossSectionPerAtom()", "em2018",
                  JustWarning, ed);
    }
                                                       ...
                                                       G4double, G4double)
{
  ...
  if (!fEffectiveCharge->count(mat))
  {
    ...
    if (fVerboseLevel > 0)
    {
      // Issue a G4Exception (warning) only in verbose mode
      G4ExceptionDescription ed;
      ed << "Unable to allocate the EffectiveCharge data for " << mat->GetName() << G4endl;
      ed << "This can happen only in Unit Tests" << G4endl;
      G4Exception("G4PenelopeGammaConversionModel::SampleSecondaries()", "em2019", JustWarning, ed);
    }
    // protect file reading via autolock
    G4AutoLock lock(&PenelopeGammaConversionModelMutex);
  ...
  else
  {
    ...
    do
    {
      ...
      if (G4UniformRand() <= p1)
      {
        ...
        g1 = scree.first;
        g1 = std::max(g1 + g0, 0.);
        if (G4UniformRand() * g1min > g1) loopAgain = true;
      }
      else
      {
        ...
        g2 = scree.second;
        g2 = std::max(g2 + g0, 0.);
        if (G4UniformRand() * g2min > g2) loopAgain = true;
      }
    } while (loopAgain);
  }
  if (fVerboseLevel > 4) G4cout << "Sampled eps = " << eps << G4endl;
  ...
  if (electronKineEnergy > 0.0)
  {
    ...
    G4DynamicParticle* electron =
      new G4DynamicParticle(G4Electron::Electron(), electronDirection, electronKineEnergy);
  ...
  G4DynamicParticle* positron =
    new G4DynamicParticle(G4Positron::Positron(), positronDirection, positronKineEnergy);
  ...
  if (fVerboseLevel > 1)
  {
    ...
    if (electronKineEnergy)
      G4cout << "Electron (explicitly produced) " << electronKineEnergy / keV << " keV" << G4endl;
    ...
    G4cout << "Total final state: "
           << (electronKineEnergy + positronKineEnergy + localEnergyDeposit
  ...
  if (fVerboseLevel > 0)
  {
    G4double energyDiff = std::fabs(electronKineEnergy + positronKineEnergy + localEnergyDeposit
                                    + 2.0 * electron_mass_c2 - photonEnergy);
...
void G4PenelopeGammaConversionModel::ReadDataFile(const G4int Z)
{
  if (!IsMaster())
    // Should not be here!
    G4Exception("G4PenelopeGammaConversionModel::ReadDataFile()", "em0100", FatalException,
  ...
  if (!path)
  {
    ...
    G4Exception("G4PenelopeGammaConversionModel::ReadDataFile()", "em0006", FatalException, excep);
    return;
  }
  ...
  */
  ...
  {
    G4String excep =
      "G4PenelopeGammaConversionModel - data file " + G4String(ost.str()) + " not found!";
    G4Exception("G4PenelopeGammaConversionModel::ReadDataFile()", "em0003", FatalException, excep);
  }
  ...
  if (fVerboseLevel > 3) G4cout << "Element Z=" << Z << G4endl;
  ...
  if (readZ != Z)
  {
    ...
    G4Exception("G4PenelopeGammaConversionModel::ReadDataFile()", "em0005", FatalException, ed);
  }
...
void G4PenelopeGammaConversionModel::InitializeScreeningFunctions(const G4Material* material)
{
  ...
  else  // many elements...let's do the calculation
  {
    ...
    if (intZ <= 0) intZ = 1;
    if (intZ > fMaxZ) intZ = fMaxZ;
  }

  if (fEffectiveCharge) fEffectiveCharge->insert(std::make_pair(material, zeff));
              ...
              - alzSquared
                  * (0.00835
                     - alzSquared
                         * (0.00201
                            - alzSquared
                                * (0.00049 - alzSquared * (0.00012 - alzSquared * 0.00003)))))
       + 1.0 / (alzSquared + 1.0));
  //
  ...
  if (fScreeningFunction) fScreeningFunction->insert(std::make_pair(material, myPair));

  if (fVerboseLevel > 2)
  {
    G4cout << "Average Z for material " << material->GetName() << " = " << zeff << G4endl;
    G4cout << "Effective radius for material " << material->GetName() << " = "
           << fAtomicScreeningRadius[intZ] << " m_e*c/hbar --> BCB = " << matRadius << G4endl;
    G4cout << "Screening parameters F0 for material " << material->GetName() << " = " << f0a << ","
           << f0b << G4endl;
  }
  return;
}
