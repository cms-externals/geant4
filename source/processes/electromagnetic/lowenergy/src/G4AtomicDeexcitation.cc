...
G4AtomicDeexcitation::G4AtomicDeexcitation()
  : minGammaEnergy(100. * eV), minElectronEnergy(100. * eV), fAuger(false)
{
  G4cout << " ********************************************************** " << G4endl;
                                                                         ...
                                                                         G4int givenShellId)
{
  ...
  do
  {
    ...
    // givenShellId is given by the process
    {
      ...
      {
        G4Exception("G4AtomicDeexcitation::Constructor", "de0002", JustWarning,
...
G4int G4AtomicDeexcitation::SelectTypeOfTransition(G4int Z, G4int shellId)
{
  if (shellId <= 0)
  {
    G4Exception("G4AtomicDeexcitation::SelectTypeOfTransition()", "de0002", JustWarning,
...
G4DynamicParticle* G4AtomicDeexcitation::GenerateFluorescence(G4int Z, G4int shellId,
                                                              G4int provShellId)
{
  const G4AtomicTransitionManager* transitionManager = G4AtomicTransitionManager::Instance();
...
G4DynamicParticle* G4AtomicDeexcitation::GenerateAuger(G4int Z, G4int shellId)
{
  ...
  if (shellId <= 0)
  {
    G4Exception("G4AtomicDeexcitation::GenerateAuger()", "de0002", JustWarning,
                "zero or negative shellId");
  }
...
G4cout << "transitionShellId: " << transitionRandomShellId << G4endl;
    */
    ...
    G4DynamicParticle* newPart =
      new G4DynamicParticle(G4Electron::Electron(), newElectronDirection, transitionEnergy);
    return newPart;
  }
  else
  {
