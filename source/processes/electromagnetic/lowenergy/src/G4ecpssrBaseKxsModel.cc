...
G4ecpssrBaseKxsModel::G4ecpssrBaseKxsModel()
{
  ...
  if (!path)
  {
    G4Exception("G4ecpssrBaseKxsModel::G4ecpssrBaseKxsModel()", "em0006", FatalException,
...
{
  // this "ExpIntFunction" function allows fast evaluation of the n order exponential integral
  // function En(x)
  G4int i;
  G4int ii;
  G4int nm1;
...
G4double G4ecpssrBaseKxsModel::CalculateCrossSection(G4int zTarget, G4double massIncident,
                                                     G4double energyIncident)

{
  // this K-CrossSection calculation method is done according to W.Brandt and G.Lapicki,
  // Phys.Rev.A23(1981)//
  ...
  else
  {
    ...
    {
      G4cout << "*** WARNING in G4ecpssrBaseKxsModel::CalculateCrossSection : we can treat only "
                "Proton or Alpha incident particles "
             << G4endl;
      return 0;
    }
  }
  ...
  G4double systemMass = ((massIncident * massTarget) / (massIncident + massTarget))
                        / electron_mass_c2;  // the mass of the system (projectile, target)
  ...
  G4double screenedzTarget =
    zTarget - zkshell;  // screenedzTarget is the screened nuclear charge of the target
  // *** see Brandt, Phys Rev A23, p 1727

  constexpr G4double rydbergMeV = 13.6056923e-6;

  G4double tetaK = kBindingEnergy
                   / ((screenedzTarget * screenedzTarget)
                      * rydbergMeV);  // tetaK denotes the reduced binding energy of the electron
  // *** see Rice, ADANDT 20, p 504, f 2

  if (verboseLevel > 0) G4cout << "  tetaK=" << tetaK << G4endl;

  G4double velocity =
    (2. / (tetaK * screenedzTarget))
    * std::pow(((energyIncident * electron_mass_c2) / (massIncident * rydbergMeV)), 0.5);
  // *** also called xiK
  // *** see Brandt, Phys Rev A23, p 1727
  ...
  G4double sigma0 =
    8. * pi * (zIncident * zIncident) * bohrPow2Barn
    * std::pow(screenedzTarget,
               -4.);  // sigma0 is the initial cross section of K shell at stable state
  ...
  else
  {
    if ((0.035 < x) && (x <= 3.))
    {
      electrIonizationEnergy = G4Exp(-2. * x)
                               / (0.031 + (0.213 * std::pow(x, 0.5)) + (0.005 * x)
                                  - (0.069 * std::pow(x, 3. / 2.)) + (0.324 * x * x));
    }
  ...
  G4double hFunction =
    (electrIonizationEnergy * 2.)
    / (tetaK
       * std::pow(velocity, 3));  // hFunction represents the correction for polarization effet
  // *** see Brandt, Phys Rev A20, p 469, f16

  if (verboseLevel > 0) G4cout << "  hFunction=" << hFunction << G4endl;

  G4double gFunction =
    (1. + (9. * velocity) + (31. * velocity * velocity) + (98. * std::pow(velocity, 3.))
     + (12. * std::pow(velocity, 4.)) + (25. * std::pow(velocity, 5.))
     + (4.2 * std::pow(velocity, 6.)) + (0.515 * std::pow(velocity, 7.)))
    / std::pow(1. + velocity, 9.);  // gFunction represents the correction for binding effet
  ...
  G4double sigmaPSS = 1.
                      + (((2. * zIncident) / (screenedzTarget * tetaK))
                         * (gFunction - hFunction));  // describes the perturbed stationnairy state
                                                      // of the affected atomic electon
  // *** also called dzeta
  // *** also called epsilon
  ...
  const G4double cNaturalUnit =
    1 / fine_structure_const;  // it's the speed of light according to Atomic-Unit-System

  if (verboseLevel > 0) G4cout << "  cNaturalUnit=" << cNaturalUnit << G4endl;

  G4double ykFormula = 0.4 * (screenedzTarget / cNaturalUnit) * (screenedzTarget / cNaturalUnit)
                       / (velocity / sigmaPSS);
  // *** also called yS
  ...
  G4double relativityCorrection = std::pow((1. + (1.1 * ykFormula * ykFormula)), 0.5)
                                  + ykFormula;  // the relativistic correction parameter
  // *** also called mRS
  // *** see Brandt, Phys Rev A20, p467, f6

  if (verboseLevel > 0) G4cout << "  relativityCorrection=" << relativityCorrection << G4endl;

  G4double reducedVelocity =
    velocity
    * std::pow(relativityCorrection, 0.5);  // presents the reduced collision velocity parameter
  // *** also called xiR
  ...
  G4double etaOverTheta2 = (energyIncident * electron_mass_c2)
  ...
  // *****************
  {
    ...
    universalFunction =
      (std::pow(2., 9.) / 45.) * std::pow(reducedVelocity / sigmaPSS, 8.)
      * std::pow((1. + (1.72 * (reducedVelocity / sigmaPSS) * (reducedVelocity / sigmaPSS))),
                 -4.);  // is the reduced universal cross section
    // *** see Brandt, Phys Rev A23, p1728

    if (verboseLevel > 0)
      G4cout << "  universalFunction by Brandt 1981 =" << universalFunction << G4endl;
  }
  else
  {
    if (etaOverTheta2 > 86.6 && (sigmaPSS * tetaK) > 0.4 && (sigmaPSS * tetaK) < 2.9996)
    {
      ...
      if (verboseLevel > 0)
        G4cout << "  Notice : FK is computed from high velocity formula" << G4endl;
      ...
      G4double etaK = (energyIncident * electron_mass_c2)
                      / (massIncident * rydbergMeV * screenedzTarget * screenedzTarget);
      // *** see Benka, ADANDT 22, p220, f4 for eta

      if (verboseLevel > 0) G4cout << "  etaK=" << etaK << G4endl;

      G4double etaT = (sigmaPSS * tetaK) * (sigmaPSS * tetaK)
                      * (86.6);  // at any theta, the largest tabulated etaOverTheta2 is 86.6
      ...
      {
        G4cout << "*** WARNING in G4ecpssrBaseKxsModel::CalculateCrossSection : unable to "
                  "interpolate FK function in high velocity region ! ***"
               << G4endl;
        return 0;
      }

      if (verboseLevel > 0)
        G4cout << "  FunctionFK=" << FunctionFK((sigmaPSS * tetaK), 86.6) << G4endl;
    ...
    else if (etaOverTheta2 >= 1.e-3 && etaOverTheta2 <= 86.6 && (sigmaPSS * tetaK) >= 0.4
             && (sigmaPSS * tetaK) <= 2.9996)
    {
      // From Benka 1978
      ...
      {
        G4cout << "*** WARNING : G4ecpssrBaseKxsModel::CalculateCrossSection is unable to "
                  "interpolate FK function in medium velocity region ! ***"
               << G4endl;
        return 0;
      }

      if (verboseLevel > 0)
        G4cout << "  universalFunction2=" << universalFunction2 << " for theta=" << sigmaPSS * tetaK
               << " and etaOverTheta2=" << etaOverTheta2 << G4endl;
  ...
  G4double sigmaPSSR =
    (sigma0 / (sigmaPSS * tetaK))
    * universalFunction;  // sigmaPSSR is the straight-line K-shell ionization cross section
  ...
  G4double pssDeltaK =
    (4. / (systemMass * sigmaPSS * tetaK)) * (sigmaPSS / velocity) * (sigmaPSS / velocity);
  ...
  G4double energyLoss =
    std::pow(1 - pssDeltaK, 0.5);  // energyLoss incorporates the straight-line energy-loss
  // *** also called zK
  // *** see Brandt, Phys Rev A23, p1727, after f B2

  if (verboseLevel > 0) G4cout << "  energyLoss=" << energyLoss << G4endl;

  G4double energyLossFunction =
    (std::pow(2., -9) / 8.)
    * ((((9. * energyLoss) - 1.) * std::pow(1. + energyLoss, 9.))
       + (((9. * energyLoss) + 1.) * std::pow(1. - energyLoss, 9.)));  // energy loss function
  // *** also called fs
  ...
  G4double coulombDeflection =
    (4. * pi * zIncident / systemMass) * std::pow(tetaK * sigmaPSS, -2.)
    * std::pow(velocity / sigmaPSS, -3.)
    * (zTarget / screenedzTarget);  // incorporates Coulomb deflection parameter
  ...
  G4double coulombDeflectionFunction =
    9. * ExpIntFunction(10, cParameter);  // this function describes Coulomb-deflection effect
  // *** see Brandt, Phys Rev A23, p1727

  if (verboseLevel > 0)
    G4cout << "  ExpIntFunction(10,cParameter) =" << ExpIntFunction(10, cParameter) << G4endl;

  if (verboseLevel > 0)
    G4cout << "  coulombDeflectionFunction =" << coulombDeflectionFunction << G4endl;
  ...
  crossSection =
    energyLossFunction * coulombDeflectionFunction
    * sigmaPSSR;  // this ECPSSR cross section is estimated at perturbed-stationnairy-state(PSS)
...
G4double G4ecpssrBaseKxsModel::FunctionFK(G4double k, G4double theta)
{
  ...
  if (theta == 8.66e-3 || theta == 8.66e-2 || theta == 8.66e-1 || theta == 8.66e+0
      || theta == 8.66e+1)
    theta = theta - 1e-12;

  if (theta == 1.e-3 || theta == 1.e-2 || theta == 1.e-1 || theta == 1.e+00 || theta == 1.e+01)
    theta = theta + 1e-12;
...
G4double G4ecpssrBaseKxsModel::LinLogInterpolate(G4double e1, G4double e2, G4double e, G4double xs1,
                                                 G4double xs2)
{
...
G4double G4ecpssrBaseKxsModel::LogLogInterpolate(G4double e1, G4double e2, G4double e, G4double xs1,
                                                 G4double xs2)
{
...
G4double G4ecpssrBaseKxsModel::QuadInterpolator(G4double e11, G4double e12, G4double e21,
