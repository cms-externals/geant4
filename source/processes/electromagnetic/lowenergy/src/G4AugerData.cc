...
std::size_t G4AugerData::AugerShellId(G4int Z, G4int vacancyIndex, G4int transId,
...
G4double G4AugerData::StartShellEnergy(G4int Z, G4int vacancyIndex, G4int transitionId,
                                       G4int augerIndex) const
{
  G4double energy = 0;
...
G4double G4AugerData::StartShellProb(G4int Z, G4int vacancyIndex, G4int transitionId,
                                     G4int augerIndex) const
{
  G4double prob = 0;
  ...
  do
  {
    ...
    if (a == -1)
    {
      if (sLocal == 0)
      {
        ...
        augerTransitionVector.push_back(G4AugerTransition(vacId, std::move(identifiers), newIdMap,
...
void G4AugerData::PrintData(G4int Z)
{
  ...
  {
    G4cout << "---- TransitionData for the vacancy nb " << i << " of the atomic number elemnt " << Z
           << "----- " << G4endl;
