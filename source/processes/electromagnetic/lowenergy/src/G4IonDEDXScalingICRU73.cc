...
G4IonDEDXScalingICRU73::G4IonDEDXScalingICRU73(G4int minAtomicNumberIon, G4int maxAtomicNumberIon)
...
{  // Target material
  ...
  if (cacheAtomicNumber >= minAtomicNumber && cacheAtomicNumber <= maxAtomicNumber
      && cacheAtomicNumber != atomicNumberRefFe && cacheAtomicNumber != atomicNumberRefAr)
  {
...
G4double
G4IonDEDXScalingICRU73::ScalingFactorDEDX(const G4ParticleDefinition* particle,  // Projectile (ion)
      ...
      && cacheAtomicNumber != atomicNumberRefFe && cacheAtomicNumber != atomicNumberRefAr)
  {
    ...
    if (useFe)
    {
      G4double equilibriumCharge =
        EquilibriumCharge(cacheMass, cacheCharge, cacheAtomicNumberPow23, kineticEnergy);

      G4double scaledKineticEnergy = kineticEnergy * (massRefFe / cacheMass);

      G4double equilibriumChargeRefFe =
        EquilibriumCharge(massRefFe, chargeRefFe, atomicNumberRefPow23Fe, scaledKineticEnergy);

      factor =
        equilibriumCharge * equilibriumCharge / (equilibriumChargeRefFe * equilibriumChargeRefFe);
    }
    else
    {
      G4double equilibriumCharge =
      ...
      G4double equilibriumChargeRefAr =
        EquilibriumCharge(massRefAr, chargeRefAr, atomicNumberRefPow23Ar, scaledKineticEnergy);

      factor =
        equilibriumCharge * equilibriumCharge / (equilibriumChargeRefAr * equilibriumChargeRefAr);
    }
  }
...
G4int G4IonDEDXScalingICRU73::AtomicNumberBaseIon(G4int atomicNumberIon,  // Atomic number of ion
                                                  const G4Material* material)
{  // Target material
  ...
  if (atomicNumberIon >= minAtomicNumber && atomicNumberIon <= maxAtomicNumber
      && atomicNumberIon != atomicNumberRefFe && atomicNumberIon != atomicNumberRefAr)
  {
    if (!referencePrepared) CreateReferenceParticles();

    if (useFe)
      atomicNumber = atomicNumberRefFe;
    else
      atomicNumber = atomicNumberRefAr;
  }
