...
G4double G4MicroElecCapture::G_Lindhard_Rob(G4double Trecoil, G4int Zrecoil, G4int Arecoil, G4int Zcible, G4int Acible)
{
  ...
  else
  {
    G4double El = 30.724 * Zcible * Zrecoil
                * pow((G4Pow::GetInstance()->Z23(Zcible) + G4Pow::GetInstance()->Z23(Zrecoil)), 0.5)
                * (Arecoil + Acible) / Acible;

    // multiplication by 1e6 to change El from eV to MeV
    G4double e = Trecoil / (El * CLHEP::eV);
    G4double Fl = (0.0793 * G4Pow::GetInstance()->Z23(Zrecoil) * pow(Zcible, 0.5) * pow((Arecoil + Acible), 1.5))
                / (pow((G4Pow::GetInstance()->Z23(Zcible) + G4Pow::GetInstance()->Z23(Zrecoil)), 3. / 4.) * pow(Arecoil, 3. / 2.) * pow(Acible, 1. / 2.));
