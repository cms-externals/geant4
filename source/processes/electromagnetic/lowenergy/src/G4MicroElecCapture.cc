...
//
// Author: C. Inguimbert 31 january 2022 derivated from G4ElectronCapture (V.Ivanchenko 31 August
// 2010)
//
//----------------------------------------------------------------------------
//
...
G4MicroElecCapture::G4MicroElecCapture(const G4String& regName, G4double ekinlim)
  : G4VDiscreteProcess("MicroElecCapture", fElectromagnetic),
...
void G4MicroElecCapture::BuildPhysicsTable(const G4ParticleDefinition&)
{
  ...
  G4cout << "### G4MicroElecCapture: Tracking cut E(MeV) = " << kinEnergyThreshold / MeV
...
void G4MicroElecCapture::Initialise()
{
  ...
  for (G4int i = 0; i < numOfCouples; ++i)
  {
    ...
    G4cout << "G4Capture, Material " << i + 1 << " / " << numOfCouples << " : "
...
G4VParticleChange* G4MicroElecCapture::PostStepDoIt(const G4Track& aTrack, const G4Step&)
{
  ...
  for (G4int i = 0; i < Nbelements; ++i)
  {
    ...
    NIEdep = NIEdep
             + moleculeMass * FractionMass[i] / Atarget
                 * G_Lindhard_Rob(aTrack.GetKineticEnergy(), Zinc, Ainc, Ztarget, Atarget);
  }
  ...
  else
  {
    ...
    {
      if ((IncPartName == "Genericion") || (IncPartName == "alpha") || (IncPartName == "He3")
          || (IncPartName == "deuteron") || (IncPartName == "triton") || (IncPartName == "proton"))
      {
...
G4double G4MicroElecCapture::G_Lindhard_Rob(G4double Trecoil, G4int Zrecoil, G4int Arecoil,
                                            G4int Zcible, G4int Acible)
{
  G4double Lind = 0.0;
  ...
  else
  {
    ...
    G4double Fl =
      (0.0793 * G4Pow::GetInstance()->Z23(Zrecoil) * std::pow(Zcible, 0.5)
       * std::pow((Arecoil + Acible), 1.5))
      / (std::pow((G4Pow::GetInstance()->Z23(Zcible) + G4Pow::GetInstance()->Z23(Zrecoil)), 3. / 4.)
         * std::pow(Arecoil, 3. / 2.) * std::pow(Acible, 1. / 2.));
