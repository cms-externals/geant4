...
G4MicroElecSurface::G4MicroElecSurface(const G4String& processName, G4ProcessType type)
  : G4VDiscreteProcess(processName, type),
    oldMomentum(0., 0., 0.),
    previousMomentum(0., 0., 0.),
    theGlobalNormal(0., 0., 0.),
    theFacetNormal(0., 0., 0.)
{
  if (verboseLevel > 0)
  {
...
void G4MicroElecSurface::Initialise()
{
  ...
  for (G4int i = 0; i < numOfCouples; ++i)
  {
    ...
    G4cout << this->GetProcessName() << ", Material " << i + 1 << " / " << numOfCouples << " : "
...
void G4MicroElecSurface::BuildPhysicsTable(const G4ParticleDefinition&)
{
  G4ProductionCutsTable* theCoupleTable = G4ProductionCutsTable::GetProductionCutsTable();
  G4int numOfCouples = (G4int)theCoupleTable->GetTableSize();
  G4cout << "G4MicroElecSurface::Initialise: Ncouples= " << numOfCouples << G4endl;

  for (G4int i = 0; i < numOfCouples; ++i)
  {
    const G4Material* material = theCoupleTable->GetMaterialCutsCouple(i)->GetMaterial();

    G4cout << "G4Surface, Material " << i + 1 << " / " << numOfCouples << " : "
           << material->GetName() << G4endl;
    if (material->GetName() == "Vacuum")
    {
      tableWF[material->GetName()] = 0;
...
G4VParticleChange* G4MicroElecSurface::PostStepDoIt(const G4Track& aTrack, const G4Step& aStep)
{
  ...
  if (pPostStepPoint->GetStepStatus() != fGeomBoundary
      || pPostStepPoint->GetPhysicalVolume()->GetName()
           == pPreStepPoint->GetPhysicalVolume()->GetName())
  {
  ...
  else
  {
    ...
    G4Exception("G4MuElecSurf::PostStepDoIt", "OpBoun01", EventMustBeAborted, ed,
  ...
  if (aTrack.GetStepLength() <= kCarTolerance / 2 * 0.0000000001)
  {
    ...
    {
      aParticleChange.ProposeEnergy(aStep.GetPostStepPoint()->GetKineticEnergy()
                                    + energyThreshold_surface);
      flag_franchissement_surface = false;
    }
  ...
  flag_normal =
    (theGlobalNormal == G4ThreeVector(0, 0, 1) || theGlobalNormal == G4ThreeVector(0, 0, -1));

  G4LogicalSurface* Surface = nullptr;

  Surface = G4LogicalBorderSurface::GetSurface(pPreStepPoint->GetPhysicalVolume(),
  ...
  if (Surface == nullptr)
  {
    ...
    {
      Surface =
        G4LogicalSkinSurface::GetSurface(pPostStepPoint->GetPhysicalVolume()->GetLogicalVolume());
  ...
  G4double waveVectort =
    std::sqrt(2 * 9.1093826E-31 * 1.602176487E-19) / (6.6260755E-34 / (2.0 * pi));
  ...
  crossingProbability = 1
                        - (std::pow(std::sinh(pi * at * (kit - kft)), 2.0)
                           / std::pow(std::sinh(pi * at * (kit + kft)), 2.0));
