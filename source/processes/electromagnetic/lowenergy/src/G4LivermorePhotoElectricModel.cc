...
{
  fCurrSection = 0.0;
  if (nullptr != fWater && (material == fWater || material->GetBaseMaterial() == fWater))
  {
...
void G4LivermorePhotoElectricModel::ReadData(G4int Z)
{
  if (Z <= 0 || Z >= ZMAXPE)
  {
    G4cout << "G4LivermorePhotoElectricModel::ReadData() Warning: Z=" << Z
           << " is out of range - request ignored" << G4endl;
    return;
  }
  if (verboseLevel > 1)
  {
...
G4double G4LivermorePhotoElectricModel::GetBindingEnergy(G4int Z, G4int shell)
{
  ...
  InitialiseOnFly(Z);
  if (fCrossSection->GetElementData(Z) == nullptr || shell < 0 || shell >= fNShellsUsed[Z])
  {
    return -1;
  }
