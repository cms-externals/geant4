    ...
    fVerbosity(verbosity)
{
  ...
  G4double tempvector[fNBinsX] = {
    ...
    0.3e0,   0.35e0,  0.4e0,   0.45e0,   0.5e0,    0.55e0,    0.6e0,     0.65e0,
    0.7e0,   0.75e0,  0.8e0,   0.85e0,   0.9e0,    0.925e0,   0.95e0,    0.97e0,
    0.99e0,  0.995e0, 0.999e0, 0.9995e0, 0.9999e0, 0.99995e0, 0.99999e0, 1.0e0};
...
void G4PenelopeBremsstrahlungFS::ClearTables(G4bool isMaster)
{
  // Just to check
  if (!isMaster)
    G4Exception("G4PenelopeBremsstrahlungFS::ClearTables()", "em0100", FatalException,
                "Worker thread in this method");
...
G4double G4PenelopeBremsstrahlungFS::GetEffectiveZSquared(const G4Material* material) const
{
  if (!fEffectiveZSq)
  {
    ...
    G4Exception("G4PenelopeBremsstrahlungFS::GetEffectiveZSquared()", "em2007", FatalException, ed);
    return 0;
  }
  ...
  {
    G4ExceptionDescription ed;
    ed << "The value of  <Z^2> is not properly set for material " << material->GetName() << G4endl;
    // requires running of BuildScaledXSTable()
    G4Exception("G4PenelopeBremsstrahlungFS::GetEffectiveZSquared()", "em2008", FatalException, ed);
  }
  return 0;
}
...
void G4PenelopeBremsstrahlungFS::BuildScaledXSTable(const G4Material* material, G4double cut,
                                                    G4bool isMaster)
{
  ...
  */
  ...
  // Just to check
  if (!isMaster)
    G4Exception("G4PenelopeBremsstrahlungFS::BuildScaledXSTable()", "em0100", FatalException,
                "Worker thread in this method");
  ...
  if (fVerbosity > 2)
  {
    ...
    G4cout << "Threshold = " << cut / keV << " keV, isMaster= " << isMaster << G4endl;
  }

  // This method should be accessed by the master only
  if (!fSamplingTable)
    fSamplingTable = new std::map<std::pair<const G4Material*, G4double>, G4PhysicsTable*>;
  if (!fPBcut) fPBcut = new std::map<std::pair<const G4Material*, G4double>, G4PhysicsFreeVector*>;

  // check if the container exists (if not, create it)
  if (!fReducedXSTable)
    fReducedXSTable = new std::map<std::pair<const G4Material*, G4double>, G4PhysicsTable*>;
  if (!fEffectiveZSq) fEffectiveZSq = new std::map<const G4Material*, G4double>;
  ...
  // Normalize
  for (std::size_t i = 0; i < nElements; i++)
    if (MaxStechiometricFactor > 0.) (*StechiometricFactors)[i] /= MaxStechiometricFactor;
  ...
  for (std::size_t iel = 0; iel < nElements; iel++)
  {
    ...
    if (!fElementData->count(iZ))
    {
      ...
      if (!fElementData->count(iZ))
      {
        ...
        G4Exception("G4PenelopeBremsstrahlungFS::BuildScaledXSTable()", "em2009", FatalException,
                    ed);
      }
    }
    ...
    {
      (*tempData)[ie] +=
        wgt * (*atomData)[ie * (fNBinsX + 1) + fNBinsX];  // last column contains total XS
      for (std::size_t ix = 0; ix < fNBinsX; ++ix)
  ...
  for (std::size_t ie = 0; ie < fNBinsE; ++ie)
  {
    ...
    delete[] tempData2;
    G4double fact =
      millibarn * (theEGrid[ie] + electron_mass_c2) * (1. / fine_structure_const)
    ...
    if (TST > 1.0)
    {
      ...
      G4Exception("G4PenelopeBremsstrahlungFS::BuildScaledXSTable()", "em2010", FatalException, ed);
    }
    for (std::size_t ix = 0; ix < fNBinsX; ++ix)
  ...
  for (std::size_t ix = 0; ix < fNBinsX; ++ix)
  {
    G4PhysicsFreeVector* theVec = (G4PhysicsFreeVector*)((*thePhysicsTable)[ix]);
    for (std::size_t ie = 0; ie < fNBinsE; ++ie)
    {
  ...
  if (!(fSamplingTable->count(theKey))) InitializeEnergySampling(material, cut);
...
void G4PenelopeBremsstrahlungFS::ReadDataFile(G4int Z)
{
  ...
  if (!path)
  {
    ...
    G4Exception("G4PenelopeBremsstrahlungFS::ReadDataFile()", "em0006", FatalException, excep);
    return;
  }
  /*
  ...
  */
  ...
  {
    G4String excep =
      "G4PenelopeBremsstrahlungFS - data file " + G4String(ost.str()) + " not found!";
    G4Exception("G4PenelopeBremsstrahlungFS::ReadDataFile()", "em0003", FatalException, excep);
    return;
  }
  ...
  if (readZ != Z)
  {
    ...
    G4Exception("G4PenelopeBremsstrahlungFS::ReadDataFile()", "em0005", FatalException, ed);
    return;
  }

  G4DataVector* theMatrix = new G4DataVector(fNBinsE * (fNBinsX + 1), 0.);  // initialized with
                                                                            // zeros
...
G4double G4PenelopeBremsstrahlungFS::GetMomentumIntegral(G4double* y, G4double xup,
...
// x is always the gridX
{
  ...
  {
    G4Exception("G4PenelopeBremsstrahlungFS::GetMomentumIntegral()", "em2011", FatalException,
                "Invalid call");
  }
  ...
  for (std::size_t i = 1; i < size; ++i)
  {
    if (theXGrid[i] < 0 || theXGrid[i] < theXGrid[i - 1])
    {
      ...
      G4Exception("G4PenelopeBremsstrahlungFS::GetMomentumIntegral()", "em2012", FatalException,
                  ed);
    }
  }
  ...
  G4double result = 0;
  if (xup < theXGrid[0]) return result;
  ...
  for (std::size_t i = 0; i < size - 1; ++i)
  {
    ...
    if (std::fabs(dx) > 1e-14 * std::fabs(dy))
    {
      ...
      if (momOrder == -1)
        ...
        ds = a * (std::pow(xtc, momOrder + 1) - std::pow(x1, momOrder + 1))
               / ((G4double)(momOrder + 1))
             + b * (std::pow(xtc, momOrder + 2) - std::pow(x1, momOrder + 2))
                 / ((G4double)(momOrder + 2));
    }
    else
      ds = 0.5 * (y1 + y2) * (xtc - x1) * std::pow(xtc, momOrder);
    result += ds;
    if (!loopAgain) return result;
  }
  return result;
}
                                                                   ...
                                                                   const G4double cut) const
{
  ...
  {
    G4Exception("G4PenelopeBremsstrahlungFS::GetScaledXSTable()", "em2013", FatalException,
...
void G4PenelopeBremsstrahlungFS::InitializeEnergySampling(const G4Material* material, G4double cut)
{
  if (fVerbosity > 2)
  ...
  if (!(fReducedXSTable->count(theKey)))
    G4Exception("G4PenelopeBremsstrahlungFS::InitializeEnergySampling()", "em2013", FatalException,
                "Unable to retrieve the cross section table");
  G4PhysicsTable* theTableReduced = fReducedXSTable->find(theKey)->second;

  for (std::size_t ie = 0; ie < fNBinsE; ++ie)
  {
    G4PhysicsFreeVector* theVec = (G4PhysicsFreeVector*)((*thePhysicsTable)[ie]);
    // Fill the table
    ...
    G4double pbval =
      (xc <= 1) ? GetMomentumIntegral(tempData, xc, -1) : GetMomentumIntegral(tempData, 1.0, -1);
                                                       ...
                                                       const G4double cut) const
{
      ...
      || !(fReducedXSTable->count(theKey)))
  {
    ...
    G4Exception("G4PenelopeBremsstrahlungFS::SampleGammaEnergy()", "em2014", FatalException, ed);
    return 0.;
  }
  ...
  if (!firstOrLastBin)
  {
    ...
    {
      G4double val = (*theVec1)[iloop]
                     + (((*theVec2)[iloop] - (*theVec1)[iloop])) * (energy - theEGrid[eBin])
                         / (theEGrid[eBin + 1] - theEGrid[eBin]);
  ...
  do
  {
    ...
    else if (pt > (*theTempVec)[fNBinsX - 1])
    {
      ...
      if (delta < pt * 1e-10)  // very small! Numerical rounding only
      {
        ...
        ed << "Found that (pt > (*theTempVec)[fNBinsX-1]) with pt = " << pt
           << " , (*theTempVec)[fNBinsX-1] = " << (*theTempVec)[fNBinsX - 1]
           << " and delta = " << (pt - (*theTempVec)[fNBinsX - 1]) << G4endl;
        ed << "Possible symptom of problem with numerical precision" << G4endl;
        G4Exception("G4PenelopeBremsstrahlungFS::SampleGammaEnergy()", "em2015", JustWarning, ed);
      }
      else  // real problem
      {
        G4ExceptionDescription ed;
        ed << "Crash at (pt > (*theTempVec)[fNBinsX-1]) with pt = " << pt
           << " , (*theTempVec)[fNBinsX-1]=" << (*theTempVec)[fNBinsX - 1]
           << " and fNBinsX = " << fNBinsX << G4endl;
        ed << "Material: " << mat->GetName() << ", energy = " << energy / keV << " keV" << G4endl;
        G4Exception("G4PenelopeBremsstrahlungFS::SampleGammaEnergy()", "em2015", FatalException,
                    ed);
      }
    }
    else
    ...
    do
    {
      ...
      if (G4UniformRand() * pmax > (A + B * eGamma)) loopAgain = true;
    } while (loopAgain);
    eGamma *= energy;
