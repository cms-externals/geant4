...
G4PenelopeOscillatorManager::G4PenelopeOscillatorManager()
  : fOscillatorStoreIonisation(nullptr),
    fOscillatorStoreCompton(nullptr),
    fAtomicNumber(nullptr),
    fAtomicMass(nullptr),
    fExcitationEnergy(nullptr),
    fPlasmaSquared(nullptr),
    fAtomsPerMolecule(nullptr),
...
{
  if (!instance) instance = new G4PenelopeOscillatorManager();
  return instance;
}
...
void G4PenelopeOscillatorManager::Dump(const G4Material* material)
{
  ...
  if (!theTable)
  {
    ...
    G4cout << "Problem in retrieving the Ionisation Oscillator Table for " << material->GetName()
           << G4endl;
    return;
  }
  G4cout << "*********************************************************************" << G4endl;
  ...
  if (theTable->size() < 10)
    for (std::size_t k = 0; k < theTable->size(); ++k)
    {
      ...
      G4cout << "Ionisation energy = " << (*theTable)[k]->GetIonisationEnergy() / eV << " eV"
             << G4endl;
      G4cout << "Occupation number = " << (*theTable)[k]->GetOscillatorStrength() << G4endl;
      G4cout << "Resonance energy = " << (*theTable)[k]->GetResonanceEnergy() / eV << " eV"
             << G4endl;
      G4cout << "Cufoff resonance energy = " << (*theTable)[k]->GetCutoffRecoilResonantEnergy() / eV
             << " eV" << G4endl;
      G4cout << "*********************************************************************" << G4endl;
    }
  for (std::size_t k = 0; k < theTable->size(); ++k)
  {
    G4cout << k << " " << (*theTable)[k]->GetOscillatorStrength() << " "
           ...
           << (*theTable)[k]->GetResonanceEnergy() / eV << " " << (*theTable)[k]->GetParentZ()
  ...
  {
    G4cout << " G4PenelopeOscillatorManager::Dump " << G4endl;
    G4cout << "Problem in retrieving the Compton Oscillator Table for " << material->GetName()
           << G4endl;
    return;
  }
  G4cout << "*********************************************************************" << G4endl;
  ...
  for (std::size_t k = 0; k < theTable->size(); ++k)
  {
    G4cout << k << " " << (*theTable)[k]->GetOscillatorStrength() << " "
           << (*theTable)[k]->GetIonisationEnergy() / eV << " "
           << (*theTable)[k]->GetHartreeFactor() << " " << (*theTable)[k]->GetParentZ() << " "
           << (*theTable)[k]->GetShellFlag() << " " << (*theTable)[k]->GetParentShellID() << G4endl;
  }
  G4cout << "*********************************************************************" << G4endl;
...
void G4PenelopeOscillatorManager::CheckForTablesCreated()
{
  ...
  if (!fOscillatorStoreIonisation)
  {
    ...
    if (!fReadElementData) ReadElementData();
    if (!fOscillatorStoreIonisation)
      // It should be ok now
      G4Exception("G4PenelopeOscillatorManager::GetOscillatorTableIonisation()", "em2034",
  ...
  if (!fOscillatorStoreCompton)
  {
    ...
    if (!fOscillatorStoreCompton)
      // It should be ok now
      G4Exception("G4PenelopeOscillatorManager::GetOscillatorTableIonisation()", "em2034",
                  FatalException, "Problem in allocating the Oscillator Store for Compton");
  }

  if (!fAtomicNumber) fAtomicNumber = new std::map<const G4Material*, G4double>;
  if (!fAtomicMass) fAtomicMass = new std::map<const G4Material*, G4double>;
  if (!fExcitationEnergy) fExcitationEnergy = new std::map<const G4Material*, G4double>;
  if (!fPlasmaSquared) fPlasmaSquared = new std::map<const G4Material*, G4double>;
  if (!fAtomsPerMolecule) fAtomsPerMolecule = new std::map<const G4Material*, G4double>;
...
G4double G4PenelopeOscillatorManager::GetTotalZ(const G4Material* mat)
{
  ...
  if (fAtomicNumber->count(mat)) return fAtomicNumber->find(mat)->second;
...
G4double G4PenelopeOscillatorManager::GetTotalA(const G4Material* mat)
{
  ...
  // (2) Check if the material has been already included
  if (fAtomicMass->count(mat)) return fAtomicMass->find(mat)->second;
...
G4PenelopeOscillatorManager::GetOscillatorTableIonisation(const G4Material* mat)
{
  // (1) First time, create fOscillatorStores and read data
  CheckForTablesCreated();
...
G4PenelopeOscillatorManager::GetOscillatorIonisation(const G4Material* material, G4int index)
{
  G4PenelopeOscillatorTable* theTable = GetOscillatorTableIonisation(material);
...
G4PenelopeOscillatorTable*
G4PenelopeOscillatorManager::GetOscillatorTableCompton(const G4Material* mat)
{
...
void G4PenelopeOscillatorManager::BuildOscillatorTable(const G4Material* material)
{
  ...
  G4double meanAtomExcitationEnergy[99] = {
    19.2 * eV,  41.8 * eV,  40.0 * eV,  63.7 * eV,  76.0 * eV,  81.0 * eV,  82.0 * eV,  95.0 * eV,
    115.0 * eV, 137.0 * eV, 149.0 * eV, 156.0 * eV, 166.0 * eV, 173.0 * eV, 173.0 * eV, 180.0 * eV,
    174.0 * eV, 188.0 * eV, 190.0 * eV, 191.0 * eV, 216.0 * eV, 233.0 * eV, 245.0 * eV, 257.0 * eV,
    272.0 * eV, 286.0 * eV, 297.0 * eV, 311.0 * eV, 322.0 * eV, 330.0 * eV, 334.0 * eV, 350.0 * eV,
    347.0 * eV, 348.0 * eV, 343.0 * eV, 352.0 * eV, 363.0 * eV, 366.0 * eV, 379.0 * eV, 393.0 * eV,
    417.0 * eV, 424.0 * eV, 428.0 * eV, 441.0 * eV, 449.0 * eV, 470.0 * eV, 470.0 * eV, 469.0 * eV,
    488.0 * eV, 488.0 * eV, 487.0 * eV, 485.0 * eV, 491.0 * eV, 482.0 * eV, 488.0 * eV, 491.0 * eV,
    501.0 * eV, 523.0 * eV, 535.0 * eV, 546.0 * eV, 560.0 * eV, 574.0 * eV, 580.0 * eV, 591.0 * eV,
    614.0 * eV, 628.0 * eV, 650.0 * eV, 658.0 * eV, 674.0 * eV, 684.0 * eV, 694.0 * eV, 705.0 * eV,
    718.0 * eV, 727.0 * eV, 736.0 * eV, 746.0 * eV, 757.0 * eV, 790.0 * eV, 790.0 * eV, 800.0 * eV,
    810.0 * eV, 823.0 * eV, 823.0 * eV, 830.0 * eV, 825.0 * eV, 794.0 * eV, 827.0 * eV, 826.0 * eV,
    841.0 * eV, 847.0 * eV, 878.0 * eV, 890.0 * eV, 902.0 * eV, 921.0 * eV, 934.0 * eV, 939.0 * eV,
    952.0 * eV, 966.0 * eV, 980.0 * eV};
  ...
  if (MaxStechiometricFactor < 1e-16)
  {
    ...
    G4Exception("G4PenelopeOscillatorManager::BuildOscillatorTable()", "em2035", FatalException,
                ed);
  }
  // Normalize
  for (G4int i = 0; i < nElements; ++i)
  ...
  if (fVerbosityLevel > 1)
  {
    ...
    {
      G4cout << "Element " << (*elementVector)[i]->GetSymbol()
             << " (Z = " << (*elementVector)[i]->GetZasInt() << ") --> "
  ...
  for (G4int i = 0; i < nElements; ++i)
  {
    ...
    meanExcitationEnergy +=
      iZ * G4Log(meanAtomExcitationEnergy[iZ - 1]) * (*StechiometricFactors)[i];
  ...
  for (G4int k = 0; k < nElements; k++)
  {
    ...
    for (G4int i = 0; i < 2000 && !finished; i++)
    {
      ...
      */
      ...
      if (fElementData[0][i] > Z) finished = true;
    }
  }
  ...
  G4double Omega =
    std::sqrt(4 * pi * moleculeDensity * totalZ * Bohr_radius) * Bohr_radius * 2.0 * RydbergEnergy;
  ...
  if (fVerbosityLevel > 1)
  {
    G4cout << "Estimated oscillator strength and energy of plasmon: " << conductionStrength
           << " and " << plasmaEnergy / eV << " eV" << G4endl;
  }

  if (conductionStrength < 0.01 || plasmaEnergy < 1.0 * eV)  // this is an insulator
  {
    if (fVerbosityLevel > 1) G4cout << material->GetName() << " is an insulator " << G4endl;
    // remove conduction band oscillator
    helper->erase(helper->begin());
  }
  else  // this is a conductor, Outer shells moved to conduction band
  {
    if (fVerbosityLevel > 1) G4cout << material->GetName() << " is a conductor " << G4endl;
    isAConductor = true;
    ...
    G4double hartree = 0.75
                       / std::sqrt(3.0 * pi * pi * moleculeDensity * Bohr_radius * Bohr_radius
                                   * Bohr_radius * conductionStrength);
  ...
  if (std::fabs(sum - totalZ) > (1e-6 * totalZ))
  {
    ...
    G4Exception("G4PenelopeOscillatorManager::BuildOscillatorTable()", "em2036", FatalException,
                ed);
  }
    ...
    do
    {
      ...
      if (fVerbosityLevel > 3)
        G4cout << "Sternheimer's adjustment factor loops: " << AALow << " " << AAHigh << " "
               << adjustmentFactor << " " << TST << " " << sumLocal << G4endl;
  ...
  if (std::fabs(xcheck - meanExcitationEnergy) > 1e-8 * meanExcitationEnergy)
  {
    ...
    G4Exception("G4PenelopeOscillatorManager::BuildOscillatorTable()", "em2037", FatalException,
                ed);
  }

  // Selection of the lowest ionisation energy for inner shells. Only the K, L and M shells with
  // ionisation energy less than the N1 shell of the heaviest element in the material are considered
  // as inner shells. As a results, the inner/outer shell character of an atomic shell depends on
  ...
  if (fVerbosityLevel > 1) G4cout << "Cutoff energy: " << cutEnergy / eV << " eV" << G4endl;
  //
  ...
  do
  {
    ...
    if (Nost > firstIndex + 1)
    {
      ...
      for (std::size_t i = firstIndex; i < theTable->size() - 1; ++i)
      {
        ...
        if (ionEne > cutEnergy)  // remove condition that shellFlag < 10!
          skipLoop = true;
        if (resEne < 1.0 * eV || resEnePlus1 < 1.0 * eV) skipLoop = true;
        if (resEnePlus1 > Rgroup * resEne) skipLoop = true;
        if (!skipLoop)
        {
          G4double newRes = G4Exp((oscStre * G4Log(resEne) + oscStrePlus1 * G4Log(resEnePlus1))
                                  / (oscStre + oscStrePlus1));
          (*theTable)[i]->SetResonanceEnergy(newRes);
          G4double newIon =
            (oscStre * ionEne + oscStrePlus1 * (*theTable)[i + 1]->GetIonisationEnergy())
            / (oscStre + oscStrePlus1);
    ...
    if (nLoops > 100 && !removedLevels) loopAgain = false;
  } while (loopAgain);
  ...
  ******************************************/
  ...
  do
  {
    ...
    if (Nost > firstIndex + 1)
    {
      ...
      for (std::size_t i = firstIndex; i < theTableC->size() - 1; ++i)
      {
        ...
        // if (shellFlag < 10 && ionEne>cutEnergy) in Penelope
        if (ionEne > cutEnergy) skipLoop = true;
        if (ionEne < 1.0 * eV || ionEnePlus1 < 1.0 * eV) skipLoop = true;
        if (ionEnePlus1 > Rgroup * ionEne) skipLoop = true;

        if (!skipLoop)
        {
          G4double newIon =
            (oscStre * ionEne + oscStrePlus1 * ionEnePlus1) / (oscStre + oscStrePlus1);
  ...
  delete helper;
  if (fVerbosityLevel > 1) Dump(material);
...
void G4PenelopeOscillatorManager::ReadElementData()
{
  ...
  if (!path)
  {
    ...
    G4Exception("G4PenelopeOscillatorManager::ReadElementData()", "em0006", FatalException, excep);
    return;
  }
  ...
  if (!file.is_open())
  {
    ...
    G4Exception("G4PenelopeOscillatorManager::ReadElementData()", "em0003", FatalException, excep);
  }

  G4AtomicTransitionManager* theTransitionManager = G4AtomicTransitionManager::Instance();
...
G4double G4PenelopeOscillatorManager::GetMeanExcitationEnergy(const G4Material* mat)
{
  ...
  // (2) Check if the material has been already included
  if (fExcitationEnergy->count(mat)) return fExcitationEnergy->find(mat)->second;
...
G4double G4PenelopeOscillatorManager::GetPlasmaEnergySquared(const G4Material* mat)
{
  ...
  // (2) Check if the material has been already included
  if (fPlasmaSquared->count(mat)) return fPlasmaSquared->find(mat)->second;
...
G4double G4PenelopeOscillatorManager::GetAtomsPerMolecule(const G4Material* mat)
{
  ...
  // (2) Check if the material has been already included
  if (fAtomsPerMolecule->count(mat)) return fAtomsPerMolecule->find(mat)->second;
  ...
  else
  {
    ...
    G4cout << "Impossible to retrieve the number of atoms per molecule for  " << mat->GetName()
           << G4endl;
    return 0;
  }
}
...
G4double G4PenelopeOscillatorManager::GetNumberOfZAtomsPerMolecule(const G4Material* mat, G4int Z)
{
  ...
  if (fAtomTablePerMolecule->count(theKey)) return fAtomTablePerMolecule->find(theKey)->second;
  ...
  {
    G4cout << "G4PenelopeOscillatorManager::GetAtomsPerMolecule() " << G4endl;
    G4cout << "Impossible to retrieve the number of atoms per molecule for Z = " << Z
           << " in material " << mat->GetName() << G4endl;
    return 0;
  }
}
