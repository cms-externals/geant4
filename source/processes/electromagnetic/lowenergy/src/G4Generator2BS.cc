...
G4Generator2BS::G4Generator2BS(const G4String&)
  : G4VEmAngularDistribution("AngularGen2BS"), fz(1.), ratio(1.), ratio1(1.), ratio2(1.), delta(0.)
{
...
G4ThreeVector& G4Generator2BS::SampleDirection(const G4DynamicParticle* dp, G4double final_energy,
                                               G4int Z, const G4Material*)
{
  ...
  do
  {
    ...
    {
      ++nwarn;
      G4cout << "### WARNING in G4Generator2BS: Etot(MeV)= " << energy / MeV << "  Egamma(MeV)"
             << (energy - final_energy) / MeV << " gMax= " << gMax << "  < " << gfun
             << "  results are not reliable!" << G4endl;
      if (20 == nwarn)
      {
