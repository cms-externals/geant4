...
G4ecpssrBaseLixsModel::G4ecpssrBaseLixsModel()
{
  ...
  if (!path)
  {
    G4Exception("G4ecpssrLCrossSection::G4ecpssrBaseLixsModel()", "em0006", FatalException,
  ...
  if (!FL1)
    G4Exception("G4ecpssrLCrossSection::G4ecpssrBaseLixsModel()", "em0003", FatalException,
...
G4double G4ecpssrBaseLixsModel::CalculateL1CrossSection(G4int zTarget, G4double massIncident,
                                                        G4double energyIncident)
{
  if (zTarget <= 4) return 0.;

  // this L1-CrossSection calculation method is done according to Werner Brandt and Grzegorz
  // Lapicki, Phys.Rev.A20 N2 (1979), and using data tables of O. Benka et al. At.Data Nucl.Data
  // Tables Vol.22 No.3 (1978).
  ...
  else
  {
    ...
    {
      G4cout << "*** WARNING in G4ecpssrBaseLixsModel::CalculateL1CrossSection : Proton or Alpha "
                "incident particles only. "
             << G4endl;
      G4cout << massIncident << ", " << aAlpha->GetPDGMass() << " (alpha)" << aProtone->GetPDGMass()
             << " (proton)" << G4endl;
      return 0;
    }
  }

  G4double l1BindingEnergy = transitionManager->Shell(zTarget, 1)
                               ->BindingEnergy();  // Observed binding energy of L1-subshell
  G4double massTarget = (massManager->GetAtomicMassAmu(zTarget)) * amu_c2;
  G4double systemMass = ((massIncident * massTarget) / (massIncident + massTarget))
                        / electron_mass_c2;  // Mass of the system (projectile, target)
  static const G4double zlshell = 4.15;
  // *** see Benka, ADANDT 22, p 223
  G4double screenedzTarget =
    zTarget - zlshell;  // Effective nuclear charge as seen by electrons in L1-sub shell
  static const G4double rydbergMeV = 13.6056923e-6;

  static const G4double nl = 2.;
  // *** see Benka, ADANDT 22, p 220, f3
  G4double tetal1 = (l1BindingEnergy * nl * nl)
                    / ((screenedzTarget * screenedzTarget) * rydbergMeV);  // Screening parameter
  // *** see Benka, ADANDT 22, p 220, f3

  if (verboseLevel > 0) G4cout << "  tetal1=" << tetal1 << G4endl;

  G4double reducedEnergy = (energyIncident * electron_mass_c2)
                           / (massIncident * rydbergMeV * screenedzTarget * screenedzTarget);
  // *** also called etaS
  // *** see Benka, ADANDT 22, p 220, f3

  static const G4double bohrPow2Barn =
    (Bohr_radius * Bohr_radius) / barn;  // Bohr radius of hydrogen

  G4double sigma0 =
    8. * pi * (zIncident * zIncident) * bohrPow2Barn * std::pow(screenedzTarget, -4.);
  // *** see Benka, ADANDT 22, p 220, f2, for protons
  // *** see Basbas, Phys Rev A7, p 1000

  G4double velocityl1 =
    CalculateVelocity(1, zTarget, massIncident, energyIncident);  // Scaled velocity
  ...
  if (x1 <= 0.035)
    electrIonizationEnergyl1 = 0.75 * pi * (std::log(1. / (x1 * x1)) - 1.);
  else
  {
    if (x1 <= 3.)
      electrIonizationEnergyl1 = G4Exp(-2. * x1)
                                 / (0.031 + (0.213 * std::pow(x1, 0.5)) + (0.005 * x1)
                                    - (0.069 * std::pow(x1, 3. / 2.)) + (0.324 * x1 * x1));
    else
    {
  ...
  G4double hFunctionl1 =
    (electrIonizationEnergyl1 * 2. * nl)
    / (tetal1 * std::pow(velocityl1, 3));  // takes into account the polarization effect
  // *** see Brandt, Phys Rev A20, p 469, f16

  if (verboseLevel > 0) G4cout << "  hFunctionl1=" << hFunctionl1 << G4endl;

  G4double gFunctionl1 =
    (1. + (9. * velocityl1) + (31. * velocityl1 * velocityl1) + (49. * std::pow(velocityl1, 3.))
     + (162. * std::pow(velocityl1, 4.)) + (63. * std::pow(velocityl1, 5.))
     + (18. * std::pow(velocityl1, 6.)) + (1.97 * std::pow(velocityl1, 7.)))
    / std::pow(1. + velocityl1, 9.);  // takes into account the reduced binding effect
  // *** see Brandt, Phys Rev A20, p 469, f19

  if (verboseLevel > 0) G4cout << "  gFunctionl1=" << gFunctionl1 << G4endl;

  G4double sigmaPSS_l1 = 1.
                         + (((2. * zIncident) / (screenedzTarget * tetal1))
                            * (gFunctionl1 - hFunctionl1));  // Binding-polarization factor
  // *** also called dzeta
  // *** also called epsilon
  ...
  G4double yl1Formula = 0.4 * (screenedzTarget / cNaturalUnit) * (screenedzTarget / cNaturalUnit)
                        / (nl * velocityl1 / sigmaPSS_l1);
  // *** also called yS
  // *** see Brandt, Phys Rev A20, p467, f6
  // *** see Brandt, Phys Rev A23, p1728

  G4double l1relativityCorrection = std::pow((1. + (1.1 * yl1Formula * yl1Formula)), 0.5)
                                    + yl1Formula;  // Relativistic correction parameter
  // *** also called mRS
  // *** see Brandt, Phys Rev A20, p467, f6

  // G4double reducedVelocity_l1 = velocityl1*std::pow(l1relativityCorrection,0.5); //Reduced
  // velocity parameter
  ...
  {
    L1etaOverTheta2 =
      (reducedEnergy * l1relativityCorrection) / ((tetal1 * sigmaPSS_l1) * (tetal1 * sigmaPSS_l1));
    ...
    if (((tetal1 * sigmaPSS_l1) >= 0.2) && ((tetal1 * sigmaPSS_l1) <= 2.6670)
        && (L1etaOverTheta2 >= 0.1e-3) && (L1etaOverTheta2 <= 0.866e2))
      universalFunction_l1 = FunctionFL1((tetal1 * sigmaPSS_l1), L1etaOverTheta2);

    if (verboseLevel > 0)
      G4cout << "at low velocity range, universalFunction_l1  =" << universalFunction_l1 << G4endl;

    sigmaPSSR_l1 =
      (sigma0 / (tetal1 * sigmaPSS_l1))
      * universalFunction_l1;  // Plane-wave Born -Aproximation L1-subshell ionisation Cross Section
    // *** see Benka, ADANDT 22, p220, f1

    if (verboseLevel > 0)
      G4cout << "  at low velocity range, sigma PWBA L1 CS  = " << sigmaPSSR_l1 << G4endl;
  }
  else
  {
    ...
    if ((tetal1 >= 0.2) && (tetal1 <= 2.6670) && (L1etaOverTheta2 >= 0.1e-3)
        && (L1etaOverTheta2 <= 0.866e2))
      universalFunction_l1 = FunctionFL1(tetal1, L1etaOverTheta2);

    if (verboseLevel > 0)
      G4cout << "at medium and high velocity range, universalFunction_l1  =" << universalFunction_l1
             << G4endl;
    ...
    if (verboseLevel > 0)
      G4cout << "  sigma PWBA L1 CS at medium and high velocity range = " << sigmaPSSR_l1 << G4endl;
  }

  G4double pssDeltal1 = (4. / (systemMass * sigmaPSS_l1 * tetal1)) * (sigmaPSS_l1 / velocityl1)
  ...
  G4double coulombDeflectionl1 =
    (8. * pi * zIncident / systemMass) * std::pow(tetal1 * sigmaPSS_l1, -2.)
    * std::pow(velocityl1 / sigmaPSS_l1, -3.) * (zTarget / screenedzTarget);
  ...
  G4double coulombDeflectionFunction_l1 =
    9. * ExpIntFunction(10, cParameterl1);  // Coulomb-deflection effect correction

  if (verboseLevel > 0)
    G4cout << "  coulombDeflectionFunction_l1 =" << coulombDeflectionFunction_l1 << G4endl;
...
G4double G4ecpssrBaseLixsModel::CalculateL2CrossSection(G4int zTarget, G4double massIncident,
                                                        G4double energyIncident)

{
  if (zTarget <= 13) return 0.;

  // this L2-CrossSection calculation method is done according to Werner Brandt and Grzegorz
  // Lapicki, Phys.Rev.A20 N2 (1979), and using data tables of O. Benka et al. At.Data Nucl.Data
  // Tables Vol.22 No.3 (1978).
  ...
  else
  {
    ...
    {
      G4cout << "*** WARNING in G4ecpssrBaseLixsModel::CalculateL2CrossSection : Proton or Alpha "
                "incident particles only. "
             << G4endl;
      G4cout << massIncident << ", " << aAlpha->GetPDGMass() << " (alpha)" << aProtone->GetPDGMass()
             << " (proton)" << G4endl;
  ...
  G4double l2BindingEnergy = transitionManager->Shell(zTarget, 2)
                               ->BindingEnergy();  // Observed binding energy of L2-subshell
  ...
  G4double screenedzTarget =
    zTarget - zlshell;  // Effective nuclear charge as seen by electrons in L2-subshell

  const G4double rydbergMeV = 13.6056923e-6;

  const G4double nl = 2.;

  G4double tetal2 = (l2BindingEnergy * nl * nl)
                    / ((screenedzTarget * screenedzTarget) * rydbergMeV);  // Screening parameter
  ...
  G4double velocityl2 =
    CalculateVelocity(2, zTarget, massIncident, energyIncident);  // Scaled velocity
  ...
  if (x2 <= 0.035)
    electrIonizationEnergyl2 = 0.75 * pi * (std::log(1. / (x2 * x2)) - 1.);
  else
  {
    if (x2 <= 3.)
      electrIonizationEnergyl2 = G4Exp(-2. * x2)
                                 / (0.031 + (0.213 * std::pow(x2, 0.5)) + (0.005 * x2)
                                    - (0.069 * std::pow(x2, 3. / 2.)) + (0.324 * x2 * x2));
    else
    {
  ...
  G4double hFunctionl2 =
    (electrIonizationEnergyl2 * 2. * nl)
    / (tetal2 * std::pow(velocityl2, 3));  // takes into account  the polarization effect

  if (verboseLevel > 0) G4cout << "  hFunctionl2=" << hFunctionl2 << G4endl;

  G4double gFunctionl2 = (1. + (10. * velocityl2) + (45. * velocityl2 * velocityl2)
                          + (102. * std::pow(velocityl2, 3.)) + (331. * std::pow(velocityl2, 4.))
                          + (6.7 * std::pow(velocityl2, 5.)) + (58. * std::pow(velocityl2, 6.))
                          + (7.8 * std::pow(velocityl2, 7.)) + (0.888 * std::pow(velocityl2, 8.)))
                         / std::pow(1. + velocityl2, 10.);
  // takes into account the reduced binding effect

  if (verboseLevel > 0) G4cout << "  gFunctionl2=" << gFunctionl2 << G4endl;

  G4double sigmaPSS_l2 = 1.
                         + (((2. * zIncident) / (screenedzTarget * tetal2))
                            * (gFunctionl2 - hFunctionl2));  // Binding-polarization factor

  if (verboseLevel > 0) G4cout << "  sigmaPSS_l2=" << sigmaPSS_l2 << G4endl;

  const G4double cNaturalUnit = 137.;

  G4double yl2Formula = 0.15 * (screenedzTarget / cNaturalUnit) * (screenedzTarget / cNaturalUnit)
                        / (velocityl2 / sigmaPSS_l2);

  G4double l2relativityCorrection = std::pow((1. + (1.1 * yl2Formula * yl2Formula)), 0.5)
                                    + yl2Formula;  // Relativistic correction parameter
  ...
  {
    L2etaOverTheta2 =
      (reducedEnergy * l2relativityCorrection) / ((sigmaPSS_l2 * tetal2) * (sigmaPSS_l2 * tetal2));

    if ((tetal2 * sigmaPSS_l2 >= 0.2) && (tetal2 * sigmaPSS_l2 <= 2.6670)
        && (L2etaOverTheta2 >= 0.1e-3) && (L2etaOverTheta2 <= 0.866e2))
    ...
    if (verboseLevel > 0)
      G4cout << "  sigma PWBA L2 CS at low velocity range = " << sigmaPSSR_l2 << G4endl;
  }
  else
  {
    L2etaOverTheta2 = reducedEnergy / (tetal2 * tetal2);

    if ((tetal2 >= 0.2) && (tetal2 <= 2.6670) && (L2etaOverTheta2 >= 0.1e-3)
        && (L2etaOverTheta2 <= 0.866e2))
      universalFunction_l2 = FunctionFL2((tetal2), L2etaOverTheta2);

    sigmaPSSR_l2 = (sigma0 / tetal2) * universalFunction_l2;

    if (verboseLevel > 0)
      G4cout << "  sigma PWBA L2 CS at medium and high velocity range = " << sigmaPSSR_l2 << G4endl;
  }

  G4double pssDeltal2 = (4. / (systemMass * sigmaPSS_l2 * tetal2)) * (sigmaPSS_l2 / velocityl2)
                        * (sigmaPSS_l2 / velocityl2);
  ...
  G4double coulombDeflectionl2 =
    (8. * pi * zIncident / systemMass) * std::pow(tetal2 * sigmaPSS_l2, -2.)
    * std::pow(velocityl2 / sigmaPSS_l2, -3.) * (zTarget / screenedzTarget);

  G4double cParameterl2 = 2. * coulombDeflectionl2 / (energyLossl2 * (energyLossl2 + 1.));

  G4double coulombDeflectionFunction_l2 =
    11. * ExpIntFunction(12, cParameterl2);  // Coulomb-deflection effect correction
  // *** see Brandt, Phys Rev A10, p477, f25

  if (verboseLevel > 0)
    G4cout << "  coulombDeflectionFunction_l2 =" << coulombDeflectionFunction_l2 << G4endl;
...
G4double G4ecpssrBaseLixsModel::CalculateL3CrossSection(G4int zTarget, G4double massIncident,
                                                        G4double energyIncident)

{
  if (zTarget <= 13) return 0.;

  // this L3-CrossSection calculation method is done according to Werner Brandt and Grzegorz
  // Lapicki, Phys.Rev.A20 N2 (1979), and using data tables of O. Benka et al. At.Data Nucl.Data
  // Tables Vol.22 No.3 (1978).
  ...
  else
  {
    ...
    {
      G4cout << "*** WARNING in G4ecpssrBaseLixsModel::CalculateL3CrossSection : Proton or Alpha "
                "incident particles only. "
             << G4endl;
      G4cout << massIncident << ", " << aAlpha->GetPDGMass() << " (alpha)" << aProtone->GetPDGMass()
             << " (proton)" << G4endl;
  ...
  G4double screenedzTarget =
    zTarget - zlshell;  // Effective nuclear charge as seen by electrons in L3-subshell

  const G4double rydbergMeV = 13.6056923e-6;

  const G4double nl = 2.;

  G4double tetal3 = (l3BindingEnergy * nl * nl)
                    / ((screenedzTarget * screenedzTarget) * rydbergMeV);  // Screening parameter
  ...
  G4double velocityl3 =
    CalculateVelocity(3, zTarget, massIncident, energyIncident);  // Scaled velocity
  ...
  if (x3 <= 0.035)
    electrIonizationEnergyl3 = 0.75 * pi * (std::log(1. / (x3 * x3)) - 1.);
  else
  {
    if (x3 <= 3.)
      electrIonizationEnergyl3 = G4Exp(-2. * x3)
                                 / (0.031 + (0.213 * std::pow(x3, 0.5)) + (0.005 * x3)
                                    - (0.069 * std::pow(x3, 3. / 2.)) + (0.324 * x3 * x3));
    else
    {
  ...
  G4double hFunctionl3 =
    (electrIonizationEnergyl3 * 2. * nl)
    / (tetal3 * std::pow(velocityl3, 3));  // takes into account the polarization effect

  if (verboseLevel > 0) G4cout << "  hFunctionl3=" << hFunctionl3 << G4endl;

  G4double gFunctionl3 = (1. + (10. * velocityl3) + (45. * velocityl3 * velocityl3)
                          + (102. * std::pow(velocityl3, 3.)) + (331. * std::pow(velocityl3, 4.))
                          + (6.7 * std::pow(velocityl3, 5.)) + (58. * std::pow(velocityl3, 6.))
                          + (7.8 * std::pow(velocityl3, 7.)) + (0.888 * std::pow(velocityl3, 8.)))
                         / std::pow(1. + velocityl3, 10.);
  // takes into account the reduced binding effect

  if (verboseLevel > 0) G4cout << "  gFunctionl3=" << gFunctionl3 << G4endl;

  G4double sigmaPSS_l3 = 1.
                         + (((2. * zIncident) / (screenedzTarget * tetal3))
                            * (gFunctionl3 - hFunctionl3));  // Binding-polarization factor

  if (verboseLevel > 0) G4cout << "sigmaPSS_l3 =" << sigmaPSS_l3 << G4endl;

  const G4double cNaturalUnit = 137.;

  G4double yl3Formula = 0.15 * (screenedzTarget / cNaturalUnit) * (screenedzTarget / cNaturalUnit)
                        / (velocityl3 / sigmaPSS_l3);

  G4double l3relativityCorrection = std::pow((1. + (1.1 * yl3Formula * yl3Formula)), 0.5)
                                    + yl3Formula;  // Relativistic correction parameter
  ...
  {
    L3etaOverTheta2 =
      (reducedEnergy * l3relativityCorrection) / ((sigmaPSS_l3 * tetal3) * (sigmaPSS_l3 * tetal3));

    if ((tetal3 * sigmaPSS_l3 >= 0.2) && (tetal3 * sigmaPSS_l3 <= 2.6670)
        && (L3etaOverTheta2 >= 0.1e-3) && (L3etaOverTheta2 <= 0.866e2))
    ...
    if (verboseLevel > 0)
      G4cout << "  sigma PWBA L3 CS at low velocity range = " << sigmaPSSR_l3 << G4endl;
  }
...

  {
    ...
    if ((tetal3 >= 0.2) && (tetal3 <= 2.6670) && (L3etaOverTheta2 >= 0.1e-3)
        && (L3etaOverTheta2 <= 0.866e2))
    ...
    if (verboseLevel > 0)
      G4cout << "  sigma PWBA L3 CS at medium and high velocity range = " << sigmaPSSR_l3 << G4endl;
  }

  G4double pssDeltal3 = (4. / (systemMass * sigmaPSS_l3 * tetal3)) * (sigmaPSS_l3 / velocityl3)
                        * (sigmaPSS_l3 / velocityl3);
  ...
  G4double coulombDeflectionl3 =
    (8. * pi * zIncident / systemMass) * std::pow(tetal3 * sigmaPSS_l3, -2.)
    * std::pow(velocityl3 / sigmaPSS_l3, -3.) * (zTarget / screenedzTarget);

  G4double cParameterl3 = 2. * coulombDeflectionl3 / (energyLossl3 * (energyLossl3 + 1.));

  G4double coulombDeflectionFunction_l3 =
    11. * ExpIntFunction(12, cParameterl3);  // Coulomb-deflection effect correction
  // *** see Brandt, Phys Rev A10, p477, f25

  if (verboseLevel > 0)
    G4cout << "  coulombDeflectionFunction_l3 =" << coulombDeflectionFunction_l3 << G4endl;
...
G4double G4ecpssrBaseLixsModel::CalculateVelocity(G4int subShell, G4int zTarget,
...

{
  ...
  {
    G4cout << "*** WARNING in G4ecpssrBaseLixsModel::CalculateVelocity : Proton or Alpha incident "
              "particles only. "
           << G4endl;
    G4cout << massIncident << ", " << aAlpha->GetPDGMass() << " (alpha)" << aProtone->GetPDGMass()
           << " (proton)" << G4endl;
...
G4double G4ecpssrBaseLixsModel::FunctionFL1(G4double k, G4double theta)
{
  ...
  if (theta == 8.66e-4 || theta == 8.66e-3 || theta == 8.66e-2 || theta == 8.66e-1
      || theta == 8.66e+00 || theta == 8.66e+01)
    theta = theta - 1e-12;

  if (theta == 1.e-4 || theta == 1.e-3 || theta == 1.e-2 || theta == 1.e-1 || theta == 1.e+00
      || theta == 1.e+01)
    theta = theta + 1e-12;
  ...
  if (verboseLevel > 0)
    G4cout << valueT1 << " " << valueT2 << " " << valueE11 << " " << valueE12 << " " << valueE21
           << " " << valueE22 << " " << xs11 << " " << xs12 << " " << xs21 << " " << xs22 << " "
           << G4endl;
...
G4double G4ecpssrBaseLixsModel::LinLinInterpolate(G4double e1, G4double e2, G4double e,
...
G4double G4ecpssrBaseLixsModel::LinLogInterpolate(G4double e1, G4double e2, G4double e,
                                                  G4double xs1, G4double xs2)
{
...
G4double G4ecpssrBaseLixsModel::LogLogInterpolate(G4double e1, G4double e2, G4double e,
                                                  G4double xs1, G4double xs2)
{
...
G4double G4ecpssrBaseLixsModel::QuadInterpolator(G4double e11, G4double e12, G4double e21,
