...
G4MicroElecInelasticModel_new::G4MicroElecInelasticModel_new(const G4ParticleDefinition*,
                                                             const G4String& nam)
...
{
  if (verboseLevel > 3) G4cout << "Calling G4MicroElecInelasticModel_new::Initialise()" << G4endl;

  const char* path = G4FindDataDir("G4LEDATA");
  if (!path)
  {
    G4Exception("G4MicroElecElasticModel_new::Initialise", "em0006", FatalException,
  ...
  for (G4int i = 0; i < numOfCouples; ++i)
  {
    ...
    G4cout << "Material " << i + 1 << " / " << numOfCouples << " : " << material->GetName()
           << G4endl;
    ...
    if (particle == electronDef)
    {
      ...
      G4MicroElecCrossSectionDataSet_new* tableE =
        new G4MicroElecCrossSectionDataSet_new(new G4LogLogInterpolation, MeV, scaleFactor);
      ...
      {
        eFullFileName << path
                      << "/microelec/Inelastic/cumulated_" + modelName + "_sigmadiff_inelastic_e-_"
                           + mat + ".dat";
        G4cout << "Faster code = true" << G4endl;
        G4cout << "Inelastic/cumulated_" + modelName + "_sigmadiff_inelastic_e-_" + mat + ".dat"
               << G4endl;
      }
      else
      {
        eFullFileName << path
                      << "/microelec/Inelastic/" + modelName + "_sigmadiff_inelastic_e-_" + mat
                           + ".dat";
      ...
      if (!eDiffCrossSection)
      {
        ...
        if (fasterCode)
          G4Exception("G4MicroElecInelasticModel_new::Initialise", "em0003", FatalException,
                      sortieString.c_str());
        else
        {
          G4Exception("G4MicroElecInelasticModel_new::Initialise", "em0003", FatalException,
      ...
      vector<TriDimensionMap>* eDiffCrossSectionData =
        new vector<TriDimensionMap>;  // Storage of [IncidentEnergy, TransfEnergy, DCS values], used
                                      // in slower code
      vector<TriDimensionMap>* eNrjTransfData =
        new vector<TriDimensionMap>;  // Storage of possible transfer energies by shell
      vector<VecMap>* eProbaShellMap =
        new vector<VecMap>;  // Storage of the vectors containing all cumulated DCS values for an
                             // initial energy, by shell
      vector<G4double>* eTdummyVec =
      ...
      for (G4int j = 0; j < currentMaterialStructure->NumberOfLevels();
           ++j)  // Filling the map vectors with an empty map for each shell
      {
    ...
    {
      // Cross section
      G4String fileProton("Inelastic/" + modelName + "_sigma_inelastic_p_" + mat);
      G4cout << fileProton << G4endl;
      G4MicroElecCrossSectionDataSet_new* tableP =
        new G4MicroElecCrossSectionDataSet_new(new G4LogLogInterpolation, MeV, scaleFactor);
      ...
      {
        pFullFileName << path
                      << "/microelec/Inelastic/cumulated_" + modelName + "_sigmadiff_inelastic_p_"
                           + mat + ".dat";
        G4cout << "Faster code = true" << G4endl;
        G4cout << "Inelastic/cumulated_" + modelName + "_sigmadiff_inelastic_p_" + mat + ".dat"
               << G4endl;
      }
      else
      {
        pFullFileName << path
                      << "/microelec/Inelastic/" + modelName + "_sigmadiff_inelastic_p_" + mat
                           + ".dat";
        G4cout << "Faster code = false" << G4endl;
        G4cout << "Inelastic/" + modelName + "_sigmadiff_inelastic_e-_" + mat + ".dat" << G4endl;
      }
  ...
  if (verboseLevel > 1)
  {
    G4cout << "MicroElec Inelastic model is initialized " << G4endl
           ...
           << " MeV for " << particle->GetParticleName() << " with mass (amu) "
           << particle->GetPDGMass() / proton_mass_c2 << " and charge " << particle->GetPDGCharge()
           << G4endl << G4endl;
  }
...
G4double
G4MicroElecInelasticModel_new::CrossSectionPerVolume(const G4Material* material,
                                                     ...
                                                     G4double ekin, G4double, G4double)
{
  ...
  if (tablepos == tableTCS.end())
  {
    ...
    G4Exception("G4MicroElecInelasticModel_new::ComputeCrossSectionPerVolume", "em0002",
                FatalException, str);
    return 0;
  }
  ...
  else
  {
    ...
    if (ekin >= lowLim && ekin < highLim)
    {
      ...
      if (pos != tableData->end())
      {
        ...
        if (table != 0)
        {
          ...
          if (Mion_c2 > proton_mass_c2)
          {
            ...
            {
              Zeff = BKZ(ekin / (proton_mass_c2 / Mion_c2), Mion_c2 / c_squared, Z,
                         currentMaterialStructure->Energy(
                           i));  // il faut garder le vrai ekin car le calcul à l'interieur de la
                                 // methode convertie l'énergie en vitesse
              Zeff2 = Zeff * Zeff;
      ...
      {
        G4Exception("G4MicroElecInelasticModel_new::CrossSectionPerVolume", "em0002",
                                                      ...
                                                      G4double)
{
  ...
  G4double originalMass =
    particleMass;  // a passer en argument dans samplesecondaryenergy pour évaluer correctement Qmax
  ...
  if (k >= lowLim && k < highLim)
  {
    ...
    if (currentMaterialStructure->IsShellWeaklyBound(Shell))
    {
      shellEnum = -1;
    }
    ...
    G4ThreeVector deltaDirection = GetAngularDistribution()->SampleDirectionForShell(
      particle, secondaryKinetic, Z, Shell, couple->GetMaterial());
    ...
    // fParticleChangeForGamma->SetProposedKineticEnergy(ekin - secondaryKinetic - limitEnergy);
    // //Ef = Ei-(Q-El)-El = Ei-Q fParticleChangeForGamma->ProposeLocalEnergyDeposit(limitEnergy -
    // deexSecEnergy);

    // correction CI 09/03/2022 limit energy  = gap
    // if (!SEFromFermiLevel && weaklyBound) limitEnergy +=
    // currentMaterialStructure->GetInitialEnergy(); if (!SEFromFermiLevel && weaklyBound)
    // limitEnergy += currentMaterialStructure->GetEnergyGap();
    fParticleChangeForGamma->SetProposedKineticEnergy(ekin - secondaryKinetic
                                                      - limitEnergy);  // Ef = Ei-(Q-El)-El = Ei-Q
    fParticleChangeForGamma->ProposeLocalEnergyDeposit(limitEnergy - deexSecEnergy);

    if (secondaryKinetic > 0)
    {
      G4DynamicParticle* dp = new G4DynamicParticle(G4Electron::Electron(), deltaDirection,
                                                    secondaryKinetic);  // Esec = Q-El
      fvect->push_back(dp);
    }
  }
}
...
G4double G4MicroElecInelasticModel_new::RandomizeEjectedElectronEnergy(
  const G4ParticleDefinition* particleDefinition, G4double k, G4int shell, G4double originalMass,
  G4int)
{
  ...
  if (particleDefinition == G4Electron::ElectronDefinition())
  {
    ...
    do
    {
      secondaryElectronKineticEnergy =
        G4UniformRand() * (maximumEnergyTransfer - currentMaterialStructure->GetLimitEnergy(shell));
    } while (G4UniformRand() * crossSectionMaximum > DifferentialCrossSection(
               particleDefinition, k,
               (secondaryElectronKineticEnergy + currentMaterialStructure->GetLimitEnergy(shell)),
               shell));
    // added 12/01/2023
  ...
  {
    G4double maximumEnergyTransfer = ComputeElasticQmax(
      k / (proton_mass_c2 / originalMass), currentMaterialStructure->Energy(shell),
  ...
  const G4ParticleDefinition* particleDefinition, G4double k, G4int shell)
{
  ...
  {
    secondaryElectronKineticEnergy =
      transf - currentMaterialStructure->GetLimitEnergy(shell);  // shell energy for core electrons,
...
G4double
G4MicroElecInelasticModel_new::TransferedEnergy(const G4ParticleDefinition* particleDefinition,
                                                G4double k, G4int ionizationLevelIndex,
                                                G4double random)
{
  G4double nrj = 0.;
  G4double valueK1 = 0;
  ...
  if (particleDefinition == G4Electron::ElectronDefinition())
  {
    ...
    {
      vector<TriDimensionMap>* eNrjTransfData =
        iterator_Nrj->second;  // Storage of possible transfer energies
      vector<VecMap>* eProbaShellMap =
        iterator_Proba->second;  // Storage of probabilities for energy transfer
      vector<G4double>* eTdummyVec = iterator_Tdummy->second;  // Incident energies for
                                                               // interpolation
          ...
          && random <= (*eProbaShellMap)[ionizationLevelIndex][(*k2)].back())
      {
        ...
        // The following condition avoid getting transfered energy < binding energy and forces cumxs
        // = 1 for maximum energy transfer.
        if (valuePROB11 == 0)
          nrjTransf11 = bindingEnergy;
        else
        ...
        if (valuePROB21 == 0)
          nrjTransf21 = bindingEnergy;
        else
          nrjTransf21 = (*eNrjTransfData)[ionizationLevelIndex][valueK2][valuePROB21];

        if (valuePROB22 == 1)
        {
          if ((valueK2 + bindingEnergy) / 2. > valueK2)
            maximumEnergyTransfer2 = valueK2;
          else
            maximumEnergyTransfer2 = (valueK2 + bindingEnergy) / 2.;
  ...
  else if (particleDefinition == G4Proton::ProtonDefinition())
  {
    ...
    else
    {
      ...
      vector<VecMap>* pProbaShellMap =
        iterator_Proba->second;  // Storage of probabilities for energy transfer
      vector<G4double>* pTdummyVec = iterator_Tdummy->second;  // Incident energies for
                                                               // interpolation
          ...
          && random <= (*pProbaShellMap)[ionizationLevelIndex][(*k2)].back())
      {
        ...
        if (valuePROB12 == 1)
          nrjTransf12 = maximumEnergyTransferP;
        else
        ...
        if (valuePROB22 == 1)
          nrjTransf22 = maximumEnergyTransferP;
        else
  ...
  {
    nrj = QuadInterpolator(valuePROB11, valuePROB12, valuePROB21, valuePROB22, nrjTransf11,
                           nrjTransf12, nrjTransf21, nrjTransf22, valueK1, valueK2, k, random);
  }
...
G4double G4MicroElecInelasticModel_new::DifferentialCrossSection(
  const G4ParticleDefinition* particleDefinition, G4double k, G4double energyTransfer,
  G4int LevelIndex)
{
  G4double sigma = 0.;
  ...
  if (energyTransfer >= currentMaterialStructure->GetLimitEnergy(LevelIndex))
  {
    ...
    if (particleDefinition == G4Electron::ElectronDefinition())
    {
          ...
          && iterator_TransfNrj != eVecmStorage.end())
      {
        ...
        {
          auto e12 =
            std::upper_bound((*eVecm)[(*t1)].begin(), (*eVecm)[(*t1)].end(), energyTransfer);
          auto e11 = e12 - 1;
          auto e22 =
            std::upper_bound((*eVecm)[(*t2)].begin(), (*eVecm)[(*t2)].end(), energyTransfer);
          auto e21 = e22 - 1;
      ...
      else
      {
        G4String str = "Material ";
        str += currentMaterial + " not found!";
        G4Exception("G4MicroElecDielectricModels::DifferentialCrossSection", "em0002",
                    FatalException, str);
      }
    }
    ...
    if (particleDefinition == G4Proton::ProtonDefinition())
    {
          ...
          && iterator_TransfNrj != pVecmStorage.end())
      {
        ...
        {
          auto e12 =
            std::upper_bound((*pVecm)[(*t1)].begin(), (*pVecm)[(*t1)].end(), energyTransfer);
          auto e11 = e12 - 1;
          auto e22 =
    ...
    if (xsProduct != 0.)
    {
      sigma = QuadInterpolator(valueE11, valueE12, valueE21, valueE22, xs11, xs12, xs21, xs22,
                               valueT1, valueT2, k, energyTransfer);
    }
  }
...
G4double G4MicroElecInelasticModel_new::Interpolate(G4double e1, G4double e2, G4double e,
...
G4double G4MicroElecInelasticModel_new::QuadInterpolator(G4double e11, G4double e12, G4double e21,
...
G4int G4MicroElecInelasticModel_new::RandomSelect(G4double k, const G4String& particle,
                                                  G4double originalMass, G4int originalZ)
{
  ...
  if (tablepos == tableTCS.end())
  {
    G4Exception("G4MicroElecInelasticModel_new::RandomSelect", "em0002", FatalException,
                "Model not applicable to material");
    return level;
  }
  ...
  if (originalMass > proton_mass_c2)
  {
    ...
    {
      Zeff[nl] = BKZ(k / (proton_mass_c2 / originalMass), originalMass / c_squared, originalZ,
                     currentMaterialStructure->Energy(nl));
    }
  }
...
G4double G4MicroElecInelasticModel_new::ComputeElasticQmax(G4double T1i, G4double T2i, G4double M1,
                                                           G4double M2)
{
...
{
  G4double r =
    vF * (std::pow(v / vF + 1., 3.) - fabs(std::pow(v / vF - 1., 3.)) + 4. * (v / vF) * (v / vF))
    + stepFunc(v / vF - 1.)
        * (3. / 2. * v / vF - 4. * (v / vF) * (v / vF) + 3. * std::pow(v / vF, 3.)
           - 0.5 * std::pow(v / vF, 5.));
  return r / (10. * v / vF);
}
...
G4double G4MicroElecInelasticModel_new::BKZ(G4double Ep, G4double mp, G4int Zp, G4double Eplasmon)
{
  ...
  G4double hbar = hbar_Planck, hbar2 = hbar * hbar, me = electron_mass_c2 / c_squared,
           Ry = me * elm_coupling * elm_coupling / (2 * hbar2);
  ...
  if (Neq <= 2)
    ...
    l0 = 0.48 * std::pow(Neq, 2. / 3.) / (Zp - Neq / 7.);
  if (Zp == 2)
    c = 1.0;
  else
    c = 3. / 2.;
