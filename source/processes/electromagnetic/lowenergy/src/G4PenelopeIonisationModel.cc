...
namespace
{
G4Mutex PenelopeIonisationModelMutex = G4MUTEX_INITIALIZER;
}
...
G4PenelopeIonisationModel::G4PenelopeIonisationModel(const G4ParticleDefinition* part,
  ...
  : G4VEmModel(nam),
    ...
    fKineticEnergy1(0. * eV),
    fCosThetaPrimary(1.0),
    fEnergySecondary(0. * eV),
    fCosThetaSecondary(0.0),
    fTargetOscillator(-1),
...
G4PenelopeIonisationModel::~G4PenelopeIonisationModel()
{
  ...
  {
    if (fCrossSectionHandler) delete fCrossSectionHandler;
  }
}
...
{
  if (fVerboseLevel > 3) G4cout << "Calling G4PenelopeIonisationModel::Initialise()" << G4endl;
  ...
  if (fAtomDeexcitation) fPIXEflag = fAtomDeexcitation->IsPIXEActive();
  ...
  if (IsMaster() && particle == fParticle)
  {
    ...
    {
      const G4Material* theMat = theCoupleTable->GetMaterialCutsCouple(i)->GetMaterial();
      // Forces the building of the cross section tables
      fCrossSectionHandler->BuildXSTable(theMat, theCuts.at(i), particle, IsMaster());
    }
...
G4double G4PenelopeIonisationModel::CrossSectionPerVolume(const G4Material* material,
                                                          ...
                                                          G4double energy, G4double cutEnergy,
                                                          G4double)
{
  ...
  const G4PenelopeCrossSection* theXS =
    fCrossSectionHandler->GetCrossSectionTableForCouple(theParticle, material, cutEnergy);
  if (!theXS)
  {
    ...
    if (fVerboseLevel > 0)
    {
      ...
      G4Exception("G4PenelopeIonisationModel::CrossSectionPerVolume()", "em2038", JustWarning, ed);
    }
    ...
    lock.unlock();
    // now it should be ok
    theXS = fCrossSectionHandler->GetCrossSectionTableForCouple(theParticle, material, cutEnergy);
  }

  if (theXS) crossPerMolecule = theXS->GetHardCrossSection(energy);

  G4double atomDensity = material->GetTotNbOfAtomsPerVolume();
  G4double atPerMol = fOscManager->GetAtomsPerMolecule(material);

  if (fVerboseLevel > 3)
    G4cout << "Material " << material->GetName() << " has " << atPerMol << "atoms per molecule"
           << G4endl;
  ...
  if (fVerboseLevel > 2)
  {
    ...
    G4cout << "Mean free path for delta emission > " << cutEnergy / keV << " keV at "
           << energy / keV << " keV = " << (crossPerVolume ? (1. / crossPerVolume) / mm : DBL_MAX)
           << " mm" << G4endl;
    if (theXS) totalCross = (theXS->GetTotalCrossSection(energy)) * moleculeDensity;
    G4cout << "Total free path for ionisation (no threshold) at " << energy / keV
           << " keV = " << (totalCross ? (1. / totalCross) / mm : DBL_MAX) << " mm" << G4endl;
  }
                                                               ...
                                                               G4double, G4double)
{
  ...
  G4cout << "Penelope Ionisation model v2008 does not calculate cross section _per atom_ "
         << G4endl;
                                                         ...
                                                         G4double kineticEnergy, G4double cutEnergy)
{
  ...
  if (fVerboseLevel > 3) G4cout << "Calling ComputeDEDX() of G4PenelopeIonisationModel" << G4endl;
  ...
  if (!theXS)
  {
    ...
    if (fVerboseLevel > 0)
    {
      ...
      ed << "Unable to retrieve the cross section table for " << theParticle->GetParticleName()
         << " in " << material->GetName() << ", cut = " << cutEnergy / keV << " keV " << G4endl;
      ed << "This can happen only in Unit Tests or via G4EmCalculator" << G4endl;
      G4Exception("G4PenelopeIonisationModel::ComputeDEDXPerVolume()", "em2038", JustWarning, ed);
    }
    // protect file reading via autolock
    G4AutoLock lock(&PenelopeIonisationModelMutex);
    fCrossSectionHandler->BuildXSTable(material, cutEnergy, theParticle);
  ...
  if (theXS) sPowerPerMolecule = theXS->GetSoftStoppingPower(kineticEnergy);
  ...
  {
    G4cout << "G4PenelopeIonisationModel " << G4endl;
    G4cout << "Stopping power < " << cutEnergy / keV << " keV at " << kineticEnergy / keV
           << " keV = " << sPowerPerVolume / (keV / mm) << " keV/mm" << G4endl;
  }
                                                  ...
                                                  G4double cutE, G4double)
{
  ...
  else
  {
    ...
    G4Exception("G4PenelopeIonisationModel::SamplingSecondaries()", "em0001", FatalException, ed);
  }
  ...
  G4double ionEnergyInPenelopeDatabase = (*theTable)[fTargetOscillator]->GetIonisationEnergy();
  ...
  if (fEnergySecondary > cutE)
  {
    ...
    electron = new G4DynamicParticle(G4Electron::Electron(), eDirection, fEnergySecondary);
  ...
  {
    G4Exception("G4PenelopeIonisationModel::SampleSecondaries()", "em2099", JustWarning,
  ...
  if (fVerboseLevel > 1)
  {
    ...
    if (energyInAuger) G4cout << "Auger electrons: " << energyInAuger / keV << " keV" << G4endl;
    G4cout << "Local energy deposit " << localEnergyDeposit / keV << " keV" << G4endl;
    G4cout << "Total final state: "
           << (fEnergySecondary + energyInFluorescence + fKineticEnergy1 + localEnergyDeposit
               + energyInAuger)
                / keV
           << " keV" << G4endl;
    G4cout << "-----------------------------------------------------------" << G4endl;
    ...
    if (energyDiff > 0.05 * keV)
      G4cout << "Warning from G4PenelopeIonisation: problem with energy conservation: "
             << (fEnergySecondary + energyInFluorescence + fKineticEnergy1 + localEnergyDeposit
                 + energyInAuger)
                  / keV
             << " keV (final) vs. " << kineticEnergy0 / keV << " keV (initial)" << G4endl;
  }
}

//....oooOO0OOooo........oooOO0OOooo........oooOO0OOooo........oooOO0OOooo....
void G4PenelopeIonisationModel::SampleFinalStateElectron(const G4Material* mat, G4double cutEnergy,
                                                         G4double kineticEnergy)
{
  ...
  const G4PenelopeCrossSection* theXS =
    fCrossSectionHandler->GetCrossSectionTableForCouple(G4Electron::Electron(), mat, cutEnergy);
  ...
  if (fVerboseLevel > 3)
  {
    G4cout << "SampleFinalStateElectron: sampled oscillator #" << fTargetOscillator << "."
           << G4endl;
  ...
  if (resEne > cutEnergy && resEne < kineticEnergy)
  {
    ...
    {
      G4double cpp =
        std::sqrt((kineticEnergy - resEne) * (kineticEnergy - resEne + 2.0 * electron_mass_c2));
      QM =
        std::sqrt((cp - cpp) * (cp - cpp) + electron_mass_c2 * electron_mass_c2) - electron_mass_c2;
    }
    else
    {
    ...
    if (QM < cutoffEne)
    {
      XHDL = G4Log(cutoffEne * (QM + 2.0 * electron_mass_c2)
  ...
  if (wcl < wmaxc)
  {
    ...
    XHC = (amol * (0.5 - rcl) + 1.0 / rcl - rrl1 + (1.0 - amol) * G4Log(rcl * rrl1)) / EE;
  }
  ...
  if (TST < TS1)
  {
    ...
    do
    {
      ...
      if (G4UniformRand() * (1.0 + A * rk2) > phi) loopAgain = true;
    } while (loopAgain);
    ...
    fCosThetaSecondary =
      std::sqrt(deltaE * rb / (kineticEnergy * (deltaE + 2.0 * electron_mass_c2)));
    if (fVerboseLevel > 3) G4cout << "SampleFinalStateElectron: sampled close collision " << G4endl;
    return;
  }
  ...
  if (TST < TS1)
  {
    ...
    G4double Q =
      QS
      / (std::pow((QS / cutoffEne) * (1.0 + cutoffEne * 0.5 / electron_mass_c2), G4UniformRand())
    ...
    if (fCosThetaPrimary > 1.) fCosThetaPrimary = 1.0;
    // energy and emission angle of the delta ray
    fEnergySecondary = deltaE - ionEne;
    fCosThetaSecondary =
      0.5 * (deltaE * (kineticEnergy + rb - deltaE) + QTREV) / std::sqrt(cps * QTREV);
    if (fCosThetaSecondary > 1.0) fCosThetaSecondary = 1.0;
    if (fVerboseLevel > 3)
...
void G4PenelopeIonisationModel::SampleFinalStatePositron(const G4Material* mat, G4double cutEnergy,
                                                         G4double kineticEnergy)
{
  // This method sets the final ionisation parameters
  // fKineticEnergy1, fCosThetaPrimary (= updates of the primary e-)
  ...
  std::size_t numberOfOscillators = theTable->size();
  const G4PenelopeCrossSection* theXS =
    fCrossSectionHandler->GetCrossSectionTableForCouple(G4Positron::Positron(), mat, cutEnergy);
  G4double delta = fCrossSectionHandler->GetDensityCorrection(mat, kineticEnergy);
  ...
  if (fVerboseLevel > 3)
  {
    G4cout << "SampleFinalStatePositron: sampled oscillator #" << fTargetOscillator << "."
           << G4endl;
    G4cout << "Ionisation energy: " << (*theTable)[fTargetOscillator]->GetIonisationEnergy() / eV
           << " eV " << G4endl;
    G4cout << "Resonance energy: : " << (*theTable)[fTargetOscillator]->GetResonanceEnergy() / eV
  ...
  {
    G4double rl1 = 1.0 - rcl;
    XHC = ((1.0 / rcl - 1.0) + bha1 * G4Log(rcl) + bha2 * rl1 + (bha3 / 2.0) * (rcl * rcl - 1.0)
           + (bha4 / 3.0) * (1.0 - rcl * rcl * rcl))
          / kineticEnergy;
  }
  ...
  if (TST < TS1)
  {
    ...
    do
    {
      ...
      if (G4UniformRand() > phi) loopAgain = true;
    } while (loopAgain);
    // energy and scattering angle (primary electron)
    ...
    fCosThetaSecondary =
      std::sqrt(deltaE * rb / (kineticEnergy * (deltaE + 2.0 * electron_mass_c2)));
    if (fVerboseLevel > 3) G4cout << "SampleFinalStatePositron: sampled close collision " << G4endl;
    return;
  }
