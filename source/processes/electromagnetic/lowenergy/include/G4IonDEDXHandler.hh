...
class G4IonDEDXHandler
{
  public:
    ...
    explicit G4IonDEDXHandler(G4VIonDEDXTable* tables, G4VIonDEDXScalingAlgorithm* algorithm,
                              const G4String& name, G4int maxCacheSize = 5, G4bool splines = true);
    ~G4IonDEDXHandler();
    ...
    G4bool IsApplicable(const G4ParticleDefinition*,  // Projectile (ion)
    ...
    G4double GetDEDX(const G4ParticleDefinition*,  // Projectile (ion)
                     const G4Material*,  // Target material
                     G4double);  // Kinetic energy of projectile

    // Function for building stopping power vectors according to Bragg's
    // additivity rule
    G4bool BuildDEDXTable(const G4ParticleDefinition*,  // Projectile (ion)
                          const G4Material*);  // Target material

    // Function for building stopping power vectors according to Bragg's
    // additivity rule
    G4bool BuildDEDXTable(G4int atomicNumberIon,  // Atomic number of ion
                          const G4Material*);  // Target material
    ...
    G4double GetLowerEnergyEdge(const G4ParticleDefinition*,  // Projectile (ion)
                                const G4Material*);  // Target material

    // Function returning the upper energy edge of stopping power tables
    G4double GetUpperEnergyEdge(const G4ParticleDefinition*,  // Projectile (ion)
                                const G4Material*);  // Target material
  ...
  private:
    ...
    G4CacheValue UpdateCacheValue(const G4ParticleDefinition*,  // Projectile (ion)
                                  const G4Material*);  // Target material

    // Function for retrieving cache values
    G4CacheValue GetCacheValue(const G4ParticleDefinition*,  // Projectile (ion)
                               const G4Material*);  // Target material
