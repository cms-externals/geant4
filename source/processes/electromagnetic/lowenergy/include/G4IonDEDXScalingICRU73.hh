...
class G4IonDEDXScalingICRU73 : public G4VIonDEDXScalingAlgorithm
{
  public:
    ...
    explicit G4IonDEDXScalingICRU73(G4int minAtomicNumberIon = 19, G4int maxAtomicNumberIon = 102);
    ...
    G4double ScalingFactorEnergy(const G4ParticleDefinition* particle,  // Projectile (ion)
    ...
    G4double ScalingFactorDEDX(const G4ParticleDefinition* particle,  // Projectile (ion)
    ...
    // particle.
    G4int AtomicNumberBaseIon(G4int atomicNumberIon,  // Atomic number of ion
                              const G4Material*) override;  // Target material

  private:

    void UpdateCacheParticle(const G4ParticleDefinition* particle);  // Projectile (ion)

    void UpdateCacheMaterial(const G4Material* material);  // Target material

    void CreateReferenceParticles();

    G4double EquilibriumCharge(G4double mass,  // Ion mass
...
inline void G4IonDEDXScalingICRU73::UpdateCacheParticle(const G4ParticleDefinition* particle)
{  // Projectile (ion)
...
inline void G4IonDEDXScalingICRU73::UpdateCacheMaterial(const G4Material* material)
{  // Target material
...
inline G4double G4IonDEDXScalingICRU73::EquilibriumCharge(G4double mass, G4double charge,
                                                          G4double atomicNumberPow,
                                                          G4double kineticEnergy)
{
  G4double totalEnergy = kineticEnergy + mass;
  G4double betaSquared = kineticEnergy * (totalEnergy + mass) / (totalEnergy * totalEnergy);
