...
#define G4MICROELECINELASTICMODEL_NEW_HH
...
class G4MicroElecInelasticModel_new : public G4VEmModel
{
  public:
    ...
    G4double DifferentialCrossSection(const G4ParticleDefinition* aParticleDefinition, G4double k,
  ...
  private:
    ...
    // private methods
    //
    G4int RandomSelect(G4double energy, const G4String& particle, G4double originalMass,
                       G4int originalZ);
    ...
    G4double RandomizeEjectedElectronEnergyFromCumulatedDcs(const G4ParticleDefinition*, G4double k,
                                                            G4int shell);

    G4double TransferedEnergy(const G4ParticleDefinition*, G4double k, G4int ionizationLevelIndex,
                              G4double random);
    ...
    dataDiffCSMap eNrjTransStorage,
      pNrjTransStorage;  // Transfered energies and corresponding probability (faster code)
    typedef std::map<G4String, std::vector<VecMap>*> dataProbaShellMap;
    dataProbaShellMap eProbaShellStorage,
      pProbaShellStorage;  // Cumulated Transfer probabilities (faster code)
    typedef std::map<G4String, std::vector<G4double>*> incidentEnergyMap;
    incidentEnergyMap eIncidentEnergyStorage,
      pIncidentEnergyStorage;  // Incident energies for interpolation (faster code)
