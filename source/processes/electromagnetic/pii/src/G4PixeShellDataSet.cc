...
G4PixeShellDataSet::G4PixeShellDataSet(G4int zeta, G4IInterpolator* algo, const G4String& modelK,
                                       const G4String& modelL, const G4String& modelM,
                                       G4double eUnit, G4double dataUnit)
  : z(zeta), algorithm(algo), unitEnergies(eUnit), unitData(dataUnit)
{
  if (algorithm == 0)
    G4Exception("G4PixeShellDataSet::G4PixeShellDataSet", "pii00000301", FatalException,
                "interpolation == 0");
...
void G4PixeShellDataSet::SetEnergiesData(G4DataVector* energies, G4DataVector* data,
                                         G4int componentId)
{
  ...
  G4Exception("G4PixeShellDataSet::SetEnergiesData", "pii000000310", FatalException,
              message.str().c_str());
}
...
G4String G4PixeShellDataSet::FullFileName(const G4String& file, const G4String& subShell) const
{
  const char* path = G4FindDataDir("G4PIIDATA");
  if (!path)
    G4Exception("G4PixeShellDataSet::FullFileName", "pii00000320", FatalException,
  ...
  fullFileName
    //<< path
    << "pixe/" << file << '/' << shellString << '/' << shellModel << '/' << subShell << '-';
  //	       << z
  //       << ".dat";
...
G4int G4PixeShellDataSet::TranslateShell(const G4String& subShell) const
{
  ...
  if (subShell == "m1" || subShell == "m2" || subShell == "m3" || subShell == "m4"
      || subShell == "m5")
    index = 2;
  return index;
}
