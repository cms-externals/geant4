...
// 20 Jun 2011 MGP     Corrected some compilation warnings. The whole class will be heavily
// refactored anyway.
...
G4ThreadLocal G4double G4hRDEnergyLoss::Mass, G4hRDEnergyLoss::taulow, G4hRDEnergyLoss::tauhigh,
  G4hRDEnergyLoss::ltaulow, G4hRDEnergyLoss::ltauhigh;
...
void G4hRDEnergyLoss::BuildDEDXTable(const G4ParticleDefinition& aParticleType)
{
  ...
  if (((Charge > 0.) && (theDEDXTable == 0)) || ((Charge < 0.) && (theDEDXTable == 0)))
  {
  ...
  G4EnergyLossTables::Register(&aParticleType, (Charge > 0) ? theDEDXpTable : theDEDXpbarTable,
...
void G4hRDEnergyLoss::BuildRangeTable(const G4ParticleDefinition& aParticleType)
{
  ...
  {
    G4PhysicsLogVector* aVector;
    aVector = new G4PhysicsLogVector(LowestKineticEnergy, HighestKineticEnergy, TotBin);
...
void G4hRDEnergyLoss::BuildTimeTables(const G4ParticleDefinition& aParticleType)
{
  ...
  for (G4int J = 0; J < numOfCouples; ++J)
  {
    ...
    bVector = new G4PhysicsLogVector(LowestKineticEnergy, HighestKineticEnergy, TotBin);
...
void G4hRDEnergyLoss::BuildLabTimeVector(G4int materialIndex, G4PhysicsLogVector* timeVector)
{
  ...
  G4double losslim, clim, taulim, timelim, LowEdgeEnergy, tau, Value;
...
void G4hRDEnergyLoss::BuildProperTimeVector(G4int materialIndex, G4PhysicsLogVector* timeVector)
{
...
void G4hRDEnergyLoss::InvertRangeVector(G4int materialIndex, G4PhysicsLogVector* aVector)
{
