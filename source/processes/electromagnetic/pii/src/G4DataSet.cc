...
G4DataSet::G4DataSet(G4int Z, G4IInterpolator* algo, G4double xUnit, G4double yUnit, G4bool random)
  : z(Z),
    energies(0),
    data(0),
    algorithm(algo),
...
{
  if (algorithm == 0)
    G4Exception("G4DataSet::G4DataSet", "pii00000101", FatalException, "Interpolation == 0");
  if (randomSet) BuildPdf();
}

G4DataSet::G4DataSet(G4int argZ, G4DataVector* dataX, G4DataVector* dataY, G4IInterpolator* algo,
...
{
  if (algorithm == 0)
    G4Exception("G4DataSet::G4DataSet", "pii00000110", FatalException, "Interpolation == 0");

  if ((energies == 0) ^ (data == 0))
    G4Exception("G4DataSet::G4DataSet", "pii00000111-", FatalException,
                "different size for energies and data (zero case)");

  if (energies == 0) return;

  if (energies->size() != data->size())
    G4Exception("G4DataSet::G4DataSet", "pii00000112", FatalException,
...
{
  if (!energies)
    G4Exception("G4DataSet::FindValue", "pii00000120", FatalException, "energies == 0");
  ...
  G4double interpolated =
    algorithm->Calculate(energy, (G4int)FindLowerBound(energy), *energies, *data);
  return interpolated;
}
...
void G4DataSet::SetEnergiesData(G4DataVector* dataX, G4DataVector* dataY, G4int /* componentId */)
{
  if (energies) delete energies;
  energies = dataX;
  ...
  if ((energies == 0) ^ (data == 0))
    G4Exception("G4DataSet::SetEnergiesData", "pii00000130", FatalException,
                "different size for energies and data (zero case)");

  if (energies == 0) return;

  if (energies->size() != data->size())
    G4Exception("G4DataSet::SetEnergiesData", "pii00000131", FatalException,
                "different size for energies and data");
}
...
G4bool G4DataSet::LoadData(const G4String& fileName)
{
  ...
  if (!in.is_open())
  {
    ...
    G4Exception("G4CompositeDataSet::LoadData", "pii00000140", FatalException,
                message.str().c_str());
  }
...
G4bool G4DataSet::SaveData(const G4String& name) const
{
  ...
  if (!out.is_open())
  {
    ...
    G4Exception("G4CompositeDataSet::SaveData", "pii00000150", FatalException,
                message.str().c_str());
  }
...
G4String G4DataSet::FullFileName(const G4String& name) const
{
  ...
  if (!path)
    G4Exception("G4DataSet::FullFileName", "pii00000160", FatalException,
...
G4double G4DataSet::RandomSelect(G4int /* componentId */) const
{
  ...
  if (!pdf)
    G4Exception("G4DataSet::RandomSelect", "pii00000170", FatalException,
