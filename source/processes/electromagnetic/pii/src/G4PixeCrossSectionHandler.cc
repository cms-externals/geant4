...
G4PixeCrossSectionHandler::G4PixeCrossSectionHandler(G4IInterpolator* algorithm,
                                                     const G4String& modelK, const G4String& modelL,
                                                     const G4String& modelM, G4double minE,
...
void G4PixeCrossSectionHandler::Initialise(G4IInterpolator* algorithm, const G4String& modelK,
                                           const G4String& modelL, const G4String& modelM,
                                           G4double minE, G4double maxE, G4int numberOfBins,
                                           G4double unitE, G4double unitData, G4int minZ,
                                           G4int maxZ)
{
  if (algorithm != 0)
  {
...
void G4PixeCrossSectionHandler::LoadShellData(const G4String& fileName)
{
  ...
  for (std::size_t i = 0; i < nZ; ++i)
  {
    ...
    G4IDataSet* dataSet =
      new G4PixeShellDataSet(Z, algo, crossModel[0], crossModel[1], crossModel[2]);
...
G4double G4PixeCrossSectionHandler::FindValue(G4int Z, G4double energy) const
{
  ...
  {
    G4cout << "WARNING: G4PixeCrossSectionHandler::FindValue(Z,e) did not find Z = " << Z << G4endl;
  }
  return value;
}

G4double G4PixeCrossSectionHandler::FindValue(G4int Z, G4double energy, G4int shellIndex) const
{
  G4double value = 0.;
  ...
  {
    G4cout << "WARNING: G4PixeCrossSectionHandler::FindValue did not find Z = " << Z << G4endl;
  }
  return value;
}
...
/*
  G4IDataSet* G4PixeCrossSectionHandler::BuildMeanFreePathForMaterials(const G4DataVector*
  energyCuts )
  {
...
void G4PixeCrossSectionHandler::BuildForMaterials()
{
  ...
  if (crossSections == 0)
    G4Exception("G4PixeCrossSectionHandler::BuildForMaterials", "pii00000210", FatalException,
                ", crossSections = 0");

  return;
}

G4int G4PixeCrossSectionHandler::SelectRandomAtom(const G4Material* material, G4double e) const
{
...
/*
  const G4Element* G4PixeCrossSectionHandler::SelectRandomElement(const G4MaterialCutsCouple*
  couple, G4double e) const
  {
...
void G4PixeCrossSectionHandler::ActiveElements()
{
  ...
  if (materialTable == 0)
    G4Exception("G4PixeCrossSectionHandler::ActiveElements", "pii00000220", FatalException,
...
G4PixeCrossSectionHandler::BuildCrossSectionsForMaterials(const G4DataVector& energyVector)
{
  ...
  const G4MaterialTable* materialTable = G4Material::GetMaterialTable();
  if (materialTable == 0)
    G4Exception("G4PixeCrossSectionHandler::BuildCrossSectionsForMaterials", "pii00000230",
