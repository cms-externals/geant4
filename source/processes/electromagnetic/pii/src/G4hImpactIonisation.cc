...
//                     Calculation of MicroscopicCrossSection delegated to specialised cla//
...
//  just call BuildLossTable+BuildLambdaTable
{
  ...
  if (verboseLevel > 0)
  {
    G4cout << "G4hImpactIonisation::BuildPhysicsTable for " << particleDef.GetParticleName()
           << " mass(MeV)= " << particleDef.GetPDGMass() / MeV
           << " charge= " << particleDef.GetPDGCharge() / eplus
           << " type= " << particleDef.GetParticleType() << G4endl;
    ...
    if (verboseLevel > 1)
    {
      ...
      G4cout << " 0: " << (*pv)[0]->GetProcessName() << " " << (*pv)[0]
             ...
             << G4endl;
      G4cout << "ionModel= " << theIonEffChargeModel << " MFPtable= " << theMeanFreePathTable
             << " iniMass= " << initialMass << G4endl;
    }
  }
  // End of verbose print-out

  if (particleDef.GetParticleType() == "nucleus" && particleDef.GetParticleName() != "GenericIon"
      && particleDef.GetParticleSubType() == "generic")
  {
    G4EnergyLossTables::Register(&particleDef, theDEDXpTable, theRangepTable, theInverseRangepTable,
                                 theLabTimepTable, theProperTimepTable, LowestKineticEnergy,
                                 HighestKineticEnergy, proton_mass_c2 / particleDef.GetPDGMass(),
                                 TotBin);
  ...
  {
    {
      ...
      //      It happended in the past that caused memory corruption errors. The problem is still
      //      pending, even if temporary solved
      //        G4cout << "[NOTE]: __LINE__=" << __LINE__ << ", particleDef=" <<
      //        particleDef.GetParticleName() << ", proton=" << proton << ", theLossTable=" <<
      //        theLossTable << ", CounterOfpProcess=" << CounterOfpProcess << G4endl;
  ...
  {
    {
      ...
      //      pending, even if temporary solved
      //        G4cout << "[NOTE]: __LINE__=" << __LINE__ << ", particleDef=" <<
      //        particleDef.GetParticleName() << ", antiproton=" << antiproton << ", theLossTable="
      //        << theLossTable << ", CounterOfpbarProcess=" << CounterOfpbarProcess << G4endl;
  ...
  if (verboseLevel > 0)
  {
    G4cout << "G4hImpactIonisation::BuildPhysicsTable: end for " << particleDef.GetParticleName()
           << G4endl;
  }
}
...
void G4hImpactIonisation::BuildLossTable(const G4ParticleDefinition& particleDef)
{
  ...
  for (G4int j = 0; j < numOfCouples; ++j)
  {
    ...
    G4PhysicsLogVector* aVector =
      new G4PhysicsLogVector(LowestKineticEnergy, HighestKineticEnergy, TotBin);
    ...
    for (G4int i = 0; i < TotBin; i++)
    {
      ...
      else
      {
        ...
        ionloss = betheBlochModel->TheValue(proton, material, lowEdgeEnergy);
      ...
      if (verboseLevel > 1)
      {
        G4cout << "E(MeV)= " << lowEdgeEnergy / MeV << "  dE/dx(MeV/mm)= " << ionloss * mm / MeV
...

{
  ...
  if (verboseLevel > 1)
  {
    G4cout << "G4hImpactIonisation::BuildLambdaTable for " << particleDef.GetParticleName()
           << " is started" << G4endl;
  }
  ...
  for (G4int j = 0; j < numOfCouples; ++j)
  {
    ...
    for (G4int i = 0; i < TotBin; i++)
    {
      ...
      for (G4int iel = 0; iel < numberOfElements; iel++)
      {
        ...
        G4double microCross = MicroscopicCrossSection(particleDef, lowEdgeEnergy, Z, deltaCut);
                                                      ...
                                                      G4double deltaCutInEnergy) const
{
  ...
  G4double tMax =
    2. * electron_mass_c2 * (gamma * gamma - 1.) / (1. + 2. * gamma * var + var * var);
  ...
  if (tMax > deltaCutInEnergy)
  {
    ...
    {
      totalCrossSection +=
        -std::log(var) / (3. * deltaCutInEnergy)
        + (tMax - deltaCutInEnergy)
            * ((5. + 1. / var) * 0.25 / (energy * energy) - beta2 / (tMax * deltaCutInEnergy)) / 3.;
    }
                                              ...
                                              enum G4ForceCondition* condition)
{
  ...
  *condition = NotForced;
  ...
  G4double kineticEnergy =
    (dynamicParticle->GetKineticEnergy()) * initialMass / (dynamicParticle->GetMass());
  ...
  else
  {
    ...
    meanFreePath =
      (((*theMeanFreePathTable)(couple->GetIndex()))->GetValue(kineticEnergy, isOutRange))
      / chargeSquare;
  }
                                             ...
                                             const G4MaterialCutsCouple* couple)
{
  ...
  if (charge > 0.)
  {
    ...
    fdEdx = G4EnergyLossTables::GetDEDX(proton, tScaled, couple) * chargeSquare;
  ...
  */
  ...
  stepLimit = fRangeNow;
  G4double r =
    std::min(finalRange, couple->GetProductionCuts()->GetProductionCut(idxG4ElectronCut));
...
// --------------------------------------------------------------------
G4VParticleChange* G4hImpactIonisation::AlongStepDoIt(const G4Track& track, const G4Step& step)
{
  ...
  else
  {
    ...
    if (stepLength > linLossLimit * fRangeNow)
    {
      ...
      else
      {
        // Antiproton
        eLoss =
          G4EnergyLossTables::GetPreciseEnergyFromRange(antiproton, rScaled, couple)
                                                     ...
                                                     G4double kineticEnergy) const
{
  ...
  if (verboseLevel > 2)
  {
    G4cout << "p E(MeV)= " << kineticEnergy / MeV << " dE/dx(MeV/mm)= " << eLoss * mm / MeV
           << " for " << material->GetName() << " model: " << protonModel << G4endl;
  }
                                                         ...
                                                         G4double kineticEnergy) const
{
  ...
  if (antiprotonModel->IsInCharge(antiproton, material))
  {
    ...
    {
      eLoss = antiprotonModel->TheValue(antiproton, material, kineticEnergy);
    }
  ...
  else
  {
    ...
    {
      eLoss = protonModel->TheValue(G4Proton::Proton(), material, kineticEnergy);
    }
  ...
  if (verboseLevel > 2)
  {
    G4cout << "pbar E(MeV)= " << kineticEnergy / MeV << " dE/dx(MeV/mm)= " << eLoss * mm / MeV
           << " for " << material->GetName() << " model: " << protonModel << G4endl;
  }
...
G4double G4hImpactIonisation::DeltaRaysEnergy(const G4MaterialCutsCouple* couple,
                                              G4double kineticEnergy, G4double particleMass) const
{
  G4double dLoss = 0.;
  ...
  G4double tMax =
    2. * electron_mass_c2 * bg2 / (1.0 + 2.0 * gamma * rateMass + rateMass * rateMass);
...
G4VParticleChange* G4hImpactIonisation::PostStepDoIt(const G4Track& track, const G4Step& step)
{
  ...
  if (deltaCut >= tMax) return G4VContinuousDiscreteProcess::PostStepDoIt(track, step);
  ...
  G4double deltaTotalMomentum =
    std::sqrt(deltaKineticEnergy * (deltaKineticEnergy + 2. * electron_mass_c2));
  G4double totalMomentum = std::sqrt(pSquare);
  G4double cosTheta =
    deltaKineticEnergy * (totalEnergy + electron_mass_c2) / (deltaTotalMomentum * totalMomentum);
  ...
  deltaRay->SetMomentumDirection(deltaDirection.x(), deltaDirection.y(), deltaDirection.z());
  ...
  {
    G4double finalPx =
      totalMomentum * particleDirection.x() - deltaTotalMomentum * deltaDirection.x();
    G4double finalPy =
      totalMomentum * particleDirection.y() - deltaTotalMomentum * deltaDirection.y();
    G4double finalPz =
      totalMomentum * particleDirection.z() - deltaTotalMomentum * deltaDirection.z();
  ...
  else
  {
    ...
    aParticleChange.ProposeMomentumDirection(particleDirection.x(), particleDirection.y(),
                                             particleDirection.z());
...
G4double G4hImpactIonisation::BlochTerm(const G4Material* material, G4double kineticEnergy,
...
//
{
  ...
  eLoss *= -1.0 * y * cSquare * twopi_mc2_rcl2 * (material->GetElectronDensity()) / beta2;
...
G4double G4hImpactIonisation::ElectronicLossFluctuation(const G4DynamicParticle* particle,
                                                        const G4MaterialCutsCouple* couple,
                                                        G4double meanLoss, G4double step) const
...
// as in Glandz in Geant3.
{
  ...
  {
    siga = tMax * (1.0 - 0.5 * beta2) * step * twopi_mc2_rcl2 * electronDensity / beta2;
...
void G4hImpactIonisation::PrintInfoDefinition() const
{
  ...
  G4cout << G4endl << GetProcessName() << ":  " << comments << "\n        PhysicsTables from "
         << LowestKineticEnergy / eV << " eV "
         << " to " << HighestKineticEnergy / TeV << " TeV "
         << " in " << TotBin << " bins."
         << "\n        Electronic stopping power model is  " << protonTable << "\n        from "
         << protonLowEnergy / keV << " keV "
         << " to " << protonHighEnergy / MeV << " MeV " << "." << G4endl;
  G4cout << "\n        Parametrisation model for antiprotons is  " << antiprotonTable
  ...
  if (nStopping)
  {
    G4cout << "        Nuclear stopping power model is " << theNuclearTable << G4endl;
  }
  ...
  for (G4int j = 0; j < numOfCouples; ++j)
  {
    ...
    if (excitationEnergy > deltaCutNow)
    {
      ...
      G4cout << std::setw(20) << material->GetName() << std::setw(15) << excitationEnergy / keV
             << G4endl;
    }
  }
}
