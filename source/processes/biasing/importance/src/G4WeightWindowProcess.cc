...
G4WeightWindowProcess::G4WeightWindowProcess(const G4VWeightWindowAlgorithm& aWeightWindowAlgorithm,
                                             ...
                                             G4PlaceOfAction placeOfAction, const G4String& aName,
                                             G4bool para)
  : G4VProcess(aName),
    ...
    fPlaceOfAction(placeOfAction)
{
  ...
  {
    G4Exception("G4WeightWindowProcess::G4WeightWindowProcess()", "FatalError", FatalException,
...
void G4WeightWindowProcess::StartTracking(G4Track* trk)
{
  ...
  if (fParaflag)
  {
    ...
    {
      G4Exception(
        "G4WeightWindowProcess::StartTracking", "ProcParaWorld000", FatalException,
...
G4double G4WeightWindowProcess::PostStepGetPhysicalInteractionLength(const G4Track&, G4double,
...
G4VParticleChange* G4WeightWindowProcess::PostStepDoIt(const G4Track& aTrack, const G4Step& aStep)
{
  ...
  if (aStep.GetStepLength() > kCarTolerance)
  {
    ...
    if (fParaflag)
    {
              ...
              && (fGhostPostStepPoint->GetStepStatus() != fGeomBoundary)))
      {
        ...
        G4Nsplit_Weight nw = fWeightWindowAlgorithm.Calculate(
          aTrack.GetWeight(),
          fWeightWindowStore.GetLowerWeight(postCell, aTrack.GetKineticEnergy()));
...
G4VParticleChange* G4WeightWindowProcess::AtRestDoIt(const G4Track&, const G4Step&)
{
  return nullptr;
}

G4VParticleChange* G4WeightWindowProcess::AlongStepDoIt(const G4Track& track, const G4Step&)
{
