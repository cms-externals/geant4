...
void G4InteractionLawPhysical::SetPhysicalCrossSection(G4double crossSection)
{
  if (crossSection < 0.0)
  {
    G4Exception("G4InteractionLawPhysical::SetPhysicalCrossSection(..)", "BIAS.GEN.14", JustWarning,
...
G4double G4InteractionLawPhysical::ComputeEffectiveCrossSectionAt(G4double) const
{
  ...
  {
    G4Exception("G4InteractionLawPhysical::ComputeEffectiveCrossSection(..)", "BIAS.GEN.15",
                JustWarning,
...
G4double G4InteractionLawPhysical::ComputeNonInteractionProbabilityAt(G4double stepLength) const
{
  if (!fCrossSectionDefined)
  {
    G4Exception("G4InteractionLawPhysical::ComputeNonInteractionProbability(..)", "BIAS.GEN.16",
                JustWarning,
                "Non interaction probabitlity value requested, but cross section has not been "
                "defined yet. Assumes it to be 0 !");
  }
  // -- allows zero cross-section case, by convention:
  if (fCrossSection == 0.0)
    return 1.0;
  else
    return std::exp(-fCrossSection * stepLength);
}
...
G4double G4InteractionLawPhysical::SampleInteractionLength()
{
  ...
  {
    G4Exception("G4InteractionLawPhysical::Sample(..)", "BIAS.GEN.17", FatalException,
...
G4double G4InteractionLawPhysical::UpdateInteractionLengthForStep(G4double truePathLength)
{
  ...
  if (fNumberOfInteractionLength < 0)
  {
    ...
    G4Exception("G4InteractionLawPhysical::UpdateInteractionLengthForStep(...)", "BIAS.GEN.13",
                JustWarning, ed);
