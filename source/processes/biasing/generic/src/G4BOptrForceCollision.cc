...
G4BOptrForceCollision::G4BOptrForceCollision(const G4String& particleName, const G4String& name)
    ...
    fForceCollisionModelID(G4PhysicsModelCatalog::GetModelID("model_GenBiasForceCollision"))
{
  ...
  if (fParticleToBias == nullptr)
  {
    ...
    G4Exception(" G4BOptrForceCollision::G4BOptrForceCollision(...)", "BIAS.GEN.07", JustWarning,
                ed);
  }
}

G4BOptrForceCollision::G4BOptrForceCollision(const G4ParticleDefinition* particle,
                                             const G4String& name)
  : G4VBiasingOperator(name),
    fForceCollisionModelID(G4PhysicsModelCatalog::GetModelID("model_GenBiasForceCollision"))
{
...
{
  for (auto it = fFreeFlightOperations.cbegin(); it != fFreeFlightOperations.cend(); ++it)
  {
    delete (*it).second;
  }
...
void G4BOptrForceCollision::ConfigureForWorker()
{
  ...
  if (fSetup)
  {
    ...
    if (interfaceProcessSharedData)  // -- sharedData tested, as is can happen a user attaches an
                                     // operator
                                     // -- to a volume but without defining BiasingProcessInterface
                                     // processes.
    {
      for (std::size_t i = 0;
           i < (interfaceProcessSharedData->GetPhysicsBiasingProcessInterfaces()).size(); ++i)
      {
        const G4BiasingProcessInterface* wrapperProcess =
          (interfaceProcessSharedData->GetPhysicsBiasingProcessInterfaces())[i];
        const G4String& operationName =
          "FreeFlight-" + wrapperProcess->GetWrappedProcess()->GetProcessName();
...
G4VBiasingOperation* G4BOptrForceCollision::ProposeOccurenceBiasingOperation(
  const G4Track* track, const G4BiasingProcessInterface* callingProcess)
{
  ...
  {
    // ... and if the track has no aux. track data, it means its biasing is not started yet (note
    // that cloning is to happen first):
    fCurrentTrackData = (G4BOptrForceCollisionTrackData*)(track->GetAuxiliaryTrackInformation(
  ...
  if (fCurrentTrackData->fForceCollisionState == ForceCollisionState::toBeForced)
  {
    ...
    if (isFirstPhysGPIL)
    {
      ...
      else
      {
        ...
        {
          // -- means that some other physics process, not under control of the forced interaction
          // operation,
          // -- has occured, need to re-initialize the operation as distance to boundary has
          // changed.
        ...
        {
          // -- means that some other non-physics process (biasing or not, like step limit), has
          // occured,
          // -- but track conserves its momentum direction, only need to reduced the maximum
          // distance for
          // -- forced interaction.
    ...
    // -- [* first process*] collect cross-sections and sample force law to determine interaction
    // length
    // -- and winning process:
    if (isFirstPhysGPIL)
    {
      ...
      for (std::size_t i = 0; i < (sharedData->GetPhysicsBiasingProcessInterfaces()).size(); ++i)
      {
        ...
        // -- keep only well defined cross-sections, other processes are ignored. These are not
        // pathological cases
        // -- but cases where a threhold effect par example (eg pair creation) may be at play.
        // (**!**)
        if (interactionLength < DBL_MAX / 10.)
          fSharedForceInteractionOperation->AddCrossSection(wrapperProcess->GetWrappedProcess(),
                                                            1.0 / interactionLength);
      }
...
G4VBiasingOperation* G4BOptrForceCollision::ProposeNonPhysicsBiasingOperation(
  const G4Track* track, const G4BiasingProcessInterface* /* callingProcess */)
{
  if (track->GetDefinition() != fParticleToBias) return nullptr;
  ...
  {
    // -- check that track is free of undergoing biasing scheme ( no biasing data, or no active
    // active )
...
G4VBiasingOperation* G4BOptrForceCollision::ProposeFinalStateBiasingOperation(
  ...
  if (fCurrentTrackData != nullptr)
  {
    if (!fCurrentTrackData->IsFreeFromBiasing())
    {
          ...
          || (fCurrentTrack->GetTrackStatus() == fKillTrackAndSecondaries))
      {
        ...
        G4Exception(" G4BOptrForceCollision::EndTracking()", "BIAS.GEN.18", JustWarning, ed);
      }
    }
  }
}
                                             ...
                                             const G4VParticleChange*)
{
  if (fCurrentTrackData == nullptr)
  {
    if (BAC != BAC_None)
    {
      ...
      G4Exception(" G4BOptrForceCollision::OperationApplied(...)", "BIAS.GEN.20.1", JustWarning,
                  ed);
    }
    return;
  }
  ...
  if (fCurrentTrackData->fForceCollisionState == ForceCollisionState::toBeCloned)
  {
    ...
    fCloningOperation->GetCloneTrack()->SetAuxiliaryTrackInformation(fForceCollisionModelID,
                                                                     cloneData);
  }
  ...
  else if (fCurrentTrackData->fForceCollisionState == ForceCollisionState::toBeForced)
  {
    if (operationApplied != fSharedForceInteractionOperation)
    {
      G4ExceptionDescription ed;
      ed << " Internal inconsistency : please submit bug report. " << G4endl;
      G4Exception(" G4BOptrForceCollision::OperationApplied(...)", "BIAS.GEN.20.2", JustWarning,
                  ed);
    }
    if (fSharedForceInteractionOperation->GetInteractionOccured())
    {
      if (operationApplied != fSharedForceInteractionOperation)
      {
        G4ExceptionDescription ed;
        ed << " Internal inconsistency : please submit bug report. " << G4endl;
        G4Exception(" G4BOptrForceCollision::OperationApplied(...)", "BIAS.GEN.20.3", JustWarning,
                    ed);
      }
    }
  }
  else
  {
    if (fCurrentTrackData->fForceCollisionState != ForceCollisionState::free)
    {
      G4ExceptionDescription ed;
      ed << " Internal inconsistency : please submit bug report. " << G4endl;
      G4Exception(" G4BOptrForceCollision::OperationApplied(...)", "BIAS.GEN.20.4", JustWarning,
                  ed);
    }
  }
}
                                             ...
                                             const G4VParticleChange* /*particleChangeProduced*/)
{
  if (fCurrentTrackData->fForceCollisionState == ForceCollisionState::toBeForced)
  {
    if (finalStateOperationApplied != fSharedForceInteractionOperation)
    {
      G4ExceptionDescription ed;
      ed << " Internal inconsistency : please submit bug report. " << G4endl;
      G4Exception(" G4BOptrForceCollision::OperationApplied(...)", "BIAS.GEN.20.5", JustWarning,
                  ed);
    }
    if (fSharedForceInteractionOperation->GetInteractionOccured())
      fCurrentTrackData->Reset();  // -- off biasing for this track
  ...
  else
  {
    G4ExceptionDescription ed;
    ed << " Internal inconsistency : please submit bug report. " << G4endl;
    G4Exception(" G4BOptrForceCollision::OperationApplied(...)", "BIAS.GEN.20.6", JustWarning, ed);
  }
}
