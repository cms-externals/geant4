...
G4BOptnLeadingParticle::G4BOptnLeadingParticle(const G4String& name) : G4VBiasingOperation(name) {}

G4BOptnLeadingParticle::~G4BOptnLeadingParticle() {}

G4VParticleChange*
G4BOptnLeadingParticle::ApplyFinalStateBiasing(const G4BiasingProcessInterface* callingProcess,
                                               const G4Track* track, const G4Step* step, G4bool&)
{
  // -- collect wrapped process particle change:
  auto wrappedProcessParticleChange =
  ...
  if (wrappedProcessParticleChange->GetNumberOfSecondaries() == 0)
    return wrappedProcessParticleChange;
  if (wrappedProcessParticleChange->GetTrackStatus() == fKillTrackAndSecondaries)
    return wrappedProcessParticleChange;
  // -- ... else, collect the secondaries in a same vector (the primary is pushed in this vector, if
  // surviving, later see [**]):
  std::vector<G4Track*> secondariesAndPrimary;
  for (auto i = 0; i < wrappedProcessParticleChange->GetNumberOfSecondaries(); i++)
    secondariesAndPrimary.push_back(wrappedProcessParticleChange->GetSecondary(i));

  // -- If case the primary survives, need to collect its new state. In the general case of the base
  // class G4VParticleChange
  // -- this is tricky, as this class does not hold the primary changes (and we have to build a fake
  // step and fake track
  // -- caring about the touchables, etc.) So we limit here to the G4ParticleChange case, checking
  // the reality of this
  // -- class with a dynamic cast. If we don't have such an actual G4DynamicParticle object, we give
  // up the biasing and return
  // -- the untrimmed process final state.
  // -- Note that this case does not happen often, as the technique is intended for inelastic
  // processes. For case where several
  // -- particles can be produced without killing the primary, we have for example the
  ...
  if ((wrappedProcessParticleChange->GetTrackStatus() != fStopAndKill))
  {
    ...
    {
      G4cout << " **** G4BOptnLeadingParticle::ApplyFinalStateBiasing(...) : can not bias for "
             << callingProcess->GetProcessName() << ", this is just a warning." << G4endl;
      return wrappedProcessParticleChange;
    }
  ...
  // ---- collect primary track weight, from updated by process if alive, or from original copy if
  // died:
  G4double primaryWeight;
  if (finalStatePrimary)
    primaryWeight = finalStatePrimary->GetWeight();
  else
  ...
  // -- finds the leading particle, initialize a map of surviving tracks, tag as surviving the
  // leading track:
  ...
  survivingMap[secondariesAndPrimary[leadingIDX]] =
    true;  // -- tag as surviving the leading particle

  // -- now make track vectors of given types ( choose type = abs(PDG) ), excluding the leading
  // particle:
  ...
  for (std::size_t idx = 0; idx < secondariesAndPrimary.size(); ++idx)
  {
    ...
    auto GROUP =
      std::abs(currentTrack->GetDefinition()
                 ->GetPDGEncoding());  // -- merge particles and anti-particles in the same category
                                       // -- §§ this might be proposed as an option in future
    if (currentTrack->GetDefinition()->GetBaryonNumber() >= 2)
      GROUP = -1000;  // -- merge all baryons above proton/neutron in one same group -- §§ might be
                      // proposed as an option too
  ...
  // ---- note we loop up to on the number of secondaries, which excludes the primary, last in
  // secondariesAndPrimary vector:
  ...
  for (auto idx = 0; idx < wrappedProcessParticleChange->GetNumberOfSecondaries(); ++idx)
  {
    ...
    ///// else G4cout << " ";
    ///// G4cout << secondary->GetDefinition()->GetParticleName() << " " <<
