...
G4BOptnForceFreeFlight::G4BOptnForceFreeFlight(const G4String& name) : G4VBiasingOperation(name)
{
...
const G4VBiasingInteractionLaw* G4BOptnForceFreeFlight::ProvideOccurenceBiasingInteractionLaw(
...
G4VParticleChange*
G4BOptnForceFreeFlight::ApplyFinalStateBiasing(const G4BiasingProcessInterface* callingProcess,
                                               ...
                                               G4bool& forceFinalState)
{
  ...
  if (step->GetPostStepPoint()->GetStepStatus() == fGeomBoundary)
  {
    ...
    if (fInitialTrackWeight <= DBL_MIN)
    {
      ...
      G4Exception(" G4BOptnForceFreeFlight::ApplyFinalStateBiasing(...)", "BIAS.GEN.05",
                  JustWarning, ed);
    }
    if (fCumulatedWeightChange <= DBL_MIN)
    {
      ...
      G4Exception(" G4BOptnForceFreeFlight::ApplyFinalStateBiasing(...)", "BIAS.GEN.06",
                  JustWarning, ed);
    }
...
void G4BOptnForceFreeFlight::AlongMoveBy(const G4BiasingProcessInterface*, const G4Step*,
