...
G4ProcessPlacer::G4ProcessPlacer(const G4String& particlename) : fParticleName(particlename) {}

G4ProcessPlacer::~G4ProcessPlacer() {}

void G4ProcessPlacer::RemoveProcess(G4VProcess* process)
{
  G4cout << "=== G4ProcessPlacer::RemoveProcess: for: " << fParticleName << G4endl;
  G4cout << "  ProcessName: " << process->GetProcessName() << ", will be removed!" << G4endl;
...
void G4ProcessPlacer::AddProcessAs(G4VProcess* process, SecondOrLast sol)
{
  ...
  else if (sol == eSecond)
  {
    ...
    G4VProcess* transportation = (*(GetProcessManager()->GetProcessList()))[0];

    if (!transportation)
    {
      G4Exception("G4ProcessPlacer::AddProcessAs", "Bias0001", RunMustBeAborted,
                  " could not get process id=0");
    }
    if (transportation->GetProcessName() != "Transportation"
        && transportation->GetProcessName() != "Transportation8"
        && transportation->GetProcessName() != "CoupledTransportation")
    {
      //      G4cout << " GOT HERE CoupledTransportation" << G4endl;
      G4cout << transportation->GetProcessName() << G4endl;
      G4Exception("G4ProcessPlacer::AddProcessAs", "Bias0002", RunMustBeAborted,
    ...
    GetProcessManager()->SetProcessOrderingToSecond(process, idxAlongStep);
    GetProcessManager()->SetProcessOrderingToSecond(process, idxPostStep);
  }
...
{
  G4cout << "=== G4ProcessPlacer::AddProcessAsSecondDoIt: for: " << fParticleName << G4endl;
  AddProcessAs(process, eSecond);
}

void G4ProcessPlacer::AddProcessAsLastDoIt(G4VProcess* process)
{
  G4cout << "=== G4ProcessPlacer::AddProcessAsLastDoIt: for: " << fParticleName << G4endl;
...
G4ProcessManager* G4ProcessPlacer::GetProcessManager()
{
  ...
  if (!processmanager)
  {
    G4Exception("G4ProcessPlacer::GetProcessManager()", "InvalidSetup", FatalException,
                "NULL pointer to Process Manager ! Sampler.Configure() must be after PhysicsList "
                "instantiation");
  }
...
void G4ProcessPlacer::PrintAlongStepGPILVec()
{
  ...
  G4ProcessVector* processGPILVec = GetProcessManager()->GetAlongStepProcessVector(typeGPIL);
...
void G4ProcessPlacer::PrintAlongStepDoItVec()
{
  ...
  G4ProcessVector* processDoItVec = GetProcessManager()->GetAlongStepProcessVector(typeDoIt);
...
{
  G4cout << "GPIL Vector: " << G4endl;
  G4ProcessVector* processGPILVec = GetProcessManager()->GetPostStepProcessVector(typeGPIL);
  PrintProcVec(processGPILVec);
}
...
{
  G4cout << "DoIt Vector: " << G4endl;
  G4ProcessVector* processDoItVec = GetProcessManager()->GetPostStepProcessVector(typeDoIt);
  PrintProcVec(processDoItVec);
}
...
void G4ProcessPlacer::PrintProcVec(G4ProcessVector* processVec)
{
  if (!processVec)
  {
    G4Exception("G4ProcessPlacer::G4ProcessPlacer()", "InvalidArgument", FatalException,
                "NULL pointer to process-vector !");
  }
  G4int len = (G4int)processVec->length();
  if (len == 0)
  {
    G4Exception("G4ProcessPlacer::G4ProcessPlacer()", "InvalidSetup", FatalException,
  ...
  for (G4int i = 0; i < len; ++i)
  {
    ...
    {
      G4cout << "   " << "no process found for position: " << i << ", in vector of length: " << len
             << G4endl;
    }
  }
}
