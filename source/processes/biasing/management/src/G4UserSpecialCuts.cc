...
G4UserSpecialCuts::G4UserSpecialCuts(const G4String& aName) : G4VProcess(aName, fGeneral)
{
...
G4UserSpecialCuts::G4UserSpecialCuts(G4UserSpecialCuts& right) : G4VProcess(right)
{
  theLossTableManager = G4LossTableManager::Instance();
}
...
G4double G4UserSpecialCuts::PostStepGetPhysicalInteractionLength(const G4Track& aTrack,
                                                                 ...
                                                                 G4ForceCondition* condition)
{
  ...
  *condition = NotForced;
  ...
  if (pUserLimits)
  {
    ...
    if (Ekine <= pUserLimits->GetUserMinEkine(aTrack))
    {
      return 0.;
    }
