    ...
    fFirstStepInMassVolume(true)
{
  ...
  if (verboseLevel > 0)
  {
    ...
    G4cout << " Reports First/Last in " << (fSignifyStepInAnyVolume ? " any " : " mass ")
           << " geometry " << G4endl;
  }
...
G4double G4CoupledTransportation::AlongStepGetPhysicalInteractionLength(
  const G4Track& track,
  G4double,  //  previousStepSize
  G4double currentMinimumStep, G4double& proposedSafetyForStart, G4GPILSelection* selection)
{
  ...
  *selection = CandidateForSelection;
  ...
  G4cout << "  CoupledTransport::AlongStep GPIL:  "
         << "  1st-step:  any= " << fFirstStepInVolume << " ( geom= " << fGeometryLimitedStep
         << " ) "
  ...
  if (fFieldExertedForce)
  {
    ...
    G4ChargeState chargeState(particleCharge,  // Charge can change (dynamic)
                              magneticMoment, pParticleDef->GetPDGSpin());
    if (equationOfMotion)
    {
  ...
  G4FieldTrack aFieldTrack =
    G4FieldTrack(startPosition,
                 ...
                 track.GetMomentumDirection(), track.GetKineticEnergy(), restMass, particleCharge,
                 polarizationVec, pParticleDef->GetPDGMagneticMoment(),
  ...
  if (currentMinimumStep > 0)
  {
    ...
    //
    lengthAlongCurve = fPathFinder->ComputeStep(
      aFieldTrack, currentMinimumStep, G4TransportationManager::kMassNavigatorId, stepNo,
      newMassSafety, limitedStep, endTrackState, currentVolume);
  ...
  safetyProposal =
    startFullSafety;  // used to be startMassSafety
  ...
  {
    G4double endFullSafety = fPathFinder->ComputeSafety(fTransportEndPosition);
    ...
    G4double endMassSafety =
      fPathFinder->ObtainSafety(G4TransportationManager::kMassNavigatorId, centerPt);
    ...
    G4cout << "  Revised Safety at endpoint " << fTransportEndPosition
           << "   give safety values: Mass= " << endMassSafety << "  All= " << endFullSafety
           << G4endl;
  ...
  else
  {
    ...
    G4cout << "  Quick Safety estimate at endpoint " << fTransportEndPosition
           ...
           << "  using start-point value " << startFullSafety << "  and endpointDistance "
           << fEndPointDistance << G4endl;
    G4cout.precision(prec);
#endif
  }
...
void G4CoupledTransportation::ReportMove(G4ThreeVector OldVector, G4ThreeVector NewVector,
                                         const G4String& Quantity)
{
  ...
  G4cerr << "Endpoint has moved between value expected from TransportEndPosition "
         << " and value from Track in PostStepDoIt. " << G4endl << "Change of " << Quantity
         << " is " << moveVec.mag() / mm << " mm long, "
         << " and its vector is " << (1.0 / mm) * moveVec << " mm " << G4endl
         << "Endpoint of ComputeStep was " << OldVector << " and current position to locate is "
         << NewVector << G4endl;
}

/////////////////////////////////////////////////////////////////////////////

G4VParticleChange* G4CoupledTransportation::PostStepDoIt(const G4Track& track, const G4Step&)
{
...
#ifdef G4DEBUG_TRANSPORT
  if ((verboseLevel > 0) && ((fTransportEndPosition - track.GetPosition()).mag2() >= 1.0e-16))
  {
  ...
  {
    fPathFinder->Locate(track.GetPosition(), track.GetMomentumDirection(), true);
  ...
  else  // fGeometryLimitedStep  is false
  {
    ...
    if (verboseLevel > 1)
    {
      G4cout << "G4CoupledTransportation::PostStepDoIt -- "
             << " fGeometryLimitedStep  = " << fGeometryLimitedStep << " must be false " << G4endl;
    }
#endif
...
void G4CoupledTransportation::ReportInexactEnergy(G4double startEnergy, G4double endEnergy)
{
  ...
  if (std::fabs(startEnergy - endEnergy) > perThousand * endEnergy)
  {
    ...
    if ((no_large_ediff % warnModulo) == 0)
    {
      ...
      G4Exception("G4CoupledTransportation::AlongStepGetPIL()", "EnergyChange", JustWarning,
                  message);
