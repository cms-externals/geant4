...
void G4Transportation::PrintStatistics(std::ostream& outStr) const
{
  ...
  {
    outStr << "   Sum of energy of looping tracks killed: " << fSumEnergyKilled / CLHEP::MeV
           << " MeV "
           << " from " << fNumLoopersKilled << "  tracks " << G4endl
           << "  Sum of energy of non-electrons        : "
           << fSumEnergyKilled_NonElectron / CLHEP::MeV << " MeV "
           << "  from " << fNumLoopersKilled_NonElectron << " tracks " << G4endl;
    ...
    {
      outStr << "   Max energy of non-electron looper killed: " << fMaxEnergyKilled_NonElectron
    ...
    if (fMaxEnergySaved > 0.0)
    {
      ...
      outStr << "   Sum of energy of loopers 'saved': " << fSumEnergySaved << G4endl;
      outStr << "   Sum of energy of unstable loopers 'saved': " << fSumEnergyUnstableSaved
             << G4endl;
    }
  }
  else
                                                                 ...
                                                                 G4GPILSelection* selection)
{
  ...
  *selection = CandidateForSelection;
  ...
  //
  fFieldExertedForce = fNewTrack ? fFieldExertedForceAtTrackStart : ConfigureFieldForTrack(track);
  ...
  else  //  A field exerts force
  {
    ...
    equationOfMotion->SetChargeMomentumMass(
      G4ChargeState(particleCharge, magneticMoment, particlePDGSpin), pParticle->GetTotalMomentum(),
      particleMass);

    G4FieldTrack aFieldTrack(startPosition,
                             track.GetGlobalTime(),  // Lab.
                             startMomentumDir, kineticEnergy, particleMass, particleCharge,
                             particleSpin, particlePDGMagM,
    ...
    const G4double lengthAlongCurve =
      fFieldPropagator->ComputeStep(aFieldTrack, currentMinimumStep, currentSafety,
                                    track.GetVolume(), kineticEnergy < fThreshold_Important_Energy);

    if (lengthAlongCurve < geometryStepLength) geometryStepLength = lengthAlongCurve;
    ...
    G4bool changesEnergy = fFieldPropagator->GetCurrentFieldManager()->DoesFieldChangeEnergy();
    ...
    fTransportEndKineticEnergy = changesEnergy ? aFieldTrack.GetKineticEnergy() : kineticEnergy;
      ...
      if (verboseLevel > 1)
      {
        if (std::fabs(startEnergy - endEnergy) > perThousand * endEnergy)
        {
          ...
          if ((no_large_ediff % warnModulo) == 0)
          {
            ...
            message << "Energy change in Step is above 1^-3 relative value. " << G4endl
                    << "     Relative change in 'tracking' step = " << std::setw(15)
                    << (endEnergy - startEnergy) / startEnergy << G4endl
                    << "     Starting E= " << std::setw(12) << startEnergy / MeV << " MeV "
                    << G4endl << "     Ending   E= " << std::setw(12) << endEnergy / MeV << " MeV "
                    << G4endl << "Energy has been corrected -- however, review"
            ...
            message << "Bad 'endpoint'. Energy change detected and corrected." << G4endl
                    << "Has occurred already " << no_large_ediff << " times.";
            G4Exception("G4Transportation::AlongStepGetPIL()", "EnergyChange", JustWarning,
                        message);
...
#  define noCallsASDI 0
  ...
  if (fParticleIsLooping)
  {
    ...
    G4bool candidateForEnd =
      (endEnergy < fThreshold_Important_Energy) || (fNoLooperTrials >= fThresholdTrials);
    G4bool unstableAndKillable = !stable && (fAbandonUnstableTrials != 0);
    G4bool unstableForEnd =
      (endEnergy < fThreshold_Important_Energy) && (fNoLooperTrials >= fAbandonUnstableTrials);
    if ((candidateForEnd && stable) || (unstableAndKillable && unstableForEnd))
    {
      ...
      {
        fpLogger->ReportLoopingTrack(track, stepData, fNoLooperTrials, noCallsASDI, __func__);
      }
      fNoLooperTrials = 0;
    }
    else
    {
      ...
      if (fNoLooperTrials == 1)
      {
        ...
        if (!stable) fSumEnergyUnstableSaved += endEnergy;
      }
#ifdef G4VERBOSE
      if (verboseLevel > 2 && !fSilenceLooperWarnings)
      {
        G4cout << "   " << __func__ << " Particle is looping but is saved ..." << G4endl
...
G4VParticleChange* G4Transportation::PostStepDoIt(const G4Track& track, const G4Step&)
{
  ...
  if (fGeometryLimitedStep)
  {
    ...
    if (!fFieldExertedForce)
      isLastStep =
        fLinearNavigator->ExitedMotherVolume() || fLinearNavigator->EnteredDaughterVolume();
    else
