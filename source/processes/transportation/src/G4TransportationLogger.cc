...
G4TransportationLogger::G4TransportationLogger(const G4String& className, G4int verbosity)
  : fClassName(className),
    fVerbose(verbosity),
    fThldWarningEnergy(0.),
    fThldImportantEnergy(0.),
    fThldTrials(0)
{}

G4TransportationLogger::G4TransportationLogger(const char* className, G4int verbosity)
  : fClassName(className),
    fVerbose(verbosity),
    fThldWarningEnergy(0.),
    fThldImportantEnergy(0.),
...
// SetThresholds
// ********************************************************************
void G4TransportationLogger::SetThresholds(G4double newEnWarn, G4double importantEnergy,
                                           G4int newMaxTrials)
{
...
void G4TransportationLogger::ReportLooperThresholds(const char* className)
{
  G4cout << className << ":  Current values for thresholds related to "
         ...
         << " 'important' tracks, i.e. those above 'important' energy " << G4endl;
}
...
// ********************************************************************
//
void G4TransportationLogger::ReportLoopingTrack(const G4Track& track, const G4Step& stepData,
                                                G4int numTrials, G4long noCalls,
                                                const char* methodName) const
{
  ...
  msg << " Transportation is killing track that is looping or stuck. " << G4endl << "   Track is "
      << track.GetParticleDefinition()->GetParticleName() << " and has "
      << track.GetKineticEnergy() / MeV << " MeV energy  ( pre-Step = " << preStepEn << " ) "
      << G4endl;
  msg << "   momentum = " << track.GetMomentum() << " mag= " << track.GetMomentum().mag() << G4endl
  ...
  if (material)
  {
    ...
    msg << " with density = " << material->GetDensity() / gramPerCm3 << " g/cm^3 ";
  }
  else
  {
  ...
  msg << G4endl;
  msg << " Total number of Steps by this track: " << track.GetCurrentStepNumber() << G4endl
      << " Length of this step = " << stepData.GetStepLength() / mm << " mm " << G4endl
      << " Number of propagation trials = " << numTrials
      << " ( vs maximum = " << GetThresholdTrials() << " for 'important' particles ) " << G4endl;
  ...
  if (numAdviceExcessSteps++ < numPrints)
  {
    ...
    msg << " This warning is controlled by the SetThresholdWarningEnergy "
        ...
        << " Current value of 'warning' threshold= " << GetThresholdWarningEnergy() / MeV << " MeV "
        << G4endl;
    ...
    msg << " - If particles of high-enough energy to be important are being "
        ...
        << "  The default value was 10, and the current value is " << GetThresholdTrials() << G4endl
        ...
        << G4endl << "      Note: this can incur a potentially high cost in extra simulation time "
        ...
        << " or else higher order RK methods such as DormandPrince78 " << G4endl;
    msg << " This information is provided " << numPrints
        << " times. Current count: " << numAdviceExcessSteps << " / " << numPrints << G4endl;
    msg << " =============================================================" << G4endl;
  }
