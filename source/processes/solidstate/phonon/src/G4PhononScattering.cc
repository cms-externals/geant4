...
G4PhononScattering::G4PhononScattering(const G4String& aName) : G4VPhononProcess(aName)
{
  ;
}
...
G4double G4PhononScattering::GetMeanFreePath(const G4Track& aTrack, G4double /*previousStepSize*/,
                                             G4ForceCondition* condition)
{
  ...
  if (verboseLevel > 1) G4cout << "G4PhononScattering::GetMeanFreePath = " << mfp << G4endl;
...
G4VParticleChange* G4PhononScattering::PostStepDoIt(const G4Track& aTrack, const G4Step& aStep)
{
  ...
  G4int polarization =
    ChoosePolarization(theLattice->GetLDOS(), theLattice->GetSTDOS(), theLattice->GetFTDOS());

  // Generate the new track after scattering
  // FIXME:  If polarization state is the same, just step the track!
  G4Track* sec = CreateSecondary(polarization, newDir, aTrack.GetKineticEnergy());
