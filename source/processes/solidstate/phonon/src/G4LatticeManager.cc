...
{
  for (LatticeLogReg::iterator lm = fLLattices.begin(); lm != fLLattices.end(); ++lm)
  {
    delete (*lm);
  }

  for (LatticePhyReg::iterator pm = fPLattices.begin(); pm != fPLattices.end(); ++pm)
  {
    delete (*pm);
  }
...
G4bool G4LatticeManager::RegisterLattice(G4Material* Mat, G4LatticeLogical* Lat)
{
...
G4LatticeLogical* G4LatticeManager::LoadLattice(G4Material* Mat, const G4String& latDir)
{
  if (verboseLevel)
  {
    G4cout << "G4LatticeManager::LoadLattice material " << Mat->GetName() << " " << latDir
           << G4endl;
  }
  ...
  {
    G4cerr << "ERROR creating " << latDir << " lattice for material " << Mat->GetName() << G4endl;
  }
...
G4LatticePhysical* G4LatticeManager::LoadLattice(G4VPhysicalVolume* Vol, const G4String& latDir)
{
  if (verboseLevel)
  {
    G4cout << "G4LatticeManager::LoadLattice volume " << Vol->GetName() << " " << latDir << G4endl;
  }
  ...
  G4LatticePhysical* pLattice = new G4LatticePhysical(lLattice, Vol->GetFrameRotation());
...
G4bool G4LatticeManager::RegisterLattice(G4VPhysicalVolume* Vol, G4LatticePhysical* Lat)
{
...
G4bool G4LatticeManager::RegisterLattice(G4VPhysicalVolume* Vol, G4LatticeLogical* LLat)
{
...
G4double G4LatticeManager::MapKtoV(G4VPhysicalVolume* Vol, G4int polarizationState,
                                   const G4ThreeVector& k) const
{
  G4LatticePhysical* theLattice = GetLattice(Vol);
  if (verboseLevel) G4cout << "G4LatticeManager::MapKtoV using lattice " << theLattice << G4endl;
...
G4ThreeVector G4LatticeManager::MapKtoVDir(G4VPhysicalVolume* Vol, G4int polarizationState,
                                           const G4ThreeVector& k) const
{
  G4LatticePhysical* theLattice = GetLattice(Vol);
  if (verboseLevel) G4cout << "G4LatticeManager::MapKtoVDir using lattice " << theLattice << G4endl;
