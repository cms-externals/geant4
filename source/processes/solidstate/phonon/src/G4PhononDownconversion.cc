                                                 ...
                                                 G4ForceCondition* condition)
{
  ...
  if (verboseLevel > 1) G4cout << "G4PhononDownconversion::GetMeanFreePath = " << mfp << G4endl;
...
G4VParticleChange* G4PhononDownconversion::PostStepDoIt(const G4Track& aTrack, const G4Step&)
{
  ...
  if (G4UniformRand() > 0.740)
    MakeLTSecondaries(aTrack);
  else
    MakeTTSecondaries(aTrack);
...
{
  // d=delta= ratio of group velocities vl/vt and x is the fraction of energy in the longitudinal
  // mode, i.e. x=EL'/EL
  return (1 / (x * x)) * (1 - x * x) * (1 - x * x)
         * ((1 + x) * (1 + x) - d * d * ((1 - x) * (1 - x)))
         * (1 + x * x - d * d * (1 - x) * (1 - x)) * (1 + x * x - d * d * (1 - x) * (1 - x));
}
...
G4double G4PhononDownconversion::GetTTDecayProb(G4double d, G4double x) const
{
  ...
  return (A + B * d * x - B * x * x) * (A + B * d * x - B * x * x)
         + (C * x * (d - x) - D / (d - x) * (x - d - (1 - d * d) / (4 * x)))
             * (C * x * (d - x) - D / (d - x) * (x - d - (1 - d * d) / (4 * x)));
}
...
void G4PhononDownconversion::MakeTTSecondaries(const G4Track& aTrack)
{
  ...
  G4int polarization1 = ChoosePolarization(0., theLattice->GetSTDOS(), theLattice->GetFTDOS());

  // Make FT or ST phonon (0. means no longitudinal)
  G4int polarization2 = ChoosePolarization(0., theLattice->GetSTDOS(), theLattice->GetFTDOS());
