...
G4ChannelingECHARM::G4ChannelingECHARM(const G4String& fileName, G4double vConversion)
  : fVectorEC(0), fDistances{0., 0., 0.}, fPoints{0, 0, 0}, fMaximum(-DBL_MAX), fMinimum(DBL_MAX)
{
  fDistances[0] = 0;
  fDistances[1] = 0;
  fDistances[2] = 0;
...
void G4ChannelingECHARM::ReadFromECHARM(const G4String& filename, G4double vConversion)
{
  ...
  if (fPoints[1] < 1)
  {
    ...
    G4Exception("G4ChannelingECHARM::ReadFromECHARM(...)", "G4ChannelingECHARM", FatalException,
                ed);
    return;
  }
  else if (fPoints[1] == 1)
  ...
  G4cout << "G4ChannelingECHARM::ReadFromECHARM() - " << vConversion << " " << fPoints[0] << " "
         << fDistances[0] << " " << fPoints[1] << " " << fDistances[1] << " " << fMinimum << " "
         << fMaximum << G4endl;
