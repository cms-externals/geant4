...
void G4Channeling::GetEF(const G4Track& aTrack, G4ThreeVector& pos, G4ThreeVector& out)
{
  out = G4ThreeVector((GetMatData(aTrack)->GetEFX()->GetEC(pos)),
                      (GetMatData(aTrack)->GetEFY()->GetEC(pos)), 0.);
}
...
G4bool G4Channeling::UpdateParameters(const G4Track& aTrack)
{
  ...
  if (integrationLimit > 0.)
  {
    ...
    do
    {
      ...
      if (isBent)
        momk1.setX(momk1.x() - step * mom.z() * beta / (GetMatData(aTrack)->GetBR(pos)).x());

      GetEF(aTrack, pos_temp = pos + posk1 * 0.5, efxy);
      posk2 = step / mom.z() * (mom + momk1 * 0.5);
      momk2 = step / beta * Z * efxy;
      if (isBent)
        momk2.setX(momk2.x() - step * mom.z() * beta / (GetMatData(aTrack)->GetBR(pos_temp)).x());

      GetEF(aTrack, pos_temp = pos + posk2 * 0.5, efxy);
      posk3 = step / mom.z() * (mom + momk2 * 0.5);
      momk3 = step / beta * Z * efxy;
      if (isBent)
        momk3.setX(momk3.x() - step * mom.z() * beta / (GetMatData(aTrack)->GetBR(pos_temp)).x());

      GetEF(aTrack, pos_temp = pos + posk3, efxy);
      posk4 = step / mom.z() * (mom + momk3);
      momk4 = step / beta * Z * efxy;
      if (isBent)
        momk4.setX(momk4.x() - step * mom.z() * beta / (GetMatData(aTrack)->GetBR(pos_temp)).x());
      ...
      momk1 = mom;
      if(isBent) momk1.setX(momk1.x() - step * 0.5 * mom.z() * beta /
      (GetMatData(aTrack)->GetBR(pos)).x());
...
G4bool G4Channeling::UpdateIntegrationStep(const G4Track& aTrack, G4ThreeVector& mom,
                                           G4double& step)
{
  if (mom.x() != 0.0 || mom.y() != 0.0)
  {
    ...
    if (xy2 != 0.)
    {
      step = std::fabs(fTransverseVariationMax * GetPre(aTrack)->GetKineticEnergy()
                       / std::pow(xy2, 0.5));
...
G4VParticleChange* G4Channeling::PostStepDoIt(const G4Track& aTrack, const G4Step&)
{
      ...
      && G4LogicalCrystalVolume::IsLattice(aNLV) == true)
  {
    ...
    if (bModifiedTraj == true)
    {
      ...
      //----------------------------------------
      G4LogicalCrystalVolume* aLCV =
        (G4LogicalCrystalVolume*)(aTrack.GetVolume()->GetLogicalVolume());
