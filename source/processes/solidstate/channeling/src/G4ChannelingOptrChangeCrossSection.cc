    ...
    fSetup(true)
{
  ...
  if (fParticleToBias == 0)
  {
    ...
    G4Exception("G4ChannelingOptrChangeCrossSection(...)", "G4Channeling", JustWarning, ed);
  }
      ...
      for (size_t i = 0; i < (sharedData->GetPhysicsBiasingProcessInterfaces()).size(); i++)
      {
        ...
        switch (type)
        {
          ...
          case fElectromagnetic:
            if (subType == fCoulombScattering || subType == fMultipleScattering)
            {
              fProcessToDensity[processName] = fDensityRatioNuD;
            }
            if (subType == fIonisation || subType == fPairProdByCharged || subType == fAnnihilation
                || subType == fAnnihilationToMuMu || subType == fAnnihilationToHadrons)
            {
              fProcessToDensity[processName] = fDensityRatioElD;
            }
            if (subType == fBremsstrahlung || subType == fNuclearStopping)
            {
              fProcessToDensity[processName] = fDensityRatioNuDElD;
            }

            if (subType == fCerenkov || subType == fScintillation
                || subType == fSynchrotronRadiation || subType == fTransitionRadiation)
            {
              fProcessToDensity[processName] = fDensityRatioNone;
            }
            if (subType == fRayleigh || subType == fPhotoElectricEffect
                || subType == fComptonScattering || subType == fGammaConversion
...
G4VBiasingOperation* G4ChannelingOptrChangeCrossSection::ProposeOccurenceBiasingOperation(
...
void G4ChannelingOptrChangeCrossSection::OperationApplied(
