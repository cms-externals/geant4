...
G4VParticleChange* G4OpMieHG::PostStepDoIt(const G4Track& aTrack, const G4Step& aStep)
{
  ...
  if (verboseLevel > 1)
  {
    G4cout << "OpMie Scattering Photon!" << G4endl
           ...
           << " MIE Old Polarization: " << aParticle->GetPolarization() << G4endl;
  }

  G4bool isForward = (G4UniformRand() <= forwardRatio);
  G4double gg = MPT->GetConstProperty(isForward ? kMIEHG_FORWARD : kMIEHG_BACKWARD);
  ...
  if (!isForward) costh *= -1.0;  // backward scattering
