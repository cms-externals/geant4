...
G4ReactionProduct::G4ReactionProduct(const G4ParticleDefinition* p) : G4ReactionProduct()
{
  if (nullptr != p)
  {
    ...
    mass = p->GetPDGMass();
    totalEnergy = mass;
    (p->GetPDGEncoding() < 0) ? timeOfFlight = -1.0 : timeOfFlight = 1.0;
  }
}
...
void G4ReactionProduct::SetDefinition(const G4ParticleDefinition* p)
{
  if (nullptr != p)
  {
    theParticleDefinition = p;
    mass = p->GetPDGMass();
    totalEnergy = mass + kineticEnergy;
    (p->GetPDGEncoding() < 0) ? timeOfFlight = -1.0 : timeOfFlight = 1.0;
  }
}

void G4ReactionProduct::SetMomentum(const G4double x, const G4double y, const G4double z)
{
  momentum.set(x, y, z);
}

void G4ReactionProduct::SetMomentum(const G4double x, const G4double y)
{
  momentum.setX(x);
  momentum.setY(y);
}
...
void G4ReactionProduct::Lorentz(const G4ReactionProduct& p1, const G4ReactionProduct& p2)
{
  ...
  G4double p2z = p2M.z();
  G4double a =
    ((p1x * p2x + p1y * p2y + p1z * p2z) / (p2.totalEnergy + p2.mass) - p1.totalEnergy) / p2.mass;
