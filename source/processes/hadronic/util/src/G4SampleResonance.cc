...
G4double G4SampleResonance::GetMinimumMass(const G4ParticleDefinition* p) const
{
  ...
  G4SampleResonance::minMassMapType& minMassCache = *minMassCache_G4MT_TLS_;
  ;
  ;
  ;
        ...
        if (decayBr > std::min(highestChannelProbability, thresholdChannelProbability))
        {
          ...
          for (G4int j = 0; j < nDaughters; j++)
          {
            const G4ParticleDefinition* aDaughter =
              const_cast<G4VDecayChannel*>(aDecay)->GetDaughter(j);
  ...
  // G4cout << "minimal mass for " << p->GetParticleName() << " is " << minResonanceMass/MeV <<
  // G4endl;
...
G4double G4SampleResonance::SampleMass(const G4double poleMass, const G4double gamma,
                                       const G4double minMass, const G4double maxMass) const
{
  if (!minMassCache_G4MT_TLS_) minMassCache_G4MT_TLS_ = new G4SampleResonance::minMassMapType;
