                                              ...
                                              std::vector<G4LorentzVector>& finalState)
{
  ...
  nTrials = 0;
  do
  {  // Apply accept/reject to get distribution
    ++nTrials;
    FillRandomBuffer();
...
void G4HadPhaseSpaceGenbod::Initialize(G4double initialMass, const std::vector<G4double>& masses)
{
  ...
  if (GetVerboseLevel() > 2)
  {
    ...
    G4cout << " totalMass " << totalMass << " massExcess " << massExcess << G4endl;
  }
...
{
  if (GetVerboseLevel() > 1) G4cout << GetName() << "::ComputeWeightScale" << G4endl;
...
G4double G4HadPhaseSpaceGenbod::ComputeWeight() const
{
  ...
  return (std::accumulate(pd.begin(), pd.end(), 1. / weightMax, std::multiplies<G4double>()));
}

G4bool G4HadPhaseSpaceGenbod::AcceptEvent() const
{
  if (GetVerboseLevel() > 1) G4cout << GetName() << "::AcceptEvent? " << nTrials << G4endl;
                                            ...
                                            std::vector<G4LorentzVector>& finalState)
{
  ...
  for (size_t i = 0; i < nFinal; i++)
  {
    ...
    if (GetVerboseLevel() > 2) G4cout << " finalState[" << i << "] " << finalState[i] << G4endl;
  }
}

// Process final state daughters up to current index

void G4HadPhaseSpaceGenbod::AccumulateFinalState(size_t i, const std::vector<G4double>& masses,
                                                 std::vector<G4LorentzVector>& finalState)
{
  if (GetVerboseLevel() > 2) G4cout << GetName() << "::AccumulateFinalState " << i << G4endl;

  if (i == 0)
  {  // First final state particle left alone
  ...
  if (GetVerboseLevel() > 2)
  {
    G4cout << " initialized Py " << -pd[i - 1] << " phi " << phi << " theta " << theta << G4endl;
  }

  G4double esys = 0., beta = 0., gamma = 1.;
  if (i < nFinal - 1)
  {  // Do not boost final particle
    esys = std::sqrt(pd[i] * pd[i] + meff[i] * meff[i]);
    beta = pd[i] / esys;
    gamma = esys / meff[i];

    if (GetVerboseLevel() > 2)
      G4cout << " esys " << esys << " beta " << beta << " gamma " << gamma << G4endl;
  }
