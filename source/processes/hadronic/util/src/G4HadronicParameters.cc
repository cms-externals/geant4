...
G4HadronicParameters::G4HadronicParameters()
{
  ...
  if (nullptr != ep1)
  {
    fRelativeDiff = std::strtod(ep1, 0);
  }
  const char* ep2 = std::getenv("G4Hadronic_epCheckAbsoluteLevel");
  if (nullptr != ep2)
  {
    fAbsoluteDiff = std::strtod(ep2, 0);
  }
  ...
  if (nullptr != x)
  {
    fPhysListDocDir = G4String(x);
  }
  const char* y = std::getenv("G4PhysListName");
  if (nullptr != y)
  {
    fPhysListName = G4String(y);
  }
...
void G4HadronicParameters::StreamInfo(std::ostream& os) const
{
  ...
  os << "=======================================================================" << "\n";
  os << "Enable integral method for inelastic cross sections "
     << boolToString(fEnableIntegralInelasticXS) << "\n";
  os << "Enable integral method for elastic cross sections   "
     << boolToString(fEnableIntegralElasticXS) << "\n";
  os << "Enable diffraction dissociation for B > 10          "
     << boolToString(fEnableDiffDissociationForBGreater10) << "\n";
  ...
  // A ternary operation can't be used here as it leads to a C2445 error on Windows with C++20 and
  // newer
  ...
  os << "Bertini angular emissions as in G4 11.2             "
     << boolToString(fBertiniAngularEmissionsAs11_2) << "\n";
  os << "Bertini nuclei model as in G4 11.2                  "
     << boolToString(fBertiniNucleiModelAs11_2) << "\n";
