...
G4Nucleus::G4Nucleus() : theA(0), theZ(0), theL(0), aEff(0.0), zEff(0)
{
...
//-------------------------------------------------------------------------------------------------
G4ReactionProduct G4Nucleus::GetBiasedThermalNucleus(G4double aMass, G4ThreeVector aVelocity,
                                                     G4double temp) const
{
  // If E_neutron <= E_threshold, Then apply the Sampling ot the Velocity of the Target (SVT)
  // method; Else consider the target nucleus being without motion.
  ...
  {  // target nucleus considered as being without motion
...
void G4Nucleus::DoKinematicsOfThermalNucleus(const G4double mu, const G4double vT_norm,
                                             const G4ThreeVector& aVelocity,
                                             G4ReactionProduct& result) const
{
  // Get target nucleus direction from the neutron direction and the relative angle between target
  // nucleus and neutron (mu)
  G4double cosTh = mu;
  ...
  if (uNorm[0])
    ortho[0] = -(uNorm[1] + uNorm[2]) / uNorm[0];
  else if (uNorm[1])
    ortho[1] = -(uNorm[0] + uNorm[2]) / uNorm[1];
  else if (uNorm[2])
    ortho[2] = -(uNorm[0] + uNorm[1]) / uNorm[2];

  // Normalize the vector
  ortho = (1 / ortho.mag()) * ortho;

  // Find vector to draw a plan perpendicular to uNorm (i.e neutron velocity) with vectors ortho &
  // orthoComp
  ...
  G4ThreeVector directionTarget(
    cosTh * uNorm[0] + sinTh * (cosPhi * orthoComp[0] + sinPhi * ortho[0]),
  ...
  G4double tEtot =
    std::sqrt((tMom + result.GetMass()) * (tMom + result.GetMass()) - 2. * tMom * result.GetMass());
...
G4ReactionProduct G4Nucleus::GetThermalNucleus(G4double targetMass, G4double temp) const
{
  ...
  G4double tMom = std::sqrt(px * px + py * py + pz * pz);
  G4double tEtot = std::sqrt((tMom + theTarget.GetMass()) * (tMom + theTarget.GetMass())
...
void G4Nucleus::SetParameters(const G4double A, const G4double Z, const G4int numberOfLambdas)
{
  theZ = G4lrint(Z);
  theA = G4lrint(A);
...
void G4Nucleus::SetParameters(const G4int A, const G4int Z, const G4int numberOfLambdas)
{
  theZ = Z;
  theA = A;
  theL = std::max(numberOfLambdas, 0);
...
G4double G4Nucleus::AtomicMass(const G4double A, const G4double Z,
                               const G4int numberOfLambdas) const
{
...
G4double G4Nucleus::AtomicMass(const G4int A, const G4int Z, const G4int numberOfLambdas) const
{
  // Now returns (atomic mass - electron masses)
  if (numberOfLambdas > 0)
  {
...
G4double G4Nucleus::GetThermalPz(const G4double mass, const G4double temp) const
{
...
G4double G4Nucleus::EvaporationEffects(G4double kineticEnergy)
{
  ...
  G4float exnu = 7.716 * cfa * G4Exp(-cfa) * ((atno - 1.0) / 120.) * G4Exp(-(atno - 1.0) / 120.);
...
G4ThreeVector G4Nucleus::GetFermiMomentum()
{
  ...
  G4double ranflat1 = G4RandFlat::shoot((G4double)0., (G4double)fermiMomentum);
  G4double ranflat2 = G4RandFlat::shoot((G4double)0., (G4double)fermiMomentum);
  G4double ranflat3 = G4RandFlat::shoot((G4double)0., (G4double)fermiMomentum);
