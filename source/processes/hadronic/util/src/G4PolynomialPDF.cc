...
G4PolynomialPDF::G4PolynomialPDF(size_t n, const G4double* coeffs, G4double x1, G4double x2)
  : fX1(x1), fX2(x2), fChanged(true), fTolerance(1.e-8), fVerbose(0)
{
  if (coeffs != nullptr)
    SetCoefficients(n, coeffs);
  else if (n > 0)
    SetNCoefficients(n);
}
...
{
  while (i >= fCoefficients.size())
    fCoefficients.push_back(0);
...
void G4PolynomialPDF::SetCoefficients(size_t nCoeffs, const G4double* coefficients)
{
...
void G4PolynomialPDF::Normalize()
{
  ...
  while (fCoefficients.size())
  { /* Loop checking, 30-Oct-2015, G.Folger */
    if (fCoefficients[fCoefficients.size() - 1] == 0.0)
      fCoefficients.pop_back();
    else
      break;
  }
  ...
  if (sum <= 0)
  {
    if (fVerbose > 0)
    {
      G4cout << "G4PolynomialPDF::Normalize() WARNING: PDF has non-positive area: " << sum
             << G4endl;
      Dump();
    }
    return;
...
G4double G4PolynomialPDF::Evaluate(G4double x, G4int ddxPower)
{
  ...
  if (ddxPower < -1 || ddxPower > 2)
  {
    if (fVerbose > 0)
    {
      G4cout << "G4PolynomialPDF::GetX() WARNING: ddxPower " << ddxPower << " not implemented"
             << G4endl;
    }
    return 0.0;
  }
  ...
  for (size_t i = 0; i <= GetNCoefficients(); ++i)
  {
    ...
    else if (ddxPower == 0 && i < GetNCoefficients())
      f += GetCoefficient(i) * xN;  // PDF
    else if (ddxPower == 1)
    {  // (d/dx) PDF
...
G4bool G4PolynomialPDF::HasNegativeMinimum(G4double x1, G4double x2)
{
  ...
  if (x1 < fX1 || x2 > fX2 || x2 < x1)
  {
    if (fVerbose > 0)
    {
      G4cout << "G4PolynomialPDF::HasNegativeMinimum() WARNING: Invalid range " << x1 << " - " << x2
             << G4endl;
    }
    return false;
  }
  ...
  if (GetNCoefficients() == 2 || (GetNCoefficients() == 3 && GetCoefficient(2) <= 0))
  {
  ...
  if (Evaluate(extremum) < -fTolerance)
    return true;
  else if (extremum <= x1 + (x2 - x1) * fTolerance || extremum >= x2 - (x2 - x1) * fTolerance)
    return false;
  else
    return HasNegativeMinimum(x1, extremum) || HasNegativeMinimum(extremum, x2);
}
...
G4double G4PolynomialPDF::GetX(G4double p, G4double x1, G4double x2, G4int ddxPower, G4double guess,
                               G4bool bisect)
{
  ...
  if ((ddxPower == 0 && GetNCoefficients() == 1) || (ddxPower == 1 && GetNCoefficients() == 2))
    return x2;

  // Solve p = mx + b -> x = (p-b)/m for linear functions
  if ((ddxPower == -1 && GetNCoefficients() == 1) || (ddxPower == 0 && GetNCoefficients() == 2)
      || (ddxPower == 1 && GetNCoefficients() == 3))
  {
    ...
    if (slope == 0)
    {  // the highest-order coefficient should never be zero if simplified
      if (fVerbose > 0)
      {
  ...
  if ((ddxPower == -1 && GetNCoefficients() == 2) || (ddxPower == 0 && GetNCoefficients() == 3)
      || (ddxPower == 1 && GetNCoefficients() == 4))
  {
    ...
    if (a == 0)
    {  // the highest-order coefficient should never be 0 if simplified
      if (fVerbose > 0)
      {
    ...
    if (valueMinus >= x1 && valueMinus <= x2)
      return valueMinus;
    else if (valueMinus > x2)
      return x2;
    G4double valuePlus = -b / 2. / a + sqrtFactor;
    if (valuePlus >= x1 && valuePlus <= x2)
      return valuePlus;
    else if (valuePlus < x1)
      return x2;
  ...
  size_t iterations = 0;
  while (fabs(lastChange) > fTolerance)
  { /* Loop checking, 02.11.2015, A.Ribon */
    ...
    if (iterations > 50)
    {
      if (p != 0)
      {
        if (fVerbose > 0)
        {
          G4cout << "G4PolynomialPDF::GetX() WARNING: got stuck searching for " << p << " between "
                 << x1 << " and " << x2 << " with ddxPower = " << ddxPower << ". Last guess was "
                 << guess << "." << G4endl;
        }
      }
      if (ddxPower == -1 && bisect)
      {
        if (fVerbose > 0)
        {
          G4cout << "G4PolynomialPDF::GetX() WARNING: Biseting and trying again..." << G4endl;
        }
...
void G4PolynomialPDF::Dump()
{
  ...
  G4cout << G4endl;
  G4cout << "G4PolynomialPDF::Dump() - Interval: " << fX1 << " <= x < " << fX2 << G4endl;
}
