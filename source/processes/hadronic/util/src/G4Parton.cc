...
G4Parton::G4Parton(G4int PDGcode)
{
  ...
  //
  if ((theDefinition->GetParticleType() == "quarks")
      || (theDefinition->GetParticleType() == "diquarks"))
  {
...
void G4Parton::DefineMomentumInZ(G4double aLightConeMomentum, G4double aLightConeE,
                                 G4bool aDirection)
{
  G4double Mass = GetMass();
  G4LorentzVector a4Momentum = Get4Momentum();
  ...
  aLightConeE *= theX;
  G4double TransverseMass2 = sqr(a4Momentum.px()) + sqr(a4Momentum.py()) + sqr(Mass);
  a4Momentum.setPz(0.5 * (aLightConeMomentum - aLightConeE - TransverseMass2 / aLightConeMomentum)
                   * (aDirection ? 1 : -1));
