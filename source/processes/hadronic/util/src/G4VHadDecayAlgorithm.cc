...
void G4VHadDecayAlgorithm::Generate(G4double initialMass, const std::vector<G4double>& masses,
...
{
  G4bool okay = (initialMass > 0. && masses.size() >= 2
                 && initialMass >= std::accumulate(masses.begin(), masses.end(), 0.));

  if (verboseLevel)
  {
    G4cout << GetName() << "::IsDecayAllowed? initialMass " << initialMass << " " << masses.size()
           << " masses sum " << std::accumulate(masses.begin(), masses.end(), 0.) << G4endl;
...
G4double G4VHadDecayAlgorithm::TwoBodyMomentum(G4double M0, G4double M1, G4double M2) const
{
  ...
  if (PSQ < 0.)
  {
    G4cout << GetName() << ":  problem of decay of M(GeV) " << M0 / GeV << " to M1(GeV) "
           << M1 / GeV << " and M2(GeV) " << M2 / GeV << " PSQ(MeV) " << PSQ / MeV << " < 0"
           << G4endl;
...
void G4VHadDecayAlgorithm::PrintVector(const std::vector<G4double>& v, const G4String& vname,
