...
G4double G4Clebsch::ClebschGordanCoeff(G4int twoJ1, G4int twoM1, G4int twoJ2, G4int twoM2,
                                       G4int twoJ)
{
  if (twoJ1 < 0 || twoJ2 < 0 || twoJ < 0 || ((twoJ1 - twoM1) % 2) || ((twoJ2 - twoM2) % 2))
  {
    return 0;
  }

  G4int twoM = twoM1 + twoM2;
  if (twoM1 > twoJ1 || twoM1 < -twoJ1 || twoM2 > twoJ2 || twoM2 < -twoJ2 || twoM > twoJ
      || twoM < -twoJ)
  {
    return 0;
  }
  ...
  G4double factor =
    g4pow->logfactorial((twoJ1 + twoM1) / 2) + g4pow->logfactorial((twoJ1 - twoM1) / 2);
  factor += g4pow->logfactorial((twoJ2 + twoM2) / 2) + g4pow->logfactorial((twoJ2 - twoM2) / 2);
  factor += g4pow->logfactorial((twoJ + twoM) / 2) + g4pow->logfactorial((twoJ - twoM) / 2);
  factor *= 0.5;
  ...
  if (kMin < 0)
  {
    G4Exception("G4Clebsch::ClebschGordanCoeff()", "Clebsch001", JustWarning, "kMin < 0");
    return 0;
  }
  if (kMax < kMin)
  {
    G4Exception("G4Clebsch::ClebschGordanCoeff()", "Clebsch002", JustWarning, "kMax < kMin");
    return 0;
  }
  if (kMax >= G4POWLOGFACTMAX)
  {
    G4Exception("G4Clebsch::ClebschGordanCoeff()", "Clebsch003", JustWarning,
                "kMax too big for G4Pow");
    return 0;
  }
  ...
  for (G4int k = kMin; k <= kMax; k++)
  {
    ...
    kSum += sign
            * G4Exp(factor - g4pow->logfactorial(sum1 - k) - g4pow->logfactorial(sum2 + k)
                    - g4pow->logfactorial(sum3 - k) - g4pow->logfactorial(sum4 + k)
                    - g4pow->logfactorial(k) - g4pow->logfactorial(sum5 - k));
  }
...
std::vector<G4double> G4Clebsch::GenerateIso3(G4int twoJ1, G4int twoM1, G4int twoJ2, G4int twoM2,
                                              G4int twoJOut1, G4int twoJOut2)
{
  ...
  if (twoJ1 == 0 && twoJ2 == 0)
  {
    G4Exception("G4Clebsch::GenerateIso3()", "Clebsch010", JustWarning, "both twoJ are zero");
    temp.push_back(0.);
    temp.push_back(0.);
    return temp;
  }
  ...
  if (twoJMin > twoJMax)
  {
    G4Exception("G4Clebsch::GenerateIso3()", "Clebsch020", JustWarning, "twoJMin > twoJMax");
    return temp;
  }
  ...
  {
    G4Exception("G4Clebsch::GenerateIso3()", "Clebsch021", JustWarning,
  ...
  if (nJ == 0)
  {
    G4Exception("G4Clebsch::GenerateIso3()", "Clebsch022", JustWarning,
  ...
  {
    G4Exception("G4Clebsch::GenerateIso3()", "Clebsch023", JustWarning, "nJ inconsistency");
    return temp;
  }
  ...
  if (sum <= 0.)
  {
    G4Exception("G4Clebsch::GenerateIso3()", "Clebsch024", JustWarning,
  ...
  for (j12 = std::abs(twoJOut1 - twoJOut2); j12 <= (twoJOut1 + twoJOut2); j12 += 2)
  {
    ...
    for (m1pr = static_cast<G4int>(mMin[0] + .00001); m1pr <= mMax[0]; m1pr += 2)
    {
      ...
      if (m1pos >= size)
      {
        G4Exception("G4Clebsch::GenerateIso3()", "Clebsch025", JustWarning, "m1pos > size");
        return temp;
      }
      ...
      for (m2pr = static_cast<G4int>(mMin[1] + .00001); m2pr <= mMax[1]; m2pr += 2)
      {
        ...
        if (m2pos >= size)
        {
          G4Exception("G4Clebsch::GenerateIso3()", "Clebsch026", JustWarning, "m2pos > size");
          return temp;
        }
  ...
  if (sum <= 0.)
  {
    G4Exception("G4Clebsch::GenerateIso3()", "Clebsch027", JustWarning, "sum (out) <=0");
    return temp;
  }
  ...
  G4Exception("G4Clebsch::GenerateIso3()", "Clebsch028", JustWarning, "Should never get here");
  return temp;
}

G4double G4Clebsch::Weight(G4int twoJ1, G4int twoM1, G4int twoJ2, G4int twoM2, G4int twoJOut1,
                           G4int twoJOut2)
{
  G4double value = 0.;
...
G4double G4Clebsch::Wigner3J(G4double j1, G4double j2, G4double j3, G4double m1, G4double m2,
                             G4double m3)
{
...
G4double G4Clebsch::Wigner3J(G4int twoJ1, G4int twoM1, G4int twoJ2, G4int twoM2, G4int twoJ3)
{
...
G4double G4Clebsch::NormalizedClebschGordan(G4int twoJ, G4int twoM, G4int twoJ1, G4int twoJ2,
                                            G4int twoM1, G4int twoM2)
{
  ...
  for (G4int twoM1Current = -twoJ1; twoM1Current <= twoJ1; twoM1Current += 2)
  {
    ...
    G4double prob = ClebschGordan(twoJ1, twoM1Current, twoJ2, twoM2Current, twoJ);
    sum += prob;
...
G4double G4Clebsch::Wigner6J(G4int twoJ1, G4int twoJ2, G4int twoJ3, G4int twoJ4, G4int twoJ5,
                             G4int twoJ6)
{
  if (twoJ1 < 0 || twoJ2 < 0 || twoJ3 < 0 || twoJ4 < 0 || twoJ5 < 0 || twoJ6 < 0) return 0;
  ...
  if (i < 0 || i % 2)
    ...
    triangles += g4pow->logfactorial(i / 2);
  i = twoJ1 - twoJ2 + twoJ3;
  if (i < 0 || i % 2)
    return 0;
  else
  ...
  if (kMin < 0)
  {
    G4Exception("G4Clebsch::Wigner6J()", "Clebsch040", JustWarning, "kMin < 0");
    return 0;
  }
  if (kMax < kMin)
  {
    G4Exception("G4Clebsch::Wigner6J()", "Clebsch041", JustWarning, "kMax < kMin");
    return 0;
  }
  if (kMax >= G4POWLOGFACTMAX)
  {
    G4Exception("G4Clebsch::Wigner6J()", "Clebsch041", JustWarning, "kMax too big for G4Pow");
    return 0;
  }
  ...
  {
    kSum += sign
            * G4Exp(g4pow->logfactorial(k + 1) - g4pow->logfactorial(k - sum1)
                    - g4pow->logfactorial(k - sum2) - g4pow->logfactorial(k - sum3)
                    - g4pow->logfactorial(k - sum4) - g4pow->logfactorial(sum5 - k)
                    - g4pow->logfactorial(sum6 - k) - g4pow->logfactorial(sum7 - k));
    sign *= -1;
  }
  return triangles * kSum;
}

G4double G4Clebsch::Wigner9J(G4int twoJ1, G4int twoJ2, G4int twoJ3, G4int twoJ4, G4int twoJ5,
                             G4int twoJ6, G4int twoJ7, G4int twoJ8, G4int twoJ9)
{
  if (twoJ1 < 0 || twoJ2 < 0 || twoJ3 < 0 || twoJ4 < 0 || twoJ5 < 0 || twoJ6 < 0 || twoJ7 < 0
      || twoJ8 < 0 || twoJ9 < 0)
    return 0;
...
G4double G4Clebsch::WignerLittleD(G4int twoJ, G4int twoM, G4int twoN, G4double cosTheta)
{
  if (twoM < -twoJ || twoM > twoJ || twoN < -twoJ || twoN > twoJ || ((twoM % 2) != (twoJ % 2))
      || ((twoN % 2) != (twoJ % 2)))
  {
    return 0;
  }
  ...
  for (G4int k = kMin; k <= kMax; k++)
  {
    G4double logSum =
      0.5
      * (g4pow->logfactorial((twoJ + twoM) / 2) + g4pow->logfactorial((twoJ - twoM) / 2)
         + g4pow->logfactorial((twoJ + twoN) / 2) + g4pow->logfactorial((twoJ - twoN) / 2));
    logSum += -g4pow->logfactorial((twoJ + twoM) / 2 - k)
              - g4pow->logfactorial((twoJ - twoN) / 2 - k) - g4pow->logfactorial(k)
