...
G4KineticTrack::G4KineticTrack(const G4ParticleDefinition* aDefinition, G4double aFormationTime,
    ...
    if (nDaughters == 2 || nDaughters == 3)
    {
      ...
      if (nDaughters == 2)
      {
        if (!theDaughterIsShortLived[0] && !theDaughterIsShortLived[1])
        {
          ...
          theActualMom = EvaluateCMMomentum(theActualMass, theDaughterMass);
          thePoleMom = EvaluateCMMomentum(thePoleMass, theDaughterMass);
          //	      cout << G4endl;
          //	      cout << " LB: ActualMass/DaughterMass  " << theActualMass << "   " <<
          // theDaughterMass << G4endl; 	      cout << " LB: ActualMom " << theActualMom << G4endl;
          // cout << " LB: PoleMom   " << thePoleMom << G4endl; 	      cout << G4endl;
        }
        else if (!theDaughterIsShortLived[0] && theDaughterIsShortLived[1])
        {
          //              G4cout << G4endl << "Only the first of the " << nDaughters <<" decay
          //              products is stable!";
      ...
      else  // (nDaughter==3)
      {
        ...
        //	      else
        //		{
        //		  throw G4HadronicException(__FILE__, __LINE__,  ("can't handle more than one
        // shortlived in 3 particle output channel");
        //		}
      }
      ...
      theActualWidth[index] = thePoleWidth * theMassRatio * theMomRatio;
...
G4KineticTrackVector* G4KineticTrack::Decay()
{
  ...
  /*
      ...
      for (index1 = nChannels - 1; index1 >= 0; --index1)
        G4cout << "DECAY Actual Width IND/ActualW " << index1 << "  " << theActualWidth[index1] <<
     G4endl; G4cout << "DECAY Actual Mass " << theActualMass << G4endl;
  */
  ...
  if (theTotalActualWidth != 0)
  {
    ...
    do
    {
      ...
      switch (theNumberOfDaughters)
      {
        ...
        case 2:
          ...
          if (numberOfShortliveds == 1)
          {
            ...
            const G4ParticleDefinition* aDaughter =
              theDecayChannel->GetDaughter(shortlivedDaughters[0]);
          ...
          else if (numberOfShortliveds == 2)
          {
            ...
            G4double massmax =
              theParentMass
              - aSampler.GetMinimumMass(theDecayChannel->GetDaughter(shortlivedDaughters[one]));
            const G4ParticleDefinition* aDaughter =
              theDecayChannel->GetDaughter(shortlivedDaughters[zero]);
      ...
      // AR-16Aug2016 : Check whether the sum of the masses of the daughters is smaller than the
      // parent mass.
      //                If this is still not the case, but the max number of attempts has been
      //                reached, then the subsequent call thePhaseSpaceDecayChannel.DecayIt() will
      //                throw an exception.
      G4double sumDaughterMasses = 0.0;
      for (G4int i = 0; i < 4; ++i)
        sumDaughterMasses += masses[i];
      if (theParentMass - sumDaughterMasses > 0.0) isChannelBelowThreshold = false;

    } while (isChannelBelowThreshold
             && ++loopCounter < maxNumberOfLoops); /* Loop checking, 16.08.2016, A.Ribon */

    //
    //      Get the decay products List
    //

    G4GeneralPhaseSpaceDecay thePhaseSpaceDecayChannel(
      theParentName, theParentMass, theBR, theNumberOfDaughters, theDaughtersName1,
      theDaughtersName2, theDaughtersName3, theDaughtersName4, masses);
    ...
    if (theDecayProducts == nullptr)
    {
      ...
      ed << "Error condition encountered: phase-space decay failed." << G4endl
         ...
         << "\t " << theNumberOfDaughters << " daughter particles: " << theDaughtersName1 << " "
         << theDaughtersName2 << " " << theDaughtersName3 << " " << theDaughtersName4 << G4endl;
    ...
    for (G4int i = dEntries; i > 0; --i)
    {
      ...
      G4KineticTrack* aDaughter = new G4KineticTrack(aProduct, formationTime, position, momentum);
    ...
    if (std::getenv("DecayEnergyBalanceCheck"))
      std::cout << "DEBUGGING energy balance in cms and lab, charge baryon balance : "
                << momentumBalanceCMS << " " << energyMomentumBalance << " " << chargeBalance << " "
                << baryonBalance << " " << G4endl;
...
G4double G4KineticTrack::IntegrandFunction4(G4double xmass) const
{
  ...
  G4double result = BrWig(gamma1, mass1, xmass)
                    * integral.Simpson(this, &G4KineticTrack::IntegrandFunction3, theLowerLimit,
                                       theUpperLimit, nIterations);
...
G4double G4KineticTrack::IntegrateCMMomentum(const G4double theLowerLimit,
                                             const G4double poleMass) const
{
