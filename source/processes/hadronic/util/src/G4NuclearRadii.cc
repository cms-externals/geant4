...
G4double G4NuclearRadii::CoulombFactor(G4int Z, G4int A, const G4ParticleDefinition* theParticle,
                                       G4double ekin)
{
...
G4double G4NuclearRadii::NeutronInelasticShape(G4int Z, G4double ekin)
{
...
G4double G4NuclearRadii::ProtonInelasticShape(G4int Z, G4double ekin)
{
  G4double A = (Z < 100) ? aeff[Z] : aeff[100];
  G4double elog = G4Log(ekin / CLHEP::GeV) / llog10;
...
const G4double G4NuclearRadii::r0[] = {
  1.2,  1.3,  1.3,  1.3,  1.3, 1.17, 1.54, 1.65, 1.71, 1.7,  1.75,  // 1-10
