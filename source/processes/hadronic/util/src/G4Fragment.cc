...
G4Fragment::G4Fragment(G4int A, G4int Z, G4int numberOfLambdas, const G4LorentzVector& aMomentum)
  : theA(A),
    theZ(Z),
...
G4Fragment::G4Fragment(const G4LorentzVector& aMomentum,
  ...
  : theA(0),
    ...
    theCreationTime(0.0)
{
  if (aParticleDefinition->GetPDGEncoding() != 22 && aParticleDefinition->GetPDGEncoding() != 11)
  {
...
void G4Fragment::CalculateMassAndExcitationEnergy()
{
  ...
  // compute mass
  theGroundStateMass = (theL == 0) ? G4NucleiProperties::GetNuclearMass(theA, theZ)
...
void G4Fragment::SetExcEnergyAndMomentum(G4double eexc, const G4LorentzVector& v)
{
  ...
  if (v.vect().mag2() > elim2) theMomentum.boost(v.boostVector());
}

G4double G4Fragment::GetBindingEnergy() const
{
  const G4double lambdaMass = 1.115683 * CLHEP::GeV;
  return (theA - theZ - theL) * CLHEP::neutron_mass_c2 + theZ * CLHEP::proton_mass_c2
         + theL * lambdaMass - theGroundStateMass;
}
...
std::ostream& operator<<(std::ostream& out, const G4Fragment& theFragment)
{
  ...
  out << "Fragment: A = " << std::setw(3) << theFragment.theA << ", Z = " << std::setw(3)
      << theFragment.theZ << ", numberOfLambdas = " << std::setw(3) << theFragment.theL;
  out.setf(std::ios::scientific, std::ios::floatfield);

  // Store user's precision setting and reset to (3) here: back-compatibility
  std::streamsize floatPrec = out.precision();

  out << std::setprecision(3) << ", U = " << theFragment.GetExcitationEnergy() / CLHEP::MeV
      << " MeV  ";
  ...
  out << G4endl << "          P = (" << theFragment.GetMomentum().x() / CLHEP::MeV << ","
      << theFragment.GetMomentum().y() / CLHEP::MeV << ","
      << theFragment.GetMomentum().z() / CLHEP::MeV
      << ") MeV   E = " << theFragment.GetMomentum().t() / CLHEP::MeV << " MeV" << G4endl;
...
void G4Fragment::ExcitationEnergyWarning()
{
  ...
  G4cout << "G4Fragment::CalculateExcitationEnergy(): WARNING "
         << " GraundStateMass(MeV)= " << theGroundStateMass << G4endl;
  G4cout << *this << G4endl;
#endif
}
...
void G4Fragment::NumberOfExitationWarning(const G4String& value)
{
  ...
  G4Exception("G4Fragment::NumberOfExitationWarning : wrong exciton number ! ", "HAD_FRAGMENT_03",
              FatalException, ed);
}
