                                                 ...
                                                 std::vector<G4LorentzVector>& finalState)
{
  ...
  G4double sumofmasses = std::accumulate(masses.begin(), masses.end(), 0.);
  ...
  do
  {
    ...
    for (i = 0; i < numberOfDaughters; i++)
    {
      ...
      {
        G4cout << i << " random number:" << rd[i] << " virtual mass:" << sm[i] / GeV << " GeV/c2"
               << G4endl;
      }
    }
    ...
    if (GetVerboseLevel() > 1)
    {
      G4cout << " daughter " << i << ": momentum " << daughtermomentum[i] / GeV << " GeV/c"
             << G4endl;
    }
      ...
      if (daughtermomentum[i] < 0.0)
      {
        ...
        if (GetVerboseLevel() > 0)
        {
          G4cout << "G4HadPhaseSpaceNBodyAsai::Generate "
                 ...
                 << "\n daughter " << i << ": mass " << masses[i] / GeV << " GeV/c2; momentum "
