...
G4double G4LegendrePolynomial::GetCoefficient(std::size_t i, std::size_t order)
{
  ...
  if (order >= fCoefficients.size() || i / 2 >= fCoefficients[order].size() || (i % 2) != order % 2)
    return 0;
...
G4double G4LegendrePolynomial::EvalAssocLegendrePoly(G4int l, G4int m, G4double x)
{
  ...
  if (m < 0)
  {
    ...
    return complementary_value * (m % 2 == 0 ? 1.0 : -1.0) * g4pow->factorial(l + m)
           / g4pow->factorial(l - m);
  }
  ...
  {
    current = (m % 2 == 0 ? 1.0 : -1.0) * G4Exp(g4pow->logfactorial(2 * m) - g4pow->logfactorial(m))
  ...
  {
    G4double next =
      (-(G4double(i + m - 1)) * previous + x * G4double(2 * i - 1) * current) / G4double(i - m);
    previous = current;
    current = next;
  }
...
void G4LegendrePolynomial::BuildUpToOrder(std::size_t orderMax)
{
  if (orderMax > 30)
  {
    G4cout << "G4LegendrePolynomial::GetCoefficient(): "
           << "I refuse to make a Legendre Polynomial of order " << orderMax << G4endl;
    return;
  }
  while (fCoefficients.size() < orderMax + 1)
  { /* Loop checking, 30-Oct-2015, G.Folger */
    std::size_t order = fCoefficients.size();
    fCoefficients.resize(order + 1);
    if (order <= 1)
      fCoefficients[order].push_back(1.);
    else
    {
      for (std::size_t iCoeff = 0; iCoeff < order + 1; ++iCoeff)
      {
        if ((order % 2) == (iCoeff % 2))
        {
          G4double coeff = 0;
          if (iCoeff <= order - 2)
            coeff -= fCoefficients[order - 2][iCoeff / 2] * G4double(order - 1);
          if (iCoeff > 0)
            coeff += fCoefficients[order - 1][(iCoeff - 1) / 2] * G4double(2 * order - 1);
