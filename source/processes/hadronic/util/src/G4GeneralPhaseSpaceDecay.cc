...
G4GeneralPhaseSpaceDecay::G4GeneralPhaseSpaceDecay(G4int Verbose)
  : G4VDecayChannel("Phase Space", Verbose), parentmass(0.), theDaughterMasses(0)
{
  if (GetVerboseLevel() > 1) G4cout << "G4GeneralPhaseSpaceDecay:: constructor " << G4endl;
}

G4GeneralPhaseSpaceDecay::G4GeneralPhaseSpaceDecay(const G4String& theParentName, G4double theBR,
...
G4GeneralPhaseSpaceDecay::G4GeneralPhaseSpaceDecay(
  const G4String& theParentName, G4double theParentMass, G4double theBR, G4int theNumberOfDaughters,
  const G4String& theDaughterName1, const G4String& theDaughterName2,
  const G4String& theDaughterName3, const G4double* masses)
  : G4VDecayChannel("Phase Space", theParentName, theBR, theNumberOfDaughters, theDaughterName1,
                    theDaughterName2, theDaughterName3),
    parentmass(theParentMass),
...
G4GeneralPhaseSpaceDecay::G4GeneralPhaseSpaceDecay(
  const G4String& theParentName, G4double theParentMass, G4double theBR, G4int theNumberOfDaughters,
  const G4String& theDaughterName1, const G4String& theDaughterName2,
  const G4String& theDaughterName3, const G4String& theDaughterName4, const G4double* masses)
  : G4VDecayChannel("Phase Space", theParentName, theBR, theNumberOfDaughters, theDaughterName1,
                    theDaughterName2, theDaughterName3, theDaughterName4),
    parentmass(theParentMass),
    theDaughterMasses(masses)
{
  if (GetVerboseLevel() > 1) G4cout << "G4GeneralPhaseSpaceDecay:: constructor " << G4endl;
...
G4DecayProducts* G4GeneralPhaseSpaceDecay::TwoBodyDecayIt()
{
  ...
  G4double Etotal =
    std::sqrt(daughtermass[0] * daughtermass[0] + daughtermomentum * daughtermomentum);
  G4DynamicParticle* daughterparticle =
    new G4DynamicParticle(G4MT_daughters[0], Etotal, direction * daughtermomentum);
  products->PushProducts(daughterparticle);
  Etotal = std::sqrt(daughtermass[1] * daughtermass[1] + daughtermomentum * daughtermomentum);
  daughterparticle =
    new G4DynamicParticle(G4MT_daughters[1], Etotal, direction * (-1.0 * daughtermomentum));
  products->PushProducts(daughterparticle);
...
// algorism of this code is originally written in GDECA3 of GEANT3
{
  ...
  if (loopCounter >= maxNumberOfLoops)
  {
    ...
    G4Exception(" G4GeneralPhaseSpaceDecay::ThreeBodyDecayIt ", "HAD_PHASESPACE_001",
                FatalException, ed);
  }
  ...
  G4double Etotal =
    std::sqrt(daughtermass[0] * daughtermass[0] + daughtermomentum[0] * daughtermomentum[0]);
  G4DynamicParticle* daughterparticle =
  ...
  Etotal = std::sqrt(daughtermass[2] * daughtermass[2]
                     + daughtermomentum[2] * daughtermomentum[2] / direction2.mag2());
  daughterparticle = new G4DynamicParticle(G4MT_daughters[2], Etotal,
                                           direction2 * (daughtermomentum[2] / direction2.mag()));
  products->PushProducts(daughterparticle);
  G4ThreeVector mom =
    (direction0 * daughtermomentum[0] + direction2 * (daughtermomentum[2] / direction2.mag()))
    * (-1.0);
  Etotal = std::sqrt(daughtermass[1] * daughtermass[1] + mom.mag2());
  daughterparticle = new G4DynamicParticle(G4MT_daughters[1], Etotal, mom);
  products->PushProducts(daughterparticle);
...
//
{
  ...
  daughterparticle[index1] =
    new G4DynamicParticle(G4MT_daughters[index1], direction * daughtermomentum[index1]);
  daughterparticle[index1 + 1] = new G4DynamicParticle(
    G4MT_daughters[index1 + 1], direction * (-1.0 * daughtermomentum[index1]));
  ...
  for (index1 = numberOfDaughters - 3; index1 >= 0; index1--)
  {
    ...
    beta /= std::sqrt(daughtermomentum[index1] * daughtermomentum[index1]
                      + sm[index1 + 1] * sm[index1 + 1]);
    for (G4int index2 = index1 + 1; index2 < numberOfDaughters; index2++)
    {
      G4LorentzVector p4;
    ...
    // create daughter G4DynamicParticle
    daughterparticle[index1] =
      new G4DynamicParticle(G4MT_daughters[index1], direction * (-1.0 * daughtermomentum[index1]));
  }
