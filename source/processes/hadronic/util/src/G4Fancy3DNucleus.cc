...
{
  return ((currentNucleon >= 0 && currentNucleon < myA) ? &theNucleons[currentNucleon++] : 0);
}
...
void G4Fancy3DNucleus::SortNucleonsIncZ()
{
  ...
  std::sort(theNucleons.begin(), theNucleons.end(), G4Fancy3DNucleusHelperForSortInZ);
}
...
{
  if (myL <= 0)
    return myZ * G4Proton::Proton()->GetPDGMass() + (myA - myZ) * G4Neutron::Neutron()->GetPDGMass()
           - BindingEnergy();
  else
    return G4HyperNucleiProperties::GetNuclearMass(myA, myZ, myL);
}
...
void G4Fancy3DNucleus::DoLorentzContraction(const G4ThreeVector& theBeta)
{
  ...
  if (beta2 > 0)
  {
    ...
    {
      rprime =
        theNucleons[i].GetPosition() - factor * (theBeta * theNucleons[i].GetPosition()) * theBeta;
  ...
  else
  {
    ...
    for (G4int ii = 1; ii < 4; ii++)  // 2 - 4 nucleons of the first He-4
    {
      ...
      do
      {
        ...
        {
          if ((theNucleons[ii].GetPosition() - theNucleons[jj].GetPosition()).mag2() <= nd2)
          {
            Continue = true;
            break;
          }
    ...
    {
      Pos = G4LorentzVector(theNucleons[ii].GetPosition(), 0.);
      Pos *= RandomRotation;
...
void G4Fancy3DNucleus::ChooseFermiMomenta()
{
  ...
  for (G4int ntry = 0; ntry < 1; ntry++)
  {
    for (i = 0; i < myA; i++)  // momenta for all, including last, in case we swap nucleons
    {
      ...
      {
        G4double eMax =
          std::sqrt(sqr(fermiM[i]) + sqr(theNucleons[i].GetDefinition()->GetPDGMass()))
          - CoulombBarrier();
        ...
        else
        {
          ...
          G4Exception(
            "G4Fancy3DNucleus::ChooseFermiMomenta(): difficulty finding proton momentum, set it to "
            "(0,0,0)",
  ...
  for (i = 0; i < myA; i++)
  {
    energy = theNucleons[i].GetParticleType()->GetPDGMass() - BindingEnergy() / myA;
...
G4bool G4Fancy3DNucleus::ReduceSum()
{
  ...
  while ((sum - testSums[--index].Vector).mag() > PFermi
         && index > 0) /* Loop checking, 30-Oct-2015, G.Folger */
  {
  ...
  if ((sum - testSums[index].Vector).mag() <= PFermi)
  {
    ...
    for (G4int aNucleon = 0; aNucleon <= index; aNucleon++)
    {
      ...
      if (pTry < PFermi && std::abs(momentum[myA - 1].mag() - pTry) < pBest)
      {
