...
G4bool G4HadronicDeveloperParameters::SetDefault(const G4String& name, const G4int value,
                                                 G4int lower_limit, G4int upper_limit)
{
  G4bool status = false;
  ...
  if (it == i_defaults.cend())
  {
    ...
    i_limits.insert(std::pair<G4String, std::pair<const G4int, const G4int>>(
...
G4bool G4HadronicDeveloperParameters::SetDefault(const G4String& name, const G4double value,
                                                 G4double lower_limit, G4double upper_limit)
{
  G4bool status = false;
  ...
  if (it == defaults.cend())
  {
    ...
    limits.insert(std::pair<G4String, std::pair<const G4double, const G4double>>(
...
void G4HadronicDeveloperParameters::Dump(const G4String& name)
{
  ...
  if (b_values.find(name) != b_values.cend())
  {
    G4cout << "G4HadronicDeveloperParameters: "
           << "name = " << name << ", default value = " << b_defaults.find(name)->second
           << ", current value = " << b_values.find(name)->second << "." << G4endl;
  }
  else if (i_values.find(name) != i_values.cend())
  {
    G4cout << "G4HadronicDeveloperParameters: "
           << "name = " << name << ", default value = " << i_defaults.find(name)->second
           << ", lower limit = " << i_limits.find(name)->second.first
           << ", upper limit = " << i_limits.find(name)->second.second
           << ", current value = " << i_values.find(name)->second << "." << G4endl;
  }
  else if (values.find(name) != values.cend())
  {
    G4cout << "G4HadronicDeveloperParameters: "
           << "name = " << name << ", default value = " << defaults.find(name)->second
           << ", lower limit = " << limits.find(name)->second.first
           << ", upper limit = " << limits.find(name)->second.second
           << ", current value = " << values.find(name)->second << "." << G4endl;
  }
  else
  {
    /*Parameter of "name" does not exist*/
...
G4bool G4HadronicDeveloperParameters::check_value_within_limits(
  std::pair<const G4double, const G4double>& alimits, const G4double value)
{
...
G4bool G4HadronicDeveloperParameters::check_value_within_limits(
  std::pair<const G4int, const G4int>& alimits, const G4int value)
{
  if (alimits.first <= value && value <= alimits.second)
  {
    return true;
