  ...
  : verboseLevel(verbose), theAlgorithm(0)
{
  ...
  {
    case Kopylov:
      theAlgorithm = new G4HadPhaseSpaceKopylov(verboseLevel);
      break;
    case GENBOD:
      theAlgorithm = new G4HadPhaseSpaceGenbod(verboseLevel);
      break;
    case NBody:
      theAlgorithm = new G4HadPhaseSpaceNBodyAsai(verboseLevel);
      break;
    case NONE:
      theAlgorithm = 0;
      break;  // User may explicitly set no algorithm
    default:
      ReportInvalidAlgorithm(alg);
  }
...
G4HadDecayGenerator::G4HadDecayGenerator(G4VHadDecayAlgorithm* alg, G4int verbose)
...
{
  if (verboseLevel) G4cerr << "G4HadDecayGenerator: bad algorithm code " << alg << G4endl;
...
{
  if (verboseLevel) G4cerr << "G4HadDecayGenerator: no algorithm specified" << G4endl;
...
G4bool G4HadDecayGenerator::Generate(G4double initialMass, const std::vector<G4double>& masses,
                                     std::vector<G4LorentzVector>& finalState)
{
  if (verboseLevel) G4cout << " >>> G4HadDecayGenerator::Generate (mass)" << G4endl;

  if (!theAlgorithm) ReportMissingAlgorithm();

  if (masses.size() == 1U) return GenerateOneBody(initialMass, masses, finalState);
...
G4bool G4HadDecayGenerator::Generate(const G4ParticleDefinition* initialPD,
                                     ...
                                     std::vector<G4LorentzVector>& finalState)
{
  if (verboseLevel) G4cout << " >>> G4HadDecayGenerator::Generate (particle)" << G4endl;
...
G4bool G4HadDecayGenerator::Generate(const G4LorentzVector& initialState,
                                     ...
                                     std::vector<G4LorentzVector>& finalState)
{
  if (verboseLevel) G4cout << " >>> G4HadDecayGenerator::Generate (frame)" << G4endl;
...
{
  if (verboseLevel > 1) G4cout << " >>> G4HadDecayGenerator::GenerateOneBody" << G4endl;
