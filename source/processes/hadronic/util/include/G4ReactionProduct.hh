...
class G4ReactionProduct
{
  ...
  public:
    ...
    inline const G4ParticleDefinition* GetDefinition() const { return theParticleDefinition; }

    void SetDefinition(const G4ParticleDefinition* aParticleDefinition);

    inline void SetDefinitionAndUpdateE(const G4ParticleDefinition* p) { SetDefinition(p); }
    ...
    inline void SetMomentum(const G4ThreeVector& mom) { momentum = mom; }
    ...
    {
      mass = mas;
      totalEnergy = kineticEnergy + mas;
    }

    inline G4double GetMass() const { return mass; }

    inline void SetTOF(const G4double t) { timeOfFlight = t; }

    inline G4double GetTOF() const { return timeOfFlight; }

    inline void SetSide(const G4int sid) { side = sid; }

    inline G4int GetSide() const { return side; }

    inline void SetCreatorModelID(const G4int mod) { theCreatorModel = mod; }
    ...
    inline void SetNewlyAdded(const G4bool f) { NewlyAdded = f; }

    inline G4bool GetNewlyAdded() const { return NewlyAdded; }

    inline void SetMayBeKilled(const G4bool f) { MayBeKilled = f; }

    inline G4bool GetMayBeKilled() const { return MayBeKilled; }
    ...
    inline void SetPositionInNucleus(G4ThreeVector& aPosition) { positionInNucleus = aPosition; }

    inline const G4ThreeVector& GetPositionInNucleus() const { return positionInNucleus; }
