...
G4CRCoalescence::G4CRCoalescence()
  : G4HadronicInteraction("G4CRCoalescence"), fP0_d(0.0), fP0_dbar(0.0), secID(-1)
{
...
void G4CRCoalescence::SetP0Coalescence(const G4HadProjectile& thePrimary, G4String /* model */)
{
  ...
  {  // proton
    ...
    {
      fP0_dbar =
        130.0 / (1.0 + std::exp(21.6 - std::log(0.001 * ekin) / 0.089));  // set p0 for antideuteron
      fP0_d =
        118.1 * (1.0 + std::exp(5.53 - std::log(0.001 * ekin) / 0.43));  // set p0 for deuteron
    }
  }
  // G4cout << "Coalescence parameter p0 deuteron / antideuteron: " << fP0_d << " / " << fP0_dbar <<
  // G4endl;
}
...
void G4CRCoalescence::GenerateDeuterons(G4ReactionProductVector* result)
{
  ...
  {  // loop over protons
    ...
    if (partner1 == -1)
    {  // if no partner found, then the proton is a final-state secondary
  ...
  {  // loop over neutrons
    if (neutron.at(i).first == -1)
      continue;  // Skip already bound neutron, else it is a final-state secondary
  ...
  for (unsigned int i = 0; i < antiproton.size(); ++i)
  {  // loop over antiprotons
    ...
    int partner1 = FindPartner(p1, G4Proton::Proton()->GetPDGMass(), antineutron,
                               G4Neutron::Neutron()->GetPDGMass(), -1);
    if (partner1 == -1)
    {  // if no partner found, then the antiproton is a final-state secondary
  ...
  for (unsigned int i = 0; i < antineutron.size(); ++i)
  {  // loop over antineutrons
    if (antineutron.at(i).first == -1)
      continue;  // Skip already bound antineutron, else it is a final-state secondary
...
void G4CRCoalescence::PushDeuteron(const G4ThreeVector& p1, const G4ThreeVector& p2,
                                   G4int charge,  // input
...
{  // output
  ...
  {
    G4ParticleDefinition* antideuteron =
      G4ParticleTable::GetParticleTable()->FindAntiParticle("deuteron");
...
G4bool G4CRCoalescence::Coalescence(const G4ThreeVector& p1, G4double m1, const G4ThreeVector& p2,
                                    G4double m2, G4int charge)
{
  // Returns true if the momenta of the two nucleons/antinucleons (depending on "charge") are
  // inside of an sphere of radius p0 (assuming that the two particles are in the same spatial
  // place).
...
{
  // Returns true if the momenta of the two nucleons/antinucleons (depending on "charge") are
  // inside of a sphere of radius p0 (assuming that the two particles are in the same spatial
  // place).
  G4double deltaP = GetPcm(p1x, p1y, p1z, m1, p2x, p2y, p2z, m2);
  if (charge > 0)
    return (deltaP < fP0_d);
  else
    return (deltaP < fP0_dbar);
}

G4double G4CRCoalescence::GetPcm(const G4ThreeVector& p1, G4double m1, const G4ThreeVector& p2,
                                 G4double m2)
{
                                 ...
                                 G4double p2x, G4double p2y, G4double p2z, G4double m2)
{
  ...
  return std::sqrt((scm - (m1 - m2) * (m1 - m2)) * (scm - (m1 + m2) * (m1 + m2)))
         / (2.0 * std::sqrt(scm));
}

G4double G4CRCoalescence::GetS(G4double p1x, G4double p1y, G4double p1z, G4double m1, G4double p2x,
                               G4double p2y, G4double p2z, G4double m2)
{
  ...
  return (E1 + E2) * (E1 + E2) - (p1x + p2x) * (p1x + p2x) - (p1y + p2y) * (p1y + p2y)
         - (p1z + p2z) * (p1z + p2z);
}
