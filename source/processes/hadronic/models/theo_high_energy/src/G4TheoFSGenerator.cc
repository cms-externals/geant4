...
G4TheoFSGenerator::G4TheoFSGenerator(const G4String& name)
  : G4HadronicInteraction(name),
    ...
    theHighEnergyGenerator(nullptr),
    theQuasielastic(nullptr),
    theCosmicCoalescence(nullptr),
    theStringModelID(-1),
    theFTFQuasiElasticID(-1),
    theFTFDiffractiveID(-1)
{
                                                  ...
                                                  G4Nucleus& theNucleus)
{
  ...
  if (thePrimary.GetKineticEnergy() < energyThresholdForCharmAndBottomHadrons
      && (thePrimary.GetDefinition()->GetQuarkContent(4) != 0
          ||  // Has charm       constituent quark
          thePrimary.GetDefinition()->GetAntiQuarkContent(4) != 0
          ||  // Has anti-charm  constituent anti-quark
          thePrimary.GetDefinition()->GetQuarkContent(5) != 0
          ||  // Has bottom      constituent quark
          thePrimary.GetDefinition()->GetAntiQuarkContent(5) != 0))
  {  // Has anti-bottom constituent anti-quark
  ...
  if (theQuasielastic)
  {
    if (theQuasielastic->GetFraction(theNucleus, aPart) > G4UniformRand())
    {
      ...
      if (result)
      {
        for (auto& ptr : *result)
        {
          G4DynamicParticle* aNew = new G4DynamicParticle(
            ptr->GetDefinition(), ptr->Get4Momentum().e(), ptr->Get4Momentum().vect());
...
// #define DEBUG_initial_result
  ...
  {
    final_mass =
      ionTable->GetIonMass(theNucleus.GetZ_asInt() - resZ, theNucleus.GetA_asInt() - resA);
  }
  ...
  G4cout << "  final E, mass = " << E_out << ", " << final_mass << "  excitation_E " << E_excit
         << G4endl;
#endif
  ...
  if (theProjectileNucleus == nullptr)
  {
    ...
    {
      theTransportResult =
        theDecay.Propagate(theInitialResult, theHighEnergyGenerator->GetWoundedNucleus());
      if (theTransportResult == nullptr)
      {
  ...
  {
    theTransport->SetPrimaryProjectile(thePrimary);
    theTransportResult = theTransport->PropagateNuclNucl(
      theInitialResult, theHighEnergyGenerator->GetWoundedNucleus(), theProjectileNucleus);
    if (theTransportResult == nullptr)
    {
      G4cout << "G4TheoFSGenerator: null ptr from transport propagate " << G4endl;
      throw G4HadronicException(__FILE__, __LINE__, "Null ptr from transport propagate");
  ...
  if (G4HadronicParameters::Instance()->EnableCRCoalescence())
  {
    ...
    {
      theCosmicCoalescence =
        (G4CRCoalescence*)G4HadronicInteractionRegistry::Instance()->FindModel("G4CRCoalescence");
      if (theCosmicCoalescence == nullptr)
      {
  ...
  for (auto& ptr : *theTransportResult)
  {
    G4DynamicParticle* aNewDP =
      new G4DynamicParticle(ptr->GetDefinition(), ptr->GetTotalEnergy(), ptr->GetMomentum());
