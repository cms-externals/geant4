...
G4QuasiElRatios::G4QuasiElRatios()
{
  ...
  PCSmanager =
    (G4ChipsProtonElasticXS*)G4CrossSectionDataSetRegistry::Instance()->GetCrossSectionDataSet(
      G4ChipsProtonElasticXS::Default_Name());

  NCSmanager =
    (G4ChipsNeutronElasticXS*)G4CrossSectionDataSetRegistry::Instance()->GetCrossSectionDataSet(
...
std::pair<G4double, G4double> G4QuasiElRatios::GetRatios(G4double pIU, G4int pPDG, G4int tgZ,
                                                         G4int tgN)
{
  G4double R = 0.;
  ...
  if (pPDG > 999 && pIU < 227. && tgZ + tgN > 1)
    R = 1.;  // To accelerate @lowE
...
G4double G4QuasiElRatios::GetQF2IN_Ratio(G4double m_s, G4int A)
{
  ...
  if (nDB)
    for (i = 0; i < nDB; i++)
      if (A == vARatio[i])  // Search for this A in AMDB
      {
  ...
  if (!nDB || !found)  // Create new line in the AMDB
  {
    ...
    if (lastNRatio > nps)
    {
      ...
      lastHRatio = lastNRatio * ds;  // Calculate max initialized s for LinTab
    G4double sv = 0;
    lastTRatio[0] = 1.;
    ...
    if (m_s > sma)
    {
      ...
      if (lastKRatio > nls)
      {
        ...
        lastMRatio = lastKRatio * dls;  // Calculate max initialized ln(s)-lsi for LogTab
      sv = mi;
...
G4double G4QuasiElRatios::CalcQF2IN_Ratio(G4double m_s, G4int A)
{
  ...
  return C * G4Exp(-E * G4Pow::GetInstance()->powA(G4double(A - 1.), F))
         / G4Pow::GetInstance()->powA(G4double(A), P);
}  // End of CalcQF2IN_Ratio
...
std::pair<G4double, G4double> G4QuasiElRatios::CalcElTot(G4double p, G4int I)
{
  ...
  else if (I == 2)  // pimp/pipn
  {
    ...
    else
    {
      ...
      El = LE + (pbe * ld2 + 2.4 + 7. / sp) / (1. + .7 / p4) + .6 / md
           + .05 / hd;  // p9(pbe=.0557),p10,p11,p12,p13,p14
  ...
  else if (I == 3)  // pipp/pimn
  {
    ...
    else
    {
      ...
      El = LE + (pbe * ld2 + 2.4 + 6. / sp) / (1. + 3. / p4)
           + .7 / md;  // p7(pbe=.0557), p8, p9, p10, p11
...
std::pair<G4double, G4double> G4QuasiElRatios::GetElTotXS(G4double p, G4int PDG, G4bool F)
{
  ...
  //  - treat mesons with constituent cbar antiquark or bbar antiquark as a meson with sbar
  //  antiquark (e.g. K+);
  //  - treat all heavy baryons (i.e. hyperons, charmed and bottom baryons) as lambda;
  //  - treat all heavy anti-baryons (i.e. anti-hyperons, charmed and bottom anti-baryons) as
  //  anti-p/anti-n.
  else if (PDG == -321 || PDG == -311 || (kf && !kfl) ||  // mesons with s quark
           PDG == 411 || PDG == 421 || PDG == 431 ||  // mesons with c quark
           PDG == -521 || PDG == -511 || PDG == -531 || PDG == -541)
    ind = 4;  // mesons with b quark
  else if (PDG == 321 || PDG == 311 || (kf && kfl) ||  // mesons with sbar antiquark
           PDG == -411 || PDG == -421 || PDG == -431 ||  // mesons with cbar antiquark
           PDG == 521 || PDG == 511 || PDG == 531 || PDG == 541)
    ind = 5;  // mesons with bbar antiquark
  else if (PDG > 3000 && PDG < 5333)
    ind = 6;  // @@ for all heavy baryons - take Lambda
  else if (PDG > -5333 && PDG < -2000)
...
std::pair<G4double, G4double> G4QuasiElRatios::FetchElTot(G4double p, G4int PDG, G4bool F)
{
  ...
  if (nDB && lastHtot == PDG && lastFtot == F && p > 0. && p == lastPtot)
    return lastRtot;  // VI don't use toler.
  ...
  if (nDB)
    for (i = 0; i < nDB; i++)
      if (ind == vItot[i])  // Sirch for this index in AMDB
      {
  ...
  if (!nDB || !found)  // Create new line in the AMDB
  {
    ...
    if (lastKtot > nlp)
    {
      ...
      lastMtot = lastKtot * dlp;  // Calculate max initialized ln(p)-lpi for LogTab
    G4double pv = mip;
...
std::pair<G4double, G4double> G4QuasiElRatios::GetElTot(G4double pIU, G4int hPDG, G4int Z, G4int N)
{
  G4double pGeV = pIU / gigaelectronvolt;
  if (Z < 1 && N < 1)
  {
    G4cout << "-Warning-G4QuasiElRatio::GetElTot:Z=" << Z << ",N=" << N << ", return zero"
           << G4endl;
...
// (Mean Elastic and Mean Total) Cross-Sections (mb) for PDG+(Z,N) at P=p[GeV/c]
std::pair<G4double, G4double> G4QuasiElRatios::GetChExFactor(G4double pIU, G4int hPDG, G4int Z,
                                                             G4int N)
{
  G4double pGeV = pIU / gigaelectronvolt;
  G4double resP = 0.;
  ...
  if (Z < 1 && N < 1)
  {
    G4cout << "-Warning-G4QuasiElRatio::GetChExF:Z=" << Z << ",N=" << N << ", return zero"
           << G4endl;
  ...
  if (hPDG == -211 || hPDG == -321 || hPDG == 3112 || hPDG == 3212 || hPDG == 3312)
    pf = Z / (A + N);
  else if (hPDG == 211 || hPDG == 321 || hPDG == 3222 || hPDG == 3212 || hPDG == 3322)
    nf = N / (A + Z);
...
G4QuasiElRatios::Scatter(G4int NPDG, G4LorentzVector N4M, G4int pPDG, G4LorentzVector p4M)
{
  ...
  if (PDG == 2212)
    xSec = PCSmanager->GetChipsCrossSection(P, Z, N, PDG);  // P CrossSect *TMP*
  else
    xSec = NCSmanager->GetChipsCrossSection(P, Z, N, PDG);  // N CrossSect *TMP*
  ...
  if (PDG == 2212)
    mint = PCSmanager->GetExchangeT(Z, N, PDG);  // P functional rand -t(MeV^2) *TMP*
  else
    mint = NCSmanager->GetExchangeT(Z, N, PDG);  // N functional rand -t(MeV^2) *TMP*
  G4double maxt = 0.;  // Prototype of max possible -t
  if (PDG == 2212)
    maxt = PCSmanager->GetHMaxT();  // max possible -t
  else
    maxt = NCSmanager->GetHMaxT();  // max possible -t
  ...
  if (std::abs(cost) > 1.)
  {
    ...
    else
    {
      G4double tm = 0.;
      if (PDG == 2212)
        tm = PCSmanager->GetHMaxT();
      else
        tm = NCSmanager->GetHMaxT();
      G4cerr << "G4QuasiFreeRatio::Scat:*NAN* cost=" << cost << ",-t=" << mint << ",tm=" << tm
             << G4endl;
      return std::make_pair(G4LorentzVector(0., 0., 0., 0.), p4M);  // Do Nothing Action
    }
  }
  ...
  {
    G4cerr << "G4QFR::Scat:t=" << tot4M << tot4M.m() << ",mT=" << mT << ",mP=" << std::sqrt(mP2)
           << G4endl;
...
std::pair<G4LorentzVector, G4LorentzVector> G4QuasiElRatios::ChExer(G4int NPDG, G4LorentzVector N4M,
                                                                    G4int pPDG, G4LorentzVector p4M)
{
  ...
  if (NPDG == 2212)
  {
    ...
    else if (pPDG == -311 || pPDG == 311 || pPDG == 130 || pPDG == 310)
      sPDG = 321;  // K0     -> K+ (?)
    else if (pPDG == 3112)
      sPDG = 3212;  // Sigma- -> Sigma0
    else if (pPDG == 3212)
      sPDG = 3222;  // Sigma0 -> Sigma+
    else if (pPDG == 3312)
      sPDG = 3322;  // Xi-    -> Xi0
  }
  ...
  if (std::abs(cost) > 1.)
  {
    ...
    {
      G4cerr << "G4QuasiFreeRatio::ChExer:*NAN* c=" << cost << ",t=" << mint << ",tm=" << maxt
             << G4endl;
      return std::make_pair(G4LorentzVector(0., 0., 0., 0.), p4M);  // Do Nothing Action
    }
  }
...
G4double G4QuasiElRatios::ChExElCoef(G4double p, G4int Z, G4int N, G4int pPDG)
{
  ...
  if (pPDG == 2212)
    ...
    G4cout << "*Warning*G4CohChrgExchange::ChExElCoef: wrong PDG=" << pPDG << G4endl;
...
G4bool G4QuasiElRatios::RelDecayIn2(G4LorentzVector& theMomentum, G4LorentzVector& f4Mom,
                                    G4LorentzVector& s4Mom, G4LorentzVector& dir, G4double maxCost,
                                    G4double minCost)
{
  ...
  if (dE < vP)
  {
    G4cerr << "***G4QHad::RelDecIn2: Tachionic 4-mom=" << theMomentum << ", E-p=" << dE - vP
           << G4endl;
  ...
  {  //@@ Later on make a quark content check for the decay
    G4cerr << "***G4QH::RelDecIn2: fM=" << fM << "+sM=" << sM << ">iM=" << iM
           << ",d=" << iM - fM - sM << G4endl;
    return false;
  }
  ...
  if (f4Mom.e() + .001 < f4Mom.rho())
    G4cerr << "*G4QH::RDIn2:*Boost* f4M=" << f4Mom << ",e-p=" << f4Mom.e() - f4Mom.rho() << G4endl;
  f4Mom.boost(ltb);  // Lor.Trans. of 1st hadron back to LS
  if (s4Mom.e() + .001 < s4Mom.rho())
    G4cerr << "*G4QH::RDIn2:*Boost* s4M=" << s4Mom << ",e-p=" << s4Mom.e() - s4Mom.rho() << G4endl;
