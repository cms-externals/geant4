...
void G4QMDMeanField::CalGraduate()
{
  ...
  for (G4int i = 0; i < system->GetTotalNumberOfParticipant(); ++i)
  {
    ...
    for (G4int j = 0; j < system->GetTotalNumberOfParticipant(); ++j)
    {
      ...
      G4double ccpp = c0g * rha[j][i] + c3g * rha[j][i] * (rh3d[j] + rh3d[i])
...
G4double G4QMDMeanField::GetPotential(G4int i)
{
  ...
  for (G4int j = 0; j < n; ++j)
  {
    ...
    rhoa += rha[j][i];
    rhoc += rhe[j][i];
    rhos += rha[j][i] * jnuc * inuc * (1 - 2 * std::abs(jcharge - icharge));
  }
...
G4double G4QMDMeanField::GetTotalPotential()
{
  ...
  for (G4int i = 0; i < n; ++i)
  {
    ...
    for (G4int j = 0; j < n; ++j)
    {
      ...
      rhoa[i] += rha[j][i];
      rhoc[i] += rhe[j][i];
      rhos[i] += rha[j][i] * jnuc * inuc * (1 - 2 * std::abs(jcharge - icharge));
    }
