  ...
  for (G4int iitry = 0; iitry < 4; iitry++)
  {
    ...
    if (secs)
    {
      ...
      else if (secs->size() == 1)
      {
        // 081118
        abs = true;
        // G4cout << "G4QMDCollision pion absrorption " <<
        // secs->front()->GetDefinition()->GetParticleName() << G4endl; secs->front()->Decay();
...
G4bool G4QMDCollision::CalFinalStateOfTheBinaryCollisionJQMD(G4double sig, G4double cutoff,
                                                             G4ThreeVector pcm, G4double prcm,
                                                             G4double srt, G4ThreeVector beta,
