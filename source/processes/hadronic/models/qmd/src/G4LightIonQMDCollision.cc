...
// 230308 Energy difference evaluated by "GetTotalEnergy" calculated in Mean Field (Y-H. Sato and A.
// Haga)
//
...
void G4LightIonQMDCollision::CalKinematicsOfBinaryCollisions(G4double dt)
{
  ...
  for (G4int i = 0; i < n; i++)
  {
    // std::cout << i << " " << theSystem->GetParticipant( i )->GetDefinition()->GetParticleName()
    // << " " << theSystem->GetParticipant( i )->GetPosition() << std::endl;
    ...
    if (theSystem->GetParticipant(i)->GetDefinition()->IsShortLived())
    {
      ...
      for (G4int ii = 0; ii < maximumNumberOfTrial; ii++)
      {
        ...
        p400 *= GeV;
        // G4KineticTrack kt( theSystem->GetParticipant( i )->GetDefinition() , 0.0 ,
        // (theSystem->GetParticipant( i )->GetPosition())*fermi , p4 );
        G4KineticTrack kt(pd0, 0.0, r0 * fermi, p400);
        // std::cout << "G4KineticTrack " << i << " " <<  kt.GetDefinition()->GetParticleName() <<
        // kt.GetPosition() << std::endl;
        ...
        if (secs)
        {
          for (G4KineticTrackVector::iterator it = secs->begin(); it != secs->end(); it++)
          {
            /*
            ...
            */
            ...
            {
              // Append end;
              theSystem->SetParticipant(new G4QMDParticipant((*it)->GetDefinition(),
                                                             (*it)->Get4Momentum().v() / GeV,
                                                             (*it)->GetPosition() / fermi));
              // et += (*it)->Get4Momentum().e()/GeV;
              if (id > 1)
              {
  ...
  for (G4int i = 1; i < theSystem->GetTotalNumberOfParticipant(); i++)
  {
    ...
    {
      /*
               G4cout << "Collision " << i << " " << theSystem->GetParticipant( i
         )->IsThisProjectile() << G4endl; G4cout << "Collision " << j << " " <<
         theSystem->GetParticipant( j )->IsThisProjectile() << G4endl; G4cout << "Collision " << i
         << " " << theSystem->GetParticipant( i )->IsThisTarget() << G4endl; G4cout << "Collision "
         << j << " " << theSystem->GetParticipant( j )->IsThisTarget() << G4endl;
      */
      ...
      */
      ...
      G4bool energetically_forbidden =
        !(CalFinalStateOfTheBinaryCollision(i, j));  // Use Geant4 Collision Library
      // G4bool energetically_forbidden = !( CalFinalStateOfTheBinaryCollisionJQMD ( sig , cutoff ,
      // pcm , prcm , srt, beta , gamma , i , j ) ); // JQMD Elastic

      /*
               G4bool pauli_blocked = false;
               if ( energetically_forbidden == false ) // result true
               {
                  if ( theMeanField->IsPauliBlocked ( i ) == true || theMeanField->IsPauliBlocked (
         j ) == true )
                  {
                     pauli_blocked = true;
      ...
      else
      {
        ...
        /*
                    ...
                    G4cout << "G4QMDRESULT Collsion Diff "
                              << p4i + p4j - theSystem->GetParticipant( i )->Get4Momentum() -
           theSystem->GetParticipant( j )->Get4Momentum()
                              << G4endl;
...
{
  // 081103
  // G4cout << "CalFinalStateOfTheBinaryCollision " << i << " " << j << " " <<
  // theSystem->GetTotalNumberOfParticipant() << G4endl;
  ...
  for (G4int iitry = 0; iitry < 4; iitry++)
  {
    ...
    // std::cout << "G4QMDSCATTERER BEFORE " << kt1.GetDefinition()->GetParticleName() << " " <<
    // kt1.Get4Momentum()/GeV << " " << kt1.GetPosition()/fermi << std::endl; std::cout <<
    // "G4QMDSCATTERER BEFORE " << kt2.GetDefinition()->GetParticleName() << " " <<
    // kt2.Get4Momentum()/GeV << " " << kt2.GetPosition()/fermi << std::endl; std::cout <<
    // "THESCATTERER " << theScatterer->GetCrossSection ( kt1 , kt2 )/millibarn << " " << elastic <<
    // " " << sig << std::endl;
    ...
    if (secs)
    {
      ...
      if (secs->size() == 2)
      {
        for (G4KineticTrackVector::iterator it = secs->begin(); it != secs->end(); it++)
        {
          if (iti == 0)
          {
            ...
            // std::cout << "THESCATTERER " << (*it)->GetDefinition()->GetParticleName() <<
            // std::endl;
          ...
          if (iti == 1)
          {
            ...
            if ((*it)->GetDefinition()->IsShortLived())
            {
              ...
              {
                // G4cout << "decay " << (*jter)->GetDefinition()->GetParticleName() << " " <<
                // (*jter)->Get4Momentum()/GeV << " " << (*jter)->GetDefinition()->GetBaryonNumber()
                // << G4endl;

                if (ita == 0)
                {
                  theSystem->SetParticipant(new G4QMDParticipant((*jter)->GetDefinition(),
                                                                 (*jter)->Get4Momentum().v() / GeV,
                                                                 (*jter)->GetPosition() / fermi));
                  // G4cout << "decay " << (*jter)->GetDefinition()->GetParticleName() << " " <<
                  // (*jter)->Get4Momentum()/GeV << G4endl; theMeanField->Update(); G4cout << "decay
                  // " << (*jter)->GetDefinition()->GetParticleName() << " " <<
                  // theMeanField->GetTotalEnergy() << " " << eini << G4endl;
                  theSystem->GetParticipant(k)->SetDefinition((*jter)->GetDefinition());
                  // theMeanField->Update();
                  // G4cout << "decay " << (*jter)->GetDefinition()->GetParticleName() << " " <<
                  // theSystem->GetParticipant( k )->GetNuc() << " " <<
                  // theMeanField->GetTotalEnergy() << G4endl;
              ...
              // std::cout << "THESCATTERER " << pdi0->GetParticleName() << " " <<
              // pdj0->GetParticleName() << " " << p4i.e() + p4j.e() << std::endl; std::cout <<
              // "THESCATTERER " << theSystem->GetParticipant( k
              // )->GetDefinition()->GetParticleName() << " " << p4kx_new.m() << " " <<
              // (*it)->Get4Momentum() << " " << theSystem->GetParticipant( k )->Get4Momentum() << "
              // " << theSystem->GetParticipant( k )->Get4Momentum().m() << std::endl; std::cout <<
              // "THESCATTERER " << theSystem->GetParticipant( i
              // )->GetDefinition()->GetParticleName() << " " << theSystem->GetParticipant( j
              // )->GetDefinition()->GetParticleName() << " " << p4ix_new.e() + p4jx_new.e() <<
              // std::endl;
            }
            else
            {
          ...
          // std::cout << "G4QMDSCATTERER AFTER " << (*it)->GetDefinition()->GetParticleName() << "
          // " << (*it)->Get4Momentum()/GeV << std::endl;
          iti++;
        }
      }
      else if (secs->size() == 1)
      {
        // 081118
        abs = true;
        // G4cout << "G4LightIonQMDCollision pion absrorption " <<
        // secs->front()->GetDefinition()->GetParticleName() << G4endl;
        ...
        */
        ...
        // std::cout << "THESCATTERER " << p4ix_new.e()+p4jx_new.e() << " " << p4ix_new << " " <<
        // p4jx_new << std::endl; exit(0);
      }
      ...
      if (secs->size() > 2)
      {
        ...
        for (G4KineticTrackVector::iterator it = secs->begin(); it != secs->end(); it++)
        {
          G4cout << "G4QMDSCATTERER AFTER " << (*it)->GetDefinition()->GetParticleName() << " "
                 << (*it)->Get4Momentum() / GeV << G4endl;
        }
      }
    ...
    G4double efin = theMeanField->GetTotalEnergy();  // R-JQMD, Skyrme, RMF
    // std::cout << "Collision NEW epot " << i << " " << j << " " << epot << " " << std::abs ( eini
    // - efin ) - fepse << std::endl;

    /*
          G4cout << "Collision efin " << i << " " << j << " " << efin << G4endl;
          G4cout << "Collision " << i << " " << j << " " << std::abs ( eini - efin ) << " " << fepse
       << G4endl; G4cout << "Collision " << std::abs ( eini - efin ) << " " << fepse << G4endl;
    */
    ...
    if (std::abs(eini - efin) < fepse)
    {
      ...
      // std::cout << "collisions after " << theSystem->GetParticipant( i )->Get4Momentum() << " "
      // << theSystem->GetParticipant( j )->Get4Momentum() << std::endl; std::cout << "collisions
      // dif " << ( p4i + p4j ) - ( theSystem->GetParticipant( i )->Get4Momentum() +
      // theSystem->GetParticipant( j )->Get4Momentum() ) << std::endl; std::cout << "collisions
      // before " << ri0/fermi << " " << rj0/fermi << std::endl; std::cout << "collisions after " <<
      // theSystem->GetParticipant( i )->GetPosition() << " " << theSystem->GetParticipant( j
    ...
    {
      // if(pion_prod || pion_abs) G4cout << "EnergyNotOK " << std::abs ( eini - efin )*1000 << "
      // orig " << std::abs ( eini0 - efin0 )*1000 << G4endl; G4cout << "EnergyNotOK " << std::abs (
      // eini - efin )*1000 << G4endl; if(iitry == 3) G4cout << "Energy non-conserved and go
      // through" << G4endl;
      /*
      if(std::abs ( eini - efin )*1000 > 20)
      {
        //G4cout << "EnergyNotOK " << std::abs ( eini - efin )*1000 << " orig " << std::abs ( eini0
      - efin0 )*1000 << G4endl; G4cout << p4ix_new + p4jx_new << " " <<  theSystem->GetParticipant(
      i )->Get4Momentum() + theSystem->GetParticipant( j )->Get4Momentum() << G4endl; G4cout <<
      p4ix_new.m() << " " <<  p4jx_new.m() << " " <<  theSystem->GetParticipant( i
      )->Get4Momentum().m() << " " << theSystem->GetParticipant( j )->Get4Momentum().m() << G4endl;
       G4cout << "G4QMDRESULT Collsion Really Happened between "
       << i << " and " << j
       << G4endl;
       G4cout << "G4QMDRESULT Collsion initial p4 i and j "
       ...
       << G4endl;
       G4cout << "G4QMDRESULT Collsion Diff "
       << p4i + p4j - theSystem->GetParticipant( i )->Get4Momentum() - theSystem->GetParticipant( j
      )->Get4Momentum()
       << G4endl;
          G4cout << "G4QMDRESULT Particle Name "
          << theSystem->GetParticipant( i )->GetDefinition()->GetParticleName() << " " <<
      theSystem->GetParticipant( j )->GetDefinition()->GetParticleName() << " mj "
      <<theSystem->GetParticipant( j )->Get4Momentum().m()
          << G4endl;
       //G4cout << "G4QMDRESULT Collsion initial r i and j "
       //<< ri << " " << rj
       //<< G4endl;
  ...
  if (energyOK)
  {
    ...
    if (!abs)
    {
      ...
      {
        // G4cout << "Binary Collision Happen " << theSystem->GetTotalNumberOfParticipant() <<
        // G4endl;
        pauliOK = true;
      }
    }
...
G4bool G4LightIonQMDCollision::CalFinalStateOfTheBinaryCollisionJQMD(
  G4double sig, G4double cutoff, G4ThreeVector pcm, G4double prcm, G4double srt, G4ThreeVector beta,
  G4double gamma, G4int i, G4int j)
{
  ...
  if (zi == zj)
  {
    ...
    {
      elastic = (-std::atan((csrt - 0.4286) * 1.5 - 0.8) * 2. / pi + 1.0) * 9.65 + 7.0;
    }
  }
  else
  {
    ...
    {
      elastic = (-std::atan((csrt - 0.4286) * 1.5 - 0.8) * 2. / pi + 1.0) * 12.34 + 10.0;
    }
  }
  ...
  for (G4int itry = 0; itry < 4; itry++)
  {
    ...
    */
    ...
    if (std::abs(eini - efin) < fepse)
    {
      ...
      // dif " << ( p4i + p4j ) - ( theSystem->GetParticipant( i )->Get4Momentum() +
      // theSystem->GetParticipant( j )->Get4Momentum() ) << std::endl; std::cout << "collisions
      // before " << rix/fermi << " " << rjx/fermi << std::endl; std::cout << "collisions after " <<
      // theSystem->GetParticipant( i )->GetPosition() << " " << theSystem->GetParticipant( j
      // )->GetPosition() << std::endl;
    }
    // 071031
