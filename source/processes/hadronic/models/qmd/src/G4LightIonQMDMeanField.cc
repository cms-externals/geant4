...
{
  if (system->GetTotalNumberOfParticipant() < 2)
  {
    return;
  }
...
void G4LightIonQMDMeanField::CalGraduate()
{
  ...
  for (G4int i = 0; i < system->GetTotalNumberOfParticipant(); ++i)
  {
    ...
    for (G4int j = 0; j < system->GetTotalNumberOfParticipant(); ++j)
    {
      ...
      G4double ccpp = c0g * rha[j][i] + c3g * rha[j][i] * (rh3d[j] + rh3d[i]) + cg0 * rha[j][i] / wl
                      + cg0 * rha[j][i] * fsij + cgtau0 * rha[j][i] * (rh3d_tau[j] + rh3d_tau[i])
                      + csg * rha[j][i] * jnuc * inuc * (1. - 2. * std::abs(jcharge - icharge))
...
G4double G4LightIonQMDMeanField::GetPotential(G4int i)
{
  ...
  for (G4int j = 0; j < n; ++j)
  {
    ...
    rhoc += rhe[j][i];
    rhos += rha[j][i] * jnuc * inuc * (1. - 2. * std::abs(jcharge - icharge))  // Skyrme-QMD
...
std::vector<G4LightIonQMDNucleus*> G4LightIonQMDMeanField::DoClusterJudgment()
{
  ...
  G4double cpf2 =
    G4Pow::GetInstance()->A23(1.5 * pi * pi * G4Pow::GetInstance()->powA(4.0 * pi * wl, -1.5)) * hbc
    * hbc;
  ...
  for (G4int i = 0; i < n; ++i)
  {
    ...
    if (system->GetParticipant(i)->GetBaryonNumber() == 1)
    {
      for (G4int j = 0; j < n; ++j)
      {
        if (system->GetParticipant(j)->GetBaryonNumber() == 1) rhoa[i] += rha[i][j];
      }
    }
    ...
    if (system->GetParticipant(i)->GetBaryonNumber() == 1)
    {
      ...
      for (G4int j = j1; j < n; ++j)
      {
        ...
        if (system->GetParticipant(j)->GetBaryonNumber() == 1)
        {
          ...
          G4double pcc2 = cpf2 * (rhoa[i] + rhoa[j]) * (rhoa[i] + rhoa[j]);
                ...
                if (is_assigned_to[i] != is_assigned_to[j])
                {
                  ...
                  for (auto it = clusters.cbegin(); it != clusters.cend(); ++it)
                  {
                    if (it->first == target_cluster_id)
                    {
                      ...
                      clusters_tmp.insert(
                        std::multimap<G4int, G4int>::value_type(is_assigned_to[j], it->second));
                    }
                    else
                    {
                      clusters_tmp.insert(
                        std::multimap<G4int, G4int>::value_type(it->first, it->second));
                    }
                  }
