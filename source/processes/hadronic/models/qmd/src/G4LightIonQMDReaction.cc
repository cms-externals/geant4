...
G4HadFinalState* G4LightIonQMDReaction::ApplyYourself(const G4HadProjectile& projectile,
                                                      G4Nucleus& target)
{
  ...
  G4ThreeVector boostLABtoCM =
    targ4p.findBoostToCM(proj_dp->Get4Momentum() / GeV);  // CM of target and proj;
  ...
  G4double beta_nncm =
    (-boostLABtoCM.beta() + boostLABtoNN.beta()) / (1 - boostLABtoCM.beta() * boostLABtoNN.beta());
  ...
  while (elastic)  // Loop checking, 11.03.2015, T. Koi
  {
    ...
    // G4cout << "G4QMDRESULT bmax_0 = " << bmax_0/fermi << " fm, bmax = " << bmax << " fm , b = "
    // << b  << " fm " << G4endl;

    G4double plab = projectile.GetTotalMomentum() / GeV;
    G4double elab =
      (projectile.GetKineticEnergy() + proj_pd->GetPDGMass() + targ_pd->GetPDGMass()) / GeV;
    ...
    for (G4int i = 0; i < targ->GetTotalNumberOfParticipant(); i++)
    {
      ...
      G4ThreeVector p(p0.x() + coulomb_collision_px_targ, p0.y(),
                      p0.z() * coulomb_collision_gamma_targ + coulomb_collision_pz_targ);

      G4ThreeVector r(r0.x() + coulomb_collision_rx_targ, r0.y(),
    ...
    if (proj_A != 1)
    {
      ...
      for (G4int i = 0; i < proj->GetTotalNumberOfParticipant(); i++)
      {
        ...
        G4ThreeVector p(p0.x() + coulomb_collision_px_proj, p0.y(),
                        p0.z() * coulomb_collision_gamma_proj + coulomb_collision_pz_proj);

        G4ThreeVector r(r0.x() + coulomb_collision_rx_proj, r0.y(),
                        r0.z() / coulomb_collision_gamma_proj + coulomb_collision_rz_proj);

        system->SetParticipant(
          new G4QMDParticipant(proj->GetParticipant(i)->GetDefinition(), p, r));
    ...
    else
    {
      ...
      // G4cout << "system Total Participants : " << system->GetTotalNumberOfParticipant() << ",
      // target : " << targ->GetTotalNumberOfParticipant() << G4endl;
      if (system->GetTotalNumberOfParticipant() == targ->GetTotalNumberOfParticipant())
      {
        ...
        system->SetParticipant(
          new G4QMDParticipant((G4ParticleDefinition*)projectile.GetDefinition(), p, r));
    ...
    if (numberOfSecondary == 2)
    {
      ...
      else
      {
        ...
        // QMD should be inelastic collision, so that nucleon-nucleon collision should also be
        // inelastic in this phase. by Y-H. S and A. H, Mar. 6, 2023.
            ...
            || (proj_pd->GetParticleName() == "pi-" && targ_pd->GetParticleName() == "proton"))
        {
          ...
          // G4cout << "elasticLike_system = false proton NOCollision " << system->GetNOCollision()
          // << G4endl;
    ...
    if (elastic == true)
    {
      ...
      for (std::vector<G4LightIonQMDNucleus*>::iterator it = nucleuses.begin();
           it != nucleuses.end(); it++)
      {
        delete *it;
      }
      nucleuses.clear();
      // system->Clear() should be included here. Otherwise, the nucleon is repeatedly regstered if
      // the nucleon is the projectile. by Y-H. S. and A. H, Mar. 6, 2023.
      system->Clear();
    }
  }
  ...
  for (std::vector<G4LightIonQMDNucleus*>::iterator it = nucleuses.begin(); it != nucleuses.end();
       it++)
  {
    /*
          G4cout << "G4QMDRESULT "
    ...
    */
        ...
        || (*it)->GetAtomicNumber() == (*it)->GetMassNumber())  // proton cluster
    {
      ...
      {
        G4QMDParticipant* aP = new G4QMDParticipant(((*it)->GetParticipant(i))->GetDefinition(),
                                                    ((*it)->GetParticipant(i))->GetMomentum(),
                                                    ((*it)->GetParticipant(i))->GetPosition());
    ...
    G4double nucleus_e =
      std::sqrt(G4Pow::GetInstance()->powN((*it)->GetNuclearMass() / GeV, 2)
                + G4Pow::GetInstance()->powN((*it)->Get4Momentum().vect().mag(), 2));
    ...
    G4LorentzVector lv(G4ThreeVector(0.0), (*it)->GetExcitationEnergy() * GeV
                                             + G4IonTable::GetIonTable()->GetIonMass(iz, ia));
    ...
    for (G4ReactionProductVector::iterator itt = rv->begin(); itt != rv->end(); itt++)
    {
      notBreak = false;
      // Secondary from this nucleus (*it)
      const G4ParticleDefinition* pd = (*itt)->GetDefinition();

      G4LorentzVector p4((*itt)->GetMomentum() / GeV,
                         (*itt)->GetTotalEnergy() / GeV);  // in nucleus(*it) rest system
      ...
      else
      {
        ...
        G4double p_decay =
          std::sqrt(G4Pow::GetInstance()->powN(G4Alpha::Alpha()->GetPDGMass() + q_decay / 2, 2)
                    - G4Pow::GetInstance()->powN(G4Alpha::Alpha()->GetPDGMass(), 2));
        G4LorentzVector p4_a1(p_decay * randomized_direction,
                              G4Alpha::Alpha()->GetPDGMass() + q_decay / 2);  // in Be8 rest system
        ...
        G4LorentzVector p4_a2(-p_decay * randomized_direction,
                              G4Alpha::Alpha()->GetPDGMass() + q_decay / 2);  // in Be8 rest system
    ...
    if (notBreak == true)
    {
      const G4ParticleDefinition* pd = G4IonTable::GetIonTable()->GetIon(
        (*it)->GetAtomicNumber(), (*it)->GetMassNumber(), (*it)->GetExcitationEnergy() * GeV);
  ...
  {
    // G4cout << "Particle : " <<
    // theParticleChange.GetSecondary(i)->GetParticle()->GetParticleDefinition()->GetParticleName()
    // << G4endl; G4cout << "KEnergy : " <<
    // theParticleChange.GetSecondary(i)->GetParticle()->GetKineticEnergy() << G4endl; G4cout <<
    // "modelID : " << theParticleChange.GetSecondary(i)->GetCreatorModelID() << G4endl;
...
void G4LightIonQMDReaction::calcOffSetOfCollision(G4double b, const G4ParticleDefinition* pd_proj,
...
{
  outFile << "Lorentz covarianted Quantum Molecular Dynamics model for nucleus (particle) vs "
             "nucleus reactions\n";
}
