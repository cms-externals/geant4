...
G4HadFinalState* G4QMDReaction::ApplyYourself(const G4HadProjectile& projectile, G4Nucleus& target)
{
  ...
  while (elastic)  // Loop checking, 11.03.2015, T. Koi
  {
    ...
    if (elastic == true)
    {
      ...
      for (std::vector<G4QMDNucleus*>::iterator it = nucleuses.begin(); it != nucleuses.end(); it++)
      {
        delete *it;
      }
      nucleuses.clear();
...
void G4QMDReaction::calcOffSetOfCollision(G4double b, const G4ParticleDefinition* pd_proj,
                                          const G4ParticleDefinition* pd_targ, G4double ptot,
                                          G4double etot, G4double bmax, G4ThreeVector boostToCM)
{
