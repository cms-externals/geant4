...
G4int G4LightIonQMDNucleus::GetAtomicNumber()
{
  ...
  for (it = participants.begin(); it != participants.end(); it++)
  {
    if ((*it)->GetDefinition() == G4Proton::Proton()) Z++;
  }
  return Z;
}
...
G4double G4LightIonQMDNucleus::GetNuclearMass()
{
  ...
  if (mass == 0.0)
  {
    ...
    G4double BE = Av * A - As * G4Pow::GetInstance()->A23(G4double(A))
                  - Ac * Z * Z / G4Pow::GetInstance()->A13(G4double(A))
                  - Asym * (N - Z) * (N - Z) / A;

    mass = Z * G4Proton::Proton()->GetPDGMass() + N * G4Neutron::Neutron()->GetPDGMass() - BE;
  }
...
{
  // G4cout << "CalEnergyAndAngularMomentumInCM " << this->GetAtomicNumber() << " " <<
  // GetMassNumber() << G4endl;
  ...
  // binding energy should be evaluated with a relativistic version: 20230308 by Y-H. Sato and A.
  // Haga
  for (G4int i = 0; i < n; i++)
  {
    ...
    es[i] = std::sqrt(G4Pow::GetInstance()->powN(GetParticipant(i)->GetMass(), 2) + pcm[i] * pcm[i]
                      + 2.0 * GetParticipant(i)->GetMass() * nucpote)
            - GetParticipant(i)->GetMass();  // R-JQMD
 ...
 */
  ...
  // G4double kineticEnergyPerNucleon = ( std::accumulate ( es.begin() , es.end() , 0.0 ) -
  // totalMass )/n;
  ...
  // G4cout << "n " << n << "totalpote " << totalpote << " " << potentialEnergy << " " <<
  // bindingEnergy << G4endl; G4cout << "KineticEnergyPerNucleon in GeV " << kineticEnergyPerNucleon
  // << G4endl; G4cout << "KineticEnergySum in GeV " << std::accumulate ( es.begin() , es.end() ,
  // 0.0 ) - totalMass << G4endl; G4cout << "PotentialEnergy in GeV " << potentialEnergy << G4endl;
  // G4cout << "BindingEnergy in GeV " << bindingEnergy << G4endl;
  // G4cout << "G4BindingEnergy in GeV " << G4NucleiProperties::GetBindingEnergy( GetAtomicNumber()
  // , GetMassNumber() )/GeV << G4endl;

  excitationEnergy =
    bindingEnergy + G4NucleiProperties::GetBindingEnergy(GetMassNumber(), GetAtomicNumber()) / GeV;
...
G4double G4LightIonQMDNucleus::GetNuclPotential(G4int i)
{
  ...
  for (G4int j = 0; j < n; j++)
  {
    ...
    rhos += ibry * jbry * rh1 * jnuc * inuc * cef * (1 - 2 * std::abs(jcharge - icharge))
            * (1. - kappas * fsij);
  ...
  G4double potential = c0 * rhoa + c3 * rho3
