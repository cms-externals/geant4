...
void G4QMDNucleus::CalEnergyAndAngularMomentumInCM()
{
  ...
  for (G4int i = 0; i < n; i++)
  {
    ...
    es[i] =
      std::sqrt(G4Pow::GetInstance()->powN(GetParticipant(i)->GetMass(), 2) + pcm[i] * pcm[i]);
  ...
  G4double bindingEnergy =
    (std::accumulate(es.begin(), es.end(), 0.0) - totalMass) + potentialEnergy;

  // G4cout << "KineticEnergyPerNucleon in GeV " << kineticEnergyPerNucleon << G4endl;
  // G4cout << "KineticEnergySum in GeV " << std::accumulate ( es.begin() , es.end() , 0.0 ) -
  // totalMass << G4endl; G4cout << "PotentialEnergy in GeV " << potentialEnergy << G4endl; G4cout
  // << "BindingEnergy in GeV " << bindingEnergy << G4endl; G4cout << "G4BindingEnergy in GeV " <<
  // G4NucleiProperties::GetBindingEnergy( GetAtomicNumber() , GetMassNumber() )/GeV << G4endl;
