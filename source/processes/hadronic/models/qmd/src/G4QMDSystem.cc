...
void G4QMDSystem::SubtractSystem(G4QMDSystem* nucleus)
{
  ...
  {
    participants.erase(
      std::find(participants.begin(), participants.end(), nucleus->GetParticipant(i)));
  }
}
...
void G4QMDSystem::ShowParticipants()
{
  ...
  {
    G4cout << i << " " << (*it)->GetDefinition()->GetParticleName() << " " << std::setprecision(8)
           << (*it)->GetMomentum() << " " << (*it)->GetPosition() << G4endl;
