...
// 230308 Sampling momentum of the particle does not consider its binding energy (S-H. Sato and A.
// Haga) 230308 Binding energy evaluated by "GetTotalEnergy" calculated in Mean Field (S-H. Sato and
// A. Haga)
...
G4LightIonQMDGroundStateNucleus::G4LightIonQMDGroundStateNucleus(G4int z, G4int a)
  : maxTrial(1000),
    ...
    meanfield(NULL)
{
  // std::cout << " G4LightIonQMDGroundStateNucleus( G4int z , G4int a ) Begin " << z << " " << a <<
  // std::endl;
  ...
  if (z == 1 && a == 1)
  {  // Hydrogen  Case or proton primary
    SetParticipant(
      new G4QMDParticipant(G4Proton::Proton(), G4ThreeVector(0.0), G4ThreeVector(0.0)));
    //      ebini = 0.0;
    return;
  }
  else if (z == 0 && a == 1)
  {  // Neutron primary
    SetParticipant(
      new G4QMDParticipant(G4Neutron::Neutron(), G4ThreeVector(0.0), G4ThreeVector(0.0)));
    //      ebini = 0.0;
    return;
  }
  ...
  // std::cout << "G4LightIonQMDGroundStateNucleus( G4int z , G4int a ) packNucleons Begin ( z , a )
  // ( " << z << ", " << a << " )" << std::endl;
  packNucleons();
  // std::cout << "G4LightIonQMDGroundStateNucleus( G4int z , G4int a ) packNucleons End" <<
  // std::endl;
...
void G4LightIonQMDGroundStateNucleus::packNucleons()
{
  ...
  ebini =
    -G4NucleiProperties::GetBindingEnergy(GetMassNumber(), GetAtomicNumber()) / GetMassNumber();
  ...
  while (n0Try < maxTrial)  // Loop checking, 11.03.2015, T. Koi
  {
    ...
    for (G4int i = 0; i < GetMassNumber(); i++)
    {
      ...
      d_pot[i] = c0p * rho_a[i] + c3p * G4Pow::GetInstance()->powA(rho_a[i], gamm) + csp * rho_s[i]
                 + clp * rho_c[i];
  ...
  if (isThisOK == false)
  {
    G4cout << "GroundStateNucleus state cannot be created. Try again with another parameters."
           << G4endl;
  }
...
G4bool G4LightIonQMDGroundStateNucleus::samplingPosition(G4int i)
{
  ...
  while (nTry < maxTrial)  // Loop checking, 11.03.2015, T. Koi
  {
    ...
    for (G4int j = 0; j < i; j++)
    {
      ...
      // std::cout << "distance between j and i " << j << " " << i << " " << r2 << " " << dmin2 <<
      // std::endl;
      if (r2 < dmin2)
      {
        isThisOK = false;
          ...
          if (expa > epsx)
          {
            ...
            if (phase[j] * cpc > 0.2)
            {
              /*
                       ...
                       G4cout << "TKDB Check Pauli Principle phase[j]*cpc > 0.2 " << phase[j]*cpc <<
                 G4endl;
              */
              isThisOK = false;
              break;
            ...
            if ((phase_g[j] + phase[j]) * cpc > 0.5)
            {
              /*
                       ...
                       G4cout << "TKDB Check Pauli Principle B phase_g[j] + phase[j] ) * cpc > 0.5
                 " <<  ( phase_g[j] + phase[j] ) * cpc << G4endl;
              */
              isThisOK = false;
              break;
            }
            ...
            if (phase[i] * cpc > 0.3)
            {
              /*
                       ...
                       G4cout << "TKDB Check Pauli Principle C phase[i] * cpc > 0.3 " <<  phase[i] *
                 cpc << G4endl;
              */
              isThisOK = false;
              break;
...
G4bool G4LightIonQMDGroundStateNucleus::samplingPosition_cluster(G4int z)
{
  ...
  if (z == 6)
  {
    ...
    for (G4int j = 0; j < 3; j++)
    {
      G4double xx = (sca * scb * scg - ssa * ssg) * sub_x[j].x()
                    + (-sca * scb * ssg - ssa * scg) * sub_x[j].y() + sca * ssb * sub_x[j].z();
      G4double yy = (ssa * scb * scg + sca * ssg) * sub_x[j].x()
                    + (-ssa * scb * ssg + sca * scg) * sub_x[j].y() + ssa * ssb * sub_x[j].z();
      ...
      for (G4int i = 0; i < 4; i++)
      {
        xx = (ca * cb * cg - sa * sg) * base_x[i].x() + (-ca * cb * sg - sa * cg) * base_x[i].y()
             + ca * sb * base_x[i].z();
        yy = (sa * cb * cg + ca * sg) * base_x[i].x() + (-sa * cb * sg + ca * cg) * base_x[i].y()
             + sa * sb * base_x[i].z();
        ...
        // std::cout << GetParticipant(k)->GetChargeInUnitOfEplus() << " samplingPosition " << pos
        // << std::endl;
