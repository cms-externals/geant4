...
//
      double pincl = sqrt(pxrem*pxrem + pyrem*pyrem + pzrem*pzrem);
// PPRFP is in MeV/c
      double ETOT_incl = sqrt(pincl*pincl + (AAINCL * amu)*(AAINCL * amu));
...
// Velocities of IMF and partner: 1 denotes partner, 2 denotes IMF
               ...
               double V1 = sqrt(EkinR1/AFBU) * 1.3887;
               double V2 = sqrt(EkinR2/AIMFBU) * 1.3887;
               double VZ1_IMF = (2.0 * G4AblaRandom::flat() - 1.0) * V1;
               double VPERP1 = sqrt(V1*V1 - VZ1_IMF*VZ1_IMF);
...
// Decay of heavy partner
     double IINERTTOT = 0.40 * 931.490 * 1.160*1.160 *( pow(AIMFBU,5.0/3.0) + pow(AFBU,5.0/3.0)) + 931.490 * 1.160*1.160*AIMFBU*AFBU/(AIMFBU+AFBU)*(pow(AIMFBU,1./3.) + pow(AFBU,1./3.))*(pow(AIMFBU,1./3.) + pow(AFBU,1./3.));

     double JPRFHEAVY = BU_TAB[i][9] * 0.4 * 931.49 * 1.16*1.16 * pow(AFBU,5.0/3.0) / IINERTTOT;
     double JPRFLIGHT = BU_TAB[i][9] * 0.4 * 931.49 * 1.16*1.16 * pow(AIMFBU,5.0/3.0) / IINERTTOT;
...
//  Velocities of IMF and partner: 1 denotes partner, 2 denotes IMF
      ...
      double V1 = sqrt(EkinR1/af) * 1.3887;
      double V2 = sqrt(EkinR2/aimf) * 1.3887;
      double VZ1_IMF = (2.0 * G4AblaRandom::flat() - 1.0) * V1;
      double VPERP1 = sqrt(V1*V1 - VZ1_IMF*VZ1_IMF);
      double ALPHA1 = G4AblaRandom::flat() * 2. * 3.142;
      double VX1_IMF = VPERP1 * std::sin(ALPHA1);
...
// Decay of heavy partner
     double IINERTTOT = 0.40 * 931.490 * 1.160*1.160 *( pow(aimf,5.0/3.0) + pow(af,5.0/3.0)) + 931.490 * 1.160*1.160*aimf*af/(aimf+af)*(pow(aimf,1./3.) + pow(af,1./3.))*(pow(aimf,1./3.) + pow(af,1./3.));

     double JPRFHEAVY = jprf0 * 0.4 * 931.49 * 1.16*1.16 * pow(af,5.0/3.0) / IINERTTOT;
     double JPRFLIGHT = jprf0 * 0.4 * 931.49 * 1.16*1.16 * pow(aimf,5.0/3.0) / IINERTTOT;
...
// In the CM system
        if(gammadecay==0){
// Light particle
           double ETOT_LP = sqrt(pc*pc + amoins*amoins * mu2);
...
// Heavy residue
        if(gammadecay==0){
        double v2 = pow(EV_TEMP[IEV_TAB_TEMP][2],2.) +
             pow(EV_TEMP[IEV_TAB_TEMP][3],2.) +
             pow(EV_TEMP[IEV_TAB_TEMP][4],2.);
        double gamma = 1.0/sqrt(1.0 - v2 / (c*c));
...
// in case of gammas, EV_TEMP contains momentum components and not velocity
        ...
        double pteva = sqrt(pxeva*pxeva + pyeva*pyeva);
// To be checked:
        double etot = sqrt ( pleva*pleva + pteva*pteva + af*af * mu2 );
...
//std::cout << "z:" << BU_TAB[i][7]  << std::endl;
   ...
   // if(ia>0){
    ...
    /* }else{
     ...
     varntp->enerj[intp] = sqrt(BU_TAB[i][2]*BU_TAB[i][2]+BU_TAB[i][3]*BU_TAB[i][3]+BU_TAB[i][4]*BU_TAB[i][4]);
     }
*/
    intp++;
    }
...
// Heavy residue
        if(gammadecay==0){
        double v2 = pow(EV_TAB_SSC[IEV_TAB_SSC][2],2.) +
...
//    Velocities
      ...
      double VPERP1 = sqrt(VFF1*VFF1 - VZ1_FISSION*VZ1_FISSION);
      double ALPHA1 = G4AblaRandom::flat() * 2. * 3.142;
