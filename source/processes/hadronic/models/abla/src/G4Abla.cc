...
//
      G4double pincl = sqrt(pxrem*pxrem + pyrem*pyrem + pzrem*pzrem);
// PPRFP is in MeV/c
      G4double ETOT_incl = sqrt(pincl*pincl + (AAINCL * amu)*(AAINCL * amu));
...
*/
        if(T_freeze_out_in >= 0.0){
         ...
         T_freeze_out = max(9.33*exp(-0.00282*AAINCL),5.5);
//         ! See: J. Natowitz et al, PRC65 (2002) 034618
//        T_freeze_out=DMAX1(9.0D0*DEXP(-0.001D0*AAABRA),
//     &                     5.5D0)
        }
//
        a_tilda = ald->av*aprf + ald->as*pow(aprf,2.0/3.0) + ald->ak*pow(aprf,1.0/3.0);

        T_init = sqrt(EINCL/a_tilda);
...
// have changed (AK).
            ...
            a_tilda = ald->av*A_FINAL + ald->as*pow(A_FINAL,2.0/3.0) + ald->ak*pow(A_FINAL,1.0/3.0);
...
// e.g. Sfienti et al, NPA 2007)
          ...
          a_tilda = ald->av*AAINCL + ald->as*pow(AAINCL,2.0/3.0) + ald->ak*pow(AAINCL,1.0/3.0);
...
// and not excitation energy.
           if(BU_TAB[i][0]>2.0){
            a_tilda_BU = ald->av*BU_TAB[i][1] + ald->as*pow(BU_TAB[i][1],2.0/3.0) + ald->ak*pow(BU_TAB[i][1],1.0/3.0);
...
// "Heavy" residue
        ...
        PPRFP = sqrt(PXPRFP*PXPRFP + PYPRFP*PYPRFP + PZPRFP*PZPRFP);
// ********************************************************
// PPRFP is in MeV/c
        ETOT_PRF = sqrt(PPRFP*PPRFP + (aprf * amu)*(aprf * amu));
...
//       For bu products:
          ...
          P_BU = sqrt(PX_BU*PX_BU + PY_BU*PY_BU + PZ_BU*PZ_BU);
// *******************************************************
//        PPRFP is in MeV/c
          ETOT_BU = sqrt(P_BU*P_BU + (BU_TAB[I_Breakup][1]*amu)*(BU_TAB[I_Breakup][1]*amu));
...
//   In the frame of source (i.e. prefragment after abrasion or INCL)
        P_BU_SUM = sqrt(PX_BU_SUM*PX_BU_SUM + PY_BU_SUM*PY_BU_SUM +
                   PZ_BU_SUM*PZ_BU_SUM);
// ********************************************************
// PPRFP is in MeV/c
        ETOT_SUM = sqrt(P_BU_SUM*P_BU_SUM +
...
// Velocities of IMF and partner: 1 denotes partner, 2 denotes IMF
               ...
               G4double V1 = sqrt(EkinR1/AFBU) * 1.3887;
               G4double V2 = sqrt(EkinR2/AIMFBU) * 1.3887;
               G4double VZ1_IMF = (2.0 * G4AblaRandom::flat() - 1.0) * V1;
               G4double VPERP1 = sqrt(V1*V1 - VZ1_IMF*VZ1_IMF);
...
// Decay of heavy partner
     G4double IINERTTOT = 0.40 * 931.490 * 1.160*1.160 *( pow(AIMFBU,5.0/3.0) + pow(AFBU,5.0/3.0)) + 931.490 * 1.160*1.160*AIMFBU*AFBU/(AIMFBU+AFBU)*(pow(AIMFBU,1./3.) + pow(AFBU,1./3.))*(pow(AIMFBU,1./3.) + pow(AFBU,1./3.));

     G4double JPRFHEAVY = BU_TAB[i][9] * 0.4 * 931.49 * 1.16*1.16 * pow(AFBU,5.0/3.0) / IINERTTOT;
     G4double JPRFLIGHT = BU_TAB[i][9] * 0.4 * 931.49 * 1.16*1.16 * pow(AIMFBU,5.0/3.0) / IINERTTOT;
...
//  Velocities of IMF and partner: 1 denotes partner, 2 denotes IMF
      ...
      G4double V1 = sqrt(EkinR1/af) * 1.3887;
      G4double V2 = sqrt(EkinR2/aimf) * 1.3887;
      G4double VZ1_IMF = (2.0 * G4AblaRandom::flat() - 1.0) * V1;
      G4double VPERP1 = sqrt(V1*V1 - VZ1_IMF*VZ1_IMF);
      G4double ALPHA1 = G4AblaRandom::flat() * 2. * 3.142;
      G4double VX1_IMF = VPERP1 * std::sin(ALPHA1);
...
// Decay of heavy partner
     G4double IINERTTOT = 0.40 * 931.490 * 1.160*1.160 *( pow(aimf,5.0/3.0) + pow(af,5.0/3.0)) + 931.490 * 1.160*1.160*aimf*af/(aimf+af)*(pow(aimf,1./3.) + pow(af,1./3.))*(pow(aimf,1./3.) + pow(af,1./3.));

     G4double JPRFHEAVY = jprf0 * 0.4 * 931.49 * 1.16*1.16 * pow(af,5.0/3.0) / IINERTTOT;
     G4double JPRFLIGHT = jprf0 * 0.4 * 931.49 * 1.16*1.16 * pow(aimf,5.0/3.0) / IINERTTOT;
...
// Velocities of IMF and partner: 1 denotes partner, 2 denotes IMF
      ...
      V1 = sqrt(EkinR1/af) * 1.3887;
      V2 = sqrt(EkinR2/aimf) * 1.3887;
      G4double VZ1_IMFS = (2.0 * G4AblaRandom::flat() - 1.0) * V1;
             VPERP1 = sqrt(V1*V1 - VZ1_IMFS*VZ1_IMFS);
...
// Decay of heavy partner
      IINERTTOT = 0.40 * 931.490 * 1.160*1.160 *( pow(aimf,5.0/3.0) + pow(af,5.0/3.0)) + 931.490 * 1.160*1.160*aimf*af/(aimf+af)*(pow(aimf,1./3.) + pow(af,1./3.))*(pow(aimf,1./3.) + pow(af,1./3.));

      JPRFHEAVY = jprf1 * 0.4 * 931.49 * 1.16*1.16 * pow(af,5.0/3.0) / IINERTTOT;
      JPRFLIGHT = jprf1 * 0.4 * 931.49 * 1.16*1.16 * pow(aimf,5.0/3.0) / IINERTTOT;
...
// to pairing shift in Fermi-gas model (there, energy is shifted taking odd-odd nuclei as bassis)
              ...
              parite(j+i,&para);
                if(para<0.0){
// e-o, o-e
                 e0 =  0.285+11.17*pow(j+i,-0.464) -0.390-0.00058*(j+i);
                }else{
                  G4double parz=0.;
                  parite(i,&parz);
                  if (parz>0.0){
// e-e
                   e0 = 22.34*pow(j+i,-0.464)-0.235;
                  }else{
// o-o
                   e0 = 0.0;
                  }
...
G4double G4Abla::fissility(G4int a, G4int z, G4int ny, G4double sn, G4double slam, G4int optxfis)
{
  ...
  zz = G4double(z);
  i  = G4double(a-2*z) / aa;
  z2a= zz*zz/aa-ny*(1115.-939.+sn-slam)/(0.7053*pow(a,2./3.));
...
//
  ...
  if(sortie==1){
   if (probn!=0.0) {
    ...
    zmoins = 1.0;
    epsiln = sp + ecp;
    pc = sqrt(pow((1.0 + ecp/9.3827e2),2.) - 1.0) * 9.3827e2;
    malpha = 0.0;
    fgamma = 0;
    fimf = 0;
    flamb0decay=0;
    ...
    zmoins = 1.0;
    epsiln = sd + ecd;
    pc = sqrt(pow((1.0 + ecd/1.875358e3),2) - 1.0) * 1.875358e3;
    malpha = 0.0;
    fgamma = 0;
    fimf = 0;
    flamb0decay=0;
    ...
    zmoins = 1.0;
    epsiln = st + ect;
    pc = sqrt(pow((1.0 + ect/2.80828e3),2) - 1.0) * 2.80828e3;
    malpha = 0.0;
    fgamma = 0;
    fimf = 0;
    flamb0decay=0;
    ...
    zmoins = 2.0;
    epsiln = she + eche;
    pc = sqrt(pow((1.0 + eche/2.80826e3),2) - 1.0) * 2.80826e3;
    malpha = 0.0;
    fgamma = 0;
    fimf = 0;
    flamb0decay=0;
    ...
    zmoins = 2.0;
    epsiln = sa + eca;
    pc = sqrt(pow((1.0 + eca/3.72834e3),2) - 1.0) * 3.72834e3;
    malpha = 4.0;
    fgamma = 0;
    fimf = 0;
    flamb0decay=0;
  ...
  if (x < proba) {
    ...
    pc = sqrt(pow((1.0 + (eclamb0)/11.1568e2),2.) - 1.0) * 11.1568e2;
    malpha = 0.0;
    fgamma = 0;
    fimf = 0;
    ff = 0;
...
// Determination of x,y,z components of momentum from known emission momentum PC
      if(ff==0 && fimf==0){
        ...
        stet1 = sqrt(1.0 - pow(ctet1,2)); // component perpendicular to z
        rnd = G4AblaRandom::flat();
...
// In the CM system
        if(gammadecay==0){
// Light particle
           G4double ETOT_LP = sqrt(pc*pc + amoins*amoins * mu2);
           if(flamb0decay==1)ETOT_LP = sqrt(pc*pc + 1115.683*1115.683);
...
// Heavy residue
        if(gammadecay==0){
        G4double v2 = pow(EV_TEMP[IEV_TAB_TEMP][2],2.) +
             pow(EV_TEMP[IEV_TAB_TEMP][3],2.) +
             pow(EV_TEMP[IEV_TAB_TEMP][4],2.);
        G4double gamma = 1.0/sqrt(1.0 - v2 / (c*c));
...
// in case of gammas, EV_TEMP contains momentum components and not velocity
        ...
        G4double pteva = sqrt(pxeva*pxeva + pyeva*pyeva);
// To be checked:
        G4double etot = sqrt ( pleva*pleva + pteva*pteva + af*af * mu2 );
...
//
    if(NbLam0>0){
     ef = ef + 0.51*(1115.-938.+sn-slamb0)/pow(a,2./3.);
    }
//
// Set fission barrier
//
...
//  AK 2007 - Now DENSNIV called with correct BS, BK
  ...
  iinert = 0.4 * 931.49 * 1.16*1.16 * pow(a,5.0/3.0)*(1.0 + 0.5*sqrt(5./(4.*pi))*defbet);
...
//C Here BS=BK=1, as this was assumed in the parameterization
         pa = (ald->av)*a + (ald->as)*pow(a,2./3.) + (ald->ak)*pow(a,1./3.);
         gamma = 2.5 * pa * pow(a,-4./3.);
         gfactor = 1.+gamma*ecld->ecgnz[in][iz];
         if(gfactor<=0.){
          gfactor = 0.0;
         }
//
         gtemp = 17.60/(pow(a,0.699) * sqrt(gfactor));
         ecg = 4.0 * gtemp;
//
         goto direct70;
  }
...
// JLRS Nov-2016 - Added these caculations in abla++
  ...
  if (in >= 2) {
    ...
    iinert = 0.4 * 931.49 * 1.16*1.16 * pow(a-1.,5.0/3.0)*(1.0 + 0.5*sqrt(5./(4.*pi))*defbet);
...
// Reduction of angular momentum due to orbital angular momentum of emitted fragment
  if ((in >= 2) && (iz >= 2)) {
    ...
    iinert = 0.4 * 931.49 * 1.16*1.16 * pow(a-2.,5.0/3.0)*(1.0 + 0.5*sqrt(5./(4.*pi))*defbet);
...
// Reduction of angular momentum due to orbital angular momentum of emitted fragment
  if ((in >= 3) && (iz >= 2)) {
    ...
    iinert = 0.4 * 931.49 * 1.16*1.16 * pow(a-3.,5.0/3.0)*(1.0 + 0.5*sqrt(5./(4.*pi))*defbet);
...
// Reduction of angular momentum due to orbital angular momentum of emitted fragment
  if ((in >= 3) && (iz >= 3)) {
    ...
    iinert = 0.4 * 931.49 * 1.16*1.16 * pow(a-4.,5.0/3.0)*(1.0 + 0.5*sqrt(5./(4.*pi))*defbet);
...
//      *** Find the limits that both IMF and partner are bound :
        ...
        for(G4int iaimf=izimf+inmin;iaimf<=izimf+inmax;iaimf++){
         ...
         if(aimf>=a || zimf>=zprf){
          ...
          iinert= 0.40 * 931.490 * 1.160*1.160 * pow(a,5.0/3.0)*(pow(aimf,5.0/3.0) + pow(a - aimf,5.0/3.0)) + 931.490 * 1.160*1.160 * aimf * (a-aimf) / a *(pow(aimf,1.0/3.0) + pow(a - aimf,1.0/3.0))*(pow(aimf,1.0/3.0) + pow(a - aimf,1.0/3.0));
...
// is equal to 1.
           width_imf = 0.0;
         //
           width_imf = width(a,zprf,aimf,zimf,timf,bimf,sbimf,ee-erot)*exp(-imfarg)*qr/qrcn;
          }// if densg
         }// if aimf
...
// is equal to 1.
           width_imf = 0.0;
           width_imf = width(a,zprf,aimf,zimf,timf,bimf,sbimf,ee-erot)*exp(-imfarg)*qr/qrcn;//*densimf/densg;
          }// if densg
         }// if aimf
...
//
         a_imf = gimf3 / pow(3.0,b_imf);
         gimf = a_imf * ( pow(zprf,b_imf+1.0) - pow(3.0,b_imf+1.0)) /(b_imf + 1.0);
        }
...
//C If one switches gammas off, one should also switch off tunneling through the fission barrier.
      gg = 0.624e-9*pow(a,1.6)*pow(gtemp,5.);
//gammaemission==1
...
//
   if(e<0.0&&ifis!=1){  // TUNNELING
        ...
        if(ald->optafan == 1) {
         ...
         pa = (ald->av)*a + (ald->as)*bsin*pow(a,(2.e0/3.e0)) + (ald->ak)*bkin*pow(a,(1.e0/3.e0));
        }
        gamma = 2.5 * pa * pow(a,-4.0/3.0);
        fecor=0.0;
        goto densniv100;
   }
//
...
// - With increasing excitation energy system in getting less and less deformed:
     ...
     if(ponq>700.0) ponq = 700.0;
       bs = 1.0/(1.0+exp(-ponq)) + 1.0/(1.0+exp(ponq)) * bsin;
       bk = 1.0/(1.0+exp(-ponq)) + 1.0/(1.0+exp(ponq)) * bkin;
   }
//
  // level density parameter                                               
  if(ald->optafan == 1) {
...
// determines critical energy for pairing.
  if(a>0.0){
   ecr = pa*17.60/(pow(a,0.699) * sqrt(1.0+gamma*BSHELLCT))*17.60/(pow(a,0.699) * sqrt(1.0+gamma*BSHELLCT));
  }
  ...
  if (optshp > 0) {
    ...
    //deltpp = -0.25e0* (delta0/pow(sqrt(a),2)) * pa /pi6 + 2.e0*delta0/sqrt(a);
      deltpp = -0.25e0* pow((delta0/sqrt(a)),2) * pa /pi6 + 22.34e0*pow(a,-0.464)-0.235;
      // Odd A
      if (IPARITE == 1) {
	//e = e - delta0/sqrt(a);
        e=e-(0.285+11.17*pow(a,-0.464)-0.390-0.00058*a);//-30./a;//FIXME
      }
      // Even Z, even N
      if(IPARITE==2){ 
           e=e-(22.34*pow(a,-0.464)-0.235);//-30./a;//FIXME
      }
      // Odd Z, odd N
      if(IPARITE==0){
        if(in==iz){
  ...
  if (ee < 5.0) {
    y1 = sqrt(pa*fecor);
    for(G4int j = 0; j < 5; j++) {
      y2 = pa*fecor*(1.e0-exp(-y1));
      y1 = sqrt(y2);
    }
    y0 = pa/y1;
    ftemp=1.0/y0;
    fdens = exp(y0*fecor)/ (pow((pow(fecor,3)*y0),0.5)*pow((1.0-0.5*y0*fecor*exp(-y1)),0.5))* exp(y1)*(1.0-exp(-y1))*0.1477045;
    if (fecor < 1.0) {
      ecor1=1.0;
      y11 = sqrt(pa*ecor1);
      for(G4int j = 0; j < 7; j++) {
	...
	y11 = sqrt(y21);
      ...
      y01 = pa/y11;
      fdens = fdens*pow((y01/y0),1.5);
      ftemp = ftemp*pow((y01/y0),1.5);
    }
  }
  else {
    ponniv = 2.0*sqrt(pa*fecor);
    if (ponniv > 700.0) {
      ponniv = 700.0;
    }
    // fermi gas state density                                               
    fdens = 0.1477045 * exp(ponniv)/(pow(pa,0.25)*pow(fecor,1.25));
    ftemp = sqrt(fecor/pa);
  }
//
  densfm = fdens;
...
//
  ...
  tempct = 17.60/( pow(a,0.699) * sqrt(1.+gamma*BSHELLCT));
...
//  as bassis)
// e-o, o-e
            if (IPARITE == 1) { e0 = 0.285+11.17*pow(a,-0.464) - 0.390-0.00058*a;}
// e-e
            if (IPARITE == 2) { e0 = 22.34*pow(a,-0.464)-0.235;}
// o-o
            if (IPARITE == 0){ e0 = 0.0;}
        ...
        densct = exp(ponniv)/tempct*exp(0.079*BSHELLCT/tempct);
...
// Parametrization of CT model by Ignatyuk done for masses > 20
         ftemp = 17.60/( pow(a,0.699) * sqrt(1.0+gamma*BSHELLCT));
...
c in units 1/MeV
*/
 fnorm = pow(1.16,2)*931.49*1.e-2/(9.0* pow(6.582122,2));
...
*/
    fp_per = 0.4*pow(a,5.0/3.0)*fnorm*(1.0+0.50*defbet*sqrt(5.0/(4.0*pi)));
    fp_par = 0.40*pow(a,5.0/3.0)*fnorm*(1.0-defbet*sqrt(5.0/(4.0*pi)));
...
*/
    fp_per = 2.0/5.0*pow(a,5.0/3.0)*fnorm*(1.0+7.0/6.0*defbet*(1.0+1396.0/255.0*defbet));
// Parallel moment of inertia
    fp_par = 2.0/5.0*pow(a,5.0/3.0)*fnorm*(1.0-7.0/3.0*defbet*(1.0-389.0/255.0*defbet));
  }else{
   if(ifis==20){
...
// Perpendicular moment of inertia
   fp_per = 0.4*pow(a,5.0/3.0)*fnorm*3.50*(1.0 + pow(defbet,5.))/pow(1.0 + defbet*defbet*defbet,5.0/3.0);
   fp_par = 0.4*pow(a,5.0/3.0)*fnorm*(1.0 + pow(defbet,5.0))/pow(1.0 + defbet*defbet*defbet,5.0/3.0);
   }
  }
 }
...
//
  sig_per = sqrt(fp_per * ftemp);
  sig_par = sqrt(fp_par * ftemp);
//
  sigma2 = sig_per*sig_per + sig_par*sig_par;
  jfact = (2.*jprf+1.)*exp(-1.*jprf*(jprf+1.0)/(2.0*sigma2))/(sqrt(8.0*3.1415)*pow(sigma2,1.5));
  erot = jprf*jprf/(2.0*sqrt(fp_par*fp_par+fp_per*fp_per));
//
...
G4double G4Abla::eflmac(G4int ia, G4int iz, G4int flag, G4int optshp)
{
  ...
  ff = -1.0/8.0*rp*rp*esq/pow(r0,3) * (145.0/48.0 - 327.0/2880.0*pow(kf,2) * pow(rp,2) + 1527.0/1209600.0*pow(kf,4) * pow(rp,4));
  i   = (n-z)/a;
  ...
  efl = -1.0 * av*(1.0 - kv*i*i)*a + as*(1.0 - ks*i*i)*b1 * pow(a,(2.0/3.0)) + a0
    ...
    + ff*pow(z,2)/a -ca*(n-z) - ael * pow(z,(2.39e0));
...
// as bassis)
    ...
    if(para<0.0){
// e-o, o-e
      e0 =  0.285+11.17*pow(a,-0.464) -0.390-0.00058*(a);
    }else{
        G4double parz=0.;
        parite(z,&parz);
        if (parz>0.0){
// e-e
         e0 = 22.34*pow(a,-0.464)-0.235;
        }else{
// o-o
         e0 = 0.0;
        }
...
G4double G4Abla::tau(G4double bet, G4double homega, G4double ef, G4double t)
{
  ...
  if (bet/(sqrt(2.0)*10.0*(homega/6.582122)) <= 1.0) {
    tauResult = log(10.0*ef/t)/(bet*1.0e21);
  }
  else {
...
//
      MR02 = pow(AF,5.0/3.0)*1.0340*0.01*1.175*1.175;
      MINERT = 3.*MR02/10.0;
      C = 17.9439*(1.-1.7826*pow((AF-2.0*ZF)/AF,2));
      fk1 = 0.4*C*pow(AF,2.0/3.0)-0.1464*pow(ZF,2)/pow(AF,1./3.);
...
void G4Abla::barfit(G4int iz, G4int ia, G4int il, G4double *sbfis, G4double *segs, G4double *selmax)
{
  ...
  if(el <= sel20) {
    // low l              
    q = 0.2/(pow(sel20,2)*pow(sel80,2)*(sel20-sel80));
    qa = q*(4.0*pow(sel80,3) - pow(sel20,3));
    qb = -q*(4.0*pow(sel80,2) - pow(sel20,2));
    bfis = bfis*(1.0 + qa*pow(el,2) + qb*pow(el,3));
  }
  else {
    // high l             
    aj = (-20.0*pow(x,5) + 25.e0*pow(x,4) - 4.0)*pow((y-1.0),2)*y*y;
    ak = (-20.0*pow(y,5) + 25.0*pow(y,4) - 1.0) * pow((x-1.0),2)*x*x;
    q = 0.2/(pow((y-x)*((1.0-x)*(1.0-y)*x*y),2));
    qa = q*(aj*y - ak*x);
    qb = -q*(aj*(2.0*y + 1.0) - ak*(2.0*x + 1.0));
    z = el/(*selmax);
    a1 = 4.0*pow(z,5) - 5.0*pow(z,4) + 1.0;
    a2 = qa*(2.e0*z + 1.e0);
...
-1.231739572450155,0.1208650973866179e-2,-0.5395239384953e-5};
 ...
 return fgammln=tmp+log(stp*ser/x);
}
...
G4double G4Abla::f(G4double E)
{
  ...
  return (1.0 - (E + 1.0) * exp(-E));
}

G4double G4Abla::fmaxhaz(G4double x)
{
 return ( -x*log(G4AblaRandom::flat()) -x*log(G4AblaRandom::flat()) -x*log(G4AblaRandom::flat()) ) ;
}
...
void G4Abla::FillData(G4int IMULTBU,G4int IEV_TAB){
    ...
    G4double gamma = sqrt(1.0 - v2 / (c*c));
    ...
    varntp->itypcasc[intp] = 0;
     ...
     varntp->enerj[intp] = sqrt(EV_TAB[i][2]*EV_TAB[i][2]+EV_TAB[i][3]*EV_TAB[i][3]+EV_TAB[i][4]*EV_TAB[i][4]);
     }
    intp++;
    }
// 
...
// in m
      ...
      RGEOM = R0 * (pow(APART,1.0/3.0)+pow(AMOTHER-APART,1.0/3.0));

// in m*sqrt(MeV)
      AKONST = HBAR*sqrt(1.0 / MU);
...
// USING ANALYTICAL APPROXIMATION
      ...
      INT1 = 2.0 * pow(TEMP,3.) / (2.0 * TEMP + B);

      ARG = sqrt(B/TEMP);
      EARG = (erf(ARG) - 1.0);
      if(std::abs(EARG)<1.e-9) EARG = 0.0;
      if(B==0.0){
        INT2 = 0.5 * sqrt(PI) * pow(TEMP,3.0/2.0);
      }else{
         AEXP = B/TEMP;
          if(AEXP>700.0) AEXP = 700.0;
         INT2 = (2.0*B*B +TEMP*B)/sqrt(B) + exp(AEXP) * sqrt(PI/(4.0*TEMP))*(4.0*B*B+4.0*B*TEMP - TEMP*TEMP) *EARG;
...
// Only QM contribution (Rgeom -> Rgeom + Rlamda) seems to be too strong for neutrons
       fwidth = PI * BKONST *  G * sqrt((RGEOM * RGEOM * INT1 + 2.0 * AKONST * RGEOM * INT2 + AKONST * AKONST * INT3) * RGEOM * RGEOM * INT1);
...
// numerical solution for energies close to the particle threshold:
       if(IZPART<3.0){
        if(AMOTHER<155.0){
         PAR_A1=exp(2.302585*0.2083*exp(-0.01548472*AMOTHER))-0.05;
       ...
       FACT = exp((2.302585*PAR_A1*exp(-PAR_B1*(EXC-SB))));
       if(FACT<1.0) FACT = 1.0;
       if(IZPART<-1.&&ZMOTHER<151.0){
//       IF(IZPART.LT.1)THEN
        fwidth = fwidth / sqrt(FACT);
       }else{
        fwidth = fwidth / FACT;
       }
       }//if IZPART<3.0
...
// alphaN = sqrt(2*N/(4*pi))*BetaN
      G4double ALPHA2 = sqrt(5.0/(4.0*PI))*ecld->beta2[IN][IZ];
      G4double ALPHA4 = sqrt(9.0/(4.0*PI))*ecld->beta4[IN][IZ];
...
return (3.0 * T * pow(-1.*log(G4AblaRandom::flat()) * log(G4AblaRandom::flat())*log(G4AblaRandom::flat()),0.333333));
}
...
// For the overdamped regime BET**2 > 4*OMEGA**2
         if((bet*bet)>4.0*OMEGA_GS*OMEGA_GS){
          BET1=sqrt(bet*bet-4.0*OMEGA_GS*OMEGA_GS);
//
...
// and the expression with sinh and cosh can be transformed in one with sin and cos
          BET1=sqrt(4.0*OMEGA_GS*OMEGA_GS-bet*bet);
...
//     larger than what predicted by the liquid drop model, because of
//     shell effects.
           t_0 = (log(2.*FT/(2.*FT-HOMEGA/16.)))/(BET*1.e21);
         }else{
...
// This is the probability to survive the decay at this step
                 REAC_PROB = exp(-1.0*STEP_LENGTH*LAMBDA);
...
// than pairing gap (no levels at which system can be)
      ...
      E1 = EF + HOMEGA1/2.0/PI*log(HOMEGA1*(2.0*PI+HOMEGA2)/4.0/PI/PI);

      E2 = EF + HOMEGA2/(2.0*PI)*log(1.0+2.0*PI/HOMEGA2);
...
// around fission barrier.
      ...
      CORR_FUNCT = HOMEGA1 * (1.0-1.0/(1.0+exp(EXP_FACT)));
      ...
      if(EE<E1){
      GFTUN = FACT1*(exp(EE/TEMP)*exp(2.0*PI*(EE-EF)/HOMEGA1)-exp(-2.0*PI*EF/HOMEGA1));
      }else{
        if(EE>=E1&&EE<E2){
         GFTUN = exp(EE/TEMP)*(0.50+FACT2*(EE-EF-TEMP))-exp(E1/TEMP)*(0.5+FACT2*(E1-EF-TEMP))+FACT1*(exp(E1/TEMP)*exp(2.0*PI*(E1-EF)/HOMEGA1)-exp(-2.0*PI*EF/HOMEGA1));
        }else{
         GFTUN = exp(EE/TEMP)*(1.0+FACT3*exp(-2.0*PI*(EE-EF)/HOMEGA2))-exp(E2/TEMP)*(1.0+FACT3*exp(-2.0*PI*(E2-EF)/HOMEGA2))+exp(E2/TEMP)*(0.5+FACT2*(E2-EF-TEMP))-exp(E1/TEMP)*(0.5+FACT2*(E1-EF-TEMP))+FACT1*(exp(E1/TEMP)*exp(2.0*PI*(E1-EF)/HOMEGA1)-exp(-2.0*PI*EF/HOMEGA1));
        }
      }
      GFTUN = GFTUN/exp(EE/TEMP)*DENSF*ENH_FACT/DENSG/2.0/PI;
...
// Perpendicular moment of inertia
      FP_PER = 2.0/5.0*pow(A,5.0/3.0)*FNORM*(1. + 7.0/6.0*Y*(1.0+1396.0/255.*Y));
...
// Parallel moment of inertia
      FP_PAR = 2.0/5.0*pow(A,5.0/3.0)*FNORM*(1.0 - 7.0/3.0*Y*(1.0-389.0/255.0*Y));
      if(FP_PAR<0.0) FP_PAR = 0.0;

      EROT = JPR * JPR / (2.0 * sqrt(FP_PAR*FP_PAR + FP_PER*FP_PER));
      if(IEROT==1) EROT = 0.0;

// Perpendicular spin cut-off parameter
      SIG_PER_SP = sqrt(FP_PER * ftemp);

      if(SIG_PER_SP<1.0) SIG_PER_SP = 1.0;

// Parallel spin cut-off parameter
      SIG_PAR_SP = sqrt(FP_PAR * ftemp);
      ENH_FACT = 1.0;
//
        if(A>223.0){
...
// Barrier A is axial asymmetric
       ENH_FACTA = sqrt(8.0*PI) * SIG_PER_SP*SIG_PER_SP * SIG_PAR_SP;
...
// super-heavy nuclei  (only barrier A plays a role, axial asymmetric)
          ENH_FACT =  sqrt(8.0*PI) * SIG_PER_SP*SIG_PER_SP* SIG_PAR_SP;
        }
      }
...
// Fading-out according to Junghans:
      ENH_FACT = 1.0/(1.0+exp(PONFE))*ENH_FACT+1.0;
...
*/
    if (EEFINAL<=0.01) EEFINAL = 0.01;
        ...
        ALEVDENS = 0.073*AMOTHER + 0.095*pow(AMOTHER,2.0/3.0);
        ...
        THETA_MOTHER = 0.0111 * pow(AMOTHER,1.66667);
        THETA_ORBITAL = 0.0323 / pow(AMOTHER,2.) *pow(pow(AFRAGMENT,0.33333) + pow(ADAUGHTER,0.33333),2.) * AFRAGMENT*ADAUGHTER*(AFRAGMENT+ADAUGHTER);

        *LORBITAL = -1.* THETA_ORBITAL * (LMOTHER / THETA_MOTHER + sqrt(S4FINAL) /(ALEVDENS*LMOTHER));

        *SIGMA_LORBITAL = sqrt(sqrt(S4FINAL) * THETA_ORBITAL / ALEVDENS);
...
G4double G4Abla::fvmaxhaz_neut(G4double x){
 ...
 return (2.0 * x * sqrt(log(G4AblaRandom::flat()) * log(G4AblaRandom::flat())));
}
...
// take the half of the CN and transform it in integer (floor it)
  ...
  IINERT = 0.4 * 931.49 * 1.16*1.16 * pow(ACN,5.0/3.0)*(1.0 + 0.5*sqrt(5./(4.*PI))*DEFBET);
...
// *****  Width *********************
          ...
          IINERT = 0.40 * 931.490 * 1.160*1.160 * pow(ACN,5.0/3.0)*(pow(AIMF_1,5.0/3.0) + pow(ACN - AIMF_1,5.0/3.0)) + 931.490 * 1.160*1.160 * AIMF_1 * (ACN-AIMF_1) / ACN *(pow(AIMF_1,1.0/3.0) + pow(ACN - AIMF_1,1.0/3.0))*(pow(AIMF_1,1.0/3.0) + pow(ACN - AIMF_1,1.0/3.0));
...
//     &                ACN**(2.D0/3.D0)
           ...
           WIDTH_IMF = WIDTH1 * exp(-IMFARG) * QR / QRCN;
           ...
           if(WIDTH_IMF<=0.0){
            ...
            std::cout << "DEXP(-IMFARG) = " << exp(-IMFARG) << std::endl;
...
//     End loop to calculate where gamma of IMF has the minimum
      ...
      for(G4int II_ZIMF = IZSTOP;II_ZIMF<=IZIMFMAX;II_ZIMF++){
       SDW[II_ZIMF] =  pow(10.0,A2PAR) * pow(1.0*II_ZIMF,A1PAR);  // Power-low
...
// in sec
      tau0 = 1.0e-21;
      a0 = 0.66482503 - 3.4678935 * exp(-0.0104002*ee);
      a1  = 5.6846e-04 + 0.00574515 * exp(-0.01114307*ee);
      tauf = (a0 + a1 * zf*zf/pow(af,0.3333333)) * tau0;
//
      post10:
...
// Heavy residue
        if(gammadecay==0){
        G4double v2 = pow(EV_TAB_SSC[IEV_TAB_SSC][2],2.) +
             pow(EV_TAB_SSC[IEV_TAB_SSC][3],2.) +
             pow(EV_TAB_SSC[IEV_TAB_SSC][4],2.);
        G4double gamma = 1.0/sqrt(1.0 - v2 / (c*c));
        G4double etot_lp = amoins*mu * gamma;
...
// in case of gammas, EV_TEMP contains momentum components and not velocity
        ...
        pteva = sqrt(pxeva*pxeva + pyeva*pyeva);
// To be checked:
        etot = sqrt ( pleva*pleva + pteva*pteva + af*af * mu2 );
...
G4double G4Abla::getdeltabinding(G4double A, G4int H){
 if(A<1.)return (1.*H)/A*(10.68*A-21.27*pow(A,2./3.))*10.;
 return (1.*H)/A*(10.68*A-21.27*pow(A,2./3.));
}
...
c         versa.  */
      ...
      gamma = A_levdens / (0.40 * pow(A,1.3333)) * FGAMMA;
      A_levdens_heavy1 = Aheavy1 / xLevdens;
      gamma_heavy1 = A_levdens_heavy1 / (0.40 * pow(Aheavy1,1.3333)) * FGAMMA * FGAMMA1;
      A_levdens_heavy2 = Aheavy2 / xLevdens;
      gamma_heavy2 = A_levdens_heavy2 / (0.40 * pow(Aheavy2,1.3333)) * FGAMMA;

//     Energy dissipated from saddle to scission
//     F. Rejmund et al., Nucl. Phys. A 678 (2000) 215, fig. 4 b    */
      E_saddle_scission = (-24. + 0.02227 * Z*Z/pow(A,0.33333))*Friction_factor;
...
//     MassCurv taken at scission.    */
      ...
      if( Z2_over_A_eff< 34.0 )
        MassCurv_scis = pow(10., -1.093364 + 0.082933 * Z2_over_A_eff - 0.0002602 * Z2_over_A_eff*Z2_over_A_eff);
      else
        MassCurv_scis = pow(10., 3.053536 - 0.056477 * Z2_over_A_eff+ 0.0002454 * Z2_over_A_eff*Z2_over_A_eff );
...
//     replace then (all) cN_symm by cN_symm_saddle (at least for Yields)
      ...
      cN_symm      = 8.0 / pow(N,2.) * MassCurv_scis;
      cN_symm_sadd = 8.0 / pow(N,2.) * MassCurv_sadd;
...
//    calculate widhts at the saddle
      ...
      wNasymm1_saddle =
        sqrt(0.50 * sqrt(1.0/A_levdens*E_eff1_saddle) /
        (cN_asymm1_shell *
      Uwash(epsilon_1_saddle/A*Aheavy1,Ecrit,FREDSHELL,gamma_heavy1)+
        cN_symm_sadd));
      ...
      wNasymm2_saddle =
        sqrt(0.50 * sqrt(1.0/A_levdens*E_eff2_saddle) /
        (cN_asymm2_shell *
       Uwash(epsilon_2_saddle/A*Aheavy2,Ecrit,FREDSHELL,gamma_heavy2)+
        cN_symm_sadd));
      ...
      wNsymm_saddle =
          sqrt(0.50 * sqrt(1.0/A_levdens*E_eff0_saddle) /
                    cN_symm_sadd);

      if(epsilon_symm_scission > 0.0 ){
        E_HELP = max(E_saddle_scission,epsilon_symm_scission);
        wNsymm_scission =
        sqrt(0.50 * sqrt(1.0/A_levdens*(E_HELP)) /
                    cN_symm);
      }else{
        wNsymm_scission =
        sqrt(0.50 * sqrt(1.0/A_levdens*E_saddle_scission) /
                    cN_symm);
      }
...
//    fits of ref. Beizin 1991 (Plots by Sergei Zhdanov) 
      ...
      if( E_saddle_scission == 0.0 ){
        ...
        if( Nheavy1_eff > 75.0 ){
          wNasymm1_scission = sqrt(21.0)*N/A;
...
//     mass and charge of fragments using UCD, needed for level densities
      ...
      gamma_heavy1 = A_levdens_heavy1 / (0.40 * pow(Aheavy1_eff,1.3333)) * FGAMMA * FGAMMA1;
      gamma_heavy2 = A_levdens_heavy2 / (0.40 * pow(Aheavy2_eff,1.3333)) * FGAMMA;

      if( epsilon_symm_saddle < A_levdens * hbom3*hbom3)
        Ssymm = 2.0 * sqrt(A_levdens*A_levdens * hbom3*hbom3) +
      ...
      if( epsilon0_1_saddle < A_levdens * hbom1*hbom1 )
        Ssymm_mode1 = 2.0 * sqrt(A_levdens*A_levdens * hbom1*hbom1) +
      ...
      if( epsilon0_2_saddle < A_levdens * hbom2*hbom2 )
        Ssymm_mode2 = 2.0 * sqrt(A_levdens*A_levdens * hbom2*hbom2) +
      ...
      if( epsilon0_1_saddle -
       ...
       Uwash(epsilon_1_saddle/A*Aheavy1,Ecrit,FREDSHELL,gamma_heavy1)
        ...
        Sasymm1 = 2.0 * sqrt( A_levdens*A_levdens * hbom1*hbom1 ) +
         (epsilon0_1_saddle - Delta_U1 *
       Uwash(epsilon_1_saddle/A*Aheavy1,Ecrit,FREDSHELL,gamma_heavy1)
         - A_levdens * hbom1*hbom1)/hbom1;
      else
        Sasymm1 = 2.0 *sqrt( A_levdens*(epsilon0_1_saddle - Delta_U1 *
        Uwash(epsilon_1_saddle/A*Aheavy1,Ecrit,FREDSHELL,gamma_heavy1)));
      ...
      if( epsilon0_2_saddle -
        ...
        Sasymm2 = 2.0 * sqrt( A_levdens*A_levdens * hbom2*hbom2 ) +
         (epsilon0_1_saddle-Delta_U1 *
         Uwash(epsilon_2_saddle/A*Aheavy2,Ecrit,FREDSHELL,gamma_heavy2)
         - A_levdens * hbom2*hbom2)/hbom2;
      else
        Sasymm2 = 2.0 *
             sqrt( A_levdens*(epsilon0_2_saddle - Delta_U2 *
       Uwash(epsilon_2_saddle/A*Aheavy2,Ecrit,FREDSHELL,gamma_heavy2)));
...
//       search minimum threshold and attribute all events to this mode */
  ...
  // Parametrization from Rejmund et al. 
     if (mod(Z,2.0)== 0)
      r_e_o = pow(10.0,-0.0170 * (E_saddle_scission + Eld)*(E_saddle_scission + Eld));
     else
      r_e_o = 0.0;
...
//
      ...
      if(0.50 * sqrt(1.0/A_levdens*E_scission_post) / CZ < 0.0){
        std::cout << "DSQRT 2 < 0 " << CZ << std::endl;
        std::cout << "This event was not considered" << std::endl;
        goto fiss321;
      }

      ZA1width = sqrt(0.5*sqrt(1.0/A_levdens*E_scission_post)/CZ);
...
//  pre-neutron-emission total kinetic energy */
    TKER = ( z1 * z2 * 1.440 ) /
           ( R0 * pow(a1,0.333330) * (1.0 + 2.0/3.0 * beta1 ) +
             R0 * pow(a2,0.333330) * (1.0 + 2.0/3.0 * beta2 ) + 2.0 );
...
double G4Abla::umass(G4double z,G4double n,G4double beta)
{
  ...
  xvs = - xcom * ( 15.4941 * a - 
		   17.9439 * pow(a,2.0/3.0) * (1.0+0.4*alpha*alpha) );
  // sum of volume and surface energy
  xe = z*z * (0.7053/(pow(a,1.0/3.0)) * (1.0-0.2*alpha*alpha) - 1.1529/a);
  fumass = xvs + xe;
...
double G4Abla::ecoul(G4double z1,G4double n1,G4double beta1,G4double z2,G4double n2,G4double beta2,G4double d)
{
  ...
  dtot = r0 * ( pow((z1+n1),1.0/3.0) * (1.0+0.6666667*beta1)
		+ pow((z2+n2),1.0/3.0) * (1.0+0.6666667*beta2) ) + d;
 ...
 G4double G4Abla::Uwash(G4double E, G4double Ecrit,G4double Freduction,G4double gamma){
        ...
        if(E < Ecrit)
          R_wash = exp(-E * Freduction * gamma);
        else
          R_wash = exp(- Ecrit * Freduction * gamma -(E-Ecrit) * gamma);
...
//
      ...
      PTOT = anew*AMU*sqrt((EKIN_P1/(anew*AMU)+1.0)*(EKIN_P1/(anew*AMU)+1.0)-1.0);  // MeV/C
//
      RNDT = G4AblaRandom::flat();
      CTET1 = 2.0*RNDT-1.0;
      STET1 = sqrt(1.0-CTET1*CTET1);
...
// we need to multiply them by -1
      ...
      VZR = -1.0 * VZRIN;
//
      VR = sqrt(VXR*VXR + VYR*VYR + VZR*VZR);
      if(VR<1e-9){
         *VXOUT = VXIN;
         *VYOUT = VYIN;
         *VZOUT = VZIN;
...
///
       ...
       G4double EV_TEMP1[200][6], EV_TEMP2[200][6],mtota;
       ...
       G4double et = EE - JPRF * JPRF * 197. * 197./(2.*0.4*931.*pow(AF,5.0/3.0)*1.16*1.16);
...
//    Velocities
      ...
      G4double VPERP1 = sqrt(VFF1*VFF1 - VZ1_FISSION*VZ1_FISSION);
      G4double ALPHA1 = G4AblaRandom::flat() * 2. * 3.142;
...
void G4Abla::tke_bu(G4double Z,G4double A,G4double ZALL,G4double AAL,G4double *VX,G4double *VY,G4double *VZ){
       ...
       RALL = R0 * pow(V_over_V0,1.0/3.0) * pow(AAL,1.0/3.0);
       RHAZ = G4double(haz(1));
       R = pow(RHAZ,1.0/3.0) * RALL;
       TKE = 1.44 * Z * ZALL * R*R * (1.0 - A/AAL)*(1.0 - A/AAL) / pow(RALL,3.0);

       Ekin = TKE * (AAL - A) / AAL;
//       print*,'!!!',IDNINT(AAl),IDNINT(A),IDNINT(ZALL),IDNINT(Z)
       V = sqrt(Ekin/A) * 1.3887;
       *VZ = (2.0 * G4double(haz(1)) - 1.0) * V;
       VPERP = sqrt(V*V - (*VZ)*(*VZ));
...
//       y=(HAZ(k)*(rxmax**l_plus-rxmin**l_plus)+ rxmin**l_plus)**(1.E0/l_plus)
         y=pow(G4AblaRandom::flat()*(pow(rxmax,l_plus)-pow(rxmin,l_plus))+ pow(rxmin,l_plus),1.0/l_plus);
         return nint(y);
}
...
// The factor (AABRA-APRF)/AP comes from momentum conservation:
        GOLDHA_BU = sqrt(APRF *  T_freeze_out * 931.494 *
                   (AABRA - APRF) / AABRA);
        GOLDHA   = GOLDHA_BU;
...
//     &                          GOLDHA_BU
      ...
      GOLDHA = SIGMA_0 * sqrt((APRF*(AABRA-APRF))/(AABRA-1.0));
      }
