...
G4PreCompoundTransitionsInt::G4PreCompoundTransitionsInt(G4int verb) : fVerbose(verb)
{
...
G4double G4PreCompoundTransitionsInt::CalculateProbability(const G4Fragment& aFragment)
{
  ...
  */
  ...
  if (useCEMtr)
  {
    ...
    G4double ppXSection =
      (10.63 / RelativeVelocitySqr - 29.92 / RelativeVelocity + 42.9) * CLHEP::millibarn;
    // Proton-Neutron Cross Section
    G4double npXSection =
      (34.10 / RelativeVelocitySqr - 82.20 / RelativeVelocity + 82.2) * CLHEP::millibarn;

    // Averaged Cross Section: \sigma(V_{rel})
    G4double xs = (ChargedNucleon) ? ((Z - 1) * ppXSection + (A - Z) * npXSection) / G4double(A - 1)
    ...
    TransitionProb1 = std::max(
      0.0, xs * PauliFactor * std::sqrt(2.0 * RelativeEnergy / CLHEP::proton_mass_c2) / Vint);
  ...
  else
  {
    ...
    //  GE = g*E where E is Excitation Energy
    TransitionProb1 =
      std::max(0.0, U * (4.2e+12 - 3.6e+10 * U / G4double(2 * P + 1))) / (16 * CLHEP::c_light);
...
void G4PreCompoundTransitionsInt::PerformTransition(G4Fragment& frag)
{
  ...
  if (2 < fVerbose)
  {
    G4cout << "           # After transition Npart=" << Npart << " Ncharged=" << Ncharged
           << " deltaN=" << deltaN << G4endl;
  }
}
