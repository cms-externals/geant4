...
G4double G4PreCompoundTransitions::CalculateProbability(const G4Fragment& aFragment)
{
  ...
  if (2 < fVerbose)
  {
    G4cout << "G4PreCompoundTransitions::CalculateProbability H/P/N/Z/A= " << H << " " << P << " "
           << N << " " << Z << " " << A << " U=" << U << G4endl;
  }
  ...
  if (useCEMtr)
  {
    ...
    TransitionProb1 =
      std::max(0.0, AveragedXSection * PauliFactor
  ...
  else
  {
    ...
    //  GE = g*E where E is Excitation Energy
    TransitionProb1 =
      std::max(0.0, U * (4.2e+12 - 3.6e+10 * U / G4double(N + 1))) / (16 * CLHEP::c_light);
...
void G4PreCompoundTransitions::PerformTransition(G4Fragment& result)
{
  ...
  if (2 < fVerbose)
  {
    G4cout << "G4PreCompoundTransitions::PerformTransition: deltaN=" << deltaN << G4endl;
  }
  ...
  if (deltaN < 0)
  {
    if ((Ncharged == Npart) || (Ncharged >= 1 && G4int(Npart * G4UniformRand()) <= Ncharged))
    {
