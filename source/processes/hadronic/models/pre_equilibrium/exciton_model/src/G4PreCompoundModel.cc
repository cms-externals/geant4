...
G4PreCompoundModel::G4PreCompoundModel(G4ExcitationHandler* ptr) : G4VPreCompoundModel(ptr, "PRECO")
{
                                                   ...
                                                   G4Nucleus& theNucleus)
{
  ...
  if (primary != neutron && primary != proton)
  {
    ...
    G4Exception("G4PreCompoundModel::ApplyYourself()", "had0033", FatalException, ed, "");
    return nullptr;
  }
...
G4ReactionProductVector* G4PreCompoundModel::DeExcite(G4Fragment& aFragment)
{
  ...
  if (1 < fVerbose)
  {
    G4cout << "### G4PreCompoundModel::DeExcite Z=" << Z << " A=" << A << " U(MeV)=" << U
           << " Umin=" << fLowLimitExc * A << G4endl;
    G4cout << aFragment << G4endl;
  }

  // Conditions to skip pre-compound and perform equilibrium emission
  if (!isActive || (Z < minZ && A < minA) || U < fLowLimitExc * A || U > fHighLimitExc * A
      || 0 < aFragment.GetNumberOfLambdas() || 0 == Z || Z >= A)
  {
...
void G4PreCompoundModel::DoIt(G4ReactionProductVector* result, G4Fragment& aFragment)
{
  ...
  for (;;)
  {
    ...
    if (nExcitons <= 0 || (Z < minZ && A < minA) || U > fHighLimitExc * A || U <= fLowLimitExc * A)
    {
      PerformEquilibriumEmission(aFragment, result);
      return;
    }
    G4int eqExcitonNumber = G4lrint(std::sqrt(ldfact * U * fNuclData->GetLevelDensity(Z, A, U)));
    if (2 < fVerbose)
    {
      G4cout << "   1st loop " << count << ". Z=" << Z << " A=" << A << " U(MeV)=" << U
             << " eqExcitationNumber=" << eqExcitonNumber << G4endl;
    }
    ...
    do
    {
      ...
      if (useSCO && go_ahead)
      {
        ...
        if (G4UniformRand() < 1.0 - G4Exp(-x * x / 0.32))
        {
          go_ahead = false;
        }
      }
      ...
      if (2 < fVerbose)
      {
        G4cout << "   2nd loop " << count << ". Nex=" << nExcitons << " P1=" << P1 << " P2=" << P2
               << " P3=" << P3 << G4endl;
        G4cout << aFragment << G4endl;
      }
      ...
      if (2 < fVerbose)
      {
        G4cout << "             prob_em=" << emissionProbability << " prob_tot=" << totProbability
               << G4endl;
      }
    ...
    if (count >= countmax)
    {
      ...
      ed << "G4PreCompoundModel loop over " << countmax << " iterations; "
         << "current G4Fragment: \n"
         << aFragment;
      G4Exception("G4PreCompoundModel::DeExcite()", "had0034", JustWarning, ed, "");
      break;
    }
  }  // end of for (;;) loop
  PerformEquilibriumEmission(aFragment, result);
