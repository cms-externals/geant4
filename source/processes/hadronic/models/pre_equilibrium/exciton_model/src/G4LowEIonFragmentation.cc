                                                       ...
                                                       G4Nucleus& theNucleus)
{
  ...
  while (0 == particlesFromProjectile)
  {
    ...
    if (projectileHorizon > empirical * projectileOuterRadius)
    {
      continue;
    }
  ...
  G4Fragment anInitialState(aTargetA + particlesFromProjectile, aTargetZ + chargedFromProjectile,
                            fragment4Momentum);
  // M.A. Cortes fix
  ...
  if (particlesFromProjectile < aProjectileA)
  {
    ...
    G4Fragment initialState2(aProjectileA - particlesFromProjectile,
                             aProjectileZ - chargedFromProjectile, residual4Momentum);
  ...
  std::size_t nexc = (nullptr != theExcitationResult) ? theExcitationResult->size() : 0;
  std::size_t npre = (nullptr != thePreCompoundResult) ? thePreCompoundResult->size() : 0;
