...
#ifndef G4PRECOMPOUNDINTERFACE_HH
#define G4PRECOMPOUNDINTERFACE_HH
...
class G4PreCompoundInterface : public G4VPreCompoundModel
{
  public:
    ...
    const G4PreCompoundInterface& operator=(const G4PreCompoundInterface& right) = delete;
  ...
  private:
    ...
    inline void PerformEquilibriumEmission(const G4Fragment&, G4ReactionProductVector*) const;
...
inline void
G4PreCompoundInterface::PerformEquilibriumEmission(const G4Fragment& aFragment,
                                                   G4ReactionProductVector* result) const
{
  auto deexResult = GetExcitationHandler()->BreakItUp(aFragment);
  for (auto& frag : *deexResult)
  {
