...
void G4NuDEXInternalConversion::FillElectronHole(G4int i_shell){
  ...
  if(i_shell==1){ //K-shell
    ...
    G4double w_fac=pow(C0+C1*theZ+C2*theZ*theZ+C3*theZ*theZ*theZ,4);
...
G4double G4NuDEXInternalConversion::Interpolate(G4double val,G4int npoints,G4double* x,G4double* y){
  ...
  */
  ...
  //log(y)=a0+a1*log(x)
  G4double a1=log(y[i_interp+1]/y[i_interp])/log(x[i_interp+1]/x[i_interp]);
  G4double a0=log(y[i_interp])-a1*log(x[i_interp]);

  G4double result=exp(a0+a1*log(val));
  return result;
}
