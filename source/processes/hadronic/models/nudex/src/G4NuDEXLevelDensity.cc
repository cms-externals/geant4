...
G4double G4NuDEXLevelDensity::GetNucleusTemperature(G4double ExcEnergy){
  ...
  if(Uval<=0){return 0;}
  G4double a_ldpar=ainf_ldpar*(1.+dW_ldpar/Uval*(1.-exp(-gamma_ldpar*Uval)));
  if(LDType==3){
    a_ldpar=ainf_ldpar;
  }
...
G4double G4NuDEXLevelDensity::GetLevelDensity(G4double ExcEnergy,G4double spin,G4bool ,G4bool TotalLevelDensity){
  ...
  if(LDType==3){
    G4double sig2=0.0888*pow(A_mass,2./3.)*sqrt(ainf_ldpar*Uval);
    G4double sig=sqrt(sig2);
    G4double rho=0.05893*exp(2.*sqrt(ainf_ldpar*Uval))/sig/pow(ainf_ldpar,0.25)/pow(Uval,1.25);
    G4double xj2=(spin+0.5)*(spin+0.5);
    G4double fj=(2.*spin+1.)/2./sig2*exp(-xj2/2./sig2);
    return 0.5*fj*rho;
  }
  //----------------------------------------------------------------
  ...
  G4double a_ldpar=ainf_ldpar*(1.+dW_ldpar/Uval*(1.-exp(-gamma_ldpar*Uval)));
  G4double a_ldpar_Sn=ainf_ldpar*(1.+dW_ldpar/Uval_Sn*(1.-exp(-gamma_ldpar*Uval_Sn)));
  G4double sigma2_f=0.01389*pow(A_mass,5./3.)/ainf_ldpar*sqrt(a_ldpar*Uval);
  G4double sigma2_f_Sn=0.01389*pow(A_mass,5./3.)/ainf_ldpar*sqrt(a_ldpar_Sn*Uval);
  G4double sigma2_d=(0.83*pow(A_mass,0.26))*(0.83*pow(A_mass,0.26));
  ...
  G4double statfactor=1./2.*(2.*spin+1.)/(2.*sigma2)*exp(-(spin+1/2.)*(spin+1/2.)/2./sigma2);
  ...
  if(LDType==2 && ExcEnergy<Ex_ldpar){
    G4double totalrho=exp((ExcEnergy-E0_ldpar)/T_ldpar)/T_ldpar;
    return totalrho*statfactor;
  }

  //Else: BSFGM (LDType==1 or ExcEnergy>Ex_ldpar)
  G4double rhotot_f=1./sqrt(2.*sigma2)/12.*exp(2.*sqrt(a_ldpar*Uval))/pow(a_ldpar,1./4.)/pow(Uval,5./4.);
  G4double rhotot_0=exp(1.)*a_ldpar/12./sqrt(sigma2)*exp(a_ldpar*Uval);
