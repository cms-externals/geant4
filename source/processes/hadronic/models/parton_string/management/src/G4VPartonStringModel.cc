...
G4VPartonStringModel::G4VPartonStringModel(const G4String& modelName)
  : G4VHighEnergyGenerator(modelName), stringFragmentationModel(nullptr)
{
                                                    ...
                                                    const G4DynamicParticle& aPrimary)
{
  ...
  G4cout << "Target nucleus   A Z " << theNucleus.GetA_asInt() << " " << theNucleus.GetZ_asInt()
         << G4endl << G4endl;
  ...
  G4cout << "-------------- Parton-String model:  Generation of strings -------" << G4endl
         << G4endl;
  Bsum -= theNucleus.GetA_asInt();
  Qsum -= theNucleus.GetZ_asInt();
  ...
  do
  {
    if (attempts++ > maxAttempts)
    {
      ...
      ed << "Target nucleus   A Z " << theNucleus.GetA_asInt() << " " << theNucleus.GetZ_asInt()
         << G4endl;
      ...
      G4KineticTrack* Hadron =
        new G4KineticTrack(aPrimary.GetParticleDefinition(), 0., Position, aPrimary.Get4Momentum());
...
#ifdef debug_PartonStringModel
    G4cout << "------------ Parton-String model: Number of produced strings ---- "
           << strings->size() << G4endl;
#endif
    ...
    for (unsigned int astring = 0; astring < strings->size(); astring++)
    {
      ...
      if ((*strings)[astring]->IsExcited())
      {
        ...
        G4cout << "String No " << astring + 1 << " " << (*strings)[astring]->Get4Momentum() << " "
               << (*strings)[astring]->Get4Momentum().mag() << " Partons   "
               << (*strings)[astring]->GetLeftParton()->GetDefinition()->GetPDGEncoding()
               << "          "
...
#ifdef debug_heavyHadrons
        G4int left_charm =
          (*strings)[astring]->GetLeftParton()->GetDefinition()->GetQuarkContent(4);
        G4int left_anticharm =
          (*strings)[astring]->GetLeftParton()->GetDefinition()->GetAntiQuarkContent(4);
        G4int right_charm =
          (*strings)[astring]->GetRightParton()->GetDefinition()->GetQuarkContent(4);
        G4int right_anticharm =
          (*strings)[astring]->GetRightParton()->GetDefinition()->GetAntiQuarkContent(4);
        G4int left_bottom =
          (*strings)[astring]->GetLeftParton()->GetDefinition()->GetQuarkContent(5);
        G4int left_antibottom =
          (*strings)[astring]->GetLeftParton()->GetDefinition()->GetAntiQuarkContent(5);
        G4int right_bottom =
          (*strings)[astring]->GetRightParton()->GetDefinition()->GetQuarkContent(5);
        G4int right_antibottom =
          (*strings)[astring]->GetRightParton()->GetDefinition()->GetAntiQuarkContent(5);
        if (left_charm != 0 || left_anticharm != 0 || right_charm != 0 || right_anticharm != 0
            || left_bottom != 0 || left_antibottom != 0 || right_bottom != 0
            || right_antibottom != 0)
        {
          ...
          G4cout << "G4VPartonStringModel::Scatter : string #" << astring << " ("
                 ...
                 << " , "
                 << (*strings)[astring]->GetRightParton()->GetDefinition()->GetParticleName() << ")"
                 << G4endl;
        }
#endif
      }
      else
      {
        ...
        G4int anticharm =
          (*strings)[astring]->GetKineticTrack()->GetDefinition()->GetAntiQuarkContent(4);
        G4int bottom = (*strings)[astring]->GetKineticTrack()->GetDefinition()->GetQuarkContent(5);
        G4int antibottom =
          (*strings)[astring]->GetKineticTrack()->GetDefinition()->GetAntiQuarkContent(5);
    ...
    if (ProjResNucleus != 0)
    {
      ...
      while (theNuclNucleon)
      {
        if (theNuclNucleon->AreYouHit())
        {
          ...
          if (def == G4Proton::Definition() || def == G4AntiProton::Definition())
            ++numberProtonProjectileHits;
          if (def == G4Neutron::Definition() || def == G4AntiNeutron::Definition())
            ++numberNeutronProjectileHits;
          if (def == G4Lambda::Definition() || def == G4AntiLambda::Definition())
      ...
      {
        numberLambdaProjectile =
          thePrimary.GetDefinition()->GetNumberOfAntiLambdasInAntiHypernucleus();
      }
#ifdef debug_PartonStringModel
      G4cout << "Projectile residual A, Z (numberOfLambdasOrAntiLambdas) and E* "
             ...
             << numberLambdaProjectile - numberLambdaProjectileHits << ") " << ExcitationEp
             << G4endl;
      G4cout << "Projectile residual 4 momentum  " << ProjectileResidual4Momentum << G4endl;
#endif
      numberProtonProjectileResidual = std::max(
        std::abs(G4int(thePrimary.GetDefinition()->GetPDGCharge())) - numberProtonProjectileHits,
        0);
      numberLambdaProjectileResidual =
        std::max(numberLambdaProjectile - numberLambdaProjectileHits, 0);
      numberNeutronProjectileResidual =
        std::max(std::abs(thePrimary.GetDefinition()->GetBaryonNumber())
                   - std::abs(G4int(thePrimary.GetDefinition()->GetPDGCharge()))
                   - numberLambdaProjectile - numberNeutronProjectileHits,
                 0);
    }
...
#ifdef debug_PartonStringModel
    G4cout << "Target residual A, Z and E* " << theNucleus.GetA_asInt() - hitsT << " "
           << theNucleus.GetZ_asInt() - charged_hitsT << " " << ExcitationEt << G4endl;
    ...
    G4cout << "Hitted # of protons of projectile and target  " << charged_hitsP << " "
           << charged_hitsT << G4endl << G4endl;
    ...
    G4int numberNeutronTargetResidual =
      theNucleus.GetA_asInt() - theNucleus.GetZ_asInt() - numberNeutronTargetHits;
...
//                              Fragment strings
    ...
    if (fIsQuasiElasticInteraction)
    {
      ...
      // A quasi-elastic interaction must have neither extra pions, nor Delta resonances in the
      // final state.
    ...
    for (unsigned int i = 0; i < theResult->size(); i++)
    {
      ...
      G4cout << i << " : " << (*theResult)[i]->GetDefinition()->GetParticleName() << " "
             << (*theResult)[i]->Get4Momentum() << " " << (*theResult)[i]->Get4Momentum().mag()
...
#ifdef debug_PartonStringModel
    G4cout << G4endl << "-----------------------Parton-String model: balances -------------"
           << G4endl;
    if (Qsum != QsumSec)
    {
  ...
  G4cout << "---------------------End of Parton-String model work -------------" << G4endl
         << G4endl;
#endif
