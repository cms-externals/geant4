  ...
  : G4HadronicInteraction(name), ProbCCbar(0.0), ProbBBbar(0.0)
{
  ...
  StrangeSuppress =
    0.44;  // =0.27/2.27 suppression of strange quark pair production, ie. u:d:s=1:1:0.27
  ...
  MaxMass =
    -350.0
    * GeV;  // If there will be a particle with mass larger than Higgs the value must be changed.

  SetMinMasses();  // Re-calculation of minimal mass of strings and weights of particles in 2-part.
                   // decays
...
G4VLongitudinalStringDecay::ProduceOneHadron(const G4ExcitedString* const string)
{
  ...
  G4cout << "G4VLongitudinalStringDecay::ProduceOneHadron: PossibleHmass StrMass " << aString.Mass()
         << " MassCut " << MassCut << G4endl;
#endif
  ...
  if (hadrons.first != nullptr)
  {
...
#ifdef debug_VStringDecay
      G4cout << "VlongSD Warning replacing string by single hadron (G4VLongitudinalStringDecay)"
             << G4endl;
      G4cout << hadrons.first->GetParticleName() << G4endl << "string .. " << string->Get4Momentum()
    ...
    else
    {
      ...
      G4cout
        ...
        << hadrons.first->GetParticleName() << " / " << hadrons.second->GetParticleName()
        << "string .. " << string->Get4Momentum() << " " << string->Get4Momentum().m() << G4endl;
#endif
...
#ifdef debug_VStringDecay
  G4cout << "VlongSD *Hadrons 1 and 2, proposed mass " << Hadron1 << " " << Hadron2 << " " << mass
         << G4endl;
#endif
...
{
  /*
  G4cout<<Encoding<<" G4VLongitudinalStringDecay::FindParticle Check di-quarks
  *******************"<<G4endl; for (G4int i=4; i<6;i++){ for (G4int j=1;j<6;j++){ G4cout<<i<<"
  "<<j<<" "; G4int Code = 1000 * i + 100 * j +1; G4ParticleDefinition* ptr1 =
  G4ParticleTable::GetParticleTable()->FindParticle(Code); Code +=2; G4ParticleDefinition* ptr2 =
  G4ParticleTable::GetParticleTable()->FindParticle(Code); G4cout<<"Code "<<Code - 2<<" ptr
  "<<ptr1<<" :: Code "<<Code<<" ptr "<<ptr2<<G4endl;
    }
    G4cout<<G4endl;
  }
  */
  ...
  if (ptr == nullptr)
  {
    ...
    {
      if (Encoding == NewParticles[i]->GetPDGEncoding())
      {
        ptr = NewParticles[i];
        return ptr;
      }
                                                               ...
                                                               G4ParticleDefinition*& created)
{
  ...
  G4int IsParticle =
    (decay->GetPDGEncoding() > 0) ? -1 : +1;  // if we have a quark, we need antiquark (or diquark)
...
#ifdef debug_VStringDecay
  G4cout << "VlongSD QuarkSplitup: " << decay->GetPDGEncoding() << " -> "
         << QuarkPair.second->GetPDGEncoding() << G4endl;
...
G4VLongitudinalStringDecay::CreatePartonPair(G4int NeedParticle, G4bool AllowDiquarks)
{
...
void G4VLongitudinalStringDecay::SetSpinThreeHalfBarionProbability(G4double aValue)
{
  if (PastInitPhase)
  {
    throw G4HadronicException(__FILE__, __LINE__,
                              "G4VLongitudinalStringDecay::SetSpinThreeHalfBarionProbability after "
                              "FragmentString() not allowed");
  }
  else
  {
    pspin_barion = aValue;
...
void G4VLongitudinalStringDecay::SetMinMasses()
{
  ...
  for (G4int i = 1; i < 6; i++)
  {
    ...
    if (hadron1 != nullptr)
    {
      for (G4int j = 1; j < 6; j++)
      {
        ...
        if (hadron2 != nullptr)
        {
          minMassQQbarStr[i - 1][j - 1] =
            hadron1->GetPDGMass() + hadron2->GetPDGMass() + 70.0 * MeV;
        }
      }
    }
  }
  ...
  G4int kfla, kflb;
  //  MaxMass = -350.0*GeV;   // If there will be a particle with mass larger than Higgs the value
  //  must be changed.
  ...
  {  // i=1
    ...
    for (G4int j = 1; j < 6; j++)
    {
      for (G4int k = 1; k < 6; k++)
      {
        ...
        if ((j == 1) && (k == 1))
          Code2 = 1000 * 2 + 100 * 1 + 10 * 1 + 2;  // In the case - add u-quark.
        ...
        {
          minMassQDiQStr[i - 1][j - 1][k - 1] = MaxMass;
          continue;
        };

        if ((hadron2 != nullptr) && (hadron3 != nullptr))
        {
          if (hadron2->GetPDGMass() > hadron3->GetPDGMass())
          {
            hadron2 = hadron3;
          }
        };
  ...
  // qqs indexes dd1 -> scalarMesonMix[0] * 111 + (1-scalarMesonMix[0]-scalarMesonMix[1]) * 221 +
  // scalarMesonMix[1] * 331     (000) dd1 ->                     Pi0 Eta                       Eta'

  Meson[0][0][0] = 111;
  MesonWeight[0][0][0] = (pspin_meson[0]) * (scalarMesonMix[0]);  // Pi0
  Meson[0][0][2] = 221;
  MesonWeight[0][0][2] = (pspin_meson[0]) * (1 - scalarMesonMix[0] - scalarMesonMix[1]);  // Eta
  Meson[0][0][3] = 331;
  MesonWeight[0][0][3] = (pspin_meson[0]) * (scalarMesonMix[1]);  // Eta'

  // dd3 -> (1-vectorMesonMix[1] * 113 + vectorMesonMix[1] * 223 (001) dd3 -> rho_0 omega

  Meson[0][0][1] = 113;
  MesonWeight[0][0][1] = (1. - pspin_meson[0]) * (1 - vectorMesonMix[1]);  // Rho
  Meson[0][0][4] = 223;
  MesonWeight[0][0][4] = (1. - pspin_meson[0]) * (vectorMesonMix[1]);  // omega

  // uu1 -> scalarMesonMix[0] * 111 + (1-scalarMesonMix[0]-scalarMesonMix[1]) * 221 +
  // scalarMesonMix[1] * 331     (110) uu1 ->                     Pi0 Eta                       Eta'

  Meson[1][1][0] = 111;
  MesonWeight[1][1][0] = (pspin_meson[0]) * (scalarMesonMix[0]);  // Pi0
  Meson[1][1][2] = 221;
  MesonWeight[1][1][2] = (pspin_meson[0]) * (1 - scalarMesonMix[0] - scalarMesonMix[1]);  // Eta
  Meson[1][1][3] = 331;
  MesonWeight[1][1][3] = (pspin_meson[0]) * (scalarMesonMix[1]);  // Eta'

  // uu3 -> (1-vectorMesonMix[1]) * 113 + vectorMesonMix[1] * 223 (111) uu3 -> rho_0 omega

  Meson[1][1][1] = 113;
  MesonWeight[1][1][1] = (1. - pspin_meson[0]) * (1 - vectorMesonMix[1]);  // Rho
  Meson[1][1][4] = 223;
  MesonWeight[1][1][4] = (1. - pspin_meson[0]) * (vectorMesonMix[1]);  // omega

  // ss1     ->                                             (1-scalarMesonMix[5]) * 221 +
  // scalarMesonMix[5] * 331   (220) ss1     -> Eta                       Eta'

  Meson[2][2][0] = 221;
  MesonWeight[2][2][0] = (pspin_meson[2]) * (1 - scalarMesonMix[5]);  // Eta
  Meson[2][2][2] = 331;
  MesonWeight[2][2][2] = (pspin_meson[2]) * (scalarMesonMix[5]);  // Eta'

  // ss3     -> vectorMesonMix[5] * 333   (221) ss3     -> phi

  Meson[2][2][1] = 333;
  MesonWeight[2][2][1] = (1. - pspin_meson[2]) * (vectorMesonMix[5]);  // phi
  ...
  // Delta-  ddd - only 1114
  Baryon[0][0][0][0] = 1114;
  BaryonWeight[0][0][0][0] = 1.0;
  Baryon[0][0][0][1] = 0;
  BaryonWeight[0][0][0][1] = 0.0;

  // Delta++ uuu - only 2224
  Baryon[1][1][1][0] = 2224;
  BaryonWeight[1][1][1][0] = 1.0;
  Baryon[1][1][1][1] = 0;
  BaryonWeight[1][1][1][1] = 0.0;

  // Omega- sss - only 3334
  Baryon[2][2][2][0] = 3334;
  BaryonWeight[2][2][2][0] = 1.0;
  Baryon[2][2][2][1] = 0;
  BaryonWeight[2][2][2][1] = 0.0;

  // Omega_cc++ ccc - only 4444
  Baryon[3][3][3][0] = 4444;
  BaryonWeight[3][3][3][0] = 1.0;
  Baryon[3][3][3][1] = 0;
  BaryonWeight[3][3][3][1] = 0.0;

  // Omega_bb-  bbb - only 5554
  Baryon[4][4][4][0] = 5554;
  BaryonWeight[4][4][4][0] = 1.0;
  Baryon[4][4][4][1] = 0;
  BaryonWeight[4][4][4][1] = 0.0;

  // Lambda/Sigma0 sud - 3122/3212
  Baryon[0][1][2][0] = 3122;
  BaryonWeight[0][1][2][0] *= 0.5;  // Lambda
  Baryon[0][2][1][0] = 3122;
  BaryonWeight[0][2][1][0] *= 0.5;
  Baryon[1][0][2][0] = 3122;
  BaryonWeight[1][0][2][0] *= 0.5;
  Baryon[1][2][0][0] = 3122;
  BaryonWeight[1][2][0][0] *= 0.5;
  Baryon[2][0][1][0] = 3122;
  BaryonWeight[2][0][1][0] *= 0.5;
  Baryon[2][1][0][0] = 3122;
  BaryonWeight[2][1][0][0] *= 0.5;

  Baryon[0][1][2][2] = 3212;
  BaryonWeight[0][1][2][2] = 0.5 * pspin_barion;  // Sigma0
  Baryon[0][2][1][2] = 3212;
  BaryonWeight[0][2][1][2] = 0.5 * pspin_barion;
  Baryon[1][0][2][2] = 3212;
  BaryonWeight[1][0][2][2] = 0.5 * pspin_barion;
  Baryon[1][2][0][2] = 3212;
  BaryonWeight[1][2][0][2] = 0.5 * pspin_barion;
  Baryon[2][0][1][2] = 3212;
  BaryonWeight[2][0][1][2] = 0.5 * pspin_barion;
  Baryon[2][1][0][2] = 3212;
  BaryonWeight[2][1][0][2] = 0.5 * pspin_barion;

  // Lambda_c+/Sigma_c+ cud - 4122/4212
  Baryon[0][1][3][0] = 4122;
  BaryonWeight[0][1][3][0] *= 0.5;  // Lambda_c+
  Baryon[0][3][1][0] = 4122;
  BaryonWeight[0][3][1][0] *= 0.5;
  Baryon[1][0][3][0] = 4122;
  BaryonWeight[1][0][3][0] *= 0.5;
  Baryon[1][3][0][0] = 4122;
  BaryonWeight[1][3][0][0] *= 0.5;
  Baryon[3][0][1][0] = 4122;
  BaryonWeight[3][0][1][0] *= 0.5;
  Baryon[3][1][0][0] = 4122;
  BaryonWeight[3][1][0][0] *= 0.5;

  Baryon[0][1][3][2] = 4212;
  BaryonWeight[0][1][3][2] = 0.5 * pspin_barion;  // SigmaC+
  Baryon[0][3][1][2] = 4212;
  BaryonWeight[0][3][1][2] = 0.5 * pspin_barion;
  Baryon[1][0][3][2] = 4212;
  BaryonWeight[1][0][3][2] = 0.5 * pspin_barion;
  Baryon[1][3][0][2] = 4212;
  BaryonWeight[1][3][0][2] = 0.5 * pspin_barion;
  Baryon[3][0][1][2] = 4212;
  BaryonWeight[3][0][1][2] = 0.5 * pspin_barion;
  Baryon[3][1][0][2] = 4212;
  BaryonWeight[3][1][0][2] = 0.5 * pspin_barion;

  // Xi_c+/Xi_c+' cus - 4232/4322
  Baryon[1][2][3][0] = 4232;
  BaryonWeight[1][2][3][0] *= 0.5;  // Xi_c+
  Baryon[1][3][2][0] = 4232;
  BaryonWeight[1][3][2][0] *= 0.5;
  Baryon[2][1][3][0] = 4232;
  BaryonWeight[2][1][3][0] *= 0.5;
  Baryon[2][3][1][0] = 4232;
  BaryonWeight[2][3][1][0] *= 0.5;
  Baryon[3][1][2][0] = 4232;
  BaryonWeight[3][1][2][0] *= 0.5;
  Baryon[3][2][1][0] = 4232;
  BaryonWeight[3][2][1][0] *= 0.5;

  Baryon[1][2][3][2] = 4322;
  BaryonWeight[1][2][3][2] = 0.5 * pspin_barion;  // Xi_c+'
  Baryon[1][3][2][2] = 4322;
  BaryonWeight[1][3][2][2] = 0.5 * pspin_barion;
  Baryon[2][1][3][2] = 4322;
  BaryonWeight[2][1][3][2] = 0.5 * pspin_barion;
  Baryon[2][3][1][2] = 4322;
  BaryonWeight[2][3][1][2] = 0.5 * pspin_barion;
  Baryon[3][1][2][2] = 4322;
  BaryonWeight[3][1][2][2] = 0.5 * pspin_barion;
  Baryon[3][2][1][2] = 4322;
  BaryonWeight[3][2][1][2] = 0.5 * pspin_barion;

  // Xi_c0/Xi_c0' cus - 4132/4312
  Baryon[0][2][3][0] = 4132;
  BaryonWeight[0][2][3][0] *= 0.5;  // Xi_c0
  Baryon[0][3][2][0] = 4132;
  BaryonWeight[0][3][2][0] *= 0.5;
  Baryon[2][0][3][0] = 4132;
  BaryonWeight[2][0][3][0] *= 0.5;
  Baryon[2][3][0][0] = 4132;
  BaryonWeight[2][3][0][0] *= 0.5;
  Baryon[3][0][2][0] = 4132;
  BaryonWeight[3][0][2][0] *= 0.5;
  Baryon[3][2][0][0] = 4132;
  BaryonWeight[3][2][0][0] *= 0.5;

  Baryon[0][2][3][2] = 4312;
  BaryonWeight[0][2][3][2] = 0.5 * pspin_barion;  // Xi_c0'
  Baryon[0][3][2][2] = 4312;
  BaryonWeight[0][3][2][2] = 0.5 * pspin_barion;
  Baryon[2][0][3][2] = 4312;
  BaryonWeight[2][0][3][2] = 0.5 * pspin_barion;
  Baryon[2][3][0][2] = 4312;
  BaryonWeight[2][3][0][2] = 0.5 * pspin_barion;
  Baryon[3][0][2][2] = 4312;
  BaryonWeight[3][0][2][2] = 0.5 * pspin_barion;
  Baryon[3][2][0][2] = 4312;
  BaryonWeight[3][2][0][2] = 0.5 * pspin_barion;

  // Lambda_b0/Sigma_b0 bud - 5122/5212
  Baryon[0][1][4][0] = 5122;
  BaryonWeight[0][1][4][0] *= 0.5;  // Lambda_b0
  Baryon[0][4][1][0] = 5122;
  BaryonWeight[0][4][1][0] *= 0.5;
  Baryon[1][0][4][0] = 5122;
  BaryonWeight[1][0][4][0] *= 0.5;
  Baryon[1][4][0][0] = 5122;
  BaryonWeight[1][4][0][0] *= 0.5;
  Baryon[4][0][1][0] = 5122;
  BaryonWeight[4][0][1][0] *= 0.5;
  Baryon[4][1][0][0] = 5122;
  BaryonWeight[4][1][0][0] *= 0.5;

  Baryon[0][1][4][2] = 5212;
  BaryonWeight[0][1][4][2] = 0.5 * pspin_barion;  // Sigma_b0
  Baryon[0][4][1][2] = 5212;
  BaryonWeight[0][4][1][2] = 0.5 * pspin_barion;
  Baryon[1][0][4][2] = 5212;
  BaryonWeight[1][0][4][2] = 0.5 * pspin_barion;
  Baryon[1][4][0][2] = 5212;
  BaryonWeight[1][4][0][2] = 0.5 * pspin_barion;
  Baryon[4][0][1][2] = 5212;
  BaryonWeight[4][0][1][2] = 0.5 * pspin_barion;
  Baryon[4][1][0][2] = 5212;
  BaryonWeight[4][1][0][2] = 0.5 * pspin_barion;

  // Xi_b0/Xi_b0' bus - 5232/5322
  Baryon[1][2][4][0] = 5232;
  BaryonWeight[1][2][4][0] *= 0.5;  // Xi_b0
  Baryon[1][4][2][0] = 5232;
  BaryonWeight[1][4][2][0] *= 0.5;
  Baryon[2][1][4][0] = 5232;
  BaryonWeight[2][1][4][0] *= 0.5;
  Baryon[2][4][1][0] = 5232;
  BaryonWeight[2][4][1][0] *= 0.5;
  Baryon[4][1][2][0] = 5232;
  BaryonWeight[4][1][2][0] *= 0.5;
  Baryon[4][2][1][0] = 5232;
  BaryonWeight[4][2][1][0] *= 0.5;

  Baryon[1][2][4][2] = 5322;
  BaryonWeight[1][2][4][2] = 0.5 * pspin_barion;  // Xi_b0'
  Baryon[1][4][2][2] = 5322;
  BaryonWeight[1][4][2][2] = 0.5 * pspin_barion;
  Baryon[2][1][4][2] = 5322;
  BaryonWeight[2][1][4][2] = 0.5 * pspin_barion;
  Baryon[2][4][1][2] = 5322;
  BaryonWeight[2][4][1][2] = 0.5 * pspin_barion;
  Baryon[4][1][2][2] = 5322;
  BaryonWeight[4][1][2][2] = 0.5 * pspin_barion;
  Baryon[4][2][1][2] = 5322;
  BaryonWeight[4][2][1][2] = 0.5 * pspin_barion;

  // Xi_b-/Xi_b-' bus - 5132/5312
  Baryon[0][2][4][0] = 5132;
  BaryonWeight[0][2][4][0] *= 0.5;  // Xi_b-
  Baryon[0][4][2][0] = 5132;
  BaryonWeight[0][4][2][0] *= 0.5;
  Baryon[2][0][4][0] = 5132;
  BaryonWeight[2][0][4][0] *= 0.5;
  Baryon[2][4][0][0] = 5132;
  BaryonWeight[2][4][0][0] *= 0.5;
  Baryon[4][0][2][0] = 5132;
  BaryonWeight[4][0][2][0] *= 0.5;
  Baryon[4][2][0][0] = 5132;
  BaryonWeight[4][2][0][0] *= 0.5;

  Baryon[0][2][4][2] = 5312;
  BaryonWeight[0][2][4][2] = 0.5 * pspin_barion;  // Xi_b-'
  Baryon[0][4][2][2] = 5312;
  BaryonWeight[0][4][2][2] = 0.5 * pspin_barion;
  Baryon[2][0][4][2] = 5312;
  BaryonWeight[2][0][4][2] = 0.5 * pspin_barion;
  Baryon[2][4][0][2] = 5312;
  BaryonWeight[2][4][0][2] = 0.5 * pspin_barion;
  Baryon[4][0][2][2] = 5312;
  BaryonWeight[4][0][2][2] = 0.5 * pspin_barion;
  Baryon[4][2][0][2] = 5312;
  BaryonWeight[4][2][0][2] = 0.5 * pspin_barion;
    ...
    for (G4int j = 0; j < 5; j++)
    {
      for (G4int k = 0; k < 5; k++)
      {
        for (G4int l = 0; l < 4; l++)
        {
          ...
          /*
          G4cout<<i<<" "<<j<<" "<<k<<" "<<l<<" "<<Baryon[i][j][k][l]<<" "<<TestHadron<<"
          "<<BaryonWeight[i][j][k][l]; if (TestHadron != nullptr) G4cout<<"
          "<<TestHadron->GetParticleName(); if ((TestHadron == nullptr)&&(Baryon[i][j][k][l] != 0))
          G4cout<<" *****"; if ((TestHadron == nullptr)&&(Baryon[i][j][k][l] == 0)) G4cout<<"
          ---------------"; G4cout<<G4endl;
          */
...
void G4VLongitudinalStringDecay::SetMinimalStringMass(const G4FragmentingString* const string)
{
  ...
  if (LeftParton->GetParticleSubType() == RightParton->GetParticleSubType())
  {  // q qbar, qq qqbar
...
// G4cout<<"MinStringMass// Input String mass "<<string->Get4Momentum().mag()<<" Qleft
// "<<Qleft<<G4endl;
#endif
  ...
  if ((EstimatedMass1 > 0.) && (EstimatedMass2 > 0.))
  {
    ...
    if (StringM > EstimatedMass)
    {  // 2 baryon production is possible.
