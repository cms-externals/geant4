...
G4KineticTrackVector* G4ExcitedStringDecay::FragmentStrings(const G4ExcitedStringVector* theStrings)
{
  ...
  {
    // G4cout<<"theStrings->operator[](astring)->IsExcited() "<<" "<<astring<<"
    // "<<theStrings->operator[](astring)->IsExcited()<<G4endl;
  ...
  do
  {
    ...
    {
#ifdef debug_G4ExcitedStringDecay
      G4cout << "String No " << astring + 1 << " Excited?  "
             << theStrings->operator[](astring)->IsExcited() << G4endl;

      G4cout << "String No " << astring + 1 << " 4Momentum "
      ...
      else
      {
        ...
        G4KineticTrack* aTrack =
          new G4KineticTrack(theStrings->operator[](astring)->GetKineticTrack()->GetDefinition(),
...
#ifdef debug_G4ExcitedStringDecay
        G4cout << "   A particle stored in the track is "
      ...
      G4cout << "String secondaries(" << generatedKineticTracks->size() << ")" << G4endl
             ...
             << G4endl << "Final hadrons momentum: " << KTsum1 << G4endl;
#endif

      if (KTsum1.e() > 0
          && std::abs((KTsum1.e() - theStrings->operator[](astring)->Get4Momentum().e())
                      / KTsum1.e())
               > perMillion)
      {
  ...
  if (!success)
  {
    ...
    for (unsigned int astring = 0; astring < theStrings->size(); astring++)
    // for ( unsigned int astring=0; astring < 1; astring++) // Need more correct. For testing
    // purposes.
    {
      if (theStrings->operator[](astring)->IsExcited())
      {
                                                        ...
                                                        G4LorentzVector& TotalCollisionMom)
{
  ...
  std::vector<G4double> HadronMass;
  G4double HadronM(0.);

#ifdef debug_G4ExcitedStringCorr
  G4cout << G4endl << "EnergyAndMomentumCorrector. Number of particles: " << Output->size()
         << G4endl;
#endif
  ...
  for (cHadron = 0; cHadron < Output->size(); cHadron++)
  {
    ...
    HadronM = Output->operator[](cHadron)->Get4Momentum().mag();
    HadronMass.push_back(HadronM);
    SumMass += Output->operator[](cHadron)->Get4Momentum().mag();  // GetDefinition()->GetPDGMass();
  }

#ifdef debug_G4ExcitedStringCorr
  G4cout << "Sum part mom " << SumMom << " " << SumMom.mag() << G4endl << "Sum str  mom "
         << TotalCollisionMom << " " << TotalCollisionMom.mag() << G4endl;
...
#ifdef debug_G4ExcitedStringCorr
    G4cout << "Scale-1=" << Scale - 1 << ",  TotalCollisionMass=" << TotalCollisionMass
           << ",  Sum=" << Sum << G4endl;
#endif
