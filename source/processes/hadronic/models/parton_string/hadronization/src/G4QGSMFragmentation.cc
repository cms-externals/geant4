...
G4KineticTrackVector* G4QGSMFragmentation::FragmentString(const G4ExcitedString& theString)
{
  ...
  G4cout << G4endl << "QGSM StringFragm: String Mass " << theString.Get4Momentum().mag() << " Pz "
         << theString.Get4Momentum().pz() << "------------------------------------" << G4endl;
...
G4bool G4QGSMFragmentation::StopFragmenting(const G4FragmentingString* string)
{
  ...
  G4double x = (string->IsAFourQuarkString())
                 ? 0.005 * (smass - MinimalStringMass)
                                                 ...
                                                 G4FragmentingString* NewString)
{
  ...
  {
    attempt++;
    if (attempt > StringLoopInterrupt) return 0;
  ...
  } while (std::sqrt(HadronMassT2) + std::sqrt(ResidualMassT2)
           > StringMT); /* Loop checking, 07.08.2015, A.Ribon */
...
#ifdef debug_QGSMfragmentation
  G4cout << "string->GetDecayDirection() string->LightConeDecay() " << string->GetDecayDirection()
         << " " << string->LightConeDecay() << G4endl;
...
G4double G4QGSMFragmentation::GetLightConeZ(G4double zmin, G4double zmax,
                                            G4int /* PartonEncoding */,
                                            G4ParticleDefinition* /* pHadron */, G4double ptx,
                                            G4double pty)
{
  G4double lambda = 2.0 * (sqr(ptx) + sqr(pty)) / sqr(GeV);

#ifdef debug_QGSMfragmentation
  G4cout << "GetLightConeZ zmin zmax Parton pHadron " << zmin << " " << zmax
         << " " /*<< PartonEncoding */
  ...
  {
    d1 = FFq2q[absDecayQuarkCode - 1][absNewQuarkCode - 1][0];
    d2 = FFq2q[absDecayQuarkCode - 1][absNewQuarkCode - 1][1];
  }

  if ((absDecayQuarkCode < 6) && (absNewQuarkCode > 6))
  {
    qA = absNewQuarkCode / 1000;
    qB = (absNewQuarkCode % 1000) / 100;
    DiQnew = IndexDiQ[qA - 1][qB - 1];
    d1 = FFq2qq[absDecayQuarkCode - 1][DiQnew][0];
    d2 = FFq2q[absDecayQuarkCode - 1][DiQnew][1];
  }

  if ((absDecayQuarkCode > 6) && (absNewQuarkCode < 6))
  {
    q1 = absDecayQuarkCode / 1000;
    q2 = (absDecayQuarkCode % 1000) / 100;
    DiQold = IndexDiQ[q1 - 1][q2 - 1];
    d1 = FFqq2q[DiQold][absNewQuarkCode - 1][0];
    d2 = FFqq2q[DiQold][absNewQuarkCode - 1][1];
  }
  ...
  if (d1 < 0.)
  {
    ...
    qB = (absNewQuarkCode % 1000) / 100;
    DiQnew = IndexDiQ[qA - 1][qB - 1];
    d1 = FFqq2qq[DiQold][DiQnew][0];
    d2 = FFqq2qq[DiQold][DiQnew][1];
  }
  ...
  {
    z =
      0.5 * (zmin + zmax);  // Just a value between zmin and zmax, no physics considerations at all!
  }
...
G4bool G4QGSMFragmentation::SplitLast(G4FragmentingString* string, G4KineticTrackVector* LeftVector,
                                      G4KineticTrackVector* RightVector)
{
  ...
  G4cout << "StrMass " << ResidualMass << " q's " << string->GetLeftParton()->GetParticleName()
  ...
  G4double LeftHadronMass(0.);
  G4double RightHadronMass(0.);
  do
  {
    if (cClusterInterrupt++ >= ClusterLoopInterrupt) return false;
    LeftHadronMass = -MaxMass;
    RightHadronMass = -MaxMass;
    ...
    {  // Diquark and anti-diquark are on the string ends
      if (cClusterInterrupt++ >= ClusterLoopInterrupt) return false;
...
void G4QGSMFragmentation::SetFFq2q()  // q-> q' + Meson (q anti q')
{
  ...
  {
    FFq2q[i][0][0] = 2.0;
    FFq2q[i][0][1] = -arho + alft;  // q->d + (q dbar) Pi0, Eta, Eta', Rho0, omega
    FFq2q[i][1][0] = 2.0;
    FFq2q[i][1][1] = -arho + alft;  // q->u + (q ubar) Pi-, Rho-
    FFq2q[i][2][0] = 1.0;
    FFq2q[i][2][1] = -aphi + alft;  // q->s + (q sbar) K0, K*0
    FFq2q[i][3][0] = 1.0;
    FFq2q[i][3][1] = -aJPs + alft;  // q->c + (q+cbar) D-, D*-
    FFq2q[i][4][0] = 1.0;
    FFq2q[i][4][1] = -aUps + alft;  // q->b + (q bbar) EtaB, Upsilon
  }
}
...
void G4QGSMFragmentation::SetFFq2qq()  // q-> anti (q1'q2') + Baryon (q + q1 + q2)
{
  ...
  {
    FFq2qq[i][0][0] = 0.0;
    FFq2qq[i][0][1] = arho - 2.0 * an + alft;  // q->dd bar + (q dd)
    FFq2qq[i][1][0] = 0.0;
    FFq2qq[i][1][1] = arho - 2.0 * an + alft;  // q->ud bar + (q ud)
    FFq2qq[i][2][0] = 0.0;
    FFq2qq[i][2][1] = arho - 2.0 * ala + alft;  // q->sd bar + (q sd)
    FFq2qq[i][3][0] = 0.0;
    FFq2qq[i][3][1] = arho - 2.0 * alaC + alft;  // q->cd bar + (q cd)
    FFq2qq[i][4][0] = 0.0;
    FFq2qq[i][4][1] = arho - 2.0 * alaB + alft;  // q->bd bar + (q bd)
    FFq2qq[i][5][0] = 0.0;
    FFq2qq[i][5][1] = arho - 2.0 * an + alft;  // q->uu bar + (q uu)
    FFq2qq[i][6][0] = 0.0;
    FFq2qq[i][6][1] = arho - 2.0 * ala + alft;  // q->su bar + (q su)
    FFq2qq[i][7][0] = 0.0;
    FFq2qq[i][7][1] = arho - 2.0 * alaC + alft;  // q->cu bar + (q cu)
    FFq2qq[i][8][0] = 0.0;
    FFq2qq[i][8][1] = arho - 2.0 * alaB + alft;  // q->bu bar + (q bu)
    FFq2qq[i][9][0] = 0.0;
    FFq2qq[i][9][1] = arho - 2.0 * aXi + alft;  // q->ss bar + (q ss)
    FFq2qq[i][10][0] = 0.0;
    FFq2qq[i][10][1] = arho - 2.0 * aXiC + alft;  // q->cs bar + (q cs)
    FFq2qq[i][11][0] = 0.0;
    FFq2qq[i][11][1] = arho - 2.0 * aXiB + alft;  // q->bs bar + (q bc)
    FFq2qq[i][12][0] = 0.0;
    FFq2qq[i][12][1] = arho - 2.0 * aXiCC + alft;  // q->cc bar + (q cc)
    FFq2qq[i][13][0] = 0.0;
    FFq2qq[i][13][1] = arho - 2.0 * aXiCB + alft;  // q->bc bar + (q bc)
    FFq2qq[i][14][0] = 0.0;
    FFq2qq[i][14][1] = arho - 2.0 * aXiBB + alft;  // q->bb bar + (q bb)
  }
}
...
void G4QGSMFragmentation::SetFFqq2q()  // q1q2-> anti(q') + Baryon (q1 + q2 + q')
{
  ...
  {
    FFqq2q[i][0][0] = 2.0 * (arho - an);
    FFqq2q[i][0][1] = -arho + alft;
    FFqq2q[i][1][0] = 2.0 * (arho - an);
    FFqq2q[i][1][1] = -arho + alft;
    FFqq2q[i][2][0] = 2.0 * (arho - an);
    FFqq2q[i][2][1] = -aphi + alft;
    FFqq2q[i][3][0] = 2.0 * (arho - an);
    FFqq2q[i][3][1] = -aJPs + alft;
    FFqq2q[i][4][0] = 2.0 * (arho - an);
    FFqq2q[i][4][1] = -aUps + alft;
  }
}
...
void G4QGSMFragmentation::SetFFqq2qq()  // q1(q2)-> q'(q2) + Meson(q1 anti q')
{
  ...
  {
    FFqq2qq[i][0][0] = 0.;
    FFqq2qq[i][0][1] = 2.0 * arho - 2.0 * an - arho + alft;  // dd -> dd + Pi0 (d d bar)
    FFqq2qq[i][1][0] = 0.;
    FFqq2qq[i][1][1] = 2.0 * arho - 2.0 * an - arho + alft;  // dd -> ud + Pi- (d u bar)
    FFqq2qq[i][2][0] = 0.;
    FFqq2qq[i][2][1] = 2.0 * arho - 2.0 * an - aphi + alft;  // dd -> sd + K0  (d s bar)
    FFqq2qq[i][3][0] = 0.;
    FFqq2qq[i][3][1] = 2.0 * arho - 2.0 * an - aJPs + alft;  // dd -> cd + D-  (d c bar)
    FFqq2qq[i][4][0] = 0.;
    FFqq2qq[i][4][1] = 2.0 * arho - 2.0 * an - aUps + alft;  // dd -> bd + B0  (d b bar)
  }
}
