  ...
  : G4VLongitudinalStringDecay("LundStringFragmentation")
{
  ...
  {
    SetProbCCbar(
      0.0002);  // According to O.I. Piskunova Yad. Fiz. 56 (1993) 1094; tuned by Uzhi Oct. 2022
...
G4KineticTrackVector* G4LundStringFragmentation::FragmentString(const G4ExcitedString& theString)
{
  ...
  G4cout << G4endl << "LUND StringFragm: String Mass " << theString.Get4Momentum().mag() << G4endl
         << "4Mom " << theString.Get4Momentum() << G4endl << "------------------------------------"
         << G4endl;
  G4cout << "String ends Direct " << theString.GetLeftParton()->GetPDGcode() << " "
         << theString.GetRightParton()->GetPDGcode() << " " << theString.GetDirection() << G4endl;
...
G4bool G4LundStringFragmentation::IsItFragmentable(const G4FragmentingString* const string)
{
  ...
  // MinimalStringMass is negative and large for a string with unknown particles in a final
  // 2-particle decay.
}
                                                        ...
                                                        G4KineticTrackVector*& RightVector)
{
  ...
  G4cout << "Loop_toFrag " << theString.GetLeftParton()->GetPDGcode() << " "
         ...
         << theString.GetRightParton()->Get4Momentum() << G4endl << "Direction   "
...
// G4cout<<"2 "<<currentString->GetDecayDirection()<<G4endl;
    ...
    if (inner_success
        && SplitLast(currentString, LeftVector, RightVector))  // Close to protect Last Str. Decay
    {
      final_success = true;
    }
...
G4bool G4LundStringFragmentation::StopFragmenting(const G4FragmentingString* const string)
{
  ...
  {
    if (MinimalStringMass < 0.0)
      return false;  // For a string with di-quark having c or b quarks and s, c, b quarks

    G4bool Result =
      G4UniformRand()
      < G4Exp(-0.66e-6 * (string->Mass() * string->Mass() - MinimalStringMass * MinimalStringMass));
    // G4bool Result = string->Mass() < MinimalStringMass + 150.*MeV*G4UniformRand();     // a'la
    // LUND
                                                   ...
                                                   G4FragmentingString*& newString)
{
...
#ifdef debug_LUNDfragmentation
  G4cout << "StrMass DiquarkSuppression           " << StringMass << " " << GetDiquarkSuppress()
         << G4endl;
#endif
  ...
  ActualProb *=
    (1.0 - G4Pow::GetInstance()->powA(NumberOfpossibleBaryons * 1400.0 / StringMass, 8.0));
  if (ActualProb < 0.0) ActualProb = 0.;

  SetDiquarkSuppression(ActualProb);

  G4double Mth = 1250.0;  // 2 Mk + Mpi
  if (NumberOfpossibleBaryons == 3)
  {
    Mth = 2520.0;
  }  // Mlambda/Msigma + Mk + Mpi
  else if (NumberOfpossibleBaryons == 4)
  {
    Mth = 2380.0;
  }  // 2 Mlambda/Msigma + Mk + Mpi
  else
  {}
...
#ifdef debug_LUNDfragmentation
  G4cout << "StrMass DiquarkSuppression corrected " << StringMass << " " << GetDiquarkSuppress()
         << G4endl;
#endif
  ...
  G4cout << "The parton " << string->GetDecayParton()->GetPDGEncoding() << " "
         << " produces hadron " << HadronDefinition->GetParticleName() << " and is transformed to "
         << newStringEnd->GetPDGEncoding() << G4endl;
  ...
  newString = new G4FragmentingString(
    *string, newStringEnd);  // To store possible quark containt of new string
                                                                ...
                                                                G4ParticleDefinition*& created)
{
  ...
  else
  {
    ...
    G4int IsParticle =
      (decay->GetPDGEncoding() > 0) ? +1 : -1;  // if we have a diquark, we need quark
                                                       ...
                                                       G4FragmentingString* newString)
{
  ...
  {
    attempt++;
    if (attempt > StringLoopInterrupt)
    {
      return 0;
    }

    HadronMt = HadronMass - TmtCur * G4Log(G4UniformRand());
    Pt2 = sqr(HadronMt) - sqr(HadronMass);
    Pt = std::sqrt(Pt2);
  ...
  G4double Pz2 =
    (sqr(StringMT2 - HadronMassT2 - ResidualMassT2) - 4 * HadronMassT2 * ResidualMassT2) / 4.
    / StringMT2;
  ...
  // G4double zMin = (std::sqrt(HadronMassT2+Pz2) - 0.)/std::sqrt(StringMT2); // For testing
  // purposes
  G4double zMax = (std::sqrt(HadronMassT2 + Pz2) + Pz) / std::sqrt(StringMT2);

  if (zMin >= zMax) return 0;  // have to start all over!

  G4double z = GetLightConeZ(zMin, zMax, string->GetDecayParton()->GetPDGEncoding(), pHadron,
  ...
  G4double HadronE =
    0.5 * (z * string->LightConeDecay() + HadronMassT2 / (z * string->LightConeDecay()));

  G4LorentzVector* a4Momentum = new G4LorentzVector(HadronPt, HadronE);

#ifdef debug_LUNDfragmentation
  G4cout << G4endl << " string->GetDecayDirection() " << string->GetDecayDirection() << G4endl
         << G4endl;
...
G4double G4LundStringFragmentation::GetLightConeZ(G4double zmin, G4double zmax,
                                                  ...
                                                  G4ParticleDefinition* pHadron, G4double Px,
                                                  G4double Py)
{
  ...
  {  // ---------------- Quark fragmentation  and qq-> meson ----------------------
    ...
    {
      zOfMaxyf =
        ((1.0 + BMt2) - std::sqrt(sqr(1.0 - BMt2) + 4.0 * BMt2 * Alund)) / 2.0 / (1. - Alund);
    }
    ...
    {
      z = 0.5
          * (zmin + zmax);  // Just a value between zmin and zmax, no physics considerations at all!
    }
    return z;
  }
                                            ...
                                            G4KineticTrackVector* RightVector)
{
  ...
  G4cout << "Left  " << string->GetLeftParton()->GetPDGEncoding() << " " << string->GetPleft()
         << G4endl;
  G4cout << "Right " << string->GetRightParton()->GetPDGEncoding() << " " << string->GetPright()
         << G4endl;
...
#ifdef debug_LUNDfragmentation
  G4cout << "StrMass " << StringMass << " q's " << string->GetLeftParton()->GetParticleName() << " "
...
#ifdef debug_LUNDfragmentation
  G4cout << "Sampled hadrons: " << LeftHadron->GetParticleName() << " "
         << RightHadron->GetParticleName() << G4endl;
#endif
  ...
  Sample4Momentum(&LeftMom, LeftHadron->GetPDGMass(), &RightMom, RightHadron->GetPDGMass(),
                  StringMass);
...
G4bool G4LundStringFragmentation::Diquark_AntiDiquark_belowThreshold_lastSplitting(
  G4FragmentingString*& string, G4ParticleDefinition*& LeftHadron,
  G4ParticleDefinition*& RightHadron)
{
  ...
  do
  {
    ...
    if (G4UniformRand() < 0.5)
    {
      ...
      RightHadron = (LeftHadron == nullptr)
                      ? nullptr
                      : hadronizer->Build(FindParticle(LeftQuark2), FindParticle(RightQuark2));
    }
    else
    {
      ...
      RightHadron = (LeftHadron == nullptr)
                      ? nullptr
                      : hadronizer->Build(FindParticle(LeftQuark2), FindParticle(RightQuark1));
    }
    ...
    if (isOK)
    {
      isOK = (StringMass > LeftHadron->GetPDGMass() + RightHadron->GetPDGMass());
    }
    ++cClusterInterrupt;
...
G4bool G4LundStringFragmentation::Diquark_AntiDiquark_aboveThreshold_lastSplitting(
  G4FragmentingString*& string, G4ParticleDefinition*& LeftHadron,
  G4ParticleDefinition*& RightHadron)
{
                                                              ...
                                                              G4ParticleDefinition*& RightHadron)
{
  ...
  NumberOf_FS = 0;
  for (G4int ProdQ = 1; ProdQ < 4;
       ProdQ++)  // Loop over quark-antiquark cases: u-ubar, d-dbar, s-sbar
      ...
      do  // while(Baryon[Di_q1-1][Di_q2-1][ProdQ-1][StateDiQ]<>0);
      {
        ...
        if (StringMass > LeftHadronMass + RightHadronMass)
        {
          if (NumberOf_FS > 349)
          {
            G4ExceptionDescription ed;
            ed << " NumberOf_FS exceeds its limit: NumberOf_FS=" << NumberOf_FS << G4endl;
            G4Exception("G4LundStringFragmentation::Quark_Diquark_lastSplitting ", "HAD_LUND_002",
                        JustWarning, ed);
            NumberOf_FS = 349;
          }

          G4double FS_Psqr = lambda(StringMassSqr, sqr(LeftHadronMass), sqr(RightHadronMass));
          FS_Weight[NumberOf_FS] =
            std::sqrt(FS_Psqr) * MesonWeight[AbsIDquark - 1][ProdQ - 1][StateQ]
            * BaryonWeight[Di_q1 - 1][Di_q2 - 1][ProdQ - 1][StateDiQ] * Prob_QQbar[ProdQ - 1];
                                                                ...
                                                                G4ParticleDefinition*& RightHadron)
{
  ...
  {  // (as last splitting, do not consider c-cbar and b-bbar cases)
    ...
    G4int SignQ = LeftHadronCharge / 3;
    if (SignQ == 0) SignQ = 1;
    ...
    G4int SignAQ = RightHadronCharge / 3;
    ...
    do
    {
      ...
      do
      {
        ...
        //       <<ProdQ-1<<" "<<StateAQ<<"
        //       "<<SignAQ*Meson[AbsIDanti_quark-1][ProdQ-1][StateAQ]<<G4endl;
        RightHadron = G4ParticleTable::GetParticleTable()->FindParticle(
        ...
        if (StringMass > LeftHadronMass + RightHadronMass)
        {
          if (NumberOf_FS > 349)
          {
            G4ExceptionDescription ed;
            ed << " NumberOf_FS exceeds its limit: NumberOf_FS=" << NumberOf_FS << G4endl;
            G4Exception("G4LundStringFragmentation::Quark_AntiQuark_lastSplitting ", "HAD_LUND_003",
                        JustWarning, ed);
            NumberOf_FS = 349;
          }
          ...
          FS_Weight[NumberOf_FS] =
            std::sqrt(FS_Psqr) * MesonWeight[AbsIDquark - 1][ProdQ - 1][StateQ]
            * MesonWeight[AbsIDanti_quark - 1][ProdQ - 1][StateAQ] * Prob_QQbar[ProdQ - 1];
          if (string->GetLeftParton()->GetPDGEncoding() > 0)
          {
    ...
    } while ((Meson[AbsIDquark - 1][ProdQ - 1][StateQ] != 0) && ++loopCounter < maxNumberOfLoops);
    if (loopCounter >= maxNumberOfLoops)
    {
      return false;
    }
                                                ...
                                                G4double InitialMass)
{
  ...
  G4cout << "Init Mass " << InitialMass << " FirstM " << Mass << " SecondM " << AntiMass
         << " ProbIsotropy " << G4endl;
#endif

  G4double r_val =
    sqr(InitialMass * InitialMass - Mass * Mass - AntiMass * AntiMass) - sqr(2. * Mass * AntiMass);
  ...
  if (Mass < 930. && AntiMass < 930.)
  {}  // q-qbar string
  if ((Mass < 930. && AntiMass > 930.) || (Mass > 930. && AntiMass < 930.))
  {  // q-di_q string
  ...
  do
  {
    Pt = SampleQuarkPt(Pabs);
    Pt.setZ(0);
    G4double Pt2 = Pt.mag2();
  ...
  AvailablePz2 =
    sqr(InitialMass * InitialMass - sqr(MassMt) - sqr(AntiMassMt)) - 4. * sqr(MassMt * AntiMassMt);
