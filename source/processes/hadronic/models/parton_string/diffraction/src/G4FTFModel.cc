...
{
    void operator()(G4VSplitableHadron* aH) { delete aH; }
};
...
G4FTFModel::~G4FTFModel()
{
  ...
  {
    std::for_each(theAdditionalString.begin(), theAdditionalString.end(), DeleteVSplitableHadron());
  }
...
void G4FTFModel::Init(const G4Nucleus& aNucleus, const G4DynamicParticle& aProjectile)
{
  ...
  G4cout << "FTF init Proj Name " << theProjectile.GetDefinition()->GetParticleName() << G4endl
         << "FTF init Proj Mass " << theProjectile.GetMass() << " " << theProjectile.GetMomentum()
         << G4endl << "FTF init Proj B Q  " << theProjectile.GetDefinition()->GetBaryonNumber()
         << " " << (G4int)theProjectile.GetDefinition()->GetPDGCharge() << G4endl
         << "FTF init Target A Z " << aNucleus.GetA_asInt() << " " << aNucleus.GetZ_asInt()
         << G4endl;
#endif
  ...
  TargetResidualExcitationEnergy = 0.0;
  TargetResidual4Momentum = tmp;
  G4double TargetResidualMass = G4ParticleTable::GetParticleTable()->GetIonTable()->GetIonMass(
  ...
  else
  {
    ...
    else if (theProjectile.GetDefinition()->GetBaryonNumber() < -1)
    {
      ...
      ProjectileResidualLambdaNumber =
        theProjectile.GetDefinition()->GetNumberOfAntiLambdasInAntiHypernucleus();
      PlabPerParticle = theProjectile.GetMomentum().z() / ProjectileResidualMassNumber;
      if (PlabPerParticle < LowEnergyLimit)
      {
        HighEnergyInter = false;
  ...
  if (std::abs(theProjectile.GetDefinition()->GetBaryonNumber()) <= 1 && aNucleus.GetA_asInt() < 2)
    theParameters->SetProbabilityOfElasticScatt(0.0);
...
#ifdef debugFTFmodel
    G4cout << "FTF BuildStrings " << theStrings << " OK" << G4endl << "FTF GetResiduals of Nuclei "
           << G4endl;
#endif
...
#ifdef debugFTFmodel
  G4cout << "End of FTF. Go to fragmentation" << G4endl << "To continue - enter 1, to stop - ^C"
         << G4endl;
#endif
...
#ifdef debugReggeonCascade
  G4cout << "Final NumberOfInvolvedNucleonsOfTarget " << NumberOfInvolvedNucleonsOfTarget << G4endl
         << G4endl;
#endif
...
G4bool G4FTFModel::PutOnMassShell()
{
  ...
  {  // nucleus-nucleus or antinucleus-nucleus collision
#ifdef debugPutOnMassShell
    G4cout << "Projectile : ";
#endif
  ...
  if (isProjectileNucleus)
  {
    ...
    SumMasses +=
      std::sqrt(sqr(PrResidualMass + ProjectileResidualExcitationEnergy) + PprojResidual.perp2());
  }
  SumMasses -= std::sqrt(sqr(TargetResidualMass) + PtargetResidual.perp2());
  SumMasses +=
    std::sqrt(sqr(TargetResidualMass + TargetResidualExcitationEnergy) + PtargetResidual.perp2());
  ...
  G4cout << "TargetResidualMass TargetResidualExcitationEnergy " << TargetResidualMass / GeV << " "
         << TargetResidualExcitationEnergy << " MeV" << G4endl << "Sum masses " << SumMasses / GeV
         << G4endl;
#endif
  ...
  G4cout << "Y targetNucleus     " << YtargetNucleus << G4endl << "Dcor "
         << theParameters->GetDofNuclearDestruction() << " DcorP DcorT " << DcorP << " " << DcorT
  ...
  {  // while ( ! OuterSuccess )
    ...
    {  // while ( SqrtS < Mprojectile + std::sqrt( M2target ) )
      ...
      if (isProjectileNucleus)
      {
        ...
        isOk = SamplingNucleonKinematics(
          AveragePt2, maxPtSquare, DcorP, thePrNucleus, PprojResidual, PrResidualMass,
      ...
      isOk = isOk
             && SamplingNucleonKinematics(
               AveragePt2, maxPtSquare, DcorT, theTargetNucleus, PtargetResidual,
               TargetResidualMass, TargetResidualMassNumber, NumberOfInvolvedNucleonsOfTarget,
               TheInvolvedNucleonsOfTarget, M2target);
#ifdef debugPutOnMassShell
      G4cout << "SqrtS, Mp+Mt, Mp, Mt " << SqrtS / GeV << " "
             << (std::sqrt(M2proj) + std::sqrt(M2target)) / GeV << " " << std::sqrt(M2proj) / GeV
    ...
    if (isProjectileNucleus)
    {
      isOk = CheckKinematics(S, SqrtS, M2proj, M2target, YprojectileNucleus, true,
                             NumberOfInvolvedNucleonsOfProjectile, TheInvolvedNucleonsOfProjectile,
...
G4bool G4FTFModel::ExciteParticipants()
{
  ...
  { /* Loop checking, 10.08.2015, A.Ribon */
    ...
    G4cout << G4endl << "Interaction # Status    " << CurrentInteraction << " "
           << collision.GetStatus() << G4endl << "Pr* Tr* " << projectile << " " << target << G4endl
    ...
    if (collision.GetStatus())
    {
      if (G4UniformRand() < theParameters->GetProbabilityOfElasticScatt())
      {
        ...
        if (!HighEnergyInter)
        {
          ...
          G4bool Result =
            AdjustNucleons(projectile, ProjectileNucleon, target, TargetNucleon, Annihilation);
          if (!Result) continue;
        }
...
#ifdef debugBuildString
        G4cout << "Inelastic interaction" << G4endl << "MaxNumOfInelCollisions per hadron/nucleon "
...
//        << target->Get4Momentum().mag() << G4endl;
          ...
          else
          {
            InnerSuccess = theElastic->ElasticScattering(projectile, target, theParameters);
#ifdef debugBuildString
            G4cout << "FTF excitation Non InnerSuccess of Elastic scattering " << InnerSuccess
                   << G4endl;
#endif
          }
        }
      ...
      {  // Annihilation
        ...
        if (theAnnihilation->Annihilate(projectile, target, AdditionalString, theParameters))
        {
          ...
          fIsQuasiElasticInteraction = false;
#ifdef debugBuildString
          G4cout << "Annihilation successfull. " << "*AdditionalString " << AdditionalString
                 << G4endl;
...
// G4cout << "After  tar " << target->Get4Momentum() << G4endl;
          ...
          theParticipants.StartLoop();
          for (G4int i = 0; i < CurrentInteraction; ++i)
            theParticipants.Next();
...
G4bool G4FTFModel::AdjustNucleons(G4VSplitableHadron* SelectedAntiBaryon,
                                  ...
                                  G4Nucleon* TargetNucleon, G4bool Annihilation)
{
#ifdef debugAdjust
  G4cout << "AdjustNucleons ---------------------------------------" << G4endl
         << "Proj is nucleus? " << GetProjectileNucleus() << G4endl << "Proj 4mom "
         << SelectedAntiBaryon->Get4Momentum() << G4endl << "Targ 4mom "
         << SelectedTargetNucleon->Get4Momentum() << G4endl
         ...
         << TargetResidualMassNumber << " " << TargetResidualCharge << " "
         << TargetResidualExcitationEnergy << G4endl << "Collis. pr tr "
         << SelectedAntiBaryon->GetSoftCollisionCount() << " "
  ...
  if ((!GetProjectileNucleus() && SelectedAntiBaryon->GetSoftCollisionCount() == 0
       && SelectedTargetNucleon->GetSoftCollisionCount() == 0)
           ...
           && SelectedTargetNucleon->GetSoftCollisionCount() != 0)
  {
    ...
    if (ProjectileResidual4Momentum.rapidity() <= SelectedTargetNucleon->Get4Momentum().rapidity())
    {
      return false;
    }
    if (ProjectileResidualMassNumber == 1)
  ...
  {  // It has to be a nucleus-nucleus interaction
    interactionCase = 3;
#ifdef debugAdjust
...
#ifdef debugAdjust
    G4cout
      << "Proj res Init " << ProjectileResidual4Momentum << G4endl << "Targ res Init "
  ...
  G4int returnCode = AdjustNucleonsAlgorithm_beforeSampling(
    interactionCase, SelectedAntiBaryon, ProjectileNucleon, SelectedTargetNucleon, TargetNucleon,
    Annihilation, common);
  ...
  else if (returnCode == 1)
  {
    ...
    if (returnResult)
    {  // The sampling has completed successfully: do the last part
...
G4int G4FTFModel::AdjustNucleonsAlgorithm_beforeSampling(
  G4int interactionCase, G4VSplitableHadron* SelectedAntiBaryon, G4Nucleon* ProjectileNucleon,
  G4VSplitableHadron* SelectedTargetNucleon, G4Nucleon* TargetNucleon, G4bool Annihilation,
  G4FTFModel::CommonVariables& common)
{
  ...
  if (interactionCase == 1)
  {
    ...
    common.TResidualCharge =
      TargetResidualCharge - G4int(TargetNucleon->GetDefinition()->GetPDGCharge());
    common.TResidualExcitationEnergy =
      TargetResidualExcitationEnergy - ExcitationEnergyPerWoundedNucleon * G4Log(G4UniformRand());
    ...
    {
      common.TResidualMass = G4ParticleTable::GetParticleTable()->GetIonTable()->GetIonMass(
        common.TResidualCharge, common.TResidualMassNumber);
    }
    common.TNucleonMass = TargetNucleon->GetDefinition()->GetPDGMass();
    common.SumMasses =
      SelectedAntiBaryon->Get4Momentum().mag() + common.TNucleonMass + common.TResidualMass;
#ifdef debugAdjust
  ...
  else if (interactionCase == 2)
  {
    ...
    common.SumMasses =
      SelectedTargetNucleon->Get4Momentum().mag() + common.TNucleonMass + common.TResidualMass;
#ifdef debugAdjust
  ...
  else if (interactionCase == 3)
  {
    ...
    if (common.PResidualMassNumber != 0)
    {
      ...
      else
      {
        if (common.PResidualLambdaNumber > 0)
        {
          ...
          {
            common.PResidualMass = G4HyperNucleiProperties::GetNuclearMass(
              common.PResidualMassNumber, std::abs(common.PResidualCharge),
              common.PResidualLambdaNumber);
          }
        }
        else
        {
          common.PResidualMass = G4ParticleTable::GetParticleTable()->GetIonTable()->GetIonMass(
            std::abs(common.PResidualCharge), common.PResidualMassNumber);
        }
      }
    }
    common.PNucleonMass =
      ProjectileNucleon->GetDefinition()->GetPDGMass();  // On-shell (anti-)nucleon mass
    common.TResidualMassNumber = TargetResidualMassNumber - 1;
    common.TResidualCharge =
      TargetResidualCharge - G4int(TargetNucleon->GetDefinition()->GetPDGCharge());
    common.TResidualExcitationEnergy =
    ...
    common.SumMasses =
      common.PNucleonMass + common.PResidualMass + common.TNucleonMass + common.TResidualMass;
#ifdef debugAdjust
    G4cout << "PNucleonMass PResidualMass TNucleonMass TResidualMass " << common.PNucleonMass << " "
           << common.PResidualMass << " " << common.TNucleonMass << " " << common.TResidualMass
           << G4endl << "PResidualExcitationEnergy " << common.PResidualExcitationEnergy << G4endl
  ...
  if (!Annihilation)
  {
    ...
    if (interactionCase == 1 || interactionCase == 2)
    {
      ...
      {
#ifdef debugAdjust
        G4cout << "TResidualExcitationEnergy : before " << common.TResidualExcitationEnergy
               << G4endl;
#endif
        common.TResidualExcitationEnergy = common.SqrtS - common.SumMasses;
#ifdef debugAdjust
        G4cout << "TResidualExcitationEnergy : after " << common.TResidualExcitationEnergy
               << G4endl;
#endif
        Stopping = true;
        return returnCode;  // unsuccessfully ended, nothing else can be done
    ...
    else if (interactionCase == 3)
    {
      ...
      G4cout << "SqrtS < SumMasses + PResidualExcitationEnergy + TResidualExcitationEnergy "
             << common.SqrtS << " "
             << common.SumMasses + common.PResidualExcitationEnergy
                  + common.TResidualExcitationEnergy
             << G4endl;
#endif
      if (common.SqrtS
  ...
  if (Stopping)
  {
    ...
    common.Ptmp.setPx(0.0);
    common.Ptmp.setPy(0.0);
    common.Ptmp.setPz(0.0);
    // New projectile
    if (interactionCase == 1)
    {
    ...
    if (interactionCase == 1 || interactionCase == 3)
    {
      ...
      common.Ptmp.setE(std::sqrt(sqr(common.TResidualMass + TargetResidualExcitationEnergy)
                                 + common.Ptmp.vect().mag2()));
//---
#ifdef debugAdjust
    ...
    if (interactionCase == 2 || interactionCase == 3)
    {
      ...
      if (interactionCase == 2)
      {
        ...
        ProjectileResidualLambdaNumber =
          0;  // The target nucleus and its residual are never hypernuclei
      ...
      else
      {
        ...
        common.Ptmp.setE(std::sqrt(sqr(common.PResidualMass + ProjectileResidualExcitationEnergy)
                                   + common.Ptmp.vect().mag2()));
        //---
      }
#ifdef debugAdjust
                                                    ...
                                                    G4FTFModel::CommonVariables& common)
{
  ...
  G4int NumberOfTries = 0;
  do
  {  // Outmost do while loop
    OuterSuccess = true;
    G4bool loopCondition = false;
    do
    {  // Intermediate do while loop
      if (NumberOfTries == 100 * (NumberOfTries / 100))
      {
...
// G4cout << "NumberOfTries ScaleFactor " << NumberOfTries << " " << ScaleFactor << G4endl;
      ...
      {
        G4bool InnerSuccess = true;
        G4bool isTargetToBeHandled =
          (interactionCase == 1 || (interactionCase == 3 && iExecute == 1));
          ...
          {  // Inner do while loop
            ...
            if (isTargetToBeHandled)
            {
              ...
              if (interactionCase == 1)
              {
                ...
                Xcenter =
                  std::sqrt(sqr(common.TNucleonMass) + common.PtNucleon.mag2()) / common.Mtarget;
              }
              else
              {
                Xcenter =
                  std::sqrt(sqr(common.TNucleonMass) + common.PtNucleonT.mag2()) / common.Mtarget;
              }
            ...
            {  // Projectile to be handled
              ...
              common.XplusResidual = 1.0 - common.XplusNucleon;
                   ...
                   ++innerLoopCounter
                     < maxNumberOfInnerLoops); /* Loop checking, 10.08.2015, A.Ribon */
        ...
        {  // condition is false
          if (isTargetToBeHandled)
          {
            ...
            common.XminusResidual =
              1.0;  // It must be 0, but in the calculation of Pz, E is problematic
          }
          else
          {  // Projectile to be handled
      ...
      {
        common.M2target =
          (sqr(common.TNucleonMass) + common.PtNucleon.mag2()) / common.XminusNucleon
          + (sqr(common.TResidualMass) + common.PtResidual.mag2()) / common.XminusResidual;
...
#endif
        common.M2projectile =
          (sqr(common.TNucleonMass) + common.PtNucleon.mag2()) / common.XplusNucleon
          + (sqr(common.TResidualMass) + common.PtResidual.mag2()) / common.XplusResidual;
#ifdef debugAdjust
      ...
      else if (interactionCase == 3)
      {
        ...
        G4cout << "PtNucleonP " << common.PtNucleonP << " " << common.PtResidualP << G4endl
               ...
               << common.XplusResidual << G4endl << "PtNucleonT " << common.PtNucleonT << " "
               << common.PtResidualT << G4endl << "XminusNucleon XminusResidual "
               << common.XminusNucleon << " " << common.XminusResidual << G4endl;
#endif
        common.M2projectile =
          (sqr(common.PNucleonMass) + common.PtNucleonP.mag2()) / common.XplusNucleon
          + (sqr(common.PResidualMass) + common.PtResidualP.mag2()) / common.XplusResidual;
        common.M2target =
          (sqr(common.TNucleonMass) + common.PtNucleonT.mag2()) / common.XminusNucleon
          + (sqr(common.TResidualMass) + common.PtResidualT.mag2()) / common.XminusResidual;
        loopCondition =
          (common.SqrtS < (std::sqrt(common.M2projectile) + std::sqrt(common.M2target)));
      }  // End-if on interactionCase
    ...
    G4double DecayMomentum2 = sqr(common.S) + sqr(common.M2projectile) + sqr(common.M2target)
                              - 2.0
                                  * (common.S * (common.M2projectile + common.M2target)
                                     + common.M2projectile * common.M2target);
    if (interactionCase == 1)
    {
      common.WminusTarget =
        (common.S - common.M2projectile + common.M2target + std::sqrt(DecayMomentum2)) / 2.0
        / common.SqrtS;
      common.WplusProjectile = common.SqrtS - common.M2target / common.WminusTarget;
      common.Pzprojectile =
        common.WplusProjectile / 2.0 - common.M2projectile / 2.0 / common.WplusProjectile;
      common.Eprojectile =
        common.WplusProjectile / 2.0 + common.M2projectile / 2.0 / common.WplusProjectile;
      Yprojectile = 0.5
      ...
      G4cout << "DecayMomentum2 " << DecayMomentum2 << G4endl << "WminusTarget WplusProjectile "
             << common.WminusTarget << " " << common.WplusProjectile << G4endl << "Yprojectile "
             << Yprojectile << G4endl;
#endif
      common.Mt2targetNucleon = sqr(common.TNucleonMass) + common.PtNucleon.mag2();
      common.PztargetNucleon =
        -common.WminusTarget * common.XminusNucleon / 2.0
        + common.Mt2targetNucleon / (2.0 * common.WminusTarget * common.XminusNucleon);
      ...
      G4cout << "YtN Ytr YtN-Ytr " << " " << YtargetNucleon << " " << common.YtargetNucleus << " "
             << YtargetNucleon - common.YtargetNucleus << G4endl << "YtN Ypr YtN-Ypr " << " "
             << YtargetNucleon << " " << Yprojectile << " " << YtargetNucleon - Yprojectile
             << G4endl;
#endif
      if (std::abs(YtargetNucleon - common.YtargetNucleus) > 2 || Yprojectile < YtargetNucleon)
      {
        OuterSuccess = false;
        continue;
      }
    ...
    else if (interactionCase == 2)
    {
      ...
      Ytarget =
        0.5 * G4Log((common.Etarget + common.Pztarget) / (common.Etarget - common.Pztarget));
#ifdef debugAdjust
      G4cout << "DecayMomentum2 " << DecayMomentum2 << G4endl << "WminusTarget WplusProjectile "
             << common.WminusTarget << " " << common.WplusProjectile << G4endl << "Ytarget "
             << Ytarget << G4endl;
#endif
      common.Mt2projectileNucleon = sqr(common.TNucleonMass) + common.PtNucleon.mag2();
      common.PzprojectileNucleon =
        common.WplusProjectile * common.XplusNucleon / 2.0
        - common.Mt2projectileNucleon / (2.0 * common.WplusProjectile * common.XplusNucleon);
      common.EprojectileNucleon =
        common.WplusProjectile * common.XplusNucleon / 2.0
        + common.Mt2projectileNucleon / (2.0 * common.WplusProjectile * common.XplusNucleon);
      YprojectileNucleon = 0.5
...
#ifdef debugAdjust
    G4cout << "Tr N R " << common.Ptarget << G4endl << "       " << TargetResidual4Momentum
           << G4endl;
#endif
  }
...
#ifdef debugAdjust
    G4cout << "ProjectileResidualMassNumber ProjectileResidualCharge  Lambdas "
              "ProjectileResidualExcitationEnergy "
           << ProjectileResidualMassNumber << " " << ProjectileResidualCharge << " "
           << ProjectileResidualLambdaNumber << " " << ProjectileResidualExcitationEnergy << G4endl;
#endif
...
#ifdef debugAdjust
    G4cout << "Pr N R " << common.Pprojectile << G4endl << "       " << ProjectileResidual4Momentum
           << G4endl;
#endif
  }
}
...
void G4FTFModel::BuildStrings(G4ExcitedStringVector* strings)
{
  ...
  if (!GetProjectileNucleus())
  {
...
#ifdef debugBuildString
    G4cout << "G4FTFModel::BuildStrings()" << G4endl << "Number of projectile strings "
           << primaries.size() << G4endl;
#endif
    ...
    for (unsigned int ahadron = 0; ahadron < primaries.size(); ahadron++)
    {
      ...
      {
        theExcitation->CreateStrings(primaries[ahadron], isProjectile, FirstString, SecondString,
                                     theParameters);
               ...
               && primaries[ahadron]->GetSoftCollisionCount() == 0)
      {
        ...
        G4KineticTrack* aTrack = new G4KineticTrack(
          primaries[ahadron]->GetDefinition(), primaries[ahadron]->GetTimeOfCreation(),
          primaries[ahadron]->GetPosition(), ParticleMomentum);
  ...
  {  // Projectile is a nucleus
    ...
    for (G4int ahadron = 0; ahadron < NumberOfInvolvedNucleonsOfProjectile; ahadron++)
    {
      ...
      {  // A nucleon took part in non-diffractive interaction
        ...
        theExcitation->CreateStrings(TheInvolvedNucleonsOfProjectile[ahadron]->GetSplitableHadron(),
               ...
               && HighEnergyInter)
      {
        ...
        G4KineticTrack* aTrack =
          new G4KineticTrack(aProjectile->GetDefinition(), aProjectile->GetTimeOfCreation(),
                             aProjectile->GetPosition(), ParticleMomentum);
        FirstString = new G4ExcitedString(aTrack);
  ...
  for (G4int ahadron = 0; ahadron < NumberOfInvolvedNucleonsOfTarget; ahadron++)
  {
    ...
    {  // A nucleon took part in non-diffractive interaction
      theExcitation->CreateStrings(aNucleon, isProjectile, FirstString, SecondString,
                                   theParameters);
             ...
             && HighEnergyInter)
    {
      ...
      G4KineticTrack* aTrack =
        new G4KineticTrack(aNucleon->GetDefinition(), aNucleon->GetTimeOfCreation(),
                           aNucleon->GetPosition(), ParticleMomentum);
  ...
  if (theAdditionalString.size() != 0)
  {
    for (unsigned int ahadron = 0; ahadron < theAdditionalString.size(); ahadron++)
    {
      ...
      FirstString = 0;
      SecondString = 0;
      theExcitation->CreateStrings(theAdditionalString[ahadron], isProjectile, FirstString,
...
void G4FTFModel::GetResiduals()
{
...
#ifdef debugFTFmodel
  G4cout << "GetResiduals(): HighEnergyInter? GetProjectileNucleus()?" << HighEnergyInter << " "
  ...
  if (HighEnergyInter)
  {
    ...
    G4double DeltaExcitationE =
      TargetResidualExcitationEnergy / G4double(NumberOfInvolvedNucleonsOfTarget);
    G4LorentzVector DeltaPResidualNucleus =
      TargetResidual4Momentum / G4double(NumberOfInvolvedNucleonsOfTarget);
    ...
    if (TargetResidualMassNumber != 0)
    {
      ...
      {
#ifdef debugFTFmodel
        G4cout << "BAD situation: forced exit of the first while loop in G4FTFModel::GetResidual"
               << G4endl << "\t return immediately from the method!" << G4endl;
#endif
        return;
      }
    ...
    DeltaExcitationE =
      ProjectileResidualExcitationEnergy / G4double(NumberOfInvolvedNucleonsOfProjectile);
    DeltaPResidualNucleus =
      ProjectileResidual4Momentum / G4double(NumberOfInvolvedNucleonsOfProjectile);
    ...
    if (ProjectileResidualMassNumber != 0)
    {
      ...
      G4Nucleon* aNucleon = 0;
      theProjectileNucleus->StartLoop();
      while ((aNucleon = theProjectileNucleus->GetNextNucleon()))
      { /* Loop checking, 10.08.2015, A.Ribon */
        if (!aNucleon->AreYouHit())
        {
          G4LorentzVector tmp = aNucleon->Get4Momentum();
      ...
      {
#ifdef debugFTFmodel
        G4cout << "BAD situation: forced exit of the second while loop in G4FTFModel::GetResidual"
               << G4endl << "\t return immediately from the method!" << G4endl;
#endif
        return;
      }
  ...
  {  // Related to the condition: if ( HighEnergyInter )
    ...
    {
      DeltaExcitationE =
        ProjectileResidualExcitationEnergy / G4double(NumberOfProjectileParticipant);
...
G4bool G4FTFModel::ComputeNucleusProperties(
...
{  // input & output parameter
  ...
  { /* Loop checking, 10.08.2015, A.Ribon */
    ...
    {  // Involved nucleons
      ...
      sumMasses +=
        std::sqrt(sqr(aNucleon->GetDefinition()->GetPDGMass()) + aNucleon->Get4Momentum().perp2());
  ...
  G4cout << "ExcitationEnergyPerWoundedNucleon " << ExcitationEnergyPerWoundedNucleon << G4endl
         ...
         << residualMassNumber << " (" << residualNumberOfLambdas << ") " << G4endl
         << "\t Initial Momentum " << nucleusMomentum << G4endl << "\t Residual Momentum   "
         << residualMomentum << G4endl;
#endif
  ...
  else
  {
    ...
    else
    {
      if (residualNumberOfLambdas > 0)
      {
        ...
        {
          residualMass = G4HyperNucleiProperties::GetNuclearMass(
            residualMassNumber, std::abs(residualCharge), residualNumberOfLambdas);
        }
      }
      else
      {
        residualMass = G4ParticleTable::GetParticleTable()->GetIonTable()->GetIonMass(
          std::abs(residualCharge), residualMassNumber);
      }
    }
...
G4bool G4FTFModel::GenerateDeltaIsobar(const G4double sqrtS,  // input parameter
  ...
  for (G4int i = 0; i < numberOfInvolvedNucleons; ++i)
  {
    if (G4UniformRand() < probDeltaIsobar && numberOfDeltas < maxNumberOfDeltas)
    {
      ...
      if (involvedNucleons[i]->GetDefinition() == G4Lambda::Definition()
          || involvedNucleons[i]->GetDefinition() == G4AntiLambda::Definition())
        continue;
      ...
      G4int newPdgCode = pdgCode / 10;
      newPdgCode = newPdgCode * 10 + 4;  // Delta
      ...
      //        << " " << massNuc << G4endl;
      if (sqrtS < sumMasses + massDelta - massNuc)
...
G4bool
G4FTFModel::SamplingNucleonKinematics(G4double averagePt2,  // input parameter
...
{  // output parameter
  ...
  G4cout << " averagePt2= " << averagePt2 << " maxPt2= " << maxPt2 << " dCor= " << dCor
         << " resMass(GeV)= " << residualMass / GeV << " resMassN= " << residualMassNumber
         << " nNuc= " << numberOfInvolvedNucleons << " lv= " << pResidual << G4endl;
#endif
  ...
  do
  {
    ...
    for (G4int i = 0; i < numberOfInvolvedNucleons; ++i)
    {
      ...
      G4LorentzVector tmp(aNucleon->Get4Momentum().x(), aNucleon->Get4Momentum().y(), x,
    ...
    for (G4int i = 0; i < numberOfInvolvedNucleons; ++i)
    {
      ...
      G4LorentzVector tmp(aNucleon->Get4Momentum().px(), aNucleon->Get4Momentum().py(), x,
                          aNucleon->Get4Momentum().e());
      aNucleon->SetMomentum(tmp);
    }
    if (!success) continue;
...
#ifdef debugPutOnMassShell
    G4cout << "success: " << success << " Mt(GeV)= " << std::sqrt(mass2) / GeV << G4endl;
#endif

  } while ((!success) && ++loopCounter < maxNumberOfLoops); /* Loop checking, 10.08.2015, A.Ribon */
  return (loopCounter < maxNumberOfLoops);
}

//============================================================================

G4bool G4FTFModel::CheckKinematics(const G4double sValue,  // input parameter
...
{  // input & output parameter
  ...
  G4double decayMomentum2 =
    sqr(sValue) + sqr(projectileMass2) + sqr(targetMass2)
    - 2.0 * (sValue * (projectileMass2 + targetMass2) + projectileMass2 * targetMass2);
  targetWminus = (sValue - projectileMass2 + targetMass2 + std::sqrt(decayMomentum2)) / 2.0 / sqrtS;
  ...
  G4double projectileY = 0.5 * G4Log((projectileE + projectilePz) / (projectileE - projectilePz));
...
#ifdef debugPutOnMassShell
  G4cout << "decayMomentum2 " << decayMomentum2 << G4endl << "\t targetWminus projectileWplus "
         << targetWminus << " " << projectileWplus << G4endl << "\t projectileY targetY "
         << projectileY << " " << targetY << G4endl;
  if (isProjectileNucleus)
  {
  ...
  for (G4int i = 0; i < numberOfInvolvedNucleons; ++i)
  {
    ...
    if (isProjectileNucleus)
    {
      G4cout << " " << i << " " << nucleonY << " " << projectileY << " " << nucleonY - projectileY
             << G4endl;
    }
    else
    {
      G4cout << " " << i << " " << nucleonY << " " << targetY << " " << nucleonY - targetY
             << G4endl;
    }
    G4cout << G4endl;
#endif

    if (std::abs(nucleonY - nucleusY) > 2 || (isProjectileNucleus && targetY > nucleonY)
...
G4bool
G4FTFModel::FinalizeKinematics(const G4double w,  // input parameter
                               const G4bool isProjectileNucleus,  // input parameter
...
{  // output parameter
  ...
  G4double residualMt2 =
    sqr(residualMass) + sqr(residual3Momentum.x()) + sqr(residual3Momentum.y());

#ifdef debugPutOnMassShell
  if (isProjectileNucleus)
  {
    G4cout << "Wminus Proj and residual3Momentum.z() " << w << " " << residual3Momentum.z()
           << G4endl;
  }
  else
  {
    G4cout << "Wplus  Targ and residual3Momentum.z() " << w << " " << residual3Momentum.z()
           << G4endl;
  }
#endif
  ...
  if (residualMassNumber != 0)
  {
    residualPz = -w * residual3Momentum.z() / 2.0 + residualMt2 / (2.0 * w * residual3Momentum.z());
    residualE = w * residual3Momentum.z() / 2.0 + residualMt2 / (2.0 * w * residual3Momentum.z());
