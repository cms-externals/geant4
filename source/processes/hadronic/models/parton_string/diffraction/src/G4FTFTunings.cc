...
{
  // For the time being, select the first alternative (i.e. with index > 0) tune which is switched
  // on. If nothing is found, then returns 0 (which corresponds to the default set of parameters).
  G4int indexTune = 0;
  for (G4int i = 1; i < sNumberOfTunes; ++i)
  {
    if (GetTuneApplicabilityState(i) != 0)
    {  // tune i-th is switched on
      indexTune = i;
      break;
    }
  }
...
class G4FTFSettingDefaultHDP
{
  public:
    ...
    G4FTFSettingDefaultHDP()
    {
      ...
      //      (see example below)
      // SetParams( 2, 6.0/Xinel, 0.0 ,-6.0/Xinel*16.28, 3.0 , 0.0, 0.0, 0.93 );  // Projectile
      // diffraction SetParams( 3, 6.0/Xinel, 0.0 ,-6.0/Xinel*16.28, 3.0 , 0.0, 0.0, 0.93 );  //
      // Target diffraction Also, for ( AbsProjectileBaryonNumber > 10 ||  NumberOfTargetNucleons >
      // 10 ) projectile and/or target diffraction (dissociation) may be switched ON/OFF
      ...
      HDP.SetDefault("FTF_BARYON_DIFF_M_PROJ", 1.16, 1.16,
                     3.0);  // it's supposed to be in GeV but do NOT do (*CLHEP::GeV)
                            // because it'll be done in the G4FTFParameters::SetProjMinDiffMass
      HDP.SetDefault("FTF_BARYON_NONDIFF_M_PROJ", 1.16, 1.16,
                     3.0);  // do NOT (*CLHEP::GeV) - same as above
      HDP.SetDefault("FTF_BARYON_DIFF_M_TGT", 1.16, 1.16,
                     3.0);  // do NOT (*CLHEP::GeV) - same as above
      HDP.SetDefault("FTF_BARYON_NONDIFF_M_TGT", 1.16, 1.16,
                     3.0);  // do NOT (*CLHEP::GeV) - same as above
      ...
      */
      ...
      HDP.SetDefault(
        "FTF_PION_DIFF_M_TGT", 1.16, 1.16,
        3.0);  // All (NON)DIFF_M's are supposed to be in GeV but do NOT do (*CLHEP::GeV)
               // because it'll be done in the G4FTFParameters::SetProjMinDiffMass
      ...
      HDP.SetDefault("FTF_BARYON_NUCDESTR_P1_PROJ", 1.0, 0.0,
                     1.0);  // in principle, it should be 1./NBRN - FIXME later !
      ...
      //
      HDP.SetDefault("FTF_BARYON_NUCDESTR_R2", 1.5 * CLHEP::fermi * CLHEP::fermi,
                     0.5 * CLHEP::fermi * CLHEP::fermi, 2.0 * CLHEP::fermi * CLHEP::fermi);
...
G4FTFParamCollBaryonProj::G4FTFParamCollBaryonProj() : G4FTFParamCollection()
{
  ...
  //
  HDP.DeveloperGet("FTF_BARYON_NUCDESTR_DISP",
                   fDofNuclearDestruct);  // Note: "Dof" means "Dispersion of..."
  //
...
G4FTFParamCollMesonProj::G4FTFParamCollMesonProj() : G4FTFParamCollection()
{
  ...
  HDP.DeveloperGet("FTF_MESON_NUCDESTR_DISP",
                   fDofNuclearDestruct);  // Note: "Dof" means "Dispersion of..."
