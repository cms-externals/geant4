...
G4FTFParticipants::G4FTFParticipants()
  : Bimpact(0.0), BinInterval(false), Bmin2(-1.0), Bmax2(-1.0), currentInteraction(-1)
{}
...
void G4FTFParticipants::GetList(const G4ReactionProduct& thePrimary, G4FTFParameters* theParameters)
{
#ifdef debugFTFparticipant
  G4cout << "Participants::GetList" << G4endl << "thePrimary " << thePrimary.GetMomentum() << G4endl
         << G4endl;
#endif
  ...
  for (unsigned int i = 0; i < theInteractions.size(); i++)
  ...
  {  // Hadron-nucleus or anti-baryon-nucleus interactions
    ...
    G4double xyradius =
      theNucleus->GetOuterRadius() + deltaxy;  // Range of impact parameter sampling
    ...
    do
    {
      ...
      { /* Loop checking, 10.08.2015, A.Ribon */
        ...
        G4double impact2 =
          sqr(impactX - nucleon->GetPosition().x()) + sqr(impactY - nucleon->GetPosition().y());

        if (theParameters->GetProbabilityOfInteraction(impact2 / fermi / fermi) > G4UniformRand())
        {
  ...
  // G4cout<<theProjectileNucleus->GetOuterRadius()/fermi<<"
  // "<<theNucleus->GetOuterRadius()/fermi<<" "<<deltaxy/fermi<<G4endl;

  // Range of impact parameter sampling
  G4double xyradius =
    theProjectileNucleus->GetOuterRadius() + theNucleus->GetOuterRadius() + deltaxy;
...
#ifdef debugFTFparticipant
    G4cout << "New interaction list, " << "b[fm] " << std::sqrt(sqr(impactX) + sqr(impactY)) / fermi
           << G4endl;
#endif
      ...
      { /* Loop checking, 10.08.2015, A.Ribon */
        ...
        {  // An interaction has happend!
          ...
          G4cout << "Part anInteraction->GetInteractionTime() [fm] "
                 << anInteraction->GetInteractionTime() / fermi << G4endl << "Splitable Pr* Tr* "
                 << ProjectileSplitable << " " << TargetSplitable << G4endl;
#endif
...
#ifdef debugFTFparticipant
  G4cout << G4endl << "Number of primary collisions " << theInteractions.size() << "\t Bx[fm] "
         << impactX / fermi << "\t By[fm] " << impactY / fermi << "\t B[fm] "
