                                                   ...
                                                   G4bool& isDiffractive) const
{
  ...
  common.M0projectile =
    projectile->GetDefinition()->GetPDGMass();  // Uzhi Aug.2019  common.Pprojectile.mag();
  ...
  */
  ...
  if (common.M0projectile > common.ProjectileDiffStateMinMass)
  {
    common.ProjectileDiffStateMinMass =
      common.MminProjectile + 220.0 * MeV;  // Uzhi Aug.2019  common.M0projectile + 220.0*MeV;
    common.ProjectileNonDiffStateMinMass =
      common.MminProjectile + 220.0 * MeV;  // Uzhi Aug.2019  common.M0projectile + 220.0*MeV;
    if (common.absProjectilePDGcode > 3000)
    {  // Strange baryon
  ...
  */
  ...
  if (common.M0target > common.TargetDiffStateMinMass)
  {
    common.TargetDiffStateMinMass =
      common.MminTarget + 220.0 * MeV;  // Uzhi Aug.2019  common.M0target + 220.0*MeV;
    common.TargetNonDiffStateMinMass =
      common.MminTarget + 220.0 * MeV;  // Uzhi Aug.2019  common.M0target + 220.0*MeV;
    if (common.absTargetPDGcode > 3000)
    {  // Strange baryon
...
#ifdef debugFTFexcitation
  G4cout << "Proj Targ PDGcodes " << common.ProjectilePDGcode << " " << common.TargetPDGcode
         << G4endl << "Mprojectile  Y " << common.Pprojectile.mag() << " " << ProjectileRapidity
         << G4endl  // Uzhi Aug.2019
         << "M0projectile Y " << common.M0projectile << " " << ProjectileRapidity << G4endl;
  G4cout << "Mtarget      Y " << common.Ptarget.mag() << " " << TargetRapidity
         << G4endl  // Uzhi Aug.2019
  ...
  common.PZcms2 = (sqr(common.S) + sqr(common.M0projectile2) + sqr(common.M0target2)
                   - 2.0
                       * (common.S * (common.M0projectile2 + common.M0target2)
                          + common.M0projectile2 * common.M0target2))
                  / 4.0 / common.S;
#ifdef debugFTFexcitation
  G4cout << "PZcms2 after toBePutOnMassShell " << common.PZcms2 << G4endl;
#endif
  if (common.PZcms2 < 0.0)
  ...
  if (toBePutOnMassShell)
  {
    ...
    common.Pprojectile.setE(
      std::sqrt(common.M0projectile2 + common.Pprojectile.x() * common.Pprojectile.x()
                + common.Pprojectile.y() * common.Pprojectile.y() + common.PZcms2));
    common.Ptarget.setE(std::sqrt(common.M0target2 + common.Ptarget.x() * common.Ptarget.x()
                                  + common.Ptarget.y() * common.Ptarget.y() + common.PZcms2));
  }
#ifdef debugFTFexcitation
  G4cout << "Start --------------------" << G4endl << "Proj M0 Mdif Mndif " << common.M0projectile
         ...
         << G4endl << "Targ M0 Mdif Mndif " << common.M0target << " "
         << common.TargetDiffStateMinMass << " " << common.TargetNonDiffStateMinMass << G4endl
         << "SqrtS " << common.SqrtS << G4endl << "Proj CMS " << common.Pprojectile << G4endl
         << "Targ CMS " << common.Ptarget << G4endl;
#endif
  ...
  common.ProbTargetDiffraction = theParameters->GetProcProb(3, ProjectileRapidity - TargetRapidity);
  common.ProbOfDiffraction = common.ProbProjectileDiffraction + common.ProbTargetDiffraction;
#ifdef debugFTFexcitation
  G4cout << "Proc Probs " << QeNoExc << " " << QeExc << " " << common.ProbProjectileDiffraction
         << " " << common.ProbTargetDiffraction << G4endl << "ProjectileRapidity "
  ...
  {
    returnCode =
      ExciteParticipants_doChargeExchange(projectile, target, theParameters, theElastic, common);
  }
  ...
  else if (returnCode == 1)
  {
    common.ProbOfDiffraction = common.ProbProjectileDiffraction + common.ProbTargetDiffraction;
#ifdef debugFTFexcitation
    G4cout << "Excitation --------------------" << G4endl << "Proj M0 MdMin MndMin "
           << common.M0projectile << " " << common.ProjectileDiffStateMinMass << "  "
           << common.ProjectileNonDiffStateMinMass << G4endl << "Targ M0 MdMin MndMin "
           << common.M0target << " " << common.TargetDiffStateMinMass << " "
...
G4int G4DiffractiveExcitation::ExciteParticipants_doChargeExchange(
  G4VSplitableHadron* projectile, G4VSplitableHadron* target, G4FTFParameters* theParameters,
  G4ElasticHNScattering* theElastic, G4DiffractiveExcitation::CommonVariables& common) const
{
  ...
  if (common.absProjectilePDGcode < 1000)
  {  // projectile is meson
...
#ifdef debugFTFexcitation
  G4cout << "Proj Quarks " << ProjQ1 << " " << ProjQ2 << " " << ProjQ3 << G4endl << "Targ Quarks "
  ...
  {  // projectile is meson
    ...
    { /* Loop checking, 10.08.2015, A.Ribon */
      ...
      if (aProjQ1 == aProjQ2)
      {
        if (G4UniformRand() < ProbSpin0)
        {  // Meson spin = 0 (pseudo-scalar)
          if (aProjQ1 < 3)
          {
            NewProjCode = 111;  // pi0
            if (Ksi < 0.5)
      ...
      for (G4int iQ = 0; iQ < 2; ++iQ)
      {  // 0 : first quark; 1 : second quark
        if (iQ == 1)
        {
          value = ProjQ2;
          absValue = aProjQ2;
        }
        if (absValue == 2 || absValue == 4)
        {  // u or c
      ...
      G4cout << "TestParticle Name " << NewProjCode << " " << TestParticle->GetParticleName()
             << G4endl << "MtestPart MtestPart0 " << MtestPr << " " << TestParticle->GetPDGMass()
             << G4endl << "M0projectile projectile PDGMass " << common.M0projectile << " "
...
#ifdef debugFTFexcitation
      G4cout << "New TrQ " << TargQ1 << " " << TargQ2 << " " << TargQ3 << G4endl << "NewTargCode "
             << NewTargCode << G4endl;
#endif
      ...
      if (TargQ1 <= 3 && TargQ2 <= 3 && TargQ3 <= 3)
      {
        ...
        {
          NewTargCode += 2;
          ProjExcited = true;  // Create Delta isobar
        }
        else if (target->GetDefinition()->GetPDGiIsospin() == 3)
        {  // Delta was the target
        ...
        else if (!ProjExcited && G4UniformRand() < DeltaProbAtQuarkExchange
                 &&  // Nucleon was the target
                 common.SqrtS
                   > common.M0projectile +  // Delta mass
      ...
      if (NewTargCode == 3124 ||  // Lambda* NOT existing in PDG !
          ...
          NewTargCode == 3314)
      {  // Xi*-    NOT existing in Geant4
        // G4cout << "G4DiffractiveExcitation::ExciteParticipants_doChargeExchange INEXISTING TARGET
        // PDGcode="
        //        << NewTargCode << G4endl;
...
// so we need to transform them by hand to Xi_c and Xi_b, respectively.
...
#endif
      if (NewTargCode == 4322)
        NewTargCode = 4232;  // Xi_c'+  ->  Xi_c+
      else if (NewTargCode == 4312)
        NewTargCode = 4132;  // Xi_c'0  ->  Xi_c0
      else if (NewTargCode == 5312)
        NewTargCode = 5132;  // Xi_b'-  ->  Xi_b-
      else if (NewTargCode == 5322)
        NewTargCode = 5232;  // Xi_b'0  ->  Xi_b0
#ifdef debug_heavyHadrons
      if (NewTargCode != initialNewTargCode)
      {
        G4cout << "G4DiffractiveExcitation::ExciteParticipants_doChargeExchange : forcing "
                  "(inexisting in G4)"
               << G4endl << "\t target heavy baryon with pdgCode=" << initialNewTargCode
    ...
    common.ProjectileNonDiffStateMinMass =
      common.M0projectile + 220.0 * MeV;  // 220 MeV=m_pi+80 MeV
  ...
  {  // of the if ( common.absProjectilePDGcode < 1000 ) ; the projectile is baryon now
    ...
    do
    {
      ...
      {
        otherExchangedQ = otherFirstQ;
        otherFirstQ = exchangedQ;
        exchangedQ = otherExchangedQ;
      }
      else
      {
        if (exchangedQ != otherSecondQ || G4UniformRand() < probSame)
        {
          otherExchangedQ = otherSecondQ;
          otherSecondQ = exchangedQ;
          exchangedQ = otherExchangedQ;
        }
        else
        {
          ...
          {
            otherExchangedQ = otherThirdQ;
            otherThirdQ = exchangedQ;
            exchangedQ = otherExchangedQ;
          }
        }
      }
    ...
    if (count >= MaxCount)
      return returnCode;  // All attempts failed: unsuccessfully ended, nothing else can be done
    ...
    for (G4int iHadron = 0; iHadron < 2; iHadron++)
    {
      ...
      if (codeQ1 > 3 || codeQ2 > 3 || codeQ3 > 3)
        continue;  // No excited charm or bottom states in Geant4
      if (codeQ1 == codeQ2 && codeQ1 == codeQ3)
      {  // The three quarks are the same
        newHadCode +=
          2;  // Delta++ (uuu) or Delta- (ddd) : spin 3/2, last PDG digit = 4 (so +2 wrt p or n)
      }
      else if (isHadronADelta)
      {  // Hadron (projectile or target) was Delta
        if (G4UniformRand() > DeltaProbAtQuarkExchange)
        {
          newHadCode +=
            2;  // Delta+ (uud) or Delta0 (udd) : spin 3/2, last PDG digit = 4 (so +2 wrt p or n)
        }
        else
        {
      ...
      {  // Hadron (projectile or target) was Nucleon
        if (G4UniformRand() < DeltaProbAtQuarkExchange
    ...
    if (NewProjCode == 3124 ||  // Lambda* NOT existing in PDG !
        ...
        NewProjCode == 3314)
    {  // Xi*-    NOT existing in Geant4
      // G4cout << "G4DiffractiveExcitation::ExciteParticipants_doChargeExchange INEXISTING
      // PROJECTILE PDGcode="
      //        << NewProjCode << G4endl;
...
// so we need to transform them by hand to the, respectively, Xi_c and Xi_b.
...
#endif
    if (NewProjCode == 4322)
      NewProjCode = 4232;  // Xi_c'+  ->  Xi_c+
    else if (NewProjCode == 4312)
      NewProjCode = 4132;  // Xi_c'0  ->  Xi_c0
    else if (NewProjCode == 5312)
      NewProjCode = 5132;  // Xi_b'-  ->  Xi_b-
    else if (NewProjCode == 5322)
      NewProjCode = 5232;  // Xi_b'0  ->  Xi_b0
    if (NewTargCode == 4322)
      NewTargCode = 4232;  // Xi_c'+  ->  Xi_c+
    else if (NewTargCode == 4312)
      NewTargCode = 4132;  // Xi_c'0  ->  Xi_c0
    ...
    if (NewProjCode != initialNewProjCode || NewTargCode != initialNewTargCode)
    {
      G4cout << "G4DiffractiveExcitation::ExciteParticipants_doChargeExchange : forcing "
                "(inexisting in G4)"
             << G4endl << "\t heavy baryon into an existing one:" << G4endl;
    ...
    if (G4UniformRand() < 0.5)
    {
      ...
      firstHadron = projectile;
      secondHadron = target;
      firstHadronCode = NewProjCode;
      secondHadronCode = NewTargCode;
    ...
    for (int iSamplingCase = 0; iSamplingCase < 2; iSamplingCase++)
    {
      ...
      if (iSamplingCase == 1)
      {  // Second nucleon mass sampling
        aHadron = secondHadron;
        aHadronCode = secondHadronCode;
        massConstraint = Mtest1st;
      }
      ...
      if (aHadron->GetStatus() == 1 || aHadron->GetStatus() == 2)
      {
        ...
        if (!TestParticle)
          return returnCode;  // Not possible to find such a hadron: unsuccessfully ended, nothing
                              // else can be done
        MminHadron = common.BrW.GetMinimumMass(TestParticle);
        if (common.SqrtS - massConstraint < MminHadron)
          return returnCode;  // Kinematically impossible: unsuccessfully ended, nothing else can be
                              // done
        ...
        else
        {
          ...
          {
            attempts++;
            MtestHadron = common.BrW.SampleMass(
              TestParticle, TestParticle->GetPDGMass() + 5.0 * TestParticle->GetPDGWidth());
          ...
          if (attempts >= maxNumberOfAttempts)
            return returnCode;  // All attempts failed: unsuccessfully ended, nothing else can be
                                // done
        }
      }
      if (iSamplingCase == 0)
      {
        Mtest1st = MtestHadron;
        Mmin1st = MminHadron;
      }
      else
      {
        Mtest2nd = MtestHadron;
        Mmin2nd = MminHadron;
      }
  ...
  if (common.SqrtS < common.M0projectile + common.M0target)
    return returnCode;  // unsuccessfully ended, nothing else can be done
  ...
  common.PZcms2 = (sqr(common.S) + sqr(common.M0projectile2) + sqr(common.M0target2)
                  ...
                  / 4.0 / common.S;
#ifdef debugFTFexcitation
  G4cout << "At the end// NewProjCode " << NewProjCode << G4endl << "At the end// NewTargCode "
         << NewTargCode << G4endl << "M0pr  M0tr  SqS " << common.M0projectile << " "
         << common.M0target << " " << common.SqrtS << G4endl << "M0pr2 M0tr2 SqS "
         << common.M0projectile2 << " " << common.M0target2 << " " << common.SqrtS << G4endl
         << "PZcms2 after the change " << common.PZcms2 << G4endl << G4endl;
#endif
  if (common.PZcms2 < 0.0)
    return returnCode;  // It can be if energy is not sufficient for Delta;
  ...
  if (G4UniformRand() > common.ProbExc)
...
G4bool G4DiffractiveExcitation::ExciteParticipants_doDiffraction(
  G4VSplitableHadron* projectile, G4VSplitableHadron* target, G4FTFParameters* theParameters,
  G4DiffractiveExcitation::CommonVariables& common) const
{
  ...
  if (G4UniformRand() < common.ProbProjectileDiffraction)
  ...
  common.PZcms2 = (sqr(common.S) + sqr(common.ProjMassT2) + sqr(common.TargMassT2)
                   - 2.0
                       * (common.S * (common.ProjMassT2 + common.TargMassT2)
                          + common.ProjMassT2 * common.TargMassT2))
                  / 4.0 / common.S;
  ...
  {  // Generate pt and mass of projectile
    ...
    {  // projectile diffraction
      ...
      loopCondition =
        ((common.Pprojectile + common.Qmomentum).mag2() < common.ProjectileDiffStateMinMass2);
    }
    else
    {  // target diffraction
      ...
      common.Qmomentum.setPz((common.Qplus - common.Qminus) / 2.0);
      common.Qmomentum.setE((common.Qplus + common.Qminus) / 2.0);
      loopCondition = ((common.Ptarget - common.Qmomentum).mag2() < common.TargetDiffStateMinMass2);
    }
...
G4bool G4DiffractiveExcitation::ExciteParticipants_doNonDiffraction(
  G4VSplitableHadron* projectile, G4VSplitableHadron* target, G4FTFParameters* theParameters,
  G4DiffractiveExcitation::CommonVariables& common) const
{
  ...
  {  // Generate pt and masses
    ...
    common.Qmomentum =
      G4LorentzVector(GaussianPt(theParameters->GetAveragePt2(), common.maxPtSquare), 0);
    common.Pt2 = G4ThreeVector(common.Qmomentum.vect()).mag2();
    ...
    if (G4UniformRand() <= 0.5)
    {  // Random choice projectile or target sampling
      ...
      {
        common.PMinusNew =
          (common.PMinusMax - common.PMinusMin) * G4UniformRand() + common.PMinusMin;
      }
    ...
    G4cout << "Sampled: Mpr, MdifPr, Mtr, MdifTr " << G4endl
           ...
           << (common.Ptarget - common.Qmomentum).mag() << " " << common.TargetNonDiffStateMinMass
           << G4endl;
#endif

  } while ((common.Pprojectile + common.Qmomentum).mag2() < common.ProjectileNonDiffStateMinMass2
           ||  // No double Diffraction
...
void G4DiffractiveExcitation::CreateStrings(G4VSplitableHadron* hadron, G4bool isProjectile,
                                            ...
                                            G4FTFParameters* theParameters) const
{
    ...
    {  // Kink is possible
      if (hadron->GetStatus() == 0)
      {
        ...
        if (Pt2kink)
        {
          Pt = std::sqrt(
            Pt2kink
            * (G4Pow::GetInstance()->powA(W2 / 16.0 / Pt2kink + 1.0, G4UniformRand()) - 1.0));
        }
        else
        {
          Pt = 0.0;
      ...
      if (Pt > 500.0 * MeV)
      {
        ...
        else
        {
          ...
          else
          {
            ...
            Pstart.setPx(-Pt);
            Pstart.setPy(0.0);
            Pstart.setPz(P_3 * CosT13);
            Pend.setPx(0.0);
            Pend.setPy(0.0);
            Pend.setPz(P_1);
            Pkink.setPx(Pt);
            Pkink.setPy(0.0);
            Pkink.setPz(P_2 * CosT12);
  ...
  {  // Kink is possible
    ...
    {  // meson
      if (isProjectile)
      {  // Projectile
        if (end->GetDefinition()->GetPDGEncoding() > 0)
        {  // A quark on the end
  ...
  {  // Kink is impossible
    ...
    G4LorentzVector Pstart1 =
      G4LorentzVector(HadronMom.px() / 2.0, HadronMom.py() / 2.0, 0.0, 0.0);  //    Quark momentum
    G4LorentzVector Pend1 =
      G4LorentzVector(HadronMom.px() / 2.0, HadronMom.py() / 2.0, 0.0, 0.0);  // Di-quark momentum
    ...
    G4double Pzq = Pz / 2.0 - Exp;
    Pstart1.setZ(Pzq);
    G4double Eq = std::sqrt(sqr(Pzq) + Pt2 / 4.0);
    Pstart1.setE(Eq);
    G4double Pzqq = Pz / 2.0 + Exp;
    Pend1.setZ(Pzqq);
    G4double Eqq = std::sqrt(sqr(Pzqq) + Pt2 / 4.0);
    Pend1.setE(Eqq);
  ...
  G4cout << " generated string flavors          " << start->GetPDGcode() << " / "
         ...
         << " generated string momenta: Diquark " << end->Get4Momentum()
         << "mass : " << end->Get4Momentum().mag() << G4endl
         << " sum of ends                       " << Pstart + Pend << G4endl
         << " Original                          " << hadron->Get4Momentum() << " "
         << hadron->Get4Momentum().mag() << G4endl;
#endif
...
void G4DiffractiveExcitation::UnpackMeson(const G4int IdPDG, G4int& Q1, G4int& Q2) const
{
  ...
  if (!(absIdPDG == 111 || absIdPDG == 221 || absIdPDG == 331 ||  // Pi0  ,  Eta ,   Eta'
        absIdPDG == 441 || absIdPDG == 443 || absIdPDG == 553))
  {  // Etac ,  J/psi , Upsilon
