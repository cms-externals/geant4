...
G4bool G4FTFAnnihilation::Annihilate(G4VSplitableHadron* projectile, G4VSplitableHadron* target,
                                     ...
                                     G4FTFParameters* theParameters) const
{
...
#ifdef debugFTFannih
  G4cout << "PDG codes " << ProjectilePDGcode << " " << TargetPDGcode << G4endl << "Pprojec "
         << common.Pprojectile << " " << common.Pprojectile.mag() << G4endl << "Ptarget "
         << common.Ptarget << " " << common.Ptarget.mag() << G4endl << "M0 proj target "
         << std::sqrt(M0projectile2) << " " << std::sqrt(M0target2) << G4endl;
#endif
  ...
  {  // Annihilation in flight!
    ...
    {
      X_b =
        3.13 + 140.0 * G4Pow::GetInstance()->powA((MesonProdThreshold - common.SqrtS) / GeV, 2.5);
    }
    else
    {
    ...
    X_c = 2.0 * FlowF
          * sqr(projectile->GetDefinition()->GetPDGMass() + target->GetDefinition()->GetPDGMass())
          / common.S;  // mb re-arrangement of
  ...
  if (TargetPDGcode == 2212 || TargetPDGcode == 2214)
  {  // Target proton or Delta+
    if (ProjectilePDGcode == -2212 || ProjectilePDGcode == -2214)
    {  // anti_proton  or anti_Delta+
      X_b *= 5.0;
      X_c *= 5.0;
      X_d *= 6.0;  // Pbar P
    }
    else if (ProjectilePDGcode == -2112 || ProjectilePDGcode == -2114)
    {  // anti_neutron or anti_Delta0
      X_b *= 4.0;
      X_c *= 4.0;
      X_d *= 4.0;  // NeutrBar P
    }
    else if (ProjectilePDGcode == -3122)
    {  // anti_Lambda (no anti_Lambda* in PDG)
      X_b *= 3.0;
      X_c *= 3.0;
      X_d *= 2.0;  // LambdaBar P
    }
    else if (ProjectilePDGcode == -3112)
    {  // anti_Sigma- (no anti_Sigma*- in G4)
      X_b *= 2.0;
      X_c *= 2.0;
      X_d *= 0.0;  // Sigma-Bar P
    }
    else if (ProjectilePDGcode == -3212)
    {  // anti_Sigma0 (no anti_Sigma*0 in G4)
      X_b *= 3.0;
      X_c *= 3.0;
      X_d *= 2.0;  // Sigma0Bar P
    }
    ...
    {  // anti_Xi-    (no anti_Xi*-    in G4)
      X_b *= 1.0;
      X_c *= 1.0;
      X_d *= 0.0;  // Xi-Bar P
    }
    else if (ProjectilePDGcode == -3322)
    {  // anti_Xi0    (no anti_Xi*0    in G4)
      X_b *= 2.0;
      X_c *= 2.0;
      X_d *= 0.0;  // Xi0Bar P
    }
    else if (ProjectilePDGcode == -3334)
    {  // anti_Omega- (no anti_Omega*- in PDG)
      X_b *= 0.0;
      X_c *= 0.0;
      X_d *= 0.0;  // Omega-Bar P
    }
  ...
  if (isUnknown)
  {
    G4cout << "Unknown anti-baryon for FTF annihilation: PDGcodes - " << ProjectilePDGcode << " "
  ...
  {
    return Create3QuarkAntiQuarkStrings(projectile, target, AdditionalString, theParameters,
                                        common);
  }
...
G4bool G4FTFAnnihilation::Create3QuarkAntiQuarkStrings(
  G4VSplitableHadron* projectile, G4VSplitableHadron* target, G4VSplitableHadron*& AdditionalString,
  G4FTFParameters* theParameters, G4FTFAnnihilation::CommonVariables& common) const
{
  ...
  do
  {
    ...
    {  // anti-baryon (1st case), baryon (2nd case)
    ...
    for (G4int i = 0; i < 6; ++i)
    {  // Loop over the quarks and (anti-)quarks
      Quark_Mom[i] -= PtSum;
  ...
  G4double DecayMomentum2 =
    sqr(common.S) + sqr(Alfa) + sqr(Beta) - 2.0 * (common.S * (Alfa + Beta) + Alfa * Beta);
  if (DecayMomentum2 < 0.0)
  {
    // The kinematic check above guarantees a non-negative value, but
    // cancellation near threshold can leave a small negative remainder.
    DecayMomentum2 = 0.0;
  }
  ...
  {  // anti-baryon (1st case), baryon (2nd case)
    ...
    for (G4int i = 0; i < 3; ++i)
    {
      G4double Pz = w * Quark_Xs[index + i] / 2.0
                    - (Quark_Mom[index + i].mag2() + MassQ2) / (2.0 * w * Quark_Xs[index + i]);
  ...
  switch (SampledCase)
  {
    case 1:
      Tmp1 = common.AQ[1];
      common.AQ[1] = common.AQ[2];
      common.AQ[2] = Tmp1;
      break;
    case 2:
      Tmp1 = common.AQ[0];
      common.AQ[0] = common.AQ[1];
      common.AQ[1] = Tmp1;
      break;
    case 3:
      Tmp1 = common.AQ[0];
      Tmp2 = common.AQ[1];
      common.AQ[0] = common.AQ[2];
      common.AQ[1] = Tmp1;
      common.AQ[2] = Tmp2;
      break;
    case 4:
  ...
  for (G4int iString = 0; iString < 3; ++iString)
  {  // Loop over the 3 string cases
    if (iString == 0)
    {
      antiQuark = common.AQ[0];
      quark = common.Q[0];
    ...
    {
      if (absAntiQuark != 3)
      {  // Not yet considered the case absAntiQuark 4 (charm) and 5 (bottom)
    ...
    {  // Vector mesons - rho, omega, phi (not yet considered the analogous cases for charm and
       // bottom)
      if (absAntiQuark > absQuark)
      {
        NewCode = absAntiQuark * 100 + absQuark * 10 + 1;
        NewCode *= absAntiQuark / antiQuark;
      }
      else
      {
        NewCode = absQuark * 100 + absAntiQuark * 10 + 1;
        NewCode *= absQuark / quark;
      }
    }
  ...
  for (G4int i = 0; i < 3; ++i)
  {
    ...
    {
      if (Pstring.e() + Pstring.pz() < 1.0e-30)
      {  // Very small numerator   in the logarithm
        Ystring = -1.0e30;  // A very large negative value (E ~= -Pz)
        if (Pstring.e() - Pstring.pz() < 1.0e-30)
        {  // Very small denominator in the logarithm
...
G4int G4FTFAnnihilation::Create1DiquarkAntiDiquarkString(
  G4VSplitableHadron* projectile, G4VSplitableHadron* target,
  G4FTFAnnihilation::CommonVariables& common) const
{
  ...
  {  // index of the 3 constituent anti-quarks of the antibaryon projectile
    for (G4int iQ = 0; iQ < 3; ++iQ)
    {  // index of the 3 constituent quarks of the target nucleon
      if (-common.AQ[iAQ] == common.Q[iQ])
      {  // antiquark - quark that can annihilate
        // Here "0", "1", "2" means, respectively, "first", "second" and "third" constituent
        // of the (anti-baryon) projectile or (nucleon) target.
        if (iAQ == 0)
        {
          CandAQ[CandidatsN][0] = 1;
          CandAQ[CandidatsN][1] = 2;
        }
        if (iAQ == 1)
        {
          CandAQ[CandidatsN][0] = 0;
          CandAQ[CandidatsN][1] = 2;
        }
        if (iAQ == 2)
        {
          CandAQ[CandidatsN][0] = 0;
          CandAQ[CandidatsN][1] = 1;
        }
        if (iQ == 0)
        {
          CandQ[CandidatsN][0] = 1;
          CandQ[CandidatsN][1] = 2;
        }
        if (iQ == 1)
        {
          CandQ[CandidatsN][0] = 0;
          CandQ[CandidatsN][1] = 2;
        }
        if (iQ == 2)
        {
          CandQ[CandidatsN][0] = 0;
          CandQ[CandidatsN][1] = 1;
        }
        ++CandidatsN;
      }
    }
  ...
  if (CandidatsN != 0)
  {
    ...
    // Calculation of the creation time
    // Creation time and position of target nucleon were determined in ReggeonCascade() of
    // G4FTFModel
    projectile->SetTimeOfCreation(target->GetTimeOfCreation());
    projectile->SetPosition(target->GetPosition());
    projectile->Set4Momentum(common.Pprojectile);
...
G4int G4FTFAnnihilation::Create2QuarkAntiQuarkStrings(
  ...
  G4FTFAnnihilation::CommonVariables& common) const
{
  ...
  {  // index of the 3 constituent anti-quarks of the antibaryon projectile
    for (G4int iQ = 0; iQ < 3; ++iQ)
    {  // index of the 3 constituent quarks of the nucleon target
      if (-common.AQ[iAQ] == common.Q[iQ])
      {  // antiquark - quark that can annihilate
        // Here "0", "1", "2" means, respectively, "first", "second" and "third" constituent
        // of the (anti-baryon) projectile or (nucleon) target.
  ...
  if (CandidatsN != 0)
  {
    ...
    G4int NewCode = 0, antiQuark = 0, quark = 0;
    G4ParticleDefinition* TestParticle = nullptr;
    for (G4int iString = 0; iString < 2; ++iString)
    {  // Loop over the 2 string cases
      if (iString == 0)
      {
        antiQuark = LeftAQ1;
        quark = LeftQ1;
    ...
    {
      common.Pprojectile = Pstring1;
      common.Ptarget = Pstring2;
      QuarkOrder[0] = 0;
      QuarkOrder[1] = 1;
    }
    else
    {
      common.Pprojectile = Pstring2;
      common.Ptarget = Pstring1;
      QuarkOrder[0] = 1;
      QuarkOrder[1] = 0;
    }
...
G4bool G4FTFAnnihilation::Create1QuarkAntiQuarkString(
  G4VSplitableHadron* projectile, G4VSplitableHadron* target, G4FTFParameters* theParameters,
  G4FTFAnnihilation::CommonVariables& common) const
{
  ...
  if (CandidatsN != 0)
  {
    ...
    else
    {
      if (aAQ > aQ)
      {
        NewCode = aAQ * 100 + aQ * 10 + 1;
        NewCode *= aAQ / LeftAQ;
      }
      else
      {
        NewCode = aQ * 100 + aAQ * 10 + 1;
        NewCode *= aQ / LeftQ;
      }
    }
