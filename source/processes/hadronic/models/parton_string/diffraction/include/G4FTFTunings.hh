...
//----------------------------------------------------------------------------
//
#ifndef G4FTFTUNINGS_HH
#define G4FTFTUNINGS_HH
...
class G4FTFTunings
{
  ...
  private:
    ...
    const std::array<G4String, sNumberOfTunes> fNameOfTunes = {{
      ...
      "pion-tune2022-v0",  // 2nd tuning: Julia Yarba's presentations on 26-Sept-2022 and
                           // 19-Oct-2022
      "combined-tune2022-v0",  // 3rd tuning: combination of the 1st and 2nd tuning
      "energy-resolution-tune2023-v0",  // 4th tuning: overcome too optimistic (i.e. narrow) pion
                                        // shower energy resolutions in ATLAS calorimeters w.r.t.
                                        // test-beam data
...
class G4FTFParamCollection
{
  ...
  public:
    ...
    // Update: Proc=2 & Proc=3 in case ( AbsProjectileBaryonNumber > 10 ||  NumberOfTargetNucleons >
    // 10 ) (diffraction dissociation) Other parameters have a complex form for baryon projectile
    ...
    // NOTE (JVY): There is also the Pt2Kind parameter but for now it's set to 0., so we'll leave it
    // aside
    // --> FIXME !!! --> void Get/SetBaryonMaxNumberOfCollisions( const double, const double ); //
    // 1st is Plab, 2nd - D=2.
  ...
  protected:
    ...
    // void SetBaryonProbOfInteraction( const double ); // ??? this is prob. of inelastic
    // interaction
