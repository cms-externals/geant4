...
G4bool G4QGSDiffractiveExcitation::ExciteParticipants(G4VSplitableHadron* projectile,
                                                      G4VSplitableHadron* target, G4bool) const
{
#ifdef debugDoubleDiffraction
  G4cout << G4endl << "G4QGSDiffractiveExcitation::ExciteParticipants - Double diffraction."
         << G4endl;
  G4cout << "Proj Targ " << projectile->GetDefinition()->GetParticleName() << " "
         << target->GetDefinition()->GetParticleName() << G4endl;
  G4cout << "Proj 4 Mom " << projectile->Get4Momentum() << " " << projectile->Get4Momentum().mag()
         << G4endl;
  G4cout << "Targ 4 Mom " << target->Get4Momentum() << " " << target->Get4Momentum().mag()
         << G4endl;
#endif
  ...
  if (SqrtS < M0projectile + M0target)
  {
    return false;
  }  // The model cannot work for pp-interactions
  // at Plab < 1.3 GeV/c.
  ...
  if (PutOnMassShell)
  {
    ...
    Pprojectile.setE(
      std::sqrt(Mprojectile2 + sqr(Pprojectile.x()) + sqr(Pprojectile.y()) + PZcms2));
  ...
  {  // Normal projectile
    ...
    else if (absPrPDGcode == 321 || absPrPDGcode == 130
             || absPrPDGcode == 310)  //-Projectile is Kaon-
    {
  ...
  if (SqrtS < MinPrDiffMass + MinTrDiffMass)
  {
    return false;
  ...
  do
  {
    ...
    PZcms2 = (S * S + ProjMassT2 * ProjMassT2 + TargMassT2 * TargMassT2 - 2. * S * ProjMassT2
              - 2. * S * TargMassT2 - 2. * ProjMassT2 * TargMassT2)
             / 4. / S;
    if (PZcms2 < 0)
    {
      PZcms2 = 0;
...
G4ExcitedString* G4QGSDiffractiveExcitation::String(G4VSplitableHadron* hadron,
                                                    G4bool isProjectile) const
{
  hadron->SplitUp();
  ...
  G4double tm2 = std::sqrt(std::max(
    0.,
    sqr(tm1) - 4. * Pend.perp2() * hadron->Get4Momentum().minus() / hadron->Get4Momentum().plus()));
  ...
  G4cout << " generated string momenta:   quark " << start->Get4Momentum()
         << "mass : " << start->Get4Momentum().mag() << G4endl;
  G4cout << " generated string momenta: Diquark " << end->Get4Momentum()
