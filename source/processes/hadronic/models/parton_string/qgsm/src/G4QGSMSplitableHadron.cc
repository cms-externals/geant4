...
// sampling of x replaced by formula, taking X_min into account in the correlated sampling. HPW, Feb
// 1999. logic much clearer now. HPW Feb 1999 Removed the ordering problem. No Direction needed in
// selection of valence quark types. HPW Mar'99. Fixing p-t distributions for scattering of nuclei.
...
void G4QGSMSplitableHadron::DiffractiveSplitUp()
{
  ...
  G4double Local2 = std::sqrt(
    std::max(0., sqr(Local1) - 4. * RightMom.perp2() * HadronMom.minus() / HadronMom.plus()));
...
G4Parton* G4QGSMSplitableHadron::BuildSeaQuark(G4bool isAntiQuark, G4int aPDGCode,
                                               G4int /* nSeaPair*/)
{
...
G4double G4QGSMSplitableHadron::SampleX(G4double anXmin, G4int nSea, G4int totalSea, G4double aBeta)
{
  G4double result;
  G4double x1, x2;
  G4double ymax = 0;
  for (G4int ii = 1; ii < 100; ii++)
  {
    ...
    y *= G4Pow::GetInstance()->powN(
      G4Pow::GetInstance()->powA(1 - anXmin - totalSea * anXmin, alpha + 1)
        - G4Pow::GetInstance()->powA(anXmin, alpha + 1),
      nSea);
  ...
  do
  {
    ...
    y *=
      G4Pow::GetInstance()->powN(G4Pow::GetInstance()->powA(1 - x1 - totalSea * anXmin, alpha + 1)
                                   - G4Pow::GetInstance()->powA(anXmin, alpha + 1),
                                 nSea);
