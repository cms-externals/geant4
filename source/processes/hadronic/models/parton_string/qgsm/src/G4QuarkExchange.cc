...
{
  StrangeSuppress =
    (1.0 - 0.04) / 2.0;  // Uzhi June 2020 : suppression of strange quark pair prodution,
                         //                  i.e. u:d:s=1:1:0.04 . Need to be tuned!
}

G4QuarkExchange::~G4QuarkExchange() {}

G4bool G4QuarkExchange::ExciteParticipants(G4VSplitableHadron* projectile,
                                           G4VSplitableHadron* target) const
{
...
#ifdef debugQuarkExchange
  G4cout << "Proj Targ " << projectile->GetDefinition()->GetPDGEncoding() << " "
         << target->GetDefinition()->GetPDGEncoding() << G4endl;
  G4cout << "Proj. 4-Mom " << Pprojectile << " " << Pprojectile.mag() << G4endl << "Targ. 4-Mom "
  ...
  if (SqrtS - Mprojectile - Mtarget <= 250.0 * MeV)
  {
    ...
    G4cerr << "Projectile: " << projectile->GetDefinition()->GetPDGEncoding() << " " << Pprojectile
           << " " << Pprojectile.mag() << G4endl;
    G4cerr << "Target:     " << target->GetDefinition()->GetPDGEncoding() << " " << Ptarget << " "
           << Ptarget.mag() << G4endl;
  ...
  if (absPDGcode > 1000)
  {
    ProjectileDiffraction = G4UniformRand() <= 0.5;
  }
  if ((absPDGcode == 211) || (absPDGcode == 111))
  {
    ProjectileDiffraction = G4UniformRand() <= 0.66;
  }
  if ((absPDGcode == 321) || (absPDGcode == 311) || (PDGcode == 130) || (PDGcode == 310))
  {
    ProjectileDiffraction = G4UniformRand() <= 0.5;
  }
  if (absPDGcode > 400 && absPDGcode < 600)
  {
    ProjectileDiffraction = G4UniformRand() <= 0.5;
  }
    ...
    if (absPDGcode > 1000)  //------Projectile is baryon --------
    {
      if (absPDGcode > 4000 && absPDGcode < 6000)  // Projectile is a charm or bottom baryon
      {
        ProjectileMinDiffrMass =
          projectile->GetDefinition()->GetPDGMass() / CLHEP::GeV + 0.25;  // GeV
  ...
  do
  {
    ...
    G4cout << whilecount << " " << Pt2 << " " << ProjMassT << " " << TargMassT << " " << SqrtS
           << " " << S << " " << ProjectileDiffraction << G4endl;
#endif

    if (SqrtS < ProjMassT + TargMassT + 220.0 * MeV) continue;

    PZcms2 = (S * S + ProjMassT2 * ProjMassT2 + TargMassT2 * TargMassT2 - 2.0 * S * ProjMassT2
              - 2.0 * S * TargMassT2 - 2.0 * ProjMassT2 * TargMassT2)
             / 4.0 / S;
    ...
    {  // The projectile will fragment, the target will saved.
      ...
      sqrtPMinusMin = std::sqrt(PMinusMin);
      sqrtPMinusMax = std::sqrt(PMinusMax);
      ...
      if (absPDGcode > 1000)
      {
        PMinusNew =
          PMinusMax
          * (1.0
             - (1.0 - PMinusMin / PMinusMax) * G4Pow::GetInstance()->powA(G4UniformRand(), 0.3333));
      }
      ...
      else if ((absPDGcode == 321) || (absPDGcode == 311) || (PDGcode == 130) || (PDGcode == 310))
    ...
    {  // The target will fragment, the projectile will saved.
      ...
      sqrtTPlusMin = std::sqrt(TPlusMin);
      sqrtTPlusMax = std::sqrt(TPlusMax);
      ...
      if (absPDGcode > 1000)
      {
        TPlusNew =
          TPlusMax
          * (1.0
             - (1.0 - TPlusMin / TPlusMax) * G4Pow::GetInstance()->powA(G4UniformRand(), 0.3333));
      }
      else if ((absPDGcode == 211) || (absPDGcode == 111))
      {
        while (true)
    ...
    G4cout << ProjectileDiffraction << " " << (Pprojectile + Qmomentum).mag2() << " "
           << Mprojectile2 << G4endl;
    G4cout << "TargetDiffraction     (Ptarget     - Qmomentum).mag2()  Mtarget2" << G4endl;
    G4cout << !ProjectileDiffraction << " " << (Ptarget - Qmomentum).mag2() << " " << Mtarget2
           << G4endl;
#endif
  ...
  if (projectile->GetDefinition()->GetBaryonNumber() >= 0)
  {  // Uzhi June 2020
    // Quark exchange ----
