...
G4Reggeons::G4Reggeons(const G4ParticleDefinition* particle)
{
  ...
  if (absPDGcode > 1000)
  {  //  Projectile is baryon or anti_baryon --------
    ...
    {  // Sigma_c0    or  anti_Sigma_c0
      coeff = lBarCof1C;
    }
    if (PDGcode == 4332 || PDGcode == -4332)
    {  // Omega_c0    or  anti_Omega_c0
      coeff = lBarCof2SC;
    }
    if (PDGcode == 4232 || PDGcode == 4132 ||  // Xi_c+       or  Xi_c0           or
        PDGcode == -4232 || PDGcode == -4132)
    {  // anti_Xi_c+  or  anti_Xi_c0
      coeff = lBarCofSC;
    }
  ...
  {  // Projectile is a nonstrange meson
  ...
  XndR = 0.;
  /*
  G4cout<<G4endl<<"Reggeon's parameters for Particle "<<particle->GetParticleName()<<"
  "<<PDGcode<<G4endl<<G4endl; G4cout<<"Alpha_pomeron "<<Alpha_pomeron; G4cout<<" Alphaprime_pomeron
  "<<Alphaprime_pomeron*GeV*GeV; G4cout<<" S0_pomeron "<<S0_pomeron/GeV/GeV<<G4endl;
...
void G4Reggeons::CalculateXs()
{
  ...
  for (G4int i = 0; i < 10000; i++)
  {
    ...
    chiP = Chi_pomeron(1., B);
    chiR = Chi_reggeon(1., B);
    chiPin = Chi_pomeron(2., B);
    chiRin = Chi_reggeon(2., B);
  ...
  /*
  G4cout<<"Total totalP totalR  "<<Xtotal/millibarn  <<" "<<XtotalP/millibarn <<"
  "<<XtotalR/millibarn<<" mb"<<G4endl; G4cout<<"Elastic              "<<Xelastic/millibarn <<G4endl;
  G4cout<<"PrDiff TrDiff W_Diff "<<Xpr_Diff/millibarn<<" "<<Xtr_Diff/millibarn<<"
  "<<XDiff/millibarn<<G4endl; G4cout<<"Inelastic            "<<Xinel/millibarn <<G4endl;
  G4cout<<"NonDiff Pom & Reg    "<<Xnd/millibarn     <<" "<<XndP/millibarn    <<" "<<XndR/millibarn
  <<G4endl;
  */
}
...
//----------------------------------------------------------------------------------------------
void G4Reggeons::GetProbabilities(G4double B, G4int Mode, G4double& Pint, G4double& Pprd,
                                  G4double& Ptrd, G4double& Pdd, G4double& Pnd, G4double& Pnvr)
{
  ...
  G4double chiP = Chi_pomeron(1., B);
  G4double chiR = Chi_reggeon(1., B);
  chiPin = Chi_pomeron(2., B);
  G4double chiRin = Chi_reggeon(2., B);
