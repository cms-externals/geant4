...
G4QGSParticipants::G4QGSParticipants()
  : theDiffExcitaton(),
    ModelMode(SOFT),
    nCutMax(7),
    ThresholdParameter(0.0 * GeV),
    QGSMThreshold(3.0 * GeV),
    theNucleonRadius(1.5 * fermi),
    theCurrentVelocity(G4ThreeVector()),
    theProjectileSplitable(nullptr),
    Regge(nullptr),
    InteractionMode(ALL),
    alpha(-0.5),
    beta(2.5),
...
G4QGSParticipants::G4QGSParticipants(const G4QGSParticipants& right)
  : G4VParticipants(),
    ModelMode(right.ModelMode),
    nCutMax(right.nCutMax),
    ...
    Regge(right.Regge),
    InteractionMode(right.InteractionMode),
    alpha(right.alpha),
    beta(right.beta),
    sigmaPt(right.sigmaPt),
...
void G4QGSParticipants::BuildInteractions(const G4ReactionProduct& thePrimary)
{
  ...
  G4cout << G4endl << "G4QGSParticipants::BuildInteractions---------" << G4endl << "thePrimary "
         << thePrimary.GetDefinition()->GetParticleName() << " " << ProjectileResidual4Momentum
         << G4endl;
  ...
  do
  {
    ...
    if (Success)
    {
      ...
      {
#ifdef debugQGSParticipants
        G4cout << "Perform non-Diffractive Collisions if they happend. Determine Parton Momenta "
                  "and so on."
               << G4endl;
#endif
...
#ifdef debugQGSParticipants
  G4cout << "Delition of target nucleons from soft interactions " << theTargets.size() << G4endl
         << G4endl;
#endif
...
void G4QGSParticipants::GetList(const G4ReactionProduct& thePrimary)
{
  ...
  G4int MaxPower = thePrimary.GetMomentum().mag() / (3.3 * GeV);
  ...
  while ((theInteractions.size() == 0) && ++loopCounter < maxNumberOfLoops)
  {
    ...
    G4cout << "Impact parameter (fm ) " << std::sqrt(sqr(impactX) + sqr(impactY)) / fermi << " "
           << G4endl;
    ...
    while ((tNucleon = theNucleus->GetNextNucleon()))
    {
      ...
      G4double Distance2 =
        sqr(impactX - tNucleon->GetPosition().x()) + sqr(impactY - tNucleon->GetPosition().y());

      G4double Pint(0.);  // A probability of interaction at given impact parameter
      G4double Pprd(0.), Ptrd(0.),
        Pdd(0.);  // Probabilities of Proj. diffr., Target diffr., Double diffr.
      G4double Pnd(0.), Pnvr(0.);  // Probabilities of non-diffr. and quark exchange
      G4int NcutPomerons(0);  // Number of cutted pomerons

      Regge->GetProbabilities(std::sqrt(Distance2), InteractionMode, Pint, Pprd, Ptrd, Pdd, Pnd,
                              Pnvr);
#ifdef debugQGSParticipants
      nucleonCount++;
      G4cout << "Nucleon & its impact parameter: " << nucleonCount << " "
             << std::sqrt(Distance2) / fermi << " (fm)" << G4endl;
      G4cout << "Probability of interaction:     " << Pint << G4endl;
      G4cout << "Probability of PrD, TrD, DD:    " << Pprd << " " << Ptrd << " " << Pdd << G4endl;
      G4cout << "Probability of NonDiff, QuarkExc.: " << Pnd << " " << Pnvr << " in inel. inter."
             << G4endl;
#endif
      ...
      {  // An interaction is happend.
        ...
        if ((InteractionMode == ALL)
            || (InteractionMode == WITHOUT_R))  // Mode = ALL, WITHOUT_R, NON_DIFF
        {
          if (rndNumber < Pprd)
          {
            InteractionType = PrD;
            InteractionMode = WITHOUT_R;
          }
          else if (rndNumber < Pprd + Ptrd)
          {
            InteractionType = TrD;
            InteractionMode = WITHOUT_R;
          }
          else if (rndNumber < Pprd + Ptrd + Pdd)
          {
            InteractionType = DD;
            InteractionMode = WITHOUT_R;
          }
          else if (rndNumber < Pprd + Ptrd + Pdd + Pnd)
          {
            InteractionType = NonD;
        ...
        G4cout << "New Inter.  mode:" << InteractionMode << " (0 -ALL, 1 - WITHOUT_R, 2 - NON_DIFF)"
               << G4endl;
        ...
        {  // nondiffractive interaction occurs
#ifdef debugQGSParticipants
          G4cout << "Non-diffractive interaction occurs, max NcutPomerons " << NcutPomerons
                 << G4endl;
#endif
          ...
          {
            if (G4UniformRand() < Power / MaxPower)
            {
              Vncut++;
              Power--;
              if (Power <= 0.) break;
  ...
  if (theInteractions.size() != 0)
  {
    ...
    {  // Only the first one will be saved, all other will be erased.
      ...
      while (theInteractions.size() != 1)
      {
        ...
        G4Nucleon* pNucleon = anInteraction->GetTargetNucleon();
        pNucleon->Hit(nullptr);
...
{  // To store nucleons involved in the interaction
  ...
  G4cout << G4endl << "G4QGSParticipants::StoreInvolvedNucleon() if they were " << G4endl
         << "Stored # of wounded nucleons of target " << NumberOfInvolvedNucleonsOfTarget << G4endl;
#endif
  return;
}
...
{  // Implementation of the reggeon theory inspired model of nuclear destruction
  ...
  G4cout << "C of nucl. desctruction " << GetCofNuclearDestruction() << " R2 "
  ...
  for (G4int InvTN = 0; InvTN < InitNINt; InvTN++)
  {
    ...
    while ((Neighbour = theTargetNucleus->GetNextNucleon()))
    {
      ...
      if (!Neighbour->AreYouHit())
      {
        ...
        if (G4UniformRand()
            < GetCofNuclearDestruction() * G4Exp(-impact2 / GetR2ofNuclearDestruction()))
        {
...
#ifdef debugQGSParticipants
  G4cout << "Number of new involved nucleons " << NumberOfInvolvedNucleonsOfTarget - InitNINt
         << G4endl;
#endif
  return;
}
...
G4bool G4QGSParticipants::PutOnMassShell()
{
  ...
  G4cout << "TargetResidualMass TargetResidualExcitationEnergy " << TargetResidualMass / GeV
         << " GeV " << TargetResidualExcitationEnergy << " MeV" << G4endl << "Sum masses "
         << SumMasses / GeV << G4endl;
#endif
  ...
  if (Ptmp.pz() <= 0.0)
  {  // "String" moving backwards in c.m.s., abort collision!
    return false;
  }
  ...
  G4cout << "Y targetNucleus     " << YtargetNucleus << G4endl << "Dcor "
         << GetDofNuclearDestruction() << " DcorP DcorT " << DcorP << " " << DcorT << " AveragePt2 "
         << AveragePt2 << G4endl;
#endif
  ...
  do
  {
    ...
    do
    {
      ...
      if (M2proj < 0.0)
      {
        if (M2proj < -0.000001)
        {
          ...
          ed << "Projectile " << theProjectile.GetDefinition()->GetParticleName()
             << "  Target (Z,A)=(" << theTargetNucleus->GetCharge() << ","
             << theTargetNucleus->GetMassNumber() << ")  M2proj=" << M2proj
             << "  ->  sets it to 0.0 !" << G4endl;
      ...
      if (M2target < 0.0)
      {
        ...
        ed << "Projectile " << theProjectile.GetDefinition()->GetParticleName()
           << "  Target (Z,A)=(" << theTargetNucleus->GetCharge() << ","
           << theTargetNucleus->GetMassNumber() << ")  M2target=" << M2target
           << "  ->  sets it to 0.0 !" << G4endl;
...
#ifdef debugPutOnMassShell
      G4cout << "SqrtS, Mp+Mt, Mp, Mt " << SqrtS / GeV << " " << (sqrtM2proj + sqrtM2target) / GeV
...
#ifdef debugPutOnMassShell
    G4cout << "Projectile Residual4Momentum in CMS " << ProjectileResidual4Momentum / GeV << " GeV"
           << G4endl;
#endif
...
#ifdef debugPutOnMassShell
    G4cout << "Projectile Residual4Momentum in Lab " << ProjectileResidual4Momentum / GeV << " GeV"
           << G4endl;
#endif
  }
...
#ifdef debugPutOnMassShell
  G4cout << "Target Residual4Momentum in CMS " << TargetResidual4Momentum / GeV << " GeV "
         << G4endl;
#endif
...
#ifdef debugPutOnMassShell
  G4cout << "Target Residual4Momentum in Lab " << TargetResidual4Momentum / GeV << " GeV "
         << G4endl;
#endif
...
G4ThreeVector G4QGSParticipants::GaussianPt(G4double AveragePt2, G4double maxPtSquare) const
{
  ...
  if (AveragePt2 > 0.0)
  {
    ...
    Pt2 = (x < 200) ? -AveragePt2 * G4Log(1.0 + G4UniformRand() * (G4Exp(-x) - 1.0))
...
G4bool G4QGSParticipants::ComputeNucleusProperties(
...
{  // input & output parameter
  ...
  G4cout << "ExcitationEnergyPerWoundedNucleon " << ExcitationEPerWoundedNucleon << " MeV" << G4endl
         ...
         << G4endl << "\t Initial Momentum " << nucleusMomentum / GeV << " GeV" << G4endl
         << "\t Residual Momentum   " << residualMomentum / GeV << " GeV" << G4endl;
#endif
...
G4bool
G4QGSParticipants::GenerateDeltaIsobar(const G4double sqrtS,  // input parameter
...
G4bool G4QGSParticipants::SamplingNucleonKinematics(
  G4double averagePt2,  // input parameter
...
{  // output parameter
  ...
  do
  {
    ...
    for (G4int i = 0; i < numberOfInvolvedNucleons; i++)
    {
      ...
      G4double x =
        tmpX.x() + aNucleon->GetSplitableHadron()->GetDefinition()->GetPDGMass() / SumMasses;
      if (x < 0.0 || x > 1.0)
      {
        success = false;
        break;
    ...
    for (G4int i = 0; i < numberOfInvolvedNucleons; i++)
    {
      ...
      mass2 +=
        (sqr(aNucleon->GetSplitableHadron()->GetDefinition()->GetPDGMass()) + sqr(px) + sqr(py))
        / x;
...
G4bool G4QGSParticipants::CheckKinematics(const G4double sValue,  // input parameter
...
{  // input & output parameter
  ...
  {
    projectileY = 0.5 * G4Log((projectileE + projectilePz) / (projectileE - projectilePz));
  }
...
G4bool G4QGSParticipants::FinalizeKinematics(
  const G4double w,  // input parameter
  const G4bool isProjectileNucleus,  // input parameter
...
#ifdef debugPutOnMassShell
    G4cout << "Target involved nucleon No, name, 4Mom " << i << " "
           << aNucleon->GetDefinition()->GetParticleName() << " " << tmp << G4endl;
#endif
  }
...
{
#ifdef debugQGSParticipants
  G4cout << G4endl << "PerformDiffractiveCollisions()......" << G4endl << "theInteractions.size() "
         << theInteractions.size() << G4endl;
#endif
  ...
  for (i = 0; i < theInteractions.size(); i++)
  {
...
#ifdef debugQGSParticipants
    G4cout << "Interaction # and its status " << i << " " << theInteractions[i]->GetStatus()
           << G4endl;
#endif
    ...
    {  // Selection of diffractive interactions
      ...
      if (InterStatus == DD) theDiffExcitaton.ExciteParticipants(theProjectileSplitable, aTarget);
...
G4bool G4QGSParticipants::DeterminePartonMomenta()
{
  ...
  G4cout << "theProjectile status (0 -nondiffr, #0 diffr./reggeon):  "
         << theProjectileSplitable->GetStatus() << G4endl;
#endif

  if (theProjectileSplitable->GetStatus() != 0)
  {
    return false;
  }  // There were only diffractive interactions.
  ...
  if (std::abs(theProjectile.GetDefinition()->GetBaryonNumber()) != 0)
  {
    VqM_pr = 350 * MeV;
    VaqM_pr = 700 * MeV;
  }
...
#ifdef debugQGSParticipants
  G4cout << "Projectile 4 momentum " << Psum << G4endl << "Target nucleon momenta at start"
         << G4endl;
  G4int NuclNo = 0;
#endif
  ...
  for (i = theTargets.begin(); i != theTargets.end(); i++)
  {
    G4LorentzVector tmp = (*i)->Get4Momentum();
    tmp.transform(toCms);
    (*i)->Set4Momentum(tmp);
#ifdef debugQGSParticipants
    G4cout << "Target nucleon # and 4Mom " << " " << NuclNo << " " << (*i)->Get4Momentum()
           << G4endl;
    NuclNo++;
#endif
    Target4Momentum += tmp;
  ...
  for (i = theTargets.begin(); i != theTargets.end(); i++)
  {
    ...
    {
      G4ExceptionDescription ed;
      ed << "Projectile " << theProjectile.GetDefinition()->GetParticleName() << "  4-momentum "
         << Psum << G4endl;
    ...
    tmp.setPz(tmp.minus() / PminusTarget);
    tmp.setE(Mass);
    (*i)->Set4Momentum(tmp);
#ifdef debugQGSParticipants
  ...
  do
  {
    ...
    for (G4int aSeaPair = 0; aSeaPair < nSeaPair; aSeaPair++)
    {
      ...
      tmp.setPx(aPtVector.x());
      tmp.setPy(aPtVector.y());
      SumPx += aPtVector.x();
      SumPy += aPtVector.y();
...
#ifdef debugQGSParticipants
      G4cout << "nSeaPair of target N " << nSeaPair << G4endl << "Target nucleon 4Mom "
  ...
  } while ((!Success) && attempt < maxNumberOfAttempts); /* Loop checking, 07.08.2015, A.Ribon */
  ...
  G4double DecayMomentum2 = sqr(S) + sqr(ProjSumMt2perX) + sqr(TargSumMt2perX)
                            - 2.0 * S * ProjSumMt2perX - 2.0 * S * TargSumMt2perX
                            - 2.0 * ProjSumMt2perX * TargSumMt2perX;
...
//======================================================
G4double G4QGSParticipants::SampleX(G4double, G4int nSea, G4int, G4double aBeta)
{
  ...
  {
    Ksi1 = G4UniformRand();
    r1 = G4Pow::GetInstance()->powA(Ksi1, Oalfa);
    Ksi2 = G4UniformRand();
    r2 = G4Pow::GetInstance()->powA(Ksi2, Obeta);
    r12 = r1 + r2;
  } while ((r12 > 1.) && ++loopCounter < maxNumberOfLoops); /* Loop checking, 07.08.2015, A.Ribon */
  if (loopCounter >= maxNumberOfLoops)
  {
...
void G4QGSParticipants::CreateStrings()
{
  ...
  if (theProjectileSplitable->GetStatus()
      == 1)  // The projectile has participated only in diffr. inter.
  {
    ...
    G4cout << "              " << aPair->GetParton1()->GetPDGcode() << " "
           << aPair->GetParton1()->Get4Momentum() << " " << aPair->GetParton1()->GetX() << " "
           << G4endl;
    G4cout << "              " << aPair->GetParton2()->GetPDGcode() << " "
           << aPair->GetParton2()->Get4Momentum() << " " << aPair->GetParton2()->GetX() << " "
           << G4endl;
  ...
  for (G4int i = 0; i < N_EnvTarg; i++)
  {
...
#ifdef debugQGSParticipants
    G4cout << "Involved Nucleon # and its status " << i << " " << HitTargetNucleon->GetStatus()
           << G4endl;
#endif
    if (HitTargetNucleon->GetStatus() >= 1)  // Create diffractive string
    {
      ...
      G4PartonPair* aPair =
        new G4PartonPair(HitTargetNucleon->GetNextParton(), HitTargetNucleon->GetNextAntiParton(),
                         G4PartonPair::DIFFRACTIVE, G4PartonPair::TARGET);
#ifdef debugQGSParticipants
...
G4VSplitableHadron* G4QGSParticipants::SelectInteractions(const G4ReactionProduct& thePrimary)
{
  ...
  G4cout << "Energy and Nucleus " << thePrimary.GetTotalEnergy() << " "
