...
G4RKPropagation::G4RKPropagation()
  : theOuterRadius(0), theNucleus(0), theFieldMap(0), theEquationMap(0), theField(0)
{}
...
//----------------------------------------------------------------------------
{
  ...
  opticalCoeff = (*theFieldMap)[G4AntiProton::AntiProton()->GetPDGEncoding()]->GetCoeff();
...
//----------------------------------------------------------------------------
void G4RKPropagation::Transport(
  G4KineticTrackVector& active,
  //----------------------------------------------------------------------------
  const G4KineticTrackVector&, G4double timeStep)
{
  ...
  for (i = active.begin(); i != active.end(); ++i)
  {
    ...
    std::map<G4int, G4VNuclearField*, std::less<G4int>>::iterator fieldIter =
    ...
    G4cout << " kt,timeStep, Intersection times tenter, tleave  " << kt
           << " / state= " << kt->GetState() << " / " << " " << currTimeStep << " / " << t_enter
           << " / " << t_leave << G4endl;
#endif
    ...
    //     if(t_leave < 0 )
    //     {
    //        throw G4HadronicException(__FILE__, __LINE__, "G4RKPropagation:: Attempt to track
    //        particle past a  nucleus"); continue;
    //     }
    ...
    if (t_enter > 0)  // the particle is out. Transport free to the surface
    {
      ...
      else
      {
        ...
        if (newE <= kt->GetActualMass())  // the particle cannot enter the nucleus
        {
          ...
          //	   G4cout << " enter nucleus, E out/in: " << kt->GetTrackingMomentum().e() << " / " <<
          // newE <<G4endl; 	   G4cout << " the Field "<< currentField->GetField(kt->GetPosition())
          // << "
          //"<< kt->GetPosition()<<G4endl;
 ...
 << G4endl;
    ...
    //       G4cerr << "RKPropagation t_leave, curTimeStep " <<t_leave << " "
    //       <<currTimeStep<<G4endl;
...
#ifdef debug_1_RKPropagation
    G4cerr << "RKPropagation is_exiting?, t_leave, curTimeStep " << is_exiting << " " << t_leave
           << " " << currTimeStep << G4endl;
    G4cout << "RKPropagation Ekin, field, projectile potential, p "
           << kt->GetTrackingMomentum().e() - kt->GetTrackingMomentum().mag() << " "
           << kt->GetPosition() << " " << G4endl << currentField->GetField(kt->GetPosition()) << " "
           << kt->GetProjectilePotential() << G4endl << kt->GetTrackingMomentum() << G4endl;
#endif
    ...
    if (currTimeStep > 0 && !FieldTransport(kt, currTimeStep))
    {
      FreeTransport(kt, currTimeStep);
    }
    ...
    G4cout << "RKPropagation Ekin, field, p "
           ...
           << currentField->GetField(kt->GetPosition()) << G4endl << kt->GetTrackingMomentum()
           << G4endl << "delta p " << momold - kt->GetTrackingMomentum() << G4endl << "del pos "
           << posold - kt->GetPosition() << G4endl;
#endif
    ...
    if (is_exiting
        || (GetSphereIntersectionTimes(kt, t_in, t_out) && t_in < 0
            && t_out <= 0))  // particle is exiting
    {
      if (t_in < 0 && t_out >= 0)  // still inside, transport safely out.
      {
        ...
        {  // FixMe GF: savePos/pos may be out of nucleus, where GetField(..)=0
          ...
          //           this is done later.
          newE += currentField->GetField(savePos) - currentField->GetField(kt->GetPosition());
        }

        //       G4cout << " go border nucleus, E in/border: " << kt->GetTrackingMomentum() << " / "
        //       << newE <<G4endl;
        ...
        if (newE < kt->GetActualMass())
        {
          ...
          G4cout << " cannot leave nucleus, E in/out: " << kt->GetTrackingMomentum() << " / "
                 << newE << G4endl;
#endif
...
#ifdef debug_1_RKPropagation
        G4cout << " cannot leave nucleus, E in/out: " << kt->GetTrackingMomentum() << " / " << newE
               << G4endl;
#endif
        if (kt->GetDefinition() == G4Proton::Proton()
            || kt->GetDefinition() == G4Neutron::Neutron())
...
//----------------------------------------------------------------------------
{
...
#ifdef debug_1_RKPropagation
    std::cerr << "G4RKPropagation::FieldTransport() warning: integration error." << G4endl
              << "position " << kt->GetPosition() << " 4mom " << kt->GetTrackingMomentum() << G4endl
              << " timestep " << timeStep << G4endl;
#endif
    delete driver;
    delete stepper;
    return false;
  ...
  // Correct for momentum ( thus energy) transfered to nucleus, boost particle into moving nucleus
  // frame.
  G4ThreeVector MomentumTranfer = kt->GetTrackingMomentum().vect() - track.GetMomentum();
  G4ThreeVector boost =
    MomentumTranfer / std::sqrt(MomentumTranfer.mag2() + sqr(theNucleus->GetMass()));

  // update the kt
  kt->SetPosition(track.GetPosition());
  G4LorentzVector mom(track.GetMomentum(),
                      std::sqrt(track.GetMomentum().mag2() + sqr(kt->GetActualMass())));
  mom *= G4LorentzRotation(boost);
  theMomentumTranfer += (kt->GetTrackingMomentum() - mom).vect();
  kt->SetTrackingMomentum(mom);

  //    G4cout <<"Stepper output"<<kt<<" "<<kt->GetTrackingMomentum()<<"
  //    "<<kt->GetPosition()<<G4endl;
  /*
...
//----------------------------------------------------------------------------
G4bool G4RKPropagation::GetSphereIntersectionTimes(
  const G4double radius,
  //----------------------------------------------------------------------------
  const G4ThreeVector& currentPos, const G4LorentzVector& momentum, G4double& t1, G4double& t2)
{
  ...
  G4double sqrtArg = scalarProd * scalarProd - speedMag2 * (currentPos.mag2() - radius * radius);
...
//----------------------------------------------------------------------------
G4bool G4RKPropagation::GetSphereIntersectionTimes(const G4KineticTrack* kt, G4double& t1,
                                                   G4double& t2)
{
  G4double radius = theOuterRadius + 3 * fermi;  // "safety" of 3 fermi
  G4ThreeVector speed =
    kt->GetTrackingMomentum().vect() / kt->GetTrackingMomentum().e();  // bost vector
  G4double scalarProd = kt->GetPosition().dot(speed);
  G4double speedMag2 = speed.mag2();
  G4double sqrtArg =
    scalarProd * scalarProd - speedMag2 * (kt->GetPosition().mag2() - radius * radius);
  if (sqrtArg <= 0.)  // particle will not intersect the sphere
  {
    return false;
  }
