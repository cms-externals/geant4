...
G4PionPlusField::G4PionPlusField(G4V3DNucleus* nucleus, G4double coeff) : G4VNuclearField(nucleus)
{
  theCoeff = coeff;
}
...
G4double G4PionPlusField::GetField(const G4ThreeVector& aPosition)
{
  ...
  return 2. * pi * hbarc * hbarc / reducedMass * (1 + pionPlusMass / nucleonMass) * theCoeff
           * density
         + GetBarrier();
}
