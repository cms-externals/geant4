...
G4PionMinusField::G4PionMinusField(G4V3DNucleus* nucleus, G4double coeff) : G4VNuclearField(nucleus)
{
  theCoeff = coeff;
}
...
G4double G4PionMinusField::GetField(const G4ThreeVector& aPosition)
{
  ...
  return 2. * pi * hbarc * hbarc / reducedMass * (1 + pionMinusMass / nucleonMass) * theCoeff
           * density
         + GetBarrier();
}
