...
//----------------------------------------------------------------------------
G4HadFinalState* G4BinaryCascade::ApplyYourself(const G4HadProjectile& aTrack, G4Nucleus& aNucleus)
//----------------------------------------------------------------------------
{
  ...
  if (!fBCDEBUG)
  {
    if (definition != G4Neutron::NeutronDefinition() && definition != G4Proton::ProtonDefinition()
        && definition != G4PionPlus::PionPlusDefinition()
        && definition != G4PionMinus::PionMinusDefinition())
    {
      G4cerr << "You are trying to use G4BinaryCascade with " << definition->GetParticleName()
             << " as projectile." << G4endl;
      G4cerr << "G4BinaryCascade should not be used for projectiles other than nucleons or pions."
             << G4endl;
  ...
  if (products && products->size() > 0)
  {
    ...
    {
      G4DynamicParticle* aNewDP = new G4DynamicParticle(
        (*iter)->GetDefinition(), (*iter)->GetTotalEnergy(), (*iter)->GetMomentum());
  ...
  {  // no interaction, return primary
    if (fBCDEBUG)
      G4cerr << " ######### Binary Cascade Reaction void, return initial state ######### "
             << G4endl;
...
//----------------------------------------------------------------------------
{
  ...
  if (!success)  // fails if no excitation energy left....
  {
    ...
    G4cout << "G4BinaryCascade::Propagate: warning - high energy model failed energy conservation, "
              "returning unchanged high energy final state"
           << G4endl;
#endif
  ...
  while (theCollisionMgr->Entries() > 0 && currentZ
         && --collisionLoopMaxCount > 0) /* Loop checking, 31.08.2015, G.Folger */
  {
    if (Absorb())
    {  // absorb secondaries, pions only
      haveProducts = true;
    }
    if (Capture())
    {  // capture secondaries, nucleons only
      haveProducts = true;
    }
    ...
    {
      G4CollisionInitialState* nextCollision = theCollisionMgr->GetNextCollision();
#ifdef debug_BIC_Propagate_Collisions
      G4cout << " NextCollision  * , Time, curtime = " << nextCollision << " "
             << nextCollision->GetCollisionTime() << " " << theCurrentTime << G4endl;
#endif
  ...
  if (!theTargetList.size() || !nProtons)
  {
    ...
    G4cout << " Mom Transfered to nucleus : " << theMomentumTransfer << " "
           << theMomentumTransfer.mag() << G4endl;
...
#ifdef debug_BIC_Propagate
  G4cout << " Momentum transfer to Nucleus " << theMomentumTransfer << " "
         << theMomentumTransfer.mag() << G4endl;
  ...
  G4cout << " Excitation Energy prefinal,  #collisions:, out, captured  " << ExcitationEnergy << " "
         << collisionCount << " " << theFinalState.size() << " " << theCapturedList.size()
         << G4endl;
#endif
  ...
  G4cout << " Excitation Energy final,  #collisions:, out, captured  " << ExcitationEnergy << " "
         << collisionCount << " " << theFinalState.size() << " " << theCapturedList.size()
         << G4endl;
#endif
...
//----------------------------------------------------------------------------
{
  ...
  if ((currentA - finalA) != 0 || (currentZ - finalZ) != 0)
  {
    G4cerr << "G4BIC:GetExcitationEnergy(): Nucleon counting error current/final{A,Z} "
           << "(" << currentA << "," << finalA << ") (" << currentZ << "," << finalZ << ")"
           << G4endl;
  }
  ...
  else if (currentZ == 0)
  {
    ...
    {
      nucleusMass = GetFinalNucleusMomentum().mag() - 3. * MeV * currentA;
    }
  }
  else
  {
  ...
  if (excitationE < 0)
  {
    ...
    if (finalZ > .5)
      G4cout << " Final nuclmom/mass " << Nucl_mom << " " << Nucl_mom.mag() << " (A,Z)=(" << finalA
             << "," << finalZ << ")"
             << " mass " << nucleusMass << " "
    ...
    if (Z > .5)
    {
      ...
      G4cout << "GetExcitationEnergy: Initial nucleus A Z " << A << " " << Z << " " << initialExc
             << G4endl;
    }
  }
...
//----------------------------------------------------------------------------
{
  ...
  currentA = 0;
  currentZ = 0;
  while ((nucleon = the3DNucleus->GetNextNucleon())
         != nullptr) /* Loop checking, 31.08.2015, G.Folger */
  {
    ...
    if (!nucleon->AreYouHit())
    {
      ...
      // theInitial4Mom += mom;
      //        the potential inside the nucleus is taken into account, and nucleons are on mass
      //        shell.
    ...
    {
      G4cout << "nucleon is hit" << nucleon << G4endl;
    }
#endif
  }
  massInNucleus = 0;
...
#ifdef debug_BIC_BuildTargetList
  G4cout << "G4BinaryCascade::BuildTargetList():  nucleus (A,Z)=(" << currentA << "," << currentZ
         << ") mass: " << massInNucleus << ", theInitial4Mom " << theInitial4Mom
...
//----------------------------------------------------------------------------
{
  ...
  {
    if ((*iter)->GetFormationTime() < StartingTime) StartingTime = (*iter)->GetFormationTime();
  }
...
#ifdef debug_BIC_Propagate
      G4cout << " Adding initial secondary " << *iter << " time" << (*iter)->GetFormationTime()
  ...
  if (primary)
  {
    ...
    G4double excitation = theProjectile4Momentum.e() + initial_nuclear_mass
                          - lateParticles4Momentum.e() - massInNucleus;
#ifdef debug_BIC_GetExcitationEnergy
    G4cout << "BIC: Proj.e, nucl initial, nucl final, lateParticles" << theProjectile4Momentum
           << ",  " << initial_nuclear_mass << ",  " << massInNucleus << ",  "
           << lateParticles4Momentum << G4endl;
    G4cout << "BIC: Proj.e / initial excitation: " << theProjectile4Momentum.e() << " / "
           << excitation << G4endl;
#endif
    ...
    if (!success)
    {
      G4cout << "G4BinaryCascade::BuildLateParticleCollisions(): Proj.e / initial excitation: "
             << theProjectile4Momentum.e() << " / " << excitation << G4endl;
...
//----------------------------------------------------------------------------
{
  ...
  if (fragment)
  {
    ...
    {  // fragment->GetA_asInt() <= 1, so a single proton, as a fragment must have Z>0
      ...
      if (theCapturedList.size() == 1)
      {
        i = theCapturedList.begin();
      }
...
//----------------------------------------------------------------------------
{
  ...
  if ((theTargetList.size() + theCapturedList.size()) > 0)
  {
    ...
    if (theTargetList.size() != 0)
    {
      for (aNuc = theTargetList.begin(); (aNuc != theTargetList.end()) && (aMom != momenta->end());
           aNuc++, aMom++)
      {
    ...
    if (theCapturedList.size() != 0)
    {
      for (aNuc = theCapturedList.begin();
           (aNuc != theCapturedList.end()) && (aMom != momenta->end()); aNuc++, aMom++)
      {
        G4ReactionProduct* aNew = new G4ReactionProduct((*aNuc)->GetDefinition());
        aNew->SetTotalEnergy((*aMom)->e());
        aNew->SetMomentum((*aMom)->vect());
...
G4BinaryCascade::ProductsAddPrecompound(G4ReactionProductVector* products,
                                        G4ReactionProductVector* precompoundProducts)
//----------------------------------------------------------------------------
{
  ...
  if (precompoundProducts)
  {
    ...
    // G4cout << " unboosted preco result mom " << pPreco / MeV << "  ..- fragmentMom " << (pPreco -
    // pFragment)/MeV<< G4endl; G4cout << " preco result mom " << pSumPreco / MeV << "  ..-file4Mom
    // " << (pSumPreco - GetFinal4Momentum())/MeV<< G4endl;
...
//----------------------------------------------------------------------------
{
  ...
  G4cout << "FindLateP Particle, 4-mom, times newState "
         << secondary->GetDefinition()->GetParticleName() << " " << secondary->Get4Momentum()
         << " times " << tin << " " << tout << " " << secondary->GetState() << G4endl;
#endif
...
//----------------------------------------------------------------------------
{
  ...
  {  // for late particles, reset charges
     ...
     //        << initialCharge<< " " << primary->GetState() << " "<<
     //        primary->GetDefinition()->GetParticleName()<< G4endl;
#ifdef debug_G4BinaryCascade
  ...
  if (!lateParticleCollision)
  {
    ...
    if (Success && primary->GetState() == G4KineticTrack::inside)
    {  // if the primary was outside, nothing to correct
  ...
  {
    theFinalState.insert(theFinalState.cend(), toFinalState.cbegin(), toFinalState.cend());
...
#ifdef debug_G4BinaryCascade
  if ((finalBaryon - initialBaryon - lateBaryon) != 0
      || (finalCharge - initialCharge - lateCharge) != 0)
  {
    G4cout << "G4BinaryCascade: Error in Balancing: " << G4endl;
    G4cout << "initial/final baryon number, initial/final Charge " << initialBaryon << " "
           << finalBaryon << " " << initialCharge << " " << finalCharge << " "
    ...
    {
      G4cout << "targ: " << collision->GetTargetCollection()[it]->GetDefinition()->GetParticleName()
             << G4endl;
    }
...
//----------------------------------------------------------------------------
{
  ...
  if (absorbList.empty()) return false;
  ...
  for (iter = absorbList.cbegin(); iter != absorbList.cend(); ++iter)
  {
    ...
    while (!CheckPauliPrinciple(products)
           && --maxLoopCount > 0) /* Loop checking, 31.08.2015, G.Folger */
    {
      ClearAndDestroy(products);
      if (!absorber.FindProducts(*kt))
        throw G4HadronicException(__FILE__, __LINE__,
...
//----------------------------------------------------------------------------
{
  ...
  for (i = theSecondaryList.cbegin(); i != theSecondaryList.cend(); ++i)
  {
    G4KineticTrack* kt = *i;
    if (verbose)
      G4cout << "Capture position, radius, state " << kt->GetPosition().mag() << " "
             << theOuterRadius << " " << kt->GetState() << G4endl;
    if (kt->GetState() == G4KineticTrack::inside)  // capture happens only inside the nucleus
    {
          ...
          || (kt->GetDefinition() == G4Neutron::Neutron()))
      {
        ...
        if (verbose)
          G4cout << "Capture: .e(), mass, field, energy" << kt->Get4Momentum().e() << " "
                 << kt->GetActualMass() << " " << field << " " << energy << G4endl;
        //	 if( energy < theCutOnP )
        //	 {
  ...
  if (verbose)
    G4cout << "Capture particlesAboveCut,particlesBelowCut, "
              "capturedEnergy,capturedEnergy/particlesBelowCut <? 0.2*theCutOnP "
           << particlesAboveCut << " " << particlesBelowCut << " " << capturedEnergy << " "
           << G4endl;
  //    << " " << (particlesBelowCut>0) ? (capturedEnergy/particlesBelowCut) : (capturedEnergy) << "
  //    " << 0.2*theCutOnP << G4endl;
  //  if(particlesAboveCut==0 && particlesBelowCut>0 &&
  //  capturedEnergy/particlesBelowCut<0.2*theCutOnP)
...
//----------------------------------------------------------------------------
{
  ...
  for (i = products->cbegin(); i != products->cend(); ++i)
  {
    ...
    if ((definition == G4Proton::Proton()) || (definition == G4Neutron::Neutron()))
    {
  ...
  if (myflag)
  {
    for (i = products->cbegin(); i != products->cend(); ++i)
    {
      ...
      if ((definition == G4Proton::Proton()) || (definition == G4Neutron::Neutron()))
      {
        ...
        G4LorentzVector mom = (*i)->Get4Momentum();
        G4double field =
          ((G4RKPropagation*)thePropagator)->GetField(definition->GetPDGEncoding(), pos);
        if (mom.e() - mom.mag() + field > 160 * MeV)
        {
          G4cout << "momentum problem pFermi=" << pFermi << " mom, mom.m " << mom << " "
                 << mom.mag() << " field " << field << G4endl;
        }
      }
    }
  }
...
//----------------------------------------------------------------------------
{
  ...
  // if countreset reaches limit, there is a break from while, see below.
  {
    ...
    for (auto i = theSecondaryList.cbegin(); i != theSecondaryList.cend(); ++i)
    {
      ...
      if (kt->GetState() == G4KineticTrack::inside)
      {
        ...
        G4cout << " minTimeStep, tStep Particle " << minTimeStep << " " << tStep << " "
               << kt->GetDefinition()->GetParticleName() << " 4mom " << kt->GetTrackingMomentum()
               << G4endl;
        if (!intersect)
          ;
        {
    ...
    if (counter > 100
        && theCollisionMgr->Entries() == 0)  // no collision, and stepping for some time....
    {
#ifdef debug_BIC_StepParticlesOut
...
//----------------------------------------------------------------------------
{
  ...
  if (primary->GetState() == G4KineticTrack::inside)
  {
    ...
    {
      Efermi = ((G4RKPropagation*)thePropagator)
                 ->GetField(G4Neutron::Neutron()->GetPDGEncoding(), primary->GetPosition());
      G4LorentzVector mom4Primary = primary->Get4Momentum();
...
//
{
...
#ifdef debug_BIC_CorrectFinalPandE
    G4cout << "CorrectFinalPandE a final " << (*i)->GetDefinition()->GetParticleName() << " 4mom "
  ...
  if (s0 - (m10 + m20) * (m10 + m20) < 0)
  {
    ...
    G4cout << "not enough mass to correct: mass^2, A,Z, mass(nucl), mass(finals) "
           << (s0 - (m10 + m20) * (m10 + m20)) << " " << currentA << " " << currentZ << " " << m10
           << " " << m20 << G4endl;
  ...
  G4double pInCM =
    std::sqrt((s0 - (m10 + m20) * (m10 + m20)) * (s0 - (m10 - m20) * (m10 - m20)) / (4. * s0));
#ifdef debug_BIC_CorrectFinalPandE
  ...
  if (pFinals.vect().mag() > pInCM)
  {
    ...
    G4cout << "CorrectFinalPandE nucleus corrected mass : " << GetFinal4Momentum() << " "
           << GetFinal4Momentum().mag() << G4endl << " CMS pFinals , mag, 3.mag : " << qFinals
           << " " << qFinals.mag() << " " << qFinals.vect().mag() << G4endl;
  ...
  {
    G4cerr << " -CorrectFinalPandE 6 - no correction done" << G4endl;
  }
#endif
}
...
void G4BinaryCascade::UpdateTracksAndCollisions(
  //----------------------------------------------------------------------------
  G4KineticTrackVector* oldSecondaries, G4KineticTrackVector* oldTarget,
...
//----------------------------------------------------------------------------
{
  ...
  theFinalState.insert(theFinalState.end(), kt_gone_out->begin(), kt_gone_out->end());
  ...
  if (kt_captured->size())
  {
    theCapturedList.insert(theCapturedList.end(), kt_captured->begin(), kt_captured->end());
          ...
          + GetTotalCharge(*kt_inside)))
  {
    G4cout << " error-DoTimeStep aft, A, Z: " << currentA << " " << currentZ
           ...
           << " active: " << GetTotalCharge(*kt_inside) << G4endl;
  }
#endif
...
//----------------------------------------------------------------------------
{
  ...
  if (in->size())
  {
    ...
    G4cout << "CorrectBarionsOnBoundary,currentZ,currentA,"
           ...
           << "energy correction,m_secondry,m_nucl_init,m_nucl_final " << currentZ << " "
           << currentA << " " << secondaryCharge_in << " " << secondaryBarions_in << " "
           << correction << " " << secondaryMass_in << " " << mass_initial << " " << mass_final
           << " " << G4endl;
...
#ifdef debug_BIC_CorrectBarionsOnBoundary
    G4cout << " CorrectBarionsOnBoundary, aft, Z, A, sec-Z,A,m,m_in_nucleus " << currentZ << " "
           << currentA << " " << secondaryCharge_in << " " << secondaryBarions_in << " "
           << secondaryMass_in << " " << G4endl;
  ...
  if (out->size())
  {
    ...
    // G4cout << "G4BinaryCascade::CorrectBarionsOnBoundary() total out correction: " << correction
    // << G4endl;
    ...
    G4cout << "DoTimeStep,(current Z,A),"
           ...
           << "(" << currentZ << "," << currentA << ") (" << secondaries_out << ","
           << secondaryCharge_out << "," << secondaryBarions_out << ") * " << correction << " ("
           << secondaryMass_out << ", " << mass_initial << ", " << mass_final << ")" << G4endl;
    ...
    for (iter = out->cbegin(); iter != out->cend(); ++iter)
    {
      ...
      else
      {
            ...
            || ((*iter)->GetDefinition() == G4Neutron::Neutron()))
        {
          ...
          // Undo correction for Colomb Barrier
          G4double barrier = ((G4RKPropagation*)thePropagator)
                               ->GetBarrier((*iter)->GetDefinition()->GetPDGEncoding());
          (*iter)->UpdateTrackingMomentum((*iter)->GetTrackingMomentum().e() - barrier);
    ...
    G4cout << " DoTimeStep, nucl-update, A, Z, sec-Z,A,m,m_in_nucleus, table-mass, delta "
           << currentA << " " << currentZ << " " << secondaryCharge_out << " "
           << secondaryBarions_out << " " << secondaryMass_out << " " << massInNucleus << " "
           << GetIonMass(currentZ, currentA) << " "
           << massInNucleus - GetIonMass(currentZ, currentA) << G4endl;
#endif
  }
...
//----------------------------------------------------------------------------
{
  ...
  G4cout << "target, captured, secondary: " << theTargetList.size() << " " << theCapturedList.size()
         << " " << theSecondaryList.size() << G4endl;
#endif
  ...
  if (z != (GetTotalCharge(theTargetList) + GetTotalCharge(theCapturedList)))
  {
    G4cout << " FindFragment Counting error z a " << z << " " << a << " "
           << GetTotalCharge(theTargetList) << " " << GetTotalCharge(theCapturedList) << G4endl;
    PrintKTVector(&theTargetList, std::string("theTargetList"));
    PrintKTVector(&theCapturedList, std::string("theCapturedList"));
  }
#endif
  ...
  G4cout << "Fragment: a= " << a << " z= " << z << " particles= " << excitons
         << " Charged= " << zCaptured << " holes= " << holes << " excitE= " << GetExcitationEnergy()
         << " Final4Momentum= " << GetFinalNucleusMomentum()
         << " capturMomentum= " << CapturedMomentum << G4endl;
#endif
...
//  ie. difference of (initial state(primaries+nucleus) - final state) particles, ignoring remnant
//  nucleus
{
  ...
  if (final4Momentum.e() > 0 && (final4Momentum.vect() / final4Momentum.e()).mag() > 1.0
      && currentA > 0)
  {
    ...
    {
      G4cerr << " Final state: " << (*i)->Get4Momentum() << (*i)->GetDefinition()->GetParticleName()
             << G4endl;
    }
...
//----------------------------------------------------------------------------
{
  ...
  {
    // G4cout << "GetFinalNucleusMomentum GetFinal4Momentum= " <<NucleusMomentum <<"
    // "<<NucleusMomentum.mag()<<G4endl; boost nucleus to a frame such that the momentum of nucleus
    // == momentum of Captured
    ...
    G4cout << "GetFinalNucleusMomentum be4 boostNucleusMomentum, CapturedMomentum"
           << NucleusMomentum << " " << CapturedMomentum << " " << G4endl;
#endif
...
//----------------------------------------------------------------------------
G4ThreeVector G4BinaryCascade::GetSpherePoint(G4double r, const G4LorentzVector& mom4)
//----------------------------------------------------------------------------
{
  ...
  /* Loop checking, 31.08.2015, G.Folger */  // or random is badly broken.....
...
//----------------------------------------------------------------------------
{
  ...
  if (ktv)
  {
    G4cout << "  vector: " << ktv << ", number of tracks: " << ktv->size() << G4endl;
    std::vector<G4KineticTrack*>::const_iterator i;
    G4int count;
...
//----------------------------------------------------------------------------
{
  ...
  if (kt)
  {
    ...
    G4cout << "    definition: " << definition->GetPDGEncoding() << " pos: " << 1 / fermi * pos
           << " R: " << 1 / fermi * pos.mag() << " 4mom: " << 1 / MeV * mom << "Tr_mom"
           << 1 / MeV * tmom << " P: " << 1 / MeV * mom.vect().mag()
           << " M: " << 1 / MeV * mom.mag() << G4endl;
    G4cout << "    trackstatus: " << kt->GetState() << " isParticipant "
           << (kt->IsParticipant() ? "T" : "F") << G4endl;
  }
  else
  {
...
//----------------------------------------------------------------------------
{
  ...
  {
    G4cerr << "G4BinaryCascade::GetIonMass() - invalid (A,Z) = (" << A << "," << Z << ")" << G4endl;
    throw G4HadronicException(__FILE__, __LINE__, "G4BinaryCascade::GetIonMass() - giving up");
  }
  //  G4cout << "G4BinaryCascade::GetIonMass() Z, A, mass " << Z << " " << A << " " << mass <<
  //  G4endl;
  return mass;
}
G4ReactionProductVector* G4BinaryCascade::FillVoidNucleusProducts(G4ReactionProductVector* products)
{
  ...
  while (theCollisionMgr->Entries() > 0) /* Loop checking, 31.08.2015, G.Folger */
  {
    G4CollisionInitialState* collision = theCollisionMgr->GetNextCollision();
  ...
  // decay must be after loop on Collisions, and Decay() will delete entries in theSecondaryList,
  // refered
  //   to by Collisions.
  ...
  G4double Ekinetic =
    theProjectile4Momentum.e() + initial_nuclear_mass - Esecondaries - SumMassNucleons;
#ifdef debug_BIC_FillVoidnucleus
  G4LorentzVector deltaP =
    theProjectile4Momentum + G4LorentzVector(initial_nuclear_mass) - psecondaries - pNucleons;
  ...
  else
  {
    ...
    if (theTargetList.size())
      Ekineticrdm = (0.1 + G4UniformRand() * 5.) * MeV;  // leave some  Energy for Nucleons
    ...
    if (std::abs(Ekinetic) < 20 * perCent * TotalEkin)
    {
      correction =
        1.
        + (Ekinetic - Ekineticrdm) / TotalEkin;  // Ekinetic < 0 == IS < FS, need to reduce energies
    }
#ifdef debug_G4BinaryCascade
    else
    {
      G4cout << "BLIC::FillVoidNucleus() fail correction, Ekinetic, TotalEkin " << Ekinetic << ""
             << TotalEkin << G4endl;
    }
#endif
    ...
    for (rpiter = products->cbegin(); rpiter != products->cend(); ++rpiter)
    {
      (*rpiter)->SetKineticEnergy((*rpiter)->GetKineticEnergy()
                                  * correction);  // this sets kinetic & total energy
...
G4ReactionProductVector*
G4BinaryCascade::HighEnergyModelFSProducts(G4ReactionProductVector* products,
                                           G4KineticTrackVector* secondaries)
{
  ...
  {
    fragment = G4ParticleTable::GetParticleTable()->GetIonTable()->GetIon(currentZ, currentA, 0.0);
  }
  if (fragment != 0)
  {
                                              ...
                                              G4KineticTrackVector* products)
{
  ...
  //  if ( lateParticleCollision ) G4cout << " Added late
  //  particle--------------------------"<<G4endl; if ( lateParticleCollision && products )
  //  PrintKTVector(products, " reaction products");
}
...
G4bool G4BinaryCascade::CheckChargeAndBaryonNumber(const G4String& where)
{
  ...
  G4cout << where << " A: iState= " << iStateA << ", secs= " << secsA << ", fState= " << fStateA
         << ", current= " << currentA << ", late= " << lateA << G4endl;
  G4cout << where << " Z: iState= " << iStateZ << ", secs= " << secsZ << ", fState= " << fStateZ
         << ", current= " << currentZ << ", late= " << lateZ << G4endl;
#endif
  ...
  if (deltaA != 0 || deltaZ != 0)
  {
    ...
    {
      G4cout << "baryon/charge imbalance - " << where << G4endl << "deltaA " << deltaA
             << ", iStateA " << iStateA << ",  CapturedA " << CapturedA << ",  secsA " << secsA
             << ", fStateA " << fStateA << ", currentA " << currentA << ", lateA " << lateA
             << G4endl << "deltaZ " << deltaZ << ", iStateZ " << iStateZ << ",  CapturedZ "
             << CapturedZ << ",  secsZ " << secsZ << ", fStateZ " << fStateZ << ", currentZ "
             << currentZ << ", lateZ " << lateZ << G4endl << G4endl;
      lastdA = deltaA;
      lastdZ = deltaZ;
    }
  }
                                          ...
                                          G4KineticTrackVector* products)
{
  ...
  G4cout << "prim. E/field/Barr/Sum " << kt->Get4Momentum().e() << " "
         ...
         << RKprop->GetBarrier(kt->GetDefinition()->GetPDGEncoding()) << " " << initial << G4endl;
  ;
  ...
  for (unsigned int it = 0; it < ktv.size(); ++it)
  {
    kt = ktv[it];
    initial += kt->Get4Momentum().e();
    thisExcitation += kt->GetDefinition()->GetPDGMass() - kt->Get4Momentum().e()
                      - RKprop->GetField(kt->GetDefinition()->GetPDGEncoding(), kt->GetPosition());
    //     initial +=  RKprop->GetField(kt->GetDefinition()->GetPDGEncoding(),kt->GetPosition());
    //     initial -=  RKprop->GetBarrier(kt->GetDefinition()->GetPDGEncoding());
    G4cout << "Targ. def/E/field/Barr/Sum " << kt->GetDefinition()->GetPDGEncoding() << " "
           << kt->Get4Momentum().e() << " "
           << RKprop->GetField(kt->GetDefinition()->GetPDGEncoding(), kt->GetPosition()) << " "
           << RKprop->GetBarrier(kt->GetDefinition()->GetPDGEncoding()) << " " << initial
  ...
  if (products)
  {
    for (unsigned int it = 0; it < products->size(); ++it)
    {
      ...
      G4cout << "sec. def/E/field/Barr/Sum " << kt->GetDefinition()->GetPDGEncoding() << " "
             << kt->Get4Momentum().e() << " "
             << RKprop->GetField(kt->GetDefinition()->GetPDGEncoding(), kt->GetPosition()) << " "
             << RKprop->GetBarrier(kt->GetDefinition()->GetPDGEncoding()) << " " << fin << G4endl;
      ;
    }
  }
  ...
  G4cout << " initE/ E_out/ Mfinal/ Excit " << currentInitialEnergy << " " << fin << " " << mass_out
         << " " << currentInitialEnergy - fin - mass_out << G4endl;
...
//----------------------------------------------------------------------------
{
  ...
  {
    G4cout << " Secondary E - Ekin / p " << (*iter)->GetDefinition()->GetParticleName() << " "
           << (*iter)->GetTotalEnergy() << " - " << (*iter)->GetKineticEnergy() << " / "
           << (*iter)->GetMomentum().x() << " " << (*iter)->GetMomentum().y() << " "
  ...
  G4cout << "BIC E/p delta " << (aTrack.Get4Momentum().e() + theInitial4Mom.e() - Efinal) / MeV
         << " MeV / mom " << (aTrack.Get4Momentum() - pFinal) / MeV << G4endl;

  return (aTrack.Get4Momentum().e() + theInitial4Mom.e() - Efinal) / aTrack.Get4Momentum().e()
         < perCent;
}
//----------------------------------------------------------------------------
...
//----------------------------------------------------------------------------
{
  ...
  {
    G4cout << " Secondary E - Ekin / p " << (*ktiter)->GetDefinition()->GetParticleName() << " "
  ...
  {
    G4cout << " Target E - Ekin / p " << (*ktiter)->GetDefinition()->GetParticleName() << " "
           << (*ktiter)->Get4Momentum().e() << " - "
           << (*ktiter)->Get4Momentum().e() - (*ktiter)->Get4Momentum().mag() << " / "
           << (*ktiter)->Get4Momentum().vect() << G4endl;
  ...
  {
    G4cout << " Captured E - Ekin / p " << (*ktiter)->GetDefinition()->GetParticleName() << " "
           << (*ktiter)->Get4Momentum().e() << " - "
           << (*ktiter)->Get4Momentum().e() - (*ktiter)->Get4Momentum().mag() << " / "
           << (*ktiter)->Get4Momentum().vect() << G4endl;
  ...
  {
    G4cout << " Finals E - Ekin / p " << (*ktiter)->GetDefinition()->GetParticleName() << " "
           << (*ktiter)->Get4Momentum().e() << " - "
           << (*ktiter)->Get4Momentum().e() - (*ktiter)->Get4Momentum().mag() << " / "
           << (*ktiter)->Get4Momentum().vect() << G4endl;
  ...
  G4cout << " Secondaries " << psecs << ", Targets " << ptgts << G4endl << " Captured    " << pcpts
         << ", Finals  " << pfins << G4endl << " Sum " << psecs + ptgts + pcpts + pfins
         << " PTransfer " << theMomentumTransfer << " Sum+PTransfer "
         << psecs + ptgts + pcpts + pfins + theMomentumTransfer << G4endl << G4endl;
...
//----------------------------------------------------------------------------
{
  ...
  //        if  (fragment != 0 ) {
  //            aNew = new G4ReactionProduct(G4Gamma::GammaDefinition());   // we only want to pass
  //            e/p aNew->SetMomentum(fragment->GetMomentum().vect());
