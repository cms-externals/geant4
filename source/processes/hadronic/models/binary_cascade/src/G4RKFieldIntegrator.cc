...
void G4RKFieldIntegrator::Transport(G4KineticTrackVector& theActive,
                                    const G4KineticTrackVector& theSpectators, G4double theTimeStep)
{
  (void)theActive;
  (void)theSpectators;
  (void)theTimeStep;
  ...
  for (G4int c1 = 0; c1 < nBarion; ++c1)
  {
    ...
    for (G4int c2 = c1 + 1; c2 < nBarion; ++c2)
    {
      ...
      Etot += Vo * 0.5 / r12 * G4Exp(1 / (4 * Alpha * GammaY * GammaY))
              * (G4Exp(-r12 / GammaY)
                   * (1 - Erf(0.5 / GammaY / std::sqrt(Alpha) - std::sqrt(Alpha) * r12))
                 - G4Exp(r12 / GammaY)
                     * (1 - Erf(0.5 / GammaY / std::sqrt(Alpha) + std::sqrt(Alpha) * r12)));

      // Ecoul
      Etot += 1.44 * p1->GetDefinition()->GetPDGCharge() * p2->GetDefinition()->GetPDGCharge() / r12
      ...
      for (G4int c3 = c2 + 1; c3 < nBarion; c3++)
      {
        ...
        // Esk3
        Etot = tGamma * G4Pow::GetInstance()->powA(4 * Alpha * Alpha / 3 / pi / pi, 1.5)
               * G4Exp(-Alpha * (r12 * r12 + r13 * r13));
      }
    }
  }
  return Etot;
...
G4double G4RKFieldIntegrator::Erf(G4double X)
{
  ...
  //   static G4ThreadLocal G4double P2[5] = { 7.3738883, 6.8650185,  3.0317993,
  //   0.56316962, 4.3187787e-5 }; static G4ThreadLocal G4double Q2[5] =
  //   { 7.3739609, 15.184908, 12.79553,   5.3542168,  1. };
...
void G4RKFieldIntegrator::Integrate(G4KineticTrackVector& theParticles)
   ...
   for(G4int cParticle = 0; cParticle < theParticles.length(); ++cParticle)
      {
      G4KineticTrack* pKineticTrack = theParticles.at(cParticle);
      pKineticTrack->SetPosition(pKineticTrack->GetPosition() +
theTimeStep*pKineticTrack->Get4Momentum().boostVector());
      }
   }
*/
//*************************************************************************************************************************************
