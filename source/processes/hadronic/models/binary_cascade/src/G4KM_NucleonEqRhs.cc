...
G4KM_NucleonEqRhs::G4KM_NucleonEqRhs(G4KM_DummyField* field, G4V3DNucleus* nucleus)
...
void G4KM_NucleonEqRhs::EvaluateRhsGivenB(const G4double y[], const G4double*,
                                          G4double dydx[]) const
{
  ...
  G4double deriv(0);
  if (density > 0)
    deriv = (factor / theMass) / G4Pow::GetInstance()->A13(density) * nuclearDensity->GetDeriv(pos);
