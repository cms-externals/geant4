...
bool G4Absorber::WillBeAbsorbed(const G4KineticTrack& kt)
{
  ...
  {
    if (kt.GetDefinition() == G4PionPlus::PionPlus() || kt.GetDefinition() == G4PionZero::PionZero()
...
{
  if (!FindAbsorbers(kt, tgt)) return false;
  return FindProducts(kt);
}

G4bool G4Absorber::FindAbsorbers(G4KineticTrack& kt, G4KineticTrackVector& tgt)
{
  ...
  for (iter = tgt.begin(); iter != tgt.end(); ++iter)
  {
    ...
    G4double charge = curr->GetDefinition()->GetPDGCharge();
    if ((charge0 + charge1 + charge < 0.) || (charge0 + charge1 + charge) > 2 * eplus)
  ...
  if ((kt1 == NULL) || (kt2 == NULL)) return false;
...
G4bool G4Absorber::FindProducts(G4KineticTrack& kt)
{
  ...
  G4LorentzVector momLab = kt.Get4Momentum() + abs1->Get4Momentum() + abs2->Get4Momentum();
  ...
  G4double squareP = (e0 * e0 * e0 * e0 - 2 * e0 * e0 * (ms1 * ms1 + ms2 * ms2)
                      + (ms2 * ms2 - ms1 * ms1) * (ms2 * ms2 - ms1 * ms1))
                     / (4 * e0 * e0);
  ...
  */
  ...
  G4KineticTrack* kt1 = new G4KineticTrack(prod1, 0., abs1->GetPosition(), mom1);
  G4KineticTrack* kt2 = new G4KineticTrack(prod2, 0., abs2->GetPosition(), mom2);
  // ------ debug
  /*
...
G4ThreeVector G4Absorber::GetRandomDirection()
{
  ...
  G4ThreeVector direction(std::sin(theta) * std::cos(phi), std::sin(theta) * std::sin(phi),
                          std::cos(theta));
  return direction;
}
