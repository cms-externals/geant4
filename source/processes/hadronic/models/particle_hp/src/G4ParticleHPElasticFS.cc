...
G4ReactionProduct 
G4ParticleHPElasticFS::GetBiasedThermalNucleus(const G4double aMass, G4ThreeVector aVelocity, const G4double temp)
{
  ...
  G4double eMin = 0.5*G4Neutron::Neutron()->GetPDGMass()*(vN_norm - 4./beta)*(vN_norm - 4./beta);
  G4double eMax = 0.5*G4Neutron::Neutron()->GetPDGMass()*(vN_norm + 4./beta)*(vN_norm + 4./beta);
  ...
  do {
    ...
    xsRelative = xsForDBRC->GetXsec( 0.5*G4Neutron::Neutron()->GetPDGMass()*vRelativeSpeed*vRelativeSpeed );
