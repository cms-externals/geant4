                                                       ...
                                                       G4Nucleus& aNucleus)
{
  ...
  {
    // Call capture final state in G4ParicleHPChannel and SELECT ISOTOPE (to be improved in the
    // future)
    G4ParticleHPManager::GetInstance()->OpenReactionWhiteBoard();
    theFinalState =
      (*G4ParticleHPManager::GetInstance()->GetCaptureFinalStates())[elementI]->ApplyYourself(
        aTrack);
  ...
  {  // the energy is inside the limits of the URR
    // calls the final state for the found element and isotope
    theFinalState = ((*G4ParticleHPManager::GetInstance()->GetCaptureFinalStates())[elementI]
...
{
  outFile << "High Precision model based on Evaluated Nuclear Data Files (ENDF) for capture "
