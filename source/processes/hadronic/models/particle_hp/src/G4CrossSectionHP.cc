...
G4CrossSectionHP::G4CrossSectionHP(const G4ParticleDefinition* p, const G4String& nameData,
    ...
    fDataDirectory(nameDir)
{
  ...
  if (verboseLevel > 1)
  {
    G4cout << "G4CrossSectionHP::G4CrossSectionHP: Initialise for " << fDataName << "  " << minZ
           << " < Z < " << maxZ << "  EmaxT(MeV)=" << emaxT << G4endl;
...
G4bool G4CrossSectionHP::IsIsoApplicable(const G4DynamicParticle* dp, G4int, G4int,
                                         const G4Element*, const G4Material*)
{
  return (dp->GetKineticEnergy() <= emax);
}

G4double G4CrossSectionHP::GetIsoCrossSection(const G4DynamicParticle* dp, G4int Z, G4int A,
...
G4double G4CrossSectionHP::ComputeIsoCrossSection(G4double e, G4double le,
...
G4double G4CrossSectionHP::IsoCrossSection(const G4double ekin, const G4double logek, const G4int Z,
                                           const G4int A, const G4double T)
{
  // G4cout << "G4CrossSectionHP::IsoCrossSection Z=" << Z << " A=" << A
  // << " E(MeV)=" << ekin/MeV << " T=" << T << "  " << GetName() << G4endl;
  G4double xs = 0.0;
  if (ekin > emax || Z > maxZ || Z < minZ || ekin < elimit)
  {
    return xs;
  }
  ...
  else
  {
    ...
    for (G4int i = 0; i < nn; ++i)
    {
      ...
      fLV.set(massTarget * vx, massTarget * vy, massTarget * vz,
              massTarget * (1.0 + 0.5 * (vx * vx + vy * vy + vz * vz)));
  ...
  if (verboseLevel > 1)
  {
    G4cout << "G4CrossSectionHP::IsoXS " << fDataName << "  Z= " << Z << "  A= " << A
           << " Ekin(MeV)= " << ekin / MeV << "  xs(b)= " << xs / barn << "  size=" << fZA.size()
           << G4endl;
  }
#endif
...
const G4Isotope* G4CrossSectionHP::SelectIsotope(const G4Element* elm, G4double, G4double)
{
  ...
  std::size_t j;
  if (Z < minZ || Z > maxZ || !CheckCache(Z) || 0 == fData->GetNumberOfComponents(Z - minZ))
  {
    for (j = 0; j < nIso; ++j)
    {
      sum += abundVector[j];
  ...
  for (j = 0; j < nIso; ++j)
  {
    sum += abundVector[j] * GetCrossSection(Z - minZ, elm->GetIsotope((G4int)j)->GetN());
    fTemp[j] = sum;
  }
  sum *= q;
  for (j = 0; j < nIso; ++j)
...
void G4CrossSectionHP::BuildPhysicsTable(const G4ParticleDefinition& p)
{
  if (verboseLevel > 1)
  {
    G4cout << "G4CrossSectionHP::BuildPhysicsTable for " << p.GetParticleName() << " and "
           << fDataName << G4endl;
  }
...
{
  if (fManagerHP->GetVerboseLevel() == 0 || fPrinted) return;
  ...
  fPrinted = true;
  G4cout << G4endl;
  G4cout << "HP Cross Section " << fDataName << " for " << fParticle->GetParticleName() << G4endl;
...
void G4CrossSectionHP::Initialise(const G4int Z)
{
  ...
  {
    G4cout << " G4CrossSectionHP::Initialise: Z=" << Z << " for " << fDataName << " minZ=" << minZ
           << " maxZ=" << maxZ << G4endl;
  }
  if (Z < minZ || Z > maxZ || nullptr != fData->GetElementData(Z - minZ))
  {
  ...
  for (G4int A = amin[Z]; A <= amax[Z]; ++A)
  {
    ...
    if (theXSData)
    {
      ...
      if (fManagerHP->GetVerboseLevel() > 1)
      {
        G4cout << "## G4CrossSectionHP::Initialise for Z=" << Z << " A=" << A << " Npoints=" << n
               << G4endl;
      }
      G4double x, y;
  ...
  if (noComp)
  {
    fData->InitialiseForComponent(Z - minZ, 0);
  }
  l.unlock();
}
