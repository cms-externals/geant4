...
G4bool G4CrossSectionHP::IsIsoApplicable(const G4DynamicParticle*,
					 G4int, G4int,
                                         const G4Element*,
					 const G4Material*)
{
...
G4double G4CrossSectionHP::IsoCrossSection(const G4double ek,
                                           const G4double,
                                           ...
                                           const G4double T)
{
  G4cout << "G4CrossSectionHP::IsoCrossSection Z=" << Z << " A=" << A
	 << " E=" << ek << " T=" << T << G4endl;
  G4double xs = 0.0;
  G4double ekin = std::max(ek, elimit);
  if (ekin > emax || Z > maxZ || Z < minZ) { return xs; }
  ...
  if (ekin >= emaxT*factT || fManagerHP->GetNeglectDoppler()) {
    ...
    G4RandGamma rand(G4Random::getTheEngine(), 2.0, lambda);
    ...
    const G4double lim = 0.03;
    const G4int nmin = 6;
    G4int nn = std::max(nmin, static_cast<G4int>(10*factT))*nmin;

    for (G4int i=0; i<nn; ++i) {
      ++counter;
      G4double erand = rand.fire();
      ...
      G4double delta = pv->Value(e);
      xs += delta;
      if (counter >= nmin && delta < lim*xs) { break; } 
    }
    xs /= (G4double)counter;
  }
#ifdef G4VERBOSE
                                                 ...
                                                 G4double, G4double)
{
  ...
  // reuse cache
  for (j=0; j<nIso; ++j) {
    sum += abundVector[j]*GetCrossSection(Z - minZ, elm->GetIsotope(j)->GetN());
    fTemp[j] = sum;
  }
  sum *= q;
  for (j = 0; j<nIso; ++j) {
...
void G4CrossSectionHP::BuildPhysicsTable(const G4ParticleDefinition& p)
{
  ...
  if (p.GetParticleName() != "neutron") { 
    ...
    G4Exception("G4CrossSectionHP::BuildPhysicsTable(..)","had012",
		FatalException, ed, "");
    return; 
  }
...
{
  if (fManagerHP->GetVerboseLevel() == 0 || !isInitializer)
    return;
