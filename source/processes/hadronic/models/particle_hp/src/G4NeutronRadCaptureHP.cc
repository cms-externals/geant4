...
void G4NeutronRadCaptureHP::BuildPhysicsTable(const G4ParticleDefinition&)
{
  ...
  if (nullptr != p)
  {
    auto handler = (static_cast<G4VPreCompoundModel*>(p))->GetExcitationHandler();
    if (nullptr != handler) photonEvaporation = handler->GetPhotonEvaporation();
  }
                                                      ...
                                                      G4Nucleus& theNucleus)
{
  ...
  if (A <= 4)
  {
    if (verboseLevel > 1)
    {
      G4cout << "G4NeutronRadCaptureHP::DoIt: Eini(MeV)=" << ekin / MeV
             << "  Eexc(MeV)= " << (M - mass) / MeV << "  Z= " << Z << "  A= " << A << G4endl;
    }
    if (M - mass > lowestEnergyLimit)
    {
      ...
      lab4mom -= lv2;
      G4HadSecondary* news = new G4HadSecondary(new G4DynamicParticle(G4Gamma::Gamma(), lv2));
      news->SetTime(time);
    ...
    {
      theDef = G4Alpha::Alpha();
    }
    else
    {
      theDef = theTableOfIons->GetIon(Z, A, 0.0, noFloat, 0);
    }

    if (nullptr != theDef)
    {
      G4HadSecondary* news = new G4HadSecondary(new G4DynamicParticle(theDef, lab4mom));
      news->SetTime(time);
      news->SetCreatorModelID(secID);
      theParticleChange.AddSecondary(*news);
      delete news;
  ...
  else
  {
    ...
    if (verboseLevel > 1)
    {
      G4cout << "G4NeutronRadCaptureHP::ApplyYourself initial G4Fragmet:" << G4endl;
    ...
    if (nullptr == fv)
    {
      fv = new G4FragmentVector();
    }
    fv->push_back(aFragment);

    if (verboseLevel > 1)
    {
      G4cout << "G4NeutronRadCaptureHP: " << fv->size() << " final particle icID= " << icID
             << G4endl;
    }
    for (auto const& f : *fv)
    {
      ...
      if (0 == Z && 0 == A)
      {
        theDef = f->GetParticleDefinition();
      }
      else if (Z == 1 && A == 2)
      {
        theDef = G4Deuteron::Deuteron();
      ...
      if (verboseLevel > 1)
      {
        G4cout << theDef->GetParticleName() << " Ekin(MeV)= " << ekin / MeV
               << " p: " << f->GetMomentum().vect() << G4endl;
      }
      G4HadSecondary* news =
        new G4HadSecondary(new G4DynamicParticle(theDef, f->GetMomentum().vect().unit(), ekin));
