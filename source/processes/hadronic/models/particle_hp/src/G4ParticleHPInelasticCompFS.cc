...
G4ParticleHPInelasticCompFS::G4ParticleHPInelasticCompFS() : G4ParticleHPFinalState()
{
  QI.resize(51);
  LR.resize(51);
                                       ...
                                       const G4String& aFSType, G4ParticleDefinition*)
{
  ...
  {
#ifdef G4VERBOSE
    if (fManager->GetDEBUG()) G4cout << "Skipped = " << filename << " " << A << " " << Z << G4endl;
#endif
    hasAnyData = false;
    hasFSData = false;
    hasXsec = false;
  ...
  while (theData >> infoType)  // Loop checking, 11.05.2015, T. Koi
  {
    ...
    else
    {
      ...
      G4Exception("G4ParticleHPInelasticCompFS::Init", "hadr01", JustWarning, ed,
                  "Data-type unknown");
    }
  }
}
                                                 ...
                                                 G4ParticleDefinition* aDefinition)
{
...
#ifdef G4VERBOSE
  if (fManager->GetDEBUG())
    G4cout << "G4ParticleHPInelasticCompFS::CompositeApply A=" << theBaseA << " Z=" << theBaseZ
           << " incident " << hadProjectile->GetDefinition()->GetParticleName() << G4endl;
#endif
  ...
  theTarget = aNucleus.GetBiasedThermalNucleus(targetMass / CLHEP::neutron_mass_c2, neuVelo,
  ...
  else if (theEnergyAngData[it] != nullptr)  // MF6
  {
    ...
    if (theParticles != nullptr)
    {
      ...
      G4int dZ =
        theBaseZ + G4lrint(hadProjectile->GetDefinition()->GetPDGCharge() / CLHEP::eplus) - sumZ;
      if (dA < 0 || dZ < 0)
      {
  ...
  if (theParticles != nullptr)
  {
    ...
    if (totalBaryonNumber != theBaseA + hadProjectile->GetDefinition()->GetBaryonNumber())
    {
      needsSeparateRecoil = true;
      residualA = theBaseA + hadProjectile->GetDefinition()->GetBaryonNumber() - totalBaryonNumber;
      residualZ =
        theBaseZ
                                                    ...
                                                    G4double resExcitationEnergy)
{
  ...
  // Residual definition:
  G4int resZ =
    G4lrint((proj->GetDefinition()->GetPDGCharge() + targ->GetDefinition()->GetPDGCharge()
             - product->GetDefinition()->GetPDGCharge())
            / CLHEP::eplus);
