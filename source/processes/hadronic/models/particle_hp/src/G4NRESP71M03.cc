...
void G4NRESP71M03::DKINMA(G4ReactionProduct *p1, G4ReactionProduct *p2, G4ReactionProduct *p3, G4ReactionProduct *p4, const G4double Q, const G4double costhcm3)
	...
	if ( p2 ) // If it is not a decay reaction...
		...
		CM.SetMass(sqrt(TotalEnergyLAB*TotalEnergyLAB-TotalMomentumLAB*TotalMomentumLAB));
	...
	G4RotationMatrix rot(acos(p1unit*G4ThreeVector(0., 1., 0.)), acos(p1unit*G4ThreeVector(0., 0., 1.)), 0.);
	rot.inverse();

	const G4double theta = acos(costhcm3);
	const G4double phi = twopi*G4UniformRand();

	const G4double Energy3CM = (pow(TotalEnergyCM, 2.)+pow(p3->GetMass(), 2.)-pow(p4->GetMass(), 2.))/(2.*TotalEnergyCM);
	p3->SetTotalEnergy(Energy3CM);

	const G4double Momentum3CM = sqrt(pow(Energy3CM, 2.)-pow(p3->GetMass(), 2.));
	p3->SetMomentum(rot*G4ThreeVector(Momentum3CM*sin(theta)*cos(phi), Momentum3CM*sin(theta)*sin(phi), Momentum3CM*costhcm3));

	const G4double Energy4CM = TotalEnergyCM-Energy3CM;
	p4->SetTotalEnergy(Energy4CM);

	const G4double Momentum4CM = sqrt(pow(Energy4CM, 2.)-pow(p4->GetMass(), 2.));
