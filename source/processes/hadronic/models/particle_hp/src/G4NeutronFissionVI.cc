...
{
  if (fFission != nullptr && fHandler != nullptr)
  {
    return;
  }
                                                   ...
                                                   G4Nucleus& theNucleus)
{
  ...
  if (verboseLevel > 1)
  {
    G4cout << "G4NeutronFissionVI::ApplyYourself initial G4Fragmet:" << G4endl;
  ...
  if (verboseLevel > 1)
  {
    G4cout << "G4NeutronFissionVI: " << final->size() << " final particle secID= " << secID
           << G4endl;
  }
  for (auto const& ptr : *final)
  {
    ...
    if (verboseLevel > 1)
    {
      G4cout << theDef->GetParticleName() << " Ekin(MeV)= " << ekin / MeV
             << " p: " << ptr->GetMomentum() << G4endl;
    }
    G4HadSecondary* news =
      new G4HadSecondary(new G4DynamicParticle(theDef, ptr->GetMomentum().unit(), ekin));
