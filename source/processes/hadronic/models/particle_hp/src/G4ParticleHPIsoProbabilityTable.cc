...
G4double G4ParticleHPIsoProbabilityTable::GetIsoCrossSectionPT(const G4DynamicParticle*, G4int,
                                                               ...
                                                               std::thread::id&)
{
  G4Exception("G4ParticleHPIsoProbabilityTable::GetIsoCrossSectionPT", "hadhp01", FatalException,
                                                                       ...
                                                                       G4int isotopeJ)
{
  ...
  G4ThreeVector neutronVelocity =
    1.0 / G4Neutron::Neutron()->GetPDGMass() * theNeutron.GetMomentum();
...
///--------------------------------------------------------------------------------------
G4double
G4ParticleHPIsoProbabilityTable::GetDopplerBroadenedInelasticXS(const G4DynamicParticle* dP,
                                                                G4int indexEl, G4int isotopeJ)
{
  G4double result = 0.0;
  G4ReactionProduct theNeutron(dP->GetDefinition());
  theNeutron.SetMomentum(dP->GetMomentum());
  ...
  while (counter == 0 || std::abs(buffer - result / std::max(1, counter)) > 0.01 * buffer)
  {
    ...
    while (counter < size)
    {
      ...
      G4ReactionProduct aThermalNuc =
        aNuc.GetThermalNucleus(eleMass / G4Neutron::Neutron()->GetPDGMass(), T);
      boosted.Lorentz(theNeutron, aThermalNuc);
      G4double theEkin = boosted.GetKineticEnergy();
      aXsection = ((*G4ParticleHPManager::GetInstance()->GetInelasticFinalStates(
                    dP->GetDefinition()))[indexEl]
                     ->GetWeightedXsec(theEkin, isotopeJ))
                  * barn;
      if (aXsection < 0.0)
      {
        if (failCount < 1000)
