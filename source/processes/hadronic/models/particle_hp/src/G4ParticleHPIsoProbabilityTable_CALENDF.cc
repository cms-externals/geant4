...
///--------------------------------------------------------------------------------------
void G4ParticleHPIsoProbabilityTable_CALENDF::Init(G4int theZ, G4int theA, G4int them,
                                                   G4double theT, const G4String& dirName)
{
  Z = theZ;
  A = theA;
  m = them;
  T = theT;
  G4cout << "The CALENDF probability tables are being initialized for Z=" << Z << " A=" << A
         << " and T=" << T << " K." << G4endl;
  ...
  if (theData.good())
  {
    ...
    G4cout << "Probability tables found and succesfully read from " << Emin / keV << " keV to "
           << Emax / keV << " keV." << G4endl;
  }
  else
  {
    G4cout << "No probability tables found for this isotope and temperature, smooth cross section "
...
///--------------------------------------------------------------------------------------
G4double G4ParticleHPIsoProbabilityTable_CALENDF::GetCorrelatedIsoCrossSectionPT(
  const G4DynamicParticle* dp, G4int MTnumber, const G4Element* ele, G4double& kineticEnergy,
  G4double& random_number, std::thread::id& id)
{
  // if this function is called again for different reaction or the particle came to different
  // region with the same temperature and unchanged temperature
  ...
  {
    // if the kinetic energy is outside of the URR limits for the given isotope, it finds the smooth
    // cross section
    ...
    {
      weightedelasticXS =
        (*G4ParticleHPManager::GetInstance()->GetElasticFinalStates())[indexEl]->GetWeightedXsec(
          kineticEnergy, isotopeJ);
      weightedcaptureXS =
        (*G4ParticleHPManager::GetInstance()->GetCaptureFinalStates())[indexEl]->GetWeightedXsec(
          kineticEnergy, isotopeJ);
      weightedinelasticXS = ((*G4ParticleHPManager::GetInstance()->GetInelasticFinalStates(
                              dp->GetDefinition()))[indexEl]
                               ->GetWeightedXsec(kineticEnergy, isotopeJ))
                            * barn;
    }
    else
    {
    ...
    else
    {
      if (G4ParticleHPManager::GetInstance()->GetNeglectDoppler())
      {
        G4double weightedfissionXS =
          (*G4ParticleHPManager::GetInstance()->GetFissionFinalStates())[indexEl]->GetWeightedXsec(
            kineticEnergy, isotopeJ);
  ...
  else
  {
    ...
    if (Z < 88)
      ...
      xsfiss_cache[id] = theFissionData->at(indexE)->at(indexP);
  }
  if (MTnumber == 2)
  {  // elastic cross section
    return xsela_cache[id];
...
///--------------------------------------------------------------------------------------
G4double G4ParticleHPIsoProbabilityTable_CALENDF::GetIsoCrossSectionPT(
  const G4DynamicParticle* dp, G4int MTnumber, const G4Element* ele, G4double& kineticEnergy,
  std::map<std::thread::id, G4double>& random_number_cache, std::thread::id& id)
{
  energy_cache[id] = kineticEnergy;
  if (kineticEnergy < Emin || kineticEnergy > Emax)
  {
    ...
    {
      weightedelasticXS =
        (*manHP->GetElasticFinalStates())[indexEl]->GetWeightedXsec(kineticEnergy, isotopeJ);
      weightedcaptureXS =
        (*manHP->GetCaptureFinalStates())[indexEl]->GetWeightedXsec(kineticEnergy, isotopeJ);
      weightedinelasticXS =
        (*manHP->GetInelasticFinalStates(dp->GetDefinition()))[indexEl]->GetWeightedXsec(
          kineticEnergy, isotopeJ)
        * barn;
    }
    else
    ...
    else
    {
      ...
      {
        G4double weightedfissionXS =
          (*manHP->GetFissionFinalStates())[indexEl]->GetWeightedXsec(kineticEnergy, isotopeJ);
        xsfiss_cache[id] = weightedfissionXS / frac;
      }
      else
      {
