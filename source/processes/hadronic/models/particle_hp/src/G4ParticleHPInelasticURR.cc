                                                         ...
                                                         G4Nucleus& aNucleus)
{
  ...
  {
    // Call inelastic final state in G4ParicleHPChannel and SELECT ISOTOPE (to be improved in the
    // future)
    const G4Element* target_element = (*G4Element::GetElementTable())[elementI];
    theFinalState = (*G4ParticleHPManager::GetInstance()->GetInelasticFinalStates(
      aTrack.GetDefinition()))[elementI]
