...
void G4ParticleHPFinalState::adjust_final_state(G4LorentzVector init_4p_lab)
{
  ...
  for (G4int i = 0; i < nSecondaries; ++i)
  {
    ...
    if (ptr->GetAtomicMass() == max_SecA) imaxA = i;
#ifdef G4PHPDEBUG
  ...
  else
  {
    ...
    if (needOneMoreSec)
    {
      ...
      else
      {
...
#endif
        oneMoreSec_pd = G4IonTable::GetIonTable()->GetIon(baseZNew - sum_Z, baseANew - sum_A, 0.0);
        if (oneMoreSec_pd == nullptr)
        {
          ...
          G4Exception("G4ParticleHPFinalState:adjust_final_state", "hadr01", JustWarning, ed,
    ...
    if (resi_pd == nullptr)
    {
      ...
      if (ndlZNew == sum_Z && ndlANew == sum_A)
      {
        ...
        {
          G4ExceptionDescription ed;
          ed << "resi_pd Z=" << max_SecZ << " - " << dif_Z << " A=" << max_SecA << " - " << dif_A
             << " projectile " << theProjectile->GetParticleName();
          G4Exception("G4ParticleHPFinalState:adjust_final_state", "hadr02", JustWarning, ed,
                      "No adjustment will be done!");
          return;
        }
