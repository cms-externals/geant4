  ...
  : Temperatures(0), ProbabilityTables(0), URRlimits(0), usedNjoy(0), usedCalendf(0)
{
  ...
  {
    G4Exception("G4ParticleHPProbabilityTablesStore::G4ParticleHPProbabilityTablesStore()",
                "hadhp01", FatalException, "Please setenv G4URRPTDATA, it is not defined.");
  }
  ...
  {
    G4Exception("G4ParticleHPProbabilityTablesStore::G4ParticleHPProbabilityTablesStore()",
                "hadhp01", FatalException,
                "The format of probability tables is not set properly, "
                "please set it with G4HadronicParameters::Instance()->SetTypeTablePT() before "
  ...
  for (std::size_t im = 0; im < G4Material::GetNumberOfMaterials(); ++im)
  {
    ...
    for (G4int e = 0; e < (G4int)theMaterial->GetNumberOfElements(); ++e)
    {
      for (G4int i = 0; i < (G4int)theMaterial->GetElement(e)->GetNumberOfIsotopes(); ++i)
      {
        ...
        if (std::find(isotemperatures.begin(), isotemperatures.end(), Temp)
            == isotemperatures.end())
        {
        ...
        else
        {
          if (isoinmat[indexI])
          {
            ...
            message << "The isotope Z=" << theMaterial->GetElement(e)->GetIsotope(i)->GetZ()
                    ...
                    << "This may cause bias in selection of target isotopes, if there are elements "
                       "with different URR limits in the material.\n"
                    << "Please make materials only with elements with different Z.";
            G4Exception("G4ParticleHPProbabilityTablesStore::InitURRlimits()", "hadhp01",
                        JustWarning, message);
          }
        }
...
G4ParticleHPProbabilityTablesStore::~G4ParticleHPProbabilityTablesStore()
{
  for (G4int i = 0; i < numIso; i++)
  {
    for (std::map<G4int, G4ParticleHPIsoProbabilityTable*>::iterator it =
           (*ProbabilityTables)[i].begin();
                                                                  ...
                                                                  const G4Material* mat)
{
  ...
  {
    return (*ProbabilityTables)[indexI][T]->GetCorrelatedIsoCrossSectionPT(
      dp, MTnumber, ele, kineticEnergy, random_number_cache[indexI][id], id);
  }
  else
  {
    energy_cache[indexI][id] = kineticEnergy;
    return (*ProbabilityTables)[iso->GetIndex()][T]->GetIsoCrossSectionPT(
      dp, MTnumber, ele, kineticEnergy, random_number_cache[indexI], id);
  }
}
...
void G4ParticleHPProbabilityTablesStore::Init()
{
  ...
  for (G4int i = 0; i < numIso; i++)
  {
    ...
    for (G4int T : (*Temperatures)[i])
    {  // create probability table for each temperature and isotope
      if (usedNjoy)
      {
...
void G4ParticleHPProbabilityTablesStore::InitURRlimits()
{
  ...
  for (std::size_t i = 0; i < G4Element::GetNumberOfElements(); ++i)
  {
    ...
    G4double maxE = 0.0;
    for (G4int ii = 0; ii < (G4int)(*(G4Element::GetElementTable()))[i]->GetNumberOfIsotopes();
         ++ii)
    {
      ...
      for (G4int T : (*Temperatures)[indexI])
      {
        ...
        if (theData.good() && hasalreadyPT)
        {
          ...
          if (emin2 != emin || emax2 != emax)
          {
            ...
            G4Exception("G4ParticleHPProbabilityTablesStore::InitURRlimits()", "hadhp01",
                        JustWarning, message);
            G4cout << "Temperature T= " << T << " emin=" << emin << " emax=" << emax
                   << " minE=" << minE << " maxE=" << maxE << G4endl;
        ...
        else if (theData.good())
        {
          ...
          {
            G4ExceptionDescription message;
            message << "There are probability tables only for some of the used temperatures for "
                       "isotope with Z="
                    << Z << " and A=" << A << ".\n"
                    << "Smooth cross section will be used for other temperature(s) of this "
                       "isotope, which may cause severe differences in simulation.";
            G4Exception("G4ParticleHPProbabilityTablesStore::InitURRlimits()", "hadhp01",
                        JustWarning, message);
        ...
        else if (hasalreadyPT && wasnotprintedyet[indexI])
        {
          ...
          message << "There are probability tables only for some of the used temperatures for "
                     "isotope with Z="
                  << Z << " and A=" << A << ".\n"
                  << "Smooth cross section will be used for other temperature(s) of this isotope, "
                     "which may cause severe differences in simulation.";
          G4Exception("G4ParticleHPProbabilityTablesStore::InitURRlimits()", "hadhp01", JustWarning,
                      message);
          wasnotprintedyet[indexI] = false;
        }
        else
