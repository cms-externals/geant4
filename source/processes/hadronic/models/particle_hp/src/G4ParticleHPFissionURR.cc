                                                       ...
                                                       G4Nucleus& aNucleus)
{
  ...
  for (G4int i = 0; i < (G4int)theMaterial->GetNumberOfElements(); ++i)
  {
    if (Z == theMaterial->GetElement(i)->GetZasInt())
    {
      ...
      // the loop cannot be ended here because the material can have two elements with same Z but
  ...
  if (elementI == -1 || isotopeJ == -1)
  {
    return theFinalState;
  }

  // Check whether the energy is out of the URR limits for the given element
  if (kineticEnergy < (*URRlimits).at(elementI).first
      || kineticEnergy > (*URRlimits).at(elementI).second)
  {
    // Call fission final state in G4ParicleHPChannel and SELECT ISOTOPE (to be improved in the
    // future)
    G4ParticleHPManager::GetInstance()->OpenReactionWhiteBoard();
    theFinalState =
      (*G4ParticleHPManager::GetInstance()->GetFissionFinalStates())[elementI]->ApplyYourself(
        aTrack, -2);
    // Update target nucleus information according to the selected isotope
    G4int selectedIsotope_A =
  ...
  {
    // the energy is inside the limits of the URR, part copied from
    // G4ParticleHPChannel::ApplyYourself
    if (G4ParticleHPManager::GetInstance()->GetUseWendtFissionModel())
    {
      if ((*G4ParticleHPManager::GetInstance()->GetFissionFinalStates())[elementI]
            ->GetWendtFissionGenerator())
      {
        theFinalState = (*G4ParticleHPManager::GetInstance()->GetFissionFinalStates())[elementI]
                          ->GetWendtFissionGenerator()
                          ->ApplyYourself(aTrack, Z, A);
      }
    }
    if (!theFinalState)
    {
...
{
  outFile << "High Precision model based on Evaluated Nuclear Data Files (ENDF) for fission "
             "reaction of neutrons in the unresolved resonance region.";
}
