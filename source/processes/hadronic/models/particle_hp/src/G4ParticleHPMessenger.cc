...
:manager(man)
{
   ...
   MinEnergyDBRCCmd->SetGuidance("Energy threshold under which the DBRC method is not applied and only the SVT is used.");
...
void G4ParticleHPMessenger::SetNewValue(G4UIcommand* command,G4String newValue)
{
   ...
   if ( command == UseDBRCCmd ) {
     ...
     if ( manager->GetUseDBRC() != bValue ) {
       ...
       if ( G4HadronicParameters::Instance()->GetVerboseLevel() > 0 ) {
         G4cout << G4endl
         << "=== G4ParticleHPMessenger CHANGED PARAMETER UseDBRC TO "
         << bValue << " ===" << G4endl;
       }
       if ( bValue ) {
