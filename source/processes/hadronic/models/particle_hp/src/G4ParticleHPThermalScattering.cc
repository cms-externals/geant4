   ...
   for ( ; i < anE_P_E_isoAng->secondary_energy_cdf_size-1 ; ++i ) 
   {
	 ...
	 if ( fabs(alpha) < 1E-8 )
	    ...
	    rndm = rndm2;
            ...
            if ( delta < 0 && fabs(delta) < 1.E-8 ) delta = 0;
	
            lambda = -beta + sqrt(delta);
...
sample_inelastic_E_mu( G4double pE , std::vector< E_P_E_isoAng* >* vNEP_EPM ) 
{
   ...
   if ( (energyLH.second - pE) <= 0. && fabs(pE/energyLH.second - 1) < 1E-11 ) factor = 0.;
   if ( (energyLH.first - pE) >= 0. && fabs(energyLH.first / pE - 1) < 1E-11 ) factor = 1.;
