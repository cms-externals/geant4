    ...
    theMaxEnergyDoppler(30. * CLHEP::keV)
{
  ...
  if (nullptr != ss && "BetweenInts" == G4String(ss))
  {
    PHP_USE_POISSON = false;
  }
  ...
  if (nullptr == nch)
  {
    G4Exception("G4ParticleHPManager::G4ParticleHPManager()", "hadhp01", FatalException,
                "G4NEUTRONXSDATA is not defined - check path");
  }
  else
  {
...
G4int G4ParticleHPManager::GetPHPIndex(const G4ParticleDefinition* part) const
{
  ...
  else
  {
    ...
    G4Exception("G4ParticleHPManager::G4ParticleHPManager()", "hadhp01", FatalException, ed, "");
  }
  return idx;
}
...
void G4ParticleHPManager::DumpSetting()
{
  ...
  G4cout << G4endl << "=======================================================" << G4endl
         ...
         << G4HadronicParameters::Instance()->GetTypeTablePT() << G4endl
         << "=======================================================" << G4endl << G4endl;
  isPrinted = true;
}
