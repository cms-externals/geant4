...
public:
  ...
  inline G4double GetLevelEnergy() const {
    return levelEnergy;
  }

  inline G4double GetNumberOfGammas() const {
    return nGammas;
  }

  inline G4double GetGammaEnergy(G4int idx) const {
    return (idx < nGammas && idx >= 0) ? 
      gammas[idx].gammaEnergy : 0.0;
  }

  G4ParticleHPNucLevel(const G4ParticleHPNucLevel&) = delete;
  const G4ParticleHPNucLevel& operator=
  (const G4ParticleHPNucLevel&) = delete;
