...
void G4FermiBreakUpVI::BreakFragment(G4FragmentVector* theResult, G4Fragment* theNucleus)
{
  ...
  if (!IsApplicable(Z, A, excitation))
  {
    return;
  }
  ...
  if (!SampleDecay(Z, A, mass, excitation, lv0))
  {
    return;
  }
  ...
  for (std::size_t i = 0; i < frag.size(); ++i)
  {
    ...
    if (unstable)
    {
      ...
      if (verbose > 1)
      {
        G4cout << "# FermiFrag " << i << ".  Z= " << Z << " A= " << A << " mass= " << mass
               << " exc= " << frag[i]->GetExcitationEnergy() << G4endl;
      }
                                     ...
                                     const G4double exc, G4LorentzVector& lv0)
{
  ...
  if (verbose > 1)
  {
    G4cout << "G4FermiBreakUpVI::SampleDecay " << nn << " channels Eex= " << chan->GetExcitation()
           << G4endl;
  }
  if (0 == nn)
  {
  ...
  else
  {
    ...
    if (verbose > 2)
    {
      G4cout << "Probabilities of 2-body decay: Nchannels=" << nn << " channels; i=" << i
             << " is selected; predefined=" << pre << G4endl;
      for (std::size_t j = 0; j < nn; ++j)
      {
        ...
        G4cout << " Z1= " << pvect[j]->GetFragment1()->GetZ()
               ...
               << " A2= " << pvect[j]->GetFragment2()->GetA() << G4endl;
      }
    }
  }
  if (nullptr == fpair)
  ...
  if (verbose > 2)
  {
    G4cout << " M= " << mass << " M1= " << mass1 << "  M2= " << mass2
           << " Exc1= " << frag1->GetExcitationEnergy() << " Exc2= " << frag2->GetExcitationEnergy()
           << G4endl;
  }
  // sample fragment1
