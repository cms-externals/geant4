...
G4FermiFragmentsPoolVI::~G4FermiFragmentsPoolVI()
{
  ...
  for (auto const& ptr : fragment_pool)
  {
    delete ptr;
  }
}

G4bool G4FermiFragmentsPoolVI::HasDecay(const G4int Z, const G4int A, const G4double eexc) const
{
...
const G4FermiChannels* G4FermiFragmentsPoolVI::ClosestChannels(G4int Z, G4int A,
                                                               G4double etot) const
{
...
G4bool G4FermiFragmentsPoolVI::IsInThePool(const G4int Z, const G4int A, const G4double exc) const
{
      ...
      if (nullptr != man)
      {
        ...
        for (std::size_t i = 0; i <= nn; ++i)
        {
          ...
          */
          ...
          if (IsInThePool(Z, A, exc))
          {
            continue;
          }
          fragment_pool.push_back(
            new G4FermiFragment(A, Z, man->TwoSpinParity(i), exc, man->LifeTime(i)));
        }
      }
    }
  }
    ...
    for (G4int Z = 0; Z < maxZ; ++Z)
    {
      ...
      for (std::size_t k = 0; k < kmax; ++k)
      {
        ...
        if (1 < nch)
        {
          ...
          {
            ptot += G4FermiBreakUpUtil::Probability(
              frag->GetA(), pairs[i]->GetFragment1(), pairs[i]->GetFragment2(),
              frag->GetTotalEnergy(), frag->GetExcitationEnergy());
...
{
  G4cout << "----------------------------------------------------------------" << G4endl;
  G4cout << "##### List of Fragments in the Fermi Fragment Pool #####" << G4endl;
  ...
  for (G4int A = 1; A < maxA; ++A)
  {
    for (G4int Z = 0; Z < maxZ; ++Z)
    {
      ...
      for (std::size_t j = 0; j < jmax; ++j)
      {
        ...
        std::size_t nch = ch->NumberPairs();
        ama += nch;
        G4cout << "   (" << a1 << "," << z1 << ");  Eex(MeV)= " << f->GetExcitationEnergy()
               << " 2S=" << f->TwoSpinParity() << "; Nchannels=" << nch << G4endl;
        for (std::size_t k = 0; k < nch; ++k)
        {
          ...
          G4cout << "         (" << fpair->GetFragment1()->GetZ() << ", "
                 ...
                 << fpair->GetFragment2()->GetZ() << ", " << fpair->GetFragment2()->GetA() << ", "
                 << fpair->GetFragment2()->GetExcitationEnergy()
                 << ")  prob= " << fpair->Probability() << G4endl;
        }
      }
    }
  }
