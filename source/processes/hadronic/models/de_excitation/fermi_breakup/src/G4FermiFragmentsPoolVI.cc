...
G4FermiFragmentsPoolVI::~G4FermiFragmentsPoolVI()
{
  for(G4int i=0; i<maxA; ++i) {
    ...
    for(auto & ptr : list_d[i]) { delete ptr; ptr = nullptr; }
    for(auto & ptr : list_u[i]) { delete ptr; ptr = nullptr; }
  }
  for(auto & ptr : fragment_pool) { delete ptr; ptr = nullptr; }
  for(auto & ptr : funstable) { delete ptr; ptr = nullptr; }
}
...
G4FermiFragmentsPoolVI::IsApplicable(G4int Z, G4int A, G4double etot) const
{
  ...
  for(auto const& ptr : list_f[A]) {
    if(Z == ptr->GetZ()) { 
      if(etot <= ptr->GetFragmentMass() + elim) { return true; }
      isInList = true;
    }
  }
  if(isInList) { return false; }
  for(auto const& ptr : list_g[A]) {
    if(Z == ptr->GetZ() && etot <= ptr->GetFragmentMass() + elim) { return true; }
  }
  return false;
}
...
void G4FermiFragmentsPoolVI::Initialise()
{
  ...
  const size_t pphm[maxA] = {
    0, 0, 2, 2, 4, 4, 6, 6, 6, 6, 6, 6, 7, 6, 7, 6, 6};
  const size_t punm[maxA] = {
