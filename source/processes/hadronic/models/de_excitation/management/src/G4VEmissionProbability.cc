...
G4VEmissionProbability::G4VEmissionProbability(G4int Z, G4int A)
  : OPTxs(3),pVerbose(1),theZ(Z),theA(A),resZ(0),resA(0),
    ...
    elimit(CLHEP::MeV),accuracy(0.02) 
{
  pNuclearLevelData = G4NuclearLevelData::GetInstance(); 
  pG4pow = G4Pow::GetInstance();
  pPairCorr = pNuclearLevelData->GetPairingCorrection();
                                                      ...
                                                      G4double cb)
{
  ...
  if(pVerbose > 1) { G4cout << " Probability= " << pProbability << G4endl; }
  return pProbability;
}
...
G4double G4VEmissionProbability::SampleEnergy()
{
  ...
  //return FindRecoilExcitation(ekin);
  return ekin;
}
...
G4double G4VEmissionProbability::FindRecoilExcitation(G4double e)
{
  ...
  G4double mres  = sqrt(m02 + m12 - 2.*pMass*(mass + e));

  fExcRes = mres - pResMass;
  const G4double tolerance = 0.1*CLHEP::keV;

  if(pVerbose > 1) {
    G4cout << "### G4VEmissionProbability::FindRecoilExcitation for resZ= " << resZ 
	   << " resA= " << resA << " evaporated Z= " << theZ << " A= " << theA
