...
G4String G4LevelManager::fFloatingLevels[] = {"-",  "+X", "+Y", "+Z", "+U", "+V", "+W",
                                              "+R", "+S", "+T", "+A", "+B", "+C"};
...
std::size_t G4LevelManager::NearestLevelIndex(const G4double energy, const std::size_t index) const
{
  ...
  if (idx < nTransitions && (fLevelEnergy[idx] + fLevelEnergy[idx + 1]) * 0.5 <= energy)
  {
    ++idx;
  }
...
void G4LevelManager::StreamInfo(std::ostream& out) const
{
  for (std::size_t i = 0; i <= nTransitions; ++i)
  {
    ...
    out << std::setw(6) << i << ". " << std::setw(8) << fLevelEnergy[i];
    if (fLevels[i])
    {
      out << std::setw(8) << fLevels[i]->GetTimeGamma() << std::setw(4)
          << fLevels[i]->NumberOfTransitions() << std::setw(4) << std::abs(TwoSpinParity(i))
          << std::setw(4) << Parity(i) << std::setw(4) << FloatingLevel(i);
    }
    out << "\n";
    out.precision(prec);
    if (fLevels[i])
    {
      fLevels[i]->StreamInfo(out);
    }
  }
}
