...
G4double G4ChatterjeeCrossSection::ComputeCrossSection(G4double K, G4double cb, G4double resA13,
                                                       G4double amu1, G4int idx, G4int Z,
                                                       G4int resA)
{
  G4double sig;
  ...
  if (0 == Z)
  {
    ...
    G4double nu =
      std::abs((paramC[idx][7] * resA + paramC[idx][8] * resA13) * resA13 + paramC[idx][9]);
  ...
  else
  {
    ...
    {
      sig = p * (Kc - ji) * (Kc - ji) + landa * Kc + mu + nu * (2 - Kc / ji) / ji;
    }
  }
  sig = std::max(sig, 0.0);
