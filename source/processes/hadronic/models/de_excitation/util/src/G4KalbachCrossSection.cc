...
G4double G4KalbachCrossSection::ComputeCrossSection(G4double K, G4double cb, G4double resA13,
                                                    G4double amu1, G4int idx, G4int Z, G4int A,
                                                    G4int resA)
{
  G4double sig = 0.0;
  G4double signor = 1.0;
  ...
  if (idx == 0)
  {  // parameterization for neutron

    if (resA < 40)
    {
      signor = 0.7 + resA * 0.0075;
    }
    else if (resA > 210)
    {
      signor = 1. + (resA - 210) * 0.004;
    }
    ...
    nu = std::abs((paramK[idx][7] * resA + paramK[idx][8] * resA13) * resA13 + paramK[idx][9]);
  }
  else
  {  // parameterization for charged
    // proton correction
    if (idx == 1)
    {
      if (resA <= 60)
  ...
  */
  ...
  if (elab < ec)
  {
    ...
    if (0 < Z)
    {
      p += paramK[idx][1] / ec + paramK[idx][2] / ecsq;
    }
