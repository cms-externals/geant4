...
{
  fragA = fragZ = 0;
  resA13 = muu = U = delta0 = delta1 = a0 = 0.0;
  ...
  G4double CoulombBarrier)
{
  ...
  delta0 = std::max(0.0, pPairCorr->GetPairingCorrection(fragA,fragZ));
  if(U < delta0) { return 0.0; }
  pMass = fragment.GetGroundStateMass() + U;
 
  resA  = fragA - theA;
  resZ  = fragZ - theZ;
  resA13 = pG4pow->Z13(resA);
  pResMass = G4NucleiProperties::GetNuclearMass(resA, resZ);
 
  delta1 = std::max(0.0, pPairCorr->GetPairingCorrection(resA,resZ));
  a0 = pNuclearLevelData->GetLevelDensity(fragZ,fragA,U);
  /*    
  G4cout << "G4EvaporationProbability: Z= " << theZ << " A= " << theA 
	 << " resZ= " << resZ << " resA= " << resA 
	 << " fragZ= " << fragZ << " fragA= " << fragA 
	 << "\n   U= " << U << " d0= " << delta0 << " d1= " << delta1 
	 << " a0= " << a0 << " OPT= " << OPTxs << " emin= " 
         << minEnergy << " emax= " << maxEnergy 
         << " CB= " << CoulombBarrier << G4endl;
  */
  if (OPTxs==0 || (OPTxs==4 && U < 10. + delta0)) {
    ...
    G4double a1 = pNuclearLevelData->GetLevelDensity(resZ,resA,U);
    ...
    pProbability = IntegrateProbability(minEnergy, maxEnergy, CoulombBarrier);
  }
  return pProbability;
}
...
{ 
  ...
  */
  ...
  //G4cout << " prob= " << prob << " pcoeff= " << pcoeff << " xs= " << xs << G4endl;
  return prob;
}
