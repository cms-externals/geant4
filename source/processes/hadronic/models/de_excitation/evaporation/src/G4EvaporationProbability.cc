...
G4double G4EvaporationProbability::TotalProbability(const G4Fragment& fragment, G4double minEnergy,
                                                    G4double maxEnergy, G4double CB,
                                                    G4double exEnergy)
{
  ...
  */
  if (OPTxs == 0)
  {
    ...
    const G4double RN2 =
      2.25 * CLHEP::fermi * CLHEP::fermi / (CLHEP::twopi * CLHEP::hbar_Planck * hbar_Planck);
...
G4double G4EvaporationProbability::CrossSection(G4double kine, G4double CB)
{
  G4double K = std::max(kine, kmin);
  // compute power once
  ...
  {
    recentXS = G4ChatterjeeCrossSection::ComputeCrossSection(K, CB, resA13, muu, index, theZ, resA);
  }
  else if (OPTxs == 3)
  {
    recentXS =
      G4KalbachCrossSection::ComputeCrossSection(K, CB, resA13, muu, index, theZ, theA, resA);
  }
  return recentXS;
}
