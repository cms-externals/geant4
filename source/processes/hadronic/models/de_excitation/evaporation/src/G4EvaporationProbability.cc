  ...
  : G4VEmissionProbability(aZ, anA), fGamma(aGamma)
{
  ...
  else if (2 == theZ) { index = theA + 1; }
  if (OPTxs == 1 && index >= 0) {
    const G4ParticleDefinition* part = nullptr;
    if (index == 0) { part = G4Neutron::Neutron(); }
    else if (index == 1) { part = G4Proton::Proton(); }
    ...
    if (nullptr != part) {
      fXSection = new G4XSectionXS(part, index);
    }
  }
...
G4EvaporationProbability::CrossSection(G4double K, G4double CB)
{
  ...
  if (OPTxs == 1) {
    recentXS = fXSection->GetXSection(K, resZ, resA)/CLHEP::millibarn;
