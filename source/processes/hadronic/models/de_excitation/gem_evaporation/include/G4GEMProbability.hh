...
#ifndef G4GEMPROBABILITY_HH
#define G4GEMPROBABILITY_HH
...
class G4GEMProbability : public G4VEmissionProbability
{
  ...
  private:
    ...
    void PrecomputeResidualQuantities(const G4Fragment& fragment, G4double& Ux, G4double& UxSqrt,
                                      G4double& UxLog) const;

    G4double CalcProbability(const G4Fragment& fragment, G4double MaximalKineticEnergy, G4double V,
                             G4double spin, G4double InitialLevelDensity, G4double Ux,
...
inline void G4GEMProbability::SetCoulomBarrier(const G4VCoulombBarrier* aCoulombBarrierStrategy)
{
  theCoulombBarrierPtr = aCoulombBarrierStrategy;
}

inline G4double G4GEMProbability::GetCoulombBarrier(const G4Fragment& fragment) const
{
  G4double res = 0.0;
...
inline G4double G4GEMProbability::CalcBetaParam(const G4Fragment& fragment) const
{
  // JMQ 190709 values according to Furihata's paper (based on notes added
  // on proof in Dostrovskii's paper)
  G4double res;
