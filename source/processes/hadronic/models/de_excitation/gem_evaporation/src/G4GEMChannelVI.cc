  ...
  : evapA(theA), evapZ(theZ)
{
  ...
  InitialiseIntegrator(0.005, 0.25, 1.05, de, 0.1*CLHEP::MeV, 2*CLHEP::MeV);
...
G4double G4GEMChannelVI::GetEmissionProbability(G4Fragment* fragment)
{
  ...
  if (indexC > 0) {
    bCoulomb = 0.5*cBarrier->GetCoulombBarrier(resA, resZ, 0.0);
  }
  ...
  if (2 < fVerbose) {
    G4cout << "## G4GEMChannelVI::GetEmissionProbability fragZ="
	   ...
	   << " nProbRes=" << nProbRes << G4endl;
  }

  // m1 is the mass of emitted excited fragment
  // e1, e2 - free energy in the decay
  G4double e1 = bCoulomb;
  G4double sump = 0.0;
  for (G4int i=0; i<nProbEvap; ++i) {
    ...
    e2 = std::max(e2, 0.0);
    G4double p = (e2 > e1) ? ComputeIntegral(e1, e2) : 0.0;
    sump += p;
    prob[i] = sump;
  }
  G4double f = (G4double)nProbEvap;
  return sump/f;
}
...
G4double G4GEMChannelVI::CrossSection(G4double e)
{
  if (indexC <= 6) {
    ...
    recentXS = R2*G4Log(1.0 + cInel*xs/R2)*(1. - bCoulomb/e)/cInel;
  }
  return recentXS;
}
...
G4Fragment* G4GEMChannelVI::EmittedFragment(G4Fragment* theNucleus)
{
  ...
  if (nProbEvap > 1) {
    G4double q = prob[nProbEvap - 1]*G4UniformRand();
    G4int i{1};
    for (; i < nProbEvap; ++i) { if (q <= prob[i]) { break; } }
    G4double de =
      fDeltaEvap*((i - 1) + (q - prob[i - 1])/(prob[i] - prob[i - 1]));
    fEvapExc = CorrectExcitation(de, lManagerEvap);
    e2 -= fEvapExc;
    e2 = std::max(e2, 0.0);
    if (ComputeIntegral(bCoulomb, e2) <= 0.0) { return evFragment; }
  }

  // sample excitation of the residual fragment
