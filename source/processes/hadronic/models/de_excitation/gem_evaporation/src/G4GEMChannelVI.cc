...
G4GEMChannelVI::G4GEMChannelVI(G4int theA, G4int theZ) : evapA(theA), evapZ(theZ)
{
  nData = G4NuclearLevelData::GetInstance();
  pairingCorrection = nData->GetPairingCorrection();
  if (evapZ > 2)
  {
    lManagerEvap = nData->GetLevelManager(evapZ, evapA);
  }
  ...
  if (indexC <= 6)
  {
    fXSection = new G4InterfaceToXS(part, indexC);
  }
}
...
G4double G4GEMChannelVI::GetEmissionProbability(G4Fragment* fragment)
{
  ...
  resZ = fragZ - evapZ;
  resA = fragA - evapA;
  // G4cout << "G4GEMChannelVI::GetEmissionProbability Z=" << evapZ <<  " A=" << evapA << " resZ="
  // << resZ << " resA=" << resA << G4endl; to avoid double counting
  if (resA < evapA || resA < resZ || resZ < 1 || (resA == evapA && resZ < evapZ))
  {
    return 0.0;
  }
  ...
  if (7 == indexC)
  {
    ...
    if (nProbEvap > 1)
    {
      fDeltaEvap /= (G4double)(nProbEvap - 1);
    }
  }

  if (2 < fVerbose)
  {
    G4cout << "## G4GEMChannelVI::GetEmissionProbability fragZ=" << fragZ << " fragA=" << fragA
           << " Z=" << evapZ << " A=" << evapA << " Eex(MeV)=" << fFragExc
           << " nProbEvap=" << nProbEvap << " nProbRes=" << nProbRes << " CB=" << bCoulomb
  ...
  for (G4int i = 0; i < nProbEvap; ++i)
  {
    ...
    if (2 < fVerbose)
    {
      G4cout << i << ". e1=" << elim << " e2=" << e2 << " e2-e1=" << e2 - elim
             << " fEvapExc=" << fEvapExc << " Probability=" << p << G4endl;
    }
  }
  if (nProbEvap > 1)
  {
...
G4double G4GEMChannelVI::ProbabilityDensityFunction(G4double e)
{
  ...
  G4double res = fCoeff * G4Exp(2.0 * (std::sqrt(a1 * fE1) - std::sqrt(a0 * fE0))) * e * xs;
...
G4Fragment* G4GEMChannelVI::EmittedFragment(G4Fragment* theNucleus)
{
  ...
  if (ComputeIntegral(0.5 * bCoulomb, e2) <= 0.0)
  {
    return evFragment;
  }
  ...
  G4LorentzVector lv(std::sqrt(ekin * (ekin + 2.0 * m1)) * G4RandomDirection(), ekin + m1);
...
G4double G4GEMChannelVI::CorrectExcitation(G4double exc, const G4LevelManager* man)
{
  if (exc <= 0.0 || nullptr == man)
  {
    return 0.0;
  }
