...
G4double G4GEMProbabilityVI::TotalProbability(const G4Fragment& fragment, const G4double tmin,
                                              const G4double tmax, const G4double CB,
                                              const G4double exEnergy, const G4double exEvap)
{
  ...
  G4double ekinmax = ((pMass - pResMass) * (pMass + pResMass) + evapMass2) / twoMass - pEvapMass;
  G4double ekinmin = std::max((CB * (twoMass - CB) + evapMass2) / twoMass - pEvapMass, 0.0);
  if (ekinmax <= ekinmin)
  {
    return 0.0;
  }
...
G4double G4GEMProbabilityVI::SampleEnergy(const G4double tmin, const G4double tmax,
