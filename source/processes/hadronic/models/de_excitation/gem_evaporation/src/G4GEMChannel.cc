...
G4double G4GEMChannel::GetEmissionProbability(G4Fragment* fragment)
{
  ...
  */
  ...
  if (ResidualA >= ResidualZ && ResidualZ >= 0 && ResidualA >= A)
  {
    ...
    G4double ExEnergy = fragment->GetExcitationEnergy() - fNucData->GetPairingCorrection(aZ, anA);
    if (ExEnergy > 0.0)
    {
      ...
      */
      if (Etot > ResidualMass + EvaporatedMass + CoulombBarrier)
      {
        ...
        MaximalKineticEnergy =
          ((Etot - ResidualMass) * (Etot + ResidualMass) + EvaporatedMass * EvaporatedMass)
            / (2.0 * Etot)
...
G4Fragment* G4GEMChannel::EmittedFragment(G4Fragment* theNucleus)
{
  ...
  G4ThreeVector momentum =
    G4RandomDirection() * std::sqrt((evEnergy - EvaporatedMass) * (evEnergy + EvaporatedMass));
...
// Samples fragment kinetic energy.
{
  ...
  G4double deltaCN = fNucData->GetPairingCorrection(fragment.GetZ_asInt(), fragment.GetA_asInt());
  ...
  if (U < ExCN)
  {
    G4double E0CN = ExCN
                    - TCN
                        * (G4Log(TCN / MeV) - G4Log(aCN * MeV) / 4.0 - 1.25 * G4Log(UxCN / MeV)
  ...
  for (G4int i = 0; i < 100; ++i)
  {
    ...
    G4double a = theLevelDensityPtr->LevelDensityParameter(ResidualA, ResidualZ, edelta);
    ...
    {
      G4double E0 =
        Ex - T * (G4Log(T) - G4Log(a) * 0.25 - 1.25 * G4Log(Ux) + 2.0 * std::sqrt(a * Ux));
      Probability *= G4Exp((theEnergy - KineticEnergy - E0) / T) / T;
    }
    else
    {
      G4double e2 = edelta * edelta;
      Probability *= G4Exp(2 * std::sqrt(a * edelta) - 0.25 * G4Log(a * edelta * e2 * e2));
    }
    if (EmissionProbability * G4UniformRand() <= Probability)
    {
      break;
    }
  }
