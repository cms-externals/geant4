...
G4GEMProbability::G4GEMProbability(G4int anA, G4int aZ, G4double aSpin)
  : G4VEmissionProbability(aZ, anA), Spin(aSpin), theCoulombBarrierPtr(nullptr)
{
                                               ...
                                               G4double MaximalKineticEnergy)
{
  ...
  if (MaximalKineticEnergy > 0.0 && fragment.GetExcitationEnergy() > 0.0)
  {
    ...
    probability = CalcProbability(fragment, MaximalKineticEnergy, CoulombBarrier, Spin,
...
G4double G4GEMProbability::ComputeInitialLevelDensity(const G4Fragment& fragment) const
{
  ...
  if (U < ExCN)
  {
    ...
    G4double E0CN = ExCN
                    - TCN
                        * (G4Log(TCN / MeV) - 0.25 * G4Log(aCN * MeV) - 1.25 * G4Log(UxCN / MeV)
...
void G4GEMProbability::PrecomputeResidualQuantities(const G4Fragment& fragment, G4double& Ux,
...
G4double G4GEMProbability::CalcProbability(const G4Fragment& fragment,
                                           G4double MaximalKineticEnergy, G4double V, G4double spin,
                                           G4double InitialLevelDensity, G4double Ux,
                                           G4double UxSqrt, G4double UxLog) const
...
// Calculate integrated probability (width) for evaporation channel
{
  ...
  G4double a =
    theEvapLDPptr->LevelDensityParameter(ResidualA, ResidualZ, MaximalKineticEnergy + V - delta0);
  G4double aSqrt = std::sqrt(a);
  G4double Ex = Ux + delta0;
  G4double T = 1.0 / (aSqrt / UxSqrt - 1.5 / Ux);
  // JMQ fixed bug in units
  G4double E0 =
    Ex - T * (G4Log(T / MeV) - G4Log(a * MeV) / 4.0 - 1.25 * UxLog + 2.0 * aSqrt * UxSqrt);
  //                      ***end RESIDUAL ***
...
void G4GEMProbability::Dump() const
{
  ...
  if (theA > 1)
  {
    efermi = G4NucleiProperties::GetNuclearMass(theA - 1, theZ) + neutron_mass_c2 - mass;
  }
  std::size_t nlev = ExcitEnergies.size();
  G4cout << "GEM: List of Excited States for Isotope Z= " << theZ << " A= " << theA
         << " Nlevels= " << nlev << " Efermi(MeV)= " << efermi << G4endl;
  for (std::size_t i = 0; i < nlev; ++i)
  {
    G4cout << "Z= " << theZ << " A= " << theA << " Mass(GeV)= " << mass / GeV
           << " Eexc(MeV)= " << ExcitEnergies[i] << " Time(ns)= " << ExcitLifetimes[i] / ns
           << G4endl;
  }
  G4cout << G4endl;
}
