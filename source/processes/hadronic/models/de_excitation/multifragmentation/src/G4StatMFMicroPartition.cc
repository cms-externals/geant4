...
{
  // throw G4HadronicException(__FILE__, __LINE__, "G4StatMFMicroPartition::operator== meant to not
  // be accessible");
  return false;
}
...
void G4StatMFMicroPartition::CoulombFreeEnergy(G4int anA)
{
  ...
  {
    // Z/A ~ 1/2
    _theCoulombFreeEnergy.push_back(CoulombConstFactor * 0.5 * anA
                                    * G4Pow::GetInstance()->Z23(anA));
  }
  else  // anA > 4
  {
    _theCoulombFreeEnergy.push_back(CoulombConstFactor * ZA * ZA * anA
                                    * G4Pow::GetInstance()->Z23(anA));
  }
}
...
G4double G4StatMFMicroPartition::GetCoulombEnergy(void)
{
  ...
  for (unsigned int i = 0; i < _thePartition.size(); i++)
    CoulombEnergy += _theCoulombFreeEnergy[i]
                     - elm_coupling * 0.6 * ZA * ZA * _thePartition[i]
...
G4double G4StatMFMicroPartition::GetPartitionEnergy(G4double T)
{
  ...
  for (unsigned int i = 0; i < _thePartition.size(); i++)
  {
    ...
    else
    {
      PartitionEnergy +=
        // Volume term
        (-G4StatMFParameters::GetE0() + T * T / InvLevelDensity(_thePartition[i]))
          * _thePartition[i]
        +

        // Symmetry term
        G4StatMFParameters::GetGamma0() * (1.0 - 2.0 * theZ / theA) * (1.0 - 2.0 * theZ / theA)
          * _thePartition[i]
        +
...
G4double G4StatMFMicroPartition::CalcPartitionProbability(G4double U, G4double FreeInternalE0,
                                                          G4double SCompound)
{
  ...
  G4double TranslationalS = G4Log(ProbA32 / Fact) + (n - 1) * G4Log(FreeVolume / ThermalWaveLenght3)
