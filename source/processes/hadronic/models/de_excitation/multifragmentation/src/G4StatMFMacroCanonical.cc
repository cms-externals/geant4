...
// Calculate total fragments multiplicity, fragment atomic numbers and charges
{
  ...
  for (i = 0; i < A; i++)
  {
    for (G4int j = 0; j < ANumbers[i]; j++)
      FragmentsA.push_back(i + 1);
  }
...
// Determines fragments multiplicities and compute total fragment multiplicity
{
  ...
  do
  {
    ...
    // Loop checking, 05-Aug-2015, Vladimir Ivanchenko
           ...
           || std::abs(pMeanMultiplicity - multiplicity) > std::sqrt(pMeanMultiplicity) + 0.5);
  ...
  do
  {
    ...
    for (G4int i = 0; i < multiplicity; ++i)
    {
      ...
      else
      {
        ...
        {
          ZMean = FragmentsA[i] * (4.0 * G4StatMFParameters::GetGamma0() + fChemPotentialNu) / CC;
        }
