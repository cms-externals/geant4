...
void G4StatMFMicroCanonical::Initialise(const G4Fragment& theFragment) 
{
  ...
  pFreeInternalE0 = A*(-G4StatMFParameters::GetE0() + fSymmetryTerm)  
...
G4double G4StatMFMicroCanonical::CalcFreeInternalEnergy(G4double T)
{
  ...
  G4double sum = VolumeTerm + fSymmetryTerm + fSurfaceTerm + fCoulombTerm;
  //G4cout << "G4StatMFMicroCanonical::CalcFreeInternalEnergy " << sum
  //	 << " " << VolumeTerm << " " << fSymmetryTerm << " " << fSurfaceTerm
  //	 <<  " " << fCoulombTerm << G4endl;
  return sum;
}
  ...
  // Calculates Temperature and Entropy of compound nucleus
{
  ...
  fSolver->FindRoot(T);
  TConf = T;
  return (2*A)*T/fInvLevelDensity - G4StatMFParameters::DBetaDT(T)*A13*A13;
}
