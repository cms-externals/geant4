...
void G4StatMFMacroMultiplicity::Initialise(const G4int anA, const G4double kappa,
...
// For that is necesary to calculate mean multiplicities
{
  ...
  fChemPotentialMu = -G4StatMFParameters::GetE0() - fMeanTemperature * fMeanTemperature / ILD5
                     ...
                     + (5.0 / 3.0) * CP * ZA5 * ZA5 * g4calc->Z23(5) - 1.5 * fMeanTemperature / 5.0;

  G4double ChemPa = fChemPotentialMu;
  if (ChemPa > 10 * fMeanTemperature)
  {
    ChemPa = 10 * fMeanTemperature;
  }
...
G4double G4StatMFMacroMultiplicity::CalcMeanA(const G4double mu)
{
  ...
  {
    G4double multip =
      (*fClusters)[i]->CalcMeanMultiplicity(V0 * fKappa, mu, fChemPotentialNu, fMeanTemperature);
    MeanA += multip * (++n);
