...
{
  throw G4HadronicException(
    __FILE__, __LINE__, "G4StatMFMacroMultiNucleon::copy_constructor meant to not be accessible");
}
                                                         ...
                                                         const G4double nu, const G4double T)
{
  ...
  G4double exponent =
    (mu + nu * theZARatio + G4StatMFParameters::GetE0() + T * T / _InvLevelDensity
  ...
  _MeanMultiplicity =
    std::max((FreeVol * theA * std::sqrt((G4double)theA) / lambda3) * G4Exp(exponent), 1.0e-30);
...
G4double G4StatMFMacroMultiNucleon::CalcEnergy(const G4double T)
{
  ...
  // Symmetry term
  G4double ESym =
    theA * G4StatMFParameters::GetGamma0() * (1. - 2. * theZARatio) * (1. - 2. * theZARatio);
...
G4double G4StatMFMacroMultiNucleon::CalcEntropy(const G4double T, const G4double FreeVol)
{
  ...
  if (_MeanMultiplicity > 0.0)
  {
    ...
    // Translational term
    G4double ST =
      2.5 + G4Log(FreeVol * std::sqrt((G4double)theA) * theA / (lambda3 * _MeanMultiplicity));
