...
{
  G4double Coulomb =
    std::accumulate(_theFragments.begin(), _theFragments.end(), 0.0,
...
// idx is index of first fragment
{
  ...
  else
  {
    ...
    while (
      AvailableE
      <= p.mag2()
           / (2.0 * (_theFragments[i1]->GetNuclearMass() + _theFragments[i2]->GetNuclearMass())));
    ...
    G4double P1 =
      (p.mag() * CosTheta1 + Sign * std::sqrt(p.mag2() * (CosTheta1 * CosTheta1 - CTM12))) / H;
...
// for fragments in the self-consistent time-dependent Coulomb field
{
  ...
  G4double CoulombEnergy = 0.6 * CLHEP::elm_coupling * anZ * anZ
                             * g4calc->A13(1.0 + G4StatMFParameters::GetKappaCoulomb())
                             / (G4StatMFParameters::Getr0() * g4calc->Z13(anA))
