...
G4ExcitationHandler::BreakItUp(const G4Fragment & theInitialState)
{
  ...
  if(0 < nL) {
	...
	const G4ParticleDefinition* part = thePartTable->FindParticle(pdg);
	  ...
	  G4ThreeVector dir = lambdaLV.vect().unit();
    ...
    theInitialStatePtr->SetZAandMomentum(lambdaLV*(1. - lambdaF), Z, A);
  ...
  for (auto & frag : theResults) {
    ...
    if(nullptr != theKindOfFragment) {
      ...
      if(isHyperN) {
        G4LorentzVector lv = lv0 + lambdaLV/(G4double)nL;
  ...
  // remaining lambda are free; conserve quantum numbers but
  // not 4-momentum
  if(0 < nL) {
    G4ThreeVector dir = lambdaLV.vect().unit();
