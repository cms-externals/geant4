...
G4ExcitationHandler::G4ExcitationHandler() : minExcitation(1. * CLHEP::eV)
{
...
void G4ExcitationHandler::SetParameters()
{
  ...
  else
  {
    ...
    {
      Zmax = std::max(Zmax, elm->GetZasInt());
    }
...
void G4ExcitationHandler::SetPhotonEvaporation(G4VEvaporationChannel* ptr)
{
  if (!isInitialised && nullptr != ptr && ptr != thePhotonEvaporation)
  {
    ...
    if (fVerbose > 1)
    {
      G4cout << "G4ExcitationHandler::SetPhotonEvaporation() " << ptr << " for handler " << this
             << G4endl;
    }
  }
}
...
void G4ExcitationHandler::SetDeexChannelsType(G4DeexChannelType val)
{
  ...
  if (fVerbose > 1)
  {
    G4cout << "G4ExcitationHandler::SetDeexChannelsType " << val << " for " << this << G4endl;
  }
  if (val == fDummy)
  {
    isActive = false;
...
G4ReactionProductVector* G4ExcitationHandler::BreakItUp(const G4Fragment& theInitialState)
{
  ...
  if (fVerbose > 2)
  {
    G4cout << "## After first step of handler " << theEvapList.size() << " for evap;  "
  ...
  for (kk = 0; kk < theEvapList.size(); ++kk)
  {
    ...
    {
      G4ExceptionDescription ed;
      ed << "Infinite loop in the de-excitation module: " << kk << " iterations \n"
         << "      Initial fragment: \n"
         << theInitialState << "\n      Current fragment: \n"
         << *frag;
      G4Exception("G4ExcitationHandler::BreakItUp", "had0333", FatalException, ed,
                  "Stop execution");
    }
    A = frag->GetA_asInt();
    Z = frag->GetZ_asInt();
    ...
    if (fVerbose > 3)
    {
      G4cout << kk << ". Evaporation: Nsec=" << results.size() << " Z=" << frag->GetZ_asInt()
             << " A=" << frag->GetA_asInt() << " Eex=" << frag->GetExcitationEnergy()
             << " stable=" << frag->IsLongLived() << G4endl;
    }
    if (0 == results.size())
    {
  ...
  if (fVerbose > 2)
  {
    G4cout << "## After 2nd step of handler " << theEvapList.size() << " was evap;  "
           << theResults.size() << " results. " << G4endl;
  }
  ...
  for (auto const& frag : theResults)
  {
    ...
    if (!isActive)
    {
      ...
      G4double fac =
        (etot <= mass || 0.0 == ptot) ? 0.0 : std::sqrt((etot - mass) * (etot + mass)) / ptot;
      G4LorentzVector lv((frag->GetMomentum()).px() * fac, (frag->GetMomentum()).py() * fac,
    ...
    else
    {
      ...
      theKindOfFragment =
        theTableOfIons->GetIon(fragmentZ, fragmentA, eexc, G4Ions::FloatLevelBase(idxf));
      if (fVerbose > 3)
      {
        G4cout << "### EXCH: Find ion Z= " << fragmentZ << " A= " << fragmentA
               << " Eexc(MeV)= " << eexc / MeV << " idx= " << idxf << " "
               << theKindOfFragment->GetParticleName() << G4endl;
      }
    }
    // fragment identified
    ...
    {
      theKindOfFragment = theTableOfIons->GetIon(fragmentZ, fragmentA, 0.0, noFloat, 0);
      if (theKindOfFragment)
      {
        ...
        if (fVerbose > 3)
        {
          G4cout << "          ground state, energy corrected E(MeV)= " << etot << G4endl;
        }
      }
    }
    delete frag;
