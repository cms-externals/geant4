...
G4FragmentVector* G4WilsonAblationModel::BreakItUp(const G4Fragment& theNucleus)
{
  ...
  if (verboseLevel >= 2)
  {
    G4cout << "oooooooooooooooooooooooooooooooooooooooo"
           << "oooooooooooooooooooooooooooooooooooooooo" << G4endl;
    G4cout.precision(6);
    G4cout << "IN G4WilsonAblationModel" << G4endl;
    G4cout << "Initial prefragment A=" << A << ", Z=" << Z << ", excitation energy = " << ex / MeV
           << " MeV" << G4endl;
  }
  //
  //
  ...
  else if (A == 1)
  {
    ...
    if (Z == 0)
    {
      ...
      if (fragment != nullptr)
      {
        fragment->SetCreatorModelID(secID);
      }
  ...
  if (AF > 0)
    massFinalFrag = G4ParticleTable::GetParticleTable()->GetIonTable()->GetIonMass(ZF, AF);
  else
  {
  ...
  if (verboseLevel >= 2)
  {
    G4cout << "Final fragment      A=" << AF << ", Z=" << ZF << G4endl;
    for (G4int ift = 0; ift < nFragTypes; ift++)
    {
      G4ParticleDefinition* type = fragType[ift];
  ...
  if (produceSecondaries && evapType.size() > 0)
  {
    if (excess > 0.0)
    {
      ...
      if (evapType.size() > 0) SelectSecondariesByDefault(boost);
    }
    else
  ...
  if (AF > 0)
  {
    G4double mass = G4ParticleTable::GetParticleTable()->GetIonTable()->GetIonMass(ZF, AF);
  ...
  if (verboseLevel >= 2)
  {
    ...
    std::size_t ie = 0;
    for (auto iter = fragmentVector->cbegin(); iter != fragmentVector->cend(); ++iter)
    {
      if (ie == nEvap)
      {
        //        G4cout <<*iter  <<G4endl;
...
void G4WilsonAblationModel::SelectSecondariesByEvaporation(G4Fragment* intermediateNucleus)
{
  ...
  while (evaporate && evapType.size() != 0)
  {
    ...
    for (auto iterEv = theChannels1.cbegin(); iterEv != theChannels1.cend(); ++iterEv)
    {
    ...
    if (totalProb > 0.0)
    {
      ...
      G4FragmentVector* evaporationResult = theChannels1[ii]->BreakUpFragment(intermediateNucleus);
...
void G4WilsonAblationModel::PrintWelcomeMessage()
{
  G4cout << G4endl;
  G4cout << " *****************************************************************" << G4endl;
  G4cout << " Nuclear ablation model for nuclear-nuclear interactions activated" << G4endl;
  G4cout << " (Written by QinetiQ Ltd for the European Space Agency)" << G4endl;
  G4cout << " !!! WARNING: This model is not well validation and should not be used for accurate "
            "simulation !!!"
         << G4endl;
  G4cout << " *****************************************************************" << G4endl;
  G4cout << G4endl;
