...
{
  SPtr = G4NuclearLevelData::GetInstance()
           ->GetShellCorrection()
...
G4double G4FissionBarrier::FissionBarrier(G4int A, G4int Z, G4double U) const
// Compute fission barrier according with Barashenkov's
// prescription for A >= 65
{
  static const G4double blimit = 100.0 * CLHEP::GeV;
  return (A >= 65) ? BarashenkovFissionBarrier(A, Z) / (1.0 + std::sqrt(U / (G4double)(2 * A)))
                   : blimit;
}

G4double G4FissionBarrier::BarashenkovFissionBarrier(G4int A, G4int Z) const
