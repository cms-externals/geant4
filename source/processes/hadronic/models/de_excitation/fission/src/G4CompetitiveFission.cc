...
G4double G4CompetitiveFission::GetEmissionProbability(G4Fragment* fragment)
{
  ...
  {
    G4double exEnergy =
      fragment->GetExcitationEnergy() - pairingCorrection->GetFissionPairingCorrection(A, Z);
    ...
    if (exEnergy > 0.0)
    {
      ...
      fissionProbability =
        theFissionProbabilityPtr->EmissionProbability(*fragment, maxKineticEnergy);
    }
  }
...
G4Fragment* G4CompetitiveFission::EmittedFragment(G4Fragment* theNucleus)
{
  ...
  do
  {
    ...
    FragmentsKineticEnergy = FissionKineticEnergy(A, Z, A1, Z1, A2, Z2, U, Tmax);
    ...
    FragmentsExcitationEnergy = Tmax - FragmentsKineticEnergy + pcorr;
  ...
  G4ParticleMomentum Momentum1 = std::sqrt((etot1 - M1) * (etot1 + M1)) * G4RandomDirection();
  ...
  if (Fragment1 != nullptr)
  {
    Fragment1->SetCreatorModelID(theSecID);
  }
...
G4double G4CompetitiveFission::MassDistribution(G4double x, G4int A)
...
// which consist of symmetric and asymmetric sum of gaussians components.
{
  ...
  G4double Xasym = LocalExp(y1) + LocalExp(y2) + 0.5 * (LocalExp(z1) + LocalExp(z2));
...
// Calculates the charge of a fission product for a given atomic number Af
{
  ...
  {
    DeltaZ = -0.45 * (Af - A * 0.5) / (134.0 - A * 0.5);
  }
...
G4double G4CompetitiveFission::FissionKineticEnergy(G4int A, G4int Z, G4int Af1, G4int /*Zf1*/,
                                                    G4int Af2, G4int /*Zf2*/, G4double /*U*/,
                                                    G4double Tmax)
// Gives the kinetic energy of fission products
{
  ...
  {  // Asymmetric Mode
    ...
    G4double ScaleFactor =
      0.5 * theParam.GetSigma1() * (AsymmetricRatio(A, A11) + AsymmetricRatio(A, A12))
      + theParam.GetSigma2() * (AsymmetricRatio(A, A21) + AsymmetricRatio(A, A22));
    // Compute average kinetic energy for fragment with AfMax
    TaverageAfMax =
      (Eaverage + 12.5 * Xsy) * (PPas / ScaleFactor) * AsymmetricRatio(A, G4double(AfMax));
  }
  else
  {  // Symmetric Mode
    G4double As0 = theParam.GetAs() + 0.7979 * theParam.GetSigmaS();
    // Compute average kinetic energy for fragment with AfMax
    TaverageAfMax = (Eaverage - 12.5 * CLHEP::MeV * Xas) * SymmetricRatio(A, G4double(AfMax))
                    / SymmetricRatio(A, As0);
  ...
  do
  {
    ...
    // Loop checking, 05-Aug-2015, Vladimir Ivanchenko
  } while (KineticEnergy < Eaverage - 3.72 * ESigma || KineticEnergy > Eaverage + 3.72 * ESigma
           || KineticEnergy > Tmax);
