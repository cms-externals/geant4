...
G4double G4PolarizationTransition::FCoefficient(G4int K, G4int LL, G4int Lprime, G4int twoJ2,
                                                G4int twoJ1) const
{
...
G4double G4PolarizationTransition::F3Coefficient(G4int K, G4int K2, G4int K1, G4int LL,
                                                 G4int Lprime, G4int twoJ2, G4int twoJ1) const
{
  G4double fCoeff = G4Clebsch::Wigner3J(2 * LL, 2, 2 * Lprime, -2, 2 * K, 0);
  if (fCoeff == 0) return 0;
  fCoeff *=
    G4Clebsch::Wigner9J(twoJ2, 2 * LL, twoJ1, twoJ2, 2 * Lprime, twoJ1, 2 * K2, 2 * K, 2 * K1);
  if (fCoeff == 0) return 0;
  ...
  return fCoeff
         * std::sqrt(G4double((twoJ1 + 1) * (twoJ2 + 1) * (2 * LL + 1))
...
G4double G4PolarizationTransition::GammaTransF3Coefficient(G4int K, G4int K2, G4int K1) const
{
...
G4double G4PolarizationTransition::GenerateGammaCosTheta(const POLAR& pol)
{
  ...
  for (G4int k = 0; k < (G4int)length; k += 2)
  {
    if ((pol[k]).size() > 0)
    {
      if (fVerbose > 1 && std::abs(((pol)[k])[0].imag()) > kEps)
      {
        G4cout << "G4PolarizationTransition::GenerateGammaCosTheta WARNING: \n"
               << "          fPolarization[" << k
               << "][0] has imag component: = " << ((pol)[k])[0].real() << " + "
               << ((pol)[k])[0].imag() << "*i" << G4endl;
      }
      G4double a_k =
        std::sqrt((G4double)(2 * k + 1)) * GammaTransFCoefficient(k) * ((pol)[k])[0].real();
    ...
    {
      G4cout << "G4PolarizationTransition::GenerateGammaCosTheta: WARNING: \n"
             << " size of pol[" << k << "] = " << (pol[k]).size() << " returning isotropic "
             << G4endl;
...
G4double G4PolarizationTransition::GenerateGammaPhi(G4double& cosTheta, const POLAR& pol)
{
  ...
  for (G4int kappa = 0; kappa < length; ++kappa)
  {
    ...
    for (G4int k = kappa + (kappa % 2); k < length; k += 2)
    {
      ...
      if (kmax > 0)
      {
        if (kappa >= kmax || std::abs(((pol)[k])[kappa]) < kEps)
        {
          continue;
        }
        ...
        if (kappa > 0)
          tmpAmp *= 2. * G4Exp(0.5 * (LnFactorial(k - kappa) - LnFactorial(k + kappa)));
  ...
  if (fVerbose > 1 && pdfMax < kEps)
  {
    G4cout << "G4PolarizationTransition::GenerateGammaPhi: WARNING "
           << "got pdfMax = 0 for \n";
    DumpTransitionData(pol);
    G4cout << "I suspect a non-allowed transition! Returning isotropic phi..." << G4endl;
    return G4UniformRand() * CLHEP::twopi;
  }
  ...
  for (std::size_t i = 0; i < 100; ++i)
  {
    ...
    {
      G4cout << "G4PolarizationTransition::GenerateGammaPhi: WARNING: \n"
             << "got pdfSum (" << pdfSum << ") > pdfMax (" << pdfMax << ") at phi = " << phi
             << G4endl;
    }
    if (prob <= pdfSum)
    {
...
void G4PolarizationTransition::SampleGammaTransition(G4NuclearPolarization* nucpol, G4int twoJ1,
                                                     G4int twoJ2, G4int L0, G4int Lp,
                                                     G4double mpRatio, G4double& cosTheta,
                                                     G4double& phi)
{
  if (nucpol == nullptr)
  {
  ...
  else
  {
    ...
    if (fVerbose > 2)
    {
      G4cout << "G4PolarizationTransition::SampleGammaTransition: cosTheta= " << cosTheta
             << " phi= " << phi << G4endl;
    }
  }
      ...
      for (G4int k = 0; k <= k1 + k2; k += 2)
      {
        ...
        for (G4int kappa2 = 0; kappa2 <= k2; ++kappa2)
        {
          ...
          for (G4int kappa1 = 1 - ll; kappa1 < ll; ++kappa1)
          {
            ...
            G4complex tmpAmp =
              (kappa1 < 0) ? conj((pol[k1])[-kappa1]) * (kappa1 % 2 ? -1. : 1.) : (pol[k1])[kappa1];
            if (std::abs(tmpAmp) < kEps) continue;
            G4int kappa = kappa1 - (G4int)kappa2;
            tmpAmp *=
              G4Clebsch::Wigner3J(2 * k1, -2 * kappa1, 2 * k, 2 * kappa, 2 * k2, 2 * kappa2);
            if (std::abs(tmpAmp) < kEps) continue;
            if (recalcTF3)
            {
            ...
            //        << " ; k2=" << k2 << " ; kappa1=" << kappa1 << " ; kappa2=" << kappa2 <<
            //        G4endl;
            tmpAmp *= fgLegendrePolys.EvalAssocLegendrePoly(k, kappa, cosTheta);
            if (kappa != 0)
            {
              tmpAmp *=
                G4Exp(0.5 * (LnFactorial(((G4int)k) - kappa) - LnFactorial(((G4int)k) + kappa)));
  ...
  for (std::size_t k2 = 0; k2 < newlength; ++k2)
  {
    ...
    while ((newPol[k2]).size() != std::size_t(lastNonZero + 1))
      (newPol[k2]).pop_back();
    if ((newPol[k2]).size() > 0) lastNonEmptyK2 = k2;
  }

  // Remove zero-value entries
  while (newPol.size() != lastNonEmptyK2 + 1)
  {
    newPol.pop_back();
  }
  (newPol[0])[0] = 1.0;
