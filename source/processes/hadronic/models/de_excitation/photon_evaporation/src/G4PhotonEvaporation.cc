...
G4Fragment* G4PhotonEvaporation::EmittedFragment(G4Fragment* nucleus)
{
  ...
  if (fCorrelatedGamma && fRDM)
  {
    ...
    fPolarization = fNucPStore->FindOrBuild(nucleus->GetZ_asInt(), nucleus->GetA_asInt(),
                                            nucleus->GetExcitationEnergy());
    nucleus->SetNuclearPolarization(fPolarization);
  }
  if (fVerbose > 2)
  {
    G4cout << "G4PhotonEvaporation::EmittedFragment: " << *nucleus << G4endl;
  ...
  if (nullptr != fNucPStore && nullptr != fPolarization && 0 == fIndex)
  {
    if (fVerbose > 3)
    {
      G4cout << "G4PhotonEvaporation::EmittedFragment: remove " << fPolarization << G4endl;
    }
  ...
  if (fVerbose > 2)
  {
    G4cout << "G4PhotonEvaporation::EmittedFragment: RDM= " << fRDM << " done:" << G4endl;
    if (gamma)
    {
      G4cout << *gamma << G4endl;
    }
...
G4bool G4PhotonEvaporation::BreakUpChain(G4FragmentVector* products, G4Fragment* nucleus)
{
  if (!isInitialised)
  {
    Initialise();
  ...
  if (fVerbose > 1)
  {
    G4cout << "G4PhotonEvaporation::BreakUpChain RDM= " << fRDM << " " << *nucleus << G4endl;
  }
  ...
  if (fCorrelatedGamma)
  {
    fPolarization = new G4NuclearPolarization(nucleus->GetZ_asInt(), nucleus->GetA_asInt(),
                                              nucleus->GetExcitationEnergy());
    nucleus->SetNuclearPolarization(fPolarization);
  }
...
G4double G4PhotonEvaporation::GetEmissionProbability(G4Fragment* nucleus)
{
  if (!isInitialised)
  {
    Initialise();
  ...
  if (fVerbose > 2)
  {
    G4cout << "G4PhotonEvaporation::GetEmissionProbability: Z=" << Z << " A=" << A
           << " Eexc(MeV)= " << fExcEnergy << G4endl;
  }
  ...
  if (fVerbose > 2)
    G4cout << "   GREnergy=" << GREnergy[A] << " GRWidth=" << GRWidth[A] << " Edelta=" << edelta
           << G4endl;
  ...
  if (fVerbose > 2)
  {
    G4cout << "  Npoints= " << fPoints << "  Eex=" << fExcEnergy << " Estep=" << fStep << G4endl;
  }
  ...
  for (G4int i = 1; i < fPoints; ++i)
  {
    ...
    else
    {
      ...
      p1 = egam * G4Exp(2.0 * (std::sqrt(levelDensity * e))) * gammaR2 * gammaE2
           / (egdp2 * egdp2 + gammaR2);
    }
    fProbability += (p1 + p0);
    ...
    if (fVerbose > 3)
    {
      G4cout << "Egamma= " << egam << "  Eex= " << fExcEnergy << "  p0= " << p0 << " p1= " << p1
             << " sum= " << fCummProbability[i] << G4endl;
    }
    p0 = p1;
  }

  static const G4double NormC =
    1.25 * CLHEP::millibarn / (CLHEP::pi2 * CLHEP::hbarc * CLHEP::hbarc);
...
G4double G4PhotonEvaporation::ComputeInverseXSection(G4Fragment*, G4double)
{
  return 0.0;
}

G4double G4PhotonEvaporation::ComputeProbability(G4Fragment* theNucleus, G4double)
{
...
G4double G4PhotonEvaporation::GetFinalLevelEnergy(G4int Z, G4int A, G4double energy)
{
  ...
  if (nullptr != fLevelManager)
  {
    ...
    if (E > fLevelEnergyMax + fTolerance)
    {
      E = energy;
    }
  }
...
G4Fragment* G4PhotonEvaporation::GenerateGamma(G4Fragment* nucleus)
{
  ...
  else if (nullptr != fLevelManager && eexc <= fLevelEnergyMax + fTolerance)
  {
    ...
    if (fVerbose > 2)
    {
      G4cout << "          Level index=" << fIndex << " lTime=" << fLevelManager->LifeTime(fIndex)
             << " Elevel=" << elevel << " isDiscrete:" << isDiscrete << G4endl;
    }
    if (isDiscrete && 0 < fIndex)
    {
      ...
      if (nullptr != level)
      {
        ...
        if (fVerbose > 2)
        {
          G4cout << "          ntrans= " << ntrans << " JP= " << JP1 << " RDM: " << fRDM << G4endl;
        }
      }
  ...
  if (fVerbose > 2)
  {
    ...
    G4cout << "   Z=" << nucleus->GetZ_asInt() << " A=" << nucleus->GetA_asInt() << " Exc=" << eexc
           << " Emax=" << fLevelEnergyMax << " idx=" << fIndex << " fPoints= " << fPoints
           << " Ntr=" << ntrans << " discrete:" << isDiscrete << G4endl;
  ...
  if (!isDiscrete)
  {
    ...
    else
    {
      ...
      for (G4int i = 1; i < fPoints; ++i)
      {
        if (fVerbose > 3)
        {
          G4cout << "y= " << y << " cummProb= " << fCummProbability[i] << " fPoints= " << fPoints
                 << " fStep= " << fStep << G4endl;
        }
        if (y <= fCummProbability[i])
        {
          efinal =
            fStep
            * ((i - 1)
               + (y - fCummProbability[i - 1]) / (fCummProbability[i] - fCummProbability[i - 1]));
          break;
        }
      }
    }
    ...
    if (fVerbose > 2)
    {
      G4cout << "Continues proposes Efinal=" << efinal << " Initial Idx=" << fIndex << G4endl;
    }
    ...
    if (fVerbose > 2)
    {
      G4cout << "Continues emission efinal(MeV)= " << efinal << " idxFinal=" << fIndex
  ...
  else
  {
    if (fVerbose > 2)
    {
      G4cout << "Discrete emission from level Index=" << fIndex
             << " Elevel=" << fLevelManager->LevelEnergy(fIndex)
             << " Ltime=" << fLevelManager->LifeTime(fIndex) << " LtimeMax=" << fLocalTimeLimit
    ...
    if (fVerbose > 2)
    {
      G4cout << "Ntrans= " << ntrans << " idx= " << idx << " ICM= " << fICM << "  abs(JP1)= " << JP1
             << G4endl;
    }
  ...
  {
    result = fTransition->SampleTransition(nucleus, efinal, ratio, JP1, std::abs(JP2), multiP,
                                           vShellNumber, isDiscrete, isGamma);
    if (nullptr != result)
    {
      result->SetCreationTime(time);
    }
  }
  ...
  if (fVerbose > 2)
  {
    ...
    G4cout << "   " << ss << " Efinal=" << efinal << " Efrag=" << nucleus->GetExcitationEnergy()
           << " lt=" << ltime << " idxFin=" << fIndex << " isDiscrete:" << isDiscrete
           << " isGamma:" << isGamma << " isStable:" << isLL << " multiP=" << multiP
           << " shell=" << vShellNumber << " abs(JP1)= " << JP1 << " abs(JP2)= " << JP2 << G4endl;
  }
  return result;
}
