...
G4ANuTauNucleusCcModel::G4ANuTauNucleusCcModel(const G4String& name) : G4NeutrinoNucleusModel(name)
{
  fData = fMaster = false;
  fMtau = 1.77686 * GeV;
...
G4bool G4ANuTauNucleusCcModel::IsApplicable(const G4HadProjectile& aPart, G4Nucleus&)
{
  G4bool result = false;
...
G4HadFinalState* G4ANuTauNucleusCcModel::ApplyYourself(const G4HadProjectile& aTrack,
...
*/
  ...
  else  // if ( eX < 9500000.*GeV ) // <  25.*GeV) // < 95.*GeV ) // < 2.5*GeV ) //cluster decay
  {
    ...
    else if (!fProton && pName == "anti_nu_tau")
      qB = 1;
