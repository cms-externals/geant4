                                                           ...
                                                           G4Nucleus& targetNucleus)
{
  ...
  // Note that the muon is treated as if it were flying along the z-axis :
  ...
  G4ThreeVector finalDirection(std::sin(theta)*std::cos(phi), std::sin(theta)*std::sin(phi), std::cos(theta));
...
void G4MuonVDNuclearModel::CalculateHadronicVertex(G4DynamicParticle* incident, G4Nucleus& target)
{
  ...
  if ( hfs != nullptr ) {
    ...
    for ( size_t i = 0; i < hfs->GetNumberOfSecondaries(); ++i ) {
      ...
      // because the muon (that emitted the virtual gamma from which the
      // real-gamma/pi0 derives from) was already assumed to be along the
      // z-axis by the method G4MuonVDNuclearModel::ApplyYourself.
