...
const G4double G4NeutrinoNucleusModel::fNuMuEnergyLogVector[50] = {
  115.603, 133.424, 153.991, 177.729, 205.126, 236.746, 273.24,  315.361, 363.973, 420.08,
  484.836, 559.573, 645.832, 745.387, 860.289, 992.903, 1145.96, 1322.61, 1526.49, 1761.8,
  2033.38, 2346.83, 2708.59, 3126.12, 3608.02, 4164.19, 4806.1,  5546.97, 6402.04, 7388.91,
  8527.92, 9842.5,  11359.7, 13110.8, 15131.9, 17464.5, 20156.6, 23263.8, 26849.9, 30988.8,
  35765.7, 41279,   47642.2, 54986.3, 63462.4, 73245.2, 84536,   97567.2, 112607,  129966};
...
G4bool G4NeutrinoNucleusModel::IsApplicable(const G4HadProjectile& aPart, G4Nucleus&)
{
  G4bool result = false;
...
G4double G4NeutrinoNucleusModel::SampleXkr(G4double energy)
{
  ...
  else
  {
    ...
    if (e2 <= e1)
      xx = x1 + G4UniformRand() * (x2 - x1);
    else
      xx = x1 + (e - e1) * (x2 - x1) / (e2 - e1);  // lin in energy log-scale
  }
  return xx;
}
...
G4double G4NeutrinoNucleusModel::GetXkr(G4int iEnergy, G4double prob)
{
  ...
  for (i = 0; i < nBin; ++i)
  {
    if (prob <= fNuMuXdistrKR[iEnergy][i]) break;
  }
  if (i <= 0)  // X-edge
  {
    fXindex = 0;
...
G4double G4NeutrinoNucleusModel::SampleQkr(G4double energy, G4double xx)
{
  ...
  else
  {
    ...
    if (e2 <= e1)
      qq1 = q1 + G4UniformRand() * (q2 - q1);
    else
      qq1 = q1 + (e - e1) * (q2 - q1) / (e2 - e1);  // lin in energy log-scale
  }
  ...
  else
  {
    ...
    if (e2 <= e1)
      qq2 = q1 + G4UniformRand() * (q2 - q1);
    else
      qq2 = q1 + (e - e1) * (q2 - q1) / (e2 - e1);  // lin in energy log-scale
  }
  qq = 0.5 * (qq1 + qq2);
...
G4double G4NeutrinoNucleusModel::GetQkr(G4int iE, G4int jX, G4double prob)
{
  ...
  for (i = 0; i < nBin; ++i)
  {
    if (prob <= fNuMuQdistrKR[iE][jX][i]) break;
  }
  if (i <= 0)  // Q-edge
  {
    fQindex = 0;
...
void G4NeutrinoNucleusModel::FinalMeson(G4LorentzVector& lvM, G4int, G4int pdgM)  // qM
{
  ...
  else  // meson resonances
  {
    ...
    {
      G4DynamicParticle* aNew = new G4DynamicParticle(ddktv->operator[](i)->GetDefinition(),
                                                      ddktv->operator[](i)->Get4Momentum());

      // G4cout<<"       "<<i<<", "<<aNew->GetDefinition()->GetParticleName()<<",
      // "<<aNew->Get4Momentum()<<G4endl;
...
///////////////////////////////////////////////////////
//
// Get final particles from excited recoil nucleus and write them to theParticleChange, delete the
// particle vector
...
void G4NeutrinoNucleusModel::CoherentPion(G4LorentzVector& lvP, G4int pdgP,
...
G4double G4NeutrinoNucleusModel::FinalMomentum(G4double mI, G4double mF, G4double mP,
                                               G4LorentzVector lvX)
{
...
G4double G4NeutrinoNucleusModel::GetNuMuQeTotRat(G4int index, G4double energy)
{
  G4double ratio(0.);
  // GetMinNuMuEnergy()
  if (index <= 0 || energy < fNuMuEnergy[0])
    ratio = 0.;
  else if (index >= fIndex)
    ratio = fNuMuQeTotRat[fIndex - 1] * fOnePionEnergy[fIndex - 1] * GeV / energy;
  else
  {
...
const G4double G4NeutrinoNucleusModel::fNuMuEnergy[50] = {
  0.112103, 0.117359, 0.123119, 0.129443, 0.136404, 0.144084, 0.152576, 0.161991, 0.172458,
  0.184126, 0.197171, 0.211801, 0.228261, 0.24684,  0.267887, 0.291816, 0.319125, 0.350417,
  0.386422, 0.428032, 0.47634,  0.532692, 0.598756, 0.676612, 0.768868, 0.878812, 1.01062,
  1.16963,  1.36271,  1.59876,  1.88943,  2.25002,  2.70086,  3.26916,  3.99166,  4.91843,
  6.11836,  7.6872,   9.75942,  12.5259,  16.2605,  21.3615,  28.4141,  38.2903,  52.3062,
...
const G4double G4NeutrinoNucleusModel::fNuMuQeTotRat[50] = {
  ...
  0.98,       0.98,      0.98,      0.98,      0.98,      0.98,      0.98,      0.98,
  0.98,       0.98,      0.98,      0.98,      0.97,      0.96,      0.95,      0.93,
  0.917794,   0.850239,  0.780412,  0.709339,  0.638134,  0.568165,  0.500236,  0.435528,
  0.375015,   0.319157,  0.268463,  0.2232,    0.183284,  0.148627,  0.119008,  0.0940699,
  0.0733255,  0.0563819, 0.0427312, 0.0319274, 0.0235026, 0.0170486, 0.0122149, 0.00857825,
  0.00594018, 0.00405037};
...
G4double G4NeutrinoNucleusModel::GetNuMuOnePionProb(G4int index, G4double energy)
{
  ...
  if (index <= 0 || energy < fOnePionEnergy[0])
    ratio = 0.;
  else if (index >= fOnePionIndex)
    ratio = fOnePionProb[fOnePionIndex - 1] * fOnePionEnergy[fOnePionIndex - 1] * GeV / energy;
  else
  {
...
const G4double G4NeutrinoNucleusModel::fOnePionEnergy[58] = {
  ...
  0.275314, 0.293652, 0.31729,  0.33409,  0.351746, 0.365629, 0.380041, 0.400165, 0.437941,
  0.479237, 0.504391, 0.537803, 0.588487, 0.627532, 0.686839, 0.791905, 0.878332, 0.987405,
  1.08162,  1.16971,  1.2982,   1.40393,  1.49854,  1.64168,  1.7524,   1.87058,  2.02273,
  2.15894,  2.3654,   2.55792,  2.73017,  3.03005,  3.40733,  3.88128,  4.53725,  5.16786,
  5.73439,  6.53106,  7.43879,  8.36214,  9.39965,  10.296,   11.5735,  13.1801,  15.2052,
  17.5414,  19.7178,  22.7462,  25.9026,  29.4955,  33.5867,  39.2516,  46.4716,  53.6065,
  63.4668,  73.2147,  85.5593,  99.9854};

////////////////////////////////////////////////////////////////////////////////////////////////////

const G4double G4NeutrinoNucleusModel::fOnePionProb[58] = {
  0.0019357,  0.0189361, 0.0378722, 0.0502758, 0.0662559, 0.0754581, 0.0865008, 0.0987275,
  0.124112,   0.153787,  0.18308,   0.213996,  0.245358,  0.274425,  0.301536,  0.326612,
  0.338208,   0.337806,  0.335948,  0.328092,  0.313557,  0.304965,  0.292169,  0.28481,
  0.269474,   0.254138,  0.247499,  0.236249,  0.221654,  0.205492,  0.198781,  0.182216,
  0.162251,   0.142878,  0.128631,  0.116001,  0.108435,  0.0974843, 0.082092,  0.0755204,
  0.0703121,  0.0607066, 0.0554278, 0.0480401, 0.0427023, 0.0377123, 0.0323248, 0.0298584,
  0.0244296,  0.0218526, 0.019121,  0.016477,  0.0137309, 0.0137963, 0.0110371, 0.00834028,
  0.00686127, 0.00538226};
...
G4double G4NeutrinoNucleusModel::CalculateQEratioA(G4int Z, G4int A, G4double energy, G4int nepdg)
{
  ...
  else
  {
    ...
    if (nepdg == 12 || nepdg == 14)
      qerata = N * rr / (N * rr + A * (1 - rr));
    else
      qerata = Z * rr / (Z * rr + A * (1 - rr));
  }
  fQEratioA = qerata;
...
const G4double G4NeutrinoNucleusModel::fANeMuQEratio[50] = {
  1,          1,          1,          1,          1,          1,          1,          0.97506,
  0.920938,   0.847671,   0.762973,   0.677684,   0.597685,   0.52538,    0.461466,   0.405329,
  0.356154,   0.312944,   0.274984,   0.241341,   0.211654,   0.185322,   0.161991,   0.141339,
  0.123078,   0.106952,   0.0927909,  0.0803262,  0.0693698,  0.0598207,  0.0514545,  0.044193,
  0.0378696,  0.0324138,  0.0276955,  0.0236343,  0.0201497,  0.0171592,  0.014602,   0.0124182,
  0.0105536,  0.00896322, 0.00761004, 0.00645821, 0.00547859, 0.00464595, 0.00393928, 0.00333961,
  0.00283086, 0.00239927};

const G4double G4NeutrinoNucleusModel::fNeMuQEratio[50] = {
  1,          1,          1,          1,          1,          1,          1,          0.977592,
  0.926073,   0.858783,   0.783874,   0.706868,   0.63113,    0.558681,   0.490818,   0.428384,
  0.371865,   0.321413,   0.276892,   0.237959,   0.204139,   0.1749,     0.149706,   0.128047,
  0.109456,   0.093514,   0.0798548,  0.0681575,  0.0581455,  0.0495804,  0.0422578,  0.036002,
  0.0306614,  0.0261061,  0.0222231,  0.0189152,  0.0160987,  0.0137011,  0.0116604,  0.00992366,
  0.00844558, 0.00718766, 0.00611714, 0.00520618, 0.00443105, 0.00377158, 0.00321062, 0.0027335,
  0.00232774, 0.00198258};
