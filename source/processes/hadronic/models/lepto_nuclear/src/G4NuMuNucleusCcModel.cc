...
G4NuMuNucleusCcModel::G4NuMuNucleusCcModel(const G4String& name) : G4NeutrinoNucleusModel(name)
{
  fData = fMaster = false;
  InitialiseModel();
}
...
G4bool G4NuMuNucleusCcModel::IsApplicable(const G4HadProjectile& aPart, G4Nucleus&)
{
  G4bool result = false;
