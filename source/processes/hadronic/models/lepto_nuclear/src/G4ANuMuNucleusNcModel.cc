...
G4ANuMuNucleusNcModel::G4ANuMuNucleusNcModel(const G4String& name) : G4NeutrinoNucleusModel(name)
{
...
G4bool G4ANuMuNucleusNcModel::IsApplicable(const G4HadProjectile& aPart, G4Nucleus&)
{
  G4bool result = false;
...
G4HadFinalState* G4ANuMuNucleusNcModel::ApplyYourself(const G4HadProjectile& aTrack,
                                                      G4Nucleus& targetNucleus)
{
  ...
  else  // if ( eX < 9500000.*GeV ) // < 25.*GeV) //  < 95.*GeV ) // < 2.5*GeV ) //cluster decay
  {
    if (fProton && pName == "anti_nu_mu")
      qB = 1;
    else if (!fProton && pName == "anri_nu_mu")
      qB = 0;
