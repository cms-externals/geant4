...
G4ANuTauNucleusNcModel::G4ANuTauNucleusNcModel(const G4String& name) : G4NeutrinoNucleusModel(name)
{
...
G4bool G4ANuTauNucleusNcModel::IsApplicable(const G4HadProjectile& aPart, G4Nucleus&)
{
  G4bool result = false;
...
G4HadFinalState* G4ANuTauNucleusNcModel::ApplyYourself(const G4HadProjectile& aTrack,
                                                       G4Nucleus& targetNucleus)
{
  ...
  {
    if (fProton && pName == "nu_tau")
      qB = 1;
    else if (!fProton && pName == "nu_tau")
      qB = 0;
