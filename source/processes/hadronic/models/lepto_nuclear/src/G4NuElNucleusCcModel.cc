...
G4NuElNucleusCcModel::G4NuElNucleusCcModel(const G4String& name) : G4NeutrinoNucleusModel(name)
{
...
G4bool G4NuElNucleusCcModel::IsApplicable(const G4HadProjectile& aPart, G4Nucleus&)
{
  G4bool result = false;
  ...
  if (pName == "nu_e" && energy > fMinNuEnergy)
  {
    result = true;
  }
                                                     ...
                                                     G4Nucleus& targetNucleus)
{
      ...
      && fCosTheta > 0.9)  // && fQtransfer < 0.95*GeV ) // mu- & coherent pion + nucleus
  {
    ...
    if (pName == "nu_e")
      aLept = new G4DynamicParticle(theElectron, lv2);
    else
    {
      theParticleChange.SetEnergyChange(energy);
      theParticleChange.SetMomentumChange(aTrack.Get4Momentum().vect().unit());
...
*/
  ...
  {
    if (fProton && pName == "nu_e")
      qB = 2;
    else if (!fProton && pName == "nu_e")
      qB = 1;
