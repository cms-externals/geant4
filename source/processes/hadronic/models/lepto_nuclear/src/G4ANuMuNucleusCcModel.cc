...
G4ANuMuNucleusCcModel::G4ANuMuNucleusCcModel(const G4String& name) : G4NeutrinoNucleusModel(name)
{
  fData = fMaster = false;
  InitialiseModel();
}
...
G4bool G4ANuMuNucleusCcModel::IsApplicable(const G4HadProjectile& aPart, G4Nucleus&)
{
  G4bool result = false;
  G4String pName = aPart.GetDefinition()->GetParticleName();
  G4double energy = aPart.GetTotalEnergy();

  if (pName == "anti_nu_mu" && energy > fMinNuEnergy)
  {
    result = true;
  }
                                                      ...
                                                      G4Nucleus& targetNucleus)
{
  ...
  if (p1pi > G4UniformRand()
      && fCosTheta > 0.9)  // && fQtransfer < 0.95*GeV ) // mu- & coherent pion + nucleus
  {
    // lvsum = lvp1 + lvpip1;
    lvsum = lvp1 + lvt1;
    // cost = fCosThetaPi;
    ...
    if (pName == "anti_nu_mu")
      aLept = new G4DynamicParticle(theMuonPlus, lv2);
    else
    {
      theParticleChange.SetEnergyChange(energy);
      theParticleChange.SetMomentumChange(aTrack.Get4Momentum().vect().unit());
...
*/
  ...
  {
    if (fProton && pName == "anti_nu_mu")
      qB = 0;
    else if (!fProton && pName == "anti_nu_mu")
      qB = -1;
...
void G4ANuMuNucleusCcModel::SampleLVkr(const G4HadProjectile& aTrack, G4Nucleus& targetNucleus)
{
  ...
  if (A == 1)  // hydrogen, no Fermi motion ???
  {
    ...
    do
    {
      ...
      if (e3 < sqrt(fW2))
        G4cout << "energyX = " << e3 / GeV << ", fW = " << sqrt(fW2) / GeV << G4endl;
