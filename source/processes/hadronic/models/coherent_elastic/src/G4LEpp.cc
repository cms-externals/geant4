...
G4HadFinalState* G4LEpp::ApplyYourself(const G4HadProjectile& aTrack, G4Nucleus& targetNucleus)
{
  ...
  if (verboseLevel > 1)
  {
    G4cout << "  particle 1 momentum in LAB " << newP->GetMomentum() / GeV << " "
           << newP->GetTotalMomentum() / GeV << G4endl;
    G4cout << "  particle 2 momentum in LAB " << targetParticle->GetMomentum() / GeV << " "
...
G4double G4LEpp::SampleInvariantT(const G4ParticleDefinition* p, G4double plab, G4int, G4int)
{
