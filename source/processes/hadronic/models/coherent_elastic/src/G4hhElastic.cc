...
G4hhElastic::G4hhElastic() : G4HadronElastic("HadrHadrElastic")
{
  SetMinEnergy(1. * GeV);
...
G4hhElastic::~G4hhElastic()
{
  ...
  for (std::vector<G4PhysicsTable*>::iterator it = fBankT.begin(); it != fBankT.end(); ++it)
  {
...
G4double G4hhElastic::SampleInvariantT(const G4ParticleDefinition* aParticle, G4double p, G4int,
                                       G4int)
{
...
G4double G4hhElastic::SampleBisectionalT(const G4ParticleDefinition* aParticle, G4double p)
{
  ...
  else  // Tkin inside between energy table edges
  {
    ...
    t = (*fTableT)(iTkin)->GetLowEdgeEnergy(
      sTransfer);  // +(-0.5+rand)*(*fTableT)(iTkin)->GetLowEdgeEnergy(3);
  }
  return t;
}
...
void G4hhElastic::BuildTableTest(G4ParticleDefinition* target, G4ParticleDefinition* projectile,
                                 G4double plab)
{
  G4int jTransfer;
  ...
  G4cout << "s = " << std::sqrt(fSpp) / GeV << " GeV; fPcms = " << fPcms / GeV
         << " GeV; qMax = " << tMax / GeV / GeV << " GeV2; dt = " << dt / GeV / GeV << " GeV2"
         << G4endl;
...
G4double G4hhElastic::GetTransfer(G4int iTkin, G4int iTransfer, G4double position)
{
