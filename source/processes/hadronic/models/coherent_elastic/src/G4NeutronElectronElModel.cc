...
G4NeutronElectronElModel::G4NeutronElectronElModel(const G4String& name) : G4HadronElastic(name)
{
...
G4double G4NeutronElectronElModel::GetTransfer(G4int iTkin, G4int iTransfer, G4double position)
{
...
G4HadFinalState* G4NeutronElectronElModel::ApplyYourself(const G4HadProjectile& aTrack, G4Nucleus&)
{
