  ...
  : G4HadronElastic("NNDiffuseElastic"), fParticle(0)
{
  ...
  fEnergyBin =
    300;  // Increased from the original 200 to have no wider log-energy-bins up to 10 PeV
  fAngleBin = 200;
  ...
  fNuclearRadius1 = fNuclearRadius2 = fNuclearRadiusSquare = fRutherfordRatio = fCoulombPhase0 =
    fHalfRutThetaTg = fHalfRutThetaTg2 = fRutherfordTheta = fProfileLambda = fCofPhase = fCofFar =
...
G4NuclNuclDiffuseElastic::~G4NuclNuclDiffuseElastic()
{
  ...
  for (std::vector<G4PhysicsTable*>::iterator it = fAngleBank.begin(); it != fAngleBank.end(); ++it)
  {
...
G4double G4NuclNuclDiffuseElastic::GetDiffuseElasticXsc(const G4ParticleDefinition* particle,
                                                        G4double theta, G4double momentum,
                                                        G4double A)
{
  fParticle = particle;
...
G4double G4NuclNuclDiffuseElastic::GetInvElasticXsc(const G4ParticleDefinition* particle,
                                                    G4double tMand, G4double plab, G4double A,
                                                    G4double Z)
{
  ...
  if (iZ == 1 && iA == 1)
    ...
    theDef = G4Triton::Triton();
  else if (iZ == 2 && iA == 3)
    theDef = G4He3::He3();
  else if (iZ == 2 && iA == 4)
    theDef = theAlpha;
  else
    theDef = G4ParticleTable::GetParticleTable()->GetIonTable()->GetIon(iZ, iA, 0);
...
G4double G4NuclNuclDiffuseElastic::GetDiffElasticProb(  // G4ParticleDefinition* particle,
  G4double theta
  //  G4double momentum,
  // G4double A
)
{
  ...
  */
  ...
  G4double pikdt =
    lambda * (1. - G4Exp(-pi * fWaveVector * diffuse * theta / lambda));  // wavek*delta;
...
G4double G4NuclNuclDiffuseElastic::GetDiffElasticSumProb(  // G4ParticleDefinition* particle,
  G4double theta
  //  G4double momentum,
  // G4double A
)
...
G4double G4NuclNuclDiffuseElastic::GetIntegrandFunction(G4double alpha)
{
  G4double result;
...
G4double G4NuclNuclDiffuseElastic::SampleT(const G4ParticleDefinition* aParticle, G4double p,
                                           G4double A)
{
...
G4double G4NuclNuclDiffuseElastic::SampleThetaCMS(const G4ParticleDefinition* particle,
                                                  G4double momentum, G4double A)
{
  ...
  for (i = 1; i <= iMax; i++)
  {
    ...
    sum +=
      integral.Legendre10(this, &G4NuclNuclDiffuseElastic::GetIntegrandFunction, theta1, theta2);
...
G4double G4NuclNuclDiffuseElastic::SampleInvariantT(const G4ParticleDefinition* aParticle,
                                                    G4double p, G4int Z, G4int A)
{
  fParticle = aParticle;
...
G4double G4NuclNuclDiffuseElastic::SampleTableThetaCMS(const G4ParticleDefinition* particle,
                                                       G4double momentum, G4double Z, G4double A)
{
  ...
  {
    // G4cout<<iMomentum<<", kinE = "<<kinE/MeV<<", vectorE =
    // "<<fEnergyVector->GetLowEdgeEnergy(iMomentum)/MeV<<G4endl;
...
void G4NuclNuclDiffuseElastic::InitialiseOnFly(G4double Z, G4double A)
{
  ...
  if (verboseLevel > 0)
  {
    G4cout << "G4NuclNuclDiffuseElastic::Initialise() the element with Z = " << Z
           << "; and A = " << A << G4endl;
  }
  fElementNumberVector.push_back(fAtomicNumber);
...
void G4NuclNuclDiffuseElastic::BuildAngleTable()
{
  ...
  for (i = 0; i < fEnergyBin; i++)
  {
    ...
    for (j = fAngleBin - 1; j >= 1; j--)
    {
      ...
      delta = integral.Legendre10(this, &G4NuclNuclDiffuseElastic::GetFresnelIntegrandXsc, alpha1,
                                  alpha2);
      // delta = integral.Legendre96(this, &G4NuclNuclDiffuseElastic::GetIntegrandFunction, alpha1,
      // alpha2);
...
G4double G4NuclNuclDiffuseElastic::GetScatteringAngle(G4int iMomentum, G4int iAngle,
                                                      G4double position)
{
  ...
  else
  {
    ...
    {
      if (y1 == y2)
        randAngle = x1 + (x2 - x1) * G4UniformRand();
      else
      {
...
G4double G4NuclNuclDiffuseElastic::SampleThetaLab(const G4HadProjectile* aParticle, G4double tmass,
                                                  G4double A)
{
  ...
  if (t > tmax)
  {
    if (verboseLevel > 0)
    {
      G4cout << "G4NuclNuclDiffuseElastic:WARNING: A = " << A << " mom(GeV)= " << plab / GeV
             << " S-wave will be sampled" << G4endl;
    }
    t = G4UniformRand() * tmax;
  }
  if (verboseLevel > 1)
  {
    G4cout << " t= " << t << " tmax= " << tmax << " ptot= " << ptot << G4endl;
  }
                                              ...
                                              G4double partMom, G4double Z, G4double A)
{
  ...
  G4cout << "G4NuclNuclDiffuseElastic::TestAngleTable() init the element with Z = " << Z
         << "; and A = " << A << G4endl;
  ...
  for (j = 1; j < fAngleBin; j++)
  {
    ...
    deltaL10 =
      integral.Legendre10(this, &G4NuclNuclDiffuseElastic::GetIntegrandFunction, alpha1, alpha2);
    deltaL96 =
      integral.Legendre96(this, &G4NuclNuclDiffuseElastic::GetIntegrandFunction, alpha1, alpha2);
    deltaAG = integral.AdaptiveGauss(this, &G4NuclNuclDiffuseElastic::GetIntegrandFunction, alpha1,
                                     alpha2, epsilon);
    ...
    G4cout << alpha1 << "\t" << std::sqrt(alpha1) / degree << "\t" << sumL10 << "\t" << sumL96
           << "\t" << sumAG << G4endl;
...
G4double G4NuclNuclDiffuseElastic::GetLegendrePol(G4int n, G4double theta)
{
  ...
  if (n < 0)
    ...
    legPol = (5. * x * x * x - 3. * x) / 2.;
  else if (n == 4)
    legPol = (35. * x * x * x * x - 30. * x * x + 3.) / 8.;
  else if (n == 5)
    legPol = (63. * x * x * x * x * x - 70. * x * x * x + 15. * x) / 8.;
  else if (n == 6)
    legPol = (231. * x * x * x * x * x * x - 315. * x * x * x * x + 105. * x * x - 5.) / 16.;
  else
  {
    // legPol = ( (2*n-1)*x*GetLegendrePol(n-1,x) - (n-1)*GetLegendrePol(n-2,x) )/n;

    legPol = std::sqrt(2. / (n * CLHEP::pi * std::sin(theta + epsilon)))
...
G4complex G4NuclNuclDiffuseElastic::GammaLess(G4double theta)
{
  ...
  G4complex gamma =
    CLHEP::pi * kappa * GetErfcInt(-order) * std::exp(im * (u * u + 0.25 * CLHEP::pi));
                                                         ...
                                                         G4ParticleDefinition* tParticle)
{
                             ...
                             // if( proj_momentum >= 2.)
  {
    ...
    // AR-12Aug2016  if( proj_momentum >= 10.)
    {
      ...
      xsection = A0 + B0 * G4Log(proj_energy) - 11
                 + 103
                     * G4Pow::GetInstance()->powA(2 * 0.93827 * proj_energy + proj_mass * proj_mass
                                                    + 0.93827 * 0.93827,
                                                  -0.165);  //  mb
    }
  }
  else  // low energy pp = nn != np
  {
    if (pParticle == tParticle)  // pp or nn      // nn to be pp
    {
      ...
      {
        hnXsc = 23 + 40 * (G4Log(proj_momentum / 0.73)) * (G4Log(proj_momentum / 0.73));
      }
...
G4double G4NuclNuclDiffuseElastic::GetRatioGen(G4double theta)
{
  ...
  if (theta <= fRutherfordTheta)
  {
    out = 1.
          + 0.5
              * ((0.5 - cosFresnel) * (0.5 - cosFresnel) + (0.5 - sinFresnel) * (0.5 - sinFresnel))
              * prof2;
    out += (cosFresnel + sinFresnel - 1.) * prof;
  }
  else
  {
    out = 0.5 * ((0.5 - cosFresnel) * (0.5 - cosFresnel) + (0.5 - sinFresnel) * (0.5 - sinFresnel))
          * prof2;
  }
...
{
  const G4double cof[6] = {76.18009172947146,  -86.50532032941677,    24.01409824083091,
...
G4double G4NuclNuclDiffuseElastic::BesselJzero(G4double value)
{
  ...
  if (value < 8.0 && value > -8.0)
  {
    ...
    fact1 =
      ...
      + value2
          * (-13362590354.0
             + value2
                 * (651619640.7
                    + value2 * (-11214424.18 + value2 * (77392.33017 + value2 * (-184.9052456)))));
    ...
    fact2 =
      ...
      + value2
          * (1029532985.0
             + value2
                 * (9494680.718 + value2 * (59272.64853 + value2 * (267.8532712 + value2 * 1.0))));
  ...
  else
  {
    ...
    fact1 =
      ...
      + value2
          * (-0.1098628627e-2
             + value2 * (0.2734510407e-4 + value2 * (-0.2073370639e-5 + value2 * 0.2093887211e-6)));

    fact2 =
      -0.1562499995e-1
      + value2
          * (0.1430488765e-3
             + value2 * (-0.6911147651e-5 + value2 * (0.7621095161e-6 - value2 * 0.934945152e-7)));

    bessel =
      std::sqrt(0.636619772 / modvalue) * (std::cos(shift) * fact1 - arg * std::sin(shift) * fact2);
  }
  return bessel;
}
  ...
  if (modvalue < 8.0)
  {
         ...
         + value2
             * (-7895059235.0
                + value2
                    * (242396853.1
                       + value2
                           * (-2972611.439 + value2 * (15704.48260 + value2 * (-30.16036606))))));
    ...
    fact2 =
      ...
      + value2
          * (2300535178.0
             + value2
                 * (18583304.74 + value2 * (99447.43394 + value2 * (376.9991397 + value2 * 1.0))));
    bessel = fact1 / fact2;
  }
  else
  {
    ...
    fact1 =
      ...
      + value2
          * (0.183105e-2
             + value2
                 * (-0.3516396496e-4 + value2 * (0.2457520174e-5 + value2 * (-0.240337019e-6))));

    fact2 =
      0.04687499995
      + value2
          * (-0.2002690873e-3
             + value2 * (0.8449199096e-5 + value2 * (-0.88228987e-6 + value2 * 0.105787412e-6)));
