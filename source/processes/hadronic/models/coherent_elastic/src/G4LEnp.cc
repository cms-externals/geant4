...
G4LEnp::G4LEnp() : G4HadronElastic("G4LEnp")  // G4HadronicInteraction("G4LEnp")
{
...
G4HadFinalState* G4LEnp::ApplyYourself(const G4HadProjectile& aTrack, G4Nucleus& targetNucleus)
{
  ...
  if (verboseLevel > 1)
  {
    ...
    G4cout << "A = " << A << ", Z = " << Z << ", atomic mass "
           << G4Proton::Proton()->GetPDGMass() / GeV << "GeV" << G4endl;
    //
  ...
  if (verboseLevel > 1)
  {
    G4cout << "sample=" << sample << G4endl << ke1 << " " << ke2 << " " << sigint1 << " " << sigint2
           << G4endl;
  }
  do
  {
    ...
    if (verboseLevel > 1)
    {
      G4cout << ke1 << " " << ke2 << " " << sigint1 << " " << sigint2 << G4endl;
    }
  ...
  if (verboseLevel > 1)
  {
    ...
    G4cout << "  particle  1 momentum in CM " << px / GeV << " " << py / GeV << " " << pz / GeV
           << " " << p / GeV << G4endl;
  }
  ...
  if (verboseLevel > 1)
  {
    G4cout << "  betaCM " << betaCMx << " " << betaCMy << " " << betaCMz << " " << betaCM << G4endl;
  ...
  if (verboseLevel > 1)
  {
    G4cout << "  particle 1 momentum in LAB " << newP->GetMomentum() * (1. / GeV) << " "
           << newP->GetTotalMomentum() / GeV << G4endl;
    G4cout << "  particle 2 momentum in LAB " << targetParticle->GetMomentum() * (1. / GeV) << " "
           << targetParticle->GetTotalMomentum() / GeV << G4endl;
    G4cout << "  TOTAL momentum in LAB "
           << (newP->GetMomentum() + targetParticle->GetMomentum()) * (1. / GeV) << " "
           << (newP->GetMomentum() + targetParticle->GetMomentum()).mag() / GeV << G4endl;
  }
...
G4double G4LEnp::SampleInvariantT(const G4ParticleDefinition* p, G4double plab, G4int, G4int)
{
