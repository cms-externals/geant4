...
G4AntiNuclElastic::G4AntiNuclElastic() : G4HadronElastic("AntiAElastic")
{
...
G4double G4AntiNuclElastic::SampleInvariantT(const G4ParticleDefinition* particle, G4double Plab,
                                             G4int Z, G4int A)
{
  G4double T;
  ...
  if (Z == 1 && A == 1)
    theTargetDef = theProton;
  else if (Z == 1 && A == 2)
    theTargetDef = theDeuteron;
  else if (Z == 1 && A == 3)
    theTargetDef = G4Triton::Triton();
  else if (Z == 2 && A == 3)
    theTargetDef = G4He3::He3();
  else if (Z == 2 && A == 4)
    theTargetDef = theAlpha;
  ...
  if (std::abs(theParticle->GetBaryonNumber()) == 1)
  {
    PrNucleonMass = theParticle->GetPDGMass();
  }
  else
  {
    PrNucleonMass = NucleonMass;
  }
  ...
  G4double XsCoulomb =
    mevToBarn * sqr(n / fWaveVector) * pi * (1 + ctet1) / (1. + Am) / (1. + 2. * Am - ctet1);
  ...
  XsElastHadronic /= millibarn;
  XsTotalHadronic /= millibarn;
  ...
  else
  {
    ...
    fRa = 1.113 * G4Pow::GetInstance()->Z13(A) - 0.227 / G4Pow::GetInstance()->Z13(A);
    if (A == 3) fRa = 1.81;
    if (A == 4) fRa = 1.37;
    ...
    if ((theParticle == theAProton) || (theParticle == theANeutron))
    {
      ...
      {
        Ref2 =
          fRa * fRa + 2.48 * 0.01 * sig_pbarp * fRa - 2.23e-6 * sig_pbarp * sig_pbarp * fRa * fRa;
    ...
    do
    {
      ...
      BracFunct = ((sqr(BesselOneByArg(x)) + sqr(rho / 2. * BesselJzero(x)))
                   * sqr(DampFactor(pi * fceff * Q)))
                  / (Amag * G4Exp(-SlopeMag * Q));
...
G4double G4AntiNuclElastic::SampleThetaCMS(const G4ParticleDefinition* p, G4double plab, G4int Z,
                                           G4int A)
{
  ...
  if (T > fTmax)
  {
    if (verboseLevel > 0)
    {
      G4cout << "G4AntiNuclElastic::SampleThetaCMS WARNING: A = " << A << " mom(GeV)=" << plab / GeV
             << "  t(GeV2)=" << T / (GeV * GeV) << " > Tmax(GeV2)=" << fTmax / (GeV * GeV)
...
G4double G4AntiNuclElastic::SampleThetaLab(const G4ParticleDefinition* p, G4double plab, G4int Z,
                                           G4int A)
{
  G4double T = SampleInvariantT(p, plab, Z, A);
  if (T <= 0.0 || fTmax <= 0.0)
  ...
  if (T > fTmax)
  {
    if (verboseLevel > 0)
    {
      G4cout << "G4AntiNuclElastic::SampleThetaLab WARNING: A = " << A << " mom(GeV)=" << plab / GeV
             << "  t(GeV2)=" << T / (GeV * GeV) << " > Tmax(GeV2)=" << fTmax / (GeV * GeV)
             << " S-wave will be sampled" << G4endl;
    }
    T = G4UniformRand() * fTmax;
