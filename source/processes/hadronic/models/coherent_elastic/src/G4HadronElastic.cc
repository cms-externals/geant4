...
G4HadronElastic::G4HadronElastic(const G4String& name) : G4HadronicInteraction(name), secID(-1)
{
                                                ...
                                                G4Nucleus& targetNucleus)
{
  ...
  if (verboseLevel > 1)
  {
    G4cout << "G4HadronElastic: " << aParticle->GetDefinition()->GetParticleName()
           << " Plab(GeV/c)= " << plab / GeV << " Ekin(MeV) = " << ekin / MeV
           << " scattered off Z= " << Z << " A= " << A << G4endl;
  }
  ...
  if (t < 0.0 || t > pLocalTmax)
  {
    ...
    if (nwarn < 2)
    {
      ...
      ed << GetModelName() << " wrong sampling t= " << t << " tmax= " << pLocalTmax << " for "
         << aParticle->GetDefinition()->GetParticleName() << " ekin=" << ekin << " MeV"
  ...
  if (verboseLevel > 1)
  {
    G4cout << " t= " << t << " tmax(GeV^2)= " << pLocalTmax / (GeV * GeV)
           << " Pcms(GeV)= " << momentumCMS / GeV << " cos(t)=" << cost << " sin(t)=" << sint
           << G4endl;
  }
  G4LorentzVector nlv1(momentumCMS * sint * std::cos(phi), momentumCMS * sint * std::sin(phi),
                       momentumCMS * cost, std::sqrt(momentumCMS * momentumCMS + m1 * m1));
  ...
  if (verboseLevel > 1)
  {
    G4cout << "G4HadronElastic: m= " << m1 << " Efin(MeV)= " << eFinal << " 4-M Final: " << nlv1
           << G4endl;
  }
  ...
  if (verboseLevel > 1)
  {
    G4cout << "Recoil: " << " m= " << mass2 << " Erec(MeV)= " << erec << " 4-mom: " << lv << G4endl;
  }
...
G4double G4HadronElastic::SampleInvariantT(const G4ParticleDefinition* part, G4double mom, G4int,
                                           G4int A)
{
