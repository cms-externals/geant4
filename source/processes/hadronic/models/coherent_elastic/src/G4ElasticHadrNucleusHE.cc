...
const G4int G4ElasticHadrNucleusHE::fHadronCode[] = {
  211,  -211, 2112, 2212, 321,   -321,  130,   310,   311,   -311,  3122,  3222,  3112,
  3212, 3312, 3322, 3334, -2212, -2112, -3122, -3222, -3112, -3212, -3312, -3322, -3334};

const G4int G4ElasticHadrNucleusHE::fHadronType[] = {2, 3, 6, 0, 4, 5, 4, 4, 4, 5, 0, 0, 0,
                                                     0, 0, 0, 0, 1, 7, 1, 1, 1, 1, 1, 1, 1};

const G4int G4ElasticHadrNucleusHE::fHadronType1[] = {3, 4, 1, 0, 5, 6, 5, 5, 5, 6, 0, 0, 0,
...
G4ElasticData* G4ElasticHadrNucleusHE::fElasticData[NHADRONS][ZMAX] = {{nullptr}};
...
G4ElasticData::G4ElasticData(const G4ParticleDefinition* p, G4int Z, G4int A, const G4double* e)
{
...
{
  dQ2 = hMass = hMass2 = hLabMomentum = hLabMomentum2 = HadrEnergy = R1 = R2 = Pnucl = Aeff =
    HadrTot = HadrSlope = HadrReIm = TotP = DDSect2 = DDSect3 = ConstU = Slope1 = Slope2 = Coeff1 =
      Coeff2 = Slope0 = Coeff0 = aAIm = aDIm = Dtot11 = Q2max = 0.0;
  ...
  if (fEnergy[0] == 0.0)
  {
    ...
    if (fEnergy[0] == 0.0)
    {
      ...
      if (verboseLevel > 0)
      {
        ...
        {
          G4cout << "  " << i << "   " << fLowEdgeEnergy[i] << "  " << fEnergy[i] << G4endl;
        }
      }
#ifdef G4MULTITHREADED
    }
    ...
    for (G4int j = 0; j < NHADRONS; ++j)
    {
      for (G4int k = 0; k < ZMAX; ++k)
      {
        ...
        if (ptr)
        {
          ...
          {
            if (ptr == fElasticData[l][k])
            {
              fElasticData[l][k] = nullptr;
            }
          }
        }
      }
...
G4double G4ElasticHadrNucleusHE::SampleInvariantT(const G4ParticleDefinition* p, G4double inLabMom,
                                                  G4int iZ, G4int A)
{
  ...
  if (verboseLevel > 1)
  {
    G4cout << "G4ElasticHadrNucleusHE::SampleT: "
           << " for " << p->GetParticleName() << " at Z= " << Z << " A= " << A
           << " plab(GeV)= " << plab << " hadrCode= " << iHadrCode << G4endl;
  }
  iHadron = -1;
  G4int idx;
  ...
  else
  {
    ...
    if (verboseLevel > 1)
    {
      G4cout << " SampleT: Q2(GeV^2)= " << Q2 << "  t/tmax= " << Q2 / tmax << G4endl;
    }
  }
  return Q2 * GeV2;
}
...
void G4ElasticHadrNucleusHE::FillData(const G4ParticleDefinition* p, G4int idx, G4int Z)
{
  ...
  if (!fElasticData[idx][Z])
  {
    ...
    if (verboseLevel > 0)
    {
      G4cout << "### FillData for " << p->GetParticleName() << " Z= " << Z << " idx= " << idx
             << " iHadron= " << iHadron << " iHadron1= " << iHadron1 << " iHadrCode= " << iHadrCode
             << "\n   R1= " << R1 << " R2= " << R2 << " Aeff= " << Aeff << " Pnucl= " << Pnucl
             << G4endl;
    }
    ...
    if (!fRetrieveFromFile)
    {
      for (G4int i = 0; i < NENERGY; ++i)
      {
        ...
        if (verboseLevel > 0)
        {
          G4cout << "### i= " << i << " Z= " << Z << " A= " << A << " length= " << length
                 << " Q2max= " << Q2max << G4endl;
        }
    ...
    if (verboseLevel > 0)
    {
      G4cout << " G4ElasticHadrNucleusHE::FillData done for idx= " << idx << " for "
             << p->GetParticleName() << " Z= " << Z << " A= " << A << G4endl;
    }
...
void G4ElasticHadrNucleusHE::InterpolateHN(G4int n, const G4double EnP[], const G4double C0P[],
                                           const G4double C1P[], const G4double B0P[],
                                           const G4double B1P[])
{
  G4int i;
...
G4double G4ElasticHadrNucleusHE::HadronNucleusQ2_2(const G4ElasticData* pElD, G4double plab,
                                                   G4double tmax)
{
  ...
  if (verboseLevel > 1)
  {
    G4cout << "Q2_2: ekin(GeV)= " << ekin << "  plab(GeV/c)= " << plab << "  tmax(GeV2)= " << tmax
           << G4endl;
  }
  ...
  {
    if (Rand <= (pElD->fCumProb[idx])[iNumbQ2])
    {
      break;
    }
  }
  ...
  if (verboseLevel > 1)
  {
    G4cout << " HadrNucleusQ2_2(2): Q2= " << Q2 << " iNumbQ2= " << iNumbQ2 << " rand= " << Rand
           << " Q2max= " << Q2max << " tmax= " << tmax << G4endl;
  }
  return Q2;
}
...
G4double G4ElasticHadrNucleusHE::GetQ2_2(G4int kk, G4int kmax, const std::vector<G4double>& F,
                                         G4double ranUni)
{
  ...
  if (verboseLevel > 1)
  {
    G4cout << "GetQ2_2 kk= " << kk << " X2= " << X2 << " X3= " << X3 << " F2= " << F2
           << " F3= " << F3 << " Rndm= " << ranUni << G4endl;
  }
  ...
  else
  {
    ...
    G4double DC =
      X3 * F2 * F12 + X2 * F1 * F32 + X1 * F3 * F22 - X1 * F2 * F32 - X2 * F3 * F12 - X3 * F1 * F22;
...
G4int G4ElasticHadrNucleusHE::FillFq2(G4int A)
{
  ...
  for (ii = 1; ii < ONQ2 - 1; ++ii)
  {
    ...
    if (verboseLevel > 2)
    {
      G4cout << ii << ". FillFq2: A= " << A << " Q2= " << Q2l << " dQ2= " << dQ2
             << " Tot= " << totSum << " dTot " << curSum << " curSec= " << curSec << G4endl;
    }
    if (totSum * 1.e-4 > curSum || Q2l >= Q2max)
    {
      break;
    }
  }
  ...
  if (verboseLevel > 1)
  {
    G4cout << "### FillFq2 done curQ2= " << curQ2 << " Q2max= " << Q2max
           << " sumG= " << fLineF[ONQ2 - 2] << "  totSum= " << totSum << " Nbins= " << ii + 1
           << G4endl;
  }
  return ii + 2;
}
...
G4double G4ElasticHadrNucleusHE::GetLightFq2(G4int Z, G4int A, G4double Q2)
{
  ...
  if (Z == 1)
  {
    ...
    G4double y =
      (1. - Coeff1 - Coeff0) / HadrSlope * (1. - G4Exp(-HadrSlope * Q2))
  ...
  if (verboseLevel > 1)
  {
    G4cout << " Fq2 Before for i Tot B Im " << HadrTot << "  " << HadrSlope << "  " << HadrReIm
           << G4endl;
  }
  if (verboseLevel > 1)
  {
    G4cout << "GetFq2: Stot= " << Stot << " Bhad= " << Bhad << "  Im " << HadrReIm
           << " Asq= " << Asq << G4endl;
  ...
  if (verboseLevel > 2)
  {
    G4cout << "UnucRho2= " << UnucRho2 << " FiH= " << FiH << " NN2= " << NN2 << " Norm= " << Norm
           << G4endl;
  }
  G4double Prod0 = 0.;
  G4double N1 = -1.0;
    ...
    for (G4int i2 = 1; i2 <= A; ++i2)  ////+++++++++ i2
    {
      ...
      for (G4int j2 = 0; j2 <= i2; ++j2)  ////+++++++++ j2
      {
        ...
        for (G4int j1 = 0; j1 <= i1; ++j1)  ////++++++++ j1
        {
          ...
          N4 *= (-NN2);
          Prod3 += N4 * exp1 * exp2 * (1. - G4Exp(-Q2 * dddd)) * GetBinomCof(i1, j1) / dddd;
        }  // j1
  ...
  if (verboseLevel > 1)
  {
    G4cout << "GetLightFq2 Z= " << Z << " A= " << A << " Q2= " << Q2 << " Res= " << Prod0 << G4endl;
  }
  return Prod0;
}
...
G4double G4ElasticHadrNucleusHE::HadrNucDifferCrSec(G4int A, G4double aQ2)
{
  ...
  if (A == 1)
  {
    ...
    G4double dSigPodT =
      HadrTot * HadrTot * (1 + HadrReIm * HadrReIm)
      * (Coeff1 * G4Exp(-Slope1 * SqrQ2) + Coeff2 * G4Exp(Slope2 * (valueConstU) + aQ2)
         + (1 - Coeff1 - Coeff0) * G4Exp(-HadrSlope * aQ2) + Coeff0 * G4Exp(-Slope0 * aQ2))
      * 2.568 / (16 * pi);
  ...
  G4double DDSec2p = (DDSect2 + DDSect3 * G4Log(0.53 * HadrEnergy / std::sqrt((R12 + R22) * 0.5)));
  ...
  G4double Din1 =
    0.5
    * (C1 * G4Exp(-aQ2 / 8 * R12Ap) / 2 * R12Ap - C2 / R12ApdR22Ap * G4Exp(-aQ2 / (4 * R12ApdR22Ap))
  ...
  for (G4int i = 1; i <= A - 2; ++i)
  {
    ...
    for (G4int l = 0; l <= i; ++l)
    {
      ...
      Din2 += N2p * BinCoeff
              * (C1 / exp1p * G4Exp(-aQ2 / (4 * exp1p)) - C2 / exp2p * G4Exp(-aQ2 / (4 * exp2p))
  ...
  G4double DiffCrSec2 =
    (ReElasticAmpl0 * ReElasticAmpl0 + (ImElasticAmpl0 + Din1) * (ImElasticAmpl0 + Din1)) / twopi;
...
void G4ElasticHadrNucleusHE::DefineHadronValues(G4int Z)
{
  ...
  if (verboseLevel > 2)
  {
    G4cout << "GetHadrValues: Z= " << Z << " iHadr= " << iHadron << " E(GeV)= " << HadrEnergy
           << " sqrS= " << sqrS << " plab= " << hLabMomentum << "  E - m  " << HadrEnergy - hMass
           << G4endl;
  }
  G4double TotN = 0.0;
  ...
  switch (iHadron)
  {
    ...
    case 6:
      ...
      else
      {
        ...
        if (hLabMomentum > 1.4)
        {
          TotN = 33.3 + 15.2 * (hLabMomentum2 - 1.35) / (G4Exp(G4Log(hLabMomentum) * 2.37) + 0.95);
        }
      ...
      if (hLabMomentum >= 2.)
      {
        HadrSlope = 5.44 + 0.88 * logS;
      }
      else if (hLabMomentum >= 0.5)
      {
        HadrSlope = 3.73 * hLabMomentum - 0.37;
      }
      else
      {
        HadrSlope = 1.5;
  ...
  if (verboseLevel > 2)
  {
    G4cout << "HadrTot= " << HadrTot << " HadrSlope= " << HadrSlope << " HadrReIm= " << HadrReIm
           << " DDSect2= " << DDSect2 << " DDSect3= " << DDSect3 << G4endl;
  }
  if (Z != 1) return;
  ...
  if (verboseLevel > 2)
  {
    G4cout << "  HadrVal : Plasb  " << hLabMomentum << "  iHadron  " << iHadron << "  HadrTot  "
           << HadrTot << G4endl;
  }
}
...
G4double G4ElasticHadrNucleusHE::GetFt(G4double Q2)
{
  ...
  if (verboseLevel > 1)
  {
    G4cout << "Old:  Coeff0 Coeff1 Coeff2 " << Coeff0 << "  " << Coeff1 << "  " << Coeff2
           << "  Slope Slope0 Slope1 Slope2 " << HadrSlope << "  " << Slope0 << "  " << Slope1
           << "  " << Slope2 << "  Fdistr " << Fdistr << G4endl;
  }
  return Fdistr;
}
...
G4double G4ElasticHadrNucleusHE::HadronProtonQ2(G4double plab, G4double tmax)
{
  ...
  G4double MaxTR = (plab < BoundaryP[iHadron1]) ? BoundaryTL[iHadron1] : BoundaryTG[iHadron1];

  if (verboseLevel > 1)
  {
    G4cout << "3  GetKin. : iHadron1  " << iHadron1 << "  Bound.P[iHadron1] " << BoundaryP[iHadron1]
           << "  Bound.TL[iHadron1] " << BoundaryTL[iHadron1] << "  Bound.TG[iHadron1] "
           << BoundaryTG[iHadron1] << "  MaxT MaxTR " << tmax << "  " << MaxTR << G4endl;
  }
...
void G4ElasticHadrNucleusHE::InFileName(std::ostringstream& ss, const G4ParticleDefinition* p,
                                        G4int Z)
{
  if (!fDirectory)
  {
...
void G4ElasticHadrNucleusHE::OutFileName(std::ostringstream& ss, const G4ParticleDefinition* p,
                                         G4int Z)
{
