...
G4double G4ElasticHadrNucleusHE::LineF[]  = {0.0};
...
{ 
  ...
  */
  ...
  G4double dQ2 = limitQ2/(G4double)(ONQ2-2);
...
void G4ElasticHadrNucleusHE::InitialiseModel()
{
  ...
  for(G4int i=0; i<2; ++i) {
    ...
    iHadrCode = HadronCode[i]; 
    iHadron   = HadronType[i];
    iHadron1  = HadronType1[i];
					 ...
					 G4int iZ, G4int A)
{
  ...
  if(Z==1) {
    ...
    if(!ElD1) { FillData(p, idx, Z); }
    ElD1 = SetOfElasticData[idx][Z];
    if(!ElD1) { return 0.0; }
                                      ...
                                      G4int idx, G4int Z)
{
  ...
  if(!SetOfElasticData[idx][Z]) {
    ...
    auto pElD = new G4ElasticData(p, Z, A, Energy);
    SetOfElasticData[idx][Z] = pElD;
    ...
    for(G4int i=0; i<NENERGY; ++i) {
      ...
      FillFq2(A); 
      G4double norm = 1.0/LineF[ONQ2-1];

      for(G4int ii=1; ii<ONQ2-1; ++ii) {
	pElD->TableCrossSec[index+ii] = LineF[ii]*norm;
      }
      pElD->TableCrossSec[index] = 0.0;
      pElD->TableCrossSec[index+ONQ2-1] = 1.0;
    }
...
G4double 
G4ElasticHadrNucleusHE::HadronNucleusQ2_2(G4ElasticData* pElD,
...
void G4ElasticHadrNucleusHE::FillFq2(G4int A) 
{
  ...
  for(G4int ii=1; ii<ONQ2-1; ++ii) {
    ...
    totSum += curSum;
    LineF[ii] = totSum;
    if(totSum*1.e-6 > curSum || Q2l > Q2max) {
      for(G4int kk=ii+1; kk<ONQ2-1; ++kk) { 
	LineF[kk] = totSum;
      }
      break;
    }
  ...
  if (verboseLevel>1) {
    G4cout << "### FillFq2 done curQ2= " << curQ2 << " Q2max= "<< Q2max 
           << " sumG= " << LineF[ONQ2-2] << "  totSum= " << totSum 
	   << G4endl;
  }
}
  ...
  for(G4int i1 = 1; i1<= A; ++i1) ////++++++++++  i1
    {
	  ...
	  for(G4int j2=0; j2<= i2; ++j2) ////+++++++++ j2
	      ...
	      for(G4int j1=0; j1<=i1; ++j1) ////++++++++ j1
		{
		  ...
		  N4    *= (-NN2);
		  Prod3 += N4*exp1*exp2*(1-G4Exp(-Q2*dddd))*ABinom[i1][j1]/dddd;
		}                                   // j1
	      Prod2 += Prod3*N5*ABinom[i2][j2];
	    }                                      // j2
