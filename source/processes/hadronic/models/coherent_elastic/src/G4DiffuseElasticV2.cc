...
G4DiffuseElasticV2::G4DiffuseElasticV2() : G4HadronElastic("DiffuseElasticV2"), fParticle(0)
{
...
G4double G4DiffuseElasticV2::GetDiffElasticSumProbA(G4double theta)
{
...
G4double G4DiffuseElasticV2::GetIntegrandFunction(G4double alpha)
{
  G4double result;
...
G4double G4DiffuseElasticV2::SampleTableThetaCMS(const G4ParticleDefinition* particle,
...
void G4DiffuseElasticV2::InitialiseOnFly(G4double Z, G4double A)
{
  ...
  if (verboseLevel > 0)
  {
    G4cout << "G4DiffuseElasticV2::InitialiseOnFly() the element with Z = " << Z
           << "; and A = " << A << G4endl;
  }
  fElementNumberVector.push_back(fAtomicNumber);
...
G4double G4DiffuseElasticV2::GetScatteringAngle(G4int iMomentum, unsigned long iAngle,
                                                G4double position)
{
...
G4double G4DiffuseElasticV2::ThetaCMStoThetaLab(const G4DynamicParticle* aParticle, G4double tmass,
                                                G4double thetaCMS)
{
...
G4double G4DiffuseElasticV2::ThetaLabToThetaCMS(const G4DynamicParticle* aParticle, G4double tmass,
                                                G4double thetaLab)
{
  const G4ParticleDefinition* theParticle = aParticle->GetDefinition();
  G4double m1 = theParticle->GetPDGMass();
