...
G4DiffuseElastic::G4DiffuseElastic() : G4HadronElastic("DiffuseElastic"), fParticle(0)
{
...
void G4DiffuseElastic::Initialise()
{
  ...
  for (jEl = 0; jEl < numOfEl; ++jEl)  // application element loop
  {
    ...
    if (verboseLevel > 0)
    {
      G4cout << "G4DiffuseElastic::Initialise() the element: " << (*theElementTable)[jEl]->GetName()
             << G4endl;
    }
...
G4double G4DiffuseElastic::GetDiffuseElasticXsc(const G4ParticleDefinition* particle,
                                                G4double theta, G4double momentum, G4double A)
{
  fParticle = particle;
...
G4double G4DiffuseElastic::GetInvElasticXsc(const G4ParticleDefinition* particle, G4double tMand,
                                            G4double plab, G4double A, G4double Z)
{
...
G4double G4DiffuseElastic::GetInvElasticSumXsc(const G4ParticleDefinition* particle, G4double tMand,
                                               G4double plab, G4double A, G4double Z)
{
  G4double m1 = particle->GetPDGMass();
...
G4double G4DiffuseElastic::GetDiffElasticProb(  // G4ParticleDefinition* particle,
  G4double theta
  //  G4double momentum,
  // G4double A
)
...
G4double G4DiffuseElastic::GetDiffElasticSumProb(  // G4ParticleDefinition* particle,
  G4double theta
  //  G4double momentum,
  // G4double A
)
...
G4double G4DiffuseElastic::GetIntegrandFunction(G4double alpha)
{
  G4double result;
...
G4double G4DiffuseElastic::IntegralElasticProb(const G4ParticleDefinition* particle, G4double theta,
...
G4double G4DiffuseElastic::SampleThetaCMS(const G4ParticleDefinition* particle, G4double momentum,
                                          G4double A)
{
  G4int i, iMax = 100;
...
G4double G4DiffuseElastic::SampleTableThetaCMS(const G4ParticleDefinition* particle,
...
void G4DiffuseElastic::InitialiseOnFly(G4double Z, G4double A)
{
  ...
  if (verboseLevel > 0)
  {
    G4cout << "G4DiffuseElastic::InitialiseOnFly() the element with Z = " << Z << "; and A = " << A
           << G4endl;
  }
  fElementNumberVector.push_back(fAtomicNumber);
...
void G4DiffuseElastic::BuildAngleTable()
{
  ...
  for (i = 0; i < fEnergyBin; i++)
  {
    ...
    for (j = fAngleBin - 1; j >= 1; j--)
    {
      ...
      //      G4cout<<"j-1 = "<<j-1<<"; alpha2 = "<<alpha2 << " delta "<< delta <<"; sum =
      //      "<<sum<<G4endl;
    }
...
G4double G4DiffuseElastic::GetScatteringAngle(G4int iMomentum, G4int iAngle, G4double position)
{
...
G4double G4DiffuseElastic::SampleThetaLab(const G4HadProjectile* aParticle, G4double tmass,
                                          G4double A)
{
  ...
  if (t > tmax)
  {
    if (verboseLevel > 0)
    {
      G4cout << "G4DiffuseElastic:WARNING: A = " << A << " mom(GeV)= " << plab / GeV
...
G4double G4DiffuseElastic::ThetaCMStoThetaLab(const G4DynamicParticle* aParticle, G4double tmass,
                                              G4double thetaCMS)
{
  const G4ParticleDefinition* theParticle = aParticle->GetDefinition();
  G4double m1 = theParticle->GetPDGMass();
...
G4double G4DiffuseElastic::ThetaLabToThetaCMS(const G4DynamicParticle* aParticle, G4double tmass,
                                              G4double thetaLab)
{
  const G4ParticleDefinition* theParticle = aParticle->GetDefinition();
  G4double m1 = theParticle->GetPDGMass();
                                      ...
                                      G4double Z, G4double A)
{
  ...
  G4cout << "G4DiffuseElastic::TestAngleTable() init the element with Z = " << Z
         << "; and A = " << A << G4endl;
  ...
  for (j = 1; j < fAngleBin; j++)
  {
    ...
    deltaAG = integral.AdaptiveGauss(this, &G4DiffuseElastic::GetIntegrandFunction, alpha1, alpha2,
                                     epsilon);
