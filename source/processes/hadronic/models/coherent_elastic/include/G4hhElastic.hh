...
#ifndef G4HHELASTIC_HH
#define G4HHELASTIC_HH
...
class G4hhElastic : public G4HadronElastic
{
  public:
    ...
    void BuildTableT(G4ParticleDefinition* target,
                     G4ParticleDefinition* projectile);  // , G4double plab );
...
inline G4bool G4hhElastic::IsApplicable(const G4HadProjectile& projectile, G4Nucleus& nucleus)
{
...
inline void G4hhElastic::SetParametersCMS(G4double plab)
{
  ...
  G4DynamicParticle* theDynamicParticle =
    new G4DynamicParticle(fProjectile, G4ParticleMomentum(0., 0., 1.), Tkin);
  fSigmaTot = fHadrNuclXsc->GetHadronNucleonXscNS(theDynamicParticle, fTarget);

  delete theDynamicParticle;

  fSpp = fMassProj * fMassProj + fMassTarg * fMassTarg
         + 2. * fMassTarg * std::sqrt(plab * plab + fMassProj * fMassProj);
  ...
  if (fMassProj > trMass)  // p,n,pb on p
  {
    ...
    {
      for (i = 0; i < 19; i++)
        if (sCMS <= theNuclNuclData[i][0] * CLHEP::GeV) break;
      if (i == 0) i++;
      if (i == 19) i--;
  ...
  else  // pi, K
  {
    ...
    {
      for (i = 0; i < 8; i++)
        if (sCMS <= thePiKaNuclData[i][0] * CLHEP::GeV) break;
      if (i == 0) i++;
      if (i == 8) i--;
...
inline G4complex G4hhElastic::GetF1qQgG(G4double t)
{
  ...
  G4complex exp13 =
    fBq * std::exp(-(Phi13() + fBeta * fBeta * fLambda + fDelta * fDelta * fEta) * t);
  G4complex exp14 =
    fBq * std::exp(-(Phi14() + fBeta * fBeta * fLambda + fGamma * fGamma * fEta) * t);
  G4complex exp23 =
    fBQ * std::exp(-(Phi23() + fAlpha * fAlpha * fLambda + fDelta * fDelta * fEta) * t);
  G4complex exp24 =
    fBQ * std::exp(-(Phi24() + fAlpha * fAlpha * fLambda + fGamma * fGamma * fEta) * t);
...
inline G4double G4hhElastic::GetdsdtF13qQG(G4double spp, G4double t)
{
  ...
  G4complex exp14 =
    fBqQ * std::exp(-(Phi14() + fBeta * fBeta * fLambda + fGamma * fGamma * fEta) * t);
  G4complex exp24 =
    fBQ * std::exp(-(Phi24() + fAlpha * fAlpha * fLambda + fGamma * fGamma * fEta) * t);
...
inline G4complex G4hhElastic::GetF2qQgG(G4double t)
{
  ...
  G4complex z1324 =
    -(Phi24() + fAlpha * fLambda + fGamma * fEta) * (Phi24() + fAlpha * fLambda + fGamma * fEta);
  ...
  G4complex z1423 =
    -(Phi23() + fAlpha * fLambda + fDelta * fEta) * (Phi24() + fAlpha * fLambda + fDelta * fEta);
  ;
  ...
  res *= 0.25 * k / CLHEP::pi;
  res *= G4complex(0., 1.);
  res *= fBq * fBQ * fSigmaTot * fSigmaTot
         / (8. * CLHEP::pi * CLHEP::hbarc * CLHEP::hbarc);  // or 4. ???
...
inline G4complex G4hhElastic::GetF3qQgG(G4double t)
{
  ...
  G4complex res =
    fBq * fBq * exp1314 + fBQ * fBQ * exp2324 + fBq * fBQ * exp1323 + fBq * fBQ * exp1424;
...
inline void G4hhElastic::CalculateBQ(G4double b1)
{
  ...
  G4cout << "re_x1 = " << real(x1) << "; re_x2 = " << real(x2) << "; re_x3 = " << real(x3)
         << G4endl;
  G4cout << "im_x1 = " << imag(x1) << "; im_x2 = " << imag(x2) << "; im_x3 = " << imag(x3)
         << G4endl;
...
inline G4complex G4hhElastic::GetF3(G4double t)
{
  ...
  res *= G4complex(0., 1.);
  res *=
    std::sqrt(fBq * fBQ) * fSigmaTot * fSigmaTot / (4. * CLHEP::pi * CLHEP::hbarc * CLHEP::hbarc);
