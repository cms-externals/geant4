...
void G4LightTargetCollider::collide(G4InuclParticle* bullet, G4InuclParticle* target,
                                    G4CollisionOutput& globalOutput)
{
  ...
  else if (target->getDefinition() == G4Deuteron::Deuteron())
  {
    if (ke < mP + mN - mD)
    {
      ...
      G4Exception("G4LightTargetCollider::collide()", "HAD_BERT_201", JustWarning,
  ...
  {
    G4Exception("G4LightTargetCollider::collide()", "HAD_BERT_203", FatalException,
                "Scattering from this target not implemented");
  }
...
NucleonPair G4LightTargetCollider::AbsorptionOnDeuteron(G4InuclParticle* bullet)
{
  ...
  {
    G4Exception("G4LightTargetCollider::collide()", "HAD_BERT_204", FatalException,
                "Illegal bullet type");
  }
                                               ...
                                               const G4InuclElementaryParticle& nucleon)
{
  ...
  while (mult > 1)
  {
    ...
    while (generate && itry < itry_max)
    {
      ...
      fsGen.Configure(const_cast<G4InuclElementaryParticle*>(&projectile),
                      const_cast<G4InuclElementaryParticle*>(&nucleon), particle_kinds);
  ...
  if (mult < 2)
  {
    G4Exception("G4LightTargetCollider::SingleNucleonScattering()", "HAD_BERT_202", JustWarning,
                "Failed to generate final state");
