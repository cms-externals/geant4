...
G4EquilibriumEvaporator::G4EquilibriumEvaporator()
  : G4CascadeDeexciteBase("G4EquilibriumEvaporator"), theParaMaker(verboseLevel), QFinterp(XREP)
{
...
void G4EquilibriumEvaporator::deExcite(const G4Fragment& target, G4CollisionOutput& output)
{
  ...
  while (try_again && itry_global < itry_global_max)
  {
    ...
    if (verboseLevel > 2)
    {
      ...
      G4cout << " A " << A << " Z " << Z << " mass " << nuc_mass << " EEXS " << EEXS << G4endl;
    }

    if (explosion(A, Z, EEXS))
    {  // big bang
      if (verboseLevel > 2) G4cout << " big bang in eql step " << itry_global << G4endl;
    ...
    if (EEXS < cut_off_energy)
    {  // Evaporation not possible
      if (verboseLevel > 2)
        G4cout << " no energy for evaporation in eql step " << itry_global << G4endl;
    ...
    for (i = 0; i < 6; i++)
    {
      ...
      if (goodRemnant(A1[i], Z1[i]))
      {
        ...
        V[i] = coul_coeff * Z * Q[i] * AK[i] / (1.0 + EEXS / E0) / (G4cbrt(A1[i]) + G4cbrt(AN[i]));
        TM[i] = EEXS - QB - V[i] * A / A1[i];
        if (verboseLevel > 3)
        {
          G4cout << " i=" << i << " : QB " << QB << " u " << u[i] << " V " << V[i] << " TM "
                 << TM[i] << G4endl;
        }
      }
    }
    ...
    {  // photon emission chain
      G4double UCR0 = 2.5 + 150.0 / A;
      G4double T00 = 1.0 / (std::sqrt(u1 / UCR0) - 1.25 / UCR0);

      if (verboseLevel > 3) G4cout << " UCR0 " << UCR0 << " T00 " << T00 << G4endl;
      ...
      while (EEXS > cut_off_energy && try_again)
      {
        ...
        if (verboseLevel > 3) G4cout << " T04 " << T04 << " FMAX (EEXS^4) " << FMAX << G4endl;
        ...
        {  // Valid evaporate
          ...
          if (verboseLevel > 3)
          {
            G4cout << " nucleus   px " << PEX.px() << " py " << PEX.py() << " pz " << PEX.pz()
                   << " E " << PEX.e() << G4endl << " evaporate px " << mom.px() << " py "
                   << mom.py() << " pz " << mom.pz() << " E " << mom.e() << G4endl;
          }
          ...
          output.addOutgoingParticle(
            G4InuclElementaryParticle(mom, photon, G4InuclParticle::Equilib));

          if (verboseLevel > 3) G4cout << output.getOutgoingParticles().back() << G4endl;
    ...
    {  // if (prob_sum < prob_cut_off)
      ...
      if (icase < 6)
      {  // particle or light nuclei escape
        if (verboseLevel > 2)
        {
        ...
        while (itry1 < itry_max && bad)
        {
          ...
          {  // real escape
            ...
            if (verboseLevel > 2)
              G4cout << " escape itry1 " << itry1 << " icase " << icase << " S (MeV) " << S
                     << G4endl;
            ...
            {  // particle escape
              G4int ptype = 2 - icase;
              if (verboseLevel > 2) G4cout << " particle " << ptype << " escape" << G4endl;

              // generate particle momentum
              G4double mass = G4InuclElementaryParticle::getParticleMass(ptype);
              ...
              G4double mass_new = G4InuclNuclei::getNucleiMass(A1[icase], Z1[icase]);
              ...
              output.addOutgoingParticle(
                G4InuclElementaryParticle(mom, ptype, G4InuclParticle::Equilib));
            ...
            {  // if (icase < 2)
              if (verboseLevel > 2)
              {
                G4cout << " nucleus A " << AN[icase] << " Z " << Q[icase] << " escape icase "
                       << icase << G4endl;
              }

              G4double mass = G4InuclNuclei::getNucleiMass(AN[icase], Q[icase]);
              ...
              output.addOutgoingNucleus(
                G4InuclNuclei(mom, AN[icase], Q[icase], 0. * GeV, G4InuclParticle::Equilib));

              if (verboseLevel > 3) G4cout << output.getOutgoingNuclei().back() << G4endl;
      ...
      {  // if (icase < 6)
        if (verboseLevel > 2)
        {
          G4cout << " fission: A " << A << " Z " << Z << " eexs " << EEXS << " Wn " << W[0]
                 << " Wf " << W[6] << G4endl;
        }
        ...
        if (fission_output.numberOfFragments() == 2)
        {  // fission ok
  ...
  output.addOutgoingNucleus(G4InuclNuclei(pnuc, A, Z, 0., G4InuclParticle::Equilib));

  if (verboseLevel > 3)
  {
    G4cout << " remaining nucleus \n" << output.getOutgoingNuclei().back() << G4endl;
  }
...
G4bool G4EquilibriumEvaporator::explosion(G4int a, G4int z, G4double e) const
{
  if (verboseLevel > 3)
  {
  ...
  G4bool bigb = (!(a >= 12 && z >= 0 && z < 3 * (a - z)) && (e >= be_cut * bindingEnergy(a, z)));
...
G4bool G4EquilibriumEvaporator::goodRemnant(G4int a, G4int z) const
{
  if (verboseLevel > 3)
  {
...
G4double G4EquilibriumEvaporator::getQF(G4double x, G4double x2, G4int a, G4int /*z*/,
                                        G4double) const
{
  if (verboseLevel > 3)
  {
...
G4double G4EquilibriumEvaporator::getAF(G4double, G4int /*a*/, G4int /*z*/, G4double e) const
{
  if (verboseLevel > 3)
  {
...
G4double G4EquilibriumEvaporator::getPARLEVDEN(G4int /*a*/, G4int /*z*/) const
{
  if (verboseLevel > 3)
  {
