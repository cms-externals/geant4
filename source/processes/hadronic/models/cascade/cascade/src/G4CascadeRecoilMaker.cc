...
void G4CascadeRecoilMaker::collide(G4InuclParticle* bullet, G4InuclParticle* target,
                                   G4CollisionOutput& output)
{
  if (verboseLevel > 1) G4cout << " >>> G4CascadeRecoilMaker::collide" << G4endl;
...
{
  if (verboseLevel > 1) G4cout << " >>> G4CascadeRecoilMaker::collide(<EP>,<CP>)" << G4endl;
...
void G4CascadeRecoilMaker::fillRecoil()
{
  ...
  if (verboseLevel > 2)
  {
    G4cout << "  recoil px " << recoilMomentum.px() << " py " << recoilMomentum.py() << " pz "
           << recoilMomentum.pz() << " E " << recoilMomentum.e() << " baryon " << recoilA
...
G4InuclNuclei* G4CascadeRecoilMaker::makeRecoilNuclei(G4InuclParticle::Model model)
{
  if (verboseLevel > 1) G4cout << " >>> G4CascadeRecoilMaker::makeRecoilNuclei" << G4endl;
  ...
  theRecoilNuclei.fill(recoilMomentum, recoilA, recoilZ, excitationEnergy, model);
...
{
  if (verboseLevel > 1) G4cout << " >>> G4CascadeRecoilMaker::makeRecoilFragment" << G4endl;
  ...
  G4double fragMass = G4InuclNuclei::getNucleiMass(recoilA, recoilZ) + excitationEnergy / GeV;

  G4LorentzVector fragMom;
  fragMom.setVectM(recoilMomentum.vect(), fragMass);
  ...
  G4int nexcit = (theExcitons.protonQuasiParticles + theExcitons.neutronQuasiParticles);
  theRecoilFragment.SetNumberOfExcitedParticle(nexcit, theExcitons.protonQuasiParticles);
...
G4bool G4CascadeRecoilMaker::wholeEvent() const
{
  if (verboseLevel > 2)
  {
    G4cout << " >>> G4CascadeRecoilMaker::wholeEvent:"
           << " A " << recoilA << " Z " << recoilZ << " P " << recoilMomentum.rho() << " E "
           << recoilMomentum.e() << "\n wholeEvent returns "
           << (recoilA == 0 && recoilZ == 0 && recoilMomentum.rho() < excTolerance / GeV
               && std::abs(recoilMomentum.e()) < excTolerance / GeV)
           << G4endl;
  }

  return (recoilA == 0 && recoilZ == 0 && recoilMomentum.rho() < excTolerance / GeV
          && std::abs(recoilMomentum.e()) < excTolerance / GeV);
}
...
G4bool G4CascadeRecoilMaker::goodNucleus() const
{
  ...
  if (!goodRecoil())
  {
    ...
    {
      if (!goodFragment())
        G4cerr << " goodNucleus: invalid A/Z" << G4endl;
  ...
  if (verboseLevel > 3)
  {
    G4cout << " eexs " << excitationEnergy << " max " << exc_max << " dm " << dm << G4endl;
  }
