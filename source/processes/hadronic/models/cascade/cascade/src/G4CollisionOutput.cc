...
G4CollisionOutput::G4CollisionOutput() : verboseLevel(0), eex_rest(0), on_shell(false)
{
  if (verboseLevel > 1) G4cout << " >>> G4CollisionOutput::G4CollisionOutput" << G4endl;
}
...
{
  return ((index >= 0 && index < numberOfFragments()) ? recoilFragments[index] : emptyFragment);
}
...
void G4CollisionOutput::addOutgoingParticles(const G4ReactionProductVector* rproducts)
{
  ...
  if (verboseLevel)
  {
    G4cout << " >>> G4CollisionOutput::addOutgoingParticles(G4RPVector)" << G4endl;
  }
  ...
  for (j = rproducts->begin(); j != rproducts->end(); ++j)
  {
    ...
    if (verboseLevel > 1)
      G4cout << " Processing " << pd->GetParticleName() << " (" << type << "), momentum " << mom
             << " GeV" << G4endl;
    ...
    else
    {
      ...
      outgoingNuclei.back().fill(mom, pd->GetAtomicMass(), pd->GetAtomicNumber(), 0.,
...
{
  particleIterator pos = std::find(outgoingParticles.begin(), outgoingParticles.end(), particle);
...
{
  nucleiIterator pos = std::find(outgoingNuclei.begin(), outgoingNuclei.end(), nuclei);
...
{
  if (verboseLevel > 1) G4cout << " >>> G4CollisionOutput::getTotalOutputMomentum" << G4endl;
...
{
  if (verboseLevel > 1) G4cout << " >>> G4CollisionOutput::getTotalCharge" << G4endl;
...
{
  if (verboseLevel > 1) G4cout << " >>> G4CollisionOutput::getTotalBaryonNumber" << G4endl;
...
{
  if (verboseLevel > 1) G4cout << " >>> G4CollisionOutput::getTotalStrangeness" << G4endl;
...
{
  os << " Output: " << G4endl << " Outgoing Particles: " << numberOfOutgoingParticles() << G4endl;
...
{
  if (verboseLevel > 1) G4cout << " >>> G4CollisionOutput::boostToLabFrame" << G4endl;
...
{
  if (verboseLevel > 1) G4cout << " >>> G4CollisionOutput::rotateEvent" << G4endl;
...
void G4CollisionOutput::trivialise(G4InuclParticle* bullet, G4InuclParticle* target)
{
  if (verboseLevel > 1) G4cout << " >>> G4CollisionOutput::trivialize" << G4endl;
  ...
  {
    G4InuclElementaryParticle* particle = dynamic_cast<G4InuclElementaryParticle*>(target);
  ...
  {
    G4InuclElementaryParticle* particle = dynamic_cast<G4InuclElementaryParticle*>(bullet);
    outgoingParticles.push_back(*particle);
  }
}
...
{
  if (verboseLevel > 1) G4cout << " >>> G4CollisionOutput::setOnShell" << G4endl;
  ...
  if (verboseLevel > 2)
  {
    G4cout << " bullet momentum = " << ini_mom.e() << ", " << ini_mom.x() << ", " << ini_mom.y()
           << ", " << ini_mom.z() << G4endl;
    G4cout << " target momentum = " << momt.e() << ", " << momt.x() << ", " << momt.y() << ", "
           << momt.z() << G4endl;
    G4cout << " Fstate momentum = " << out_mom.e() << ", " << out_mom.x() << ", " << out_mom.y()
           << ", " << out_mom.z() << G4endl;
  }
  ...
  {
    if (outgoingParticles[i].getDefinition() == G4Electron::Electron()) momt += el4mom;
  }
  ...
  if (verboseLevel > 2)
  {
    ...
    G4cout << " momentum non conservation: " << G4endl << " e " << enc << " p " << pnc << G4endl
  ...
  {
    printCollisionOutput();
    G4cout << " momentum non conservation after (1): " << G4endl << " e " << enc << " p " << pnc
           << G4endl;
  }
  ...
  *****/
  ...
  G4bool tuning_possible =
    (tune_particles.first >= 0 && tune_particles.second >= 0 && mom_ind >= G4LorentzVector::X);
  ...
  if (verboseLevel > 2)
  {
    G4cout << " p1 " << tune_particles.first << " p2 " << tune_particles.second << " ind "
           << mom_ind << G4endl;
  }
  ...
  if (verboseLevel > 2)
  {
    G4cout << " momentum non conservation tuning: " << G4endl << " e " << enc << " p " << pnc
...
{
  if (verboseLevel > 2) G4cout << " >>> G4CollisionOutput::selectPairToTune" << G4endl;
...
G4bool G4CollisionOutput::tuneSelectedPair(G4LorentzVector& mom1, G4LorentzVector& mom2,
                                           G4int mom_ind) const
{
  if (verboseLevel > 2) G4cout << " >>> G4CollisionOutput::tuneSelectedPair" << G4endl;
