...
G4NuclWatcher::G4NuclWatcher(G4int z, const std::vector<G4double>& expa,
...
void G4NuclWatcher::print()
{
  ...
  for (std::size_t iz = 0; iz < experAsSize; ++iz)
  {
    ...
    for (std::size_t i = 0; i < simulatedAsSize && !found; ++i)
    {
      if (std::fabs(simulated_as[i] - a) < small)
      {
        ...
        G4double rat_err = std::sqrt(simulated_errors[i] * simulated_errors[i]
                                     + exper_err[iz] * exper_err[iz] * rat * rat)
                           / exper_cs[iz];
        average_ratio += rat;
        aver_rat_err += rat_err;

        G4cout << " A " << a << " exp.cs " << exper_cs[iz] << " err " << exper_err[iz] << G4endl
               << " sim. cs " << simulated_cs[i] << " err " << simulated_errors[i] << G4endl
               << " ratio " << rat << " err " << rat_err << G4endl << " simulated production rate "
               << simulated_prob[i] << G4endl;
    ...
    if (found)
      ...
      G4cout << " not found exper.: A " << a << " exp.cs " << exper_cs[iz] << " err "
             << exper_err[iz] << G4endl;
  }

  G4cout << " not found in simulations " << nmatched << G4endl << " not found in exper: " << nused
         << G4endl;
  ...
  for (std::size_t i = 0; i < simulatedAsSize; ++i)
  {
    ...
    if (not_used[i])
      G4cout << " extra simul.: A " << simulated_as[i] << " sim. cs " << simulated_cs[i] << " err "
             << simulated_errors[i] << G4endl;
  ...
  if (matched > 0)
  {
    ...
    G4cout << " matched " << matched << " CHSQ " << std::sqrt(izotop_chsq) / matched << G4endl
           << " raw chsq " << izotop_chsq << G4endl << " average ratio " << average_ratio / matched
           << " err " << aver_rat_err / matched << G4endl << " lhood " << lhood << G4endl;
  }
  else
  {
    izotop_chsq = 0.0;
  ...
  G4cout << " exper. cs " << exp_cs << " err " << exp_cs_err << G4endl << " inucl. cs " << inucl_cs
