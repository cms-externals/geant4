...
G4LorentzConvertor::G4LorentzConvertor(const G4LorentzVector& bmom, G4double bmass,
...
G4LorentzConvertor::G4LorentzConvertor(const G4InuclParticle* bullet, const G4InuclParticle* target)
  : verboseLevel(0), v2(0.), ecm_tot(0.), valong(0.), degenerated(false)
{
  setBullet(bullet);
  setTarget(target);
...
{
  if (verboseLevel > 2) G4cout << " >>> G4LorentzConvertor::toTheCenterOfMass" << G4endl;
...
{
  if (verboseLevel > 2) G4cout << " >>> G4LorentzConvertor::toTheTargetRestFrame" << G4endl;
...
void G4LorentzConvertor::fillKinematics()
{
  ...
  if (degenerated && verboseLevel > 2) G4cout << " degenerated case (already along Z) " << G4endl;

  if (verboseLevel > 3)
  {
    G4cout << " v2 " << v2 << " valong " << valong << " valong*valong " << valong * valong
           << G4endl;
  }
}
...
{
  if (verboseLevel > 2) G4cout << " >>> G4LorentzConvertor::backToTheLab" << G4endl;

  if (verboseLevel > 3)
    G4cout << " at rest: px " << mom.x() << " py " << mom.y() << " pz " << mom.z() << " e "
           << mom.e() << G4endl << " v2 " << v2 << G4endl;

  G4LorentzVector mom1 = mom;
  if (v2 > small) mom1.boost(velocity);

  if (verboseLevel > 3)
    G4cout << " at lab: px " << mom1.x() << " py " << mom1.y() << " pz " << mom1.z() << G4endl;
...
{
  if (verboseLevel > 2) G4cout << " >>> G4LorentzConvertor::getKinEnergyInTheTRS" << G4endl;
...
{
  if (verboseLevel > 2) G4cout << " >>> G4LorentzConvertor::getTRSMomentum" << G4endl;
...
{
  if (verboseLevel > 2) G4cout << " >>> G4LorentzConvertor::rotate(G4LorentzVector)" << G4endl;
  ...
  if (!degenerated)
  {
    if (verboseLevel > 2) G4cout << " rotating to align with reference z axis " << G4endl;

    G4ThreeVector vscm = velocity - valong * scm_direction;
    G4ThreeVector vxcm = scm_direction.cross(velocity);

    if (vscm.mag() > small && vxcm.mag() > small)
    {  // Double check
      if (verboseLevel > 3)
      {
        G4cout << " reference z axis " << scm_direction << " vscm " << vscm << " vxcm " << vxcm
               << G4endl;
      }
    ...
    {
      if (verboseLevel) G4cerr << ">>> G4LorentzVector::rotate zero with !degenerated" << G4endl;
    }
  }

  if (verboseLevel > 3)
  {
    G4cout << " after rotation: px " << mom_rot.x() << " py " << mom_rot.y() << " pz "
           << mom_rot.z() << G4endl;
  }
...
{
  if (verboseLevel > 2)
    G4cout << " >>> G4LorentzConvertor::rotate(G4LorentzVector,G4LorentzVector)" << G4endl;

  if (verboseLevel > 3)
  {
    G4cout << " before rotation: px " << mom.x() << " py " << mom.y() << " pz " << mom.z()
           << G4endl;
  }
  ...
  if (vperp > small)
  {
    if (verboseLevel > 2) G4cout << " rotating to align with first z axis " << G4endl;

    G4ThreeVector vmom1 = velocity - pv * mom1_dir;
    G4ThreeVector vxm1 = mom1_dir.cross(velocity);

    if (vmom1.mag() > small && vxm1.mag() > small)
    {  // Double check
      if (verboseLevel > 3)
      {
        G4cout << " first z axis " << mom1_dir << G4endl << " vmom1 " << vmom1 << " vxm1 " << vxm1
               << G4endl;
      }
...
{
  if (verboseLevel > 2) G4cout << " >>> G4LorentzConvertor::reflectionNeeded (query)" << G4endl;

  if (verboseLevel > 3)
  {
    G4cout << " v2 = " << v2 << " SCM z = " << scm_momentum.z() << " degenerated? " << degenerated
           << G4endl;
  }
  ...
  if (verboseLevel > 2)
  {
    G4cout << " reflection across XY is"
           << ((v2 >= small && (!degenerated || scm_momentum.z() < 0.0)) ? "" : " NOT") << " needed"
           << G4endl;
  }
...
{
  G4cout << " G4LC bullet: px " << bullet_mom.px() << " py " << bullet_mom.py() << " pz "
         << bullet_mom.pz() << " e " << bullet_mom.e() << " mass " << bullet_mom.m() << G4endl;
}

void G4LorentzConvertor::printTarget() const
{
  G4cout << " G4LC target: px " << target_mom.px() << " py " << target_mom.py() << " pz "
         << target_mom.pz() << " e " << target_mom.e() << " mass " << target_mom.m() << G4endl;
}
