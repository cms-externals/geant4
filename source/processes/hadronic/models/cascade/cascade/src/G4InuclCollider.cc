                              ...
                              G4CollisionOutput& globalOutput)
{
  ...
  { /* Loop checking 08.06.2015 MHK */
    ...
    // This collider is producing an inelastic final state.  If the cascade
    // only returned the original projectile and an unchanged ground-state
    // target, retry the cascade attempt before de-excitation.
    // Without this early retry, the de-excitation step can return the
    // unchanged target as a recoil fragment, and the later recoil-fragment
    // cleanup removes it.  The remaining projectile-only state violates
    // baryon/charge conservation and is rejected only later by the normal
    // retry logic.
    if (interCase.hadNucleus() && elasticLikeNucleusTrial(output, btype, *ntarget))
    {
      if (verboseLevel > 2)
      {
        G4cout << " InuclCollider -> elastic-like cascade trial; retrying" << G4endl;
      }
      continue;
    }
...
// Identify a cascade trial that ended as the original projectile plus the
// unchanged target nucleus in its ground state.
G4bool G4InuclCollider::elasticLikeNucleusTrial(const G4CollisionOutput& checkOutput,
                                                G4int projectileType, const G4InuclNuclei& target)
{
  if (checkOutput.numberOfOutgoingParticles() != 1) return false;
  if (checkOutput.numberOfOutgoingNuclei() != 0) return false;
  if (checkOutput.numberOfFragments() != 1) return false;

  const G4InuclElementaryParticle& particle = checkOutput.getOutgoingParticles().front();
  const G4Fragment& fragment = checkOutput.getRecoilFragment(0);

  // finishCascade() resets this quasi-elastic recoil excitation to literal zero.
  return (particle.type() == projectileType && fragment.GetA_asInt() == target.getA()
          && fragment.GetZ_asInt() == target.getZ() && fragment.GetExcitationEnergy() == 0.0);
}
