...
class BERTParameters
{
  public:
    ...
    BERTParameters()
    {
      ...
      HDP.SetDefault("BERT_FERMI_SCALE", 1.932 / OLD_RADIUS_UNITS, 1.932 / OLD_RADIUS_UNITS / 2,
                     1.932 / OLD_RADIUS_UNITS * 2);
      HDP.SetDefault("BERT_RADIUS_SCALE", OLD_RADIUS_UNITS, OLD_RADIUS_UNITS / 2,
                     OLD_RADIUS_UNITS * 2);
...
void G4CascadeParameters::Initialize()
{
  ...
  USE_PHASESPACE = (0 != G4CASCADE_USE_PHASESPACE && G4CASCADE_USE_PHASESPACE[0] != '0');
  PIN_ABSORPTION = (G4CASCADE_PIN_ABSORPTION ? strtod(G4CASCADE_PIN_ABSORPTION, 0) : 0.);
  ...
  RADIUS_SCALE =
    (G4NUCMODEL_RAD_SCALE ? strtod(G4NUCMODEL_RAD_SCALE, 0) : (BEST_PAR ? 1.0 : OLD_RADIUS_UNITS));
  if (G4NUCMODEL_RAD_SCALE == 0 && BEST_PAR == 0)
    HDP.DeveloperGet("BERT_RADIUS_SCALE", RADIUS_SCALE);
  RADIUS_SMALL = ((G4NUCMODEL_RAD_SMALL ? strtod(G4NUCMODEL_RAD_SMALL, 0)
                                        : (BEST_PAR ? 1.992 : (8.0 / OLD_RADIUS_UNITS)))
                  * RADIUS_SCALE);
  RADIUS_ALPHA =
    (G4NUCMODEL_RAD_ALPHA ? strtod(G4NUCMODEL_RAD_ALPHA, 0) : (BEST_PAR ? 0.84 : 0.70));
  RADIUS_TRAILING =
    ((G4NUCMODEL_RAD_TRAILING ? strtod(G4NUCMODEL_RAD_TRAILING, 0) : 0.) * RADIUS_SCALE);
  if (G4NUCMODEL_RAD_TRAILING == 0)
    HDP.DeveloperGet("BERT_RAD_TRAILING", RADIUS_TRAILING), RADIUS_TRAILING *= RADIUS_SCALE;
  FERMI_SCALE = ((G4NUCMODEL_FERMI_SCALE ? strtod(G4NUCMODEL_FERMI_SCALE, 0)
                                         : (BEST_PAR ? 0.685 : (1.932 / OLD_RADIUS_UNITS)))
                 * RADIUS_SCALE);
  if (G4NUCMODEL_FERMI_SCALE == 0 && BEST_PAR == 0)
    HDP.DeveloperGet("BERT_FERMI_SCALE", FERMI_SCALE), FERMI_SCALE *= RADIUS_SCALE;
  XSEC_SCALE = (G4NUCMODEL_XSEC_SCALE ? strtod(G4NUCMODEL_XSEC_SCALE, 0) : (BEST_PAR ? 0.1 : 1.0));
...
{
  if (G4CASCADE_VERBOSE) os << "G4CASCADE_VERBOSE = " << G4CASCADE_VERBOSE << endl;
  if (G4CASCADE_CHECK_ECONS) os << "G4CASCADE_CHECK_ECONS = " << G4CASCADE_CHECK_ECONS << endl;
  if (G4CASCADE_USE_PRECOMPOUND)
    os << "G4CASCADE_USE_PRECOMPOUND = " << G4CASCADE_USE_PRECOMPOUND << endl;
  if (G4CASCADE_USE_ABLA) os << "G4CASCADE_USE_ABLA = " << G4CASCADE_USE_ABLA << endl;
  ...
  if (G4CASCADE_SHOW_HISTORY) os << "G4CASCADE_SHOW_HISTORY = " << G4CASCADE_SHOW_HISTORY << endl;
  if (G4CASCADE_USE_3BODYMOM) os << "G4CASCADE_USE_3BODYMOM = " << G4CASCADE_USE_3BODYMOM << endl;
  if (G4CASCADE_USE_PHASESPACE)
    os << "G4CASCADE_USE_PHASESPACE = " << G4CASCADE_USE_PHASESPACE << endl;
  if (G4CASCADE_RANDOM_FILE) os << "G4CASCADE_RANDOM_FILE = " << G4CASCADE_RANDOM_FILE << endl;
  if (G4NUCMODEL_USE_BEST) os << "G4NUCMODEL_USE_BEST = " << G4NUCMODEL_USE_BEST << endl;
  if (G4NUCMODEL_RAD_2PAR) os << "G4NUCMODEL_RAD_2PAR = " << G4NUCMODEL_RAD_2PAR << endl;
  if (G4NUCMODEL_RAD_SCALE) os << "G4NUCMODEL_RAD_SCALE = " << G4NUCMODEL_RAD_SCALE << endl;
  if (G4NUCMODEL_RAD_SMALL) os << "G4NUCMODEL_RAD_SMALL = " << G4NUCMODEL_RAD_SMALL << endl;
  if (G4NUCMODEL_RAD_ALPHA) os << "G4NUCMODEL_RAD_ALPHA = " << G4NUCMODEL_RAD_ALPHA << endl;
  if (G4NUCMODEL_RAD_TRAILING)
    os << "G4NUCMODEL_RAD_TRAILING = " << G4NUCMODEL_RAD_TRAILING << endl;
  if (G4NUCMODEL_FERMI_SCALE) os << "G4NUCMODEL_FERMI_SCALE = " << G4NUCMODEL_FERMI_SCALE << endl;
  if (G4NUCMODEL_XSEC_SCALE) os << "G4NUCMODEL_XSEC_SCALE = " << G4NUCMODEL_XSEC_SCALE << endl;
  if (G4NUCMODEL_GAMMAQD) os << "G4NUCMODEL_GAMMAQD = " << G4NUCMODEL_GAMMAQD << endl;
  if (DPMAX_2CLUSTER) os << "DPMAX_2CLUSTER = " << DPMAX_2CLUSTER << endl;
  if (DPMAX_3CLUSTER) os << "DPMAX_3CLUSTER = " << DPMAX_3CLUSTER << endl;
  if (DPMAX_4CLUSTER) os << "DPMAX_4CLUSTER = " << DPMAX_4CLUSTER << endl;
}
