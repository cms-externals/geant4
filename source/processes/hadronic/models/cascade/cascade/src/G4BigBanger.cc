...
void G4BigBanger::deExcite(const G4Fragment& target, G4CollisionOutput& globalOutput)
{
  ...
  if (verboseLevel > 3)
  {
    ...
    G4cout << " target rest frame: px " << PEXrest.px() << " py " << PEXrest.py() << " pz "
           << PEXrest.pz() << " E " << PEXrest.e() << G4endl;
  }
  ...
  if (particles.empty())
  {  // No bang!  Don't know why...
    G4cerr << " >>> G4BigBanger unable to process fragment " << target << G4endl;
  ...
  if (verboseLevel > 2)
  {
    G4cout << " In SCM: total outgoing momentum " << G4endl << " E " << totscm.e() << " px "
           << totscm.x() << " py " << totscm.y() << " pz " << totscm.z() << G4endl;
    G4cout << " In Lab: mom cons " << G4endl << " E "
           << PEX.e() - totlab.e()  // PEX now includes EEXS
           << " px " << PEX.x() - totlab.x() << " py " << PEX.y() - totlab.y() << " pz "
           << PEX.z() - totlab.z() << G4endl;
  }
...
void G4BigBanger::generateBangInSCM(G4double etot, G4int a, G4int z)
{
  ...
  if (a == 1)
  {  // Special -- bare nucleon doesn't really "explode"
    G4int knd = (z > 0) ? 1 : 2;
  ...
  { /* Loop checking 08.06.2015 MHK */
    ...
    else
    {
      ...
      for (G4int i = 0; i < a - 2; i++)
      {  // All but last two are thrown
        // This is only a three-vector, not a four-vector
      ...
      G4double ct = -0.5
                    * (tot_mod * tot_mod + momModules[a - 2] * momModules[a - 2]
                       - momModules[a - 1] * momModules[a - 1])
                    / tot_mod / momModules[a - 2];
...
void G4BigBanger::generateMomentumModules(G4double etot, G4int a, G4int z)
{
  ...
  if (a > 2)
  {  // For "large" nuclei, energy is distributed
  ...
  {  // Two-body case is special, must be 50%
    xtot = 1.;
