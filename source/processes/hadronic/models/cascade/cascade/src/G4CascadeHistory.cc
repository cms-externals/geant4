...
G4int G4CascadeHistory::AddVertex(G4CascadParticle& cpart, std::vector<G4CascadParticle>& daug)
{
  ...
  if (verboseLevel > 3)
  {
    G4cout << " entry " << id << " " << &theHistory[id] << " got " << theHistory[id].n
           << " daughters:";
...
void G4CascadeHistory::FillDaughters(G4int iEntry, std::vector<G4CascadParticle>& daug)
{
  ...
  if (verboseLevel > 1) G4cout << " >>> G4CascadeHistory::FillDaughters " << iEntry << G4endl;
...
G4int G4CascadeHistory::AddEntry(G4CascadParticle& cpart)
{
  ...
  if (id < size())
  {
    if (verboseLevel > 2) G4cout << " AddEntry updating " << id << " " << &theHistory[id] << G4endl;
...
void G4CascadeHistory::AssignHistoryID(G4CascadParticle& cpart)
{
  ...
  if (verboseLevel > 2)
  {
    G4cout << " >>> G4CascadeHistory::NewHistoryID assigning ID " << size() << G4endl;
  }
...
void G4CascadeHistory::Print(std::ostream& os) const
{
  ...
  os << " Cascade structure: vertices, (-O-) exciton, (***) outgoing" << std::endl;
...
void G4CascadeHistory::PrintEntry(std::ostream& os, G4int iEntry) const
{
  ...
  os << cpart.getParticle().getDefinition()->GetParticleName() << " p " << cpart.getMomentum()
     << " (cosTh " << cpart.getMomentum().vect().unit().z() << ")"
     << " @ " << cpart.getPosition() << " zone " << cpart.getCurrentZone();
...
const char* G4CascadeHistory::GuessTarget(const G4CascadeHistory::HistoryEntry& entry) const
{
  ...
  for (G4int i = 0; i < entry.n; i++)
  {
    ...
    if (verboseLevel > 3) G4cout << "cdaug " << i << " ID " << entry.dId[i] << ": " << cdaug;
  ...
  if (verboseLevel > 2)
  {
    G4cout << " ERROR identifying target: deltaB " << targetB << " deltaQ " << targetQ << " from\n"
           << cpart << " to" << G4endl;
