...
void G4Fissioner::deExcite(const G4Fragment& target, G4CollisionOutput& output)
{
  ...
  if (verboseLevel > 1) G4cout << " Fissioner input\n" << target << G4endl;
  ...
  G4FissionConfiguration config = fissionStore.generateConfiguration(ALMA, G4UniformRand());
...
G4double G4Fissioner::getC2(G4int A1, G4int A2, G4double X3, G4double X4, G4double R12) const
{
  ...
  G4double C2 = 124.57 * (1.0 / A1 + 1.0 / A2) + 0.78 * (X3 + X4)
                - 176.9 * ((X3 * X3 * X3 * X3) + (X4 * X4 * X4 * X4))
                + 219.36 * (1.0 / (A1 * A1) + 1.0 / (A2 * A2)) - 1.108 / R12;

  return C2;
}

G4double G4Fissioner::getZopt(G4int A1, G4int A2, G4int ZT, G4double X3, G4double X4,
                              G4double R12) const
{
  if (verboseLevel > 3)
  {
  ...
  G4double Zopt =
    (87.7 * (X4 - X3) * (1.0 - 1.25 * (X4 + X3))
     + ZT
         * ((124.57 / A2 + 0.78 * X4 - 176.9 * (X4 * X4 * X4 * X4) + 219.36 / (A2 * A2))
            - 0.554 / R12))
...
void G4Fissioner::potentialMinimization(G4double& VP, G4double (&ED)[2], G4double& VC, G4int AF,
                                        G4int AS, G4int ZF, G4int ZS, G4double AL1[2],
                                        G4double BET1[2], G4double& R12) const
{
  if (verboseLevel > 3)
  {
    ...
    for (i = 0; i < 2; i++)
    {
      for (G4int j = 0; j < 2; j++)
      {
        ...
        AA[i][j] = R3 * RBE[i] * RBE[j]
                   - R2 * (-0.6 * (X1[i] * SAL[j] + X1[j] * SAL[i]) + SAL[i] * SAL[j] * Y4)
                   + DEL * C[i] + DEL1 * DX1;
        G4int i1 = i + 2;
        G4int j1 = j + 2;
        AA[i1][j1] = R3 * RBE[i] * RBE[j]
                     - R2 * (0.857 * (X2[i] * SBE[j] + X2[j] * SBE[i]) + SBE[i] * SBE[j] * Y4)
                     + DEL * F[i] + DEL1 * DX2;
        AA[i][j1] = R3 * RAL[i] * RBE[j]
                    - R2
                        * (0.857 * (X2[j] * SAL[i] - 0.6 * X1[i] * SBE[j]) + SBE[j] * SAL[i] * Y4
                           - 0.257 * R[i] * Y3 * DEL1);
        AA[j1][i] = AA[i][j1];
      };
    };
  ...
  {
    if (itry == itry_max)
      G4cout << " maximal number of iterations in potentialMinimization " << G4endl << " A1 " << AF
             << " Z1 " << ZF << G4endl;
  };
