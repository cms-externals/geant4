...
{
  if (verboseLevel) G4cout << " >>> G4CascadeCoalescence::FindClusters()" << G4endl;
...
{
  if (verboseLevel) G4cout << " >>> G4CascadeCoalescence::selectCandidates()" << G4endl;
  ...
  if (verboseLevel > 1)
  {
    G4cout << " Found " << allClusters.size() << " candidates, using " << usedNucleons.size()
           << " nucleons" << G4endl;
  }
}
...
{
  if (nucleonUsed(idx1) || nucleonUsed(idx2) || nucleonUsed(idx3) || nucleonUsed(idx4)) return;
...
{
  if (verboseLevel) G4cout << " >>> G4CascadeCoalescence::createNuclei()" << G4endl;
  ...
  for (size_t i = 0; i < allClusters.size(); i++)
  {
    ...
    if (makeLightIon(cand))
    {  // Success, put ion in output
...
{
  if (verboseLevel > 1) G4cout << " >>> G4CascadeCoalescence::removeNucleons()" << G4endl;
...
bool G4CascadeCoalescence::makeLightIon(const ClusterCandidate& aCluster)
{
  ...
  thisLightIon.fill(getClusterMomentum(aCluster), A, Z, 0., G4InuclParticle::Coalescence);
...
void G4CascadeCoalescence::reportArgs(const G4String& name, const ClusterCandidate& aCluster) const
{
  G4cout << " >>> G4CascadeCoalescence::" << name << " ";
  std::copy(aCluster.begin(), aCluster.end(), std::ostream_iterator<size_t>(G4cout, " "));
  G4cout << G4endl;
