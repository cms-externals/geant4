...
const G4double kebins[] = {0.0,  0.0024, 0.0032, 0.0042, 0.0056, 0.0075, 0.01, 0.024, 0.075, 0.1,
...
G4NucleiModel::G4NucleiModel()
  : verboseLevel(0),
    ...
    neutronNumber(0),
    protonNumber(0),
    neutronNumberCurrent(0),
    protonNumberCurrent(0),
    current_nucl1(0),
    current_nucl2(0),
    dinucleonDensityScale(1.0),
    gammaQDinterp(kebins),
                          ...
                          const std::vector<G4ThreeVector>* hitPoints)
{
  ...
  if (!hitPoints || !hitPoints->empty())
    collisionPts.clear();
  else
    collisionPts = *hitPoints;
}
...
void G4NucleiModel::generateModel(G4int a, G4int z)
{
  if (verboseLevel)
  {
    G4cout << " >>> G4NucleiModel::generateModel A " << a << " Z " << z << G4endl;
  }
  ...
  if (verboseLevel > 3)
  {
    G4cout << "  crossSectionUnits = " << crossSectionUnits << G4endl
           << "  radiusUnits = " << radiusUnits << G4endl << "  skinDepth = " << skinDepth << G4endl
           << "  radiusScale = " << radiusScale << G4endl << "  radiusScale2 = " << radiusScale2
           << G4endl << "  radiusForSmall = " << radiusForSmall << G4endl
  ...
  if (A > 4)
    nuclearRadius = radiusScale * G4cbrt(A) + radiusScale2 / G4cbrt(A);
  else
    nuclearRadius = radiusForSmall * (A == 4 ? radScaleAlpha : 1.);
...
{
  if (verboseLevel > 1) G4cout << " >>> G4NucleiModel::fillBindingEnergies" << G4endl;
...
{
  if (verboseLevel > 1) G4cout << " >>> G4NucleiModel::fillZoneRadii" << G4endl;
  ...
  {  // Small nuclei have Gaussian potential
  ...
  {  // Intermediate nuclei have three-zone W-S
    ur[0] = -skinRatio;
    for (G4int i = 0; i < number_of_zones; i++)
    {
  ...
  {  // Heavy nuclei have six-zone W-S
    ur[0] = -skinRatio;
    for (G4int i = 0; i < number_of_zones; i++)
    {
...
{
  if (verboseLevel > 1) G4cout << " >>> G4NucleiModel::fillZoneVolumes" << G4endl;

  G4double tot_vol = 0.;  // Return value omitting 4pi/3 for sphere!

  if (A < 5)
  {  // Light ions are treated as simple balls
    v[0] = v1[0] = 1.;
...
{
  if (verboseLevel > 1) G4cout << " >>> G4NucleiModel::fillZoneVolumes(" << type << ")" << G4endl;
  ...
  rod.clear();
  rod.reserve(number_of_zones);
  pf.clear();
  pf.reserve(number_of_zones);
  vz.clear();
  vz.reserve(number_of_zones);
                                               ...
                                               G4double nuclearRadius) const
{
  ...
  return skinDepth3
         * (fun + skinRatio * skinRatio * G4Log((1.0 + G4Exp(-r1)) / (1.0 + G4Exp(-r2))));
}

// Zone integral of Gaussian density function
G4double G4NucleiModel::zoneIntegralGaussian(G4double r1, G4double r2, G4double nucRad) const
{
  if (verboseLevel > 1)
  {
...
void G4NucleiModel::printModel() const
{
  ...
  G4cout << " nuclei model for A " << A << " Z " << Z << G4endl << " proton binding energy "
         << binding_energies[0] << " neutron binding energy " << binding_energies[1] << G4endl
         << " Nuclei radius " << nuclei_radius << " volume " << nuclei_volume << " number of zones "
         << number_of_zones << G4endl;

  for (G4int i = 0; i < number_of_zones; i++)
    G4cout << " zone " << i + 1 << " radius " << zone_radii[i] << " volume " << zone_volumes[i]
           << G4endl << " protons: density " << getDensity(1, i) << " PF " << getFermiMomentum(1, i)
           << " VP " << getPotential(1, i) << G4endl << " neutrons: density " << getDensity(2, i)
           << " PF " << getFermiMomentum(2, i) << " VP " << getPotential(2, i) << G4endl
...
G4LorentzVector G4NucleiModel::generateNucleonMomentum(G4int type, G4int zone) const
{
...
G4InuclElementaryParticle G4NucleiModel::generateNucleon(G4int type, G4int zone) const
{
  if (verboseLevel > 1)
  {
...
G4InuclElementaryParticle G4NucleiModel::generateQuasiDeuteron(G4int type1, G4int type2,
                                                               G4int zone) const
{
  if (verboseLevel > 1)
  {
  ...
  G4int dtype = 0;
  if (type1 * type2 == pro * pro)
    dtype = 111;
  else if (type1 * type2 == pro * neu)
    dtype = 112;
  else if (type1 * type2 == neu * neu)
    dtype = 122;
...
void G4NucleiModel::generateInteractionPartners(G4CascadParticle& cparticle)
{
  ...
  if (path < -small)
  {  // something wrong
    if (verboseLevel) G4cerr << " generateInteractionPartners-> negative path length" << G4endl;
    return;
  }

  if (std::fabs(path) < small)
  {  // Not moving, or just at boundary
    if (cparticle.getMomentum().vect().mag() > small)
    {
      if (verboseLevel > 3) G4cout << " generateInteractionPartners-> zero path" << G4endl;
  ...
  for (G4int ip = 1; ip < 3; ip++)
  {
    ...
    if (path < small || spath < path)
    {
      if (verboseLevel > 3)
      {
        G4cout << " adding partner[" << thePartners.size() << "]: " << particle << G4endl;
      }
  ...
  if (useQuasiDeuteron(cparticle.getParticle().type()))
  {
    if (verboseLevel > 2)
    {
      G4cout << " trying quasi-deuterons with bullet: " << cparticle.getParticle() << G4endl;
    }
    ...
    if (protonNumberCurrent >= 2 && ptype != pip)
    {
      ...
      if (verboseLevel > 2) G4cout << " ptype=" << ptype << " using pp target\n" << ppd << G4endl;
    ...
    if (protonNumberCurrent >= 1 && neutronNumberCurrent >= 1)
    {
      ...
      if (verboseLevel > 2) G4cout << " ptype=" << ptype << " using np target\n" << npd << G4endl;
    ...
    if (neutronNumberCurrent >= 2 && ptype != pim && ptype != mum)
    {
      ...
      if (verboseLevel > 2) G4cout << " ptype=" << ptype << " using nn target\n" << nnd << G4endl;
    ...
    {  // Must have absorption cross-section
      ...
      {  // choose the qdeutron
        ...
        for (std::size_t i = 0; i < qdeutrons.size(); i++)
        {
          ...
          if (sl < as)
          {
            if (verboseLevel > 2) G4cout << " deut type " << qdeutrons[i] << G4endl;
  ...
  if (verboseLevel > 2)
  {
    G4cout << " after deuterons " << thePartners.size() << " partners" << G4endl;
  }

  if (thePartners.size() > 1)
...
{
  if (verboseLevel > 1) G4cout << " >>> G4NucleiModel::generateParticleFate" << G4endl;
  ...
  if (thePartners.empty())
  {  // smth. is wrong -> needs special treatment
    if (verboseLevel)
      G4cerr << " generateParticleFate-> got empty interaction-partners list " << G4endl;
    return;
  }
  ...
  if (npart == 1)
  {  // cparticle is on the next zone entry
    if (verboseLevel > 1) G4cout << " no interactions; moving to next zone" << G4endl;
  ...
  if (verboseLevel > 1) G4cout << " processing " << npart - 1 << " possible interactions" << G4endl;
  ...
  for (std::size_t i = 0; i < npart - 1; ++i)
  {  // Last item is a total-path placeholder
    ...
    if (verboseLevel > 3)
    {
      ...
      G4cout << " target " << target.type() << " bullet " << bullet.type() << G4endl;
    }
    ...
    std::vector<G4InuclElementaryParticle>& outgoing_particles = EPCoutput.getOutgoingParticles();
    ...
    std::sort(outgoing_particles.begin(), outgoing_particles.end(), G4ParticleLargerBeta());

    if (verboseLevel > 2)
      G4cout << " adding " << outgoing_particles.size() << " output particles" << G4endl;
    ...
    {
      outgoing_cparticles.push_back(
        G4CascadParticle(outgoing_particles[ip], new_position, zone, 0.0, nextGen));
    }
...
#ifdef G4CASCADE_DEBUG_CHARGE
    {
      ...
      G4cout << " multiplicity " << outgoing_particles.size() << " bul type " << bullet.type()
             << " targ type " << target.type() << "\n initial charge "
             << bullet.getCharge() + target.getCharge() << " out charge " << out_charge << G4endl;
    }
#endif

    if (verboseLevel > 2) G4cout << " partner type " << target.type() << G4endl;
...
G4bool G4NucleiModel::passFermi(const std::vector<G4InuclElementaryParticle>& particles, G4int zone)
{
  ...
  for (G4int i = 0; i < G4int(particles.size()); i++)
  {
    ...
    if (verboseLevel > 2) G4cout << " type " << type << " p " << mom << " pf " << pfermi << G4endl;
...
{
  if (verboseLevel > 1) G4cout << " >>> G4NucleiModel::passTrailing " << hit_position << G4endl;
...
void G4NucleiModel::boundaryTransition(G4CascadParticle& cparticle)
{
  ...
  if (verboseLevel > 3)
  {
    G4cout << "Potentials for type " << type << " = " << getPotential(type, zone) << " , "
  ...
  if (verboseLevel > 3)
  {
    G4cout << " type " << type << " zone " << zone << " next " << next_zone << " qv " << qv
           << " dv " << dv << G4endl;
  }
  ...
  {  // transition via transverse kinetic energy (allowed for thick walls)  // NAT
  ...
  if (verboseLevel > 3)
  {
    G4cout << " prr " << prr << " delta px " << prr * pos.x() << " py " << prr * pos.y() << " pz "
           << prr * pos.z() << " mag " << std::fabs(prr * r) << G4endl;
  }
...
{
  if (verboseLevel > 1) G4cout << " >>> G4NucleiModel::choosePointAlongTraj" << G4endl;
  ...
  if (verboseLevel > 3) G4cout << " pos " << pos << " phat " << phat << " rhat " << rhat << G4endl;

  G4ThreeVector posout = pos;
  G4double prang = rhat.angle(-phat);

  if (prang < 1e-6)
    posout = -pos;  // Check for radial incidence
  else
  {
  ...
  if (verboseLevel > 3)
  {
    G4cout << " posmid " << posmid << " lenmid " << lenmid << " zoneout " << zoneout << " zonemid "
           << zonemid << " ncross " << ncross << G4endl;
  }
  ...
  for (i = 0; i < ncross / 2; i++)
  {
    ...
    if (verboseLevel > 3)
    {
      G4cout << " i " << i << " iz " << iz << " ds " << ds << " len " << len[i] << G4endl;
    }
  }
  ...
  for (i = 1; i < ncross; i++)
  {
    ...
    if (verboseLevel > 3)
    {
      G4cout << " i " << i << " iz " << iz << " avg.mfp " << 1. / invmfp << " dlen " << dlen
  ...
  if (verboseLevel > 3)
  {
    G4cout << " rand " << rand << " ir " << ir << " frac " << frac << " drand " << drand << G4endl;
  }
  ...
  if (verboseLevel > 2)
  {
    G4cout << " moved particle to zone " << cparticle.getCurrentZone() << " @ " << pos << G4endl;
  }
}
...
G4bool G4NucleiModel::forceFirst(const G4CascadParticle& cparticle) const
{
  return (isProjectile(cparticle)
          && (cparticle.getParticle().isPhoton() || cparticle.getParticle().isMuon()));
}
...
G4bool G4NucleiModel::worthToPropagate(const G4CascadParticle& cparticle) const
{
  ...
  {  // Just reflected -- keep going?
    G4int zone = cparticle.getCurrentZone();
    G4int ip = cparticle.getParticle().type();

    // NOTE:  Temporarily backing out use of potential for non-nucleons
    G4double ekin_cut = (cparticle.getParticle().nucleon()) ? getFermiKinetic(ip, zone)
                                                            : 0.;  //*** getPotential(ip, zone);

    worth = cparticle.getParticle().getKineticEnergy() / ekin_scale > ekin_cut;

    if (verboseLevel > 3)
    {
      G4cout << " type=" << ip << " ekin=" << cparticle.getParticle().getKineticEnergy()
             << " potential=" << ekin_cut << " : worth? " << worth << G4endl;
    }
  }
...
G4double G4NucleiModel::getRatio(G4int ip) const
{
  ...
  switch (ip)
  {
    ...
    case diproton:
      return getRatio(proton) * getRatio(proton);
    case unboundPN:
      return getRatio(proton) * getRatio(neutron);
    case dineutron:
      return getRatio(neutron) * getRatio(neutron);
    default:
      return 0.;
  }
...
void G4NucleiModel::setDinucleonDensityScale()
{
  ...
  {
    num_Naive_QDs +=
      getVolume(zone) * getDensity(proton, zone) * getVolume(zone) * getDensity(neutron, zone);
  }
  ...
  if (verboseLevel > 4)
  {
    ...
    G4cout << " >>> Naive number of quasi-deuterons in nucleus (" << Z << ", " << A
           << ") = " << num_Naive_QDs << G4endl;
    G4cout << " >>> Number of quasi-deuterons expected from Levinger LDA is " << num_LDA_QDs
           << G4endl;
...
G4double G4NucleiModel::getCurrentDensity(G4int ip, G4int izone) const
{
  ...
  if (ip < 100)
    dens = getDensity(ip, izone);
  else
  {  // For dibaryons, remove extra 1/volume term in density product
    switch (ip)
    {
      case diproton:
        dens = getDensity(proton, izone) * getDensity(proton, izone) * dinucleonDensityScale
               * combinatoric_factor;
        break;
      case unboundPN:
        dens = getDensity(proton, izone) * getDensity(neutron, izone) * dinucleonDensityScale;
        break;
      case dineutron:
        dens = getDensity(neutron, izone) * getDensity(neutron, izone) * dinucleonDensityScale
               * combinatoric_factor;
        break;
      default:
        dens = 0.;
...
void G4NucleiModel::initializeCascad(G4InuclNuclei* bullet, G4InuclNuclei* target,
                                     modelLists& output)
{
  if (verboseLevel)
  {
    G4cout << " >>> G4NucleiModel::initializeCascad(bullet,target,output)" << G4endl;
  }
    ...
    if (bullet->getKineticEnergy() / ab > ekin_cut * ben)
    {
      ...
      while (casparticles.size() == 0 && itryg < itry_max)
      {
        ...
        else
        {
            ...
            { /* Loop checking 08.06.2015 MHK */
              ...
              for (i = 0; i < ab - 1; i++)
              {
                ...
                { /* Loop checking 08.06.2015 MHK */
                  ...
                  if (std::sqrt(ss) * G4Exp(-ss) * (1.0 + ss / b) > u && ss < s4max)
                  {
          ...
          if (badco)
          {
            G4cout << " can not generate the nucleons coordinates for a " << ab << G4endl;
        ...
        for (G4int ip = 0; ip < G4int(raw_particles.size()); ip++)
        {
          ...
          G4double det = t0 * t0 + nuclei_radius * nuclei_radius - coordinates[ip].mag2();
          G4double tr = -1.0;
          ...
          {  // cascad particle
            ...
            casparticles.push_back(
              G4CascadParticle(raw_particles[ip], coordinates[ip], number_of_zones, large, 0));
          }
          else
          {
...
G4double G4NucleiModel::inverseMeanFreePath(const G4CascadParticle& cparticle,
                                            const G4InuclElementaryParticle& target, G4int zone)
{
  G4int ptype = cparticle.getParticle().type();
  ...
  G4double csec =
    (ip < 100) ? totalCrossSection(ekin, ptype * ip) : absorptionCrossSection(ekin, ptype);

  if (verboseLevel > 2)
  {
    G4cout << " ip " << ip << " zone " << zone << " ekin " << ekin << " dens "
           << getCurrentDensity(ip, zone) << " csec " << csec << G4endl;
  }
...
G4double G4NucleiModel::generateInteractionLength(const G4CascadParticle& cparticle, G4double path,
                                                  G4double invmfp) const
{
  ...
  if (verboseLevel > 2) G4cout << " mfp " << 1. / invmfp << " pw " << pw << G4endl;
  ...
  if (forceFirst(cparticle) || (G4UniformRand() < pw))
  {
    ...
    if (verboseLevel > 2) G4cout << " spath " << spath << " path " << path << G4endl;
  }
...
G4double G4NucleiModel::absorptionCrossSection(G4double ke, G4int type) const
{
  ...
  {
    G4cerr << "absorptionCrossSection() only valid for incident pions or gammas" << G4endl;
    return 0.;
  }
  ...
  if (type == pionPlus || type == pionMinus || type == pionZero || type == muonMinus)
  {
    if (ke < 0.3)
