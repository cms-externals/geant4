...
G4CascadParticle::G4CascadParticle(const G4InuclElementaryParticle& particle,
                                   const G4ThreeVector& pos, G4int izone, G4double cpath, G4int gen)
  : verboseLevel(G4CascadeParameters::verbose()),
    ...
    historyId(-1)
{
  if (verboseLevel > 3)
  {
    G4cout << " >>> G4CascadParticle::G4CascadParticle "
           << particle.getDefinition()->GetParticleName() << " @ " << pos << G4endl;
  }
}
...
void G4CascadParticle::fill(const G4InuclElementaryParticle& particle, const G4ThreeVector& pos,
                            G4int izone, G4double cpath, G4int gen)
{
...
G4double G4CascadParticle::getPathToTheNextZone(G4double rz_in, G4double rz_out)
{
  if (verboseLevel > 3)
  {
    G4cout << " >>> G4CascadParticle::getPathToTheNextZone rz_in " << rz_in << " rz_out " << rz_out
           << G4endl;
  }
  ...
  if (std::abs(pp) < 1e-9)
  {  // Cut-off for "at rest" is 1 eV momentum
  ...
  if (verboseLevel > 3)
  {
    G4cout << " current_zone " << current_zone << " rr " << rr << " rp " << rp << " pp " << pp
           << " ra " << ra << G4endl;
  }
...
{
  os << " pos " << position << " zone " << current_zone << " current_path " << current_path
     << " reflectionCounter " << reflectionCounter << G4endl << theParticle << G4endl;
}
