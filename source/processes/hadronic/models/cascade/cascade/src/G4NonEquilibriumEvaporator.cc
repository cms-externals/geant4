...
void G4NonEquilibriumEvaporator::deExcite(const G4Fragment& target, G4CollisionOutput& output)
{
  ...
  { /* Loop checking 08.06.2015 MHK */
    if (A >= a_cut && Z >= z_cut && EEXS > eexs_cut)
    {  // ok
      ...
      if (QEX < std::sqrt(2.0 * EG))
      {  // ok
        if (verboseLevel > 3)
          G4cout << " QEX " << QEX << " < sqrt(2*EG) " << std::sqrt(2. * EG) << " NEX " << NEX
                 << G4endl;
        ...
        G4double VP = coul_coeff * Z * AK1 / (G4cbrt(A - 1) + 1.0) / (1.0 + EEXS / E0);
        ...
        {  // ok
          ...
          if (NEX > 1)
          {
            ...
            if (verboseLevel > 3)
              G4cout << " APH " << APH << " APH1 " << APH1 << " ESP " << ESP << G4endl;
            ...
            if (verboseLevel > 3)
              G4cout << " MELE " << MELE << " F1 " << F1 << " F2 " << F2 << G4endl;
            ...
            if (F1 > 0.0 && F2 > 0.0)
            {
              ...
              if (verboseLevel > 3)
              {
                G4cout << " D[0] " << D[0] << " with F " << F << " powN(F,NEX-1) "
                       << theG4Pow->powN(F, NEX - 1) << G4endl;
              }
              ...
              if (D[0] > 0.0)
              {
                if (NEX >= 2)
                {
                  ...
                  if (EMP > eexs_cut) D[2] = D[1] * theG4Pow->powN(EMP / EEXS, NEX) * (1.0 + CPA1);
                  D[1] *= theG4Pow->powN(EMN / EEXS, NEX) * getAL(A);

                  if (verboseLevel > 3)
                  {
                    G4cout << " D[1] " << D[1] << " with powN(EMN/EEXS, NEX) "
                           << theG4Pow->powN(EMN / EEXS, NEX) << G4endl << " D[2] " << D[2]
                           << " with powN(EMP/EEXS, NEX) " << theG4Pow->powN(EMP / EEXS, NEX)
                           << G4endl;
                  }
                  ...
                  if (try_again)
                  {
                    ...
                    if (verboseLevel > 3) G4cout << " D5 " << D5 << " SL " << SL << G4endl;
                    ...
                    if (verboseLevel > 3) G4cout << " got icase " << icase << G4endl;
                  }  // if (try_again)
                }  // if (NEX >= 2)
              }
              else
                try_again = false;  // if (D[0] > 0)
            }
            else
              try_again = false;  // if (F1>0 && F2>0)
          }  // if (NEX > 1)

          if (try_again)
          {
            if (icase > 0)
            {  // N -> N-1 with particle escape
              if (verboseLevel > 3) G4cout << " try_again icase " << icase << G4endl;
              ...
              if (try_again)
              {
                if (icase == 1)
                {  // neutron escape
                  if (verboseLevel > 3) G4cout << " trying neutron escape" << G4endl;
                ...
                {  // proton esape
                  if (verboseLevel > 3) G4cout << " trying proton escape" << G4endl;
                ...
                {
                  if (verboseLevel > 3)
                    G4cout << " ptype " << ptype << " B " << B << " V " << V << G4endl;
                    ...
                    while (itry1 < itry_max && icase > 0 && bad)
                    {
                      ...
                      while (EEXS_new < 0.0 && itry < itry_max)
                      {
                        ...
                        else
                        {
                          ...
                          if (verboseLevel > 3)
                          {
                            G4cout << " R " << R << " QEX2 " << QEX2 << " powA(R, QEX2) " << X
                                   << G4endl;
                          }
                          ...
                          for (G4int i = 0; i < 1000; i++)
                          {
                            ...
                            if (verboseLevel > 3)
                            {
                              G4cout << " NEX " << NEX << " powN(X, NEX) " << theG4Pow->powN(X, NEX)
                                     << G4endl;
                            }
                      ...
                      if (verboseLevel > 3)
                      {
                        G4cout << " nucleus   px " << PEX.px() << " py " << PEX.py() << " pz "
                               << PEX.pz() << " E " << PEX.e() << G4endl << " evaporate px "
                               << mom.px() << " py " << mom.py() << " pz " << mom.pz() << " E "
                               << mom.e() << G4endl;
                      }
                      ...
                      if (verboseLevel > 3) G4cout << " EEXS_new " << EEXS_new << G4endl;
                      ...
                      if (verboseLevel > 3)
                      {
                        G4cout << particle << G4endl << " ppout px " << ppout.px() << " py "
                               << ppout.py() << " pz " << ppout.pz() << " E " << ppout.e()
                               << G4endl;
                      }
          ...
          try_again = false;  // if (EMN > eexs_cut)
      }
      else
        try_again = false;  // if (QEX < sqrg(2*EG)
    }
    else
      try_again = false;  // if (A > a_cut ...
  }  // while (try_again)
