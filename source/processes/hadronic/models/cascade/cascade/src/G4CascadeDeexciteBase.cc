...
const G4Fragment& G4CascadeDeexciteBase::makeFragment(G4int fragA, G4int fragZ, G4double EX)
{
  return makeFragment(zero, fragA, fragZ, EX);
}

const G4Fragment& G4CascadeDeexciteBase::makeFragment(G4LorentzVector mom, G4int fragA, G4int fragZ,
                                                      G4double EX)
{
  if (verboseLevel > 2)
  {
    G4cout << " >>> " << theName << "::makeFragment " << mom << " " << fragA << " " << fragZ << " "
           << EX << G4endl;
  }

  // Adjust four-momentum so that mass is nucleus + excitation
  G4double mass = G4InuclNuclei::getNucleiMass(fragA, fragZ) + EX / GeV;
...
G4bool G4CascadeDeexciteBase::explosion(G4int fragA, G4int fragZ, G4double excitation) const
{
  ...
  return ((fragA <= a_cut || fragZ == 0) && (excitation >= be_cut * bindingEnergy(fragA, fragZ)));
}
...
G4bool
G4CascadeDeexciteBase::validateOutput(const G4Fragment& target,
