...
G4TwoBodyAngularDist::G4TwoBodyAngularDist()
  : gp_npip(new G4GamP2NPipAngDst),
    gp_ppi0(new G4GamP2PPi0AngDst),
    ppAngDst(new G4PP2PPAngDst),
    npAngDst(new G4NP2NPAngDst),
    nnAngDst(new G4NuclNuclAngDst),
    pi0pAngDst(new G4Pi0P2Pi0PAngDst),
    pipCXAngDst(new G4PimP2Pi0NAngDst),
    pimpAngDst(new G4PimP2PimPAngDst),
    pippAngDst(new G4PipP2PipPAngDst),
    qxAngDst(new G4PiNInelasticAngDst),
    hn1AngDst(new G4HadNElastic1AngDst),
    hn2AngDst(new G4HadNElastic2AngDst),
    gnAngDst(new G4GammaNuclAngDst),
    hn3BodyDst(new G4HadNucl3BodyAngDst),
...
const G4VTwoBodyAngDst* G4TwoBodyAngularDist::ChooseDist(G4int is, G4int fs, G4int kw) const
{
  ...
  if ((is == gam * pro && fs == pro * pi0) || (is == gam * neu && fs == neu * pi0))
  {
    return gp_ppi0;
  }

  // gamma-nucleon charge exchange
  if ((is == gam * pro && fs == neu * pip) || (is == gam * neu && fs == pro * pim))
  {
    return gp_npip;
  }
  ...
  // hyperon-nucleon
  if (is == pro * lam || is == pro * sp || is == pro * s0 || is == pro * sm || is == pro * xi0
      || is == pro * xim || is == pro * om || is == neu * lam || is == neu * sp || is == neu * s0
      || is == neu * sm || is == neu * xi0 || is == neu * xim || is == neu * om)
  {
    return nnAngDst;
  }
  ...
  if (is == pro * gam || is == pro * kpl || is == pro * k0b || is == neu * gam || is == neu * kmi
      || is == neu * k0)
  {
    return hn1AngDst;
  }
  ...
  // k-p, k0bn, k+n, or k0p
  if (is == pro * kmi || is == pro * k0 || is == neu * kpl || is == neu * k0b)
  {
    return hn2AngDst;
  }
