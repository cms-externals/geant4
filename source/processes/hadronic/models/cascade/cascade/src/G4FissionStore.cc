...
{
  if (verboseLevel > 1) G4cout << " >>> G4FissionStore::G4FissionStore" << G4endl;
}

void G4FissionStore::addConfig(G4double a, G4double z, G4double ez, G4double ek, G4double ev)
{
...
G4FissionConfiguration G4FissionStore::generateConfiguration(G4double amax, G4double rand) const
{
  if (verboseLevel > 1) G4cout << " >>> G4FissionStore::generateConfiguration" << G4endl;
  ...
  if (verboseLevel > 3) G4cout << " amax " << amax << " ic " << size() << G4endl;
  ...
  while (configProbs[igen] <= st && igen < size())
    igen++;
