...
G4FragmentVector* G4InuclEvaporation::BreakItUp(const G4Fragment& theNucleus)
{
  ...
  if (verboseLevel > 2)
    G4cout << " G4InuclEvaporation : initial kinematics : boostToLab vector = " << boostToLab
           << G4endl << "                     excitation energy  : " << exitationE << G4endl;

  if (verboseLevel > 2)
  {
    G4cout << "G4InuclEvaporation::BreakItUp >>> A: " << A << " Z: " << Z
           << " exitation E: " << exitationE << " mass: " << mTar / GeV << " GeV" << G4endl;
  };
  ...
  if (!outgoingNuclei.empty())
  {
    ...
    for (i = 1; ifrag != outgoingNuclei.cend(); ++ifrag)
    {
      if (verboseLevel > 2)
      {
        G4cout << " Nuclei fragment: " << i << G4endl;
        ++i;
      }
