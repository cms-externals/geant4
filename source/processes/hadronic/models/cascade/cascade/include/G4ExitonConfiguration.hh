...
class G4ExitonConfiguration
{
  public:
    ...
    G4ExitonConfiguration()
      : protonQuasiParticles(0), neutronQuasiParticles(0), protonHoles(0), neutronHoles(0)
    {}

    G4ExitonConfiguration(G4int qpp, G4int qnp, G4int qph, G4int qnh)
      : protonQuasiParticles(qpp), neutronQuasiParticles(qnp), protonHoles(qph), neutronHoles(qnh)
    {}
    ...
    bool empty() const
    {
      return (protonQuasiParticles == 0 && neutronQuasiParticles == 0 && protonHoles == 0
              && neutronHoles == 0);
    }
    ...
    G4bool operator==(const G4ExitonConfiguration& right) const
    {
      return ((&right == this)
              || (protonQuasiParticles == right.protonQuasiParticles
                  ...
                  && protonHoles == right.protonHoles && neutronHoles == right.neutronHoles));
    }

    G4bool operator!=(const G4ExitonConfiguration& right) const { return !operator==(right); }
