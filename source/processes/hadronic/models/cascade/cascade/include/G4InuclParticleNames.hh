...
inline const char* name(G4int ptype)
{
  return nameLong(ptype);
}

// Classify particle types to reduce if-blocks in client code
inline G4bool isPhoton(G4int ityp)
{
  return (ityp == photon);
}

inline G4bool isMuon(G4int ityp)
{
  return (ityp == muonMinus || ityp == muonPlus);
}

inline G4bool isElectron(G4int ityp)
{
  return (ityp == electron || ityp == positron);
}

inline G4bool isNeutrino(G4int ityp)
{
  return (ityp == electronNu || ityp == muonNu || ityp == tauNu || ityp == antiElectronNu
          || ityp == antiMuonNu || ityp == antiTauNu);
}
...
inline G4bool nucleon(G4int ityp)
{
  return (ityp == proton || ityp == neutron);
}
...
inline G4bool quasi_deutron(G4int ityp)
{
  return (ityp > 100);
}
...
{
  return ((ityp == pro || ityp == neu || ityp == lam || ityp == sp || ityp == s0 || ityp == sm
           || ityp == xi0 || ityp == xim || ityp == om)
            ? 1
...
inline G4bool antibaryon(G4int ityp)
{
  return baryon(ityp) < 0;
}

inline G4bool hyperon(G4int ityp)
{
  return (ityp == lam || ityp == sp || ityp == s0 || ityp == sm || ityp == xi0 || ityp == xim
          || ityp == om);
}
}  // namespace G4InuclParticleNames
