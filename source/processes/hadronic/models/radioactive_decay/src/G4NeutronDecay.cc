...
G4NeutronDecay::G4NeutronDecay(const G4ParticleDefinition* theParentNucleus, const G4double& branch,
...
G4DecayProducts* G4NeutronDecay::DecayIt(G4double)
{
  ...
  G4double cmMomentum =
    std::sqrt(transitionQ * (transitionQ + 2. * neutronMass) * (transitionQ + 2. * nucleusMass)
  ...
  G4double KE = std::sqrt(cmMomentum * cmMomentum + neutronMass * neutronMass) - neutronMass;
