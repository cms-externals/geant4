  ...
  : theRadDecay(ptr)
{
  ...
  thresholdForVeryLongDecayTimeCmd =
    new G4UIcmdWithADoubleAndUnit("/process/had/rdm/thresholdForVeryLongDecayTime", this);
  thresholdForVeryLongDecayTimeCmd->SetGuidance(
...
void G4RadioactiveDecayMessenger::SetNewValue(G4UIcommand* command, G4String newValues)
{
  ...
  {
    theRadDecay->SetNucleusLimits(nucleuslimitsCmd->GetNewNucleusLimitsValue(newValues));
  }
  ...
  {
    theRadDecay->SetThresholdForVeryLongDecayTime(
      thresholdForVeryLongDecayTimeCmd->GetNewDoubleValue(newValues));
  }
}
