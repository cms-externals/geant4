...
G4VRadioactiveDecay::G4VRadioactiveDecay(const G4String& processName, const G4double timeThreshold)
  : G4VRestDiscreteProcess(processName, fDecay), fThresholdForVeryLongDecayTime(1.0 * CLHEP::year)
{
  if (GetVerboseLevel() > 1)
  {
    G4cout << "G4VRadioactiveDecay constructor: processName = " << processName << G4endl;
  }
  ...
  if (dirPath.empty())
  {
    ...
    else
    {
      ...
      if (!testFile.is_open())
        G4Exception(
          "G4VRadioactiveDecay()", "HAD_RDM_201", FatalException,
  ...
  // 1. Via C++ interface:
  // G4HadronicParameters::Instance()->SetTimeThresholdForRadioactiveDecay(value)
  ...
  // If both 1. and 2. are specified (at the moment when the G4VRadioactiveDecay constructor is
  // called), then we take the larger value, to be conservative. If, later on (after invoking the
  // G4VRadioactiveDecay constructor) 3. is specified, then this value is used (and the eventual
  // values 1. and/or 2. are ignored).
  G4double timeThresholdBis =
    G4HadronicParameters::Instance()->GetTimeThresholdForRadioactiveDecay();
...
G4bool G4VRadioactiveDecay::IsApplicable(const G4ParticleDefinition& aParticle)
{
  ...
  if (pname == "GenericIon" || pname == "triton")
  {
    return true;
  }
...
G4VParticleChange* G4VRadioactiveDecay::AtRestDoIt(const G4Track& theTrack, const G4Step& theStep)
{
  return DecayIt(theTrack, theStep);
}

G4VParticleChange* G4VRadioactiveDecay::PostStepDoIt(const G4Track& theTrack, const G4Step& theStep)
{
  return DecayIt(theTrack, theStep);
}
...
void G4VRadioactiveDecay::SelectAVolume(const G4String& aVolume)
{
  ...
  if (volume != nullptr)
  {
    ...
    if (GetVerboseLevel() > 0) G4cout << " Radioactive decay applied to " << aVolume << G4endl;
  }
  else
  {
    ...
    G4Exception("G4VRadioactiveDecay::SelectAVolume()", "HAD_RDM_300", JustWarning, ed);
  }
}
  ...
  if (volume != nullptr)
  {
    ...
    if (location != ValidVolumes.cend())
    {
      ...
      if (GetVerboseLevel() > 0)
        G4cout << " G4VRadioactiveDecay::DeselectAVolume: " << aVolume << " is removed from list "
               << G4endl;
    }
    else
    {
      ...
      G4Exception("G4VRadioactiveDecay::DeselectAVolume()", "HAD_RDM_300", JustWarning, ed);
    }
  }
  else
  {
    G4ExceptionDescription ed;
    ed << aVolume << " is not a valid logical volume name.  No action taken." << G4endl;
    G4Exception("G4VRadioactiveDecay::DeselectAVolume()", "HAD_RDM_300", JustWarning, ed);
  }
}
...
void G4VRadioactiveDecay::SelectAllVolumes()
{
  ...
  ValidVolumes.clear();
#ifdef G4VERBOSE
  if (GetVerboseLevel() > 1) G4cout << " RDM Applies to all Volumes" << G4endl;
#endif
...
G4double G4VRadioactiveDecay::GetMeanLifeTime(const G4Track& theTrack, G4ForceCondition*)
{
  ...
  {
    G4cout << "G4VRadioactiveDecay::GetMeanLifeTime() for " << theParticleDef->GetParticleName()
           << G4endl;
...
#ifdef G4VERBOSE
  if (GetVerboseLevel() > 2)
    G4cout << "G4VRadioactiveDecay::GetMeanLifeTime: " << meanlife / CLHEP::s << " second "
           << G4endl;
#endif
...
G4double G4VRadioactiveDecay::GetMeanFreePath(const G4Track& aTrack, G4double, G4ForceCondition* fc)
{
  ...
  if (GetVerboseLevel() > 2)
  {
    ...
    G4cout << "  kinEnergy(GeV)=" << aTrack.GetKineticEnergy() / CLHEP::GeV
           << " lifeTime(ns)=" << lifeTime << " mean free path(cm)=" << res / CLHEP::cm << G4endl;
  }
#endif
  return res;
}
...
void G4VRadioactiveDecay::BuildPhysicsTable(const G4ParticleDefinition& p)
{
  ...
  isInitialised = true;
  if (G4HadronicParameters::Instance()->GetVerboseLevel() > 0 && G4Threading::IsMasterThread()
      && "GenericIon" == p.GetParticleName())
  {
    StreamInfo(G4cout, "\n");
  }
...
void G4VRadioactiveDecay::StreamInfo(std::ostream& os, const G4String& endline)
{
  ...
  os << "======================================================================" << endline;
  os << "======          Radioactive Decay Physics Parameters           =======" << endline;
  os << "======================================================================" << endline;
  os << "min MeanLife (from G4NuclideTable)                " << G4BestUnit(minMeanLife, "Time")
     << endline;
  ...
  os << "Internal e- conversion flag                       " << deex->GetInternalConversionFlag()
     << endline;
  os << "Stored internal conversion coefficients           " << deex->StoreICLevelData() << endline;
  os << "Enabled atomic relaxation mode                    " << applyARM << endline;
  os << "Enable correlated gamma emission                  " << deex->CorrelatedGamma() << endline;
  os << "Max 2J for sampling of angular correlations       " << deex->GetTwoJMAX() << endline;
  os << "Atomic de-excitation enabled                      " << emparam->Fluo() << endline;
  os << "Auger electron emission enabled                   " << emparam->Auger() << endline;
  os << "Check EM cuts disabled for atomic de-excitation   " << emparam->DeexcitationIgnoreCut()
     << endline;
  os << "Use Bearden atomic level energies                 " << emparam->BeardenFluoDir()
     << endline;
  os << "Use ANSTO fluorescence model                      " << emparam->ANSTOFluoDir() << endline;
...
G4DecayTable* G4VRadioactiveDecay::LoadDecayTable(const G4Ions* theIon)
{
  ...
  if (DecaySchemeFile.good())
  {
    ...
    while (!complete && !DecaySchemeFile.getline(inputChars, 120).eof())
    {
      ...
      if (loop > 100000)
      {
        G4Exception("G4VRadioactiveDecay::LoadDecayTable()", "HAD_RDM_100", JustWarning,
        ...
        else if (found)
        {
          ...
          if (inputLine.length() < 72)
          {
            ...
            switch (theDecayMode)
            {
              ...
              default:
                G4Exception("G4VRadioactiveDecay::LoadDecayTable()", "HAD_RDM_000", FatalException,
                            "Selected decay mode does not exist");
            }  // switch
          }
          else
          {
            ...
            switch (theDecayMode)
            {
              case BetaMinus: {
                G4BetaMinusDecay* aBetaMinusChannel =
                  new G4BetaMinusDecay(theIon, b, c * MeV, a * MeV, daughterFloatLevel, betaType);
              ...
              case BetaPlus: {
                G4BetaPlusDecay* aBetaPlusChannel =
                  new G4BetaPlusDecay(theIon, b, c * MeV, a * MeV, daughterFloatLevel, betaType);
              ...
              case KshellEC:  // K-shell electron capture
              {
                G4ECDecay* aKECChannel =
                  new G4ECDecay(theIon, b, c * MeV, a * MeV, daughterFloatLevel, KshellEC);
              ...
              case LshellEC:  // L-shell electron capture
              {
                G4ECDecay* aLECChannel =
                  new G4ECDecay(theIon, b, c * MeV, a * MeV, daughterFloatLevel, LshellEC);
              ...
              case MshellEC:  // M-shell electron capture
              {
                G4ECDecay* aMECChannel =
                  new G4ECDecay(theIon, b, c * MeV, a * MeV, daughterFloatLevel, MshellEC);
              ...
              case NshellEC:  // N-shell electron capture
              {
                G4ECDecay* aNECChannel =
                  new G4ECDecay(theIon, b, c * MeV, a * MeV, daughterFloatLevel, NshellEC);
              ...
              case Alpha: {
                G4AlphaDecay* anAlphaChannel =
                  new G4AlphaDecay(theIon, b, c * MeV, a * MeV, daughterFloatLevel);
              ...
              case Proton: {
                G4ProtonDecay* aProtonChannel =
                  new G4ProtonDecay(theIon, b, c * MeV, a * MeV, daughterFloatLevel);
              ...
              case Neutron: {
                G4NeutronDecay* aNeutronChannel =
                  new G4NeutronDecay(theIon, b, c * MeV, a * MeV, daughterFloatLevel);
              ...
              case BDProton:
                // Not yet implemented
                // G4cout << " beta-delayed proton decay, a = " << a << ", b = " << b << ", c = " <<
                // c << G4endl;
                break;

              case BDNeutron:
                // Not yet implemented
                // G4cout << " beta-delayed neutron decay, a = " << a << ", b = " << b << ", c = "
                // << c << G4endl;
                break;

              case Beta2Minus:
                // Not yet implemented
                // G4cout << " Double beta- decay, a = " << a << ", b = " << b << ", c = " << c <<
                // G4endl;
                break;
              ...
              case Proton2:
                // Not yet implemented
                // G4cout << " Double proton decay, a = " << a << ", b = " << b << ", c = " << c <<
                // G4endl;
                break;
...
void G4VRadioactiveDecay::AddUserDecayDataFile(G4int Z, G4int A, const G4String& filename)
{
  ...
  else
  {
    ...
    G4Exception("G4VRadioactiveDecay::AddUserDecayDataFile()", "HAD_RDM_001", FatalException, ed);
  }
}
...
G4VParticleChange* G4VRadioactiveDecay::DecayIt(const G4Track& theTrack, const G4Step&)
{
  ...
  if (!isAllVolumesMode)
  {
                            ...
                            theTrack.GetVolume()->GetLogicalVolume()->GetName()))
    {
      ...
      if (GetVerboseLevel() > 1)
      {
        ...
        for (auto const& vol : ValidVolumes)
        {
          G4cout << vol << " " << G4endl;
        }
        G4cout << G4endl;
  ...
  if (theDecayTable == nullptr || theDecayTable->entries() == 0)
  {
    ...
    if (GetVerboseLevel() > 1)
    {
      G4cout << "G4VRadioactiveDecay::DecayIt : " << theParticleDef->GetParticleName()
             << " is outside (Z,A) limits set for the decay or has no decays." << G4endl;
    }
#endif
    fParticleChangeForRadDecay.SetNumberOfSecondaries(0);
...
void G4VRadioactiveDecay::DecayAnalog(const G4Track& theTrack, G4DecayTable* decayTable)
{
  const G4DynamicParticle* theParticle = theTrack.GetDynamicParticle();
  const G4ParticleDefinition* theParticleDef = theParticle->GetDefinition();
  ...
  G4double ParentEnergy =
    theParticle->GetKineticEnergy() + theParticle->GetParticleDefinition()->GetPDGMass();
  ...
  {
    G4Track* secondary =
      new G4Track(products->PopProducts(), finalGlobalTime, theTrack.GetPosition());
...
G4DecayProducts* G4VRadioactiveDecay::DoDecay(const G4ParticleDefinition& theParticleDef,
                                              G4DecayTable* theDecayTable)
{
  ...
  if (theDecayChannel == nullptr)
  {
    ...
    G4Exception("G4VRadioactiveDecay::DoDecay", "HAD_RDM_013", FatalException, ed);
  }
  else
  {
    ...
    if (GetVerboseLevel() > 1)
    {
      G4cout << "G4VRadioactiveDecay::DoIt : selected decay channel addr: " << theDecayChannel
             << G4endl;
    }
#endif
...
void G4VRadioactiveDecay::CollimateDecay(G4DecayProducts* products)
{
  ...
  for (G4int i = 0; i < products->entries(); ++i)
  {
    ...
    if (daughterType == electron || daughterType == positron || daughterType == neutron
        || daughterType == gamma || daughterType == alpha || daughterType == triton
        || daughterType == proton)
    {
...
#ifdef G4VERBOSE
  if (GetVerboseLevel() > 1) G4cout << " ChooseCollimationDirection returns " << dir << G4endl;
#endif
