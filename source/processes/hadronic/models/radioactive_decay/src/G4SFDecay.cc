...
G4SFDecay::G4SFDecay(const G4ParticleDefinition* theParentNucleus, const G4double& branch,
...
G4DecayProducts* G4SFDecay::DecayIt(G4double)
{
  ...
  // is done later
  G4LorentzVector atRest(G4MT_parent->GetPDGMass(), G4ThreeVector(0., 0., 0.));
  G4DynamicParticle parentParticle(G4MT_parent, atRest);
  G4DecayProducts* products = new G4DecayProducts(parentParticle);

  // Generate LLNL code for parent nucleus
  G4int code = 1000 * G4MT_parent->GetAtomicNumber() + G4MT_parent->GetAtomicMass();
  ...
  else
  {
    ...
    G4DynamicParticle* parent = new G4DynamicParticle(G4MT_parent, G4ThreeVector(0., 0., 0.));
...
void G4SFDecay::DumpNuclearInfo()
{
  ...
  G4cout << " decays to neutrons and gammas, with branching ratio " << GetBR() << "% and Q value "
         << transitionQ << G4endl;
}
