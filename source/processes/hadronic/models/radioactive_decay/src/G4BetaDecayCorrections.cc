  ...
  : Z(theZ), A(theA)
{
  ...
  V0 = 1.13 * fine_structure_const * fine_structure_const * std::pow(std::abs(Z), 4. / 3.);
...
G4double G4BetaDecayCorrections::ModSquared(const G4double& re, G4double im)
{
...
G4double G4BetaDecayCorrections::ShapeFactor(const G4BetaDecayType& bdt, const G4double& p_e,
                                             const G4double& e_nu)
{
  ...
  switch (bdt)
  {
    ...
    case (uniqueFirstForbidden): {
      ...
      G4double term2 = 12. * (2. + gamma1) * p_e * p_e * std::pow(twoPR, 2. * (gamma1 - gamma0 - 1))
                       * gamterm1 * gamterm1 * ModSquared(gamma1, eta) / ModSquared(gamma0, eta);
      factor = term1 + term2;
    }
    break;
    ...
    case (uniqueSecondForbidden): {
      ...
      G4double term2 = 4. * (2. + gamma1) * e_nu * e_nu * p_e * p_e
                       * std::pow(twoPR, 2. * (gamma1 - gamma0 - 1.)) * gamterm1 * gamterm1
                       * ModSquared(gamma1, eta) / ModSquared(gamma0, eta);

      G4double term3 = 180. * (3. + gamma2) * p_e * p_e * p_e * p_e
                       * std::pow(twoPR, 2. * (gamma2 - gamma0 - 2)) * gamterm2 * gamterm2
    ...
    case (uniqueThirdForbidden): {
      ...
      G4double term4 = 2240. * p_e * p_e * p_e * p_e * p_e * p_e * (4. + gamma3)
                       * std::pow(twoPR, 2. * (gamma3 - gamma0 - 3.)) * gamterm3 * gamterm3
    ...
    default:
      G4Exception("G4BetaDecayCorrections::ShapeFactor()", "HAD_RDM_010", JustWarning,
