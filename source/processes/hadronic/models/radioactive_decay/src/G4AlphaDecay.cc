...
G4AlphaDecay::G4AlphaDecay(const G4ParticleDefinition* theParentNucleus, const G4double& branch,
...
G4DecayProducts* G4AlphaDecay::DecayIt(G4double)
{
  ...
  G4double cmMomentum =
    std::sqrt(transitionQ * (transitionQ + 2. * alphaMass) * (transitionQ + 2. * nucleusMass)
  ...
  G4double KE = std::sqrt(cmMomentum * cmMomentum + alphaMass * alphaMass) - alphaMass;
