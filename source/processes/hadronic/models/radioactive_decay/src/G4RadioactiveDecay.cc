  ...
  : G4VRadioactiveDecay(processName, timeThresholdForRadioactiveDecays)
{
  ...
  {
    G4cout << "G4RadioactiveDecay constructor: processName = " << processName << G4endl;
  }
#endif
...
void G4RadioactiveDecay::GetChainsFromParent(const G4ParticleDefinition& aParticle)
{
  ...
  if (GetVerboseLevel() > 1)
  {
    G4cout << "The DecayRate Table for " << aParticleName << " is selected." << G4endl;
  }
#endif
}
...
G4double G4RadioactiveDecay::ConvolveSourceTimeProfile(const G4double t, const G4double tau)
{
  ...
  else
  {
    ...
    {  // Loop checking, 01.09.2015, D.Wright
      ...
      if (loop > 1000)
      {
        G4Exception("G4RadioactiveDecay::ConvolveSourceTimeProfile()", "HAD_RDM_100", JustWarning,
  ...
  if (nbin > 0)
  {
    for (G4int i = 0; i < nbin; ++i)
    {
      ...
      if (earg < 100.)
      {
        convolvedTime += SProfile[i] * std::exp((SBin[i] - t) / tau) * std::expm1(earg);
      }
      else
      {
        convolvedTime +=
          SProfile[i] * (std::exp(-(t - SBin[i + 1]) / tau) - std::exp(-(t - SBin[i]) / tau));
      }
    }
  }
...
#ifdef G4VERBOSE
  if (GetVerboseLevel() > 2) G4cout << " Convolved time: " << convolvedTime << G4endl;
#endif
...
G4double G4RadioactiveDecay::GetDecayTime()
{
  ...
  { /* Loop checking, 01.09.2015, D.Wright */
    ...
    if (loop > 100000)
    {
      G4Exception("G4RadioactiveDecay::GetDecayTime()", "HAD_RDM_100", JustWarning,
                  "While loop count exceeded");
      break;
    }
  }
...
#ifdef G4VERBOSE
  if (GetVerboseLevel() > 2) G4cout << " Decay time: " << decaytime / s << "[s]" << G4endl;
#endif
  return decaytime;
}
...
G4int G4RadioactiveDecay::GetDecayTimeBin(const G4double aDecayTime)
{
  ...
  { /* Loop checking, 01.09.2015, D.Wright */
    ...
    if (loop > 100000)
    {
      G4Exception("G4RadioactiveDecay::GetDecayTimeBin()", "HAD_RDM_100", JustWarning,
                  "While loop count exceeded");
      break;
    }
  }
...
G4double G4RadioactiveDecay::GetMeanLifeTime(const G4Track& theTrack, G4ForceCondition* fc)
{
...
void G4RadioactiveDecay::SetDecayRate(G4int theZ, G4int theA, G4double theE, G4int theG,
...
void G4RadioactiveDecay::CalculateChainsFromParent(const G4ParticleDefinition& theParentNucleus)
{
  ...
  { /* Loop checking, 01.09.2015, D.Wright */
    ...
    if (loop > 10000)
    {
      G4Exception("G4RadioactiveDecay::CalculateChainsFromParent()", "HAD_RDM_100", JustWarning,
                  "While loop count exceeded");
      break;
    }
    nGeneration++;
    for (j = nS; j < nT; ++j)
    {
      ...
      if (GetVerboseLevel() > 1)
      {
        G4cout << "G4RadioactiveDecay::CalculateChainsFromParent: daughters of (" << ZP << ", "
               << AP << ", " << EP << ") are being calculated,  generation = " << nGeneration
               << G4endl;
      }

      aParentNucleus = theIonTable->GetIon(ZP, AP, EP);
      parentDecayTable = GetDecayTable(aParentNucleus);
      if (nullptr == parentDecayTable)
      {
        continue;
      }
      ...
      brs[BetaPlus] = brs[BetaPlus] + brs[KshellEC] + brs[LshellEC] + brs[MshellEC]
                      + brs[NshellEC];  // Combine beta+ and EC
      ...
      {  // loop over decay modes
        if (brs[i] > 0.)
        {
          switch (i)
          {
            ...
            case BetaMinus:
              // Decay mode is beta-
              theBetaMinusChannel = new G4BetaMinusDecay(aParentNucleus, brs[BetaMinus], 0. * MeV,
                                                         0. * MeV, noFloat, allowed);
              summedDecayTable->Insert(theBetaMinusChannel);
              break;

            case BetaPlus:
              // Decay mode is beta+ + EC.
              theBetaPlusChannel = new G4BetaPlusDecay(aParentNucleus, brs[BetaPlus], 0. * MeV,
                                                       0. * MeV, noFloat, allowed);
            ...
            case Alpha:
              // Decay mode is alpha.
              theAlphaChannel =
                new G4AlphaDecay(aParentNucleus, brs[Alpha], 0. * MeV, 0. * MeV, noFloat);
            ...
            case Proton:
              ...
              theProtonChannel =
                new G4ProtonDecay(aParentNucleus, brs[Proton], 0. * MeV, 0. * MeV, noFloat);
            ...
            case Neutron:
              ...
              theNeutronChannel =
                new G4NeutronDecay(aParentNucleus, brs[Neutron], 0. * MeV, 0. * MeV, noFloat);
            ...
            case SpFission:
              ...
              theFissionChannel =
                new G4SFDecay(aParentNucleus, brs[SpFission], 0. * MeV, 0. * MeV, noFloat);
            ...
            case Triton:
              ...
              theTritonChannel =
                new G4TritonDecay(aParentNucleus, brs[Triton], 0. * MeV, 0. * MeV, noFloat);
...
void G4RadioactiveDecay::SetSourceTimeProfile(const G4String& filename)
{
  ...
  if (!infile)
  {
    ...
    G4Exception("G4RadioactiveDecay::SetSourceTimeProfile()", "HAD_RDM_001", FatalException, ed);
  }
  ...
  { /* Loop checking, 01.09.2015, D.Wright */
    ...
    if (loop > 10000)
    {
      G4Exception("G4RadioactiveDecay::SetSourceTimeProfile()", "HAD_RDM_100", JustWarning,
                  "While loop count exceeded");
      break;
    }
    ...
    if (NSourceBin > 99)
    {
      G4Exception("G4RadioactiveDecay::SetSourceTimeProfile()", "HAD_RDM_002", FatalException,
                  "Input source time file too big (>100 rows)");
    }
    else
    {
...
#ifdef G4VERBOSE
  if (GetVerboseLevel() > 2) G4cout << " Source Timeprofile Nbin = " << NSourceBin << G4endl;
#endif
}
...
void G4RadioactiveDecay::SetDecayBias(const G4String& filename)
{
  std::ifstream infile(filename, std::ios::in);
  if (!infile)
    G4Exception("G4RadioactiveDecay::SetDecayBias()", "HAD_RDM_001", FatalException,
  ...
  { /* Loop checking, 01.09.2015, D.Wright */
    ...
    if (loop > 10000)
    {
      G4Exception("G4RadioactiveDecay::SetDecayBias()", "HAD_RDM_100", JustWarning,
                  "While loop count exceeded");
      break;
    }
    ...
    if (NDecayBin > 99)
    {
      G4Exception("G4RadioactiveDecay::SetDecayBias()", "HAD_RDM_002", FatalException,
                  "Input bias file too big (>100 rows)");
    }
    else
    {
  ...
  for (i = 1; i <= NDecayBin; ++i)
    DProfile[i] += DProfile[i - 1];  // Cumulative flux vs i
...
#ifdef G4VERBOSE
  if (GetVerboseLevel() > 2) G4cout << " Decay Bias Profile  Nbin = " << NDecayBin << G4endl;
#endif
}
...
G4VParticleChange* G4RadioactiveDecay::DecayIt(const G4Track& theTrack, const G4Step&)
{
  ...
  if (!(IsApplicable(*theParticleDef)))
  {
    ...
    if (GetVerboseLevel() > 1)
    {
      G4cout << "G4RadioactiveDecay::DecayIt : " << theParticleDef->GetParticleName()
             << " is not an ion or is outside (Z,A) limits set for the decay. "
             << " Set particle change accordingly. " << G4endl;
    }
#endif
    fParticleChangeForRadDecay.SetNumberOfSecondaries(0);
  ...
  if (theDecayTable == nullptr || theDecayTable->entries() == 0)
  {
    ...
    {
      G4cout << "G4RadioactiveDecay::DecayIt : "
             << "decay table not defined for " << theParticleDef->GetParticleName()
             << ". Set particle change accordingly. " << G4endl;
    }
#endif
    fParticleChangeForRadDecay.SetNumberOfSecondaries(0);
  ...
  else
  {
    ...
    else
    {
      ...
      for (G4int n = 0; n < NSplit; ++n)
      {
        ...
        if (nbin == 1)
        {
          weight1 = 1. / DProfile[nbin - 1] * (DBin[nbin] - DBin[nbin - 1])
                    / NSplit;  // width of window in ns
        }
        else if (nbin > 1)
        {
          // Go from nbin to nbin-2 because flux entries in file alternate between 0 and 1
          weight1 =
            1. / (DProfile[nbin] - DProfile[nbin - 2]) * (DBin[nbin] - DBin[nbin - 1]) / NSplit;
        ...
        for (i = 0; i < theDecayRateVector.size(); ++i)
        {
          ...
          theRadioactivityTables[decayWindows[nbin - 1]]->AddIsotope(
            PZ, PA, PE, weight1 * decayRate, theTrack.GetWeight());
          ...
          for (index = 0; index < numberOfSecondaries; ++index)
          {
            ...
            else if (((const G4Ions*)(asecondaryparticle->GetDefinition()))->GetExcitationEnergy()
                       > 0.
                     && weight > 0.)
            {  // Compute the gamma
              G4ParticleDefinition* apartDef = asecondaryparticle->GetDefinition();
              AddDeexcitationSpectrumForBiasMode(apartDef, weight, currentTime, pw, ptime,
                                                 secondaryparticles);
            }
          }
      ...
      {
        G4Track* secondary = new G4Track(secondaryparticles[index], ptime[index], currentPosition);
...
void G4RadioactiveDecay::AddDeexcitationSpectrumForBiasMode(
  G4ParticleDefinition* apartDef, G4double weight, G4double currentTime,
  std::vector<double>& weights_v, std::vector<double>& times_v,
