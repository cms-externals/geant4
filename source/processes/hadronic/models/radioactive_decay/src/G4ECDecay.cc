...
G4DecayProducts* G4ECDecay::DecayIt(G4double)
{
  ...
  if(Q < 0.0) {
    ...
    ed << "### Negative energy remains for " << G4MT_daughters[1]->GetParticleName() 
	   << " Q(MeV)=" << Q << " transitionQ(MeV)=" << transitionQ 
	   << " eBind(MeV)=" << eBind << " ResidualMass(GeV)=" << daughterMass/GeV
           << "  " << G4MT_daughters[0]->GetParticleName() << G4endl;
    G4Exception("G4ECDecay::DecayIt()", "HAD_RDM_009",
                JustWarning, ed,"Q set to zero");
    Q = 0.0;
  }
