...
G4ITDecay::G4ITDecay(const G4ParticleDefinition* theParentNucleus, const G4double& branch,
                     const G4double&, const G4double& excitationE)
...
G4DecayProducts* G4ITDecay::DecayIt(G4double)
{
  ...
  G4DynamicParticle* dynDaughter = new G4DynamicParticle(daughterIon, parentNucleus.GetMomentum());

  if (nullptr != eOrGamma)
  {
    G4DynamicParticle* eOrGammaDyn =
      new G4DynamicParticle(eOrGamma->GetParticleDefinition(), eOrGamma->GetMomentum());
    ...
    if (applyARM)
    {
      ...
      if (shellIndex > -1)
      {
        ...
        if (atomDeex->IsFluoActive() && parentZ > 5 && parentZ < 105)
        {
          ...
          atomDeex->GenerateParticles(&armProducts, shell, parentZ, deexLimit, deexLimit);
