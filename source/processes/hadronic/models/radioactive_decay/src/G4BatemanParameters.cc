...
G4BatemanParameters::G4BatemanParameters() : Z(0), A(0), E(0.0), generation(0) {}
...
void G4BatemanParameters::SetParameters(G4int aZ, G4int anA, G4double anE, G4int aG,
...
{
  G4cout << " Z: " << Z << "  A: " << A << "  E: " << E << " Generation: " << generation << G4endl;

  G4cout << " A coefficients: ";
  for (G4int i = 0; i < G4int(Acoeffs.size()); i++)
    G4cout << Acoeffs[i];
  G4cout << G4endl;

  G4cout << " Mean lifes (tau): ";
  for (G4int i = 0; i < G4int(taus.size()); i++)
    G4cout << taus[i];
  G4cout << G4endl;
}
