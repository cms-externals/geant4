...
G4LFission::G4LFission(const G4String& name) : G4HadronicInteraction(name), secID(-1)
{
  init();
...
void G4LFission::init()
{
  ...
  for (i = 1; i <= 10; i++)
  {
    ...
    if (verboseLevel > 1)
      G4cout << "G4LFission::init: i=" << i << " spneut=" << spneut[i - 1] << G4endl;
  }
}
...
G4HadFinalState* G4LFission::ApplyYourself(const G4HadProjectile& aTrack, G4Nucleus& targetNucleus)
{
  ...
  if (verboseLevel > 1)
  {
    ...
    G4cout << "A      " << N << G4endl;
    G4cout << "Z      " << Z << G4endl;
    G4cout << "atomic mass " << Atomas(N, Z) << "MeV" << G4endl;
  }
  E = E + Atomas(N, Z);
  ...
  label12:
    ...
    exn = exn + ekin;
    aNeutron = new G4DynamicParticle(G4Neutron::NeutronDefinition(), G4ParticleMomentum(1., 0., 0.),
                                     ekin * MeV);
  ...
  for (i = 1; i <= ng; i++)
  {
    ...
    exg = exg + ekin;
    aGamma =
      new G4DynamicParticle(G4Gamma::GammaDefinition(), G4ParticleMomentum(1., 0., 0.), ekin * MeV);
  ...
  for (i = 1; i <= nn + ng; i++)
  {
    ...
    theSecondary->GetParticle()->SetMomentumDirection(
      G4ParticleMomentum(px / pp, py / pp, pz / pp));
...
G4double G4LFission::Atomas(const G4double A, const G4double Z)
{
  ...
  G4double mass = (A - Z) * rmn + Z * rmp + Z * rmel - 15.67 * A + 17.23 * Pow->A23(A)
                  + 93.15 * (A / 2. - Z) * (A / 2. - Z) / A + 0.6984523 * Z * Z / Pow->A13(A);
  G4int ipp = (ia - iz) % 2;
  G4int izz = iz % 2;
