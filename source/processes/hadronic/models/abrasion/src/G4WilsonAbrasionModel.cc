                                                      ...
                                                      G4Nucleus& theTarget)
{
  ...
  G4double TotalEPre =
    theTrack.GetTotalEnergy() + theTarget.AtomicMass(AT, ZT) + theTarget.GetEnergyDeposit();
  ...
  if (verboseLevel >= 2)
  {
    ...
    G4cout << "Initial projectile A=" << AP << ", Z=" << ZP << ", radius = " << rP / fermi << " fm"
           << G4endl;
    G4cout << "Initial target     A=" << AT << ", Z=" << ZT << ", radius = " << rT / fermi << " fm"
           << G4endl;
  ...
  while (Dabr == 0 && ++loopCounter < maxNumberOfLoops) /* Loop checking, 07.08.2015, A.Ribon */
  {
    ...
    if (rT > rP)
    {
      ...
      if (x > 0.0)
        CT = 2.0 * std::sqrt(rTsq - x * x);
      else
        CT = 2.0 * std::sqrt(rTsq - rsq);
    }
    else
    {
  ...
  for (i = 0; i < nSecP; ++i)
  {
    TotalEPost += theParticleChange.GetSecondary(i)->GetParticle()->GetTotalEnergy();
  }
  //
  //
  ...
  if (fragmentP != nullptr)
  {
    ...
    G4double ExP = 0.0;
    if (Dabr < AT) excitationAbsorbedByProjectile = G4UniformRand() < 0.5;
    if (excitationAbsorbedByProjectile) ExP = GetNucleonInducedExcitation(rP, rT, r);
    G4double xP = EsP + ExP;
  ...
  if (fragmentT != nullptr)
  {
    ...
    G4double ExT = 0.0;
    if (!excitationAbsorbedByProjectile) ExT = GetNucleonInducedExcitation(rT, rP, r);
    G4double xT = EsT + ExT;
  ...
  {
    G4double beta =
      std::sqrt(1.0 - EMassP * EMassP / G4Pow::GetInstance()->powN(deltaE + EMassP, 2));
  ...
  for (i = 0; i < nSecP; ++i)
  {
    G4DynamicParticle* dynamicP = theParticleChange.GetSecondary(i)->GetParticle();
  ...
  if (fragmentP != nullptr)
  {
    ...
    if (conserveMomentum)
      fragmentP->SetMomentum(G4LorentzVector(
        pBalance, std::sqrt(pBalance.mag2() + fragmentM * fragmentM + 1.0 * eV * eV)));
    else
    {
    ...
    for (iter = products->begin(); iter != products->end(); ++iter)
    {
      ...
      if (verboseLevel >= 2 && particleName.find("[", 0) < particleName.size())
      {
        ...
        G4cout << " fragmentP = " << particleName
               << " Energy    = " << secondary->GetKineticEnergy() << G4endl;
      }
    }
    delete products;
  }
...
////////////////////////////////////////////////////////////////////////////////
//
G4Fragment* G4WilsonAbrasionModel::GetAbradedNucleons(G4int Dabr, G4double A, G4double Z,
                                                      G4double r)
{
  //
  //
  ...
  for (G4int i = 0; i < Dabr; ++i)
  {
    ...
    {
      while (p <= 0.0)
        p = npK * pK * G4UniformRand(); /* Loop checking, 07.08.2015, A.Ribon */
      G4double psq = p * p;
      found =
        maxn * G4UniformRand() < C1 * G4Exp(-psq / p1sq / 2.0) + C2 * G4Exp(-psq / p2sq / 2.0)
                                   + C3 * G4Exp(-psq / p3sq / 2.0)
                                   + p / gamma / (0.5 * (G4Exp(p / gamma) - G4Exp(-p / gamma)));
    }
  ...
  {
    G4double ionMass = G4ParticleTable::GetParticleTable()->GetIonTable()->GetIonMass(
      G4lrint(Z - Zabr), G4lrint(A - Aabr));
    G4double E = std::sqrt(pabr.mag2() + ionMass * ionMass);
    G4LorentzVector lorentzVector = G4LorentzVector(-pabr, E + 1.0 * eV);
    fragment = new G4Fragment((G4int)(A - Aabr), (G4int)(Z - Zabr), lorentzVector);
  }
...
G4double G4WilsonAbrasionModel::GetNucleonInducedExcitation(G4double rP, G4double rT, G4double r)
{
  ...
  //
  if (r > rT)
    Cl = 2.0 * std::sqrt(rPsq + 2.0 * r * rT - rsq - rTsq);
  else
    Cl = 2.0 * rP;
  //
  //
  ...
  //
  G4double Ct = 0.0;
  if (rT > rP && rsq < rTsq - rPsq)
    Ct = 2.0 * rP;
  else if (rP > rT && rsq < rPsq - rTsq)
    Ct = 2.0 * rT;
  else
  {
    ...
    if (x < 0.0)
    {
      G4cerr << "########################################"
             << "########################################" << G4endl;
      G4cerr << "ERROR IN G4WilsonAbrasionModel::GetNucleonInducedExcitation" << G4endl;
  ...
  if (Ct > 1.5 * fermi) Ex += 13.0 * Cl / fermi / 3.0 * (Ct / fermi - 1.5);
...
void G4WilsonAbrasionModel::PrintWelcomeMessage()
{
  G4cout << G4endl;
  G4cout << " *****************************************************************" << G4endl;
  G4cout << " Nuclear abrasion model for nuclear-nuclear interactions activated" << G4endl;
