...
////////////////////////////////////////////////////////////////////////////////
//
G4NuclearAbrasionGeometry::G4NuclearAbrasionGeometry(G4double AP1, G4double AT1, G4double r1)
{
  //
  //
...
G4double G4NuclearAbrasionGeometry::P()
{
  ...
  {
    if (rT - rP <= r && r <= rT + rP)
      valueP = 0.125 * R * U * S - 0.125 * (0.5 * R * U + 1.0) * T;
    else
      valueP = -1.0;
  }
  else
  {
    if (rP - rT <= r && r <= rP + rT)
      valueP = 0.125 * R * U * S
               - 0.125
                   * (0.5 * std::sqrt(n / m) * U
                      - (std::sqrt(1.0 - m * m) / n - 1.0)
                          * std::sqrt((2.0 - m) / G4Pow::GetInstance()->powN(m, 5)))
                   * T;
    else
      valueP = (std::sqrt(1.0 - m * m) / n - 1.0) * std::sqrt(1.0 - b * b / n / n);
  }
...
G4double G4NuclearAbrasionGeometry::F()
{
  ...
  {
    if (rT - rP <= r && r <= rT + rP)
      valueF = 0.75 * R * S - 0.125 * (3.0 * R - 1.0) * T;
    else
      valueF = 1.0;
  }
  else
  {
    if (rP - rT <= r && r <= rP + rT)
      valueF = 0.75 * R * S
               - 0.125
                   * (3.0 * std::sqrt(n / m)
                      - (1.0 - G4Pow::GetInstance()->powA(1.0 - m * m, 3.0 / 2.0))
                          * std::sqrt(1.0 - G4Pow::GetInstance()->powN(1.0 - m, 2))
                          / G4Pow::GetInstance()->powN(m, 3))
                   * T;
    else
      valueF =
        (1.0 - G4Pow::GetInstance()->powA(1.0 - m * m, 3.0 / 2.0)) * std::sqrt(1.0 - b * b / n / n);
  }
...
G4double G4NuclearAbrasionGeometry::GetExcitationEnergyOfTarget()
{
  ...
  G4NuclearAbrasionGeometry* revAbrasionGeometry = new G4NuclearAbrasionGeometry(AT, AP, r);
