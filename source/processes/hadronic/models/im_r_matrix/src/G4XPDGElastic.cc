...
G4double G4XPDGElastic::CrossSection(const G4KineticTrack& trk1, const G4KineticTrack& trk2) const
{
  ...
  {
    pLab = std::sqrt((sqrtS * sqrtS - (m_1 + m_2) * (m_1 + m_2))
                     * (sqrtS * sqrtS - (m_1 - m_2) * (m_1 - m_2)))
           / (2 * m_max);
        ...
        if (thePair == trkPair)
        {
          ...
          if (pLab > pMaxFit)
            G4cout << "WARNING! G4XPDGElastic::PDGElastic "
                   ...
                   << trk2.GetDefinition()->GetParticleName() << " elastic cross section: momentum "
                   << pLab / GeV << " GeV outside valid fit range " << pMinFit / GeV << "-"
                   << pMaxFit / GeV << G4endl;
          ...
          if (pLab > 0.)
          {
            ...
            sigma = aFit + bFit * G4Pow::GetInstance()->powA(pLab, nFit) + cFit * logP * logP
                    + dFit * logP;
  ...
  if (sigma < 0.)
  {
    G4cout << "WARNING! G4XPDGElastic::PDGElastic " << def1->GetParticleName() << "-"
           << def2->GetParticleName() << " elastic cross section: momentum " << pLab
           << " GeV, negative cross section " << sigma / millibarn << " mb set to 0" << G4endl;
    sigma = 0.;
  }
