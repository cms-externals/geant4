...
const G4double G4CollisionComposite::theT[nPoints] = {
  .01, .03, .05, .1,  .15, .2,  .3,  .4,  .5,  .6,  .7,  .8,  .9, 1.0, 1.2, 1.4,
  1.6, 1.8, 2.0, 2.2, 2.5, 3.0, 3.5, 4.0, 5.0, 6.0, 8.0, 10., 15, 20,  50,  100};
                                            ...
                                            const G4KineticTrack& trk2) const
{
  ...
  else
  {
    ...
    const_cast<G4CollisionComposite*>(this)->BufferCrossSection(trk1.GetDefinition(),
                                                                trk2.GetDefinition());
    //    G4cerr << "Buffer filled, reying with sqrts = "<<
    //    (trk1.Get4Momentum()+trk2.Get4Momentum()).mag() <<G4endl;
                                                       ...
                                                       const G4KineticTrack& trk2) const
{
  ...
  //  G4cerr <<"Cross-section = "<<CrossSection(trk1, trk2)/millibarn<<" "<<running<<"
  //  "<<cxCache.size()<<G4endl; G4cerr <<"Names = "<<trk1.GetDefinition()->GetParticleName()<<",
  //  "<<trk2.GetDefinition()->GetParticleName()<<G4endl; throw G4HadronicException(__FILE__,
  //  __LINE__, "G4CollisionComposite: no final state found!");
  return NULL;
}
