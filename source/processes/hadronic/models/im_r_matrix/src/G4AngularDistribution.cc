...
G4AngularDistribution::G4AngularDistribution(G4bool symmetrize) : sym(symmetrize)
{
  ...
  cm6gp =
    1.5 * (cmPion2 * cmPion2 * cmPion2) * (gPion * gPion * gPion * gPion) * m42 * m42 / dPion2;
  ...
  cMix_oLc =
    fac2
    * (3. * cmOmega2 * cmOmega4 - cmOmega4 * cmSigma2 - 2. * cmOmega4 * mOmega2
       - 2. * cmOmega4 * mSigma2 + cmOmega2 * mOmega2 * mSigma2 + cmSigma2 * mOmega2 * mSigma2
       - 4. * cmOmega4 * m42 + 2. * cmOmega2 * cmSigma2 * m42 + 3. * cmOmega2 * mOmega2 * m42
       - cmSigma2 * mOmega2 * m42 + 3. * cmOmega2 * mSigma2 * m42 - cmSigma2 * mSigma2 * m42
       - 2. * mOmega2 * mSigma2 * m42);

  cMix_oLs =
    fac2
    * (8. * cmOmega4 - 4. * cmOmega2 * cmSigma2 - 6. * cmOmega2 * mOmega2 + 2. * cmSigma2 * mOmega2
       - 6. * cmOmega2 * mSigma2 + 2. * cmSigma2 * mSigma2 + 4. * mOmega2 * mSigma2);

  cMix_sLc =
    fac3
    * (cmOmega2 * cmSigma4 - 3. * cmSigma6 + 2. * cmSigma4 * mOmega2 + 2. * cmSigma4 * mSigma2
       - cmOmega2 * mOmega2 * mSigma2 - cmSigma2 * mOmega2 * mSigma2
       - 2. * cmOmega2 * cmSigma2 * m42 + 4. * cmSigma4 * m42 + cmOmega2 * mOmega2 * m42
       - 3. * cmSigma2 * mOmega2 * m42 + cmOmega2 * mSigma2 * m42 - 3. * cmSigma2 * mSigma2 * m42
       + 2. * mOmega2 * mSigma2 * m42);

  cMix_sLs =
    fac3
    * (4. * cmOmega2 * cmSigma2 - 8. * cmSigma4 - 2. * cmOmega2 * mOmega2 + 6. * cmSigma2 * mOmega2
       - 2. * cmOmega2 * mSigma2 + 6. * cmSigma2 * mSigma2 - 4. * mOmega2 * mSigma2);
}
                                                         ...
                                                         G4double cosTheta) const
{
  ...
  G4double bOmega_1 = cOmega_1
                      * (-4. * cmOmega2 * mOmega2 - 2. * mOmega2 * mOmega2
                         - 2. * (cmOmega2 + 2 * mOmega2) * twoS - 3. * brak1);
  ...
  G4double norm =
    Cross(t1_Pion, t1_Sigma, t1_Omega, t2_Pion, t2_Sigma, t2_Omega, bMix_o1, bMix_s1, bMix_Omega,
          bMix_sm, bMix_oL, bMix_sL, bOmega_0, bOmega_1, bOmega_2, bOmega_3, bOmega_m, bOmega_L);
  ...
  if (sym)
  {
    ...
    dSigma =
      (Cross(t1_Pion, t1_Sigma, t1_Omega, t2_Pion, t2_Sigma, t2_Omega, bMix_o1, bMix_s1, bMix_Omega,
             bMix_sm, bMix_oL, bMix_sL, bOmega_0, bOmega_1, bOmega_2, bOmega_3, bOmega_m, bOmega_L)
       - Cross(t3_Pion, t3_Sigma, t3_Omega, t4_Pion, t4_Sigma, t4_Omega, bMix_o1, bMix_s1,
               bMix_Omega, bMix_sm, bMix_oL, bMix_sL, bOmega_0, bOmega_1, bOmega_2, bOmega_3,
               bOmega_m, bOmega_L))
        / norm
      + 0.5;
  }
...
G4double G4AngularDistribution::Cross(G4double tpPion, G4double tpSigma, G4double tpOmega,
                                      ...
                                      G4double bOmega_3, G4double bOmega_m, G4double bOmega_L) const
{
  G4double cross = 0;
  //  Pion
  cross += ((cPion_3 * tpPion + cPion_2) * tpPion + cPion_1) * tpPion + cPion_m / tmPion + cPion_0
           + cPion_L * G4Log(tpPion * tmPion);
  //    G4cout << "cross1 "<< cross<<G4endl;
  //  Sigma
  cross += ((cSigma_3 * tpSigma + cSigma_2) * tpSigma + cSigma_1) * tpSigma + cSigma_m / tmSigma
           + cSigma_0 + cSigma_L * G4Log(tpSigma * tmSigma);
  //    G4cout << "cross2 "<< cross<<G4endl;
  // Omega
  cross += ((bOmega_3 * tpOmega + bOmega_2) * tpOmega + bOmega_1) * tpOmega + bOmega_m / tmOmega
           + bOmega_0
           + bOmega_L * G4Log(tpOmega * tmOmega)
           // Mix
           + bMix_o1 * (tpOmega - 1.) + bMix_s1 * (tpSigma - 1.) + bMix_Omega * G4Log(tmOmega)
           + bMix_sm * G4Log(tmSigma) + bMix_oL * G4Log(tpOmega) + bMix_sL * G4Log(tpSigma);
  /*      G4cout << "cross3 "<< cross<<" "
               <<bMix_o1<<" "
               <<bMix_s1<<" "
               <<bMix_Omega<<" "
