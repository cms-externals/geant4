                                           ...
                                           const G4KineticTrack& trk2) const
{
  ...
  else
  {
    ...
    G4ThreeVector velocity =
      mom1.vect() / mom1.e() * c_light;  // mom1.boostVector() will exit on slightly negative mass
  ...
  if (collisionTime > 0)
  {
    ...
    G4ThreeVector pos = ((toCMSFrame * coordinate1).vect() - (toCMSFrame * coordinate2).vect());
                                           ...
                                           const G4KineticTrack& trk2) const
{
  ...
  G4int chargeBalance =
    G4lrint(trk1.GetDefinition()->GetPDGCharge() + trk2.GetDefinition()->GetPDGCharge());
  G4int baryonBalance =
    trk1.GetDefinition()->GetBaryonNumber() + trk2.GetDefinition()->GetBaryonNumber();
  ...
  if (collision != 0)
  {
    ...
    if (aCrossSection > 0.0)
    {
      ...
      G4cout << "be4 FinalState 1(p,e,m): " << trk1.Get4Momentum() << " "
             << trk1.Get4Momentum().mag() << ", 2: " << trk2.Get4Momentum() << " "
             << trk2.Get4Momentum().mag() << " " << G4endl;
#endif
...
#ifdef debug_G4Scatterer
      G4cout << "    FinalState 1: " << final->Get4Momentum() << " " << final->Get4Momentum().mag();
#endif

      if (products->size() == 1) return products;
      final = products->operator[](1);
#ifdef debug_G4Scatterer
      G4cout << ", 2: " << final->Get4Momentum() << " " << final->Get4Momentum().mag() << " "
             << G4endl;
#endif
      ...
      {
        G4cout << "G4Scatterer: momentum imbalance, pInitial= " << pInitial << " pFinal= " << pFinal
               << G4endl;
      }
      G4cout << "Scatterer costh= "
             << trk1.Get4Momentum().vect().unit()
                  * (products->operator[](0))->Get4Momentum().vect().unit()
             << G4endl;
#endif
      ...
      if (std::getenv("ScattererEnergyBalanceCheck"))
        std::cout << "DEBUGGING energy balance A: " << energyBalance << " " << pxBalance << " "
                  << pyBalance << " " << pzBalance << " " << chargeBalance << " " << baryonBalance
                  << " " << G4endl;
      if (chargeBalance != 0)
      {
        ...
        G4Exception("G4Scatterer", "im_r_matrix001", FatalException, "Problem in ChargeBalance");
      }
      return products;
    }
  }
...
G4double G4Scatterer::GetCrossSection(const G4KineticTrack& trk1, const G4KineticTrack& trk2) const
{
  const G4VCollision* collision = FindCollision(trk1, trk2);
...
const std::vector<G4CollisionInitialState*>&
G4Scatterer::GetCollisions(G4KineticTrack* aProjectile,
                           std::vector<G4KineticTrack*>& someCandidates, G4double aCurrentTime)
{
  theCollisions.clear();
  ...
  for (; j != someCandidates.end(); ++j)
  {
    ...
    //      G4cerr <<" !!!!!! debug collisions "<<collisionTime<<"
    //      "<<pkt->GetDefinition()->GetParticleName()<<G4endl;
  }
