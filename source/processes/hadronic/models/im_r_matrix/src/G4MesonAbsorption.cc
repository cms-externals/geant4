...
const std::vector<G4CollisionInitialState*>& G4MesonAbsorption::GetCollisions(
  G4KineticTrack* aProjectile, std::vector<G4KineticTrack*>& someCandidates, G4double aCurrentTime)
{
  theCollisions.clear();
...
G4KineticTrackVector* G4MesonAbsorption::GetFinalState(G4KineticTrack* projectile,
                                                       std::vector<G4KineticTrack*>& targets)
{
  ...
  G4ThreeVector pFinal(p * std::sin(std::acos(costh)) * std::cos(phi),
                       p * std::sin(std::acos(costh)) * std::sin(phi), p * costh);
  ...
  G4KineticTrack* f1 = new G4KineticTrack(d1, 0., targets[0]->GetPosition(), final1);
  G4KineticTrack* f2 = new G4KineticTrack(d2, 0., targets[1]->GetPosition(), final2);
  ...
  if (std::getenv("AbsorptionEnergyBalanceCheck"))
    std::cout << "DEBUGGING energy balance B: " << energyBalance << " " << chargeBalance << " "
...
G4double G4MesonAbsorption::AbsorptionCrossSection(const G4KineticTrack& aT,
                                                   const G4KineticTrack& bT)
{
  G4double t = 0;
  ...
  static const G4double it[26] = {0,    4,   50, 5.5, 75, 8,   95, 10,  120, 11.5, 140, 12,  160,
  ...
  if (t <= it[24])
  {
    G4int count = 0;
    while (t > it[count])
      count += 2; /* Loop checking, 30-Oct-2015, G.Folger */
