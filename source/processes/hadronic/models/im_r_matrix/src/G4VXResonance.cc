...
G4double G4VXResonance::IsospinCorrection(const G4KineticTrack& trk1, const G4KineticTrack& trk2,
                                          G4int isoOut1, G4int isoOut2, G4double /*iSpinOut1*/,
                                          G4double /*iSpinOut2*/) const
{
  ...
  if (in1->IsShortLived() || in2->IsShortLived())
  {
    ...
    if (factor > DBL_MIN)
    {
      ...
      std::vector<G4double> iso =
        clebsch.GenerateIso3(isoIn1, iso3In1, isoIn2, iso3In2, isoProton, isoProton);
      G4int isoA = G4lrint(iso[0]);
      G4int isoB = G4lrint(iso[1]);
      G4double rWeight = clebsch.Weight(isoProton, isoA, isoProton, isoB, isoOut1, isoOut2);
...
G4double G4VXResonance::DetailedBalance(const G4KineticTrack& trk1, const G4KineticTrack& trk2,
                                        G4int isoOut1, G4int isoOut2, G4double iSpinOut1,
                                        G4double iSpinOut2, G4double mOut1, G4double mOut2) const
{
  ...
  if (weight > 00001)
  {
    ...
    // on-shell
    G4double pinitial2 =
      (S - (m_1 + m_2) * (m_1 + m_2)) * (S - (m_1 - m_2) * (m_1 - m_2)) / (4.0 * S);
    G4double pfinal2 =
      (S - (mOut1 + mOut2) * (mOut1 + mOut2)) * (S - (mOut1 - mOut2) * (mOut1 - mOut2)) / (4.0 * S);
...
G4double G4VXResonance::DegeneracyFactor(const G4KineticTrack& trk1, const G4KineticTrack& trk2,
