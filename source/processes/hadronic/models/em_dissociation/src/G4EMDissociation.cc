...
G4EMDissociation::G4EMDissociation()
  : G4HadronicInteraction("EMDissociation"),
    secID_projectileDissociation(-1),
    secID_targetDissociation(-1)
{
  ...
  secID_projectileDissociation =
    G4PhysicsModelCatalog::GetModelID("model_projectile" + GetModelName());
                                                 ...
                                                 G4Nucleus& theTarget)
{
  ...
  if (verboseLevel >= 2)
  {
    ...
    G4cout << "Initial projectile A=" << AP << ", Z=" << ZP << G4endl;
    G4cout << "Initial target     A=" << AT << ", Z=" << ZT << G4endl;
  ...
  G4PhysicsFreeVector* crossSectionP =
    dissociationCrossSection->GetCrossSectionForProjectile(AP, ZP, AT, ZT, b, bmin);
  G4PhysicsFreeVector* crossSectionT =
    dissociationCrossSection->GetCrossSectionForTarget(AP, ZP, AT, ZT, b, bmin);
  ...
  if (G4UniformRand() * (totCrossSectionP + totCrossSectionT) < totCrossSectionP)
  {
    ...
    mass = MP;
    if (G4UniformRand()
        < dissociationCrossSection->GetWilsonProbabilityForProtonDissociation(AP, ZP))
    {
      if (verboseLevel >= 2)
        G4cout << "Projectile underwent EM dissociation producing a proton" << G4endl;
      typeNucleon = G4Proton::ProtonDefinition();
      typeDaughter = G4IonTable::GetIonTable()->GetIon((G4int)ZP - 1, (G4int)AP - 1, 0.0);
    }
    else
    {
      if (verboseLevel >= 2)
        G4cout << "Projectile underwent EM dissociation producing a neutron" << G4endl;
      typeNucleon = G4Neutron::NeutronDefinition();
      typeDaughter = G4IonTable::GetIonTable()->GetIon((G4int)ZP, (G4int)AP - 1, 0.0);
    }
    if (G4UniformRand() < (*crossSectionP)[0] / totCrossSectionP)
    {
      Eg = crossSectionP->GetLowEdgeEnergy(0);
      if (verboseLevel >= 2) G4cout << "Transition type was E1" << G4endl;
    }
    else
    {
      Eg = crossSectionP->GetLowEdgeEnergy(1);
      if (verboseLevel >= 2) G4cout << "Transition type was E2" << G4endl;
    }
  ...
  else
  {
    ...
    mass = MT;
    if (G4UniformRand()
        < dissociationCrossSection->GetWilsonProbabilityForProtonDissociation(AT, ZT))
    {
      if (verboseLevel >= 2)
        G4cout << "Target underwent EM dissociation producing a proton" << G4endl;
      typeNucleon = G4Proton::ProtonDefinition();
      typeDaughter = G4IonTable::GetIonTable()->GetIon((G4int)ZT - 1, (G4int)AT - 1, 0.0);
    }
    else
    {
      if (verboseLevel >= 2)
        G4cout << "Target underwent EM dissociation producing a neutron" << G4endl;
      typeNucleon = G4Neutron::NeutronDefinition();
      typeDaughter = G4IonTable::GetIonTable()->GetIon((G4int)ZT, (G4int)AT - 1, 0.0);
    }
  ...
  G4double pp = (e + mass1 + mass2) * (e + mass1 - mass2) * (e - mass1 + mass2)
                * (e - mass1 - mass2) / (4.0 * e * e);
  if (pp < 0.0)
  {
    pp = 1.0 * eV;
    //    if (verboseLevel >`= 1)
  ...
  G4DynamicParticle* dynamicNucleon = new G4DynamicParticle(typeNucleon, direction * pp);
  dynamicNucleon->Set4Momentum(dynamicNucleon->Get4Momentum().boost(-boost));
  G4DynamicParticle* dynamicDaughter = new G4DynamicParticle(typeDaughter, -direction * pp);
  ...
  G4Fragment* theFragment = new G4Fragment(typeDaughter->GetBaryonNumber(),
  ...
  G4ReactionProductVector* products = theExcitationHandler->BreakItUp(*theFragment);
  delete theFragment;
  theFragment = NULL;
  ...
  {
    secondary = new G4DynamicParticle((*iter)->GetDefinition(), (*iter)->GetTotalEnergy(),
                                      (*iter)->GetMomentum());
...
void G4EMDissociation::PrintWelcomeMessage()
{
  G4cout << G4endl;
  G4cout << " ****************************************************************" << G4endl;
  G4cout << " EM dissociation model for nuclear-nuclear interactions activated" << G4endl;
  G4cout << " (Written by QinetiQ Ltd for the European Space Agency)" << G4endl;
  G4cout << " ****************************************************************" << G4endl;
  G4cout << G4endl;
