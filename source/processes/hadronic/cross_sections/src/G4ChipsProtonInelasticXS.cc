...
G4double G4ChipsProtonInelasticXS::GetIsoCrossSection(const G4DynamicParticle* Pt, G4int tgZ,
...
G4double G4ChipsProtonInelasticXS::CalculateCrossSection(G4int F, G4int I, G4int, G4int targZ,
                                                         G4int targN, G4double Momentum)
{
  ...
  if (F <= 0)  // This isotope was not the last used isotop
  {
    if (F < 0)  // This isotope was found in DAMDB =-----=> RETRIEVE
    {
      ...
      if (sync <= I)
        G4cout << "*!*G4QProtonNuclCS::CalcCrossSect:Sync=" << sync << "<=" << I << G4endl;
    ...
    else  // This isotope wasn't calculated before => CREATE
    {
      ...
      if (sync != I)
      {
        G4cout << "***G4ChipsProtonNuclCS::CalcCrossSect: Sinc=" << sync << "#" << I
...
G4double G4ChipsProtonInelasticXS::ThresholdMomentum(G4int tZ, G4int tN)
{
  ...
  static const G4double pM =
    G4Proton::Proton()->Definition()->GetPDGMass();  // Projectile mass in MeV
  ...
  if (tZ < .99 || tN < 0.)
    ...
    return 800.;  // A threshold on the free proton
...
// Calculation formula for proton-nuclear inelastic cross-section (mb) log(P in GeV/c)
G4double G4ChipsProtonInelasticXS::CrossSectionFormula(G4int tZ, G4int tN, G4double P, G4double lP)
{
  G4double sigma = 0.;
...
G4double G4ChipsProtonInelasticXS::EquLinearFit(G4double X, G4int N, G4double X0, G4double DX,
                                                G4double* Y)
{
  if (DX <= 0. || N < 2)
  {
