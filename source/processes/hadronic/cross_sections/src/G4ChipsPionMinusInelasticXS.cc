...
G4double G4ChipsPionMinusInelasticXS::GetIsoCrossSection(const G4DynamicParticle* Pt, G4int tgZ,
...
G4double G4ChipsPionMinusInelasticXS::GetChipsCrossSection(G4double pMom, G4int tgZ, G4int tgN,
                                                           G4int)
{
...
G4double G4ChipsPionMinusInelasticXS::CalculateCrossSection(G4int F, G4int I, G4int, G4int targZ,
...
// Calculation formula for piMinus-nuclear inelastic cross-section (mb) log(P in GeV/c)
G4double G4ChipsPionMinusInelasticXS::CrossSectionFormula(G4int tZ, G4int tN, G4double P,
                                                          G4double lP)
{
  G4double sigma = 0.;
  ...
  {
    G4cerr << "-Warning-G4ChipsPiMinusNuclearCroSect::CSForm:*Bad A* Z=" << tZ << ", N=" << tN
           << G4endl;
    sigma = 0.;
  }
  if (sigma < 0.) return 0.;
...
G4double G4ChipsPionMinusInelasticXS::EquLinearFit(G4double X, G4int N, G4double X0, G4double DX,
                                                   G4double* Y)
{
  if (DX <= 0. || N < 2)
  {
