...
namespace
{
G4double mK;  //= G4KaonPlus::KaonPlus()->GetPDGMass()*.001; // MeV to GeV //Cannot initialize here,
              // needs particles
G4double mK2;  //= mK*mK;
...
G4double kpp_el[n_kppel] = {.7,    .38,   .0676, .0557, 3.5,   2.23,  .7,    .1,    2.,
                            1.,    .372,  5.,    74.,   3.,    3.4,   .2,    .17,   .001,
...
G4double G4ChipsKaonPlusElasticXS::GetIsoCrossSection(const G4DynamicParticle* Pt, G4int tgZ,
...
G4double G4ChipsKaonPlusElasticXS::CalculateCrossSection(G4bool CS, G4int F, G4int I, G4int PDG,
  ...
  if (lastLP > lPMin && lastLP <= lastPIN)  // Linear fit is made using precalculated tables
  {
    if (lastLP == lastPIN)
    {
      ...
      if (blast < 0 || blast >= nLast)
        G4cout << "G4QKPElCS::CCS:b=" << blast << ",n=" << nLast << G4endl;
...
G4double G4ChipsKaonPlusElasticXS::GetPTables(G4double LP, G4double ILP, G4int PDG, G4int tgZ,
                                              G4int tgN)
{
  if (PDG == 321)
  {
    ...
    if (lastPAR[nLast] != pwd)  // A unique flag to avoid the repeatable definition
    {
      if (tgZ == 1 && tgN == 0)
      {
        for (G4int ip = 0; ip < n_kppel; ip++)
          lastPAR[ip] = kpp_el[ip];  // KPlus+P
      }
      else
      {
        G4double a = tgZ + tgN;
    ...
    if (LP > ILP)
    {
      ...
      if (ini < nPoints)
      {
        ...
        if (fin >= ini)
        {
          ...
          G4cout << "*Warning*G4ChipsKaonPlusElasticXS::GetPTables: PDG=" << PDG << ", Z=" << tgZ
  ...
  else
  {
    ...
    G4Exception("G4ChipsKaonPlusElasticXS::GetPTables()", "HAD_CHPS_0000", FatalException, ed);
  }
  return ILP;
}
...
G4double G4ChipsKaonPlusElasticXS::GetExchangeT(G4int tgZ, G4int tgN, G4int PDG)
{
  ...
  if (q2 < 0.) q2 = 0.;
  if (!(q2 >= -1. || q2 <= 1.))
    G4cout << "*NAN*G4QKaonPlusElasticCS::GetExchT: -t=" << q2 << G4endl;
  if (q2 > lastTM)
  {
    q2 = lastTM;
  }
...
G4double G4ChipsKaonPlusElasticXS::GetSlope(G4int tgZ, G4int tgN, G4int PDG)
{
  ...
  if (PDG != 321)
  {
    ...
    ed << "PDG = " << PDG << ", Z = " << tgZ << ", N = " << tgN
       << ", while it is defined only for PDG=321 (K+)" << G4endl;
    G4Exception("G4ChipsKaonPlusElasticXS::GetSlope()", "HAD_CHPS_0000", FatalException, ed);
  }
  if (theB1 < 0.) theB1 = 0.;
  if (!(theB1 >= -1. || theB1 <= 1.))
    G4cout << "*NAN*G4QKaonPlusElCS::GetSlope:B1=" << theB1 << G4endl;
  return theB1 / GeVSQ;
}
...
G4double G4ChipsKaonPlusElasticXS::GetTabValues(G4double lp, G4int PDG, G4int tgZ, G4int tgN)
{
  ...
  if (tgZ == 1 && tgN == 0)  // KaonPlus+P
  {
    ...
    // G4cout<<"lastPAR[8] "<<lastPAR[8]<<" lastPAR[9] "<<lastPAR[9]<<" lastPAR[10]
    // "<<lastPAR[10]<<G4endl;
    return lastPAR[0] / (lastPAR[2] + sqr(p - lastPAR[1]))
           + (lastPAR[3] * dp * dp + lastPAR[5]) / (1. - lastPAR[6] / sp + lastPAR[7] / p4)
...
G4double G4ChipsKaonPlusElasticXS::GetQ2max(G4int PDG, G4int tgZ, G4int tgN, G4double pP)
{
  ...
  else
  {
    ...
    G4Exception("G4ChipsKaonPlusElasticXS::GetQ2max()", "HAD_CHPS_0000", FatalException, ed);
    return 0;
  }
}
