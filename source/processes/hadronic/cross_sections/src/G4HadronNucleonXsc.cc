                                              ...
                                              const G4ParticleDefinition* nucleon, G4double ekin)
{
  ...
  {
    if (pdg == 3122 || pdg == 3222 || pdg == 3112 || pdg == 3212 || pdg == 3322 || pdg == 3312
        || pdg == 3324 || pdg == 4122 || pdg == 4332 || pdg == 4122 || pdg == 4212 || pdg == 4222
        || pdg == 4112 || pdg == 4232 || pdg == 4132 || pdg == 5122 || pdg == 5332 || pdg == 5122
        || pdg == 5112 || pdg == 5222 || pdg == 5212 || pdg == 5132
        || pdg == 5232)  // heavy s-,c-,b-hyperons
    {
  ...
  {
    if (pdg == 511 || pdg == 421 || pdg == 531 || pdg == 541 || pdg == 431 || pdg == 411
        || pdg == 521 || pdg == 221 || pdg == 331 || pdg == 441 || pdg == 443
        || pdg == 543)  // s-,c-,b-mesons
    {
                                                ...
                                                const G4ParticleDefinition* nucleon, G4double ekin0)
{
  ...
  */
  ...
  if (theParticle == theNeutron)
  {
    ...
    else  // pLab < 10 GeV/c
    {
      ...
      if (proton)  // pn to be np
      {
        ...
        {
          fTotalXsc =
            33.3 + 20.8 * (pLab * pLab - 1.35) / (std::sqrt(g4calc->powN(pLab, 5)) + 0.95);
          fElasticXsc = 6 + 20 / ((logP - 0.182) * (logP - 0.182) + 1.0);
        }
      }
    }
  ...
  else if ((pdg == 211 && neutron) || (pdg == -211 && proton))
  {
    ...
    {
      fTotalXsc = 36.1 + 0.079 - 4.313 * logP + 3 * G4Exp(-(pLab - 2.1) * (pLab - 2.1) / 0.4 / 0.4)
  ...
  else if ((theParticle == theKMinus) && proton)  // K-p
  {
    ...
    else
    {
      ...
      fTotalXsc = 14. / psp + (1.1 * cofLogT * ld2 + 19.5) / (1. - .21 / sp + .52 / p4) + .006 / md
  ...
  else if ((theParticle == theKMinus) && neutron)  // K-n
  {
    ...
    else
    {
      ...
      fElasticXsc = 5.0 + 8.1 * G4Exp(-logP * 1.8) + 0.16 * sqrLogPlab - 1.3 * logP + .15 / hd;
                                                ...
                                                const G4ParticleDefinition* nucleon, G4double ekin)
{
  ...
  else if (PDGcode == 111)  //------Projectile is PionZero  --
  {
    ...
    {
      fTotalXsc =
        (16.4 + 19.3 * G4Exp(-logPlab * 0.42) + 0.19 * sqrLogPlab - 0.0 * logPlab +  // Pi+
         33.0 + 14.0 * G4Exp(-logPlab * 1.36) + 0.456 * sqrLogPlab - 4.03 * logPlab)
        / 2;  // Pi-

      fElasticXsc =
        (0.0 + 11.4 * G4Exp(-logPlab * 0.40) + 0.079 * sqrLogPlab - 0.0 * logPlab +  // Pi+
         1.76 + 11.2 * G4Exp(-logPlab * 0.64) + 0.043 * sqrLogPlab - 0.0 * logPlab)
        / 2;  // Pi-
    }
    if (neutron)
    {
      fTotalXsc =
        (33.0 + 14.0 * G4Exp(-logPlab * 1.36) + 0.456 * sqrLogPlab - 4.03 * logPlab +  // Pi+
         16.4 + 19.3 * G4Exp(-logPlab * 0.42) + 0.19 * sqrLogPlab - 0.0 * logPlab)
        / 2;  // Pi-
      fElasticXsc =
        (1.76 + 11.2 * G4Exp(-logPlab * 0.64) + 0.043 * sqrLogPlab - 0.0 * logPlab +  // Pi+
         0.0 + 11.4 * G4Exp(-logPlab * 0.40) + 0.079 * sqrLogPlab - 0.0 * logPlab)
        / 2;  // Pi-
    }
  }
  ...
  else if (PDGcode == 311)  //------Projectile is KaonZero -----
  {
    ...
    if (neutron)
    {
      ...
      fElasticXsc = (7.3 + 0.29 * sqrLogPlab - 2.4 * logPlab +  // K+
                     5.0 + 8.1 * G4Exp(-logPlab * 1.8) + 0.16 * sqrLogPlab - 1.3 * logPlab)
                    / 2;  // K-
    }
  }
                                                ...
                                                const G4ParticleDefinition*, G4double ekin)
{
  G4int pdg = theParticle->GetPDGEncoding();
  G4double xsection(0.);
  static const G4double targ_mass = 0.5 * (CLHEP::proton_mass_c2 + CLHEP::neutron_mass_c2);
  G4double sMand = CalcMandelstamS(ekin, theParticle->GetPDGMass(), targ_mass) * invGeV2;
                                            ...
                                            const G4ParticleDefinition* nucleon, G4double ekin)
{
  ...
  if (theParticle == theProton)
    pR = 0.895 * CLHEP::fermi;
  else if (theParticle == thePiPlus)
    pR = 0.663 * CLHEP::fermi;
  else if (theParticle == theKPlus)
    pR = 0.340 * CLHEP::fermi;
