...
G4double G4ChipsProtonElasticXS::CalculateCrossSection(G4bool CS, G4int F, G4int I, G4int PDG,
...
G4double G4ChipsProtonElasticXS::GetPTables(G4double LP, G4double ILP, G4int PDG, G4int tgZ,
                                            G4int tgN)
{
    ...
    if (lastPAR[nLast] != pwd)  // A unique flag to avoid the repeatable definition
    {
      if (tgZ == 0 && tgN == 1)
      {
        for (G4int ip = 0; ip < n_npel; ip++)
          lastPAR[ip] = np_el[ip];  // pn
      }
      else if (tgZ == 1 && tgN == 0)
      {
        for (G4int ip = 0; ip < n_ppel; ip++)
          lastPAR[ip] = pp_el[ip];  // pp
      }
      else
      {
        G4double a = tgZ + tgN;
        ...
        lastPAR[7] =
          (5.E-9 * a4 * sa + .27 / a) / (1. + 5.E16 / a20) / (1. + 6.E-9 * a4) + .015 / a2;  // p8
        lastPAR[8] =
          (.001 * a + .07 / a) / (1. + 5.E13 / a16 + 5.E-7 * a3) + .0003 / sa;  // p9 (10)
    ...
    if (LP > ILP)
    {
      ...
      if (ini < nPoints)
      {
        ...
        if (fin >= ini)
        {
          ...
          G4cout << "*Warning*G4ChipsProtonElasticXS::GetPTables: PDG=" << PDG << ", Z=" << tgZ
  ...
  else
  {
    ...
    G4Exception("G4ChipsProtonElasticXS::GetPTables()", "HAD_CHPS_0000", FatalException, ed);
  }
  return ILP;
}
...
G4double G4ChipsProtonElasticXS::GetSlope(G4int tgZ, G4int tgN, G4int PDG)
{
  ...
  if (PDG != 2212)
  {
    ...
    ed << "PDG = " << PDG << ", Z = " << tgZ << ", N = " << tgN
       << ", while it is defined only for PDG=2212 (p)" << G4endl;
    G4Exception("G4ChipsProtonElasticXS::GetSlope()", "HAD_CHPS_0000", FatalException, ed);
  }
  if (theB1 < 0.) theB1 = 0.;
  if (!(theB1 >= -1. || theB1 <= 1.))
...
G4double G4ChipsProtonElasticXS::GetTabValues(G4double lp, G4int PDG, G4int tgZ, G4int tgN)
{
  ...
  if (tgZ == 1 && tgN == 0)  // pp/nn
  {
    ...
    return lastPAR[0] / p2s / (1. + lastPAR[7] / p2s)
           + (lastPAR[1] + lastPAR[2] * dl1 * dl1 + lastPAR[4] / p) / (1. + lastPAR[5] * lp)
               / (1. + lastPAR[6] / p4);
  }
  else
  {
    ...
    if (a < 6.5)
    {
      ...
      theS4 = p2
              * (pah * lastPAR[38] * std::exp(-pah * lastPAR[39])
    ...
    return (lastPAR[0] * dl * dl + lastPAR[1]) / (1. + lastPAR[2] / p + lastPAR[5] / p6)
           + lastPAR[3] / (p3 + lastPAR[4] / p3)
           + lastPAR[7] / (p4 + std::pow((lastPAR[8] / p), lastPAR[6]));
    //   p4            p5               p8                 p9             p7
  }
  return 0.;
...
G4double G4ChipsProtonElasticXS::GetQ2max(G4int PDG, G4int tgZ, G4int tgN, G4double pP)
{
  ...
  else
  {
    ...
    G4Exception("G4ChipsProtonElasticXS::GetQ2max()", "HAD_CHPS_0000", FatalException, ed);
    return 0;
  }
}
