    ...
    lowElimit(1.0e-7 * CLHEP::eV)
{
  ...
  if (verboseLevel > 0)
  {
    G4cout << "G4NeutronInelasticXS::G4NeutronInelasticXS Initialise for Z < " << MAXZINEL
           << G4endl;
  }
...
G4bool G4NeutronInelasticXS::IsElementApplicable(const G4DynamicParticle*, G4int, const G4Material*)
{
  return true;
}

G4bool G4NeutronInelasticXS::IsIsoApplicable(const G4DynamicParticle*, G4int, G4int,
                                             const G4Element*, const G4Material*)
{
  return true;
}

G4double G4NeutronInelasticXS::GetElementCrossSection(const G4DynamicParticle* aParticle, G4int Z,
                                                      const G4Material*)
{
  return ElementCrossSection(aParticle->GetKineticEnergy(), aParticle->GetLogKineticEnergy(), Z);
}

G4double G4NeutronInelasticXS::ComputeCrossSectionPerElement(G4double ekin, G4double loge,
...
G4double G4NeutronInelasticXS::ElementCrossSection(G4double eKin, G4double logE, G4int ZZ)
{
  ...
  if (!done)
  {
    ...
    {
      xs = coeff[Z] * ggXsection->GetInelasticElementCrossSection(neutron, ekin, Z, aeff[Z]);
    }
  }
  ...
  if (verboseLevel > 1)
  {
    G4cout << "G4NeutronInelasticXS::ElementCrossSection Z= " << Z
           << " Ekin(MeV)= " << ekin / CLHEP::MeV << ", ElmXSinel(b)= " << xs / CLHEP::barn
           << G4endl;
  }
#endif
  return xs;
...
G4double G4NeutronInelasticXS::ComputeIsoCrossSection(G4double ekin, G4double loge,
...
G4double G4NeutronInelasticXS::GetIsoCrossSection(const G4DynamicParticle* aParticle, G4int Z,
...
G4double G4NeutronInelasticXS::IsoCrossSection(G4double ekin, G4double loge, G4int ZZ, G4int A)
{
  G4double xs{0.0};
  G4int Z = std::min(ZZ, MAXZINEL - 1);
  G4bool done{false};
  ...
  if (verboseLevel > 1)
  {
    G4cout << "G4NeutronInelasticXS::IsoXS: Z= " << Z << " A= " << A
           << " Ekin(MeV)= " << ekin / CLHEP::MeV << ", ElmXS(b)= " << xs / CLHEP::barn << G4endl;
  }
#endif
  return xs;
}

const G4Isotope* G4NeutronInelasticXS::SelectIsotope(const G4Element* anElement, G4double kinEnergy,
                                                     G4double logE)
{
  ...
  {
    sum +=
      abundVector[j] * IsoCrossSection(kinEnergy, logE, Z, anElement->GetIsotope((G4int)j)->GetN());
    temp[j] = sum;
  }
  sum *= q;
  for (j = 0; j < nIso; ++j)
...
void G4NeutronInelasticXS::BuildPhysicsTable(const G4ParticleDefinition& p)
{
  if (verboseLevel > 0)
  {
    G4cout << "G4NeutronInelasticXS::BuildPhysicsTable for " << p.GetParticleName() << G4endl;
  }
  if (p.GetParticleName() != "neutron")
  {
    ...
    G4Exception("G4NeutronInelasticXS::BuildPhysicsTable(..)", "had012", FatalException, ed, "");
    return;
  }
...
void G4NeutronInelasticXS::Initialise(G4int Z)
{
  ...
  G4double sig2 = ggXsection->GetInelasticElementCrossSection(neutron, ehigh, Z, aeff[Z]);
  coeff[Z] = (sig2 > 0.) ? sig1 / sig2 : 1.0;
}

G4PhysicsVector* G4NeutronInelasticXS::RetrieveVector(std::ostringstream& ost, G4bool warn)
{
  ...
  if (!filein.is_open())
  {
    if (warn)
    {
      ...
      G4Exception("G4NeutronInelasticXS::RetrieveVector(..)", "had014", FatalException, ed,
  ...
  else
  {
    if (verboseLevel > 1)
    {
      G4cout << "File " << ost.str() << " is opened by G4NeutronInelasticXS" << G4endl;
    }
    ...
    if (!v->Retrieve(filein, true))
    {
      ...
      G4Exception("G4NeutronInelasticXS::RetrieveVector(..)", "had015", FatalException, ed,
                  "Check G4PARTICLEXSDATA");
    }
  }
  return v;
