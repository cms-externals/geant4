...
G4double G4ChipsKaonMinusInelasticXS::GetIsoCrossSection(const G4DynamicParticle* Pt, G4int tgZ,
...
G4double G4ChipsKaonMinusInelasticXS::GetChipsCrossSection(G4double pMom, G4int tgZ, G4int tgN,
                                                           G4int)
{
...
G4double G4ChipsKaonMinusInelasticXS::CalculateCrossSection(G4int F, G4int I, G4int, G4int targZ,
                                                            G4int targN, G4double Momentum)
{
  if (F <= 0)  // This isotope was not the last used isotop
  {
    if (F < 0)  // This isotope was found in DAMDB =-----=> RETRIEVE
    {
      ...
      if (sync <= I)
        G4cerr << "*!*G4QPiMinusNuclCS::CalcCrosSect:Sync=" << sync << "<=" << I << G4endl;
    ...
    else  // This isotope wasn't calculated before => CREATE
    {
      ...
      if (sync != I)
      {
        G4cerr << "***G4ChipsKaonMinusCS::CalcCrossSect: Sinc=" << sync << "#" << I
...
// Calculation formula for piMinus-nuclear inelastic cross-section (mb) log(P in GeV/c)
G4double G4ChipsKaonMinusInelasticXS::CrossSectionFormula(G4int tZ, G4int tN, G4double P,
                                                          G4double lP)
{
  G4double sigma = 0.;
  ...
  else if (tZ == 1 && tN == 1)  // kmp_tot
  {
    ...
    G4double dR = P - .94;
    G4double sp = std::sqrt(P);
    sigma = (.6 * dX * dX + 36.) / (1. - .11 / sp + .52 / p2 / p2) + .7 / (dR * dR + .0256)
            + 18. / P / sp;
  }
  ...
  {
    G4cerr << "-Warning-G4ChipsKMinusNuclearCroSect::CSForm:*Bad A* Z=" << tZ << ", N=" << tN
           << G4endl;
    sigma = 0.;
  }
  if (sigma < 0.) return 0.;
...
G4double G4ChipsKaonMinusInelasticXS::EquLinearFit(G4double X, G4int N, G4double X0, G4double DX,
                                                   G4double* Y)
{
  if (DX <= 0. || N < 2)
  {
