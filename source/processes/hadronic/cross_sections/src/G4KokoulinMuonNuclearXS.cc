...
G4bool G4KokoulinMuonNuclearXS::IsElementApplicable(const G4DynamicParticle*, G4int,
                                                    const G4Material*)
{
  return true;
}
...
G4double G4KokoulinMuonNuclearXS::ComputeMicroscopicCrossSection(G4double KineticEnergy, G4double A)
{
  ...
  for (G4int l = 0; l < kkk; ++l)
  {
    ...
    for (G4int ll = 0; ll < 8; ++ll)
    {
      epln = x + xgi[ll] * hhh;
      ep = G4Exp(epln);
      CrossSection += ep * wgi[ll] * ComputeDDMicroscopicCrossSection(KineticEnergy, 0, A, ep);
    }
  }
...
G4double G4KokoulinMuonNuclearXS::ComputeDDMicroscopicCrossSection(G4double KineticEnergy, G4double,
                                                                   G4double A, G4double epsilon)
{
  ...
  if ((epsilon >= TotalEnergy - 0.5 * proton_mass_c2) || (epsilon <= CutFixed))
  {
...
G4double G4KokoulinMuonNuclearXS::GetElementCrossSection(const G4DynamicParticle* aPart, G4int ZZ,
                                                         const G4Material*)
{
  // AR-24Apr2018 Switch to treat transuranic elements as uranium
