...
G4double G4ChipsHyperonElasticXS::GetIsoCrossSection(const G4DynamicParticle* Pt, G4int tgZ,
...
G4double G4ChipsHyperonElasticXS::GetChipsCrossSection(G4double pMom, G4int tgZ, G4int tgN,
                                                       G4int pPDG)
{
  G4bool fCS = false;
  G4double pEn = pMom;
...
G4double G4ChipsHyperonElasticXS::CalculateCrossSection(G4bool CS, G4int F, G4int I, G4int PDG,
  ...
  if (lastLP > lPMin && lastLP <= lastPIN)  // Linear fit is made using precalculated tables
  {
    if (lastLP == lastPIN)
    {
      ...
      if (blast < 0 || blast >= nLast)
        G4cout << "G4QHyperElCS::CCS:b=" << blast << "," << nLast << G4endl;
...
G4double G4ChipsHyperonElasticXS::GetPTables(G4double LP, G4double ILP, G4int PDG, G4int tgZ,
                                             G4int tgN)
{
  ...
  G4double hyp_el[n_hypel] = {1.,    .002,  .12,   .0557, 3.5,   6.72,  99.,   2.,    3.,
                              5.,    74.,   3.,    3.4,   .2,    .17,   .001,  8.,    .055,
                              3.64,  5.e-5, 4000., 1500., .46,   1.2e6, 3.5e6, 5.e-5, 1.e10,
  ...
  if (PDG > 3000 && PDG < 3335)
  {
    ...
    if (lastPAR[nLast] != pwd)  // A unique flag to avoid the repeatable definition
    {
      if (tgZ == 1 && tgN == 0)
      {
        for (G4int ip = 0; ip < n_hypel; ip++)
          lastPAR[ip] = hyp_el[ip];  // Hyperon+P
      }
      else
      {
        G4double a = tgZ + tgN;
    ...
    if (LP > ILP)
    {
      ...
      if (ini < nPoints)
      {
        ...
        if (fin >= ini)
        {
          ...
          G4cout << "*Warning*G4ChipsHyperonElasticXS::GetPTables: PDG=" << PDG << ", Z=" << tgZ
  ...
  else
  {
    ...
    G4Exception("G4ChipsHyperonElasticXS::GetPTables()", "HAD_CHPS_0000", FatalException, ed);
  }
  return ILP;
}
...
G4double G4ChipsHyperonElasticXS::GetExchangeT(G4int tgZ, G4int tgN, G4int PDG)
{
  ...
  // AR-04Jun2014  if(PDG==3222 || PDG<3000 ||
  // PDG>3334)G4cout<<"*Warning*G4QHyElCS::GET:PDG="<<PDG<<G4endl;
  ...
  else
  {
    ...
    else if (rand < I12)
    {
      ...
      if (q2 < 0.) q2 = 0.;
      if (a < 6.5)
        q2 = G4Pow::GetInstance()->powA(q2, third);
      else
        q2 = G4Pow::GetInstance()->powA(q2, fifth);
    }
    else if (rand < I13)
    {
      G4double ran = R3 * G4UniformRand();
  ...
  if (q2 < 0.) q2 = 0.;
  if (!(q2 >= -1. || q2 <= 1.))
    G4cout << "*NAN*G4QHyElasticCrossSect::GetExchangeT:-t=" << q2 << G4endl;
  if (q2 > lastTM)
  {
    q2 = lastTM;
  }
...
G4double G4ChipsHyperonElasticXS::GetSlope(G4int tgZ, G4int tgN, G4int PDG)
{
  ...
  if (PDG < 3000 || PDG > 3334)
  {
    ...
    ed << "PDG = " << PDG << ", Z = " << tgZ << ", N = " << tgN
       << ", while it is defined only for Hyperons" << G4endl;
    G4Exception("G4ChipsHyperonElasticXS::GetSlope()", "HAD_CHPS_0000", FatalException, ed);
  }
  if (theB1 < 0.) theB1 = 0.;
  if (!(theB1 >= -1. || theB1 <= 1.))
    G4cout << "*NAN*G4QHyElasticCrossS::Getslope:" << theB1 << G4endl;
  return theB1 / GeVSQ;
}
...
G4double G4ChipsHyperonElasticXS::GetTabValues(G4double lp, G4int PDG, G4int tgZ, G4int tgN)
{
  // AR-04Jun2014  if(PDG==3222 || PDG<3000 || PDG>3334)
  // G4cout<<"*Warning*G4QHypElCS::GTV:P="<<PDG<<G4endl;
  ...
  if (tgZ == 1 && tgN == 0)  // Hyperon+P
  {
    ...
    return lastPAR[0] / (lastPAR[1] + p2 * (lastPAR[2] + p2))
           + (lastPAR[3] * dp * dp + lastPAR[5] + lastPAR[6] / p2)
               / (1. + lastPAR[7] / sp + lastPAR[8] / p4);
  }
  else
  {
    G4double p5 = p4 * p;
    ...
    if (a < 6.5)
    {
      theS1 = lastPAR[9] / (1. + lastPAR[10] * p4 * pa)
              + lastPAR[11] / (p4 + lastPAR[12] * p4 / pa2)
      ...
      theB2 = lastPAR[27] * G4Pow::GetInstance()->powA(p, lastPAR[28])
              + lastPAR[29] / (p8 + lastPAR[30] / p16);
      theS3 = lastPAR[31] / (pa * p + lastPAR[32] / pa) + lastPAR[33];
      theB3 = lastPAR[34] / (p3 + lastPAR[35] / p6) + lastPAR[36] / (1. + lastPAR[37] / p2);
      theS4 = p2
              * (pah * lastPAR[38] * G4Exp(-pah * lastPAR[39])
    ...
    else
    {
      ...
      theB1 = (lastPAR[15] / p8 + lastPAR[19])
                / (p + lastPAR[16] / G4Pow::GetInstance()->powA(p, lastPAR[20]))
              + lastPAR[17] / (1. + lastPAR[18] / p4);
      theSS = lastPAR[21] / (p4 / G4Pow::GetInstance()->powA(p, lastPAR[23]) + lastPAR[22] / p4);
      theS2 = lastPAR[24] / p4 / (G4Pow::GetInstance()->powA(p, lastPAR[25]) + lastPAR[26] / p12)
              + lastPAR[27];
      theB2 = lastPAR[28] / G4Pow::GetInstance()->powA(p, lastPAR[29])
              + lastPAR[30] / G4Pow::GetInstance()->powA(p, lastPAR[31]);
    ...
    return (lastPAR[0] * dlp * dlp + lastPAR[1]) / (1. + lastPAR[2] / p)
           + lastPAR[3] / (p3 + lastPAR[4]);
  }
  return 0.;
}  // End of GetTableValues

// Returns max -t=Q2 (GeV^2) for the momentum pP(GeV) and the target nucleus (tgN,tgZ)
G4double G4ChipsHyperonElasticXS::GetQ2max(G4int PDG, G4int tgZ, G4int tgN, G4double pP)
{
  ...
  if (tgZ || tgN > -1)  // --> Hyperon-A
  {
    ...
    G4double dmt = mt + mt;
    G4double mds =
      dmt * std::sqrt(pP2 + mLa2) + mLa2 + mt * mt;  // Mondelstam mds (@@ other hyperons?)
  ...
  else
  {
    ...
    G4Exception("G4ChipsHyperonElasticXS::GetQ2max()", "HAD_CHPS_0000", FatalException, ed);
    return 0;
  }
}
