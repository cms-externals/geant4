...
G4double G4ChipsHyperonInelasticXS::GetIsoCrossSection(const G4DynamicParticle* Pt, G4int tgZ,
...
G4double G4ChipsHyperonInelasticXS::GetChipsCrossSection(G4double pMom, G4int tgZ, G4int tgN,
                                                         G4int PDG)
{
...
G4double G4ChipsHyperonInelasticXS::CalculateCrossSection(G4int F, G4int I, G4int, G4int targZ,
...
// Calculation formula for piMinus-nuclear inelastic cross-section (mb) log(P in GeV/c)
G4double G4ChipsHyperonInelasticXS::CrossSectionFormula(G4int tZ, G4int tN, G4double P, G4double lP)
{
  G4double sigma = 0.;
...
#ifdef pdebug
    G4cout << "G4QHyperonNucCS::CSForm: A=" << a << ",P=" << P << ",CS=" << sigma << ",c=" << c
           << ",g=" << gg << ",d=" << d << ",r=" << r << ",e=" << e << ",h=" << h << G4endl;
#endif
  }
  else
  {
    G4cerr << "-Warning-G4QHyperonNuclearCroSect::CSForm:*Bad A* Z=" << tZ << ", N=" << tN
           << G4endl;
    sigma = 0.;
  }
  if (sigma < 0.) return 0.;
...
G4double G4ChipsHyperonInelasticXS::EquLinearFit(G4double X, G4int N, G4double X0, G4double DX,
                                                 G4double* Y)
{
  if (DX <= 0. || N < 2)
  {
