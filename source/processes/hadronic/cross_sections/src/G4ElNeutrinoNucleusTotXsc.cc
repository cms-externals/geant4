...
G4ElNeutrinoNucleusTotXsc::G4ElNeutrinoNucleusTotXsc() : G4VCrossSectionDataSet("NuElNuclTotXsc")
{
...
G4bool G4ElNeutrinoNucleusTotXsc::IsIsoApplicable(const G4DynamicParticle* aPart, G4int, G4int,
...
G4double G4ElNeutrinoNucleusTotXsc::GetElementCrossSection(const G4DynamicParticle* part, G4int Z,
...
G4double G4ElNeutrinoNucleusTotXsc::GetIsoCrossSection(const G4DynamicParticle* aPart, G4int,
...
G4double G4ElNeutrinoNucleusTotXsc::GetNuElTotCsXsc(G4int index, G4double energy)
{
  ...
  if (index <= 0 || energy < theElectron->GetPDGMass())
    xsc = fNuElTotXsc[0];
  else if (index >= fIndex)
    xsc = fNuElTotXsc[fIndex - 1];
  else
  {
...
G4double G4ElNeutrinoNucleusTotXsc::GetANuElTotCsXsc(G4int index, G4double energy)
{
  ...
  if (index <= 0 || energy < thePositron->GetPDGMass())
    xsc = fANuElTotXsc[0];
  else if (index >= fIndex)
    xsc = fANuElTotXsc[fIndex - 1];
  else
  {
    G4double x1 = fNuElEnergy[index - 1] * GeV;
    G4double x2 = fNuElEnergy[index] * GeV;
...
const G4double G4ElNeutrinoNucleusTotXsc::fNuElEnergy[50] = {
  0.000561138, 0.000735091, 0.000962969, 0.00126149, 0.00165255, 0.00216484, 0.00283594, 0.00371508,
  0.00486676,  0.00637546,  0.00835185,  0.0109409,  0.0143326,  0.0187757,  0.0245962,  0.032221,
  0.0422095,   0.0552945,   0.0724358,   0.0948908,  0.124307,   0.162842,   0.213323,   0.279453,
  0.366084,    0.47957,     0.628237,    0.82299,    1.07812,    1.41233,    1.85016,    2.42371,
  3.17505,     4.15932,     5.44871,     7.13781,    9.35053,    12.2492,    16.0464,    21.0208,
  27.5373,     36.0739,     47.2568,     61.9064,    81.0973,    106.238,    139.171,    182.314,
  238.832,     312.869};
...
const G4double G4ElNeutrinoNucleusTotXsc::fNuElTotXsc[50] = {
  0.0026484, 0.00609503, 0.00939421, 0.0132163, 0.0178983, 0.0237692, 0.0312066, 0.0406632,
  0.0526867, 0.0679357,  0.0871913,  0.111359,  0.141458,  0.178584,  0.223838,  0.27822,
  0.342461,  0.416865,   0.501361,   0.596739,  0.713623,  0.905749,  1.20718,   1.52521,
  1.75286,   1.82072,    1.67119,    1.50074,   1.3077,    1.14923,   1.0577,    0.977911,
  0.918526,  0.792889,   0.702282,   0.678615,  0.687099,  0.725167,  0.706795,  0.678045,
  0.649791,  0.651328,   0.651934,   0.658062,  0.660659,  0.662534,  0.662601,  0.660261,
  0.656724,  0.65212};
...
const G4double G4ElNeutrinoNucleusTotXsc::fANuElTotXsc[50] = {
  0.00103385, 0.00237807, 0.00366358, 0.00515192, 0.00697434, 0.00925859, 0.0121508, 0.0158252,
  0.0204908,  0.0263959,  0.0338304,  0.0431234,  0.0546346,  0.068735,   0.0857738, 0.106025,
  0.129614,   0.15643,    0.186063,   0.21784,    0.251065,   0.28525,    0.319171,  0.348995,
  0.369448,   0.378165,   0.377353,   0.371224,   0.363257,   0.355433,   0.348618,  0.343082,
  0.338825,   0.33574,    0.333684,   0.332504,   0.332052,   0.332187,   0.332781,  0.333716,
  0.33489,    0.336213,   0.337608,   0.339008,   0.340362,   0.341606,   0.342706,  0.343628,
  0.344305,   0.344675};
