...
G4ChargeExchangeXS::G4ChargeExchangeXS()
{
  ...
  for (G4int i = 0; i < 5; ++i)
  {
    ...
    if (nullptr == fPionSecPD[i])
    {
      ...
      G4Exception("G4ChargeExchangeXS::G4ChargeExchangeXS()", "had044", FatalException, ed, "");
    }
  }
}
...
G4bool G4ChargeExchangeXS::IsElementApplicable(const G4DynamicParticle*, G4int, const G4Material*)
{
  return true;
}

G4double G4ChargeExchangeXS::GetElementCrossSection(const G4DynamicParticle* dp, G4int Z,
                                                    const G4Material* mat)
{
  G4double pE = dp->GetTotalEnergy();
  return (pE > fEnergyLimit) ? GetCrossSection(dp->GetDefinition(), mat, Z, pE) : 0.0;
}

G4double G4ChargeExchangeXS::GetCrossSection(const G4ParticleDefinition* part,
                                             const G4Material* mat, G4int ZZ, G4double pEtot)
{
  G4double result = 0.0;
  fFactor = fFactorK;
  ...
  if (verboseLevel > 1)
  {
    G4cout << "### G4ChargeExchangeXS: " << part->GetParticleName() << " Z=" << Z << " A=" << A
           << " Etot(GeV)=" << pEtot / CLHEP::GeV << " s(GeV^2)=" << lorentz_s / GeV2 << G4endl;
  }
  ...
  else if (pdg == 211)
  {
    ...
    if (1 == Z)
    {
      n23 = ComputeDeuteronFraction(mat);
    }
    G4double sum = 0.0;
    for (G4int i = 0; i < 5; ++i)
    {
  ...
  else if (pdg == 130)
  {
    ...
    result = 0.5 * (g4calc->Z23(Z) + g4calc->Z23(A - Z)) * g4calc->powA(p_momentum, -1.60) * kfact;
  }
  result *= fFactor;
  if (verboseLevel > 1)
  {
    G4cout << "   Done for " << part->GetParticleName() << " Etot(GeV)=" << pEtot / CLHEP::GeV
...
G4ChargeExchangeXS::SampleSecondaryType(const G4ParticleDefinition* part, const G4Material* mat,
                                        G4int Z, G4int A, G4double etot)
{
  ...
  {
    G4cout << "G4ChargeExchangeXS::SampleSecondaryType for " << pd->GetParticleName()
           << "  findex=" << findex << G4endl;
  }
  return pd;
}

G4double G4ChargeExchangeXS::SampleTforPion(const G4double etot, const G4double ltmax) const
{
...
G4double G4ChargeExchangeXS::ComputeDeuteronFraction(const G4Material* mat) const
{
