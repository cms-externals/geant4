...
G4double G4ChipsKaonPlusInelasticXS::GetIsoCrossSection(const G4DynamicParticle* Pt, G4int tgZ,
...
G4double G4ChipsKaonPlusInelasticXS::GetChipsCrossSection(G4double pMom, G4int tgZ, G4int tgN,
                                                          G4int)
{
...
G4double G4ChipsKaonPlusInelasticXS::CalculateCrossSection(G4int F, G4int I, G4int, G4int targZ,
                                                           G4int targN, G4double Momentum)
{
  ...
  if (F <= 0)  // This isotope was not the last used isotop
  {
    if (F < 0)  // This isotope was found in DAMDB =-----=> RETRIEVE
    {
      ...
      if (sync <= I)
        G4cerr << "*!*G4ChipsKPlusNuclCS::CalcCrosSect:Sync=" << sync << "<=" << I << G4endl;
    ...
    else  // This isotope wasn't calculated before => CREATE
    {
      ...
      if (sync != I)
      {
        G4cerr << "***G4ChipsKPlusNuclCS::CalcCrossSect: Sinc=" << sync << "#" << I
  ...
  {
    if (A <= 1. && Momentum < 600.)
      sigma = 0.;  // Approximation tot/el uncertainty
    else
...
G4double G4ChipsKaonPlusInelasticXS::ThresholdMomentum(G4int tZ, G4int tN)
{
  ...
  if (tZ == 1 && tN == 0)
    tM = prM;  // A threshold on the free proton
  else
    dE = tZ / (1. + std::pow(tA, third));  // Safety for diffused edge of the nucleus (QE)
...
// Calculation formula for piMinus-nuclear inelastic cross-section (mb) log(P in GeV/c)
G4double G4ChipsKaonPlusInelasticXS::CrossSectionFormula(G4int tZ, G4int tN, G4double P,
                                                         G4double lP)
{
  G4double sigma = 0.;
  ...
  {
    G4cerr << "-Warning-G4ChipsKaonPlusNuclearCroSect::CSForm:Bad A, Z=" << tZ << ", N=" << tN
           << G4endl;
    sigma = 0.;
  }
  if (sigma < 0.) return 0.;
...
G4double G4ChipsKaonPlusInelasticXS::EquLinearFit(G4double X, G4int N, G4double X0, G4double DX,
                                                  G4double* Y)
{
  if (DX <= 0. || N < 2)
  {
