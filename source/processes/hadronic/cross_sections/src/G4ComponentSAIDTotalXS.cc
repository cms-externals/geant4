...
G4ComponentSAIDTotalXS::G4ComponentSAIDTotalXS() : G4VComponentCrossSection("xsSAID")
{
...
{
  PrintWarning(part, 0, Z, G4lrint(N), "G4ComponentSAIDTotalXS::GetTotalElementCrossSection",
...
G4SAIDCrossSectionType G4ComponentSAIDTotalXS::GetType(const G4ParticleDefinition* prim,
                                                       const G4ParticleDefinition* sec, G4int Z,
                                                       G4int N)
{
...
void G4ComponentSAIDTotalXS::Initialise(G4SAIDCrossSectionType tp)
{
  ...
  if (!inelastdata[idx])
  {
    ...
    if (!path)
    {
      G4Exception("G4ComponentSAIDTotalXS::Initialise(..)", "had013", FatalException,
...
void G4ComponentSAIDTotalXS::ReadData(G4int index, G4PhysicsVector* v, const G4String& ss1,
                                      const G4String& ss2)
{
  ...
  if (!(filein))
  {
    ...
    G4Exception("G4ComponentSAIDTotalXS::ReadData(..)", "had014", FatalException, ed,
                "Check G4SAIDXSDATA");
  }
  else
  {
    ...
    {
      G4cout << "File " << ost.str() << " is opened by G4ComponentSAIDTotalXS" << G4endl;
    }
...
void G4ComponentSAIDTotalXS::PrintWarning(const G4ParticleDefinition* prim,
                                          ...
                                          const G4String& ss1, const G4String& ss2)
{
  G4cout << ss1 << ": " << ss2 << G4endl;
  G4cout << "For Z= " << Z << " N= " << N << " of ";
  if (prim)
  {
    G4cout << prim->GetParticleName() << " ";
  }
  if (sec)
  {
    G4cout << " x-section to " << sec->GetParticleName();
  }
  G4cout << G4endl;
}
