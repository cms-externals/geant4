...
namespace
{
G4double mK;  //= G4KaonMinus::KaonMinus()->GetPDGMass()*.001; // MeV to GeV//Cannot initialize
              // here, needs particles
G4double mK2;  //= mK*mK;
...
const G4double kmp_el[n_kmpel] = {5.2,   .0557, 3.5,   2.23,  .7,    .075,  .004,  .39,   .000156,
                                  .15,   1.,    .0156, 5.,    74.,   3.,    3.4,   .2,    .17,
                                  .001,  8.,    .055,  3.64,  5.e-5, 4000., 1500., .46,   1.2e6,
...
G4double G4ChipsKaonMinusElasticXS::GetIsoCrossSection(const G4DynamicParticle* Pt, G4int tgZ,
...
G4double G4ChipsKaonMinusElasticXS::CalculateCrossSection(G4bool CS, G4int F, G4int I, G4int PDG,
  ...
  if (lastLP > lPMin && lastLP <= lastPIN)  // Linear fit is made using precalculated tables
  {
    if (lastLP == lastPIN)
    {
      ...
      if (blast < 0 || blast >= nLast)
        G4cout << "G4QKMElCS::CCS:b=" << blast << ",n=" << nLast << G4endl;
...
G4double G4ChipsKaonMinusElasticXS::GetPTables(G4double LP, G4double ILP, G4int PDG, G4int tgZ,
                                               G4int tgN)
{
  ...
  if (PDG == -321)
  {
    ...
    if (lastPAR[nLast] != pwd)  // A unique flag to avoid the repeatable definition
    {
      if (tgZ == 1 && tgN == 0)
      {
        for (G4int ip = 0; ip < n_kmpel; ip++)
          lastPAR[ip] = kmp_el[ip];  // PiMinus+P
      }
      else
      {
        G4double a = tgZ + tgN;
    ...
    if (LP > ILP)
    {
      ...
      if (ini < nPoints)
      {
        ...
        if (fin >= ini)
        {
          ...
          G4cout << "*Warning*G4ChipsKaonMinusElasticXS::GetPTables: PDG=" << PDG << ", Z=" << tgZ
  ...
  else
  {
    ...
    G4Exception("G4ChipsKaonMinusElasticXS::GetPTables()", "HAD_CHPS_0000", FatalException, ed);
  }
  return ILP;
}
...
G4double G4ChipsKaonMinusElasticXS::GetExchangeT(G4int tgZ, G4int tgN, G4int PDG)
{
  ...
  if (q2 < 0.) q2 = 0.;
  if (!(q2 >= -1. || q2 <= 1.))
    G4cout << "*NAN*G4QKaonMinusElasticCS::GetExchT: -t=" << q2 << G4endl;
  if (q2 > lastTM)
  {
    q2 = lastTM;
  }
...
G4double G4ChipsKaonMinusElasticXS::GetSlope(G4int tgZ, G4int tgN, G4int PDG)
{
  ...
  if (theB1 < 0.) theB1 = 0.;
  if (!(theB1 >= -1. || theB1 <= 1.))
    G4cout << "*NAN*G4QKaonMinusElCS::GetSlope:B1=" << theB1 << G4endl;
  return theB1 / GeVSQ;
}
...
G4double G4ChipsKaonMinusElasticXS::GetTabValues(G4double lp, G4int PDG, G4int tgZ, G4int tgN)
{
  ...
  if (tgZ == 1 && tgN == 0)  // KaonMinus+P
  {
    ...
    return lastPAR[0] / psp
           + (lastPAR[1] * dp * dp + lastPAR[3]) / (1. - lastPAR[4] / sp + lastPAR[5] / p4)
           + lastPAR[6] / (sqr(p - lastPAR[7]) + lastPAR[8])
           + lastPAR[9] / (sqr(p - lastPAR[10]) + lastPAR[11]);
  }
  else
  {
    G4double p5 = p4 * p;
...
G4double G4ChipsKaonMinusElasticXS::GetQ2max(G4int PDG, G4int tgZ, G4int tgN, G4double pP)
{
  ...
  else
  {
    ...
    G4Exception("G4ChipsKaonMinusElasticXS::GetQ2max()", "HAD_CHPS_0000", FatalException, ed);
    return 0;
  }
}
