...
G4double G4ChipsPionMinusElasticXS::GetIsoCrossSection(const G4DynamicParticle* Pt, G4int tgZ,
...
G4double G4ChipsPionMinusElasticXS::CalculateCrossSection(G4bool CS, G4int F, G4int I, G4int PDG,
...
G4double G4ChipsPionMinusElasticXS::GetPTables(G4double LP, G4double ILP, G4int PDG, G4int tgZ,
                                               G4int tgN)
{
  ...
  G4double pimp_el[n_pimpel] = {
    1.27,  1.53, .0676, 3.5,   .36,   .04,   .017,  .0025, .0557, 2.4,   7.,    .7,    .6,
    .05,   5.,   74.,   3.,    3.4,   .2,    .17,   .001,  8.,    .055,  3.64,  5.e-5, 4000.,
    1500., .46,  1.2e6, 3.5e6, 5.e-5, 1.e10, 8.5e8, 1.e10, 1.1,   3.4e6, 6.8e6, 0.};
  ...
  if (PDG == -211)
  {
    ...
    if (lastPAR[nLast] != pwd)  // A unique flag to avoid the repeatable definition
    {
      if (tgZ == 1 && tgN == 0)
      {
        for (G4int ip = 0; ip < n_pimpel; ip++)
          lastPAR[ip] = pimp_el[ip];  // PiMinus+P
      }
      else
      {
        G4double a = tgZ + tgN;
    ...
    if (LP > ILP)
    {
      ...
      if (ini < nPoints)
      {
        ...
        if (fin >= ini)
        {
          ...
          G4cout << "*Warning*G4ChipsPionMinusElasticXS::GetPTables: PDG=" << PDG << ", Z=" << tgZ
  ...
  else
  {
    ...
    G4Exception("G4ChipsPionMinusElasticXS::GetPTables()", "HAD_CHPS_0000", FatalException, ed);
  }
  return ILP;
}
...
G4double G4ChipsPionMinusElasticXS::GetSlope(G4int tgZ, G4int tgN, G4int PDG)
{
  ...
  if (PDG != -211)
  {
    ...
    G4Exception("G4ChipsPionMinusElasticXS::GetSlope()", "HAD_CHPS_0000", FatalException, ed);
  }
  if (theB1 < 0.) theB1 = 0.;
  if (!(theB1 >= -1. || theB1 <= 1.))
...
G4double G4ChipsPionMinusElasticXS::GetTabValues(G4double lp, G4int PDG, G4int tgZ, G4int tgN)
{
  ...
  if (tgZ == 1 && tgN == 0)  // PiMin+P
  {
    ...
    // G4cout<<"lastPAR[13] "<<lastPAR[13]<<" lastPAR[6] "<<lastPAR[6]<<" lastPAR[7]
    // "<<lastPAR[7]<<G4endl;
...
G4double G4ChipsPionMinusElasticXS::GetQ2max(G4int PDG, G4int tgZ, G4int tgN, G4double pP)
{
  ...
  else
  {
    ...
    G4Exception("G4ChipsPionMinusElasticXS::GetQ2max()", "HAD_CHPS_0000", FatalException, ed);
    return 0;
  }
}
