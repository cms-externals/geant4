...
G4NeutronElasticXS::G4NeutronElasticXS()
  : G4VCrossSectionDataSet(Default_Name()), neutron(G4Neutron::Neutron())
{
  if (verboseLevel > 0)
  {
    G4cout << "G4NeutronElasticXS::G4NeutronElasticXS Initialise for Z < " << MAXZEL << G4endl;
  }
  ggXsection =
...
G4bool G4NeutronElasticXS::IsElementApplicable(const G4DynamicParticle*, G4int, const G4Material*)
{
  return true;
}

G4bool G4NeutronElasticXS::IsIsoApplicable(const G4DynamicParticle*, G4int, G4int, const G4Element*,
                                           const G4Material*)
{
  return false;
}

G4double G4NeutronElasticXS::GetElementCrossSection(const G4DynamicParticle* aParticle, G4int Z,
                                                    const G4Material*)
{
  return ElementCrossSection(aParticle->GetKineticEnergy(), aParticle->GetLogKineticEnergy(), Z);
}

G4double G4NeutronElasticXS::ComputeCrossSectionPerElement(G4double ekin, G4double loge,
...
G4double G4NeutronElasticXS::ElementCrossSection(G4double ekin, G4double loge, G4int ZZ)
{
  ...
  if (!done)
  {
    ...
    xs = (ekin <= pv->GetMaxEnergy())
           ...
           : coeff[Z] * ggXsection->GetElasticElementCrossSection(neutron, ekin, Z, aeff[Z]);
  }
  ...
  if (verboseLevel > 1)
  {
    G4cout << "Z= " << Z << " Ekin(MeV)= " << ekin / CLHEP::MeV
           << ",  nElmXSel(b)= " << xs / CLHEP::barn << G4endl;
  }
#endif
  return xs;
}

G4double G4NeutronElasticXS::ComputeIsoCrossSection(G4double ekin, G4double loge,
                                                    const G4ParticleDefinition*, G4int ZZ, G4int A,
...
G4double G4NeutronElasticXS::GetIsoCrossSection(const G4DynamicParticle* aParticle, G4int ZZ,
                                                G4int A, const G4Isotope*, const G4Element*,
                                                const G4Material*)
{
  G4int Z = std::min(ZZ, MAXZEL - 1);
  return ElementCrossSection(aParticle->GetKineticEnergy(), aParticle->GetLogKineticEnergy(), Z) * A
         / aeff[Z];
}

const G4Isotope* G4NeutronElasticXS::SelectIsotope(const G4Element* anElement, G4double, G4double)
{
...
void G4NeutronElasticXS::BuildPhysicsTable(const G4ParticleDefinition& p)
{
  if (verboseLevel > 0)
  {
    G4cout << "G4NeutronElasticXS::BuildPhysicsTable for " << p.GetParticleName() << G4endl;
  }
  if (p.GetParticleName() != "neutron")
  {
    ...
    G4Exception("G4NeutronElasticXS::BuildPhysicsTable(..)", "had012", FatalException, ed, "");
    return;
  }
...
void G4NeutronElasticXS::Initialise(G4int Z)
{
  ...
  G4double sig2 = ggXsection->GetElasticElementCrossSection(neutron, ehigh, Z, aeff[Z]);
  coeff[Z] = (sig2 > 0.) ? sig1 / sig2 : 1.0;
}

G4PhysicsVector* G4NeutronElasticXS::RetrieveVector(std::ostringstream& ost, G4bool warn)
{
  ...
  if (!filein.is_open())
  {
    if (warn)
    {
      ...
      G4Exception("G4NeutronElasticXS::RetrieveVector(..)", "had014", FatalException, ed,
  ...
  else
  {
    if (verboseLevel > 1)
    {
      G4cout << "File " << ost.str() << " is opened by G4NeutronElasticXS" << G4endl;
    }
    ...
    if (!v->Retrieve(filein, true))
    {
      ...
      G4Exception("G4NeutronElasticXS::RetrieveVector(..)", "had015", FatalException, ed,
                  "Check G4PARTICLEXSDATA");
    }
  }
  return v;
