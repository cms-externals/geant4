...
G4double G4ChipsAntiBaryonInelasticXS::GetIsoCrossSection(const G4DynamicParticle* Pt, G4int tgZ,
...
G4double G4ChipsAntiBaryonInelasticXS::GetChipsCrossSection(G4double pMom, G4int tgZ, G4int tgN,
                                                            G4int cPDG)
{
...
G4double G4ChipsAntiBaryonInelasticXS::CalculateCrossSection(G4int F, G4int I, G4int, G4int targZ,
...
// Calculation formula for piMinus-nuclear inelastic cross-section (mb) log(P in GeV/c)
G4double G4ChipsAntiBaryonInelasticXS::CrossSectionFormula(G4int tZ, G4int tN, G4double P,
                                                           G4double lP)
{
  G4double sigma = 0.;
  ...
  else if (tZ < 97 && tN < 152)  // General solution
  {
    ...
    G4double c = (170. + 3600. / a2s) / (1. + 65. / a2s)
                 + 40. * G4Pow::GetInstance()->powA(a, 0.712) / (1. + 12.2 / a) / (1. + 34. / a2);
    G4double r = (170. + 0.01 * a3) / (1. + a3 / 28000.);
    sigma = c + d * d + r / sp;
  }
  else
  {
    G4cerr << "-Warning-G4QAntiBarNuclearCroSect::CSForm:*Bad A* Z=" << tZ << ", N=" << tN
           << G4endl;
    sigma = 0.;
  }
  if (sigma < 0.) return 0.;
...
G4double G4ChipsAntiBaryonInelasticXS::EquLinearFit(G4double X, G4int N, G4double X0, G4double DX,
                                                    G4double* Y)
{
  if (DX <= 0. || N < 2)
  {
