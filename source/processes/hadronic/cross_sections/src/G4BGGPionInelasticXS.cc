 ...
 : G4VCrossSectionDataSet("BarashenkovGlauberGribov")
{
  ...
  fLowEnergyPiPlus  = 1.*CLHEP::MeV;
  fLowEnergyPiMinus = 1.*CLHEP::MeV;
  SetMinKinEnergy(0.0);
					     ...
					     G4int ZZ, const G4Material*)
{
  ...
  } else if(isPiplus && ekin <= fLowEnergyPiPlus) {
    cross = theLowEPiPlus[Z]*CoulombFactor(ekin, Z);
  } else if(!isPiplus && ekin <= fLowEnergyPiMinus) {
    cross = theLowEPiMinus[Z];
...
void G4BGGPionInelasticXS::BuildPhysicsTable(const G4ParticleDefinition& p)
{
  ...
  if(isMaster && 0 == theA[0]) {
    ...
    dp.SetDefinition(G4PionPlus::PionPlus());
    dp.SetKineticEnergy(fLowEnergyPiPlus);
    for(G4int iz=2; iz<93; ++iz) {
      theLowEPiPlus[iz] = fPion->GetInelasticCrossSection(&dp, iz, theA[iz])
	/CoulombFactor(fLowEnergyPiPlus, iz);
    }

    dp.SetDefinition(G4PionMinus::PionMinus());
    dp.SetKineticEnergy(fLowEnergyPiMinus);
    for(G4int iz=2; iz<93; ++iz) {
