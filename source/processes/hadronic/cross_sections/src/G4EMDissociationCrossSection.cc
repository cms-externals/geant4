...
/////////////////////////////////////////////////////////////////////////////
//
G4bool G4EMDissociationCrossSection::IsElementApplicable(const G4DynamicParticle*, G4int /*ZZ*/,
                                                         const G4Material*)
{
  return true;
}
...
//
G4double
G4EMDissociationCrossSection::GetElementCrossSection(const G4DynamicParticle* theDynamicParticle,
                                                     G4int Z, const G4Material*)
{
  ...
  if (theDynamicParticle->GetKineticEnergy() < 2.0 * CLHEP::MeV)
  {
    return 0.0;
  }
  ...
  G4double crossSection = (*theProjectileCrossSections)[0] + (*theProjectileCrossSections)[1];
  delete theProjectileCrossSections;
  G4PhysicsFreeVector* theTargetCrossSections = GetCrossSectionForTarget(AP, ZP, AT, ZT, b, bmin);
  crossSection += (*theTargetCrossSections)[0] + (*theTargetCrossSections)[1];
  delete theTargetCrossSections;
  return crossSection;
}
////////////////////////////////////////////////////////////////////////////////
//
G4PhysicsFreeVector* G4EMDissociationCrossSection::GetCrossSectionForProjectile(
  G4double AP, G4double ZP, G4double /* AT */, G4double ZT, G4double b, G4double bmin)
{
  //
  //
  ...
  G4double E_GDR =
    hbarc
    / std::sqrt(0.7 * amu_c2 * R0 * R0 / 8.0 / J
  ...
  //
  G4double ZTsq = ZT * ZT;
  G4double nE1 = ZTsq * thePhotonSpectrum->GetGeneralE1Spectrum(E_GDR, b, bmin);
  G4double nE2 = ZTsq * thePhotonSpectrum->GetGeneralE2Spectrum(E_GQR, b, bmin);
  //
  //
...
//
G4PhysicsFreeVector*
G4EMDissociationCrossSection::GetCrossSectionForTarget(G4double AP, G4double ZP, G4double AT,
                                                       G4double ZT, G4double b, G4double bmin)
{
  //
