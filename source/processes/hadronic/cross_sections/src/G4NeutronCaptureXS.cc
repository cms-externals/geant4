                                             ...
                                             G4int ZZ, G4int A)
{
  ...
  if (nullptr != pv && ekin < cap_max_r_e[Z]) {
    ...
    if (dataR->GetNumberOfComponents(Z) > 0) {
      G4PhysicsVector* pviso = dataR->GetComponentDataByID(Z, A);
      if (pviso != nullptr) { 
...
#ifdef G4VERBOSE
	if (verboseLevel > 0) {
	  G4cout << "G4NeutronCaptureXS::IsoXS: Ekin(MeV)= "
		 << ekin/CLHEP::MeV 
		 << "  xs(b)= " << xs/CLHEP::barn 
		 << "  Z= " << Z << "  A= " << A << G4endl;
	}
...
void G4NeutronCaptureXS::Initialise(G4int Z)
{
  ...
  G4PhysicsVector* vr = RetrieveVector(ostr, false);
