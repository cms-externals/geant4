...
const G4int G4GammaNuclearXS::freeVectorException[] = {4, 6, 7, 8, 27, 39, 45, 65, 67, 69, 73};
  ...
  : G4VCrossSectionDataSet(Default_Name()), gamma(G4Gamma::Gamma())
{
  ...
  if (verboseLevel > 0)
  {
    G4cout << "G4GammaNuclearXS::G4GammaNuclearXS Initialise for Z < " << MAXZGAMMAXS << G4endl;
  }
...
G4bool G4GammaNuclearXS::IsElementApplicable(const G4DynamicParticle*, G4int, const G4Material*)
{
  return true;
}

G4bool G4GammaNuclearXS::IsIsoApplicable(const G4DynamicParticle*, G4int, G4int, const G4Element*,
                                         const G4Material*)
{
  return true;
}

G4double G4GammaNuclearXS::GetElementCrossSection(const G4DynamicParticle* aParticle, G4int Z,
                                                  const G4Material*)
{
  return ElementCrossSection(aParticle->GetKineticEnergy(), Z);
}

G4double G4GammaNuclearXS::ElementCrossSection(const G4double ekin, const G4int ZZ)
{
  // check cache
  ...
  if (pv == nullptr || 1 == Z || Z == 40 || Z == 74 || (Z == 24 && ekin >= limCHIPS1)
      || (Z == 39 && ekin >= limCHIPS1) || (Z == 50 && ekin >= limCHIPS2)
      || (Z == 64 && ekin >= limCHIPS2))
  {
  ...
  if (verboseLevel > 1)
  {
    G4cout << "Z= " << Z << " Ekin(MeV)= " << ekin / CLHEP::MeV
           << ",  nElmXS(b)= " << fXS / CLHEP::barn << G4endl;
  }
#endif
  return fXS;
}
...
G4double G4GammaNuclearXS::GetIsoCrossSection(const G4DynamicParticle* aParticle, G4int Z, G4int A,
                                              const G4Isotope*, const G4Element*, const G4Material*)
{
  return IsoCrossSection(aParticle->GetKineticEnergy(), Z, A);
}

G4double G4GammaNuclearXS::IsoCrossSection(const G4double ekin, const G4int ZZ, const G4int A)
{
  const G4int Z = (ZZ < MAXZGAMMAXS) ? ZZ : MAXZGAMMAXS - 1;
...
const G4Isotope* G4GammaNuclearXS::SelectIsotope(const G4Element* anElement, G4double kinEnergy,
                                                 G4double)
{
  ...
  for (std::size_t j = 0; j < nIso; ++j)
  {
    ...
    sum += abundVector[j] * IsoCrossSection(kinEnergy, Z, anElement->GetIsotope((G4int)j)->GetN());
    temp[j] = sum;
  }
  sum *= G4UniformRand();
  for (std::size_t j = 0; j < nIso; ++j)
...
void G4GammaNuclearXS::BuildPhysicsTable(const G4ParticleDefinition& p)
{
  if (verboseLevel > 1)
  {
    G4cout << "G4GammaNuclearXS::BuildPhysicsTable for " << p.GetParticleName() << G4endl;
  }
  if (p.GetParticleName() != "gamma")
  {
    ...
    G4Exception("G4GammaNuclearXS::BuildPhysicsTable(..)", "had012", FatalException, ed, "");
    return;
  }
...
G4PhysicsVector* G4GammaNuclearXS::RetrieveVector(std::ostringstream& ost, G4bool warn, G4int Z)
{
  ...
  if (!filein.is_open())
  {
    if (warn)
    {
      ...
      G4Exception("G4GammaNuclearXS::RetrieveVector(..)", "had014", FatalException, ed,
  ...
  else
  {
    if (verboseLevel > 1)
    {
      G4cout << "File " << ost.str() << " is opened by G4GammaNuclearXS" << G4endl;
    }
    ...
    if (!v->Retrieve(filein, true))
    {
      ...
      G4Exception("G4GammaNuclearXS::RetrieveVector(..)", "had015", FatalException, ed,
                  "Check G4PARTICLEXSDATA");
    }
  }
  return v;
