...
G4double G4ChipsPionPlusInelasticXS::GetIsoCrossSection(const G4DynamicParticle* Pt, G4int tgZ,
...
G4double G4ChipsPionPlusInelasticXS::GetChipsCrossSection(G4double pMom, G4int tgZ, G4int tgN,
                                                          G4int)
{
...
G4double G4ChipsPionPlusInelasticXS::CalculateCrossSection(G4int F, G4int I, G4int, G4int targZ,
                                                           G4int targN, G4double Momentum)
{
  ...
  if (F <= 0)  // This isotope was not the last used isotop
  {
    if (F < 0)  // This isotope was found in DAMDB =-----=> RETRIEVE
    {
      ...
      if (sync <= I)
        G4cerr << "*!*G4ChipsPiMinusNuclCS::CalcCrosSect:Sync=" << sync << "<=" << I << G4endl;
    ...
    else  // This isotope wasn't calculated before => CREATE
    {
      ...
      if (sync != I)
      {
        G4cerr << "***G4ChipsPiMinusNuclCS::CalcCrossSect: Sinc=" << sync << "#" << I
...
G4double G4ChipsPionPlusInelasticXS::ThresholdMomentum(G4int tZ, G4int tN)
{
  ...
  static const G4double pM =
    G4PionPlus::PionPlus()->Definition()->GetPDGMass();  // Projectile mass in MeV
  static const G4double tpM = pM + pM;  // Doubled projectile mass (MeV)
  G4double tA = tZ + tN;
  if (tZ < .99 || tN < 0.)
    return 0.;
  else if (tZ == 1 && tN == 0)
    return 300.;  // A threshold on the free proton
  // G4double dE=1.263*tZ/(1.+G4Pow::GetInstance()->powA(tA,third));
  G4double dE =
    tZ
    / (1. + G4Pow::GetInstance()->powA(tA, third));  // Safety for diffused edge of the nucleus (QE)
...
// Calculation formula for piMinus-nuclear inelastic cross-section (mb) log(P in GeV/c)
G4double G4ChipsPionPlusInelasticXS::CrossSectionFormula(G4int tZ, G4int tN, G4double P,
                                                         G4double lP)
{
  G4double sigma = 0.;
  ...
  else if (tZ == 1 && tN == 1)  // pimp_tot
  {
    ...
    G4double f = lP + 1.25;
    G4double gg = lP - .017;
    sigma = (.55 * d * d + 38. + 23. / std::sqrt(P)) / (1. + .3 / p2 / p2) + 18. / (f * f + .1089)
            + .02 / (gg * gg + .0025);
  }
  ...
  {
    G4cerr << "-Warning-G4ChipsPiPlusNuclearCroSect::CSForm:*Bad A* Z=" << tZ << ", N=" << tN
           << G4endl;
    sigma = 0.;
  }
  if (sigma < 0.) return 0.;
...
G4double G4ChipsPionPlusInelasticXS::EquLinearFit(G4double X, G4int N, G4double X0, G4double DX,
                                                  G4double* Y)
{
  if (DX <= 0. || N < 2)
  {
