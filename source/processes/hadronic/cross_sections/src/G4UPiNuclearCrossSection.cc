...
G4double 
G4UPiNuclearCrossSection::Interpolate(G4int Z, G4int A, G4double ekin, 
                                      const G4PhysicsTable* table, 
                                      const G4bool isPiPlus) const
{
  G4double res = 0.0;
  ...
  if(ekin < elow) {
    if(isPiPlus) {
      res *= std::sqrt(ekin/elow)*G4NuclearRadii::CoulombFactor(Z, A, piPlus, ekin);
    } else {
      res *= std::sqrt(elow/std::max(ekin, elowest));
    }
  }
