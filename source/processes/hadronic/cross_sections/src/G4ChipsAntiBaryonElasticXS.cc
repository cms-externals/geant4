...
G4double G4ChipsAntiBaryonElasticXS::GetIsoCrossSection(const G4DynamicParticle* Pt, G4int tgZ,
...
G4double G4ChipsAntiBaryonElasticXS::GetChipsCrossSection(G4double pMom, G4int tgZ, G4int tgN,
                                                          G4int pPDG)
{
  G4bool fCS = false;
...
G4double G4ChipsAntiBaryonElasticXS::CalculateCrossSection(G4bool CS, G4int F, G4int I, G4int PDG,
  ...
  if (lastLP > lPMin && lastLP <= lastPIN)  // Linear fit is made using precalculated tables
  {
    if (lastLP == lastPIN)
    {
      ...
      if (blast < 0 || blast >= nLast)
        G4cout << "G4QaBarElCS::CCS:b=" << blast << "," << nLast << G4endl;
...
G4double G4ChipsAntiBaryonElasticXS::GetPTables(G4double LP, G4double ILP, G4int PDG, G4int tgZ,
                                                G4int tgN)
{
  ...
  G4double app_el[n_appel] = {1.25,  3.5,   80.,   1.,    .0557, 6.72,  5.,    74.,   3.,    3.4,
                              .2,    .17,   .001,  8.,    .055,  3.64,  5.e-5, 4000., 1500., .46,
  ...
  if (PDG > -3335 && PDG < -1111)
  {
    ...
    if (lastPAR[nLast] != pwd)  // A unique flag to avoid the repeatable definition
    {
      if (tgZ == 1 && tgN == 0)
      {
        for (G4int ip = 0; ip < n_appel; ip++)
          lastPAR[ip] = app_el[ip];  // PiMinus+P
      }
      else
      {
        G4double a = tgZ + tgN;
    ...
    if (LP > ILP)
    {
      ...
      if (ini < nPoints)
      {
        ...
        if (fin >= ini)
        {
          ...
          G4cout << "*Warning*G4ChipsAntiBaryonElasticXS::GetPTables: PDG=" << PDG << ", Z=" << tgZ
  ...
  else
  {
    ...
    G4Exception("G4ChipsAntiBaryonElasticXS::GetPTables()", "HAD_CHPS_0000", FatalException, ed);
  }
  return ILP;
}
...
G4double G4ChipsAntiBaryonElasticXS::GetExchangeT(G4int tgZ, G4int tgN, G4int PDG)
{
  ...
  if (PDG < -3334 || PDG > -1111)
    G4cout << "*Warning*G4QAntiBaryonElCS::GetExT:PDG=" << PDG << G4endl;
  ...
  if (q2 < 0.) q2 = 0.;
  if (!(q2 >= -1. || q2 <= 1.))
    G4cout << "*NAN*G4QaBElasticCrossSect::GetExchangeT:-t=" << q2 << G4endl;
  if (q2 > lastTM)
  {
    q2 = lastTM;
  }
...
G4double G4ChipsAntiBaryonElasticXS::GetSlope(G4int tgZ, G4int tgN, G4int PDG)
{
  ...
  if (PDG < -3334 || PDG > -1111)
  {
    ...
    ed << "PDG = " << PDG << ", Z = " << tgZ << ", N = " << tgN
       << ", while it is defined only for Anti Baryons" << G4endl;
    G4Exception("G4ChipsAntiBaryonElasticXS::GetSlope()", "HAD_CHPS_0000", FatalException, ed);
  }
  if (theB1 < 0.) theB1 = 0.;
  if (!(theB1 >= -1. || theB1 <= 1.))
    G4cout << "*NAN*G4QaBaElasticCrossS::Getslope:" << theB1 << G4endl;
  return theB1 / GeVSQ;
}
...
G4double G4ChipsAntiBaryonElasticXS::GetTabValues(G4double lp, G4int PDG, G4int tgZ, G4int tgN)
{
  if (PDG < -3334 || PDG > -1111)
    G4cout << "*Warning*G4QAntiBaryElCS::GetTabV:PDG=" << PDG << G4endl;
...
G4double G4ChipsAntiBaryonElasticXS::GetQ2max(G4int PDG, G4int tgZ, G4int tgN, G4double pP)
{
  ...
  if (tgZ || tgN > -1)  // ---> pipA
  {
    ...
    G4double dmt = mt + mt;
    G4double mds =
      dmt * std::sqrt(pP2 + mNuc2) + mNuc2 + mt * mt;  // Mondelstam mds (@@ other AntiBar?)
  ...
  else
  {
    ...
    G4Exception("G4ChipsAntiBaryonElasticXS::GetQ2max()", "HAD_CHPS_0000", FatalException, ed);
    return 0;
  }
}
