...
// ********************************************************************
//
// $Id: G4CrossSectionDataSetRegistry.cc 111432 2018-08-10 07:32:25Z gcosmo $
//
// -------------------------------------------------------------------
//
// GEANT4 Class file
...
{
  //std::cout << "Clean " << xSections.size() << " x-sections " 
  //	    << this << std::endl; 
  for (auto xsec : xSections) {
    delete xsec;
  }
  /*
  std::cout << "Clean " << xComponents.size() << " components x-sections " 
	    << this << std::endl; 

  size_t n = xComponents.size(); 
  for (size_t i=0; i<n; ++i) {
    std::cout << i << ". " << xComponents[i] << std::endl;
    if(xComponents[i]) {
      std::cout << i << ". Delete component x-section: " << xComponents[i]->GetName() 
           << "  " << this << std::endl;
      delete xComponents[i];
    }
  }
  */
}
...
void G4CrossSectionDataSetRegistry::Register(G4VCrossSectionDataSet* p)
{
  ...
  //G4cout << "Register x-section " << xSections.size() << ". " << p->GetName() 
  //       << "  " << p << "  " << this << G4endl;
...
void G4CrossSectionDataSetRegistry::DeRegister(G4VCrossSectionDataSet* p)
{
  ...
  for (size_t i=0; i<n; ++i) {
    if(xSections[i] == p) {
      //std::cout << "DeRegister x-section: " << p->GetName() << "  " << p 
      //       << "  " << this << std::endl;
      xSections[i] = nullptr;
      return;
    }
...
void G4CrossSectionDataSetRegistry::Register(G4VComponentCrossSection* p)
{
  ...
  //G4cout << "Register " << xComponents.size() << "-th component x-section: " 
  //	 << p->GetName() << "  " << p << "  " << this << G4endl;
...
void G4CrossSectionDataSetRegistry::DeRegister(G4VComponentCrossSection* p)
{
  ...
  for (size_t i=0; i<n; ++i) {
    //std::cout << i << ". " << xComponents[i] << std::endl;
    if(xComponents[i] == p) {
      // std::cout << "DeRegister component x-section: " << p->GetName() 
      // << "  " << p << "  " << this << std::endl;
