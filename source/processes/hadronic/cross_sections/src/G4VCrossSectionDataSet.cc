...
G4VCrossSectionDataSet::G4VCrossSectionDataSet(const G4String& nam)
  : name(nam), maxKinEnergy(G4HadronicParameters::Instance()->GetMaxEnergy())
{
...
G4bool G4VCrossSectionDataSet::IsElementApplicable(const G4DynamicParticle*, G4int,
                                                   const G4Material*)
{
  return false;
}

G4bool G4VCrossSectionDataSet::IsIsoApplicable(const G4DynamicParticle*, G4int, G4int,
...
G4double G4VCrossSectionDataSet::ComputeCrossSectionPerElement(G4double kinEnergy, G4double loge,
                                                               ...
                                                               const G4Material* mat)
{
  ...
  for (std::size_t j = 0; j < nIso; ++j)
  {
    const G4Isotope* iso = (*isoVector)[j];
    G4int A = iso->GetN();
    xsec += abundVector[j] * ComputeIsoCrossSection(kinEnergy, loge, pd, Z, A, iso, elm, mat);
  }
  return xsec;
}

G4double G4VCrossSectionDataSet::GetElementCrossSection(const G4DynamicParticle* dynPart, G4int Z,
                                                        const G4Material* mat)
{
  ...
  if (nullptr != mat)
  {
    ed << "  material: " << mat->GetName();
  }
  ed << " target Z= " << Z << G4endl;
  G4Exception("G4VCrossSectionDataSet::GetElementCrossSection", "had001", FatalException, ed);
  return 0.0;
}

G4double G4VCrossSectionDataSet::GetIsoCrossSection(const G4DynamicParticle* dynPart, G4int Z,
                                                    G4int A, const G4Isotope*, const G4Element* elm,
                                                    const G4Material* mat)
{
  G4ExceptionDescription ed;
  ...
  if (nullptr != elm)
  {
    ed << " element: " << elm->GetName();
  }
  ed << " target Z= " << Z << " A= " << A << G4endl;
  G4Exception("G4VCrossSectionDataSet::GetIsoCrossSection", "had001", FatalException, ed);
  return 0.0;
}

G4double G4VCrossSectionDataSet::ComputeIsoCrossSection(G4double kinEnergy, G4double,
                                                        const G4ParticleDefinition* pd, G4int Z,
                                                        G4int A, const G4Isotope*,
                                                        const G4Element* elm, const G4Material* mat)
{
  G4ExceptionDescription ed;
  ed << "GetIsoCrossSection is not implemented in <" << name << ">\n"
     << "Particle: " << pd->GetParticleName() << "  Ekin(MeV)= " << kinEnergy / CLHEP::MeV;
  if (nullptr != mat)
  {
    ed << "  material: " << mat->GetName();
  }
...
const G4Isotope* G4VCrossSectionDataSet::SelectIsotope(const G4Element* anElement, G4double,
                                                       G4double)
{
