...
G4bool G4BGGNucleonElasticXS::IsElementApplicable(const G4DynamicParticle*, G4int,
                                                  const G4Material*)
{
  return true;
}
...
G4bool G4BGGNucleonElasticXS::IsIsoApplicable(const G4DynamicParticle*, G4int Z, G4int,
...
G4double G4BGGNucleonElasticXS::GetElementCrossSection(const G4DynamicParticle* dp, G4int ZZ,
                                                       const G4Material*)
{
  ...
  if (verboseLevel > 1)
  {
    G4cout << "G4BGGNucleonElasticXS::GetElementCrossSection  for "
           ...
           << "  Ekin(GeV)= " << dp->GetKineticEnergy() / CLHEP::GeV << " in nucleus Z= " << Z
           << "  A= " << theA[Z] << " XS(b)= " << cross / barn << G4endl;
  }
#endif
  return cross;
}
...
G4double G4BGGNucleonElasticXS::GetIsoCrossSection(const G4DynamicParticle* dp, G4int, G4int A,
                                                   ...
                                                   const G4Material*)
{
  ...
  if (verboseLevel > 1)
  {
    G4cout << "G4BGGNucleonElasticXS::GetIsoCrossSection  for "
           << dp->GetDefinition()->GetParticleName()
           << "  Ekin(GeV)= " << dp->GetKineticEnergy() / CLHEP::GeV << " in nucleus  Z=1  A=" << A
           << " XS(b)= " << cross / barn << G4endl;
  }
#endif
  return cross;
}
...
void G4BGGNucleonElasticXS::BuildPhysicsTable(const G4ParticleDefinition& p)
{
  ...
  else
  {
    ...
    ed << "This BGG cross section is applicable only to nucleons and not to " << p.GetParticleName()
       << G4endl;
    G4Exception("G4BGGNucleonElasticXS::BuildPhysicsTable", "had001", FatalException, ed);
  }
}
  ...
  for (G4int iz = 2; iz < 93; ++iz)
  {
    ...
    if (verboseLevel > 1)
    {
      G4cout << "G4BGGNucleonElasticXS::Initialise Z=" << iz << "  A=" << theA[iz]
             << " GFactorP=" << theGlauberFacP[iz] << " GFactorN=" << theGlauberFacN[iz] << G4endl;
    }
  }

  theCoulombFacP[0] = theCoulombFacP[1] = theCoulombFacN[0] = theCoulombFacN[1] = 1.0;
  dp.SetDefinition(theProton);
  dp.SetKineticEnergy(fLowEnergy);
  for (G4int iz = 2; iz < 93; ++iz)
  {
    theCoulombFacP[iz] = fNucleon->GetElasticCrossSection(&dp, iz) / CoulombFactor(fLowEnergy, iz);
  }
  dp.SetDefinition(G4Neutron::Neutron());
  for (G4int iz = 2; iz < 93; ++iz)
  {
    theCoulombFacN[iz] = fNucleon->GetElasticCrossSection(&dp, iz) / CoulombFactor(fLowEnergy, iz);

    if (verboseLevel > 1)
    {
      G4cout << "G4BGGNucleonElasticXS::Initialise Z=" << iz << "  A=" << theA[iz]
             << " CFactorP=" << theCoulombFacP[iz] << " CFactorN=" << theCoulombFacN[iz] << G4endl;
    }
  }
}
...
{
  return (isProton) ? G4NuclearRadii::CoulombFactor(Z, theA[Z], theProton, kinEnergy) : 1.0;
}
