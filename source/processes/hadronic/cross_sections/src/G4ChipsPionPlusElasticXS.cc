...
G4double G4ChipsPionPlusElasticXS::GetIsoCrossSection(const G4DynamicParticle* Pt, G4int tgZ,
...
G4double G4ChipsPionPlusElasticXS::CalculateCrossSection(G4bool CS, G4int F, G4int I, G4int PDG,
  ...
  if (lastLP > lPMin && lastLP <= lastPIN)  // Linear fit is made using precalculated tables
  {
    if (lastLP == lastPIN)
    {
      ...
      if (blast < 0 || blast >= nLast)
        G4cout << "G4QEleastCS::CCS:b=" << blast << "," << nLast << G4endl;
...
G4double G4ChipsPionPlusElasticXS::GetPTables(G4double LP, G4double ILP, G4int PDG, G4int tgZ,
                                              G4int tgN)
{
  ...
  G4double pipp_el[n_pippel] = {1.27,  13.,   .0676, 3.5,   .32,   .0576, .0557, 2.4,   6.,
                                3.,    .7,    5.,    74.,   3.,    3.4,   .2,    .17,   .001,
                                8.,    .055,  3.64,  5.e-5, 4000., 1500., .46,   1.2e6, 3.5e6,
  ...
  if (PDG == 211)
  {
    ...
    if (lastPAR[nLast] != pwd)  // A unique flag to avoid the repeatable definition
    {
      if (tgZ == 1 && tgN == 0)
      {
        for (G4int ip = 0; ip < n_pippel; ip++)
          lastPAR[ip] = pipp_el[ip];  // PiPlus+P
      }
      else
      {
        G4double a = tgZ + tgN;
    ...
    if (LP > ILP)
    {
      ...
      if (ini < nPoints)
      {
        ...
        if (fin >= ini)
        {
          ...
          G4cout << "*Warning*G4ChipsPionPlusElasticXS::GetPTables: PDG=" << PDG << ", Z=" << tgZ
  ...
  else
  {
    ...
    G4Exception("G4ChipsPionPlusElasticXS::GetPTables()", "HAD_CHPS_0000", FatalException, ed);
  }
  return ILP;
}
...
G4double G4ChipsPionPlusElasticXS::GetExchangeT(G4int tgZ, G4int tgN, G4int PDG)
{
  ...
  if (q2 < 0.) q2 = 0.;
  if (!(q2 >= -1. || q2 <= 1.))
    G4cout << "*NAN*G4QElasticCrossSect::GetExchangeT: -t=" << q2 << G4endl;
  if (q2 > lastTM)
  {
    q2 = lastTM;
  }
...
G4double G4ChipsPionPlusElasticXS::GetSlope(G4int tgZ, G4int tgN, G4int PDG)
{
  ...
  if (PDG != 211)
  {
    ...
    ed << "PDG = " << PDG << ", Z = " << tgZ << ", N = " << tgN
       << ", while it is defined only for PDG=211 (pi-)" << G4endl;
    G4Exception("G4ChipsPionPlusElasticXS::GetSlope()", "HAD_CHPS_000", FatalException, ed);
  }
  if (theB1 < 0.) theB1 = 0.;
  if (!(theB1 >= -1. || theB1 <= 1.))
    G4cout << "*NAN*G4QElasticCrossSect::Getslope:" << theB1 << G4endl;
  return theB1 / GeVSQ;
}
...
G4double G4ChipsPionPlusElasticXS::GetTabValues(G4double lp, G4int PDG, G4int tgZ, G4int tgN)
{
  ...
  if (tgZ == 1 && tgN == 0)  // PiPlus+P
  {
    ...
    return lastPAR[1] / (lr2 + lr2 * lr2 + lastPAR[2])
           + (lastPAR[6] * dl3 * dl3 + lastPAR[7] + lastPAR[8] / sp) / (1. + lastPAR[9] / p4)
           + lastPAR[10] / (dl4 * dl4 + lastPAR[5]);
  }
  else
  {
    G4double p5 = p4 * p;
...
G4double G4ChipsPionPlusElasticXS::GetQ2max(G4int PDG, G4int tgZ, G4int tgN, G4double pP)
{
  ...
  else
  {
    ...
    G4Exception("G4ChipsPionPlusElasticXS::GetQ2max()", "HAD_CHPS_0000", FatalException, ed);
    return 0;
  }
}
