...
G4bool G4BGGPionElasticXS::IsElementApplicable(const G4DynamicParticle*, G4int, const G4Material*)
{
  return true;
}
...
G4bool G4BGGPionElasticXS::IsIsoApplicable(const G4DynamicParticle*, G4int Z, G4int,
...
G4double G4BGGPionElasticXS::GetElementCrossSection(const G4DynamicParticle* dp, G4int ZZ,
                                                    const G4Material*)
{
...
G4double G4BGGPionElasticXS::GetIsoCrossSection(const G4DynamicParticle* dp, G4int, G4int A,
...
void G4BGGPionElasticXS::BuildPhysicsTable(const G4ParticleDefinition& p)
{
  if (verboseLevel > 1)
  {
    G4cout << "G4BGGPionElasticXS::BuildPhysicsTable for " << p.GetParticleName() << G4endl;
  }
  ...
  else
  {
    ...
    ed << "This BGG cross section is applicable only to pions and not to " << p.GetParticleName()
       << G4endl;
    G4Exception("G4BGGPionElasticXS::BuildPhysicsTable", "had001", FatalException, ed);
  }
}
...
void G4BGGPionElasticXS::Initialise()
{
  ...
  for (G4int iz = 2; iz < 93; ++iz)
  {
    theCoulombFacPiPlus[iz] =
      fPion->GetElasticCrossSection(&dp, iz, theA[iz]) / CoulombFactorPiPlus(fLowEnergy, iz);
  }
  dp.SetDefinition(G4PionMinus::PionMinus());
  for (G4int iz = 2; iz < 93; ++iz)
  {
    theCoulombFacPiMinus[iz] =
      fPion->GetElasticCrossSection(&dp, iz, theA[iz]) / FactorPiMinus(fLowEnergy);

    if (verboseLevel > 1)
    {
      G4cout << "Z= " << iz << "  A= " << theA[iz]
             << " CoulombFactorPiPlus= " << theCoulombFacPiPlus[iz]
             << " CoulombFactorPiMinus= " << theCoulombFacPiMinus[iz] << G4endl;
    }
  }
}
