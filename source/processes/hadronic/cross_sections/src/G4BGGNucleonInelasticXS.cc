...
G4bool G4BGGNucleonInelasticXS::IsElementApplicable(const G4DynamicParticle*, G4int,
                                                    const G4Material*)
{
  return true;
}
...
G4bool G4BGGNucleonInelasticXS::IsIsoApplicable(const G4DynamicParticle*, G4int Z, G4int,
...
G4double G4BGGNucleonInelasticXS::GetElementCrossSection(const G4DynamicParticle* dp, G4int ZZ,
                                                         const G4Material*)
{
  G4double cross = 0.0;
...
G4double G4BGGNucleonInelasticXS::GetIsoCrossSection(const G4DynamicParticle* dp, G4int, G4int A,
...
void G4BGGNucleonInelasticXS::BuildPhysicsTable(const G4ParticleDefinition& p)
{
  ...
  else
  {
    ...
    ed << "This BGG cross section is applicable only to nucleons and not to " << p.GetParticleName()
       << G4endl;
    G4Exception("G4BGGNucleonInelasticXS::BuildPhysicsTable", "had001", FatalException, ed);
    return;
  }
}
...
void G4BGGNucleonInelasticXS::Initialise()
{
  ...
  for (G4int iz = 2; iz < 93; ++iz)
  {
    ...
    if (verboseLevel > 1)
    {
      G4cout << "G4BGGNucleonInelasticXS::Initialise Z= " << iz << "  A= " << theA[iz]
             << " GFactorP= " << theGlauberFacP[iz] << " GFactorN= " << theGlauberFacN[iz]
             << G4endl;
    }
  }
  ...
  for (G4int iz = 2; iz < 93; ++iz)
  {
    theCoulombFacP[iz] = fNucleon->GetElementCrossSection(&dp, iz) / CoulombFactor(fLowEnergy, iz);
  }
  dp.SetDefinition(G4Neutron::Neutron());
  for (G4int iz = 2; iz < 93; ++iz)
  {
    theCoulombFacN[iz] = fNucleon->GetElementCrossSection(&dp, iz) / CoulombFactor(fLowEnergy, iz);

    if (verboseLevel > 1)
    {
      G4cout << "G4BGGNucleonInelasticXS::Initialise Z= " << iz << "  A= " << theA[iz]
             << " CFactorP= " << theCoulombFacP[iz] << " CFactorN= " << theCoulombFacN[iz]
             << G4endl;
    }
  }
}
