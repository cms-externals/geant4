...
#ifndef G4PHOTONUCLEARCROSSSECTION_HH
#define G4PHOTONUCLEARCROSSSECTION_HH
...
class G4PhotoNuclearCrossSection : public G4VCrossSectionDataSet
{
  public:
    ...
    G4PhotoNuclearCrossSection& operator=(const G4PhotoNuclearCrossSection& right) = delete;
    G4PhotoNuclearCrossSection(const G4PhotoNuclearCrossSection&) = delete;

  private:

    G4int GetFunctions(G4double a, G4double* y, G4double* z);
    G4double EquLinearFit(G4double X, G4int N, const G4double X0, const G4double XD,
                          const G4double* Y);
