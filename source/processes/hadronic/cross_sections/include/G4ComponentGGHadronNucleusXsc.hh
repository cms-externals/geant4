...
#ifndef G4COMPONENTGGHADRONNUCLEUSXSC_HH
#define G4COMPONENTGGHADRONNUCLEUSXSC_HH
...
inline G4double G4ComponentGGHadronNucleusXsc::GetIsoCrossSection(const G4DynamicParticle* dp,
                                                                  G4int Z, G4int A,
                                                                  const G4Isotope*,
                                                                  const G4Element*,
                                                                  const G4Material*)
...
inline G4double G4ComponentGGHadronNucleusXsc::GetElasticGlauberGribov(const G4DynamicParticle* dp,
                                                                       G4int Z, G4int A)
{
  ComputeCrossSections(dp->GetDefinition(), dp->GetKineticEnergy(), Z, A);
  return fElasticXsc;
...
inline G4double
G4ComponentGGHadronNucleusXsc::GetInelasticGlauberGribov(const G4DynamicParticle* dp, G4int Z,
                                                         G4int A)
{
  ComputeCrossSections(dp->GetDefinition(), dp->GetKineticEnergy(), Z, A);
...
inline G4double
G4ComponentGGHadronNucleusXsc::GetParticleBarCorTot(const G4ParticleDefinition* theParticle,
                                                    G4int ZZ)
{
  G4double cor = 1.0;
  G4int z = std::min(92, std::max(ZZ, 1));
  if (theParticle == theProton)
    cor = fProtonBarCorrectionTot[z];
  else if (theParticle == theNeutron)
    cor = fNeutronBarCorrectionTot[z];
  else if (theParticle == thePiPlus)
    cor = fPionPlusBarCorrectionTot[z];
  else if (theParticle == thePiMinus)
    cor = fPionMinusBarCorrectionTot[z];
  return cor;
}
...
inline G4double
G4ComponentGGHadronNucleusXsc::GetParticleBarCorIn(const G4ParticleDefinition* theParticle,
                                                   G4int ZZ)
{
  G4double cor = 1.0;
  G4int z = std::min(92, std::max(ZZ, 1));
  if (theParticle == theProton)
    cor = fProtonBarCorrectionIn[z];
  else if (theParticle == theNeutron)
    cor = fNeutronBarCorrectionIn[z];
  else if (theParticle == thePiPlus)
    cor = fPionPlusBarCorrectionIn[z];
  else if (theParticle == thePiMinus)
    cor = fPionMinusBarCorrectionIn[z];
  return cor;
}
