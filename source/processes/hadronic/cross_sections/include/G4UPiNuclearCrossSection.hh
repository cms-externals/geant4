...
private:
  ...
  G4double Interpolate(G4int Z, G4int A, G4double ekin, 
                       const G4PhysicsTable*, const G4bool isPiPlus) const;
...
{
  const G4bool isPiPlus = (dp->GetDefinition() == piPlus);
  const G4PhysicsTable* table = (isPiPlus) ? piPlusElastic : piMinusElastic; 
  return Interpolate(Z, A, dp->GetKineticEnergy(), table, isPiPlus);
}
...
{
  const G4bool isPiPlus = (dp->GetDefinition() == piPlus);
  const G4PhysicsTable* table = (isPiPlus) ? piPlusInelastic : piMinusInelastic; 
  return Interpolate(Z, A, dp->GetKineticEnergy(), table, isPiPlus);
}
