...
G4MuMinusCapturePrecompound::G4MuMinusCapturePrecompound(G4VPreCompoundModel* ptr)
...
G4HadFinalState* G4MuMinusCapturePrecompound::ApplyYourself(const G4HadProjectile& projectile,
                                                            G4Nucleus& targetNucleus)
{
  ...
  */
  ...
  if ((1 == Z && 1 == A) || (2 == Z && 3 == A))
  {
    ...
    //
    G4double e = 0.5 * (availableEnergy - residualMass * residualMass / availableEnergy);
  ...
  else
  {
    ...
    do
    {
      ...
      if (reentryCount > 100 && eEx < 0.0)
      {
        ...
        ed << "Target  Z= " << Z << "  A= " << A << "  Eex(MeV)= " << eEx / MeV << G4endl;
        ed << " ApplyYourself does not completed after 100 attempts -"
           << " excitation energy is set to zero";
        G4Exception("G4MuMinusCapturePrecompound::ApplyYourself", "had006", JustWarning, ed);
        momResidual.set(0.0, 0.0, 0.0, residualMass);
        eEx = 0.0;
      }
  ...
  if (verboseLevel > 1)
    G4cout << "G4MuMinusCapturePrecompound::ApplyYourself:  Nsec= "
           << result.GetNumberOfSecondaries() << " E0(MeV)= " << availableEnergy / MeV
           << " Mres(GeV)= " << residualMass / GeV << G4endl;
