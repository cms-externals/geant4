...
G4MuonicAtomDecay::G4MuonicAtomDecay(G4HadronicInteraction* hiptr, const G4String& name)
...
G4double G4MuonicAtomDecay::PostStepGetPhysicalInteractionLength(const G4Track&, G4double,
                                                                 G4ForceCondition* condition)
{
  *condition = NotForced;
...
G4double G4MuonicAtomDecay::GetMeanLifeTime(const G4Track& aTrack, G4ForceCondition*)
{
...
G4VParticleChange* G4MuonicAtomDecay::DecayIt(const G4Track& aTrack, const G4Step&)
{
  ...
  if (aTrack.GetTrackStatus() != fAlive && aTrack.GetTrackStatus() != fStopButAlive)
  {
  ...
  if (G4UniformRand() * lambda < lambdac)
  {
    ...
    if (decaychannel == nullptr)
    {
      ...
      ed << "Can not determine decay channel for " << aParticleDef->GetParticleName() << G4endl
      ...
      for (G4int ic = 0; ic < decaytable->entries(); ++ic)
      {
        ...
        ed << ic << ": BR " << dc->GetBR() << ", IsOK? " << dc->IsOKWithParentMass(checkedmass)
           << ", --> ";
    ...
    else
    {
      ...
      if (GetVerboseLevel() > 1)
      {
        G4cout << "G4MuonicAtomDecay::DecayIt  : selected decay channel  addr:" << decaychannel
               << G4endl;
      ...
      {
        G4ExceptionDescription ed;
        ed << "No products are generated for " << aParticleDef->GetParticleName();
        G4Exception("G4MuonicAtomDecay::DecayIt", "DECAY003", FatalException, ed);
        return &theTotalResult;
      }
#ifdef G4VERBOSE
    ...
    if (ParentEnergy < ParentMass)
    {
      ...
      G4Exception("G4MuonicAtomDecay::DecayIt ", "DECAY102", JustWarning,
  ...
  {  // either or
    ...
    G4HadProjectile theMuPro(
      G4DynamicParticle(G4MuonMinus::MuonMinus(), G4ThreeVector(0., 0., 0.)));
    ...
    do
    {
      ...
      catch (G4HadronicException& aR)
      {
        ...
        ed << "  Z= " << nucleus.GetZ_asInt() << "  A= " << nucleus.GetA_asInt() << G4endl;
        DumpState(aTrack, "ApplyYourself", ed);
        ed << " ApplyYourself failed" << G4endl;
        G4Exception("G4MuonicAtomDecay::DecayIt", "HAD_MAD_101", FatalException, ed);
      }
      ...
      if (reentryCount > 100)
      {
        ...
        G4Exception("G4MuonicAtomDecay::DecayIt", "HAD_MAD_102", FatalException, ed);
      }
    ...
    for (G4int i = 0; i < nsec; ++i)
    {
      ...
      if (GetVerboseLevel() > 1)
      {
        G4cout << "G4MuonicAtomDecay::DecayIt time set to: " << (maDTime + ctime) / ns << "[ns]"
               << G4endl;
      }
#endif
    }
...
{
  outFile << "MuonicAtom process where Muon decays in orbit or is captured by the nucleus."
          << G4endl;
}
...
void G4MuonicAtomDecay::DumpState(const G4Track& aTrack, const G4String& method,
                                  G4ExceptionDescription& ed)
{
