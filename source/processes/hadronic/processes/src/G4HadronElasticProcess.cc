...
G4HadronElasticProcess::G4HadronElasticProcess(const G4String& pName)
  : G4HadronicProcess(pName, fHadronElastic), fDiffraction(nullptr), fDiffractionRatio(nullptr)
{}
...
G4VParticleChange* G4HadronElasticProcess::PostStepDoIt(const G4Track& track, const G4Step&)
{
  ...
  if (fXSType != fHadNoIntegral)
  {
    ...
    G4double xs =
      aScaleFactor * theCrossSectionDataStore->ComputeCrossSection(dynParticle, material);
  ...
  // Select element
  const G4Element* elm = theCrossSectionDataStore->SampleZandA(dynParticle, material, *targNucleus);
  ...
  {
    G4double ratio = fDiffractionRatio->ComputeRatio(part, kineticEnergy, targNucleus->GetZ_asInt(),
    ...
    if (ratio > 0.0 && G4UniformRand() < ratio)
    {
      ...
      catch (G4HadronicException& aR)
      {
        ...
        ed << part->GetParticleName() << " off target element " << elm->GetName()
           << "  Z= " << targNucleus->GetZ_asInt() << "  A= " << targNucleus->GetA_asInt()
           << G4endl;
        DumpState(track, "ApplyYourself", ed);
        ed << " ApplyYourself failed" << G4endl;
        G4Exception("G4HadronElasticProcess::PostStepDoIt", "had006", FatalException, ed);
      }
  ...
  if (nullptr == hadi)
  {
    G4ExceptionDescription ed;
    ed << part->GetParticleName() << " off target element " << elm->GetName()
       << "  Z= " << targNucleus->GetZ_asInt() << "  A= " << targNucleus->GetA_asInt() << G4endl;
    DumpState(track, "ChooseHadronicInteraction", ed);
    ed << " No HadronicInteraction found out" << G4endl;
    G4Exception("G4HadronElasticProcess::PostStepDoIt", "had005", FatalException, ed);
    return theTotalResult;
  }
...
void G4HadronElasticProcess::SetDiffraction(G4HadronicInteraction* hi, G4VCrossSectionRatio* xsr)
{
  if (hi && xsr)
  {
    fDiffraction = hi;
