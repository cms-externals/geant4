...
void G4NuVacOscProcess::InitParameters()
{
  ...
  u21 = -s12 * c23 - s13 * s23 * c12 * expdcp;
  u22 = c12 * c23 - s12 * s23 * s13 * expdcp;
  u23 = c13 * s23;

  u31 = s12 * s23 - s13 * c12 * c23 * expdcp;
  u32 = -c12 * s23 - s12 * s13 * c23 * expdcp;
  u33 = c13 * c23;
...
G4double G4NuVacOscProcess::GetMeanFreePath(const G4Track& aTrack, G4double, G4ForceCondition*)
{
  const G4String rName = aTrack.GetStep()
                           ->GetPreStepPoint()
                           ->GetPhysicalVolume()
                           ->GetLogicalVolume()
                           ->GetRegion()
...
G4VParticleChange* G4NuVacOscProcess::PostStepDoIt(const G4Track& track, const G4Step& step)
{
  ...
  G4int aa = 2;
  if (part == theNuE || part == theAntiNuE)
  {
    aa = 0;
  }
  else if (part == theNuMu || part == theAntiNuMu)
  {
    aa = 1;
  }
  ...
  {
    if (!fAnti)
      aLept = new G4DynamicParticle(theNuE, lv1);
    else
      aLept = new G4DynamicParticle(theAntiNuE, lv1);
  }
  else if (bb == 1)
  {
    if (!fAnti)
      aLept = new G4DynamicParticle(theNuMu, lv1);
    else
      aLept = new G4DynamicParticle(theAntiNuMu, lv1);
  }
  else if (bb == 2)
  {
    if (!fAnti)
      aLept = new G4DynamicParticle(theNuTau, lv1);
    else
      aLept = new G4DynamicParticle(theAntiNuTau, lv1);
  }
...
G4int G4NuVacOscProcess::NuVacProbability(G4int aa, G4double Enu, G4double Lnu)
{
  ...
  else
  {
    ...
    if (rr <= probab)
      return bb;
    else if (rr > probab && rr <= probab + probac)
      return cc;
    else
      return aa;
  }
