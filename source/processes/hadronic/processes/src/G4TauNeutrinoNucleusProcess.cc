...
G4double G4TauNeutrinoNucleusProcess::PostStepGetPhysicalInteractionLength(
...
G4double G4TauNeutrinoNucleusProcess::GetMeanFreePath(const G4Track& aTrack, G4double,
                                                      G4ForceCondition*)
{
...
G4VParticleChange* G4TauNeutrinoNucleusProcess::PostStepDoIt(const G4Track& track,
                                                             const G4Step& step)
{
  ...
  if (track.GetTrackStatus() != fAlive && track.GetTrackStatus() != fSuspend)
  {
        ...
        || track.GetTrackStatus() == fPostponeToNextEvent)
    {
      ...
      ed << "G4TauNeutrinoNucleusProcess: track in unusable state - " << track.GetTrackStatus()
         << G4endl;
  ...
  else  // Nc-model
  {
    ...
    if (verboseLevel > 1)
    {
      G4cout << "G4TauNeutrinoNucleusProcess::PostStepDoIt for " << part->GetParticleName()
             << " in " << material->GetName() << " Target Z= " << targNucleus->GetZ_asInt()
    ...
    catch (G4HadronicException& aR)
    {
      ...
      G4Exception("G4TauNeutrinoNucleusProcess::PostStepDoIt", "had006", FatalException, ed);
    }
    // directions
