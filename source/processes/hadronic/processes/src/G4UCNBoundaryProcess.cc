...
G4UCNBoundaryProcess::G4UCNBoundaryProcess(const G4String& processName, G4ProcessType type)
...
G4VParticleChange* G4UCNBoundaryProcess::PostStepDoIt(const G4Track& aTrack, const G4Step& aStep)
{
  ...
  aMaterialPropertiesTable1 =
    (G4UCNMaterialPropertiesTable*)Material1->GetMaterialPropertiesTable();
  aMaterialPropertiesTable2 =
    (G4UCNMaterialPropertiesTable*)Material2->GetMaterialPropertiesTable();
  ...
  G4ThreeVector theGlobalNormal = (iNav[hNavId])->GetGlobalExitNormal(theGlobalPoint, &valid);
  ...
  else
  {
    ...
    G4Exception("G4UCNBoundaryProcess::PostStepDoIt", "UCNBoun01", EventMustBeAborted, ed,
  ...
  if (OldMomentum * theGlobalNormal > 0.0)
  {
    ...
    ed << "  So it MUST have dot < 0 with the normal that Exits the new volume (globalNormal)."
       << G4endl;
    ...
    G4Exception("G4UCNBoundaryProcess::PostStepDoIt", "UCNBoun02",
                ...
                ed,
                "Invalid Surface Normal - Geometry must return valid surface normal pointing in "
                "the right direction");
#else
    theGlobalNormal = -theGlobalNormal;
#endif
  ...
  G4double theVelocityNormal = aTrack.GetVelocity() * (OldMomentum * theGlobalNormal);
  ...
  else
  {
    ...
    if (!aMaterialPropertiesTable2->ConditionsValid(Energy, FermiPotDiff, theta_i))
    {
      nNoMRCondition++;
  ...
  if (DoMicroRoughnessReflection)
  {
    ...
    MRpDiffuse = aMaterialPropertiesTable2->GetMRIntProbability(theta_i, Energy);

    // Integral probability for non-specular transmission with microroughness

    MRpDiffuseTrans = aMaterialPropertiesTable2->GetMRIntTransProbability(theta_i, Energy);
    ...
    if (verboseLevel > 1)
    {
      ...
      G4cout << "Reflection_prob: " << MRpDiffuse << ", Transmission_prob: " << MRpDiffuseTrans
             << G4endl;
    }
  }
  ...
  if (!High(Enormal, FermiPotDiff))
  {
    ...
    if (DoMicroRoughnessReflection)
      NewMomentum = MRreflect(MRpDiffuse, OldMomentum, theGlobalNormal, Energy, FermiPotDiff);
    else
  ...
  else
  {
    ...
    if (DoMicroRoughnessReflection)
    {
      ...
      NewMomentum = MRreflectHigh(MRpDiffuse, MRpDiffuseTrans, 0., OldMomentum, theGlobalNormal,
                                  Energy, FermiPotDiff, Enew);
    ...
    else
    {
      ...
      if (verboseLevel > 1) G4cout << "UCNBoundaryProcess: reflectivity " << reflectivity << G4endl;
      ...
      else
      {
        ...
        G4double mass =
          -std::sqrt(theMomentumNormal * theMomentumNormal - neutron_mass_c2 * 2. * FermiPotDiff);

        // --- Momentum direction in new media

        NewMomentum = theNeutronMomentum - (theMomentumNormal - mass) * theGlobalNormal;
        ...
        if (verboseLevel > 1)
        {
          G4cout << "Energy: " << Energy / neV << "neV, Enormal: " << Enormal / neV
                 << "neV, fpdiff " << FermiPotDiff / neV << "neV, Enew " << Enew / neV << "neV"
                 << G4endl;
...
G4double G4UCNBoundaryProcess::GetMeanFreePath(const G4Track&, G4double,
...
G4bool G4UCNBoundaryProcess::Loss(G4double pUpScatter, G4double theVelocityNormal,
                                  G4double FermiPot)
{
...
G4ThreeVector G4UCNBoundaryProcess::Reflect(G4double pDiffuse, G4ThreeVector OldMomentum,
...
G4ThreeVector G4UCNBoundaryProcess::MRreflect(G4double pDiffuse, G4ThreeVector OldMomentum,
                                              G4ThreeVector Normal, G4double Energy,
                                              G4double FermiPot)
{
  ...
  if (G4UniformRand() <= pDiffuse)
  {
    ...
    NewMomentum = MRDiffRefl(Normal, Energy, FermiPot, OldMomentum, pDiffuse);
...
G4ThreeVector G4UCNBoundaryProcess::MRreflectHigh(G4double pDiffuse, G4double pDiffuseTrans,
                                                  G4double pLoss, G4ThreeVector OldMomentum,
                                                  G4ThreeVector Normal, G4double Energy,
                                                  G4double FermiPot, G4double& Enew)
{
  // Only for Enormal > VFermi
  ...
  G4double pSpecular = Reflectivity(FermiPot, Enormal) * (1. - pDiffuse - pDiffuseTrans - pLoss);
  ...
  if (decide < pSpecular + pDiffuse)
  {
    ...
    if (verboseLevel > 0) G4cout << "Diffuse normal " << Normal << ", " << NewMomentum << G4endl;
    Enew = Energy;
  ...
  if (decide < pSpecular + pDiffuse + pDiffuseTrans)
  {
    ...
    NewMomentum = MRDiffTrans(Normal, Energy, FermiPot, OldMomentum, pDiffuseTrans);
  ...
  NewMomentum =
    palt * OldMomentum
    - (palt * produ
       + std::sqrt(palt * palt * produ * produ - 2 * neutron_mass_c2 / c_squared * FermiPot))
        * Normal;
...
G4ThreeVector G4UCNBoundaryProcess::MRDiffRefl(G4ThreeVector Normal, G4double Energy,
                                               G4double FermiPot, G4ThreeVector OldMomentum,
                                               G4double pDiffuse)
{
  ...
  while (!accepted)
  {
    ...
    if (1.5 * G4UniformRand() * aMaterialPropertiesTable2->GetMRMaxProbability(theta_i, Energy)
          / pDiffuse
        <= aMaterialPropertiesTable2->GetMRProbability(theta_i, Energy, FermiPot, theta_o, phi_o)
             / pDiffuse)
    ...
    if (aMaterialPropertiesTable2->GetMRProbability(theta_i, Energy, FermiPot, theta_o, phi_o)
          / (1.5 * aMaterialPropertiesTable2->GetMRMaxProbability(theta_i, Energy))
        > 1)
    {
      G4cout << "MRMax Wahrscheinlichkeitsueberschreitung!" << G4endl;
      G4cout << aMaterialPropertiesTable2->GetMRProbability(theta_i, Energy, FermiPot, theta_o,
                                                            phi_o)
                  / (1.5 * aMaterialPropertiesTable2->GetMRMaxProbability(theta_i, Energy))
             << G4endl;
      aMaterialPropertiesTable2->SetMRMaxProbability(
        theta_i, Energy,
        aMaterialPropertiesTable2->GetMRProbability(theta_i, Energy, FermiPot, theta_o, phi_o));
    }
  ...
  G4RotationMatrix TransCoord = GetCoordinateTransformMatrix(Normal, OldMomentum);
...
G4ThreeVector G4UCNBoundaryProcess::MRDiffTrans(G4ThreeVector Normal, G4double Energy,
                                                G4double FermiPot, G4ThreeVector OldMomentum,
                                                G4double pDiffuseTrans)
{
  ...
  while (!accepted)
  {
    ...
    if (1.5 * G4UniformRand() * aMaterialPropertiesTable2->GetMRMaxTransProbability(theta_i, Energy)
          / pDiffuseTrans
        <= aMaterialPropertiesTable2->GetMRTransProbability(theta_i, Energy, FermiPot, theta_o,
                                                            phi_o)
             / pDiffuseTrans)
    ...
    if (aMaterialPropertiesTable2->GetMRTransProbability(theta_i, Energy, FermiPot, theta_o, phi_o)
          / (1.5 * aMaterialPropertiesTable2->GetMRMaxTransProbability(theta_i, Energy))
        > 1)
    {
      G4cout << "MRMaxTrans Wahrscheinlichkeitsueberschreitung!" << G4endl;
      G4cout << aMaterialPropertiesTable2->GetMRTransProbability(theta_i, Energy, FermiPot, theta_o,
                                                                 phi_o)
                  / (1.5 * aMaterialPropertiesTable2->GetMRMaxTransProbability(theta_i, Energy))
             << G4endl;
      aMaterialPropertiesTable2->SetMRMaxTransProbability(
        theta_i, Energy,
        aMaterialPropertiesTable2->GetMRTransProbability(theta_i, Energy, FermiPot, theta_o,
                                                         phi_o));
    }
...
void G4UCNBoundaryProcess::BoundaryProcessVerbose() const
{
  ...
  if (theStatus == NoMPT) G4cout << " *** No G4UCNMaterialPropertiesTable *** " << G4endl;
  if (theStatus == NoMRT) G4cout << " *** No MicroRoughness Table *** " << G4endl;
  if (theStatus == NoMRCondition)
    G4cout << " *** MicroRoughness Condition not satisfied *** " << G4endl;
  if (theStatus == Absorption) G4cout << " *** Loss on Surface *** " << G4endl;
  if (theStatus == Ezero) G4cout << " *** Ezero on Surface *** " << G4endl;
  if (theStatus == Flip) G4cout << " *** Spin Flip on Surface *** " << G4endl;
  if (theStatus == SpecularReflection) G4cout << " *** Specular Reflection *** " << G4endl;
  if (theStatus == LambertianReflection) G4cout << " *** LambertianR Reflection *** " << G4endl;
  if (theStatus == MRDiffuseReflection) G4cout << " *** MR Model Diffuse Reflection *** " << G4endl;
  if (theStatus == SnellTransmit) G4cout << " *** Snell Transmission *** " << G4endl;
  if (theStatus == MRDiffuseTransmit) G4cout << " *** MR Model Diffuse Transmission *** " << G4endl;
}

void G4UCNBoundaryProcess::BoundaryProcessSummary(void) const
{
  G4cout << "Sum NoMT:                            " << nNoMPT << G4endl;
  G4cout << "Sum NoMRT:                           " << nNoMRT << G4endl;
  G4cout << "Sum NoMRCondition:                   " << nNoMRCondition << G4endl;
  G4cout << "Sum No. E < V Loss:                  " << nAbsorption << G4endl;
  G4cout << "Sum No. E > V Ezero:                 " << nEzero << G4endl;
  G4cout << "Sum No. E < V SpinFlip:              " << nFlip << G4endl;
  G4cout << "Sum No. E > V Specular Reflection:   " << aSpecularReflection << G4endl;
  G4cout << "Sum No. E < V Specular Reflection:   " << bSpecularReflection << G4endl;
  G4cout << "Sum No. E < V Lambertian Reflection: " << bLambertianReflection << G4endl;
  G4cout << "Sum No. E > V MR DiffuseReflection:  " << aMRDiffuseReflection << G4endl;
  G4cout << "Sum No. E < V MR DiffuseReflection:  " << bMRDiffuseReflection << G4endl;
  G4cout << "Sum No. E > V SnellTransmit:         " << nSnellTransmit << G4endl;
  G4cout << "Sum No. E > V MR SnellTransmit:      " << mSnellTransmit << G4endl;
  G4cout << "Sum No. E > V DiffuseTransmit:       " << aMRDiffuseTransmit << G4endl;
  G4cout << "                                     " << G4endl;
}
