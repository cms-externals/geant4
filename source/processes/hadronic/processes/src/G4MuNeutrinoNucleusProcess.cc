...
// 24.04.19 V. Grichine - G4Region name and optionally total cross section biased in the region
// only.
...
G4MuNeutrinoNucleusProcess::G4MuNeutrinoNucleusProcess(const G4String& anEnvelopeName,
...
G4double G4MuNeutrinoNucleusProcess::PostStepGetPhysicalInteractionLength(
  const G4Track& track, G4double previousStepSize, G4ForceCondition* condition)
{
  return G4VDiscreteProcess::PostStepGetPhysicalInteractionLength(track, previousStepSize,
                                                                  condition);
}

//////////////////////////////////////////////////

G4double G4MuNeutrinoNucleusProcess::GetMeanFreePath(const G4Track& aTrack, G4double,
                                                     G4ForceCondition*)
{
  // G4cout << "GetMeanFreePath " << aTrack.GetDefinition()->GetParticleName()
  //	 << " Ekin= " << aTrack.GetKineticEnergy() << G4endl;
  G4String rName = aTrack.GetStep()
                     ->GetPreStepPoint()
                     ->GetPhysicalVolume()
                     ->GetLogicalVolume()
                     ->GetRegion()
...
G4VParticleChange* G4MuNeutrinoNucleusProcess::PostStepDoIt(const G4Track& track,
                                                            const G4Step& step)
{
  G4String rName = track.GetStep()
                     ->GetPreStepPoint()
                     ->GetPhysicalVolume()
                     ->GetLogicalVolume()
                     ->GetRegion()
  ...
  //
  if (track.GetTrackStatus() != fAlive && track.GetTrackStatus() != fSuspend)
  {
        ...
        || track.GetTrackStatus() == fPostponeToNextEvent)
    {
      ...
      ed << "G4HadronicProcess: track in unusable state - " << track.GetTrackStatus() << G4endl;
  ...
  if (fNuNuclCcBias > 1.0
      || fNuNuclNcBias > 1.0)  // = true, if fBiasingfactor != 1., i.e. xsc is biased
  {
    ...
    G4double forward =
      track.GetVolume()->GetLogicalVolume()->GetSolid()->DistanceToOut(localP, localV);
    G4double backward =
      track.GetVolume()->GetLogicalVolume()->GetSolid()->DistanceToOut(localP, -localV);
  ...
  if (G4UniformRand() < ccTotRatio)  // Cc-model
  {
    ...
    if (pName == "nu_mu")
      ...
      hadi = (GetHadronicInteractionList())[2];
  ...
  else  // Nc-model
  {
    if (pName == "nu_mu")
      ...
      hadi = (GetHadronicInteractionList())[3];
    ...
    if (verboseLevel > 1)
    {
      G4cout << "G4MuNeutrinoNucleusProcess::PostStepDoIt for " << part->GetParticleName() << " in "
             << material->GetName() << " Target Z= " << targNucleus->GetZ_asInt()
    ...
    catch (G4HadronicException& aR)
    {
      ...
      ed << "  Z= " << targNucleus->GetZ_asInt() << "  A= " << targNucleus->GetA_asInt() << G4endl;
      DumpState(track, "ApplyYourself", ed);
      ed << " ApplyYourself failed" << G4endl;
      G4Exception("G4MuNeutrinoNucleusProcess::PostStepDoIt", "had006", FatalException, ed);
    }
    // directions
    ...
    if (verboseLevel > 1)
    {
      G4cout << "Efin= " << result->GetEnergyChange() << " de= " << result->GetLocalEnergyDeposit()
             << " nsec= " << result->GetNumberOfSecondaries() << " dir= " << outdir << G4endl;
    }
    // energies
...
void G4MuNeutrinoNucleusProcess::SetLowestEnergy(G4double val)
{
  lowestEnergy = val;
}
