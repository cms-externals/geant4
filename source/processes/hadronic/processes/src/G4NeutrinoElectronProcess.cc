...
// 03.10.18 V. Grichine - G4Region name and optionally total cross section biased in the region
// only.
...
G4double G4NeutrinoElectronProcess::PostStepGetPhysicalInteractionLength(
...
G4double G4NeutrinoElectronProcess::GetMeanFreePath(const G4Track& aTrack, G4double,
                                                    G4ForceCondition*)
{
...
G4VParticleChange* G4NeutrinoElectronProcess::PostStepDoIt(const G4Track& track, const G4Step& step)
{
  ...
  if (fNuEleCcBias > 1.0
      || fNuEleNcBias > 1.0)  // = true, if fBiasingfactor != 1., i.e. xsc is biased
  {
  ...
  catch (G4HadronicException& aR)
  {
    ...
    G4Exception("G4NeutrinoElectronProcess::PostStepDoIt", "had003", FatalException, ed);
  }
  ...
  else  // Nc-model, like 'elastic', 2->2 scattering
  {
    ...
    if (verboseLevel > 1)
    {
      G4cout << "G4NeutrinoElectronProcess::PostStepDoIt for " << part->GetParticleName() << " in "
    ...
    catch (G4HadronicException& aR)
    {
      ...
      ed << "Target element " << elm->GetName() << "  Z= " << targNucleus->GetZ_asInt()
         << "  A= " << targNucleus->GetA_asInt() << G4endl;
      DumpState(track, "ApplyYourself", ed);
      ed << " ApplyYourself failed" << G4endl;
      G4Exception("G4NeutrinoElectronProcess::PostStepDoIt", "had006", FatalException, ed);
    }
    // directions
