...
G4double G4ElNeutrinoNucleusProcess::PostStepGetPhysicalInteractionLength(
...
G4double G4ElNeutrinoNucleusProcess::GetMeanFreePath(const G4Track& aTrack, G4double,
                                                     G4ForceCondition*)
{
...
G4VParticleChange* G4ElNeutrinoNucleusProcess::PostStepDoIt(const G4Track& track,
                                                            const G4Step& step)
{
  ...
  else  // Nc-model
  {
    ...
    if (verboseLevel > 1)
    {
      G4cout << "G4ElNeutrinoNucleusProcess::PostStepDoIt for " << part->GetParticleName() << " in "
    ...
    catch (G4HadronicException& aR)
    {
      ...
      G4Exception("G4ElNeutrinoNucleusProcess::PostStepDoIt", "had006", FatalException, ed);
    }
    // directions
