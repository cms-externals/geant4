...
G4VCrossSectionDataSet* G4NeutronGeneralProcess::InitialisationXS(G4HadronicProcess* proc)
{
...
G4VCrossSectionDataSet* G4NeutronGeneralProcess::GetXSection(G4int subtype)
{
  ...
  if (type == fHadronElastic)
  {
    xs = fElasticXS;
  }
  else if (type == fHadronInelastic)
  {
    xs = fInelasticXS;
  }
  else if (type == fCapture)
  {
    xs = fCaptureXS;
  }
  return xs;
...
void G4NeutronGeneralProcess::PreparePhysicsTable(const G4ParticleDefinition& part)
{
  if (1 < verboseLevel)
  {
    G4cout << "G4NeutronGeneralProcess::PreparePhysicsTable() for " << GetProcessName()
           << " and particle " << part.GetParticleName() << " isMaster: " << isMaster << G4endl;
  }
  ...
  if (noEl || noInel || noCap)
  {
    ...
    if (noEl)
    {
      ed << "Neutron elastic process is not defined" << G4endl;
    }
    if (noInel)
    {
      ed << "Neutron inelastic process is not defined" << G4endl;
    }
    if (noCap)
    {
      ed << "Neutron capture process is not defined" << G4endl;
    }
    G4Exception("G4NeutronGeneralProcess::PreparePhysicsTable(..)", "had001", FatalException, ed,
                "");
    return;
  }
...
void G4NeutronGeneralProcess::BuildPhysicsTable(const G4ParticleDefinition& part)
{
  if (1 < verboseLevel)
  {
    G4cout << "### G4NeutronGeneralProcess::BuildPhysicsTable() for " << GetProcessName()
  ...
  if (isMaster)
  {
    ...
    for (std::size_t i = 0; i < nmat; ++i)
    {
      ...
      if (1 < verboseLevel)
      {
        G4cout << "======= Zone 0 ======= N= " << nn << " for " << mat->GetName() << G4endl;
      }
      ...
      for (std::size_t j = 0; j < nn; ++j)
      {
        ...
        {
          G4cout << j << ". E= " << e << " xs=" << sum << " sigEl=" << sigEl
                 << " sigInel=" << sigInel << " factInel=" << fXSFactorInel << G4endl;
        }
...
G4double G4NeutronGeneralProcess::ComputeCrossSection(G4VCrossSectionDataSet* xs,
                                                      const G4Material* mat, G4double e,
                                                      G4double loge)
{
  ...
  {
    sig += natom[i] * xs->ComputeCrossSectionPerElement(e, loge, fNeutron, mat->GetElement(i), mat);
  }
  return sig;
}
                                                  ...
                                                  const G4String& directory, G4bool ascii)
{
  ...
  {
    G4String nam =
      (0 == i || 3 == i) ? "LambdaNeutronGeneral" + nameT[i] : "ProbNeutronGeneral" + nameT[i];
...
G4double G4NeutronGeneralProcess::GetMeanFreePath(const G4Track& track, G4double,
                                                  G4ForceCondition* condition)
{
  *condition = NotForced;
  // recompute total cross section if needed
...
{
  return (nullptr != fSelectedProc) ? fSelectedProc->GetProcessSubType() : fNeutronGeneral;
}
