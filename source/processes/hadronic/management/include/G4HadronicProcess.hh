...
#ifndef G4HADRONICPROCESS_HH
#define G4HADRONICPROCESS_HH
...
class G4HadronicProcess : public G4VDiscreteProcess
{
  public:
    ...
    G4double GetElementCrossSection(const G4DynamicParticle* part, const G4Element* elm,
                                    const G4Material* mat = nullptr);

    // obsolete method to get cross section per element
    inline G4double GetMicroscopicCrossSection(const G4DynamicParticle* part, const G4Element* elm,
                                               const G4Material* mat = nullptr);
  ...
  protected:
    ...
    inline G4HadronicInteraction* ChooseHadronicInteraction(const G4HadProjectile& aHadProjectile,
    ...
    G4HadFinalState* CheckResult(const G4HadProjectile& thePro, const G4Nucleus& targetNucleus,
...
inline G4double G4HadronicProcess::GetMicroscopicCrossSection(const G4DynamicParticle* part,
                                                              const G4Element* elm,
...
inline G4HadronicInteraction* G4HadronicProcess::GetHadronicInteraction() const
{
  return theInteraction;
}

inline const G4Nucleus* G4HadronicProcess::GetTargetNucleus() const
{
...
inline G4HadXSType G4HadronicProcess::CrossSectionType() const
{
  return fXSType;
}

inline void G4HadronicProcess::SetCrossSectionType(G4HadXSType val)
{
  fXSType = val;
}
...
inline void G4HadronicProcess::SetEnergyMomentumCheckLevels(G4double relativeLevel,
...
inline std::pair<G4double, G4double> G4HadronicProcess::GetEnergyMomentumCheckLevels() const
{
  return epCheckLevels;
}

inline G4CrossSectionDataStore* G4HadronicProcess::GetCrossSectionDataStore()
{
  return theCrossSectionDataStore;
}

inline std::vector<G4TwoPeaksHadXS*>* G4HadronicProcess::TwoPeaksXS() const
{
  return fXSpeaks;
}

inline std::vector<G4double>* G4HadronicProcess::EnergyOfCrossSectionMax() const
{
  return theEnergyOfCrossSectionMax;
}

inline G4HadronicInteraction*
G4HadronicProcess::ChooseHadronicInteraction(const G4HadProjectile& aHadProjectile,
                                             G4Nucleus& aTargetNucleus, const G4Material* aMaterial,
                                             const G4Element* anElement)
{
  return theEnergyRangeManager.GetHadronicInteraction(aHadProjectile, aTargetNucleus, aMaterial,
                                                      anElement);
}
...
{
  theLastCrossSection =
    aScaleFactor * theCrossSectionDataStore->GetCrossSection(fDynParticle, currentMat);
...
{
  theLastCrossSection =
    aScaleFactor * theCrossSectionDataStore->ComputeCrossSection(fDynParticle, currentMat);
  theMFP = (theLastCrossSection > 0.0) ? 1.0 / theLastCrossSection : DBL_MAX;
}
