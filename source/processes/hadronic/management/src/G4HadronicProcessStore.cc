...
G4double G4HadronicProcessStore::GetInteractionLength(
    const G4Material* material,
    const G4ParticleDefinition* aParticle,
    const G4double kineticEnergy) 
{
  ...
  {
    auto xsec = GetInelasticCrossSectionPerAtom(
                     aParticle,kineticEnergy,(*theElementVector)[i],material);
    if(xsec==0.) { return DBL_MAX; }
    G4double elemMassIntLen = 
                    (*theElementVector)[i]->GetA() / (CLHEP::Avogadro * xsec);
