...
G4HadronicProcessStore::G4HadronicProcessStore()
{
  theGenericIon = G4ParticleTable::GetParticleTable()->FindParticle("GenericIon");
...
G4double G4HadronicProcessStore::GetCrossSectionPerAtom(const G4ParticleDefinition* part,
                                                        G4double energy, const G4VProcess* proc,
...
G4double G4HadronicProcessStore::GetCrossSectionPerVolume(const G4ParticleDefinition* part,
                                                          G4double energy, const G4VProcess* proc,
                                                          const G4Material* material)
{
  G4double cross = 0.;
...
G4double G4HadronicProcessStore::GetElasticCrossSectionPerVolume(
  const G4ParticleDefinition* aParticle, G4double kineticEnergy, const G4Material* material)
{
  G4double cross = 0.0;
...
G4double G4HadronicProcessStore::GetElasticCrossSectionPerAtom(
  const G4ParticleDefinition* aParticle, G4double kineticEnergy, const G4Element* anElement,
...
G4double G4HadronicProcessStore::GetElasticCrossSectionPerIsotope(const G4ParticleDefinition*,
...
G4double G4HadronicProcessStore::GetInelasticCrossSectionPerIsotope(const G4ParticleDefinition*,
                                                                    G4double, G4int, G4int)
{
  return 0.0;
}
...
G4double G4HadronicProcessStore::GetCaptureCrossSectionPerIsotope(const G4ParticleDefinition*,
                                                                  G4double, G4int, G4int)
{
  return 0.0;
}
...
G4double G4HadronicProcessStore::GetFissionCrossSectionPerIsotope(const G4ParticleDefinition*,
                                                                  G4double, G4int, G4int)
{
  return 0.0;
}
...
G4double
G4HadronicProcessStore::GetChargeExchangeCrossSectionPerIsotope(const G4ParticleDefinition*,
                                                                G4double, G4int, G4int)
{
  return 0.0;
}
...
G4double G4HadronicProcessStore::GetInteractionLength(const G4Material* material,
                                                      const G4ParticleDefinition* aParticle,
                                                      const G4double kineticEnergy)
{
  const G4ElementVector* theElementVector = material->GetElementVector();
  const G4double* theFractionalMassVector = material->GetFractionVector();
  G4double massIntLen = 0.;
  for (size_t i = 0; i < theElementVector->size(); i++)
  {
    auto xsec =
      GetInelasticCrossSectionPerAtom(aParticle, kineticEnergy, (*theElementVector)[i], material);
    if (xsec == 0.)
    {
      return DBL_MAX;
    }
    G4double elemMassIntLen = (*theElementVector)[i]->GetA() / (CLHEP::Avogadro * xsec);
    massIntLen += theFractionalMassVector[i] / elemMassIntLen;
  }
  G4double intLen = 1. / (material->GetDensity() * massIntLen);
  return intLen;
}
                                              ...
                                              const G4ParticleDefinition* part)
{
  ...
  for (; i < n_proc; ++i)
  {
    if (process[i] == proc) break;
  }
  G4int j = 0;
  for (; j < n_part; ++j)
  {
    if (particle[j] == part) break;
  }

  if (1 < param->GetVerboseLevel())
  {
    G4cout << "G4HadronicProcessStore::RegisterParticle " << part->GetParticleName() << " for  "
           << proc->GetProcessName() << G4endl;
  }
  if (j == n_part)
  {
...
void G4HadronicProcessStore::DeRegisterExtraProcess(G4VProcess* proc)
{
  for (G4int i = 0; i < n_extra; ++i)
  {
    if (extraProcess[i] == proc)
    {
      ...
      if (1 < param->GetVerboseLevel())
      {
        G4cout << "Extra Process: " << i << "  " << proc->GetProcessName() << " is deregisted "
               << G4endl;
      }
      return;
    }
                                       ...
                                       std::ofstream& outFile)
{
  ...
  outFile << "<br> <li><h2><font color=\" ff0000 \">" << theParticle->GetParticleName()
          << "</font></h2></li>\n";

  typedef std::multimap<PD, HP, std::less<PD>> PDHPmap;
  typedef std::multimap<HP, HI, std::less<HP>> HPHImap;

  std::pair<PDHPmap::iterator, PDHPmap::iterator> itpart = p_map.equal_range(theParticle);
  ...
  for (PDHPmap::iterator it = itpart.first; it != itpart.second; ++it)
  {
    ...
    std::pair<HPHImap::iterator, HPHImap::iterator> itmod = m_map.equal_range(theProcess);

    outFile << "    <ul>\n";

    for (HPHImap::iterator jt = itmod.first; jt != itmod.second; ++jt)
    {
      outFile << "    <li><b><a href=\"" << pl << "_" << HtmlFileName((*jt).second->GetModelName())
              << "\"> " << (*jt).second->GetModelName() << "</a>"
              << " from " << (*jt).second->GetMinEnergy() / GeV << " GeV to "
              << (*jt).second->GetMaxEnergy() / GeV << " GeV </b></li>\n";
...
G4String G4HadronicProcessStore::HtmlFileName(const G4String& in) const
{
  ...
  // replace blanks:
  std::transform(str.begin(), str.end(), str.begin(), [](char ch) { return ch == ' ' ? '_' : ch; });
  str = str + ".html";
  return str;
}
...
void G4HadronicProcessStore::Dump(G4int verb)
{
  ...
  G4cout << "\n====================================================================\n"
         << std::setw(60) << "HADRONIC PROCESSES SUMMARY (verbose level " << level << ")" << G4endl;
  ...
  for (G4int i = 0; i < n_part; ++i)
  {
    ...
    if (level == 1
        && (pname == "proton" || pname == "neutron" || pname == "deuteron" || pname == "triton"
            || pname == "He3" || pname == "alpha" || pname == "pi+" || pname == "pi-"
            || pname == "gamma" || pname == "e+" || pname == "e-" || pname == "nu_e"
            || pname == "anti_nu_e" || pname == "nu_mu" || pname == "anti_nu_mu" || pname == "mu+"
            || pname == "mu-" || pname == "kaon+" || pname == "kaon-" || pname == "lambda"
            || pname == "anti_lambda" || pname == "sigma-" || pname == "D-" || pname == "B-"
            || pname == "GenericIon" || pname == "hypertriton" || pname == "anti_neutron"
            || pname == "anti_proton" || pname == "anti_deuteron" || pname == "anti_triton"
            || pname == "anti_He3" || pname == "anti_alpha" || pname == "anti_hypertriton"))
      yes = true;
    if (level > 1) yes = true;
    if (yes)
    {
      ...
      for (itp = ep_map.lower_bound(part); itp != ep_map.upper_bound(part); ++itp)
      {
        if (itp->first == part)
        {
          ...
          {
            G4cout << "-------------------------------------------------------------------------\n"
                   << std::setw(50) << "Hadronic Processes for " << part->GetParticleName() << "\n";
            wasPrinted[i] = 1;
          }
...
void G4HadronicProcessStore::Print(G4int idxProc, G4int idxPart)
{
  ...
  if (part == G4GenericIon::Definition() || std::abs(part->GetBaryonNumber()) > 1)
  {
                                                       ...
                                                       G4HadronicProcessType subType)
{
  ...
  {
    std::multimap<PD, HP, std::less<PD>>::iterator it;
    for (it = p_map.lower_bound(currentParticle); it != p_map.upper_bound(currentParticle); ++it)
    {
      if (it->first == currentParticle && subType == (it->second)->GetProcessSubType())
      {
        hp = it->second;
        break;
      }
...
{
  G4cout << " Setting energy/momentum report level to " << level << " for " << process.size()
         << " hadronic processes " << G4endl;
