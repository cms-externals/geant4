...
G4HadFinalState* G4HadronicInteraction::ApplyYourself(const G4HadProjectile&, G4Nucleus&)
{
  return nullptr;
}

G4double G4HadronicInteraction::SampleInvariantT(const G4ParticleDefinition*, G4double, G4int,
                                                 G4int)
{
  return 0.0;
}

G4bool G4HadronicInteraction::IsApplicable(const G4HadProjectile&, G4Nucleus&)
{
  return true;
}
                                             ...
                                             const G4Element* anElement) const
{
  ...
  if (IsBlocked(aMaterial) || IsBlocked(anElement))
  {
    return DBL_MAX;
  }
