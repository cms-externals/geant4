...
G4EnergyRangeManager::G4EnergyRangeManager() : theHadronicInteractionCounter(0) {}
                                             ...
                                             const G4Element* anElement) const
{
  ...
  if (1 == theHadronicInteractionCounter)
  {
  ...
  {
    kineticEnergy /=
      static_cast<G4double>(std::abs(aHadProjectile.GetDefinition()->GetBaryonNumber()));
  }
...
void G4EnergyRangeManager::Dump(G4int verbose)
{
  ...
  for (G4int i = 0; i < theHadronicInteractionCounter; i++)
  {
    ...
    if (verbose > 0)
    {
      G4cout << "      Minimum Energy " << theHadronicInteraction[i]->GetMinEnergy() / GeV
             << " [GeV], "
