...
G4double G4RToEConvForElectron::ComputeValue(const G4int Z, const G4double kinEnergy)
{
  ...
  const G4double ionpot = 1.6e-5 * CLHEP::MeV * G4Exp(0.9 * Zlog) / CLHEP::electron_mass_c2;
  ...
  if (tau < taul)
  {
    ...
    G4double f =
      1. - beta2 + G4Log(tsq / 2.) + (0.5 + 0.25 * tsq + (1. + 2. * taul) * log05) / (t1 * t1);
  ...
  else
  {
    ...
    G4double f =
      1. - beta2 + G4Log(tsq / 2.) + (0.5 + 0.25 * tsq + (1. + 2. * tau) * log05) / (t1 * t1);
