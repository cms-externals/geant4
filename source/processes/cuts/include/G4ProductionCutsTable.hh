...
#define G4PRODUCTIONCUTSTABLE_HH
...
class G4ProductionCutsTable
{
  public:
    ...
    G4double ConvertRangeToEnergy(const G4ParticleDefinition* particle, const G4Material* material,
                                  G4double range);
...
inline std::size_t G4ProductionCutsTable::GetTableSize() const
{
  return coupleTable.size();
}

inline const G4MaterialCutsCouple* G4ProductionCutsTable::GetMaterialCutsCouple(G4int i) const
{
...
inline void G4ProductionCutsTable::PhysicsTableUpdated()
{
  for (auto itr = coupleTable.cbegin(); itr != coupleTable.cend(); ++itr)
  {
...
inline G4double* G4ProductionCutsTable::GetRangeCutsDoubleVector(std::size_t pcIdx) const
{
  return rangeDoubleVector[pcIdx];
}

inline G4double* G4ProductionCutsTable::GetEnergyCutsDoubleVector(std::size_t pcIdx) const
{
  return energyDoubleVector[pcIdx];
}

inline G4ProductionCuts* G4ProductionCutsTable::GetDefaultProductionCuts() const
{
  return defaultProductionCuts;
}

inline G4bool G4ProductionCutsTable::IsCoupleUsedInTheRegion(const G4MaterialCutsCouple* aCouple,
                                                             const G4Region* aRegion) const
{
  ...
  {
    if (aCouple->GetMaterial() == (*mItr) && aCouple->GetProductionCuts() == fProductionCut)
    {
      return true;
    }
  }
...
inline G4int G4ProductionCutsTable::GetCoupleIndex(const G4MaterialCutsCouple* aCouple) const
{
  G4int idx = 0;
  for (auto cItr = coupleTable.cbegin(); cItr != coupleTable.cend(); ++cItr)
  {
...
inline G4int G4ProductionCutsTable::GetCoupleIndex(const G4Material* aMat,
                                                   const G4ProductionCuts* aCut) const
{
...
inline G4int G4ProductionCutsTable::GetVerboseLevel() const
{
  return verboseLevel;
}

inline const G4MCCIndexConversionTable* G4ProductionCutsTable::GetMCCIndexConversionTable() const
{
