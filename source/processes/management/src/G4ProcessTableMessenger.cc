...
// --------------------------------------------------------------------
G4ProcessTableMessenger::G4ProcessTableMessenger(G4ProcessTable* pTable) : theProcessTable(pTable)
{
  ...
  verboseCmd->AvailableForStates(G4State_PreInit, G4State_Init, G4State_Idle, G4State_GeomClosed,
                                 G4State_EventProc);
...
{
  G4ProcessTable::G4ProcNameVector* procNameVector = theProcessTable->GetNameList();
  G4int type = -1;
  ...
  else if (command == procVerboseCmd)
  {
    ...
    for (auto itr = procNameVector->cbegin(); itr != procNameVector->cend(); ++itr)
    {
      ...
      if (isAll || (!isProcName && (p->GetProcessType() == type))
  ...
  else
  {
    ...
    if (!isProcName)
    {
      ...
      if (type < 0)
      {
        ...
        ed << " illegal process (or type) name [" << currentProcessName << "]";
    ...
    else if ((command == activateCmd) || (command == inactivateCmd))
    {
      ...
      if (isProcName)
      {
        ...
        {
          theProcessTable->SetProcessActivation(currentProcessName, fActive);
        }
        else
        {
          theProcessTable->SetProcessActivation(currentProcessName, currentParticle, fActive);
        }
      }
      else
      {
        if (currentParticle == nullptr)
        {
          theProcessTable->SetProcessActivation(G4ProcessType(type), fActive);
        }
        else
        {
          theProcessTable->SetProcessActivation(G4ProcessType(type), currentParticle, fActive);
        }
      }
...
void G4ProcessTableMessenger::SetNumberOfProcessType()
{
  ...
  {
    G4Exception("G4ProcessTableMessenger::SetNumberOfProcessType()", "ProcMan014", FatalException,
                "No End Mark");
  }
}
