  ...
  : theParticleType(aParticleType)
{
  ...
  {
    G4Exception("G4ProcessManager::G4ProcessManager()", "ProcMan012", FatalException,
  ...
  for (G4int i = 0; i < SizeOfProcVectorArray; ++i)
  {
    ...
    {
      G4Exception("G4ProcessManager::G4ProcessManager()", "ProcMan012", FatalException,
                  "Can not create G4ProcessVector ");
    }
  }
...
G4ProcessManager::G4ProcessManager(G4ProcessManager& right)
  : theParticleType(right.theParticleType), verboseLevel(right.verboseLevel)
{
#ifdef G4VERBOSE
  if (GetVerboseLevel() > 2)
  {
    G4cout << "G4ProcessManager::G4ProcessManager() [copy constructor]" << G4endl;
  }
#endif
  ...
  {
    G4Exception("G4ProcessManager::G4ProcessManager() [copy constructor]", "ProcMan011",
                FatalException, "Cannot create G4ProcessList");
  }
  ...
  for (G4int i = 0; i < SizeOfProcVectorArray; ++i)
  {
    ...
    {
      G4Exception("G4ProcessManager::G4ProcessManager() [copy constructor]", "ProcMan011",
                  FatalException, "Cannot create G4ProcessVector");
    }
...
// --------------------------------------------------------------------
G4int G4ProcessManager::GetProcessVectorIndex(G4VProcess* aProcess, G4ProcessVectorDoItIndex idx,
                                              G4ProcessVectorTypeIndex typ) const
{
  ...
  else
  {
    ...
    if (verboseLevel > 0)
    {
      ...
      if (ivec < 0)
      {
        G4cout << " illegal DoIt Index [= " << G4int(idx) << "," << G4int(typ) << "]";
      }
      G4cout << G4endl;
    }
#endif
...
G4ProcessAttribute* G4ProcessManager::GetAttribute(G4int index) const
{
  ...
  if (aProcess == nullptr)
  {
    ...
    G4Exception("G4ProcessManager::GetAttribute()", "ProcMan012", FatalException, aErrorMessage);
    return nullptr;
  }
  ...
  else
  {
    ...
    {
      G4cout << "G4ProcessManager::GetAttribute():";
      G4cout << " particle[" << theParticleType->GetParticleName() << "]" << G4endl;
      G4cout << "Warning: attribute vector index is inconsistent"
             << " with process List index" << G4endl;
    }
#endif
                                   ...
                                   G4int ordAlongStepDoIt, G4int ordPostStepDoIt)
{
  ...
  if (numberOfProcesses != idx)
  {
    ...
    G4Exception("G4ProcessManager::AddProcess()", "ProcMan012", FatalException, anErrorMessage);
    return -1;
  }
  ...
  for (G4int ivec = 0; ivec < SizeOfProcVectorArray; ++ivec)
  {
    ...
    if ((idx >= 0) && (idx < G4int(pVector->entries())))
    {
      ...
      if (RemoveAt(idx, removedProcess, ivec) < 0)
      {
        ...
        G4Exception("G4ProcessManager::RemoveProcess()", "Fatal Error", FatalException,
                    anErrorMessage);
        return nullptr;
      }
    }
    ...
    else
    {
      ...
      anErrorMessage += "for particle[" + theParticleType->GetParticleName() + "] ";
      anErrorMessage += "process[" + removedProcess->GetProcessName() + "]  ";
      G4Exception("G4ProcessManager::RemoveProcess()", "ProcMan012", FatalException,
                  anErrorMessage);
      return nullptr;
    }
  }
...
// --------------------------------------------------------------------
G4int G4ProcessManager::GetProcessOrdering(G4VProcess* aProcess, G4ProcessVectorDoItIndex idDoIt)
{
                                                 ...
                                                 G4ProcessVectorDoItIndex idDoIt)
{
  ...
  if (isSetOrderingFirstInvoked[idDoIt])
  {
    ...
    G4Exception("G4ProcessManager::SetProcessOrderingToFirst()", "ProcMan113", JustWarning,
                anErrMsg);
  }
                                                  ...
                                                  G4ProcessVectorDoItIndex idDoIt)
{
  ...
  for (G4int iproc = 0; iproc < numberOfProcesses; ++iproc)
  {
    ...
    {
      if ((aAttr->ordProcVector[ivec] != 0) && (tmp >= aAttr->ordProcVector[ivec]))
      {
        tmp = aAttr->ordProcVector[ivec];
        if (ip > aAttr->idxProcVector[ivec])
        {
                                                ...
                                                G4ProcessVectorDoItIndex idDoIt)
{
  ...
  if (isSetOrderingLastInvoked[idDoIt])
  {
    ...
    anErrMsg += " to ";
    anErrMsg += theParticleType->GetParticleName();
    G4Exception("G4ProcessManager::SetProcessOrderingToLast()", "ProcMan114", JustWarning,
                anErrMsg);
  }
    ...
    for (G4int i = 0; i < SizeOfProcVectorArray; ++i)
    {
      ...
      else if ((idx >= 0) && (idx < G4int(pVector->entries())))
      {
        ...
        else
        {
          ...
          G4Exception("G4ProcessManager::InactivateProcess()", "ProcMan012", FatalException,
                      anErrorMessage);
          return nullptr;
        }
      }
    ...
    for (G4int i = 0; i < SizeOfProcVectorArray; ++i)
    {
      ...
      else if ((idx >= 0) && (idx < G4int(pVector->entries())))
      {
        ...
        else
        {
          ...
          anErrorMessage += "for particle[" + theParticleType->GetParticleName() + "] ";
          anErrorMessage += "process[" + pProcess->GetProcessName() + "]  ";
          G4Exception("G4ProcessManager::ActivateProcess()", "ProcMan012", FatalException,
                      anErrorMessage);
          return nullptr;
        }
      }
...
void G4ProcessManager::DumpInfo()
{
  ...
  // particle type
  G4cout << "G4ProcessManager: particle[" << theParticleType->GetParticleName() << "]" << G4endl;
...
void G4ProcessManager::StartTracking(G4Track* aTrack)
{
  for (G4int idx = 0; idx < (G4int)theProcessList->entries(); ++idx)
  {
    if (GetAttribute(idx)->isActive) ((*theProcessList)[idx])->StartTracking(aTrack);
  }
...
void G4ProcessManager::EndTracking()
{
  for (G4int idx = 0; idx < (G4int)theProcessList->entries(); ++idx)
  {
    if (GetAttribute(idx)->isActive) ((*theProcessList)[idx])->EndTracking();
  }
...
// --------------------------------------------------------------------
G4VProcess* G4ProcessManager::SetProcessActivation(G4VProcess* aProcess, G4bool fActive)
{
...
G4VProcess* G4ProcessManager::SetProcessActivation(G4int index, G4bool fActive)
{
  if (fActive)
    ...
    return InActivateProcess(index);
}
...
void G4ProcessManager::CheckOrderingParameters(G4VProcess* aProcess) const
{
  ...
  if (pAttr == nullptr)
  {
    ...
    if (GetVerboseLevel() > 0)
    {
      ...
      G4cout << " process " << aProcess->GetProcessName() << " has no attribute" << G4endl;
    }
#endif
    return;
  }
  ...
  if ((pAttr->ordProcVector[0] >= 0) && (!aProcess->isAtRestDoItIsEnabled()))
  {
    ...
    if (GetVerboseLevel() > 0)
    {
      ...
      G4cerr << "You cannot set ordering parameter [" << pAttr->ordProcVector[0]
             << "] for AtRest DoIt  to the process " << aProcess->GetProcessName() << G4endl;
    }
#endif
    isOK = false;
  }
  ...
  if ((pAttr->ordProcVector[2] >= 0) && (!aProcess->isAlongStepDoItIsEnabled()))
  {
    ...
    {
      G4cerr << "G4ProcessManager::CheckOrderingParameters()" << G4endl;
      G4cerr << "You cannot set ordering parameter [" << pAttr->ordProcVector[2]
             << "] for AlongStep DoIt to the process " << aProcess->GetProcessName() << G4endl;
    }
#endif
    isOK = false;
  }
  ...
  if ((pAttr->ordProcVector[4] >= 0) && (!aProcess->isPostStepDoItIsEnabled()))
  {
    ...
    {
      G4cerr << "G4ProcessManager::CheckOrderingParameters()" << G4endl;
      G4cerr << "You cannot set ordering parameter [" << pAttr->ordProcVector[4]
             << "] for PostStep DoIt to the process" << aProcess->GetProcessName() << G4endl;
    }
#endif
    isOK = false;
  }
  ...
  if (!isOK)
  {
    ...
    G4Exception("G4ProcessManager::CheckOrderingParameters()", "ProcMan013", FatalException, msg);
  }
