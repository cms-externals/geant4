...
class G4VProcess
{
  public:
    ...
    // thePILfactor
    //
    inline G4double AlongStepGPIL(const G4Track& track, G4double previousStepSize,
                                  G4double currentMinimumStep, G4double& proposedSafety,
                                  G4GPILSelection* selection);

    inline G4double AtRestGPIL(const G4Track& track, G4ForceCondition* condition);

    inline G4double PostStepGPIL(const G4Track& track, G4double previousStepSize,
                                 G4ForceCondition* condition);
    ...
    const G4String& GetPhysicsTableFileName(const G4ParticleDefinition*, const G4String& directory,
...
inline const G4String& G4VProcess::GetProcessName() const
{
  return theProcessName;
}

inline G4ProcessType G4VProcess::GetProcessType() const
{
  return theProcessType;
}

inline void G4VProcess::SetProcessType(G4ProcessType aType)
{
  theProcessType = aType;
}

inline G4int G4VProcess::GetProcessSubType() const
{
  return theProcessSubType;
}

inline void G4VProcess::SetProcessSubType(G4int value)
{
...
inline G4double G4VProcess::AlongStepGPIL(const G4Track& track, G4double previousStepSize,
                                          G4double currentMinimumStep, G4double& proposedSafety,
                                          G4GPILSelection* selection)
{
  return AlongStepGetPhysicalInteractionLength(track, previousStepSize, currentMinimumStep,
                                               proposedSafety, selection);
}

inline G4double G4VProcess::AtRestGPIL(const G4Track& track, G4ForceCondition* condition)
{
  return thePILfactor * AtRestGetPhysicalInteractionLength(track, condition);
}

inline G4double G4VProcess::PostStepGPIL(const G4Track& track, G4double previousStepSize,
                                         G4ForceCondition* condition)
{
  return thePILfactor * PostStepGetPhysicalInteractionLength(track, previousStepSize, condition);
}

inline void G4VProcess::SetProcessManager(const G4ProcessManager* procMan)
{
  aProcessManager = procMan;
}

inline const G4ProcessManager* G4VProcess::GetProcessManager()
{
  return aProcessManager;
}

inline G4bool G4VProcess::isAtRestDoItIsEnabled() const
{
  return enableAtRestDoIt;
}

inline G4bool G4VProcess::isAlongStepDoItIsEnabled() const
{
  return enableAlongStepDoIt;
}

inline G4bool G4VProcess::isPostStepDoItIsEnabled() const
{
  return enablePostStepDoIt;
}

inline const G4VProcess* G4VProcess::GetMasterProcess() const
{
  return masterProcessShadow;
}

inline void G4VProcess::SubtractNumberOfInteractionLengthLeft(G4double prevStepSize)
{
  ...
  else
  {
    ...
    msg += theProcessName;
    G4Exception("G4VProcess::SubtractNumberOfInteractionLengthLeft()", "ProcMan201",
