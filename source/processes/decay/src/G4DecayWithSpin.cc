...
G4VParticleChange* G4DecayWithSpin::AtRestDoIt(const G4Track& aTrack, const G4Step& aStep)
{
  ...
  else
  {
    ...
    if (fieldMgr == nullptr)
    {
      ...
      if (fFieldPropagator) fieldMgr = fFieldPropagator->GetCurrentFieldManager();
    }
    ...
    if (field != nullptr)
    {
      ...
      if (B.mag2() > 0.) parent_polarization = Spin_Precession(aStep, B, fRemainderLifeTime);
    }
  }
...
G4ThreeVector G4DecayWithSpin::Spin_Precession(const G4Step& aStep, G4ThreeVector B,
                                               G4double deltatime)
{
...
{
  outFile << GetProcessName() << ": Decay of particles considering parent polarization \n"
