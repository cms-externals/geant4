...
G4double G4Decay::GetMeanLifeTime(const G4Track& aTrack, G4ForceCondition*)
{
...
G4VParticleChange* G4Decay::DecayIt(const G4Track& aTrack, const G4Step&)
{
  ...
  if ((decaytable == nullptr) && !isExtDecayer && !isPreAssigned)
  {
    ...
    ed << "For " << aParticle->GetDefinition()->GetParticleName()
       ...
       << "    isExtDecayer: " << isExtDecayer << "; isPreAssigned: " << isPreAssigned;
    G4Exception("G4Decay::DecayIt ", "DECAY101", JustWarning, ed);
  ...
  if (ParentEnergy < ParentMass)
  {
    ...
    G4Exception("G4Decay::DecayIt ", "DECAY102", JustWarning, ed);
...
{
  if ((aTrack.GetTrackStatus() == fStopButAlive) || (aTrack.GetTrackStatus() == fStopAndKill))
  {
    fParticleChangeForDecay.Initialize(aTrack);
    return &fParticleChangeForDecay;
  }
