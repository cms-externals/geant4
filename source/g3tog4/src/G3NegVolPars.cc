...
// this method remain negative.
{
  ...
  if (shapem == shape)
  {
    if (shape == "BOX" || shape == "TRD1" || shape == "TRD2" || shape == "ELTU")
    {
  ...
  if (shape == "BOX")
  {
    if (shapem == "TRD1")
    {
      ...
      if (rpar[0] < 0)
      {
        if (rparm != 0)
          rpar[0] =
            std::min(rparm[0], rparm[1]) + std::abs(rparm[0] - rparm[1]) * .5 * rpar[2] / rparm[3];
    ...
    if (shapem == "TRD2")
    {
      ...
      if (rpar[0] < 0)
      {
        if (rparm != 0)
          rpar[0] =
            std::min(rparm[0], rparm[1]) + std::abs(rparm[0] - rparm[1]) * .5 * rpar[2] / rparm[4];
        if (rpar[0] < 0) NegPresent = TRUE;
      }
      if (rpar[1] < 0)
      {
        if (rparm != 0)
          rpar[1] =
            std::min(rparm[2], rparm[3]) + std::abs(rparm[2] - rparm[3]) * .5 * rpar[2] / rparm[4];
        if (rpar[1] < 0) NegPresent = TRUE;
      }
    }
    if (shapem == "TRAP")
    {
      ...
      if (rpar[0] < 0)
      {
        ...
        {
          G4double xlo =
            std::min(rparm[4], rparm[8]) + std::abs(rparm[4] - rparm[8]) * .5 * rpar[2] / rparm[0];
          G4double xhi =
            std::min(rparm[5], rparm[9]) + std::abs(rparm[5] - rparm[9]) * .5 * rpar[2] / rparm[0];
      ...
      if (rpar[1] < 0)
      {
        if (rparm != 0)
          rpar[1] =
            std::min(rparm[3], rparm[7]) + std::abs(rparm[3] - rparm[7]) * .5 * rpar[2] / rparm[0];
        if (rpar[1] < 0) NegPresent = TRUE;
      }
    }
  }
