...
G3VolTableEntry* G4CreateVTE(G4String vname, G4String shape, G4int nmed, G4double Rpar[],
                             G4int npar);

G3Division::G3Division(G3DivType type, G3VolTableEntry* vte, G3VolTableEntry* mvte,
                       G4int nofDivisions, G4int iaxis, G4int nmed, G4double c0, G4double step)
  : fType(type),
    fVTE(vte),
    fMVTE(mvte),
...
G3Division::G3Division(G3VolTableEntry* vte, G3VolTableEntry* mvte, const G3Division& division)
  : fVTE(vte), fMVTE(mvte)
{
...
#ifndef G3G4_NO_REFLECTION
      G4ReflectionFactory::Instance()->Place(G4Translate3D(position), name, lv, mlv, 0, i);
...
#ifdef G3G4DEBUG
  G4cout << "Create G4PVReplica name " << name << " logical volume name " << lv->GetName()
         << " mother logical volme name " << mlv->GetName() << " axis " << fAxis << " ndivisions "
         << fNofDivisions << " width " << fWidth << " Offset " << fOffset << G4endl;
#endif

#ifndef G3G4_NO_REFLECTION
  G4ReflectionFactory::Instance()->Replicate(name, lv, mlv, fAxis, fNofDivisions, fWidth, fOffset);
...
{
  G4String err_message = "G3Division::" + where + " for " + what + " is not implemented";
  G4Exception("G3Division::Exception()", "G3toG40004", FatalException, err_message);
  return;
}
...
// set fHighRange, fLowRange, fAxis
{
  ...
  switch (fIAxis)
  {
    ...
    case 3:
      ...
      break;
    default:
      G4Exception("G3Division::SetRangeAndAxis()", "G3toG40005", FatalException,
                  "Wrong axis index definition!");
  }
  ...
  else if (shape == "ELTU" || shape == "HYPE" || shape == "GTRA" || shape == "CTUB")
  {
...
G3VolTableEntry* G3Division::CreateEnvelope(G4String shape, G4double hi, G4double lo,
...
// envelope of divided volume
{
  ...
  G3VolTableEntry* envVTE = G4CreateVTE(envName, shape, fNmed, Rpar, npar);
...
// and set the fOffset for replica
{
  ...
  if (hasNegPars)
  {
    G4String err_message = "CreateSolid VTE " + fVTE->GetName() + " has negative parameters.";
    G4Exception("G3Division::CreateSolid()", "G3toG40006", FatalException, err_message);
    return;
  }
...
G3VolTableEntry* G3Division::Dvt()
{
  ...
  if (delta > Rmin)
  {
    envVTE = CreateEnvelope(shape, fHighRange - delta / 2., fLowRange + delta / 2., Rpar, npar);
    Rpar = envVTE->GetRpar();
    npar = envVTE->GetNpar();
  }
...
G3VolTableEntry* G3Division::Dvt2()
{
  ...
  if (std::abs(c0 - fLowRange) > Rmin)
  {
    envVTE = CreateEnvelope(shape, fHighRange - delta / 2., c0 + delta / 2., Rpar, npar);
    Rpar = envVTE->GetRpar();
    npar = envVTE->GetNpar();
  }
