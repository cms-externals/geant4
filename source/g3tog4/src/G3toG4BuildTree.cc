...
void G3toG4BuildLVTree(G3VolTableEntry* curVTE, G3VolTableEntry* motherVTE)
{
  ...
  if (curVTE->GetSolid())
  {
    ...
    if (!curLog)
    {
      ...
      if (!material)
      {
        G4String err_message = "VTE " + curVTE->GetName() + " has not defined material!!";
        G4Exception("G3toG4BuildLVTree()", "G3toG40001", FatalException, err_message);
        return;
      }

      // logical volume
      curLog = new G4LogicalVolume(curVTE->GetSolid(), material, curVTE->GetName());
      curVTE->SetLV(curLog);
  ...
  else
  {
          ...
          && motherVTE->GetNoClones() > 1))
    {
      ...
      G4String err_message = "VTE " + curVTE->GetName() + " has not defined solid!!";
      G4Exception("G3toG4BuildLVTree()", "G3toG40002", FatalException, err_message);
      return;
    }
  }
        ...
        for (G4int im = 0; im < curVTE->GetNoMothers(); im++)
        {
          ...
          if (theG3Pos->GetMotherName() == motherVTE->GetMasterClone()->GetName())
          {
            ...
            if (curVTE->FindMother(motherName)->GetName() != motherName)
            {
              ...
              G4String err_message = "G3toG4BuildTree: Inconsistent mother <-> daughter !!";
              G4Exception("G3toG4BuildPVTree()", "G3toG40003", FatalException, err_message);
              return;
            }
            ...
            if (mothLV != 0)
            {
              ...
              G4ReflectionFactory::Instance()->Place(transform3D,  // transformation
              ...
              // verbose
#ifdef G3G4DEBUG
              G4cout << "PV: " << i << "th copy of " << curVTE->GetName() << "  in "
                     << motherVTE->GetName() << "  copyNo: " << copyNo << "  irot: " << irot
                     << "  pos: " << *(theG3Pos->GetPos()) << G4endl;
#endif
            }
          }
        }
...
// verbose
#ifdef G3G4DEBUG
      G4cout << "CreatePVReplica: " << curVTE->GetName() << " in " << curVTE->GetMother()->GetName()
             << G4endl;
#endif
