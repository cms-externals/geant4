...
void G3toG4MANY(G3VolTableEntry* curVTE)
{
  if (curVTE->GetNoOverlaps() > 0)
  {
    ...
    if (!curVTE->HasMANY())
    {
      ...
      G4Exception("G3toG4MANY()", "G3toG40009", FatalException, text);
      return;
    }
    ...
    if (curVTE->NPCopies() != 1)
    {
      ...
      G4Exception("G3toG4MANY()", "G3toG40010", FatalException, text);
      return;
    }
    ...
    for (G4int ij = 0; ij < overlapVTE->GetMasterClone()->GetNoClones(); ij++)
    {
      ...
      for (G4int j = 0; j < cloneVTE->NPCopies(); j++)
      {
        ...
        G4cout << "From '" << curVTE->GetName() << "' "
               << "cut '" << cloneVTE->GetName() << "' :" << i << "th overlap (from "
               << overlaps->size() << ") " << ij << "th clone (from "
               << overlapVTE->GetMasterClone()->GetNoClones() << ") " << j << "th copy (from "
               << cloneVTE->NPCopies() << ")  " << G4endl;
#endif
                     ...
                     const G4Transform3D& transform)
{
  ...
  G4VSolid* newSolid =
    new G4SubtractionSolid(newName, vte1->GetSolid(), vte2->GetSolid(), transform * transform2);
  ...
  for (G4int k = 0; k < vte1->GetNoDaughters(); k++)
  {
    ...
    if (dVTE->NPCopies() != 1)
    {
      ...
      G4Exception("G3toG4MANY()", "G3toG40011", FatalException, text);
      return;
    }
