...
void G4CreateCloneVTEWithDivision(G4String vname, G3VolTableEntry* mvte, G3DivType divType,
                                  G4int nofDivisions, G4int iaxis, G4int, G4double c0,
                                  G4double step)
{
  ...
  for (G4int i = 0; i < mvte->GetNoClones(); i++)
  {
    ...
    G3VolTableEntry* vteClone = new G3VolTableEntry(newName, shape, 0, 0, nmed, 0, true);
    ...
    G3Division* division =
      new G3Division(divType, vteClone, mvteClone, nofDivisions, iaxis, nmed, c0, step);
