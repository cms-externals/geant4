...
G3VolTable::G3VolTable() : G3toG4TopVTE(0), _FirstKey("UnDefined"), _NG3Pos(0) {}
...
void G3VolTable::PrintAll()
{
  if (VTD.size())
  {
    ...
    for (VTDiterator v = VTD.begin(); v != VTD.end(); v++)
    {
      ...
      G4cout << "G3VolTable element " << std::setw(3) << i++ << " name " << VTE->GetName()
             << " has " << VTE->GetNoDaughters() << " daughters" << G4endl;
    }
  }
}

G3VolTableEntry* G3VolTable::PutVTE(G3VolTableEntry* aG3VolTableEntry)
{
...
{
  G4cout << "Instantiated " << VTD.size() << " volume table entries \n"
