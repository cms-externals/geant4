...
G4bool G3NegVolPars(G4double pars[], G4int* nparpt, G3VolTableEntry* vte, G3VolTableEntry* mvte,
                    const char routine[]);
...
// vte daughters
{
  if (vte->HasNegPars())
  {
    ...
    G4cerr << " G4ProcessDaughters: Ignored (vte has negative parameters)." << G4endl;
  }
  else
  {
    for (G4int i = 0; i < vte->GetNoDaughters(); i++)
    {
      ...
      if (dvte->HasNegPars())
      {
        ...
        else
        {
          ...
          G4bool negpars = G3NegVolPars(pars, &npar, dvte, vte, "GSPOS");

          if (negpars)
          {
            G4String text = "G3NegVolPars still returns negative parameters!";
            G4Exception("G4ProcessDaughters()", "G3toG40019", FatalException, text);
            return;
          }
          ...
          G4bool deferred;
          G4bool okAxis[3];
          G4VSolid* solid = G3toG4MakeSolid(dvte->GetName(), dvte->GetShape(), pars, npar,
                                            hasNegPars, deferred, okAxis);
          if (hasNegPars)
          {
            G4String text = "G3toG4MakeSolid still returns negative parameters!";
            G4Exception("G4ProcessDaughters()", "G3toG40020", FatalException, text);
            return;
          }
    ...
    for (G4int id = 0; id < nofDaughters; id++)
    {
      ...
      if (dvte->HasNegPars() || dvte->GetDivision())
      {
        ...
        G3VolTableEntry* dvteClone =
          new G3VolTableEntry(newName, dvteShape, dvteRpar, dvteNpar, dvteNmed, 0, hasNegPars);
        ...
        if (dvteDivision)
        {
          G3Division* dvteCloneDivision = new G3Division(dvteClone, vteClone, *dvteDivision);
...
void G4CreateCloneVTE(G3VolTableEntry* vte, G3VolTableEntry* mvte, G4double pars[], G4int npar,
                      G4int num, G4double x, G4double y, G4double z, G4int irot, G4String vonly)
//
...
// and derive its parameters from the mother if possible
{
  ...
  for (G4int i = 0; i < mvte->GetNoClones(); i++)
  {
    ...
    G3VolTableEntry* vteClone =
      new G3VolTableEntry(tmpName, vteShape, pars, npar, vte->GetNmed(), 0, true);
    ...
    G4bool negpars = G3NegVolPars(clonePars, &cloneNpar, vteClone, mvteClone, "GSPOS");
    ...
    if (!negpars)
    {
      ...
      else
      {
        ...
        solid =
          G3toG4MakeSolid(vteName, cloneShape, clonePars, cloneNpar, hasNegPars, deferred, okAxis);
      }
    }
...
void G4gsposp(G4String vname, G4int num, G4String vmoth, G4double x, G4double y, G4double z,
              G4int irot, G4String vonly, G4double pars[], G4int npar)
{
  // find VTEs
