...
G3VolTableEntry::G3VolTableEntry(G4String& vname, G4String& shape, G4double* rpar, G4int npar,
                                 G4int nmed, G4VSolid* solid, G4bool hasNegPars)
  : fVname(vname),
    fShape(shape),
    fRpar(0),
    fNpar(npar),
...
G4bool G3VolTableEntry::operator==(const G3VolTableEntry& lv) const
{
...
void G3VolTableEntry::AddMother(G3VolTableEntry* itsMother)
{
...
void G3VolTableEntry::AddClone(G3VolTableEntry* itsClone)
{
...
void G3VolTableEntry::AddOverlap(G3VolTableEntry* overlap)
{
  fOverlaps.push_back(overlap);
}

void G3VolTableEntry::ReplaceDaughter(G3VolTableEntry* vteOld, G3VolTableEntry* vteNew)
{
  G4int index = -1;
  ...
  if (index < 0)
  {
    G4String err_message = "Old daughter " + vteOld->GetName() + " does not exist.";
    G4Exception("G3VolTableEntry::ReplaceDaughter()", "G3toG40007", FatalException, err_message);
    return;
  }
  fDaughters[index] = vteNew;
}

void G3VolTableEntry::ReplaceMother(G3VolTableEntry* vteOld, G3VolTableEntry* vteNew)
{
  G4int index = -1;
  ...
  if (index < 0)
  {
    G4String err_message = "Old mother " + vteOld->GetName() + " does not exist.";
    G4Exception("G3VolTableEntry::ReplaceMother()", "G3toG40008", FatalException, err_message);
    return;
  }
  fMothers[index] = vteNew;
}

G3VolTableEntry* G3VolTableEntry::FindDaughter(const G4String& Dname)
{
...
G3VolTableEntry* G3VolTableEntry::FindMother(const G4String& Mname)
{
  for (G4int i = 0; i < GetNoMothers(); i++)
  {
...
G3VolTableEntry* G3VolTableEntry::FindClone(const G4String& Cname)
{
...
G3Pos* G3VolTableEntry::GetG3PosCopy(G4int copy)
{
