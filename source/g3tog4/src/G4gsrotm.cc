              ...
              G4double theta3, G4double phi3)
{
  ...
  G4ThreeVector x(std::sin(th1r) * std::cos(phi1r), std::sin(th1r) * std::sin(phi1r),
                  std::cos(th1r));
  G4ThreeVector y(std::sin(th2r) * std::cos(phi2r), std::sin(th2r) * std::sin(phi2r),
                  std::cos(th2r));
  G4ThreeVector z(std::sin(th3r) * std::cos(phi3r), std::sin(th3r) * std::sin(phi3r),
                  std::cos(th3r));
  ...
  {
    G4cerr << "Coordinate axes forming rotation matrix " << irot << " are not orthonormal.("
           << 1 - std::abs(check) << ")" << G4endl;
    ...
    G4cerr << " phi3=" << phi3;
    G4cerr << G4endl;
    G4Exception("G4gsrotm()", "G3toG40023", FatalException, "Non orthogonal axes!");
    return;
  }
  // else if (1+check<=tol) {
