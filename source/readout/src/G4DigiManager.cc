...
void G4DigiManager::AddNewModule(G4VDigitizerModule* DM)
{
  ...
  if (verboseLevel > 0)
  {
    G4cout << "New DigitizerModule <" << DMname << "> is registered." << G4endl;
  }
  ...
  for (G4int i = 0; i < numberOfCollections; ++i)
  {
    ...
    {
      G4cout << "DigiCollection <" << DCname << "> has already been registered with " << DMname
             << " DigitizerModule." << G4endl;
    }
    else if (verboseLevel > 0)
    {
      G4cout << "DigiCollection " << DCname << " is registered. " << G4endl;
    }
  }
...
void G4DigiManager::SetDigiCollection(G4int DCID, G4VDigiCollection* aDC)
{
  ...
  if (verboseLevel > 0)
  {
    G4cout << aDC->GetName() << " is stored at " << DCID << "-th slot of G4DCofThisEvent."
           << G4endl;
  }
}
