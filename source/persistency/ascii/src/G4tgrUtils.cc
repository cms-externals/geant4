...
{
  G4cout << msg << std::setprecision(8) << vec << std::setprecision(6) << G4endl;
}

// --------------------------------------------------------------------
void G4tgrUtils::Dumprm(const G4RotationMatrix& rm, const char* msg)
{
  G4cout << msg << G4endl << " xx=" << rm.xx() << " yx=" << rm.yx() << " zx=" << rm.zx() << G4endl
         << " xy=" << rm.xy() << " yy=" << rm.yy() << " zy=" << rm.zy() << G4endl
         << " xz=" << rm.xz() << " yz=" << rm.yz() << " zz=" << rm.zz() << G4endl;
}

// --------------------------------------------------------------------
void G4tgrUtils::DumpVS(const std::vector<G4String>& wl, const char* msg, std::ostream& outs)
{
  outs << msg << G4endl;
...
G4String G4tgrUtils::SubColon(const G4String& str)
{
  if (str.find(':') != 0)
  {
    ...
    G4Exception("G4tgrUtils::SubColon()", "ParseError", FatalException, ErrMessage);
  }
  ...
  if (cstr[0] == '$')
  {
    ...
    if (G4tgrMessenger::GetVerboseLevel() >= 3)
    {
      G4cout << " G4tgrUtils::GetString() - Substitute parameter: "
             << G4tgrParameterMgr::GetInstance()->FindParameter(str.substr(1, str.size()))
             << G4endl;
    }
#endif
    return G4tgrParameterMgr::GetInstance()->FindParameter(str.substr(1, str.size()));
  }
  else
  {
    return str;
...
G4double G4tgrUtils::GetDouble(const G4String& str, G4double unitval)
{
  ...
  if (G4tgrMessenger::GetVerboseLevel() >= 3)
  {
    G4cout << "G4tgrUtils::GetDouble() - Processing: " << str << " default unit " << unitval
           << G4endl;
  }
#endif
  if (str == "DBL_MAX")
  ...
  for (; site2 != separators.cend(); ++site, ++site2)
  {
    ...
    if (G4tgrMessenger::GetVerboseLevel() >= 3)
    {
      G4cout << "   Loop to find word between " << *site << " " << *site2 << G4endl;
    }
#endif
    ...
    {
      G4String parstr =
        G4tgrParameterMgr::GetInstance()->FindParameter(word.substr(1, word.size()));
      ...
      if (G4tgrMessenger::GetVerboseLevel() >= 3)
      {
        G4cout << " G4tgrutils::GetDouble() - Param found: " << word << " in string " << str
    ...
    else
    {
      ...
      if (IsNumber(word))
      {
        ...
        {
          G4String ErrMessage =
            "There cannot be a ')' before a number: " + word + " in string: " + str;
          G4Exception("G4tgrUtils::GetDouble()", "ParseError", FatalException, ErrMessage);
        }
        //--- right separator cannot be '('
        if ((*site2 != strlen) && (cstr[*site2] == '('))
        {
          G4String ErrMessage =
            "There cannot be a '(' after a number: " + word + " in string: " + str;
          G4Exception("G4tgrUtils::GetDouble()", "ParseError", FatalException, ErrMessage);
        }
        strnew += word;
      ...
      else
      {
        ...
        {
          G4String ErrMessage =
            "String words cannot start with a digit: " + word + " in string: " + str;
          G4Exception("G4tgrUtils::GetDouble()", "ParseError", FatalException, ErrMessage);
        }
        ...
        if (G4tgrUtils::IsFunction(word))
        {
          ...
          {
            G4String ErrMessage =
              "There must be a '(' after a function: " + word + " in string: " + str;
            G4Exception("G4tgrUtils::GetDouble()", "ParseError", FatalException, ErrMessage);
          }
          strnew += word;
          bWordOK = true;
        ...
        else if (G4tgrUtils::WordIsUnit(word))
        {
          ...
          {
            G4String ErrMess =
              "There must be a '*' before a unit definition: " + word + " in string " + str;
            G4Exception("G4tgrUtils::GetDouble()", "ParseError", FatalException, ErrMess);
          }
  ...
  if (G4tgrMessenger::GetVerboseLevel() >= 3)
  {
    G4cout << " G4tgrUtils::GetDouble() - RESULT= " << val << G4endl << "   from string: " << str
           << " converted to: " << strnew.c_str() << " with unit val: " << unitval << G4endl;
  }
#endif
...
G4int G4tgrUtils::GetInt(const G4String& str)
{
  ...
  if (!IsInteger(val))
  {
    ...
    G4Exception("G4tgrUtils::GetInt()", "ParseError", FatalException, ErrMessage);
  }
  return G4int(val);
}
...
G4bool G4tgrUtils::GetBool(const G4String& str)
{
  ...
  else
  {
    G4String ErrMessage = G4String("Trying to get a float from a string")
                          + G4String(" which is not 'ON'/'OFF'/'TRUE'/'FALSE' ") + str;
    G4Exception("G4tgrUtils::GetBool()", "ParseError", FatalException, ErrMessage);
  }
...
// --------------------------------------------------------------------
void G4tgrUtils::CheckWLsize(const std::vector<G4String>& wl, unsigned int nWcheck, WLSIZEtype st,
                             const G4String& methodName)
{
  ...
  if (!isOK)
  {
    ...
    G4Exception("G4tgrUtils::CheckWLsize()", "ParseError", FatalException, ErrMessage);
  }
}

// --------------------------------------------------------------------
G4bool G4tgrUtils::CheckListSize(unsigned int nWreal, unsigned int nWcheck, WLSIZEtype st,
                                 G4String& outStr)
{
  G4bool isOK = true;
  switch (st)
...
G4bool G4tgrUtils::WordIsUnit(const G4String& word)
{
  ...
  if (word == "mm" || word == "cm" || word == "m" || word == "km" || word == "millimeter"
      || word == "centimeter" || word == "meter" || word == "kilometer" || word == "parsec"
      || word == "micrometer" || word == "nanometer" || word == "angstrom" || word == "fermi"
      || word == "nm" || word == "um" || word == "pc" || word == "radian" || word == "milliradian"
      || word == "degree" || word == "rad" || word == "mrad" || word == "deg" || word == "ns"
      || word == "becquerel" || word == "curie")
  {
    return true;
  }
  else
...
{
  if (word == "sin" || word == "cos" || word == "tan" || word == "asin" || word == "acos"
      || word == "atan" || word == "atan2" || word == "sinh" || word == "cosh" || word == "tanh"
      || word == "asinh" || word == "acosh" || word == "atanh" || word == "sqrt" || word == "exp"
      || word == "log" || word == "log10" || word == "pow")
  {
    return true;
  }
  else
...
G4RotationMatrix G4tgrUtils::GetRotationFromDirection(G4ThreeVector dir)
{
  ...
  if (std::fabs(dir.mag() - 1.) > G4GeometryTolerance::GetInstance()->GetSurfaceTolerance())
  {
    ...
    G4Exception("G4tgrUtils::GetRotationFromDirection()", "WrongArgument", JustWarning, WarMessage);
    dir /= dir.mag();
  }
...
G4bool G4tgrUtils::AreWordsEquivalent(const G4String& word1, const G4String& word2)
{
      ...
      if (cAster == cStart)
      {
        if (cAster != 0)
        {
          G4Exception("G4tgrUtils::AreWordsEquivalent()",
                      "A word has two asterisks together, please correct it", FatalException,
                      ("Offending word is: " + word1).c_str());
        }
        else
        {
          // word1 == *
  ...
  if (cStart <= word1.length())
  {
    ...
    {
      stringPairs.push_back(std::pair<size_t, size_t>(cStart, word1.length() - cStart));
    }
  }
