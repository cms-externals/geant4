...
{
  G4String ErrMessage =
    "Should only be called for composite solids... " + G4UIcommand::ConvertToString(ii);
  G4Exception("G4tgrVolume::GetVolume()", "InvalidCall", FatalException, ErrMessage);
  return nullptr;
}
...
G4tgrPlace* G4tgrVolume::AddPlace(const std::vector<G4String>& wl)
{
  ...
  for (auto ite = thePlacements.cbegin(); ite != thePlacements.cend(); ++ite)
  {
    ...
    {
      G4String ErrMessage = "Repeated placement. Volume " + theName + " in " + pl->GetParentName();
      G4Exception("G4tgrVolume::AddPlace()", "InvalidArgument", FatalErrorInArgument, ErrMessage);
    }
  }
  ...
  {
    G4cout << " G4tgrVolume:  New placement: " << thePlacements.size() << " added for Volume "
           << theName << " inside " << pl->GetParentName() << " type " << pl->GetType() << G4endl;
  }
#endif
...
G4tgrPlaceDivRep* G4tgrVolume::AddPlaceReplica(const std::vector<G4String>& wl)
{
  ...
  if ((wl.size() == 7) && (G4tgrUtils::GetDouble(wl[6]) != 0.) && (wl[3] != "PHI"))
  {
    G4Exception("G4tgrVolume::AddPlaceReplica",
                "Offset set for replica not along PHI, it will not be used", JustWarning,
  ...
  if (G4tgrMessenger::GetVerboseLevel() >= 2)
  {
    G4cout << " G4tgrVolume:  New placement replica: " << thePlacements.size()
           << " added for Volume " << theName << " inside " << pl->GetParentName() << G4endl;
  }
#endif
  //---------- register parent - child
  G4tgrVolumeMgr::GetInstance()->RegisterParentChild(pl->GetParentName(), pl);
...
G4tgrPlaceParameterisation* G4tgrVolume::AddPlaceParam(const std::vector<G4String>& wl)
{
  ...
  if (G4tgrMessenger::GetVerboseLevel() >= 2)
  {
    G4cout << " G4tgrVolume:  New placement Param: " << thePlacements.size() << " added for Volume "
           << theName << " inside " << pl->GetParentName() << G4endl;
  }
#endif
  //---------- register parent - child
  G4tgrVolumeMgr::GetInstance()->RegisterParentChild(pl->GetParentName(), pl);
...
std::ostream& operator<<(std::ostream& os, const G4tgrVolume& obj)
{
  os << "G4tgrVolume= " << obj.theName << " Type= " << obj.theType
     << " Material= " << obj.theMaterialName << " Visibility " << obj.theVisibility << " Colour "
     << (obj.theRGBColour)[0] << " " << (obj.theRGBColour)[1] << " " << (obj.theRGBColour)[2] << " "
     << (obj.theRGBColour)[3] << " "
     << " CheckOverlaps " << obj.theCheckOverlaps << " N placements " << obj.thePlacements.size()
     << G4endl;
