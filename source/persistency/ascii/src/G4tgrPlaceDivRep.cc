...
EAxis G4tgrPlaceDivRep::BuildAxis(const G4String& axisName)
{
  ...
  {
    G4String ErrMessage =
      "Axis type not found: " + axisName + ". Only valid axis are: X, Y, Z, R, PHI !";
    G4Exception("G4tgrVolumeDivision::GetReplicaAxis()", "InvalidAxis", FatalException, ErrMessage);
  }
...
std::ostream& operator<<(std::ostream& os, const G4tgrPlaceDivRep& obj)
{
  os << "G4tgrPlaceDivRep= in " << obj.theParentName << " NDiv= " << obj.theNDiv
     << " Width= " << obj.theWidth << " Axis= " << obj.theAxis << " Offset= " << obj.theOffset
