...
G4tgrSolid* G4tgrVolumeMgr::CreateSolid(const std::vector<G4String>& wl, G4bool bVOLUtag)
{
  ...
  if (sol != nullptr)
  {
    ...
    G4Exception("G4tgrVolumeMgr::CreateSolid()", "InvalidSetup", FatalException, ErrMessage);
  }
...
void G4tgrVolumeMgr::RegisterMe(G4tgrSolid* sol)
{
  ...
  {
    G4String ErrMessage = "Cannot be two solids with the same name... " + sol->GetName();
    G4Exception("G4tgrVolumeMgr::RegisterMe()", "InvalidSetup", FatalException, ErrMessage);
  }
...
void G4tgrVolumeMgr::UnRegisterMe(G4tgrSolid* sol)
{
  if (theG4tgrSolidMap.find(sol->GetName()) != theG4tgrSolidMap.cend())
  {
    G4String ErrMessage = "Cannot unregister a solid that is not registered... " + sol->GetName();
    G4Exception("G4tgrSolidMgr::unRegisterMe()", "InvalidSetup", FatalException, ErrMessage);
  }
  else
  {
...
void G4tgrVolumeMgr::RegisterMe(G4tgrVolume* vol)
{
  ...
  {
    G4String ErrMessage = "Cannot be two volumes with the same name... " + vol->GetName();
    G4Exception("G4tgrVolumeMgr::RegisterMe()", "InvalidSetup", FatalException, ErrMessage);
  }
...
void G4tgrVolumeMgr::UnRegisterMe(G4tgrVolume* vol)
{
  ...
  for (ite = theG4tgrVolumeList.cbegin(); ite != theG4tgrVolumeList.cend(); ++ite)
  {
    if ((*ite) == vol)
    {
      break;
  ...
  {
    G4String ErrMessage = "Cannot unregister a volume not registered... " + vol->GetName();
    G4Exception("G4tgrVolumeMgr::unRegisterMe()", "InvalidSetup", FatalException, ErrMessage);
  }
  else
  {
...
G4tgrSolid* G4tgrVolumeMgr::FindSolid(const G4String& volname, G4bool exists)
{
  ...
  if (svite == theG4tgrSolidMap.cend())
  {
    if (exists)
    {
      for (svite = theG4tgrSolidMap.cbegin(); svite != theG4tgrSolidMap.cend(); ++svite)
      {
        G4cerr << " VOL:" << (*svite).first << G4endl;
      }
      G4String ErrMessage = "Solid not found... " + volname;
      G4Exception("G4tgrVolumeMgr::FindSolid()", "InvalidSetup", FatalException, ErrMessage);
    }
  }
  else
  {
...
G4tgrVolume* G4tgrVolumeMgr::FindVolume(const G4String& volname, G4bool exists)
{
  ...
  if (svite == theG4tgrVolumeMap.cend())
  {
    if (exists)
    {
      for (svite = theG4tgrVolumeMap.cbegin(); svite != theG4tgrVolumeMap.cend(); ++svite)
      {
        G4cerr << " VOL:" << (*svite).first << G4endl;
      }
      G4String ErrMessage = "Volume not found... " + volname;
      G4Exception("G4tgrVolumeMgr::FindVolume()", "InvalidSetup", FatalException, ErrMessage);
    }
    else
    {
      G4String WarMessage = "Volume does not exists... " + volname;
      G4Exception("G4tgrVolumeMgr::FindVolume()", "SearchFailed", JustWarning, WarMessage);
    }
  }
  else
  {
...
// --------------------------------------------------------------------
std::vector<G4tgrVolume*> G4tgrVolumeMgr::FindVolumes(const G4String& volname, G4bool exists)
{
  ...
  if (vols.size() == 0)
  {
    if (exists)
    {
      ...
      G4String ErrMessage = "Volume not found... " + volname;
      G4Exception("G4tgrVolumeMgr::FindVolumes()", "InvalidSetup", FatalException, ErrMessage);
    }
    else
    {
      G4String WarMessage = "Volume does not exists... " + volname;
      G4Exception("G4tgrVolumeMgr::FindVolumes()", "SearchFailed", JustWarning, WarMessage);
    }
  }
...
const G4tgrVolume* G4tgrVolumeMgr::GetTopVolume()
{
  ...
  for (auto itetv = theG4tgrVolumeMap.cbegin(); itetv != theG4tgrVolumeMap.cend(); ++itetv)
  {
    ...
    if ((topVol != nullptr) && (topVol != vol) && (topVol->GetType() != "VOLDivision")
        && (vol->GetType() != "VOLDivision"))
    {
      G4Exception("G4tgrVolumeMgr::GetTopVolume()", "Two world volumes found, second will be taken",
                  JustWarning,
                  (G4String("Both volumes are at the top of a hierarchy: ") + topVol->GetName()
                   + " & " + vol->GetName())
                    .c_str());
    }
    topVol = vol;
...
void G4tgrVolumeMgr::DumpSummary()
{
  ...
  G4cout << " @@@ Geometry built inside world volume: " << GetTopVolume()->GetName() << G4endl;
  G4cout << " Number of G4tgrVolume's: " << theG4tgrVolumeMap.size() << G4endl;
  unsigned int nPlace = 0;
  for (auto cite = theG4tgrVolumeMap.cbegin(); cite != theG4tgrVolumeMap.cend(); ++cite)
  {
