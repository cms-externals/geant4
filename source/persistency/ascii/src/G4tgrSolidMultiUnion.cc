...
G4tgrSolidMultiUnion::G4tgrSolidMultiUnion(const std::vector<G4String>& wl)
{
  ...
  if (G4int(wl.size()) != 4 + 5 * nSolid)
  {
    ...
    G4Exception("G4tgrSolidMultiUnion::G4tgrSolidMultiUnion()", "InvalidInput", FatalException,
                ErrMessage);
  }
  ...
  for (G4int i = 4; i < nSolid * 5; i += 5)
  {
    ...
    theRotMat = G4tgbRotationMatrixMgr::GetInstance()->FindOrBuildG4RotMatrix(theRotMatName);
    thePosition = G4ThreeVector(G4tgrUtils::GetDouble(wl[i + 2]), G4tgrUtils::GetDouble(wl[i + 3]),
...
{
  os << "G4tgrSolidMultiUnion= " << sol.theName << " of type " << sol.theType << " PARAMS: ";
