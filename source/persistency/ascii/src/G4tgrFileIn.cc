...
void G4tgrFileIn::OpenNewFile(const char* filename)
{
  ...
  if (!fin->is_open())
  {
    ...
    G4Exception("G4tgrFileIn::OpenNewFile()", "InvalidInput", FatalException, ErrMessage);
  }
#endif
}
...
G4tgrFileIn& G4tgrFileIn::GetInstanceOpened(const G4String& filename)
{
  ...
  if (filein.GetName() != filename)
  {
    ...
    G4Exception("G4tgrFileIn::GetInstanceOpened()", "InvalidInput", FatalException, ErrMessage);
  }
  else
  {
    return filein;
...
G4int G4tgrFileIn::GetWordsInLine(std::vector<G4String>& wordlist)
{
  ...
  for (;;)
  {
    ...
    if (ii == NMAXLIN - 1)
    {
      ErrorInLine();
      G4String ErrMessage =
        "Too long line. Please split it " + G4String("putting a '\\' at the end!");
      G4Exception("G4tgrFileIn::GetWordsInLine()", "InvalidInput", FatalException, ErrMessage);
    }
    ...
    do
    {
      if (*tt != ' ' && *(tt) != '\0')
      {
        if (tt == ltemp)
        {
          ...
          {
            G4cout << "G4tgrFileIn::GetWordsInLine() - NoWords" << NoWords << ltemp << G4endl;
          }
#endif
        }
  ...
  if (wordlist[0] == "#include")
  {
    ...
    {
      ErrorInLine();
      G4String ErrMessage = "'#include' should have as second argument, the filename !";
      G4Exception("G4tgrFileIn::GetWordsInLine()", "InvalidInput", FatalException, ErrMessage);
    }
...
G4bool G4tgrFileIn::EndOfFile()
{
  ...
  if (G4tgrMessenger::GetVerboseLevel() >= 3)
  {
    G4cout << " G4tgrFileIn::EndOfFile() - EOF: " << isok << " " << theCurrentFile << G4endl;
  }
#endif
  if (theCurrentFile != -1)
  {
...
void G4tgrFileIn::DumpException(const G4String& sent)
{
  ...
  const G4String& Err2 = ", line No: " + G4UIcommand::ConvertToString(theLineNo[theCurrentFile]);
  const G4String& ErrMessage = Err1 + Err2;
  G4Exception("G4tgrFileIn::DumpException()", "FileError", FatalException, ErrMessage);
}
