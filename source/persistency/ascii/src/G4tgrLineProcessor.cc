...
G4bool G4tgrLineProcessor::ProcessLine(const std::vector<G4String>& wl)
{
  ...
  else if ((wl0 == ":MIXT") || (wl0 == ":MIXT_BY_WEIGHT"))
  {
    G4tgrMaterialMixture* mate =
      G4tgrMaterialFactory::GetInstance()->AddMaterialMixture(wl, "MaterialMixtureByWeight");
    volmgr->RegisterMe(mate);
  ...
  {
    G4tgrMaterialMixture* mate =
      G4tgrMaterialFactory::GetInstance()->AddMaterialMixture(wl, "MaterialMixtureByNoAtoms");
    volmgr->RegisterMe(mate);
  ...
  {
    G4tgrMaterialMixture* mate =
      G4tgrMaterialFactory::GetInstance()->AddMaterialMixture(wl, "MaterialMixtureByVolume");
    volmgr->RegisterMe(mate);
  ...
  {
    G4tgrMaterial* mate =
      G4tgrMaterialFactory::GetInstance()->FindMaterial(G4tgrUtils::GetString(wl[1]));
    if (mate == 0)
    {
      G4Exception("G4tgrLineProcessor::ProcessLine()", "Material not found", FatalException,
  ...
  else if (wl0 == ":MATE_STATE")
  {
    ...
    if (mate == 0)
    {
      G4Exception("G4tgrLineProcessor::ProcessLine()", "Material not found", FatalException, wl[1]);
    }
    mate->SetState(wl[2]);
  ...
  else if ((wl0 == ":DIV_NDIV") || (wl0 == ":DIV_WIDTH") || (wl0 == ":DIV_NDIV_WIDTH"))
  {
...
G4tgrVolume* G4tgrLineProcessor::FindVolume(const G4String& volname)
{
  ...
  {
    G4Exception("G4tgrLineProcessor::FindVolume()", "InvalidSetup", FatalException,
