...
{
  theRotationMatrix =
    G4tgbRotationMatrixMgr::GetInstance()->FindOrBuildG4RotMatrix(tgrParam->GetRotMatName());
}
...
// --------------------------------------------------------------------
void G4tgbPlaceParameterisation::ComputeTransformation(const G4int, G4VPhysicalVolume*) const {}
                                                 ...
                                                 const G4String& methodName)
{
  ...
  if (!isOK)
  {
    ...
    G4Exception("G4tgbPlaceParameterisation::CheckNExtraData", "InvalidData", FatalException,
                "Invalid data size.");
  }
}
