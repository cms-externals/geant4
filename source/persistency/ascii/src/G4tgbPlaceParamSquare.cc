  ...
  : G4tgbPlaceParameterisation(tgrParam)
{
  ...
  if (tgrParam->GetParamType() == "SQUARE")
  {
    ...
    theDirection1 = G4ThreeVector(tgrParam->GetExtraData()[6], tgrParam->GetExtraData()[7],
                                  tgrParam->GetExtraData()[8]);
    theDirection2 = G4ThreeVector(tgrParam->GetExtraData()[9], tgrParam->GetExtraData()[10],
  ...
  {
    G4Exception("G4tgbPlaceParamSquare::G4tgbPlaceParamSquare()", "InvalidSetup", FatalException,
                "Direction1 is zero !");
  }
  else
  {
  ...
  if (G4tgrMessenger::GetVerboseLevel() >= 2)
    G4cout << "G4tgbPlaceParamSquare: no copies " << theNCopies << " = " << theNCopies1 << " X "
           << theNCopies2 << G4endl << " offset1 " << theOffset1 << G4endl << " offset2 "
           << theOffset1 << G4endl << " step1 " << theStep1 << G4endl << " step2 " << theStep2
           << G4endl << " direction1 " << theDirection1 << G4endl << " direction2 " << theDirection2
           << G4endl << " translation " << theTranslation << G4endl;
#endif
}
                                                  ...
                                                  G4VPhysicalVolume* physVol) const
{
  ...
  {
    G4cout << " G4tgbPlaceParamSquare::ComputeTransformation():" << physVol->GetName() << G4endl
           << "   no copies " << theNCopies << G4endl << "   offset1 " << theOffset1 << G4endl
           << "   offset2 " << theOffset2 << G4endl << "   step1 " << theStep1 << G4endl
  ...
  if (G4tgrMessenger::GetVerboseLevel() >= 3)
  {
    G4cout << " G4tgbPlaceParamSquare::ComputeTransformation() - " << copyNo << " = " << copyNo1
           << ", X " << copyNo2 << G4endl << " pos: " << origin << ", axis: " << theAxis << G4endl;
  }
#endif
