  ...
  : G4tgbPlaceParameterisation(tgrParam)
{
  ...
  if (tgrParam->GetParamType() == "LINEAR")
  {
    ...
    theDirection = G4ThreeVector(tgrParam->GetExtraData()[3], tgrParam->GetExtraData()[4],
  ...
  {
    G4Exception("G4tgbPlaceParamLinear::G4tgbPlaceParamLinear()", "InvalidSetup", FatalException,
                "Direction is zero !");
  }
  else
  {
  ...
  if (G4tgrMessenger::GetVerboseLevel() >= 2)
  {
    G4cout << " G4tgbPlaceParamLinear::G4tgbPlaceParamLinear(): "
           << " param type " << tgrParam->GetParamType() << G4endl << "   N copies " << theNCopies
           << G4endl << "   step " << theStep << G4endl << "   offset " << theOffset << G4endl
           << "   translation " << theTranslation << G4endl << "   direction " << theDirection
                                                  ...
                                                  G4VPhysicalVolume* physVol) const
{
  ...
  {
    G4cout << " G4tgbPlaceParamLinear::ComputeTransformation() -" << physVol->GetName() << G4endl
