...
void G4GDMLReadStructure::BorderSurfaceRead(const xercesc::DOMElement* const bordersurfaceElement)
{
  ...
  for (XMLSize_t attribute_index = 0; attribute_index < attributeCount; ++attribute_index)
  {
    ...
    {
      G4Exception("G4GDMLReadStructure::BorderSurfaceRead()", "InvalidRead", FatalException,
                  "No attribute found!");
      return;
    }
  ...
  for (xercesc::DOMNode* iter = bordersurfaceElement->getFirstChild(); iter != nullptr;
...
void G4GDMLReadStructure::DivisionvolRead(const xercesc::DOMElement* const divisionvolElement)
{
  ...
  for (XMLSize_t attribute_index = 0; attribute_index < attributeCount; ++attribute_index)
  {
    ...
    if (attribute == nullptr)
    {
      G4Exception("G4GDMLReadStructure::DivisionvolRead()", "InvalidRead", FatalException,
                  "No attribute found!");
      return;
    }
    const G4String attName = Transcode(attribute->getName());
  ...
  for (xercesc::DOMNode* iter = divisionvolElement->getFirstChild(); iter != nullptr;
       iter = iter->getNextSibling())
  {
    if (iter->getNodeType() != xercesc::DOMNode::ELEMENT_NODE)
    {
  ...
  {
    pair = G4ReflectionFactory::Instance()->Divide(pv_name, logvol, pMotherLogical, axis, number,
                                                   offset);
  }
  else if ((number == 0) && (width != 0.0))
  {
    pair =
      G4ReflectionFactory::Instance()->Divide(pv_name, logvol, pMotherLogical, axis, width, offset);
  }
  else
  {
    pair = G4ReflectionFactory::Instance()->Divide(pv_name, logvol, pMotherLogical, axis, number,
...
G4LogicalVolume* G4GDMLReadStructure::FileRead(const xercesc::DOMElement* const fileElement)
{
  ...
  for (XMLSize_t attribute_index = 0; attribute_index < attributeCount; ++attribute_index)
  {
    ...
    if (attribute == nullptr)
    {
      G4Exception("G4GDMLReadStructure::FileRead()", "InvalidRead", FatalException,
                  "No attribute found!");
      return nullptr;
    }
    const G4String attName = Transcode(attribute->getName());
                                      ...
                                      G4AssemblyVolume* pAssembly)
{
  ...
  for (XMLSize_t attribute_index = 0; attribute_index < attributeCount; ++attribute_index)
  {
    ...
    if (attribute == nullptr)
    {
      G4Exception("G4GDMLReadStructure::PhysvolRead()", "InvalidRead", FatalException,
                  "No attribute found!");
      return;
    }
    const G4String attName = Transcode(attribute->getName());
       ...
       iter = iter->getNextSibling())
  {
    ...
    else
    {
      ...
      G4Exception("G4GDMLReadStructure::PhysvolRead()", "ReadError", FatalException, error_msg);
      return;
    }
  }
...
{
  G4LogicalVolume* logvol = nullptr;
  for (xercesc::DOMNode* iter = replicavolElement->getFirstChild(); iter != nullptr;
       iter = iter->getNextSibling())
  {
    if (iter->getNodeType() != xercesc::DOMNode::ELEMENT_NODE)
    {
    ...
    if (child == nullptr)
    {
      G4Exception("G4GDMLReadStructure::ReplicavolRead()", "InvalidRead", FatalException,
                  "No child found!");
      return;
    }
    const G4String tag = Transcode(child->getTagName());
    ...
    else
    {
      ...
      G4Exception("G4GDMLReadStructure::ReplicavolRead()", "ReadError", FatalException, error_msg);
    }
  }
}
                                      ...
                                      G4LogicalVolume* logvol, G4int number)
{
       ...
       iter = iter->getNextSibling())
  {
    ...
    if (child == nullptr)
    {
      G4Exception("G4GDMLReadStructure::ReplicaRead()", "InvalidRead", FatalException,
                  "No child found!");
      return;
    }
    const G4String tag = Transcode(child->getTagName());
    ...
    else
    {
      ...
      G4Exception("G4GDMLReadStructure::ReplicaRead()", "ReadError", FatalException, error_msg);
    }
  }
...
EAxis G4GDMLReadStructure::AxisRead(const xercesc::DOMElement* const axisElement)
{
  ...
  const xercesc::DOMNamedNodeMap* const attributes = axisElement->getAttributes();
  XMLSize_t attributeCount = attributes->getLength();
  ...
  for (XMLSize_t attribute_index = 0; attribute_index < attributeCount; ++attribute_index)
  {
    ...
    if (attribute == nullptr)
    {
      G4Exception("G4GDMLReadStructure::AxisRead()", "InvalidRead", FatalException,
                  "No attribute found!");
      return axis;
    }
    const G4String attName = Transcode(attribute->getName());
...
G4double G4GDMLReadStructure::QuantityRead(const xercesc::DOMElement* const readElement)
{
  G4double value = 0.0;
  G4double unit = 0.0;
  const xercesc::DOMNamedNodeMap* const attributes = readElement->getAttributes();
  XMLSize_t attributeCount = attributes->getLength();
  ...
  for (XMLSize_t attribute_index = 0; attribute_index < attributeCount; ++attribute_index)
  {
    ...
    if (attribute == nullptr)
    {
      G4Exception("G4GDMLReadStructure::QuantityRead()", "InvalidRead", FatalException,
                  "No attribute found!");
      return value;
    }
    const G4String attName = Transcode(attribute->getName());
...
void G4GDMLReadStructure::VolumeRead(const xercesc::DOMElement* const volumeElement)
{
       ...
       iter = iter->getNextSibling())
  {
    ...
    if (child == nullptr)
    {
      G4Exception("G4GDMLReadStructure::VolumeRead()", "InvalidRead", FatalException,
                  "No child found!");
      return;
    }
    const G4String tag = Transcode(child->getTagName());
...
void G4GDMLReadStructure::AssemblyRead(const xercesc::DOMElement* const assemblyElement)
{
  ...
  for (xercesc::DOMNode* iter = assemblyElement->getFirstChild(); iter != nullptr;
       iter = iter->getNextSibling())
  {
    if (iter->getNodeType() != xercesc::DOMNode::ELEMENT_NODE)
    {
    ...
    if (child == nullptr)
    {
      G4Exception("G4GDMLReadStructure::AssemblyRead()", "InvalidRead", FatalException,
                  "No child found!");
      return;
    }
    const G4String tag = Transcode(child->getTagName());
    ...
    {
      G4cout << "Unsupported GDML tag '" << tag << "' for Geant4 assembly structure !" << G4endl;
    }
  }
}
...
void G4GDMLReadStructure::SkinSurfaceRead(const xercesc::DOMElement* const skinsurfaceElement)
{
  ...
  for (XMLSize_t attribute_index = 0; attribute_index < attributeCount; ++attribute_index)
  {
    ...
    if (attribute == nullptr)
    {
      G4Exception("G4GDMLReadStructure::SkinsurfaceRead()", "InvalidRead", FatalException,
                  "No attribute found!");
      return;
    }
    const G4String attName = Transcode(attribute->getName());
  ...
  for (xercesc::DOMNode* iter = skinsurfaceElement->getFirstChild(); iter != nullptr;
       iter = iter->getNextSibling())
  {
    if (iter->getNodeType() != xercesc::DOMNode::ELEMENT_NODE)
    {
    ...
    else
    {
      ...
      G4Exception("G4GDMLReadStructure::SkinsurfaceRead()", "ReadError", FatalException, error_msg);
    }
  }
...
void G4GDMLReadStructure::Volume_contentRead(const xercesc::DOMElement* const volumeElement)
{
       ...
       iter = iter->getNextSibling())
  {
    ...
    if (child == nullptr)
    {
      G4Exception("G4GDMLReadStructure::Volume_contentRead()", "InvalidRead", FatalException,
                  "No child found!");
      return;
    }
    const G4String tag = Transcode(child->getTagName());
    ...
    {
      G4cout << "Treating unknown GDML tag in volume '" << tag << "' as GDML extension..."
             << G4endl;
    }
  }
}
...
#endif
  for (xercesc::DOMNode* iter = structureElement->getFirstChild(); iter != nullptr;
       iter = iter->getNextSibling())
  {
    if (iter->getNodeType() != xercesc::DOMNode::ELEMENT_NODE)
    {
    ...
    if (child == nullptr)
    {
      G4Exception("G4GDMLReadStructure::StructureRead()", "InvalidRead", FatalException,
                  "No child found!");
      return;
    }
    const G4String tag = Transcode(child->getTagName());
    ...
    else
    {
      ...
      G4Exception("G4GDMLReadStructure::StructureRead()", "ReadError", FatalException, error_msg);
    }
  }
}
...
G4VPhysicalVolume* G4GDMLReadStructure::GetPhysvol(const G4String& ref) const
{
  ...
  if (physvolPtr == nullptr)
  {
    ...
    G4Exception("G4GDMLReadStructure::GetPhysvol()", "ReadError", FatalException, error_msg);
  }
...
G4LogicalVolume* G4GDMLReadStructure::GetVolume(const G4String& ref) const
{
  ...
  if (volumePtr == nullptr)
  {
    ...
    G4Exception("G4GDMLReadStructure::GetVolume()", "ReadError", FatalException, error_msg);
  }
...
G4VPhysicalVolume* G4GDMLReadStructure::GetWorldVolume(const G4String& setupName)
{
  ...
  {
    pvWorld = new G4PVPlacement(nullptr, G4ThreeVector(0, 0, 0), volume, volume->GetName() + "_PV",
                                0, 0, 0);
