...
void G4STRead::TessellatedRead(const std::string& line)
{
  ...
  volumeMap[tessellated] = new G4LogicalVolume(tessellated, solid_material, name + "_LV", 0, 0, 0);
...
void G4STRead::FacetRead(const std::string& line)
{
  ...
  if (line[2] == '3')  // Triangular facet
  {
    ...
    tessellatedList.back()->AddFacet(new G4TriangularFacet(
      G4ThreeVector(x1, y1, z1), G4ThreeVector(x2, y2, z2), G4ThreeVector(x3, y3, z3), ABSOLUTE));
  }
  else if (line[2] == '4')  // Quadrangular facet
  {
    ...
    tessellatedList.back()->AddFacet(
      new G4QuadrangularFacet(G4ThreeVector(x1, y1, z1), G4ThreeVector(x2, y2, z2),
...
void G4STRead::PhysvolRead(const std::string& line)
{
  ...
  std::istringstream stream(line.substr(2));
  stream >> level >> name >> r1 >> r2 >> r3 >> n1 >> r4 >> r5 >> r6 >> n2 >> r7 >> r8 >> r9 >> n3
    >> pX >> pY >> pZ >> n4 >> n5;
  ...
  {
    G4Exception("G4STRead::PhysvolRead()", "ReadError", FatalException, "Invalid input stream!");
    return;
  }
  ...
  if (tessellated == nullptr)
  {
    ...
    G4Exception("G4STRead::PhysvolRead()", "ReadError", FatalException, error_msg);
  }
  if (volumeMap.find(tessellated) == volumeMap.cend())
  {
    G4String error_msg = "Referenced solid '" + name + "' is not associated with a logical volume!";
    G4Exception("G4STRead::PhysvolRead()", "InvalidSetup", FatalException, error_msg);
  }
  const G4RotationMatrix rot(G4ThreeVector(r1, r2, r3), G4ThreeVector(r4, r5, r6),
                             G4ThreeVector(r7, r8, r9));
  const G4ThreeVector pos(pX, pY, pZ);

  new G4PVPlacement(G4Transform3D(rot.inverse(), pos), volumeMap[tessellated], name + "_PV",
                    world_volume, 0, 0);
...
// --------------------------------------------------------------------
G4LogicalVolume* G4STRead::Read(const G4String& name, G4Material* mediumMaterial,
                                G4Material* solidMaterial)
{
  ...
  world_volume = new G4LogicalVolume(world_box, mediumMaterial, "TessellatedWorldLV", 0, 0, 0);
