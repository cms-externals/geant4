...
// --------------------------------------------------------------------
G4GDMLWriteMaterials::G4GDMLWriteMaterials() : G4GDMLWriteDefine() {}
...
void G4GDMLWriteMaterials::ElementWrite(const G4Element* const elementPtr)
{
  ...
  if (NumberOfIsotopes > 0)
  {
    ...
    {
      G4String fractionref =
        GenerateName(elementPtr->GetIsotope(i)->GetName(), elementPtr->GetIsotope(i));
          ...
          && materialPtr->GetElement(0)->GetNumberOfIsotopes() > 1))
  {
    ...
    for (G4int i = 0; i < NumberOfElements; ++i)
    {
      const G4String fractionref =
        GenerateName(materialPtr->GetElement(i)->GetName(), materialPtr->GetElement(i));
      xercesc::DOMElement* fractionElement = NewElement("fraction");
...
// --------------------------------------------------------------------
void G4GDMLWriteMaterials::PropertyConstWrite(const G4String& key, const G4double pval,
                                         ...
                                         const G4Material* const mat)
{
  ...
  for (size_t i = 0; i < cvec.size(); ++i)
  {
    if (cvec[i].second == true)
    {
      ...
      propElement->setAttributeNode(
        NewAttribute("ref", GenerateName(ptable->GetMaterialConstPropertyNames()[i], ptable)));
      PropertyConstWrite(ptable->GetMaterialConstPropertyNames()[i], cvec[i].first, ptable);
      matElement->appendChild(propElement);
    }
  }
}
