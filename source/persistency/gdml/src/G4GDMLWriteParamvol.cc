...
// --------------------------------------------------------------------
G4GDMLWriteParamvol::G4GDMLWriteParamvol() : G4GDMLWriteSetup() {}
...
void G4GDMLWriteParamvol::ParametersWrite(xercesc::DOMElement* paramvolElement,
                                          ...
                                          const G4int& index)
{
  paramvol->GetParameterisation()->ComputeTransformation(index,
  ...
  else
  {
    ...
    G4Exception("G4GDMLWriteParamvol::ParametersWrite()", "InvalidSetup", FatalException,
                error_msg);
  }
}
                                        ...
                                        const G4VPhysicalVolume* const paramvol)
{
  const G4String volumeref =
    GenerateName(paramvol->GetLogicalVolume()->GetName(), paramvol->GetLogicalVolume());
