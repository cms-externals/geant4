...
// --------------------------------------------------------------------
G4GDMLWriteStructure::G4GDMLWriteStructure() : G4GDMLWriteParamvol(), maxLevel(INT_MAX)
{
                                            ...
                                            const G4PVDivision* const divisionvol)
{
  ...
  const G4String volumeref =
    GenerateName(divisionvol->GetLogicalVolume()->GetName(), divisionvol->GetLogicalVolume());
                                           ...
                                           const G4VPhysicalVolume* const replicavol)
{
  ...
  const G4String volumeref =
    GenerateName(replicavol->GetLogicalVolume()->GetName(), replicavol->GetLogicalVolume());
...
// --------------------------------------------------------------------
void G4GDMLWriteStructure::AssemblyWrite(xercesc::DOMElement* volumeElement, const G4int assemblyID)
{
  ...
  for (std::size_t i5 = 0; i5 < myassembly->TotalTriplets(); ++i5)
  {
    ...
    if (lvol == nullptr)
    {
      ...
      G4Exception("G4GDMLWriteStructure::AssemblyWrite()", "InvalidSetup", FatalException, message);
      return;
    }
...
void G4GDMLWriteStructure::BorderSurfaceCache(const G4LogicalBorderSurface* const bsurf)
{
  ...
  if (FindOpticalSurface(psurf))
  {
    ...
    if (opsurf == nullptr)
    {
      G4Exception("G4GDMLWriteStructure::BorderSurfaceCache()", "InvalidSetup", FatalException,
...
void G4GDMLWriteStructure::SkinSurfaceCache(const G4LogicalSkinSurface* const ssurf)
{
  ...
  const G4String volumeref =
    GenerateName(ssurf->GetLogicalVolume()->GetName(), ssurf->GetLogicalVolume());
  xercesc::DOMElement* volumerefElement = NewElement("volumeref");
  volumerefElement->setAttributeNode(NewAttribute("ref", volumeref));
  ...
  if (FindOpticalSurface(psurf))
  {
    ...
    if (opsurf == nullptr)
    {
      G4Exception("G4GDMLWriteStructure::SkinSurfaceCache()", "InvalidSetup", FatalException,
                  "No optical surface found!");
      return;
    }
    OpticalSurfaceWrite(solidsElement, opsurf);
                                                       ...
                                                       const G4int depth)
{
  ...
  {  // of the referenced solid!
    ...
    {
      G4String ErrorMessage = "Referenced solid in volume '" + volumePtr->GetName()
                              + "' was displaced/reflected too many times!";
      G4Exception("G4GDMLWriteStructure::TraverseVolumeTree()", "InvalidSetup", FatalException,
                  ErrorMessage);
    }
  ...
  for (std::size_t i = 0; i < daughterCount; ++i)  // Traverse all the children!
  {
          ...
          dynamic_cast<const G4PVDivision*>(physvol))  // Is it division?
    {
      if (!G4Transform3D::Identity.isNear(invR * daughterR, kRelativePrecision))
      {
        G4String ErrorMessage =
          "Division volume in '" + name + "' can not be related to reflected solid!";
        G4Exception("G4GDMLWriteStructure::TraverseVolumeTree()", "InvalidSetup", FatalException,
                    ErrorMessage);
      }
    ...
    else
    {
      if (physvol->IsParameterised())  // Is it a paramvol?
      {
        ...
        {
          G4String ErrorMessage =
            "Parameterised volume in '" + name + "' can not be related to reflected solid!";
          G4Exception("G4GDMLWriteStructure::TraverseVolumeTree()", "InvalidSetup", FatalException,
                      ErrorMessage);
        }
      ...
      else
      {
        if (physvol->IsReplicated())  // Is it a replicavol?
        {
          ...
          {
            G4String ErrorMessage =
              "Replica volume in '" + name + "' can not be related to reflected solid!";
        ...
        else  // Is it a physvol or an assembly?
        {
          if (assemblyVolMap.find(physvol) != assemblyVolMap.cend())
          {
            ...
            if (std::find(addedAssemblies.cbegin(), addedAssemblies.cend(), assemblyID)
                == addedAssemblies.cend())
            {
            ...
            if (std::find(addedImprints.cbegin(), addedImprints.cend(), imprintID)
                == addedImprints.cend())
            {
...
void G4GDMLWriteStructure::ExportEnergyCuts(const G4LogicalVolume* const lvol)
{
  ...
  G4double gamma_cut =
    ctab->ConvertRangeToEnergy(gamma, lvol->GetMaterial(), pcuts->GetProductionCut("gamma"));
  G4double eminus_cut =
    ctab->ConvertRangeToEnergy(eminus, lvol->GetMaterial(), pcuts->GetProductionCut("e-"));
  G4double eplus_cut =
    ctab->ConvertRangeToEnergy(eplus, lvol->GetMaterial(), pcuts->GetProductionCut("e+"));
  G4double proton_cut =
    ctab->ConvertRangeToEnergy(proton, lvol->GetMaterial(), pcuts->GetProductionCut("proton"));
