                      ...
                      G4bool isModule, G4bool strip)
{
  ...
  if(validate)
  {
    ...
    if (schema != "")  // Load alternative schema grammar specified by user
    {                  // NOTE: expecting a valid XML schema!
       G4cout << "G4GDML: Using alternative schema file: " << schema << " ..."
              << G4endl;
       parser->loadGrammar(schema.c_str(),
