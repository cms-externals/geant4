...
void G4GDMLRead::StripNames() const
{
  ...
  G4cout << "Stripping off GDML names of materials, solids and volumes ..." << G4endl;
                          ...
                          void (G4GDMLRead::*func)(const xercesc::DOMElement* const))
{
  ...
  for (XMLSize_t attribute_index = 0; attribute_index < attributeCount; ++attribute_index)
  {
    ...
    if (!attribute)
    {
      G4Exception("G4GDMLRead::LoopRead()", "InvalidRead", FatalException, "No attribute found!");
      return;
    }
  ...
  {
    G4Exception("G4GDMLRead::loopRead()", "InvalidRead", FatalException, "Infinite loop!");
  }
  if ((_from > _to) && (_step >= 0))
  {
    G4Exception("G4GDMLRead::loopRead()", "InvalidRead", FatalException, "Infinite loop!");
...
// --------------------------------------------------------------------
G4GDMLAuxStructType G4GDMLRead::AuxiliaryRead(const xercesc::DOMElement* const auxiliaryElement)
{
  ...
  for (xercesc::DOMNode* iter = auxiliaryElement->getFirstChild(); iter != nullptr;
       iter = iter->getNextSibling())
  {
    ...
    if (!child)
    {
      G4Exception("G4GDMLRead::AuxiliaryRead()", "InvalidRead", FatalException, "No child found!");
      break;
    }
...
#endif
  for (xercesc::DOMNode* iter = userinfoElement->getFirstChild(); iter != nullptr;
       iter = iter->getNextSibling())
  {
    if (iter->getNodeType() != xercesc::DOMNode::ELEMENT_NODE)
    {
    ...
    if (!child)
    {
      G4Exception("G4GDMLRead::UserinfoRead()", "InvalidRead", FatalException, "No child found!");
      return;
    }
    const G4String tag = Transcode(child->getTagName());
    ...
    else
    {
      ...
      G4Exception("G4GDMLRead::UserinfoRead()", "ReadError", FatalException, error_msg);
    }
  }
}
...
void G4GDMLRead::ExtensionRead(const xercesc::DOMElement* const)
{
  ...
  G4Exception("G4GDMLRead::ExtensionRead()", "NotImplemented", JustWarning, error_msg);
}
...
// --------------------------------------------------------------------
void G4GDMLRead::Read(const G4String& fileName, G4bool validation, G4bool isModule, G4bool strip)
{
  dostrip = strip;
#ifdef G4VERBOSE
  if (isModule)
  ...
  if (validate)
  {
    ...
    // Will fall back to default validation otherwise (`xsi:noNamespaceSchemaLocation` in input
    // file)
    if (auto schemaPath = G4GetEnv<G4String>("G4GDML_SCHEMA_FILE", schema); schemaPath != "")
    {
      // Pre-parse grammar to check it's present/valid
      if (parser->loadGrammar(schemaPath.c_str(), xercesc::Grammar::SchemaGrammarType, true)
          != nullptr)
      {
      ...
      {
        G4Exception("G4GDMLRead::Read()", "InvalidGDMLSchemaFile", FatalException,
  ...
  if (element == nullptr)
  {
    ...
    message << "ERROR - Empty document or unable to validate schema!" << G4endl
            << "        Check Internet connection is ON in case of schema" << G4endl
            << "        validation enabled and location defined as URL in" << G4endl
            << "        the GDML file - " << fileName << " - being imported!" << G4endl
       ...
       iter = iter->getNextSibling())
  {
    ...
    if (child == nullptr)
    {
      G4Exception("G4GDMLRead::Read()", "InvalidRead", FatalException, "No child found!");
      return;
    }
    const G4String tag = Transcode(child->getTagName());
