...
// --------------------------------------------------------------------
G4GDMLParser::G4GDMLParser() : strip(true)
{
...
// --------------------------------------------------------------------
G4GDMLParser::G4GDMLParser(G4GDMLReadStructure* extr) : urcode(true), strip(true)
{
  reader = extr;
  writer = new G4GDMLWriteStructure;
  messenger = new G4GDMLMessenger(this);
...
// --------------------------------------------------------------------
G4GDMLParser::G4GDMLParser(G4GDMLReadStructure* extr, G4GDMLWriteStructure* extw)
    ...
    else  // Create region and loop over all region attributes
    {
      ...
      for (auto raux = iaux->auxList->cbegin(); raux != iaux->auxList->cend(); ++raux)
      {
        ...
        if (tag == "volume")
        {
          ...
          if (pos != store->GetMap().cend())
          {
            ...
            if (pos->second.size() > 1)
            {
              ...
              G4Exception("G4GDMLParser::ImportRegions()", "Notification", JustWarning, message);
            }
          ...
          else
          {
            ...
            G4Exception("G4GDMLParser::ImportRegions()", "InvalidSetup", JustWarning, message);
          }
        }
        else if (tag == "pcut")
        {
          ...
          {
            G4Exception("G4GDMLParser::ImportRegions()", "InvalidRead", FatalException,
        ...
        else if (tag == "ulimits")
        {
          ...
          G4UserLimits* ulimits =
            new G4UserLimits(ulname, ustepMax, utrakMax, utimeMax, uekinMin, urangMin);
