...
// --------------------------------------------------------------------
G4MCTSimParticle::G4MCTSimParticle(const G4String& aname, G4int apcode, G4int atid, G4int ptid,
                                   const G4LorentzVector& p)
  : name(aname), momentumAtVertex(p), pdgID(apcode), trackID(atid), parentTrackID(ptid)
{}
...
void G4MCTSimParticle::PrintSingle(std::ostream& ostr) const
{
  ...
  ostr << ": P(" << std::setw(7) << std::setprecision(3) << momentumAtVertex.x() / GeV << ","
       << std::setw(7) << std::setprecision(3) << momentumAtVertex.y() / GeV << "," << std::setw(7)
       << std::setprecision(3) << momentumAtVertex.z() / GeV << "," << std::setw(7)
       << std::setprecision(3) << momentumAtVertex.e() / GeV << ") @";
  ostr << name << "(" << pdgID << ")";
  ...
  if (vertex != nullptr)
  {
    ...
    ostr << "- X(" << std::setw(9) << std::setprecision(2) << vertex->GetPosition().x() / mm << ","
         << std::setw(9) << std::setprecision(2) << vertex->GetPosition().y() / mm << ","
         << std::setw(9) << std::setprecision(2) << vertex->GetPosition().z() / mm << ","
         << std::setw(9) << std::setprecision(2) << vertex->GetTime() / ns << ")";
...
void G4MCTSimParticle::Print(std::ostream& ostr, G4bool qrevorder) const
{
  ...
  if (!qrevorder)
  {  // parent -> child
    for (auto itr = associatedParticleList.cbegin(); itr != associatedParticleList.cend(); ++itr)
    {
      (*itr)->Print(ostr);
    }
  }
