...
// --------------------------------------------------------------------
G4PersistencyManager::G4PersistencyManager(G4PersistencyCenter* ptc, const G4String& n)
  : f_pc(ptc), nameMgr(n)
{
...
void G4PersistencyManager::SetVerboseLevel(G4int v)
{
  ...
  if (m_verbose > 2)
  {
    G4cout << "G4PersistencyManager[\"" << nameMgr << "\"," << this << "]: verbose level is set to "
           << m_verbose << "." << G4endl;
  }
  if (EventIO() != nullptr) EventIO()->SetVerboseLevel(m_verbose);
  if (MCTruthIO() != nullptr) MCTruthIO()->SetVerboseLevel(m_verbose);
  if (HitIO() != nullptr) HitIO()->SetVerboseLevel(m_verbose);
  if (DigitIO() != nullptr) DigitIO()->SetVerboseLevel(m_verbose);
  if (TransactionManager() != nullptr) TransactionManager()->SetVerboseLevel(m_verbose);
...
G4bool G4PersistencyManager::Store(const G4Event* evt)
{
  if (m_verbose > 2)
  {
    G4cout << "G4PersistencyManager::Store() is called for event# " << evt->GetEventID() << "."
           << G4endl;
  }

  if (TransactionManager() == nullptr) return true;
  ...
  if (!f_is_initialized)
  {
    ...
    if (m_verbose > 1)
    {
      G4cout << "G4PersistencyManager:: Initializing Transaction ... " << G4endl;
    }
    Initialize();
  }
  ...
  if (TransactionManager()->StartUpdate())
  {
    if (m_verbose > 2)
    {
      G4cout << "G4PersistencyManager: Update transaction started for event#" << evt->GetEventID()
             << "." << G4endl;
    }
  }
  else
  {
    G4cerr << "TransactionManager::Store(G4Event) - StartUpdate() failed." << G4endl;
    return false;
  }
  ...
  if (mctevt != 0 || evt != 0)
  {
    ...
    if (TransactionManager()->SelectWriteFile(obj, file))
    {
      ...
      if (st3 && m_verbose > 1)
      {
        G4cout << " -- File name: " << f_pc->CurrentWriteFile("Hits") << " -- Event# "
               << evt->GetEventID() << " -- G4Pevent is Stored." << G4endl;
      }
    }
    else
    {
  ...
  if (st)
  {
    ...
    if (m_verbose > 0)
      G4cout << "G4PersistencyManager: event# " << evt->GetEventID() << " is stored." << G4endl;
  }
  else
  {
    G4cerr << "G4PersistencyManager::Store(G4Event) - Transaction aborted." << G4endl;
...
G4bool G4PersistencyManager::Retrieve(G4Event*& evt)
{
  ...
  if (f_pc->CurrentRetrieveMode("MCTruth") == false && f_pc->CurrentRetrieveMode("Hits") == false
  ...
  {
    G4cerr << "TransactionManager::Retrieve(G4Event) - StartRead() failed." << G4endl;
    return false;
  }
  ...
  {
    G4cerr << "G4PersistencyManager::Retrieve() - Transaction aborted." << G4endl;
    TransactionManager()->Abort();
  }
