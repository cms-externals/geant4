...
G4VTreeSceneHandler::G4VTreeSceneHandler(G4VGraphicsSystem& system, const G4String& name)
  : G4VSceneHandler(system, fSceneIdCount++, name), fpCurrentObjectTransformation(0)
{}
                                      ...
                                      const G4VisAttributes& visAttribs)
{
  ...
  // physical volume...
  fDrawnLVStore.insert(drawnPVPath.back().GetPhysicalVolume()->GetLogicalVolume());
  ...
  if (ri != drawnPVPath.rend())
  {
    ...
    G4LogicalVolume* drawnMotherLV = ri->GetPhysicalVolume()->GetLogicalVolume();
