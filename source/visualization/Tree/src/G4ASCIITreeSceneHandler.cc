...
G4ASCIITreeSceneHandler::G4ASCIITreeSceneHandler(G4VGraphicsSystem& system, const G4String& name)
...
void G4ASCIITreeSceneHandler::WriteHeader(std::ostream& os)
{
  ...
  {
    firstTime = false;
    for (size_t i = 0; i < pSystem->GetVerbosityGuidance().size(); ++i)
    {
...
void G4ASCIITreeSceneHandler::EndModeling()
{
  ...
  {
    if (fLastCopyNo == fLastNonSequentialCopyNo + 1)
      *fpOutFile << ',';
    else
      *fpOutFile << '-';
  ...
  if (detail >= 4)
  {
    ...
    {
      G4PhysicalVolumeModel* pvModel = dynamic_cast<G4PhysicalVolumeModel*>(i->fpModel);
      if (pvModel)
      {
        ...
        G4cout << "Overall volume of \"" << pvModel->GetTopPhysicalVolume()->GetName()
               << "\":" << pvModel->GetTopPhysicalVolume()->GetCopyNo() << ", is "
               << G4BestUnit(volume, "Volume") << " and the daughter-included mass";
        ...
        {
          G4cout << ", ignoring daughters at depth " << requestedDepth << " and below,";
        }
        G4cout << " is " << G4BestUnit(mass, "Mass") << G4endl;
...
void G4ASCIITreeSceneHandler::RequestPrimitives(const G4VSolid& solid)
{
  ...
  if (isAmenableToSupression)
  {
    ...
    G4bool ignore = false;
    for (ReplicaSetIterator i = fReplicaSet.begin(); i != fReplicaSet.end(); ++i)
    {
          ...
          == thisID->GetPhysicalVolume()->GetLogicalVolume())
      {
        ...
        // by
        //   (*i)[i->size() - 2].
        if (motherID == drawnPVPath.rend() && i->size() <= 1)
  ...
  // taken out).
  if (verbosity < 10 && !isParameterised && currentPVName == fLastPVName
      && currentCopyNo != fLastCopyNo)
  {
    // Check...
    if (isAmenableToSupression)
    {
      G4Exception("G4ASCIITreeSceneHandler::RequestPrimitives", "vistree0001", JustWarning,
  ...
  for (size_t i = 0; i < drawnPVPath.size(); i++)
    *fpOutFile << "  ";
  *fpOutFile << "\"" << currentPVName << "\":" << currentCopyNo;
      ...
      if (pP)
      {
        if (detail < 3)
        {
          ...
          if (nReplicas > 2)
            ...
            fRestOfLine << ',';
          fRestOfLine << nReplicas - 1 << " (" << nReplicas << " parametrised volumes)";
        }
      }
      else
      {
        ...
        if (nReplicas > 2)
          ...
          fRestOfLine << ',';
        fRestOfLine << nReplicas - 1 << " (" << nReplicas << " replicas)";
      }
    }
  }
  else
  ...
  if (detail >= 1)
  {
    fRestOfLine << " / \"" << pCurrentLV->GetName() << "\"";
  ...
  if (detail >= 2)
  {
    fRestOfLine << " / \"" << solid.GetName() << "\"(" << solid.GetEntityType() << ")";
  }

  if (detail >= 3)
  {
    fRestOfLine << ", " << G4BestUnit(((G4VSolid&)solid).GetCubicVolume(), "Volume") << ", ";
    if (pCurrentMaterial)
    {
      fRestOfLine << G4BestUnit(pCurrentMaterial->GetDensity(), "Volumic Mass") << " ("
  ...
  if (detail >= 5)
  {
    if (pCurrentMaterial)
    {
      ...
      G4double daughter_subtracted_mass =
        pCurrentLV->GetMass(pCurrentPV->IsParameterised(),  // Force if parametrised.
      ...
      fRestOfLine << ", " << G4BestUnit(daughter_subtracted_volume, "Volume") << ", "
