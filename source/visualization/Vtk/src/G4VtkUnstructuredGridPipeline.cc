...
G4VtkUnstructuredGridPipeline::G4VtkUnstructuredGridPipeline(G4String nameIn,
...
{
  if (fpPVModel->GetCurrentDepth() == fDepth)
  {  // Leaf-level cells only
    ...
    try
    {
      ...
      if (iCell > 0 && iCell < 100000000)
      {
        ...
        fpCellColourValues->InsertNextTuple4(colour.GetRed(), colour.GetGreen(), colour.GetBlue(),
                                             colour.GetAlpha());
...
void G4VtkUnstructuredGridPipeline::PseudoSceneForCubicalCells::AddSolid(const G4Box& box)
{
  ...
  {  // Leaf-level cells only
    try
    {
      ...
      fpPoints->InsertNextPoint(position.x() - box.GetXHalfLength(),
                                position.y() - box.GetYHalfLength(),
                                position.z() - box.GetZHalfLength());
      fpPoints->InsertNextPoint(position.x() - box.GetXHalfLength(),
                                position.y() + box.GetYHalfLength(),
                                position.z() - box.GetZHalfLength());
      fpPoints->InsertNextPoint(position.x() + box.GetXHalfLength(),
      ...
      double cols[] = {colour.GetRed(), colour.GetGreen(), colour.GetBlue(), colour.GetAlpha()};
...
void G4VtkUnstructuredGridPipeline::SetUnstructuredGridData(const G4Mesh& mesh)
{
  ...
  {
    PseudoSceneForTetCells pseudoScene(&tmpPVModel, mesh.GetMeshDepth(), points, pointColourValues,
                                       cellColourValues, pointColourIndices, cellColourIndices,
                                       colourLUT, unstructuredGrid);
    tmpPVModel.DescribeYourselfTo(pseudoScene);
    // G4cout << pseudoScene.GetNumberOfPoints() << " " << pseudoScene.GetNumberOfCells() << " " <<
    // pseudoScene.GetNumberOfAddedCells() << G4endl; pseudoScene.DumpColourMap();
  }
  else if (mesh.GetMeshType() == G4Mesh::nested3DRectangular)
  {
    PseudoSceneForCubicalCells pseudoScene(&tmpPVModel, mesh.GetMeshDepth(), points,
                                           pointColourValues, cellColourValues, pointColourIndices,
                                           cellColourIndices, colourLUT, unstructuredGrid);
    tmpPVModel.DescribeYourselfTo(pseudoScene);
  }
