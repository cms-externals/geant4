...
void G4VisCommandSceneAddArrow::SetNewValue(G4UIcommand*, G4String newValue)
{
  ...
  G4double arrowWidth = 0.005 * fCurrentLineWidth * sceneExtent.GetExtentRadius();

  G4VModel* model = new G4ArrowModel(x1, y1, z1, x2, y2, z2, arrowWidth, fCurrentColour, newValue,
...
void G4VisCommandSceneAddArrow2D::SetNewValue(G4UIcommand*, G4String newValue)
{
  ...
  Arrow2D* arrow2D = new Arrow2D(x1, y1, x2, y2, fCurrentLineWidth, fCurrentColour);
  G4VModel* model = new G4CallbackModel<G4VisCommandSceneAddArrow2D::Arrow2D>(arrow2D);
  ...
  if (successful)
  {
    if (verbosity >= G4VisManager::confirmations)
    {
      G4cout << "A 2D arrow has been added to scene \"" << currentSceneName << "\"." << G4endl;
    }
  }
  else
    G4VisCommandsSceneAddUnsuccessful(verbosity);
...
G4VisCommandSceneAddArrow2D::Arrow2D::Arrow2D(G4double x1, G4double y1, G4double x2, G4double y2,
...
G4VisCommandSceneAddAxes::G4VisCommandSceneAddAxes()
{
  ...
  fpCommand->SetGuidance("Draws axes at (x0, y0, z0) of given length and colour.");
  fpCommand->SetGuidance(
    ...
    "\n  use \"/vis/list\".");
  fpCommand->SetGuidance("If \"length\" is negative, it is set to about 25% of scene extent.");
  fpCommand->SetGuidance("If \"showtext\" is false, annotations are suppressed.");
  G4UIparameter* parameter;
...
void G4VisCommandSceneAddAxes::SetNewValue(G4UIcommand*, G4String newValue)
{
  ...
  else
  {
    if (pScene->GetExtent().GetExtentRadius() <= 0.)
    {
      if (verbosity >= G4VisManager::errors)
      {
        G4warn << "ERROR: Scene has no extent. Add volumes or use \"/vis/scene/add/extent\"."
               << G4endl;
      }
      return;
    }
  ...
  std::istringstream is(newValue);
  is >> x0 >> y0 >> z0 >> length >> unitString >> colourString >> showTextString;
  ...
  if (length < 0.)
  {
    ...
    if (5. * length < lengthMax)
      length *= 5.;
    else if (2. * length < lengthMax)
      length *= 2.;
  }
  else
  {
  ...
  // Consult scene for arrow width...
  G4double arrowWidth = 0.05 * fCurrentLineWidth * sceneExtent.GetExtentRadius();
  // ...but limit it to length/30.
  if (arrowWidth > length / 30.) arrowWidth = length / 30.;

  G4VModel* model = new G4AxesModel(x0, y0, z0, length, arrowWidth, colourString, newValue,
  ...
  if (successful)
  {
    if (verbosity >= G4VisManager::confirmations)
    {
      G4cout << "Axes of length " << G4BestUnit(length, "Length") << "have been added to scene \""
             << currentSceneName << "\"." << G4endl;
    }
  }
  else
    G4VisCommandsSceneAddUnsuccessful(verbosity);
...
void G4VisCommandSceneAddDate::SetNewValue(G4UIcommand*, G4String newValue)
{
  ...
  if (layoutString[0] == 'l')
    layout = G4Text::left;
  else if (layoutString[0] == 'c')
    layout = G4Text::centre;
  else if (layoutString[0] == 'r')
    layout = G4Text::right;

  Date* date = new Date(fpVisManager, size, x, y, layout, dateString);
  G4VModel* model = new G4CallbackModel<G4VisCommandSceneAddDate::Date>(date);
  model->SetType("Date");
  ...
  if (successful)
  {
    if (verbosity >= G4VisManager::confirmations)
    {
      G4cout << "Date has been added to scene \"" << currentSceneName << "\"." << G4endl;
    }
  }
  else
    G4VisCommandsSceneAddUnsuccessful(verbosity);
...
void G4VisCommandSceneAddDigis::SetNewValue(G4UIcommand*, G4String)
{
  ...
  if (successful)
  {
    if (verbosity >= G4VisManager::confirmations)
    {
      G4cout << "Digis, if any, will be drawn at end of run in scene \"" << currentSceneName
             << "\"." << G4endl;
    }
  }
  else
...
G4VisCommandSceneAddElectricField::G4VisCommandSceneAddElectricField()
{
  ...
  fpCommand->SetGuidance("Adds electric field representation to current scene.");
  fpCommand->SetGuidance(
...
void G4VisCommandSceneAddElectricField::SetNewValue(G4UIcommand*, G4String newValue)
{
  G4VisManager::Verbosity verbosity = fpVisManager->GetVerbosity();
  G4bool warn(verbosity >= G4VisManager::warnings);
  ...
  G4ElectricFieldModel::Representation modelRepresentation = G4ElectricFieldModel::fullArrow;
  if (representation == "lightArrow")
  {
    modelRepresentation = G4ElectricFieldModel::lightArrow;
  }
  G4VModel* model;
  model = new G4ElectricFieldModel(nDataPointsPerHalfExtent, modelRepresentation,
                                   fCurrentArrow3DLineSegmentsPerCircle, fCurrentExtentForField,
  ...
  if (successful)
  {
    if (verbosity >= G4VisManager::confirmations)
    {
      G4cout << "Electric field, if any, will be drawn in scene \"" << currentSceneName
             << "\"\n  with " << nDataPointsPerHalfExtent
             << " data points per half extent and with representation \"" << representation << '\"'
             << G4endl;
    }
  }
  else
...
void G4VisCommandSceneAddEndOfRunMacro::SetNewValue(G4UIcommand*, G4String newValue)
{
  ...
  if (successful)
  {
    if (verbosity >= G4VisManager::confirmations)
    {
      G4cout << "EndOfRunMacro has been added to scene \"" << currentSceneName << "\"." << G4endl;
    }
  }
  else
    G4VisCommandsSceneAddUnsuccessful(verbosity);
...
void G4VisCommandSceneAddEndOfRunMacro::EndOfRunMacro::operator()(G4VGraphicsScene&,
                                                                  const G4ModelingParameters*)
{
...
void G4VisCommandSceneAddEventID::SetNewValue(G4UIcommand*, G4String newValue)
{
  ...
  EventID* eoeEventID = new EventID(forEndOfEvent, fpVisManager, size, x, y, layout);
  G4VModel* eoeModel = new G4CallbackModel<G4VisCommandSceneAddEventID::EventID>(eoeEventID);
  ...
  // For End of Run
  EventID* eorEventID = new EventID(forEndOfRun, fpVisManager, size, x, y, layout);
  G4VModel* eorModel = new G4CallbackModel<G4VisCommandSceneAddEventID::EventID>(eorEventID);
  ...
  if (successfulEoE && successfulEoR)
  {
    ...
    {
      const G4String& currentSceneName = pScene->GetName();
      G4cout << "EventID has been added to scene \"" << currentSceneName << "\"." << G4endl;
    }
  }
  else
    G4VisCommandsSceneAddUnsuccessful(verbosity);
...
void G4VisCommandSceneAddExtent::SetNewValue(G4UIcommand*, G4String newValue)
{
  ...
  G4VModel* model = new G4CallbackModel<G4VisCommandSceneAddExtent::Extent>(extent);
  ...
  if (successful)
  {
    if (verbosity >= G4VisManager::confirmations)
    {
      G4cout << "A benign model with extent " << visExtent << " has been added to scene \""
             << currentSceneName << "\"." << G4endl;
    }
  }
  else
...
G4VisCommandSceneAddExtent::Extent::Extent(G4double xmin, G4double xmax, G4double ymin,
                                           G4double ymax, G4double zmin, G4double zmax)
  : fExtent(xmin, xmax, ymin, ymax, zmin, zmax)
{}

void G4VisCommandSceneAddExtent::Extent::operator()(G4VGraphicsScene&, const G4ModelingParameters*)
{}
...
void G4VisCommandSceneAddFrame::SetNewValue(G4UIcommand*, G4String newValue)
{
  ...
  G4VModel* model = new G4CallbackModel<G4VisCommandSceneAddFrame::Frame>(frame);
  model->SetType("Frame");
  ...
  if (successful)
  {
    if (verbosity >= G4VisManager::confirmations)
    {
      G4cout << "Frame has been added to scene \"" << currentSceneName << "\"." << G4endl;
    }
  }
  else
    G4VisCommandsSceneAddUnsuccessful(verbosity);
  ...
  if (successful)
  {
    if (verbosity >= G4VisManager::confirmations)
    {
      G4cout << "A representation of the source(s) of the General Particle Source will be drawn"
                "\n  in colour "
             << colour << " for scene \"" << currentSceneName << "\" if applicable." << G4endl;
    }
  }
  else
    G4VisCommandsSceneAddUnsuccessful(verbosity);
...
void G4VisCommandSceneAddHits::SetNewValue(G4UIcommand*, G4String)
{
  ...
  if (successful)
  {
    if (verbosity >= G4VisManager::confirmations)
    {
      G4cout << "Hits, if any, will be drawn at end of run in scene \"" << currentSceneName << "\"."
             << G4endl;
    }
  }
  else
...
void G4VisCommandSceneAddLine::SetNewValue(G4UIcommand*, G4String newValue)
{
  ...
  Line* line = new Line(x1, y1, z1, x2, y2, z2, fCurrentLineWidth, fCurrentColour);
  G4VModel* model = new G4CallbackModel<G4VisCommandSceneAddLine::Line>(line);
  model->SetType("Line");
  ...
  if (successful)
  {
    if (verbosity >= G4VisManager::confirmations)
    {
      G4cout << "Line has been added to scene \"" << currentSceneName << "\"." << G4endl;
    }
  }
  else
    G4VisCommandsSceneAddUnsuccessful(verbosity);
...
G4VisCommandSceneAddLine::Line::Line(G4double x1, G4double y1, G4double z1, G4double x2,
                                     G4double y2, G4double z2, G4double width,
...
void G4VisCommandSceneAddLine2D::SetNewValue(G4UIcommand*, G4String newValue)
{
  ...
  Line2D* line2D = new Line2D(x1, y1, x2, y2, fCurrentLineWidth, fCurrentColour);
  G4VModel* model = new G4CallbackModel<G4VisCommandSceneAddLine2D::Line2D>(line2D);
  ...
  if (successful)
  {
    if (verbosity >= G4VisManager::confirmations)
    {
      G4cout << "A 2D line has been added to scene \"" << currentSceneName << "\"." << G4endl;
    }
  }
  else
    G4VisCommandsSceneAddUnsuccessful(verbosity);
...
G4VisCommandSceneAddLine2D::Line2D::Line2D(G4double x1, G4double y1, G4double x2, G4double y2,
                                           G4double width, const G4Colour& colour)
  : fWidth(width), fColour(colour)
{
...
G4VisCommandSceneAddLocalAxes::G4VisCommandSceneAddLocalAxes()
{
  ...
  fpCommand->SetGuidance("Adds local axes to physical volume(s).");
  G4UIparameter* parameter;
...
void G4VisCommandSceneAddLocalAxes::SetNewValue(G4UIcommand*, G4String newValue)
{
  G4VisManager::Verbosity verbosity = fpVisManager->GetVerbosity();
  G4bool warn = verbosity >= G4VisManager::warnings;
  ...
  for (size_t i = 0; i < nWorlds; ++i, ++iterWorld)
  {
    ...
    G4PhysicalVolumeModel searchModel(
      *iterWorld, G4PhysicalVolumeModel::UNLIMITED, G4Transform3D(), &mp,
  ...
  for (const auto& findings : findingsVector)
  {
    ...
    std::ostringstream oss;
    oss << "Local Axes for " << findings.fpFoundPV->GetName() << ':' << findings.fFoundPVCopyNo
        << ':' << id++;
    ...
    if (successful)
    {
      if (verbosity >= G4VisManager::confirmations)
      {
        G4cout << "\"" << findings.fpFoundPV->GetName() << "\", copy no. "
               << findings.fFoundPVCopyNo << ",\n  found in searched volume \""
               << findings.fpSearchPV->GetName() << "\" at depth " << findings.fFoundDepth
               << ",\n  base path: \"" << findings.fFoundBasePVPath
               << "\".\n  Local axes have been added to scene \"" << pScene->GetName() << "\".";
...
void G4VisCommandSceneAddLogicalVolume::SetNewValue(G4UIcommand*, G4String newValue)
{
  G4VisManager::Verbosity verbosity = fpVisManager->GetVerbosity();
  G4bool warn(verbosity >= G4VisManager::warnings);
  ...
  std::istringstream is(newValue);
  is >> name >> requestedDepthOfDescent >> booleansString >> voxelsString >> readoutString
    >> axesString >> overlapString;
  ...
  if (i != rdModelList.end())
  {
    if (verbosity >= G4VisManager::errors)
    {
      G4warn << "There is already a volume, \"" << i->fpModel->GetGlobalDescription()
  ...
  if (successful)
  {
    ...
    if (axes)
    {
      ...
      if (5. * axisLength < axisLengthMax)
        axisLength *= 5.;
      else if (2. * axisLength < axisLengthMax)
        axisLength *= 2.;
    ...
    if (verbosity >= G4VisManager::confirmations)
    {
      G4cout << "Logical volume \"" << pLV->GetName() << "\" with requested depth of descent "
...
G4VisCommandSceneAddLogo::G4VisCommandSceneAddLogo()
{
  ...
  fpCommand->SetGuidance("If \"unit\" is \"auto\", height is roughly one tenth of scene extent.");
  fpCommand->SetGuidance(
...
void G4VisCommandSceneAddLogo::SetNewValue(G4UIcommand*, G4String newValue)
{
  ...
  if (!pViewer)
  {
    if (verbosity >= G4VisManager::errors)
    {
      G4warn << "ERROR: G4VisCommandSceneAddLogo::SetNewValue: no viewer."
  ...
  std::istringstream is(newValue);
  is >> userHeight >> userHeightUnit >> direction >> red >> green >> blue >> placement >> xmid
    >> ymid >> zmid >> positionUnit;
  ...
  if (direction == "auto")
  {
    ...
    const G4Vector3D& vp = pViewer->GetViewParameters().GetViewpointDirection();
    if (vp.x() > vp.y() && vp.x() > vp.z())
      logoDirection = X;
    else if (vp.x() < vp.y() && vp.x() < vp.z())
      logoDirection = minusX;
    else if (vp.y() > vp.x() && vp.y() > vp.z())
      logoDirection = Y;
    else if (vp.y() < vp.x() && vp.y() < vp.z())
      logoDirection = minusY;
    else if (vp.z() > vp.x() && vp.z() > vp.y())
      logoDirection = Z;
    else if (vp.z() < vp.x() && vp.z() < vp.y())
      logoDirection = minusZ;
  }
  ...
  else
  {
    if (verbosity >= G4VisManager::errors)
    {
      G4warn << "ERROR: Unrecogniseed direction: \"" << direction << "\"." << G4endl;
      return;
    }
  }

  G4bool autoPlacing = false;
  if (placement == "auto") autoPlacing = true;
  ...
  if (sceneExtent.GetExtentRadius() == 0)
  {
    ...
    if (verbosity >= G4VisManager::warnings)
    {
      G4warn << "WARNING: Existing scene does not yet have any extent."
  ...
  if (!room)
  {
    ...
    if (verbosity >= G4VisManager::warnings)
    {
      G4warn << "WARNING: Not enough room in existing scene.  Maybe logo is too large." << G4endl;
    }
  }
  if (worried)
  {
    if (verbosity >= G4VisManager::warnings)
    {
      G4warn << "WARNING: The logo you have asked for is bigger than the existing"
  ...
  G4VModel* model = new G4CallbackModel<G4VisCommandSceneAddLogo::G4Logo>(logo);
  ...
  if (successful)
  {
    if (verbosity >= G4VisManager::confirmations)
    {
      G4cout << "G4 Logo of height " << userHeight << ' ' << userHeightUnit << ", " << direction
             << "-direction, added to scene \"" << currentSceneName << "\"";
      if (verbosity >= G4VisManager::parameters)
      {
        G4cout << "\n  with extent " << extent << "\n  at " << transform.getRotation() << "  "
               << transform.getTranslation();
...
G4VisCommandSceneAddLogo::G4Logo::G4Logo(G4double height, const G4VisAttributes& visAtts,
                                         const G4Transform3D& transform)
{
  ...
  G4SubtractionSolid s4("s4", &s3, &bS, G4Transform3D(rm, G4ThreeVector(x8, y8, 0.)));
...
void G4VisCommandSceneAddLogo2D::SetNewValue(G4UIcommand*, G4String newValue)
{
  ...
  G4VModel* model = new G4CallbackModel<G4VisCommandSceneAddLogo2D::Logo2D>(logo2D);
  ...
  if (successful)
  {
    if (verbosity >= G4VisManager::confirmations)
    {
      G4cout << "2D logo has been added to scene \"" << currentSceneName << "\"." << G4endl;
    }
  }
  else
    G4VisCommandsSceneAddUnsuccessful(verbosity);
...
G4VisCommandSceneAddMagneticField::G4VisCommandSceneAddMagneticField()
{
  ...
  fpCommand->SetGuidance("Adds magnetic field representation to current scene.");
...
void G4VisCommandSceneAddMagneticField::SetNewValue(G4UIcommand*, G4String newValue)
{
  G4VisManager::Verbosity verbosity = fpVisManager->GetVerbosity();
  G4bool warn(verbosity >= G4VisManager::warnings);
  ...
  G4VModel* model;
  model = new G4MagneticFieldModel(nDataPointsPerHalfScene, modelRepresentation,
                                   fCurrentArrow3DLineSegmentsPerCircle, fCurrentExtentForField,
                                   fCurrrentPVFindingsForField);
  const G4String& currentSceneName = pScene->GetName();
  G4bool successful = pScene->AddRunDurationModel(model, warn);
  if (successful)
  {
    if (verbosity >= G4VisManager::confirmations)
    {
      G4cout << "Magnetic field, if any, will be drawn in scene \"" << currentSceneName
             << "\"\n  with " << nDataPointsPerHalfScene
             << " data points per half extent and with representation \"" << representation << '\"'
             << G4endl;
    }
  }
...
G4VisCommandSceneAddPSHits::G4VisCommandSceneAddPSHits()
{
  ...
  fpCommand->SetGuidance("Adds Primitive Scorer Hits (PSHits) to current scene.");
  fpCommand->SetGuidance(
...
void G4VisCommandSceneAddPSHits::SetNewValue(G4UIcommand*, G4String newValue)
{
  G4VisManager::Verbosity verbosity = fpVisManager->GetVerbosity();
  G4bool warn(verbosity >= G4VisManager::warnings);
  ...
  if (successful)
  {
    if (verbosity >= G4VisManager::confirmations)
    {
      ...
      G4cout << " will be drawn at end of run in scene \"" << currentSceneName << "\"." << G4endl;
    }
  }
  else
    G4VisCommandsSceneAddUnsuccessful(verbosity);
...
G4VisCommandSceneAddScale::G4VisCommandSceneAddScale()
{
  ...
  fpCommand->SetGuidance("Adds an annotated scale line to the current scene.");
  fpCommand->SetGuidance(
...
void G4VisCommandSceneAddScale::SetNewValue(G4UIcommand*, G4String newValue)
{
  ...
  std::istringstream is(newValue);
  is >> userLength >> userLengthUnit >> direction >> red >> green >> blue >> placement >> xmid
    >> ymid >> zmid >> positionUnit;
  ...
  if (!pViewer)
  {
    if (verbosity >= G4VisManager::errors)
    {
      G4warn << "ERROR: G4VisCommandSceneAddScale::SetNewValue: no viewer."
                "\n  Auto direction needs a viewer."
             << G4endl;
    }
    return;
  ...
  const G4Vector3D& vp = pViewer->GetViewParameters().GetViewpointDirection();
  const G4Vector3D& up = pViewer->GetViewParameters().GetUpVector();

  if (direction == "auto")
  {  // Takes cue from viewer.
    if (std::abs(vp.x()) > std::abs(vp.y()) && std::abs(vp.x()) > std::abs(vp.z()))
    {  // x viewpoint
      if (std::abs(up.y()) > std::abs(up.z()))
        scaleDirection = Scale::z;
      else
        scaleDirection = Scale::y;
    }
    else if (std::abs(vp.y()) > std::abs(vp.x()) && std::abs(vp.y()) > std::abs(vp.z()))
    {  // y viewpoint
      if (std::abs(up.x()) > std::abs(up.z()))
        scaleDirection = Scale::z;
      else
        scaleDirection = Scale::x;
    }
    else if (std::abs(vp.z()) > std::abs(vp.x()) && std::abs(vp.z()) > std::abs(vp.y()))
    {  // z viewpoint
  ...
  if (!room)
  {
    ...
    if (verbosity >= G4VisManager::warnings)
    {
      G4warn << "WARNING: Not enough room in existing scene.  Maybe scale is too long." << G4endl;
    }
  }
  if (worried)
  {
    if (verbosity >= G4VisManager::warnings)
    {
      G4warn << "WARNING: The scale you have asked for is bigger than the existing"
                "\n  scene.  Maybe you have added it too soon.  It is recommended that"
  ...
  Scale* scale = new Scale(visAttr, length, transform, annotation, fCurrentTextSize, colour);
  ...
  if (successful)
  {
    if (verbosity >= G4VisManager::confirmations)
    {
      G4cout << "Scale of " << annotation << " added to scene \"" << currentSceneName << "\".";
      if (verbosity >= G4VisManager::parameters)
      {
...
G4VisCommandSceneAddScale::Scale::Scale(const G4VisAttributes& visAtts, G4double length,
                                        const G4Transform3D& transform, const G4String& annotation,
                                        G4double annotationSize, const G4Colour& annotationColour)
  : fVisAtts(visAtts)
{
  // Useful constants...
  const G4double halfLength(length / 2.);
...
G4VisCommandSceneAddText::G4VisCommandSceneAddText()
{
  ...
  fpCommand->SetGuidance("Use \"/vis/set/textColour\" to set colour.");
  fpCommand->SetGuidance("Use \"/vis/set/textLayout\" to set layout:");
  G4UIparameter* parameter;
...
void G4VisCommandSceneAddText::SetNewValue(G4UIcommand*, G4String newValue)
{
  ...
  if (successful)
  {
    if (verbosity >= G4VisManager::confirmations)
    {
      G4cout << "Text '" << text << "' has been added to scene \"" << currentSceneName << "\"."
             << G4endl;
    }
  }
  else
...
void G4VisCommandSceneAddText2D::SetNewValue(G4UIcommand*, G4String newValue)
{
  ...
  G4VModel* model = new G4CallbackModel<G4VisCommandSceneAddText2D::G4Text2D>(g4text2D);
  model->SetType("Text2D");
  model->SetGlobalTag("Text2D");
  std::ostringstream oss;
  oss << "Text2D: '" << g4text.GetText() << "' at " << g4text.GetPosition().x() << ','
      << g4text.GetPosition().y() << " with size " << g4text.GetScreenSize() << " with offsets "
      << g4text.GetXOffset() << ',' << g4text.GetYOffset();
  ...
  if (successful)
  {
    if (verbosity >= G4VisManager::confirmations)
    {
      G4cout << "2D text '" << text << "' has been added to scene \"" << currentSceneName << "\"."
             << G4endl;
    }
  }
  else
...
G4VisCommandSceneAddText2D::G4Text2D::G4Text2D(const G4Text& text) : fText(text) {}
...
{
  G4bool omitable;
  fpCommand = new G4UIcmdWithAString("/vis/scene/add/trajectories", this);
  fpCommand->SetGuidance("Adds trajectories to current scene.");
  fpCommand->SetGuidance(
...
void G4VisCommandSceneAddTrajectories::SetNewValue(G4UIcommand*, G4String newValue)
{
  G4VisManager::Verbosity verbosity = fpVisManager->GetVerbosity();
  G4bool warn = verbosity >= G4VisManager::warnings;
  ...
  if (verbosity >= G4VisManager::errors)
  {
    G4warn << "Attributes available for modeling and filtering with"
    ...
    if (rich)
    {
      G4warn << *G4RichTrajectory().GetAttDefs() << *G4RichTrajectoryPoint().GetAttDefs();
    }
    else if (smooth)
    {
      G4warn << *G4SmoothTrajectory().GetAttDefs() << *G4SmoothTrajectoryPoint().GetAttDefs();
    }
    else
    {
      G4warn << *G4Trajectory().GetAttDefs() << *G4TrajectoryPoint().GetAttDefs();
    }
  }
  ...
  if (verbosity >= G4VisManager::confirmations)
  {
    G4cout << "Default trajectory type " << defaultTrajectoryType
           << "\n  will be used to store trajectories for scene \"" << currentSceneName << "\"."
           << G4endl;
  }

  if (verbosity >= G4VisManager::warnings)
  {
    G4warn << "WARNING: Trajectory storing has been requested.  This action may be"
...
G4VisCommandSceneAddUserAction::G4VisCommandSceneAddUserAction()
{
  ...
  fpCommand->SetGuidance("Add named Vis User Action to current scene.");
  fpCommand->SetGuidance(
    "Attempts to match search string to name of action - use unique sub-string.");
  fpCommand->SetGuidance("(Use /vis/list to see names of registered actions.)");
  fpCommand->SetGuidance("If name == \"all\" (default), all actions are added.");
...
void G4VisCommandSceneAddUserAction::SetNewValue(G4UIcommand*, G4String newValue)
{
  G4VisManager::Verbosity verbosity = fpVisManager->GetVerbosity();
...
void G4VisCommandSceneAddUserAction::AddVisAction(const G4String& name, G4VUserVisAction* visAction,
                                                  G4Scene* pScene,
                                                  ...
                                                  G4VisManager::Verbosity verbosity)
{
  ...
  G4VisExtent extent;
  std::map<G4VUserVisAction*, G4VisExtent>::const_iterator i = visExtentMap.find(visAction);
  if (i != visExtentMap.end()) extent = i->second;
  if (warn)
  {
    if (extent.GetExtentRadius() <= 0.)
    {
      G4warn << "WARNING: User Vis Action \"" << name << "\" extent is null." << G4endl;
    }
  }
  ...
  if (successful)
  {
    ...
    {
      const G4String& currentSceneName = pScene->GetName();
      G4cout << "User Vis Action added to scene \"" << currentSceneName << "\"";
      if (verbosity >= G4VisManager::parameters)
      {
        G4cout << "\n  with extent " << extent;
      }
...
G4VisCommandSceneAddVolume::G4VisCommandSceneAddVolume()
{
  ...
  fpCommand->SetGuidance("Adds a physical volume to current scene, with optional clipping volume.");
  fpCommand->SetGuidance(
  ...
  fpCommand->SetParameter(parameter);
  parameter = new G4UIparameter("depth-of-descent", 'i', omitable = true);
  parameter->SetGuidance("Depth of descent of geometry hierarchy. Default = unlimited depth.");
...
void G4VisCommandSceneAddVolume::SetNewValue(G4UIcommand*, G4String newValue)
{
  G4VisManager::Verbosity verbosity = fpVisManager->GetVerbosity();
  G4bool warn = verbosity >= G4VisManager::warnings;
  ...
  std::istringstream is(newValue);
  is >> name >> copyNo >> requestedDepthOfDescent >> clipVolumeType >> parameterUnit >> param1
  ...
  if (clipVolumeType == "box")
  {
    ...
    clippingSolid = new G4DisplacedSolid(
      "_displaced_clipping_box", new G4Box("_clipping_box", dX, dY, dZ), G4Translate3D(x0, y0, z0));
  }
  ...
  size_t nWorlds = transportationManager->GetNoWorlds();
  if (nWorlds > 1)
  {  // Parallel worlds in operation...
    if (verbosity >= G4VisManager::warnings)
    {
      static G4bool warned = false;
      if (!warned && name != "worlds")
      {
        G4warn << "WARNING: Parallel worlds in operation.  To visualise, specify"
        ...
        for (size_t i = 0; i < nWorlds; ++i, ++iterWorld)
        {
          G4warn << "  World " << i << ": " << (*iterWorld)->GetName() << G4endl;
          warned = true;
        }
      }
    }
  ...
  if (!world)
  {
    if (verbosity >= G4VisManager::errors)
    {
      G4warn << "ERROR: G4VisCommandSceneAddVolume::SetNewValue:"
  ...
  if (name == "world")
  {
    findingsVector.push_back(G4PhysicalVolumesSearchScene::Findings(world, world));
  }
  else if (name == "worlds")
  {
    if (nWorlds <= 1)
    {
      if (verbosity >= G4VisManager::warnings)
      {
        G4warn << "WARNING: G4VisCommandSceneAddVolume::SetNewValue:"
    ...
    for (size_t i = 0; i < nWorlds; ++i, ++iterWorld)
    {
      findingsVector.push_back(G4PhysicalVolumesSearchScene::Findings(*iterWorld, *iterWorld));
    }
  }
  else
  {  // Search all worlds...
    ...
    {
      G4ModelingParameters mp;  // Default - no culling.
      G4PhysicalVolumeModel searchModel(*iterWorld, requestedDepthOfDescent, G4Transform3D(), &mp,
                                        useFullExtent);
      G4PhysicalVolumesSearchScene searchScene(&searchModel, name, copyNo);
      searchModel.DescribeYourselfTo(searchScene);  // Initiate search.
      for (const auto& findings : searchScene.GetFindings())
  ...
  for (const auto& findings : findingsVector)
  {
    ...
    G4PhysicalVolumeModel* foundPVModel = new G4PhysicalVolumeModel(
      findings.fpFoundPV, requestedDepthOfDescent, findings.fFoundObjectTransformation,
      0,  // No modelling parameters (these are set later by the scene handler).
      useFullExtent, findings.fFoundBasePVPath);
    if (clippingSolid)
    {
    ...
    if (successful)
    {
      if (verbosity >= G4VisManager::confirmations)
      {
        ...
        G4cout << ",\n  has been added to scene \"" << pScene->GetName() << "\"." << G4endl;
      }
    }
    else
    {
...
G4String G4VisCommandSceneAddPlotter::GetCurrentValue(G4UIcommand*)
{
  return "";
}
...
void G4VisCommandSceneAddPlotter::SetNewValue(G4UIcommand*, G4String newValue)
{
  ...
  if (successful)
  {
    if (verbosity >= G4VisManager::confirmations)
    {
      G4cout << "Plotter \"" << model->GetCurrentDescription() << "\" has been added to scene \""
             << currentSceneName << "\"." << G4endl;
    }
  }
  else
