...
void G4VisCommandAbortReviewKeptEvents::SetNewValue(G4UIcommand*, G4String newValue)
{
...
void G4VisCommandAbortReviewPlots::SetNewValue(G4UIcommand*, G4String newValue)
{
...
void G4VisCommandDrawOnlyToBeKeptEvents::SetNewValue(G4UIcommand*, G4String newValue)
{
...
void G4VisCommandEnable::SetNewValue(G4UIcommand* command, G4String newValue)
{
  if (command == fpCommand)
  {
    G4bool enable = G4UIcommand::ConvertToBool(newValue);
    if (enable)
      fpVisManager->Enable();  // Printing is in vis manager.
    else
      fpVisManager->Disable();  // Printing is in vis manager.
  }
  else
    fpVisManager->Disable();  // Printing is in vis manager.
...
void G4VisCommandInitialize::SetNewValue(G4UIcommand*, G4String)
{
...
void G4VisCommandList::SetNewValue(G4UIcommand*, G4String newValue)
{
  ...
  G4cout << *G4RichTrajectory().GetAttDefs() << *G4RichTrajectoryPoint().GetAttDefs();
  G4cout << *G4SmoothTrajectory().GetAttDefs() << *G4SmoothTrajectoryPoint().GetAttDefs();
  G4cout << *G4Trajectory().GetAttDefs() << *G4TrajectoryPoint().GetAttDefs();
...
G4VisCommandReviewKeptEvents::G4VisCommandReviewKeptEvents()
{
  ...
  fpCommand->SetGuidance("If a macro file is specified, it is executed for each event.");
  fpCommand->SetGuidance(
...
void G4VisCommandReviewKeptEvents::SetNewValue(G4UIcommand*, G4String newValue)
{
  ...
  {
    G4warn << "\"/vis/reviewKeptEvents\" not allowed within an already started review."
              "\n  No action taken."
           << G4endl;
    return;
  }
  ...
  const std::vector<const G4Event*>* events = run ? run->GetEventVector() : nullptr;
  ...
  if (!nKeptEvents)
  {
    ...
    {
      G4warn << "ERROR: G4VisCommandReviewKeptEvents::SetNewValue: No kept events,"
  ...
  G4int newVerbose(0);
  if (keepControlVerbose >= 2 || verbosity >= G4VisManager::confirmations) newVerbose = 2;
    ...
    for (size_t i = 0; i < nKeptEvents; ++i)
    {
      ...
      if (verbosity >= G4VisManager::warnings)
      {
        G4warn << "Drawing event : " << event->GetEventID()
               << ".  At EndOfEvent, enter any command, then \"cont[inue]\"..." << G4endl;
        static G4bool first = true;
        if (first)
        {
          first = false;
          G4warn << "  Useful commands might be:"
  ...
  else
  {
    ...
    for (size_t i = 0; i < nKeptEvents; ++i)
    {
      ...
      if (verbosity >= G4VisManager::warnings)
      {
        G4warn << "Drawing event : " << event->GetEventID() << " with macro file \""
               << macroFileName << G4endl;
      }
      fpVisManager->SetRequestedEvent(event);
...
G4bool ReviewPlots(const G4String& plotType)
{  // h1, etc.
...
void G4VisCommandReviewPlots::SetNewValue(G4UIcommand*, G4String)
{
  ...
  {
    G4warn << "\"/vis/reviewPlots\" not allowed within an already started review."
              "\n  No action taken."
           << G4endl;
    return;
  }
...
void G4VisCommandVerbose::SetNewValue(G4UIcommand*, G4String newValue)
{
  G4VisManager::Verbosity verbosity = fpVisManager->GetVerbosityValue(newValue);
  fpVisManager->SetVerboseLevel(verbosity);
  // Always prints whatever the verbosity...
  G4cout << "Visualization verbosity is now " << G4VisManager::VerbosityString(verbosity) << G4endl;
}
