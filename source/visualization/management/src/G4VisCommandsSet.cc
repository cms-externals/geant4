...
void G4VisCommandSetArrow3DLineSegmentsPerCircle::SetNewValue(G4UIcommand*, G4String newValue)
{
  ...
  {
    G4cout << "Number of line segments per circle for drawing 3D arrows for future"
              "\n  \"/vis/scene/add/\" commands has been set to "
           << fCurrentArrow3DLineSegmentsPerCircle << G4endl;
  }
}
...
G4VisCommandSetColour::G4VisCommandSetColour()
{
  ...
  fpCommand->SetGuidance("Defines colour and opacity for future \"/vis/scene/add/\" commands.");
  fpCommand->SetGuidance(
...
G4VisCommandSetExtentForField::G4VisCommandSetExtentForField()
{
  ...
  fpCommand->SetGuidance("Sets an extent for future \"/vis/scene/add/*Field\" commands.");
  fpCommand->SetGuidance(
...
void G4VisCommandSetExtentForField::SetNewValue(G4UIcommand*, G4String newValue)
{
  ...
  if (verbosity >= G4VisManager::confirmations)
  {
    G4cout << "Extent for future \"/vis/scene/add/*Field\" commands has been set to "
           << fCurrentExtentForField << "\nVolume for field has been cleared." << G4endl;
  }
}
...
void G4VisCommandSetLineWidth::SetNewValue(G4UIcommand*, G4String newValue)
{
  ...
  {
    G4warn << "Line width for *future* \"/vis/scene/add/\" commands has been set to "
           << fCurrentLineWidth << "\nSee \"/vis/viewer/set/lineWidth\" for more information."
           << G4endl;
  }
}
...
G4VisCommandSetTextColour::G4VisCommandSetTextColour()
{
  ...
  fpCommand->SetGuidance("Defines colour and opacity for future \"/vis/scene/add/text\" commands.");
  fpCommand->SetGuidance(ConvertToColourGuidance());
...
void G4VisCommandSetTextColour::SetNewValue(G4UIcommand*, G4String newValue)
{
  ...
  if (verbosity >= G4VisManager::confirmations)
  {
    G4cout << "Colour for future \"/vis/scene/add/text\" commands has been set to "
           << fCurrentTextColour << '.' << G4endl;
  }
}
...
G4VisCommandSetTextLayout::G4VisCommandSetTextLayout()
{
  ...
  fpCommand->SetGuidance("Defines layout future \"/vis/scene/add/text\" commands.");
  fpCommand->SetGuidance("\"left\" (default) for left justification to provided coordinate.");
  fpCommand->SetGuidance("\"centre\" or \"center\" for text centered on provided coordinate.");
  fpCommand->SetGuidance("\"right\" for right justification to provided coordinate.");
...
void G4VisCommandSetTextLayout::SetNewValue(G4UIcommand*, G4String newValue)
{
  ...
  if (verbosity >= G4VisManager::confirmations)
  {
    G4cout << "Text layout (for future \"text\" commands) has been set to \"" << fCurrentTextLayout
           << "\"." << G4endl;
  }
}
...
G4VisCommandSetTextSize::G4VisCommandSetTextSize()
{
  ...
  fpCommand->SetGuidance("Defines text size (pixels) for future \"/vis/scene/add/\" commands.");
...
void G4VisCommandSetTextSize::SetNewValue(G4UIcommand*, G4String newValue)
{
  ...
  if (verbosity >= G4VisManager::confirmations)
  {
    G4cout << "Text size for future \"/vis/scene/add/\" commands has been set to "
           << fCurrentTextSize << G4endl;
  }
}
...
G4VisCommandSetTouchable::G4VisCommandSetTouchable()
{
  ...
  fpCommand->SetGuidance("Defines touchable for future \"/vis/touchable/set/\" commands.");
  fpCommand->SetGuidance(
...
void G4VisCommandSetTouchable::SetNewValue(G4UIcommand*, G4String newValue)
{
  ...
  if (newValue.empty())
  {
    ...
    if (verbosity >= G4VisManager::confirmations)
    {
      G4cout << "Current touchable reset to: " << fCurrentTouchableProperties.fTouchablePath
             << G4endl;
    }
    return;
  }
  ...
  while (iBegin != G4String::npos)
  {
    ...
    if (iBegin == G4String::npos)
    {
      if (verbosity >= G4VisManager::warnings)
      {
        G4warn << "WARNING: G4VisCommandSetTouchable::SetNewValue"
    ...
    currentTouchablePath.push_back(G4ModelingParameters::PVNameCopyNo(name, copyNo));
    iBegin = newValue.find_first_not_of(' ', iEnd);
  }
  ...
  if (successful)
  {
    if (verbosity >= G4VisManager::confirmations)
    {
      G4cout << "Current touchable: " << fCurrentTouchableProperties.fTouchablePath << G4endl;
      return;
    }
  }
  else
...
G4VisCommandSetVolumeForField::G4VisCommandSetVolumeForField()
{
  ...
  fpCommand->SetGuidance("Sets a volume for \"/vis/scene/add/*Field\" commands.");
  fpCommand->SetGuidance(
  ...
  if (verbosity >= G4VisManager::confirmations)
  {
    for (const auto& findings : fCurrrentPVFindingsForField)
    {
      G4cout << "\"" << findings.fpFoundPV->GetName() << "\", copy no. " << findings.fFoundPVCopyNo
             << ", found\nin searched volume \"" << findings.fpSearchPV->GetName() << "\" at depth "
             << findings.fFoundDepth << ",\nbase path: \"" << findings.fFoundBasePVPath
             << "\",\nand has been set as volume for field." << G4endl;
    }
  }
}
