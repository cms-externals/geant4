...
std::list<G4SceneTreeItem>::iterator G4VViewer::SceneTreeScene::FindOrInsertTouchable
 (const G4String& modelID,G4SceneTreeItem& mother,
  const G4String& partialPathString,const G4String& fullPathString)
{
  ...
  if (childIter != children.end()) {  // Item already exists
    if (childIter->GetType() == G4SceneTreeItem::ghost) {
      // Previously it was a ghost - but maybe this time it's real
      if (partialPathString == fullPathString) {  // This time its real
        ...
        childIter->SetAttDefs(pPVModel->GetAttDefs());
        childIter->SetAttValues(pPVModel->CreateCurrentAttValues());
      }  // Not real - do nothing
    } else {  // Pre-existing as a full touchable
      if (partialPathString == fullPathString) {
        // It's real - replace vis attributes (if any)
        if (fpVisAttributes) childIter->SetVisAttributes(*fpVisAttributes);
      }  // Not real - do nothing
    }
  } else {  // Item does not yet exist
    if (partialPathString == fullPathString) {  // Insert new touchable item
      ...
      touchable.SetAttDefs(pPVModel->GetAttDefs());
      touchable.SetAttValues(pPVModel->CreateCurrentAttValues());
      childIter = mother.InsertChild(childIter,touchable);
    } else {  // Not full path - must be a ghost
...
void G4VViewer::SceneTreeScene::ProcessVolume(const G4VSolid&)
{
  ...
  if (pPVModel) {  // G4PhysicalVolumeModel
    ...
    for (const auto& nodeID: nodeIDs) {
      ...
      (modelID,*currentIter,partialPathString,fullPathString);
    }
  } else {
