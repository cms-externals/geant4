...
fViewpointVector (G4ThreeVector(0.,0.,1.))
{
  ...
  fpCommandNoOfCloudPoints = new G4UIcmdWithAnInteger
  ("/vis/viewer/set/numberOfCloudPoints",this);
  fpCommandNoOfCloudPoints->SetGuidance
  ("Set number of points to be used for cloud representation of volumes.");
  fpCommandNoOfCloudPoints->SetParameterName("line-segments",omitable = true);
  fpCommandNoOfCloudPoints->SetDefaultValue(24);
...
G4VisCommandsViewerSet::~G4VisCommandsViewerSet() {
  ...
  delete fpCommandNoOfCloudPoints;
...
(G4UIcommand* command,G4String newValue) {
  ...
  else if (command == fpCommandNoOfCloudPoints) {
    G4int nPoints = G4UIcommand::ConvertToInt(newValue);
    nPoints = vp.SetNoOfCloudPoints(nPoints);
    if (verbosity >= G4VisManager::confirmations) {
      G4cout <<
