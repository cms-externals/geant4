...
void G4VSceneHandler::PseudoSceneFor3DRectMeshPositions::AddSolid(const G4Box&) {
  if (fpPVModel->GetCurrentDepth() == 3) {  // Leaf-level cells only
    const auto& material = fpPVModel->GetCurrentLV()->GetMaterial();
    const auto& name = material->GetName();
    const auto& visAtts = fpPVModel->GetCurrentLV()->GetVisAttributes();
    fpPVModel->GetModelingParameters()->GetDefaultVisAttributes()->GetColour();
    ...
    if (fNameAndVisAttsByMaterial.find(material) == fNameAndVisAttsByMaterial.end())
      ...
      fNameAndVisAttsByMaterial[material] = NameAndVisAtts(name,visAtts);
  }
}

void G4VSceneHandler::PseudoSceneForTetVertices::AddSolid(const G4VSolid& solid) {
  if (fpPVModel->GetCurrentDepth() == 1) {  // Leaf-level cells only
    ...
    try {
      ...
      if (fpCurrentObjectTransformation->xx() == 1. && // No transformation necessary
...
// appropriate or implement its own special mesh rendereing.
{
  ...
  if (!implemented) {
    G4VSceneHandler::AddCompound(mesh);  // Base class function - just prints warning
  }
  return;
}
...
// one dot randomly placed in each visible mesh cell.
{
  ...
  if (mesh.GetMeshType() != G4Mesh::rectangle ||
      mesh.GetMeshDepth() != 3) {
    G4ExceptionDescription ed;
    ed << "Called with a mesh that is either not 3-D or not rectangular:" << mesh;
  ...
  if (dotsByMaterial.empty()) {
    ...
    (&tmpPVModel,positionByMaterial,nameAndVisAttsByMaterial);
    ...
    for (const auto& entry: nameAndVisAttsByMaterial) {
      ...
      const auto& visAtts = nameAndVisAtts.fpVisAtts;
      G4Polymarker dots;
      dots.SetInfo(name);
...
// one dot randomly placed in each visible mesh cell.
{
  ...
  if (dotsByMaterial.empty()) {
    ...
    (&tmpPVModel,verticesByMaterial,nameAndVisAttsByMaterial);
    // Make private descent into the parameterisation
    tmpPVModel.DescribeYourselfTo(pseudoScene);
