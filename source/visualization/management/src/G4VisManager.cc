...
// All other objects use default constructors.
{
  ...
  if (fpInstance)
  {
    G4Exception("G4VisManager::G4VisManager", "visman0001", FatalException,
  ...
  {
    G4cout << "Visualization Manager instantiating with verbosity \"" << VerbosityString(fVerbosity)
           << "\"..." << G4endl;
  }
...
G4VisManager* G4VisManager::GetInstance()
{
  if (!fpInstance)
  {
    G4Exception("G4VisManager::GetInstance", "visman0002", FatalException,
                "VisManager not yet instantiated");
  }
  return fpInstance;
}
...
void G4VisManager::Initialise()
{
  ...
  {
    G4warn << "WARNING: G4VisManager::Initialise: already initialised." << G4endl;
    return;
  }
  ...
  if (fVerbosity >= startup)
  {
    G4cout << "\nYou have successfully registered the following graphics systems." << G4endl;
  ...
  if (fVerbosity >= startup)
  {
    G4cout << "\nYou have successfully registered the following model factories." << G4endl;
  ...
  if (fVerbosity >= startup)
  {
    G4cout << "Some /vis commands (optionally) take a string to specify colour."
...
void G4VisManager::RegisterMessengers()
{
  ...
  directory->SetGuidance("Set quantities for use in future commands where appropriate.");
  fDirectoryList.push_back(directory);
...
PlotResults ResultsOfHnPlots(const G4String& plotType)
{  // h1, etc.
...
G4bool PrintListOfPlots()
{
  ...
  if (numberOfPlots > 0)
  {
    ...
    G4warn << "\n  List them with \"/analysis/list\"."
           << "\n  View them immediately with \"/vis/plot\" or \"/vis/reviewPlots\"." << G4endl;
    if (numberOfEntries == 0)
    {
      G4warn << "  But...there are no entries. To make your histograms available for"
...
G4bool PrintNumberOfKeptEvents()
{
  ...
  G4warn << "There " << isare << ' ' << nKeptEvents << " kept event" << plural << '.' << G4endl;
  if (nKeptEvents > 0)
  {
    G4warn << "  \"/vis/reviewKeptEvents\" to review one by one."
              "\n  To see accumulated, \"/vis/viewer/flush\" or \"/vis/viewer/rebuild\"."
           << G4endl;
    return true;
  }
  else
...
void G4VisManager::Enable()
{
  ...
  else
  {
    if (fVerbosity >= warnings)
    {
      G4warn << "G4VisManager::Enable: WARNING: visualization remains disabled for"
...
void G4VisManager::Disable()
{
  ...
  if (fVerbosity >= confirmations)
  {
    G4cout << "G4VisManager::Disable: visualization disabled."
  ...
  if (fVerbosity >= warnings)
  {
    ...
    {
      G4warn << "You may wish to disable trajectory production too:"
                "\n  \"/tracking/storeTrajectory 0\""
                "\nbut don't forget to re-enable with"
                "\n  \"/vis/enable\""
                "\n  \"/tracking/storeTrajectory "
             << currentTrajectoryType << "\"\n  and maybe \"/vis/viewer/rebuild\"" << G4endl;
    }
  }
}
...
G4bool G4VisManager::RegisterGraphicsSystem(G4VGraphicsSystem* pSystem)
{
  ...
  else
  {
    ...
    {
      G4warn << "G4VisManager::RegisterGraphicsSystem: null pointer!" << G4endl;
    }
    happy = false;
  }
  return happy;
...
const G4VTrajectoryModel* G4VisManager::CurrentTrajDrawModel() const
{
  ...
  if (0 == model)
  {
    ...
    if (fVerbosity >= warnings)
    {
      G4warn << "G4VisManager: Using G4TrajectoryDrawByCharge as fallback trajectory model."
             << G4endl;
...
void G4VisManager::BeginDraw(const G4Transform3D& objectTransform)
{
  ...
  {
    G4Exception("G4VisManager::BeginDraw", "visman0008", JustWarning,
...
void G4VisManager::BeginDraw2D(const G4Transform3D& objectTransform)
{
  ...
  if (fDrawGroupNestingDepth > 1)
  {
    G4Exception("G4VisManager::BeginDraw2D", "visman0009", JustWarning,
...
template<class T>
void G4VisManager::DrawT(const T& graphics_primitive, const G4Transform3D& objectTransform)
{
  if (G4Threading::IsWorkerThread()) return;
  ...
  if (fIsDrawGroup)
  {
    ...
    {
      G4Exception("G4VSceneHandler::DrawT", "visman0010", FatalException,
...
template<class T>
void G4VisManager::DrawT2D(const T& graphics_primitive, const G4Transform3D& objectTransform)
{
  if (G4Threading::IsWorkerThread()) return;
  ...
  if (fIsDrawGroup)
  {
    if (objectTransform != fpSceneHandler->GetObjectTransformation())
    {
      G4Exception("G4VSceneHandler::DrawT", "visman0011", FatalException,
...
void G4VisManager::Draw(const G4Circle& circle, const G4Transform3D& objectTransform)
{
  DrawT(circle, objectTransform);
}

void G4VisManager::Draw(const G4Polyhedron& polyhedron, const G4Transform3D& objectTransform)
{
  DrawT(polyhedron, objectTransform);
}

void G4VisManager::Draw(const G4Polyline& line, const G4Transform3D& objectTransform)
{
  DrawT(line, objectTransform);
}

void G4VisManager::Draw(const G4Polymarker& polymarker, const G4Transform3D& objectTransform)
{
  DrawT(polymarker, objectTransform);
}

void G4VisManager::Draw(const G4Square& square, const G4Transform3D& objectTransform)
{
  DrawT(square, objectTransform);
}

void G4VisManager::Draw(const G4Text& text, const G4Transform3D& objectTransform)
{
  DrawT(text, objectTransform);
}

void G4VisManager::Draw2D(const G4Circle& circle, const G4Transform3D& objectTransform)
{
  DrawT2D(circle, objectTransform);
}

void G4VisManager::Draw2D(const G4Polyhedron& polyhedron, const G4Transform3D& objectTransform)
{
  DrawT2D(polyhedron, objectTransform);
}

void G4VisManager::Draw2D(const G4Polyline& line, const G4Transform3D& objectTransform)
{
  DrawT2D(line, objectTransform);
}

void G4VisManager::Draw2D(const G4Polymarker& polymarker, const G4Transform3D& objectTransform)
{
  DrawT2D(polymarker, objectTransform);
}

void G4VisManager::Draw2D(const G4Square& square, const G4Transform3D& objectTransform)
{
  DrawT2D(square, objectTransform);
}

void G4VisManager::Draw2D(const G4Text& text, const G4Transform3D& objectTransform)
{
...
void G4VisManager::Draw(const G4LogicalVolume& logicalVol, const G4VisAttributes& attribs,
...
void G4VisManager::Draw(const G4VSolid& solid, const G4VisAttributes& attribs,
                        const G4Transform3D& objectTransform)
{
  if (G4Threading::IsWorkerThread()) return;
...
void G4VisManager::Draw(const G4VPhysicalVolume& physicalVol, const G4VisAttributes& attribs,
                        const G4Transform3D& objectTransform)
{
  if (G4Threading::IsWorkerThread()) return;
...
void G4VisManager::DrawGeometry(G4VPhysicalVolume* v, const G4Transform3D& t)
...
void G4VisManager::CreateSceneHandler(const G4String& name)
{
  ...
  if (fpGraphicsSystem)
  {
    G4VSceneHandler* pSceneHandler = fpGraphicsSystem->CreateSceneHandler(name);
    if (pSceneHandler)
    {
    ...
    else
    {
      if (fVerbosity >= errors)
      {
        G4warn << "ERROR in G4VisManager::CreateSceneHandler during " << fpGraphicsSystem->GetName()
               << " scene handler creation.\n  No action taken." << G4endl;
      }
    }
  }
  else
...
void G4VisManager::CreateViewer(const G4String& name, const G4String& XGeometry)
{
  if (!fInitialised) Initialise();
  ...
  if (!p)
  {
    if (fVerbosity >= errors)
    {
      G4warn << "ERROR in G4VisManager::CreateViewer: null pointer during "
             << fpGraphicsSystem->GetName() << " viewer creation.\n  No action taken." << G4endl;
    }
    return;
  }
  ...
  if (p->GetViewId() < 0)
  {
    if (fVerbosity >= errors)
    {
      G4warn << "ERROR in G4VisManager::CreateViewer during " << fpGraphicsSystem->GetName()
  ...
  if (vp.IsCulling() && vp.IsCullingInvisible())
  {
    ...
    if (fVerbosity >= confirmations)
    {
      if (!warned)
      {
        G4cout << "NOTE: objects with visibility flag set to \"false\""
                  " will not be drawn!"
  ...
  if (vp.IsCullingCovered())
  {
    ...
    if (fVerbosity >= warnings)
    {
      if (!warned)
      {
        G4warn << "WARNING: covered objects in solid mode will not be rendered!"
...
void G4VisManager::GeometryHasChanged()
{
  ...
  for (iScene = 0; iScene < nScenes; ++iScene)
  {
    ...
    if (modelList.size())
    {
      G4bool modelInvalid;
      do
      {  // Remove, if required, one at a time.
        modelInvalid = false;
        std::vector<G4Scene::Model>::iterator iterModel;
        for (iterModel = modelList.begin(); iterModel != modelList.end(); ++iterModel)
        {
          ...
          if (modelInvalid)
          {
            ...
            if (fVerbosity >= warnings)
            {
              G4warn << "WARNING: Model \"" << iterModel->fpModel->GetGlobalDescription()
                     << "\" is no longer valid - being removed\n  from scene \""
                     << pScene->GetName() << "\". You may have to re-establish the scene."
                     << G4endl;
            }
      ...
      if (modelList.size() == 0)
      {
        if (fVerbosity >= warnings)
        {
          G4warn << "WARNING: No run-duration models left in this scene \"" << pScene->GetName()
                 << "\"." << G4endl;
        }
        if (pWorld)
        {
          if (fVerbosity >= warnings)
          {
            G4warn << "  Adding current world to \"" << pScene->GetName() << "\"." << G4endl;
          }
          ...
          G4UImanager::GetUIpointer()->ApplyCommand(
            G4String("/vis/scene/notifyHandlers " + pScene->GetName()));
        }
      }
      else
      {
  ...
  if (fpScene && fpScene->GetRunDurationModelList().size() == 0)
  {
    if (fVerbosity >= warnings)
    {
      G4warn << "WARNING: The current scene \"" << fpScene->GetName()
             << "\" has no run duration models."
             << "\n  Use \"/vis/scene/add/volume\" or create a new scene." << G4endl;
    }
    // Clean up
    if (fpSceneHandler)
    {
...
void G4VisManager::DispatchToModel(const G4VTrajectory& trajectory)
{
  ...
  if (!passed)
  {
    ...
    if (fpTrajFilterMgr->GetMode() == FilterMode::Soft)
      visible = false;
    else
    {
      return;
                                                    ...
                                                    const G4VisExtent& extent)
{
  ...
  else
  {
    if (fVerbosity >= warnings)
    {
      G4warn << "WARNING: No extent set for user vis action \"" << name << "\"." << G4endl;
    }
  }
  if (fVerbosity >= confirmations)
  {
    G4cout << "Run duration user vis action \"" << name << "\" registered" << G4endl;
  }
}
                                                   ...
                                                   const G4VisExtent& extent)
{
  ...
  if (fVerbosity >= confirmations)
  {
    G4cout << "End of event user vis action \"" << name << "\" registered" << G4endl;
  }
}

void G4VisManager::RegisterEndOfRunUserVisAction(const G4String& name, G4VUserVisAction* pVisAction,
                                                 const G4VisExtent& extent)
{
  ...
  if (fVerbosity >= confirmations)
  {
    G4cout << "End of run user vis action \"" << name << "\" registered" << G4endl;
  }
}
...
void G4VisManager::SetCurrentGraphicsSystem(G4VGraphicsSystem* pSystem)
{
  ...
  if (fVerbosity >= confirmations)
  {
    G4cout << "G4VisManager::SetCurrentGraphicsSystem: system now " << pSystem->GetName() << G4endl;
  }
  ...
  if (!(fpSceneHandler && fpSceneHandler->GetGraphicsSystem() == pSystem))
  {
    ...
    if (iSH >= 0)
    {
      ...
      if (fVerbosity >= confirmations)
      {
        G4cout << "  Scene Handler now " << fpSceneHandler->GetName() << G4endl;
      }
      if (fpScene != fpSceneHandler->GetScene())
      {
        ...
        if (fVerbosity >= confirmations)
        {
          G4cout << "  Scene now \"" << fpScene->GetName() << "\"" << G4endl;
        }
      }
...
void G4VisManager::SetCurrentSceneHandler(G4VSceneHandler* pSceneHandler)
{
  ...
  if (fpGraphicsSystem != pSceneHandler->GetGraphicsSystem())
  {
    ...
    if (fVerbosity >= confirmations)
    {
      G4cout << "  Graphics system now \"" << fpGraphicsSystem->GetName() << "\"" << G4endl;
    }
  }
  const G4ViewerList& viewerList = fpSceneHandler->GetViewerList();
  ...
  if (nViewers)
  {
    ...
    if (!IsValidView())
    {
      if (fVerbosity >= warnings)
      {
        G4warn << "WARNING: Problem setting scene handler - please report circumstances." << G4endl;
      }
    }
  }
  else
  {
    ...
    if (fVerbosity >= warnings)
    {
      G4warn << "WARNING: No viewers for this scene handler - please create one." << G4endl;
    }
  }
}
...
void G4VisManager::SetCurrentViewer(G4VViewer* pViewer)
{
  ...
  if (fpViewer == nullptr)
  {
    if (fVerbosity >= confirmations)
    {
      G4cout << "G4VisManager::SetCurrentViewer: current viewer pointer zeroed " << G4endl;
    }
    return;
  }
  if (fVerbosity >= confirmations)
  {
    G4cout << "G4VisManager::SetCurrentViewer: viewer now " << pViewer->GetName() << G4endl;
  }
  ...
  if (!fpSceneHandler)
  {
    if (fVerbosity >= warnings)
    {
      G4warn << "WARNING: No scene handler for this viewer - please create one." << G4endl;
    }
    return;
  }
  ...
  if (!IsValidView())
  {
    if (fVerbosity >= warnings)
    {
      G4warn << "WARNING: Problem setting viewer - please report circumstances." << G4endl;
    }
  }
}

void G4VisManager::PrintAvailableGraphicsSystems(Verbosity verbosity, std::ostream& out) const
{
  ...
  if (fAvailableGraphicsSystems.size())
  {
    ...
    out << "You may choose a graphics system (driver) with a parameter of"
...
{
  {
    ...
    const G4VisListManager<G4VTrajectoryModel>* listManager = fpTrajDrawModelMgr->ListManager();
    const std::map<G4String, G4VTrajectoryModel*>& modelMap = listManager->Map();
    if (modelMap.empty())
      G4cout << "  None" << G4endl;
    else
    {
...

  {
    ...
    const std::vector<G4VModelFactory<G4VFilter<G4VTrajectory>>*>& factoryList =
      fpTrajFilterMgr->FactoryList();
    if (factoryList.empty())
      G4cout << "  None" << G4endl;
    else
    {
    ...
    const std::vector<G4VFilter<G4VTrajectory>*>& filterList = fpTrajFilterMgr->FilterList();
    if (filterList.empty())
      G4cout << "  None" << G4endl;
    else
    {
...
{
  G4cout << "You have successfully registered the following user vis actions." << G4endl;
  G4cout << "Run Duration User Vis Actions:";
  if (fRunDurationUserVisActions.empty())
    G4cout << " none" << G4endl;
  else
  {
    G4cout << G4endl;
  ...
  if (fEndOfEventUserVisActions.empty())
    G4cout << " none" << G4endl;
  else
  {
    G4cout << G4endl;
  ...
  if (fEndOfRunUserVisActions.empty())
    G4cout << " none" << G4endl;
  else
  {
    G4cout << G4endl;
...
void G4VisManager::PrintInvalidPointers() const
{
  if (fVerbosity >= errors)
  {
    ...
    {
      G4warn << "\n  Graphics system is " << fpGraphicsSystem->GetName() << " but:";
      if (!fpScene)
...
G4ThreadFunReturnType G4VisManager::G4VisSubThread(G4ThreadFunArgType p)
{
  ...
  navigator->SetWorldVolume(G4RunManagerFactory::GetMasterRunManagerKernel()->GetCurrentWorld());
...
void G4VisManager::BeginOfRun()
{
  ...
  isSubEventRunManagerType =
    (rmType == G4RunManager::subEventMasterRM) || (rmType == G4RunManager::subEventWorkerRM);
  ...
  if (G4Threading::IsMultithreadedApplication())
  {
    ...
    while (fMaxEventQueueSize > 0 && (G4int)eventQueueSize >= fMaxEventQueueSize)
    {
      if (fWaitOnEventQueueFull)
      {
        ...
        if (!warned)
        {
          G4warn << "WARNING: The number of events in the visualisation queue has exceeded"
                    "\n  the maximum, "
                 << fMaxEventQueueSize
...
// i.e., kept by the run manager at least to the beginning of the next run.
{
  ...
  {  //  Accumulating events
    ...
    {  // Event keeping requested
      ...
      else
      {
        ...
        if (!warned)
        {
          if (fVerbosity >= warnings)
          {
            G4warn << "WARNING: G4VisManager::EndOfEvent: Automatic event keeping suspended."
                   << G4endl;
            if (maxNumberOfKeptEvents > 0)
            {
              G4warn << "\n  The number of events exceeds the maximum, " << maxNumberOfKeptEvents
                     << ", that may be kept by\n  the vis manager." << G4endl;
            }
          }
          warned = true;
        }
    ...
    {  // Indefinite event keeping (maxNumberOfKeptEvents < 0)
...
void G4VisManager::EndOfRun()
{
  ...
  if (G4Threading::IsWorkerThread()) return;
  if (isFakeRun) return;
  if (!GetConcreteInstance())
  {  // Print even if disabled
    if (fVerbosity >= warnings)
    {
      G4bool stuffToPrint = false;
      stuffToPrint |= PrintListOfPlots();
      stuffToPrint |= PrintNumberOfKeptEvents();
      if (stuffToPrint) G4warn << "You may need to issue \"/vis/enable\"." << G4endl;
    }
    return;
  }
  if (!isValidViewForRun) return;
  ...
  if (G4Threading::IsMultithreadedApplication())
  {
    ...
    if (fNoOfEventsDrawnThisRun != noOfEventsRequested)
    {
      ...
      {
        G4warn << "WARNING: Number of events drawn this run, " << fNoOfEventsDrawnThisRun
               << ", is different to number requested, " << noOfEventsRequested
  ...
  {
    G4warn << "WARNING: G4VisManager::EndOfRun: Automatic event keeping was suspended." << G4endl;
    if (fpScene->GetMaxNumberOfKeptEvents() > 0)
    {
      G4warn << "  The number of events in the run exceeded the maximum, "
             << fpScene->GetMaxNumberOfKeptEvents() << ", that may be\n  kept by the vis manager."
  ...
  {
    if (fpGraphicsSystem->GetFunctionality() == G4VGraphicsSystem::fileWriter)
    {
      if (fVerbosity >= warnings)
      {
...
void G4VisManager::ResetTransientsDrawnFlags()
{
  ...
  for (i = fAvailableSceneHandlers.begin(); i != fAvailableSceneHandlers.end(); ++i)
  {
...
{
  Verbosity verbosity;
  if (intVerbosity < quiet)
    verbosity = quiet;
  else if (intVerbosity > all)
    verbosity = all;
  else
    verbosity = Verbosity(intVerbosity);
  return verbosity;
}
...
G4bool G4VisManager::IsValidView()
{
  ...
  if (!fpGraphicsSystem)
  {
    ...
    if (noGSPrinting)
    {
      ...
      if (fVerbosity >= warnings)
      {
        G4warn << "WARNING: G4VisManager::IsValidView(): Attempt to draw when no graphics system"
  ...
  if ((!fpScene) || (!fpSceneHandler) || (!fpViewer))
  {
    if (fVerbosity >= errors)
    {
      G4warn << "ERROR: G4VisManager::IsValidView(): Current view is not valid." << G4endl;
      PrintInvalidPointers();
    }
    return false;
  }
  ...
  if (fpScene != fpSceneHandler->GetScene())
  {
    if (fVerbosity >= errors)
    {
      ...
      {
        G4warn << "\n  The current scene \"" << fpScene->GetName()
               << "\" is not handled by"
      ...
      {
        G4warn << "\n  Scene handler \"" << fpSceneHandler->GetName()
  ...
  if (viewerList.size() == 0)
  {
    if (fVerbosity >= errors)
    {
      G4warn << "ERROR: G4VisManager::IsValidView (): the current scene handler\n  \""
             << fpSceneHandler->GetName() << "\" has no viewers.  Do /vis/viewer/create." << G4endl;
    }
    return false;
  }
  ...
  {  // Add world by default if possible...
    ...
    else
    {
      ...
      if (fVerbosity >= warnings)
      {
        G4warn << "WARNING: G4VisManager: the scene was empty, \"world\" has been"
