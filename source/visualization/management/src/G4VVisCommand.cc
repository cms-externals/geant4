...
// Not yet used: G4VisAttributes::LineStyle G4VVisCommand::fCurrentLineStyle =
// G4VisAttributes::unbroken; Not yet used: G4VMarker::FillStyle G4VVisCommand::fCurrentFillStyle =
// G4VMarker::filled; Not yet used: G4VMarker::SizeType        G4VVisCommand::fCurrentSizeType =
// G4VMarker::screen;
...
G4String G4VVisCommand::ConvertToString(G4double x, G4double y, const char* unitName)
{
...
G4bool G4VVisCommand::ConvertToDoublePair(const G4String& paramString, G4double& xval,
                                          G4double& yval)
{
  G4double x, y;
  G4String unit;
...
void G4VVisCommand::ConvertToColour(G4Colour& colour, const G4String& redOrString, G4double green,
                                    G4double blue, G4double opacity)
{
  ...
  if (std::isalpha(redOrString[iPos0]))
  {
    ...
    if (!G4Colour::GetColour(redOrString, colour))
    {
      ...
      {
        G4warn << "WARNING: Colour \"" << redOrString << "\" not found.  Defaulting to " << colour
               << G4endl;
      }
      return;
    }
  ...
  else
  {
    ...
    if (iss.fail())
    {
      if (verbosity >= G4VisManager::warnings)
      {
        G4warn << "WARNING: String \"" << redOrString << "\" cannot be parsed.  Defaulting to "
               << colour << G4endl;
      }
      return;
    }
...
G4bool G4VVisCommand::ProvideValueOfUnit(const G4String& where, const G4String& unit,
...
void G4VVisCommand::CheckSceneAndNotifyHandlers(G4Scene* pScene)
{
  ...
  if (!pScene)
  {
    if (verbosity >= G4VisManager::warnings)
    {
      G4warn << "WARNING: Scene pointer is null." << G4endl;
    }
    return;
  }
...
void G4VVisCommand::G4VisCommandsSceneAddUnsuccessful(G4VisManager::Verbosity verbosity)
{
  ...
  if (verbosity >= G4VisManager::warnings)
  {
    G4warn << "WARNING: For some reason, possibly mentioned above, it has not been"
              "\n  possible to add to the scene."
           << G4endl;
  }
}

void G4VVisCommand::SetViewParameters(G4VViewer* viewer, const G4ViewParameters& viewParams)
{
                                     ...
                                     const G4String& exportString)
{
  ...
  do
  {
    ...
    if (exportString == "export" && G4StrUtil::contains(currentViewer->GetName(), "OpenGL"))
    {
...
void G4VVisCommand::InterpolateToNewView(G4VViewer* currentViewer, const G4ViewParameters& oldVP,
...
void G4VVisCommand::Twinkle
  ...
  (G4VViewer* currentViewer, const G4ViewParameters& baseVP,
   const std::vector<std::vector<G4PhysicalVolumeModel::G4PhysicalVolumeNodeID>>& paths)
{
  ...
  for (const auto& path : paths)
  {
    ...
    auto loVisAtts = *(
      currentViewer->GetApplicableVisAttributes(touchable->GetLogicalVolume()->GetVisAttributes()));
    ...
    auto pvNameCopyNoPath = G4PhysicalVolumeModel::GetPVNameCopyNoPath(path);
...
void G4VVisCommand::CopyGuidanceFrom(const G4UIcommand* fromCmd, G4UIcommand* toCmd,
                                     G4int startLine)
{
  if (fromCmd && toCmd)
  {
...
void G4VVisCommand::CopyParametersFrom(const G4UIcommand* fromCmd, G4UIcommand* toCmd)
{
  if (fromCmd && toCmd)
  {
...
void G4VVisCommand::CopyCameraParameters(G4ViewParameters& target, const G4ViewParameters& from)
{
