...
G4VisCommandViewerAddCutawayPlane::G4VisCommandViewerAddCutawayPlane()
{
  ...
  fpCommand->SetGuidance("Add cutaway plane to current viewer.");
...
void G4VisCommandViewerAddCutawayPlane::SetNewValue(G4UIcommand*, G4String newValue)
{
  ...
  if (!viewer)
  {
    ...
    {
      G4warn << "ERROR: No current viewer - \"/vis/viewer/list\" to see possibilities." << G4endl;
    }
    return;
  }
...
G4VisCommandViewerCentreOn::G4VisCommandViewerCentreOn()
{
  ...
  fpCommandCentreAndZoomInOn->SetGuidance("Centre and zoom in on the given physical volume.");
...
void G4VisCommandViewerCentreOn::SetNewValue(G4UIcommand* command, G4String newValue)
{
  ...
  for (const auto& findings : findingsVector)
  {
    ...
    {
      G4cout << "\"" << findings.fpFoundPV->GetName() << "\", copy no. " << findings.fFoundPVCopyNo
             << ",\n  found in searched volume \"" << findings.fpSearchPV->GetName()
             << "\" at depth " << findings.fFoundDepth << ",\n  base path: \""
             << findings.fFoundBasePVPath << ",\n  has been added to temporary scene \""
             << tempScene.GetName() << "\"." << G4endl;
    }
  ...
  if (verbosity >= G4VisManager::confirmations)
  {
    G4cout << "Viewer \"" << currentViewer->GetName() << "\" centred ";
    if (fpCommandCentreAndZoomInOn)
    {
      G4cout << "and zoomed in";
    }
    G4cout << " on physical volume(s) \"" << pvName << '\"' << G4endl;
  }
...
void G4VisCommandViewerChangeCutawayPlane::SetNewValue(G4UIcommand*, G4String newValue)
{
  ...
  G4ViewParameters vp = viewer->GetViewParameters();
  // Make sure normal is normalised.
  vp.ChangeCutawayPlane(index, G4Plane3D(G4Normal3D(nx, ny, nz).unit(), G4Point3D(x, y, z)));
  if (verbosity >= G4VisManager::confirmations)
  {
    G4cout << "Cutaway planes for viewer \"" << viewer->GetName() << "\" now:";
    const G4Planes& cutaways = vp.GetCutawayPlanes();
...
G4VisCommandViewerClear::G4VisCommandViewerClear()
{
  ...
  fpCommand->SetParameterName("viewer-name", omitable = true, currentAsDefault = true);
}
  ...
  if (!viewer)
  {
    if (verbosity >= G4VisManager::errors)
    {
      G4warn << "ERROR: Viewer \"" << clearName
             << "\" not found - \"/vis/viewer/list\" to see possibilities." << G4endl;
    }
    return;
  }
...
{
  fpCommand = new G4UIcmdWithoutParameter("/vis/viewer/clearCutawayPlanes", this);
...
void G4VisCommandViewerClearCutawayPlanes::SetNewValue(G4UIcommand*, G4String)
{
  ...
  if (verbosity >= G4VisManager::confirmations)
  {
    G4cout << "Cutaway planes for viewer \"" << viewer->GetName() << "\" now cleared." << G4endl;
  }
...
void G4VisCommandViewerClearTransients::SetNewValue(G4UIcommand*, G4String newValue)
{
  ...
  if (verbosity >= G4VisManager::confirmations)
  {
    G4cout << "Viewer \"" << clearName << "\" cleared of transients." << G4endl;
  }
}
...
{
  fpCommand = new G4UIcmdWithoutParameter("/vis/viewer/clearVisAttributesModifiers", this);
...
void G4VisCommandViewerClearVisAttributesModifiers::SetNewValue(G4UIcommand*, G4String)
{
  ...
  if (verbosity >= G4VisManager::confirmations)
  {
    G4cout << "Vis attributes modifiers for viewer \"" << viewer->GetName() << "\" now cleared."
           << G4endl;
  }
...
void G4VisCommandViewerClone::SetNewValue(G4UIcommand*, G4String newValue)
{
  ...
  if (cloneName == "none")
  {
    ...
    do
    {
      ...
      if ((lastDashPosition = cloneName.rfind('-')) != G4String::npos
          && (nextSpacePosition = cloneName.find(" ", lastDashPosition)) != G4String::npos)
      {
  ...
  if (errorWhileNaming)
  {
    if (verbosity >= G4VisManager::errors)
    {
      G4warn << "ERROR: While naming clone viewer \"" << cloneName << "\"." << G4endl;
    }
    return;
  }
  ...
  if (fpVisManager->GetViewer(cloneName))
  {
    if (verbosity >= G4VisManager::errors)
    {
      G4warn << "ERROR: Putative clone viewer \"" << cloneName << "\" already exists." << G4endl;
    }
    return;
  }

  G4String windowSizeHint = originalViewer->GetViewParameters().GetXGeometryString();

  G4UImanager* UImanager = G4UImanager::GetUIpointer();
  UImanager->ApplyCommand(G4String("/vis/viewer/select " + originalName));
  UImanager->ApplyCommand(G4String("/vis/viewer/create ! \"" + cloneName + "\" " + windowSizeHint));
...
G4VisCommandViewerColourByDensity::G4VisCommandViewerColourByDensity()
{
  ...
  fpCommand->SetGuidance("If a volume has no vis attributes, colour it by density.");
  fpCommand->SetGuidance(
...
void G4VisCommandViewerColourByDensity::SetNewValue(G4UIcommand*, G4String newValue)
{
  ...
  if (algorithmNumber < 0 || algorithmNumber > 1)
  {
    if (verbosity >= G4VisManager::errors)
    {
      G4warn << "ERROR: Unrecognised algorithm number: " << algorithmNumber << G4endl;
    }
    return;
  }
  ...
  if (algorithmNumber > 0)
  {
    ...
    else
    {
      if (verbosity >= G4VisManager::errors)
      {
        G4warn << "ERROR: Unrecognised or inappropriate unit: " << unit << G4endl;
      }
      return;
    }
...
G4VisCommandViewerCopyViewFrom::G4VisCommandViewerCopyViewFrom()
{
  ...
  fpCommand->SetGuidance("Copy the camera-specific parameters from the specified viewer.");
  fpCommand->SetGuidance(
...
void G4VisCommandViewerCopyViewFrom::SetNewValue(G4UIcommand*, G4String newValue)
{
  ...
  if (!currentViewer)
  {
    if (verbosity >= G4VisManager::errors)
    {
      G4warn << "ERROR: G4VisCommandsViewerCopyViewFrom::SetNewValue: no current viewer." << G4endl;
    }
    return;
  }
  ...
  if (fromViewer == currentViewer)
  {
    if (verbosity >= G4VisManager::warnings)
    {
      G4warn << "WARNING: G4VisCommandsViewerSet::SetNewValue:"
...
G4VisCommandViewerCreate::G4VisCommandViewerCreate() : fId(0)
{
  ...
  fpCommand->SetGuidance("Window size and placement hints, e.g. 600x600-100+100 (in pixels):");
  fpCommand->SetGuidance("- single number, e.g., \"600\": square window;");
  fpCommand->SetGuidance("- two numbers, e.g., \"800x600\": rectangluar window;");
  fpCommand->SetGuidance(
...
G4String G4VisCommandViewerCreate::GetCurrentValue(G4UIcommand*)
{
  ...
  {  // Get name of last scene handler
    sceneHandlerName = currentSceneHandler->GetName();
  }
  else
  {  // No scene handler - ensure a warning message
...
void G4VisCommandViewerCreate::SetNewValue(G4UIcommand* command, G4String newValue)
{
  ...
  if (nHandlers == 0)
  {
    ...
    ed << "ERROR: G4VisCommandViewerCreate::SetNewValue: no scene handlers."
  ...
  if (iHandler >= nHandlers)
  {
    ...
    G4ExceptionDescription ed;
    ed << "G4VisCommandViewerCreate::SetNewValue: invalid scene handler specified.";
    command->CommandFailed(ed);
    return;
  }
  ...
  for (std::size_t ih = 0; ih < nHandlers; ++ih)
  {
    ...
    for (const auto* iViewer : viewerList)
    {
      ...
      {
        G4ExceptionDescription ed;
        ed << "ERROR: Viewer \"" << newShortName << "\" already exists.";
        command->CommandFailed(ed);
        return;
      }
    }
  ...
  if (newViewer)
  {
    ...
    else
    {
      if (verbosity >= G4VisManager::warnings)
      {
        G4warn << "Issue /vis/viewer/refresh or flush to see effect." << G4endl;
      }
    }
  }
}
...
G4VisCommandViewerDolly::G4VisCommandViewerDolly() : fDollyIncrement(0.), fDollyTo(0.)
{
  G4bool omitable, currentAsDefault;

  fpCommandDolly = new G4UIcmdWithADoubleAndUnit("/vis/viewer/dolly", this);
  fpCommandDolly->SetGuidance("Incremental dolly.");
  fpCommandDolly->SetGuidance("Moves the camera incrementally towards target point.");
  fpCommandDolly->SetParameterName("increment", omitable = true, currentAsDefault = true);
  fpCommandDolly->SetDefaultUnit("m");

  fpCommandDollyTo = new G4UIcmdWithADoubleAndUnit("/vis/viewer/dollyTo", this);
  fpCommandDollyTo->SetGuidance("Dolly to specific coordinate.");
  fpCommandDollyTo->SetGuidance(
    "Places the camera towards target point relative to standard camera point.");
  fpCommandDollyTo->SetParameterName("distance", omitable = true, currentAsDefault = true);
...
void G4VisCommandViewerDolly::SetNewValue(G4UIcommand* command, G4String newValue)
{
  G4VisManager::Verbosity verbosity = fpVisManager->GetVerbosity();
  ...
  if (!currentViewer)
  {
    if (verbosity >= G4VisManager::errors)
    {
      G4warn << "ERROR: G4VisCommandsViewerDolly::SetNewValue: no current viewer." << G4endl;
    }
    return;
  }
...
G4VisCommandViewerFlush::G4VisCommandViewerFlush()
{
  ...
  fpCommand->SetGuidance("Compound command: \"/vis/viewer/refresh\" + \"/vis/viewer/update\".");
  fpCommand->SetGuidance(
...
G4String G4VisCommandViewerFlush::GetCurrentValue(G4UIcommand*)
{
  G4VViewer* viewer = fpVisManager->GetCurrentViewer();
  return viewer ? viewer->GetName() : G4String("none");
}
...
void G4VisCommandViewerInterpolate::SetNewValue(G4UIcommand*, G4String newValue)
{
  ...
  if (!currentViewer)
  {
    if (verbosity >= G4VisManager::errors)
    {
      G4warn << "ERROR: G4VisCommandViewerInterpolate::SetNewValue: no current viewer." << G4endl;
    }
    return;
  }
  ...
  iss >> pattern >> nInterpolationPoints >> waitTimePerPointString >> timeUnit >> exportString;
  ...
  G4String parentPathString(pathPattern.parent_path().string().length()
  ...
  InterpolateViews(currentViewer, viewVector, nInterpolationPoints, waitTimePerPointmilliseconds,
                   exportString);
...
G4VisCommandViewerList::G4VisCommandViewerList()
{
  ...
  fpCommand->SetGuidance("See \"/vis/verbose\" for definition of verbosity.");
  G4UIparameter* parameter;
  parameter = new G4UIparameter("viewer-name", 's', omitable = true);
...
void G4VisCommandViewerList::SetNewValue(G4UIcommand*, G4String newValue)
{
  ...
  G4VisManager::Verbosity verbosity = fpVisManager->GetVerbosityValue(verbosityString);
...
G4VisCommandViewerPan::G4VisCommandViewerPan()
  : fPanIncrementRight(0.), fPanIncrementUp(0.), fPanToRight(0.), fPanToUp(0.)
{
  G4bool omitable;

  fpCommandPan = new G4UIcommand("/vis/viewer/pan", this);
  fpCommandPan->SetGuidance("Incremental pan.");
  ...
  fpCommandPanTo = new G4UIcommand("/vis/viewer/panTo", this);
  fpCommandPanTo->SetGuidance("Pan to specific coordinate.");
...
void G4VisCommandViewerPan::SetNewValue(G4UIcommand* command, G4String newValue)
{
  G4VisManager::Verbosity verbosity = fpVisManager->GetVerbosity();
  ...
  if (!currentViewer)
  {
    if (verbosity >= G4VisManager::errors)
    {
      G4warn << "ERROR: G4VisCommandsViewerPan::SetNewValue: no current viewer." << G4endl;
    }
    return;
  }
  ...
  if (verbosity >= G4VisManager::confirmations)
  {
    G4cout << "Current target point now " << vp.GetCurrentTargetPoint() << G4endl;
  }
  ...
  if (!sceneHandler)
  {
    if (verbosity >= G4VisManager::errors)
    {
      G4warn << "ERROR: Viewer \"" << viewer->GetName() << "\""
             << " has no scene handler - report serious bug." << G4endl;
    }
    return;
  }
...
G4VisCommandViewerRefresh::G4VisCommandViewerRefresh()
{
  ...
  fpCommand->SetGuidance("Refreshes viewer.");
  fpCommand->SetGuidance(
    "By default, acts on current viewer.  \"/vis/viewer/list\""
    "\nto see possible viewers.  Viewer becomes current.");
  fpCommand->SetParameterName("viewer-name", omitable = true, currentAsDefault = true);
...
void G4VisCommandViewerRefresh::SetNewValue(G4UIcommand*, G4String newValue)
{
  ...
  if (!scene)
  {
    if (verbosity >= G4VisManager::confirmations)
    {
      G4cout << "NOTE: SceneHandler \"" << sceneHandler->GetName() << "\", to which viewer \""
             << refreshName << "\""
  ...
  if (scene->GetRunDurationModelList().empty())
  {
    ...
    if (!successful)
    {
      if (verbosity >= G4VisManager::warnings)
      {
        G4warn << "WARNING: Scene is empty.  Perhaps no geometry exists."
                  "\n  Try /run/initialize."
               << G4endl;
      }
      return;
  ...
  else
  {
    if (verbosity >= G4VisManager::confirmations)
    {
      G4cout << "Refreshing viewer \"" << viewer->GetName() << "\"..." << G4endl;
    }
    viewer->SetView();
    viewer->ClearView();
    viewer->DrawView();
...
G4VisCommandViewerSave::G4VisCommandViewerSave()
{
  ...
  fpCommand->SetGuidance("Write commands that define the current view to file.");
  fpCommand->SetGuidance("Read them back into the same or any viewer with \"/control/execute\".");
  fpCommand->SetGuidance(
    "If the filename is omitted the view is saved to a file "
    "\"g4_nn.g4view\", where nn is a sequential two-digit number.");
  fpCommand->SetGuidance("If the filename is \"-\", the data are written to G4cout.");
  fpCommand->SetGuidance(
...
G4String G4VisCommandViewerSave::GetCurrentValue(G4UIcommand*)
{
  return "";
}
...
namespace
{
void WriteCommands(std::ostream& os, const G4ViewParameters& vp,
                   const G4Point3D& stp)  // Standard Target Point
{
  os << vp.CameraAndLightingCommands(stp) << vp.DrawingStyleCommands()
     << vp.SceneModifyingCommands() << vp.TouchableCommands() << vp.TimeWindowCommands()
     << std::endl;
}
}  // namespace
...
void G4VisCommandViewerSave::SetNewValue(G4UIcommand*, G4String newValue)
{
  ...
  if (!currentViewer)
  {
    if (verbosity >= G4VisManager::errors)
    {
      G4warn << "ERROR: G4VisCommandsViewerSave::SetNewValue: no current viewer." << G4endl;
    }
    return;
  }
  ...
  if (!currentScene)
  {
    if (verbosity >= G4VisManager::errors)
    {
      G4warn << "ERROR: G4VisCommandsViewerSave::SetNewValue: no current scene." << G4endl;
    }
    return;
  }
  ...
  if (newValue.length() == 0)
  {
    ...
    if (sequenceNumber >= maxNoOfFiles)
    {
      if (verbosity >= G4VisManager::errors)
      {
        G4warn << "ERROR: G4VisCommandsViewerSave::SetNewValue: Maximum number, " << maxNoOfFiles
               << ", of files exceeded." << G4endl;
      }
      return;
    }
    std::ostringstream oss;
  ...
  else
  {
    ...
    if (!ofs)
    {
      if (verbosity >= G4VisManager::errors)
      {
        G4warn << "ERROR: G4VisCommandsViewerSave::SetNewValue: Trouble opening file \"" << filename
               << "\"." << G4endl;
      }
      ofs.close();
      return;
  ...
  if (verbosity >= G4VisManager::warnings)
  {
    G4warn << "Viewer \"" << currentViewer->GetName() << "\"" << " saved to ";
    if (filename == "-")
    {
      G4warn << "G4cout.";
    }
...
G4VisCommandViewerScale::G4VisCommandViewerScale()
  : fScaleMultiplier(G4Vector3D(1., 1., 1.)), fScaleTo(G4Vector3D(1., 1., 1.))
{
  G4bool omitable, currentAsDefault;

  fpCommandScale = new G4UIcmdWith3Vector("/vis/viewer/scale", this);
  ...
  fpCommandScale->SetGuidance("");
  fpCommandScale->SetParameterName("x-scale-multiplier", "y-scale-multiplier", "z-scale-multiplier",
                                   omitable = true, currentAsDefault = true);

  fpCommandScaleTo = new G4UIcmdWith3Vector("/vis/viewer/scaleTo", this);
  fpCommandScaleTo->SetGuidance("Absolute (non-uniform) scaling.");
  fpCommandScaleTo->SetGuidance("Scales (x,y,z) by corresponding components of this factor.");
  fpCommandScaleTo->SetParameterName("x-scale-factor", "y-scale-factor", "z-scale-factor",
                                     omitable = true, currentAsDefault = true);
}
...
void G4VisCommandViewerScale::SetNewValue(G4UIcommand* command, G4String newValue)
{
  G4VisManager::Verbosity verbosity = fpVisManager->GetVerbosity();
  ...
  if (!currentViewer)
  {
    if (verbosity >= G4VisManager::errors)
    {
      G4warn << "ERROR: G4VisCommandsViewerScale::SetNewValue: no current viewer." << G4endl;
    }
    return;
  }
...
G4VisCommandViewerSelect::G4VisCommandViewerSelect()
{
  ...
  fpCommand->SetGuidance("Specify viewer by name.  \"/vis/viewer/list\" to see possible viewers.");
...
void G4VisCommandViewerUpdate::SetNewValue(G4UIcommand*, G4String newValue)
{
  ...
  if (!viewer)
  {
    if (verbosity >= G4VisManager::errors)
    {
      G4warn << "WARNING: command \"/vis/viewer/update\" could not be applied: no current viewer."
             << G4endl;
    }
    return;
  }
...
G4VisCommandViewerZoom::G4VisCommandViewerZoom() : fZoomMultiplier(1.), fZoomTo(1.)
{
  G4bool omitable, currentAsDefault;

  fpCommandZoom = new G4UIcmdWithADouble("/vis/viewer/zoom", this);
  fpCommandZoom->SetGuidance("Incremental zoom.");
  fpCommandZoom->SetGuidance("Multiplies current magnification by this factor.");
  fpCommandZoom->SetParameterName("multiplier", omitable = true, currentAsDefault = true);

  fpCommandZoomTo = new G4UIcmdWithADouble("/vis/viewer/zoomTo", this);
  fpCommandZoomTo->SetGuidance("Absolute zoom.");
  fpCommandZoomTo->SetGuidance("Magnifies standard magnification by this factor.");
  fpCommandZoomTo->SetParameterName("factor", omitable = true, currentAsDefault = true);
}
...
void G4VisCommandViewerZoom::SetNewValue(G4UIcommand* command, G4String newValue)
{
  G4VisManager::Verbosity verbosity = fpVisManager->GetVerbosity();
  ...
  if (!currentViewer)
  {
    if (verbosity >= G4VisManager::errors)
    {
      G4warn << "ERROR: G4VisCommandsViewerZoom::SetNewValue: no current viewer." << G4endl;
    }
    return;
  }
