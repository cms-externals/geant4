...
G4VisCommandsTouchable::G4VisCommandsTouchable()
{
  ...
  fpCommandCentreAndZoomInOn =
    new G4UIcmdWithoutParameter("/vis/touchable/centreAndZoomInOn", this);
  fpCommandCentreAndZoomInOn->SetGuidance("Centre and zoom in on the current touchable.");
  fpCommandCentreAndZoomInOn->SetGuidance("Use \"/vis/set/touchable\" to set current touchable.");
  fpCommandCentreAndZoomInOn->SetGuidance("You may also need \"/vis/touchable/findPath\".");
  fpCommandCentreAndZoomInOn->SetGuidance("Use \"/vis/touchable/set\" to set attributes.");
  ...
  fpCommandDraw->SetGuidance("If parameter == true, also draw extent as a white wireframe box.");
  // Pick up additional guidance from /vis/viewer/centreAndZoomInOn
...
void G4VisCommandsTouchable::SetNewValue(G4UIcommand* command, G4String newValue)
{
  ...
  if (!world)
  {
    ...
    {
      G4warn << "ERROR: G4VisCommandsTouchable::SetNewValue:"
  ...
  if (command == fpCommandDump)
  {
    ...
    if (properties.fpTouchablePV)
    {
      ...
      G4PhysicalVolumeModel tempPVModel(
        properties.fpTouchablePV, G4PhysicalVolumeModel::UNLIMITED,
  ...
  else if (command == fpCommandFindPath)
  {
    ...
    {
      G4cout << findings.fFoundBasePVPath << ' ' << findings.fpFoundPV->GetName() << ' '
             << findings.fFoundPVCopyNo
             << " (mother logical volume: " << findings.fpFoundPV->GetMotherLogical()->GetName()
             << ')' << G4endl;
    }
    if (findingsVector.size())
    {
      G4cout << "Use this to set a particular touchable with \"/vis/set/touchable <path>\""
  ...
  if (!currentScene)
  {
    if (verbosity >= G4VisManager::errors)
    {
      G4warn << "ERROR: No current scene - \"/vis/scene/list\" to see possibilities." << G4endl;
    }
    return;
  }
  ...
  if (command == fpCommandCentreOn || command == fpCommandCentreAndZoomInOn)
  {
    ...
    if (properties.fpTouchablePV)
    {
      ...
      {
        G4cout << "Touchable " << fCurrentTouchableProperties.fTouchablePath
      ...
      if (verbosity >= G4VisManager::confirmations)
      {
        ...
        G4cout << " on touchable\n" << fCurrentTouchableProperties.fTouchablePath << G4endl;
      }
  ...
  else if (command == fpCommandDraw)
  {
    ...
    if (properties.fpTouchablePV)
    {
      ...
      if (successful)
      {
        ...
        if (verbosity >= G4VisManager::confirmations)
        {
          G4cout << "\"" << properties.fpTouchablePV->GetName() << "\", copy no. "
                 << properties.fCopyNo << " drawn";
          if (fpCommandDraw->GetNewBoolValue(newValue))
          {
  ...
  else if (command == fpCommandExtentForField)
  {
    ...
    if (properties.fpTouchablePV)
    {
      G4VisExtent extent = properties.fpTouchablePV->GetLogicalVolume()->GetSolid()->GetExtent();
  ...
  else if (command == fpCommandLocalAxes)
  {
    ...
    if (properties.fpTouchablePV)
    {
      ...
      const auto& extent =
        fCurrentTouchableProperties.fpTouchablePV->GetLogicalVolume()->GetSolid()->GetExtent();
  ...
  else if (command == fpCommandVolumeForField)
  {
    ...
    if (properties.fpTouchablePV)
    {
      ...
      fCurrentExtentForField = extent;
      fCurrrentPVFindingsForField.clear();
      fCurrrentPVFindingsForField.push_back(G4PhysicalVolumesSearchScene::Findings(properties));
      if (verbosity >= G4VisManager::confirmations)
      {
        G4cout << "Volume for field set to " << properties.fpTouchablePV->GetName() << ':'
               << properties.fCopyNo << " at " << properties.fTouchableBaseFullPVPath << G4endl;
      }
  ...
  else
  {
    if (verbosity >= G4VisManager::errors)
    {
      G4warn << "ERROR: G4VisCommandsTouchable::SetNewValue: unrecognised command." << G4endl;
    }
    return;
  }
}
