...
G4VisCommandSceneActivateModel::G4VisCommandSceneActivateModel()
{
  ...
  fpCommand->SetGuidance("Activate or de-activate model.");
  fpCommand->SetGuidance(
    "Attempts to match search string to name of model - use unique sub-string.");
  fpCommand->SetGuidance("Use \"/vis/scene/list\" to see model names.");
  fpCommand->SetGuidance("If name == \"all\" (default), all models are activated.");
...
void G4VisCommandSceneActivateModel::SetNewValue(G4UIcommand*, G4String newValue)
{
  ...
  if (lastQuotationMark != std::string::npos)
  {  // We have at least one quotation mark
    auto firstQuotationMark = newValue.find_first_of('"');
    if (lastQuotationMark != firstQuotationMark)
    {  // We have at least two quotations marks
      // Note: there must not be more quotation marks - it upsets the command system.
      searchString =
        newValue.substr(firstQuotationMark + 1, lastQuotationMark - firstQuotationMark - 1);
  ...
  if (searchString == "all" && !activate)
  {
    ...
    {
      G4warn << "WARNING: You are not allowed to de-activate all models."
                "\n  Command ignored."
             << G4endl;
    }
    return;
  ...
  std::vector<G4Scene::Model>& runDurationModelList = pScene->SetRunDurationModelList();
  ...
  std::vector<G4Scene::Model>& endOfEventModelList = pScene->SetEndOfEventModelList();
  ...
  std::vector<G4Scene::Model>& endOfRunModelList = pScene->SetEndOfRunModelList();
...
G4VisCommandSceneCreate::G4VisCommandSceneCreate() : fId(0)
{
  ...
  fpCommand->SetGuidance("Invents a name if not supplied.  This scene becomes current.");
...
G4VisCommandSceneEndOfEventAction::G4VisCommandSceneEndOfEventAction()
{
  ...
  fpCommand->SetGuidance("Accumulate or refresh the viewer for each new event.");
  fpCommand->SetGuidance("\"accumulate\": viewer accumulates hits, etc., event by event, or");
  fpCommand->SetGuidance(
  ...
  parameter->SetGuidance("Maximum number of events kept.  Unlimited if negative.");
  fpCommand->SetParameter(parameter);
}
...
void G4VisCommandSceneEndOfEventAction::SetNewValue(G4UIcommand*, G4String newValue)
{
  G4VisManager::Verbosity verbosity = fpVisManager->GetVerbosity();
  ...
  else if (action == "refresh")
  {
    if (!pScene->GetRefreshAtEndOfRun())
    {
      if (verbosity >= G4VisManager::errors)
      {
        G4warn << "ERROR: Cannot refresh events unless runs refresh too."
  ...
  else
  {
    if (verbosity >= G4VisManager::errors)
    {
      G4warn << "ERROR: unrecognised parameter \"" << action << "\"." << G4endl;
    }
    return;
  }
  ...
  if (!pScene->GetRefreshAtEndOfEvent() && maxNumberOfKeptEvents != 0
      && verbosity >= G4VisManager::warnings)
  {
    ...
    else
    {
      ...
      if (maxNumberOfKeptEvents < 0)
        G4warn << "an unlimited number of";
      else
        G4warn << "up to " << maxNumberOfKeptEvents;
      G4warn << " events.";
...
G4VisCommandSceneEndOfRunAction::G4VisCommandSceneEndOfRunAction()
{
  ...
  fpCommand->SetGuidance("Accumulate or refresh the viewer for each new run.");
  fpCommand->SetGuidance("\"accumulate\": viewer accumulates hits, etc., run by run, or");
  fpCommand->SetGuidance(
...
void G4VisCommandSceneEndOfRunAction::SetNewValue(G4UIcommand*, G4String newValue)
{
  G4VisManager::Verbosity verbosity = fpVisManager->GetVerbosity();
  ...
  if (action == "accumulate")
  {
    if (pScene->GetRefreshAtEndOfEvent())
    {
      if (verbosity >= G4VisManager::errors)
      {
        G4warn << "ERROR: Cannot accumulate runs unless events accumulate too."
...
G4VisCommandSceneList::G4VisCommandSceneList()
{
  ...
  fpCommand->SetGuidance("\"help /vis/verbose\" for definition of verbosity.");
  G4UIparameter* parameter;
...
G4VisCommandSceneNotifyHandlers::G4VisCommandSceneNotifyHandlers()
{
  ...
  fpCommand->SetGuidance("Notifies scene handlers and forces re-rendering.");
  fpCommand->SetGuidance(
  ...
  fpCommand->SetGuidance("The default for <scene-name> is the current scene name.");
  fpCommand->SetGuidance("This command does not change current scene, scene handler or viewer.");
  G4UIparameter* parameter;
  parameter = new G4UIparameter("scene-name", 's', omitable = true);
  parameter->SetCurrentAsDefault(true);
  fpCommand->SetParameter(parameter);
  parameter = new G4UIparameter("refresh-flush", 's', omitable = true);
  parameter->SetDefaultValue("refresh");
...
void G4VisCommandSceneNotifyHandlers::SetNewValue(G4UIcommand*, G4String newValue)
{
  G4VisManager::Verbosity verbosity = fpVisManager->GetVerbosity();
  ...
  G4SceneHandlerList& sceneHandlerList = fpVisManager->SetAvailableSceneHandlers();
  ...
  if (iScene >= nScenes)
  {
    if (verbosity >= G4VisManager::warnings)
    {
      G4warn << "WARNING: Scene \"" << sceneName
             << "\" not found."
  ...
  G4VSceneHandler* pCurrentSceneHandler = fpVisManager->GetCurrentSceneHandler();
  if (!pCurrentSceneHandler)
  {
    if (verbosity >= G4VisManager::warnings)
    {
      G4warn << "WARNING: No current scene handler." << G4endl;
    }
    return;
  }
  ...
  if (!pCurrentViewer)
  {
    if (verbosity >= G4VisManager::warnings)
    {
      G4warn << "WARNING: No current viewer." << G4endl;
    }
    return;
  }
  ...
  if (!pCurrentScene)
  {
    if (verbosity >= G4VisManager::warnings)
    {
      G4warn << "WARNING: No current scene." << G4endl;
    }
    return;
  }
      ...
      if (sceneName == aSceneName)
      {
        ...
        for (std::size_t iV = 0; iV < nViewers; ++iV)
        {
          ...
          if (aViewer->GetViewParameters().IsAutoRefresh())
          {
            ...
            if (verbosity >= G4VisManager::confirmations)
            {
              G4cout << "Viewer \"" << aViewer->GetName() << "\" of scene handler \""
                     << aSceneHandler->GetName() << "\"\n  ";
              if (flush)
                G4cout << "flushed";
              else
                G4cout << "refreshed";
              G4cout << " at request of scene \"" << sceneName << "\"." << G4endl;
            }
          }
          else
          {
            if (verbosity >= G4VisManager::confirmations)
            {
              G4cout << "NOTE: The scene, \"" << sceneName << "\", of viewer \""
                     << aViewer->GetName() << "\"\n  of scene handler \""
                     << aSceneHandler->GetName() << "\"  has changed.  To see effect,"
                     << "\n  \"/vis/viewer/select " << aViewer->GetShortName()
                     << "\" and \"/vis/viewer/rebuild\"." << G4endl;
            }
          }
        }
      }
    ...
    else
    {
      if (verbosity >= G4VisManager::warnings)
      {
        G4warn << "WARNING: G4VisCommandSceneNotifyHandlers: scene handler \""
               << aSceneHandler->GetName() << "\" has a null scene." << G4endl;
      }
    }
  }
...
void G4VisCommandSceneRemoveModel::SetNewValue(G4UIcommand*, G4String newValue)
{
  G4VisManager::Verbosity verbosity = fpVisManager->GetVerbosity();
...
void G4VisCommandSceneSelect::SetNewValue(G4UIcommand*, G4String newValue)
{
  ...
  if (iScene >= nScenes)
  {
    if (verbosity >= G4VisManager::warnings)
    {
      G4warn << "WARNING: Scene \"" << selectName
             << "\" not found - \"/vis/scene/list\" to see possibilities." << G4endl;
    }
    return;
  }
  ...
  if (verbosity >= G4VisManager::confirmations)
  {
    G4cout << "Scene \"" << selectName << "\" selected." << G4endl;
  }
