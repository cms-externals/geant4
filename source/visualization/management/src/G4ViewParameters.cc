...
G4double G4ViewParameters::GetNearDistance(G4double cameraDistance, G4double radius) const
{
...
G4double G4ViewParameters::GetFarDistance(G4double cameraDistance, G4double nearDistance,
...
G4double G4ViewParameters::GetFrontHalfHeight(G4double nearDistance, G4double radius) const
{
...
void G4ViewParameters::AddCutawayPlane(const G4Plane3D& cutawayPlane)
{
  ...
  {
    G4warn << "ERROR: G4ViewParameters::AddCutawayPlane:"
              "\n  A maximum of 3 cutaway planes supported."
           << G4endl;
  }
}

void G4ViewParameters::ChangeCutawayPlane(size_t index, const G4Plane3D& cutawayPlane)
{
  if (index >= fCutawayPlanes.size())
  {
    G4warn << "ERROR: G4ViewParameters::ChangeCutawayPlane:"
              "\n  Plane "
           << index << " does not exist." << G4endl;
  }
  else
  {
...
G4int G4ViewParameters::SetNumberOfCloudPoints(G4int nPoints)
{
  ...
  if (nPoints < nPointsMin)
  {
    ...
    G4warn << "G4ViewParameters::SetNumberOfCloudPoints:"
              "\nnumber of points per cloud set to minimum "
           << nPoints << G4endl;
  }
...
void G4ViewParameters::SetViewAndLights(const G4Vector3D& viewpointDirection)
{
  ...
  if (fViewpointDirection.unit() * fUpVector.unit() > .9999)
  {
    ...
    {
      firstTime = false;
      G4warn << "WARNING: Viewpoint direction is very close to the up vector direction."
  ...
  if (fLightsMoveWithCamera)
  {
    ...
    fActualLightpointDirection = fRelativeLightpointDirection.x() * xprime
  ...
  const G4ModelingParameters::VisAttributesModifier& vam)
{
  ...
  if (duplicateTarget)
    (*i).SetVisAttributes(vam.GetVisAttributes());
  else
...
G4String G4ViewParameters::CameraAndLightingCommands(const G4Point3D standardTargetPoint) const
{
  ...
  oss << "\n/vis/viewer/set/viewpointVector " << fViewpointDirection.x() << ' '
      << fViewpointDirection.y() << ' ' << fViewpointDirection.z();

  oss << "\n/vis/viewer/set/upVector " << fUpVector.x() << ' ' << fUpVector.y() << ' '
      << fUpVector.z();
  ...
  {
    oss << "perspective " << fFieldHalfAngle / deg << " deg";
  }

  oss << "\n/vis/viewer/zoomTo " << fZoomFactor;

  oss << "\n/vis/viewer/scaleTo " << fScaleFactor.x() << ' ' << fScaleFactor.y() << ' '
      << fScaleFactor.z();
  ...
  oss << "\n/vis/viewer/dollyTo " << G4BestUnit(fDolly, "Length");
  ...
  oss << "\n/vis/viewer/set/lightsVector " << fRelativeLightpointDirection.x() << ' '
      << fRelativeLightpointDirection.y() << ' ' << fRelativeLightpointDirection.z();
  ...
  oss << "\n/vis/viewer/set/background " << c.GetRed() << ' ' << c.GetGreen() << ' ' << c.GetBlue()
      << ' ' << c.GetAlpha();

  c = fDefaultVisAttributes.GetColour();
  oss << "\n/vis/viewer/set/defaultColour " << c.GetRed() << ' ' << c.GetGreen() << ' '
      << c.GetBlue() << ' ' << c.GetAlpha();

  c = fDefaultTextVisAttributes.GetColour();
  oss << "\n/vis/viewer/set/defaultTextColour " << c.GetRed() << ' ' << c.GetGreen() << ' '
      << c.GetBlue() << ' ' << c.GetAlpha();
...
G4String G4ViewParameters::DrawingStyleCommands() const
{
  ...
  oss << "\n/vis/viewer/set/globalLineWidthScale " << fGlobalLineWidthScale;

  oss << "\n/vis/viewer/set/globalMarkerScale " << fGlobalMarkerScale;

  oss << "\n/vis/viewer/set/numberOfCloudPoints " << fNumberOfCloudPoints;
  ...
  oss << "\n/vis/viewer/set/specialMeshRenderingOption " << fSpecialMeshRenderingOption;
...
G4String G4ViewParameters::SceneModifyingCommands() const
{
  ...
  oss << "\n/vis/viewer/colourByDensity " << fCBDAlgorithmNumber << " g/cm3";
  ...
  if (fSection)
  {
    oss << "on " << G4BestUnit(fSectionPlane.point(), "Length") << fSectionPlane.normal().x() << ' '
        << fSectionPlane.normal().y() << ' ' << fSectionPlane.normal().z();
  }
  else
  {
    oss << "off";
  ...
  if (fCutawayPlanes.size())
  {
    ...
    {
      oss << "\n/vis/viewer/addCutawayPlane " << G4BestUnit(fCutawayPlane.point(), "Length")
          << fCutawayPlane.normal().x() << ' ' << fCutawayPlane.normal().y() << ' '
  ...
  oss << "\n/vis/viewer/set/explodeFactor " << fExplodeFactor << ' '
      << G4BestUnit(fExplodeCentre, "Length");

  oss << "\n/vis/viewer/set/lineSegmentsPerCircle " << fNoOfSides;

  oss << "\n/vis/viewer/set/transparencyByDepth " << fTransparencyByDepth << ' '
...
G4String G4ViewParameters::TouchableCommands() const
{
  ...
  const std::vector<G4ModelingParameters::VisAttributesModifier>& vams = fVisAttributesModifiers;
  ...
  for (iModifier = vams.begin(); iModifier != vams.end(); ++iModifier)
  {
    ...
    if (vamPath != lastPath)
    {
      ...
      for (iVAM = vamPath.begin(); iVAM != vamPath.end(); ++iVAM)
      {
    ...
    switch (iModifier->GetVisAttributesSignifier())
    {
      ...
      case G4ModelingParameters::VASColour:
        oss << "\n/vis/touchable/set/colour " << c.GetRed() << ' ' << c.GetGreen() << ' '
            << c.GetBlue() << ' ' << c.GetAlpha();
        break;
      ...
      case G4ModelingParameters::VASLineWidth:
        oss << "\n/vis/touchable/set/lineWidth " << vamVisAtts.GetLineWidth();
        break;
...
G4String G4ViewParameters::TimeWindowCommands() const
{
  ...
  oss << "\n/vis/viewer/set/timeWindow/startTime " << fTimeParameters.fStartTime / ns << " ns ";

  oss << "\n/vis/viewer/set/timeWindow/endTime " << fTimeParameters.fEndTime / ns << " ns ";

  oss << "\n/vis/viewer/set/timeWindow/fadeFactor " << fTimeParameters.fFadeFactor;
  ...
  {
    oss << "true" << ' ' << fTimeParameters.fDisplayHeadTimeX << ' '
        << fTimeParameters.fDisplayHeadTimeY << ' ' << fTimeParameters.fDisplayHeadTimeSize << ' '
        << fTimeParameters.fDisplayHeadTimeRed << ' ' << fTimeParameters.fDisplayHeadTimeGreen
  ...
  {
    oss << "true" << ' ' << fTimeParameters.fDisplayLightFrontX / mm << ' '
        << fTimeParameters.fDisplayLightFrontY / mm << ' '
        << fTimeParameters.fDisplayLightFrontZ / mm << " mm" << ' '
        << fTimeParameters.fDisplayLightFrontT / ns << " ns" << ' '
        << fTimeParameters.fDisplayLightFrontRed << ' ' << fTimeParameters.fDisplayLightFrontGreen
...
void G4ViewParameters::PrintDifferences(const G4ViewParameters& v) const
{
  ...
  if (IsExplode())
  {
    if (fExplodeFactor != v.fExplodeFactor) G4cout << "Difference in explode factor." << G4endl;
    if (fExplodeCentre != v.fExplodeCentre) G4cout << "Difference in explode centre." << G4endl;
  }
...
std::ostream& operator<<(std::ostream& os, G4ViewParameters::DrawingStyle style)
{
  switch (style)
  {
    ...
    case G4ViewParameters::hlr:
      os << "hlr - hidden lines removed";
      break;
    case G4ViewParameters::hsr:
      os << "hsr - hidden surfaces removed";
      break;
    case G4ViewParameters::hlhsr:
      os << "hlhsr - hidden line, hidden surface removed";
      break;
    case G4ViewParameters::cloud:
      os << "cloud - draw volume as a cloud of dots";
      break;
    default:
      os << "unrecognised";
      break;
...
std::ostream& operator<<(std::ostream& os, const G4ViewParameters& v)
{
  ...
  {
    os << "on - invisible if density less than " << v.fVisibleDensity / (1. * g / cm3)
       << " g cm^-3";
  }
  else
    os << "off";
  ...
  os << "\n  Section flag: ";
  if (v.fSection)
    os << "true, section/cut plane: " << v.fSectionPlane;
  else
    os << "false";
  ...
  os << "\n  Explode factor: " << v.fExplodeFactor << " about centre: " << v.fExplodeCentre;

  os << "\n  No. of sides used in circle polygon approximation: " << v.fNoOfSides;
  ...
  os << "\n  Light ";
  if (v.fLightsMoveWithCamera)
    os << "moves";
  else
    os << "does not move";
  os << " with camera";

  os << "\n  Relative lightpoint direction: " << v.fRelativeLightpointDirection;

  os << "\n  Actual lightpoint direction: " << v.fActualLightpointDirection;
  ...
  const G4double nearDistance = tempVP.GetNearDistance(cameraDistance, radius);
  const G4double farDistance = tempVP.GetFarDistance(cameraDistance, nearDistance, radius);
  ...
  os << "\n  Window size hint: " << v.fWindowSizeHintX << 'x' << v.fWindowSizeHintX;

  os << "\n  X geometry string: " << v.fXGeometryString;
  os << "\n  X geometry mask: " << std::showbase << std::hex << v.fGeometryMask << std::noshowbase
     << std::dec;
  ...
  switch (v.fRotationStyle)
  {
    case G4ViewParameters::constrainUpDirection:
      os << "constrainUpDirection (conventional HEP view)";
      break;
    case G4ViewParameters::freeRotation:
      os << "freeRotation (Google-like rotation, using mouse-grab)";
      break;
    default:
      os << "unrecognised";
      break;
  ...
  const std::vector<G4ModelingParameters::VisAttributesModifier>& vams = v.fVisAttributesModifiers;
  if (vams.empty())
  {
    os << "None";
  }
  ...
  {
    os << "\n  Head time position: " << v.fTimeParameters.fDisplayHeadTimeX << ' '
       << v.fTimeParameters.fDisplayHeadTimeY
  ...
  {
    os << "\n  Light front position: " << v.fTimeParameters.fDisplayLightFrontX / mm << ' '
       << v.fTimeParameters.fDisplayLightFrontY / mm << ' '
       ...
       << v.fTimeParameters.fDisplayLightFrontGreen << ' '
       << v.fTimeParameters.fDisplayLightFrontBlue;
  }
...
G4bool G4ViewParameters::operator!=(const G4ViewParameters& v) const
{
  ...
  if (fDensityCulling && (fVisibleDensity != v.fVisibleDensity)) return true;
  ...
  if (fSection && (!(fSectionPlane == v.fSectionPlane))) return true;
  ...
  if (IsExplode() && ((fExplodeFactor != v.fExplodeFactor) || (fExplodeCentre != v.fExplodeCentre)))
    return true;
  ...
  if (fTimeParameters.fStartTime != v.fTimeParameters.fStartTime
      || fTimeParameters.fEndTime != v.fTimeParameters.fEndTime
      || fTimeParameters.fFadeFactor != v.fTimeParameters.fFadeFactor)
    return true;
  ...
  if (fSpecialMeshRendering)
  {
    if (fSpecialMeshVolumes != v.fSpecialMeshVolumes) return true;
    ;
  }
...
void G4ViewParameters::SetXGeometryString(const G4String& geomString)
{
  ...
  if (i == G4String::npos)
  {
    ...
    if (fWindowLocationHintYNegative)
      ...
      signY = '+';
    std::ostringstream oss;
    oss << fWindowSizeHintX << 'x' << fWindowSizeHintY << signX << fWindowLocationHintX << signY
  ...
  if (((fGeometryMask & fHeightValue) == 0) && ((fGeometryMask & fWidthValue) == 0))
  {
    h = fWindowSizeHintY;
    w = fWindowSizeHintX;
  }
  else if ((fGeometryMask & fHeightValue) == 0)
  {
    ...
    G4warn << "Unrecognised geometry string \"" << geomString
           << "\".  No Height found. Using Width value instead" << G4endl;
    h = w;
  }
  if (((fGeometryMask & fXValue) == 0) || ((fGeometryMask & fYValue) == 0))
  {
    // Using defaults
    x = fWindowLocationHintX;
    y = fWindowLocationHintY;
  ...
  if (((fGeometryMask & fXValue)) && ((fGeometryMask & fYValue)))
  {
...
int G4ViewParameters::ParseGeometry(const char* string, G4int* x, G4int* y, unsigned int* width,
                                    unsigned int* height)
{
  G4int mask = fNoValue;
  char* strind;
  ...
  if (mask & fXValue) *x = tempX;
  if (mask & fYValue) *y = tempY;
  if (mask & fWidthValue) *width = tempWidth;
  if (mask & fHeightValue) *height = tempHeight;
  return (mask);
}
  ...
  G4int nInterpolationPoints)  // No of interpolations points per interval
{
  ...
  if (views.size() < 2)
  {
    G4Exception("G4ViewParameters::CatmullRomCubicSplineInterpolation", "visman0301", JustWarning,
  ...
  {
    G4Exception("G4ViewParameters::CatmullRomCubicSplineInterpolation", "visman0302", JustWarning,
...
#define INTERPOLATELOG(param)                                           \
  ...
  {                                                                     \
    ...
    if (n == 1)                                                         \
      ...
      mi1 = 0.5 * (std::log(v[2].param) - std::log(v[0].param));        \
  }                                                                     \
  else if (i >= n - 1)                                                  \
  {                                                                     \
  ...
  real =                                                                \
    std::exp(h00 * std::log(v[i].param) + h10 * mi + h01 * std::log(v[i + 1].param) + h11 * mi1);
...
#define INTERPOLATEUNITVECTOR(vector) \
  INTERPOLATE(vector.x());            \
  x = real;                           \
  INTERPOLATE(vector.y());            \
  y = real;                           \
  INTERPOLATE(vector.z());            \
  z = real;
...
#define INTERPOLATECOLOUR(colour) \
  INTERPOLATE(colour.GetRed());   \
  red = real;                     \
  INTERPOLATE(colour.GetGreen()); \
  green = real;                   \
  INTERPOLATE(colour.GetBlue());  \
  blue = real;                    \
  INTERPOLATE(colour.GetAlpha()); \
  alpha = real;
...
#define CONTINUITY(quantity)                                                        \
  ...
  {                                                                                 \
    if (v[i - 1].quantity == v[i].quantity && v[i + 1].quantity == v[i].quantity    \
