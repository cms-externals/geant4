...
G4int G4ViewParameters::SetNoOfCloudPoints(G4int nPoints) {
  const G4int nPointsMin = 100;
  if (nPoints < nPointsMin) {
    nPoints = nPointsMin;
    G4cout << "G4ViewParameters::SetNoOfCloudPoints:"
    "\nnumber of points per cloud set to minimum " << nPoints
    << G4endl;
  }
  fNoOfCloudPoints = nPoints;
  return fNoOfCloudPoints;
}
...
void G4ViewParameters::PrintDifferences (const G4ViewParameters& v) const {
  ...
  if (
      ...
      (fNoOfCloudPoints      != v.fNoOfCloudPoints)
      )
...
std::ostream& operator << (std::ostream& os, const G4ViewParameters& v) {
  ...
  os << "\n  Number of cloud points: " << v.fNoOfCloudPoints;
