...
G4VisCommandSceneHandlerAttach::G4VisCommandSceneHandlerAttach()
{
  ...
  fpCommand->SetParameterName("scene-name", omitable = true, currentAsDefault = true);
}
...
void G4VisCommandSceneHandlerAttach::SetNewValue(G4UIcommand*, G4String newValue)
{
  ...
  if (sceneName.length() == 0)
  {
    ...
    {
      G4cout << "WARNING: No scene specified.  Maybe there are no scenes available"
  ...
  if (!pSceneHandler)
  {
    ...
    {
      G4warn << "ERROR: Current scene handler not defined.  Please select or create one." << G4endl;
    }
    return;
  }
  ...
  if (sceneList.empty())
  {
    if (verbosity >= G4VisManager::errors)
    {
      G4warn << "ERROR: No valid scenes available yet.  Please create one." << G4endl;
    }
    return;
  }
  ...
  if (iScene < nScenes)
  {
    ...
    {
      G4cout << "Scene \"" << sceneName << "\" attached to scene handler \""
  ...
  else
  {
    if (verbosity >= G4VisManager::errors)
    {
      G4warn << "ERROR: Scene \"" << sceneName
             << "\" not found.  Use \"/vis/scene/list\" to see possibilities." << G4endl;
    }
  }
}
...
G4VisCommandSceneHandlerCreate::G4VisCommandSceneHandlerCreate() : fId(0)
{
  ...
  fpCommand->SetGuidance("Creates an scene handler for a specific graphics system.");
  fpCommand->SetGuidance(
  ...
  const G4GraphicsSystemList& gslist = fpVisManager->GetAvailableGraphicsSystems();
...
G4String G4VisCommandSceneHandlerCreate::GetCurrentValue(G4UIcommand*)
{
  ...
  const G4VGraphicsSystem* graphicsSystem = fpVisManager->GetCurrentGraphicsSystem();
  if (graphicsSystem)
  {
...
void G4VisCommandSceneHandlerCreate::SetNewValue(G4UIcommand* command, G4String newValue)
{
  G4VisManager::Verbosity verbosity = fpVisManager->GetVerbosity();
  ...
  const G4GraphicsSystemList& gsl = fpVisManager->GetAvailableGraphicsSystems();
  std::size_t nSystems = gsl.size();
  if (nSystems <= 0)
  {
    G4ExceptionDescription ed;
    ed << "ERROR: G4VisCommandSceneHandlerCreate::SetNewValue:"
  ...
  if (!found)
  {
    ...
    {
      G4Exception("G4VisCommandSceneHandlerCreate::SetNewValue", "visman1001", JustWarning,
  ...
  while (!gsl[iGS]->IsUISessionCompatible())
  {
    ...
    if (iGS >= nSystems || loopCounter >= 3)
    {
      ...
      G4Exception("G4VisCommandSceneHandlerCreate::SetNewValue", "visman1002", JustWarning, ed);
      return;
    }
  ...
  if (fallback && verbosity >= G4VisManager::warnings)
  {
    G4warn << "WARNING: G4VisCommandSceneHandlerCreate::SetNewValue:"
           ...
           << pSystem->GetName() << " (" << pSystem->GetNickname() << ')' << G4endl;
  }
  ...
  for (iScene = 0; iScene < list.size(); ++iScene)
  {
    ...
    {
      G4ExceptionDescription ed;
      ed << "ERROR: Scene handler \"" << newName << "\" already exists.";
      command->CommandFailed(ed);
      return;
    }
  }
  ...
  if (verbosity >= G4VisManager::confirmations)
  {
    G4cout << "Graphics system set to " << pSystem->GetName() << " (" << pSystem->GetNickname()
           << ')' << G4endl;
  }
  ...
  if (fpVisManager->GetCurrentSceneHandler()->GetName() != newName)
  {
    ...
    ed << "ERROR: G4VisCommandSceneHandlerCreate::SetNewValue:"
          ...
          "\n Current name \""
       << fpVisManager->GetCurrentSceneHandler()->GetName() << "\" is not the new name \""
...
void G4VisCommandSceneHandlerList::SetNewValue(G4UIcommand*, G4String newValue)
{
  ...
  const G4VSceneHandler* currentSceneHandler = fpVisManager->GetCurrentSceneHandler();
...
G4VisCommandSceneHandlerSelect::G4VisCommandSceneHandlerSelect()
{
  ...
  fpCommand->SetParameterName("scene-handler-name", omitable = false);
}
...
void G4VisCommandSceneHandlerSelect::SetNewValue(G4UIcommand*, G4String newValue)
{
  G4VisManager::Verbosity verbosity = fpVisManager->GetVerbosity();
  ...
  else
  {
    if (verbosity >= G4VisManager::errors)
    {
      G4warn << "ERROR: Scene handler \"" << selectName << "\""
             << " not found - \"/vis/sceneHandler/list\" to see possibilities." << G4endl;
    }
  }
}
