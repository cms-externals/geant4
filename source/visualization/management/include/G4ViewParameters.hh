...
class G4ViewParameters
{
  public:  // With description
    ...
    enum SMROption
    {  // Special Mesh Rendering Option
      meshAsDefault,
      meshAsDots,
      meshAsSurfaces
    };

    friend std::ostream& operator<<(std::ostream&, DrawingStyle);

    friend std::ostream& operator<<(std::ostream&, SMROption);

    friend std::ostream& operator<<(std::ostream&, const G4ViewParameters&);
  ...
  private:
    ...
    G4int ParseGeometry(const char* string, G4int* x, G4int* y, unsigned int* width,
                        unsigned int* height);
    ...
    G4bool
      fWindowLocationHintXNegative;  //  Reference of location hints for pixel-based window systems.
    ...
    enum
    {  // Constants for geometry mask in ParseGeometry and related functions.
      fNoValue = 0,
      fXValue = 0x0001,
      fYValue = 0x0002,
      fWidthValue = 0x0004,
