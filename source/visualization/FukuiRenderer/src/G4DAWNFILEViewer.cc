...
G4DAWNFILEViewer::G4DAWNFILEViewer(G4DAWNFILESceneHandler& sceneHandler, const G4String& name)
  : G4VViewer(sceneHandler, sceneHandler.IncrementViewCount(), name), fSceneHandler(sceneHandler)
{
  ...
  if (fSceneHandler.FRIsInModeling())
  {
    ...
    else if (strcmp(GetG4PrimViewerInvocation(), ""))
    {
      ...
      if (iErr != 0)
      {
        ...
        ed << "Error " << iErr << " when calling system with \"" << GetG4PrimViewerInvocation()
           << "\".";
        G4Exception("G4DAWNFILEViewer::ShowView()", "dawn0005", JustWarning, ed);
      }
    }
    else
