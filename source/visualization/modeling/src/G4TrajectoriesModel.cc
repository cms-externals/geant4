...
G4TrajectoriesModel::G4TrajectoriesModel() : fpCurrentTrajectory(0), fRunID(-1), fEventID(-1)
{
...
void G4TrajectoriesModel::DescribeYourselfTo(G4VGraphicsScene& sceneHandler)
{
  ...
  for (std::size_t iT = 0; iT < TC->entries(); ++iT)
  {
    ...
    if (fpCurrentTrajectory) sceneHandler.AddCompound(*fpCurrentTrajectory);
  }
...
{
  G4bool isNew;
  std::map<G4String, G4AttDef>* store = G4AttDefStore::GetInstance("G4TrajectoriesModel", isNew);
  if (isNew)
  {
    (*store)["RunID"] = G4AttDef("RunID", "Run ID", "Physics", "", "G4int");
    (*store)["EventID"] = G4AttDef("EventID", "Event ID", "Physics", "", "G4int");
  }
  return store;
}
...
std::vector<G4AttValue>* G4TrajectoriesModel::CreateCurrentAttValues() const
{
  ...
  values->push_back(G4AttValue("RunID", G4UIcommand::ConvertToString(fRunID), ""));
  values->push_back(G4AttValue("EventID", G4UIcommand::ConvertToString(fEventID), ""));
  return values;
}
