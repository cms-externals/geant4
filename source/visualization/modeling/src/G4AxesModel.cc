...
G4AxesModel::G4AxesModel(G4double x0, G4double y0, G4double z0, G4double length,
                         G4double arrowWidth, const G4String& colourString,
                         const G4String& description, G4bool withAnnotation, G4double textSize,
                         const G4Transform3D& transform)
{
  Construct(x0, y0, z0, length, arrowWidth, colourString, description, withAnnotation, textSize,
            transform);
}
...
void G4AxesModel::Construct(G4double x0, G4double y0, G4double z0, G4double length,
                            G4double arrowWidth, const G4String& colourString,
                            const G4String& description, G4bool withAnnotation, G4double textSize,
                            const G4Transform3D& transform)
{
  ...
  else
  {
    if (!G4Colour::GetColour(colourString, colour))
    {
      ...
      ed << "Colour \"" << colourString << "\" not found.  Defaulting to white and opaque.";
      G4Exception("G4AxesModel::G4AxesModel", "modeling0011", JustWarning, ed);
    }
  }
  ...
  fXAxisModel = new G4ArrowModel(x0, y0, z0, x0 + length, y0, z0, arrowWidth, xColour,
                                 "x-axis: " + description, lineSegmentsPerCircle, fTransform);
  if (withAnnotation)
  {
  ...
  fYAxisModel = new G4ArrowModel(x0, y0, z0, x0, y0 + length, z0, arrowWidth, yColour,
                                 "y-axis: " + description, lineSegmentsPerCircle, fTransform);
  if (withAnnotation)
  {
  ...
  fZAxisModel = new G4ArrowModel(x0, y0, z0, x0, y0, z0 + length, arrowWidth, zColour,
                                 "z-axis: " + description, lineSegmentsPerCircle, fTransform);
  if (withAnnotation)
  {
