...
bool G4TrajectoryChargeFilter::Evaluate(const G4VTrajectory& traj) const
{
  ...
  if (GetVerbose())
    G4cout << "G4TrajectoryChargeFilter processing trajectory with charge: " << charge << G4endl;
  ...
  std::vector<MyCharge>::const_iterator iter =
    std::find(fCharges.begin(), fCharges.end(), myCharge);
...
void G4TrajectoryChargeFilter::Add(const G4String& charge)
{
  MyCharge myCharge;
  ...
  if (!ConvertToCharge(charge, myCharge))
  {
    ...
    G4Exception("G4TrajectoryChargeFilter::Add(const G4String& charge)", "modeling0115",
                JustWarning, ed);
    return;
  }
...
void G4TrajectoryChargeFilter::Add(const MyCharge& charge)
{
...
G4bool G4TrajectoryChargeFilter::ConvertToCharge(const G4String& string, MyCharge& myCharge)
{
  bool result(true);
