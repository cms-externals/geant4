...
//
//
// $Id: G4LogicalVolumeModel.cc 111772 2018-09-03 07:21:15Z gcosmo $
//
// 
...
(G4VGraphicsScene& sceneHandler) {
  ...
  if (fCheckOverlaps) {
    ...
    for (G4int iSister = 0; iSister < nSisters; ++iSister) {
      G4VPhysicalVolume* pPV = lv->GetDaughter(iSister);
      // Avoid multiple printing.
      // Print only first time for a given instantiation of G4LogicalVolume.
      if (!fOverlapsPrinted) {
        pPV->CheckOverlaps();
      }
//      pPV->DrawOverlaps();
