...
G4LogicalVolumeModel::G4LogicalVolumeModel(G4LogicalVolume* pLV, G4int soughtDepth, G4bool booleans,
                                           G4bool voxels, G4bool readout, G4bool checkOverlaps,
  ...
  :  // Instantiate a G4PhysicalVolumeModel with a G4PVPlacement to
     ...
     // has no mother.
    G4PhysicalVolumeModel(new G4PVPlacement(0,  // No rotation.
                                            ...
                                            false,  // Not "MANY".
                                            0),  // Copy number.
                          soughtDepth, modelTransformation, pMP,
...
void G4LogicalVolumeModel::DescribeYourselfTo(G4VGraphicsScene& sceneHandler)
{
  ...
  if (fVoxels)
  {
    if (fpTopPV->GetLogicalVolume()->GetVoxelHeader())
    {
      // Add Voxels.
      G4DrawVoxels dv;
      G4PlacedPolyhedronList* pPPL = dv.CreatePlacedPolyhedra(fpTopPV->GetLogicalVolume());
    ...
    for (G4int iDaughter = 0; iDaughter < nDaughters; ++iDaughter)
    {
      ...
      else if (daughterPVParam)
      {
        ...
        for (G4int iP = 0; iP < multiplicity; iP++)
        {
          ...
          G4AffineTransform tDaughter(daughterPVParam->GetRotation(),
                                      daughterPVParam->GetTranslation());
          for (G4int i = 0; i < nPoints; ++i)
          {
            G4ThreeVector point = daughterSolid->GetPointOnSurface();
            // Transform to mother's coordinate system
          ...
          for (G4int iPP = iP + 1; iPP < multiplicity; iPP++)
          {
            ...
            G4AffineTransform tSister(daughterPVParam->GetRotation(),
                                      daughterPVParam->GetTranslation());
            for (const auto& motherPoint : motherPoints)
            {
              ...
              if (sisterSolid->Inside(sisterPoint) == kInside)
              {
                ...
                daughterSolid->ComputeDimensions(param, iP, daughterPhys);
                param->ComputeTransformation(iP, daughterPhys);
                tDaughter = G4AffineTransform(daughterPVParam->GetRotation(),
                                              daughterPVParam->GetTranslation());
                DrawSolid(sceneHandler, daughterSolid, iP, tDaughter);
                DrawPoint(sceneHandler, motherPoint);
              }
...
void G4LogicalVolumeModel::DescribeSolid(const G4Transform3D& theAT, G4VSolid* pSol,
                                         ...
                                         G4VGraphicsScene& sceneHandler)
{
  if (fBooleans)
  {
    ...
    {  // Composite solid...
      ...
      if (!pSol1)
      {
        G4Exception("G4PhysicalVolumeModel::DescribeSolid", "modeling0001", FatalException,
