...
G4PhysicalVolumesSearchScene::G4PhysicalVolumesSearchScene(
  G4PhysicalVolumeModel* pSearchVolumesModel, const G4String& requiredPhysicalVolumeName,
...
void G4PhysicalVolumesSearchScene::ProcessVolume(const G4VSolid&)
{
  ...
  if (fMatcher.Match(name))
  {
         ...
         fRequiredCopyNo == copyNo))
    {
      ...
      for (auto& node : basePath)
        node.SetDrawn(false);
      fFindings.push_back(Findings(fpSearchVolumesModel->GetTopPhysicalVolume(), pCurrentPV, copyNo,
                                   fpSearchVolumesModel->GetCurrentDepth(), basePath,
...
G4PhysicalVolumesSearchScene::Matcher::Matcher(const G4String& requiredMatch) : fRegexFlag(false)
{
  ...
  {
    G4Exception("G4PhysicalVolumesSearchScene::Matcher::Matcher", "modeling0013", JustWarning,
