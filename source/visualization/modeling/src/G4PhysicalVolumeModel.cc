...
G4PhysicalVolumeModel::G4PhysicalVolumeModel(
  G4VPhysicalVolume* pVPV, G4int requestedDepth, const G4Transform3D& modelTransform,
  const G4ModelingParameters* pMP, G4bool useFullExtent,
    ...
    fMaxFullDepth(0)
{
  ...
  else
  {
    ...
    oss << fpTopPV->GetName() << ':' << fpTopPV->GetCopyNo() << " BasePath:" << fBaseFullPVPath;
    fGlobalTag = oss.str();
    fGlobalDescription = "G4PhysicalVolumeModel " + fGlobalTag;
...
G4PhysicalVolumeModel::GetPVNameCopyNoPath(const std::vector<G4PhysicalVolumeNodeID>& path)
{
  G4ModelingParameters::PVNameCopyNoPath PVNameCopyNoPath;
  for (const auto& node : path)
  {
    PVNameCopyNoPath.push_back(
      G4ModelingParameters::PVNameCopyNo(node.GetPhysicalVolume()->GetName(), node.GetCopyNo()));
  }
...
void G4PhysicalVolumeModel::CalculateExtent()
{
  ...
  else
  {
    ...
    G4ModelingParameters mParams(0,  // No default vis attributes needed.
  ...
  if (radius < 0.)
  {  // Nothing in the scene - revert to top extent
    fExtent = fpTopPV->GetLogicalVolume()->GetSolid()->GetExtent();
  }
...
void G4PhysicalVolumeModel::DescribeYourselfTo(G4VGraphicsScene& sceneHandler)
{
  if (!fpTopPV)
  {
    G4Exception("G4PhysicalVolumeModel::DescribeYourselfTo", "modeling0012", FatalException,
                "No model.");
  ...
  if (!fpMP)
  {
    G4Exception("G4PhysicalVolumeModel::DescribeYourselfTo", "modeling0013", FatalException,
  ...
  VisitGeometryAndGetVisReps(fpTopPV, fRequestedDepth, startingTransformation, sceneHandler);
                                                       ...
                                                       G4VGraphicsScene& sceneHandler)
{
  ...
  if (!(pVPV->IsReplicated()))
  {
    ...
    DescribeAndDescend(pVPV, requestedDepth, pLV, pSol, pMaterial, theAT, sceneHandler);
  }
  else
  {
      ...
      for (int n = nBegin; n < nEnd; n++)
      {
        ...
        switch (axis)
        {
          ...
          case kRho:
            ...
            {
              if (fpMP->IsWarning())
                G4warn << "G4PhysicalVolumeModel::VisitGeometryAndGetVisReps: WARNING:"
                          "\n  built-in replicated volumes replicated in radius for "
                       << pSol->GetEntityType() << "-type\n  solids (your solid \""
                       << pSol->GetName() << "\") are not visualisable." << G4endl;
              visualisable = false;
            }
            break;
          case kPhi:
...
void G4PhysicalVolumeModel::DescribeAndDescend(G4VPhysicalVolume* pVPV, G4int requestedDepth,
                                               G4LogicalVolume* pLV, G4VSolid* pSol,
                                               G4Material* pMaterial, const G4Transform3D& theAT,
                                               G4VGraphicsScene& sceneHandler)
{
  ...
  auto nodeID = G4PhysicalVolumeNodeID(fpCurrentPV, copyNo, fCurrentDepth, fCurrentTransform);
  ...
  if (!pVisAttribs)
  {
    ...
    if (fpMP->GetCBDAlgorithmNumber() == 1)
    {
      ...
      {
        G4Exception("G4PhysicalVolumeModel::DescribeAndDescend", "modeling0014",
                    FatalErrorInArgument, "Algorithm-parameter mismatch for Colour By Density");
      }
      else
      {
        ...
        if (d < d0)
        {  // Density < d0 is invisible.
          tempVisAtts->SetVisibility(false);
        }
        else
        {  // Intermediate densities are on a spectrum.
  ...
  if (vams.size())
  {
    ...
    for (const auto& vam : vams)
    {
      ...
      if (vamPath.size() == fFullPVPath.size())
      {
        ...
        {
          if (!(iVAMNameCopyNo->GetName() == iPVNodeId->GetPhysicalVolume()->GetName()
                && iVAMNameCopyNo->GetCopyNo() == iPVNodeId->GetPhysicalVolume()->GetCopyNo()))
          {
        ...
        if (iVAMNameCopyNo == vamPath.end())
        {
          ...
          switch (vam.GetVisAttributesSignifier())
          {
            ...
            case G4ModelingParameters::VASDaughtersInvisible:
              modifiedVisAtts.SetDaughtersInvisible(transVisAtts.IsDaughtersInvisible());
              break;
            ...
            case G4ModelingParameters::VASForceWireframe:
              ...
              {
                if (transVisAtts.GetForcedDrawingStyle() == G4VisAttributes::wireframe)
                {
            ...
            case G4ModelingParameters::VASForceSolid:
              if (transVisAtts.IsForceDrawingStyle())
              {
                if (transVisAtts.GetForcedDrawingStyle() == G4VisAttributes::solid)
                {
            ...
            case G4ModelingParameters::VASForceCloud:
              if (transVisAtts.IsForceDrawingStyle())
              {
                if (transVisAtts.GetForcedDrawingStyle() == G4VisAttributes::cloud)
                {
            ...
            case G4ModelingParameters::VASForceAuxEdgeVisible:
              ...
              {
                modifiedVisAtts.SetForceAuxEdgeVisible(transVisAtts.IsForcedAuxEdgeVisible());
              }
              break;
  ...
  G4bool markedVisible = pVisAttribs->IsVisible() && pVisAttribs->GetColour().GetAlpha() > 0;
  ...
  if (thisToBeDrawn)
  {
    ...
    if (fpMP->IsExplode() && fDrawnPVPath.size() == 1)
    {
      ...
      G4Translate3D newTranslation =
        G4Translate3D(explodeFactor * oldTranslation.dx(), explodeFactor * oldTranslation.dy(),
  ...
  else
  {
    ...
    G4bool surfaceDrawing = fpMP->GetDrawingStyle() == G4ModelingParameters::hsr
  ...
  if (daughtersToBeDrawn)
  {
    for (G4int iDaughter = 0; iDaughter < nDaughters; iDaughter++)
    {
      ...
      fCurrentDepth++;
      VisitGeometryAndGetVisReps(pDaughterVPV, requestedDepth - 1, theNewAT, sceneHandler);
      fCurrentDepth--;
    }
  }
...
G4bool IntersectionBoundingLimits(const G4VSolid* target, const G4DisplacedSolid* intersector)
{
  ...
  pMin.set(std::max(minA.x(), minB.x()), std::max(minA.y(), minB.y()),
           std::max(minA.z(), minB.z()));
  pMax.set(std::min(maxA.x(), maxB.x()), std::min(maxA.y(), maxB.y()),
...
void G4PhysicalVolumeModel::DescribeSolid(const G4Transform3D& theAT, G4VSolid* pSol,
    ...
    if (fpClippingSolid)
    {
      ...
      switch (fClippingMode)
      {
        case subtraction:
          ...
          {
            pResultantSolid =
              new G4SubtractionSolid("subtracted_clipped_solid", pSol, pDisplacedSolid);
          }
          break;
        case intersection:
          ...
          {
            pResultantSolid =
              new G4IntersectionSolid("intersected_clipped_solid", pSol, pDisplacedSolid);
          }
          break;
      }
    }
...
{
  G4bool isNew;
  std::map<G4String, G4AttDef>* store = G4AttDefStore::GetInstance("G4PhysicalVolumeModel", isNew);
  if (isNew)
  {
    (*store)["PVPath"] = G4AttDef("PVPath", "Physical Volume Path", "Physics", "", "G4String");
    (*store)["BasePVPath"] =
      G4AttDef("BasePVPath", "Base Physical Volume Path", "Physics", "", "G4String");
    (*store)["LVol"] = G4AttDef("LVol", "Logical Volume", "Physics", "", "G4String");
    (*store)["Solid"] = G4AttDef("Solid", "Solid Name", "Physics", "", "G4String");
    (*store)["EType"] = G4AttDef("EType", "Entity Type", "Physics", "", "G4String");
    (*store)["DmpSol"] = G4AttDef("DmpSol", "Dump of Solid properties", "Physics", "", "G4String");
    (*store)["LocalTrans"] =
    ...
    (*store)["Material"] = G4AttDef("Material", "Material Name", "Physics", "", "G4String");
    (*store)["Density"] =
      G4AttDef("Density", "Material Density", "Physics", "G4BestUnit", "G4double");
    (*store)["State"] = G4AttDef("State", "Material State (enum undefined,solid,liquid,gas)",
                                 "Physics", "", "G4String");
    (*store)["Radlen"] =
      G4AttDef("Radlen", "Material Radiation Length", "Physics", "G4BestUnit", "G4double");
    (*store)["Region"] = G4AttDef("Region", "Cuts Region", "Physics", "", "G4String");
    (*store)["RootRegion"] =
...
std::vector<G4AttValue>* G4PhysicalVolumeModel::CreateCurrentAttValues() const
{
  ...
  if (!fpCurrentLV)
  {
    G4Exception("G4PhysicalVolumeModel::CreateCurrentAttValues", "modeling0004", JustWarning,
  ...
  const G4PhysicalVolumeModel::G4PhysicalVolumeNodeID& right) const
{
  ...
  if (fpPV == right.fpPV)
  {
    ...
    if (fCopyNo == right.fCopyNo) return fNonCulledDepth < right.fNonCulledDepth;
  }
  return false;
}
...
G4bool G4PhysicalVolumeModel::G4PhysicalVolumeNodeID::operator!=(
  const G4PhysicalVolumeModel::G4PhysicalVolumeNodeID& right) const
{
  if (fpPV != right.fpPV || fCopyNo != right.fCopyNo || fNonCulledDepth != right.fNonCulledDepth
      || fTransform != right.fTransform || fDrawn != right.fDrawn)
    return true;
  return false;
}
