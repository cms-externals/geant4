...
G4TrajectoryDrawByAttribute::G4TrajectoryDrawByAttribute(const G4String& name,
...
void G4TrajectoryDrawByAttribute::Draw(const G4VTrajectory& object, const G4bool& /*visible*/) const
{
  ...
  if (fAttName.empty())
  {
    if (!fWarnedMissingAttribute)
    {
      ...
      G4Exception("G4TrajectoryDrawByAttribute::Draw", "modeling0116", JustWarning, ed);
    ...
    if (!G4AttUtils::ExtractAttDef(object, fAttName, attDef))
    {
      ...
      if (!warnedUnableToExtract)
      {
        ...
        ed << "Unable to extract attribute definition named " << fAttName << '\n'
           ...
           << *object.GetAttDefs();
        G4Exception("G4TrajectoryDrawByAttribute::Draw", "modeling0117", JustWarning, ed,
  ...
  if (!G4AttUtils::ExtractAttValue(object, fAttName, attVal))
  {
    ...
    if (!warnedUnableToExtract)
    {
      ...
      ed << "Unable to extract attribute definition named " << fAttName << '\n'
         << "Available attributes:\n"
         << *object.GetAttDefs();
      G4Exception("G4TrajectoryDrawByAttribute::Draw", "modeling0118", JustWarning, ed,
                  ". Invalid attribute name");
      warnedUnableToExtract = true;
    }
    return;
...
{
  ostr << "G4TrajectoryDrawByAttribute, dumping configuration for model named " << Name() << ":"
       << std::endl;
  ;
...
void G4TrajectoryDrawByAttribute::Set(const G4String& name)
{
  fAttName = name;
}
                                                     ...
                                                     G4VisTrajContext* context)
{
  ...
  if (iter != fContextMap.end())
  {
    ...
    G4Exception("G4TrajectoryDrawByAttribute::AddIntervalContext", "modeling0119",
                FatalErrorInArgument, ed, ". Invalid interval");
  }

  fContextMap[myPair] = context;
}

void G4TrajectoryDrawByAttribute::AddValueContext(const G4String& name, G4VisTrajContext* context)
{
  // Takes ownership of context
  ...
  if (iter != fContextMap.end())
  {
    ...
    G4Exception("G4TrajectoryDrawByAttribute::AddSingleValueContext", "modeling0120",
                FatalErrorInArgument, ed, ". Invalid value");
  }
