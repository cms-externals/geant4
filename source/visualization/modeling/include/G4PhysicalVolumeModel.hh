...
class G4PhysicalVolumeModel : public G4VModel
{
  public:  // With description
    ...
    class G4PhysicalVolumeNodeID
    {
      public:
        ...
        G4bool operator!=(const G4PhysicalVolumeNodeID& right) const;
        G4bool operator==(const G4PhysicalVolumeNodeID& right) const { return !operator!=(right); }
    ...
    G4PhysicalVolumeModel(G4VPhysicalVolume* = 0, G4int requestedDepth = UNLIMITED,
                          const G4Transform3D& modelTransformation = G4Transform3D(),
                          const G4ModelingParameters* = 0, G4bool useFullExtent = false,
    ...
    const G4VSolid* GetClippingSolid() const { return fpClippingSolid; }
    ...
    const std::vector<G4PhysicalVolumeNodeID>& GetBaseFullPVPath() const { return fBaseFullPVPath; }
    ...
    const std::vector<G4PhysicalVolumeNodeID>& GetFullPVPath() const { return fFullPVPath; }
    ...
    const std::vector<G4PhysicalVolumeNodeID>& GetDrawnPVPath() const { return fDrawnPVPath; }
    ...
    void SetRequestedDepth(G4int requestedDepth) { fRequestedDepth = requestedDepth; }

    void SetClippingSolid(G4VSolid* pClippingSolid) { fpClippingSolid = pClippingSolid; }

    void SetClippingMode(ClippingMode mode) { fClippingMode = mode; }
  ...
  protected:
    ...
    void VisitGeometryAndGetVisReps(G4VPhysicalVolume*, G4int requestedDepth, const G4Transform3D&,
                                    G4VGraphicsScene&);

    void DescribeAndDescend(G4VPhysicalVolume*, G4int requestedDepth, G4LogicalVolume*, G4VSolid*,
                            G4Material*, const G4Transform3D&, G4VGraphicsScene&);

    virtual void DescribeSolid(const G4Transform3D& theAT, G4VSolid* pSol,
