...
void G4VRML2FileSceneHandler::closePort()
{
  ...
  if (!strcmp(viewer, NO_VRML_VIEWER))
  {
    if (G4VisManager::GetVerbosity() >= G4VisManager::errors)
    {
      ...
      G4cout << "ALSO you may change the file header with " << ENV_WRL_FILE_HEADER << G4endl;
      G4cout << "    or the whole filename with " << ENV_WRL_FILE_NAME << G4endl;
    }
  }
  else
  {
    ...
    if (iErr != 0)
    {
      ...
      G4Exception("G4VRML2FileSceneHandler::closePort()", "VRML-2006", JustWarning, ed);
    }
  }
}
