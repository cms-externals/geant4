...
class G4ToolsSGOffscreenViewer
  : public G4ToolsSGViewer<tools::offscreen::session, tools::offscreen::sg_viewer>
{
    typedef G4ToolsSGViewer<tools::offscreen::session, tools::offscreen::sg_viewer> parent;

  public:

    G4ToolsSGOffscreenViewer(tools::offscreen::session& a_session,
                             G4ToolsSGSceneHandler& a_scene_handler, const G4String& a_name)
  ...
  public:
    ...
    virtual void Initialise()
    {
      ...
      fSGViewer =
        new tools::offscreen::sg_viewer(fSGSession, fVP.GetWindowAbsoluteLocationHintX(1440),
                                        fVP.GetWindowAbsoluteLocationHintY(900),
                                        fVP.GetWindowSizeHintX(), fVP.GetWindowSizeHintY(), fName);
      ...
      private:
        ...
        Messenger()
        {
          ...
          cmd_size->SetGuidance(
            "If width and/or height is set to zero, the viewer size specified with "
            "/vis/viewer/create (/vis/open) is taken.");
          cmd_size->SetGuidance(
            " About the picture size, note that the gl2ps files will grow with the number of "
            "primitives");
          cmd_size->SetGuidance(
            "(gl2ps does not have a zbuffer logic). The \"zb\" files will not grow with the number "
            "of");
          cmd_size->SetGuidance(
          ...
          cmd_do_transparency->SetGuidance(
            "True/false to enable/disable rendering of transparent objects.");
          cmd_do_transparency->SetGuidance(
            "This may be usefull if using file formats, as the gl2ps ones, unable to handle "
            "transparency.");
  ...
  protected:
    ...
    static bool GetFormatExtension(const std::string& a_format, std::string& a_ext)
    {
      ...
      if (a_format == "gl2ps_ps")
      {
        a_ext = "ps";
        return true;
      }
      if (a_format == "gl2ps_pdf")
      {
        a_ext = "pdf";
        return true;
      }
      if (a_format == "gl2ps_svg")
      {
        a_ext = "svg";
        return true;
      }
      if (a_format == "gl2ps_tex")
      {
        a_ext = "tex";
        return true;
      }
      if (a_format == "gl2ps_pgf")
      {
        a_ext = "pgf";
        return true;
      }
