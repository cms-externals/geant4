...
class G4ToolsSGViewer : public G4VViewer, public G4ToolsSG_viewer, tools::sg::device_interactor
{
  ...
  public:
    ...
    G4ToolsSGViewer(SG_SESSION& a_session, G4ToolsSGSceneHandler& a_scene_handler,
    ...
    {
      // WARNING : nodes may refer f_gl2ps_mgr, f_zb_mgr (to handle gstos (for GPU) or textures),
      // then
      //           we have to delete them first.
      fSGViewer->sg().clear();
      fSGSceneHandler.GetTransient2DObjects().clear();
      fSGSceneHandler.GetPersistent2DObjects().clear();
      fSGSceneHandler.GetTransient3DObjects().clear();
      fSGSceneHandler.GetPersistent3DObjects().clear();
      fSGSceneHandler.EmptyTrash();
      delete fSGViewer;
    }
  ...
  public:
    ...
    virtual void Initialise()
    {
      ...
      //////////////////////////////////////////////////////////
      fSGViewer = new SG_VIEWER(fSGSession, fVP.GetWindowAbsoluteLocationHintX(1440),
                                fVP.GetWindowAbsoluteLocationHintY(900), fVP.GetWindowSizeHintX(),
                                fVP.GetWindowSizeHintY(), fName);
    ...
    virtual void SetView()
    {
      ...
      {
        fSGViewer->set_clear_color(
          0.3, 0.3, 0.3, 1);  // some grey color to signal the user that something is wrong.
      ...
      {  // With Qt/Cocoa/OpenGL, Qt/Windows/OpenGL, the received size in
        // tools::sg::glarea::resizeGL is the QWidget::[width(),height()] multiplied by the
        // "devicePixelRatio" (with a value of 2 seen on a MacBookPro and 1.5 seen on a Windows
        // laptop).
      ...
      G4cout << "debug : targetPoint : " << targetPoint.x() << " " << targetPoint.y() << " " <<
      targetPoint.z() << std::endl; G4cout << "debug : cameraPosition : " << cameraPosition.x() << "
      " << cameraPosition.y() << " " << cameraPosition.z() << std::endl; G4cout << "debug : camera :
      znear " << pnear << ", zfar " << pfar << std::endl;
      */
      //////////////////////////////////////////////////////////
      /// create scene graph ///////////////////////////////////
      //////////////////////////////////////////////////////////
      ...
      {
        // G4cout << "debug : camera : ortho : top " << top << " bottom " << bottom << " top-bottom
        // " << top-bottom << std::endl;
        if ((top - bottom) <= 0)
        {
          fSGViewer->set_clear_color(0.3, 0.3, 0.3, 1);
      ...
      {
        // G4cout << "debug : camera : perspec : heightAngle " << float(2*fVP.GetFieldHalfAngle())
        // << std::endl;
        tools::sg::perspective* perspective_camera = new tools::sg::perspective;
        perspective_camera->height_angle.value(
          2 * std::atan(std::tan(fVP.GetFieldHalfAngle()) / fVP.GetZoomFactor()));
        _camera = perspective_camera;
      }

      _camera->position.value(tools::vec3f(float(cameraPosition.x()), float(cameraPosition.y()),
                                           float(cameraPosition.z())));
      _camera->znear.value(float(pnear));
      _camera->zfar.value(float(pfar));

      _camera->look_at(
        dir, tools::vec3f(up.x(), up.y(), up.z()));  // same logic as in G4OpenInventorViewer.

      /*
      const G4Vector3D& lightDirection = fVP.GetLightpointDirection();
      G4cout << "debug : lightDirection : " << lightDirection.x() << " " << lightDirection.y() << "
      " << lightDirection.z() << std::endl; const G4Vector3D& actualLightDirection =
      fVP.GetActualLightpointDirection(); G4cout << "debug : actualLightDirection : " <<
      actualLightDirection.x() << " " << actualLightDirection.y() << " " << actualLightDirection.z()
      << std::endl;
      */
...

      {
        ...
        fSGViewer->set_clear_color(float(background.GetRed()), float(background.GetGreen()),
                                   float(background.GetBlue()), 1);
      }
    }
  ...
  protected:
    ...
    G4bool CompareForKernelVisit(G4ViewParameters& vp)
    {
      ...
      if ((vp.GetDrawingStyle() != fVP.GetDrawingStyle())
          ...
          || (vp.GetCBDAlgorithmNumber() != fVP.GetCBDAlgorithmNumber())
          || (vp.IsSection() != fVP.IsSection()) || (vp.IsCutaway() != fVP.IsCutaway()) ||
          ...
          (vp.IsExplode() != fVP.IsExplode()) || (vp.GetNoOfSides() != fVP.GetNoOfSides())
          ...
          || (vp.IsDotsSmooth() != fVP.IsDotsSmooth()) || (vp.GetDotsSize() != fVP.GetDotsSize()))
        return true;

      if (vp.IsDensityCulling() && (vp.GetVisibleDensity() != fVP.GetVisibleDensity())) return true;

      if (vp.GetCBDAlgorithmNumber() > 0)
      {
        if (vp.GetCBDParameters().size() != fVP.GetCBDParameters().size())
          return true;
        else if (vp.GetCBDParameters() != fVP.GetCBDParameters())
          return true;
      }

      if (vp.IsSection() && (vp.GetSectionPlane() != fVP.GetSectionPlane())) return true;

      if (vp.IsCutaway())
      {
        if (vp.GetCutawayMode() != fVP.GetCutawayMode()) return true;
        if (vp.GetCutawayPlanes().size() != fVP.GetCutawayPlanes().size()) return true;
        for (size_t i = 0; i < vp.GetCutawayPlanes().size(); ++i)
          if (vp.GetCutawayPlanes()[i] != fVP.GetCutawayPlanes()[i]) return true;
      }

      if (vp.IsExplode() && (vp.GetExplodeFactor() != fVP.GetExplodeFactor())) return true;
  ...
  protected:
    ...
    void CreateSG(tools::sg::base_camera* a_camera, const G4Vector3D& a_light_dir)
    {
...

      {
        ...
        light->ambient = tools::colorf(
          0.2f, 0.2f, 0.2f,
          1.0f);  // same as in G4OpenGLViewer.cc glLight(GL_LIGHT0,GL_AMBIENT and GL_DIFFUSE).
    ...
    void Export(const G4String& a_format, const G4String& a_file, G4bool a_do_transparency)
    {
      ...
      if (!tools::sg::write_paper(
            G4cout, f_gl2ps_mgr, f_zb_mgr, tools::fpng::write, tools::toojpeg::write,
            float(back_color.GetRed()), float(back_color.GetGreen()), float(back_color.GetBlue()),
            float(back_color.GetAlpha()), fSGViewer->sg(), fSGViewer->width(), fSGViewer->height(),
  ...
  protected:
    ...
    class Messenger : public G4VVisCommand
    {
      ...
      private:
        ...
        virtual ~Messenger() { delete write_scene; }
