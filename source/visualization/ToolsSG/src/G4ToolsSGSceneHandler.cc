...
G4ToolsSGSceneHandler::G4ToolsSGSceneHandler(G4VGraphicsSystem& system, const G4String& name)
  : parent(system, fSceneIdCount++, name), fFreetypeNode(0), fMarkerScale(1)
{
  //::printf("debug : G4ToolsSGSceneHandler : %lu, %s\n",this,name.c_str());
  EstablishBaseNodes();
#if defined(TOOLS_USE_FREETYPE)
  fFreetypeNode = new toolx::sg::text_freetype();
  fFreetypeNode->add_embedded_font(tools::sg::font_lato_regular_ttf(),
...
G4ToolsSGSceneHandler::~G4ToolsSGSceneHandler()
{
  ...
  if (fpTransient2DObjects.size() || fpPersistent2DObjects.size() || fpTransient3DObjects.size()
      || fpPersistent3DObjects.size() || fpTrash.size())
  {
    // WARNING : nodes may refer graphics managers, as
    // tools/sg/[GL_manager,gl2ps_manager,zb_manager]
    //           used by viewers, to handle gstos (for GPU) or textures, then they must be deleted
    //           from the viewers before deleting the managers, and not here, since the
    //           G4VisManager deletes the viewers before deleting their scene handler.
    G4cerr << "G4ToolsSGSceneHandler::~G4ToolsSGSceneHandler: some nodes are not cleared."
           << G4endl;
  }
  delete fFreetypeNode;
}

void G4ToolsSGSceneHandler::EmptyTrash()
{
  fpTrash.clear();
}
...
{  // Retrieve or create a G4ToolsSGNode node suitable for next solid or primitive
  ...
  {  // All transients hang from this node
  ...
  for (; iWorld < nWorlds; ++iWorld)
  {
    if (fullPVPath[0].GetPhysicalVolume()
        == fpPhysicalVolumeObjects[iWorld]->GetPVNodeID().GetPhysicalVolume())
      break;
  }
  if (iWorld == nWorlds)
  {
...
{
  // Nodes may refer graphics managers (as tools/sg/[GL_manager,gl2ps_manager,zb_manager]
  // used by viewers to handle gstos (for GPU) or textures. In multi-threads, we can't
  // then delete them here (which is called from the vis thread), but from the main vis
  // thread. The no more used nodes are put on the fpTrash group to be deleted from
  // the viewer when back in the main vis thread (see usage of EmptyTrash() in G4ToolsSGViewer).
  fpTrash.takeover_nodes(fpTransient2DObjects);
  fpTrash.takeover_nodes(fpPersistent2DObjects);
  fpTrash.takeover_nodes(fpTransient3DObjects);
  fpTrash.takeover_nodes(fpPersistent3DObjects);
  EstablishBaseNodes();
}
...
{
  // see upper comment about handling no more used nodes.
  fpTrash.takeover_nodes(fpTransient2DObjects);
  fpTrash.takeover_nodes(fpTransient3DObjects);
  if (fpViewer)
  {
...
void G4ToolsSGSceneHandler::AddPrimitive(const G4Polyline& a_polyline)
{
  ...
  else
  {
    ...
    mtx->mtx.value().set_matrix(elem(0, 0), elem(0, 1), elem(0, 2), elem(0, 3), elem(1, 0),
                                elem(1, 1), elem(1, 2), elem(1, 3), elem(2, 0), elem(2, 1),
                                elem(2, 2), elem(2, 3), 0, 0, 0, 1);
    parentNode->add(mtx);
  }

  {
    ...
    mat->color = tools::colorf(float(colour.GetRed()), float(colour.GetGreen()),
                               float(colour.GetBlue()), float(colour.GetAlpha()));
    parentNode->add(mat);
  }
...
{
  fMarkerScale = a_scale;
}

void G4ToolsSGSceneHandler::AddPrimitive(const G4Text& a_text)
{
  //::printf("debug : G4ToolsSGSceneHandler::AddPrimitive(const G4Text&) : 000 :
  //:\"%s\"\n",a_text.GetText().c_str()); :printf("debug : G4ToolsSGSceneHandler::AddPrimitive(const
  //: G4Text&) : 2D ? %d\n",fProcessing2D);
...
void G4ToolsSGSceneHandler::AddPrimitive(const G4Polyhedron& a_polyhedron)
{
  ...
  //::printf("debug : G4ToolsSGSceneHandler::AddPrimitive(const G4Polyhedron&) :
  ...
  do
  {
    ...
    {
      G4cerr << "G4ToolsSGSceneHandler::AddPrimitive(G4Polyhedron): WARNING:"
  ...
  if (drawing_style == G4ViewParameters::hlr || drawing_style == G4ViewParameters::hsr
  ...
  if (drawing_style == G4ViewParameters::wireframe || drawing_style == G4ViewParameters::hlr
      || drawing_style == G4ViewParameters::hlhsr)
  {
    {
      tools::sg::draw_style* ds = new tools::sg::draw_style;
      ds->style = tools::sg::draw_lines;
      auto visAtts =
        GetCurrentViewer()->GetApplicableVisAttributes(a_polyhedron.GetVisAttributes());
      ds->line_width = GetLineWidth(visAtts);  // Multiplies by GetGlobalLineWidthScale
      sep->add(ds);
    }
...
// plotting:
inline void SetRegionStyles(tools::xml::styles& a_styles, tools::sg::plots& a_plots,
                            tools::sg::plotter& a_plotter, const G4String& a_style)
{
  if (a_style == "reset")
  {
  ...
  {
    tools::sg::set_inlib_default_style(G4cout, a_styles.cmaps(), a_plotter,
                                       tools::sg::font_hershey());
  }
  else if (a_style == "ROOT_default")
  {
    tools::sg::set_ROOT_default_style(G4cout, a_styles.cmaps(), a_plotter,
                                      tools::sg::font_roboto_bold_ttf());
  }
  else if (a_style == "hippodraw")
  {
    tools::sg::set_hippodraw_style(G4cout, a_styles.cmaps(), a_plotter,
                                   tools::sg::font_lato_regular_ttf());
  }
  else
  {
...
inline tools::xml::styles::style_t* find_style(tools::xml::styles& a_styles,
                                 ...
                                 const std::vector<G4Plotter::RegionParameter>& a_region_parameters)
{
  ...
  for (const auto& [region, param] : a_region_parameters)
  {
    ...
    if (_plotter)
    {
      ...
      {
        G4cout << "G4ToolsSGSceneHandler::SetPlotterParameters: plotter.set_from_string() failed "
                  "for field "
...
void G4ToolsSGSceneHandler::SetPlotterHistograms(tools::sg::plots& a_plots)
{
  ...
  for (const auto& [region, hid] : fRegionH1s)
  {
    ...
    if (_plotter)
    {
      ...
      {
        G4cerr << "G4ToolsSGSceneHandler::SetPlotterHistograms: cannot get histogram - maybe "
                  "doesn't exist?"
...
class plots_cbk : public tools::sg::ecbk
{
  ...
  public:
    ...
    virtual tools::sg::return_action action()
    {
      ...
      if (const tools::sg::size_event* sz_evt =
            tools::sg::event_cast<tools::sg::event, tools::sg::size_event>(*m_event))
      {
  ...
  public:
    ...
    plots_cbk(tools::sg::plots& a_plots) : parent(), m_plots(a_plots) {}
    virtual ~plots_cbk() {}
...
void G4ToolsSGSceneHandler::TouchPlotters(tools::sg::node& a_sg)
{
  ...
  const tools::sg::search_action::paths_t& paths =
    tools::sg::find_paths<tools::sg::plots>(sa, a_sg);
...
void G4ToolsSGSceneHandler::Messenger::SetNewValue(G4UIcommand* a_cmd, G4String)
{
  ...
  {
    G4cout << "G4ToolsSGSceneHandler::Messenger::SetNewValue: current sceneHandler not a "
