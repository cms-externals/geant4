...
G4GMocrenFileViewer::G4GMocrenFileViewer(G4GMocrenFileSceneHandler& sceneHandler,
                                         G4GMocrenMessenger&, const G4String& name)
  : G4VViewer(sceneHandler, sceneHandler.IncrementViewCount(), name), kSceneHandler(sceneHandler)
{
  ...
  if (std::getenv("G4GMocrenFile_VIEWER") != NULL)
  {
    ...
    if (len >= 32)
    {
      G4Exception("G4GMocrenFileViewer::G4GMocrenFileViewer(*)", "gMocren1000", FatalException,
  ...
  {
    std::strncpy(kG4GddViewerInvocation, kG4GddViewer, sizeof(kG4GddViewerInvocation) - 1);
    kG4GddViewerInvocation[sizeof(kG4GddViewerInvocation) - 1] = '\0';
    G4int n = sizeof(kG4GddViewerInvocation) - (G4int)std::strlen(kG4GddViewerInvocation) - 1;
    std::strncat(kG4GddViewerInvocation, " ", n);
    const char* gddfname = kSceneHandler.GetGddFileName();
    G4int len = (G4int)std::strlen(gddfname);
    if (len >= 64)
    {
      G4Exception("G4GMocrenFileViewer::G4GMocrenFileViewer(*)", "gMocren1001", FatalException,
                  "Invalid length of the GDD file name");
    }
    n = sizeof(kG4GddViewerInvocation) - (G4int)std::strlen(kG4GddViewerInvocation) - 1;
...
void G4GMocrenFileViewer::ShowView(void)
{
  ...
  if (kSceneHandler.GFIsInModeling())
  {
    ...
    else
    {
      ...
      if (len >= 64)
      {
        G4Exception("G4GMocrenFileViewer::ShowView()", "gMocren1002", FatalException,
                    "Invalid length of the GDD file name");
      }
      n = sizeof(kG4GddViewerInvocation) - (G4int)std::strlen(kG4GddViewerInvocation) - 1;
      std::strncat(kG4GddViewerInvocation, gddfname, n);
