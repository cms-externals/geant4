...
GMocrenDataPrimitive<T>& GMocrenDataPrimitive<T>::operator+=(const GMocrenDataPrimitive<T>& _right)
{
  ...
  {
    if (G4VisManager::GetVerbosity() >= G4VisManager::errors)
      G4cout << "Warning: operator += " << G4endl << "         Cannot do the operator +=" << G4endl;
    return *this;
  }
...
void GMocrenTrack::addStep(float _startx, float _starty, float _startz, float _endx, float _endy,
                           float _endz)
{
  struct Step step;
...
void GMocrenTrack::getStep(float& _startx, float& _starty, float& _startz, float& _endx,
                           float& _endy, float& _endz, int _num)
{
...
void GMocrenDetector::addEdge(float _startx, float _starty, float _startz, float _endx, float _endy,
                              float _endz)
{
  struct Edge edge;
...
void GMocrenDetector::getEdge(float& _startx, float& _starty, float& _startz, float& _endx,
                              float& _endy, float& _endz, int _num)
{
...
// constructor
G4GMocrenIO::G4GMocrenIO() : kTracksWillBeStored(true)
{
  ;
}
...
void G4GMocrenIO::initialize()
{
  ...
  for (itr = kSteps.begin(); itr != kSteps.end(); itr++)
    delete[] *itr;
  kSteps.clear();
...
bool G4GMocrenIO::storeData4()
{
  ...
  if (DEBUG || kVerbose > 0) G4cout << ">>>>>>>  store data (ver.4) <<<<<<<" << G4endl;
  if (DEBUG || kVerbose > 0) G4cout << "         " << kFileName << G4endl;

  // output file open
  std::ofstream ofile(kFileName.c_str(), std::ios_base::out | std::ios_base::binary);
  if (DEBUG || kVerbose > 0) G4cout << "         file open status: " << ofile.rdbuf() << G4endl;
  ...
  if (DEBUG || kVerbose > 0)
  {
    G4cout << "Data comment : " << kComment << G4endl;
  }
  ...
  if (DEBUG || kVerbose > 0)
  {
    G4cout << "Voxel spacing : (" << kVoxelSpacing[0] << ", " << kVoxelSpacing[1] << ", "
           << kVoxelSpacing[2] << ") mm " << G4endl;
  }
  ...
  if (DEBUG || kVerbose > 0)
  {
    G4cout << "Each pointer to data : " << kPointerToModalityData << ", ";
    for (int i = 0; i < nDoseDist; i++)
    {
      G4cout << kPointerToDoseDistData[i] << ", ";
    }
    G4cout << kPointerToROIData << ", " << kPointerToTrackData << ", " << kPointerToDetectorData
           << G4endl;
  }
  ...
  if (DEBUG || kVerbose > 0)
  {
    G4cout << "Modality image size : (" << size[0] << ", " << size[1] << ", " << size[2] << ")"
           << G4endl;
  }
  ...
  if (DEBUG || kVerbose > 0)
  {
    G4cout << "Modality image min., max., scale : " << minmax[0] << ", " << minmax[1] << ", "
           << scale << G4endl;
  }
  ...
  for (int i = 0; i < (int)msize; i++)
    pdmap[i] = kModalityImageDensityMap[i];
  ...
  if (!isDoseEmpty())
  {
    ...
    for (int ndose = 0; ndose < nDoseDist; ndose++)
    {
      ...
      if (DEBUG || kVerbose > 0)
      {
        G4cout << "Dose dist. [" << ndose << "] image size : (" << size[0] << ", " << size[1]
               << ", " << size[2] << ")" << G4endl;
      }
      ...
      if (DEBUG || kVerbose > 0)
      {
        G4cout << "Dose dist. [" << ndose << "] image min., max., scale : " << minmax[0] << ", "
               << minmax[1] << ", " << scale << G4endl;
      }
      ...
      for (int z = 0; z < size[2]; z++)
      {
        ...
        if (DEBUG || kVerbose > 0)
        {
          for (int j = 0; j < dsize; j++)
          {
            if (dimage[j] < 0) G4cout << "[" << j << "," << z << "]" << dimage[j] << ", ";
          }
        }
      }
      if (DEBUG || kVerbose > 0) G4cout << G4endl;
      ...
      if (DEBUG || kVerbose > 0)
      {
        G4cout << "Dose dist. [" << ndose << "]image relative location : (" << iCenter[0] << ", "
               << iCenter[1] << ", " << iCenter[2] << ")" << G4endl;
      }
  ...
  if (!isROIEmpty())
  {
    ...
    if (DEBUG || kVerbose > 0)
    {
      G4cout << "ROI image size : (" << size[0] << ", " << size[1] << ", " << size[2] << ")"
             << G4endl;
    }
    ...
    if (DEBUG || kVerbose > 0)
    {
      G4cout << "ROI image min., max., scale : " << minmax[0] << ", " << minmax[1] << ", " << scale
             << G4endl;
    }
    ...
    if (DEBUG || kVerbose > 0)
    {
      G4cout << "ROI image relative location : (" << iCenter[0] << ", " << iCenter[1] << ", "
             << iCenter[2] << ")" << G4endl;
    }
  }
  ...
  if (kPointerToTrackData > 0)
  {
    ...
    for (int nt = 0; nt < ntrk; nt++)
    {
      ...
      {
        kTracks[nt].getStep(stepPoints[0], stepPoints[1], stepPoints[2], stepPoints[3],
                            stepPoints[4], stepPoints[5], isteps);
  ...
  if (kPointerToDetectorData > 0)
  {
    ...
    if (DEBUG || kVerbose > 0)
    {
      G4cout << "# of detectors : " << ndet << G4endl;
    }
    ...
    for (int nd = 0; nd < ndet; nd++)
    {
      ...
      {
        kDetectors[nd].getEdge(edgePoints[0], edgePoints[1], edgePoints[2], edgePoints[3],
                               edgePoints[4], edgePoints[5], ne);
        ...
        if (DEBUG || kVerbose > 0)
        {
          if (ne < 1)
          {
            G4cout << " edge : (" << edgePoints[0] << ", " << edgePoints[1] << ", " << edgePoints[2]
      ...
      if (DEBUG || kVerbose > 0)
      {
        G4cout << " rgb : (" << (int)dcolor[0] << ", " << (int)dcolor[1] << ", " << (int)dcolor[2]
               << ")" << G4endl;
      }
  ...
  ofile.close();
  if (DEBUG || kVerbose > 0) G4cout << ">>>> closed gdd file: " << kFileName << G4endl;
...
bool G4GMocrenIO::storeData3()
{
  ...
  if (DEBUG || kVerbose > 0)
  {
    ...
    G4cout << kPointerToROIData << ", " << kPointerToTrackData << G4endl;
  }
  ...
  if (ntrk != ntcolor)
    if (G4VisManager::GetVerbosity() >= G4VisManager::errors)
      G4cout << "# of track color information must be the same as # of tracks." << G4endl;
...
bool G4GMocrenIO::storeData2()
{
  ...
  */
  ...
  {
    G4cout << "Each pointer to data : " << kPointerToModalityData << ", "
           << kPointerToDoseDistData[0] << ", " << kPointerToROIData << ", " << kPointerToTrackData
           << G4endl;
  }
  ...
  if (!isDoseEmpty())
  {
    ...
    if (DEBUG || kVerbose > 0)
    {
      G4cout << "Dose dist. image size : (" << size[0] << ", " << size[1] << ", " << size[2] << ")"
             << G4endl;
    }
    ...
    if (DEBUG || kVerbose > 0)
    {
      G4cout << "Dose dist. image min., max., scale : " << minmax[0] << ", " << minmax[1] << ", "
             << scale << G4endl;
    }
    ...
    if (DEBUG || kVerbose > 0)
    {
      G4cout << "Dose dist. image relative location : (" << iCenter[0] << ", " << iCenter[1] << ", "
             << iCenter[2] << ")" << G4endl;
    }
  }
...
bool G4GMocrenIO::retrieveData()
{
  ...
  {
    if (G4VisManager::GetVerbosity() >= G4VisManager::errors)
      G4cout << "Cannot open file: " << kFileName << " in G4GMocrenIO::retrieveData()." << G4endl;
    return false;
  }
  ...
  if (std::strncmp(verid, "gMocren", 7) == 0)
  {
    ...
    else
    {
      if (G4VisManager::GetVerbosity() >= G4VisManager::errors)
      {
        G4cout << "Error -- invalid file version : " << (int)ver << G4endl;
        G4cout << "         " << kFileName << G4endl;
      }
      G4Exception("G4GMocrenIO::retrieveDadta()", "gMocren2001", FatalException, "Error.");
    }
  }
...
bool G4GMocrenIO::retrieveData4()
{
  ...
  {
    if (G4VisManager::GetVerbosity() >= G4VisManager::errors)
      G4cout << "Cannot open file: " << kFileName << " in G4GMocrenIO::retrieveData3()." << G4endl;
    return false;
  }
  ...
  if (kPointerToModalityData == 0 && kPointerToDoseDistData.size() == 0 && kPointerToROIData == 0
      && kPointerToTrackData == 0)
  {
    if (DEBUG || kVerbose > 0)
    {
  ...
  */
  ...
  if (kPointerToModalityData != 0)
  {
    ...
    for (int i = 0; i < size[2]; i++)
    {
      ...
      if (DEBUG || kVerbose > 0)
        G4cout << "[" << i << "]" << mimage[(size_t)(psize * 0.55)] << ", ";
    }
    if (DEBUG || kVerbose > 0) G4cout << G4endl;
    delete[] cimage;
    ...
    if (DEBUG || kVerbose > 0)
    {
      ...
      for (int i = 0; i < 10; i++)
        G4cout << "..";
      G4cout << G4endl;
      for (size_t i = kModalityImageDensityMap.size() - 10; i < kModalityImageDensityMap.size();
           i++)
        G4cout << kModalityImageDensityMap[i] << ", ";
      G4cout << G4endl;
    }
  ...
  for (int ndose = 0; ndose < nDoseDist; ndose++)
  {
    ...
    double dminmax[2];
    for (int i = 0; i < 2; i++)
      dminmax[i] = minmax[i] * dscale;
    kDose[ndose].setMinMax(dminmax);

    if (DEBUG || kVerbose > 0)
    {
      G4cout << "Dose dist. image min., max., scale : " << dminmax[0] << ", " << dminmax[1] << ", "
             << scale << G4endl;
    }
    ...
    for (int z = 0; z < size[2]; z++)
    {
      ...
      if (DEBUG || kVerbose > 0)
        G4cout << "[" << z << "]" << dimage[(size_t)(dsize * 0.55)] << ", ";
    ...
    for (int i = 0; i < 3; i++)
      fCenter[i] = (float)iCenter[i];
    kDose[ndose].setCenterPosition(fCenter);

    if (DEBUG || kVerbose > 0)
    {
      G4cout << "Dose dist. image relative location : (" << fCenter[0] << ", " << fCenter[1] << ", "
             << fCenter[2] << ")" << G4endl;
    }
  ...
  if (kPointerToROIData != 0)
  {
    ...
    if (DEBUG || kVerbose > 0)
    {
      G4cout << "ROI image relative location : (" << fCenter[0] << ", " << fCenter[1] << ", "
             << fCenter[2] << ")" << G4endl;
    }
  }
  ...
  if (kPointerToDetectorData != 0)
  {
    ...
    for (int nd = 0; nd < ndet; nd++)
    {
      ...
      for (int ne = 0; ne < nedges; ne++)
      {
        ...
        for (int j = 0; j < 6; j++)
          convertEndian(&cftmp[sizeof(float) * j], edgePoints[j]);
        detector.push_back(edgePoints);
      }

      if (DEBUG || kVerbose > 0)
      {
        G4cout << " first edge : (" << detector[0][0] << ", " << detector[0][1] << ", "
               << detector[0][2] << ") - (" << detector[0][3] << ", " << detector[0][4] << ", "
               << detector[0][5] << ")" << G4endl;
      }
      ...
      if (DEBUG || kVerbose > 0)
      {
        G4cout << " detector color : rgb(" << (int)dcolor[0] << ", " << (int)dcolor[1] << ", "
               << (int)dcolor[2] << G4endl;
      }
  ...
  if (kPointerToTrackData != 0)
  {
    ...
    for (int i = 0; i < ntrk; i++)
    {
      ...
      for (int j = 0; j < 3; j++)
      {
        ...
        if (DEBUG || kVerbose > 0)
          if (i < 10) G4cout << tp[j + 3] << ", ";
      }
      addTrack(tp);
      if (DEBUG || kVerbose > 0)
        if (i < 10) G4cout << G4endl;
...
bool G4GMocrenIO::retrieveData2()
{
  ...
  {
    if (G4VisManager::GetVerbosity() >= G4VisManager::errors)
      G4cout << "Cannot open file: " << kFileName << " in G4GMocrenIO::retrieveData2()." << G4endl;
    return false;
  }
...
void G4GMocrenIO::setID()
{
  ...
  char cmonth[12][4] = {"Jan", "Feb", "Mar", "Apr", "May", "Jun",
                        "Jul", "Aug", "Sep", "Oct", "Nov", "Dec"};
  std::stringstream ss;
  ss << std::setfill('0') << std::setw(2) << ti->tm_hour << ":" << std::setw(2) << ti->tm_min << ":"
     << std::setw(2) << ti->tm_sec << "," << cmonth[ti->tm_mon] << "." << std::setw(2)
...
{
  for (int i = 0; i < 3; i++)
    kVoxelSpacing[i] = _spacing[i];
}
void G4GMocrenIO::getVoxelSpacing(float _spacing[3])
{
  for (int i = 0; i < 3; i++)
    _spacing[i] = kVoxelSpacing[i];
}
...
{
  if (kPointerToDoseDistData.size() == 0 || kPointerToDoseDistData.size() < (size_t)_elem)
    return 0;
  else
...
void G4GMocrenIO::calcPointers4()
{
  ...
  unsigned int pointer =
    1070;  // up to "pointer to the detector data" except for "pointer to the dose dist data"
  int nDoseDist = getNumDoseDist();
  pointer += nDoseDist * 4;
  ...
  if (kVerbose > 0) G4cout << " pointer to the track data :" << kPointerToTrackData << G4endl;
  ...
  if (kVerbose > 0) G4cout << " pointer to the detector data :" << kPointerToDetectorData << G4endl;
}
...
void G4GMocrenIO::calcPointers3()
{
  // pointer to modality data
  unsigned int pointer =
    1066;  // up to "pointer to the track data" except for "pointer to the dose dist data"
  int nDoseDist = getNumDoseDist();
  pointer += nDoseDist * 4;
...
void G4GMocrenIO::setDoseDistMinMax(short _minmax[2], int _num)
{
  ...
  for (int i = 0; i < 2; i++)
    minmax[i] = (double)_minmax[i] * scale;
  ...
  if (_data == NULL)
  {
    if (G4VisManager::GetVerbosity() >= G4VisManager::errors)
      G4cout << "In G4GMocrenIO::getShortDoseDist(), "
             ...
             << "The argument must be allocated array." << G4endl;
    G4Exception("G4GMocrenIO::getShortDoseDist()", "gMocren2002", FatalException, "Error.");
    return;
  }
...
bool G4GMocrenIO::mergeDoseDist(std::vector<class GMocrenDataPrimitive<double>>& _dose)
{
  ...
  {
    if (G4VisManager::GetVerbosity() >= G4VisManager::errors)
      if (kVerbose > 0) G4cout << "merged dose distribution [" << i << "]" << G4endl;
    *itr1 += *itr2;
  }
...
void G4GMocrenIO::copyTracks(std::vector<float*>& _tracks, std::vector<unsigned char*>& _colors)
{
...
void G4GMocrenIO::mergeTracks(std::vector<float*>& _tracks, std::vector<unsigned char*>& _colors)
{
  std::vector<float*>::iterator titr;
                           ...
                           std::vector<unsigned char*>& _color)
{
  if (_num > (int)kTracks.size())
  {
    ...
    G4Exception("G4GMocrenIO::getTrack()", "gMocren2003", FatalException, "Error.");
  }
  ...
  for (int isteps = 0; isteps < nsteps; isteps++)
  {
    ...
    kTracks[_num].getStep(stepPoints[0], stepPoints[1], stepPoints[2], stepPoints[3], stepPoints[4],
                          stepPoints[5], isteps);
...
void G4GMocrenIO::addDetector(std::string& _name, std::vector<float*>& _det,
...
void G4GMocrenIO::getDetector(int _num, std::vector<float*>& _edges,
                              std::vector<unsigned char*>& _color, std::string& _detName)
{
  if (_num > (int)kDetectors.size())
  {
    ...
    G4Exception("G4GMocrenIO::getDetector()", "gMocren2004", FatalException, "Error.");
  }
  ...
  {
    float* edgePoints = new float[6];
    kDetectors[_num].getEdge(edgePoints[0], edgePoints[1], edgePoints[2], edgePoints[3],
                             edgePoints[4], edgePoints[5], ne);
...
void G4GMocrenIO::convertEndian(char* _val, T& _rval)
{
  if ((kLittleEndianOutput && !kLittleEndianInput) ||  // big endian
      (!kLittleEndianOutput && kLittleEndianInput))
  {  // little endian
