...
G4GMocrenFile::G4GMocrenFile()
  : G4VGraphicsSystem("gMocrenFile", "gMocrenFile", "A gMocren file driver (ver.4)",
                      G4VGraphicsSystem::fileWriter),
    // GMOCRENFILE_FEATURES, G4VGraphicsSystem::threeD),
    kViewer(NULL),
    kSceneHandler(NULL),
    kMessenger(new G4GMocrenMessenger())
{
  ;
}
...
G4VViewer* G4GMocrenFile::CreateViewer(G4VSceneHandler& scene, const G4String& name)
{
  kViewer = new G4GMocrenFileViewer((G4GMocrenFileSceneHandler&)scene, *kMessenger, name);
  return kViewer;
}
