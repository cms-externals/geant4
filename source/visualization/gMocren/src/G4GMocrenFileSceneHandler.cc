    ...
    kFlagProcessedInteractiveScorer(false)
{
  ...
  else
  {
    ...
    if (len > 256)
    {
      G4Exception("G4GMocrenFileSceneHandler::G4GMocrenFileSceneHandler(*)", "gMocren1000",
                  FatalException, "Invalid length of string set in G4GMocrenFile_DEST_DIR");
    }
...
void G4GMocrenFileSceneHandler::SetGddFileName()
{
  ...
  for (G4int i = currentNumber; i < kMaxFileNum; i++)
  {
    ...
    filename << kGddDestDir << GDD_FILE_HEADER << std::setw(2) << std::setfill('0') << i << ".wrl";
    strncpy(kGddFileName, filename.str().c_str(), sizeof(kGddFileName) - 1);
    kGddFileName[sizeof(kGddFileName) - 1] = '\0';

    // check validity of the file name
    std::ifstream fin(kGddFileName);
    if (GFDEBUG) G4cout << "FILEOPEN: " << i << " : " << kGddFileName << fin.fail() << G4endl;
    if (!fin)
    {
      // new file
      fin.close();
...
void G4GMocrenFileSceneHandler::EndSavingGdd(void)
{
  ...
  {
    if (GFDEBUG || G4VisManager::GetVerbosity() >= G4VisManager::confirmations)
      G4cout << "*****                 (started) (close " << kGddFileName << ")" << G4endl;
    ...
    kgMocrenIO->setModalityImageSize(kModalitySize);
    if (GFDEBUG)
      G4cout << "gMocren-file driver : modality size : " << kModalitySize[0] << " x "
             << kModalitySize[1] << " x " << kModalitySize[2] << G4endl;
...
void G4GMocrenFileSceneHandler::AddPrimitive(const G4Polyline& polyline)
{
  ...
  if (fProcessing2D)
  {
    ...
    {
      warned = true;
      G4Exception("G4GMocrenFileSceneHandler::AddPrimitive (const G4Polyline&)", "gMocren1001",
                  JustWarning, "2D polylines not implemented.  Ignored.");
    }
    return;
  }
  ...
  if (kbModelingTrajectory)
  {
    ...
    if (!pTrModel)
    {
      G4Exception("G4VSceneHandler::AddCompound(const G4Polyline&)", "gMocren0002", FatalException,
                  "Not a G4TrajectoriesModel.");
    }
    ...
    G4Polyline::const_iterator postitr = preitr;
    postitr++;
    for (; postitr != polyline.end(); preitr++, postitr++)
    {
      ...
      if (GFDEBUG_TRK)
      {
        G4cout << "                       (" << stepPts[0] << ", " << stepPts[1] << ", "
               << stepPts[2] << ") - (" << stepPts[3] << ", " << stepPts[4] << ", " << stepPts[5]
               << ")" << G4endl;
      }
    }
    ...
    if (GFDEBUG_TRK)
    {
      G4cout << "              color  : [" << color.GetRed() << ", " << color.GetGreen() << ", "
...
void G4GMocrenFileSceneHandler::AddPrimitive(const G4Text& text)
{
  if (fProcessing2D)
  {
    ...
    {
      warned = true;
      G4Exception("G4GMocrenFileSceneHandler::AddPrimitive (const G4Text&)", "gMocren1002",
                  JustWarning, "2D text not implemented.  Ignored.");
    }
    return;
  }
...
void G4GMocrenFileSceneHandler::AddPrimitive(const G4Circle& mark_circle)
{
  ...
  if (fProcessing2D)
  {
    ...
    {
      warned = true;
      G4Exception("G4GMocrenFileSceneHandler::AddPrimitive (const G4Circle&)", "gMocren1003",
                  JustWarning, "2D circles not implemented.  Ignored.");
    }
    return;
  }
...
void G4GMocrenFileSceneHandler::AddPrimitive(const G4Square& mark_square)
{
  ...
  if (fProcessing2D)
  {
    ...
    {
      warned = true;
      G4Exception("G4GMocrenFileSceneHandler::AddPrimitive (const G4Square&)", "gMocren1004",
                  JustWarning, "2D squares not implemented.  Ignored.");
    }
    return;
  }
...
void G4GMocrenFileSceneHandler::AddPrimitive(const G4Polyhedron& polyhedron)
{
  ...
  if (fProcessing2D)
  {
    ...
    {
      warned = true;
      G4Exception("G4GMocrenFileSceneHandler::AddPrimitive (const G4Polyhedron&)", "gMocren1005",
                  JustWarning, "2D polyhedra not implemented.  Ignored.");
    }
    return;
  }
  ...
  for (G4int f = polyhedron.GetNoFacets(); f; f--)
  {
    ...
    switch (i)
    {
      ...
      default:
        if (GFDEBUG || G4VisManager::GetVerbosity() >= G4VisManager::confirmations)
          G4cout << "ERROR G4GMocrenFileSceneHandler::AddPrimitive(G4Polyhedron)" << G4endl;
        ...
        if (pPVModel)
          if (G4VisManager::GetVerbosity() >= G4VisManager::confirmations)
            G4cout << "Volume " << pPVModel->GetCurrentPV()->GetName() << ", Solid "
...
void G4GMocrenFileSceneHandler::AddSolid(const G4Box& box)
{
  ...
  if (GFDEBUG_DET > 0)
    G4cout << "G4GMocrenFileSceneHandler::AddSolid(const G4Box&)  : " << box.GetName() << G4endl;
  ...
  if (GFDEBUG_DET > 1)
  {
    ...
    {  // # of edges is 12.
      ...
      if (next == 0) break;
      G4cout << "    (" << v1.x() << ", " << v1.y() << ", " << v1.z() << ") - (" << v2.x() << ", "
             << v2.y() << ", " << v2.z() << ") [" << next << "]" << G4endl;
    }
    delete poly;
  }
  ...
  if (kFlagParameterization != 2)
  {
    ...
    if (pScrMan)
    {
      ...
      if (GFDEBUG_DET > 0)
        G4cout << "   G4ScoringManager::FindMesh() : " << volName << " - " << bMesh << G4endl;
    }
  }
  ...
  if (box.GetName() == volName)
  {
    ...
    if (!pv[0])
    {
      G4Exception("G4GMocrenFileSceneHandler::AddSolid( const G4Box& box )", "gMocren0003",
                  FatalException, "Unexpected volume.");
    }
    ...
    switch (axis)
    {
      ...
      case kZAxis:
        ...
        break;
      default:
        G4Exception("G4GMocrenFileSceneHandler::AddSolid( const G4Box& box )", "gMocren0004",
                    FatalException, "Error.");
    }
    kNestedVolumeNames.push_back(pv[0]->GetName());
    if (GFDEBUG_DET)
      G4cout << "        daughter name :  " << pv[0]->GetName() << "   # : " << nDaughters[0]
             << G4endl;
    ...
    if (GFDEBUG_DET)
    {
      ...
      {
        G4cout << "# of daughters : " << pv[0]->GetLogicalVolume()->GetNoDaughters() << G4endl;
      }
      else
      {
    ...
    if (kFlagParameterization == 0)
    {
      ...
      if (pv[1])
      {
        ...
        switch (axis)
        {
          ...
          case kZAxis:
            ...
            break;
          default:
            G4Exception("G4GMocrenFileSceneHandler::AddSolid( const G4Box& box )", "gMocren0007",
                        FatalException, "Error.");
        }
        ...
        if (pv[2])
        {
          ...
          if (nDaughters[2] > 1)
          {
            ...
            if (nestPara == NULL)
              G4Exception("G4GMocrenFileSceneHandler::AddSolid( const G4Box& box )", "gMocren0008",
                          FatalException, "Non-nested parameterisation");
            ...
            if (GFDEBUG_DET) G4cout << trans0 << " - " << trans1 << " - " << diff << G4endl;
            ...
            if (diff.x() != 0.)
              ...
              G4Exception("G4GMocrenFileSceneHandler::AddSolid( const G4Box& box )", "gMocren0009",
                          FatalException, "Unexpected nested parameterisation");
          }
        }
      }
      ...
      if (nestPara != NULL)
      {
        ...
        for (G4int n0 = 0; n0 < nDaughters[0]; n0++)
        {
          for (G4int n1 = 0; n1 < nDaughters[1]; n1++)
          {
            for (G4int n2 = 0; n2 < nDaughters[2]; n2++)
            {
              ...
              if (GFDEBUG_DET)
                G4cout << "   retrieve volume : copy # : " << n0 << ", " << n1 << ", " << n2
                       << G4endl;
              G4Material* mat = nestPara->ComputeMaterial(pv[2], n2, touch);
              delete touch;
              G4double dens = mat->GetDensity() / (g / cm3);

              if (GFDEBUG_DET) G4cout << "           density :" << dens << " [g/cm3]" << G4endl;
              ...
              if (GFDEBUG_DET)
                G4cout << "              size : " << tbox.GetXHalfLength() / mm << " x "
                       << tbox.GetYHalfLength() / mm << " x " << tbox.GetZHalfLength() / mm
                       << " [mm3]" << G4endl;
        ...
        kVolumeSize.set(box.GetXHalfLength() * 2 / mm, box.GetYHalfLength() * 2 / mm,
                        box.GetZHalfLength() * 2 / mm);
        // mesh size
        if (!kbSetModalityVoxelSize)
        {
          G4float spacing[3] = {static_cast<G4float>(2 * xyz[0]), static_cast<G4float>(2 * xyz[1]),
      ...
      {
        if (GFDEBUG_DET) G4cout << pv[2]->GetName() << G4endl;
        G4Exception("G4GMocrenFileSceneHandler::AddSolid( const G4Box& box )", "gMocren0011",
                    FatalException, "Non-nested parameterisation");
      }
      ...
      if (GFDEBUG_DET > 1)
      {
        if (pPVModel->GetCurrentPV()->IsParameterised())
        {
          ...
          {
            G4cout << pPVModel->GetDrawnPVPath()[i].GetPhysicalVolume()->GetName() << " [param:"
                   << pPVModel->GetDrawnPVPath()[i].GetPhysicalVolume()->IsParameterised()
                   << ",rep:" << pPVModel->GetDrawnPVPath()[i].GetPhysicalVolume()->IsReplicated();
            if (pPVModel->GetDrawnPVPath()[i].GetPhysicalVolume()->GetParameterisation())
            {
              G4cout << ",nest:"
                     << pPVModel->GetDrawnPVPath()[i]
                          .GetPhysicalVolume()
                          ->GetParameterisation()
                          ->IsNested();
            }
            G4cout << ",copyno:" << pPVModel->GetDrawnPVPath()[i].GetPhysicalVolume()->GetCopyNo();
            G4cout << "] - ";
          }
          G4cout << G4endl;
          ...
          G4Box* pbox = dynamic_cast<G4Box*>(pPVModel->GetDrawnPVPath()[npvp - 2]
                                               .GetPhysicalVolume()
                                               ->GetLogicalVolume()
                                               ->GetSolid());
          if (pbox)
          {
            ...
            G4cout << "     mother size ["
                   ...
                   << "] : " << pareDims[0] << " x " << pareDims[1] << " x " << pareDims[2]
                   << " [mm3]" << G4endl;
          }
          G4double paraDims[3];
          G4Box* boxP = dynamic_cast<G4Box*>(pPVModel->GetDrawnPVPath()[npvp - 1]
                                               .GetPhysicalVolume()
                                               ->GetLogicalVolume()
                                               ->GetSolid());
          if (boxP)
          {
            ...
            G4cout << "     parameterised volume? ["
                   ...
                   << "] : " << paraDims[0] << " x " << paraDims[1] << " x " << paraDims[2]
                   << " [mm3]  : " << G4int(pareDims[0] / paraDims[0]) << " x "
                   << G4int(pareDims[1] / paraDims[1]) << " x " << G4int(pareDims[2] / paraDims[2])
                   << G4endl;
          }
          else
          {
    ...
    {  // G4PhantomParameterisation based geom. construnction
      ...
      if (phantomPara == NULL)
      {
        G4Exception("G4GMocrenFileSceneHandler::AddSolid( const G4Box& box )", "gMocren0012",
  ...
  if (!kFlagProcessedInteractiveScorer)
  {
    ...
    if (pScrMan)
    {
      G4ScoringBox* scoringBox = dynamic_cast<G4ScoringBox*>(pScrMan->FindMesh(volName));
      ...
      if (scoringBox)
      {
        ...
        if (GFDEBUG_DET > 1)
        {
          G4cout << "Interactive Scorer : size - " << boxSize.x() / cm << " x " << boxSize.y() / cm
                 << " x " << boxSize.z() / cm << " [cm3]" << G4endl;
          G4cout << "Interactive Scorer : # voxels - " << nVoxels[0] << " x " << nVoxels[1] << " x "
                 << nVoxels[2] << G4endl;
        }
        kVolumeSize.set(boxSize.x() * 2, boxSize.y() * 2, boxSize.z() * 2);
  ...
  if (!kMessenger.getDrawVolumeGrid())
  {
    ...
    {  // phantom paramemterisation

      G4String volDSolidName;
      if (volPV)
      {
        volDSolidName =
          volPV->GetLogicalVolume()->GetDaughter(0)->GetLogicalVolume()->GetSolid()->GetName();
      }
...
//----- Add tubes
void G4GMocrenFileSceneHandler::AddSolid(const G4Tubs& tubes)
{
  if (GFDEBUG || G4VisManager::GetVerbosity() >= G4VisManager::confirmations)
  ...
  if (GFDEBUG_DET > 0)
  {
    ...
    {
      G4cout << "    (" << poly->GetVertex(i).x() << ", " << poly->GetVertex(i).y() << ", "
...
void G4GMocrenFileSceneHandler::AddCompound(const G4VTrajectory& traj)
{
  ...
  if (GFDEBUG_TRK)
  {
    ...
    if (!pTrModel)
    {
      G4Exception("G4VSceneHandler::AddCompound(const G4VTrajectory&)", "gMocren0013",
                  FatalException, "Not a G4TrajectoriesModel.");
    }
    else
    {
      traj.DrawTrajectory();
...
void G4GMocrenFileSceneHandler::AddCompound(const G4VHit& hit)
{
  ...
  */
  ...
  else
  {
    ...
    {
      if (GFDEBUG_HIT)
        G4cout << " Hit : index(" << id.x << ", " << id.y << ", " << id.z << ")" << G4endl;
    ...
    {
      G4Exception("G4GMocrenFileSceneHandler::AddCompound(const G4VHit &)", "gMocren0014",
                  FatalException, "Error");
    }
...
void G4GMocrenFileSceneHandler::AddCompound(const G4THitsMap<G4double>& hits)
{
  ...
  */
  ...
  if (GFDEBUG_HIT)
  {
    ...
    G4cout << " dimension: " << kNestedVolumeDimension[0] << " x " << kNestedVolumeDimension[1]
           << " x " << kNestedVolumeDimension[2] << G4endl;
  ...
  */
  ...
  if (GFDEBUG_HIT)
  {
    ...
    for (; itr != map->end(); itr++)
    {
      GetNestedVolumeIndex(itr->first, id);
      G4cout << "[" << itr->first << "] "
             << "(" << id[0] << "," << id[1] << "," << id[2] << ")" << itr->second->sum_wx()
             << ", ";
    }
    G4cout << G4endl;
  }
...
G4bool G4GMocrenFileSceneHandler::IsVisible()
{
  ...
  const G4VisAttributes* pVisAttribs = fpViewer->GetApplicableVisAttributes(fpVisAttribs);
...
void G4GMocrenFileSceneHandler::AddDetector(const G4VSolid& solid)
{
  ...
  if (GFDEBUG_DET > 1) G4cout << "0 Detector name : " << detector.name << G4endl;
  ...
  for (G4int i = 0; i < 3; i++)
    detector.color[i] = uccolor[i];
  //
  kDetectors.push_back(detector);

  if (GFDEBUG_DET > 1)
  {
    G4cout << "0     color:   (" << (G4int)uccolor[0] << ", " << (G4int)uccolor[1] << ", "
           << (G4int)uccolor[2] << ")" << G4endl;
  }
}
...
void G4GMocrenFileSceneHandler::ExtractDetector()
{
  ...
  for (; itr != kDetectors.end(); itr++)
  {
    ...
    if (GFDEBUG_DET > 1) G4cout << "Detector name : " << detname << G4endl;
    ...
    // delete poly;
    //  detector color
    unsigned char uccolor[3] = {itr->color[0], itr->color[1], itr->color[2]};
    //
    kgMocrenIO->addDetector(detname, dedges, uccolor);
    for (G4int i = 0; i < nedges; i++)
    {  // # of edges is 12.
      delete[] dedges[i];
    }
    dedges.clear();
    ...
    if (GFDEBUG_DET > 1)
    {
      G4cout << "    color:   (" << (G4int)uccolor[0] << ", " << (G4int)uccolor[1] << ", "
             << (G4int)uccolor[2] << ")" << G4endl;
    }
  }
}
...
{
  if (kNestedVolumeDimension[0] == 0 || kNestedVolumeDimension[1] == 0
...
//-- --//
G4GMocrenFileSceneHandler::Detector::Detector() : polyhedron(0)
{
...
//-- --//
G4GMocrenFileSceneHandler::Index3D::Index3D() : x(0), y(0), z(0)
{
  ;
}
...
G4GMocrenFileSceneHandler::Index3D::Index3D(G4int _x, G4int _y, G4int _z) : x(_x), y(_y), z(_z)
{
  ;
}
G4bool G4GMocrenFileSceneHandler::Index3D::operator<(const Index3D& _right) const
{
  ...
  {
    if (y < static_cast<Index3D>(_right).y)
      return true;
    else if (y == _right.y)
