...
G4OpenInventorViewer::G4OpenInventorViewer(G4OpenInventorSceneHandler& sceneHandler,
                                           const G4String& name)
...
//,fCameraSensor(0)
{
  ...
  fInteractorManager =
    ((G4OpenInventor*)fG4OpenInventorSceneHandler.GetGraphicsSystem())->GetInteractorManager();
...
G4bool G4OpenInventorViewer::CompareForKernelVisit(G4ViewParameters& vp)
{
  if ((vp.GetDrawingStyle() != fVP.GetDrawingStyle())
      ...
      || (vp.IsAuxEdgeVisible() != fVP.IsAuxEdgeVisible()) || (vp.IsCulling() != fVP.IsCulling())
      ...
      || (vp.GetDefaultVisAttributes()->GetColour() != fVP.GetDefaultVisAttributes()->GetColour())
  ...
  if (vp.IsSpecialMeshRendering() && (vp.GetSpecialMeshVolumes() != fVP.GetSpecialMeshVolumes()))
    return true;
...
void G4OpenInventorViewer::SetView()
{
  ...
  */
  ...
  // viewer camera setup :
  camera->position.setValue((float)cameraPosition.x(), (float)cameraPosition.y(),
                            (float)cameraPosition.z());

  SbVec3f sbTarget((float)target.x(), (float)target.y(), (float)target.z());
  SbVec3f sbUp((float)up.x(), (float)up.y(), (float)up.z());
  sbUp.normalize();
...
void G4OpenInventorViewer::pointAt(SoCamera* camera, const SbVec3f& targetpoint,
...
void G4OpenInventorViewer::lookAt(SoCamera* camera, const SbVec3f& dir, const SbVec3f& up)
{
  SbVec3f z = -dir;
  SbVec3f y = up;
  SbVec3f x = y.cross(z);
...
void G4OpenInventorViewer::lookedAt(SoCamera* camera, SbVec3f& dir, SbVec3f& up)
{
...
void G4OpenInventorViewer::GroupCameraSensorCB(void* aThis, SoSensor* aSensor)
{
  ...
  if (node->isOfType(SoCamera::getClassTypeId()))
  {
    ...
    This->fVP.SetViewpointDirection(G4Vector3D(-direction[0], -direction[1], -direction[2]));
...
void G4OpenInventorViewer::SelectionCB(void* aThis, SoPath* aPath)
{
  G4OpenInventorViewer* This = (G4OpenInventorViewer*)aThis;
...
void G4OpenInventorViewer::WritePixmapPostScript(const G4String& aFile)
{
  ...
  {
    G4cout << G4String(fSoImageWriter->fileName.getValue().getString()) << " produced." << G4endl;
  }
  else
  {
    G4cout << G4String(fSoImageWriter->fileName.getValue().getString()) << " not produced."
           << G4endl;
  }
}
...
static void CountTrianglesCB(void* userData, SoCallbackAction*, const SoPrimitiveVertex*,
...
static void CountLineSegmentsCB(void* userData, SoCallbackAction*, const SoPrimitiveVertex*,
...
void G4OpenInventorViewer::SceneGraphStatistics()
{
  ...
  callbackAction.addTriangleCallback(SoShape::getClassTypeId(), CountTrianglesCB, (void*)&counter);
  callbackAction.addLineSegmentCallback(SoShape::getClassTypeId(), CountLineSegmentsCB,
                                        (void*)&counter);
  callbackAction.addPointCallback(SoShape::getClassTypeId(), CountPointsCB, (void*)&counter);
