...
#define GEN_VERTEX(pv, x, y, z, s, t, nx, ny, nz)         \
  ...
  double deltaPhi = pDPhi.getValue() / NPHI, phi0 = pSPhi.getValue(),
         phi1 = phi0 + pDPhi.getValue();
...
void SoTubs::updateChildren()
{
  ...
  for (int np = 0; np < NPOINTS; np++)
    theCoordinates->point.set1Value(np, points[np][0], points[np][1], points[np][2]);
  for (int ni = 0; ni < NINDICES; ni++)
    theFaceSet->coordIndex.set1Value(ni, indices[ni]);
  for (int nf = 0; nf < NFACES; nf++)
    theNormals->vector.set1Value(nf, normals[nf][0], normals[nf][1], normals[nf][2]);
