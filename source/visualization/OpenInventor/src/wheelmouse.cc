...
extern "C"
{
  ...
  // static void mouseScroll(Widget, XtPointer client_data, XEvent *event)
  {
    ...
    XmScrollBarGetValues(sb, &value_return, &slider_size_return, &increment_return,
    ...
    {
      value_return -= step;
      if (value_return < 0) value_return = 0;
    }
    else if (event->xbutton.button == Button5)
    {
      ...
      value_return += step;
      if (value_return > count - slider_size_return) value_return = count - slider_size_return;
    }

    /* finally perform scrolling with the calculated step */
    if (event->xbutton.button == Button4 || event->xbutton.button == Button5)
    {
      XmScrollBarSetValues(sb, value_return, slider_size_return, increment_return,
                           page_increment_return, True);
    }
  }
  ...
  void xmAddMouseEventHandler(Widget w)
  {
    ...
    XtAddEventHandler(w, ButtonReleaseMask, False, (XtEventHandler)mouseScroll, wid);
    /* and for the scrollbar itself */
    XtAddEventHandler(wid, ButtonReleaseMask, False, (XtEventHandler)mouseScroll, wid);
  }
