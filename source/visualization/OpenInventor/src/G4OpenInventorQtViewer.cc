...
G4OpenInventorQtViewer::G4OpenInventorQtViewer(G4OpenInventorSceneHandler& sceneHandler,
                                               const G4String& name)
...
void G4OpenInventorQtViewer::Initialise()
{
  ...
  connect(EtcEraseDetector, SIGNAL(triggered()), this, SLOT(EtcEraseDetectorCB()));
  etcmenu->addAction(EtcEraseDetector);

  EtcEraseEvent = new QAction("Erase event", this);
  EtcEraseEvent->setFont(*font);
  connect(EtcEraseEvent, SIGNAL(triggered()), this, SLOT(EtcEraseEventCB()));
  ...
  connect(EtcSetReducedWireframe, SIGNAL(triggered()), this, SLOT(EtcSetReducedWireframeCB()));
  ...
  connect(EtcSetFullWireframe, SIGNAL(triggered()), this, SLOT(EtcSetFullWireframeCB()));
  etcmenu->addAction(EtcSetFullWireframe);

  EtcVisibMInvisibD = new QAction("Visible mothers + invisible daughters", this);
  EtcVisibMInvisibD->setFont(*font);
  connect(EtcVisibMInvisibD, SIGNAL(triggered()), this, SLOT(EtcVisibMInvisibDCB()));
...
void G4OpenInventorQtViewer::FileWritePSCB()
{
  ...
  SbBool superimpState = fViewer->getSuperimpositionEnabled(fViewer->superimposition);
  fViewer->setSuperimpositionEnabled(fViewer->superimposition, FALSE);
  WritePostScript();
  if (superimpState) fViewer->setSuperimpositionEnabled(fViewer->superimposition, TRUE);
}
...
void G4OpenInventorQtViewer::EtcEraseDetectorCB()
{
  //   G4cout << "G4OIQtViewer: Etc: Erase Detector CALLBACK" << G4endl;
  EraseDetector();
}

void G4OpenInventorQtViewer::EtcEraseEventCB()
{
