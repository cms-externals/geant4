...
G4OpenInventorXtExtended::G4OpenInventorXtExtended()
  : G4OpenInventor("OpenInventorXtExtended", "OIXE", G4VGraphicsSystem::threeD), fInited(false)
{
...
void G4OpenInventorXtExtended::Initialize()
{
  ...
  GetInteractorManager()->RemoveDispatcher(G4Xt::xt_dispatch_event);
  // Coverity gcc8 cast warning
  //    RemoveDispatcher((G4DispatchFunction)XtDispatchEvent);
  GetInteractorManager()->AddDispatcher(soxt_dispatch_event);
  // Coverity gcc8 cast warning
