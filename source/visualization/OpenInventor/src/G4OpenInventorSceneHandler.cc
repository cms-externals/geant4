...
G4OpenInventorSceneHandler::G4OpenInventorSceneHandler(G4OpenInventor& system, const G4String& name)
...
void G4OpenInventorSceneHandler::AddPrimitive(const G4Polyline& line)
{
  if (fProcessing2D)
  {
    ...
    {
      warned = true;
      G4Exception("G4OpenInventorSceneHandler::AddPrimitive (const G4Polyline&)",
                  "OpenInventor-0001", JustWarning, "2D polylines not implemented.  Ignored.");
    }
    return;
  }

  // Get vis attributes - pick up defaults if none.
  const G4VisAttributes* pVA = fpViewer->GetApplicableVisAttributes(line.GetVisAttributes());
  ...
  {
    pCoords[iPoint].setValue((G4float)line[iPoint].x(), (G4float)line[iPoint].y(),
...
void G4OpenInventorSceneHandler::AddPrimitive(const G4Polymarker& polymarker)
{
  if (fProcessing2D)
  {
    ...
    {
      warned = true;
      G4Exception("G4OpenInventorSceneHandler::AddPrimitive (const G4Polymarker&)",
                  "OpenInventor-0002", JustWarning, "2D polymarkers not implemented.  Ignored.");
    }
    return;
  }

  // Get vis attributes - pick up defaults if none.
  const G4VisAttributes* pVA = fpViewer->GetApplicableVisAttributes(polymarker.GetVisAttributes());
  ...
  {
    points[iPoint].setValue((G4float)polymarker[iPoint].x(), (G4float)polymarker[iPoint].y(),
  ...
  // Loads G4Atts for picking...
  if (fpViewer->GetViewParameters().IsPicking()) LoadAtts(polymarker, markerSet);
...
void G4OpenInventorSceneHandler::AddPrimitive(const G4Text& text)
{
  if (fProcessing2D)
  {
    ...
    {
      warned = true;
      G4Exception("G4OpenInventorSceneHandler::AddPrimitive (const G4Text&)", "OpenInventor-0003",
  ...
  SoMaterial* material = fStyleCache->getMaterial(
    (G4float)c.GetRed(), (G4float)c.GetGreen(), (G4float)c.GetBlue(), (G4float)(1 - c.GetAlpha()));
  ...
  switch (text.GetLayout())
  {
    ...
    case G4Text::left:
      ...
      break;
    case G4Text::centre:
      g4String->justification = SoText2::CENTER;
      break;
    case G4Text::right:
      g4String->justification = SoText2::RIGHT;
      break;
  }
...
void G4OpenInventorSceneHandler::AddCircleSquare(G4OIMarker markerType, const G4VMarker& marker)
{
  if (fProcessing2D)
  {
    static G4bool warned = false;
    ...
    {
      warned = true;
      G4Exception("G4OpenInventorSceneHandler::AddCircleSquare", "OpenInventor-0004", JustWarning,
  ...
  // Get vis attributes - pick up defaults if none.
  const G4VisAttributes* pVA = fpViewer->GetApplicableVisAttributes(marker.GetVisAttributes());
  ...
  points[0].setValue((G4float)centre.x(), (G4float)centre.y(), (G4float)centre.z());
  SoCoordinate3* coordinate3 = new SoCoordinate3;
...
void G4OpenInventorSceneHandler::AddPrimitive(const G4Polyhedron& polyhedron)
{
  ...
  if (fProcessing2D)
  {
    ...
    {
      warned = true;
      G4Exception("G4OpenInventorSceneHandler::AddPrimitive (const G4Polyhedron&)",
                  "OpenInventor-0005", JustWarning, "2D polyhedra not implemented.  Ignored.");
    }
    return;
  }
  ...
  // Loads G4Atts for picking...
  if (fpViewer->GetViewParameters().IsPicking()) LoadAtts(polyhedron, soPolyhedron);
...
void G4OpenInventorSceneHandler::GeneratePrerequisites()
{
  ...
  if (pPVModel)
  {
    ...
    if (pCurrentLV->GetNoDaughters() != 0 || pCurrentPV->IsReplicated())
    {  //????Don't understand this???? JA
      ...
      SoSeparator* fullSeparator = (SoSeparator*)detectorTreeKit->getPart("fullSeparator", TRUE);
      fullSeparator->renderCaching = SoSeparator::OFF;

      if (fPreviewAndFull)
        detectorTreeKit->setPreviewAndFull();
      else
        detectorTreeKit->setPreview(TRUE);
      ...
      // Drawing style...
      G4ViewParameters::DrawingStyle drawing_style = GetDrawingStyle(pApplicableVisAttribs);
      ...
      SoMaterial* material = fStyleCache->getMaterial((G4float)red, (G4float)green, (G4float)blue,
                                                      (G4float)transparency);
      detectorTreeKit->setPart("appearance.material", material);

      SoLightModel* lightModel =
        fModelingSolid ? fStyleCache->getLightModelPhong() : fStyleCache->getLightModelBaseColor();
      ...
      if (MotherVolume)
      {
        ...
        else
        {
          ...
          // geometry tree.  Error!
          G4warn << "ERROR: G4OpenInventorSceneHandler::GeneratePrerequisites: Mother "
                 << ri->GetPhysicalVolume()->GetName() << ':' << ri->GetCopyNo()
...
void G4OpenInventorSceneHandler::AddProperties(const G4VisAttributes* visAtts)
{
  ...
  const G4VisAttributes* pApplicableVisAttribs = fpViewer->GetApplicableVisAttributes(visAtts);
  ...
  SoMaterial* material =
    fStyleCache->getMaterial((G4float)red, (G4float)green, (G4float)blue, (G4float)transparency);
  fCurrentSeparator->addChild(material);
...
void G4OpenInventorSceneHandler::AddTransform(const G4Point3D& translation)
{
  ...
  G4OpenInventorTransform3D oiTran(fObjectTransformation * G4Translate3D(translation));
  SbMatrix* sbMatrix = oiTran.GetSbMatrix();

  const G4Vector3D scale = fpViewer->GetViewParameters().GetScaleFactor();
  SbMatrix sbScale;
  sbScale.setScale(SbVec3f((G4float)scale.x(), (G4float)scale.y(), (G4float)scale.z()));
