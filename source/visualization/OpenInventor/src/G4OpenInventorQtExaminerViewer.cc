...
// Constructor
G4OpenInventorQtExaminerViewer::G4OpenInventorQtExaminerViewer(QWidget* parent, const char* name,
                                                               SbBool embed,
...
void G4OpenInventorQtExaminerViewer::construct(const SbBool)
{
  ...
  static const char* superimposed[] = {
    "#Inventor V2.1 ascii", "", "Separator ", "{", " MaterialBinding ", " {",
    "         value OVERALL", " }", "         OrthographicCamera ", " {", "         height 1",
    "         nearDistance 0", "         farDistance 1", " }",
    "         DEF soxt->callback Callback { }", "         Separator ", " {",
    "         DEF soxt->translation Translation ", "         {",
    "                 translation 0 0 0", "     }", "     DEF soxt->scale Scale ", "         {",
    "                 scaleFactor 1 1 1", "     }", "         DEF soxt->geometry Coordinate3 ",
    "         {", "             point ", "                 [",
    ...
    "                 whichChild -3", "                 Material ", "                 {",
    "                    emissiveColor 0 0 0", "             }", "                 IndexedFaceSet ",
    "                 {", "                 coordIndex ", "                         [",
    "                                 12, 11, 10, 9, -1", "                         ]",
    "         }", "                  }",
    // the coordinate system
    "         DEF soxt->axisSwitch Switch ", "         {", "                 whichChild -3",
    "                 BaseColor ", "                 {", "                     rgb 1 1 1",
    "                 }", "                 IndexedLineSet ", "                 {",
    "                         coordIndex ", "                         [",
    "                                 0, 2, -1,", "                                 3, 5, -1,",
  ...
  animSpeedOutlineSwitch =
    (SoSwitch*)getSuperimpositionNode(superimposition, "soxt->animSpeedOutlineSwitch");
  ...
  mouseOverTransLogName =
    (SoTranslation*)getSuperimpositionNode(superimposition, "soxt->mouseOverTransLogName");
  mouseOverFontLogName =
    (SoFont*)getSuperimpositionNode(superimposition, "soxt->mouseOverFontLogName");
  mouseOverTextLogName =
    (SoText2*)getSuperimpositionNode(superimposition, "soxt->mouseOverTextLogName");
  mouseOverTransSolid =
    (SoTranslation*)getSuperimpositionNode(superimposition, "soxt->mouseOverTransSolid");
  mouseOverFontSolid = (SoFont*)getSuperimpositionNode(superimposition, "soxt->mouseOverFontSolid");
  mouseOverTextSolid =
    (SoText2*)getSuperimpositionNode(superimposition, "soxt->mouseOverTextSolid");
  mouseOverTransMaterial =
    (SoTranslation*)getSuperimpositionNode(superimposition, "soxt->mouseOverTransMaterial");
  mouseOverFontMaterial =
    (SoFont*)getSuperimpositionNode(superimposition, "soxt->mouseOverFontMaterial");
  mouseOverTextMaterial =
    (SoText2*)getSuperimpositionNode(superimposition, "soxt->mouseOverTextMaterial");
  mouseOverTransZPos =
    (SoTranslation*)getSuperimpositionNode(superimposition, "soxt->mouseOverTransZPos");
...
{
  if (!parent)
    SoDebugError::post("G4OpenInventorQtExaminerViewer::buildWidget", "Error: Parent is null.");
  ...
  connect(ToolsAnimateRefParticle, SIGNAL(triggered()), this, SLOT(ToolsAnimateRefParticleCB()));
  ...
  connect(ToolsRefPathStart, SIGNAL(triggered()), this, SLOT(ToolsRefPathStartCB()));
  toolsmenu->addAction(ToolsRefPathStart);

  ToolsRefPathInvert = new QAction("Invert Ref Path", this);
  ToolsRefPathInvert->setFont(*font);
  connect(ToolsRefPathInvert, SIGNAL(triggered()), this, SLOT(ToolsRefPathInvertCB()));
  ...
  connect(saveViewPtButton, SIGNAL(clicked()), this, SLOT(SaveViewPtCB()));
  ...
  connect(nextViewPtButton, SIGNAL(clicked()), this, SLOT(NextViewPtCB()));
  ...
  connect(prevViewPtButton, SIGNAL(clicked()), this, SLOT(PrevViewPtCB()));
  ...
  connect(abbrOutputButton, SIGNAL(toggled(bool)), this, SLOT(AbbrOutputCB(bool)));
  ...
  connect(pickRefPathButton, SIGNAL(clicked()), this, SLOT(PickRefPathCB()));
  ...
  connect(switchWireFrameButton, SIGNAL(toggled(bool)), this, SLOT(SwitchWireFrameCB(bool)));
  ...
  connect(switchAxesButton, SIGNAL(toggled(bool)), this, SLOT(SwitchAxesCB(bool)));
  ...
  connect(detachButton, SIGNAL(clicked()), this, SLOT(DetachCB()));
...
FLY mode (requires Reference Path):\n\n\
  ...
  // SIGNALS
  connect(AuxWindowDialog->listWidget, SIGNAL(itemClicked(QListWidgetItem*)), this,
          SLOT(LoadBookmarkCB(QListWidgetItem*)));
  connect(AuxWindowDialog->listWidget1, SIGNAL(itemClicked(QListWidgetItem*)), this,
          SLOT(LookAtSceneElementCB(QListWidgetItem*)));
  connect(AuxWindowDialog->pushButton_2, SIGNAL(clicked()), this, SLOT(DeleteBookmarkCB()));
  connect(AuxWindowDialog->pushButton_3, SIGNAL(clicked()), this, SLOT(RenameBookmarkCB()));
  connect(AuxWindowDialog->pushButton, SIGNAL(clicked()), this, SLOT(SortBookmarksCB()));
...
void G4OpenInventorQtExaminerViewer::setupSceneGraph()
{
  ...
  if (root != nullptr)
  {
    ...
    moCB->addEventCallback(SoLocation2Event::getClassTypeId(), mouseoverCB,
...
{
  if (!searcher) searcher = new SoSearchAction;
  searcher->reset();
...
{
  if (closure) ((G4OpenInventorQtExaminerViewer*)closure)->superimpositionEvent(action);
}
...
void G4OpenInventorQtExaminerViewer::superimpositionEvent(SoAction* action)
{
  ...
  float xInfo, yInfo, xLogName, yLogName, xSolid, ySolid, xMaterial, yMaterial, xZPos, yZPos;
    ...
    if (node->getTypeId() == SoLineSet::getClassTypeId())
    {
      ...
      else if (This->abbrOutputFlag)
      {
        ...
        else
        {
          ...
          G4warn << "SoNode : " << node << " SoType : " << cls << " name : " << name << G4endl;
...
void G4OpenInventorQtExaminerViewer::setReferencePath(SoLineSet* lineset, SoCoordinate3* coords,
                                                      bool append)
{
  ...
  if (!append) refParticleTrajectory.clear();
        ...
        for (std::size_t j = 0; j < attHolder->GetAttDefs().size(); ++j)
        {
          ...
          if (idx != std::string::npos)
          {
            ...
            if (num == 1)
            {
              ...
              if (std::isdigit(nextChar)) break;  // Not a primary track, continue with next track
...
void G4OpenInventorQtExaminerViewer::distanceToTrajectory(const SbVec3f& q, float& dist,
...
{
  if (refParticleTrajectory.empty()) return;
  ...
  for (itEl = sceneElements.begin(); itEl != sceneElements.end(); ++itEl)
  {
...
void G4OpenInventorQtExaminerViewer::LookAtSceneElementCB(QListWidgetItem* item)
{
  // FWJ DEBUG
  ...
  parseString<int>(counter, elementField.substr(idxUnderscore + 1, idx), error);
...
void G4OpenInventorQtExaminerViewer::ToolsRefPathStartCB()
{
  ...
  if (currentState == ROTATING) return;
  if (currentState == ANIMATION || currentState == REVERSED_ANIMATION
      || currentState == PAUSED_ANIMATION)
  {
    if (animateSensor->isScheduled()) animateSensor->unschedule();
...
{
  std::reverse(refParticleTrajectory.begin(), refParticleTrajectory.end());
  setReferencePathZPos();
  sortElements();
}
...
void G4OpenInventorQtExaminerViewer::sceneChangeCB(void* userData, SoSensor*)
{
  ...
  G4OpenInventorQtExaminerViewer* This = (G4OpenInventorQtExaminerViewer*)userData;
  if (This->newEvents)
  {
...
void G4OpenInventorQtExaminerViewer::addViewPoints()
{
  ...
  {
#if QT_VERSION < 0x060000
    new QListWidgetItem(viewPtList[i].viewPtName, AuxWindowDialog->listWidget);
#else
    new QListWidgetItem(viewPtList[i].viewPtName.c_str(), AuxWindowDialog->listWidget);
#endif
  }
}
...
template<class T>
void G4OpenInventorQtExaminerViewer::parseString(T& t, const std::string& s, bool& error)
{
  std::istringstream str(s);
  if ((str >> t).fail()) error = true;
}

void G4OpenInventorQtExaminerViewer::FileOpenBookmarkCB()
{
  // FWJ DEBUG
...
void G4OpenInventorQtExaminerViewer::FileNewBookmarkCB()
{
...
void G4OpenInventorQtExaminerViewer::ToolsAnimateRefParticleCB()
{
...
void G4OpenInventorQtExaminerViewer::SaveViewPtCB()
{
  ...
  saveViewPtItem = new QListWidgetItem(namein, AuxWindowDialog->listWidget);
...
void G4OpenInventorQtExaminerViewer::setViewPt()
{
  ...
  if (camera == NULL)
  {
    G4warn << "setViewPt: Camera is null. Unable to set the viewpoint." << G4endl;
  ...
  {
    G4warn << "setViewPt: inconsitent viewPtIdx " << viewPtIdx << ", viewPtList.size() "
           << viewPtList.size() << G4endl;
    return;
  }
#endif
...
void G4OpenInventorQtExaminerViewer::NextViewPtCB()
{
  ...
  if (viewPtIdx >= (int)viewPtList.size() - 1) viewPtIdx = 0;
#if 0x060000 <= QT_VERSION
  else if (viewPtIdx < 0)
    viewPtIdx = 0;
#endif
...
SbBool G4OpenInventorQtExaminerViewer::processSoEvent(const SoEvent* const ev)
{
  // FWJ DEBUG
    ...
    if (SoKeyboardEvent::isKeyPressEvent(ev, ke->getKey()))
    {
      switch (ke->getKey())
      {
        case SoKeyboardEvent::E:
          ...
          {
            G4cout << "E KEY PRESSED, EXITING OIQT VIEWER SECONDARY LOOP" << G4endl;
            SoQt::exitMainLoop();
            //            escapeCallback();
            return TRUE;
          }
...
#  ifdef G4MULTITHREADED
  ...
  if (previousState == G4State_GeomClosed && requestedState == G4State_Idle)
  {  // EndOfRun
