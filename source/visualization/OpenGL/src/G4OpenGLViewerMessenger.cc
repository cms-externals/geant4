...
G4OpenGLViewerMessenger::G4OpenGLViewerMessenger()
{
  ...
  fpCommandExport = new G4UIcommand("/vis/ogl/export", this);
  ...
  fpCommandDisplayListLimit = new G4UIcmdWithoutParameter("/vis/ogl/set/displayListLimit", this);
  ...
  fpCommandExportFormat = new G4UIcommand("/vis/ogl/set/exportFormat", this);
  fpCommandExportFormat->SetGuidance("Set export format");
  fpCommandExportFormat->SetGuidance(
    "By default, pdf/eps/svg/ps are available. Depending of viewers several other format are "
    "available.");
  fpCommandExportFormat->SetGuidance(
  ...
  fpCommandPrintFilename = new G4UIcommand("/vis/ogl/set/printFilename", this);
  fpCommandPrintFilename->SetGuidance("Set print filename");
  fpCommandPrintFilename->SetGuidance(
  ...
  fpCommandPrintSize = new G4UIcommand("/vis/ogl/set/printSize", this);
  fpCommandPrintSize->SetGuidance("Set print size");
  fpCommandPrintSize->SetGuidance("Tip : -1 will mean 'print size' = 'window size'");
  fpCommandPrintSize->SetGuidance(
    "       Setting size greater than your maximum graphic card capacity , will set the size to "
    "maximum  size.");
  ...
  fpCommandTransparency = new G4UIcmdWithABool("/vis/ogl/set/transparency", this);
  fpCommandTransparency->SetGuidance(
    "True/false to enable/disable rendering of transparent objects.");
  fpCommandTransparency->SetParameterName("transparency-enabled", omitable = true);
...
void G4OpenGLViewerMessenger::SetNewValue(G4UIcommand* command, G4String newValue)
{
  ...
  if (!pSceneHandler)
  {
    G4cout << "G4OpenGLViewerMessenger::SetNewValue: This viewer has no scene handler."
  ...
  if (!pOGLViewer)
  {
    G4cout << "G4OpenGLViewerMessenger::SetNewValue: Current viewer is not of type"
              "\n  OGL.  (It is \""
  ...
  G4OpenGLSceneHandler* pOGLSceneHandler = dynamic_cast<G4OpenGLSceneHandler*>(pSceneHandler);
  if (!pOGLSceneHandler)
  {
    G4cout << "G4OpenGLViewerMessenger::SetNewValue: Current scene handler is not of type"
              "\n  OGL.  (Viewer is \""
           << pViewer->GetName()
           << "\".)"
              "\n  (Scene handler is \""
  ...
  G4OpenGLStoredViewer* pOGLSViewer = dynamic_cast<G4OpenGLStoredViewer*>(pViewer);

  if (!pOGLSViewer)
  {
    G4cout << "G4OpenGLViewerMessenger::SetNewValue: Current viewer is not of type OGLS."
              "\n  (It is \""
           << pViewer->GetName()
           << "\".)"
