...
G4OpenGLStoredSceneHandler::PO::PO()
  : fDisplayListId(0), fPickName(0), fpG4TextPlus(0), fMarkerOrPolyline(false)
{}
...
G4OpenGLStoredSceneHandler::PO::PO(G4int id, const G4Transform3D& tr)
  : fDisplayListId(id), fTransform(tr), fPickName(0), fpG4TextPlus(0), fMarkerOrPolyline(false)
{}
...
G4OpenGLStoredSceneHandler::PO&
G4OpenGLStoredSceneHandler::PO::operator=(const G4OpenGLStoredSceneHandler::PO& rhs)
{
...
G4OpenGLStoredSceneHandler::TO&
G4OpenGLStoredSceneHandler::TO::operator=(const G4OpenGLStoredSceneHandler::TO& rhs)
{
  if (&rhs == this) return *this;
  fDisplayListId = rhs.fDisplayListId;
  fTransform = rhs.fTransform;
...
G4OpenGLStoredSceneHandler::G4OpenGLStoredSceneHandler(G4VGraphicsSystem& system,
                                                       const G4String& name)
  : G4OpenGLSceneHandler(system, fSceneIdCount++, name), fDoNotUseDisplayList(false), fTopPODL(0)
{}
  ...
  else
  {
    ...
    if (glGetError() == GL_OUT_OF_MEMORY)
    {
      ...
      if (errorCount < 5)
      {
        ...
        ed << "Error attempting to create an OpenGL display list."
              "\nCurrent display list id: "
           << fDisplayListId << "\nMaybe out of memory?";
        G4Exception("G4OpenGLStoredSceneHandler::AddPrimitivePreambleInternal", "opengl1001",
                    JustWarning, ed);
      }
      return false;
    }
...
void G4OpenGLStoredSceneHandler::AddPrimitivePostamble()
{
  ...
  //  if ((glGetError() == GL_TABLE_TOO_LARGE) || (glGetError() == GL_OUT_OF_MEMORY)) {  // Could
  //  close?
  if (glGetError() == GL_OUT_OF_MEMORY)
  {  // Could close?
    G4cerr << "ERROR: G4OpenGLStoredSceneHandler::AddPrimitivePostamble: Failure"
...
void G4OpenGLStoredSceneHandler::EndModeling()
{
  ...
  if (glGetError() == GL_OUT_OF_MEMORY)
  {  // Could pre-allocate?
    G4cerr << "ERROR: G4OpenGLStoredSceneHandler::EndModeling: Failure to allocate"
  ...
  else
  {
    glNewList(fTopPODL, GL_COMPILE);
    {
      for (size_t i = 0; i < fPOList.size(); i++)
      {
        ...
        glMultMatrixd(oglt.GetGLMatrix());
        if (fpViewer->GetViewParameters().IsPicking()) glLoadName(fPOList[i].fPickName);
