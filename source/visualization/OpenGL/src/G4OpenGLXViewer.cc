...
int G4OpenGLXViewer::snglBuf_RGBA[12] = {GLX_RGBA, GLX_RED_SIZE,     1, GLX_GREEN_SIZE,
                                         1,        GLX_BLUE_SIZE,    1, GLX_DEPTH_SIZE,
                                         1,        GLX_STENCIL_SIZE, 1, None};

int G4OpenGLXViewer::dblBuf_RGBA[13] = {
  GLX_RGBA,         GLX_RED_SIZE,   1, GLX_GREEN_SIZE,   1, GLX_BLUE_SIZE, 1,
  GLX_DOUBLEBUFFER, GLX_DEPTH_SIZE, 1, GLX_STENCIL_SIZE, 1, None};

#define NewString(str)                                                                     \
  ((str) != 0                                                                              \
     ? (strncpy((char*)malloc((unsigned)strlen(str) + 1), str, (unsigned)strlen(str) + 1)) \
     : (char*)0)
...
void G4OpenGLXViewer::ShowView()
{
  ...
  if (fVP.IsPicking())
  {
    G4cout << "Window activated for picking (left-mouse), exit (middle-mouse)." << G4endl;
    while (true)
    {
      if (XPending(dpy))
      {
...
void G4OpenGLXViewer::GetXConnection()
{
  ...
  {
    fViewId = -1;  // This flags an error.
    G4cerr << "G4OpenGLXViewer::G4OpenGLXViewer X Server has no GLX extension." << G4endl;
    return;
  }
}
...
void G4OpenGLXViewer::CreateGLXContext(XVisualInfo* v)
{
  ...
  {
    fViewId = -1;  // This flags an error.
    G4cerr << "G4OpenGLXViewer::G4OpenGLXViewer couldn't return window attributes" << G4endl;
    return;
  }
  ...
  {
    fViewId = -1;  // This flags an error.
    G4cerr << "G4OpenGLXViewer::G4OpenGLXViewer couldn't create context." << G4endl;
    return;
  }
  ...
  status = XmuLookupStandardColormap(dpy, vi->screen, vi->visualid, vi->depth, XA_RGB_BEST_MAP,
                                     False, True);
  ...
  if (status == 1)
  {
    ...
    status = XGetRGBColormaps(dpy, XRootWindow(dpy, vi->screen), &standardCmaps, &numCmaps,
                              XA_RGB_BEST_MAP);
    if (status == 1)
    {
  ...
  if (!cmap)
  {
    fViewId = -1;  // This flags an error.
    if (G4VisManager::GetVerbosity() >= G4VisManager::errors)
      G4cout << "G4OpenGLXViewer::G4OpenGLXViewer failed to allocate a Colormap." << G4endl;
    return;
  }
}
...
void G4OpenGLXViewer::CreateMainWindow()
{
  ...
  win = XCreateWindow(dpy, XRootWindow(dpy, vi->screen), x_origin, y_origin, getWinWidth(),
                      getWinHeight(), 0, vi->depth, InputOutput, vi->visual,
                      CWBorderPixel | CWColormap | CWEventMask | CWBackingStore, &swa);

  XSetWMProperties(dpy, win, &windowName, &iconName, 0, 0, size_hints, wm_hints, class_hints);
  ...
  {
    fViewId = -1;  // This flags an error.
    G4cerr << "G4OpenGLXViewer::G4OpenGLXViewer failed to attach a GLX context." << G4endl;
    ...
    while ((error = glGetError()) != GL_NO_ERROR)
    {
      switch (error)
      {
        case GL_INVALID_ENUM:
          G4cout << "GL Error: GL_INVALID_ENUM" << G4endl;
          break;
        case GL_INVALID_VALUE:
          G4cout << "GL Error: GL_INVALID_VALUE" << G4endl;
          break;
        case GL_INVALID_OPERATION:
          G4cout << "GL Error: GL_INVALID_OPERATION" << G4endl;
          break;
        case GL_OUT_OF_MEMORY:
          G4cout << "GL Error: GL_OUT_OF_MEMORY" << G4endl;
          break;
        case GL_STACK_UNDERFLOW:
          G4cout << "GL Error: GL_STACK_UNDERFLOW" << G4endl;
          break;
        case GL_STACK_OVERFLOW:
          G4cout << "GL Error: GL_STACK_OVERFLOW" << G4endl;
          break;
        default:
          G4cout << "GL Error: " << error << G4endl;
          break;
      }
    }
    return;
...
void G4OpenGLXViewer::CreateFontLists()
{
  ...
  for (i = fonts.begin(); i != fonts.end(); ++i)
  {
    ...
    if (!font_info)
    {
      G4cerr << "G4OpenGLXViewer::CreateFontLists XLoadQueryFont failed for font\n  " << i->second
             << G4endl;
      continue;
    }
    ...
    if (!font_base)
    {
      G4cerr << "G4OpenGLXViewer::CreateFontLists out of display lists for fonts." << G4endl;
      continue;
    }
    ...
    G4OpenGLFontBaseStore::AddFontBase(this, font_base, i->first, i->second, width);
  }
}
...
void G4OpenGLXViewer::DrawText(const G4Text& g4text)
{
  ...
  else
  {
    ...
    if (fontInfo.fFontBase < 0)
    {
      ...
      if (callCount <= 1)
      {
        G4cout << "G4OpenGLXViewer::DrawText: No fonts available for \"" << fName
               << "\"\n  Called with " << g4text << G4endl;
      }
      return;
    }
...
G4OpenGLXViewer::G4OpenGLXViewer(G4OpenGLSceneHandler& scene)
  : G4VViewer(scene, -1), G4OpenGLViewer(scene), vi_immediate(0), vi_stored(0), vi(0), cmap(0)
{
  // To satisfy Coverity
  xwa.visual = 0;
  iconName.value = 0;
  ...
  if (!vi_single_buffer)
  {
    vi_single_buffer = glXChooseVisual(dpy, XDefaultScreen(dpy), snglBuf_RGBA);
    // G.Barrand : we should do a XFree(vi_single_buffer) at end;
  }
  if (!vi_double_buffer)
  {
    vi_double_buffer = glXChooseVisual(dpy, XDefaultScreen(dpy), dblBuf_RGBA);
  ...
  if (vi_single_buffer || vi_double_buffer)
  {
    if (!vi_double_buffer)
    {
      G4cout << "G4OpenGLXViewer::G4OpenGLXViewer: unable to get a double buffer visual."
  ...
  else
  {
    if (!vi_single_buffer)
    {
      G4cout << "G4OpenGLXViewer::G4OpenGLXViewer: unable to get a single buffer visual." << G4endl;
    }
    if (!vi_double_buffer)
    {
      G4cout << "G4OpenGLXViewer::G4OpenGLXViewer: unable to get a double buffer visual." << G4endl;
    }
  }
  ...
  {
    G4cout << "G4OpenGLXViewer::G4OpenGLXViewer: unable to get required visuals." << G4endl;
    fViewId = -1;  // This flags an error.
  }
