...
G4OpenGLQtMovieDialog::G4OpenGLQtMovieDialog(G4OpenGLQtViewer* parentViewer, QWidget* parentw)
  : QDialog(parentw), fParentViewer(parentViewer)
{
  setModal(false);
  ...
  connect(fEncoderPath, SIGNAL(textChanged(const QString&)), this,
          SLOT(checkEncoderSwParameters()));
  connect(fTempFolderPath, SIGNAL(textChanged(const QString&)), this,
          SLOT(checkTempFolderParameters()));
  connect(fSaveFileName, SIGNAL(textChanged(const QString&)), this,
          SLOT(checkSaveFileNameParameters()));
...
{
  QString nomFich =
    QFileDialog::getOpenFileName(this, "Select your encoder", tr("Select your encoder ..."));
...
{
  QString nomFich = QFileDialog::getExistingDirectory(this, "Select temporary folder",
...
{
  QString nomFich =
    QFileDialog::getSaveFileName(this, "Select saved file", tr("Select saved file ..."));
...
bool G4OpenGLQtMovieDialog::checkEncoderSwParameters()
{
  ...
  if (temp != "")
  {
    ...
    {
      setRecordingInfos(
        "No valid encode defined, screen capture had been saved in the temp folder in ppm "
