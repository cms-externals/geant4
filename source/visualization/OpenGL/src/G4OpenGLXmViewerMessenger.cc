...
G4OpenGLXmViewerMessenger::G4OpenGLXmViewerMessenger()
{
  ...
  fpCommandSetDollyHigh = new G4UIcmdWithADoubleAndUnit("/vis/oglxm/set/dolly-high", this);
  ...
  fpCommandSetDollyLow = new G4UIcmdWithADoubleAndUnit("/vis/oglxm/set/dolly-low", this);
  ...
  fpCommandSetPanHigh = new G4UIcmdWithADoubleAndUnit("/vis/oglxm/set/pan-high", this);
  fpCommandSetPanHigh->SetGuidance("Higher limit of pan slider.");
  fpCommandSetPanHigh->SetParameterName("pan-high", omitable = false);

  fpCommandSetRotationHigh = new G4UIcmdWithADoubleAndUnit("/vis/oglxm/set/rotation-high", this);
  ...
  fpCommandSetZoomHigh = new G4UIcmdWithADouble("/vis/oglxm/set/zoom-high", this);
  ...
  fpCommandSetZoomLow = new G4UIcmdWithADouble("/vis/oglxm/set/zoom-low", this);
...
void G4OpenGLXmViewerMessenger::SetNewValue(G4UIcommand* command, G4String newValue)
{
  ...
  if (!pVViewer)
  {
    G4cout << "G4OpenGLXmViewerMessenger::SetNewValue: No current viewer."
  ...
  if (!pViewer)
  {
    G4cout << "G4OpenGLXmViewerMessenger::SetNewValue: Current viewer is not of type"
  ...
  if (command == fpCommandSetDollyHigh)
  {
    ...
    {
      pViewer->dolly_high = fpCommandSetDollyHigh->GetNewDoubleValue(newValue);
  ...
  else if (command == fpCommandSetDollyLow)
  {
    if (pViewer->fpdolly_slider)
    {
      pViewer->dolly_low = fpCommandSetDollyLow->GetNewDoubleValue(newValue);
  ...
  else if (command == fpCommandSetPanHigh)
  {
    ...
    {
      pViewer->pan_sens_limit = fpCommandSetPanHigh->GetNewDoubleValue(newValue);
  ...
  else if (command == fpCommandSetRotationHigh)
  {
    if (pViewer->fprotation_slider)
    {
      ...
      pViewer->rot_sens_limit = fpCommandSetRotationHigh->GetNewDoubleValue(newValue) / deg;
  ...
  else if (command == fpCommandSetZoomHigh)
  {
    ...
    {
      pViewer->zoom_high = fpCommandSetZoomHigh->GetNewDoubleValue(newValue);
  ...
  else if (command == fpCommandSetZoomLow)
  {
    if (pViewer->fpzoom_slider)
    {
      pViewer->zoom_low = fpCommandSetZoomLow->GetNewDoubleValue(newValue);
  ...
  {
    G4cout << "G4OpenGLXmViewerMessenger::SetNewValue: pull down panning"
  ...
  {
    G4cout << "G4OpenGLXmViewerMessenger::SetNewValue: pull down rotation"
              "\n  control panel and re-issue command."
           << G4endl;
    return;
  }
