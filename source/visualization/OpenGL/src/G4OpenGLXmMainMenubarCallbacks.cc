...
void G4OpenGLXmViewer::actions_callback(Widget w, XtPointer clientData, XtPointer)
{
  ...
  switch (choice)
  {
...

    {
      if (!pView->fprotation_top)
      {
        ...
        pView->fprotation_top = new G4OpenGLXmTopLevelShell(pView, (char*)rot_Name.str().c_str());
        ...
        pView->fprotation_button1 = new G4OpenGLXmRadioButton(
          "Object", rot_cb_list, pView->GetViewParameters().GetLightsMoveWithCamera(), 0);

        pView->fprotation_button2 = new G4OpenGLXmRadioButton(
          "Camera", rot_cb_list, !(pView->GetViewParameters().GetLightsMoveWithCamera()), 1);
        ...
        pView->fprotation_slider = new G4OpenGLXmSliderBar(
          "Rotation slider", rot_slider_list, True, 2, pView->fRot_sens, pView->rot_sens_limit, 0);
    ...
    case 1: {
      ...
      if (!pView->fppanning_top)
      {
        ...
        pView->fppanning_top = new G4OpenGLXmTopLevelShell(pView, (char*)pan_Name.str().c_str());

        pView->fppanning_box = new G4OpenGLXmFramedBox("Pan up-down-left-right", False);
        ...
        pView->fppanning_slider = new G4OpenGLXmSliderBar(
          "Panning slider", pan_slider_list, True, 2,
          pView->fPan_sens =
            pView->GetSceneHandler()->GetScene()->GetExtent().GetExtentRadius() / 10.0,
          pView->pan_sens_limit =
            pView->GetSceneHandler()->GetScene()->GetExtent().GetExtentRadius(),
          0);
        pView->fppanning_box->AddChild(pView->fppanning_slider);

        pView->fpzoom_box = new G4OpenGLXmFramedBox("Zoom", False);
        ...
        pView->fpzoom_slider =
          new G4OpenGLXmSliderBar("Zoom slider", zoom_slider_list, True, 2,
                                  pView->fVP.GetZoomFactor(), pView->zoom_high, pView->zoom_low);
        pView->fpzoom_box->AddChild(pView->fpzoom_slider);

        pView->fpdolly_box = new G4OpenGLXmFramedBox("Dolly", False);
        ...
        pView->fpdolly_slider = new G4OpenGLXmSliderBar(
          "Dolly slider", dolly_slider_list, True, 2, pView->fVP.GetDolly(),
    ...
    case 2: {
      if (!pView->fpsetting_top)
      {
        ...
        pView->fpsetting_top = new G4OpenGLXmTopLevelShell(pView, (char*)set_Name.str().c_str());

        pView->fpsetting_box = new G4OpenGLXmFramedBox("Set values for control panels", False);
        pView->fpsetting_top->AddChild(pView->fpsetting_box);

        pView->fppan_set =
          new G4OpenGLXmTextField("Upper limit of pan sensitivity", &(pView->pan_sens_limit));

        pView->fprot_set =
          new G4OpenGLXmTextField("Upper limit of rotation sensitivity", &(pView->rot_sens_limit));

        pView->fpzoom_upper = new G4OpenGLXmTextField("Upper limit of zoom", &(pView->zoom_high));

        pView->fpzoom_lower = new G4OpenGLXmTextField("Lower limit of zoom", &(pView->zoom_low));

        pView->fpdolly_upper =
          new G4OpenGLXmTextField("Upper limit of dolly", &(pView->dolly_high));

        pView->fpdolly_lower = new G4OpenGLXmTextField("Lower limit of dolly", &(pView->dolly_low));
        ...
        pView->fpok_button = new G4OpenGLXmPushButton("ok", ok_list);
    ...
    default:
      G4Exception("G4OpenGLXmViewer::actions_callback", "opengl2001", FatalException,
...
void G4OpenGLXmViewer::misc_callback(Widget w, XtPointer clientData, XtPointer)
{
  G4OpenGLXmViewer* pView;
  G4long choice = (G4long)clientData;
  XtVaGetValues(XtParent(w), XmNuserData, &pView, NULL);
  ...
  switch (choice)
  {
    case 0: {
      if (!pView->fpmiscellany_top)
      {
        ...
        pView->fpmiscellany_top =
          new G4OpenGLXmTopLevelShell(pView, (char*)misc_Name.str().c_str());
        pView->fpwobble_box = new G4OpenGLXmFramedBox("Wobble view", True);
        ...
        pView->fpwobble_button = new G4OpenGLXmPushButton("Wobble", wob_cb_list);
        ...
        pView->fpwobble_slider =
          new G4OpenGLXmSliderBar("Wobble slider", wobble_slider_list, True, 0, pView->wob_sens,
                                  pView->wob_high, pView->wob_low);
        pView->fpwobble_box->AddChild(pView->fpwobble_button);
        pView->fpwobble_box->AddChild(pView->fpwobble_slider);

        pView->fpreset_box = new G4OpenGLXmFramedBox("Reset view", True);
        ...
        pView->fpreset_button = new G4OpenGLXmPushButton("Reset", rst_cb_list);

        pView->fpreset_box->AddChild(pView->fpreset_button);

        pView->fpproj_style_box = new G4OpenGLXmFramedBox("Projection style", True);
        ...
        pView->fporthogonal_button = new G4OpenGLXmRadioButton(
          "Orthographic", proj_cb_list, pView->fVP.GetFieldHalfAngle() > 0. ? False : True, 0);

        pView->fpperspective_button = new G4OpenGLXmRadioButton(
          "Perspective", proj_cb_list, pView->fVP.GetFieldHalfAngle() > 0. ? True : False, 1);

        pView->fpfov_text =
          new G4OpenGLXmTextField("Field of view 0.1 -> 89.5 degrees.", &(pView->fov));
    ...
    case 2: {
      if (!pView->fpprint_top)
      {
        ...
        pView->fpprint_top = new G4OpenGLXmTopLevelShell(pView, (char*)print_Name.str().c_str());

        pView->fpprint_box = new G4OpenGLXmFramedBox("Create EPS file of current view", False);

        pView->fpprint_top->AddChild(pView->fpprint_box);

        pView->fpprint_col_box = new G4OpenGLXmFramedBox("Colour choice", True);
        ...
        pView->fpprint_col_radio1 = new G4OpenGLXmRadioButton(
          "Black and white", prcol_cb_list, pView->fPrintColour == false ? True : False, 0);

        pView->fpprint_col_radio2 = new G4OpenGLXmRadioButton(
          "Colour", prcol_cb_list, pView->fPrintColour == true ? True : False, 1);

        pView->fpprint_col_box->AddChild(pView->fpprint_col_radio1);
        pView->fpprint_col_box->AddChild(pView->fpprint_col_radio2);

        pView->fpprint_style_box = new G4OpenGLXmFramedBox("File type", True);
        ...
        pView->fpprint_style_radio1 = new G4OpenGLXmRadioButton(
          "Screen dump (pixmap)", prsty_cb_list, pView->fVectoredPs == false ? True : False, 0);

        pView->fpprint_style_radio2 = new G4OpenGLXmRadioButton(
          "PostScript", prsty_cb_list, pView->fVectoredPs == true ? True : False, 1);
        ...
        pView->fpprint_button = new G4OpenGLXmPushButton("Create EPS file", pri_cb_list);
    ...
    default:
      G4Exception("G4OpenGLXmViewer::misc_callback", "opengl2002", FatalException,
...
void G4OpenGLXmViewer::set_wob_sens_callback(Widget w, XtPointer clientData, XtPointer callData)
{
  ...
  {
    G4Exception("G4OpenGLXmViewer::set_wob_sens_callback", "opengl2003", FatalException,
...
void G4OpenGLXmViewer::update_panels_callback(Widget, XtPointer clientData, XtPointer)
{
  G4OpenGLXmViewer* pView = (G4OpenGLXmViewer*)clientData;
