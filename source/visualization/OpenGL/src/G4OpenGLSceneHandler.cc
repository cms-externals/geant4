...
G4OpenGLSceneHandler::G4OpenGLSceneHandler(G4VGraphicsSystem& system, G4int id,
                                           const G4String& name)
...
void G4OpenGLSceneHandler::ClearAndDestroyAtts()
{
  ...
  for (i = fPickMap.begin(); i != fPickMap.end(); ++i)
    delete i->second;
  fPickMap.clear();
}

G4int G4OpenGLSceneHandler::fEntitiesFlushInterval = 100;
G4OpenGLSceneHandler::FlushAction G4OpenGLSceneHandler::fFlushAction =
  G4OpenGLSceneHandler::NthEvent;
...
void G4OpenGLSceneHandler::ScaledFlush()
{
  if (fReadyForTransients)
  {
    ...
    const G4ModelingParameters* modelingParameters = fpModel->GetModelingParameters();
    ...
    switch (fFlushAction)
    {
      ...
      case NthPrimitive: {  // Encapsulate in scope {} brackets to satisfy Windows.
...
void G4OpenGLSceneHandler::AddPrimitive(const G4Polymarker& polymarker)
{
      ...
      case G4VMarker::hashed:
        if (!hashedWarned)
        {
          G4cout << "Hashed fill style in G4OpenGLSceneHandler."
                 << "\n  Not implemented.  Using G4VMarker::filled." << G4endl;
          hashedWarned = true;
        }
        // Maybe use
  ...
  {  // Size specified in world coordinates.
    ...
    const G4Vector3D& viewpointDirection = fpViewer->GetViewParameters().GetViewpointDirection();
    ...
    for (size_t iPoint = 0; iPoint < polymarker.size(); iPoint++)
    {
      ...
      {
        G4Vector3D r = start;
        r.rotate(phi, viewpointDirection);
  ...
  {  // Size specified in screen (window) coordinates.
...
void G4OpenGLSceneHandler::AddPrimitive(const G4Polyhedron& polyhedron)
{
  ...
  do
  {
    ...
    for (G4int edgeCount = 0; edgeCount < nEdges; ++edgeCount)
    {
      ...
      glNormal3d(normals[edgeCount].x(), normals[edgeCount].y(), normals[edgeCount].z());
      glVertex3d(vertex[edgeCount].x(), vertex[edgeCount].y(), vertex[edgeCount].z());
    }
    ...
    if (nEdges > 4)
    {
      G4cerr << "G4OpenGLSceneHandler::AddPrimitive(G4Polyhedron): WARNING"
                "\n   G4Polyhedron facet with "
             << nEdges << " edges" << G4endl;
    }

    // Do it all over again (twice) for hlr...
    if (drawing_style == G4ViewParameters::hlr || drawing_style == G4ViewParameters::hlhsr)
    {
