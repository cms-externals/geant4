...
void G4OpenGLXmViewer::theta_rotation_callback(Widget w, XtPointer clientData, XtPointer callData)
{
...
void G4OpenGLXmViewer::rotate_in_theta(XtPointer clientData, XtIntervalId* timer_id)
{
  ...
  */
  ...
  pView->rotation_timer =
    XtAppAddTimeOut(pView->app, timer_id == NULL ? 500 : 1, rotate_in_theta, pView);
}

void G4OpenGLXmViewer::phi_rotation_callback(Widget w, XtPointer clientData, XtPointer callData)
{
  XmArrowButtonCallbackStruct* cbs = (XmArrowButtonCallbackStruct*)callData;
  G4OpenGLXmViewer* pView = (G4OpenGLXmViewer*)clientData;
...
void G4OpenGLXmViewer::rotate_in_phi(XtPointer clientData, XtIntervalId* timer_id)
{
  // phi spin stuff here
  ...
  */
  ...
  pView->rotation_timer =
    XtAppAddTimeOut(pView->app, timer_id == NULL ? 500 : 1, rotate_in_phi, pView);
}

void G4OpenGLXmViewer::set_rot_sens_callback(Widget w, XtPointer clientData, XtPointer callData)
{
  ...
  XtVaGetValues(w, XmNdecimalPoints, &dp, NULL);
  ...
  {
    G4Exception("G4OpenGLXmViewer::set_rot_sens_callback", "opengl2004", FatalException,
...
void G4OpenGLXmViewer::set_rot_subject_callback(Widget w, XtPointer clientData, XtPointer)
{
  G4OpenGLXmViewer* pView = (G4OpenGLXmViewer*)clientData;
  ...
  switch (choice)
  {
    ...
    default: {
      G4Exception("G4OpenGLXmViewer::set_rot_subject_callback", "opengl2005", FatalException,
                  "Unrecognised choice made in set_rot_subject_callback");
    }
  }
}

void G4OpenGLXmViewer::wobble_callback(Widget w, XtPointer, XtPointer)
{
  G4OpenGLXmViewer* pView;

  XtVaGetValues(w, XmNuserData, &pView, NULL);

  pView->original_vp = pView->fVP.GetViewpointDirection();
  pView->wobble_timer = XtAppAddTimeOut(
    pView->app, (long unsigned int)(1000. * (1. / pView->wob_sens)), wobble_timer_callback, pView);
}

void G4OpenGLXmViewer::wobble_timer_callback(XtPointer clientData, XtIntervalId*)
{
  G4OpenGLXmViewer* pView = (G4OpenGLXmViewer*)clientData;
  ...
  G4Vector3D d_third_axis =
    0.1 * (std::sin((G4double)pView->frameNo * (pi_div_by_ten))) * third_axis;
  ...
  {
    pView->wobble_timer =
      XtAppAddTimeOut(pView->app, (long unsigned int)(1000. * (1. / pView->wob_sens)),
                      wobble_timer_callback, pView);
  }
}

void G4OpenGLXmViewer::reset_callback(Widget w, XtPointer, XtPointer)
{
  G4OpenGLXmViewer* pView;
