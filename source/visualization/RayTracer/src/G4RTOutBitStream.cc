...
{
  if (size < 1) throw(G4MemoryError(size, "G4OutBitStream"));

  mHeadOfBuf = mBuf = new u_char[size];
  if (mHeadOfBuf == 0) throw(G4MemoryError(size, "G4OutBitStream"));
...
{
  if (++mBuf >= mEndOfBuf) mWriteFlag = 0;
}

void G4OutBitStream::SetBits(int v, int numBits)
{
  if (numBits == 0) return;
  if (numBits > 16) throw(G4BufferError("SetBits:Max Bit Over"));
  if (numBits > 8)
  {
...
void G4OutBitStream::SetFewBits(u_char v, int numBits)
{
...
void G4OutBitStream::SetBits2Byte(u_char v, int numBits)
{
  v &= BitFullMaskT[numBits - 1];
...
void G4OutBitStream::Set8Bits(u_char v, int numBits)
{
...
{
  if (mBitPos != 7) SetFewBits(BitFullMaskT[mBitPos], mBitPos + 1);
}
...
void G4OutBitStream::CopyByte(const char* src, int n)
{
  if (mBuf + n < mEndOfBuf)
  {
    FullBit();
