...
G4bool G4RTXScanner::GetXWindow(const G4String& name, G4ViewParameters& vp)
{
  ...
  if (!display)
  {
    G4warn << "G4RTXScanner::Initialize(): cannot get display." << G4endl;
    return false;
  }
  ...
  {
    G4int geometryResultMask =
      XParseGeometry((char*)XGeometryString.c_str(), &xOffset, &yOffset, &width, &height);
    ...
    {
      G4warn << "ERROR: Geometry string \"" << XGeometryString << "\" invalid.  Using \"600x600\"."
             << G4endl;
      width = 600;
      height = 600;
    }
  ...
  {
    G4warn << "ERROR: Geometry string \"" << XGeometryString << "\" is empty.  Using \"600x600\"."
           << G4endl;
    width = 600;
    height = 600;
  }
  ...
  win =
    XCreateSimpleWindow(display, RootWindow(display, screen_num), xOffset, yOffset, width, height,
                        0,  // Border width.
  ...
  int nMaps;
  Status status =
    XGetRGBColormaps(display, RootWindow(display, screen_num), &scmap, &nMaps, XA_RGB_BEST_MAP);
  if (!status)
  {
    ...
    if (!status)
    {
      G4warn << "G4RTXScanner::Initialize(): cannot get color map."
  ...
  if (!scmap->colormap)
  {
    G4warn << "G4RTXScanner::Initialize(): color map empty." << G4endl;
    return false;
  }
  ...
  XSetWMProperties(display, win, &windowName, &windowName, 0, 0, size_hints, wm_hints, class_hint);
...
void G4RTXScanner::Draw(unsigned char red, unsigned char green, unsigned char blue)
// Draw coloured square at current position.
{
  unsigned long pixel_value =
    scmap->base_pixel + ((unsigned long)((red * scmap->red_max) / 256.) * scmap->red_mult)
  ...
  if (theStep > 1)
  {
    XFillRectangle(display, win, gc, theIColumn - theStep / 2, theIRow - theStep / 2, theStep,
                   theStep);
  }
  else
  {
