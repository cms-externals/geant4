...
void G4JpegCoder::GetJpegData(char** aJpegData, int& size)
{
  if (mOBSP != 0)
  {
...
void G4JpegCoder::makeYCC(int ux, int uy)
{
  u_char rv, gv, bv;
  int tCrBlock[4][64];
  int tCbBlock[4][64];
...
void G4JpegCoder::CodeHuffman(int cs)
{
  ...
  if (dIdx > dcT.numOfElement) throw(G4IndexError(dcT.numOfElement, dIdx, "CodeHuffman:DC"));
  ...
  for (int n = 1; n < 64; n++)
  {
    ...
    if (absCoefficient)
    {
      ...
      if (aIdx >= acT.numOfElement) throw(G4IndexError(acT.numOfElement, aIdx, "CodeHuffman:AC"));
