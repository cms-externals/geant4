...
G4Colour G4RTRun::GetSurfaceColour(G4RayTrajectoryPoint* point)
{
  ...
  if (preVis)
  {
    ...
    preCol = G4Colour(red * brill, green * brill, blue * brill, preAtt->GetColour().GetAlpha());
  }
  else
  {
    preCol = transparent;
  ...
  if (postVis)
  {
    ...
    postCol = G4Colour(red * brill, green * brill, blue * brill, postAtt->GetColour().GetAlpha());
  }
  else
  {
    postCol = transparent;
