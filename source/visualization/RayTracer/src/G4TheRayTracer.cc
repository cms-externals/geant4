...
G4TheRayTracer::G4TheRayTracer(G4VFigureFileMaker* figMaker, G4VRTScanner* scanner)
{
...
G4bool G4TheRayTracer::CreateBitMap()
{
  ...
  while (theScanner->Coords(iRow, iColumn))
  {
    ...
    if (distortionOn)
    {
      rayDirection = G4ThreeVector(-std::tan(angleX) / std::cos(angleY),
                                   std::tan(angleY) / std::cos(angleX), 1.0);
    }
    else
    {
    ...
    EInside whereisit = pWorld->GetLogicalVolume()->GetSolid()->Inside(rayPosition);
...
G4Colour G4TheRayTracer::GetMixedColour(const G4Colour& surfCol, const G4Colour& transCol,
                                        G4double weight)
{
...
G4Colour G4TheRayTracer::GetSurfaceColour(G4RayTrajectoryPoint* point)
{
  ...
  if (preVis)
  {
    ...
    preCol = G4Colour(red * brill, green * brill, blue * brill, preAttColour.GetAlpha());
  }
  else
  {
    preCol = transparent;
  ...
  if (postVis)
  {
    ...
    postCol = G4Colour(red * brill, green * brill, blue * brill, postAttColour.GetAlpha());
  }
  else
  {
    postCol = transparent;
...
G4Colour G4TheRayTracer::Attenuate(G4RayTrajectoryPoint* point, const G4Colour& sourceCol)
{
  ...
  if (stepAlpha > 0.9999999)
  {
    stepAlpha = 0.9999999;
  }  // patch to the next line
  ...
  return G4Colour(sourceCol.GetRed() * KtRed, sourceCol.GetGreen() * KtGreen,
                  sourceCol.GetBlue() * KtBlue);
}
