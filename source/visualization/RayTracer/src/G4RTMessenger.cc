...
G4RTMessenger::G4RTMessenger(G4TheRayTracer* p1)
{
  ...
  attCmd->SetGuidance(
    "Note that this value is independent to the attenuation length for the optical photon "
    "processes.");
...
void G4RTMessenger::SetNewValue(G4UIcommand* command, G4String newValue)
{
  ...
  if (pVViewer)
  {
    ...
    {
      G4warn << "G4RTMessenger::SetNewValue: Current viewer is not of type RayTracer."
  ...
  {
    G4warn << "G4RTMessenger::SetNewValue: No valid current viewer. Using default RayTracer."
           << G4endl;
  }
