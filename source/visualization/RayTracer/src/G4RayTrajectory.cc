...
G4RayTrajectory ::G4RayTrajectory(G4RayTrajectory& right) : G4VTrajectory()
{
  positionRecord = new std::vector<G4RayTrajectoryPoint*>;
  for (size_t i = 0; i < right.positionRecord->size(); i++)
  {
    G4RayTrajectoryPoint* rightPoint = (G4RayTrajectoryPoint*)((*(right.positionRecord))[i]);
...
void G4RayTrajectory::AppendStep(const G4Step* theStep)
{
  ...
  {
    localPrePVPointerCopyNoPath.push_back(G4ModelingParameters::PVPointerCopyNo(
  ...
  {
    localPostPVPointerCopyNoPath.push_back(G4ModelingParameters::PVPointerCopyNo(
