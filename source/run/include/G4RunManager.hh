...
class G4RunManager
{
  ...
  public:
    ...
    inline void RegisterSubEventType(G4int ty, G4int maxEnt)
    {
      subEventPara = true;
  ...
  protected:
    ...
    void UpdateScoring();
    void UpdateScoringForSubEvent(const G4SubEvent* se,const G4Event* evt);
    ...
    // Returns a sub-event. This object must not be deleted by the worker thread.
    ...
    void SubEventFinished(const G4SubEvent* se,const G4Event* evt);
