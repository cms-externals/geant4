  ...
  switch (severity) {
    ...
    case JustWarning:
      if(IfPrint(exceptionCode))
      {
        ...
        {
          *(ofl->second) << ws_banner << wmessage.str() 
...
// --------------------------------------------------------------------
void G4ExceptionHandler::SetAllWarningFile(G4String& fNam)
{
  if(allWarningFile!=nullptr && allWarningFile!=&G4cout)
...
// --------------------------------------------------------------------
void G4ExceptionHandler::SetWarningFile(const char* errCode, G4String& fNam)
{
  if(fNam=="**Screen**")
...
G4String G4ExceptionHandler::ModifyFileName(G4String& fNam)
{
  ...
  if(G4Threading::IsMultithreadedApplication() && G4Threading::IsWorkerThread())
  {
    ...
    {
      fileName = fNam + "_t" 
         + G4UIcommand::ConvertToString(G4Threading::G4GetThreadId());
    }
  }
  return fileName;
}
