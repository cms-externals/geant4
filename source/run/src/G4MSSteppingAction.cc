...
G4MSSteppingAction::G4MSSteppingAction()
{
  ...
  if(defParticle==nullptr)
  {
    ...
    G4Exception("G4MSSteppingAction::G4MSSteppingAction()","MatScan0001",
                FatalException,ed);
  }
...
void G4MSSteppingAction::UserSteppingAction(const G4Step* aStep)
{
  ...
  { 
    ...
    {
      G4ExceptionDescription ed;
      ed << " PANIC ! Failure to get the interaction length of "
         << defParticle->GetParticleName() << " for " << material->GetName()
    ...
    G4cout << "Updating interaction length for " << material->GetName()
           << " : original " << material->GetNuclearInterLength() / CLHEP::cm
           << " : updated " << intLen / CLHEP::cm
           << G4endl;
#endif
  }
...
void G4MSSteppingAction::PrintEachMaterialVerbose(std::ostream & oss)
{
  ...
  oss << G4endl<< G4endl;
  oss << "Phys vol          Material          Density    Radiation Interaction Thickness  Mass       X0         Lambda                      Entry point                              Exit point\n"
      << "   name              name                       length     length               length                                                (cm)                                   (cm)\n"
  ...
  for( auto & matInfo : shape_mat_info_v)
  {
     ...
     oss << std::setw(colwidth) << std::scientific << matInfo.massThickness / (CLHEP::g/CLHEP::cm2);
...
void G4MSSteppingAction::PrintIntegratedMaterialVerbose(std::ostream& oss)
{
  ...
  oss << G4endl<< G4endl;
  oss << "   Material       Density    Radiation  Interaction Total     Total mass Total      Total\n"
      << "     name                     length     length    thickness  length     X0         Lambda\n"
      << "                  (g/cm3)     (cm)       (cm)        (cm)      (g/cm2)";
  oss << G4endl;
  ...
  for(auto & [key,mat] : mat_db)
  {
     ...
     oss << std::setw(colwidth) << std::scientific << std::setprecision(2) << mat.density / (CLHEP::g/CLHEP::cm3);
