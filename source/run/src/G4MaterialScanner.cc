...
void G4MaterialScanner::DoScan()
{
  ...
  for (G4int iTheta = 0; iTheta < nTheta; ++iTheta) {
    ...
    *(ost) << G4endl;
    *(ost) << "              Theta         Phi      Length  Mass length         x0     lambda0" << G4endl;
    ...
    *(ost) << G4endl;
    for (G4int iPhi = 0; iPhi < nPhi; ++iPhi) {
      ...
      *(ost) << "        " << std::setprecision(2)
             << std::setw(11) << std::scientific << theta / deg << " "
             << std::setw(11) << std::scientific << phi / deg << " "
             << std::setw(11) << std::scientific << length / cm << " "
             << std::setw(11) << std::scientific << massThick / (g/cm2) << " " 
             << std::setw(11) << std::scientific << x0 << " "
             << std::setw(11) << std::scientific << lambda << G4endl;
      if(1 == verbosity)
      {
    ...
    if (nPhi > 1) {
      *(ost) << G4endl;
      *(ost) << " ave. for theta = " << std::setprecision(2) 
             << std::setw(11) << std::scientific << theta / deg << " : "
             << std::setw(11) << std::scientific << aveLength / nPhi << " "
             << std::setw(11) << std::scientific << aveMassThick / nPhi << " "
             << std::setw(11) << std::scientific << aveX0 / nPhi << " "
             << std::setw(11) << std::scientific << aveLambda / nPhi
             << G4endl;
    }
  }
...
// --------------------------------------------------------------------
void G4MaterialScanner::SetDetDefaultParticle
                      (const G4ParticleDefinition* ptcl,G4double eKin)
{
  if (theMatScannerSteppingAction == nullptr) {
    theMatScannerSteppingAction = new G4MSSteppingAction();
  }
