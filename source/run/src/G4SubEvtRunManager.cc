...
//MAMAMAMAMAMAMAMA  MergeTrajectories(se,evt);
...
{
  auto masterEvt = se->GetEvent();
  for(auto& i : *(evt->GetTrajectoryContainer()->GetVector()))
  {
    auto* traj = dynamic_cast<G4Trajectory*>(i);
    if(traj==nullptr) {
      G4Exception("G4SubEvtRunManager::MergeTrajectories()","SERM0005",
        JustWarning,"Trajectory is not a type of G4Trajectory. This trajectory is not merged.");
      return;
    }
    masterEvt->GetTrajectoryContainer()->push_back(traj->CloneForMaster());
  }
}
