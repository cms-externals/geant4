...
void G4RunManager::CleanUpPreviousEvents()
{
  ...
  while (evItr != previousEvents->cend()) {
    ...
    if (evt != nullptr)
    {
      if(subEventPara && userEventAction!=nullptr) userEventAction->EndOfEventAction(evt);
      delete evt;
    }
  ...
  if(currentRun!=nullptr)
  {
    ...
    while(eItr != eventVector->cend())
    {
      ...
      if(ev!=nullptr)
      {
        if(subEventPara && userEventAction!=nullptr) userEventAction->EndOfEventAction(ev);
        delete ev;
      }
...
void G4RunManager::StackPreviousEvent(G4Event* anEvent)
{
  ...
  if (n_perviousEventsToBeStored == 0) {
    if (anEvent->GetNumberOfGrips() == 0) {
      if (!(anEvent->ToBeKept()))
      {
        if(subEventPara && userEventAction!=nullptr) userEventAction->EndOfEventAction(anEvent);
        delete anEvent;
      }
    }
    else {
...
// --------------------------------------------------------------------
void G4RunManager::UpdateScoringForSubEvent(const G4SubEvent* se,const G4Event* evt)
{
  if (isScoreNtupleWriter) {
...
// --------------------------------------------------------------------
const G4SubEvent* G4RunManager::GetSubEvent(G4int ty, G4bool& notReady)
{
...
void G4RunManager::SubEventFinished(const G4SubEvent* se,const G4Event* evt)
{
