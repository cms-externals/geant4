...
//                [0]---------[3]
//
G4PolyPhiFace::G4PolyPhiFace(const G4ReduciblePolygon* rz, G4double phi, G4double deltaPhi,
                             G4double phiOther)
{
  ...
  G4double cosMid = std::cos(midPhi), sinMid = std::sin(midPhi);
  //
  // Allocate corners
  //
  ...
  G4PolyPhiFaceVertex *prev = corners + maxEdges - 1, *here = corners;
  ...
  do  // Loop checking, 13.08.2015, G.Cosmo
  {
    ...
    G4double dr = here->r - prev->r, dz = here->z - prev->z;
    ...
    if ((here->r < DBL_MIN) && (prev->r < DBL_MIN))
    {
      ...
      sideNorm =
        G4ThreeVector(zSignOther * std::sin(phiOther), -zSignOther * std::cos(phiOther), 0);
    }
    else
    {
  ...
  do  // Loop checking, 13.08.2015, G.Cosmo
  {
    ...
    if (edge->v0->r < DBL_MIN)
    {
      ...
      //
      G4double zSignOther = start ? -1 : 1;
      G4ThreeVector normalOther(zSignOther * std::sin(phiOther), -zSignOther * std::cos(phiOther),
                                0);
  ...
  //
  G4double rAve = 0.5 * (rMax - rMin), zAve = 0.5 * (zMax - zMin);
...
void G4PolyPhiFace::Diagnose(G4VSolid* owner)
{
  ...
  do  // Loop checking, 13.08.2015, G.Cosmo
  {
    ...
    {
      G4Exception("G4PolyPhiFace::Diagnose()", "GeomSolids0002", FatalException,
                  "Bad vertex normal found.");
    }
...
//
G4PolyPhiFace::G4PolyPhiFace(__void__&) : rMin(0.), rMax(0.), zMin(0.), zMax(0.), kCarTolerance(0.)
{}
...
void G4PolyPhiFace::CopyStuff(const G4PolyPhiFace& source)
{
  ...
  //
  corners = new G4PolyPhiFaceVertex[maxEdges];
  G4PolyPhiFaceVertex *corn = corners, *sourceCorn = source.corners;
  do  // Loop checking, 13.08.2015, G.Cosmo
  {
    *corn = *sourceCorn;
  ...
  G4PolyPhiFaceVertex *prev = corners + maxEdges - 1, *here = corners;
  G4PolyPhiFaceEdge *edge = edges, *sourceEdge = source.edges;
  do  // Loop checking, 13.08.2015, G.Cosmo
  {
    *edge = *sourceEdge;
    edge->v0 = prev;
...
// BuildEdgeIndex
//
void G4PolyPhiFace::BuildEdgeIndex()
{
  fEdgeOrder.clear();
  fEdgeNode.clear();
  if (numEdges >= 16)
  {
    fEdgeOrder.resize(numEdges);
    std::iota(fEdgeOrder.begin(), fEdgeOrder.end(), 0);
    fEdgeNode.reserve(2 * numEdges);
    BuildEdgeNode(0, numEdges);
  }
}

G4int G4PolyPhiFace::BuildEdgeNode(G4int begin, G4int end)
{
  EdgeNode node;
  node.rMin = node.zMin = kInfinity;
  node.rMax = node.zMax = -kInfinity;
  for (G4int pos = begin; pos < end; ++pos)
  {
    const auto& edge = edges[fEdgeOrder[pos]];
    node.rMin = std::min(node.rMin, std::min(edge.v0->r, edge.v1->r));
    node.rMax = std::max(node.rMax, std::max(edge.v0->r, edge.v1->r));
    node.zMin = std::min(node.zMin, std::min(edge.v0->z, edge.v1->z));
    node.zMax = std::max(node.zMax, std::max(edge.v0->z, edge.v1->z));
  }
  const G4int index = G4int(fEdgeNode.size());
  fEdgeNode.push_back(node);
  const G4int count = end - begin;
  if (count <= 4)
  {
    fEdgeNode[index].begin = begin;
    fEdgeNode[index].count = count;
    return index;
  }

  const G4bool splitR = node.rMax - node.rMin > node.zMax - node.zMin;
  const G4int middle = begin + count / 2;
  std::nth_element(fEdgeOrder.begin() + begin, fEdgeOrder.begin() + middle,
                   fEdgeOrder.begin() + end, [&](G4int lhs, G4int rhs)
  {
    const auto& a = edges[lhs];
    const auto& b = edges[rhs];
    const G4double ca = splitR ? a.v0->r + a.v1->r : a.v0->z + a.v1->z;
    const G4double cb = splitR ? b.v0->r + b.v1->r : b.v0->z + b.v1->z;
    return ca < cb;
  });
  const G4int left = BuildEdgeNode(begin, middle);
  const G4int right = BuildEdgeNode(middle, end);
  fEdgeNode[index].left = left;
  fEdgeNode[index].right = right;
  return index;
}

// Intersect
//
G4bool G4PolyPhiFace::Intersect(const G4ThreeVector& p, const G4ThreeVector& v, G4bool outgoing,
                                G4double surfTolerance, G4double& distance,
                                G4double& distFromSurface, G4ThreeVector& aNormal,
                                G4bool& isAllBehind)
{
...
EInside G4PolyPhiFace::Inside(const G4ThreeVector& p, G4double tolerance, G4double* bestDistance)
{
  ...
  *bestDistance = std::sqrt(distPhi * distPhi + distRZ2);
  ...
  //
  G4ThreeVector cc(base3Dnorm->r * radial.x(), base3Dnorm->r * radial.y(), base3Dnorm->z);
  cc = p - cc;
...
G4double G4PolyPhiFace::Extent(const G4ThreeVector axis)
{
  ...
  {
    G4double here =
      axis.x() * corner->r * radial.x() + axis.y() * corner->r * radial.y() + axis.z() * corner->z;
    if (here > max)
    {
      max = here;
    }
...
//
void G4PolyPhiFace::CalculateExtent(const EAxis axis, const G4VoxelLimits& voxelLimit,
                                       ...
                                       const G4ThreeVector& p, const G4ThreeVector& v)
{
  ...
  //
  if ((r < rMin - kCarTolerance) || (r > rMax + kCarTolerance))
  {
    return false;
  }

  if ((z < zMin - kCarTolerance) || (z > zMax + kCarTolerance))
  {
    return false;
  }
  ...
  //
  G4double qx = p.x() + v.x(), qy = p.y() + v.y(), qz = p.z() + v.z();

  G4int answer = 0;
  G4PolyPhiFaceVertex *corn = corners, *prev = corners + numEdges - 1;
...
// Decide if the point in r,z is inside the edges of our face
//
G4bool G4PolyPhiFace::InsideEdges(G4double r, G4double z, G4double* bestDist2,
                                  G4PolyPhiFaceVertex** base3Dnorm, G4ThreeVector** head3Dnorm)
{
  G4double bestDistance2 = kInfinity;
  G4int bestEdge = numEdges;
  G4bool answer = false;

  auto checkEdge = [&](G4int edgeIndex)
  {
    G4PolyPhiFaceEdge* edge = edges + edgeIndex;
    ...
    //
    if (distance2 < bestDistance2
        || (distance2 == bestDistance2 && edgeIndex < bestEdge))
    {
  ...
  if (fEdgeNode.empty())
  {
    ...
    do  // Loop checking, 13.08.2015, G.Cosmo
    {
      checkEdge(G4int(edge - edges));
    } while (++edge < edges + numEdges);
  }
  else
  {
    // Exact branch-and-bound nearest-edge search.  Node boxes are
    // conservative, so an edge is skipped only when it cannot improve the
    // distance selected by the original edge kernel.
    std::array<G4int, 64> stack;
    std::size_t stackSize = 0;
    stack[stackSize++] = 0;
    while (stackSize != 0)
    {
      const G4int inode = stack[--stackSize];
      const auto& node = fEdgeNode[inode];
      if (EdgeNodeDistance2(node, r, z) > bestDistance2) continue;
      if (node.count != 0)
      {
        for (G4int pos = node.begin; pos < node.begin + node.count; ++pos)
        {
          checkEdge(fEdgeOrder[pos]);
        }
        continue;
      }
      const G4double leftDistance = EdgeNodeDistance2(fEdgeNode[node.left], r, z);
      const G4double rightDistance = EdgeNodeDistance2(fEdgeNode[node.right], r, z);
      if (leftDistance < rightDistance)
      {
        if (rightDistance <= bestDistance2) stack[stackSize++] = node.right;
        if (leftDistance <= bestDistance2) stack[stackSize++] = node.left;
      }
      else
      {
        if (leftDistance <= bestDistance2) stack[stackSize++] = node.left;
...
//
G4double G4PolyPhiFace::SurfaceTriangle(const G4ThreeVector& p1, const G4ThreeVector& p2,
...
//
G4double G4PolyPhiFace::Area2(const G4TwoVector& a, const G4TwoVector& b, const G4TwoVector& c)
{
...
//
G4bool G4PolyPhiFace::Left(const G4TwoVector& a, const G4TwoVector& b, const G4TwoVector& c)
{
...
// Boolean function for sign of Surface
//
G4bool G4PolyPhiFace::LeftOn(const G4TwoVector& a, const G4TwoVector& b, const G4TwoVector& c)
{
  return Area2(a, b, c) >= 0;
}
...
// Boolean function for sign of Surface
//
G4bool G4PolyPhiFace::Collinear(const G4TwoVector& a, const G4TwoVector& b, const G4TwoVector& c)
{
  return Area2(a, b, c) == 0;
}
...
//
G4bool G4PolyPhiFace::IntersectProp(const G4TwoVector& a, const G4TwoVector& b,
...
//
G4bool G4PolyPhiFace::Intersect(const G4TwoVector& a, const G4TwoVector& b, const G4TwoVector& c,
                                const G4TwoVector& d)
{
...
G4bool G4PolyPhiFace::Diagonalie(G4PolyPhiFaceVertex* a, G4PolyPhiFaceVertex* b)
{
  ...
  do  // Loop checking, 13.08.2015, G.Cosmo
  {
    ...
    //
    if ((corner != a) && (corner_next != a) && (corner != b) && (corner_next != b))
    {
...
G4bool G4PolyPhiFace::InCone(G4PolyPhiFaceVertex* a, G4PolyPhiFaceVertex* b)
{
  ...
  arz = G4TwoVector(a->r, a->z);
  arz0 = G4TwoVector(a0->r, a0->z);
  arz1 = G4TwoVector(a1->r, a1->z);
  brz = G4TwoVector(b->r, b->z);
...
void G4PolyPhiFace::Triangulate()
{
  ...
  {  // Inner loop searches for one ear
    ...
    if (i >= max_n_loops)
    {
      G4Exception("G4PolyPhiFace::Triangulation()", "GeomSolids0003", FatalException,
