...
G4TwistTrapParallelSide::G4TwistTrapParallelSide(__void__& a) : G4VTwistSurface(a) {}

//=====================================================================
//* GetNormal ---------------------------------------------------------

G4ThreeVector G4TwistTrapParallelSide::GetNormal(const G4ThreeVector& tmpxx, G4bool isGlobal)
{
...
G4int G4TwistTrapParallelSide::DistanceToSurface(const G4ThreeVector& gp, const G4ThreeVector& gv,
                                                 ...
                                                 EValidate validate)
{
    ...
    if (std::fabs(p.z()) <= L)  // intersection possible in z
    {
      ...
      u = (2
             * (fdeltaY * phi * v.x() - fPhiTwist * p.y() * v.x() - fdeltaX * phi * v.y()
                + fPhiTwist * p.x() * v.y())
           + (fDy2plus1 * fPhiTwist + 2 * fDy2minus1 * phi)
               * (v.x() * std::cos(phi) + v.y() * std::sin(phi)))
  ...
  else  // general solution for non-zero vz
  {
    ...
    for (G4int i = 0; i < num; ++i)  // loop over all mathematical solutions
    {
      if (si[i] == 0.0)  // only real solutions
      {
        ...
        u = (1 / std::cos(phi)
             * (2 * phixz + 4 * fDz * phi * v.x() - 2 * fdeltaX * phi * v.z()
                + (fDy2plus1 * fPhiTwist + 2 * fDy2minus1 * phi) * v.z() * std::sin(phi)))
            / (2. * fPhiTwist * v.z());
  ...
  for (auto& k : xbuf)
  {
    ...
    if (IsConverged)
    {
      ...
      {
        G4Exception("G4TwistTrapParallelSide::DistanceToSurface()", "GeomSolids0001",
...
G4int G4TwistTrapParallelSide::DistanceToSurface(const G4ThreeVector& gp, G4ThreeVector gxx[],
...
G4int G4TwistTrapParallelSide::GetAreaCode(const G4ThreeVector& xx, G4bool withTol)
{
  ...
  G4Exception("G4TwistTrapParallelSide::GetAreaCode()", "GeomSolids0001", FatalException,
              "Feature NOT implemented !");
...
void G4TwistTrapParallelSide::SetCorners()
{
  ...
  {
    G4Exception("G4TwistTrapParallelSide::SetCorners()", "GeomSolids0001", FatalException,
...
void G4TwistTrapParallelSide::GetPhiUAtX(const G4ThreeVector& p, G4double& phi, G4double& u)
{
  ...
  u = ((-(fdeltaX * phi) + fPhiTwist * p.x()) * std::cos(phi)
       + (-(fdeltaY * phi) + fPhiTwist * p.y()) * std::sin(phi))
      / fPhiTwist;
}
...
G4ThreeVector G4TwistTrapParallelSide::ProjectPoint(const G4ThreeVector& p, G4bool isglobal)
{
...
void G4TwistTrapParallelSide::GetFacets(G4int k, G4int n, G4double xyz[][3], G4int faces[][4],
                                        G4int iside)
{
  G4double phi;
