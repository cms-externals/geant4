...
// on the cone.
//
G4bool G4IntersectingCone::HitOn(const G4double r, const G4double z) const
{
  //
...
G4int G4IntersectingCone::LineHitsCylinder(const G4ThreeVector& p, const G4ThreeVector& v,
                                           G4double* s1, G4double* s2) const
{
  static const G4double EPS = DBL_EPSILON;
  const G4double x0 = p.x(), y0 = p.y();
  const G4double tx = v.x(), ty = v.y();
  const G4double a = tx * tx + ty * ty;
  if (a < 1 / kInfinity)
  {
    return 0;
  }

  const G4double b = 2. * (x0 * tx + y0 * ty);
  const G4double c = x0 * x0 + y0 * y0 - A * A;
  ...
  const G4double sa = q / a;
  const G4double sb = c / q;
  if (sa < sb)
  {
    *s1 = sa;
    *s2 = sb;
                                        ...
                                        G4double* s1, G4double* s2) const
{
  ...
  if (B == 0.)
  {
    return LineHitsCylinder(p, v, s1, s2);
  }
