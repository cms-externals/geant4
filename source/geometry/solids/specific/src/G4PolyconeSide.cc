...
// order in (r,z).
//
G4PolyconeSide::G4PolyconeSide(const G4PolyconeSideRZ* prevRZ, const G4PolyconeSideRZ* tail,
                               const G4PolyconeSideRZ* head, const G4PolyconeSideRZ* nextRZ,
                               G4double thePhiStart, G4double theDeltaPhi, G4bool thePhiIsOpen,
                               G4bool isAllBehind)
{
  ...
  if (phiIsOpen)
  {
    ...
    corners[0] = G4ThreeVector(tail->r * std::cos(startPhi), tail->r * std::sin(startPhi), tail->z);
    corners[1] = G4ThreeVector(head->r * std::cos(startPhi), head->r * std::sin(startPhi), head->z);
...
// Intersect
//
G4bool G4PolyconeSide::Intersect(const G4ThreeVector& p, const G4ThreeVector& v, G4bool outgoing,
...
EInside G4PolyconeSide::Inside(const G4ThreeVector& p, G4double tolerance, G4double* bestDistance)
{
  ...
  *bestDistance = std::sqrt(dist2);
  ...
  //
  if ((std::fabs(edgeRZnorm) < tolerance) && (distOut2 < tolerance * tolerance))
  {
    return kSurface;
  }
  if (edgeRZnorm < 0)
...
G4ThreeVector G4PolyconeSide::Normal(const G4ThreeVector& p, G4double* bestDistance)
{
  ...
  *bestDistance = std::sqrt(dFrom * dFrom + dOut2);
  ...
  if (rds != 0.)
  {
    return {rNorm * p.x() / rds, rNorm * p.y() / rds, zNorm};
  }
...
G4double G4PolyconeSide::Extent(const G4ThreeVector axis)
{
  ...
  if (phiIsOpen)
  {
    ...
    if (phi > deltaPhi + startPhi)
    {
      ...
      G4double ad = axis.dot(a), bd = axis.dot(b), cd = axis.dot(c), dd = axis.dot(d);
...
//
void G4PolyconeSide::CalculateExtent(const EAxis axis, const G4VoxelLimits& voxelLimit,
                                     ...
                                     G4SolidExtentList& extentList)
{
  ...
  if (rNorm < -DBL_MIN)
  {
    ...
    if (prevZS > DBL_MIN)
    {
      ...
      else
      {
        ...
        //
        FindLineIntersect(z0, r0, zS, rS, z0, r0 * rFudge, prevZS, prevRS * rFudge, z0, r0);
      }
    }
    ...
    if (nextZS > DBL_MIN && (rS * nextZS - zS * nextRS < 0))
    {
      ...
      //
      FindLineIntersect(z1, r1, zS, rS, z1, r1 * rFudge, nextZS, nextRS * rFudge, z1, r1);
    }
  }
  else if (rNorm > DBL_MIN)
  {
    ...
    if (prevZS < -DBL_MIN)
    {
      ...
      else
      {
        ...
        // Transition was concave: short this side
        //
        FindLineIntersect(z0, r0, zS, rS * rFudge, z0, r[0], prevZS, prevRS, z0, r0);
      }
    }
    ...
    if (nextZS < -DBL_MIN && (rS * nextZS - zS * nextRS < 0))
    {
      ...
      // concave transition: short this side
      //
      FindLineIntersect(z1, r1, zS, rS * rFudge, z1, r[1], nextZS, nextRS, z1, r1);
    }
  }
  else
  {
  ...
  // Loop
  //
  G4double phi = startPhi, cosPhi = std::cos(phi), sinPhi = std::sin(phi);

  G4ThreeVector v0(r0 * cosPhi, r0 * sinPhi, z0), v1(r1 * cosPhi, r1 * sinPhi, z1), v2, w0, w1, w2;
  ...
  do  // Loop checking, 13.08.2015, G.Cosmo
  {
    ...
    if (numPhi == 1)
    {
      phi = startPhi + deltaPhi;
    }  // Try to avoid roundoff
  ...
  if (phiIsOpen && rNorm > DBL_MIN)
  {
    ...
    G4ThreeVector a0(r[0] * cosPhi, r[0] * sinPhi, z[0]), a1(r[1] * cosPhi, r[1] * sinPhi, z[1]),
      b0(r0 * cosPhi, r0 * sinPhi, z[0]), b1(r1 * cosPhi, r1 * sinPhi, z[1]);
...
//
G4double G4PolyconeSide::DistanceAway(const G4ThreeVector& p, G4double& distOutside2,
                                      G4double* edgeRZnorm)
{
  //
  // Convert our point to r and z
                                   ...
                                   G4ThreeVector& normal)
{
  ...
  if (phiIsOpen)
  {
    ...
    if (phi > startPhi + deltaPhi - phiTolerant)
    {
      ...
      G4ThreeVector qa = qx - corners[2], qb = qx - corners[3];
    ...
    {
      G4ThreeVector qx = p + v;
      G4ThreeVector qa = qx - corners[1], qb = qx - corners[0];
      G4ThreeVector qacb = qa.cross(qb);
...
//
void G4PolyconeSide::FindLineIntersect(G4double x1, G4double y1, G4double tx1, G4double ty1,
                                       G4double x2, G4double y2, G4double tx2, G4double ty2,
