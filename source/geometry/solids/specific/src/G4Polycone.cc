                        ...
                        G4ReduciblePolygon* rz)
{
  ...
  do  // Loop checking, 13.08.2015, G.Cosmo
  {
    ...
    else
    {
      ...
      //
      allBehind = profileIsConvex
               || !rz->BisectedBy(corner->r, corner->z, next->r, next->z,
                                   kCarTolerance);
    }
...
// BuildRZIndex
//
void G4Polycone::BuildRZIndex(G4bool profileIsConvex)
{
  fRZSection.clear();
  fRZCone.clear();
  fRZVoxel.reset();
...
EInside G4Polycone::InsideZPlane(const G4ThreeVector& p) const
{
  if (!HasZPlaneInsideFastPath())
  {
    return fRZConvex ? InsideRZOnly(p)
                     : InsideRZSections(p.perp(), p.z(), fRZSection, fRZVoxel);
  }
  ...
  if (fRZVoxel != nullptr && !fRZVoxel->candidate.empty() && fRZVoxel->boundary.size() >= 2)
  {
    ...
    else
    {
      ...
      for (; slab >= 0 && slab < nSlab; slab += step)
      {
        ...
        // A ray's squared cylindrical radius is a convex quadratic in
        // distance.  Its exact minimum over this Z slab gives a safe radial
        // rejection against the slab's conservative outer radius.
        if (!outgoing && slab < G4int(fRZVoxel->rMax.size()))
        {
          G4double t0 = (fRZVoxel->boundary[slab] - p.z()) / v.z();
          G4double t1 = (fRZVoxel->boundary[slab + 1] - p.z()) / v.z();
          if (t0 > t1) std::swap(t0, t1);
          t0 = std::max(0., t0);
          t1 = std::min(distance, t1);
          if (t0 <= t1)
          {
            const G4double rLimit = fRZVoxel->rMax[slab] + tolerance;
            if (RayMissesRadialLimit(p, v, t0, t1, rLimit))
            {
              continue;
            }
          }
...
EInside G4Polycone::Inside(const G4ThreeVector& p) const
{
  ...
  // Long answer
  //
  if (fRZSection.empty()) { return G4VCSGfaceted::Inside(p); }
  if (fRZConvex || fZPlanesMonotonic) { return InsideRZ(p); }
  const EInside rz = InsideRZSections(p.perp(), p.z(), fRZSection, fRZVoxel);
...
{
  return !fRZSection.empty() ? SurfaceNormalRZ(p)
                             : G4VCSGfaceted::SurfaceNormal(p);
}
                                  ...
                                  const G4ThreeVector& v) const
{
  ...
  // Long answer
  //
  if ((fRZConvex || fZPlanesMonotonic) && !fRZSection.empty())
  {
