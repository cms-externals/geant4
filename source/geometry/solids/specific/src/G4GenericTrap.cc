...
G4GenericTrap::G4GenericTrap(__void__& a) : G4VSolid(a) {}
...
G4GenericTrap::G4GenericTrap(const G4GenericTrap& rhs)
  : G4VSolid(rhs),
    ...
    fMinBBox(rhs.fMinBBox),
    fMaxBBox(rhs.fMaxBBox),
    ...
    fCubicVolume(rhs.fCubicVolume)
{
  for (auto i = 0; i < 5; ++i)
  {
    fTwist[i] = rhs.fTwist[i];
  }
  for (auto i = 0; i < 4; ++i)
  {
    fDelta[i] = rhs.fDelta[i];
  }
  for (auto i = 0; i < 8; ++i)
  {
    fPlane[i] = rhs.fPlane[i];
  }
  for (auto i = 0; i < 4; ++i)
  {
    fSurf[i] = rhs.fSurf[i];
...
EInside G4GenericTrap::Inside(const G4ThreeVector& p) const
{
  ...
  return (dist > halfTolerance) ? kOutside : ((dist > -halfTolerance) ? kSurface : kInside);
}
...
#  ifdef G4SPECSDEBUG
  ...
  message << "Point p is not on surface of solid: " << GetName() << " !?\n"
          << "Position: " << p << " is " << ((Inside(p) == kInside) ? "inside" : "outside") << "\n";
  StreamInfo(message);
  G4Exception("G4GenericTrap::ApproxSurfaceNormal(p)", "GeomSolids1002", JustWarning, message);
#  endif
  G4double px = p.x(), py = p.y(), pz = p.z();
  ...
  G4double z =
    fSurf[iside].A * px + fSurf[iside].B * py + 2. * fSurf[iside].C * pz + fSurf[iside].F;
  G4double inv = 1. / std::sqrt(x * x + y * y + z * z);
...
G4double G4GenericTrap::DistanceToIn(const G4ThreeVector& p, const G4ThreeVector& v) const
{
  ...
  for (auto k = 0; k < 4; ++k)
  {
    ...
    else
    {
      if (cosa > 0)
      {
        xout = std::min(-dist / cosa, xout);
      }
      if (cosa < 0)
      {
        xin = std::max(-dist / cosa, xin);
      }
    }
  }
  ...
  for (auto i = 0; i < 4; ++i)
  {
    ...
    else
    {
      if (cosa > 0)
      {
        tout = std::min(-dist / cosa, tout);
      }
      if (cosa < 0)
      {
        tin = std::max(-dist / cosa, tin);
      }
    }
  ...
  if (x0 < fMinBBox.x() - halfTolerance || y0 < fMinBBox.y() - halfTolerance
      || z0 < fMinBBox.z() - halfTolerance || x0 > fMaxBBox.x() + halfTolerance
      || y0 > fMaxBBox.y() + halfTolerance || z0 > fMaxBBox.z() + halfTolerance)
  {
    t0 = tin;
    x0 += vx * t0;
    y0 += vy * t0;
  ...
  if (tin - xin < halfTolerance)
  {
    ttin[0] = xin;
  }
  if (xout - tout < halfTolerance)
  {
    ttout[0] = xout;
  }
  ...
  for (auto i = 0; i < 4; ++i)
  {
    ...
    if (std::abs(A) <= EPSILON)
    {
      ...
      if (nx * vx + ny * vy + nz * vz >= 0.)  // point is flying to outside
      {
        ...
        if (tmp < tminimal || tmp > tmaximal)
        {
          continue;
        }
        if (std::abs(tmp - ttout[0]) < halfTolerance)
        {
          continue;
        }
        if (tmp < ttout[0])
      ...
      else  // point is flying to inside
      {
        ...
        if (std::abs(tmp - ttin[0]) < halfTolerance)
        {
          continue;
        }
        if (tmp < ttin[0])
    ...
    if (tsurfout >= tminimal && tsurfout <= tmaximal)
    {
      if (std::abs(tsurfout - ttout[0]) >= halfTolerance)
      {
        ...
        {
          ttout[1] = std::min(tsurfout, ttout[1]);
        }
      }
    }
  }
  ...
  if (ttin[1] == DBL_MAX)
  {
    ...
    G4double dist =
      (distout <= halfTolerance || distout - distin <= halfTolerance) ? kInfinity : distin;
  ...
  G4double dist1 =
    (distout1 <= halfTolerance || distout1 - distin1 <= halfTolerance) ? kInfinity : distin1;
  if (dist1 != kInfinity)
  {
  ...
  G4double dist2 =
    (distout2 <= halfTolerance || distout2 - distin2 <= halfTolerance) ? kInfinity : distin2;
...
//
G4double G4GenericTrap::DistanceToOut(const G4ThreeVector& p, const G4ThreeVector& v,
...
// Return bounding box
//
void G4GenericTrap::BoundingLimits(G4ThreeVector& pMin, G4ThreeVector& pMax) const
{
  pMin = fMinBBox;
  pMax = fMaxBBox;
}
...
//
G4bool G4GenericTrap::CalculateExtent(const EAxis pAxis, const G4VoxelLimits& pVoxelLimit,
...
G4ThreeVector G4GenericTrap::GetPointOnSurface() const
{
  ...
  constexpr G4int iface[6][4] = {{0, 1, 2, 3}, {0, 4, 5, 1}, {1, 5, 6, 2},
                                 {2, 6, 7, 3}, {3, 7, 4, 0}, {4, 5, 6, 7}};
...
//
void G4GenericTrap::CheckParameters(G4double halfZ, const std::vector<G4TwoVector>& vertices)
{
  ...
  if (vertices.size() != 8)
  {
    ...
    G4Exception("G4GenericTrap::CheckParameters()", "GeomSolids0002", FatalException, message);
  }
  ...
  if (std::abs(zbot) < ldiagbot * kCarTolerance)
  {
    zbot = 0.;
  }
  ...
  if (std::abs(ztop) < ldiagtop * kCarTolerance)
  {
    ztop = 0.;
  }
  ...
  if ((zbot > 0.) || (ztop > 0.))
  {
    ...
    G4Exception("G4GenericTrap::CheckParameters()", "GeomSolids1001", JustWarning, message);
  }
  ...
  if (ndegenerated > 1 || GetCubicVolume() < fDz * std::max(ldiagbot, ldiagtop) * kCarTolerance)
  {
    std::ostringstream message;
  ...
  for (auto i = 0; i < 4; ++i)
  {
    ...
    {  // hyperbolic paraboloid: Axz + Byz + Czz + Dx + Ey + Fz + G = 0
      ...
      if (angle > CLHEP::halfpi)
      {
        ...
        StreamInfo(message);
        G4Exception("G4GenericTrap::ComputeLateralSurfaces()", "GeomSolids0002", FatalException,
                    message);
      }
      ...
      fSurf[i].F =
        2. * fDz * (p4.x() * p3.y() - p3.x() * p4.y() - p2.x() * p1.y() + p1.x() * p2.y());
      fSurf[i].G =
        fDz * fDz * ((p4.x() + p2.x()) * (p3.y() + p1.y()) - (p3.x() + p1.x()) * (p4.y() + p2.y()));
      G4double magnitude = G4ThreeVector(fSurf[i].D, fSurf[i].E, fSurf[i].F).mag();
      if (magnitude < kCarTolerance)
      {
        continue;
      }
      fSurf[i].A /= magnitude;
      ...
      G4double k =
        std::abs(fSurf[i].C) + std::sqrt(sqr(fSurf[i].A) + sqr(fSurf[i].B) + sqr(fSurf[i].C));
      f4k[i] = 4. * k;
...
void G4GenericTrap::ComputeBoundingBox()
{
  ...
  if (minX >= maxX || minY >= maxY || -fDz >= fDz)
  {
    ...
    message << "Bad bounding box (min >= max) for solid: " << GetName() << " !"
            << "\npMin = " << fMinBBox << "\npMax = " << fMaxBBox;
    G4Exception("G4GenericTrap::ComputeBoundingBox()", "GeomSolids1001", JustWarning, message);
    DumpInfo();
  }
}
...
{
  return {fMinBBox.x(), fMaxBBox.x(), fMinBBox.y(), fMaxBBox.y(), fMinBBox.z(), fMaxBBox.z()};
}
...
//
void G4GenericTrap::WarningSignB(const G4String& method, const G4String& icase, G4double f,
                                 G4double B, const G4ThreeVector& p, const G4ThreeVector& v) const
{
  std::ostringstream message;
  message.precision(16);
  message << icase << " in " << GetName() << "\n"
...
void G4GenericTrap::WarningDistanceToIn(G4int k, const G4ThreeVector& p, const G4ThreeVector& v,
                                        G4double tmin, G4double tmax, const G4double ttin[2],
                                        const G4double ttout[2]) const
{
  G4String check = "";
  if (ttin[1] != DBL_MAX)
  {
  ...
  message << k << "_Unexpected sequence of intersections in solid: " << GetName() << " !?\n"
          << "   position = "
          << ((position == kInside) ? "kInside"
                                    : ((position == kOutside) ? "kOutside" : "kSurface"))
          << "\n"
          << "   p" << p << "\n"
          << "   v" << v << "\n"
          << "   range    : [" << tmin << ", " << tmax << "]\n"
          << "   ttin[2]  : " << ((ttin[0] == DBL_MAX) ? kInfinity : ttin[0]) << ", "
          << ((ttin[1] == DBL_MAX) ? kInfinity : ttin[1]) << "\n"
          << "   ttout[2] : " << ((ttout[0] == DBL_MAX) ? kInfinity : ttout[0]) << ", "
          << ((ttout[1] == DBL_MAX) ? kInfinity : ttout[1]) << check << "\n";
  StreamInfo(message);
  G4Exception("G4GenericTrap::DistanceToIn(p,v)", "GeomSolids1002", JustWarning, message);
}
...
//
void G4GenericTrap::WarningDistanceToOut(const G4ThreeVector& p, const G4ThreeVector& v,
                                         G4double tout) const
{
  auto position = Inside(p);
  std::ostringstream message;
  ...
  message << "Unexpected final tout = " << tout << " in solid: " << GetName() << " !?\n"
          ...
          << "   v" << v << "\n";
  StreamInfo(message);
  G4Exception("G4GenericTrap::DistanceToOut(p,v)", "GeomSolids1002", JustWarning, message);
}
