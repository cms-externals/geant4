...
//
G4UGenericPolycone::G4UGenericPolycone(const G4String& name, G4double phiStart, G4double phiTotal,
...
// Copy constructor
//
G4UGenericPolycone::G4UGenericPolycone(const G4UGenericPolycone& source) : Base_t(source)
{
...
void G4UGenericPolycone::BoundingLimits(G4ThreeVector& pMin, G4ThreeVector& pMax) const
{
  ...
  if (pMin.x() >= pMax.x() || pMin.y() >= pMax.y() || pMin.z() >= pMax.z())
  {
    ...
    message << "Bad bounding box (min >= max) for solid: " << GetName() << " !"
            << "\npMin = " << pMin << "\npMax = " << pMax;
    G4Exception("G4UGenericPolycone::BoundingLimits()", "GeomMgt0001", JustWarning, message);
    StreamInfo(G4cout);
  }
}
...
G4bool G4UGenericPolycone::CalculateExtent(const EAxis pAxis, const G4VoxelLimits& pVoxelLimit,
...
#  endif
  ...
  if (!G4GeomTools::TriangulatePolygon(contourRZ, triangles))
  {
    ...
    G4Exception("G4UGenericPolycone::CalculateExtent()", "GeomMgt1002", JustWarning, message);
    return bbox.CalculateExtent(pAxis, pVoxelLimit, pTransform, pMin, pMax);
  }
