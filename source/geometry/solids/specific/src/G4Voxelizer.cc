...
//         Gabriele Cosmo (CERN), 08.07.2026 - Extended to axes voxelisation
// --------------------------------------------------------------------
...
//______________________________________________________________________________
unsigned int G4Voxelizer::GetCandidateMaskWord(G4int word,
                                               G4int totalCandidates) const
{
  constexpr G4int bitsPerWord = 8 * sizeof(unsigned int);
  G4int remaining = totalCandidates - word * bitsPerWord;
                                     ...
                                     G4ThreeVector reductionRatio)
{
  for (auto k = 0; k <= 2; ++k)
  {
    ...
    if (reduction == 0)
    {
      ...
      if (boundary.size() > 2)
      {
        std::vector<G4double> reducedBoundary(2);
        reducedBoundary[0] = boundary.front();
        reducedBoundary[1] = boundary.back();
        boundary = std::move(reducedBoundary);
      }
      continue;
    }
                                           ...
                                           G4SurfBits* crossed) const
{
  ...
  if (fNPerSlice == 1)
  {
    unsigned int mask = GetCandidateMaskWord(0, fTotalCandidates);
    G4int slice;
  ...
  else
  {
    ...
    for (G4int i = 0; i < fNPerSlice; ++i)
    {
      ...
      //
      mask = GetCandidateMaskWord(i, fTotalCandidates);
      for (auto axis = 0; axis <= 2; ++axis)
      {
        if ((fBoundaries[axis].size() > 2) && ((mask &= masks[axis][i]) == 0u))
        {
          break;
        }
      }
                                           ...
                                           G4SurfBits* crossed) const
{
  ...
  if (fNPerSlice == 1)
  {
    ...
    for (auto axis = 0; axis <= 2; ++axis)
    {
      if ((fBoundaries[axis].size() > 2)
          && ((mask &= ((unsigned int*)bitmasks[axis].fAllBits)[voxels[axis]]) == 0u))
      {
        return 0;
      }
    }
