...
G4TwistTrapAlphaSide::G4TwistTrapAlphaSide(__void__& a) : G4VTwistSurface(a) {}

//=====================================================================
//* GetNormal ---------------------------------------------------------

G4ThreeVector G4TwistTrapAlphaSide::GetNormal(const G4ThreeVector& tmpxx, G4bool isGlobal)
{
...
G4int G4TwistTrapAlphaSide::DistanceToSurface(const G4ThreeVector& gp, const G4ThreeVector& gv,
                                              G4ThreeVector gxx[], G4double distance[],
                                              G4int areacode[], G4bool isvalid[],
                                              EValidate validate)
{
  ...
  if (v.z() == 0.)
  {
    if (std::fabs(p.z()) <= L)  // intersection possible in z
    {
      ...
      u = (fDy1
           * (4
                * (-(fdeltaY * phi * v.x()) + fPhiTwist * p.y() * v.x() + fdeltaX * phi * v.y()
                   - fPhiTwist * p.x() * v.y())
              + ((fDx3plus1 + fDx4plus2) * fPhiTwist + 2 * (fDx3minus1 + fDx4minus2) * phi)
                  * (v.y() * std::cos(phi) - v.x() * std::sin(phi))))
          / (fPhiTwist * (4 * fDy1 * v.x() - (fa1md1 + 4 * fDy1 * fTAlph) * v.y()) * std::cos(phi)
             + fPhiTwist * (fa1md1 * v.x() + 4 * fDy1 * (fTAlph * v.x() + v.y())) * std::sin(phi));
      xbuftmp.phi = phi;
      xbuftmp.u = u;
  ...
  else  // general solution for non-zero vz
  {
    ...
    c[7] =
      57600 * fDy1
      * (fa1md1 * phiyz
         + fDy1 * (-4 * phixz + 4 * fTAlph * phiyz + (fDx3plus1 + fDx4plus2) * fPhiTwist * v.z()));
    c[6] = -57600 * fDy1
           * (4 * fDy1 * (phiyz + 2 * fDz * v.x() + fTAlph * (phixz - 2 * fDz * v.y()))
              - 2 * fDy1 * (2 * fdeltaX + fDx3minus1 + fDx4minus2 - 2 * fdeltaY * fTAlph) * v.z()
              + fa1md1 * (phixz - 2 * fDz * v.y() + fdeltaY * v.z()));
    c[5] =
      4800 * fDy1
      * (fa1md1 * (-5 * phiyz - 24 * fDz * v.x() + 12 * fdeltaX * v.z())
         + fDy1
             * (20 * phixz - 4 * (5 * fTAlph * phiyz + 24 * fDz * fTAlph * v.x() + 24 * fDz * v.y())
                + (48 * fdeltaY + (fDx3plus1 + fDx4plus2) * fPhiTwist + 48 * fdeltaX * fTAlph)
                    * v.z()));
    c[4] =
      4800 * fDy1
      * (fa1md1 * (phixz - 10 * fDz * v.y() + 5 * fdeltaY * v.z())
         + 2 * fDy1
             * (2 * phiyz + 20 * fDz * v.x() + (-10 * fdeltaX + fDx3minus1 + fDx4minus2) * v.z()
                + 2 * fTAlph * (phixz - 10 * fDz * v.y() + 5 * fdeltaY * v.z())));
    c[3] = -96 * fDy1
           * (-(fa1md1 * (phiyz + 100 * fDz * v.x() - 50 * fdeltaX * v.z()))
              + fDy1
                  * (4 * phixz - 400 * fDz * v.y()
    ...
    c[2] =
      32 * fDy1
      * (4 * fDy1 * (7 * fTAlph * phixz + 7 * phiyz - 6 * fDz * v.x() + 6 * fDz * fTAlph * v.y())
         + 6 * fDy1 * (2 * fdeltaX + fDx3minus1 + fDx4minus2 - 2 * fdeltaY * fTAlph) * v.z()
         + fa1md1 * (7 * phixz + 6 * fDz * v.y() - 3 * fdeltaY * v.z()));
    c[1] = -8 * fDy1
           * (fa1md1 * (-9 * phiyz - 56 * fDz * v.x() + 28 * fdeltaX * v.z())
              + 4 * fDy1
                  * (9 * phixz - 9 * fTAlph * phiyz - 56 * fDz * fTAlph * v.x() - 56 * fDz * v.y()
                     + 28 * (fdeltaY + fdeltaX * fTAlph) * v.z()));
    c[0] = 72 * fDy1
           * (fa1md1 * (2 * fDz * v.y() - fdeltaY * v.z())
              + fDy1
                  * (-8 * fDz * v.x() + 8 * fDz * fTAlph * v.y() + 4 * fdeltaX * v.z()
                     - 4 * fdeltaY * fTAlph * v.z()));

#ifdef G4TWISTDEBUG
    G4cout << "coef = " << c[0] << " " << c[1] << " " << c[2] << " " << c[3] << " " << c[4] << " "
           << c[5] << " " << c[6] << " " << c[7] << G4endl;
#endif
    ...
    for (G4int i = 0; i < num; i++)  // loop over all math solutions
    {
      if (si[i] == 0.0)  // only real solutions
      {
        ...
        u = (fDy1
             * (4 * (phiyz + 2 * fDz * phi * v.y() - fdeltaY * phi * v.z())
                - ((fDx3plus1 + fDx4plus2) * fPhiTwist + 2 * (fDx3minus1 + fDx4minus2) * phi)
                    * v.z() * std::sin(phi)))
            / (fPhiTwist * v.z()
               * (4 * fDy1 * std::cos(phi) + (fa1md1 + 4 * fDy1 * fTAlph) * std::sin(phi)));
        xbuftmp.phi = phi;
        xbuftmp.u = u;
        xbuftmp.areacode = sOutside;
        xbuftmp.distance = kInfinity;
  ...
  for (auto& k : xbuf)
  {
#ifdef G4TWISTDEBUG
    G4cout << "Solution " << k << " : "
           << "reconstructed phiR = " << xbuf[k].phi << ", uR = " << xbuf[k].u << G4endl;
#endif
    ...
    if (IsConverged)
    {
      ...
      {
        G4Exception("G4TwistTrapAlphaSide::DistanceToSurface()", "GeomSolids0001", FatalException,
  ...
  if (nxxtmp < 2 || IsParallel)  // positive end
  {
    ...
    for (std::size_t k = nxxtmp; k < xbuf.size(); ++k)
    {
      ...
      if (IsConverged)
      {
        ...
        {
          G4Exception("G4TwistedBoxSide::DistanceToSurface()", "GeomSolids0001", FatalException,
                      "Feature NOT implemented !");
        }
      }
      else
...
#ifdef G4TWISTDEBUG
    G4cout << "element Nr. " << i << ", local Intersection = " << xbuf[i].xx
           << ", distance = " << xbuf[i].distance << ", u = " << xbuf[i].u
           << ", phi = " << xbuf[i].phi << ", isvalid = " << xbuf[i].isvalid << G4endl;
#endif
...
G4int G4TwistTrapAlphaSide::DistanceToSurface(const G4ThreeVector& gp, G4ThreeVector gxx[],
...
G4int G4TwistTrapAlphaSide::GetAreaCode(const G4ThreeVector& xx, G4bool withTol)
{
  ...
  G4Exception("G4TwistTrapAlphaSide::GetAreaCode()", "GeomSolids0001", FatalException,
              "Feature NOT implemented !");
...
void G4TwistTrapAlphaSide::SetCorners()
{
  ...
  {
    G4Exception("G4TwistTrapAlphaSide::SetCorners()", "GeomSolids0001", FatalException,
...
void G4TwistTrapAlphaSide::SetBoundaries()
{
  ...
  if (fAxis[0] == kYAxis && fAxis[1] == kZAxis)
  {
    ...
    SetBoundary(sAxis0 & (sAxisY | sAxisMin), direction, GetCorner(sC0Min1Min), sAxisZ);

    // sAxis0 & sAxisMax
    direction = GetCorner(sC0Max1Max) - GetCorner(sC0Max1Min);
    direction = direction.unit();
    SetBoundary(sAxis0 & (sAxisY | sAxisMax), direction, GetCorner(sC0Max1Min), sAxisZ);

    // sAxis1 & sAxisMin
    direction = GetCorner(sC0Max1Min) - GetCorner(sC0Min1Min);
    direction = direction.unit();
    SetBoundary(sAxis1 & (sAxisZ | sAxisMin), direction, GetCorner(sC0Min1Min), sAxisY);

    // sAxis1 & sAxisMax
    direction = GetCorner(sC0Max1Max) - GetCorner(sC0Min1Max);
    direction = direction.unit();
    SetBoundary(sAxis1 & (sAxisZ | sAxisMax), direction, GetCorner(sC0Min1Max), sAxisY);
  }
  else
  {
    G4Exception("G4TwistTrapAlphaSide::SetCorners()", "GeomSolids0001", FatalException,
...
void G4TwistTrapAlphaSide::GetPhiUAtX(const G4ThreeVector& p, G4double& phi, G4double& u)
{
  ...
  u =
    (fPhiTwist
       * (2 * fDx1 * fDx1 - 2 * fDx2 * fDx2 - fa1md1 * (fDx3 + fDx4)
          - 4 * (fDx3plus1 + fDx4plus2) * fDy1 * fTAlph)
     - 2
         * (2 * fDx1 * fDx1 - 2 * fDx2 * fDx2 + fa1md1 * (fDx3 + fDx4)
            + 4 * (fDx3minus1 + fDx4minus2) * fDy1 * fTAlph)
         * phi
     - 4
         * (fa1md1 * (fdeltaX * phi - fPhiTwist * p.x())
            + 4 * fDy1
                * (fdeltaY * phi + fdeltaX * fTAlph * phi - fPhiTwist * (fTAlph * p.x() + p.y())))
         * std::cos(phi)
     - 4
         * (fa1md1 * fdeltaY * phi - 4 * fdeltaX * fDy1 * phi + 4 * fdeltaY * fDy1 * fTAlph * phi
            + 4 * fDy1 * fPhiTwist * p.x() - fPhiTwist * (fa1md1 + 4 * fDy1 * fTAlph) * p.y())
         * std::sin(phi))
    / (fDy1 * fPhiTwist
       * ((std::fabs(((fa1md1 + 4 * fDy1 * fTAlph) * std::cos(phi)) / fDy1 - 4 * std::sin(phi)))
            * (std::fabs(((fa1md1 + 4 * fDy1 * fTAlph) * std::cos(phi)) / fDy1 - 4 * std::sin(phi)))
          + (std::fabs(4 * std::cos(phi) + ((fa1md1 + 4 * fDy1 * fTAlph) * std::sin(phi)) / fDy1))
...
G4ThreeVector G4TwistTrapAlphaSide::ProjectPoint(const G4ThreeVector& p, G4bool isglobal)
{
...
void G4TwistTrapAlphaSide::GetFacets(G4int k, G4int n, G4double xyz[][3], G4int faces[][4],
                                     G4int iside)
{
  G4double phi;
  G4double b;
  ...
  for (G4int i = 0; i < n; ++i)
  {
    ...
    for (G4int j = 0; j < k; ++j)
    {
      ...
      if (i < n - 1 && j < k - 1)  // conterclock wise filling
      {
        ...
        faces[nface][0] =
          GetEdgeVisibility(i, j, k, n, 0, -1) * (GetNode(i, j, k, n, iside) + 1);  // f77 numbering
        faces[nface][1] =
          GetEdgeVisibility(i, j, k, n, 1, -1) * (GetNode(i, j + 1, k, n, iside) + 1);
        faces[nface][2] =
          GetEdgeVisibility(i, j, k, n, 2, -1) * (GetNode(i + 1, j + 1, k, n, iside) + 1);
        faces[nface][3] =
          GetEdgeVisibility(i, j, k, n, 3, -1) * (GetNode(i + 1, j, k, n, iside) + 1);
      }
    }
  }
}
