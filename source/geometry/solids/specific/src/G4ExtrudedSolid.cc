...
G4ExtrudedSolid::G4ExtrudedSolid(const G4String& pName, const std::vector<G4TwoVector>& polygon,
    ...
    fGeometryType("G4ExtrudedSolid")
{
  ...
  if (fNv < 3)
  {
    ...
    G4Exception("G4ExtrudedSolid::G4ExtrudedSolid()", "GeomSolids0002", FatalErrorInArgument,
                message);
  }
  ...
  for (std::size_t i = 0; i < fNz - 1; ++i)
  {
    ...
    {
      std::ostringstream message;
      message << "Z-sections have to be ordered by z value (z0 < z1 < z2...) - " << pName;
      G4Exception("G4ExtrudedSolid::G4ExtrudedSolid()", "GeomSolids0002", FatalErrorInArgument,
                  message);
    }
    ...
    {
      std::ostringstream message;
      message << "Z-sections with the same z position are not supported - " << pName;
      G4Exception("G4ExtrudedSolid::G4ExtrudedSolid()", "GeomSolids0001", FatalException, message);
    }
  }
  ...
  G4GeomTools::RemoveRedundantVertices(fPolygon, removedVertices, 2 * kCarTolerance);
  if (!removedVertices.empty())
  {
    std::size_t nremoved = removedVertices.size();
    std::ostringstream message;
    message << "The following " << nremoved << " vertices have been removed from polygon in "
            << pName << "\nas collinear or coincident with other vertices: " << removedVertices[0];
    for (std::size_t i = 1; i < nremoved; ++i)
    {
      message << ", " << removedVertices[i];
    }
    G4Exception("G4ExtrudedSolid::G4ExtrudedSolid()", "GeomSolids1001", JustWarning, message);
  }
  ...
  if (!result)
  {
    ...
    G4Exception("G4ExtrudedSolid::G4ExtrudedSolid()", "GeomSolids0003", FatalException, message);
  }
  ...
  //
  if ((fNz == 2) && (fZSections[0].fScale == 1) && (fZSections[1].fScale == 1)
...
G4ExtrudedSolid::G4ExtrudedSolid(const G4String& pName, const std::vector<G4TwoVector>& polygon,
                                 G4double dz, const G4TwoVector& off1, G4double scale1,
                                 const G4TwoVector& off2, G4double scale2)
  : G4TessellatedSolid(pName), fNv(polygon.size()), fNz(2), fGeometryType("G4ExtrudedSolid")
{
  ...
  // Check if the solid is a right prism, if so then set lateral planes
  //
  if ((scale1 == 1) && (scale2 == 1) && (off1 == G4TwoVector(0, 0)) && (off2 == G4TwoVector(0, 0)))
  {
    fSolidType = (fIsConvex) ? 1 : 2;  // 1 - convex, 2 - non-convex right prism
    ComputeLateralPlanes();
  }
...
G4ThreeVector G4ExtrudedSolid::GetVertex(G4int iz, G4int ind) const
{
  ...
  return {fPolygon[ind].x() * fZSections[iz].fScale + fZSections[iz].fOffset.x(),
...
G4bool G4ExtrudedSolid::IsSameLine(const G4TwoVector& p, const G4TwoVector& l1,
                                   const G4TwoVector& l2) const
{
  ...
  //
  G4bool squareComp = (dy * dy < (1 + slope * slope) * kCarToleranceHalf * kCarToleranceHalf);
...
G4bool G4ExtrudedSolid::IsSameLineSegment(const G4TwoVector& p, const G4TwoVector& l1,
                                          const G4TwoVector& l2) const
{
...
G4bool G4ExtrudedSolid::IsSameSide(const G4TwoVector& p1, const G4TwoVector& p2,
                                   const G4TwoVector& l1, const G4TwoVector& l2) const
{
  // Return true if p1 and p2 are on the same side of the line through l1, l2

  return ((p1.x() - l1.x()) * (l2.y() - l1.y()) - (l2.x() - l1.x()) * (p1.y() - l1.y()))
           * ((p2.x() - l1.x()) * (l2.y() - l1.y()) - (l2.x() - l1.x()) * (p2.y() - l1.y()))
         > 0;
}

//_____________________________________________________________________________

G4bool G4ExtrudedSolid::IsPointInside(const G4TwoVector& a, const G4TwoVector& b,
                                      const G4TwoVector& c, const G4TwoVector& p) const
{
  ...
  G4bool inside = IsSameSide(p, a, b, c) && IsSameSide(p, b, a, c) && IsSameSide(p, c, a, b);

  G4bool onEdge =
    IsSameLineSegment(p, a, b) || IsSameLineSegment(p, b, c) || IsSameLineSegment(p, c, a);
...
G4double G4ExtrudedSolid::GetAngle(const G4TwoVector& po, const G4TwoVector& pa,
...
G4bool G4ExtrudedSolid::AddGeneralPolygonFacets()
{
  ...
  static const G4double kAngTolerance = G4GeometryTolerance::GetInstance()->GetAngularTolerance();
  ...
  while (verticesToBeDone.size() > 2)  // Loop checking, 13.08.2015, G.Cosmo
  {
    ...
    while (angle >= (pi - kAngTolerance))  // Loop checking, 13.08.2015, G.Cosmo
    {
      ...
      if (counter > fNv)
      {
        G4Exception("G4ExtrudedSolid::AddGeneralPolygonFacets", "GeomSolids0003", FatalException,
...
G4bool G4ExtrudedSolid::MakeFacets()
{
  ...
  {
    good =
      AddFacet(new G4TriangularFacet(GetVertex(0, 0), GetVertex(0, 1), GetVertex(0, 2), ABSOLUTE));
    if (!good)
    {
      return false;
    }
    ...
    good =
      AddFacet(new G4TriangularFacet(GetVertex((G4int)fNz - 1, 2), GetVertex((G4int)fNz - 1, 1),
                                     GetVertex((G4int)fNz - 1, 0), ABSOLUTE));
    if (!good)
    {
      return false;
    }
  ...
  {
    good = AddFacet(new G4QuadrangularFacet(GetVertex(0, 0), GetVertex(0, 1), GetVertex(0, 2),
                                            GetVertex(0, 3), ABSOLUTE));
    if (!good)
    {
      return false;
    }
    ...
    good = AddFacet(new G4QuadrangularFacet(
      GetVertex((G4int)fNz - 1, 3), GetVertex((G4int)fNz - 1, 2), GetVertex((G4int)fNz - 1, 1),
      GetVertex((G4int)fNz - 1, 0), ABSOLUTE));
    if (!good)
    {
      return false;
  ...
  for (G4int iz = 0; iz < (G4int)fNz - 1; ++iz)
  {
    for (G4int i = 0; i < (G4int)fNv; ++i)
    {
      G4int j = (i + 1) % fNv;
      good = AddFacet(new G4QuadrangularFacet(
        GetVertex(iz, j), GetVertex(iz, i), GetVertex(iz + 1, i), GetVertex(iz + 1, j), ABSOLUTE));
      if (!good)
      {
        return false;
      }
...
EInside G4ExtrudedSolid::Inside(const G4ThreeVector& p) const
{
  switch (fSolidType)
  {
    ...
    case 2:  // non-convex right prism
    {
      ...
      if (distz > kCarToleranceHalf)
      {
        return kOutside;
      }

      G4bool in = PointInPolygon(p);
      if (distz > -kCarToleranceHalf && in)
      {
        return kSurface;
      }
  ...
  //
  if (p.x() < GetMinXExtent() - kCarToleranceHalf || p.x() > GetMaxXExtent() + kCarToleranceHalf
      || p.y() < GetMinYExtent() - kCarToleranceHalf || p.y() > GetMaxYExtent() + kCarToleranceHalf
      || p.z() < GetMinZExtent() - kCarToleranceHalf || p.z() > GetMaxZExtent() + kCarToleranceHalf)
  {
  ...
  {
    if (IsPointInside(fPolygon[(*it)[0]], fPolygon[(*it)[1]], fPolygon[(*it)[2]], pscaled))
    {
      inside = true;
    }
    ++it;
...
G4ThreeVector G4ExtrudedSolid::SurfaceNormal(const G4ThreeVector& p) const
{
  ...
  switch (fSolidType)
  {
    case 1:  // convex right prism
    {
      ...
      {
        G4double dd = fPlanes[i].a * p.x() + fPlanes[i].b * p.y() + fPlanes[i].d;
        if (std::abs(dd) > kCarToleranceHalf)
        {
          continue;
        }
        nx += fPlanes[i].a;
    ...
    case 2:  // non-convex right prism
    {
      ...
      for (std::size_t i = 0, k = fNv - 1; i < fNv; k = i++)
      {
        ...
        else if (u > fLengths[i])
        {
          ...
          if (kx * kx + ky * ky > sqrCarToleranceHalf)
          {
            continue;
          }
        }
...
#  ifdef G4CSGDEBUG
  ...
  G4Exception("G4TesselatedSolid::SurfaceNormal(p)", "GeomSolids1002", JustWarning, message);
  DumpInfo();
#  endif
...
G4ThreeVector G4ExtrudedSolid::ApproxSurfaceNormal(const G4ThreeVector& p) const
{
  ...
  if (fSolidType == 1 || fSolidType == 2)
  {
    ...
    {
      case 0: {
        if (ddz0 <= ddz1 && ddz0 <= dd)
        {
          return {0, 0, -1};
        }
        if (ddz1 <= ddz0 && ddz1 <= dd)
        {
          return {0, 0, 1};
        }
...
G4double G4ExtrudedSolid::DistanceToOut(const G4ThreeVector& p, const G4ThreeVector& v,
                                        ...
                                        G4ThreeVector* n) const
{
  ...
  G4double distOut = G4TessellatedSolid::DistanceToOut(p, v, calcNorm, validNorm, n);
...
void G4ExtrudedSolid::BoundingLimits(G4ThreeVector& pMin, G4ThreeVector& pMax) const
{
  ...
  if (pMin.x() >= pMax.x() || pMin.y() >= pMax.y() || pMin.z() >= pMax.z())
  {
    ...
    message << "Bad bounding box (min >= max) for solid: " << GetName() << " !"
            << "\npMin = " << pMin << "\npMax = " << pMax;
    G4Exception("G4ExtrudedSolid::BoundingLimits()", "GeomMgt0001", JustWarning, message);
    DumpInfo();
  }
}
...
G4bool G4ExtrudedSolid::CalculateExtent(const EAxis pAxis, const G4VoxelLimits& pVoxelLimit,
...
#  endif
  ...
  {
    std::ostringstream message;
    message << "Triangulation of the base polygon has failed for solid: " << GetName() << " !"
            << "\nExtent has been calculated using boundary box";
    G4Exception("G4ExtrudedSolid::CalculateExtent()", "GeomMgt1002", JustWarning, message);
    return bbox.CalculateExtent(pAxis, pVoxelLimit, pTransform, pMin, pMax);
  }
...
std::ostream& G4ExtrudedSolid::StreamInfo(std::ostream& os) const
{
  ...
  for (std::size_t i = 0; i < fNv; ++i)
  {
    os << std::setw(5) << "#" << i << "   vx = " << fPolygon[i].x() / mm << " mm"
