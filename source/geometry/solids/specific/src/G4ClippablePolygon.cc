...
// Constructor
//
G4ClippablePolygon::G4ClippablePolygon() : normal(0., 0., 0.)
{
...
const G4ThreeVector* G4ClippablePolygon::GetMinPoint(const EAxis axis) const
{
  ...
  if (noLeft == 0)
  {
    G4Exception("G4ClippablePolygon::GetMinPoint()", "GeomSolids0002", FatalException,
                "Empty polygon.");
  }
...
const G4ThreeVector* G4ClippablePolygon::GetMaxPoint(const EAxis axis) const
{
  ...
  if (noLeft == 0)
  {
    G4Exception("G4ClippablePolygon::GetMaxPoint()", "GeomSolids0002", FatalException,
                "Empty polygon.");
  }
...
G4bool G4ClippablePolygon::InFrontOf(const G4ClippablePolygon& other, EAxis axis) const
{
  ...
  // Easy decision
  //
  if (min < minOther - kCarTolerance)
  {
    return true;
  }  // Clear winner

  if (minOther < min - kCarTolerance)
  {
    return false;
  }  // Clear loser
...
G4bool G4ClippablePolygon::BehindOf(const G4ClippablePolygon& other, EAxis axis) const
{
  ...
  // Easy decision
  //
  if (max > maxOther + kCarTolerance)
  {
    return true;
  }  // Clear winner

  if (maxOther > max + kCarTolerance)
  {
    return false;
  }  // Clear loser
...
G4bool G4ClippablePolygon::GetPlanerExtent(const G4ThreeVector& pointOnPlane,
                                           const G4ThreeVector& planeNormal, G4double& min,
                                           G4double& max) const
{
  //
  // Okay, how many entries do we have?
