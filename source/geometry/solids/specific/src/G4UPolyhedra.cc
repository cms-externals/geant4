...
//
G4UPolyhedra::G4UPolyhedra(const G4String& name, G4double phiStart, G4double phiTotal,
                           G4int numSide, G4int numZPlanes, const G4double zPlane[],
                           const G4double rInner[], const G4double rOuter[])
  : Base_t(name, phiStart, phiTotal, numSide, numZPlanes, zPlane, rInner, rOuter)
{
  fGenericPgon = false;
...
//
G4UPolyhedra::G4UPolyhedra(const G4String& name, G4double phiStart, G4double phiTotal,
                           G4int numSide, G4int numRZ, const G4double r[], const G4double z[])
...
// Copy constructor
//
G4UPolyhedra::G4UPolyhedra(const G4UPolyhedra& source) : Base_t(source)
{
...
void G4UPolyhedra::SetOriginalParameters()
{
  ...
  G4double convertRad = fGenericPgon ? 1.0 : std::cos(0.5 * deltaPhi / numSides);
...
G4bool G4UPolyhedra::Reset()
{
  if (fGenericPgon)
  {
    ...
    G4Exception("G4UPolyhedra::Reset()", "GeomSolids1001", JustWarning, message,
...
//
void G4UPolyhedra::ComputeDimensions(G4VPVParameterisation* p, const G4int n,
...
void G4UPolyhedra::BoundingLimits(G4ThreeVector& pMin, G4ThreeVector& pMax) const
{
  ...
  if (pMin.x() >= pMax.x() || pMin.y() >= pMax.y() || pMin.z() >= pMax.z())
  {
    ...
    message << "Bad bounding box (min >= max) for solid: " << GetName() << " !"
            << "\npMin = " << pMin << "\npMax = " << pMax;
    G4Exception("G4UPolyhedra::BoundingLimits()", "GeomMgt0001", JustWarning, message);
    StreamInfo(G4cout);
  }
  ...
  if (checkPhi)
  {
    if (GetStartPhi() != GetPhiStart() || GetEndPhi() != GetPhiEnd()
        || GetNumSide() != GetSideCount() || IsOpen() != (Base_t::GetPhiDelta() < twopi))
    {
      std::ostringstream message;
      message << "Inconsistency in Phi angles or # of sides for solid: " << GetName() << " !"
              << "\nPhi start  : wrapper = " << GetStartPhi() << " solid = " << GetPhiStart()
              << "\nPhi end    : wrapper = " << GetEndPhi() << " solid = " << GetPhiEnd()
              << "\nPhi # sides: wrapper = " << GetNumSide() << " solid = " << GetSideCount()
              << "\nPhi is open: wrapper = " << (IsOpen() ? "true" : "false")
              << " solid = " << ((Base_t::GetPhiDelta() < twopi) ? "true" : "false");
      G4Exception("G4UPolyhedra::BoundingLimits()", "GeomMgt0001", JustWarning, message);
      checkPhi = false;
    }
  }
...
G4bool G4UPolyhedra::CalculateExtent(const EAxis pAxis, const G4VoxelLimits& pVoxelLimit,
...
#  endif
  ...
  if (!G4GeomTools::TriangulatePolygon(contourRZ, triangles))
  {
    ...
    G4Exception("G4UPolyhedra::CalculateExtent()", "GeomMgt1002", JustWarning, message);
