                                 ...
                                 G4bool thePhiIsOpen, G4bool isAllBehind)
{
  ...
  phiBoundaryCos.resize(maxSides + 1);
  phiBoundarySin.resize(maxSides + 1);
  ...
  const G4double cosStep = std::cos(deltaPhi);
  const G4double sinStep = std::sin(deltaPhi);
  phiBoundaryCos[0] = cosPhi;
  phiBoundarySin[0] = sinPhi;
  G4ThreeVector a1(r[0] * cosPhi, r[0] * sinPhi, z[0]),
    b1(r[1] * cosPhi, r[1] * sinPhi, z[1]),
    c1(prevRZ->r * cosPhi, prevRZ->r * sinPhi, prevRZ->z),
    d1(nextRZ->r * cosPhi, nextRZ->r * sinPhi, nextRZ->z), a2, b2, c2, d2;
  ...
  do  // Loop checking, 13.08.2015, G.Cosmo
  {
    ...
    //
    phi += deltaPhi;
    const G4double nextCosPhi = cosPhi * cosStep - sinPhi * sinStep;
    const G4double nextSinPhi = sinPhi * cosStep + cosPhi * sinStep;
    const auto iSide = (std::size_t)(vec - vecs);
    phiBoundaryCos[iSide + 1] = nextCosPhi;
    phiBoundarySin[iSide + 1] = nextSinPhi;
    a2 = G4ThreeVector(r[0] * nextCosPhi, r[0] * nextSinPhi, z[0]);
    b2 = G4ThreeVector(r[1] * nextCosPhi, r[1] * nextSinPhi, z[1]);
    c2 = G4ThreeVector(prevRZ->r * nextCosPhi, prevRZ->r * nextSinPhi, prevRZ->z);
    d2 = G4ThreeVector(nextRZ->r * nextCosPhi, nextRZ->r * nextSinPhi, nextRZ->z);
...
void G4PolyhedraSide::CopyStuff(const G4PolyhedraSide& source)
{
  ...
  phiBoundaryCos = source.phiBoundaryCos;
  phiBoundarySin = source.phiBoundarySin;
                                        ...
                                        G4int* i2)
{
  ...
  //
  *i1 = PhiSegment(p.x() + s1 * v.x(), p.y() + s1 * v.y());
  if (n == 1)
  {
    return (*i1 < 0) ? 0 : 1;
  }
  ...
  //
  *i2 = PhiSegment(p.x() + s2 * v.x(), p.y() + s2 * v.y());
  if (*i1 == *i2)
  {
    return 0;
  }
...
G4int G4PolyhedraSide::PhiSegment(const G4double x, const G4double y)
{
  if (phiBoundaryCos.size() != std::size_t(numSide + 1))
  {
    return phiIsOpen ? -1 : 0;
  }

  const G4double r2 = x * x + y * y;
  if (r2 <= kCarTolerance * kCarTolerance)
  {
    return PhiSegment(0.);
  }

  const G4double tol = 0.5 * kCarTolerance;
  const G4bool shortSection = deltaPhi <= pi;
  for (G4int i = 0; i < numSide; ++i)
  {
    const G4double startCross = phiBoundaryCos[i] * y - phiBoundarySin[i] * x;
    const G4double endCross = x * phiBoundarySin[i + 1] - y * phiBoundaryCos[i + 1];

    if (std::fabs(startCross) <= tol)
    {
      return i;
    }
    if (std::fabs(endCross) <= tol)
    {
      const G4int next = i + 1;
      return (next < numSide) ? next : (phiIsOpen ? -1 : 0);
    }
    if (shortSection ? (startCross > 0. && endCross > 0.)
                     : (startCross > 0. || endCross > 0.))
    {
      return i;
    }
  }
