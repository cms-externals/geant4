  ...
  : G4VTwistSurface(name)
{
  ...
  if (fDx1 != fDx2 || fDx3 != fDx4)
  {
    ...
    message << "TwistedTrapBoxSide is not used as a the side of a box: " << GetName() << G4endl
            << "        Not a box !";
    G4Exception("G4TwistBoxSide::G4TwistBoxSide()", "GeomSolids0002", FatalException, message);
  }
...
G4TwistBoxSide::G4TwistBoxSide(__void__& a) : G4VTwistSurface(a) {}

//=====================================================================
//* GetNormal ---------------------------------------------------------

G4ThreeVector G4TwistBoxSide::GetNormal(const G4ThreeVector& tmpxx, G4bool isGlobal)
{
...
G4int G4TwistBoxSide::DistanceToSurface(const G4ThreeVector& gp, const G4ThreeVector& gv,
                                        ...
                                        G4bool isvalid[], EValidate validate)
{
  ...
  if (v.z() == 0.)
  {
    if ((std::fabs(p.z()) <= L) && (fPhiTwist != 0.0))  // intersection possible in z
    {
      ...
      q = (2. * fPhiTwist
           * ((v.x() - fTAlph * v.y()) * std::cos(phi) + (fTAlph * v.x() + v.y()) * std::sin(phi)));
  ...
  else  // general solution for non-zero vz
  {
    ...
    c[6] = 28800
           * (phiyz + 2 * fDz * v.x() - (fdeltaX + fDx4minus2) * v.z()
              + fTAlph * (phixz - 2 * fDz * v.y() + fdeltaY * v.z()));
    c[5] = -1200
           * (10 * phixz - 48 * fDz * v.y() + 24 * fdeltaY * v.z() + fDx4plus2 * fPhiTwist * v.z()
              - 2 * fTAlph * (5 * phiyz + 24 * fDz * v.x() - 12 * fdeltaX * v.z()));
    c[4] = -2400
           * (phiyz + 10 * fDz * v.x() - 5 * fdeltaX * v.z() + fDx4minus2 * v.z()
              + fTAlph * (phixz - 10 * fDz * v.y() + 5 * fdeltaY * v.z()));
    c[3] = 24
           * (2 * phixz - 200 * fDz * v.y() + 100 * fdeltaY * v.z() - fDx4plus2 * fPhiTwist * v.z()
              - 2 * fTAlph * (phiyz + 100 * fDz * v.x() - 50 * fdeltaX * v.z()));
    c[2] = -16
           * (7 * fTAlph * phixz + 7 * phiyz - 6 * fDz * v.x() + 6 * fDz * fTAlph * v.y()
              + 3 * (fdeltaX + fDx4minus2 - fdeltaY * fTAlph) * v.z());
    c[1] = 4
  ...
  for (auto& k : xbuf)
  {
    ...
    if (IsConverged)
    {
      ...
      {
        G4Exception("G4TwistBoxSide::DistanceToSurface()", "GeomSolids0001", FatalException,
...
G4int G4TwistBoxSide::DistanceToSurface(const G4ThreeVector& gp, G4ThreeVector gxx[],
...
G4int G4TwistBoxSide::GetAreaCode(const G4ThreeVector& xx, G4bool withTol)
{
  ...
  G4Exception("G4TwistBoxSide::GetAreaCode()", "GeomSolids0001", FatalException,
              "Feature NOT implemented !");
...
void G4TwistBoxSide::SetCorners()
{
  ...
  {
    G4Exception("G4TwistBoxSide::SetCorners()", "GeomSolids0001", FatalException,
...
void G4TwistBoxSide::GetPhiUAtX(const G4ThreeVector& p, G4double& phi, G4double& u)
{
  ...
  u = -(fTAlph * (fDx4plus2 * fPhiTwist + 2 * fDx4minus2 * phi)
        + 2 * (fdeltaY * phi + fdeltaX * fTAlph * phi - fPhiTwist * (fTAlph * p.x() + p.y()))
            * std::cos(phi)
        + 2 * (-(fdeltaX * phi) + fdeltaY * fTAlph * phi + fPhiTwist * (p.x() - fTAlph * p.y()))
            * std::sin(phi))
      / (2. * (fPhiTwist + fPhiTwist * fTAlph * fTAlph));
}

//=====================================================================
//* ProjectPoint ------------------------------------------------------

G4ThreeVector G4TwistBoxSide::ProjectPoint(const G4ThreeVector& p, G4bool isglobal)
{
