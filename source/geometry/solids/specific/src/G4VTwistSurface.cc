...
G4VTwistSurface::G4VTwistSurface(const G4String& name) : fIsValidNorm(false), fName(name)
{
  fAxis[0] = kUndefined;
  fAxis[1] = kUndefined;
...
G4VTwistSurface::G4VTwistSurface(const G4String& name, const G4RotationMatrix& rot,
                                 const G4ThreeVector& tlate, G4int handedness, const EAxis axis0,
                                 const EAxis axis1, G4double axis0min, G4double axis1min,
                                 G4double axis0max, G4double axis1max)
...
G4VTwistSurface::G4VTwistSurface(__void__&)
  : fHandedness(0), fIsValidNorm(false), kCarTolerance(0.), fName("")
{
...
G4int G4VTwistSurface::AmIOnLeftSide(const G4ThreeVector& me, const G4ThreeVector& vec,
                                     G4bool withtol)
{
  ...
  const G4double kAngTolerance = G4GeometryTolerance::GetInstance()->GetAngularTolerance();
  ...
  if (withtol)
  {
    if (met.x() * ivect.y() - met.y() * ivect.x() > 0 && metcrossvect >= 0)
    {
      amIOnLeftSide = 1;
    }
    else if (met.x() * rvect.y() - met.y() * rvect.x() < 0 && metcrossvect <= 0)
    {
      amIOnLeftSide = -1;
    }
    else
...
#ifdef G4TWISTDEBUG
  G4cout << "         === G4VTwistSurface::AmIOnLeftSide() ==============" << G4endl;
  G4cout << "             Name , returncode  : " << fName << " " << amIOnLeftSide << G4endl;
  G4cout << "             me, vec    : " << std::setprecision(14) << me << " " << vec << G4endl;
...
G4double G4VTwistSurface::DistanceToBoundary(G4int areacode, G4ThreeVector& xx,
                                             const G4ThreeVector& p)
{
  ...
  if (IsAxis0(areacode) && IsAxis1(areacode))
  {
    ...
    message << "Point is in the corner area." << G4endl
            << "        Point is in the corner area. This function returns" << G4endl
            << "        a direction vector of a boundary line." << G4endl
            << "        areacode = " << areacode;
    G4Exception("G4VTwistSurface::DistanceToBoundary()", "GeomSolids0003", FatalException, message);
  }
  ...
  {
    std::ostringstream message;
    message << "Bad areacode of boundary." << G4endl << "        areacode = " << areacode;
    G4Exception("G4VTwistSurface::DistanceToBoundary()", "GeomSolids0003", FatalException, message);
  }
  return dist;
}
...
G4double G4VTwistSurface::DistanceToIn(const G4ThreeVector& gp, const G4ThreeVector& gv,
                                       G4ThreeVector& gxxbest)
{
  ...
  G4int nxx = DistanceToSurface(gp, gv, gxx, distance, areacode, isvalid, kValidateWithTol);
  ...
  for (G4int i = 0; i < nxx; ++i)
  {
    ...
    if (IsInside(areacode[i]))
    {
      if (distance[i] < bestdistance)
      {
        ...
        besti = i;
        G4cout << "   G4VTwistSurface::DistanceToIn(p,v): "
               << " areacode sInside name, distance = " << fName << " " << bestdistance << G4endl;
#endif
      }
    ...
    else
    {
      ...
      for (G4int j = 0; j < nneighbours; ++j)
      {
        ...
        G4int tmpnxx = neighbours[j]->DistanceToSurface(gp, gv, tmpgxx, tmpdist, tmpareacode,
                                                        tmpisvalid, kValidateWithTol);
        ...
        for (G4int k = 0; k < tmpnxx; ++k)
        {
          ...
          if (IsInside(tmpareacode[k]))
          {
            ...
            G4cout << "   G4VTwistSurface:DistanceToIn(p,v): "
                   ...
                   << "   is inside of neighbour surface of " << fName << " . returning kInfinity."
                   << G4endl;
          ...
          if (IsSameBoundary(this, areacode[i], neighbours[j], tmpareacode[k]))
          {
      ...
      if (isaccepted[0] && isaccepted[1])
      {
        if (distance[i] < bestdistance)
        {
          ...
          besti = i;
          G4cout << "   G4VTwistSurface::DistanceToIn(p,v): "
                 << " areacode sBoundary & sBoundary distance = " << fName << " " << distance[i]
                 << G4endl;
#endif
        }
      }
...
G4double G4VTwistSurface::DistanceToOut(const G4ThreeVector& gp, const G4ThreeVector& gv,
                                        G4ThreeVector& gxxbest)
{
#ifdef G4TWISTDEBUG
  ...
  nxx = DistanceToSurface(gp, gv, gxx, distance, areacode, isvalid, kValidateWithTol);
...
#ifdef G4TWISTDEBUG
      G4cout << "   G4VTwistSurface::DistanceToOut(p,v): normal*gv < 0 " << fName << " " << normal
             << G4endl;
#endif
    }
    else
...
G4double G4VTwistSurface::DistanceTo(const G4ThreeVector& gp, G4ThreeVector& gxxbest)
{
#ifdef G4TWISTDEBUG
...
G4bool G4VTwistSurface::IsSameBoundary(G4VTwistSurface* surf1, G4int areacode1,
                                       G4VTwistSurface* surf2, G4int areacode2) const
{
  ...
  G4bool iscorner[2] = {IsCorner(areacode1, testbitmode), IsCorner(areacode2, testbitmode)};

  if (iscorner[0] && iscorner[1])
  {
    // on corner
    G4ThreeVector corner1 = surf1->ComputeGlobalPoint(surf1->GetCorner(areacode1));
    G4ThreeVector corner2 = surf2->ComputeGlobalPoint(surf2->GetCorner(areacode2));
      ...
      && (IsBoundary(areacode2, testbitmode) && (!iscorner[1])))
  {
    ...
    return (x01 - x02).mag() < kCarTolerance && (d1 - d2).mag() < kCarTolerance;
  }
  return false;
}
...
void G4VTwistSurface::GetBoundaryParameters(const G4int& areacode, G4ThreeVector& d,
                                            G4ThreeVector& x0, G4int& boundarytype) const
{
  ...
  std::ostringstream message;
  message << "Not registered boundary." << G4endl << "        Boundary at areacode " << std::hex
          << areacode << std::dec << G4endl << "        is not registered.";
  G4Exception("G4VTwistSurface::GetBoundaryParameters()", "GeomSolids0002", FatalException,
              message);
}
...
G4ThreeVector G4VTwistSurface::GetBoundaryAtPZ(G4int areacode, const G4ThreeVector& p) const
{
  ...
  {
    std::ostringstream message;
    message << "Point is in the corner area." << G4endl << "        This function returns "
            << "a direction vector of a boundary line." << G4endl
            << "        areacode = " << areacode;
    G4Exception("G4VTwistSurface::GetBoundaryAtPZ()", "GeomSolids0003", FatalException, message);
  }
  ...
  {
    std::ostringstream message;
    message << "Not registered boundary." << G4endl << "        Boundary at areacode " << areacode
            << G4endl << "        is not registered.";
    G4Exception("G4VTwistSurface::GetBoundaryAtPZ()", "GeomSolids0002", FatalException, message);
  }

  if (((boundarytype & sAxisPhi) == sAxisPhi) || ((boundarytype & sAxisRho) == sAxisRho))
  {
    std::ostringstream message;
    message << "Not a z-depended line boundary." << G4endl << "        Boundary at areacode "
            << areacode << G4endl << "        is not a z-depended line.";
    G4Exception("G4VTwistSurface::GetBoundaryAtPZ()", "GeomSolids0002", FatalException, message);
  }
...
void G4VTwistSurface::SetCorner(G4int areacode, G4double x, G4double y, G4double z)
{
  ...
  {
    std::ostringstream message;
    message << "Area code must represents corner." << G4endl << "        areacode " << areacode;
    G4Exception("G4VTwistSurface::SetCorner()", "GeomSolids0002", FatalException, message);
  }
...
void G4VTwistSurface::GetBoundaryAxis(G4int areacode, EAxis axis[]) const
{
  ...
  {
    G4Exception("G4VTwistSurface::GetBoundaryAxis()", "GeomSolids0003", FatalException,
  ...
  for (G4int i = 0; i < 2; ++i)
  {
    ...
    if (axiscode != 0)
    {
      ...
      {
        std::ostringstream message;
        message << "Not supported areacode." << G4endl << "        areacode " << areacode;
        G4Exception("G4VTwistSurface::GetBoundaryAxis()", "GeomSolids0001", FatalException,
                    message);
      }
    }
  }
...
void G4VTwistSurface::GetBoundaryLimit(G4int areacode, G4double limit[]) const
{
  ...
  {
    std::ostringstream message;
    message << "Not located on a boundary!" << G4endl << "          areacode " << areacode;
    G4Exception("G4VTwistSurface::GetBoundaryLimit()", "GeomSolids1002", JustWarning, message);
  }
}
...
void G4VTwistSurface::SetBoundary(const G4int& axiscode, const G4ThreeVector& direction,
                                  const G4ThreeVector& x0, const G4int& boundarytype)
{
  G4int code = (~sAxisMask) & axiscode;
  if ((code == (sAxis0 & sAxisMin)) || (code == (sAxis0 & sAxisMax))
      || (code == (sAxis1 & sAxisMin)) || (code == (sAxis1 & sAxisMax)))
  {
    G4bool done = false;
    ...
    if (!done)
    {
      G4Exception("G4VTwistSurface::SetBoundary()", "GeomSolids0003", FatalException,
                  "Number of boundary exceeding 4!");
    }
  }
  else
  {
    std::ostringstream message;
    message << "Invalid axis-code." << G4endl << "        axiscode = " << std::hex << axiscode
            << std::dec;
    G4Exception("G4VTwistSurface::SetBoundary()", "GeomSolids0003", FatalException, message);
  }
}
...
G4int G4VTwistSurface::GetFace(G4int i, G4int j, G4int k, G4int n, G4int iside)
{
  ...
  std::ostringstream message;
  message << "Not correct side number: " << GetName() << G4endl << "iside is " << iside
          << " but should be "
          << "0,1,2,3,4 or 5" << ".";
  G4Exception("G4TwistSurface::G4GetFace()", "GeomSolids0002", FatalException, message);
...
G4int G4VTwistSurface::GetNode(G4int i, G4int j, G4int k, G4int n, G4int iside)
{
  ...
  message << "Not correct side number: " << GetName() << G4endl << "iside is " << iside
          << " but should be "
          << "0,1,2,3,4 or 5" << ".";
  G4Exception("G4TwistSurface::G4GetNode()", "GeomSolids0002", FatalException, message);
...
G4int G4VTwistSurface::GetEdgeVisibility(G4int i, G4int j, G4int k, G4int n, G4int number,
                                         G4int orientation)
{
  ...
  if ((j >= 1 && j <= k - 3))
  {
    ...
    G4Exception("G4TwistSurface::G4GetEdgeVisibility()", "GeomSolids0003", FatalException, message);
  }
...
void G4VTwistSurface::DebugPrint() const
{
  ...
  G4cout << "/* G4VTwistSurface::DebugPrint():--------------------------" << G4endl;
  G4cout << "/* Name = " << fName << G4endl;
  G4cout << "/* Axis = " << std::hex << fAxis[0] << " " << std::hex << fAxis[1]
         << " (0,1,2,3,5 = kXAxis,kYAxis,kZAxis,kRho,kPhi)" << std::dec << G4endl;
  G4cout << "/* BoundaryLimit(in local) fAxis0(min, max) = (" << fAxisMin[0] << ", " << fAxisMax[0]
         << ")" << G4endl;
  G4cout << "/* BoundaryLimit(in local) fAxis1(min, max) = (" << fAxisMin[1] << ", " << fAxisMax[1]
         << ")" << G4endl;
...
void G4VTwistSurface::Boundary::SetFields(const G4int& areacode, const G4ThreeVector& d,
                                          const G4ThreeVector& x0, const G4int& boundarytype)
{
...
G4bool G4VTwistSurface::Boundary::GetBoundaryParameters(const G4int& areacode, G4ThreeVector& d,
                                                        G4ThreeVector& x0,
                                                        G4int& boundarytype) const
{
  ...
  if (((areacode & sAxis0) != 0) && ((areacode & sAxis1) != 0))
  {
    ...
    message << "Located in the corner area." << G4endl
            ...
            << "a boundary line." << G4endl << "        areacode = " << areacode;
    G4Exception("G4VTwistSurface::Boundary::GetBoundaryParameters()", "GeomSolids0003",
