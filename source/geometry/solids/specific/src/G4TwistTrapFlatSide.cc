...
G4TwistTrapFlatSide::G4TwistTrapFlatSide(const G4String& name, G4double PhiTwist, G4double pDx1,
                                         G4double pDx2, G4double pDy, G4double pDz, G4double pAlpha,
                                         G4double pPhi, G4double pTheta, G4int handedness)
  : G4VTwistSurface(name)
{
  ...
  fRot.rotateZ(fHandedness > 0 ? 0.5 * fPhiTwist : -0.5 * fPhiTwist);

  fTrans.set(fHandedness > 0 ? 0.5 * fdeltaX : -0.5 * fdeltaX,
             fHandedness > 0 ? 0.5 * fdeltaY : -0.5 * fdeltaY, fHandedness > 0 ? fDz : -fDz);
...
G4TwistTrapFlatSide::G4TwistTrapFlatSide(__void__& a) : G4VTwistSurface(a) {}

//=====================================================================
//* GetNormal ---------------------------------------------------------

G4ThreeVector G4TwistTrapFlatSide::GetNormal(const G4ThreeVector& /* xx */, G4bool isGlobal)
{
...
G4int G4TwistTrapFlatSide::DistanceToSurface(const G4ThreeVector& gp, const G4ThreeVector& gv,
...
G4int G4TwistTrapFlatSide::DistanceToSurface(const G4ThreeVector& gp, G4ThreeVector gxx[],
...
G4int G4TwistTrapFlatSide::GetAreaCode(const G4ThreeVector& xx, G4bool withTol)
{
  ...
  G4Exception("G4TwistTrapFlatSide::GetAreaCode()", "GeomSolids0001", FatalException,
...
void G4TwistTrapFlatSide::SetCorners()
{
  ...
  else
  {
    ...
    message << "Feature NOT implemented !" << G4endl << "        fAxis[0] = " << fAxis[0] << G4endl
            << "        fAxis[1] = " << fAxis[1];
    G4Exception("G4TwistTrapFlatSide::SetCorners()", "GeomSolids0001", FatalException, message);
  }
}
...
void G4TwistTrapFlatSide::SetBoundaries()
{
  ...
  if (fAxis[0] == kXAxis && fAxis[1] == kYAxis)
  {
    ...
    SetBoundary(sAxis0 & (sAxisX | sAxisMin), direction, GetCorner(sC0Min1Max), sAxisY);

    // sAxis0 & sAxisMax
    direction = GetCorner(sC0Max1Max) - GetCorner(sC0Max1Min);  // inverse
    direction = direction.unit();
    SetBoundary(sAxis0 & (sAxisX | sAxisMax), direction, GetCorner(sC0Max1Min), sAxisY);

    // sAxis1 & sAxisMin
    direction = GetCorner(sC0Max1Min) - GetCorner(sC0Min1Min);
    direction = direction.unit();
    SetBoundary(sAxis1 & (sAxisY | sAxisMin), direction, GetCorner(sC0Min1Min), sAxisX);

    // sAxis1 & sAxisMax
    direction = -(GetCorner(sC0Max1Max) - GetCorner(sC0Min1Max));
    direction = direction.unit();
    SetBoundary(sAxis1 & (sAxisY | sAxisMax), direction, GetCorner(sC0Max1Max), sAxisX);
  }
  else
  {
    std::ostringstream message;
...
void G4TwistTrapFlatSide::GetFacets(G4int k, G4int n, G4double xyz[][3], G4int faces[][4],
                                    G4int iside)
{
