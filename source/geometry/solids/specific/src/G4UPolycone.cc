...
//
G4UPolycone::G4UPolycone(const G4String& name, G4double phiStart, G4double phiTotal,
                         G4int numZPlanes, const G4double zPlane[], const G4double rInner[],
...
//
G4UPolycone::G4UPolycone(const G4String& name, G4double phiStart, G4double phiTotal, G4int numRZ,
...
// Copy constructor
//
G4UPolycone::G4UPolycone(const G4UPolycone& source) : Base_t(source)
{
...
G4bool G4UPolycone::Reset()
{
  if (fGenericPcon)
  {
    ...
    G4Exception("G4UPolycone::Reset()", "GeomSolids1001", JustWarning, message,
...
//
void G4UPolycone::ComputeDimensions(G4VPVParameterisation* p, const G4int n,
...
void G4UPolycone::BoundingLimits(G4ThreeVector& pMin, G4ThreeVector& pMax) const
{
  ...
  if (pMin.x() >= pMax.x() || pMin.y() >= pMax.y() || pMin.z() >= pMax.z())
  {
    ...
    message << "Bad bounding box (min >= max) for solid: " << GetName() << " !"
            << "\npMin = " << pMin << "\npMax = " << pMax;
    G4Exception("G4UPolycone::BoundingLimits()", "GeomMgt0001", JustWarning, message);
    StreamInfo(G4cout);
  }
  ...
  if (checkPhi)
  {
    if (GetStartPhi() != Base_t::GetStartPhi() || GetEndPhi() != Base_t::GetEndPhi()
        || IsOpen() != (Base_t::GetDeltaPhi() < twopi))
    {
      ...
      message << "Inconsistency in Phi angles or # of sides for solid: " << GetName() << " !"
              ...
              << " solid = " << ((Base_t::GetDeltaPhi() < twopi) ? "true" : "false");
      G4Exception("G4UPolycone::BoundingLimits()", "GeomMgt0001", JustWarning, message);
      checkPhi = false;
    }
  }
...
G4bool G4UPolycone::CalculateExtent(const EAxis pAxis, const G4VoxelLimits& pVoxelLimit,
...
#  endif
  ...
  if (!G4GeomTools::TriangulatePolygon(contourRZ, triangles))
  {
    ...
    G4Exception("G4UPolycone::CalculateExtent()", "GeomMgt1002", JustWarning, message);
    return bbox.CalculateExtent(pAxis, pVoxelLimit, pTransform, pMin, pMax);
  }
