...
EInside G4GenericPolycone::Inside(const G4ThreeVector& p) const
{
  ...
  if (fRZConvex) return InsideRZ(p);
  const EInside rz = InsideRZSections(p.perp(), p.z(), fRZSection, fRZVoxel);
  return CombineInside(rz, PhiState(p));
}

G4ThreeVector G4GenericPolycone::SurfaceNormal(const G4ThreeVector& p) const
{
...
G4double G4GenericPolycone::DistanceToOut(const G4ThreeVector& p, const G4ThreeVector& v,
...
G4double G4GenericPolycone::DistanceToOut(const G4ThreeVector& p) const
{
  if (fRZSection.empty())
  {
...
void G4GenericPolycone::BuildRZIndex(G4bool profileIsConvex)
{
  fRZSection.clear();
  fRZVoxel.reset();
  fRZConvex = false;
...
EInside G4GenericPolycone::InsideRZOnly(const G4ThreeVector& p) const
{
...
G4ThreeVector G4GenericPolycone::SurfaceNormalRZ(const G4ThreeVector& p) const
{
  const G4double rho = p.perp();
  const G4double z = p.z();
  G4double best = kInfinity;
  ...
  auto check = [&](G4int i)
  {
    ...
    const G4double dr = rho - sec.r0;
    const G4double dz = z - sec.z0;
    const G4double q = std::max(0., std::min(sec.length, dr * sec.rS + dz * sec.zS));
