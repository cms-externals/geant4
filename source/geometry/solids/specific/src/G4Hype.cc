...
G4ThreeVector G4Hype::GetPointOnSurface() const
{
  // Select surface
  G4double ssurf[4];
  ssurf[0] = pi*(endOuterRadius2 - endInnerRadius2);
  ssurf[1] = ssurf[0] + ssurf[0];
  ssurf[2] = ssurf[1] + fOuterSurfaceArea;
  ssurf[3] = ssurf[2] + fInnerSurfaceArea;

  G4double select = ssurf[3]*G4QuickRand();
  auto k = 0;
  for (; k < 3; ++k) { if (select <= ssurf[k]) break; }
  ...
  switch (k) {
    case 0: case 1: // bases
    {
      ...
      z = (k == 0) ? -halfLenZ : halfLenZ;
      rho = std::sqrt(u*endOuterRadius2 + (1. - u)*endInnerRadius2);
      break;
    }
    case 2: case 3: // lateral surfaces (rejection sampling)
    {
      G4double hh = halfLenZ*halfLenZ;
      G4double rr = (k == 2) ? outerRadius2 : innerRadius2;
      G4double endrr = (k == 2) ? endOuterRadius2 : endInnerRadius2;
      G4double delrr = endrr - rr;
      G4double ds_max = std::sqrt(rr*hh + delrr*(delrr + hh)*hh);
      G4double u = 0.;
      for (auto i = 0; i < 10000; ++i)
      {
        u = 2.*G4QuickRand() - 1.;
        G4double ds = std::sqrt(rr*hh + delrr*(delrr + hh)*u*u);
        if (ds_max*G4QuickRand() <= ds) break;
      }
      z = halfLenZ*u;
      rho = std::sqrt(rr + delrr*u*u);
      break;
    }
  }
