...
#if !defined(G4GEOM_USE_UELLIPTICALCONE)
...
G4EllipticalCone::G4EllipticalCone(const G4String& pName, G4double pxSemiAxis, G4double pySemiAxis,
                                   G4double pzMax, G4double pzTopCut)
  : G4VSolid(pName), zTopCut(0.)
{
  ...
  if ((pxSemiAxis <= 0.) || (pySemiAxis <= 0.) || (pzMax <= 0.))
  {
    ...
    message << "Invalid semi-axis or height for solid: " << GetName()
            << "\n   X semi-axis, Y semi-axis, height = " << pxSemiAxis << ", " << pySemiAxis
            << ", " << pzMax;
    G4Exception("G4EllipticalCone::G4EllipticalCone()", "GeomSolids0002", FatalErrorInArgument,
                message);
  }
...
G4EllipticalCone::G4EllipticalCone(const G4EllipticalCone& rhs)
  : G4VSolid(rhs),
    ...
    fMinZBaseArea(rhs.fMinZBaseArea),
    fMaxZBaseArea(rhs.fMaxZBaseArea),
    xSemiAxis(rhs.xSemiAxis),
    ySemiAxis(rhs.ySemiAxis),
    zheight(rhs.zheight),
    zTopCut(rhs.zTopCut),
    cosAxisMin(rhs.cosAxisMin),
    invXX(rhs.invXX),
    invYY(rhs.invYY)
{}
...
void G4EllipticalCone::SetSemiAxis(G4double newxSemiAxis, G4double newySemiAxis, G4double newzMax)
{
...
void G4EllipticalCone::BoundingLimits(G4ThreeVector& pMin, G4ThreeVector& pMax) const
{
  ...
  if (pMin.x() >= pMax.x() || pMin.y() >= pMax.y() || pMin.z() >= pMax.z())
  {
    ...
    message << "Bad bounding box (min >= max) for solid: " << GetName() << " !"
            << "\npMin = " << pMin << "\npMax = " << pMax;
    G4Exception("G4EllipticalCone::BoundingLimits()", "GeomMgt0001", JustWarning, message);
    DumpInfo();
  }
}
...
G4bool G4EllipticalCone::CalculateExtent(const EAxis pAxis, const G4VoxelLimits& pVoxelLimit,
...
#  ifdef G4CSGDEBUG
  ...
  G4Exception("G4EllipticalCone::SurfaceNormal(p)", "GeomSolids1002", JustWarning, message);
  DumpInfo();
#  endif
...
G4double G4EllipticalCone::DistanceToIn(const G4ThreeVector& p, const G4ThreeVector& v) const
{
  ...
  if (sigz < halfCarTol)
  {
    ...
    if (v.z() <= 0)
    {
      ...
      if (sqr(p.x() / (xSemiAxis - halfCarTol)) + sqr(p.y() / (ySemiAxis - halfCarTol))
          <= sqr(zheight + zTopCut))
      {
        return kInfinity;
      }
...
#  endif
  ...
  G4double B =
    2
    * (v.x() * p.x() / sqr(xSemiAxis) + v.y() * p.y() / sqr(ySemiAxis) + v.z() * (zheight - p.z()));
  G4double C = sqr(p.x() / xSemiAxis) + sqr(p.y() / ySemiAxis) - sqr(zheight - p.z());
  ...
  {
    G4ThreeVector truenorm(p.x() / (xSemiAxis * xSemiAxis), p.y() / (ySemiAxis * ySemiAxis),
                           -(p.z() - zheight));
  ...
  if (minus > halfCarTol && minus < distMin)
  {
    ...
    {
      G4ThreeVector truenorm(pin.x() / (xSemiAxis * xSemiAxis), pin.y() / (ySemiAxis * ySemiAxis),
                             -(pin.z() - zheight));
      if (truenorm * v < 0)
...
G4double G4EllipticalCone::DistanceToOut(const G4ThreeVector& p, const G4ThreeVector& v,
                                         ...
                                         G4ThreeVector* n) const
{
  ...
  if (v.z() < 0.0)
  {
    ...
    if ((sqr((p.x() + lambda * v.x()) / xSemiAxis) + sqr((p.y() + lambda * v.y()) / ySemiAxis))
        < sqr(zheight + zTopCut + halfCarTol))
    {
      ...
      switch (surface)
      {
        ...
        case kCurvedSurf: {
          ...
          G4ThreeVector truenorm(pexit.x() / (xSemiAxis * xSemiAxis),
                                 pexit.y() / (ySemiAxis * ySemiAxis), -(pexit.z() - zheight));
        ...
        default:  // Should never reach this case ...
          ...
          message << "Undefined side for valid surface normal to solid." << G4endl
                  << "Position:" << G4endl << "   p.x() = " << p.x() / mm << " mm" << G4endl
                  << "   p.y() = " << p.y() / mm << " mm" << G4endl << "   p.z() = " << p.z() / mm
                  << " mm" << G4endl << "Direction:" << G4endl << "   v.x() = " << v.x() << G4endl
                  << "   v.y() = " << v.y() << G4endl << "   v.z() = " << v.z() << G4endl
                  << "Proposed distance :" << G4endl << "   distMin = " << distMin / mm << " mm";
          message.precision(oldprc);
          G4Exception("G4EllipticalCone::DistanceToOut(p,v,..)", "GeomSolids1002", JustWarning,
                      message);
          break;
      }
    }
...
#  ifdef G4SPECSDEBUG
  if (Inside(p) == kOutside)
  {
    ...
    message << "Point p is outside (!?) of solid: " << GetName() << "\n"
            ...
            << "   p.z() = " << p.z() / mm << " mm";
    message.precision(oldprc);
    G4Exception("G4Ellipsoid::DistanceToOut(p)", "GeomSolids1002", JustWarning, message);
    DumpInfo();
  }
#  endif
  G4double hp = std::sqrt(p.x() * p.x() * invXX + p.y() * p.y() * invYY) + p.z();
...
#endif  // !defined(G4GEOM_USE_UELLIPTICALCONE)
