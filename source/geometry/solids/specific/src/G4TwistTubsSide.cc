...
G4TwistTubsSide::G4TwistTubsSide(const G4String& name, const G4RotationMatrix& rot,
                                 const G4ThreeVector& tlate, G4int handedness, const G4double kappa,
                                 const EAxis axis0, const EAxis axis1, G4double axis0min,
                                 G4double axis1min, G4double axis0max, G4double axis1max)
    ...
    fKappa(kappa)
{
  ...
  {
    G4Exception("G4TwistTubsSide::G4TwistTubsSide()", "GeomSolids0002", FatalErrorInArgument,
...
G4TwistTubsSide::G4TwistTubsSide(const G4String& name, G4double EndInnerRadius[2],
                                 G4double EndOuterRadius[2], G4double DPhi, G4double EndPhi[2],
                                 G4double EndZ[2], G4double InnerRadius, G4double OuterRadius,
                                 G4double Kappa, G4int handedness)
  : G4VTwistSurface(name)
{
  ...
  fKappa = Kappa;
  fRot.rotateZ(fHandedness > 0 ? -0.5 * DPhi : 0.5 * DPhi);
  fTrans.set(0, 0, 0);
  fIsValidNorm = false;
...
G4TwistTubsSide::G4TwistTubsSide(__void__& a) : G4VTwistSurface(a) {}

//=====================================================================
//* GetNormal ---------------------------------------------------------

G4ThreeVector G4TwistTubsSide::GetNormal(const G4ThreeVector& tmpxx, G4bool isGlobal)
{
...
G4int G4TwistTubsSide::DistanceToSurface(const G4ThreeVector& gp, const G4ThreeVector& gv,
                                         G4ThreeVector gxx[], G4double distance[], G4int areacode[],
    ...
    for (G4int k = 0; k < 2; ++k)
    {
      ...
      if (deltaY > 0.5 * kCarTolerance)
      {
        ...
        if (l == maxcount)
        {
          ...
          message << "Exceeded maxloop count!" << G4endl << "        maxloop count " << maxcount;
          G4Exception("G4TwistTubsFlatSide::DistanceToSurface(p,v)", "GeomSolids0003",
...
G4int G4TwistTubsSide::DistanceToSurface(const G4ThreeVector& gp, G4ThreeVector gxx[],
...
G4double G4TwistTubsSide::DistanceToPlane(const G4ThreeVector& p, const G4ThreeVector& A,
                                          const G4ThreeVector& B, const G4ThreeVector& C,
                                          const G4ThreeVector& D, const G4int parity,
                                          G4ThreeVector& xx, G4ThreeVector& n)
{
  const G4double halftol = 0.5 * kCarTolerance;
  ...
  G4double distToanm =
    G4VTwistSurface::DistanceToPlane(p, A, (N - A), (M - A), xxanm, nanm) * parity;
  G4double distTocmn =
    G4VTwistSurface::DistanceToPlane(p, C, (M - C), (N - C), xxcmn, ncmn) * parity;
#ifdef G4SPECSDEBUG
  // if p is behind of both surfaces, abort.
  if (distToanm * distTocmn > 0 && distToanm < 0)
  {
    G4Exception("G4TwistTubsSide::DistanceToPlane()", "GeomSolids0003", FatalException,
...
G4int G4TwistTubsSide::GetAreaCode(const G4ThreeVector& xx, G4bool withTol)
{
  ...
  G4Exception("G4TwistTubsSide::GetAreaCode()", "GeomSolids0001", FatalException,
...
void G4TwistTubsSide::SetCorners(G4double endInnerRad[2], G4double endOuterRad[2],
                                 G4double endPhi[2], G4double endZ[2])
{
  ...
  else
  {
    ...
    message << "Feature NOT implemented !" << G4endl << "        fAxis[0] = " << fAxis[0] << G4endl
            << "        fAxis[1] = " << fAxis[1];
    G4Exception("G4TwistTubsSide::SetCorners()", "GeomSolids0001", FatalException, message);
  }
}
...
{
  G4Exception("G4TwistTubsSide::SetCorners()", "GeomSolids0001", FatalException,
...
void G4TwistTubsSide::SetBoundaries()
{
  ...
  if (fAxis[0] == kXAxis && fAxis[1] == kZAxis)
  {
    ...
    SetBoundary(sAxis0 & (sAxisX | sAxisMin), direction, GetCorner(sC0Min1Min), sAxisZ);

    // sAxis0 & sAxisMax
    direction = GetCorner(sC0Max1Max) - GetCorner(sC0Max1Min);
    direction = direction.unit();
    SetBoundary(sAxis0 & (sAxisX | sAxisMax), direction, GetCorner(sC0Max1Min), sAxisZ);

    // sAxis1 & sAxisMin
    direction = GetCorner(sC0Max1Min) - GetCorner(sC0Min1Min);
    direction = direction.unit();
    SetBoundary(sAxis1 & (sAxisZ | sAxisMin), direction, GetCorner(sC0Min1Min), sAxisX);

    // sAxis1 & sAxisMax
    direction = GetCorner(sC0Max1Max) - GetCorner(sC0Min1Max);
    direction = direction.unit();
    SetBoundary(sAxis1 & (sAxisZ | sAxisMax), direction, GetCorner(sC0Min1Max), sAxisX);
  }
  else
  {
    std::ostringstream message;
...
void G4TwistTubsSide::GetFacets(G4int k, G4int n, G4double xyz[][3], G4int faces[][4], G4int iside)
{
