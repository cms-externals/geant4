...
G4TwistTubsFlatSide::G4TwistTubsFlatSide(const G4String& name, const G4RotationMatrix& rot,
  ...
  : G4VTwistSurface(name, rot, tlate, 0, axis0, axis1, axis0min, axis1min, axis0max, axis1max)
{
  if (axis0 == kPhi && axis1 == kRho)
  {
    G4Exception("G4TwistTubsFlatSide::G4TwistTubsFlatSide()", "GeomSolids0002",
...
G4TwistTubsFlatSide::G4TwistTubsFlatSide(const G4String& name, G4double EndInnerRadius[2],
                                         G4double EndOuterRadius[2], G4double DPhi,
                                         G4double EndPhi[2], G4double EndZ[2], G4int handedness)
  : G4VTwistSurface(name)
{
  ...
  fSurfaceArea =
    0.5 * DPhi * (EndOuterRadius[i] * EndOuterRadius[i] - EndInnerRadius[i] * EndInnerRadius[i]);
}

//=====================================================================
//* Fake default constructor ------------------------------------------

G4TwistTubsFlatSide::G4TwistTubsFlatSide(__void__& a) : G4VTwistSurface(a) {}

//=====================================================================
//* GetNormal ---------------------------------------------------------

G4ThreeVector G4TwistTubsFlatSide::GetNormal(const G4ThreeVector& /* xx */, G4bool isGlobal)
{
  if (isGlobal)
  {
...
G4int G4TwistTubsFlatSide::DistanceToSurface(const G4ThreeVector& gp, const G4ThreeVector& gv,
...
G4int G4TwistTubsFlatSide::DistanceToSurface(const G4ThreeVector& gp, G4ThreeVector gxx[],
...
G4int G4TwistTubsFlatSide::GetAreaCode(const G4ThreeVector& xx, G4bool withTol)
{
  const G4double rtol = 0.5 * G4GeometryTolerance::GetInstance()->GetRadialTolerance();
  ...
  if (fAxis[0] == kRho && fAxis[1] == kPhi)
  {
    ...
    if (withTol)
    {
      ...
      else if (xx.getRho() >= fAxisMax[rhoaxis] - rtol)
      {
        ...
        if (xx.getRho() > fAxisMax[rhoaxis] + rtol)
        {
          isoutside = true;
        }
      }
  ...
  else
  {
    ...
    message << "Feature NOT implemented !" << G4endl << "        fAxis[0] = " << fAxis[0] << G4endl
            << "        fAxis[1] = " << fAxis[1];
    G4Exception("G4TwistTubsFlatSide::GetAreaCode()", "GeomSolids0001", FatalException, message);
  }
  return areacode;
}
...
void G4TwistTubsFlatSide::SetCorners()
{
  ...
  else
  {
    ...
    message << "Feature NOT implemented !" << G4endl << "        fAxis[0] = " << fAxis[0] << G4endl
            << "        fAxis[1] = " << fAxis[1];
    G4Exception("G4TwistTubsFlatSide::SetCorners()", "GeomSolids0001", FatalException, message);
  }
}
...
void G4TwistTubsFlatSide::SetBoundaries()
{
  ...
  if (fAxis[0] == kRho && fAxis[1] == kPhi)
  {
    ...
    SetBoundary(sAxis0 & (sAxisPhi | sAxisMin), direction, GetCorner(sC0Min1Min), sAxisPhi);

    // sAxis0 & sAxisMax
    direction = GetCorner(sC0Max1Max) - GetCorner(sC0Max1Min);
    direction = direction.unit();
    SetBoundary(sAxis0 & (sAxisPhi | sAxisMax), direction, GetCorner(sC0Max1Min), sAxisPhi);

    // sAxis1 & sAxisMin
    direction = GetCorner(sC0Max1Min) - GetCorner(sC0Min1Min);
    direction = direction.unit();
    SetBoundary(sAxis1 & (sAxisRho | sAxisMin), direction, GetCorner(sC0Min1Min), sAxisRho);

    // sAxis1 & sAxisMax
    direction = GetCorner(sC0Max1Max) - GetCorner(sC0Min1Max);
    direction = direction.unit();
    SetBoundary(sAxis1 & (sAxisRho | sAxisMax), direction, GetCorner(sC0Min1Max), sAxisPhi);
  }
  else
  {
    std::ostringstream message;
    message << "Feature NOT implemented !" << G4endl << "        fAxis[0] = " << fAxis[0] << G4endl
            << "        fAxis[1] = " << fAxis[1];
    G4Exception("G4TwistTubsFlatSide::SetBoundaries()", "GeomSolids0001", FatalException, message);
  }
}
...
void G4TwistTubsFlatSide::GetFacets(G4int k, G4int n, G4double xyz[][3], G4int faces[][4],
                                    G4int iside)
{
  G4ThreeVector p;
