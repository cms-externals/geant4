...
namespace
{
  ...
  G4bool CreateTaskSolidsWorkspace()
  {
  ...
  void CleanupTaskSolidsWorkspace()
  {
    G4SolidsWorkspace::GetPool()->CleanUpAndDestroyAllWorkspaces();
  }

  const G4bool registeredSolidsWorkspaceCallbacks = []
  {
    G4GeomTaskHelper::RegisterWorkspaceCallbacks(1, CreateTaskSolidsWorkspace,
                                                 CleanupTaskSolidsWorkspace);
    return true;
  }();
}
