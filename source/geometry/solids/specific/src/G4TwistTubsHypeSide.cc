...
G4TwistTubsHypeSide::G4TwistTubsHypeSide(const G4String& name, const G4RotationMatrix& rot,
                                         const G4ThreeVector& tlate, const G4int handedness,
                                         const G4double kappa, const G4double tanstereo,
                                         const G4double r0, const EAxis axis0, const EAxis axis1,
                                         G4double axis0min, G4double axis1min, G4double axis0max,
                                         G4double axis1max)
  : G4VTwistSurface(name, rot, tlate, handedness, axis0, axis1, axis0min, axis1min, axis0max,
                    axis1max),
    fKappa(kappa),
    fTanStereo(tanstereo),
    fTan2Stereo(tanstereo * tanstereo),
    fR0(r0),
    fR02(r0 * r0),
    fDPhi(twopi)
{
  ...
  {
    G4Exception("G4TwistTubsHypeSide::G4TwistTubsHypeSide()", "GeomSolids0002",
...
G4TwistTubsHypeSide::G4TwistTubsHypeSide(const G4String& name, G4double EndInnerRadius[2],
                                         G4double EndOuterRadius[2], G4double DPhi,
                                         G4double EndPhi[2], G4double EndZ[2], G4double InnerRadius,
                                         G4double OuterRadius, G4double Kappa,
                                         G4double TanInnerStereo, G4double TanOuterStereo,
                                         G4int handedness)
...
G4TwistTubsHypeSide::G4TwistTubsHypeSide(__void__& a) : G4VTwistSurface(a) {}

//=====================================================================
//* GetNormal ---------------------------------------------------------

G4ThreeVector G4TwistTubsHypeSide::GetNormal(const G4ThreeVector& tmpxx, G4bool isGlobal)
{
...
EInside G4TwistTubsHypeSide::Inside(const G4ThreeVector& gp)
{
  ...
  G4cout << "WARNING - G4TwistTubsHypeSide::Inside()" << G4endl << "          Invalid option !"
         << G4endl << "          name, areacode, distanceToOut = " << GetName() << ", " << std::hex
         << areacode << std::dec << ", " << distanceToOut << G4endl;
  return kOutside;
}
...
G4int G4TwistTubsHypeSide::DistanceToSurface(const G4ThreeVector& gp, const G4ThreeVector& gv,
                                             G4ThreeVector gxx[], G4double distance[],
                                             G4int areacode[], G4bool isvalid[], EValidate validate)
{
  ...
  if (p.mag() == 0)
  {
    ...
    if (vz != 0.0)
    {
      G4double xxz = std::sqrt(fR02 / (vslope2 - fTan2Stereo)) * (vz / std::fabs(vz));
      G4double t = xxz / vz;
...
G4int G4TwistTubsHypeSide::DistanceToSurface(const G4ThreeVector& gp, G4ThreeVector gxx[],
...
G4int G4TwistTubsHypeSide::GetAreaCode(const G4ThreeVector& xx, G4bool withTol)
{
  ...
  else
  {
    ...
    message << "Feature NOT implemented !" << G4endl << "        fAxis[0] = " << fAxis[0] << G4endl
            << "        fAxis[1] = " << fAxis[1];
    G4Exception("G4TwistTubsHypeSide::GetAreaCode()", "GeomSolids0001", FatalException, message);
  }
  return areacode;
}
...
G4int G4TwistTubsHypeSide::GetAreaCodeInPhi(const G4ThreeVector& xx, G4bool withTol)
{
  ...
  if (withTol)
  {
    if (AmIOnLeftSide(xx, lowerlimit) >= 0)  // xx is on lowerlimit
    {
      ...
      if (AmIOnLeftSide(xx, lowerlimit) > 0)
      {
        isoutside = true;
      }
    }
    else if (AmIOnLeftSide(xx, upperlimit) <= 0)  // xx is on upperlimit
    {
      ...
      if (AmIOnLeftSide(xx, upperlimit) < 0)
      {
        isoutside = true;
      }
    }
...
void G4TwistTubsHypeSide::SetCorners(G4double EndInnerRadius[2], G4double EndOuterRadius[2],
                                     G4double DPhi, G4double endPhi[2], G4double endZ[2])
{
  ...
  else
  {
    ...
    message << "Feature NOT implemented !" << G4endl << "        fAxis[0] = " << fAxis[0] << G4endl
            << "        fAxis[1] = " << fAxis[1];
    G4Exception("G4TwistTubsHypeSide::SetCorners()", "GeomSolids0001", FatalException, message);
  }
}
...
{
  G4Exception("G4TwistTubsHypeSide::SetCorners()", "GeomSolids0001", FatalException,
...
void G4TwistTubsHypeSide::SetBoundaries()
{
  ...
  if (fAxis[0] == kPhi && fAxis[1] == kZAxis)
  {
    ...
    SetBoundary(sAxis0 & (sAxisPhi | sAxisMin), direction, GetCorner(sC0Min1Min), sAxisZ);

    // sAxis0 & sAxisMax
    direction = GetCorner(sC0Max1Max) - GetCorner(sC0Max1Min);
    direction = direction.unit();
    SetBoundary(sAxis0 & (sAxisPhi | sAxisMax), direction, GetCorner(sC0Max1Min), sAxisZ);

    // sAxis1 & sAxisMin
    direction = GetCorner(sC0Max1Min) - GetCorner(sC0Min1Min);
    direction = direction.unit();
    SetBoundary(sAxis1 & (sAxisZ | sAxisMin), direction, GetCorner(sC0Min1Min), sAxisPhi);

    // sAxis1 & sAxisMax
    direction = GetCorner(sC0Max1Max) - GetCorner(sC0Min1Max);
    direction = direction.unit();
    SetBoundary(sAxis1 & (sAxisZ | sAxisMax), direction, GetCorner(sC0Min1Max), sAxisPhi);
  }
  else
  {
    std::ostringstream message;
    message << "Feature NOT implemented !" << G4endl << "        fAxis[0] = " << fAxis[0] << G4endl
            << "        fAxis[1] = " << fAxis[1];
    G4Exception("G4TwistTubsHypeSide::SetBoundaries()", "GeomSolids0001", FatalException, message);
  }
}
...
void G4TwistTubsHypeSide::GetFacets(G4int k, G4int n, G4double xyz[][3], G4int faces[][4],
                                    G4int iside)
{
  ...
  for (G4int i = 0; i < n; ++i)
  {
    ...
    for (G4int j = 0; j < k; ++j)
    {
      ...
      if (i < n - 1 && j < k - 1)  // clock wise filling
      {
        ...
        faces[nface][1] =
          GetEdgeVisibility(i, j, k, n, 1, 1) * (GetNode(i + 1, j, k, n, iside) + 1);
        faces[nface][2] =
          GetEdgeVisibility(i, j, k, n, 2, 1) * (GetNode(i + 1, j + 1, k, n, iside) + 1);
        faces[nface][3] =
          GetEdgeVisibility(i, j, k, n, 3, 1) * (GetNode(i, j + 1, k, n, iside) + 1);
      }
    }
  }
}
