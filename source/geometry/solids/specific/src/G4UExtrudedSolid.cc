...
// Constructors
//
G4UExtrudedSolid::G4UExtrudedSolid(const G4String& name, const std::vector<G4TwoVector>& polygon,
  ...
  : Base_t(name)  // General constructor
{
  ...
  {
    sections[i].fOrigin.Set(zsections[i].fOffset.x(), zsections[i].fOffset.y(), zsections[i].fZ);
...
G4UExtrudedSolid::G4UExtrudedSolid(const G4String& name, const std::vector<G4TwoVector>& polygon,
                                   G4double halfZ, const G4TwoVector& off1, G4double scale1,
...
// Copy constructor
//
G4UExtrudedSolid::G4UExtrudedSolid(const G4UExtrudedSolid& source) : Base_t(source) {}
...
G4UExtrudedSolid::ZSection G4UExtrudedSolid::GetZSection(G4int i) const
{
  ...
  return {sect.fOrigin[2], G4TwoVector(sect.fOrigin[0], sect.fOrigin[1]), sect.fScale};
}
...
std::vector<G4UExtrudedSolid::ZSection> G4UExtrudedSolid::GetZSections() const
{
  ...
  {
    vecgeom::XtruSection sect = Base_t::GetSection(i);
    sections.emplace_back(sect.fOrigin[2], G4TwoVector(sect.fOrigin[0], sect.fOrigin[1]),
                          sect.fScale);
  }
  return sections;
}
...
void G4UExtrudedSolid::BoundingLimits(G4ThreeVector& pMin, G4ThreeVector& pMax) const
{
  ...
  if (pMin.x() >= pMax.x() || pMin.y() >= pMax.y() || pMin.z() >= pMax.z())
  {
    ...
    message << "Bad bounding box (min >= max) for solid: " << GetName() << " !"
            << "\npMin = " << pMin << "\npMax = " << pMax;
    G4Exception("G4UExtrudedSolid::BoundingLimits()", "GeomMgt0001", JustWarning, message);
    StreamInfo(G4cout);
  }
...
G4bool G4UExtrudedSolid::CalculateExtent(const EAxis pAxis, const G4VoxelLimits& pVoxelLimit,
...
#  endif
  ...
  if (!G4GeomTools::TriangulatePolygon(basePolygon, triangles))
  {
    ...
    G4Exception("G4UExtrudedSolid::CalculateExtent()", "GeomMgt1002", JustWarning, message);
    return bbox.CalculateExtent(pAxis, pVoxelLimit, pTransform, pMin, pMax);
  }
