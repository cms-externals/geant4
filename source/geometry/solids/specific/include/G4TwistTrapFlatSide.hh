...
class G4TwistTrapFlatSide : public G4VTwistSurface
{
  public:
     ...
     */
    G4TwistTrapFlatSide(const G4String& name, G4double PhiTwist, G4double pDx1, G4double pDx2,
                        G4double pDy, G4double pDz, G4double pAlpha, G4double pPhi, G4double pTheta,
                        G4int handedness);
  ...
  private:
     ...
     */
    inline G4ThreeVector SurfacePoint(G4double x, G4double y, G4bool isGlobal = false) override;
...
inline G4double G4TwistTrapFlatSide::xAxisMax(G4double u, G4double fTanAlpha) const
{
  return ((fDx2 + fDx1) / 2. + u * (fDx2 - fDx1) / (2. * fDy) + u * fTanAlpha);
}

inline G4ThreeVector G4TwistTrapFlatSide::SurfacePoint(G4double x, G4double y, G4bool isGlobal)
{
...
inline G4double G4TwistTrapFlatSide::GetBoundaryMin(G4double y)
{
  return -xAxisMax(y, -fTAlph);
}

inline G4double G4TwistTrapFlatSide::GetBoundaryMax(G4double y)
{
  return xAxisMax(y, fTAlph);
}

inline G4double G4TwistTrapFlatSide::GetSurfaceArea()
{
