...
class G4Polycone : public G4VCSGfaceted
{
  ...
  private:
    ...
    /**
     * RZ section auxiliary methods for fast retrieval.
     */
    static constexpr G4int kSmallZPlaneFastPath = 10;

    void BuildRZIndex(G4bool profileIsConvex = false);
    EInside PhiState(const G4ThreeVector& p) const;
    G4double DistanceToPhiBoundary(const G4ThreeVector& p, G4ThreeVector* normal = nullptr) const;
    G4bool HasZPlaneInsideFastPath() const;
    EInside InsideZPlane(const G4ThreeVector& p) const;
    EInside InsideRZOnly(const G4ThreeVector& p) const;
    inline EInside InsideRZ(const G4ThreeVector& p) const;
    inline G4double DistanceToRZ(const G4ThreeVector& p, G4bool outgoing) const;
    G4bool IntersectRZSection(G4int i, const G4ThreeVector& p, const G4ThreeVector& v,
                              G4bool outgoing, G4double surfTolerance, G4double& distance,
                              G4double& distFromSurface, G4ThreeVector* normal) const;
    G4bool IntersectPhiPlane(const G4ThreeVector& p, const G4ThreeVector& v, G4bool outgoing,
                             G4double surfTolerance, G4double& distance,
                             G4double& distFromSurface, G4ThreeVector* normal) const;
    G4double DistanceToFullPhiTwoPlaneNoNormal(const G4ThreeVector& p,
                                               const G4ThreeVector& v,
                                               G4bool outgoing) const;
    G4double DistanceToRZ(const G4ThreeVector& p, const G4ThreeVector& v, G4bool outgoing,
                          G4bool calcNorm = false, G4ThreeVector* normal = nullptr,
                          G4bool* validNorm = nullptr) const;
    G4ThreeVector SurfaceNormalRZ(const G4ThreeVector& p) const;

  private:

    /** Cached RZ sections. */
    struct RZSection : G4RZSection
    {
    ...
    G4bool fZPlanesMonotonic = false;
    G4double fSinStartPhi = 0.;
    G4double fCosStartPhi = 1.;
    G4double fSinEndPhi = 0.;
    G4double fCosEndPhi = 1.;
    ...
    G4bool fRZConvex = false;
    std::vector<RZSection> fRZSection;
    std::vector<G4IntersectingCone> fRZCone;
    std::unique_ptr<G4ZCandidateData> fRZVoxel;
