...
struct G4PolyPhiFaceVertex
{
  ...
  G4double rNorm, zNorm;  // r/z normal
...
class G4PolyPhiFace : public G4VCSGface
{
  ...
  private:
    ...
    struct EdgeNode
    {
      G4double rMin = 0., rMax = 0., zMin = 0., zMax = 0.;
      G4int begin = 0, count = 0, left = -1, right = -1;
    };
    void BuildEdgeIndex();
    G4int BuildEdgeNode(G4int begin, G4int end);
    inline G4double EdgeNodeDistance2(const EdgeNode& node,
                                      G4double r, G4double z) const;
  ...
  private:
    ...
    std::vector<G4int> fEdgeOrder;
    std::vector<EdgeNode> fEdgeNode;
