...
class G4TwistTubsSide : public G4VTwistSurface
{
  public:
    ...
    G4TwistTubsSide(const G4String& name,
                    ...
                    const EAxis axis0 = kXAxis, const EAxis axis1 = kZAxis,
     ...
     *  @param[in] handedness Orientation: R-hand = 1, L-hand = -1.
     */
    G4TwistTubsSide(const G4String& name, G4double EndInnerRadius[2], G4double EndOuterRadius[2],
                    G4double DPhi, G4double EndPhi[2], G4double EndZ[2], G4double InnerRadius,
                    G4double OuterRadius, G4double Kappa, G4int handedness);
  ...
  private:
     ...
     */
    inline G4ThreeVector SurfacePoint(G4double x, G4double z, G4bool isGlobal = false) override;
     ...
     */
    G4double DistanceToPlane(const G4ThreeVector& p, const G4ThreeVector& A, const G4ThreeVector& B,
                             const G4ThreeVector& C, const G4ThreeVector& D, const G4int parity,
    ...
    void SetCorners(G4double endInnerRad[2], G4double endOuterRad[2], G4double endPhi[2],
                    G4double endZ[2]);
...
inline G4ThreeVector G4TwistTubsSide::ProjectAtPXPZ(const G4ThreeVector& p, G4bool isglobal) const
{
...
inline G4ThreeVector G4TwistTubsSide::SurfacePoint(G4double x, G4double z, G4bool isGlobal)
{
...
inline G4double G4TwistTubsSide::GetBoundaryMin(G4double)
{
  return fAxisMin[0];  // inner radius at z = 0
}

inline G4double G4TwistTubsSide::GetBoundaryMax(G4double)
{
  return fAxisMax[0];  // outer radius at z = 0
}

inline G4double G4TwistTubsSide::GetSurfaceArea()
{
  // approximation only
