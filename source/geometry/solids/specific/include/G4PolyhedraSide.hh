...
class G4PolyhedraSide : public G4VCSGface
{
  ...
  private:
    ...
    /**
     * Decides which phi segment an X/Y point belongs to without computing
     * the angle. Ambiguous boundary and axis points are resolved through
     * PhiSegment() to preserve the historical convention.
     */
    G4int PhiSegment(const G4double x, const G4double y);
  ...
  private:
    ...
    std::vector<G4double> phiBoundaryCos;
    std::vector<G4double> phiBoundarySin;
