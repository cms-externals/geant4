...
// 02.02.2016 Evgueni Tcherniaev - corrected testVolume()
...
{
  // one vertex at -Z: 0=1=2=3 (pyramid)
  // surface: 17794.87 cm^2 , volume: 90000 cm^3
...
G4VSolid* createSolid1(std::vector<G4ThreeVector>& inside_points,
                       ...
                       std::vector<G4ThreeVector>& outside_points)
{
  // two vertices at -Z: 0=1 2=3 (oblique triangular prism)
  // surface: 22396.65 cm^2 , volume: 135000 cm^3
...
G4VSolid* createSolid2(std::vector<G4ThreeVector>& inside_points,
                       ...
                       std::vector<G4ThreeVector>& outside_points)
{
  // one vertex at +Z: 4=5=6=7 (pyramid)
  // surface: 17794.87 cm^2 , volume: 90000 cm^3  
...
G4VSolid* createSolid3(std::vector<G4ThreeVector>& inside_points,
                       ...
                       std::vector<G4ThreeVector>& outside_points)
{
  // two vertices at +Z: 4=5 6=7 (oblique triangular prism)
  // surface: 22396.65 cm^2 , volume: 135000 cm^3
...
G4VSolid* createSolid4(std::vector<G4ThreeVector>& inside_points,
                       ...
                       std::vector<G4ThreeVector>& outside_points)
{
  // 4 top & 4 bottom vertices, Box like solid 
  // surface: 70200 cm^2 , volume: 1215000 cm^3
...
G4VSolid* createSolid5(std::vector<G4ThreeVector>& inside_points,
                       ...
                       std::vector<G4ThreeVector>& outside_points)
{
  // 4 top & 4 bottom vertices, Trap like solid 
  // surface: 61106.55 cm^2 , volume: 965000 cm^3
...
G4VSolid* createSolid6(std::vector<G4ThreeVector>& inside_points,
                       ...
                       std::vector<G4ThreeVector>& outside_points)
{
  // 4 bottom & 4 top vertices, twisted
...
G4VSolid* createSolid7(std::vector<G4ThreeVector>& inside_points,
                       ...
                       std::vector<G4ThreeVector>& outside_points)
{
  // 4 bottom & 3 top vertices: 7=8, twisted
...
// Test surface
  ...
  if ( testCase == 0 ) {
    assert( std::fabs(solid->GetSurfaceArea() - 1779486.9168738 ) < 1.e-6 );
  }
  if ( testCase == 1 ) {
    assert( std::fabs(solid->GetSurfaceArea() - 2239664.8326421 ) < 1.e-6 );
  }
  if ( testCase == 2 ) {
    assert( std::fabs(solid->GetSurfaceArea() - 1779486.9168738 ) < 1.e-6 );
  }
  ...
  if ( testCase == 4 ) {
    assert( std::fabs(solid->GetSurfaceArea() - 7020000.0       ) < 1.e-6 );
  }
  if ( testCase == 5 ) {
    assert( std::fabs(solid->GetSurfaceArea() - 6110654.8410793 ) < 1.e-6 );
  }
  if ( testCase == 6 ) { // twisted, approximate surface area
    assert( std::fabs(solid->GetSurfaceArea() - 5.6721e+06 ) < 0.001e+6 );
  }
  if ( testCase == 7 ) { // twisted, approximate surface area
    assert( std::fabs(solid->GetSurfaceArea() - 5.1599e+06 ) < 0.001e+6 );
  }
...
// The volume is evaluated via G4VSolid, that's why the precision is very low
  ...
  if ( testCase == 0 ) {
    assert( std::fabs(solid->GetCubicVolume() -   90.e+6 ) < 1.e-6 );
  }
  if ( testCase == 1 ) {
    assert( std::fabs(solid->GetCubicVolume() -  135.e+6 ) < 1.e-6 );
  }
  if ( testCase == 2 ) {
    assert( std::fabs(solid->GetCubicVolume() -   90.e+6 ) < 1.e-6 );
  }
  ...
  if ( testCase == 4 ) {
    assert( std::fabs(solid->GetCubicVolume() - 1215.e+6 ) < 1.e-6 );
  }
  if ( testCase == 5 ) {
    assert( std::fabs(solid->GetCubicVolume() -  965.e+6 ) < 1.e-6 );
  }
  if ( testCase == 6 ) { // twisted, approximate volume
    assert( std::fabs(solid->GetCubicVolume() -  821.e+6 ) < 5.e+6 );
  }
  if ( testCase == 7 ) { // twisted, approximate volume
    assert( std::fabs(solid->GetCubicVolume() -  606.e+6 ) < 5.e+6 );
  }
