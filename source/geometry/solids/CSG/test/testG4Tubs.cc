...
G4bool testG4Tubs()
{
  ...
  G4Tubs tube10("tube10",400*mm,405*mm,400*mm,0*degree,360*degree) ;
    ...
    // Problem case from ATLAS  2019.05.28    
    G4Tubs* t29 =
      new G4Tubs("Atlas2019-05", 0, 29., 2230.0 , 0., 2.0*pi); // ATLAS report May 2019
...
// Check Surface Normal
    ...
    // Problem case from ATLAS  2019.05.28
    G4ThreeVector exitPos29(-28.163878,7.1867773,-1419.5445);  // ATLAS normal issue - coded 2019.05.28
    normal=t29->SurfaceNormal(G4ThreeVector(exitPos29)); // -28.163878,7.1867773,-1419.5445));
    assert(ApproxEqual(normal.mag2(),1.0));
...
// DistanceToOut(P,V)
 ...
 G4ThreeVector(-0.1443054266272111,-0.01508874701037938,0.9894181489944458),
    ...
    // Problem case from ATLAS  2019.05.28
    G4ThreeVector dir29( 0.0001, 0.0001, 0.99989999499949993749124868);
    Dist=t29->DistanceToOut(exitPos29,dir29,calcNorm,pgoodNorm,pNorm);
    // assert(ApproxEqual(Dist,0.0)&&ApproxEqual(pNorm->mag(),1.0)&&*pgoodNorm);
    assert(ApproxEqual(Dist,0.0));
    assert(pgoodNorm);
    if( ! ApproxEqual(pNorm->mag(),1.0)) { 
       G4cerr << "ERROR> Non-unit *Normal* at position = " << exitPos29 << G4endl
              << "   Normal vector= " << *pNorm
              << "   and its magnitude = " << pNorm->mag() << G4endl;
    }
    assert( std::fabs(pNorm->mag()-1.0) < 1.0e-6 );
    
#if 0    
    G4double dist= -15.0;
    G4ThreeVector start29=  
    Dist=t29->DistanceToOut(start29,dir29,calcNorm,pgoodNorm,pNorm);
    assert(ApproxEqual(Dist,dist29)&&ApproxEqual(pNorm->unit(),vx)&&*pgoodNorm);
#endif    
    G4cout<<G4endl ;
...
// CalculateExtent
    ...
    t2.CalculateExtent(kXAxis,limitXYZ,origin,min,max);
    G4cout<<"t2.CE(kXAxis,limitXYZ,origin,min = "
          <<min<<"; max = "<<max<<G4endl;
    // assert(min<=-20&&max>=-30);

    t2.CalculateExtent(kYAxis,limitXYZ,origin,min,max);
    G4cout<<"t2.CE(kYAxis,limitXYZ,origin,min = "
          <<min<<"; max = "<<max<<G4endl;
    // assert(min<=30&&max>=40);

    t2.CalculateExtent(kZAxis,limitXYZ,origin,min,max);
    G4cout<<"t2.CE(kZAxis,limitXYZ,origin,min = "
          <<min<<"; max = "<<max<<G4endl<<G4endl;
    // assert(min<=-40&&max>=-30);

    t2a.CalculateExtent(kXAxis,limitXYZ,origin,min,max);
    G4cout<<"t2a.CE(kXAxis,limitXYZ,origin,min = "
          <<min<<"; max = "<<max<<G4endl;
    // assert(min<=-20&&max>=-30);

    t2a.CalculateExtent(kYAxis,limitXYZ,origin,min,max);
    G4cout<<"t2a.CE(kYAxis,limitXYZ,origin,min = "
          <<min<<"; max = "<<max<<G4endl;
    // assert(min<=30&&max>=40);

    t2a.CalculateExtent(kZAxis,limitXYZ,origin,min,max);
    G4cout<<"t2a.CE(kZAxis,limitXYZ,origin,min = "
          <<min<<"; max = "<<max<<G4endl<<G4endl;
    // assert(min<=-40&&max>=-30);

    t2b.CalculateExtent(kXAxis,limitXYZ,origin,min,max);
    G4cout<<"t2b.CE(kXAxis,limitXYZ,origin,min = "
          <<min<<"; max = "<<max<<G4endl;
    // assert(min<=-20&&max>=-30);

    t2b.CalculateExtent(kYAxis,limitXYZ,origin,min,max);
    G4cout<<"t2b.CE(kYAxis,limitXYZ,origin,min = "
          <<min<<"; max = "<<max<<G4endl;
    // assert(min<=30&&max>=40);

    t2b.CalculateExtent(kZAxis,limitXYZ,origin,min,max);
    G4cout<<"t2b.CE(kZAxis,limitXYZ,origin,min = "
          <<min<<"; max = "<<max<<G4endl<<G4endl;
    // assert(min<=-40&&max>=-30);

    t2c.CalculateExtent(kXAxis,limitXYZ,origin,min,max);
    G4cout<<"t2c.CE(kXAxis,limitXYZ,origin,min = "
          <<min<<"; max = "<<max<<G4endl;
    // assert(min<=-20&&max>=-30);

    t2c.CalculateExtent(kYAxis,limitXYZ,origin,min,max);
    G4cout<<"t2c.CE(kYAxis,limitXYZ,origin,min = "
          <<min<<"; max = "<<max<<G4endl;
    // assert(min<=30&&max>=40);

    t2c.CalculateExtent(kZAxis,limitXYZ,origin,min,max);
    G4cout<<"t2c.CE(kZAxis,limitXYZ,origin,min = "
          <<min<<"; max = "<<max<<G4endl<<G4endl;
    // assert(min<=-40&&max>=-30);

    t2d.CalculateExtent(kXAxis,limitXYZ,origin,min,max);
    G4cout<<"t2d.CE(kXAxis,limitXYZ,origin,min = "
          <<min<<"; max = "<<max<<G4endl;
    // assert(min<=-20&&max>=-30);

    t2d.CalculateExtent(kYAxis,limitXYZ,origin,min,max);
    G4cout<<"t2d.CE(kYAxis,limitXYZ,origin,min = "
          <<min<<"; max = "<<max<<G4endl;
    // assert(min<=30&&max>=40);

    t2d.CalculateExtent(kZAxis,limitXYZ,origin,min,max);
    G4cout<<"t2d.CE(kZAxis,limitXYZ,origin,min = "
          <<min<<"; max = "<<max<<G4endl<<G4endl;
    // assert(min<=-40&&max>=-30);


    t3.CalculateExtent(kXAxis,limitXYZ,origin,min,max);
    G4cout<<"t3.CE(kXAxis,limitXYZ,origin,min = "
          <<min<<"; max = "<<max<<G4endl;
    // assert(min<=-20&&max>=-30);


    t3.CalculateExtent(kYAxis,limitXYZ,origin,min,max);
    G4cout<<"t3.CE(kYAxis,limitXYZ,origin,min = "
          <<min<<"; max = "<<max<<G4endl;
    // assert(min<=30&&max>=40);

    t3.CalculateExtent(kZAxis,limitXYZ,origin,min,max);
    G4cout<<"t3.CE(kZAxis,limitXYZ,origin,min = "
          <<min<<"; max = "<<max<<G4endl<<G4endl;
    // assert(min<=-40&&max>=-30);

    t1.CalculateExtent(kXAxis,limitXsYZ,origin,min,max);
    G4cout<<"t1.CE(kXAxis,limitXsYZ,origin,min = "
          <<min<<"; max = "<<max<<G4endl;
    // assert(min<=-20&&max>=-30);

    t1.CalculateExtent(kYAxis,limitXsYZ,origin,min,max);
    G4cout<<"t1.CE(kYAxis,limitXsYZ,origin,min = "
          <<min<<"; max = "<<max<<G4endl;
    // assert(min<=30&&max>=40);

    t1.CalculateExtent(kZAxis,limitXsYZ,origin,min,max);
    G4cout<<"t1.CE(kZAxis,limitXsYZ,origin,min = "
          <<min<<"; max = "<<max<<G4endl<<G4endl;
    // assert(min<=-40&&max>=-30);

    t2.CalculateExtent(kXAxis,limitXsYZ,origin,min,max);
    G4cout<<"t2.CE(kXAxis,limitXsYZ,origin,min = "
          <<min<<"; max = "<<max<<G4endl;
    // assert(min<=-20&&max>=-30);

    t2.CalculateExtent(kYAxis,limitXsYZ,origin,min,max);
    G4cout<<"t2.CE(kYAxis,limitXsYZ,origin,min = "
          <<min<<"; max = "<<max<<G4endl;
    // assert(min<=30&&max>=40);

    t2.CalculateExtent(kZAxis,limitXsYZ,origin,min,max);
    G4cout<<"t2.CE(kZAxis,limitXsYZ,origin,min = "
          <<min<<"; max = "<<max<<G4endl<<G4endl;
    // assert(min<=-40&&max>=-30);

    t2a.CalculateExtent(kXAxis,limitXsYZ,origin,min,max);
    G4cout<<"t2a.CE(kXAxis,limitXsYZ,origin,min = "
          <<min<<"; max = "<<max<<G4endl;
    // assert(min<=-20&&max>=-30);

    t2a.CalculateExtent(kYAxis,limitXsYZ,origin,min,max);
    G4cout<<"t2a.CE(kYAxis,limitXsYZ,origin,min = "
          <<min<<"; max = "<<max<<G4endl;
    // assert(min<=30&&max>=40);

    t2a.CalculateExtent(kZAxis,limitXsYZ,origin,min,max);
    G4cout<<"t2a.CE(kZAxis,limitXsYZ,origin,min = "
          <<min<<"; max = "<<max<<G4endl<<G4endl;
    // assert(min<=-40&&max>=-30);

    t2b.CalculateExtent(kXAxis,limitXsYZ,origin,min,max);
    G4cout<<"t2b.CE(kXAxis,limitXsYZ,origin,min = "
          <<min<<"; max = "<<max<<G4endl;
    // assert(min<=-20&&max>=-30);

    t2b.CalculateExtent(kYAxis,limitXsYZ,origin,min,max);
    G4cout<<"t2b.CE(kYAxis,limitXsYZ,origin,min = "
          <<min<<"; max = "<<max<<G4endl;
    // assert(min<=30&&max>=40);

    t2b.CalculateExtent(kZAxis,limitXsYZ,origin,min,max);
    G4cout<<"t2b.CE(kZAxis,limitXsYZ,origin,min = "
          <<min<<"; max = "<<max<<G4endl<<G4endl;
    // assert(min<=-40&&max>=-30);

    t2c.CalculateExtent(kXAxis,limitXsYZ,origin,min,max);
    G4cout<<"t2c.CE(kXAxis,limitXsYZ,origin,min = "
          <<min<<"; max = "<<max<<G4endl;
    // assert(min<=-20&&max>=-30);

    t2c.CalculateExtent(kYAxis,limitXsYZ,origin,min,max);
    G4cout<<"t2c.CE(kYAxis,limitXsYZ,origin,min = "
          <<min<<"; max = "<<max<<G4endl;
    // assert(min<=30&&max>=40);

    t2c.CalculateExtent(kZAxis,limitXsYZ,origin,min,max);
    G4cout<<"t2c.CE(kZAxis,limitXsYZ,origin,min = "
          <<min<<"; max = "<<max<<G4endl<<G4endl;
    // assert(min<=-40&&max>=-30);

    t2d.CalculateExtent(kXAxis,limitXsYZ,origin,min,max);
    G4cout<<"t2d.CE(kXAxis,limitXsYZ,origin,min = "
          <<min<<"; max = "<<max<<G4endl;
    // assert(min<=-20&&max>=-30);

    t2d.CalculateExtent(kYAxis,limitXsYZ,origin,min,max);
    G4cout<<"t2.CE(kYAxis,limitXsYZ,origin,min = "
          <<min<<"; max = "<<max<<G4endl;
    // assert(min<=30&&max>=40);
    ...
    t1.CalculateExtent(kXAxis,limitX,tRotZ,min,max);
    G4cout<<"t1.CE(kXAxis,limitX,tRotZ,min = "
          <<min<<"; max = "<<max<<G4endl;
    // assert(min<=-20&&max>=-30);

    t1.CalculateExtent(kYAxis,limitY,tRotZ,min,max);
    G4cout<<"t1.CE(kYAxis,limitY,tRotZ,min = "
          <<min<<"; max = "<<max<<G4endl;
    // assert(min<=30&&max>=40);

    t1.CalculateExtent(kZAxis,limitZ,tRotZ,min,max);
    G4cout<<"t1.CE(kZAxis,limitZ,tRotZ,min = "
          <<min<<"; max = "<<max<<G4endl<<G4endl;
    // assert(min<=-40&&max>=-30);

    t3.CalculateExtent(kXAxis,limitX,tRotZ,min,max);
    G4cout<<"t3.CE(kXAxis,limitX,tRotZ,min = "
          <<min<<"; max = "<<max<<G4endl;
    // assert(min<=-20&&max>=-30);

    t3.CalculateExtent(kYAxis,limitY,tRotZ,min,max);
    G4cout<<"t3.CE(kYAxis,limitY,tRotZ,min = "
          <<min<<"; max = "<<max<<G4endl;
    // assert(min<=30&&max>=40);

    t3.CalculateExtent(kZAxis,limitZ,tRotZ,min,max);
    G4cout<<"t3.CE(kZAxis,limitZ,tRotZ,min = "
          <<min<<"; max = "<<max<<G4endl<<G4endl;
    // assert(min<=-40&&max>=-30);

    t2.CalculateExtent(kXAxis,limitX,tRotZ,min,max);
    G4cout<<"t2.CE(kXAxis,limitX,tRotZ,min = "
          <<min<<"; max = "<<max<<G4endl;
    // assert(min<=-20&&max>=-30);

    t2.CalculateExtent(kYAxis,limitY,tRotZ,min,max);
    G4cout<<"t2.CE(kYAxis,limitY,tRotZ,min = "
          <<min<<"; max = "<<max<<G4endl;
    // assert(min<=30&&max>=40);

    t2.CalculateExtent(kZAxis,limitZ,tRotZ,min,max);
    ...
    ******************************** */
    ...
    t1.CalculateExtent(kXAxis,limitXYZ,tRotZ,min,max);
    G4cout<<"t1.CE(kXAxis,limitXYZ,tRotZ,min = "
          <<min<<"; max = "<<max<<G4endl;
    // assert(min<=-20&&max>=-30);


    t1.CalculateExtent(kYAxis,limitXYZ,tRotZ,min,max);
    G4cout<<"t1.CE(kYAxis,limitXYZ,tRotZ,min = "
          <<min<<"; max = "<<max<<G4endl;
    // assert(min<=30&&max>=40);

    t1.CalculateExtent(kZAxis,limitXYZ,tRotZ,min,max);
    G4cout<<"t1.CE(kZAxis,limitXYZ,tRotZ,min = "
          <<min<<"; max = "<<max<<G4endl<<G4endl;
    // assert(min<=-40&&max>=-30);

    t2.CalculateExtent(kXAxis,limitXYZ,tRotZ,min,max);
    G4cout<<"t2.CE(kXAxis,limitXYZ,tRotZ,min = "
          <<min<<"; max = "<<max<<G4endl;
    // assert(min<=-20&&max>=-30);

    t2.CalculateExtent(kYAxis,limitXYZ,tRotZ,min,max);
    G4cout<<"t2.CE(kYAxis,limitXYZ,tRotZ,min = "
          <<min<<"; max = "<<max<<G4endl;
    // assert(min<=30&&max>=40);

    t2.CalculateExtent(kZAxis,limitXYZ,tRotZ,min,max);
    G4cout<<"t2.CE(kZAxis,limitXYZ,tRotZ,min = "
          <<min<<"; max = "<<max<<G4endl<<G4endl;
    // assert(min<=-40&&max>=-30);

    t2a.CalculateExtent(kXAxis,limitXYZ,tRotZ,min,max);
    G4cout<<"t2a.CE(kXAxis,limitXYZ,tRotZ,min = "
          <<min<<"; max = "<<max<<G4endl;
    // assert(min<=-20&&max>=-30);

    t2a.CalculateExtent(kYAxis,limitXYZ,tRotZ,min,max);
    G4cout<<"t2a.CE(kYAxis,limitXYZ,tRotZ,min = "
          <<min<<"; max = "<<max<<G4endl;
    // assert(min<=30&&max>=40);

    t2a.CalculateExtent(kZAxis,limitXYZ,tRotZ,min,max);
    G4cout<<"t2a.CE(kZAxis,limitXYZ,tRotZ,min = "
          <<min<<"; max = "<<max<<G4endl<<G4endl;
    // assert(min<=-40&&max>=-30);

    t2b.CalculateExtent(kXAxis,limitXYZ,tRotZ,min,max);
    G4cout<<"t2b.CE(kXAxis,limitXYZ,tRotZ,min = "
          <<min<<"; max = "<<max<<G4endl;
    // assert(min<=-20&&max>=-30);

    t2b.CalculateExtent(kYAxis,limitXYZ,tRotZ,min,max);
    G4cout<<"t2b.CE(kYAxis,limitXYZ,tRotZ,min = "
          <<min<<"; max = "<<max<<G4endl;
    // assert(min<=30&&max>=40);

    t2b.CalculateExtent(kZAxis,limitXYZ,tRotZ,min,max);
    G4cout<<"t2b.CE(kZAxis,limitXYZ,tRotZ,min = "
          <<min<<"; max = "<<max<<G4endl<<G4endl;
    // assert(min<=-40&&max>=-30);

    t2c.CalculateExtent(kXAxis,limitXYZ,tRotZ,min,max);
    G4cout<<"t2c.CE(kXAxis,limitXYZ,tRotZ,min = "
          <<min<<"; max = "<<max<<G4endl;
    // assert(min<=-20&&max>=-30);

    t2c.CalculateExtent(kYAxis,limitXYZ,tRotZ,min,max);
    G4cout<<"t2c.CE(kYAxis,limitXYZ,tRotZ,min = "
          <<min<<"; max = "<<max<<G4endl;
    // assert(min<=30&&max>=40);

    t2c.CalculateExtent(kZAxis,limitXYZ,tRotZ,min,max);
    G4cout<<"t2c.CE(kZAxis,limitXYZ,tRotZ,min = "
          <<min<<"; max = "<<max<<G4endl<<G4endl;
    // assert(min<=-40&&max>=-30);

    t2d.CalculateExtent(kXAxis,limitXYZ,tRotZ,min,max);
    G4cout<<"t2d.CE(kXAxis,limitXYZ,tRotZ,min = "
          <<min<<"; max = "<<max<<G4endl;
    // assert(min<=-20&&max>=-30);

    t2d.CalculateExtent(kYAxis,limitXYZ,tRotZ,min,max);
    G4cout<<"t2d.CE(kYAxis,limitXYZ,tRotZ,min = "
          <<min<<"; max = "<<max<<G4endl;
    // assert(min<=30&&max>=40);

    t2d.CalculateExtent(kZAxis,limitXYZ,tRotZ,min,max);
    G4cout<<"t2d.CE(kZAxis,limitXYZ,tRotZ,min = "
          <<min<<"; max = "<<max<<G4endl<<G4endl;
    // assert(min<=-40&&max>=-30);

    t3.CalculateExtent(kXAxis,limitXYZ,tRotZ,min,max);
    G4cout<<"t3.CE(kXAxis,limitXYZ,tRotZ,min = "
          <<min<<"; max = "<<max<<G4endl;
    // assert(min<=-20&&max>=-30);

    t3.CalculateExtent(kYAxis,limitXYZ,tRotZ,min,max);
    G4cout<<"t3.CE(kYAxis,limitXYZ,tRotZ,min = "
          <<min<<"; max = "<<max<<G4endl;
    // assert(min<=30&&max>=40);

    t3.CalculateExtent(kZAxis,limitXYZ,tRotZ,min,max);
