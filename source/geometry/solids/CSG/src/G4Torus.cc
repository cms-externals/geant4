...
void G4Torus::TorusRootsJT(const G4ThreeVector& p, const G4ThreeVector& v, G4double r,
...
G4double G4Torus::SolveNumericJT(const G4ThreeVector& p, const G4ThreeVector& v, G4double r,
                                 G4bool IsDistanceToIn) const
{
  ...
  for (std::size_t k = 0; k < roots.size(); ++k)
  {
    ...
    if ((fSPhi <= -pi) && (theta > halfAngTolerance))
    {
      theta = theta - twopi;
    }
    ...
    //
    if ((theta - fSPhi >= -halfAngTolerance) && (theta - (fSPhi + fDPhi) <= halfAngTolerance))
    {
  ...
  if (pt2 >= tolRMin * tolRMin && pt2 <= tolRMax * tolRMax)
  {
    ...
    else
    {
      ...
      if (fSPhi >= 0)
      {
        ...
        else if ((pPhi >= fSPhi - halfAngTolerance) && (pPhi <= fSPhi + fDPhi + halfAngTolerance))
        {
          in = kSurface;
        }
      }
...
#  endif
  ...
  if (fDPhi < twopi)  // Phi Intersections
  {
    ...
    else if (vphi > ePhi + halfAngTolerance)
    {
      vphi -= twopi;
    }
