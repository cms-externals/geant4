...
G4CutTubs::G4CutTubs(const G4String& pName, G4double pRMin, G4double pRMax, G4double pDz,
                     G4double pSPhi, G4double pDPhi, G4ThreeVector pLowNorm,
    ...
    fZMax(0.)
{
  ...
  {
    std::ostringstream message;
    message << "Invalid values for radii in solid: " << GetName() << G4endl
  ...
  //
  if ((pLowNorm.x() == 0.0) && (pLowNorm.y() == 0.0) && (pHighNorm.x() == 0.0)
      && (pHighNorm.y() == 0.0))
  {
    std::ostringstream message;
    message << "Inexisting Low/High Normal to Z plane or Parallel to Z." << G4endl
            << "Normals to Z plane are " << pLowNorm << " and " << pHighNorm
            << " in solid: " << GetName() << " \n";
    G4Exception("G4CutTubs::G4CutTubs()", "GeomSolids1001", JustWarning, message,
                "Should use G4Tubs!");
  }
  ...
  if (pLowNorm.mag2() != 1.)
  {
    pLowNorm = pLowNorm.unit();
  }
  if (pHighNorm.mag2() != 1.)
  {
    pHighNorm = pHighNorm.unit();
  }
  ...
  if ((pLowNorm.mag2() != 0.) && (pHighNorm.mag2() != 0.))
  {
    if ((pLowNorm.z() >= 0.) || (pHighNorm.z() <= 0.))
    {
      ...
      message << "Invalid Low or High Normal to Z plane; "
                 "has to point outside Solid."
              << G4endl << "Invalid Norm to Z plane (" << pLowNorm << " or  " << pHighNorm
              << ") in solid: " << GetName();
      G4Exception("G4CutTubs::G4CutTubs()", "GeomSolids0002", FatalException, message);
    }
  }
...
//
G4CutTubs::G4CutTubs(__void__& a) : G4CSGSolid(a) {}
...
G4CutTubs& G4CutTubs::operator=(const G4CutTubs& rhs)
{
  ...
  fLowNorm = rhs.fLowNorm;
  fHighNorm = rhs.fHighNorm;
...
void G4CutTubs::BoundingLimits(G4ThreeVector& pMin, G4ThreeVector& pMax) const
{
  ...
  if (pMin.x() >= pMax.x() || pMin.y() >= pMax.y() || pMin.z() >= pMax.z())
  {
    ...
    message << "Bad bounding box (min >= max) for solid: " << GetName() << " !"
            << "\npMin = " << pMin << "\npMax = " << pMax;
    G4Exception("G4CutTubs::BoundingLimits()", "GeomMgt0001", JustWarning, message);
    DumpInfo();
  }
}
...
G4bool G4CutTubs::CalculateExtent(const EAxis pAxis, const G4VoxelLimits& pVoxelLimit,
...
EInside G4CutTubs::Inside(const G4ThreeVector& p) const
{
  ...
  if (!fPhiFullCutTube)
  {
    ...
    const G4double cosPsi = (p.x() * cosCPhi + p.y() * sinCPhi) / std::sqrt(r2);
...
G4ThreeVector G4CutTubs::SurfaceNormal(const G4ThreeVector& p) const
{
  ...
  {
#  ifdef G4CSGDEBUG
    G4Exception("G4CutTubs::SurfaceNormal(p)", "GeomSolids1002", JustWarning,
                "Point p is not on surface !?");
    G4int oldprc = G4cout.precision(20);
    G4cout << "G4CutTubs::SN ( " << p.x() << ", " << p.y() << ", " << p.z() << " ); " << G4endl
           << G4endl;
...
G4ThreeVector G4CutTubs::ApproxSurfaceNormal(const G4ThreeVector& p) const
{
  ...
  {
    distSPhi = p.x() * cosSPhi + p.y() * sinSPhi >= 0.
                 ? std::fabs(p.x() * sinSPhi - p.y() * cosSPhi)
                 : rho;
    distEPhi = p.x() * cosEPhi + p.y() * sinEPhi >= 0.
                 ? std::fabs(p.x() * sinEPhi - p.y() * cosEPhi)
                 : rho;
  ...
  switch (side)
  {
    ...
    {
      DumpInfo();
      G4Exception("G4CutTubs::ApproxSurfaceNormal()", "GeomSolids1002", JustWarning,
    ...
    if ((t3 >= tolORMax2) && (t2 < 0))  // This also handles the tangent case
    {
      ...
      if (d >= 0)  // If real root
      {
        ...
        if (sd >= 0)  // If 'forwards'
        {
          ...
          xi = p.x() + sd * v.x();
          yi = p.y() + sd * v.y();
          if ((-xi * fLowNorm.x() - yi * fLowNorm.y() - (zi + fDz) * fLowNorm.z())
              > -halfCarTolerance)
          {
            if ((-xi * fHighNorm.x() - yi * fHighNorm.y() + (fDz - zi) * fHighNorm.z())
                > -halfCarTolerance)
            {
...
G4double G4CutTubs::DistanceToOut(const G4ThreeVector& p, const G4ThreeVector& v,
                                  const G4bool calcNorm, G4bool* validNorm, G4ThreeVector* n) const
{
  ...
  //
  G4double pDistS, compS, pDistE, compE, sphi2, xi, yi, roi2;
  ...
  {
    roi2 = snxt * snxt * t1 + 2 * snxt * t2 + t3;
  }  // radius^2 on +-fDz
  ...
  if (t1 > 0)  // Check not parallel
  {
    ...
    if (!fPhiFullCutTube)
    {
      const G4double vrho = std::sqrt(v.x() * v.x() + v.y() * v.y());
      const G4bool vInPhi = vrho == 0.
                              || (v.x() * cosCPhi + v.y() * sinCPhi) / vrho >= cosHDPhiOT;
  ...
  if (calcNorm)
  {
    switch (side)
    {
      ...
      default:
        ...
        G4Exception("G4CutTubs::DistanceToOut(p,v,..)", "GeomSolids1002", JustWarning, message);
        break;
    }
  }
  if (snxt < halfCarTolerance)
...
G4ThreeVector G4CutTubs::GetPointOnSurface() const
{
  ...
  for (G4int i = 0; i < ntry; ++i)
  {
    ...
    if ((ntop.dot(p) - fDz * ntop.z()) > 0.)
    {
      continue;
    }
    if ((nbot.dot(p) + fDz * nbot.z()) > 0.)
    {
      continue;
    }
    return p;
...
G4bool G4CutTubs::IsCrossingCutPlanes() const
{
  ...
  G4ThreeVector nbot = GetLowNorm();
  G4ThreeVector ntop = GetHighNorm();
  if (std::abs(nbot.z()) < kCarTolerance)
  {
    return true;
  }
