...
G4UOrb::G4UOrb(const G4String& pName, G4double pRmax) : Base_t(pName, pRmax) {}

//////////////////////////////////////////////////////////////////////////
//
// Copy constructor

G4UOrb::G4UOrb(const G4UOrb& rhs) : Base_t(rhs) {}
...
void G4UOrb::ComputeDimensions(G4VPVParameterisation* p, const G4int n,
...
void G4UOrb::BoundingLimits(G4ThreeVector& pMin, G4ThreeVector& pMax) const
{
  ...
  if (pMin.x() >= pMax.x() || pMin.y() >= pMax.y() || pMin.z() >= pMax.z())
  {
    ...
    message << "Bad bounding box (min >= max) for solid: " << GetName() << " !"
            << "\npMin = " << pMin << "\npMax = " << pMax;
    G4Exception("G4UOrb::BoundingLimits()", "GeomMgt0001", JustWarning, message);
    StreamInfo(G4cout);
  }
}
...
G4bool G4UOrb::CalculateExtent(const EAxis pAxis, const G4VoxelLimits& pVoxelLimit,
