...
G4UCutTubs::G4UCutTubs(const G4String& pName, G4double pRMin, G4double pRMax, G4double pDz,
                       G4double pSPhi, G4double pDPhi, const G4ThreeVector& pLowNorm,
                       const G4ThreeVector& pHighNorm)
  : Base_t(pName, pRMin, pRMax, pDz, pSPhi, pDPhi, pLowNorm.x(), pLowNorm.y(), pLowNorm.z(),
...
G4UCutTubs::G4UCutTubs(const G4UCutTubs& rhs) : Base_t(rhs) {}
...
void G4UCutTubs::BoundingLimits(G4ThreeVector& pMin, G4ThreeVector& pMax) const
{
  ...
  if (pMin.x() >= pMax.x() || pMin.y() >= pMax.y() || pMin.z() >= pMax.z())
  {
    ...
    message << "Bad bounding box (min >= max) for solid: " << GetName() << " !"
            << "\npMin = " << pMin << "\npMax = " << pMax;
    G4Exception("G4CUutTubs::BoundingLimits()", "GeomMgt0001", JustWarning, message);
    StreamInfo(G4cout);
  }
  ...
  if (checkBBox)
  {
        ...
        || std::abs(pMax.z() - vmax.z()) > kCarTolerance)
    {
      ...
      G4Exception("G4UCutTubs::BoundingLimits()", "GeomMgt0001", JustWarning, message);
      checkBBox = false;
    }
  }
}
...
G4bool G4UCutTubs::CalculateExtent(const EAxis pAxis, const G4VoxelLimits& pVoxelLimit,
...
G4double G4UCutTubs::GetCutZ(const G4ThreeVector& p) const
{
  ...
  if (p.z() < 0)
  {
    if (fLowNorm.z() != 0.)
    {
      newz = -GetZHalfLength() - (p.x() * fLowNorm.x() + p.y() * fLowNorm.y()) / fLowNorm.z();
    }
  }
  else
  {
    if (fHighNorm.z() != 0.)
    {
      newz = GetZHalfLength() - (p.x() * fHighNorm.x() + p.y() * fHighNorm.y()) / fHighNorm.z();
    }
  }
  return newz;
}
...
G4Polyhedron* G4UCutTubs::CreatePolyhedron() const
{
  ...
  G4Polyhedron* ph1 = new G4PolyhedronTubs(GetInnerRadius(), GetOuterRadius(), GetZHalfLength(),
