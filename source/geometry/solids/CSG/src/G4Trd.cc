...
G4Trd::G4Trd(const G4String& pName, G4double pdx1, G4double pdx2, G4double pdy1, G4double pdy2,
             G4double pdz)
  : G4CSGSolid(pName),
...
void G4Trd::SetAllParameters(G4double pdx1, G4double pdx2, G4double pdy1, G4double pdy2,
                             G4double pdz)
{
...
void G4Trd::CheckParameters()
{
  ...
  if ((fDx1 < 0 || fDx2 < 0 || fDy1 < 0 || fDy2 < 0 || fDz < dmin) || (fDx1 < dmin && fDx2 < dmin)
      || (fDy1 < dmin && fDy2 < dmin))
  {
    std::ostringstream message;
    message << "Invalid (too small or negative) dimensions for Solid: " << GetName() << "\n  X - "
            << fDx1 << ", " << fDx2 << "\n  Y - " << fDy1 << ", " << fDy2 << "\n  Z - " << fDz;
    G4Exception("G4Trd::CheckParameters()", "GeomSolids0002", FatalException, message);
  }
}
...
void G4Trd::ComputeDimensions(G4VPVParameterisation* p, const G4int n,
...
G4bool G4Trd::CalculateExtent(const EAxis pAxis, const G4VoxelLimits& pVoxelLimit,
...
EInside G4Trd::Inside(const G4ThreeVector& p) const
{
  ...
  return (dist > halfCarTolerance) ? kOutside : ((dist > -halfCarTolerance) ? kSurface : kInside);
}
...
#  ifdef G4CSGDEBUG
  ...
  G4Exception("G4Trd::SurfaceNormal(p)", "GeomSolids1002", JustWarning, message);
  DumpInfo();
#  endif
...
G4double G4Trd::DistanceToIn(const G4ThreeVector& p, const G4ThreeVector& v) const
{
  ...
  {
    return kInfinity;  // touch or no hit
  }
...
G4double G4Trd::DistanceToOut(const G4ThreeVector& p, const G4ThreeVector& v, const G4bool calcNorm,
...
#  ifdef G4CSGDEBUG
  if (Inside(p) == kOutside)
  {
    ...
    message << "   p.z() = " << p.z() / mm << " mm";
    G4cout.precision(oldprc);
    G4Exception("G4Trd::DistanceToOut(p)", "GeomSolids1002", JustWarning, message);
    DumpInfo();
  }
#  endif
  G4double dx = fPlanes[3].a * std::abs(p.x()) + fPlanes[3].c * p.z() + fPlanes[3].d;
...
G4ThreeVector G4Trd::GetPointOnSurface() const
{
  ...
  else if (select < sbase + 2. * sxz)
  {
    ...
    G4ThreeVector p2 = (select < sbase + fDx1 * fHx) ? G4ThreeVector(fDx1, -fDy1, -fDz)
                                                     : G4ThreeVector(-fDx2, -fDy2, fDz);
  ...
  else
  {
    ...
    G4ThreeVector p0(-fDx1, fDy1, -fDz);
    G4ThreeVector p1(-fDx2, -fDy2, fDz);
    G4ThreeVector p2 = (select < sbase + 2. * sxz + fDy1 * fHy) ? G4ThreeVector(-fDx1, -fDy1, -fDz)
                                                                : G4ThreeVector(-fDx2, fDy2, fDz);
    p = p0 * (1. - u - v) + p1 * u + p2 * v;
    p.setX(xsign * p.x());
  }
