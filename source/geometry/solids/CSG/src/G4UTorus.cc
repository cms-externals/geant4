...
{
  SetAllParameters(arg, GetRmax(), GetRtor(), GetSPhi(), GetDPhi());
}

void G4UTorus::SetRmax(G4double arg)
{
  SetAllParameters(GetRmin(), arg, GetRtor(), GetSPhi(), GetDPhi());
}

void G4UTorus::SetRtor(G4double arg)
{
  SetAllParameters(GetRmin(), GetRmax(), arg, GetSPhi(), GetDPhi());
}

void G4UTorus::SetSPhi(G4double arg)
{
  SetAllParameters(GetRmin(), GetRmax(), GetRtor(), arg, GetDPhi());
}

void G4UTorus::SetDPhi(G4double arg)
{
  SetAllParameters(GetRmin(), GetRmax(), GetRtor(), GetSPhi(), arg);
}

void G4UTorus::SetAllParameters(G4double arg1, G4double arg2, G4double arg3, G4double arg4,
                                G4double arg5)
{
  // VecGeom's individual setters do not rebuild all derived Torus state
  // (notably the phi wedge, bounding tube and bounding box).  Constructing
  // and assigning a complete solid keeps parameterised instances coherent.
  G4UTorus updated(GetName(), arg1, arg2, arg3, arg4, arg5);
  *this = updated;
}
