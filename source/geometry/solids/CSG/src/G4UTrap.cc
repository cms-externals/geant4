...
// Constructors
//
G4UTrap::G4UTrap(const G4String& pName, G4double pdz, G4double pTheta, G4double pPhi, G4double pdy1,
                 G4double pdx1, G4double pdx2, G4double pAlp1, G4double pdy2, G4double pdx3,
                 G4double pdx4, G4double pAlp2)
  : Base_t(pName, pdz, pTheta, pPhi, pdy1, pdx1, pdx2, pAlp1, pdy2, pdx3, pdx4, pAlp2)
{
  G4ThreeVector pt[8];
  CheckParameters();
  GetVertices(pt);
...
G4UTrap::G4UTrap(const G4String& pName, const G4ThreeVector pt[8]) : Base_t(pName)
{
  ...
  if (pt[0].z() >= 0 || pt[0].z() != pt[1].z() || pt[0].z() != pt[2].z() || pt[0].z() != pt[3].z()
      ...
      || std::abs(pt[0].x() + pt[1].x() + pt[4].x() + pt[5].x() + pt[2].x() + pt[3].x() + pt[6].x()
                  + pt[7].x())
           >= kCarTolerance)
  {
    std::ostringstream message;
    message << "Invalid vertice coordinates for Solid: " << GetName();
    G4Exception("G4UTrap::G4UTrap()", "GeomSolids0002", FatalException, message);
  }
...
// a Trd1.
G4UTrap::G4UTrap(const G4String& pName, G4double pZ, G4double pY, G4double pX, G4double pLTX)
...
G4UTrap::G4UTrap(const G4String& pName, G4double pdx1, G4double pdx2, G4double pdy1, G4double pdy2,
                 G4double pdz)
...
G4UTrap::G4UTrap(const G4String& pName, G4double pdx, G4double pdy, G4double pdz, G4double pAlpha,
...
G4UTrap::G4UTrap(const G4String& pName) : Base_t(pName) {}

//////////////////////////////////////////////////////////////////////////
//
// Copy constructor
//
G4UTrap::G4UTrap(const G4UTrap& rhs) : Base_t(rhs) {}
...
// Modifier
//
void G4UTrap::SetAllParameters(G4double pDz, G4double pTheta, G4double pPhi, G4double pDy1,
...
void G4UTrap::CheckParameters() const
{
  ...
  if (fDz <= 0 || fDy1 <= 0 || fDx1 <= 0 || fDx2 <= 0 || fDy2 <= 0 || fDx3 <= 0 || fDx4 <= 0)
  {
    ...
    message << "Invalid Length Parameters for Solid: " << GetName() << "\n  X - " << fDx1 << ", "
            << fDx2 << ", " << fDx3 << ", " << fDx4 << "\n  Y - " << fDy1 << ", " << fDy2
            << "\n  Z - " << fDz;
    G4Exception("G4UTrap::CheckParameters()", "GeomSolids0002", FatalException, message);
  }
}
...
void G4UTrap::CheckPlanarity(const G4ThreeVector pt[8]) const
{
  ...
  for (G4int i = 0; i < 4; ++i)
  {
    ...
    if (std::abs(dmax) > 1000 * kCarTolerance)
    {
      ...
      StreamInfo(message);
      G4Exception("G4UTrap::CheckPlanarity()", "GeomSolids0002", FatalException, message);
    }
  }
}
...
//
void G4UTrap::ComputeDimensions(G4VPVParameterisation* p, const G4int n,
...
void G4UTrap::BoundingLimits(G4ThreeVector& pMin, G4ThreeVector& pMax) const
{
  ...
  if (pMin.x() >= pMax.x() || pMin.y() >= pMax.y() || pMin.z() >= pMax.z())
  {
    ...
    message << "Bad bounding box (min >= max) for solid: " << GetName() << " !"
            << "\npMin = " << pMin << "\npMax = " << pMax;
    G4Exception("G4UTrap::BoundingLimits()", "GeomMgt0001", JustWarning, message);
    StreamInfo(G4cout);
  }
  ...
  if (checkBBox)
  {
    ...
    U3Vector vmin, vmax;
    Extent(vmin, vmax);
    if (std::abs(pMin.x() - vmin.x()) > tolerance || std::abs(pMin.y() - vmin.y()) > tolerance
        || std::abs(pMin.z() - vmin.z()) > tolerance || std::abs(pMax.x() - vmax.x()) > tolerance
        || std::abs(pMax.y() - vmax.y()) > tolerance || std::abs(pMax.z() - vmax.z()) > tolerance)
    {
      std::ostringstream message;
...
G4bool G4UTrap::CalculateExtent(const EAxis pAxis, const G4VoxelLimits& pVoxelLimit,
...
{
  return new G4PolyhedronTrap(GetZHalfLength(), GetTheta(), GetPhi(), GetYHalfLength1(),
                              GetXHalfLength1(), GetXHalfLength2(), GetAlpha1(), GetYHalfLength2(),
