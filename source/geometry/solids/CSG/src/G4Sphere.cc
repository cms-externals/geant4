...
G4Sphere& G4Sphere::operator=(const G4Sphere& rhs)
{
  ...
  cosSThetaIT = rhs.cosSThetaIT;
  cosSThetaOT = rhs.cosSThetaOT;
  cosEThetaIT = rhs.cosEThetaIT;
  cosEThetaOT = rhs.cosEThetaOT;
  eTheta = rhs.eTheta;
...
{
  G4double rho2, rad2, tolRMin, tolRMax;
  EInside in = kOutside;
  ...
  {
    const G4double cosPsi = (p.x() * cosCPhi + p.y() * sinCPhi) / std::sqrt(rho2);
  ...
  {
    const G4double cosTheta = p.z() / std::sqrt(rad2);

    if (in == kInside)
    {
      if (((fSTheta > 0.0) && (cosTheta > cosSThetaIT))
          || ((eTheta < pi) && (cosTheta < cosEThetaIT)))
      {
        if ((((fSTheta > 0.0) && (cosTheta <= cosSThetaOT)) || (fSTheta == 0.0))
            && ((eTheta == pi) || (cosTheta >= cosEThetaOT)))
        {
          in = kSurface;
        }
        else
    ...
    {
      if (((fSTheta > 0.0) && (cosTheta > cosSThetaOT))
          || ((eTheta < pi) && (cosTheta < cosEThetaOT)))
      {
        in = kOutside;
      }
...
G4ThreeVector G4Sphere::SurfaceNormal(const G4ThreeVector& p) const
{
  ...
  if (!fFullThetaSphere)
  {
    if (rho != 0.0)
    {
      distSTheta = std::fabs(rho * cosSTheta - p.z() * sinSTheta) / radius;
      distETheta = std::fabs(rho * cosETheta - p.z() * sinETheta) / radius;
...
G4ThreeVector G4Sphere::ApproxSurfaceNormal(const G4ThreeVector& p) const
{
  ...
  G4ThreeVector norm;
  G4double rho, rho2, radius;
  G4double distRMin, distRMax, distSPhi = kInfinity, distEPhi = kInfinity, distSTheta, distETheta,
                               distMin;
  ...
  {
    distSTheta = std::fabs(rho * cosSTheta - p.z() * sinSTheta);
    distETheta = std::fabs(rho * cosETheta - p.z() * sinETheta);
  ...
  switch (side)
  {
    ...
    case kNEPhi:
      ...
      break;
    case kNSTheta:
      norm = G4ThreeVector(-cosSTheta * (rho != 0. ? p.x() / rho : 1.),
                           -cosSTheta * (rho != 0. ? p.y() / rho : 0.), sinSTheta);
      break;
    case kNETheta:
      norm = G4ThreeVector(cosETheta * (rho != 0. ? p.x() / rho : 1.),
                           cosETheta * (rho != 0. ? p.y() / rho : 0.), -sinETheta);
      break;
...
G4double G4Sphere::DistanceToIn(const G4ThreeVector& p, const G4ThreeVector& v) const
{
  ...
  G4double rho2, rad2, pDotV2d, pDotV3d, pTheta = 0.;
  ...
  if (!fFullThetaSphere)
  {
    if (rad2 != 0.) pTheta = std::acos(std::clamp(p.z() / std::sqrt(rad2), -1., 1.));
    ...
    {
      const G4double vTheta = std::acos(std::clamp(v.z(), -1., 1.));
        ...
        if (!fFullPhiSphere && (rhoi != 0.0))  // Check phi intersection
        {
          ...
          if (cosPsi >= cosHDPhiOT)
          {
            if (!fFullThetaSphere)  // Check theta intersection
            {
              ...
              //
              iTheta = std::acos(std::clamp(zi / std::sqrt(rhoi * rhoi + zi * zi), -1., 1.));
      ...
      if (Dist < halfCarTolerance)
      {
        ...
        if (sd < snxt)
        {
          ...
          if ((radi2 <= tolORMax2) && (radi2 >= tolORMin2) && ((yi * cosCPhi - xi * sinCPhi) <= 0))
          {
            ...
            if (!fFullThetaSphere)
            {
              iTheta = std::acos(std::clamp(zi / std::sqrt(rhoi2 + zi * zi), -1., 1.));
              if ((iTheta >= tolSTheta) && (iTheta <= tolETheta))
              {
...
G4double G4Sphere::DistanceToIn(const G4ThreeVector& p) const
{
  ...
  const G4double rds = std::hypot(rho, p.z());

  //
  // Distance to r shells
  //
  G4double safe = (fRmin != 0.0) ? std::max(fRmin - rds, rds - fRmax) : rds - fRmax;
  ...
  if (!fFullPhiSphere && (rho != 0.0))
  {
    ...
    if (cosPsi < cosHDPhi)
    {
      ...
      {
        safe = std::max(safe, std::fabs(p.x() * sinSPhi - p.y() * cosSPhi));
      }
      else
      {
        safe = std::max(safe, std::fabs(p.x() * sinEPhi - p.y() * cosEPhi));
      }
    }
  }
  //
  ...
  {
    const G4double safeStartTheta = p.z() * sinSTheta - rho * cosSTheta;
    const G4double safeEndTheta = rho * cosETheta - p.z() * sinETheta;
    safe = std::max({safe, safeStartTheta, safeEndTheta});
  }
                                 ...
                                 const G4bool calcNorm, G4bool* validNorm, G4ThreeVector* n) const
{
  ...
  G4double pDistS, compS, pDistE, compE, sphi2;
                ...
                if (calcNorm)
                {
                  *validNorm = true;
                  if (rho2 != 0.0)
                  {
                    ...
                    *n = G4ThreeVector(p.x() / rhoSecTheta, p.y() / rhoSecTheta, sinSTheta);
                  }
                  else
                  {
                    *n = G4ThreeVector(0., 0., 1.);
  ...
  if (!fFullPhiSphere)
  {
    ...
    if (sphi < snxt)  // Order intersections
    {
      snxt = sphi;
      side = sidephi;
    }
...
#  endif
  ...
  // Distance to r shells
  //
  const G4double safeRMax = fRmax - rds;
  G4double safe = (fRmin != 0.0) ? std::min(rds - fRmin, safeRMax) : safeRMax;
  ...
  if (!fFullThetaSphere)
  {
    if (rds > 0.0)
    {
      G4double safeStartTheta = kInfinity;
      G4double safeEndTheta = kInfinity;
      if (fSTheta > 0.)
      {
        safeStartTheta = rho * cosSTheta - p.z() * sinSTheta;
      }
      if (eTheta < pi)
      {
        safeEndTheta = p.z() * sinETheta - rho * cosETheta;
      }
      safe = std::min({safe, safeStartTheta, safeEndTheta});
    }
    else
    {
      safe = 0.0;
