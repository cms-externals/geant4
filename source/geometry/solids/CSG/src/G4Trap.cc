...
G4Trap::G4Trap(const G4String& pName, G4double pDz, G4double pTheta, G4double pPhi, G4double pDy1,
...
G4Trap::G4Trap(const G4String& pName, const G4ThreeVector pt[8])
  : G4CSGSolid(pName), halfCarTolerance(0.5 * kCarTolerance)
{
  ...
  if (pt[0].z() >= 0 || pt[0].z() != pt[1].z() || pt[0].z() != pt[2].z() || pt[0].z() != pt[3].z()
      ...
      || pt[4].z() <= 0 || pt[4].z() != pt[5].z() || pt[4].z() != pt[6].z()
      || pt[4].z() != pt[7].z()

      || std::fabs(pt[0].z() + pt[4].z()) >= kCarTolerance

      || pt[0].y() != pt[1].y() || pt[2].y() != pt[3].y() || pt[4].y() != pt[5].y()
      || pt[6].y() != pt[7].y()

      || std::fabs(pt[0].y() + pt[2].y() + pt[4].y() + pt[6].y()) >= kCarTolerance
      || std::fabs(pt[0].x() + pt[1].x() + pt[4].x() + pt[5].x() + pt[2].x() + pt[3].x() + pt[6].x()
                   + pt[7].x())
           >= kCarTolerance)
  {
...
G4Trap::G4Trap(const G4String& pName, G4double pZ, G4double pY, G4double pX, G4double pLTX)
  : G4CSGSolid(pName), halfCarTolerance(0.5 * kCarTolerance)
{
  fDz = 0.5 * pZ;
  fTthetaCphi = 0;
...
G4Trap::G4Trap(const G4String& pName, G4double pDx1, G4double pDx2, G4double pDy1, G4double pDy2,
               G4double pDz)
...
G4Trap::G4Trap(const G4String& pName, G4double pDx, G4double pDy, G4double pDz, G4double pAlpha,
...
void G4Trap::SetAllParameters(G4double pDz, G4double pTheta, G4double pPhi, G4double pDy1,
...
{
  if (fDz <= 0 || fDy1 <= 0 || fDx1 <= 0 || fDx2 <= 0 || fDy2 <= 0 || fDx3 <= 0 || fDx4 <= 0)
  {
    std::ostringstream message;
    message << "Invalid Length Parameters for Solid: " << GetName() << "\n  X - " << fDx1 << ", "
            << fDx2 << ", " << fDx3 << ", " << fDx4 << "\n  Y - " << fDy1 << ", " << fDy2
            << "\n  Z - " << fDz;
    G4Exception("G4Trap::CheckParameters()", "GeomSolids0002", FatalException, message);
  }
}
...
void G4Trap::MakePlanes(const G4ThreeVector pt[8])
{
  ...
  for (G4int i = 0; i < 4; ++i)
  {
    if (MakePlane(pt[iface[i][0]], pt[iface[i][1]], pt[iface[i][2]], pt[iface[i][3]], fPlanes[i]))
    {
      continue;
    }
    ...
    for (G4int k = 0; k < 4; ++k)
    {
      ...
      if (std::abs(dist) > std::abs(dmax))
      {
        dmax = dist;
      }
    }
    std::ostringstream message;
    message << "Side face " << side[i] << " is not planar for solid: " << GetName()
...
G4bool G4Trap::MakePlane(const G4ThreeVector& p1, const G4ThreeVector& p2, const G4ThreeVector& p3,
                         const G4ThreeVector& p4, TrapSidePlane& plane)
{
  G4ThreeVector normal = ((p4 - p2).cross(p3 - p1)).unit();
  if (std::abs(normal.x()) < DBL_EPSILON)
  {
    normal.setX(0);
  }
  if (std::abs(normal.y()) < DBL_EPSILON)
  {
    normal.setY(0);
  }
  if (std::abs(normal.z()) < DBL_EPSILON)
  {
    normal.setZ(0);
  }
  normal = normal.unit();
...
{
  // Set indeces
  constexpr G4int iface[6][4] = {{0, 1, 3, 2}, {0, 4, 5, 1}, {2, 3, 7, 6},
  ...
  for (G4int i = 0; i < 6; ++i)
  {
    fAreas[i] = G4GeomTools::QuadAreaNormal(pt[iface[i][0]], pt[iface[i][1]], pt[iface[i][2]],
                                            pt[iface[i][3]])
                  .mag();
  }
  for (G4int i = 1; i < 6; ++i)
  ...
  fTrapType = 0;
  if (fPlanes[0].b == -1 && fPlanes[1].b == 1 && std::abs(fPlanes[0].a) < DBL_EPSILON
      && std::abs(fPlanes[0].c) < DBL_EPSILON && std::abs(fPlanes[1].a) < DBL_EPSILON
      && std::abs(fPlanes[1].c) < DBL_EPSILON)
  {
    fTrapType = 1;  // YZ section is a rectangle ...
    if (std::abs(fPlanes[2].a + fPlanes[3].a) < DBL_EPSILON
        && std::abs(fPlanes[2].c - fPlanes[3].c) < DBL_EPSILON && fPlanes[2].b == 0
        && fPlanes[3].b == 0)
    {
    ...
    if (std::abs(fPlanes[2].a + fPlanes[3].a) < DBL_EPSILON
        && std::abs(fPlanes[2].b - fPlanes[3].b) < DBL_EPSILON && fPlanes[2].c == 0
        && fPlanes[3].c == 0)
    {
...
void G4Trap::ComputeDimensions(G4VPVParameterisation* p, const G4int n,
...
void G4Trap::BoundingLimits(G4ThreeVector& pMin, G4ThreeVector& pMax) const
{
  ...
  if (pMin.x() >= pMax.x() || pMin.y() >= pMax.y() || pMin.z() >= pMax.z())
  {
    ...
    message << "Bad bounding box (min >= max) for solid: " << GetName() << " !"
            << "\npMin = " << pMin << "\npMax = " << pMax;
    G4Exception("G4Trap::BoundingLimits()", "GeomMgt0001", JustWarning, message);
    DumpInfo();
  }
}
...
G4bool G4Trap::CalculateExtent(const EAxis pAxis, const G4VoxelLimits& pVoxelLimit,
...
EInside G4Trap::Inside(const G4ThreeVector& p) const
{
  switch (fTrapType)
  {
    case 0:  // General case
    {
      ...
      G4double dx1 =
        fPlanes[2].a * p.x() + fPlanes[2].b * p.y() + fPlanes[2].c * p.z() + fPlanes[2].d;
      G4double dx2 =
        fPlanes[3].a * p.x() + fPlanes[3].b * p.y() + fPlanes[3].c * p.z() + fPlanes[3].d;
    ...
    {  // XY section is an isosceles trapezoid
      G4double dz = std::abs(p.z()) - fDz;
      G4double dy = std::max(dz, std::abs(p.y()) + fPlanes[1].d);
      G4double dx = fPlanes[3].a * std::abs(p.x()) + fPlanes[3].b * p.y() + fPlanes[3].d;
      G4double dist = std::max(dy, dx);
  ...
  switch (fTrapType)
  {
    case 0:  // General case
    {
      for (G4int i = 0; i < 2; ++i)
      {
        ...
        if (std::abs(dy) > halfCarTolerance)
        {
          continue;
        }
        ny = fPlanes[i].b;
      ...
      {
        G4double dx =
          fPlanes[i].a * p.x() + fPlanes[i].b * p.y() + fPlanes[i].c * p.z() + fPlanes[i].d;
...
#  ifdef G4CSGDEBUG
  ...
  G4Exception("G4Trap::SurfaceNormal(p)", "GeomSolids1002", JustWarning, message);
  DumpInfo();
#  endif
...
G4ThreeVector G4Trap::ApproxSurfaceNormal(const G4ThreeVector& p) const
{
  ...
  for (G4int i = 0; i < 4; ++i)
  {
    G4double d = fPlanes[i].a * p.x() + fPlanes[i].b * p.y() + fPlanes[i].c * p.z() + fPlanes[i].d;
    if (d > dist)
    {
      dist = d;
      iside = i;
...
void G4Trap::GetVertices(G4ThreeVector pt[8]) const
{
  for (G4int i = 0; i < 8; ++i)
  {
    ...
    G4double x = -(fPlanes[ix].b * y + fPlanes[ix].c * z + fPlanes[ix].d) / fPlanes[ix].a;
    pt[i].set(x, y, z);
  }
}
...
G4double G4Trap::GetCubicVolume()
{
  if (fCubicVolume == 0)
  {
    ...
    fCubicVolume =
      ((dx1 + dx2 + dx3 + dx4) * (dy1 + dy2) + (dx4 + dx3 - dx2 - dx1) * (dy2 - dy1) / 3) * dz
      * 0.125;
    l.unlock();
  }
  return fCubicVolume;
...
G4double G4Trap::GetSurfaceArea()
{
  if (fSurfaceArea == 0)
  {
    G4AutoLock l(&trapMutex);
    G4ThreeVector pt[8];
    G4int iface[6][4] = {{0, 1, 3, 2}, {0, 4, 5, 1}, {2, 3, 7, 6},
                         {0, 2, 6, 4}, {1, 5, 7, 3}, {4, 6, 7, 5}};

    GetVertices(pt);
    for (const auto& i : iface)
    {
      fSurfaceArea += G4GeomTools::QuadAreaNormal(pt[i[0]], pt[i[1]], pt[i[2]], pt[i[3]]).mag();
    }
    l.unlock();
  }
  return fSurfaceArea;
...
G4Polyhedron* G4Trap::CreatePolyhedron() const
{
  ...
  return new G4PolyhedronTrap(fDz, theta, phi, fDy1, fDx1, fDx2, alpha1, fDy2, fDx3, fDx4, alpha2);
}
