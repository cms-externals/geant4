...
class G4Torus : public G4CSGSolid
{
  ...
  private:
     ...
     */
    G4int TorusRootsJT(const G4ThreeVector& p, const G4ThreeVector& v, G4double r,
                       std::array<G4double, 4>& roots) const;

    /** Fast analytical quartic solution with validation and JT fallback. */
    G4int TorusRoots(const G4ThreeVector& p, const G4ThreeVector& v, G4double r,
                     std::array<G4double, 4>& roots) const;

    /**
     * Interface method for DistanceToIn() and DistanceToOut().
     * Calls TorusRoots() using the validated analytical fast path.
     *  @returns The smalles possible distance to the surface.
     */
    G4double SolveNumeric(const G4ThreeVector& p, const G4ThreeVector& v, G4double r,
  ...
  private:
    ...
    /** Cached trigonometry for the phi segment. */
    G4double sinSPhi, cosSPhi, sinEPhi, cosEPhi;
    G4double sinCPhi, cosCPhi, cosHDPhi, cosHDPhiIT, cosHDPhiOT;
};
