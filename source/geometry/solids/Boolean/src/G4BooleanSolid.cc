...
{
  // Get list of primitives and total primitive surface area once. The
  // primitive list is immutable after publication, so random draws need
  // no locking.
  //
  G4bool initialized = false;

  G4AutoLock lk1(&boolMutex);
  initialized = !fPrimitives.empty();
  lk1.unlock();

  if (!initialized)
  {
    std::vector<std::pair<G4VSolid*, G4Transform3D>> newPrimitives;
    G4double newSurfaceArea = 0.;

    GetListOfPrimitives(newPrimitives, G4Transform3D());
    for (const auto& primitive : newPrimitives)
    {
      newSurfaceArea += primitive.first->GetSurfaceArea();
    }

    G4AutoLock lk2(&boolMutex);
    if (fPrimitives.empty() && !newPrimitives.empty() && newSurfaceArea > 0.)
    {
      fPrimitives = newPrimitives;
      fPrimitivesSurfaceArea = newSurfaceArea;
    }
    lk2.unlock();
  }

  if (fPrimitives.empty() || fPrimitivesSurfaceArea <= 0.)
  {
    std::ostringstream message;
    message << "Solid - " << GetName() << "\n"
            << "No primitive surface candidates available for point generation!\n"
  ...
  for (std::size_t k = 0; k < 100000; ++k)  // try 100k times
  {
    ...
    G4double area = 0.;
    auto prim = fPrimitives.front();
    for (const auto& candidate : fPrimitives)
    {
      prim = candidate;
