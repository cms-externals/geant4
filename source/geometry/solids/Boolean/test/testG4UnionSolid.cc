...
// NOTE: xRot = rotation such that x axis->-x axis & y axis->-y axis
    ...
    G4Tubs t2("Hole Tube #2",45,50,50,0,360*deg);
    G4Tubs t3("disk",0,50,5,0,360*deg);
    G4Tubs t4("t4",0,45,50,0,360*deg);
  ...
  // G4cout << " t2 : " << "  start-phi= " << t2.GetStartPhiAngle()
  //                    << "  delta-phi= " << t2.GetDeltaPhiAngle() / CLHEP::degree << " degrees " << G4endl;

  // G4cout << " t4 : " << "  start-phi= " << t4.GetStartPhiAngle()
  //                    << "  delta-phi= " << t4.GetDeltaPhiAngle() / CLHEP::degree << " degrees " << G4endl;     

  // t2Ut4.SetCubVolStatistics(10000000);
  vol = t2Ut4.GetCubicVolume();
  // oldEstimate = t2Ut4.G4BooleanSolid::GetCubicVolume();
  volCheck = 2.*pi*50.*50.*50.;
  diff = std::abs(vol-volCheck)/volCheck;
  G4double ratio = vol/volCheck;
  G4cout<<"vol = "<<vol<<"; volCheck = "<<volCheck<<"; rel. diff = "<<diff
        << " ratio = " << ratio
        <<G4endl;
  //  assert(ApproxEqual(vol,volCheck));
  assert(diff<1.0e-5);
