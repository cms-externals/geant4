...
// NOTE: xRot = rotation such that x axis->-x axis & y axis->-y axis
  ...
  const G4Transform3D transform(xRot,G4ThreeVector(0,30,0)) ;
  const G4Transform3D transform2(xRot,G4ThreeVector(50,0,0)) ;

  G4Box b1("Test Box #1",20.,30.,40.);
  G4Box b2("Test Box #2",10.,10.,10.);
  G4Box b3("Test Box #3",10.,20.,50.);
  G4Box b4("Test Box #4",20.,20.,40.);
  G4Box b5("Test Box #5",50.,50.,50.);
  ...
  G4SubtractionSolid   b1Sb4("b1Subtractionb4",&b1,&b4,transform) ;
  G4SubtractionSolid   b5St1("b5St1",&b5,&t1,transform2) ;

  G4SubtractionSolid   b1Sb2touch("b1Sb2touch",&b1,&b2,&identity,G4ThreeVector(10.,0,0)) ;
...
// Assert clipped to volume
    ...
    // b1 - b2.rotated = b1Sb2
    G4double exact=  b1.GetCubicVolume() - b2.GetCubicVolume();
    G4double estimate = b1Sb2.GetCubicVolume();
    G4double diff = std::fabs(estimate-exact);
    G4cout << " b1 - b2.transform : exact = " << exact << " estimate = " << estimate << " diff = " << diff
              << " relative diff = " << diff / exact << G4endl;
     // assert( std::fabs(estimate-exact) < 1.0e-6 * exact ); 

    // b1 - b2.moved(10,0,0) = b1Sb2touch
    exact=  b1.GetCubicVolume() - b2.GetCubicVolume();
    estimate = b1Sb2touch.GetCubicVolume();
    diff = std::fabs(estimate-exact);
    G4cout << " b1 - b2 'touch'   : exact = " << exact << " estimate = " << estimate << " diff = " << diff
              << " relative diff = " << diff / exact << G4endl;
    
    // b1 - b3 = b1Sb3
    G4SubtractionSolid b1Sb3("b1Sb3",&b1,&b3);    
    exact=  b1.GetCubicVolume() - G4Box("B3-intersect-B1",10.,20.,40.).GetCubicVolume();
    estimate = b1Sb3.GetCubicVolume();
    diff = std::fabs(estimate-exact);
    G4cout << " b1 - b3           : exact = " << exact << " estimate = " << estimate << " diff = " << diff
           << " relative diff = " << diff / exact << G4endl;    
    // assert( std::fabs(estimate-exact) < 1.0e-6 * exact ); 
    
    // b1 - b4 = b1Sb4
    exact=  b1.GetCubicVolume() - G4Box("Overlap-b1-movedB4",20.,10.,40.).GetCubicVolume();
    estimate = b1Sb4.GetCubicVolume();
    diff = std::fabs(estimate-exact);
    G4cout << " b1 - b4.transform : exact = " << exact << " estimate = " << estimate << " diff = " << diff
           << " relative diff = " << diff / exact << G4endl;    
    // assert( std::fabs(estimate-exact) < 1.0e-6 * exact );
