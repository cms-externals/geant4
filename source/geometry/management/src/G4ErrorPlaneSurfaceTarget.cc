...
G4ErrorPlaneSurfaceTarget::G4ErrorPlaneSurfaceTarget(G4double aa, G4double ab, G4double ac,
                                                     G4double ad)
...
G4ErrorPlaneSurfaceTarget::G4ErrorPlaneSurfaceTarget(const G4Normal3D& norm, const G4Point3D& pt)
  : G4Plane3D(norm, pt)
{
  theType = G4ErrorTarget_PlaneSurface;
...
G4ErrorPlaneSurfaceTarget::G4ErrorPlaneSurfaceTarget(const G4Point3D& p1, const G4Point3D& p2,
                                                     const G4Point3D& p3)
  : G4Plane3D(p1, p2, p3)
{
  theType = G4ErrorTarget_PlaneSurface;
...
G4ThreeVector G4ErrorPlaneSurfaceTarget::Intersect(const G4ThreeVector& pt,
                                                   const G4ThreeVector& dir) const
{
  ...
  {
    G4cout << " $$$ creating G4ErrorPlaneSurfaceTarget::Intersect " << inters << G4endl;
  }
#endif
...
G4double G4ErrorPlaneSurfaceTarget::GetDistanceFromPoint(const G4ThreeVector& pt,
                                                         const G4ThreeVector& dir) const
{
  if (std::fabs(dir.mag() - 1.) > 1.E-6)
  {
    ...
    G4Exception("G4ErrorPlaneSurfaceTarget::GetDistanceFromPoint()", "GeomMgt1002", JustWarning,
                message);
  }
  G4double dist =
    -(a_ * pt.x() + b_ * pt.y() + c_ * pt.z() + d_) / (a_ * dir.x() + b_ * dir.y() + c_ * dir.z());

#ifdef G4VERBOSE
  if (G4ErrorPropagatorData::verbose() >= 3)
  {
    G4cout << " G4ErrorPlaneSurfaceTarget::GetDistanceFromPoint()" << G4endl << "   Point: " << pt
           << ", Direction: " << dir << G4endl << "   Distance: " << dist << G4endl;
  }
#endif
...
G4Plane3D G4ErrorPlaneSurfaceTarget::GetTangentPlane(const G4ThreeVector&) const
{
  return *this;
}
