...
G4double G4GeomTools::TriangleArea(G4double Ax, G4double Ay, G4double Bx, G4double By, G4double Cx,
                                   G4double Cy)
{
...
G4double G4GeomTools::QuadArea(const G4TwoVector& A, const G4TwoVector& B, const G4TwoVector& C,
                               const G4TwoVector& D)
{
...
G4bool G4GeomTools::PointInTriangle(G4double Ax, G4double Ay, G4double Bx, G4double By, G4double Cx,
                                    G4double Cy, G4double Px, G4double Py)
...
G4bool G4GeomTools::PointInTriangle(const G4TwoVector& A, const G4TwoVector& B,
                                    const G4TwoVector& C, const G4TwoVector& P)
{
  G4double Ax = A.x(), Ay = A.y();
...
G4bool G4GeomTools::PointInPolygon(const G4TwoVector& p, const G4TwoVectorList& v)
{
...
{
  static const G4double kCarTolerance = G4GeometryTolerance::GetInstance()->GetSurfaceTolerance();
  ...
  for (G4int icur = 0; icur < n; ++icur)
  {
    ...
    if (std::abs(cross) < kCarTolerance)
    {
      return false;
    }
    if (cross < 0)
...
G4bool G4GeomTools::TriangulatePolygon(const G4TwoVectorList& polygon, G4TwoVectorList& result)
{
  ...
  for (G4int i = 0; i < n; ++i)
  {
    result.push_back(polygon[triangles[i]]);
  }
  return reply;
}
...
G4bool G4GeomTools::CheckSnip(const G4TwoVectorList& contour, G4int a, G4int b, G4int c, G4int n,
                              const G4int* V)
{
...
void G4GeomTools::RemoveRedundantVertices(G4TwoVectorList& polygon, std::vector<G4int>& iout,
                                          G4double tolerance)
{
  iout.resize(0);
...
G4bool G4GeomTools::DiskExtent(G4double rmin, G4double rmax, G4double startPhi, G4double delPhi,
                               G4TwoVector& pmin, G4TwoVector& pmax)
{
  ...
  if (rmax <= rmin + kCarTolerance)
  {
    return false;
  }
  if (delPhi <= 0 + kCarTolerance)
  {
    return false;
  }
  ...
  pmin.set(-rmax, -rmax);
  pmax.set(rmax, rmax);
  if (delPhi >= CLHEP::twopi)
  {
    return true;
  }

  DiskExtent(rmin, rmax, std::sin(startPhi), std::cos(startPhi), std::sin(startPhi + delPhi),
             std::cos(startPhi + delPhi), pmin, pmax);
  return true;
}
...
void G4GeomTools::DiskExtent(G4double rmin, G4double rmax, G4double sinStart, G4double cosStart,
                             G4double sinEnd, G4double cosEnd, G4TwoVector& pmin, G4TwoVector& pmax)
{
  static const G4double kCarTolerance = G4GeometryTolerance::GetInstance()->GetSurfaceTolerance();
  ...
  if (std::abs(sinEnd - sinStart) < kCarTolerance && std::abs(cosEnd - cosStart) < kCarTolerance)
  {
    return;
  }
...
G4double G4GeomTools::EllipticConeLateralArea(G4double pA, G4double pB, G4double pH)
{
  G4double x = std::abs(pA);
  G4double y = std::abs(pB);
...
G4ThreeVector G4GeomTools::TriangleAreaNormal(const G4ThreeVector& A, const G4ThreeVector& B,
...
G4ThreeVector G4GeomTools::QuadAreaNormal(const G4ThreeVector& A, const G4ThreeVector& B,
...
G4double G4GeomTools::DistancePointSegment(const G4ThreeVector& P, const G4ThreeVector& A,
...
G4ThreeVector G4GeomTools::ClosestPointOnSegment(const G4ThreeVector& P, const G4ThreeVector& A,
                                                 const G4ThreeVector& B)
{
  G4ThreeVector AP = P - A;
  G4ThreeVector AB = B - A;
...
G4ThreeVector G4GeomTools::ClosestPointOnTriangle(const G4ThreeVector& P, const G4ThreeVector& A,
                                                  const G4ThreeVector& B, const G4ThreeVector& C)
{
...
G4bool G4GeomTools::SphereExtent(G4double rmin, G4double rmax, G4double startTheta,
                                 G4double delTheta, G4double startPhi, G4double delPhi,
                                 G4ThreeVector& pmin, G4ThreeVector& pmax)
{
  ...
  {
    dtheta = CLHEP::pi - stheta;
  }
  if (dtheta <= 0 + kCarTolerance)
  {
    return false;
  }
  ...
  if (dtheta >= CLHEP::pi && delPhi >= CLHEP::twopi)
  {
    return true;
  }
  ...
  if (stheta > CLHEP::halfpi)
  {
    rhomax = rmax * sinStart;
  }
  if (etheta < CLHEP::halfpi)
  {
    rhomax = rmax * sinEnd;
  }

  G4TwoVector xymin, xymax;
  DiskExtent(rhomin, rhomax, std::sin(startPhi), std::cos(startPhi), std::sin(startPhi + delPhi),
             std::cos(startPhi + delPhi), xymin, xymax);
...
void G4GeomTools::TwistedTubeBoundingTrap(G4double twistAng, G4double endInnerRad,
                                          G4double endOuterRad, G4double dPhi,
...
G4double G4GeomTools::HyperboloidSurfaceArea(G4double dphi, G4double r0, G4double tanstereo,
