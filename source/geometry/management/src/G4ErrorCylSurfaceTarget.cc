...
G4ErrorCylSurfaceTarget::G4ErrorCylSurfaceTarget(const G4double& radius, const G4ThreeVector& trans,
                                                       ...
                                                       const G4ThreeVector& dir) const
{
  if (dir.mag() == 0.)
  {
    G4Exception("G4ErrorCylSurfaceTarget::GetDistanceFromPoint()", "GeomMgt0003", FatalException,
                "Direction is zero !");
  }
  ...
  {
    G4cout << " G4ErrorCylSurfaceTarget::GetDistanceFromPoint():" << G4endl << " Global point "
           << point << " dir " << dir << G4endl << " Intersection " << inters << G4endl
...
G4double G4ErrorCylSurfaceTarget::GetDistanceFromPoint(const G4ThreeVector& point) const
{
  ...
  {
    G4cout << " G4ErrorCylSurfaceTarget::GetDistanceFromPoint:" << G4endl << " Global point "
           << point << G4endl << " Distance " << fradius - localPoint.perp() << G4endl;
    Dump(" CylSurface: ");
  }
#endif
...
G4ThreeVector G4ErrorCylSurfaceTarget::IntersectLocal(const G4ThreeVector& localPoint,
                                                      const G4ThreeVector& localDir) const
{
  ...
  G4double eqc =
    -fradius * fradius + localPoint.x() * localPoint.x() + localPoint.y() * localPoint.y();
  ...
  else
  {
    ...
    else
    {
      ...
      message << "Intersection not possible !" << G4endl << "          Point: " << localPoint
              << ", direction: " << localDir;
      Dump(" CylSurface: ");
      G4Exception("G4ErrorCylSurfaceTarget::IntersectLocal()", "GeomMgt1002", JustWarning, message);
      lambda = kInfinity;
    }
  }
  ...
  {
    G4cout << " G4ErrorCylSurfaceTarget::IntersectLocal " << inters << " " << inters.perp()
...
G4Plane3D G4ErrorCylSurfaceTarget::GetTangentPlane(const G4ThreeVector& point) const
{
  ...
  {
    std::ostringstream message;
    message << "Local point not at surface !" << G4endl << "          Point: " << point
            << ", local: " << localPoint << G4endl
            << "          is not at surface, but far away by: " << localPoint.perp() - fradius
            << " !";
    G4Exception("G4ErrorCylSurfaceTarget::GetTangentPlane()", "GeomMgt1002", JustWarning, message);
  }
...
{
  G4cout << msg << " radius " << fradius << " centre " << ftransform.NetTranslation()
