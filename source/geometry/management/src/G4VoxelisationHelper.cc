...
// ***************************************************************************
//
void G4VoxelisationHelper::CreateListOfVolumesToOptimise(G4bool allOpts, G4bool verbose)
{
...
void G4VoxelisationHelper::UndertakeOptimisation()
{
  ...
  if (allDone && (myCount == G4Threading::GetNumberOfRunningWorkerThreads()))
  {
    ...
    if (badVolumes > 0)
    {
      ...
      G4Exception("G4VoxelisationHelper::UndertakeOptimisation()", "GeomMgt0002", FatalException,
                  errmsg);
    }
    ...
    if (fVerboseParallel)
    {
      ...
      report_stream << G4endl << "G4VoxelisationHelper::UndertakeOptimisation"
      ...
      report_stream << "  - Sum voxel time (real) = " << fSumVoxelTime << "s.";
    ...
    if (fVerboseParallel)
    {
      ...
      else
      {
            ...
            && (fNumberThreadsReporting < TotalThreads))
        {
          G4ExceptionDescription errmsg;
          errmsg << " [thread " << tid << " ] "
                 << " WARNING: All volumes optimised, yet only " << fNumberThreadsReporting
