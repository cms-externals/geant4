...
// ***************************************************************************
//
G4SmartVoxelHeader::G4SmartVoxelHeader(G4LogicalVolume* pVolume, G4int pSlice)
  : fminEquivalent(pSlice), fmaxEquivalent(pSlice), fparamAxis(kUndefined)
{
...
// ***************************************************************************
//
G4SmartVoxelHeader::G4SmartVoxelHeader(G4LogicalVolume* pVolume, const G4VoxelLimits& pLimits,
                                       const G4VolumeNosVector* pCandidates, G4int pSlice)
  : fminEquivalent(pSlice), fmaxEquivalent(pSlice), fparamAxis(kUndefined)
{
#ifdef G4GEOMETRY_VOXELDEBUG
  G4cout << "**** G4SmartVoxelHeader::G4SmartVoxelHeader" << G4endl << "     Limits " << pLimits
         << G4endl << "     Candidate #s = ";
...
{
  if ((GetAxis() == pHead.GetAxis()) && (GetNoSlices() == pHead.GetNoSlices())
      && (GetMinExtent() == pHead.GetMinExtent()) && (GetMaxExtent() == pHead.GetMaxExtent()))
  {
...
void G4SmartVoxelHeader::BuildReplicaVoxels(G4LogicalVolume* pVolume)
{
  ...
  //
  if ((pVolume->GetNoDaughters() == 1) && (pVolume->GetDaughter(0)->IsReplicated()))
  {
    ...
    if (!consuming)
    {
      ...
      if (axis != kUndefined)
      {
        ...
        pVolume->GetSolid()->CalculateExtent(faxis, limits, origin, fminExtent, fmaxExtent);
    ...
    else
    {
      ...
      switch (axis)
      {
        ...
        case kPhi:
          ...
          break;
        default:
          G4Exception("G4SmartVoxelHeader::BuildReplicaVoxels()", "GeomMgt0002", FatalException,
                      "Illegal axis.");
          break;
      }
      faxis = axis;  // Set axis
      ...
      if ((axis == kXAxis) || (axis == kYAxis) || (axis == kZAxis))
      {
        ...
        if ((std::fabs((emin - fminExtent) / fminExtent)
             + std::fabs((emax - fmaxExtent) / fmaxExtent))
            > 0.05)
        {
          std::ostringstream message;
          message << "Sanity check: wrong solid extent." << G4endl
                  << "        Replicated geometry, logical volume: " << pVolume->GetName();
          G4Exception("G4SmartVoxelHeader::BuildReplicaVoxels", "GeomMgt0002", FatalException,
                      message);
        }
      }
...
void G4SmartVoxelHeader::BuildConsumedNodes(G4int nReplicas)
{
  ...
  for (nNode = 0; nNode < nReplicas; ++nNode)
  {
    ...
    if (pNode == nullptr)
    {
      G4Exception("G4SmartVoxelHeader::BuildConsumedNodes()", "GeomMgt0003", FatalException,
...
// ***************************************************************************
//
void G4SmartVoxelHeader::BuildVoxelsWithinLimits(G4LogicalVolume* pVolume, G4VoxelLimits pLimits,
                                                 const G4VolumeNosVector* pCandidates)
{
  ...
  {
    G4Exception("G4SmartVoxelHeader::BuildVoxelsWithinLimits()", "GeomMgt0002", FatalException,
...
#ifdef G4GEOMETRY_VOXELDEBUG
  G4cout << G4endl << "     Volume = " << pVolume->GetName() << G4endl
         << "     Selected axis = " << faxis << G4endl;
  ...
  for (sliceNo = 0; sliceNo < maxNode; ++sliceNo)
  {
    ...
    if (maxNo != sliceNo)
    {
      ...
      G4cout << "**** G4SmartVoxelHeader::CollectEquivalentNodes" << G4endl
             << "     Collecting Nodes = " << sliceNo << " - " << maxNo << G4endl;
#endif
...
// ***************************************************************************
//
G4ProxyVector* G4SmartVoxelHeader::BuildNodes(G4LogicalVolume* pVolume, G4VoxelLimits pLimits,
                                              const G4VolumeNosVector* pCandidates, EAxis pAxis)
{
  G4double motherMinExtent = kInfinity, motherMaxExtent = -kInfinity, targetMinExtent = kInfinity,
           targetMaxExtent = -kInfinity;
  G4VPhysicalVolume* pDaughter = nullptr;
...
#ifdef G4GEOMETRY_VOXELDEBUG
  G4cout << "**** G4SmartVoxelHeader::BuildNodes" << G4endl << "     Limits = " << pLimits << G4endl
         << "       Axis = " << pAxis << G4endl << " Candidates = " << nCandidates << G4endl;
#endif
  ...
  if ((pVolume->GetNoDaughters() == 1) && (pVolume->GetDaughter(0)->IsReplicated()))
  {
    ...
    if (pParam == nullptr)
    {
      ...
      G4Exception("G4SmartVoxelHeader::BuildNodes()", "GeomMgt0003", FatalException, message);
      return nullptr;
    }

    // Setup daughter's transformations
    //
    targetTransform = G4AffineTransform(pDaughter->GetRotation(), pDaughter->GetTranslation());
    replicated = true;
  }
  else
  {
  ...
  for (nVol = 0; nVol < nCandidates; ++nVol)
  {
    ...
    // Calculate extents
    //
    if (!targetSolid->CalculateExtent(pAxis, pLimits, targetTransform, targetMinExtent,
                                      targetMaxExtent))
    {
      targetSolid->CalculateExtent(pAxis, noLimits, targetTransform, targetMinExtent,
                                   targetMaxExtent);
    }
    ...
    G4cout << "---------------------------------------------------" << G4endl
           ...
           << " Min Extent = " << targetMinExtent << G4endl << " Max Extent = " << targetMaxExtent
           << G4endl << "---------------------------------------------------" << G4endl;
#endif
    ...
    if ((!pLimits.IsLimited())
        && ((targetMaxExtent <= motherMinExtent) || (targetMinExtent >= motherMaxExtent)))
    {
      std::ostringstream message;
      message << "PANIC! - Overlapping daughter with mother volume." << G4endl
              << "         Daughter physical volume " << pDaughter->GetName() << G4endl
              << "         is entirely outside mother logical volume " << pVolume->GetName()
              << " !!";
      G4Exception("G4SmartVoxelHeader::BuildNodes()", "GeomMgt0002", FatalException, message);
    }
    ...
    if (!pLimits.IsLimited())
    {
      ...
      {
        G4cout << "**** G4SmartVoxelHeader::BuildNodes" << G4endl << "     WARNING : Daughter # "
               << nVol << " name = " << pDaughter->GetName() << G4endl
               << "     Crosses mother boundary of logical volume, name = " << pVolume->GetName()
               << G4endl << "     by more than " << kStraddlePercent << "%" << G4endl;
      }
    }
#endif
  }
  ...
  for (nVol = 0; nVol < nCandidates; ++nVol)
  {
    ...
    if ((currentWidth < minWidth) && (maxExtents[nVol] >= pLimits.GetMinExtent(pAxis))
  ...
  G4double smartless = (smartlessComputed <= smartlessUser) ? smartlessComputed : smartlessUser;
  G4double noNodesSmart = smartless * nCandidates;
  auto noNodesExactI = G4int(noNodesSmart);
  G4long noNodes = ((noNodesSmart - noNodesExactI) >= 0.5) ? noNodesExactI + 1 : noNodesExactI;
  if (noNodes == 0)
  {
    noNodes = 1;
  }
  ...
  G4cout << "     Smartless computed = " << smartlessComputed << G4endl
         << "     Smartless volume = " << smartlessUser << " => # Smartless = " << smartless
         << G4endl;
  G4cout << "     Min width = " << minWidth << " => # Nodes = " << noNodes << G4endl;
#endif
  ...
  if (nodeList == nullptr)
  {
    G4Exception("G4SmartVoxelHeader::BuildNodes()", "GeomMgt0003", FatalException,
                "NodeList allocation error.");
    return nullptr;
  }
...
G4double G4SmartVoxelHeader::CalculateQuality(G4ProxyVector* pSlice)
{
  ...
  for (std::size_t i = 0; i < nNodes; ++i)
  {
    ...
    {
      G4Exception("G4SmartVoxelHeader::CalculateQuality()", "GeomMgt0001", FatalException,
                  "Not applicable to replicated volumes.");
    }
  }
...
#ifdef G4GEOMETRY_VOXELDEBUG
  G4cout << "**** G4SmartVoxelHeader::CalculateQuality" << G4endl << "     Quality = " << quality
         << G4endl << "     Nodes = " << nNodes << " of which " << sumNonEmptyNodes << " non empty"
         << G4endl << "     Max Contained = " << maxContained << G4endl;
#endif
  ...
  if (refinedDepth < 2)
  {
    ...
    for (targetNo = 0; targetNo < maxNode; ++targetNo)
    {
      ...
      if (targetNode->GetNoContained() >= minVolumes)
      {
        ...
        {
          G4Exception("G4SmartVoxelHeader::RefineNodes()", "GeomMgt0003", FatalException,
...
#ifdef G4GEOMETRY_VOXELDEBUG
        G4cout << "**** G4SmartVoxelHeader::RefineNodes" << G4endl << "     Refining nodes "
               << minNo << " - " << maxNo << " inclusive" << G4endl;
#endif
        ...
        replaceHeader = new G4SmartVoxelHeader(pVolume, newLimits, targetList, (G4int)replaceNo);
        if (replaceHeader == nullptr)
        {
          G4Exception("G4SmartVoxelHeader::RefineNodes()", "GeomMgt0003", FatalException,
                      "Refined VoxelHeader allocation error.");
          return;
        }
        ...
        {
          G4Exception("G4SmartVoxelHeader::RefineNodes()", "GeomMgt0003", FatalException,
                      "Refined VoxelProxy allocation error.");
          return;
        }
        for (replaceNo = minNo; replaceNo <= maxNo; ++replaceNo)
        {
  ...
  if (haveHeaders)
  {
    ...
    for (j = 0; j < h.fslices.size(); ++j)
    {
      if (h.fslices[j]->IsHeader())
      {
        ...
        {
          os << G4endl << (*(h.fslices[j]->GetHeader()));
