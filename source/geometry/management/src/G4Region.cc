...
// *******************************************************************
//
G4Region::G4Region(const G4String& pName) : fName(pName)
{
  ...
  if (rStore->GetRegion(pName, false) != nullptr)
  {
    ...
    message << "The region has NOT been registered !" << G4endl << "          Region " << pName
            << " already existing in store !" << G4endl;
    G4Exception("G4Region::G4Region()", "GeomMgt1001", JustWarning, message);
  }
  else
  {
...
// ********************************************************************
//
G4Region::G4Region(__void__&) : fName("")
{
  instanceID = subInstanceManager.CreateSubInstance();
  G4MT_fsmanager = nullptr;
  G4MT_rsaction = nullptr;
...
void G4Region::ScanVolumeTree(G4LogicalVolume* lv, G4bool region)
{
  ...
  if ((volMat == nullptr) && fInMassGeometry)
  {
    ...
    G4Exception("G4Region::ScanVolumeTree()", "GeomMgt0002", FatalException, message,
  ...
  if (daughterPVol->IsParameterised())
  {
    ...
    if (pParam->GetMaterialScanner() != nullptr)
    {
      ...
      for (std::size_t mat = 0; mat < matNo; ++mat)
      {
        ...
        {
          std::ostringstream message;
          message << "The parameterisation for the physical volume <" << daughterPVol->GetName()
                  << ">" << G4endl << "does not return a valid material pointer." << G4endl
...
void G4Region::AddRootLogicalVolume(G4LogicalVolume* lv, G4bool search)
{
  ...
  if ((lv->IsRootRegion()) && (lv->GetRegion() != this))
  {
    ...
    message << "Logical volume <" << lv->GetName() << "> is already set as" << G4endl
            ...
            << G4endl;
    G4Exception("G4Region::AddRootLogicalVolume()", "GeomMgt0002", FatalException, message,
...
G4bool G4Region::BelongsTo(G4VPhysicalVolume* thePhys) const
{
  ...
  {
    if (BelongsTo(currLog->GetDaughter(nDaughters)))
    {
      return true;
    }
  }
  ...
  if (parent != nullptr)
  {
    ...
    else
    {
      ...
      message << "Region <" << fName << "> belongs to more than"
              ...
              << "A region cannot belong to more than one direct parent region," << G4endl
              << "to have fast-simulation assigned.";
      G4Exception("G4Region::ClearFastSimulationManager()", "GeomMgt1002", JustWarning, message);
      G4MT_fsmanager = nullptr;
    }
  }
  else
