...
{
  template <typename F>
  G4bool ParallelFor(std::size_t count, std::size_t grain, F&& task)
  {
#ifdef G4MULTITHREADED
    G4GeometryManager* geomManager = G4GeometryManager::GetInstance();
    if (count < 2 * grain
        || geomManager->IsSerialGetMassForced()
        || geomManager->IsSerialGetPointOnSurfaceForced())
    {
      return false;
    }

    auto workerContext = [](std::size_t, const auto& run)
    {
      G4GeomTaskHelper::ScopedGeometryWorkspaces workspaces;
      run();
    };

    return G4GeomTaskHelper::ParallelForWithWorkerContext(
      count, grain, std::forward<F>(task), workerContext);
#else
    (void)count;
    (void)grain;
    (void)task;
...
{
  std::atomic<G4int> iInside = 0;
  G4double volume, halfepsilon;
  ...
  auto sample = [&](std::size_t begin, std::size_t end, std::size_t worker)
  {
    (void)worker;
    G4int localInside = 0;
    std::uint64_t rng = G4GeomTaskHelper::MixSeed(1234567890ULL + begin);
    for (auto i = begin; i < end; ++i)
    {
      (void)i;
      G4double px = minX - halfepsilon
                  + (maxX - minX + epsilon) * G4GeomTaskHelper::UnitRand(rng);
      G4double py = minY - halfepsilon
                  + (maxY - minY + epsilon) * G4GeomTaskHelper::UnitRand(rng);
      G4double pz = minZ - halfepsilon
                  + (maxZ - minZ + epsilon) * G4GeomTaskHelper::UnitRand(rng);
    ...
    iInside.fetch_add(localInside, std::memory_order_relaxed);
  };

  if (!ParallelFor(static_cast<std::size_t>(nStat), 16384, sample))
  {
    sample(0, static_cast<std::size_t>(nStat), 0);
  }
  volume = (maxX - minX + epsilon)
         * (maxY - minY + epsilon)
         * (maxZ - minZ + epsilon)
         * iInside.load(std::memory_order_relaxed) / nStat;
...
G4double G4VSolid::EstimateSurfaceArea(G4int nstat, G4double ell) const
{
  ...
  auto sample = [&](std::size_t begin, std::size_t end, std::size_t worker)
  {
    ...
    {
      (void)i;
      G4double px = minX + dX * G4GeomTaskHelper::UnitRand(rng);
      G4double py = minY + dY * G4GeomTaskHelper::UnitRand(rng);
      G4double pz = minZ + dZ * G4GeomTaskHelper::UnitRand(rng);
      G4ThreeVector p = G4ThreeVector(px, py, pz);
      EInside in = Inside(p);
      G4double dist = 0;
      if (in == kInside)
    ...
    icount.fetch_add(localCount, std::memory_order_relaxed);
  };

  if (!ParallelFor(static_cast<std::size_t>(npoints), 8192, sample))
  {
    sample(0, static_cast<std::size_t>(npoints), 0);
  }
  return dX * dY * dZ * icount.load(std::memory_order_relaxed) / npoints / dd;
}
