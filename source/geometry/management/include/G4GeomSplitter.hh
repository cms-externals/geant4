...
class G4GeomSplitter
{
  public:
    ...
    {
      G4int oldspace = workertotalspace;
      workertotalspace = size;
      T* newoffset = (T*)std::realloc(offset, workertotalspace * sizeof(T));
      if (newoffset != nullptr && workertotalspace > oldspace)
      {
        std::uninitialized_value_construct(newoffset + oldspace,
                                           newoffset + workertotalspace);
      }
      return newoffset;
    }

    /**
     * Invoked by the master or worker thread to create a new subinstance
    ...
    G4int CreateSubInstance()
    {
      ...
      {
        const G4bool copyShared = (offset == nullptr && sharedOffset != nullptr);
        T* previousSharedOffset = sharedOffset;
        const G4int previousSharedSpace = totalspace;

        offset = Reallocate(totalobj + 512);
        if (offset == nullptr)
        {
          G4Exception("G4GeomSPlitter::CreateSubInstance()", "OutOfMemory",
                      FatalException, "Cannot malloc space!");
        }
        if (copyShared)
        {
          std::memcpy(offset, previousSharedOffset,
                      previousSharedSpace * sizeof(T));
        }
      }
  ...
  public:
    ...
    /** Capacity of the subinstance array owned by the current thread. */
    G4GEOM_DLL static G4ThreadLocal G4int workertotalspace;
...
template<typename T>
G4ThreadLocal G4int G4GeomSplitter<T>::workertotalspace = 0;
