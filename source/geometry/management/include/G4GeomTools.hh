...
class G4GeomTools
{
  public:
     ...
     */
    static G4double TriangleArea(G4double Ax, G4double Ay, G4double Bx, G4double By, G4double Cx,
                                 G4double Cy);
     ...
     */
    static G4double QuadArea(const G4TwoVector& A, const G4TwoVector& B, const G4TwoVector& C,
                             const G4TwoVector& D);
     ...
     */
    static G4bool PointInTriangle(G4double Px, G4double Py, G4double Ax, G4double Ay, G4double Bx,
                                  G4double By, G4double Cx, G4double Cy);

    /**
     * Decides if a point P is inside the triangle ABC.
     */
    static G4bool PointInTriangle(const G4TwoVector& P, const G4TwoVector& A, const G4TwoVector& B,
                                  const G4TwoVector& C);
     ...
     */
    static void RemoveRedundantVertices(G4TwoVectorList& polygon, std::vector<G4int>& iout,
     ...
     */
    static G4bool DiskExtent(G4double rmin, G4double rmax, G4double startPhi, G4double delPhi,
     ...
     */
    static void DiskExtent(G4double rmin, G4double rmax, G4double sinPhiStart, G4double cosPhiStart,
                           G4double sinPhiEnd, G4double cosPhiEnd, G4TwoVector& pmin,
                           G4TwoVector& pmax);
     ...
     */
    static G4ThreeVector TriangleAreaNormal(const G4ThreeVector& A, const G4ThreeVector& B,
     ...
     */
    static G4ThreeVector QuadAreaNormal(const G4ThreeVector& A, const G4ThreeVector& B,
     ...
     */
    static G4double DistancePointSegment(const G4ThreeVector& P, const G4ThreeVector& A,
                                         const G4ThreeVector& B);

    /**
     * Finds a point on a 3D line segment AB closest to point 'P'.
     */
    static G4ThreeVector ClosestPointOnSegment(const G4ThreeVector& P, const G4ThreeVector& A,
                                               const G4ThreeVector& B);

    /**
     * Finds a point on a 3D triangle ABC closest to point 'P'.
     */
    static G4ThreeVector ClosestPointOnTriangle(const G4ThreeVector& P, const G4ThreeVector& A,
     ...
     *   delPhi     >  0 + kCarTolerance.
     */
    static G4bool SphereExtent(G4double rmin, G4double rmax, G4double startTheta, G4double delTheta,
                               G4double startPhi, G4double delPhi, G4ThreeVector& pmin,
                               G4ThreeVector& pmax);
     ...
     */
    static void TwistedTubeBoundingTrap(G4double twistAng, G4double endInnerRad,
     ...
     */
    static G4double HyperboloidSurfaceArea(G4double dphi, G4double r0, G4double tanstereo,
  ...
  private:
     ...
     */
    static G4bool CheckSnip(const G4TwoVectorList& contour, G4int a, G4int b, G4int c, G4int n,
                            const G4int* V);
