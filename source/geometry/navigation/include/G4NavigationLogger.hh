...
class G4NavigationLogger
{
  public:
     ...
     */
    void CheckDaughterEntryPoint(const G4VSolid* sampleSolid, const G4ThreeVector& samplePoint,
                                 const G4ThreeVector& sampleDirection, const G4VSolid* motherSolid,
     ...
     */
    void PrintDaughterLog(const G4VSolid* sampleSolid, const G4ThreeVector& samplePoint,
                          G4double sampleSafety, G4bool onlySafety,
                          const G4ThreeVector& sampleDirection, G4double sampleStep) const;

    /**
     * Reports issue with normal from Solid - for ComputeStep().
     */
    G4bool CheckAndReportBadNormal(const G4ThreeVector& unitNormal, const G4ThreeVector& localPoint,
                                   const G4ThreeVector& localDirection, G4double step,
                                   const G4VSolid* solid, const char* msg) const;
    ...
    G4bool CheckAndReportBadNormal(const G4ThreeVector& unitNormal,
                                   ...
                                   const G4RotationMatrix& rotationM, const char* msg) const;

    /**
     * Reports if point wrongly located outside mother volume.
     */
    void ReportOutsideMother(const G4ThreeVector& localPoint, const G4ThreeVector& localDirection,
     ...
     */
    void ReportVolumeAndIntersection(std::ostream& ostrm, const G4ThreeVector& localPoint,
