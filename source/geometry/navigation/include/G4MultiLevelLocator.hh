 ...
 * point with a boundary when G4PropagationInField is used.
 * @ingroup geometry_navigation
 *
 * Derived from method LocateIntersectionPoint() from G4PropagatorInField,
 * it is based on a linear method for finding the intersection point by means of a 'depth' algorithm
...
class G4MultiLevelLocator : public G4VIntersectionLocator
{
  ...
  private:
    ...
    void ReportFieldValue(const G4FieldTrack& locationPV, const char* nameLoc,
    ...
    unsigned long int fNumAdvanceFull = 0, fNumAdvanceGood = 0, fNumAdvanceTrials = 0;
};
