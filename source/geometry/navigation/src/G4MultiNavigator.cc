                                       ...
                                       const G4double proposedStepLength, G4double& pNewSafety)
{
  ...
  for (auto num = 0; num < fNoActiveNavigators; ++pNavigatorIter, ++num)
  {
    ...
    if (fVerbose > 2)
    {
      G4cout << "G4MultiNavigator::ComputeStep : Navigator [" << num << "] -- step size " << step
                                           ...
                                           G4double& minStep, ELimited& limitedStep)
{
  if (navigatorId > fNoActiveNavigators)
  {
    ...
    G4Exception("G4MultiNavigator::ObtainFinalStep()", "GeomNav0002", FatalException, message);
  }
  ...
  if (fVerbose > 1)
  {
    G4cout << " G4MultiNavigator::ComputeStep returns " << fCurrentStepSize[navigatorId]
...
void G4MultiNavigator::PrepareNewTrack(const G4ThreeVector& position, const G4ThreeVector direction)
{
#ifdef G4DEBUG_NAVIGATION
  if (fVerbose > 1)
  {
...
void G4MultiNavigator::PrepareNavigators()
{
  ...
  if (fNoActiveNavigators > fMaxNav)
  {
    ...
    G4Exception("G4MultiNavigator::PrepareNavigators()", "GeomNav0002", FatalException, message);
  }
...
G4VPhysicalVolume* G4MultiNavigator::LocateGlobalPointAndSetup(const G4ThreeVector& position,
                                                               const G4ThreeVector* pDirection,
                                                               ...
                                                               const G4bool ignoreDirection)
{
  ...
  if (fVerbose > 2)
  {
    G4cout << " Entered G4MultiNavigator::LocateGlobalPointAndSetup() " << G4endl;
...
G4double G4MultiNavigator::ComputeSafety(const G4ThreeVector& position, const G4double maxDistance,
                                         const G4bool state)
{
  ...
  if (fVerbose > 1)
  {
    G4cout << " G4MultiNavigator::ComputeSafety - returns: " << minSafety
...
{
  G4Exception("G4MultiNavigator::CreateTouchableHistoryHandle()", "GeomNav0001", FatalException,
...
void G4MultiNavigator::ResetState()
{
  ...
  G4Exception("G4MultiNavigator::ResetState()", "GeomNav0001", FatalException,
...
{
  G4Exception("G4MultiNavigator::SetupHierarchy()", "GeomNav0001", FatalException,
...
void G4MultiNavigator::CheckMassWorld()
{
  ...
  {
    G4Exception("G4MultiNavigator::CheckMassWorld()", "GeomNav0003", FatalException,
                                                             ...
                                                             const G4TouchableHistory& MassHistory)
{
  ...
  {
    G4Exception("G4MultiNavigator::ResetHierarchyAndLocate()", "GeomNav0002", FatalException,
...
G4ThreeVector G4MultiNavigator::GetGlobalExitNormal(const G4ThreeVector& argPoint,
  ...
  else
  {
    if (fNoLimitingStep > 1)
    {
      ...
      if (!oneObtained)
      {
        ...
        G4Exception("G4MultiNavigator::GetGlobalExitNormal()", "GeomNav0002", JustWarning, message);
      }
...
G4ThreeVector G4MultiNavigator::GetLocalExitNormal(G4bool* argpObtained)
{
  ...
  else
  {
    if (fNoLimitingStep > 1)
    {
      ...
      message << "Cannot obtain normal in local coordinates of two or more "
              << "coordinate systems." << G4endl;
      G4Exception("G4MultiNavigator::GetGlobalExitNormal()", "GeomNav0002", FatalException,
                  message);
    }
  }
