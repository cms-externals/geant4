...
//
void G4DrawVoxels::SetVoxelsVisAttributes(G4VisAttributes& VA_voxelX, G4VisAttributes& VA_voxelY,
...
void G4DrawVoxels::ComputeVoxelPolyhedra(const G4LogicalVolume* lv,
                                         const G4SmartVoxelHeader* header, G4VoxelLimits& limit,
                                         G4PlacedPolyhedronList* ppl) const
{
  ...
  G4ThreeVector t_centerofBoundingBox((xmin + xmax) * 0.5, (ymin + ymax) * 0.5,
                                      (zmin + zmax) * 0.5);

  ppl->emplace_back(bounding_polyhedronBox, G4Translate3D(t_centerofBoundingBox));
...
void G4DrawVoxels::DrawVoxels(const G4LogicalVolume* lv) const
{
  ...
  G4TouchableHandle aTouchable = G4TransportationManager::GetTransportationManager()
                                   ->GetNavigatorForTracking()
                                   ->CreateTouchableHistoryHandle();
  G4AffineTransform globTransform = aTouchable->GetHistory()->GetTopTransform().Inverse();
  G4Transform3D transf3D(globTransform.NetRotation(), globTransform.NetTranslation());
  ...
  if (pVVisManager != nullptr)
  {
    ...
    {
      pVVisManager->Draw(i.GetPolyhedron(), i.GetTransform() * transf3D);
    }
  }
  else
  {
    G4Exception("G4DrawVoxels::DrawVoxels()", "GeomNav1002", JustWarning,
