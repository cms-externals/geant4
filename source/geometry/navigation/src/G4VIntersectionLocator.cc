...
//
void G4VIntersectionLocator::printStatus(const G4FieldTrack& StartFT, const G4FieldTrack& CurrentFT,
                                         G4double requestStep, G4double safety, G4int stepNo)
{
...
void G4VIntersectionLocator::printStatus(const G4FieldTrack& StartFT, const G4FieldTrack& CurrentFT,
                                         G4double requestStep, G4double safety, G4int stepNo,
                                         std::ostream& os, G4int verboseLevel)
{
  ...
  if (((stepNo == 0) && (verboseLevel < 3)) || (verboseLevel >= 3))
  {
    ...
    os << std::setw(6) << " " << std::setw(25) << " Current Position  and  Direction" << " "
       << G4endl;
    os << std::setw(5) << "Step#" << std::setw(10) << "  s  " << " " << std::setw(10) << "X(mm)"
       << " " << std::setw(10) << "Y(mm)" << " " << std::setw(10) << "Z(mm)" << " " << std::setw(7)
       << " N_x " << " " << std::setw(7) << " N_y " << " " << std::setw(7) << " N_z " << " ";
    os << std::setw(7) << " Delta|N|" << " " << std::setw(9) << "StepLen" << " " << std::setw(12)
       << "StartSafety" << " " << std::setw(9) << "PhsStep" << " ";
    os << G4endl;
    os.precision(oldprc);
  }
  ...
  if (verboseLevel <= 3)
  {
    ...
    os << std::setw(10) << CurrentPosition.x() << " " << std::setw(10) << CurrentPosition.y() << " "
       << std::setw(10) << CurrentPosition.z() << " ";
    os.precision(4);
    os << std::setw(7) << CurrentUnitVelocity.x() << " " << std::setw(7) << CurrentUnitVelocity.y()
       << " " << std::setw(7) << CurrentUnitVelocity.z() << " ";
    os.precision(3);
    os << std::setw(7) << CurrentFT.GetMomentum().mag() - StartFT.GetMomentum().mag() << " ";
  ...
  else  // if( verboseLevel > 3 )
  {
    ...
    os << "Step taken was " << step_len << " out of PhysicalStep= " << requestStep << G4endl;
    os << "Final safety is: " << safety << G4endl;
    os << "Chord length = " << (CurrentPosition - StartPosition).mag() << G4endl;
    os << G4endl;
  }
}
...
)
{
  ...
  do  // Loop checking, 07.10.2016, JA
  {
    ...
    {
      goodAdvance = integrDriver->AccurateAdvance(newEndPoint, advanceLength, GetEpsilonStepFor());
    }
  ...
  if (itrial > 1)
  {
    if (fVerboseLevel > 0)
    {
      G4cout << MethodName << " called - goodAdv= " << goodAdvance << " trials = " << itrial
             << " previous good= " << latest_good_trials << G4endl;
    }
...
#ifdef G4DEBUG_FIELD
  G4double lengthDone = newEndPoint.GetCurveLength() - CurrentStateA.GetCurveLength();
  if (!goodAdvance)
  {
    if (fVerboseLevel >= 3)
    {
  ...
  G4double linearDist = (EstimatedEndStateB.GetPosition() - CurrentStateA.GetPosition()).mag();
  static G4int noInaccuracyWarnings = 0;
  G4int maxNoWarnings = 10;
  if ((noInaccuracyWarnings < maxNoWarnings) || (fVerboseLevel > 1))
  {
    G4ThreeVector move = newEndPoint.GetPosition() - EstimatedEndStateB.GetPosition();
    ...
    message << " Integration inaccuracy requires"
            ...
            << " curve length difference" << G4endl << "   Dist = " << linearDist
            << " curve length = " << curveDist << G4endl;
    message << " Correction applied is " << move.mag() << G4endl
            << "  Old Estimated B position= " << EstimatedEndStateB.GetPosition() << G4endl
            << "  Recalculated    Position= " << newEndPoint.GetPosition() << G4endl
            << "   Change ( new - old )   = " << move;
    G4Exception("G4VIntersectionLocator::ReEstimateEndpoint()", "GeomNav1002", JustWarning,
                message);
  }
/*
#else
                                                          ...
                                                          G4int& curveError)
{
  ...
  linDistSq = (EstimatedEndB.GetPosition() - CurrentStartA.GetPosition()).mag2();
  curveDist = EstimatedEndB.GetCurveLength() - CurrentStartA.GetCurveLength();
  if ((curveDist >= 0.0) && (curveDist * curveDist * (1.0 + 2.0 * fiEpsilonStep) < linDistSq))
  {
    ...
    if (curveDist > 0.0)
    {
      ...
      RevisedEndPoint = ReEstimateEndpoint(CurrentStartA, EstimatedEndB, linDistSq, curveDist);
      recalculated = true;
    }
    else
    {
      ...
      G4Exception("G4MultiLevelLocator::EstimateIntersectionPoint()", "GeomNav1002", JustWarning,
                                                            ...
                                                            G4bool& validNormal)
{
  ...
  G4ThreeVector localPosition =
    fpTouchable->GetHistory()->GetTopTransform().TransformPoint(CurrentE_Point);
  ...
  if (located != nullptr)
  {
    ...
    if ((pLogical != nullptr) && ((pSolid = pLogical->GetSolid()) != nullptr))
    {
          ...
          || (pSolid->DistanceToOut(localPosition) < 1000.0 * kCarTolerance))
      {
        ...
        {
          G4cerr << "PROBLEM in G4VIntersectionLocator::GetLocalSurfaceNormal." << G4endl;
...
//
G4bool G4VIntersectionLocator::AdjustmentOfFoundIntersection(
  const G4ThreeVector& CurrentA_Point, const G4ThreeVector& CurrentE_Point,
  const G4ThreeVector& CurrentF_Point, const G4ThreeVector& MomentumDir, const G4bool IntersectAF,
  ...
  G4ThreeVector& fPreviousSftOrigin)  // I/O
{
  ...
  if (std::abs(n_d_m) > kCarTolerance)
  {
    ...
    if (fVerboseLevel > 1)
    {
      G4Exception("G4VIntersectionLocator::AdjustmentOfFoundIntersection()", "GeomNav0003",
                  JustWarning, "No intersection. Parallels lines!");
    }
#endif
    ...
    if (IntersectAF)  //  First part intersects
    {
      ...
      Intersects_FP = IntersectChord(Point_P, NewPoint, NewSafety, fPreviousSafety,
                                     fPreviousSftOrigin, stepLengthFP, Point_G);
    }
    else  // Second part intersects
    {
      G4double stepLengthFP;
      GetNavigatorFor()->LocateGlobalPointWithinVolume(CurrentF_Point);
      Intersects_FP = IntersectChord(CurrentF_Point, NewPoint, NewSafety, fPreviousSafety,
                                     fPreviousSftOrigin, stepLengthFP, Point_G);
    }
    if (Intersects_FP)
    {
                                                       ...
                                                       G4bool& validNormal)
{
  ...
  {
    std::ostringstream message;
    message << "PROBLEM: Normal is not unit - magnitude = " << NormalAtEntryLast.mag() << G4endl;
    message << "   at trial intersection point " << CurrentInt_Point << G4endl;
    message << "   Obtained from Get *Last* Surface Normal.";
    G4Exception("G4VIntersectionLocator::GetSurfaceNormal()", "GeomNav1002", JustWarning, message);
  }
#endif
                                                             ...
                                                             G4bool& validNormal)
{
  ...
  if (validNormal && (std::fabs(globalNormal.mag2() - 1.0) > perThousand))
  {
    std::ostringstream message;
    message << "**************************************************************" << G4endl;
    message << " Bad Normal in G4VIntersectionLocator::GetGlobalSurfaceNormal " << G4endl;
    ...
    message << "**************************************************************";
    G4Exception("G4VIntersectionLocator::GetGlobalSurfaceNormal()", "GeomNav1002", JustWarning,
                message);
  }
#endif
...
// ReportTrialStep.
//
void G4VIntersectionLocator::ReportTrialStep(G4int step_no, const G4ThreeVector& ChordAB_v,
                                             const G4ThreeVector& ChordEF_v,
                                             const G4ThreeVector& NewMomentumDir,
                                             const G4ThreeVector& NormalAtEntry, G4bool validNormal)
{
  ...
  outStream << std::setw(6) << " Step# " << std::setw(17) << " |ChordEF|(mag)" << "  "
            << std::setw(18) << " uMomentum.Normal" << "  " << std::setw(18) << " uMomentum.ABdir "
            << "  " << std::setw(16) << " AB-dist         " << " "
            << " Chord Vector (EF) " << G4endl;
  outStream.precision(7);
  outStream << " " << std::setw(5) << step_no << " " << std::setw(18) << ChordEF_v.mag() << " "
            << std::setw(18) << MomDir_dot_Norm << " " << std::setw(18) << MomDir_dot_ABchord << " "
            << std::setw(12) << ABchord_length << " " << ChordEF_v << G4endl;
  outStream << " MomentumDir= " << " " << NewMomentumDir << " Normal at Entry E= " << NormalAtEntry
            << " AB chord =   " << ChordAB_v << G4endl;
  ...
  if ((std::fabs(NormalAtEntry.mag2() - 1.0) > perThousand))
  {
    ...
    G4Exception("G4VIntersectionLocator::ReportTrialStep()", "GeomNav1002", JustWarning, message);
  }
  return;
}
...
//
G4bool G4VIntersectionLocator::LocateGlobalPointWithinVolumeAndCheck(const G4ThreeVector& position)
{
  G4bool good = true;
  ...
  if (fCheckMode)
  {
    ...
    {
      std::ostringstream message;
      message << "Position located " << (inMother == kSurface ? " on Surface " : " outside ")
              << "expected volume" << G4endl
              << "  Safety (from Outside) = " << motherSolid->DistanceToIn(localPosition);
      G4Exception("G4VIntersectionLocator::LocateGlobalPointWithinVolumeAndCheck()", "GeomNav1002",
                  JustWarning, message);
    }
    ...
    if ((nextPhysical != motherPhys) || (nextPhysical->GetCopyNo() != motherCopyNo))
    {
      G4Exception("G4VIntersectionLocator::LocateGlobalPointWithinVolumeAndCheck()", "GeomNav1002",
                  JustWarning, "Position located outside expected volume.");
    }
...
void G4VIntersectionLocator::LocateGlobalPointWithinVolumeCheckAndReport(
  const G4ThreeVector& position, const G4String& CodeLocationInfo, G4int /* CheckMode */)
{
  ...
  {
    std::ostringstream message;
    message << "Failed point location." << G4endl << "   Code Location info: " << CodeLocationInfo;
...
void G4VIntersectionLocator::ReportReversedPoints(
  std::ostringstream& msg, const G4FieldTrack& StartPointVel, const G4FieldTrack& EndPointVel,
  G4double NewSafety, G4double epsStep, const G4FieldTrack& A_PtVel, const G4FieldTrack& B_PtVel,
  const G4FieldTrack& SubStart_PtVel, const G4ThreeVector& E_Point,
  const G4FieldTrack& ApproxIntersecPointV, G4int substep_no, G4int substep_no_p, G4int depth)
{
  ...
  G4VIntersectionLocator::printStatus(A_PtVel, B_PtVel, -1.0, NewSafety, substep_no, msg,
                                      verboseLevel);
  msg << "Error in advancing propagation." << G4endl
      ...
      << "  to len(B) = " << B_PtVel.GetCurveLength() << G4endl << "      Curve distance is "
      << curveDist / CLHEP::millimeter << " mm " << G4endl << "      Point A' (start) is "
      << A_PtVel << G4endl << "      Point B' (end)   is " << B_PtVel << G4endl;
  ...
  msg << " Point A[0] (Curve   start) is " << StartPointVel << G4endl
      << " Point S    (Sub     start) is " << SubStart_PtVel << " Point A'   (Current start) is "
      << A_PtVel << G4endl << " Point E    (Trial Point)   is " << E_Point << G4endl
      << " Point F    (Intersection)  is " << ApproxIntersecPointV << G4endl
      << " Point B'   (Current end)   is " << B_PtVel << G4endl << " Point B[0] (Curve   end)   is "
      << EndPointVel << G4endl << G4endl << " LocateIntersection parameters are : " << G4endl
...
// ReportProgress.
//
void G4VIntersectionLocator::ReportProgress(std::ostream& oss, const G4FieldTrack& StartPointVel,
                                            const G4FieldTrack& EndPointVel, G4int substep_no,
                                            const G4FieldTrack& A_PtVel,
                                            const G4FieldTrack& B_PtVel, G4double safetyLast,
                                            G4int depth)

{
  ...
  printStatus(StartPointVel, EndPointVel, -1.0, -1.0, -1, oss, verboseLevel);
  oss << " * Start and end-point of requested Step:" << G4endl;
  oss << " ** State of point A: ";
  printStatus(A_PtVel, A_PtVel, -1.0, safetyPrev, substep_no - 1, oss, verboseLevel);
  oss << " ** State of point B: ";
  printStatus(A_PtVel, B_PtVel, -1.0, safetyLast, substep_no, oss, verboseLevel);
}
...
void G4VIntersectionLocator::ReportImmediateHit(const char* MethodName,
                                                ...
                                                const G4ThreeVector& TrialPoint, G4double tolerance,
                                                unsigned long int numCalls)
{
  ...
  if ((TrialPoint - StartPosition).mag2() < tolerance * tolerance)
  {
    ...
    if ((StartPosition - lastStart).mag() < tolerance)
    {
      ...
      ++numUnmoved;
      ++numStill;
      G4cout << " { Unmoved: " << " still#= " << numStill << " total # = " << numUnmoved << " } - ";
    }
    else
    {
      numStill = 0;
