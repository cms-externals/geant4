...
//
const char* G4LocatorChangeRecord::fNameChangeLocation[] = {"Invalid",
                                                            "Unknown",
                                                            "Initialising",
                                                            "IntersectsAF",
                                                            "IntersectsFB",  // 5
                                                            "NoIntersections-AForFB",
                                                            "RecalculatedB",  // 2
                                                            "InsertingMidPoint",
                                                            "RecalculatedB-2ndHalf",  // 2
                                                            "Level Pop"};

// --------------------------------------------------------------------
//
G4LocatorChangeRecord::G4LocatorChangeRecord(EChangeLocation codeLocation, G4int iter,
                                             unsigned int count, const G4FieldTrack& fieldTrack)
  : fCodeLocation(codeLocation), fIteration(iter), fEventCount(count), fFieldTrack(fieldTrack)
{}

// --------------------------------------------------------------------
//
std::ostream& G4LocatorChangeRecord::ReportVector(std::ostream& os, const std::string& name,
                                                  const std::vector<G4LocatorChangeRecord>& vecRec)
{
  ...
  auto itRec = std::vector<G4LocatorChangeRecord>::const_iterator(vecRec.cbegin());

  os << setw(7) << "Change#" << "  " << setw(4) << "Iter" << "  " << std::left << setw(prec + 9)
     << "Length" << "  " << setw(15) << "Code-Location" << "  " << G4endl;
  os << "=====================================================================" << G4endl;

  do
  {
    auto locationCode = (*itRec).GetLocation();
    os << std::internal << setw(7) << (*itRec).GetCount() << "  "  // Event Count
       << setw(4) << (*itRec).GetIteration() << "  " << std::left << setw(prec + 9)
       << (*itRec).GetLength() << "  " << setw(2) << locationCode << "  "  // location enum
       << setw(15) << fNameChangeLocation[locationCode] << std::internal;
    os << G4endl;
    ++itRec;
...
//
std::ostream&
G4LocatorChangeRecord::ReportEndChanges(std::ostream& os,
                                        ...
                                        const std::vector<G4LocatorChangeRecord>& endB)
{
  ...
  os << setw(7) << "Change#" << "  " << setw(4) << "Iter" << "  " << setw(20) << "CodeLocation"
     << "  " << setw(prec + 9) << "Length-A (start)" << "  " << setw(prec + 9) << "Length-B (end)"
     << "  " << G4endl;
  os << "=====================================================================";
  ...
  G4int maxEvent = std::max(startA[startA.size() - 1].GetCount(), endB[endB.size() - 1].GetCount());
  G4int prevA = -1;
  G4int prevB = -1;
  ...
  do
  {
    ...
    if (((G4int)eventA > prevA) && ((G4int)eventB > prevB))
    {
      ...
      os << G4endl;
      os << setw(7) << eventA << "  " << setw(4) << (*itrecA).GetIteration() << "  " << setw(3)
         << codeLocA << " " << setw(15) << fNameChangeLocation[codeLocA] << " " << setw(prec + 9)
         << (*itrecA).GetLength() << "  " << setw(prec + 9) << (*itrecB).GetLength() << "  ";
      if (confirm)
      {
        os << setw(4) << (*itrecB).GetIteration() << "  " << setw(15) << (*itrecB).GetLocation();
      }
    }
    else
    {
      if ((G4int)eventA > prevA)
      {
        auto codeLocA = (*itrecA).GetLocation();
        os << G4endl;
        os << setw(7) << (*itrecA).GetCount() << "  " << setw(4) << (*itrecA).GetIteration() << "  "
           << setw(3) << codeLocA << " " << setw(15) << fNameChangeLocation[codeLocA] << " "
           << setw(prec + 9) << (*itrecA).GetLength() << "  " << setw(prec + 9) << "       "
           << "  ";
      }
      else
      {
        ...
        os << G4endl;
        os << setw(7) << eventB << "  " << setw(4) << (*itrecB).GetIteration() << "  " << setw(3)
           << codeLocB << " " << setw(15) << fNameChangeLocation[codeLocB] << " " << setw(prec + 9)
    ...
    if (nextA != startA.end())
    {
      nextAct = (*nextA).GetCount();
    }
    if (nextB != endB.end())
    {
      nextBct = (*nextB).GetCount();
    }
    ...
    {
      os << G4endl;
      os << "  Checking isLastA= " << isLastA << " vs expected :  " << (itrecA == startA.end());
      os << " BAD --- ERROR " << G4endl;
    }
    if (isLastB != (nextB == endB.end()))
    {
      os << G4endl;
      os << "  Checking isLastB= " << isLastB << " vs expected :  " << (itrecB == endB.end());
      os << " BAD --- ERROR " << G4endl;
    }
...
std::ostream& G4LocatorChangeRecord::StreamInfo(std::ostream& os) const
{
  ...
  os << "  count = " << fEventCount << "  iter= " << fIteration
     << "  Location code = " << fCodeLocation << "  Length = " << GetLength() << G4endl;
  os.precision(oldprc);
  return os;
}
