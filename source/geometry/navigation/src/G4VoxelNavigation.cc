...
//
G4double
G4VoxelNavigation::ComputeStep(const G4ThreeVector& localPoint, const G4ThreeVector& localDirection,
                               const G4double currentProposedStepLength, G4double& newSafety,
                               /* const */ G4NavigationHistory& history, G4bool& validExitNormal,
      ...
      if (!fBList.IsBlocked(sampleNo))
      {
        ...
        if (samplePhysical != blockedExitedVol)
        {
          ...
          if (sampleSafety <= ourStep)
          {
            ...
            if (fCheck)
            {
              fLogger->PrintDaughterLog(sampleSolid, samplePoint, sampleSafety, true,
            ...
            if (sampleStep <= ourStep)
            {
              ...
              *pBlockedPhysical = samplePhysical;
              ...
              if (fCheck)
              {
                fLogger->AlongComputeStepLog(sampleSolid, samplePoint, sampleDirection,
                                             localDirection, sampleSafety, sampleStep);
              }
#endif
            }
#ifdef G4VERBOSE
            if (fCheck && (sampleStep < kInfinity) && (sampleStep >= motherStep))
            {
              ...
              fLogger->CheckDaughterEntryPoint(sampleSolid, samplePoint, sampleDirection,
                                               motherSolid, localPoint, localDirection, motherStep,
                                               sampleStep);
            }
#endif
          }
          ...
          else  // ie if sampleSafety > outStep
          {
            if (fCheck)
            {
              fLogger->PrintDaughterLog(sampleSolid, samplePoint, sampleSafety, false,
    ...
    if (initialNode)
    {
      ...
      else
      {
        ...
        if (motherSafety <= ourStep)
        {
          ...
          if (fCheck)
          {
            fLogger->PostComputeStepLog(motherSolid, localPoint, localDirection, motherStep,
                                        motherSafety);
...
G4double G4VoxelNavigation::ComputeVoxelSafety(const G4ThreeVector& localPoint) const
{
  ...
  minCurCommonDelta = localPoint(curHeaderAxis) - curHeader->GetMinExtent() - curNodeOffset;
                                          ...
                                          const G4double maxLength)
{
  ...
  if (motherSafety == 0.0)
  {
    ...
    if (insideMother == kOutside)
    {
      ...
      G4Exception("G4VoxelNavigation::ComputeSafety()", "GeomNav0003", JustWarning, message);
    }
    ...
    if (insideMother == kInside)  // && fVerbose )
    {
      ...
      G4Exception("G4VoxelNavigation::ComputeSafety()", "GeomNav0003", JustWarning, messageIn);
    }
#endif
  ...
  for (contentNo = curNoVolumes - 1; contentNo >= 0; contentNo--)
  {
    ...
    if (fCheck)
    {
      fLogger->ComputeSafetyLog(sampleSolid, samplePoint, sampleSafety, false, 0);
    }
#endif
  }
  voxelSafety = ComputeVoxelSafety(localPoint);
