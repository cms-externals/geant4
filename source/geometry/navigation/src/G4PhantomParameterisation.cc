...
//------------------------------------------------------------------
void G4PhantomParameterisation::BuildContainerSolid(G4VPhysicalVolume* pMotherPhysical)
{
...
//------------------------------------------------------------------
G4VSolid* G4PhantomParameterisation::ComputeSolid(const G4int, G4VPhysicalVolume* pPhysicalVol)
{
  return pPhysicalVol->GetLogicalVolume()->GetSolid();
}

//------------------------------------------------------------------
G4Material* G4PhantomParameterisation::ComputeMaterial(const G4int copyNo, G4VPhysicalVolume*,
                                                       const G4VTouchable*)
{
  CheckCopyNo(copyNo);
...
//------------------------------------------------------------------
std::size_t G4PhantomParameterisation::GetMaterialIndex(std::size_t nx, std::size_t ny,
                                                        std::size_t nz) const
{
...
//------------------------------------------------------------------
G4Material* G4PhantomParameterisation::GetMaterial(std::size_t nx, std::size_t ny,
                                                   std::size_t nz) const
{
...
//------------------------------------------------------------------
void G4PhantomParameterisation::ComputeVoxelIndices(const G4int copyNo, std::size_t& nx,
...
//------------------------------------------------------------------
void G4PhantomParameterisation::CheckVoxelsFillContainer(G4double contX, G4double contY,
                                                         G4double contZ) const
{
      ...
      || std::fabs(contZ - fNoVoxelsZ * fVoxelHalfZ) >= toleranceForError)
  {
    ...
    message << "Voxels do not fully fill the container: " << fContainerSolid->GetName() << G4endl
    ...
    G4Exception("G4PhantomParameterisation::CheckVoxelsFillContainer()", "GeomNav0002",
           ...
           || std::fabs(contZ - fNoVoxelsZ * fVoxelHalfZ) >= toleranceForWarning)
  {
    ...
    G4Exception("G4PhantomParameterisation::CheckVoxelsFillContainer()", "GeomNav1002", JustWarning,
                message);
  }
}
                                              ...
                                              const G4ThreeVector& localDir)
{
  ...
  if (fContainerSolid->Inside(localPoint) == kOutside)
  {
    ...
    {
      std::ostringstream message;
      message << "Point outside voxels!" << G4endl << "        localPoint - " << localPoint
              << " - is outside container solid: " << fContainerSolid->GetName() << G4endl
              << "DIFFERENCE WITH PHANTOM WALLS X: " << std::fabs(localPoint.x()) - fContainerWallX
              << " Y: " << std::fabs(localPoint.y()) - fContainerWallY
              << " Z: " << std::fabs(localPoint.z()) - fContainerWallZ;
      G4Exception("G4PhantomParameterisation::GetReplicaNo()", "GeomNav0003", FatalErrorInArgument,
                  message);
    }
  }
  ...
  if (!isOK)
  {
    ...
    {  // only if difference is big
      ...
      message << "Corrected the copy number! It was negative or too big" << G4endl
              ...
              << "          Voxel container size: " << fContainerWallX << " " << fContainerWallY
              << " " << fContainerWallZ << G4endl
              << "          LocalPoint - wall: " << localPoint.x() - fContainerWallX << " "
              << localPoint.y() - fContainerWallY << " " << localPoint.z() - fContainerWallZ;
      G4Exception("G4PhantomParameterisation::GetReplicaNo()", "GeomNav1002", JustWarning, message);
    }
...
void G4PhantomParameterisation::CheckCopyNo(const G4long copyNo) const
{
  ...
  {
    std::ostringstream message;
    message << "Copy number is negative or too big!" << G4endl << "        Copy number: " << copyNo
            << G4endl << "        Total number of voxels: " << fNoVoxels;
    G4Exception("G4PhantomParameterisation::CheckCopyNo()", "GeomNav0002", FatalErrorInArgument,
                message);
  }
}
