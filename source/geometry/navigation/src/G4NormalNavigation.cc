  ...
  for (sampleNo = localNoDaughters - 1; sampleNo >= 0; sampleNo--)
  {
    ...
    if (samplePhysical != blockedExitedVol)
    {
      ...
      if (sampleSafety <= ourStep)
      {
        ...
        if (fCheck)
        {
          fLogger->PrintDaughterLog(sampleSolid, samplePoint, sampleSafety, true, sampleDirection,
                                    sampleStep);
        }
#endif
        if (sampleStep <= ourStep)
        {
          ...
          *pBlockedPhysical = samplePhysical;
          ...
          if (fCheck)
          {
            fLogger->AlongComputeStepLog(sampleSolid, samplePoint, sampleDirection, localDirection,
        ...
        if (fCheck && (sampleStep < kInfinity) && (sampleStep >= motherStep))
        {
          ...
          fLogger->CheckDaughterEntryPoint(sampleSolid, samplePoint, sampleDirection, motherSolid,
                                           localPoint, localDirection, motherStep, sampleStep);
        }
#endif
  ...
  else
  {
    ...
    if (motherSafety <= ourStep)
    {
      ...
      else  // check_mode
      {
        ...
        {
          fLogger->CheckAndReportBadNormal(motherExitNormal, localPoint, localDirection, motherStep,
                                           motherSolid, "From motherSolid::DistanceToOut");
        }
      }
#endif
...
G4double G4NormalNavigation::ComputeSafety(const G4ThreeVector& localPoint,
                                           const G4NavigationHistory& history, const G4double)
{
