...
// ********************************************************************
//
G4double G4SafetyCalculator::SafetyInCurrentVolume(const G4ThreeVector& pGlobalpoint,
                                                   G4VPhysicalVolume* physicalVolume,
                                                   const G4double pMaxLength, G4bool /* verbose */)
{
  G4double safety = 0.0;
  ...
  G4bool endpointOnSurface = fNavigator.EnteredDaughterVolume() || fNavigator.ExitedMotherVolume();
  ...
  if (motherPhysical != physicalVolume)
  {
    ...
    G4Exception("G4SafetyCalculator::SafetyInCurrentVolume", "GeomNav0001", FatalException, msg,
  ...
  if (!(endpointOnSurface && stayedOnEndpoint))
  {
    ...
    switch (dtype)
    {
      case kNormal:
        ...
        {
          // New way: best safety
          safety = fVoxelSafety.ComputeSafety(localPoint, *motherPhysical, pMaxLength);
        }
        else
        {
      ...
      case kParameterised:
        ...
        break;
      case kReplica:
        safety = freplicaNav.ComputeSafety(pGlobalpoint, localPoint, fNavHistory, pMaxLength);
        break;
      case kExternal:
        safety = fpExternalNav->ComputeSafety(localPoint, fNavHistory, pMaxLength);
        break;
    }
                                                 ...
                                                 G4VPhysicalVolume* motherPhysical)
{
  ...
  switch (CharacteriseDaughters(motherLogical))
  {
    ...
    case kReplica:
      ...
      break;
    case kExternal:
      fpExternalNav->RelocateWithinVolume(motherPhysical, pointLocal);
      break;
  }
}
...
// CompareSafetyValues
// ********************************************************************
void G4SafetyCalculator::CompareSafetyValues(G4double oldSafety, G4double newValue,
                                             G4VPhysicalVolume* motherPhysical,
                                             const G4ThreeVector& globalPoint, G4bool keepState,
                                             G4double maxLength, G4bool enteredDaughterVol,
                                             G4bool exitedMotherVol)
{
  ...
  if (std::fabs(newValue - oldSafety) > reportThreshold * oldSafetyPlus)
  {
    ...
    if (enteredDaughterVol)
    {
      msg << "  ( Just Entered new daughter volume. ) ";
    }
    if (exitedMotherVol)
    {
      msg << "  ( Just Exited previous volume. ) ";
    }
    msg << G4endl;
    msg << " Safeties:   old= " << std::setprecision(12) << oldSafety << "   trial " << newValue
    ...
    else
    {
      ...
      msg << " Location -  Global coordinates: " << globalPoint << "  volume= '"
      ...
      {
        msg << "     d= " << i << " " << std::setw(32) << fNavHistory.GetVolume(i)->GetName()
    ...
    G4double redo = SafetyInCurrentVolume(globalPoint, motherPhysical, maxLength, true);
    ...
    G4Exception("G4SafetyCalculator::CompareSafetyValues()", "GeomNav1007", severity, msg);
  }
}
