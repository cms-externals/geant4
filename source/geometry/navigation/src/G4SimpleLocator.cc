...
G4SimpleLocator::G4SimpleLocator(G4Navigator* theNavigator) : G4VIntersectionLocator(theNavigator)
{}
...
//
G4bool
G4SimpleLocator::EstimateIntersectionPoint(const G4FieldTrack& CurveStartPointVelocity,  // A
                                           const G4FieldTrack& CurveEndPointVelocity,  // B
                                           const G4ThreeVector& TrialPoint,  // E
                                           ...
                                           G4ThreeVector& fPreviousSftOrigin)  // In/Out
{
  ...
  {
    G4Exception("G4SimpleLocator::EstimateIntersectionPoint()", "GeomNav1002", JustWarning,
  ...
  do
  {
    ...
    {
      G4Exception("G4SimpleLocator::EstimateIntersectionPoint()", "GeomNav0003", FatalException,
    ...
    //
    G4bool adequate_angle = (MomDir_dot_Norm >= 0.0) || (!validNormalAtE);  // Invalid
        ...
        || (EF_dist2 <= kCarTolerance * kCarTolerance))
    {
      ...
      if (GetAdjustementOfFoundIntersection())
      {
        ...
        G4bool goodCorrection = AdjustmentOfFoundIntersection(
          Point_A, CurrentE_Point, CurrentF_Point, MomentumDir, last_AF_intersection, IP, NewSafety,
    ...
    else  // E is NOT close enough to the curve (ie point F)
    {
      ...
      curveDist = CurrentB_PointVelocity.GetCurveLength() - CurrentA_PointVelocity.GetCurveLength();
      ...
      if (curveDist < 0.0)
      {
        ...
        message << "Error in advancing propagation." << G4endl << "        Point A (start) is "
                << CurrentA_PointVelocity << G4endl << "        Point B (end)   is "
                << CurrentB_PointVelocity << G4endl << "        Curve distance is " << curveDist
                << G4endl << G4endl << "The final curve point is not further along"
    ...
    if (substep_no >= trigger_substepno_print)
    {
      G4cout << "Difficulty in converging in "
             << "G4SimpleLocator::EstimateIntersectionPoint():" << G4endl
  ...
  if ((substep_no >= max_substeps) && !there_is_no_intersection && !found_approximate_intersection)
  {
    ...
    printStatus(CurveStartPointVelocity, CurveEndPointVelocity, -1.0, NewSafety, 0);
    G4cout << G4endl << "        Start and end-point of current Sub-Step:" << G4endl;
    printStatus(CurrentA_PointVelocity, CurrentA_PointVelocity, -1.0, NewSafety, substep_no - 1);
    printStatus(CurrentA_PointVelocity, CurrentB_PointVelocity, -1.0, NewSafety, substep_no);

    std::ostringstream message;
    message << "Convergence is requiring too many substeps: " << substep_no << G4endl
