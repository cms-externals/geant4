...
G4Navigator::G4Navigator()
{
  ...
  if (fVerbose > 2)
  {
    G4cout << " G4Navigator parameters: Action Threshold (No Zero Steps) = "
           ...
           << "  Abandon Threshold (No Zero Steps) = " << fAbandonThreshold_NoZeroSteps << G4endl;
  }
...
// ********************************************************************
//
G4VPhysicalVolume* G4Navigator::ResetHierarchyAndLocate(const G4ThreeVector& p,
...
// ********************************************************************
//
G4VPhysicalVolume* G4Navigator::LocateGlobalPointAndSetup(const G4ThreeVector& globalPoint,
                                                          ...
                                                          const G4bool ignoreDirection)
{
  ...
  G4bool considerDirection =
    (pGlobalDirection != nullptr) && ((!ignoreDirection) || fLocatedOnEdge);
      ...
      else if (fEntering)
      {
        switch (VolumeType(fBlockedPhysicalVolume))
        {
          ...
          case kParameterised:
            ...
            break;
          case kExternal:
            G4Exception("G4Navigator::LocateGlobalPointAndSetup()", "GeomNav0001", FatalException,
  ...
  do
  {
    ...
    switch (CharacteriseDaughters(targetLogical))
    {
      case kNormal:
        if (targetLogical->GetVoxelHeader() != nullptr)  // use optimised navigation
        {
          noResult = GetVoxelNavigator().LevelLocate(
            fHistory, fBlockedPhysicalVolume, fBlockedReplicaNo, globalPoint, pGlobalDirection,
            considerDirection, localPoint);
        }
      ...
      case kExternal:
        noResult =
          fpExternalNav->LevelLocate(fHistory, fBlockedPhysicalVolume, fBlockedReplicaNo,
                                     globalPoint, pGlobalDirection, considerDirection, localPoint);
        break;
    }
...
void G4Navigator::LocateGlobalPointWithinVolume(const G4ThreeVector& pGlobalpoint)
{
  ...
  switch (CharacteriseDaughters(motherLogical))
  {
    ...
    case kReplica:
      ...
      break;
    case kExternal:
      fpExternalNav->RelocateWithinVolume(motherPhysical, fLastLocatedPointLocal);
      break;
  }
...
#  endif
  ...
  if (fHistory.GetTopVolumeType() != kReplica)
  {
    switch (CharacteriseDaughters(motherLogical))
    {
      case kNormal:
        ...
        {
          Step = GetVoxelNavigator().ComputeStep(fLastLocatedPointLocal, localDirection,
        ...
        else
        {
          ...
          else  // Regular (non-voxelised) structure
          {
            ...
            {
              G4Exception("G4Navigator::ComputeStep()", "GeomNav1001", JustWarning,
      ...
      case kParameterised:
        ...
        break;
      case kReplica:
        G4Exception("G4Navigator::ComputeStep()", "GeomNav0001", FatalException,
                    "Not applicable for replicated volumes.");
        break;
      case kExternal:
        Step = fpExternalNav->ComputeStep(fLastLocatedPointLocal, localDirection,
                                          pCurrentProposedStepLength, pNewSafety, fHistory,
                                          fValidExitNormal, fExitNormal, fExiting, fEntering,
                                          &fBlockedPhysicalVolume, fBlockedReplicaNo);
        break;
  ...
  else
  {
    ...
    Step = freplicaNav.ComputeStep(pGlobalpoint, pDirection, fLastLocatedPointLocal, localDirection,
                                   pCurrentProposedStepLength, pNewSafety, fHistory,
                                   fValidExitNormal, fCalculatedExitNormal, fExitNormal, fExiting,
  ...
  if (fLastStepWasZero)
  {
    ...
    if (act || inform)
    {
      ...
      if (actAndReport || abandon || inform)
      {
        ...
        message << "  Track stuck, not moving for " << fNumberZeroSteps << " steps." << G4endl
                << "  Current  phys volume: '" << motherPhysical->GetName() << "'" << G4endl
        ...
        {
          message << "  Likely geometry overlap - else navigation problem !" << G4endl;
        }
        if (abandon)  // i.e. fNumberZeroSteps >= fAbandonThreshold_NoZeroSteps
        {
          ...
          if (fWarnPush)
          {
            CheckOverlapsIterative(motherPhysical);
          }
#endif
          message << " Track *abandoned* due to excessive number of Zero steps."
                  << " Event aborted. " << G4endl << G4endl;
          G4Exception("G4Navigator::ComputeStep()", "GeomNav0003", EventMustBeAborted, message);
        }
        else
        {
#ifdef G4VERBOSE
          if (actAndReport)  // (!fPushed => !wasPushed) && (fWarnPush))
          {
            ...
            G4Exception("G4Navigator::ComputeStep()", "GeomNav1002", JustWarning, message);
          }
#endif
#ifdef G4DEBUG_NAVIGATION
          else
          {
            ...
            {
              message << ", nav-comp-step calls # " << sNavCScalls << ", Step= " << Step << G4endl;
              G4cout << message.str();
            }
          }
#endif
  ...
  if (fExiting)
  {
    ...
    else
    {
      ...
      else
      {
        ...
        G4Exception("G4Navigator::ComputeStep()", "GeomNav0003", JustWarning, desc,
...
G4ThreeVector G4Navigator::GetLocalExitNormal(G4bool* valid)
{
  ...
  if (fLastTriedStepComputation)
  {
        ...
        // The following code should also work in case of Replica
        {
          ...
          else
          {
            ...
            if ((fVerbose == 1) && (fCheck))
            {
              ...
              G4Exception("G4Navigator::GetLocalExitNormal()", "GeomNav1001", JustWarning, message);
            }
#endif
          }
    ...
    {
      *valid = false;
      G4Exception("G4Navigator::GetLocalExitNormal()", "GeomNav0003", JustWarning,
  ...
  else  //  ( ! fLastTriedStepComputation ) i.e. last call was to Locate
  {
    if (EnteredDaughterVolume())
    {
      ...
      if (std::fabs(ExitNormal.mag2() - 1.0) > kToleranceNormalCheck)
      {
        ...
        G4Exception("G4Navigator::GetLocalExitNormal()", "GeomNav0003", FatalException, desc,
    ...
    else
    {
      ...
      else  // We are not at a boundary. ExitNormal remains (0,0,0)
      {
        *valid = false;
        ...
        G4Exception("G4Navigator::GetLocalExitNormal()", "GeomNav0003", JustWarning, message);
      }
    }
  }
  return ExitNormal;
...
// ********************************************************************
//
G4AffineTransform G4Navigator::GetMotherToDaughterTransform(G4VPhysicalVolume* pEnteringPhysVol,
                                                            ...
                                                            EVolume enteringVolumeType)
{
  switch (enteringVolumeType)
  {
    ...
    case kReplica:  // Sets the transform in the Replica - tbc
      G4Exception("G4Navigator::GetMotherToDaughterTransform()", "GeomNav0001", FatalException,
...
// ********************************************************************
//
G4ThreeVector G4Navigator::GetLocalExitNormalAndCheck(
#ifdef G4DEBUG_NAVIGATION
...
// ********************************************************************
//
G4ThreeVector G4Navigator::GetGlobalExitNormal(const G4ThreeVector& IntersectPointGlobal,
                                               G4bool* pNormalCalculated)
{
  ...
  if (usingStored)
  {
    ...
    else
    {
      ...
      message << " WARNING> Expected normal-global-frame to be valid, "
              << " i.e. a unit vector!" << G4endl << "  - but |normal|   = " << std::sqrt(normMag2)
              << "  - and |normal|^2 = " << normMag2 << G4endl << " which differs from 1.0 by "
              << normMag2 - 1.0 << G4endl << "   n = " << fExitNormalGlobalFrame << G4endl
  ...
  else
  {
    ...
    *pNormalCalculated = fCalculatedExitNormal;
    ...
    if ((!validNormal) && !fCalculatedExitNormal)
    {
      ...
      G4Exception("G4Navigator::GetGlobalExitNormal()", "GeomNav0003", JustWarning, edN,
    ...
    if (validNormal && (std::fabs(localMag2 - 1.0)) > kToleranceNormalCheck)
    {
      ...
      edN << "G4Navigator::GetGlobalExitNormal: "
          ...
          << " vec = " << globalNormal << G4endl << "  Global point: " << IntersectPointGlobal
          << G4endl;
      ...
      if (candLog != nullptr)
      {
        edN << "  Solid: " << candLog->GetSolid()->GetName()
            << ", Type: " << candLog->GetSolid()->GetEntityType() << G4endl << *candLog->GetSolid();
      }
#endif
      G4Exception("G4Navigator::GetGlobalExitNormal()", "GeomNav0003", JustWarning, edN,
  ...
  if (usingStored)
  {
    ...
    if (diffNorm.mag2() > kToleranceNormalCheck)
    {
      ...
      G4Exception("G4Navigator::GetGlobalExitNormal()", "GeomNav0003", JustWarning, edDfn);
    }
  }
#endif
...
// ********************************************************************
//
G4double G4Navigator::ComputeSafety(const G4ThreeVector& pGlobalpoint, const G4double pMaxLength,
                                    const G4bool)
{
  G4VPhysicalVolume* motherPhysical = fHistory.GetTopVolume();
  G4double safety = 0.0;
...
void G4Navigator::ComputeStepLog(const G4ThreeVector& pGlobalpoint, G4double moveLenSq) const
{
  ...
  const G4double fAccuracyForWarning = kCarTolerance, fAccuracyForException = 1000 * kCarTolerance;

  G4ThreeVector OriginalGlobalpoint =
    fHistory.GetTopTransform().InverseTransformPoint(fLastLocatedPointLocal);
...
G4bool G4Navigator::CheckOverlapsIterative(G4VPhysicalVolume* vol)
{
  ...
  while (ntrials-- > 0 && !foundOverlap)
  {
    if (fVerbose > 1)
    {
      G4cout << " ** Running overlap checks in volume " << vol->GetName()
             << " with length = " << trialLength << G4endl;
    }
    foundOverlap = vol->CheckOverlaps(nPoints, trialLength, fVerbose != 0, numOverlaps);
    trialLength *= 0.1;
    if (trialLength <= 1.0e-5)
    {
      numOverlaps = 1;
    }
  }
