...
G4BrentLocator::G4BrentLocator(G4Navigator* theNavigator) : G4VIntersectionLocator(theNavigator)
{
...
//
G4bool
G4BrentLocator::EstimateIntersectionPoint(const G4FieldTrack& CurveStartPointVelocity,  // A
                                          const G4FieldTrack& CurveEndPointVelocity,  // B
                                          const G4ThreeVector& TrialPoint,  // E
...

{
  ...
  {
    G4Exception("G4BrentLocator::EstimateIntersectionPoint()", "GeomNav1002", JustWarning,
  ...
  *ptrInterMedFT[0] = CurveEndPointVelocity;
  ...
  do  // Loop checking, 07.10.2016, J.Apostolakis
  {
    ...
    {  // REPEAT param
      ...
      {
        G4Exception("G4BrentLocator::EstimateIntersectionPoint()", "GeomNav0003", FatalException,
          ...
          || (EF_dist2 <= kCarTolerance * kCarTolerance))
      {
        ...
        if (GetAdjustementOfFoundIntersection())
        {
          ...
          G4bool goodCorrection = AdjustmentOfFoundIntersection(
            ...
            NewSafety, fPreviousSafety, fPreviousSftOrigin);
          if (goodCorrection)
          {
            IntersectedOrRecalculatedFT = ApproxIntersecPointV;
      ...
      else  // E is NOT close enough to the curve (ie point F)
      {
        ...
        G4bool Intersects_AF =
          IntersectChord(Point_A, CurrentF_Point, NewSafety, fPreviousSafety, fPreviousSftOrigin,
                         stepLengthAF, PointG, &usedNavigatorAF);
        ...
        if (Intersects_AF)
        {
          ...
          ApproxIntersecPointV = GetChordFinderFor()->ApproxCurvePointS(
            CurrentA_PointVelocity, CurrentB_PointVelocity, EndPoint, CurrentE_Point,
            CurrentF_Point, PointG, true, GetEpsilonStepFor());
        ...
        else  // not Intersects_AF
        {
          ...
          G4bool Intersects_FB =
            IntersectChord(CurrentF_Point, Point_B, NewSafety, fPreviousSafety, fPreviousSftOrigin,
                           stepLengthFB, PointH, &usedNavigatorFB);
          if (Intersects_FB)
          {
            ...
            ApproxIntersecPointV = GetChordFinderFor()->ApproxCurvePointS(
              CurrentA_PointVelocity, CurrentB_PointVelocity, InterMed, CurrentE_Point,
              CurrentF_Point, PointH, false, GetEpsilonStepFor());
          ...
          else  // not Intersects_FB
          {
            ...
            else
            {
              if (depth == 0)
              {
                ...
                SubStart_PointVelocity = CurrentA_PointVelocity;
                ApproxIntersecPointV = GetChordFinderFor()->ApproxCurvePointV(
                  CurrentA_PointVelocity, CurrentB_PointVelocity, CurrentE_Point,
                  GetEpsilonStepFor());
        ...
        linDistSq =
          (CurrentB_PointVelocity.GetPosition() - CurrentA_PointVelocity.GetPosition()).mag2();
        curveDist =
          CurrentB_PointVelocity.GetCurveLength() - CurrentA_PointVelocity.GetCurveLength();
        ...
        if (curveDist < 0.0)
        {
          ...
          message << "Error in advancing propagation." << G4endl
                  ...
                  << "        Point A (start) is " << CurrentA_PointVelocity << G4endl
                  << "        Point B (end)   is " << CurrentB_PointVelocity << G4endl
                  << "        Curve distance is " << curveDist << G4endl << G4endl
                  << "The final curve point is not further along"
                  << " than the original!" << G4endl;

          if (recalculatedEndPoint)
          {
            message << "Recalculation of EndPoint was called with fEpsStep= " << GetEpsilonStepFor()
                    << G4endl;
          }
          oldprc = G4cerr.precision(20);
          message << " Point A (Curve start)     is " << CurveStartPointVelocity << G4endl
                  << " Point B (Curve   end)     is " << CurveEndPointVelocity << G4endl
                  << " Point A (Current start)   is " << CurrentA_PointVelocity << G4endl
                  << " Point B (Current end)     is " << CurrentB_PointVelocity << G4endl
                  << " Point S (Sub start)       is " << SubStart_PointVelocity << G4endl
                  << " Point E (Trial Point)     is " << CurrentE_Point << G4endl
                  << " Old Point F(Intersection) is " << CurrentF_Point << G4endl
                  << " New Point F(Intersection) is " << ApproxIntersecPointV << G4endl
                  << "        LocateIntersection parameters are : Substep no= " << substep_no
                  << G4endl << "        Substep depth no= " << substep_no_p << " Depth= " << depth
                  << G4endl << "        Restarted no= " << restartB
                  << " Epsilon= " << GetEpsilonStepFor()
                  << " DeltaInters= " << GetDeltaIntersectionFor();
      ...
      if (substep_no >= trigger_substepno_print)
      {
        G4cout << "Difficulty in converging in "
               << "G4BrentLocator::EstimateIntersectionPoint()" << G4endl
    ...
    if ((!found_approximate_intersection) && (!there_is_no_intersection))
    {
      ...
      if ((did_len < fraction_done * all_len) && (depth < max_depth) && (!sub_final_section))
      {
        ...
        *ptrInterMedFT[depth] = start;
        ...
        GetNavigatorFor()->LocateGlobalPointWithinVolume(Point_A);
        G4bool Intersects_AB = IntersectChord(Point_A, SubE_point, NewSafety, fPreviousSafety,
  ...
  if ((substep_no >= max_substeps) && !there_is_no_intersection && !found_approximate_intersection)
  {
    ...
    message << "Too many substeps!" << G4endl
            ...
            << "          Found intersection = " << found_approximate_intersection << G4endl
  ...
  else if (substep_no >= warn_substeps)
  {
    ...
    G4Exception("G4BrentLocator::EstimateIntersectionPoint()", "GeomNav1002", JustWarning, message);
    G4cout.precision(oldprc);
  }
