...
// Constructor
//
G4PathFinder::G4PathFinder() : fEndState(G4ThreeVector(), G4ThreeVector(), 0., 0., 0., 0., 0.)
{
...
// ----------------------------------------------------------------------------
//
void G4PathFinder::EnableParallelNavigation(G4bool enableChoice)
{
...
// ----------------------------------------------------------------------------
//
G4double G4PathFinder::ComputeStep(const G4FieldTrack& InitialFieldTrack,
                                   G4double proposedStepLength, G4int navigatorNo,
                                   G4int stepNo,  // find next step
                                   G4double& pNewSafety,  // for this geom
                                   ELimited& limitedStep, G4FieldTrack& EndState,
                                   G4VPhysicalVolume* currentVolume)
{
  ...
  if (fVerboseLevel > 2)
  {
    ...
    G4cout << " PF::ComputeStep requested step "
           << " from " << InitialFieldTrack.GetPosition() << " dir  "
           << InitialFieldTrack.GetMomentumDirection() << G4endl;
  }
#endif
#ifdef G4VERBOSE
  if (navigatorNo >= fNoActiveNavigators)
  {
    ...
    message << "Bad Navigator ID !" << G4endl << "        Requested Navigator ID = " << navigatorNo
            << G4endl << "        Number of active navigators = " << fNoActiveNavigators;
    G4Exception("G4PathFinder::ComputeStep()", "GeomNav0002", FatalException, message);
  }
#endif
  ...
  if (fNewTrack || (stepNo != fLastStepNo))
  {
    ...
    if (moveLenSq > sqr(kCarTolerance))
    {
      ...
      if (fVerboseLevel > 2)
      {
        ...
        G4cout << " G4PathFinder::ComputeStep : Point moved since last step "
               << " -- at step # = " << stepNo << G4endl << " by " << moveLen << " to "
               << newPosition << G4endl;
      }
#endif
    ...
    if (particleCharge != 0.0)
    {
      ...
      //
      fieldExertsForce = (fieldMgr != nullptr) && (fieldMgr->GetDetectorField() != nullptr);
    }
    ...
    if ((fNoGeometriesLimiting < 0) || (fNoGeometriesLimiting > fNoActiveNavigators))
    {
      ...
      message << "Number of geometries limiting the step not set." << G4endl
              << "        Number of geometries limiting step = " << fNoGeometriesLimiting;
      G4Exception("G4PathFinder::ComputeStep()", "GeomNav0002", FatalException, message);
    }
#endif
  }
  ...
  else
  {
    ...
    {  // For 2nd+ geometry
      ...
      message << "Problem in step size request." << G4endl
              ...
              << "        This could happen due to an error in process ordering." << G4endl
              << "        Check that all physics processes are registered"
              << "        before all processes with a navigator/geometry." << G4endl
              << "        If using pre-packaged physics list and/or"
              << "        functionality, please report this error." << G4endl << G4endl
              ...
              << "        MinimumStep (navraw)  = " << fMinStep << G4endl
              << "        Navigator raw return value" << G4endl
              << "        Requested step now = " << proposedStepLength << G4endl
              << "        Difference min-req (absolute) = " << fTrueMinStep - proposedStepLength
              << G4endl << "        Relative (to max of two) = "
              << (fTrueMinStep - proposedStepLength) / std::max(proposedStepLength, fTrueMinStep)
              << G4endl << "     -- Step info> stepNo= " << stepNo << " last= " << fLastStepNo
              << " newTr= " << fNewTrack << G4endl;
      G4Exception("G4PathFinder::ComputeStep()", "GeomNav0003", FatalException, message);
    }
    else
    {
      ...
      if (fVerboseLevel > 1)
      {
        G4cout << " G4P::CS -> Not calling DoNextLinearStep: "
               << " stepNo= " << stepNo << " last= " << fLastStepNo << " new= " << fNewTrack
               << " Step already done" << G4endl;
      }
    }
  }
#endif
...
void G4PathFinder::PrepareNewTrack(const G4ThreeVector& position, const G4ThreeVector& direction,
                                   G4VPhysicalVolume* massStartVol)
{
  ...
  if (fNoActiveNavigators > fMaxNav)
  {
    ...
    message << "Too many active Navigators / worlds." << G4endl
            << "        Transportation Manager has " << fNoActiveNavigators << " active navigators."
            << G4endl << "        This is more than the number allowed = " << fMaxNav << " !";
    G4Exception("G4PathFinder::PrepareNewTrack()", "GeomNav0002", FatalException, message);
  }
...
void G4PathFinder::ReportMove(const G4ThreeVector& OldVector, const G4ThreeVector& NewVector,
                              const G4String& Quantity) const
{
  ...
  message << "Endpoint moved between value returned by ComputeStep()"
          << " and call to Locate(). " << G4endl << "          Change of " << Quantity << " is "
          << moveVec.mag() / mm << " mm long" << G4endl << "          and its vector is "
          << (1.0 / mm) * moveVec << " mm " << G4endl
          << "          Endpoint of ComputeStep() was     " << OldVector << G4endl
          << "          and current position to locate is " << NewVector;
  G4Exception("G4PathFinder::ReportMove()", "GeomNav1002", JustWarning, message);
}

void G4PathFinder::Locate(const G4ThreeVector& position, const G4ThreeVector& direction,
                          G4bool relative)
{
  ...
  G4ThreeVector lastEndPosition = fRelocatedPoint ? fLastLocatedPosition : fEndState.GetPosition();
  fLastLocatedPosition = position;
  ...
  {
    ReportMove(lastEndPosition, position, " (End) Position / G4PathFinder::Locate");
  }
  ...
  if (fVerboseLevel > 2)
  {
    ...
    {
      G4cout << "  lastEndPosition = " << lastEndPosition << "  moveVec = " << moveVec
             << "  newTr = " << fNewTrack << "  relocated = " << fRelocatedPoint << G4endl;
    }
  ...
  for (auto num = 0; num < fNoActiveNavigators; ++pNavIter, ++num)
  {
    ...
    G4VPhysicalVolume* pLocated =
      (*pNavIter)->LocateGlobalPointAndSetup(position, &direction, relative, false);
...
void G4PathFinder::ReLocate(const G4ThreeVector& position)
{
  ...
  G4double distCheckEnd_sq = (moveLenEndPosSq - endPointSafety_Est1 * endPointSafety_Est1);
  G4double distCheckSaf_sq = (moveLenSafSq - fMinSafety_atSafLocation * fMinSafety_atSafLocation);
  ...
  if ((!fNewTrack) && (longMoveEnd && longMoveSaf))
  {
    ...
    if (longMoveRevisedEnd && (moveMinusSafety > 0.0) && (revisedSafety > 0.0))
    {
      ...
      G4double maxCoordPos = std::max(std::max(std::fabs(position.x()), std::fabs(position.y())),
                                      std::fabs(position.z()));
      ...
      else
      {
        ...
        G4cout << " Difference of move & safety is very small in magnitude, " << absMoveMinusSafety
               << G4endl;
        if (smallRatio)
        {
          G4cout << " ratio to safety " << revisedSafety << " is "
                 << absMoveMinusSafety / revisedSafety << "smaller than " << kRadTolerance
                 << " of safety ";
        }
        else
        {
          G4cout << " as fraction " << absMoveMinusSafety / maxCoordPos
                 << " of position vector max-coord " << maxCoordPos << " smaller than "
                 << cErrorTolerance;
        }
    ...
    if (longMoveEnd && longMoveSaf && longMoveRevisedEnd && (moveMinusSafety > 0.0))
    {
      ...
      message << "ReLocation is further than end-safety value." << G4endl
              ...
              << " compared to end safety (from preStep point) = " << endPointSafety_Est1 << G4endl
              << "  --> last PreSafety Location was " << fPreSafetyLocation << G4endl
              << "       safety value =  " << fPreSafetyMinValue << G4endl
              << "  --> last PreStep Location was " << fPreStepLocation << G4endl
              << "       safety value =  " << fMinSafety_PreStepPt << G4endl
              << "  --> last EndStep Location was " << lastEndPosition << G4endl
              << "       safety value =  " << endPointSafety_Est1
              << " raw-value = " << endPointSafety_raw << G4endl
              << "  --> Calling again at this endpoint, we get " << revisedSafety
              << " as safety value." << G4endl << "  --> last position for safety "
              << fSafetyLocation << G4endl
              << "       its safety value =  " << fMinSafety_atSafLocation << G4endl
              << "       move from safety location = " << std::sqrt(moveLenSafSq) << G4endl
              << "         again= " << moveVecSafety.mag() << G4endl
              << "       safety - Move-from-end= " << revisedSafety - moveLenEndPosition
              << " (negative is Bad.)" << G4endl
              << " Debug:  distCheckRevisedEnd = " << distCheckRevisedEnd;
      ReportMove(lastEndPosition, position, "Position");
      G4Exception("G4PathFinder::ReLocate", "GeomNav0003", FatalException, message);
    }
  }
  ...
  if (fVerboseLevel > 2)
  {
    ...
    if ((fVerboseLevel > -1) || (moveLenEndPosSq > 0.0))
    {
      G4cout << "  lastEndPosition = " << lastEndPosition
             << "  moveVec from step-end = " << moveVecEndPos << "  is new Track = " << fNewTrack
...
G4double G4PathFinder::ComputeSafety(const G4ThreeVector& position)
{
  ...
  if (fVerboseLevel > 1)
  {
    G4cout << " G4PathFinder::ComputeSafety - returns " << minSafety << " at location " << position
           << G4endl;
  }
#endif
  return minSafety;
...
G4TouchableHandle G4PathFinder::CreateTouchableHandle(G4int navId) const
{
  ...
  if (fVerboseLevel > 2)
  {
    G4cout << "G4PathFinder::CreateTouchableHandle : navId = " << navId << " -- "
  ...
  if (fVerboseLevel > 2)
  {
    ...
    if (locatedVolume)
    {
      VolumeName = locatedVolume->GetName();
    }
    G4cout << " Touchable History created at address " << touchHist
           << "; volume = " << locatedVolume << "; name= " << VolumeName << G4endl;
  }
#endif
...
G4double G4PathFinder::DoNextLinearStep(const G4FieldTrack& initialState,
...
#  ifdef G4DEBUG_PATHFINDER
    if (fVerboseLevel > 2)
    {
      G4cout << "G4PathFinder::DoNextLinearStep : Quick Stepping. " << G4endl
             << " proposedStepLength " << proposedStepLength << " < (full) safety = " << fullSafety
             << " at " << initialPosition << G4endl;
    }
#  endif
  }
  else
...
#  ifdef G4DEBUG_PATHFINDER
        ...
        G4cout << "PathFinder::ComputeStep> small proposed step = " << proposedStepLength
               << " <=  safety = " << safety << " for nav " << num << " Step fully taken. "
               << G4endl;
#  endif
      }
      else
...
#endif
        step = (*pNavigatorIter)
                 ->ComputeStep(initialPosition, initialDirection, proposedStepLength, safety);
      ...
      if (fVerboseLevel > 2)
      {
        G4cout << "G4PathFinder::DoNextLinearStep : Navigator [" << num << "] -- step size " << step
               << G4endl;
      }
#endif
    }
  ...
  if (fVerboseLevel > 1)
  {
    ...
    G4cout << "G4PathFinder::DoNextLinearStep : "
           << " initialPosition = " << initialPosition << " and endPosition = " << endPosition
           << G4endl;
...
void G4PathFinder::WhichLimited()
{
  ...
  //
  G4bool transportLimited = (fCurrentStepSize[IdTransport] == fMinStep) && (fMinStep != kInfinity);
  ...
  for (num = 0; num < fNoActiveNavigators; ++num)
  {
    ...
    limitedStep = (std::fabs(step - fMinStep) < kCarTolerance) && (step != kInfinity);
...
void G4PathFinder::PrintLimited()
{
  ...
  G4cout << "  Minimum step (true)= " << fTrueMinStep << "  reported min = " << fMinStep << G4endl;
  if ((fCurrentStepNo <= 2) || (fVerboseLevel >= 2))
  {
    G4cout << std::setw(5) << " Step#" << " " << std::setw(5) << " NavId" << " " << std::setw(12)
           << " step-size " << " " << std::setw(12) << " raw-size " << " " << std::setw(12)
           << " pre-safety " << " " << std::setw(15) << " Limited / flag" << " " << std::setw(15)
           << "  World " << " " << G4endl;
  }
  for (auto num = 0; num < fNoActiveNavigators; ++num)
  {
    ...
    G4cout << std::setw(5) << fCurrentStepNo << " " << std::setw(5) << num << " " << std::setw(12)
           << stepLen << " " << std::setw(12) << rawStep << " " << std::setw(12)
           << fCurrentPreStepSafety[num] << " " << std::setw(5)
           << (fLimitTruth[num] ? "YES" : " NO") << " ";
...
G4double G4PathFinder::DoNextCurvedStep(const G4FieldTrack& initialState,
                                        G4double proposedStepLength,
                                        G4VPhysicalVolume* pCurrentPhysicalVolume)
{
  ...
  G4EquationOfMotion* equationOfMotion = fpFieldPropagator->GetCurrentEquationOfMotion();

  equationOfMotion->SetChargeMomentumMass(
    *(initialState.GetChargeState()), initialState.GetMomentum().mag(), initialState.GetRestMass());
  ...
  //
  minStep = fpFieldPropagator->ComputeStep(fieldTrack, proposedStepLength, newSafety,
                                           pCurrentPhysicalVolume, false);
  ...
  if (fVerboseLevel > 2)
  {
    G4cout << "G4PathFinder::DoNextCurvedStep : " << G4endl << " initialState = " << initialState
           << G4endl << " and endState = " << fEndState << G4endl;
    G4cout << "G4PathFinder::DoNextCurvedStep : "
           << " minStep = " << minStep << " proposedStepLength " << proposedStepLength
  ...
  if (minStep < proposedStepLength)  // if == , then a boundary found at end ??
  {
    ...
    for (numNav = 0; numNav < fNoActiveNavigators; ++numNav)
    {
      ...
      finalStep = fpMultiNavigator->ObtainFinalStep(numNav, lastPreSafety, minStepLast, didLimit);
...
#ifdef G4DEBUG_PATHFINDER
      G4bool StepError = (currentStepSize < 0) || ((minStepLast != kInfinity) && (diffStep < 0));
      if (StepError || (fVerboseLevel > 2))
      {
        ...
        G4cout << " G4PathFinder::ComputeStep. Geometry " << numNav
               << "  step= " << fCurrentStepSize[numNav] << " from final-step= " << finalStep
               << " fTrueMinStep= " << fTrueMinStep << " minStepLast= " << minStepLast
        ...
        if (StepError)
        {
          ...
          message << "Incorrect calculation of step size for one navigator" << G4endl
                  << "        currentStepSize = " << currentStepSize << ", diffStep= " << diffStep
                  << G4endl << "ERROR in computing step size for this navigator.";
          G4Exception("G4PathFinder::DoNextCurvedStep", "GeomNav0003", FatalException, message);
        }
      }
#endif
    }  // for num Navigators
  ...
  else if ((minStep == proposedStepLength) || (minStep == kInfinity)
...
G4String& G4PathFinder::LimitedString(ELimited lim)
{
  ...
  switch (lim)
  {
    case kDoNot:
      ...
      break;
    case kUnique:
      limitedStr = &StrUnique;
      break;
    case kSharedTransport:
      limitedStr = &StrSharedTransport;
      break;
    case kSharedOther:
      limitedStr = &StrSharedOther;
      break;
    default:
      limitedStr = &StrUndefined;
      break;
  }
  return *limitedStr;
}
