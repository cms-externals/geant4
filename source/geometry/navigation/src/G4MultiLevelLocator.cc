...
//
G4bool
G4MultiLevelLocator::EstimateIntersectionPoint(const G4FieldTrack& CurveStartPointVelocity,  // A
                                               const G4FieldTrack& CurveEndPointVelocity,  // B
                                               const G4ThreeVector& TrialPoint,  // E
                                               ...
                                               G4ThreeVector& previousSftOrigin)  // In/Out
{
  ...
  //----------------------------------------------
  static G4ThreadLocal G4LocatorChangeLogger endChangeA("StartPointA"), endChangeB("EndPointB"),
    recApproxPoint("ApproxPoint"), pointH_logger("Trial points 'E': position, normal");
  ...
  if (fCheckMode)
  {
    ...
    ++eventCount;
    endChangeA.AddRecord(G4LocatorChangeRecord::kInitialisingCL, substep_no, eventCount,
                         CurrentA_PointVelocity);
    endChangeB.AddRecord(G4LocatorChangeRecord::kInitialisingCL, substep_no, eventCount,
  ...
  if (fCheckMode)
  {
    pointH_logger.AddRecord(
      G4LocatorChangeRecord::kInitialisingCL, substep_no, eventCount,
      G4FieldTrack(CurrentE_Point, 0., NormalAtEntry, 0., 0., 1., G4ThreeVector(0.), 0., 0.));
#if (G4DEBUG_FIELD > 1)
    G4ThreeVector StartPosition = CurveStartPointVelocity.GetPosition();
    if ((TrialPoint - StartPosition).mag2() < sqr(tolerance))
    {
      ReportImmediateHit(MethodName, StartPosition, TrialPoint, tolerance, fNumCalls);
    }
#endif
  }
  ...
  *ptrInterMedFT[0] = CurveEndPointVelocity;
  ...
  do  // Loop checking, 07.10.2016, J.Apostolakis
  {
    ...
    {  // REPEAT param
      ...
      if (curv_lenAB < distAB * (1. - 10. * fiEpsilonStep))
      {
        ...
        G4cerr << "        Len A, B = " << lenA << " " << lenB << G4endl
               << "        Position A: " << Point_A << G4endl << "        Position B: " << Point_B
               << G4endl;
        G4cerr.precision(op);
      ...
      if (!validIntersectP)
      {
        ...
        errmsg << "Assertion FAILURE - invalid (stale) Interection point. Substep: " << substep_no
               << " call: " << fNumCalls << G4endl;
        if (fCheckMode)
        {
          G4LocatorChangeRecord::ReportEndChanges(errmsg, endChangeA, endChangeB);
        }
        G4Exception("G4MultiLevelLocator::EstimateIntersectionPoint", "GeomNav0004", JustWarning,
                    errmsg);
      }
#endif
      ...
      //
      ApproxIntersecPointV = GetChordFinderFor()->ApproxCurvePointV(
        CurrentA_PointVelocity, CurrentB_PointVelocity, CurrentE_Point, GetEpsilonStepFor());
      // The above method is the key & most intuitive part ...

#ifdef G4DEBUG_FIELD
      recApproxPoint.push_back(G4LocatorChangeRecord(G4LocatorChangeRecord::kInvalidCL, substep_no,
                                                     eventCount, ApproxIntersecPointV));
      ...
      if (fVerboseLevel > 3)
      {
        ...
        G4VIntersectionLocator::ReportTrialStep(substep_no, ChordAB, ChordEF_Vector, NewMomentumDir,
                                                NormalAtEntry, validNormalAtE);
        G4cout << " dot( MomentumDir, ABchord_unit ) = " << MomDir_dot_ABchord << G4endl;
      }
#endif
      G4bool adequate_angle = (MomDir_dot_Norm >= 0.0)  // Can use ( > -epsilon) instead
          ...
          || (EF_dist2 <= kCarTolerance * kCarTolerance))
      {
        ...
        if (GetAdjustementOfFoundIntersection())
        {
          ...
          G4bool goodCorrection = AdjustmentOfFoundIntersection(
            Point_A, CurrentE_Point, CurrentF_Point, MomentumDir, last_AF_intersection, IP,
            NewSafety, previousSafety, previousSftOrigin);
          if (goodCorrection)
          {
            IntersectedOrRecalculatedFT = ApproxIntersecPointV;
      ...
      else  // E is NOT close enough to the curve (ie point F)
      {
        ...
        G4bool Intersects_AF = IntersectChord(Point_A, CurrentF_Point, NewSafety, previousSafety,
                                              previousSftOrigin, stepLengthAF, PointG);
        ...
        if (Intersects_AF)
        {
          ...
          {
            ++eventCount;
            endChangeB.push_back(G4LocatorChangeRecord(G4LocatorChangeRecord::kIntersectsAF,
          ...
          if (fVerboseLevel > 3)
          {
            G4cout << "G4PiF::LI> Investigating intermediate point"
                   ...
                   << " on way to full s=" << CurveEndPointVelocity.GetCurveLength() << G4endl;
          }
#endif
        }
        else  // not Intersects_AF
        {
          ...
          Intersects_FB = IntersectChord(CurrentF_Point, Point_B, NewSafety, previousSafety,
                                         previousSftOrigin, stepLengthFB, PointH);
          if (Intersects_FB)
          {
            ...
            {
              ++eventCount;
              endChangeA.push_back(G4LocatorChangeRecord(G4LocatorChangeRecord::kIntersectsFB,
                                                         substep_no, eventCount,
                                                         CurrentA_PointVelocity));
              ...
              pointH_logger.AddRecord(G4LocatorChangeRecord::kIntersectsFB, substep_no, eventCount,
                                      intersectH_pn);
            }
          }
          else  // not Intersects_FB
          {
            ...
            else
            {
              ...
              CurrentB_PointVelocity = (depth == 0) ? CurveEndPointVelocity : *ptrInterMedFT[depth];
              SubStart_PointVelocity = CurrentA_PointVelocity;
              ...
              if (fCheckMode)
              {
                ...
                endChangeA.push_back(
                  G4LocatorChangeRecord(G4LocatorChangeRecord::kNoIntersectAForFB, substep_no,
        ...
        if (driverReIntegrates)
        {
          ...
          recalculatedB = CheckAndReEstimateEndpoint(CurrentA_PointVelocity, CurrentB_PointVelocity,
                                                     RevisedB_FT, errorEndPt);
          if (recalculatedB)
          {
          ...
          {
            ++eventCount;
            endChangeB.push_back(G4LocatorChangeRecord(G4LocatorChangeRecord::kRecalculatedB,
        ...
        if (errorEndPt > 1)  // errorEndPt = 1 is milder, just: len(B)=len(A)
        {
          ...
          ReportReversedPoints(errmsg, CurveStartPointVelocity, CurveEndPointVelocity, NewSafety,
                               fiEpsilonStep, CurrentA_PointVelocity, CurrentB_PointVelocity,
                               SubStart_PointVelocity, CurrentE_Point, ApproxIntersecPointV,
                               substep_no, substep_no_p, depth);
          ...
          errmsg << G4endl << " * Location: " << MethodName << "- After EndIf(Intersects_AF)"
                 << G4endl;
          errmsg << " * Bool flags:  Recalculated = " << recalculatedB
                 << "   Intersects_AF = " << Intersects_AF << "   Intersects_FB = " << Intersects_FB
                 << G4endl;
      ...
      {
        G4cout << "Difficulty in converging in " << MethodName << "    Substep no = " << substep_no
               << G4endl;
        if (substep_no == trigger_substepno_print)
        {
          G4cout << " Start:   ";
          printStatus(CurveStartPointVelocity, CurveEndPointVelocity, -1.0, NewSafety, 0);
          if (fCheckMode)
          {
        ...
        G4cout << " Point A: ";
        printStatus(CurrentA_PointVelocity, CurrentA_PointVelocity, -1.0, NewSafety,
                    substep_no - 1);
        G4cout << " Point B: ";
        printStatus(CurrentA_PointVelocity, CurrentB_PointVelocity, -1.0, NewSafety, substep_no);
      }
#endif
      ++substep_no;
      ++substep_no_p;

    } while ((!found_approximate_intersection) && (!there_is_no_intersection)
    ...
    {
      G4double did_len =
        std::abs(CurrentA_PointVelocity.GetCurveLength() - SubStart_PointVelocity.GetCurveLength());
      G4double all_len =
        std::abs(CurrentB_PointVelocity.GetCurveLength() - SubStart_PointVelocity.GetCurveLength());
      ...
      if ((did_len < fraction_done * all_len) && (depth < max_depth) && (!sub_final_section))
      {
        ...
        G4bool fullAdvance = integrDriver->AccurateAdvance(midPoint, Sub_len, fiEpsilonStep);
        ...
        G4double lenAchieved = midPoint.GetCurveLength() - CurrentA_PointVelocity.GetCurveLength();
        ...
        else  //  !goodAdvance
        {
          G4cout << "MLL> AdvanceChordLimited not full at depth=" << depth
                 << "  total length achieved = " << lenAchieved << " of " << Sub_len
                 << "  fraction= ";
          if (Sub_len != 0.0)
          {
          ...
          G4cout << "  Number of call to mll = " << fNumCalls << "   iteration " << substep_no
                 << "  inner =  " << substep_no_p << G4endl;
          G4cout << "  Epsilon of integration = " << fiEpsilonStep << G4endl;
          G4cout << "  State at start is      = " << CurrentA_PointVelocity << G4endl
          ...
          G4cout << "  Original Start = " << CurveStartPointVelocity << G4endl;
          G4cout << "  Original   End = " << CurveEndPointVelocity << G4endl;
          G4cout << "  Original TrialPoint = " << TrialPoint << G4endl;
        ...
        *ptrInterMedFT[depth] = midPoint;
        ...
        {
          ++eventCount;
          endChangeB.push_back(G4LocatorChangeRecord(G4LocatorChangeRecord::kInsertingMidPoint,
        ...
        GetNavigatorFor()->LocateGlobalPointWithinVolume(Point_A);
        G4bool Intersects_AB = IntersectChord(Point_A, Point_B, NewSafety, previousSafety,
                                              previousSftOrigin, distAB, PointGe);
        if (Intersects_AB)
        {
      ...
      while (unfinished && (depth > 0))  // Loop checking, 07.10.2016, JA
      {
        ...
        {
          ++eventCount;
          G4LocatorChangeRecord chngPop_a(G4LocatorChangeRecord::kLevelPop, substep_no, eventCount,
                                          CurrentA_PointVelocity);
          endChangeA.push_back(chngPop_a);
          G4LocatorChangeRecord chngPop_b(G4LocatorChangeRecord::kLevelPop, substep_no, eventCount,
                                          CurrentB_PointVelocity);
        ...
        if (driverReIntegrates)
        {
          ...
          {
            ++eventCount;
            endChangeB.push_back(G4LocatorChangeRecord(G4LocatorChangeRecord::kRecalculatedBagn,
        ...
        GetNavigatorFor()->LocateGlobalPointWithinVolume(Point_A);
        G4bool Intersects_AB = IntersectChord(Point_A, Point_B, NewSafety, previousSafety,
                                              previousSftOrigin, distAB, PointGi);
        if (Intersects_AB)
        {
          last_AF_intersection = Intersects_AB;
      ...
      {
        // What happened ??
        G4cout << "MLL - WARNING Potential FAILURE: Conditions not met!" << G4endl
               << " Depth = " << depth << G4endl << " Num Substeps= " << substep_no << G4endl;
        G4cout << " Found intersection= " << found_approximate_intersection << G4endl;
        G4cout << " Progress report: -- " << G4endl;
        ReportProgress(G4cout, CurveStartPointVelocity, CurveEndPointVelocity, substep_no,
                       CurrentA_PointVelocity, CurrentB_PointVelocity, NewSafety, depth);
        G4cout << G4endl;
      }
#endif
    }  // if(!found_aproximate_intersection)

    assert(validIntersectP || there_is_no_intersection || found_approximate_intersection);
  ...
  if (!there_is_no_intersection && !found_approximate_intersection)
  {
    if (substep_no >= fMaxSteps)
    {
      ...
      message << G4endl;
      message << "Convergence is requiring too many substeps: " << substep_no
              << "  ( limit = " << fMaxSteps << ")" << G4endl
      ...
      message << "        Undertaken only length: " << done_len << " out of " << full_len
              << " required." << G4endl << "        Remaining length = " << full_len - done_len;

      message << "     Start and end-point of requested Step:" << G4endl;
      printStatus(CurveStartPointVelocity, CurveEndPointVelocity, -1.0, NewSafety, 0, message, -1);
      message << "        Start and end-point of current Sub-Step:" << G4endl;
      printStatus(CurrentA_PointVelocity, CurrentA_PointVelocity, -1.0, NewSafety, substep_no - 1,
                  message, -1);
      printStatus(CurrentA_PointVelocity, CurrentB_PointVelocity, -1.0, NewSafety, substep_no,
                  message, -1);
    ...
    else if (substep_no >= fWarnSteps)
    {
      ...
      message << "Many substeps while trying to locate intersection." << G4endl
              << "          Undertaken length: " << CurrentB_PointVelocity.GetCurveLength()
...
void G4MultiLevelLocator::ReportStatistics()
{
  ...
  G4cout << " Number of split level ('advances'):  " << fNumAdvanceTrials << G4endl;
  G4cout << " Number of full advances:             " << fNumAdvanceGood << G4endl;
  G4cout << " Number of good advances:             " << fNumAdvanceFull << G4endl;
}

void G4MultiLevelLocator::ReportFieldValue(const G4FieldTrack& locationPV, const char* nameLoc,
                                           const G4EquationOfMotion* equation)
{
  ...
  G4cout << "  B-field value (" << nameLoc << ")=   " << FieldVec[0] << " " << FieldVec[1] << " "
         << FieldVec[2];
  G4double Emag2 = G4ThreeVector(FieldVec[3], FieldVec[4], FieldVec[5]).mag2();
  if (Emag2 > 0.0)
  {
    G4cout << " Electric = " << FieldVec[3] << " " << FieldVec[4] << " " << FieldVec[5] << G4endl;
  }
  return;
}
