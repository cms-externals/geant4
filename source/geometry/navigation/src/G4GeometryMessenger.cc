...
G4GeometryMessenger::G4GeometryMessenger(G4TransportationManager* tman) : tmanager(tman)
{
  ...
  serialMassCmd = new G4UIcmdWithABool("/geometry/test/forceSerialGetMass", this);
  serialMassCmd->SetToBeBroadcasted(false);
  serialMassCmd->SetGuidance("Force G4LogicalVolume::GetMass() to run serially.");
  serialMassCmd->SetGuidance("By default, tasking may be used on demand when available.");
  serialMassCmd->SetParameterName("forceSerialGetMass", true);
  serialMassCmd->SetDefaultValue(false);
  serialMassCmd->AvailableForStates(G4State_PreInit, G4State_Idle);

  serialSurfaceCmd = new G4UIcmdWithABool("/geometry/test/forceSerialGetPointOnSurface", this);
  serialSurfaceCmd->SetToBeBroadcasted(false);
  serialSurfaceCmd->SetGuidance("Force GetPointOnSurface()-related geometry work to run serially.");
  serialSurfaceCmd->SetGuidance("By default, tasking may be used on demand when available.");
  serialSurfaceCmd->SetParameterName("forceSerialGetPointOnSurface", true);
  serialSurfaceCmd->SetDefaultValue(false);
  serialSurfaceCmd->AvailableForStates(G4State_PreInit, G4State_Idle);

  serialOverlapsCmd = new G4UIcmdWithABool("/geometry/test/forceSerialOverlaps", this);
  serialOverlapsCmd->SetToBeBroadcasted(false);
  serialOverlapsCmd->SetGuidance("Force geometry overlap checks to run serially.");
  serialOverlapsCmd->SetGuidance("By default, tasking may be used on demand when available.");
  serialOverlapsCmd->SetParameterName("forceSerialOverlaps", true);
  serialOverlapsCmd->SetDefaultValue(false);
  serialOverlapsCmd->AvailableForStates(G4State_PreInit, G4State_Idle);
...
G4GeometryMessenger::~G4GeometryMessenger()
{
  ...
  delete pchkCmd;
  delete chkCmd;
  delete serialOverlapsCmd;
  delete serialSurfaceCmd;
  delete serialMassCmd;
  delete geodir;
  delete navdir;
  delete testdir;
...
void G4GeometryMessenger::SetNewValue(G4UIcommand* command, G4String newValues)
{
  ...
  else if (command == serialMassCmd)
  {
    G4GeometryManager::GetInstance()->ForceSerialGetMass(serialMassCmd->GetNewBoolValue(newValues));
  }
  else if (command == serialSurfaceCmd)
  {
    G4GeometryManager::GetInstance()->ForceSerialGetPointOnSurface(serialSurfaceCmd->GetNewBoolValue(newValues));
  }
  else if (command == serialOverlapsCmd)
  {
    G4GeometryManager::GetInstance()->ForceSerialOverlapsCheck(serialOverlapsCmd->GetNewBoolValue(newValues));
  }
...
G4String G4GeometryMessenger::GetCurrentValue(G4UIcommand* command)
{
  ...
  {
    cv = serialMassCmd->ConvertToString(geomManager->IsSerialGetMassForced());
  }
  else if (command == serialSurfaceCmd)
  {
    cv = serialSurfaceCmd->ConvertToString(geomManager->IsSerialGetPointOnSurfaceForced());
  }
  else if (command == serialOverlapsCmd)
  {
    cv = serialOverlapsCmd->ConvertToString(geomManager->IsSerialOverlapsCheckForced());
  }
  return cv;
}
