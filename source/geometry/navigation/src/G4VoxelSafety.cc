...
G4double G4VoxelSafety::ComputeSafety(const G4ThreeVector& localPoint,
                                      const G4VPhysicalVolume& currentPhysical, G4double maxLength)
{
  ...
  if (insideMother != kInside)
  {
    ...
    if (insideMother == kOutside)
    {
      ...
      message << "Safety method called for location outside current Volume." << G4endl
              ...
              << "(safety from outside) is: " << motherSolid->DistanceToIn(localPoint) << G4endl;
      ...
      message << "  Description of solid: " << G4endl << *motherSolid << G4endl;
      G4Exception("G4VoxelSafety::ComputeSafety()", "GeomNav0003", FatalException, message);
    }
#endif
    return 0.0;
  }
  ...
  if (fVerbose == 1)
  {
    G4cout << "    Invoked DistanceToOut(p) for mother solid: " << motherSolid->GetName()
           << ". Solid replied: " << motherSafety << G4endl
           << "    For local point p: " << localPoint << ", to be considered as 'mother safety'."
           << G4endl;
  }
#endif
  ...
  daughterSafety =
    SafetyForVoxelHeader(motherVoxelHeader, localPoint, maxLength, currentPhysical, 0.0, ourSafety);
    ...
    if (!fBlockList.IsBlocked(sampleNo))
    {
      ...
      if (fVerbose == 1)
      {
        G4cout << "*** G4VoxelSafety::SafetyForVoxelNode(): ***" << G4endl
               << "    Invoked DistanceToIn(p) for daughter solid: " << ptrSolid->GetName()
...
// *********************************************************************
//
G4double G4VoxelSafety::SafetyForVoxelHeader(const G4SmartVoxelHeader* pHeader,
                                             const G4ThreeVector& localPoint, G4double maxLength,
                                             const G4VPhysicalVolume& currentPhysical,  // Debug
                                             G4double distUpperDepth_Sq, G4double previousMinSafety)
{
  ...
  const auto candNodeNo = G4int((localCrd - targetHeaderMin) / targetHeaderNodeWidth);
  ...
  if (candNodeNo < 0 || candNodeNo > targetHeaderNoSlices - 1)
  {
    ...
    ed << "    Axis = " << targetHeaderAxis << "  No of slices = " << targetHeaderNoSlices
       << G4endl;
    ...
    ed << *pSolid << G4endl;
    G4Exception("G4VoxelSafety::SafetyForVoxelHeader()", "GeomNav1003", JustWarning, ed,
  ...
  if (fVerbose > 2)
  {
    ...
    G4cout << " Calculated pointNodeNo= " << pointNodeNo
           << "  from position= " << localPoint(targetHeaderAxis) << "  min= " << targetHeaderMin
           << "  max= " << targetHeaderMax << "  width= " << targetHeaderNodeWidth
           << "  no-slices= " << targetHeaderNoSlices << "  axis=  " << targetHeaderAxis << G4endl;
  }
  else if (fVerbose == 1)
  {
  ...
  do
  {
    ...
    if (fVerbose > 2)
    {
      G4cout << " -Checking node " << targetNodeNo << " is proxy with address " << sampleProxy
             << G4endl;
    }
#endif
    ...
    if (sampleProxy == nullptr)
    {
      ...
      G4Exception("G4VoxelSafety::SafetyForVoxelHeader()", "GeomNav0003", FatalException, ed,
    ...
    else if (sampleProxy->IsNode())
    {
      ...
      {
        G4cout << " -- It is a Node ";
        G4cout << "    its safety= " << nodeSafety << " our level Saf = " << ourSafety
    ...
    else
    {
      ...
      if (fVerbose > 2)
      {
        ...
        G4cout << "  Recurse to deal with next level, fVoxelDepth= " << fVoxelDepth + 1 << G4endl;
        G4cout << "  Distances:  Upper= " << distUpperDepth << " Axis= " << distAxis
      ...
      //
      headerSafety = SafetyForVoxelHeader(pNewVoxelHeader, localPoint, maxLength, currentPhysical,
      ...
      if (fVerbose > 2)
      {
        G4cout << "      >> Header safety = " << headerSafety << " our level Saf = " << ourSafety
               << G4endl;
      }
#endif
    ...
    if (targetNodeNo >= pointNodeNo)
    {
      ...
      G4double lowerEdgeOfNext = targetHeaderMin + nextUp * targetHeaderNodeWidth;
    ...
    if (targetNodeNo <= pointNodeNo)
    {
      ...
      G4double upperEdgeOfNext = targetHeaderMin + (nextDown + 1) * targetHeaderNodeWidth;
    ...
    if (fVerbose > 2)
    {
      G4cout << " Node= " << pointNodeNo << " Up:   next= " << nextUp << " d# "
             << nextUp - pointNodeNo << " trialUp=  " << trialUp << " d# " << trialUp - pointNodeNo
             << " Down: next= " << nextDown << " d# " << targetNodeNo - nextDown
             << " trialDwn= " << trialDown << " d# " << targetNodeNo - trialDown
             << " condition (next is Inside)= " << nextIsInside << G4endl;
    }
#endif
    ...
    if ((nextUp < targetHeaderNoSlices) && (UpIsClosest || (nextDown < 0)))
    {
      nextNodeNo = nextUp;
      distAxis = distUp;
      ++nextUp;  // Default
      ...
      if (fVerbose > 2)
      {
        G4cout << " > Chose Up.   Depth= " << fVoxelDepth << "   Nodes: next= " << nextNodeNo
               << " new nextUp=   " << nextUp << " Dist = " << distAxis << G4endl;
      }
#endif
    }
    else
    {
      ...
      if (fVerbose > 2)
      {
        G4cout << " > Chose Down.  Depth= " << fVoxelDepth << "  Nodes: next= " << nextNodeNo
               << " new nextDown=  " << nextDown << " Dist = " << distAxis << G4endl;
      }
#endif
    }
    ...
    else
    {
      ...
      if (fVerbose > 2)
      {
        ...
        G4cout << " VoxSaf> No more candidates:  nodeDown= " << nextDown << " nodeUp= " << nextUp
  ...
  if (fVerbose > 0)
  {
    ...
    G4cout << " ===== Returning from SafetyForVoxelHeader "
           << "  Depth= " << fVoxelDepth << G4endl << G4endl;
  }
#endif
