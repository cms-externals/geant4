  ...
  G4VPhysicalVolume*(*pBlockedPhysical), G4int& blockedReplicaNo)
{
  ...
  G4ThreeVector globalPoint = history.GetTopTransform().InverseTransformPoint(localPoint);
  G4ThreeVector globalDirection = history.GetTopTransform().InverseTransformAxis(localDirection);

  G4ThreeVector localPoint2 = localPoint;  // take away constantness

  LevelLocate(history, *pBlockedPhysical, blockedReplicaNo, globalPoint, &globalDirection, true,
              localPoint2);
  ...
  auto daughterParam = (G4PhantomParameterisation*)(daughterPhysical->GetParameterisation());
  ...
  //
  return fnormalNav->ComputeStep(daughterPoint, localDirection, currentProposedStepLength,
                                 newSafety, history, validExitNormal, exitNormal, exiting, entering,
  ...
  G4VPhysicalVolume* pCurrentPhysical)
{
  ...
  auto param = (G4PhantomParameterisation*)(pCurrentPhysical->GetParameterisation());

  if (!param->SkipEqualMaterials())
  {
    return fnormalNav->ComputeStep(localPoint, localDirection, currentProposedStepLength, newSafety,
  ...
  for (G4int ii = 0; ii < fNoStepsAllowed + 1; ++ii)
  {
    if (ii == fNoStepsAllowed)
    {
      ...
      //
      G4ThreeVector pGlobalpoint = history.GetTransform(ide).InverseTransformPoint(localPoint);
      std::ostringstream message;
      message << "G4RegularNavigation::ComputeStepSkippingEqualMaterials()"
              << "Stuck Track: potential geometry or navigation problem." << G4endl
              << "        Track stuck, moving for more than " << ii << " steps" << G4endl
              << "- at point " << pGlobalpoint << G4endl
              << "        local direction: " << localDirection << G4endl;
      G4Exception("G4RegularNavigation::ComputeStepSkippingEqualMaterials()", "GeomRegNav1001",
    ...
    if (fLastStepWasZero)
    {
      ...
      if (fNumberZeroSteps > 1)
      {
        ...
        message
          ...
          << fNumberZeroSteps << ", at " << pGlobalpoint << ", nav-comp-step calls # " << ii
          << ", Step= " << newStep;
        G4Exception("G4RegularNavigation::ComputeStepSkippingEqualMaterials()", "GeomRegNav1002",
      ...
      if (fNumberZeroSteps > fActionThreshold_NoZeroSteps - 1)
      {
        ...
        message << "Track stuck or not moving." << G4endl
                ...
                << G4endl << "- at point " << pGlobalpoint << " (local point " << localPoint << ")"
                << G4endl << "        local direction: " << localDirection
                << "          Potential geometry or navigation problem !" << G4endl
                << "          Trying pushing it of " << newStep << " mm ...";
        G4Exception("G4RegularNavigation::ComputeStepSkippingEqualMaterials()", "GeomRegNav1003",
                    JustWarning, message, "Potential overlap in geometry!");
#endif
      }
      if (fNumberZeroSteps > fAbandonThreshold_NoZeroSteps - 1)
      {
        ...
        message << "G4RegularNavigation::ComputeStepSkippingEqualMaterials()"
                ...
                << "- at point " << pGlobalpoint << G4endl
                << "        local direction: " << localDirection << G4endl;
        G4Exception("G4RegularNavigation::ComputeStepSkippingEqualMaterials()", "GeomRegNav1004",
                    EventMustBeAborted, message);
      }
    }
    else
    ...
    {
      G4RegularNavigationHelper::Instance()->AddStepLength(copyNo, newStep - totalNewStep
                                            ...
                                            const G4double pMaxLength)
{
  ...
  // Compute step in voxel
  //
  return fnormalNav->ComputeSafety(localPoint, history, pMaxLength);
}

//------------------------------------------------------------------
G4bool G4RegularNavigation::LevelLocate(G4NavigationHistory& history, const G4VPhysicalVolume*,
                                        const G4int, const G4ThreeVector& globalPoint,
