    ...
    qss_order(qssOrder <= 0 ? G4QSSMessenger::instance()->GetQssOrder() : qssOrder)
{
  ...
  if (qss_order != messenger->GetQssOrder())
  {
    ...
    if (qss_order != messenger->GetQssOrder())
    {
      ...
      {
        G4ExceptionDescription err_msg;
        err_msg << "G4QSSStepper: Trying to change order of QSS stepper(s) for " << change_of_order
                << " th time -- Maximum allowed is 1 time. "
                << " Requesting order = " << qss_order
                << " whereas current (static) value = " << messenger->GetQssOrder() << G4endl;
...
void G4QSStepper::initialize(const G4double y[])
{
  ...
  fVelocity =
    std::sqrt(velocity_vector[0] * velocity_vector[0] + velocity_vector[1] * velocity_vector[1]
              + velocity_vector[2] * velocity_vector[2]);
  ...
  // Copy into state_q
  std::copy_n(&current_substep.state_x[DERIVATIVE_0][0], 6,
              &current_substep.state_q[DERIVATIVE_0][0]);
  ...
  {
    dq_vector[i] =
      std::fmax(dqmin[INDEX_TYPE(i)],
                dqrel[INDEX_TYPE(i)] * std::fabs(current_substep.state_x[DERIVATIVE_0][i]));
...
void G4QSStepper::update_sync_time(G4int index)
{
  ...
  {
    // extra coefficient and h for the cubic polynomial and inclusion of the second order term from
    // q
      ...
      for (G4double dQ : {dq, -dq})
      {
        ...
        if (r * r < q_cube)
        {
          ...
          for (G4double t : {t1, t2, t3})
          {
            if (t > 0)
            {
              delta_sync_t = std::fmin(delta_sync_t, t);
            }
          }
        }
        // one real root
        else
        {
          ...
          if (t1 > 0)
          {
            delta_sync_t = std::fmin(delta_sync_t, t1);
          }
        }
      }
    }
  ...
  else
  {
    ...
    else
    {
      ...
      for (G4double discriminator : {discriminator_1, discriminator_2})
      {
        ...
        if (t_local > 0)
        {
          delta_sync_t = std::fmin(delta_sync_t, t_local);
        }
      }
    }
  }
...
void G4QSStepper::Stepper(const G4double y[], const G4double /*dydx*/[], G4double h,
                          G4double yout[], G4double /* yerr */[])
{
  ...
  while (t < fFinal_t && t < INFTY && substeps.current_substep_index < QSS_MAX_SUBSTEPS)
  {
    ...
    // sync q
    current_substep.state_q[DERIVATIVE_0][sync_index] =
      current_substep.state_x[DERIVATIVE_0][sync_index];
    current_substep.state_q[DERIVATIVE_1][sync_index] =
      current_substep.state_x[DERIVATIVE_1][sync_index];
    current_substep.state_q[DERIVATIVE_2][sync_index] =
      current_substep.state_x[DERIVATIVE_2][sync_index];

    current_substep.state_tq[sync_index] = current_substep.state_tx[sync_index];

    dq_vector[sync_index] = std::fmax(
      dqmin[INDEX_TYPE(sync_index)],
      dqrel[INDEX_TYPE(sync_index)] * std::fabs(current_substep.state_x[DERIVATIVE_0][sync_index]));
    ...
    // previous version called this every time which is unnecessary if field constant, and we bite
    // the bullet if not
    ...
    if (sync_index < VELOCITY_IDX && !fField_changed)
    {
      continue;
    }
