...
G4BulirschStoer::G4BulirschStoer(G4EquationOfMotion* equation, G4int nvar, G4double eps_rel,
                                 G4double max_dt)
...
G4BulirschStoer::step_result G4BulirschStoer::try_step(const G4double in[], const G4double dxdt[],
                                                       G4double& t, G4double out[], G4double& dt)
{
  ...
  for (G4int k = 0; k <= m_current_k_opt + 1; ++k)
  {
    ...
    else
    {
      ...
      const G4double error = field_utils::relativeError(out, m_err, dt, m_eps_rel);
      ...
      if (k == m_current_k_opt)  // convergence at k_opt ?
      {
        if (error < 1.0)
        {
          ...
          else if ((work[k] < KFAC2 * work[k - 1]) && !m_last_step_rejected)
          {
            ...
            new_h = h_opt[k];
            new_h *=
              static_cast<G4double>(m_cost[m_current_k_opt]) / static_cast<G4double>(m_cost[k]);
          }
          else
          {
            new_h = h_opt[m_current_k_opt];
...
G4double G4BulirschStoer::calc_h_opt(G4double h, G4double error, std::size_t k) const
{
...
G4bool G4BulirschStoer::set_k_opt(std::size_t k, G4double& dt)
{
  ...
  if ((work[k] < KFAC2 * work[k - 1]) || m_last_step_rejected || (k == m_k_max - 1))
...
G4bool G4BulirschStoer::should_reject(G4double error, G4int k) const
{
  ...
  {
    const auto d =
      G4double(m_interval_sequence[m_current_k_opt] * m_interval_sequence[m_current_k_opt + 1]);
