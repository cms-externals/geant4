...
G4RepleteEofM::G4RepleteEofM(G4Field* field, G4int nvar) : G4EquationOfMotion(field), fNvar(nvar)
{
  fGfield = field->IsGravityActive();
}

void G4RepleteEofM::SetChargeMomentumMass(G4ChargeState particleCharge,  // e+ units
                                          G4double MomentumXc, G4double particleMass)
{
...
void G4RepleteEofM::EvaluateRhsGivenB(const G4double y[], const G4double Field[],
                                      G4double dydx[]) const
{
  // Components of y:
  //    0-2 dr/ds,
