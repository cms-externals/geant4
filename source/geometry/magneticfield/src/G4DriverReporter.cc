...
void G4DriverReporter::PrintStatus(const G4double* StartArr, G4double xstart,
...
void G4DriverReporter::PrintStatus(const G4FieldTrack& StartFT, const G4FieldTrack& CurrentFT,
                                   G4double requestStep, unsigned int subStepNo)
{
  ...
  if ((subStepNo <= 1) || (verboseLevel > 3))
  {
    ...
    G4cout << std::setw(6) << " " << std::setw(25)
           << " G4DriverReporter: Current Position  and  Direction" << " " << G4endl;
    G4cout << std::setw(5) << "Step#" << " " << std::setw(prec7) << "s-curve" << " "
           << std::setw(prec9) << "X(mm)" << " " << std::setw(prec9) << "Y(mm)" << " "
           << std::setw(prec9) << "Z(mm)" << " " << std::setw(prec8) << " N_x " << " "
           << std::setw(prec8) << " N_y " << " " << std::setw(prec8) << " N_z " << " "
           << std::setw(6) << " N^2-1 " << " " << std::setw(10) << " N(0).N " << " " << std::setw(7)
           << "KinEner " << " " << std::setw(12) << "Track-l" << " "  // Add the Sub-step ??
...
void G4DriverReporter::PrintStat_Aux(const G4FieldTrack& aFieldTrack, G4double requestStep,
                                     ...
                                     G4double dotVeloc_StartCurr)
{
  ...
  // G4cout.precision(noPrecision);
  G4cout << std::setw(prec9) << Position.x() << " " << std::setw(prec9) << Position.y() << " "
         << std::setw(prec9) << Position.z() << " " << std::setw(prec8) << UnitVelocity.x() << " "
         << std::setw(prec8) << UnitVelocity.y() << " " << std::setw(prec8) << UnitVelocity.z()
         << " ";
  G4cout.precision(3);
  G4double unitMagDif = UnitVelocity.mag2() - 1.0;
  if (std::fabs(unitMagDif) < 1.0e-15)
  {
    unitMagDif = 0.0;
  }
