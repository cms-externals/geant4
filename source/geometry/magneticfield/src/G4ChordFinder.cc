...
G4ChordFinder::G4ChordFinder(G4MagneticField* theMagField, G4double stepMinimum,
                             G4MagIntegratorStepper* pItsStepper, G4int stepperDriverId)
{
  ...
  G4bool useG4QSSDriver =
    (stepperDriverId == kQss2DriverType) || (stepperDriverId == kQss3DriverType);
  ...
  using RegularStepperType = G4DormandPrince745;  // 5th order embedded method. High efficiency.
  ...
  if (verboseDebug)
  {
    ...
    G4cout << " Arguments: " << G4endl << " - min step = " << stepMinimum << G4endl
           << " - stepper ptr provided : " << (pItsStepper == nullptr ? " no  " : " yes ")
           << G4endl;
    if (pItsStepper == nullptr)
    {
      G4cout << " - stepper/driver Id = " << stepperDriverId << " i.e. "
             << "  useFSAL = " << useFSALstepper << "  , useTemplated = " << useTemplatedStepper
             << "  , useRegular = " << useRegularStepper << "  , useFSAL = " << useFSALstepper
             << G4endl;
    }
  }
#endif
  ...
  if (pItsStepper != nullptr)
  {
    ...
    if (pItsStepper->isQSS())
    {
      ...
      G4Exception("G4ChordFinder::G4ChordFinder()", "GeomField1001", FatalException,
  ...
  else if (useTemplatedStepper)
  {
    if (gVerboseCtor)
    {
      G4cout << " G4ChordFinder: Creating Templated Stepper of type> " << TemplatedStepperName
             << G4endl;
    }
    ...
    if (templatedStepper == nullptr)
    {
      ...
      message << "G4ChordFinder: Attempted to instantiate " << TemplatedStepperName
              << " type stepper " << G4endl;
    ...
    {
      fIntgrDriver =
        new G4IntegrationDriver<TemplatedStepperType>(stepMinimum, templatedStepper, nVar6);
      if (gVerboseCtor)
      {
  ...
  else if (useRegularStepper)  // Plain stepper -- not double ...
  {
    ...
    if (regularStepper == nullptr)
    {
      ...
      message << "G4ChordFinder: Attempted to instantiate " << RegularStepperName
              << " type stepper " << G4endl;
      errorInStepperCreation = true;
    }
    else
    {
      ...
      if (dp5 != nullptr)
      {
        fIntgrDriver = new G4InterpolationDriver<G4DormandPrince745>(stepMinimum, dp5, nVar6);
        if (gVerboseCtor)
        {
      ...
      {
        fIntgrDriver =
          new G4IntegrationDriver<RegularStepperType>(stepMinimum, regularStepper, nVar6);
        if (gVerboseCtor)
        {
  ...
  else if (useBfieldDriver)
  {
...

    {
      ...
      fIntgrDriver = new G4BFieldIntegrationDriver(
        std::make_unique<SmallStepDriver>(stepMinimum, regularStepper,
                                          regularStepper->GetNumberOfVariables()),
        std::make_unique<LargeStepDriver>(stepMinimum, fLongStepper.get(),
                                          regularStepper->GetNumberOfVariables()));

      if (fIntgrDriver == nullptr)
      {
        message << "Using G4BFieldIntegrationDriver with " << RegularStepperName << " type stepper "
                << G4endl;
        message << "Driver instantiation FAILED." << G4endl;
        G4Exception("G4ChordFinder::G4ChordFinder()", "GeomField1001", JustWarning, message);
      }
    }
  }
  else if (useG4QSSDriver)
  {
    ...
    if (gVerboseCtor)
    {
      G4cout << "-- Created QSS-" << G4QSSMessenger::instance()->GetQssOrder() << " stepper"
             << G4endl;
  ...
  else
  {
    ...
    if (fsalStepper == nullptr)
    {
      ...
      message << "Attempted to instantiate " << NewFSALStepperName << " type stepper " << G4endl;
      G4Exception("G4ChordFinder::G4ChordFinder()", "GeomField1001", JustWarning, message);
      errorInStepperCreation = true;
    }
    else
    {
      ...
      if (fIntgrDriver == nullptr)
      {
        message << "Using G4FSALIntegrationDriver with stepper type: " << NewFSALStepperName
                << G4endl;
  ...
  if (errorInStepperCreation || (fIntgrDriver == nullptr))
  {
    ...
    errmsg << "  Configuration:  (constructor arguments) " << G4endl
           ...
           << "   useFSAL = " << BoolName[useFSALstepper] << "   using combo BField Driver = "
           << BoolName[!(useFSALstepper || useTemplatedStepper || useRegularStepper)] << G4endl;
    errmsg << message.str();
    errmsg << "Aborting.";
    G4Exception("G4ChordFinder::G4ChordFinder() - constructor 2", "GeomField0003", FatalException,
                errmsg);
  }

  assert((pItsStepper != nullptr) || (fRegularStepperOwned != nullptr)
         || (fNewFSALStepperOwned != nullptr) || useG4QSSDriver);
...
G4FieldTrack G4ChordFinder::ApproxCurvePointS(
  const G4FieldTrack& CurveA_PointVelocity, const G4FieldTrack& CurveB_PointVelocity,
  const G4FieldTrack& ApproxCurveV, const G4ThreeVector& CurrentE_Point,
  const G4ThreeVector& CurrentF_Point, const G4ThreeVector& PointG, G4bool first, G4double eps_step)
{
  ...
  else
  {
    ...
    {
      EndPoint =
        ApproxCurvePointV(CurveA_PointVelocity, CurveB_PointVelocity, CurrentE_Point, eps_step);
      return EndPoint;
    }
  }
  ...
  else
  {
    ...
    //
    G4cout << "G4ChordFinder::ApproxCurvePointS() - test-step ShF = " << test_step
           << "  EndPoint = " << EndPoint << G4endl;
...
G4FieldTrack G4ChordFinder::ApproxCurvePointV(const G4FieldTrack& CurveA_PointVelocity,
                                              ...
                                              G4double eps_step)
{
  ...
  curve_length = CurveB_PointVelocity.GetCurveLength() - CurveA_PointVelocity.GetCurveLength();
  ...
  if (curve_length < ABdist * (1. - integrationInaccuracyLimit))
  {
    ...
    G4cerr << " Warning in G4ChordFinder::ApproxCurvePoint: " << G4endl
           ...
           << " Dist = " << ABdist << " curve length = " << curve_length
           << " relativeDiff = " << (curve_length - ABdist) / ABdist << G4endl;
    if (curve_length < ABdist * (1. - 10 * eps_step))
    {
      std::ostringstream message;
      message << "Unphysical curve length." << G4endl
              << "The size of the above difference exceeds allowed limits." << G4endl
              << "Aborting.";
      G4Exception("G4ChordFinder::ApproxCurvePointV()", "GeomField0003", FatalException, message);
    }
#endif
  ...
  if (AE_fraction > 0.0)
  {
    fIntgrDriver->AccurateAdvance(Current_PointVelocity, new_st_length, eps_step);
    //
