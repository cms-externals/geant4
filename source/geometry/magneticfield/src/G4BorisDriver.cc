...
const G4double G4BorisDriver::fErrorConstraintShrink =
  std::pow(fMaxSteppingDecrease / fSafetyFactor, 1. / fPowerShrink);

const G4double G4BorisDriver::fErrorConstraintGrow =
  std::pow(fMaxSteppingIncrease / fSafetyFactor, 1. / fPowerGrow);

// --------------------------------------------------------------------------

G4BorisDriver::G4BorisDriver(G4double hminimum, G4BorisScheme* Boris, G4int numberOfComponents,
                             G4bool verbosity)
  : fMinimumStep(hminimum), fVerbosity(verbosity), boris(Boris)
// , interval_sequence{2,4}
{
  ...
  if (boris->GetNumberOfVariables() != numberOfComponents)
  {
    ...
    msg << "Disagreement in number of variables = " << boris->GetNumberOfVariables()
        << " vs no of components = " << numberOfComponents;
    G4Exception("G4BorisDriver Constructor:", "GeomField1001", FatalException, msg);
  }
}

// --------------------------------------------------------------------------

G4bool G4BorisDriver::AccurateAdvance(G4FieldTrack& track, G4double hstep, G4double epsilon,
                                      G4double hinitial)
{
  ...
  if (hstep == 0)
  {
    ...
    G4Exception("G4BorisDriver::AccurateAdvance()", "GeomField1001", JustWarning, message);
    return true;
  }
  if (hstep < 0)
  {
    ...
    G4Exception("G4BorisDriver::AccurateAdvance()", "GeomField0003", EventMustBeAborted, message);
    return false;
  }
  ...
  //
  std::memcpy(yOut + nvar, yCurrent + nvar, sizeof(G4double) * (G4FieldTrack::ncompSVEC - nvar));
  ...
  const G4double hThreshold = std::max(epsilon * hstep, fSmallestFraction * curveLength);
  ...
  for (G4int nstp = 0; nstp < fMaxNoSteps; ++nstp)
  {
    ...
    const G4ThreeVector StartPos =
      field_utils::makeVector(yCurrent, field_utils::Value3D::Position);
...
void G4BorisDriver::OneGoodStep(G4double y[],  // InOut
                                ...
                                G4double htry, G4double epsilon_rel, G4double restMass,
                                G4double charge,
                                G4double& hdid,  // Out
                                G4double& hnext)  // Out
{
  ...
  for (G4int iter = 0; iter < max_trials; ++iter)
  {
    ...
    error2 = field_utils::relativeError2(y, yerr, std::max(h, fMinimumStep), epsilon_rel);
    if (error2 <= 1.0)
    {
      break;
    }
    ...
    {
      std::ostringstream message;
      message << "Stepsize underflow in Stepper !" << G4endl << "- Step's start x=" << curveLength
              << " and end x= " << xnew << " are equal !! " << G4endl << "  Due to step-size= " << h
              << ". Note that input step was " << htry;
      G4Exception("G4IntegrationDriver::OneGoodStep()", "GeomField1001", JustWarning, message);
      break;
    }
  }
...
G4bool G4BorisDriver::QuickAdvance(G4FieldTrack& track, const G4double /*dydx*/[], G4double hstep,
                                   G4double& missDist, G4double& dyerr)
{
  ...
  boris->StepWithMidAndErrorEstimate(yIn, restMass, charge, hstep, yMid, yOut, yError);
  ...
  //
  std::memcpy(yOut + nvar, yIn + nvar, sizeof(G4double) * (G4FieldTrack::ncompSVEC - nvar));
...
void G4BorisDriver::GetDerivatives(const G4FieldTrack& yTrack, G4double dydx[]) const
{
...
void G4BorisDriver::GetDerivatives(const G4FieldTrack& yTrack, G4double dydx[],
