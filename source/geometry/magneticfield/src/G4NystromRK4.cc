...
void G4NystromRK4::Stepper(const G4double y[], const G4double dydx[], G4double hstep,
                           G4double yOut[], G4double yError[])
{
  ...
  // Point 1
  const G4double K1[3] = {fInverseMomentum * dydx[3], fInverseMomentum * dydx[4],
                          fInverseMomentum * dydx[5]};

  // Point2
  G4double p[4] = {y[0] + S5 * (dydx[0] + S4 * K1[0]), y[1] + S5 * (dydx[1] + S4 * K1[1]),
                   y[2] + S5 * (dydx[2] + S4 * K1[2]), y[7]};

  GetFieldValue(p, field);

  const G4double A2[3] = {dydx[0] + S5 * K1[0], dydx[1] + S5 * K1[1], dydx[2] + S5 * K1[2]};

  const G4double K2[3] = {(A2[1] * field[2] - A2[2] * field[1]) * fCoefficient,
  ...
  const G4double A3[3] = {dydx[0] + S5 * K2[0], dydx[1] + S5 * K2[1], dydx[2] + S5 * K2[2]};

  const G4double K3[3] = {(A3[1] * field[2] - A3[2] * field[1]) * fCoefficient,
  ...
  const G4double A4[3] = {dydx[0] + hstep * K3[0], dydx[1] + hstep * K3[1],
                          dydx[2] + hstep * K3[2]};

  const G4double K4[3] = {(A4[1] * field[2] - A4[2] * field[1]) * fCoefficient,
...
void G4NystromRK4::SetDistanceForConstantField(G4double length)
{
  if (GetField() == nullptr)
  {
    G4Exception("G4NystromRK4::SetDistanceForConstantField", "Nystrom 001", JustWarning,
                "Provided field is not G4CachedMagneticField. Changing field type.");

    fCachedField = std::make_unique<G4CachedMagneticField>(
      dynamic_cast<G4MagneticField*>(GetEquationOfMotion()->GetFieldObj()), length);
