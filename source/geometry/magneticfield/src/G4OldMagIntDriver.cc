...
//  Constructor
//
G4OldMagIntDriver::G4OldMagIntDriver(G4double hminimum, G4MagIntegratorStepper* pStepper,
...
G4bool G4OldMagIntDriver::AccurateAdvance(G4FieldTrack& y_current, G4double hstep, G4double eps,
                                          G4double hinitial)
{
  ...
  if (hstep <= 0.0)
  {
    if (hstep == 0.0)
    {
      ...
      G4Exception("G4OldMagIntDriver::AccurateAdvance()", "GeomField1001", JustWarning, message);
      return succeeded;
    }
    ...
    G4Exception("G4OldMagIntDriver::AccurateAdvance()", "GeomField0003", EventMustBeAborted,
                message);
    return false;
  }
  ...
  G4cout << "IDriver::AccurAdv called. "
         << " Input: hstep = " << hstep << " hinitial= " << hinitial << " , current: h = " << h
         << G4endl;
#endif
  ...
  do
  {
    ...
    {
      G4FieldTrack yFldTrk(G4ThreeVector(0, 0, 0), G4ThreeVector(0, 0, 0), 0., 0., 0., 0.);
...
#ifdef G4FLD_STATS
      ++fNoSmallSteps;
      if (dyerr_len > fDyerr_max)
      {
        fDyerr_max = dyerr_len;
      }
      ...
      if (dbg > 1)
      {
        ...
        G4cout << "Another sub-min step, no " << fNoSmallSteps << " of " << fNoTotalSteps
               << " this time " << nstp << G4endl;
        PrintStatus(ySubStepStart, x1, y, x, h, nstp);  // Only this
        G4cout << " dyerr= " << dyerr_len << " relative = " << dyerr_len / h << " epsilon= " << eps
               << " hstep= " << hstep << " h= " << h << " hmin= " << fMinimumStep << G4endl;
      }
#endif
      if (h == 0.0)
      {
        G4Exception("G4OldMagIntDriver::AccurateAdvance()", "GeomField0003", FatalException,
    ...
    if ((dbg > 0) && (dbg <= 2) && (nstp > nStpPr))
    {
      if (nstp == nStpPr)
      {
        G4cout << "***** Many steps ****" << G4endl;
      }
      G4cout << "MagIntDrv: ";
      G4cout << "hdid=" << std::setw(12) << hdid << " "
             << "hnext=" << std::setw(12) << hnext << " "
             << "hstep=" << std::setw(12) << hstep << " (requested) " << G4endl;
    ...
    if (endPointDist >= hdid * (1. + perMillion))
    {
      ...
      if (endPointDist >= hdid * (1. + perThousand))
      {
        ...
        if (dbg)
        {
          ...
          G4cerr << "  Total steps:  bad " << fNoBadSteps << " current h= " << hdid << G4endl;
...
void G4OldMagIntDriver::WarnSmallStepSize(G4double hnext, G4double hstep, G4double h,
                                          G4double xDone, G4int nstp)
{
  ...
  {
    message << "Too small 'next' step " << hnext << ", step-no: " << nstp << G4endl
            << ", this sub-step: " << h << ",  req_tot_len: " << hstep << ", done: " << xDone
            << ", min: " << Hmin();
  }
  G4Exception("G4OldMagIntDriver::WarnSmallStepSize()", "GeomField1001", JustWarning, message);
  ++noWarningsIssued;
}
...
void G4OldMagIntDriver::WarnTooManyStep(G4double x1start, G4double x2end, G4double xCurrent)
{
  ...
  message << "The number of steps used in the Integration driver"
          ...
          << "Integration of the interval was not completed !" << G4endl << "Only a "
          << (xCurrent - x1start) * 100 / (x2end - x1start) << " % fraction of it was done.";
  G4Exception("G4OldMagIntDriver::WarnTooManyStep()", "GeomField1001", JustWarning, message);
}

// ---------------------------------------------------------

void G4OldMagIntDriver::WarnEndPointTooFar(G4double endPointDist, G4double h, G4double eps,
                                           G4int dbg)
{
      ...
      && ((dbg > 1) || prNewMax || (endPointDist >= h * (1. + eps))))
  {
    ...
    if ((noWarnings++ < 10) || (dbg > 2))
    {
      message << "The integration produced an end-point which " << G4endl
              << "is further from the start-point than the curve length." << G4endl;
    }
    message << "  Distance of endpoints = " << endPointDist << ", curve length = " << h << G4endl
            << "  Difference (curveLen-endpDist)= " << (h - endPointDist)
            << ", relative = " << (h - endPointDist) / h << ", epsilon =  " << eps;
    G4Exception("G4OldMagIntDriver::WarnEndPointTooFar()", "GeomField1001", JustWarning, message);
  }
}

// ---------------------------------------------------------

void G4OldMagIntDriver::OneGoodStep(G4double y[],  // InOut
                                    const G4double dydx[],
                                    G4double& x,  // InOut
...

{
  ...
  for (G4int iter = 0; iter < max_trials; ++iter)
  {
    ...
    {
      std::ostringstream message;
      message << "Found case of zero momentum." << G4endl << "- iteration= " << iter
              << "; h= " << h;
      G4Exception("G4OldMagIntDriver::OneGoodStep()", "GeomField1001", JustWarning, message);
      errvel_sq = sumerr_sq;
    }
    ...
    {
      std::ostringstream message;
      message << "Stepsize underflow in Stepper !" << G4endl << "- Step's start x=" << x
              << " and end x= " << xnew << " are equal !! " << G4endl << "  Due to step-size= " << h
...
G4bool G4OldMagIntDriver::QuickAdvance(G4FieldTrack& y_posvel,  // INOUT
                                       const G4double dydx[],
                                       G4double hstep,  // In
                                       G4double& dchord_step, G4double& dyerr_pos_sq,
                                       G4double& dyerr_mom_rel_sq)
{
  G4Exception("G4OldMagIntDriver::QuickAdvance()", "GeomField0001", FatalException,
              "Not yet implemented.");
                                       ...
                                       G4double& dchord_step, G4double& dyerr)
{
  ...
  G4double yerr_vec[G4FieldTrack::ncompSVEC], yarrin[G4FieldTrack::ncompSVEC],
    yarrout[G4FieldTrack::ncompSVEC];
  G4double s_start;
  ...
  G4cout  // << "G4MagInt_Driver::"
    << "QuickAdvance" << G4endl << " Input:  hstep= " << hstep << G4endl
    << "         track= " << startTrack << G4endl << " Output: track= " << y_posvel << G4endl
    << "         d_chord = " << dchord_step << " dyerr = " << dyerr << G4endl;
#endif
...
G4bool G4OldMagIntDriver::QuickAdvance(G4double yarrin[],  // In
                                       const G4double dydx[],
                                       G4double hstep,  // In
                                       G4double yarrout[], G4double& dchord_step,
...
//
G4double G4OldMagIntDriver::ComputeNewStepSize(G4double errMaxNorm,  // max error  (normalised)
...
//
G4double
G4OldMagIntDriver::ComputeNewStepSize_WithinLimits(G4double errMaxNorm,  // max error  (normalised)
                                                   G4double hstepCurrent)  // current step size
{
  G4double hnew;
...
void G4OldMagIntDriver::PrintStatus(const G4double* StartArr, G4double xstart,
                                    const G4double* CurrentArr, G4double xcurrent,
...
void G4OldMagIntDriver::PrintStatus(const G4FieldTrack& StartFT, const G4FieldTrack& CurrentFT,
                                    G4double requestStep, G4int subStepNo)
{
  ...
  if ((subStepNo <= 1) || (verboseLevel > 3))
  {
    ...
    G4cout << std::setw(6) << " " << std::setw(25)
           << " G4OldMagIntDriver: Current Position  and  Direction" << " " << G4endl;
    G4cout << std::setw(5) << "Step#" << " " << std::setw(7) << "s-curve" << " " << std::setw(9)
           << "X(mm)" << " " << std::setw(9) << "Y(mm)" << " " << std::setw(9) << "Z(mm)" << " "
           << std::setw(8) << " N_x " << " " << std::setw(8) << " N_y " << " " << std::setw(8)
           << " N_z " << " " << std::setw(8) << " N^2-1 " << " " << std::setw(10) << " N(0).N "
           << " " << std::setw(7) << "KinEner " << " " << std::setw(12) << "Track-l"
           << " "  // Add the Sub-step ??
           << std::setw(12) << "Step-len" << " " << std::setw(12) << "Step-len" << " "
           << std::setw(9) << "ReqStep" << " " << G4endl;
  }
...
void G4OldMagIntDriver::PrintStat_Aux(const G4FieldTrack& aFieldTrack, G4double requestStep,
                                      G4double step_len, G4int subStepNo, G4double subStepSize,
                                      G4double dotVeloc_StartCurr)
{
  ...
  G4cout << std::setw(9) << Position.x() << " " << std::setw(9) << Position.y() << " "
         << std::setw(9) << Position.z() << " " << std::setw(8) << UnitVelocity.x() << " "
         << std::setw(8) << UnitVelocity.y() << " " << std::setw(8) << UnitVelocity.z() << " ";
...
void G4OldMagIntDriver::PrintStatisticsReport()
{
  ...
  G4cout << "G4OldMagIntDriver: Number of Steps: "
         ...
         << " Non-initial small= " << (fNoSmallSteps - fNoInitialSmallSteps) << G4endl;
  G4cout.precision(oldPrec);
}
...
void G4OldMagIntDriver::SetSmallestFraction(G4double newFraction)
{
  ...
  {
    std::ostringstream message;
    message << "Smallest Fraction not changed. " << G4endl << "  Proposed value was " << newFraction
            << G4endl << "  Value must be between 1.e-8 and 1.e-16";
    G4Exception("G4OldMagIntDriver::SetSmallestFraction()", "GeomField1001", JustWarning, message);
  }
}
...
void G4OldMagIntDriver::GetDerivatives(const G4FieldTrack& track, G4double dydx[],
...
void G4OldMagIntDriver::StreamInfo(std::ostream& os) const
{
  ...
  os << "  Max number of Steps = " << fMaxNoSteps << "    (base # = " << fMaxStepBase << " )"
     << std::endl;
