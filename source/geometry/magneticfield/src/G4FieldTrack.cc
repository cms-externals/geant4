...
std::ostream& operator<<(std::ostream& os, const G4FieldTrack& SixVec)
{
  ...
  os << " Pmag= " << G4ThreeVector(SixV[3], SixV[4], SixV[5]).mag();  // mom magnitude
...
G4FieldTrack::G4FieldTrack(const G4ThreeVector& pPosition, G4double LaboratoryTimeOfFlight,
                           const G4ThreeVector& pMomentumDirection, G4double kineticEnergy,
                           G4double restMass_c2, G4double charge,
                           const G4ThreeVector& vecPolarization, G4double magnetic_dipole_moment,
                           G4double curve_length, G4double pdgSpin)
...
G4FieldTrack::G4FieldTrack(const G4ThreeVector& pPosition, const G4ThreeVector& pMomentumDirection,
                           G4double curve_length, G4double kineticEnergy,
                           const G4double restMass_c2,
                           G4double,  // velocity
                           G4double pLaboratoryTimeOfFlight, G4double pProperTimeOfFlight,
                           const G4ThreeVector* pPolarization, G4double pdgSpin)
  : fDistanceAlongCurve(curve_length),
    fKineticEnergy(kineticEnergy),
    fRestMass_c2(restMass_c2),
    ...
    fChargeState(DBL_MAX, DBL_MAX, -1.0)  //  charge not set
{
  ...
  if (pPolarization != nullptr)
  {
    PolarVec = *pPolarization;
  }
...
G4FieldTrack::G4FieldTrack(char)  //  Nothing is set !!
  : fKineticEnergy(0.),
    ...
    fLabTimeOfFlight(0.),
    fProperTimeOfFlight(0.),
    fChargeState(DBL_MAX, DBL_MAX, -1)
{
...
{
  fChargeState.SetChargesAndMoments(charge, magnetic_dipole_moment, electric_dipole_moment,
                                    magnetic_charge);
...
void G4FieldTrack::LoadFromArray(const G4double valArrIn[ncompSVEC], G4int noVarsIntegrated)
{
  ...
  fKineticEnergy =
    momentum_square / (std::sqrt(momentum_square + fRestMass_c2 * fRestMass_c2) + fRestMass_c2);
