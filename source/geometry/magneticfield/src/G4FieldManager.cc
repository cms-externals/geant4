...
G4FieldManager::G4FieldManager(G4Field* detectorField, G4ChordFinder* pChordFinder,
                               G4bool fieldChangesEnergy)
  : fDetectorField(detectorField), fChordFinder(pChordFinder)
{
  ...
  {
    G4cout << "G4FieldManager/ctor#1 fEpsilon Min/Max:  eps_min = " << fEpsilonMin
           << " eps_max=" << fEpsilonMax << G4endl;
  }
  ...
  : fDetectorField(detectorField), fAllocatedChordFinder(true)
{
  ...
  if (fVerboseConstruction)
  {
    G4cout << "G4FieldManager/ctor#2 fEpsilon Min/Max:  eps_min = " << fEpsilonMin
           << " eps_max=" << fEpsilonMax << G4endl;
  }
  // Add to store
  //
...
G4FieldManager* G4FieldManager::Clone() const
{
  ...
  G4cout << "G4FieldManager/clone fEpsilon Min/Max:  eps_min = " << fEpsilonMin
         << " eps_max=" << fEpsilonMax << G4endl;
  return aFM;
}
...
G4bool G4FieldManager::SetDetectorField(G4Field* pDetectorField, G4int failMode)
{
  ...
  if (!ableToSet && (failMode > 0))
  {
    ...
    {
      msg << "  Can NOT find reason for failure. ";
    }
    msg << G4endl;
    G4ExceptionSeverity severity = (failMode != 1) ? FatalException : JustWarning;
    G4Exception("G4FieldManager::SetDetectorField", "Geometry001", severity, msg);
  }
  return ableToSet;
}
      ...
      && (fMinAcceptedEpsilon <= newEpsMax))  // (std::fabs(1.0+newEpsMax)>1.0) )
  {
    ...
    else
    {
      ...
      erm << " Modifying both to the same value " << newEpsMax << " to ensure consistency."
          << G4endl << " To avoid this warning, please set eps_min first, and ensure that "
...
G4bool G4FieldManager::SetMinimumEpsilonStep(G4double newEpsMin)
{
  ...
  if (fMinAcceptedEpsilon <= newEpsMin && newEpsMin <= fMaxAcceptedEpsilon)
  {
    ...
    if (fVerboseConstruction)
    {
      G4cout << "G4FieldManager/SetEpsMin :  eps_min = " << std::setw(10) << fEpsilonMin << G4endl;
    }
    if (fEpsilonMax < fEpsilonMin)
    {
      // Ensure consistency
      G4ExceptionDescription erm;
      erm << "Setting eps_min = " << newEpsMin << " For consistency set eps_max= " << fEpsilonMin
  ...
  else
  {
    ...
    if (maxAcceptValue <= fMaxFinalEpsilon)
    {
      ...
      erm << "Proposed value for maximum-accepted-epsilon = " << maxAcceptValue
          << " is larger than the recommended = " << fMaxWarningEpsilon << G4endl
          << "This may impact the robustness of integration of tracks in field." << G4endl
          << "The request was accepted and the value = " << fMaxAcceptedEpsilon
          << " , but future releases are expected " << G4endl
          << " to tighten the limit of acceptable values to " << fMaxWarningEpsilon << G4endl
          << G4endl << "Suggestion: If you need better performance investigate using "
    ...
    else
    {
      ...
      erm << " Proposed value for maximum accepted epsilon " << maxAcceptValue
          << " is larger than the top of the range = " << fMaxFinalEpsilon << G4endl;
      if (softFailure)
      {
        erm << " Using the latter value instead." << G4endl;
      }
      erm << G4endl;
      erm << " Please adjust to request maxAccepted <= " << fMaxFinalEpsilon << G4endl << G4endl;
      if (!softFailure)
      {
...
void G4FieldManager::ReportBadEpsilonValue(G4ExceptionDescription& erm, G4double value,
                                           const G4String& name) const
{
  erm << "Incorrect proposed value of " << name << " = " << value << G4endl
      << " Its value is outside the permitted range from " << fMinAcceptedEpsilon << "  to "
      << fMaxAcceptedEpsilon << G4endl << " Clarification: " << G4endl;
  ...
  if (value < fMinAcceptedEpsilon)
  {
    erm << "  a) The value must be positive and enough larger than the accuracy limit"
        << " of the (G4)double type - (" << (value < fMinAcceptedEpsilon ? "FAILED" : "OK") << ")"
        << G4endl << "     i.e. std::numeric_limits<G4double>::epsilon()= "
        << std::numeric_limits<G4double>::epsilon() << " to ensure that integration " << G4endl
  ...
  {
    erm << "  b) It must be smaller than (or equal) " << std::setw(8) << std::setprecision(4)
        << fMaxAcceptedEpsilon << " to ensure robustness of integration - ("
  ...
  else
  {
    ...
    erm << "  c) as a floating point number (of type G4double) the sum (1+" << name
        << " ) must be > 1 , (" << (badRoundoff ? "FAILED" : "OK") << ")" << G4endl
        << "     Now    1+eps_min          = " << std::setw(20) << std::setprecision(17)
        << (1 + value) << G4endl << "     and   (1.0+" << name << ") - 1.0 = " << std::setw(20)
