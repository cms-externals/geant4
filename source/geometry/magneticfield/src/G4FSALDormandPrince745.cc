...
G4FSALDormandPrince745::~G4FSALDormandPrince745()
{
  ...
  pseudoDydx_for_DistChord = nullptr;
  delete[] fInitialDyDx;
  fInitialDyDx = nullptr;

  delete[] fLastInitialVector;
  fLastInitialVector = nullptr;
  delete[] fLastFinalVector;
  fLastFinalVector = nullptr;
  delete[] fLastDyDx;
  fLastDyDx = nullptr;
  delete[] fMidVector;
  fMidVector = nullptr;
...
//
void G4FSALDormandPrince745::Stepper(const G4double yInput[], const G4double dydx[], G4double Step,
                                     G4double yOut[], G4double yErr[], G4double nextDydx[])
{
  ...
  const G4double b21 = 0.2, b31 = 3.0 / 40.0, b32 = 9.0 / 40.0,
  ...
  for (i = 0; i < numberOfVariables; ++i)
  {
    yTemp[i] = yIn[i] + Step * (b51 * fInitialDyDx[i] + b52 * ak2[i] + b53 * ak3[i] + b54 * ak4[i]);
  }
  ...
  for (i = 0; i < numberOfVariables; ++i)
  {
    yTemp[i] =
      yIn[i]
      + Step * (b61 * fInitialDyDx[i] + b62 * ak2[i] + b63 * ak3[i] + b64 * ak4[i] + b65 * ak5[i]);
  }
  ...
  for (i = 0; i < numberOfVariables; ++i)
  {
    yOut[i] = yIn[i]
              + Step
                  * (b71 * fInitialDyDx[i] + b72 * ak2[i] + b73 * ak3[i] + b74 * ak4[i]
                     + b75 * ak5[i] + b76 * ak6[i]);
  }
  ...
  {
    yErr[i] = Step
              * (dc1 * fInitialDyDx[i] + dc2 * ak2[i] + dc3 * ak3[i] + dc4 * ak4[i] + dc5 * ak5[i]
                 + dc6 * ak6[i] + dc7 * ak7[i]);
...
// interpolate
//
void G4FSALDormandPrince745::interpolate(const G4double yInput[], const G4double dydx[],
                                         G4double yOut[], G4double Step, G4double tau)
{
  ...
  bf2 = 0.0;
  bf3 = -100.0 * tau
        * (15701508.0 * tau_3 - 914128567.0 * tau_2 + 2074956840.0 * tau - 1323431896.0)
        / 32700410799.0;
  bf4 = 25.0 * tau * (94209048.0 * tau_3 - 1518414297.0 * tau_2 + 2460397220.0 * tau - 889289856.0)
        / 5641041216.0;
  bf5 = -2187.0 * tau * (52338360.0 * tau_3 - 451824525.0 * tau_2 + 687873124.0 * tau - 259006536.0)
        / 199316789632.0;
  bf6 = 11.0 * tau * (106151040.0 * tau_3 - 661884105.0 * tau_2 + 946554244.0 * tau - 361440756.0)
        / 2467955532.0;
  ...
  for (G4int i = 0; i < numberOfVariables; ++i)
  {
    yOut[i] = yIn[i]
              + Step * tau
                  * (bf1 * dydx[i] + bf2 * ak2[i] + bf3 * ak3[i] + bf4 * ak4[i] + bf5 * ak5[i]
                     + bf6 * ak6[i] + bf7 * ak7[i]);
  }
}
...
// SetupInterpolate
//
void G4FSALDormandPrince745::SetupInterpolate(const G4double yInput[], const G4double dydx[],
                                              const G4double Step)
{
  // Coefficients for the additional stages
  //
  G4double b81 = 6245.0 / 62208.0, b82 = 0.0, b83 = 8875.0 / 103032.0, b84 = -125.0 / 1728.0,
           b85 = 801.0 / 13568.0, b86 = -13519.0 / 368064.0, b87 = 11105.0 / 368064.0,

           b91 = 632855.0 / 4478976.0, b92 = 0.0, b93 = 4146875.0 / 6491016.0,
           b94 = 5490625.0 / 14183424.0, b95 = -15975.0 / 108544.0, b96 = 8295925.0 / 220286304.0,
           b97 = -1779595.0 / 62938944.0, b98 = -805.0 / 4104.0;
