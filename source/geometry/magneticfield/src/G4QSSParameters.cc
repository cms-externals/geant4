...
G4bool G4QSSParameters::Set_dQMin(G4double value)
{
  ...
  else
  {
    ...
    errmsg << " OUT of RANGE: Proposed value of dQMin is not within expected limits: "
           << " Smallest = " << dQMin_smallest << " Biggest  = " << dQMin_biggest
           << " Proposed value = " << value;
    G4Exception("G4QSSParameters::Set_dQMin", "QSS-Parameters-0002", FatalException, errmsg);
  }
  return good_value;
}
...
G4bool G4QSSParameters::Set_dQRel(G4double value)
{
  ...
  else
  {
    ...
    errmsg << " OUT of RANGE: Proposed value of dQRel is not within expected limits: "
           << " Smallest = " << dQRel_smallest << " Biggest  = " << dQRel_biggest
           << " Proposed value = " << value;
    G4Exception("G4QSSParameters::Set_dQRel", "QSS-Parameters-0003", FatalException, errmsg);
  }
  return good_value;
}
...
G4bool G4QSSParameters::SetQssOrder(G4int order_val, G4bool onlyWarn)
{
  ...
  else
  {
    ...
    err_msg << "Requested order for G4QSStepper= " << order_val << "  is neither 2 nor 3.";
...
G4bool G4QSSParameters::SetMaxSubsteps(G4int maxSubSteps)
{
  ...
  {
    G4ExceptionDescription err_msg;
    err_msg << "Requested max-substeps for G4QSStepper= " << maxSubSteps << "  is not between "
            << maxSubstep_smallest << "  and " << maxSubstep_biggest;
    G4Exception("G4QSSParameters::SetMaxSubsteps", "QSS-Parameters-0001", JustWarning, err_msg);
  }
  return good_value;
}
