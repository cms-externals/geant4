...
class G4TCashKarpRKF45 : public G4MagIntegratorStepper
{
  public:
    ...
    inline void StepWithError(const G4double yInput[],  // * __restrict__ yInput,
                              const G4double dydx[],  // * __restrict__ dydx,
                              G4double Step,
                              G4double yOut[],  // * __restrict__ yOut,
                              G4double yErr[]);  // * __restrict__ yErr);

    virtual void Stepper(const G4double yInput[], const G4double dydx[], G4double hstep,
...
G4TCashKarpRKF45<T_Equation, N>::G4TCashKarpRKF45(T_Equation* EqRhs, G4bool primary)
  : G4MagIntegratorStepper(dynamic_cast<G4EquationOfMotion*>(EqRhs), N), fEquation_Rhs(EqRhs)
{
  if (dynamic_cast<G4EquationOfMotion*>(EqRhs) == nullptr)
  {
    G4Exception("G4TCashKarpRKF45: constructor", "GeomField0001", FatalException,
...
//
template<class T_Equation, unsigned int N>
inline void G4TCashKarpRKF45<T_Equation, N>::StepWithError(const G4double* yInput,
...
template<class T_Equation, unsigned int N>
inline G4double G4TCashKarpRKF45<T_Equation, N>::DistChord() const
{
  ...
  fAuxStepper->G4TCashKarpRKF45::Stepper(fLastInitialVector, fLastDyDx, 0.5 * fLastStepLength,
...
template<class T_Equation, unsigned int N>
inline void G4TCashKarpRKF45<T_Equation, N>::Stepper(const G4double yInput[], const G4double dydx[],
                                                     G4double Step, G4double yOutput[],
                                                     G4double yError[])
{
