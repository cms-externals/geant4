...
class G4TDormandPrince45 : public G4MagIntegratorStepper
{
  public:
    ...
    inline void StepWithError(const G4double yInput[], const G4double dydx[], G4double hstep,
    ...
    inline void StepWithFinalDerivate(const G4double yInput[], const G4double dydx[],
    ...
    inline void RightHandSideInl(const G4double y[], G4double dydx[])
    {
  ...
  : G4MagIntegratorStepper(dynamic_cast<G4EquationOfMotion*>(equation), N), fEquation_Rhs(equation)
{
  ...
  {
    G4Exception("G4TDormandPrince745: constructor", "GeomField0001", FatalException,
...
template<class T_Equation, unsigned int N>
inline G4TDormandPrince45<T_Equation, N>::G4TDormandPrince45(T_Equation* equation, G4int numVar)
  : G4TDormandPrince45<T_Equation, N>(equation)
{
  if (numVar != G4int(N))
  {
    ...
    G4Exception("G4TCashKarpRKF45: constructor", "GeomField0001", FatalErrorInArgument, msg);
  }
  assert(numVar == N);
}

template<class T_Equation, unsigned int N>
inline void G4TDormandPrince45<T_Equation, N>::StepWithFinalDerivate(
  const G4double yInput[], const G4double dydx[], G4double hstep, G4double yOutput[],
...
template<class T_Equation, unsigned int N>
inline void G4TDormandPrince45<T_Equation, N>::StepWithError(const G4double yInput[],
                                                             const G4double dydx[], G4double hstep,
                                                             G4double yOut[], G4double yErr[])
{
  // The parameters of the Butcher tableu
  //
  constexpr G4double b21 = 0.2, b31 = 3.0 / 40.0, b32 = 9.0 / 40.0, b41 = 44.0 / 45.0,
                     b42 = -56.0 / 15.0, b43 = 32.0 / 9.0,
    ...
    dc1 = -(b71 - 5179.0 / 57600.0), dc2 = -(b72 - .0), dc3 = -(b73 - 7571.0 / 16695.0),
  ...
  {
    yTemp[i] =
      fyIn[i] + hstep * (b61 * dydx[i] + b62 * ak2[i] + b63 * ak3[i] + b64 * ak4[i] + b65 * ak5[i]);
  }
...
template<class T_Equation, unsigned int N>
inline void G4TDormandPrince45<T_Equation, N>::Stepper(const G4double yInput[],
                                                       const G4double dydx[], G4double Step,
                                                       G4double yOutput[], G4double yError[])
{
...
inline G4double G4TDormandPrince45<T_Equation, N>::DistChord() const
{
  ...
  for (unsigned int i = 0; i < 3; ++i)
  {
    mid[i] = fyIn[i]
             + 0.5 * fLastStepLength
                 * (hf1 * fdydxIn[i] + hf3 * ak3[i] + hf4 * ak4[i] + hf5 * ak5[i] + hf6 * ak6[i]
                    + hf7 * ak7[i]);
  }
...
//
template<class T_Equation, unsigned int N>
inline void G4TDormandPrince45<T_Equation, N>::Interpolate4thOrder(G4double yOut[],
                                                                   G4double tau) const
{
  ...
  const G4double bf3 =
    -100.0 / 32700410799.0 * tau
    * (15701508.0 * tau3 - 914128567.0 * tau2 + 2074956840.0 * tau - 1323431896.0);

  const G4double bf4 =
    25.0 / 5641041216.0 * tau
    * (94209048.0 * tau3 - 1518414297.0 * tau2 + 2460397220.0 * tau - 889289856.0);

  const G4double bf5 = -2187.0 / 199316789632.0 * tau
                       * (52338360.0 * tau3 - 451824525.0 * tau2 + 687873124.0 * tau - 259006536.0);

  const G4double bf6 =
    11.0 / 2467955532.0 * tau
    * (106151040.0 * tau3 - 661884105.0 * tau2 + 946554244.0 * tau - 361440756.0);

  const G4double bf7 =
    1.0 / 29380423.0 * tau * (1.0 - tau) * (8293050.0 * tau2 - 82437520.0 * tau + 44764047.0);

  for (unsigned int i = 0; i < N; ++i)
  {
    yOut[i] = fyIn[i]
              + fLastStepLength * tau
                  * (bf1 * fdydxIn[i] + bf3 * ak3[i] + bf4 * ak4[i] + bf5 * ak5[i] + bf6 * ak6[i]
                     + bf7 * ak7[i]);
  }
}
...
inline void G4TDormandPrince45<T_Equation, N>::SetupInterpolation5thOrder()
{
  ...
  const G4double b81 = 6245.0 / 62208.0, b82 = 0.0, b83 = 8875.0 / 103032.0, b84 = -125.0 / 1728.0,
                 ...
                 b91 = 632855.0 / 4478976.0, b92 = 0.0, b93 = 4146875.0 / 6491016.0,
                 b94 = 5490625.0 / 14183424.0, b95 = -15975.0 / 108544.0,
                 b96 = 8295925.0 / 220286304.0, b97 = -1779595.0 / 62938944.0,
                 b98 = -805.0 / 4104.0;
  ...
  for (unsigned int i = 0; i < N; ++i)
  {
    yTemp[i] = fyIn[i]
               + fLastStepLength
                   * (b81 * fdydxIn[i] + b82 * ak2[i] + b83 * ak3[i] + b84 * ak4[i] + b85 * ak5[i]
                      + b86 * ak6[i] + b87 * ak7[i]);
  }
  ...
  for (unsigned int i = 0; i < N; ++i)
  {
    yTemp[i] = fyIn[i]
               + fLastStepLength
                   * (b91 * fdydxIn[i] + b92 * ak2[i] + b93 * ak3[i] + b94 * ak4[i] + b95 * ak5[i]
...
//
template<class T_Equation, unsigned int N>
inline void G4TDormandPrince45<T_Equation, N>::Interpolate5thOrder(G4double yOut[],
                                                                   G4double tau) const
{
  ...
  bi[1][0] = 1.0, bi[1][1] = -38039.0 / 7040.0, bi[1][2] = 125923.0 / 10560.0,
  bi[1][3] = -19683.0 / 1760.0, bi[1][4] = 3303.0 / 880.0,
  //  --------------------------------------------------------
  //
  ...
  //  COEFFICIENTS OF  bi[3]
    bi[3][0] = 0.0, bi[3][1] = -12500.0 / 4081.0, bi[3][2] = 205000.0 / 12243.0,
  bi[3][3] = -90000.0 / 4081.0, bi[3][4] = 36000.0 / 4081.0,
  //  --------------------------------------------------------
  //
  //  COEFFICIENTS OF  bi[4]
    bi[4][0] = 0.0, bi[4][1] = -3125.0 / 704.0, bi[4][2] = 25625.0 / 1056.0,
  bi[4][3] = -5625.0 / 176.0, bi[4][4] = 1125.0 / 88.0,
  //  --------------------------------------------------------
  //
  //  COEFFICIENTS OF  bi[5]
    bi[5][0] = 0.0, bi[5][1] = 164025.0 / 74624.0, bi[5][2] = -448335.0 / 37312.0,
  bi[5][3] = 295245.0 / 18656.0, bi[5][4] = -59049.0 / 9328.0,
  //  --------------------------------------------------------
  //
  //  COEFFICIENTS OF  bi[6]
    bi[6][0] = 0.0, bi[6][1] = -25.0 / 28.0, bi[6][2] = 205.0 / 42.0, bi[6][3] = -45.0 / 7.0,
  bi[6][4] = 18.0 / 7.0,
  //  --------------------------------------------------------
  //
  //  COEFFICIENTS OF  bi[7]
    bi[7][0] = 0.0, bi[7][1] = -2.0 / 11.0, bi[7][2] = 73.0 / 55.0, bi[7][3] = -171.0 / 55.0,
  bi[7][4] = 108.0 / 55.0,
  //  --------------------------------------------------------
  //
  ...
  for (G4int i = 0; i < N; ++i)
  {
    yOut[i] =
      fyIn[i]
      + stepLen
          * (b[1] * fdydxIn[i] + b[2] * ak2[i] + b[3] * ak3[i] + b[4] * ak4[i] + b[5] * ak5[i]
