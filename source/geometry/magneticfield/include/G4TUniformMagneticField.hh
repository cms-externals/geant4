...
class G4TUniformMagneticField : public G4MagneticField
{
  public:
    ...
    G4TUniformMagneticField(G4double vField, G4double vTheta, G4double vPhi)
    {
      ...
      {
        G4Exception("G4TUniformMagneticField::G4TUniformMagneticField()", "GeomField0002",
                    FatalException, "Invalid parameters.");
      }
    ...
    G4TUniformMagneticField(const G4TUniformMagneticField& p) : G4MagneticField(p)
    {
      for (G4int i = 0; i < 3; ++i)
    ...
    inline void GetFieldValue(const G4double yTrack[4], G4double* B) const
    {
    ...
    {
      return new G4TUniformMagneticField(G4ThreeVector(
        this->fFieldComponents[0], this->fFieldComponents[1], this->fFieldComponents[2]));
    }
