...
NTSTDetectorConstruction::Construct()
{
  ...
  using Stepper = G4DormandPrince745;
  ...
  pStepper = new Stepper( pEquation ); G4cout << "Stepper is " << "G4DormandPrince745" << G4endl;
  auto pDriver = new G4InterpolationDriver<Stepper>(
      fMinChordStep, static_cast<Stepper*>(pStepper));

#if 0
  G4cout << "Driver is G4InterpolationDriver\n";
  auto pDriver = new G4InterpolationDriver<Stepper>(
          fMinChordStep, static_cast<Stepper*>(pStepper));
#else
  //G4cout << "Driver is G4IntegrationDriver\n";
  //auto pDriver = new G4IntegrationDriver<Stepper>(
  //        fMinChordStep, static_cast<Stepper*>(pStepper));
#endif
  //pDriver = new G4FSALIntegrationDriver<G4RK547FEq1>(
  //        fMinChordStep, new G4RK547FEq1(pEquation));

  //pDriver = new G4IntegrationDriver<G4BulirschStoer>(
  //        fMinChordStep, new G4BulirschStoer(pEquation, 6, 1e-4));
  ...
  // fpChordFinder = new G4ChordFinder(
  //      (G4MagneticField *)&field, fMinChordStep, pStepper);
