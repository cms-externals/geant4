...
// $Id: $
//
// testTemplatedStepper.cc
//
// Started from testPropagateMagField
//  First version:  Josh Xie (Google Summer of Code 2014)
//  Supervisors:    Sandro Wenzel, John Apostolakis
//
//  Refinements/improvements:  John Apostolakis  October 2020
...
//typedef G4TCachedMagneticField<G4QuadrupoleMagField> Field_t;
using Field_t    = G4TCachedMagneticField<G4TQuadrupoleMagField>;
using Equation_t = G4TMagFieldEquation<Field_t>;
using StepperCK45_t   = G4TCashKarpRKF45<Equation_t, 6>;
using StepperRK4_t    = G4TClassicalRK4<Equation_t, 6>;
using StepperHeum_t   = G4TSimpleHeum<Equation_t, 6>;
using StepperRunge_t  = G4TSimpleRunge<Equation_t, 6> ;
using StepperExEuler_t= G4TExplicitEuler<Equation_t, 6> ;
// Experimenting to see whether G4TCashKarpRKF45 can be used to implement the old G4CashKarpRKF45 
using StepperFree_t  = G4TCashKarpRKF45<G4EquationOfMotion, 6>;

G4TQuadrupoleMagField   tQuadrupoleMagField( 10.*tesla/(50.*cm) ); 
...
G4FieldManager* SetupField(G4int type)
{
   ...
   case 45: pStepper = new G4DormandPrince745(fEquation); break;
      //=============test template mode================

   case 25: G4cout << "Templated CashKarpRKF45 stepper used." << G4endl;
            pStepper = new StepperCK45_t(tEquation); break;
   case 24: G4cout << "templated TClassicalRK4 stepper used." << G4endl;
            pStepper = new StepperRK4_t(tEquation); break;
   case 23: G4cout << "Templated SimpleHeum   stepper used." << G4endl;      
            pStepper = new StepperHeum_t(tEquation); break;
   case 22: G4cout << "Templated SimpleRunge  stepper used." << G4endl;
            pStepper = new StepperRunge_t(tEquation); break;
   case 21: G4cout << "Templated TExplicitEuler stepper used ." << G4endl;
            pStepper = new StepperExEuler_t(tEquation);  break;
      
   case 245: pStepper = new G4TDormandPrince45<Equation_t>(tEquation); break;
   case 114: pStepper = new StepperFree_t(tEquation); break;
...
//  
    ...
    { 
       ...
       G4ThreeVector Spin(1.0, 0.0, 0.0);
                                                   // Momentum in Mev/c ?
       // pMagFieldPropagator
       G4ChargeState charge( +1.0, 0.0, 0.0 );  // electrical charge in e+ units, mdm, pdgSpin
       equationOfMotion->SetChargeMomentumMass( chargeState,
                                                momentum, 
                                                proton_mass_c2);
       /*//G4cout << G4endl;
	  ...
	  // assert( MoveVec.mag() < physStep*(1.+1.e-9) );          
          ...
          double epsMomentum = 1.0e-9;

          double momentumMagStart = initTrack.GetMomentum().mag();
          ...
          if( displaceRatio > maxMoveRatio ) {  // Was 1.0e-9
             ...
             G4ThreeVector diffUnitMom = EndUnitMomentum - UnitMomentum;
             G4double  dotDir = UnitMomentum.dot( EndUnitMomentum );
             G4cerr << " Move displaced " << std::setw(13)
                    << move << " - further than step= "
                    << std::setw(8) << physStep << " (achieved= " << step_len << " ) " 
                    << "  by  "
                    ...
                    // << " start= " << std::setw(12) << std::setprecision(9) << UnitMomentum << "  "
                    // << " end = "  << std::setw(12) << std::setprecision(9) << EndUnitMomentum << " "
                    << " diff= " << diffUnitMom
                    << G4endl;
