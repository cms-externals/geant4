...
//--------------------------------------------------------------------------
G4VParameterisationTrd::G4VParameterisationTrd(EAxis axis, G4int nDiv, G4double width,
  ...
  : G4VDivisionParameterisation(axis, nDiv, width, offset, divType, msolid)
{
  ...
  if (msolid->GetEntityType() == "G4ReflectedSolid")
  {
    ...
    auto newSolid =
      new G4Trd(msol->GetName(), msol->GetXHalfLength2(), msol->GetXHalfLength1(),
                msol->GetYHalfLength2(), msol->GetYHalfLength1(), msol->GetZHalfLength());
    msol = newSolid;
...
//------------------------------------------------------------------------
G4ParameterisationTrdX::G4ParameterisationTrdX(EAxis axis, G4int nDiv, G4double width,
                                               G4double offset, G4VSolid* msolid,
                                               DivisionType divType)
  : G4VParameterisationTrd(axis, nDiv, width, offset, msolid, divType)
{
  ...
  if (divType == DivWIDTH)
  {
    fnDiv = CalculateNDiv(msol->GetXHalfLength1() + msol->GetXHalfLength2(), width, offset);
  }
  ...
  if (verbose >= 1)
  {
    G4cout << " G4ParameterisationTrdX - ## divisions " << fnDiv << " = " << nDiv << G4endl
...
//------------------------------------------------------------------------
G4ParameterisationTrdX::~G4ParameterisationTrdX() = default;
                                                   ...
                                                   G4VPhysicalVolume* physVol) const
{
  ...
  else
  {
    ...
    G4Exception("G4ParameterisationTrdX::ComputeTransformation()", "GeomDiv0002", FatalException,
                message);
  }
  ...
  if (verbose >= 2)
  {
    G4cout << std::setprecision(8) << " G4ParameterisationTrdX::ComputeTransformation() " << copyNo
           << G4endl << " Position: " << origin << " - Axis: " << faxis << G4endl;
  }
#endif
...
//--------------------------------------------------------------------------
void G4ParameterisationTrdX::ComputeDimensions(G4Trd& trd, [[maybe_unused]] const G4int copyNo,
...
//--------------------------------------------------------------------------
void G4ParameterisationTrdX::ComputeDimensions(G4Trap& trap, const G4int copyNo,
                                               const G4VPhysicalVolume*) const
{
  auto msol = (G4Trd*)(fmotherSolid);
  G4double pDy1 = msol->GetYHalfLength1();
  ...
  trap.SetAllParameters(pDz, alp, 0., pDy1, pDx1, pDx1, 0., pDy2, pDx2 - fhgap,
                        pDx2 - fhgap * pDx2 / pDx1, 0.);
...
//--------------------------------------------------------------------------
G4ParameterisationTrdY::G4ParameterisationTrdY(EAxis axis, G4int nDiv, G4double width,
                                               G4double offset, G4VSolid* msolid,
                                               DivisionType divType)
  : G4VParameterisationTrd(axis, nDiv, width, offset, msolid, divType)
{
  ...
  if (divType == DivWIDTH)
  {
    fnDiv = CalculateNDiv(2 * msol->GetYHalfLength1(), width, offset);
  }
  else if (divType == DivNDIV)
  {
    fwidth = CalculateWidth(2 * msol->GetYHalfLength1(), nDiv, offset);
  }

#ifdef G4DIVDEBUG
  if (verbose >= 1)
  {
    G4cout << " G4ParameterisationTrdY no divisions " << fnDiv << " = " << nDiv << G4endl
           << " Offset " << foffset << " = " << offset << G4endl << " width " << fwidth << " = "
           << width << G4endl;
  }
#endif
                                                   ...
                                                   G4VPhysicalVolume* physVol) const
{
  ...
  else
  {
    ...
    G4Exception("G4ParameterisationTrdY::ComputeTransformation()", "GeomDiv0002", FatalException,
                message);
  }
...
//--------------------------------------------------------------------------
void G4ParameterisationTrdY::ComputeDimensions(G4Trd& trd, const G4int,
                                               const G4VPhysicalVolume*) const
{
...
//--------------------------------------------------------------------------
void G4ParameterisationTrdY::ComputeDimensions(G4Trap& trap, const G4int copyNo,
                                               const G4VPhysicalVolume*) const
{
  auto msol = (G4Trd*)(fmotherSolid);
  G4double pDx1 = msol->GetXHalfLength1();
  ...
  trap.SetAllParameters(pDz, alp, 90 * CLHEP::deg, pDy1, pDx1, pDx1, 0., pDy2, pDx2 - fhgap,
                        pDx2 - fhgap * pDx2 / pDx1, 0.);

#ifdef G4DIVDEBUG
  if (verbose >= 2)
  {
    G4cout << " G4ParameterisationTrdY::ComputeDimensions():" << copyNo << G4endl;
    trap.DumpInfo();
  }
#endif
}
...
//--------------------------------------------------------------------------
G4ParameterisationTrdZ::G4ParameterisationTrdZ(EAxis axis, G4int nDiv, G4double width,
                                               G4double offset, G4VSolid* msolid,
                                               DivisionType divType)
  : G4VParameterisationTrd(axis, nDiv, width, offset, msolid, divType)
{
  ...
  if (verbose >= 1)
  {
    G4cout << " G4ParameterisationTrdZ no divisions " << fnDiv << " = " << nDiv << G4endl
           << " Offset " << foffset << " = " << offset << G4endl << " Width " << fwidth << " = "
           << width << G4endl;
  }
#endif
                                                   ...
                                                   G4VPhysicalVolume* physVol) const
{
  ...
  else
  {
    ...
    G4Exception("G4ParameterisationTrdZ::ComputeTransformation()", "GeomDiv0002", FatalException,
                message);
  }
  ...
  if (verbose >= 1)
  {
    G4cout << std::setprecision(8) << " G4ParameterisationTrdZ: " << copyNo << G4endl
           << " Position: " << origin << " - Offset: " << foffset << " - Width: " << fwidth
           << " Axis " << faxis << G4endl;
  }
#endif
...
//--------------------------------------------------------------------------
void G4ParameterisationTrdZ::ComputeDimensions(G4Trd& trd, const G4int copyNo,
                                               const G4VPhysicalVolume*) const
{
  ...
  if (verbose >= 1)
  {
    G4cout << " G4ParameterisationTrdZ::ComputeDimensions()"
           << " - Mother TRD " << G4endl;
    msol->DumpInfo();
    G4cout << " - Parameterised TRD: " << copyNo << G4endl;
    trd.DumpInfo();
  }
#endif
}
