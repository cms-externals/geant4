...
//--------------------------------------------------------------------------
G4ReplicatedSlice::G4ReplicatedSlice(const G4String& pName, G4LogicalVolume* pLogical,
...
{
  CheckAndSetParameters(pAxis, nDivs, width, half_gap, offset, DivNDIVandWIDTH, pMotherLogical,
                        pLogical);
}
...
G4ReplicatedSlice::G4ReplicatedSlice(const G4String& pName, G4LogicalVolume* pLogical,
                                     G4LogicalVolume* pMotherLogical, const EAxis pAxis,
                                     const G4int nDivs, const G4double half_gap,
                                     const G4double offset)
  : G4PVReplica(pName, nDivs, pAxis, pLogical, pMotherLogical)
{
  CheckAndSetParameters(pAxis, nDivs, 0., half_gap, offset, DivNDIV, pMotherLogical, pLogical);
}

//--------------------------------------------------------------------------
G4ReplicatedSlice::G4ReplicatedSlice(const G4String& pName, G4LogicalVolume* pLogical,
                                     G4LogicalVolume* pMotherLogical, const EAxis pAxis,
                                     const G4double width, const G4double half_gap,
                                     const G4double offset)
  : G4PVReplica(pName, 0, pAxis, pLogical, pMotherLogical)
{
  CheckAndSetParameters(pAxis, 0, width, half_gap, offset, DivWIDTH, pMotherLogical, pLogical);
}
                ...
                pMotherPhysical != nullptr ? pMotherPhysical->GetLogicalVolume() : nullptr)
{
  if (pMotherPhysical == nullptr)
  {
    ...
    G4Exception("G4ReplicatedSlice::G4ReplicatedSlice()", "GeomDiv0002", FatalException, message);
    return;
  }
  CheckAndSetParameters(pAxis, nDivs, width, half_gap, offset, DivNDIVandWIDTH,
                        pMotherPhysical->GetLogicalVolume(), pLogical);
}
                ...
                pMotherPhysical != nullptr ? pMotherPhysical->GetLogicalVolume() : nullptr)
{
  ...
  CheckAndSetParameters(pAxis, nDivs, 0., half_gap, offset, DivNDIV,
                        pMotherPhysical->GetLogicalVolume(), pLogical);
}
                ...
                pMotherPhysical != nullptr ? pMotherPhysical->GetLogicalVolume() : nullptr)
{
  ...
  CheckAndSetParameters(pAxis, 0, width, half_gap, offset, DivWIDTH,
                        pMotherPhysical->GetLogicalVolume(), pLogical);
}

//--------------------------------------------------------------------------
void G4ReplicatedSlice::CheckAndSetParameters(const EAxis pAxis, const G4int nDivs,
                                              const G4double width, const G4double half_gap,
                                              ...
                                              const G4LogicalVolume* pLogical)
{
  if (pMotherLogical == nullptr)
  {
    ...
    G4Exception("G4ReplicatedSlice::CheckAndSetParameters()", "GeomDiv0002", FatalException,
                message);
  }
  ...
  if (msolType != dsolType && (msolType != "G4Trd" || dsolType != "G4Trap"))
  {
    std::ostringstream message;
    message << "Invalid setup." << G4endl
            << "Incorrect solid type for division of volume: " << GetName() << G4endl
  ...
  SetParameterisation(pMotherLogical, pAxis, nDivs, width, half_gap, offset, divType);
...
//--------------------------------------------------------------------------
void G4ReplicatedSlice::GetReplicationData(EAxis& axis, G4int& nDivs, G4double& width,
...
//--------------------------------------------------------------------------
void G4ReplicatedSlice::SetParameterisation(G4LogicalVolume* motherLogical, const EAxis axis,
                                            const G4int nDivs, const G4double width,
                                            const G4double half_gap, const G4double offset,
                                            DivisionType divType)
{
  ...
  {
    std::ostringstream message;
    message << "Solid type not supported: " << mSolidType << "." << G4endl << "Divisions for "
            << mSolidType << " not implemented.";
    G4Exception("G4ReplicatedSlice::SetParameterisation()", "GeomDiv0001", FatalException, message);
  }
...
void G4ReplicatedSlice::ErrorInAxis(EAxis axis, G4VSolid* solid)
{
  ...
  G4Exception("G4ReplicatedSlice::ErrorInAxis()", "GeomDiv0002", FatalException, error);
}
