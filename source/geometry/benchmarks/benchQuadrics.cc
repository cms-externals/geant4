...
class CustomTwistedTubs : public G4TwistedTubs
{
  ...
  CustomTwistedTubs(const G4String& name,
                          G4double  twist,
                          G4double  rendinner,
                          G4double  rendouter,
                          G4double  halfz,
...
// Constructor
//
CustomTwistedTubs::CustomTwistedTubs(const G4String& name,
                                           G4double  twist,
                                           G4double  rendinner,
                                           G4double  rendouter,
                                           G4double  halfz,
...
//
CustomTwistedTubs::CustomTwistedTubs(__void__& a)
  : G4TwistedTubs(a)
{
}
...
//
EInside
CustomTwistedTubs::Inside(const G4ThreeVector& p) const
{
...
//
G4ThreeVector
CustomTwistedTubs::SurfaceNormal(const G4ThreeVector& p) const
{
...
//
G4double
CustomTwistedTubs::DistanceToIn(const G4ThreeVector& p ) const
{
...
//
G4double
CustomTwistedTubs::DistanceToIn(const G4ThreeVector& p,
...
//
G4double
CustomTwistedTubs::DistanceToOut(const G4ThreeVector& p) const
{
...
//
G4double
CustomTwistedTubs::DistanceToOut(const G4ThreeVector& p,
                                const G4ThreeVector& v,
...
G4ThreeVector RandomPointOnHyperboloid(G4double twist, G4double a, G4double r, G4double dz, G4double dphi);
...
G4ThreeVector PointOnTwistedTubs(G4double twist, G4double rendmin, G4double rendmax, G4double dz, G4double dphi);
...
void Check_DistanceToOut(G4VSolid* t1, const std::vector<G4ThreeVector>& pp, const std::vector<G4ThreeVector>& vv,
                         G4bool calcNorm, G4int NLOOP);
...
int main()
{
  ...
  {
    if (hype->Inside(hype_Surface[i]) != kSurface) { std::cout << "=== hype: Point is not on Surface !!!" << std::endl; }
    if (hype->Inside(hype_Inside[i])  != kInside)  { std::cout << "=== hype: Point is not Inside !!!"     << std::endl; }
    if (hype->Inside(hype_Outside[i]) != kOutside) { std::cout << "=== hype: Point is not Outside !!!"    << std::endl; }
  }
  ...
  for (G4int i = 0; i < NP; ++i) // Check positions of generated points
  {
    if (gentrap->Inside(gentrap_Surface[i]) != kSurface) { std::cout << "=== gentrap: Point is not on Surface !!!" << std::endl; }
    if (gentrap->Inside(gentrap_Inside[i])  != kInside)  { std::cout << "=== gentrap: Point is not Inside !!!"     << std::endl; }
    if (gentrap->Inside(gentrap_Outside[i]) != kOutside) { std::cout << "=== gentrap: Point is not Outside !!!"    << std::endl; }
  }
  ...
  G4VSolid* twtubs = new G4TwistedTubs("Twisted tube sector", twist = 60*deg, rinn = 30*mm, rout = 70*mm, dz = 60*mm, dphi = 100*deg);
  G4VSolid* custom = new CustomTwistedTubs("Twisted tube sector", twist = 60*deg, rinn = 30*mm, rout = 70*mm, dz = 60*mm, dphi = 100*deg);
  ...
  for (G4int i = 0; i < NP; ++i) // Check positions of generated points
  {
    if (twtubs->Inside(twtubs_Surface[i]) != kSurface) { std::cout << "=== twtubs: Point is not on Surface !!!" << std::endl; }
    if (twtubs->Inside(twtubs_Inside[i])  != kInside)  std::cout << "=== twtubs: Point is not Inside !!!" << std::endl;
    if (twtubs->Inside(twtubs_Outside[i]) != kOutside) std::cout << "=== twtubs: Point is not Outside !!!" << std::endl;
  }
  ...
  if (iftwtubs) {
    ...
    G4cout << "   Points Inside     "; Check_Inside(twtubs, twtubs_Inside,  NLOOP, kInside);
    G4cout << "   Points on Surface "; Check_Inside(twtubs, twtubs_Surface, NLOOP, kSurface);
    G4cout << "   Points Outside    "; Check_Inside(twtubs, twtubs_Outside, NLOOP, kOutside);
  }
  if (iftwtubs) {
    G4cout << "****** Time test for CustomTwistedTubs::Inside(p) ******" << G4endl;
    G4cout << "   Points Inside     "; Check_Inside(custom, twtubs_Inside,  NLOOP, kInside);
    G4cout << "   Points on Surface "; Check_Inside(custom, twtubs_Surface, NLOOP, kSurface);
    G4cout << "   Points Outside    "; Check_Inside(custom, twtubs_Outside, NLOOP, kOutside);
  }
  ...
  if (iftwtubs) {
    ...
    G4cout << "   Points on Surface "; Check_SurfaceNormal(twtubs, twtubs_Surface, NLOOP, kSurface);
  }
  if (iftwtubs) {
    G4cout << "****** Time test for CustomTwistedTubs::SurfaceNormal(p) ******" << G4endl;
    G4cout << "   Points on Surface "; Check_SurfaceNormal(custom, twtubs_Surface, NLOOP, kSurface);
  }
  ...
  if (iftwtubs) {
    ...
    G4cout << "   Points on Surface "; Check_SafetyToIn(twtubs, twtubs_Surface, NLOOP, kSurface);
    G4cout << "   Points Outside    "; Check_SafetyToIn(twtubs, twtubs_Outside, NLOOP, kOutside);
  }
  if (iftwtubs) {
    G4cout << "****** Time test for CustomTwistedTrap::DistanceToIn(p) ******" << G4endl;
    G4cout << "   Points on Surface "; Check_SafetyToIn(custom, twtubs_Surface, NLOOP, kSurface);
    G4cout << "   Points Outside    "; Check_SafetyToIn(custom, twtubs_Outside, NLOOP, kOutside);
  }
  ...
  if (iftwtubs) {
    ...
    G4cout << "   Points Inside     "; Check_SafetyToOut(twtubs, twtubs_Inside,  NLOOP, kInside);
    G4cout << "   Points on Surface "; Check_SafetyToOut(twtubs, twtubs_Surface, NLOOP, kSurface);
  }
  if (iftwtubs) {
    G4cout << "****** Time test for CustomTwistedTubs::DistanceToOut(p) ******" << G4endl;
    G4cout << "   Points Inside     "; Check_SafetyToOut(custom, twtubs_Inside,  NLOOP, kInside);
    G4cout << "   Points on Surface "; Check_SafetyToOut(custom, twtubs_Surface, NLOOP, kSurface);
  }
  ...
  if (iftwtubs) {
    ...
    G4cout << "   DistanceToIn      "; Check_DistanceToIn(twtubs, p_twtubs, vecInCone, NLOOP);
  }
  if (iftwtubs) {
    G4cout << "****** Time test for CustomTwistedTubs::DistanceToIn(p,v) ******" << G4endl;
    G4cout << "   DistanceToIn      "; Check_DistanceToIn(custom, p_twtubs, vecInCone, NLOOP);
  }
  ...
  if (iftwtubs) {
    ...
    G4cout << "   Points Inside     "; Check_DistanceToOut(twtubs, twtubs_Inside, vecOnSphere, false, NLOOP);
    G4cout << "   Points on Surface "; Check_DistanceToOut(twtubs, twtubs_Surface, vecOnSphere, false, NLOOP);
  }
  if (iftwtubs) {
    G4cout << "****** Time test for CustomTwistedTubs::DistanceToOut(p,v) ******" << G4endl;
    G4cout << "   Points Inside     "; Check_DistanceToOut(custom, twtubs_Inside, vecOnSphere, false, NLOOP);
    G4cout << "   Points on Surface "; Check_DistanceToOut(custom, twtubs_Surface, vecOnSphere, false, NLOOP);
  }
  ...
  if (iftwtubs) {
    G4cout << "****** Time test for G4TwistedTubs::DistanceToOut(p,v) ******" << G4endl;
    G4cout << "   Points Inside     "; Check_DistanceToOut(twtubs, twtubs_Inside, vecOnSphere, true, NLOOP);
    G4cout << "   Points on Surface "; Check_DistanceToOut(twtubs, twtubs_Surface, vecOnSphere, true, NLOOP);
  }
  if (iftwtubs) {
    G4cout << "****** Time test for CustomTwistedTubs::DistanceToOut(p,v) ******" << G4endl;
    G4cout << "   Points Inside     "; Check_DistanceToOut(custom, twtubs_Inside, vecOnSphere, true, NLOOP);
    G4cout << "   Points on Surface "; Check_DistanceToOut(custom, twtubs_Surface, vecOnSphere, true, NLOOP);
  }
...
G4ThreeVector
RandomPointOnHyperboloid(G4double a, G4double r, G4double dz)
{
  G4double aa = a*a;
  G4double rr = r*r;
  G4double h = dz;
...
G4ThreeVector
RandomPointOnHyperboloid(G4double twist, G4double a, G4double r, G4double dz, G4double dphi)
{
  G4double aa = a*a;
  G4double rr = r*r;
  G4double h = dz;
  ...
  G4double phi0 = atan2(pcur.y(), pcur.x());
...
G4ThreeVector
RandomPointOnGenericQuad(const G4ThreeVector& p1, const G4ThreeVector& p2,
                         const G4ThreeVector& p3, const G4ThreeVector& p4)
{
...
G4ThreeVector PointOnEllipsoid(G4double xaxis, G4double yaxis, G4double zaxis, G4double zcutmin, G4double zcutmax)
{
  ...
  {
    G4cout << "\nEllipsoid "
           << "(a=" << xaxis << ", b=" << yaxis  << ", c=" << zaxis << ", zmin=" << zcutmin << ", zmax=" << zcutmax
           << ")" << G4endl;
  }
  if (xaxis == 0)
  {
...
G4ThreeVector PointOnEllipticTube(G4double dx, G4double dy, G4double dz)
{
  ...
  if (tot == 0)
  {
    ...
    G4cout << "areas   = " << sbase << ", " << sside << ", " << sbase << "\t total = " << stotal << G4endl;
    G4cout << "areas(%)= "
           << sbase*100/stotal << "%, "
           << sside*100/stotal << "%, "
...
G4ThreeVector PointOnEllipticCone(G4double xSemiAxis, G4double ySemiAxis, G4double zheight, G4double zTopCut)
{
  ...
  if (tot == 0)
  {
    ...
    G4cout << "\nEllipic cone ("
           ...
           << ")" << G4endl;
    G4cout << "areas   = " << szmin << ", " << sside << ", " << szmax << "\t total = " << stotal << G4endl;
    G4cout << "areas(%)= "
           << szmin*100/stotal << "%, "
           << sside*100/stotal << "%, "
                          ...
                          G4double innerSt, G4double outerSt, G4double dz)
{
  ...
  if (tot == 0)
  {
    ...
    G4cout << "areas     = " << sint << ", " << sext << ", " << sbase << ", " << sbase << "\t total = " << stotal << G4endl;
    G4cout << "areas(%)  = "
           << sint*100/stotal << "%, "
           << sext*100/stotal << "%, "
           << sbase*100/stotal << "%, "
  ...
  //
  G4ThreeVector p;
  G4double rint = sqrt(rrint);
  G4double rext = sqrt(rrext);
  switch(k)
  {
...
G4ThreeVector PointOnGenericTrap(G4double dz, const std::vector<G4TwoVector>& vertices)
{
  ...
  if (ssurf[0] < 0)
  {
    ...
    G4double stotal = 0;
    for (G4int i = 0; i < 6; ++i) { stotal += ssurf[i]; }
    G4cout << "areas   = "
           << ssurf[0] << ", " << ssurf[1] << ", " << ssurf[2] << ", "
           << ssurf[3] << ", " << ssurf[4] << ", " << ssurf[5] << "\t total = " << stotal << G4endl;
    G4cout << "areas(%)= "
           << ssurf[0]*100/stotal << "%, "
           << ssurf[1]*100/stotal << "%, "
           << ssurf[2]*100/stotal << "%, "
  ...
  G4cout << "=== PointOnGenericTrap: all 1000 attempts to generate random point failed !!!" << G4endl;
...
G4ThreeVector PointOnTwistedTubs(G4double twist, G4double rmin, G4double rmax, G4double dz, G4double dphi)
{
  ...
  if (ssurf[5] <= 0)
  {
    ...
    G4double sphi = TwistedTubsPhiCutArea(twist, amax, rmax, dz) - TwistedTubsPhiCutArea(twist, amin, rmin, dz);
    ...
    G4cout << "areas   = "
           << sint << ", " << sext << ", "
           << sphi << ", " << sphi << ", "
           << sbase << ", " << sbase << "\t total = " << stotal << G4endl;
    G4cout << "areas(%)= "
           << sint*100/stotal << "%, "
           << sext*100/stotal << "%, "
           << sphi*100/stotal << "%, "
  ...
  if (dz == 0)
  {
    G4cout << "npoints = "
           << counts[0] << ", " << counts[1] << ", " << counts[2] << ", "
           << counts[3] << ", " << counts[4] << ", " << counts[5] << G4endl;
    for (G4int i = 0; i < 6; ++i) { ssurf[i] = 0;  counts[i] = 0; }
    return G4ThreeVector(0,0,0);
  }
