...
Profile MakeProfile(G4int zSections, G4bool hasRmin)
{
  Profile p;
...
std::unique_ptr<G4VSolid> MakePolycone(const G4String& name, G4int zSections, G4double sphi,
                                       G4double dphi, G4bool hasRmin)
{
  Profile p = MakeProfile(zSections, hasRmin);
...
{
  G4cout << "\n"
         << config.solidName << "  z-planes=" << config.zSections
...
void RunCaseCsv(Config& config, G4int sections, G4int nloop)
{
  ...
  G4cout << config.solidName << "," << sections << "," << config.zSections << ","
         << (config.fullPhi ? "full" : "partial") << "," << (config.hasRmin ? "nonzero" : "zero")
         << "," << inside << "," << normal << "," << dIn << "," << dInVec << "," << dOut << ","
         << dOutVec << "," << dOutVecNorm << G4endl;
}

Config MakeConfig(const G4String& solidName, G4int zPlanes, G4bool fullPhi, G4bool hasRmin)
{
  const G4double sphi = fullPhi ? 0. : 25. * deg;
  const G4double dphi = fullPhi ? twopi : 270. * deg;

  std::ostringstream suffix;
  suffix << "_nz" << zPlanes << (fullPhi ? "_full" : "_partial") << (hasRmin ? "_rmin" : "_solid");

  if (solidName == "G4Polycone")
  {
    return {"G4Polycone", zPlanes, fullPhi, hasRmin,
            MakePolycone("polycone" + suffix.str(), zPlanes, sphi, dphi, hasRmin)};
  }
  if (solidName == "G4Polyhedra")
  {
    return {"G4Polyhedra", zPlanes, fullPhi, hasRmin,
            MakePolyhedra("polyhedra" + suffix.str(), zPlanes, sphi, dphi, hasRmin)};
  }

  return {"G4GenericPolycone", zPlanes, fullPhi, hasRmin,
          MakeGenericPolycone("genericpolycone" + suffix.str(), zPlanes, sphi, dphi, hasRmin)};
}
...
std::vector<Config> MakeConfigs()
{
  ...
  for (auto nz : zSections)
  {
    for (auto fullPhi : phiModes)
    {
      ...
      {
        configs.push_back(MakeConfig("G4Polycone", nz, fullPhi, hasRmin));
        configs.push_back(MakeConfig("G4Polyhedra", nz, fullPhi, hasRmin));
#ifndef G4GEOM_USE_USOLIDS
        configs.push_back(MakeConfig("G4GenericPolycone", nz, fullPhi, hasRmin));
#endif
      }
    }
  }
...
void RunScan(G4int maxSections, G4int nloop)
{
  ...
  gCsvMode = true;
  G4cout << "solid,sections,zplanes,phi,rmin,inside_ms,surface_normal_ms,distance_to_in_ms,"
  ...
  for (G4int sections = 1; sections <= maxSections; ++sections)
  {
    ...
    for (const auto& solidName : solidNames)
    {
      for (auto fullPhi : phiModes)
      {
        for (auto hasRmin : rminModes)
        {
          auto config = MakeConfig(solidName, zPlanes, fullPhi, hasRmin);
