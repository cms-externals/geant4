...
void Check_GetPointOnSurface(G4String text, G4VSolid* solid)
{
  ...
  G4cout << text << " " << std::setw(7) << (G4double)time / CLOCKS_PER_SEC * 1000 << " ms"
         ...
         << " \tvolume = " << std::setw(7) << solid->GetCubicVolume() / 1000. << " cm^3" << G4endl;
}
...
int main()
{
  ...
  G4VSolid* para = new G4Para("para", dx = 50, dy = 50, dz = 100, alpha = 10 * deg,
  ...
  G4VSolid* trap = new G4Trap("trap", dz = 100, theta = 20 * deg, phi = 30 * deg, dy1 = 50,
                              dx1 = 60, dx2 = 40, 10 * deg, dy2 = 50, dx3 = 60, dx4 = 40, 10 * deg);

  // Construct G4Tubs
  G4double rmin = 0., rmax = 0., sphi = 0., dphi = 0.;
  G4VSolid* tubs =
    new G4Tubs("tubs", rmin = 50, rmax = 100, dz = 100, sphi = 30 * deg, dphi = 120 * deg);

  // Construct G4Cons
  G4double rmin1 = 0., rmax1 = 0., rmin2 = 0., rmax2 = 0.;
  G4VSolid* cons = new G4Cons("cons", rmin1 = 50, rmax1 = 100, rmin2 = 60, rmax2 = 110, dz = 100,
                              sphi = 30 * deg, dphi = 120 * deg);

  // Construct G4CutTubs
  G4ThreeVector n1 = G4ThreeVector(-0.1, -0.1, -1.).unit(), n2 = G4ThreeVector(0.1, 0.1, 1.).unit();
  G4VSolid* cuttubs = new G4CutTubs("cuttubs", rmin = 50, rmax = 100, dz = 100, sphi = 30 * deg,
                                    dphi = 120 * deg, n1, n2);

  // Construct G4TwistedTubs
  G4double zbot = 0., ztop = 0., twist = 0.;
  G4VSolid* twistedtubs = new G4TwistedTubs("twistedtubs", twist = 40 * deg, rmin = 50, rmax = 120,
                                            zbot = -80, ztop = 120, dphi = 60 * deg);

  // Construct G4Sphere
  G4double stheta = 0., dtheta = 0.;
  G4VSolid* sphere = new G4Sphere("sphere", rmin = 60, rmax = 120, sphi = 30 * deg,
                                  dphi = 120 * deg, stheta = 30 * deg, dtheta = 120 * deg);
  ...
  G4double rtor = 0.;
  G4VSolid* torus =
    new G4Torus("torus", rmin = 20, rmax = 40, rtor = 80, sphi = 30 * deg, dphi = 180 * deg);
  ...
  for (G4int i = 0; i < nsect; ++i) { hexagon[i].set(rhex*std::cos(i*angle),
  rhex*std::sin(i*angle)); } G4VSolid* extruded = new G4ExtrudedSolid("extruded", hexagon, dz=100.,
  ...
  */
  ...
  for (G4int i = 0; i < nsect + 1; ++i)
  {
    ...
    hexagon[2 * nsect + 1 - i].set(rmin * std::cos(sphi + i * angle),
                                   rmin * std::sin(sphi + i * angle));
  }
  G4VSolid* extruded = new G4ExtrudedSolid("extruded", hexagon, dz = 100., G4TwoVector(0., 0.), 1.,
                                           G4TwoVector(0., 0.), 1.);
  ...
  G4VSolid* polyhedra =
    new G4Polyhedra("polyhedra", sphi = 0 * deg, dphi = 180 * deg, nsect = 3, nrz = 4, rr, zz);
  ...
  G4VSolid* polycone =
    new G4Polycone("polycone", sphi = 60 * deg, dphi = 240 * deg, nz = 4, zplane, rinner, router);

  // Construct G4GenericPolycone
  G4double rrgen[] = {40, 120, 110, 60, 80, 0}, zzgen[] = {-50, -50, 0, 0, 30, 30};
  G4VSolid* genericpolycone = new G4GenericPolycone("genericpolycone", sphi = 60 * deg,
                                                    dphi = 240 * deg, nrz = 6, rrgen, zzgen);
  ...
  G4VSolid* ellipsoid =
    new G4Ellipsoid("ellipsoid", dx = 80, dy = 100, dz = 120, zmin = -75, zmax = 95);
  ...
  G4VSolid* ellipticalcone =
    new G4EllipticalCone("ellipticalcone", xa = 0.3, ya = 0.4, zh = 200, zcut = 100);

  // Construct G4Hype
  G4double rin = 0., rout = 0., stin = 0., stout = 0.;
  G4VSolid* hype =
    new G4Hype("hype", rin = 20, rout = 50, stin = 20 * deg, stout = 30 * deg, dz = 100);
  ...
  G4VSolid* twistedbox =
    new G4TwistedBox("twistedbox", twist = 40 * deg, dx = 50, dy = 50, dz = 100);

  // Construct G4TwistedTrd
  G4VSolid* twistedtrd = new G4TwistedTrd("twistedtrd", dx1 = 50, dx2 = 40, dy1 = 50, dy2 = 60,
                                          dz = 100, twist = 40 * deg);

  // Construct G4TwistedTrap
  G4VSolid* twistedtrap =
    new G4TwistedTrap("twistedtrap", twist = 40 * deg, dz = 100, theta = 20 * deg, phi = 30 * deg,
                      dy1 = 50, dx1 = 60, dx2 = 40, dy2 = 50, dx3 = 60, dx4 = 40, 10 * deg);
  ...
  */
  ...
  G4VSolid* hole =
    new G4Tubs("hole", rmin = 0, rmax = 15, dz = 21, sphi = 0 * deg, dphi = 360 * deg);
  G4VSolid* disk =
    new G4Tubs("disk", rmin = 0, rmax = 100, dz = 20, sphi = 0 * deg, dphi = 360 * deg);
  for (G4int i = 0; i < 12; ++i)
  {
    disk = new G4SubtractionSolid("disk", disk, hole,
                                  G4RotateZ3D(i * 30 * deg) * G4TranslateX3D(rmax - 20.));
  }
  for (G4int i = 0; i < 8; ++i)
  {
    disk = new G4SubtractionSolid("disk", disk, hole,
                                  G4RotateZ3D(i * 45 * deg) * G4TranslateX3D(rmax - 55.));
  }
  disk = new G4SubtractionSolid("disk", disk, hole, G4Transform3D());

  // Mesure the performance and print out the result
  std::cout << std::endl;
  std::cout << "   " << std::string(G4Version.begin() + 7, G4Version.end() - 1) << G4Date
            << std::endl;
