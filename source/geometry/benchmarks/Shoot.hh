...
void Shoot(const G4int numShoot, G4VPhysicalVolume* pTopNode, const G4ThreeVector& pSource,
           const G4ThreeVector& pVec)
{
  ...
  G4VPhysicalVolume* located = nullptr;
...
void MagneticShoot(const G4int numShoot, G4VPhysicalVolume* pTopNode, const G4ThreeVector& pSource,
                   const G4ThreeVector& pVec,
    ...
    */
    ...
    while (located)
    {
      ...
      */
      ...
      G4FieldTrack initTrack(partLoc, pVec, 0., kineticEnergy, proton_mass_c2, velocity, labTof,
                             properTof, 0);
      // G4double Step=
...
void ShootVerbose(G4VPhysicalVolume* pTopNode, const G4ThreeVector& pSource,
                  const G4ThreeVector& pVec)
{
  ...
  {
    Step = myNav.ComputeStep(partLoc, pVec, physStep, safety);
    G4cout << "Physical Location=" << located->GetName() << " #" << located->GetCopyNo() << G4endl
           << "   Step=" << Step << "  Safety=" << safety << "  ---->" << G4endl;
