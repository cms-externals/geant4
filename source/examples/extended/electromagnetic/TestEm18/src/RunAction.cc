...
//
// $Id: RunAction.cc 105252 2017-07-17 09:40:37Z gcosmo $
//
...
void RunAction::EnergyDeposited (G4double edep)
{
  fEnergyDeposited += edep;
  if (edep<fEdepMin) fEdepMin = edep;
  if (edep>fEdepMax) fEdepMax = edep;
}
...
void RunAction::EndOfRunAction(const G4Run* aRun)
{
  ...
  //continuous energy deposited dE1
  //
  G4double energyPerEvent = fEnergyDeposited/nbEvents;
  
  G4cout 
    << "\n Energy locally deposited along primary track"
    << " (restricted stopping power)  dE1 = "
    << G4BestUnit(energyPerEvent, "Energy")
    << "  ("   << G4BestUnit(fEdepMin, "Energy")
    << " --> " << G4BestUnit(fEdepMax, "Energy") << ")"
    << G4endl;
