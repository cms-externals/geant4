...
There are two roles in the process of integration:
...
the file(s) changed in the Merge Request. 
...
Assignment and Responsibility of Reviewers
=====

You will be added as a Reviewer of any Merge Request whose primary change is to the
Category you are responsible for so that you can review the changes. This should usually be 
done by the Merge Request Submitter or the Shifter. Re-assignment and adding additional Assignees 
is described later.
...
therefore needed to ensure smooth and timely integration.


Needed Tools and Documentation for Shifters
=====
...
3. Check that the Submitter has selected an Assignee.
   ...
   - If not, assign the Code Owner(s) responsible for the code being changed.
     GitLab will suggest people here based on the files touched (see the ``.gitlab/CODEOWNERS`` file).

4. Check that the Description is present and clear
